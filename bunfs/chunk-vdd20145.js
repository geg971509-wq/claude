...
import { bP } from "/$bunfs/root/chunk-rgw52f13.js";
...
import { TD } from "/$bunfs/root/chunk-zze8764r.js";
...
import { Dse, ehe, the } from "/$bunfs/root/chunk-jvgmmdgq.js";
import { r0e, sxt } from "/$bunfs/root/chunk-7b20eep4.js";
...
async function q(e, t) {
      ...
      p("auto_mode_setup_write", a.code),
      {
        ...
        reason:
          a.code === "parse_failed"
            ...
            : a.reason.replace(/Re-run to try again\.?$/, "Regenerate the proposal with --propose."),
      }
    );
  if (e.target !== void 0) {
    let r = A[e.target];
  ...
  try {
    return {
      ...
      ...(await the(
        {
          ...
          autoMode: {
            ...
            ...(a.proposal.soft_deny.length > 0 && { soft_deny: a.proposal.soft_deny }),
            ...(a.proposal.hard_deny.length > 0 && { hard_deny: a.proposal.hard_deny }),
          },
      ...
      ...(a.droppedUnsafeAllowCount > 0 && { droppedUnsafeAllowCount: a.droppedUnsafeAllowCount }),
    };
  } catch (r) {
    return {
      ok: false,
...
function R(e) {
  let t = I(e.trim());
  if (!t.ok) return { mode: "usage", message: t.message, logCode: "bad_flag_grammar" };
  let s = P(t.rest);
  return t.requestId === void 0 ? s : { ...s, requestId: t.requestId };
}
function P(e) {
  let t = e.trim();
  if (t === "" || t === "--help" || t === "-h") return { mode: "usage", message: n };
...
${n}`,
  ...
  if (/^--expect-sha256(?:\s|$)/.test(o)) {
    let c = o.match(/^--expect-sha256\s+(\S+)\s*(.*)$/s),
      l = c?.[1];
    if (l === void 0 || l.startsWith("--"))
      return {
        mode: "usage",
        message:
...
` + n,
    ...
    if (((d = l), (o = c[2]), !/^--apply-file(?:\s|$)/.test(o)))
      return {
        mode: "usage",
        message:
...
` + n,
  ...
  if (/^--apply-file(?:\s|$)/.test(o)) {
    let c = o.match(/^--apply-file\s+(.+)$/s);
    if (c) {
      let l = c[1].trim();
      if (/(?:^|\s)--expect-sha256(?:=|\s|$)/.test(l))
        return {
          mode: "usage",
          message:
...
` + n,
  ...
  let u = o.match(/^--wizard posture=(\S+) scope=(\S+) depth=(\S+)\s+--propose$/);
  if (!u) {
    if (/--apply\b/.test(o))
      return {
        mode: "usage",
...
${n}`,
  ...
  let a = m(u[1], ["personal", "open-source", "enterprise", "mixed"]),
    r = m(u[2], ["all", "project"]),
    f = m(u[3], ["both", "shell", "repos", "here"]);
  if (!a || !r || !f)
    return {
      mode: "usage",
      message: `Couldn\u2019t parse arguments.
${n}`,
  ...
  return { mode: "propose", answers: { posture: a, scope: r, depth: f } };
}
function E(e) {
...
export { V as call, I as extractRequestId, T as isAllowedApplyFilePath, R as parseNonInteractiveArgs };
