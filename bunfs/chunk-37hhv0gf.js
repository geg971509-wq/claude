// @bun @bytecode
// Claude Code is a Beta product per Anthropic's Commercial Terms of Service.
// By using Claude Code, you agree that all code acceptance or rejection decisions you make,
// and the associated conversations in context, constitute Feedback under Anthropic's Commercial Terms,
// and may be used to improve Anthropic's products, including training models.
// You are responsible for reviewing any code suggestions before use.

// (c) Anthropic PBC. All rights reserved. Use is subject to the Legal Agreements outlined here: https://code.claude.com/docs/en/legal-and-compliance.

// Version: 2.1.252
import { xGt } from "/$bunfs/root/chunk-gfr5dwzt.js";
import { Pl } from "/$bunfs/root/chunk-127gwmpc.js";
import { S, ue } from "/$bunfs/root/chunk-yz031c9r.js";
var WT = S(function (se) {
  var M = Pl(),
    N = xGt(),
    x = ue("http"),
    R = ue("https"),
    O = ue("stream"),
    D = ue("http2"),
    V = ["ECONNRESET", "EPIPE", "ETIMEDOUT"],
    j = (e) => {
      let t = {};
      for (let n of Object.keys(e)) {
        let r = e[n];
        t[n] = Array.isArray(r) ? r.join(",") : r;
      }
      return t;
    },
    m = { setTimeout: (e, t) => setTimeout(e, t), clearTimeout: (e) => clearTimeout(e) },
    U = 1000,
    z = (e, t, n = 0) => {
      if (!n) return -1;
      let r = (s) => {
        let o = m.setTimeout(() => {
            e.destroy(),
              t(
                Object.assign(
                  Error(
                    `@smithy/node-http-handler - the request socket did not establish a connection with the server within the configured timeout of ${n} ms.`,
                  ),
                  { name: "TimeoutError" },
                ),
              );
          }, n - s),
          i = (c) => {
            if (c?.connecting)
              c.on("connect", () => {
                m.clearTimeout(o);
              });
            else m.clearTimeout(o);
          };
        if (e.socket) i(e.socket);
        else e.on("socket", i);
      };
      if (n < 2000) return r(0), 0;
      return m.setTimeout(r.bind(null, U), U);
    },
    J = (e, t, n = 0, r, s) => {
      if (n)
        return m.setTimeout(() => {
          let o = `@smithy/node-http-handler - [${r ? "ERROR" : "WARN"}] a request has exceeded the configured ${n} ms requestTimeout.`;
          if (r) {
            let i = Object.assign(Error(o), { name: "TimeoutError", code: "ETIMEDOUT" });
            e.destroy(i), t(i);
          } else
            (o += " Init client requestHandler with throwOnRequestTimeout=true to turn this into an error."),
              s?.warn?.(o);
        }, n);
      return -1;
    },
    G = 3000,
    X = (e, { keepAlive: t, keepAliveMsecs: n }, r = G) => {
      if (t !== !0) return -1;
      let s = () => {
        if (e.socket) e.socket.setKeepAlive(t, n || 0);
        else
          e.on("socket", (o) => {
            o.setKeepAlive(t, n || 0);
          });
      };
      if (r === 0) return s(), 0;
      return m.setTimeout(s, r);
    },
    B = 3000,
    Y = (e, t, n = 0) => {
      let r = (s) => {
        let o = n - s,
          i = () => {
            e.destroy(),
              t(
                Object.assign(
                  Error(
                    `@smithy/node-http-handler - the request socket timed out after ${n} ms of inactivity (configured by client requestHandler).`,
                  ),
                  { name: "TimeoutError" },
                ),
              );
          };
        if (e.socket) e.socket.setTimeout(o, i), e.on("close", () => e.socket?.removeListener("timeout", i));
        else e.setTimeout(o, i);
      };
      if (0 < n && n < 6000) return r(0), 0;
      return m.setTimeout(r.bind(null, n === 0 ? 0 : B), B);
    },
    I = 6000;
  async function q(e, t, n = I, r = !1) {
    let s = t.headers ?? {},
      o = s.Expect || s.expect,
      i = -1,
      c = !0;
    if (!r && o === "100-continue")
      c = await Promise.race([
        new Promise((h) => {
          i = Number(m.setTimeout(() => h(!0), Math.max(I, n)));
        }),
        new Promise((h) => {
          e.on("continue", () => {
            m.clearTimeout(i), h(!0);
          }),
            e.on("response", () => {
              m.clearTimeout(i), h(!1);
            }),
            e.on("error", () => {
              m.clearTimeout(i), h(!1);
            });
        }),
      ]);
    if (c) Z(e, t.body);
  }
  function Z(e, t) {
    if (t instanceof O.Readable) {
      t.pipe(e);
      return;
    }
    if (t) {
      if (Buffer.isBuffer(t) || typeof t === "string") {
        e.end(t);
        return;
      }
      let n = t;
      if (typeof n === "object" && n.buffer && typeof n.byteOffset === "number" && typeof n.byteLength === "number") {
        e.end(Buffer.from(n.buffer, n.byteOffset, n.byteLength));
        return;
      }
      e.end(Buffer.from(t));
      return;
    }
    e.end();
  }
  var ee = 0;
  class P {
    config;
    configProvider;
    socketWarningTimestamp = 0;
    externalAgent = !1;
    metadata = { handlerProtocol: "http/1.1" };
    static create(e) {
      if (typeof e?.handle === "function") return e;
      return new P(e);
    }
    static checkSocketUsage(e, t, n = console) {
      let { sockets: r, requests: s, maxSockets: o } = e;
      if (typeof o !== "number" || o === 1 / 0) return t;
      let i = 15000;
      if (Date.now() - i < t) return t;
      if (r && s)
        for (let c in r) {
          let h = r[c]?.length ?? 0,
            l = s[c]?.length ?? 0;
          if (h >= o && l >= 2 * o)
            return (
              n?.warn?.(`@smithy/node-http-handler:WARN - socket usage at capacity=${h} and ${l} additional requests are enqueued.
See https://docs.aws.amazon.com/sdk-for-javascript/v3/developer-guide/node-configuring-maxsockets.html
or increase socketAcquisitionWarningTimeout=(millis) in the NodeHttpHandler config.`),
              Date.now()
            );
        }
      return t;
    }
    constructor(e) {
      this.configProvider = new Promise((t, n) => {
        if (typeof e === "function")
          e()
            .then((r) => {
              t(this.resolveDefaultConfig(r));
            })
            .catch(n);
        else t(this.resolveDefaultConfig(e));
      });
    }
    resolveDefaultConfig(e) {
      let {
          requestTimeout: t,
          connectionTimeout: n,
          socketTimeout: r,
          socketAcquisitionWarningTimeout: s,
          httpAgent: o,
          httpsAgent: i,
          throwOnRequestTimeout: c,
        } = e || {},
        h = !0,
        l = 50;
      return {
        connectionTimeout: n,
        requestTimeout: t,
        socketTimeout: r,
        socketAcquisitionWarningTimeout: s,
        throwOnRequestTimeout: c,
        httpAgent: (() => {
          if (o instanceof x.Agent || typeof o?.destroy === "function") return (this.externalAgent = !0), o;
          return new x.Agent({ keepAlive: !0, maxSockets: 50, ...o });
        })(),
        httpsAgent: (() => {
          if (i instanceof R.Agent || typeof i?.destroy === "function") return (this.externalAgent = !0), i;
          return new R.Agent({ keepAlive: !0, maxSockets: 50, ...i });
        })(),
        logger: console,
      };
    }
    destroy() {
      this.config?.httpAgent?.destroy(), this.config?.httpsAgent?.destroy();
    }
    async handle(e, { abortSignal: t, requestTimeout: n } = {}) {
      if (!this.config) this.config = await this.configProvider;
      return new Promise((r, s) => {
        let o = this.config,
          i = void 0,
          c = [],
          h = async (a) => {
            await i, c.forEach(m.clearTimeout), r(a);
          },
          l = async (a) => {
            await i, c.forEach(m.clearTimeout), s(a);
          };
        if (t?.aborted) {
          let a = Error("Request aborted");
          (a.name = "AbortError"), l(a);
          return;
        }
        let T = e.protocol === "https:",
          w = e.headers ?? {},
          _ = (w.Expect ?? w.expect) === "100-continue",
          b = T ? o.httpsAgent : o.httpAgent;
        if (_ && !this.externalAgent) b = new (T ? R.Agent : x.Agent)({ keepAlive: !1, maxSockets: 1 / 0 });
        c.push(
          m.setTimeout(
            () => {
              this.socketWarningTimestamp = P.checkSocketUsage(b, this.socketWarningTimestamp, o.logger);
            },
            o.socketAcquisitionWarningTimeout ?? (o.requestTimeout ?? 2000) + (o.connectionTimeout ?? 1000),
          ),
        );
        let A = N.buildQueryString(e.query || {}),
          k = void 0;
        if (e.username != null || e.password != null) {
          let a = e.username ?? "",
            u = e.password ?? "";
          k = `${a}:${u}`;
        }
        let v = e.path;
        if (A) v += `?${A}`;
        if (e.fragment) v += `#${e.fragment}`;
        let y = e.hostname ?? "";
        if (y[0] === "[" && y.endsWith("]")) y = e.hostname.slice(1, -1);
        else y = e.hostname;
        let C = { headers: e.headers, host: y, method: e.method, path: v, port: e.port, agent: b, auth: k },
          d = (T ? R.request : x.request)(C, (a) => {
            let u = new M.HttpResponse({
              statusCode: a.statusCode || -1,
              reason: a.statusMessage,
              headers: j(a.headers),
              body: a,
            });
            h({ response: u });
          });
        if (
          (d.on("error", (a) => {
            if (V.includes(a.code)) l(Object.assign(a, { name: "TimeoutError" }));
            else l(a);
          }),
          t)
        ) {
          let a = () => {
            d.destroy();
            let u = Error("Request aborted");
            (u.name = "AbortError"), l(u);
          };
          if (typeof t.addEventListener === "function") {
            let u = t;
            u.addEventListener("abort", a, { once: !0 }), d.once("close", () => u.removeEventListener("abort", a));
          } else t.onabort = a;
        }
        let p = n ?? o.requestTimeout;
        c.push(z(d, l, o.connectionTimeout)),
          c.push(J(d, l, p, o.throwOnRequestTimeout, o.logger ?? console)),
          c.push(Y(d, l, o.socketTimeout));
        let E = C.agent;
        if (typeof E === "object" && "keepAlive" in E)
          c.push(X(d, { keepAlive: E.keepAlive, keepAliveMsecs: E.keepAliveMsecs }));
        i = q(d, e, p, this.externalAgent).catch((a) => (c.forEach(m.clearTimeout), s(a)));
      });
    }
    updateHttpClientConfig(e, t) {
      (this.config = void 0), (this.configProvider = this.configProvider.then((n) => ({ ...n, [e]: t })));
    }
    httpHandlerConfigs() {
      return this.config ?? {};
    }
  }
  class W {
    sessions = [];
    constructor(e) {
      this.sessions = e ?? [];
    }
    poll() {
      if (this.sessions.length > 0) return this.sessions.shift();
    }
    offerLast(e) {
      this.sessions.push(e);
    }
    contains(e) {
      return this.sessions.includes(e);
    }
    remove(e) {
      this.sessions = this.sessions.filter((t) => t !== e);
    }
    [Symbol.iterator]() {
      return this.sessions[Symbol.iterator]();
    }
    destroy(e) {
      for (let t of this.sessions)
        if (t === e) {
          if (!t.destroyed) t.destroy();
        }
    }
  }
  class F {
    constructor(e) {
      if (((this.config = e), this.config.maxConcurrency && this.config.maxConcurrency <= 0))
        throw RangeError("maxConcurrency must be greater than zero.");
    }
    config;
    sessionCache = new Map();
    lease(e, t) {
      let n = this.getUrlString(e),
        r = this.sessionCache.get(n);
      if (r) {
        let c = r.poll();
        if (c && !this.config.disableConcurrency) return c;
      }
      let s = D.connect(n);
      if (this.config.maxConcurrency)
        s.settings({ maxConcurrentStreams: this.config.maxConcurrency }, (c) => {
          if (c)
            throw Error(
              "Fail to set maxConcurrentStreams to " +
                this.config.maxConcurrency +
                "when creating new session for " +
                e.destination.toString(),
            );
        });
      s.unref();
      let o = () => {
        s.destroy(), this.deleteSession(n, s);
      };
      if (
        (s.on("goaway", o),
        s.on("error", o),
        s.on("frameError", o),
        s.on("close", () => this.deleteSession(n, s)),
        t.requestTimeout)
      )
        s.setTimeout(t.requestTimeout, o);
      let i = this.sessionCache.get(n) || new W();
      return i.offerLast(s), this.sessionCache.set(n, i), s;
    }
    deleteSession(e, t) {
      let n = this.sessionCache.get(e);
      if (!n) return;
      if (!n.contains(t)) return;
      n.remove(t), this.sessionCache.set(e, n);
    }
    release(e, t) {
      let n = this.getUrlString(e);
      this.sessionCache.get(n)?.offerLast(t);
    }
    destroy() {
      for (let [e, t] of this.sessionCache) {
        for (let n of t) {
          if (!n.destroyed) n.destroy();
          t.remove(n);
        }
        this.sessionCache.delete(e);
      }
    }
    setMaxConcurrentStreams(e) {
      if (e && e <= 0) throw RangeError("maxConcurrentStreams must be greater than zero.");
      this.config.maxConcurrency = e;
    }
    setDisableConcurrentStreams(e) {
      this.config.disableConcurrency = e;
    }
    getUrlString(e) {
      return e.destination.toString();
    }
  }
  class L {
    config;
    configProvider;
    metadata = { handlerProtocol: "h2" };
    connectionManager = new F({});
    static create(e) {
      if (typeof e?.handle === "function") return e;
      return new L(e);
    }
    constructor(e) {
      this.configProvider = new Promise((t, n) => {
        if (typeof e === "function")
          e()
            .then((r) => {
              t(r || {});
            })
            .catch(n);
        else t(e || {});
      });
    }
    destroy() {
      this.connectionManager.destroy();
    }
    async handle(e, { abortSignal: t, requestTimeout: n } = {}) {
      if (!this.config) {
        if (
          ((this.config = await this.configProvider),
          this.connectionManager.setDisableConcurrentStreams(this.config.disableConcurrentStreams || !1),
          this.config.maxConcurrentStreams)
        )
          this.connectionManager.setMaxConcurrentStreams(this.config.maxConcurrentStreams);
      }
      let { requestTimeout: r, disableConcurrentStreams: s } = this.config,
        o = n ?? r;
      return new Promise((i, c) => {
        let h = !1,
          l = void 0,
          T = async (f) => {
            await l, i(f);
          },
          w = async (f) => {
            await l, c(f);
          };
        if (t?.aborted) {
          h = !0;
          let f = Error("Request aborted");
          (f.name = "AbortError"), w(f);
          return;
        }
        let { hostname: _, method: b, port: A, protocol: k, query: v } = e,
          y = "";
        if (e.username != null || e.password != null) {
          let f = e.username ?? "",
            g = e.password ?? "";
          y = `${f}:${g}@`;
        }
        let C = `${k}//${y}${_}${A ? `:${A}` : ""}`,
          H = { destination: new URL(C) },
          d = this.connectionManager.lease(H, {
            requestTimeout: this.config?.sessionTimeout,
            disableConcurrentStreams: s || !1,
          }),
          p = (f) => {
            if (s) this.destroySession(d);
            (h = !0), w(f);
          },
          E = N.buildQueryString(v || {}),
          a = e.path;
        if (E) a += `?${E}`;
        if (e.fragment) a += `#${e.fragment}`;
        let u = d.request({ ...e.headers, [D.constants.HTTP2_HEADER_PATH]: a, [D.constants.HTTP2_HEADER_METHOD]: b });
        if (
          (d.ref(),
          u.on("response", (f) => {
            let g = new M.HttpResponse({ statusCode: f[":status"] || -1, headers: j(f), body: u });
            if (((h = !0), T({ response: g }), s)) d.close(), this.connectionManager.deleteSession(C, d);
          }),
          o)
        )
          u.setTimeout(o, () => {
            u.close();
            let f = Error(`Stream timed out because of no activity for ${o} ms`);
            (f.name = "TimeoutError"), p(f);
          });
        if (t) {
          let f = () => {
            u.close();
            let g = Error("Request aborted");
            (g.name = "AbortError"), p(g);
          };
          if (typeof t.addEventListener === "function") {
            let g = t;
            g.addEventListener("abort", f, { once: !0 }), u.once("close", () => g.removeEventListener("abort", f));
          } else t.onabort = f;
        }
        u.on("frameError", (f, g, Q) => {
          p(Error(`Frame type id ${f} in stream id ${Q} has failed with code ${g}.`));
        }),
          u.on("error", p),
          u.on("aborted", () => {
            p(Error(`HTTP/2 stream is abnormally aborted in mid-communication with result code ${u.rstCode}.`));
          }),
          u.on("close", () => {
            if ((d.unref(), s)) d.destroy();
            if (!h) p(Error("Unexpected error: http2 request did not get a response"));
          }),
          (l = q(u, e, o));
      });
    }
    updateHttpClientConfig(e, t) {
      (this.config = void 0), (this.configProvider = this.configProvider.then((n) => ({ ...n, [e]: t })));
    }
    httpHandlerConfigs() {
      return this.config ?? {};
    }
    destroySession(e) {
      if (!e.destroyed) e.destroy();
    }
  }
  class K extends O.Writable {
    bufferedBytes = [];
    _write(e, t, n) {
      this.bufferedBytes.push(e), n();
    }
  }
  var te = (e) => {
      if (ne(e)) return oe(e);
      return new Promise((t, n) => {
        let r = new K();
        e.pipe(r),
          e.on("error", (s) => {
            r.end(), n(s);
          }),
          r.on("error", n),
          r.on("finish", function () {
            let s = new Uint8Array(Buffer.concat(this.bufferedBytes));
            t(s);
          });
      });
    },
    ne = (e) => typeof ReadableStream === "function" && e instanceof ReadableStream;
  async function oe(e) {
    let t = [],
      n = e.getReader(),
      r = !1,
      s = 0;
    while (!r) {
      let { done: c, value: h } = await n.read();
      if (h) t.push(h), (s += h.length);
      r = c;
    }
    let o = new Uint8Array(s),
      i = 0;
    for (let c of t) o.set(c, i), (i += c.length);
    return o;
  }
  se.DEFAULT_REQUEST_TIMEOUT = ee;
  se.NodeHttp2Handler = L;
  se.NodeHttpHandler = P;
  se.streamCollector = te;
});
export { WT };
