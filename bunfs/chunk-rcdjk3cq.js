...
import { ipe, IT, qa, XV, On, dO, bwn, tb } from "/$bunfs/root/chunk-8tgj5dp2.js";
import { _H } from "/$bunfs/root/chunk-gmj1vprd.js";
import { h6, EEe } from "/$bunfs/root/chunk-hz299nfa.js";
import { uwe, wpn } from "/$bunfs/root/chunk-dzv4k86d.js";
function kYt() {
  let t = dO();
  return t?.accountUuid ? { accountUuid: t.accountUuid, organizationUuid: t.organizationUuid || void 0 } : void 0;
}
var Q = 1000,
  L = 300000,
  A = 15000,
  ...
  q = 1e4,
  D = 3000;
async function HYt(t) {
  return _H() === void 0 && h6() && (await IT(t)) === "store";
}
async function iNn(t) {
  return (await Z(t)).pin;
}
async function Z({ getAccessToken: t, storageV5: i, credentials: e, presetOwner: o }) {
  let d = t();
  if (d === void 0) return { pin: void 0, reason: "no_token" };
  if (!EEe() || Ct()) return { pin: void 0, reason: "disabled" };
  if (o === void 0 && !(await HYt(e))) return { pin: void 0, reason: "not_store_login" };
  let a = await tb(i),
    s,
    f = d,
    c = !1;
  if (o !== void 0) {
    ...
    if (r === "foreign") return { pin: void 0, reason: "not_owner" };
    if (r === "unknown" && !p(a)) return { pin: void 0, reason: "unresolved" };
  } else {
    if (!a?.accountUuid) return { pin: void 0, reason: "no_identity" };
    s = { accountUuid: a.accountUuid, organizationUuid: a.organizationUuid || void 0 };
  }
  let I = o === void 0 ? dO() : void 0;
  if (I !== void 0 && !E(I) && !(await M()))
    return (
      n(
  ...
  function p(r) {
    return (
      r !== void 0 &&
      Boolean(r.accountUuid) &&
      r.accountUuid === s.accountUuid &&
      (!r.organizationUuid || !s.organizationUuid || r.organizationUuid === s.organizationUuid)
    );
  }
  function E(r) {
    if (!p(r)) return !1;
  ...
  async function V(r) {
    ...
    if (!u?.account_uuid) return "unknown";
    let T = { accountUuid: u.account_uuid, organizationUuid: u.organization_uuid || void 0 };
    if (!p(T)) return "foreign";
    return (
  ...
  function v(r) {
    if (((W = Date.now() + r), (z = K), r > 0)) (C = "unchanged"), (w = Date.now() + q);
  }
  function S(r) {
    if (((W = Date.now() + A), r === "inconclusive")) (C = r), (w = Date.now() + q);
    if (y === void 0 || k !== void 0 || z === 0) return r;
    return (
      z--,
      (k = setTimeout(() => {
        k = void 0;
  ...
  async function J(r) {
    ...
    if (!c && o !== void 0) {
      if (
        ((u = await XV(i, e).catch(() => {
          return;
        })),
        u === void 0)
      )
        return S("inconclusive");
      let U = (await x(u.accessToken)) ? "unknown" : await V(u.accessToken);
      if (U === "foreign") return v(0), "changed";
      if (U === "unknown") return S(p(u) ? "unchanged" : "inconclusive");
    } else if (!c) {
      let U = dO();
      if (!(U !== void 0 && E(U)) && !(await M())) return S("inconclusive");
    }
    if (
      ((u ??= await XV(i, e).catch(() => {
        return;
      ...
      return S(r ? "inconclusive" : "unchanged");
    let T = p(u);
    if (u.accessToken === f) return v(T ? A : L), "unchanged";
    let O = (await x(u.accessToken))
      ? void 0
      : await jt(
          ipe(u.accessToken).catch(() => {
            return;
          }),
          D,
        );
    if (!O?.account_uuid) return S("inconclusive");
    let H = { accountUuid: O.account_uuid, organizationUuid: O.organization_uuid || void 0 };
    if (p(H))
      return (
        n("[bridge:owner-pin] the server attributes the rotated credential to the owner \u2014 re-baselining"),
        (s = { accountUuid: s.accountUuid, organizationUuid: s.organizationUuid ?? H.organizationUuid }),
        (f = u.accessToken),
        v(T ? A : L),
        "unchanged"
      );
  ...
  let N = {
    ...
    noteAcceptedToken(r) {
      let u = r !== f;
      if (((f = r), (w = 0), !u || h !== void 0 || !EEe())) return;
      h = tb(i)
        .then(async (T) => {
          if (!T?.accountUuid || p(T)) return;
          let O = await M();
          n(
            O
    ...
    subscribe(r) {
      ...
      return () => {
        (y = void 0), clearTimeout(b), (b = void 0), clearTimeout(k), (k = void 0), u();
      };
    },
  };
  return { pin: N };
...
function ee(t, i, e, o) {
  let d = j(t, i);
  if (Y(d, e, o)) return F(d), Promise.resolve();
  return new Promise((a) => {
    let s = [a];
    if ((_(d), d.pending || d.inFlight)) {
...
function ne(t, i, e) {
  let o = Math.floor((e - i.refilledAt) / t.refillMs);
  if (o > 0) (i.tokens = Math.min(t.burst, i.tokens + o)), (i.refilledAt += o * t.refillMs);
...
function P(t, i) {
  ...
  let o = Date.now();
  if ((ne(t, e, o), e.tokens <= 0 && !e.pending.opts?.userInitiated)) {
    if (!e.timer) {
      let c = Math.max(0, e.refilledAt + t.refillMs - o);
      (e.timer = setTimeout(te, c, t, i)), e.timer.unref?.();
    }
    return;
  }
  ...
  wpn(i, d, a)
    .then(
      ...
      (c) => {
        n(`[bridge] title write failed: ${l(c)}`);
      },
    )
    .finally(() => {
...
function te(t, i) {
  let e = t.entries.get(i);
  if (e) (e.timer = void 0), P(t, i);
}
function ie(t, i, e, o) {
  let d = t.entries.get(i);
  if (!d) return;
  if (((d.retryTimer = void 0), !G(t, d, i, e))) return;
  if (o?.shouldSend !== void 0 && !o.shouldSend()) {
    n("[bridge] title write retry dropped: sending is now barred");
    return;
  }
  uwe(i, { baseUrl: o?.baseUrl, getAccessToken: o?.getAccessToken, credentials: o?.credentials }).then(
    (a) => {
      if (!G(t, d, i, e)) return;
      if (a === null) {
      ...
      if (a.title && !d.knownTitles.has(a.title) && !t.isOwnTitle(i, a.title)) {
        n("[bridge] title write retry dropped: the session was renamed elsewhere"),
          X(t, i, a.title),
...
function G(t, i, e, o) {
  return t.entries.get(e) === i && i.lastSentTitle === o && !i.lastSentOk && !i.inFlight && !i.pending && !i.retryTimer;
}
function X(t, i, e) {
  let o = j(t, i);
