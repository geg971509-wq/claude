...
function pU(i, u = 300000, l) {
  let o = (c) => (typeof u === "function" ? u(c) : u),
    e = new Map(),
    t = new Map(),
    a = async (...c) => {
      let s = b(c),
        r = e.get(s);
      if (r && l && !l(r.value, r.timestamp)) {
        if (r.refreshPromise) return r.refreshPromise;
        e.delete(s), (r = void 0);
      }
      let h = Date.now();
      if (!r) {
...
function jCe(i, u) {
  ...
  for (let o of u) {
    let e = i.get(o);
    if (e !== void 0) l.push([o, e]);
  }
  i.clear();
  for (let [o, e] of l) i.set(o, e);
}
var R = 128;
function WAt(i, u, l) {
  let o = l?.maxSize ?? R,
  ...
  function t(...a) {
    ...
    if (s) return s;
    let r = i(...a);
    if (e.size >= o) e.delete(e.keys().next().value);
    return (
      e.set(c, r),
      r.catch(() => {
        if (e.get(c) === r) e.delete(c);
      }),
      r
    );
  }
