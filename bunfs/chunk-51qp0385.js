...
import { $Ne, a5, W$, iTt, Xor, sTt, YAe, PXe, WNe } from "/$bunfs/root/chunk-6k63g5t6.js";
import { mor, ye } from "/$bunfs/root/chunk-988p40e0.js";
import { xi, _o } from "/$bunfs/root/chunk-0spqrdaj.js";
import { woe, XAn } from "/$bunfs/root/chunk-psdymar6.js";
import { r4, zAn, dGt } from "/$bunfs/root/chunk-0r9wzh7g.js";
...
import { VS } from "/$bunfs/root/chunk-30e2jew7.js";
import { c9t } from "/$bunfs/root/chunk-qekzccch.js";
...
var G = new Set([
  ...
  ...W$.filter((t) => t.startsWith("CLAUDE_CODE_ARTIFACT") || a5(t)),
]);
function l(t) {
...
function N(t, s, e) {
  if (!t || !y.has(s)) return t;
  let o;
  for (let i of Object.keys(t)) {
    if (!ryr.has(i.toUpperCase())) continue;
...
function f(t, s, e, o) {
  ...
  for (let [r, C] of Object.entries(t)) {
    if (B.has(r.toUpperCase())) continue;
    if (!i) {
      E[r] = C;
      continue;
    }
    if (iTt(r) && (e.managedByHost || !Xor(r))) {
      if (e.managedByHost) p(o, r, s);
      continue;
    }
    if (e.managedByHost && r.toUpperCase() === "ANTHROPIC_CUSTOM_HEADERS") {
      p(o, r, s);
      continue;
    }
    if (e.managedByHostFlag && (!e.desktopHost || y.has(s)) && (sTt(r) || YAe(r))) {
      p(o, r, s, e.desktopHost);
      continue;
    }
    E[r] = C;
  }
...
var V = new Set([
  ...
  F = new Set(["CLAUDE_REMOTE_WORKFLOW_SCRIPT", "CLAUDE_REMOTE_WORKFLOW_ARGS", "CLAUDE_WORKFLOW_NAME_ONLY"]),
  b = new Set(["CLAUDE_CODE_ENTRYPOINT", "CLAUDE_CODE_IS_COWORK", "CLAUDE_CODE_COWORK_FRAME_ARTIFACTS"]),
...
function SV() {
  return S.getPreSettingsEnvSnapshot();
}
function vT() {
  return S.peekPreSettingsEnvSnapshot();
}
function Rde() {
  return S.getAppliedGlobalConfigEnv();
}
mor(SV);
function wvr() {
  S.dropPreSettingsEnvSnapshot();
}
function nyn() {
  S.dropPreSettingsEnvSnapshot();
}
function bV(t, s = "policySettings") {
  return S.filterPolicyPredicateEnv(t, s);
}
var Y = ["userSettings", "flagSettings", "policySettings"],
  A = ["TRACES", "METRICS", "LOGS", "PROFILES"],
  _ = $Ne,
...
var De = new Set([`${$Ne}ENDPOINT`, `${$Ne}HEADERS`, `${$Ne}PROTOCOL`]);
function c(t) {
  return t;
}
function j(t) {
...
class RZn {
  ...
  getPreSettingsEnvSnapshot() {
    return (this.preSettingsEnvSnapshot ??= Object.freeze({ ...process.env })), this.preSettingsEnvSnapshot;
  }
  ...
  filterSettingsEnv(t, s) {
    return K(
      I(
        f(L(l(N(t, s, this.projectScopeDropWarned))), s, this.providerStripContext, this.hostManagedDropWarned),
  ...
  filterPolicyPredicateEnv(t, s) {
    let e = I(
      ...
      o = {};
    for (let [i, E] of Object.entries(e)) {
      let r = i.toUpperCase();
      if (r === "NO_COLOR" || r === "FORCE_COLOR") continue;
      o[i] = E;
    }
    return o;
  }
  dropDominatedOtelKey(t, s, e, o, i = "managed settings") {
    ...
    if (!this.otelDominanceDropWarned.has(t))
      this.otelDominanceDropWarned.add(t),
        n(`Dropping ${t}: ${e} is claimed by ${i}, so lower-trust scopes cannot redirect ${s}`, { level: "warn" });
  ...
  dropDominatedBetaTracingEndpoint(t, s, e = "managed settings") {
    this.dropDominatedOtelKey(v, "the logs and traces signals through detailed beta tracing", t, s, e);
  }
  hostSpawnOtelClaims() {
    let t = new Map(),
    ...
    let e = [...s].some((o) => {
      let i = c(o);
      return ((i.startsWith(_) && i.endsWith("_ENDPOINT")) || i === v) && (process.env[o] ?? "").trim() !== "";
    });
    for (let o of s) {
      let i = c(o),
        E = P.has(i) || i === u,
  ...
  enforceManagedOtelFamilyDominance() {
    let t = ye("policySettings"),
      ...
      o = new Map();
    for (let [i, E] of Object.entries(s ?? {})) {
      let r = i.toUpperCase();
      if (!o.has(r) || i === r) o.set(r, E);
    }
    if ((this.applyOtelFamilyClaims(this.hostSpawnOtelClaims(), o, "the host spawn env"), !s && !e)) return;
    if (e) {
      for (let i of A)
        this.dropDominatedOtelKey(`${_}${i}_ENDPOINT`, `the ${i.toLowerCase()} signal`, "otelHeadersHelper", o);
      this.dropDominatedOtelKey(`${_}ENDPOINT`, "telemetry for any signal", "otelHeadersHelper", o),
  ...
  applyOtelFamilyClaims(t, s, e) {
    for (let [o, i] of t) {
      let E = c(o);
      if (E === u) {
        if (process.env[o] === i && !Me(i)) this.dropDominatedBetaTracingEndpoint(o, s, e);
        continue;
      }
      if (P.has(E)) {
        if (process.env[o] === i && j(i)) this.dropDominatedBetaTracingEndpoint(o, s, e);
        continue;
      }
      if (!E.startsWith(_)) continue;
      if (i.trim() === "") continue;
      if (process.env[o] !== i) continue;
      let r = A.find((O) => E.startsWith(`${_}${O}_`));
      if (r) {
        let O = E.slice(`${_}${r}_`.length),
          g = r === "TRACES" || r === "LOGS";
        if (U.has(O)) {
          if ((this.dropDominatedOtelKey(`${_}${r}_ENDPOINT`, `the ${r.toLowerCase()} signal`, o, s, e), g))
            this.dropDominatedBetaTracingEndpoint(o, s, e);
        } else if (O === "ENDPOINT" && g) this.dropDominatedBetaTracingEndpoint(o, s, e);
        continue;
      }
      let C = E.slice(_.length),
        D = U.has(C),
        d = D ? [C, "ENDPOINT"] : [C];
      for (let O of d)
        for (let g of A) this.dropDominatedOtelKey(`${_}${g}_${O}`, `the ${g.toLowerCase()} signal`, o, s, e);
      if (D)
      ...
      this.dropDominatedAntAlias(E, o, s, e);
    }
  }
  applySafeConfigEnvironmentVariables() {
    if ((this.getPreSettingsEnvSnapshot(), (this.providerStripContext = T()), this.hostSpawnEnvKeys === void 0))
    ...
    QD(), Object.assign(process.env, this.filterSettingsEnv(ye("policySettings")?.env, "policySettings"));
    let t = new Map();
    for (let e of xi()) {
      let o = this.filterSettingsEnv(ye(e)?.env, e);
      for (let [i, E] of Object.entries(o)) t.set(i.toUpperCase(), { key: i, value: E });
    }
    for (let { key: e, value: o } of t.values()) if (WNe(e, o)) process.env[e] = o;
    c9t(this.settingsColorEnv);
    let s = process.env[VS];
    if (!s || s === this.materializedProcessWrapper) {
      let e = [
        ye("policySettings")?.processWrapper,
        ye("flagSettings")?.processWrapper,
        _o("userSettings") ? ye("userSettings")?.processWrapper : void 0,
      ].find((o) => typeof o === "string" && o !== "");
      if (e !== void 0) (process.env[VS] = e), (this.materializedProcessWrapper = e);
    }
    this.enforceManagedOtelFamilyDominance();
  }
  applyConfigEnvironmentVariables() {
    this.getPreSettingsEnvSnapshot(), (this.providerStripContext = T()), (this.settingsColorEnv = {});
    ...
    (this.appliedGlobalConfigEnv = this.filterSettingsEnv(ie().env, "globalConfig")),
      Object.assign(process.env, this.appliedGlobalConfigEnv);
    for (let d of xi()) Object.assign(process.env, this.filterSettingsEnv(ye(d)?.env, d));
    c9t(this.settingsColorEnv), this.enforceManagedOtelFamilyDominance();
    let E = a.NODE_EXTRA_CA_CERTS !== t || a.CLAUDE_CODE_CLIENT_CERT !== o || a.CLAUDE_CODE_CLIENT_KEY !== i;
    if (a.NODE_EXTRA_CA_CERTS !== t || a.CLAUDE_CODE_CERT_STORE !== s || a.NODE_OPTIONS !== e) XAn();
    let { certPath: r, keyPath: C } = zAn(),
      D = (a.CLAUDE_CODE_CLIENT_CERT !== r || a.CLAUDE_CODE_CLIENT_KEY !== C) && (r !== void 0 || C !== void 0);
    if (!D) dGt();
    n4(),
      dA(),
      Promise.all([woe(), r4()])
        .then(([d, O]) => {
          if (D && O.readFailed) dGt();
          if (E || D || d || O.changed) n4(), dA();
        })
        .catch(h);
  }
}
var S = new RZn(),
  de = bR({ clear: oyr });
function nre() {
  S.applySafeConfigEnvironmentVariables();
}
function eR() {
  S.applyConfigEnvironmentVariables();
}
function oyr() {
  S.reset();
}
export { ryr, SV, vT, Rde, wvr, nyn, bV, RZn, nre, eR, oyr };
