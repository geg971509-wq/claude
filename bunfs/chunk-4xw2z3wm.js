...
// Version: 2.1.252
import { jn, Oe, bn, Ym, xu, EZe } from "/$bunfs/root/chunk-vfy57cpd.js";
...
function v(r) {
  if (r.inputSchema.type !== "object")
    throw Error(`JSON schema for tool "${r.name}" must be an object, but got ${r.inputSchema.type}`);
  return {
    type: "custom",
    name: r.name,
...
async function et(r) {
  ...
  for (;;) {
    ...
    try {
      ...
      if (a) {
        if (++i > 40) throw new xu(`path ${JSON.stringify(r)} has too many levels of symbolic links`);
...
async function B(r, t, e) {
  ...
  if (i) return a;
  let s = await et(a);
  if (s !== n && !s.startsWith(n + m.sep)) throw new xu(`path ${JSON.stringify(t)} escapes workdir`);
  return s;
}
  ...
  for (let o of n.agent.skills)
    try {
      ...
      if (h !== a && !h.startsWith(a + p.sep)) {
        i.warn("skill name escapes the skills dir; skipping", { component: "agent-tool-context", name: l.name });
        continue;
      }
      ...
      i.warn("failed to download skill", { component: "agent-tool-context", skill_id: o.skill_id, error: String(c) });
    }
  return async () => {
    for (let o of s)
...
async function U(r, t, e) {
  ...
  if (i === void 0)
    throw new bn(`skill ${JSON.stringify(t)} has no concrete version to resolve ${JSON.stringify(e)} against`);
  return i;
}
function ct(r) {
...
`)) {
    ...
    if (p.isAbsolute(e) || e.split(/[\\/]/).includes(".."))
      throw new bn(`refusing to extract unsafe archive member: ${e}`);
  }
}
function lt(r) {
  for (let t of r.split(`
`)) {
    ...
    if (e === "l" || e === "h" || e === "b" || e === "c" || e === "p" || e === "s")
      throw new bn("refusing to extract archive with symlink/hardlink/device member");
  }
}
async function L(r, t) {
  try {
    ...
    if (e != null && typeof e === "object" && e.code === "ENOENT")
      throw new bn(`skill extraction requires the \`${r}\` command, but it was not found on PATH`);
    throw e;
  }
}
function ft(r) {
...
async function C(r, t) {
  ...
  if (!r.body) throw new bn("skill download response had no body");
...
class X {
  constructor(r, t = yt()) {
    I.add(this),
      ...
      Oe(this, w, "f").stdout.setEncoding("utf8"),
      Oe(this, w, "f").stderr.setEncoding("utf8"),
  ...
  async exec(r, t = {}) {
    if (Oe(this, k, "f")) throw new bn("bash session terminated");
    let e = t.timeoutMs ?? z,
      i = t.signal;
...
`;
    if ((Oe(this, w, "f").stdin.write(s), Oe(this, g, "f").indexOf(n) < 0)) {
      let { promise: M, resolve: Z } = EZe();
      jn(this, _, { sentinel: n, resolve: Z }, "f");
      let N, A;
      try {
        await Promise.race([
          M,
          new Promise((V, P) => {
            N = setTimeout(() => P(new bn(`bash command timed out after ${e}ms`)), e);
          }),
          new Promise((V, P) => {
            if (!i) return;
    ...
    let o = Oe(this, g, "f").indexOf(n);
    if (o < 0) throw new bn("bash session terminated");
    let l = Oe(this, g, "f")
        .slice(o + n.length)
        .match(/^(-?\d+)/),
      f = l ? parseInt(l[1], 10) : -1,
      h = Oe(this, g, "f").slice(0, o).replace(mt, "").replace(/\n+$/, "");
    if (Oe(this, S, "f"))
      h = `[output truncated]
${h}`;
  ...
  close() {
    ...
    jn(this, _, null, "f"),
      r?.resolve(),
      Oe(this, w, "f").stdout.destroy(),
      Oe(this, w, "f").stderr.destroy(),
      Oe(this, w, "f").stdin.destroy();
    try {
      process.kill(-Oe(this, w, "f").pid, "SIGKILL");
    } catch {
      Oe(this, w, "f").kill("SIGKILL");
    }
    Oe(this, w, "f").unref();
  }
}
    ...
    run: async ({ command: i, restart: n, timeout_ms: a }, s) => {
      ...
      try {
        ...
        if (!i) {
          ...
          throw new xu("bash: command is required");
        }
        t ?? (t = new X(r.workdir, r.env));
        try {
          let { output: l, exitCode: f } = await t.exec(i, { timeoutMs: a ?? z, signal: s?.signal });
          if (f !== 0) throw new xu(l || `exit ${f}`);
          return l;
        } catch (l) {
          if (l instanceof xu) throw l;
          throw (t.close(), (t = void 0), new xu(`bash: ${l instanceof Error ? l.message : String(l)}`));
        }
      } finally {
        c.resolve();
      }
...
function bt(r) {
  return v({
    ...
    run: async ({ file_path: t, view_range: e }) => {
      if (!t) throw new xu("read: file_path is required");
      let i = await x(r, t),
        n;
      try {
        let f = await d.stat(i);
        if (!f.isFile()) throw new xu(`read: ${t} is not a regular file`);
        ...
        if (f instanceof xu) throw f;
        throw new xu(`read: ${O(f, t)}`);
      }
      if (!e) return n;
      if (e.length !== 2) throw new xu("read: view_range must be [start_line, end_line]");
      let [a, s] = e,
        o = n.split(`
`),
        c = Math.max(0, a - 1),
...
function vt(r) {
  return v({
    ...
    run: async ({ file_path: t, old_string: e, new_string: i, replace_all: n }) => {
      if (!t) throw new xu("edit: file_path is required");
      if (!e) throw new xu("edit: old_string is required");
      let a = await x(r, t),
        s;
      try {
        let l = await d.stat(a);
        if (!l.isFile()) throw new xu(`edit: ${t} is not a regular file`);
        ...
        if (l instanceof xu) throw l;
        throw new xu(`edit: ${O(l, t)}`);
      }
      let o = s.split(e).length - 1;
      if (o === 0) throw new xu(`edit: old_string not found in ${t}`);
      let c;
      if (n) c = s.split(e).join(i);
      else {
        if (o > 1) throw new xu(`edit: old_string appears ${o} times in ${t} (must be unique)`);
        c = s.replace(e, () => i);
      }
      try {
        await F(a, c);
      } catch (l) {
        throw new xu(`edit: write: ${O(l, t)}`);
      }
...
function $t(r) {
  return v({
    ...
    run: async ({ pattern: t, path: e }) => {
      if (!t) throw new xu("glob: pattern is required");
      let i = u.resolve(r.workdir),
        n = t;
      if (u.isAbsolute(t)) {
        if (!r.unrestrictedPaths) throw new xu("glob: absolute pattern not permitted");
        (i = u.parse(t).root), (n = u.relative(i, t));
      } else if (e) i = await x(r, e);
      if (!r.unrestrictedPaths && n.split(/[\\/]/).includes(".."))
        throw new xu('glob: ".." is not permitted in the pattern');
      ...
      try {
        ...
        throw new xu(`glob: ${o instanceof Error ? o.message : String(o)}`);
      }
...
function kt(r) {
  return v({
    ...
    run: async ({ pattern: t, path: e }, i) => {
      if (!t) throw new xu("grep: pattern is required");
...
function Et(r, t, e, i) {
  return new Promise((n, a) => {
    ...
    s.stdout.on("data", (f) => {
      ...
      s.on("close", (f) => {
        if (i?.aborted) return a(new xu("grep: aborted"));
        if (l)
          return n(
            o +
              `
[output truncated at ${T} bytes]`,
        ...
        a(new xu(`grep: rg failed: ${c || `exit ${f}`}`));
      }),
      s.on("error", (f) => {
        if (i?.aborted) return a(new xu("grep: aborted"));
        a(new xu(`grep: rg failed: ${f.message}`));
      });
  });
}
async function Ot(r, t, e) {
  ...
  try {
    ...
    throw new xu(`grep: invalid regex: ${c instanceof Error ? c.message : String(c)}`);
  }
  let n = [],
    a = T,
    s = (c) => {
  ...
  if (e?.aborted) throw new xu("grep: aborted");
