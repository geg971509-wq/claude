...
// Version: 2.1.252
import { V5 } from "/$bunfs/root/chunk-f9h0bg01.js";
...
import { Gr, sn, ba } from "/$bunfs/root/chunk-rgw52f13.js";
...
import { RUe, VRt, cOn } from "/$bunfs/root/chunk-z7eptw29.js";
import { vS } from "/$bunfs/root/chunk-g9wmbw10.js";
import { Bg } from "/$bunfs/root/chunk-vx3kc9d2.js";
import "/$bunfs/root/chunk-bp3j8sq3.js";
import "/$bunfs/root/chunk-dck778n3.js";
import "/$bunfs/root/chunk-98tgyb1d.js";
import { E4e, xe } from "/$bunfs/root/chunk-zze8764r.js";
import "/$bunfs/root/chunk-h7dkgq01.js";
import { Uc } from "/$bunfs/root/chunk-2n1hsggr.js";
...
import { Dp, uV, Bh, YTe } from "/$bunfs/root/chunk-nkjrjh5n.js";
...
import { knt } from "/$bunfs/root/chunk-pwk6q778.js";
...
function A(fe) {
  let w = _(14),
    ...
    E;
  if (w[0] !== L) (E = e(Pc, { subtitle: L, children: "Current Plan" })), (w[0] = L), (w[1] = E);
  else E = w[1];
  let S;
  if (w[2] !== F) (S = e(o, { marginTop: 1, children: e(t, { children: F }) })), (w[2] = F), (w[3] = S);
  else S = w[3];
  let R;
  if (w[4] !== h)
    (R =
...
async function me(a, l, c) {
  ...
  if (f)
    V5(O, "plan"),
      M((s) => ({
        ...s,
        toolPermissionContext: Uc(E4e(s.toolPermissionContext), {
          type: "setMode",
          mode: "plan",
          destination: "session",
        }),
  ...
  if (sn()) {
    if (!ba()) return a(f ? "Enabled plan mode" : "Already in plan mode."), null;
    let s = Gr(),
      i = c.trim();
    if (f && i && i !== "open" && i !== "share") {
      try {
        await cOn(l.session, "plan", x);
      } catch (m) {
        let b = m instanceof VRt;
        return (
          p("plan_remote_query", b ? "mode_push_timeout" : "mode_push_rejected"),
          a(
            b
      ...
      let d = xe({ content: i, origin: { kind: "human" } });
      if ((l.setMessages((m) => [...m, d]), !(await s.sendMessage(i, { uuid: d.uuid }))))
        return p("plan_remote_query", "send_failed"), a("Enabled plan mode"), null;
    ...
    try {
      let d = await Xt(s.sendControlRequest({ subtype: "get_plan" }), G, "get_plan timed out");
      if (!d.exists || !d.content)
        return y("plan_remote_view"), a(f ? "Enabled plan mode" : "Already in plan mode. No plan written yet."), null;
      y("plan_remote_view");
      let u = mo(l.session),
        m = d.content,
        b = (I) =>
      ...
      let u = d instanceof Error ? d.message : String(d),
        m = u.includes("Unsupported control request subtype");
      return (
        p(
          "plan_remote_view",
          m ? "unsupported_subtype" : u.includes("get_plan timed out") ? "timeout" : "request_failed",
        ),
        a(
          f
            ? "Enabled plan mode"
  ...
  let g = Bh();
  Dp(g);
  let C = await YTe(void 0, l.storageV5);
  if (!C) return a(f ? "Enabled plan mode" : "Already in plan mode. No plan written yet."), null;
  let V = c.trim().split(/\s+/);
  if (V[0] === "open") {
    let s = await lW(g);
    Dp(g);
    let i = mo(l.session);
  ...
  if (V[0] === "share") {
    if (!RUe.isPlanArtifactEnabled()) return a("Publishing plans is not available in this session."), null;
    let s = "Couldn't publish plan \u2014 try /plan share again, or run with --debug for details.";
    try {
      let i = await RUe.publishPlanArtifact(C, l.artifactRegistries.ownPublishes, l.credentials);
      if (i.url)
        M((d) => {
          let u = RUe.workshopVerifiedSlugsWith(d.workshopVerifiedSlugs, i);
          return u === null ? d : { ...d, workshopVerifiedSlugs: u };
        }),
          a(`Published plan: ${i.url}`);
      else n(`plan share: publish returned ${i.err}`), a(s);
    } catch (i) {
      n(`plan share: publish threw ${i instanceof Error ? i.message : String(i)}`), a(s);
    }
    return null;
  }
  let N = vS(),
