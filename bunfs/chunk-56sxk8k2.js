// @bun @bytecode
// Claude Code is a Beta product per Anthropic's Commercial Terms of Service.
// By using Claude Code, you agree that all code acceptance or rejection decisions you make,
// and the associated conversations in context, constitute Feedback under Anthropic's Commercial Terms,
// and may be used to improve Anthropic's products, including training models.
// You are responsible for reviewing any code suggestions before use.

// (c) Anthropic PBC. All rights reserved. Use is subject to the Legal Agreements outlined here: https://code.claude.com/docs/en/legal-and-compliance.

// Version: 2.1.252
import { ne } from "/$bunfs/root/chunk-tx16jn0x.js";
var $t = typeof global == "object" && global && global.Object === Object && global,
  C5t = $t;
var Ht = typeof self == "object" && self && self.Object === Object && self,
  Wt = C5t || Ht || Function("return this")(),
  YT = Wt;
var zt = YT.Symbol,
  lx = zt;
var Y = Object.prototype,
  Ft = Y.hasOwnProperty,
  Mt = Y.toString,
  O = lx ? lx.toStringTag : void 0;
function Ut(t) {
  var e = Ft.call(t, O),
    r = t[O];
  try {
    t[O] = void 0;
    var n = !0;
  } catch (i) {}
  var o = Mt.call(t);
  if (n)
    if (e) t[O] = r;
    else delete t[O];
  return o;
}
var q = Ut;
var Gt = Object.prototype,
  Vt = Gt.toString;
function Jt(t) {
  return Vt.call(t);
}
var Q = Jt;
var Kt = "[object Null]",
  Zt = "[object Undefined]",
  tt = lx ? lx.toStringTag : void 0;
function Bt(t) {
  if (t == null) return t === void 0 ? Zt : Kt;
  return tt && tt in Object(t) ? q(t) : Q(t);
}
var Ij = Bt;
function Xt(t) {
  var e = typeof t;
  return t != null && (e == "object" || e == "function");
}
var Rm = Xt;
var Yt = "[object AsyncFunction]",
  qt = "[object Function]",
  Qt = "[object GeneratorFunction]",
  te = "[object Proxy]";
function ee(t) {
  if (!Rm(t)) return !1;
  var e = Ij(t);
  return e == qt || e == Qt || e == Yt || e == te;
}
var l$e = ee;
var re = YT["__core-js_shared__"],
  E = re;
var et = (function () {
  var t = /[^.]+$/.exec((E && E.keys && E.keys.IE_PROTO) || "");
  return t ? "Symbol(src)_1." + t : "";
})();
function oe(t) {
  return !!et && et in t;
}
var rt = oe;
var ie = Function.prototype,
  se = ie.toString;
function ae(t) {
  if (t != null) {
    try {
      return se.call(t);
    } catch (e) {}
    try {
      return t + "";
    } catch (e) {}
  }
  return "";
}
var dme = ae;
var ue = /[\\^$.*+?()[\]{}|]/g,
  le = /^\[object .+?Constructor\]$/,
  ce = Function.prototype,
  fe = Object.prototype,
  pe = ce.toString,
  de = fe.hasOwnProperty,
  ge = RegExp(
    "^" +
      pe
        .call(de)
        .replace(ue, "\\$&")
        .replace(/hasOwnProperty|(function).*?(?=\\\()| for .+?(?=\\\])/g, "$1.*?") +
      "$",
  );
function me(t) {
  if (!Rm(t) || rt(t)) return !1;
  var e = l$e(t) ? ge : le;
  return e.test(dme(t));
}
var nt = me;
function he(t, e) {
  return t == null ? void 0 : t[e];
}
var ot = he;
function xe(t, e) {
  var r = ot(t, e);
  return nt(r) ? r : void 0;
}
var Pj = xe;
var be = Pj(Object, "create"),
  m = be;
function _e() {
  (this.__data__ = m ? m(null) : {}), (this.size = 0);
}
var it = _e;
function we(t) {
  var e = this.has(t) && delete this.__data__[t];
  return (this.size -= e ? 1 : 0), e;
}
var st = we;
var ve = "__lodash_hash_undefined__",
  Se = Object.prototype,
  ye = Se.hasOwnProperty;
function Ce(t) {
  var e = this.__data__;
  if (m) {
    var r = e[t];
    return r === ve ? void 0 : r;
  }
  return ye.call(e, t) ? e[t] : void 0;
}
var at = Ce;
var Te = Object.prototype,
  Oe = Te.hasOwnProperty;
function Ae(t) {
  var e = this.__data__;
  return m ? e[t] !== void 0 : Oe.call(e, t);
}
var ut = Ae;
var Ne = "__lodash_hash_undefined__";
function Pe(t, e) {
  var r = this.__data__;
  return (this.size += this.has(t) ? 0 : 1), (r[t] = m && e === void 0 ? Ne : e), this;
}
var lt = Pe;
function S(t) {
  var e = -1,
    r = t == null ? 0 : t.length;
  this.clear();
  while (++e < r) {
    var n = t[e];
    this.set(n[0], n[1]);
  }
}
S.prototype.clear = it;
S.prototype.delete = st;
S.prototype.get = at;
S.prototype.has = ut;
S.prototype.set = lt;
var W = S;
function De() {
  (this.__data__ = []), (this.size = 0);
}
var ct = De;
function Ee(t, e) {
  return t === e || (t !== t && e !== e);
}
var Q5 = Ee;
function Le(t, e) {
  var r = t.length;
  while (r--) if (Q5(t[r][0], e)) return r;
  return -1;
}
var x = Le;
var Re = Array.prototype,
  je = Re.splice;
function Ie(t) {
  var e = this.__data__,
    r = x(e, t);
  if (r < 0) return !1;
  var n = e.length - 1;
  if (r == n) e.pop();
  else je.call(e, r, 1);
  return --this.size, !0;
}
var ft = Ie;
function ke(t) {
  var e = this.__data__,
    r = x(e, t);
  return r < 0 ? void 0 : e[r][1];
}
var pt = ke;
function $e(t) {
  return x(this.__data__, t) > -1;
}
var dt = $e;
function He(t, e) {
  var r = this.__data__,
    n = x(r, t);
  if (n < 0) ++this.size, r.push([t, e]);
  else r[n][1] = e;
  return this;
}
var gt = He;
function y(t) {
  var e = -1,
    r = t == null ? 0 : t.length;
  this.clear();
  while (++e < r) {
    var n = t[e];
    this.set(n[0], n[1]);
  }
}
y.prototype.clear = ct;
y.prototype.delete = ft;
y.prototype.get = pt;
y.prototype.has = dt;
y.prototype.set = gt;
var vve = y;
var We = Pj(YT, "Map"),
  Rve = We;
function ze() {
  (this.size = 0), (this.__data__ = { hash: new W(), map: new (Rve || vve)(), string: new W() });
}
var mt = ze;
function Fe(t) {
  var e = typeof t;
  return e == "string" || e == "number" || e == "symbol" || e == "boolean" ? t !== "__proto__" : t === null;
}
var ht = Fe;
function Me(t, e) {
  var r = t.__data__;
  return ht(e) ? r[typeof e == "string" ? "string" : "hash"] : r.map;
}
var b = Me;
function Ue(t) {
  var e = b(this, t).delete(t);
  return (this.size -= e ? 1 : 0), e;
}
var xt = Ue;
function Ge(t) {
  return b(this, t).get(t);
}
var bt = Ge;
function Ve(t) {
  return b(this, t).has(t);
}
var _t = Ve;
function Je(t, e) {
  var r = b(this, t),
    n = r.size;
  return r.set(t, e), (this.size += r.size == n ? 0 : 1), this;
}
var wt = Je;
function C(t) {
  var e = -1,
    r = t == null ? 0 : t.length;
  this.clear();
  while (++e < r) {
    var n = t[e];
    this.set(n[0], n[1]);
  }
}
C.prototype.clear = mt;
C.prototype.delete = xt;
C.prototype.get = bt;
C.prototype.has = _t;
C.prototype.set = wt;
var c$e = C;
var Ke = "Expected a function";
function z(t, e) {
  if (typeof t != "function" || (e != null && typeof e != "function")) throw TypeError(Ke);
  var r = function () {
    var n = arguments,
      o = e ? e.apply(this, n) : n[0],
      i = r.cache;
    if (i.has(o)) return i.get(o);
    var s = t.apply(this, n);
    return (r.cache = i.set(o, s) || i), s;
  };
  return (r.cache = new (z.Cache || c$e)()), r;
}
z.Cache = c$e;
var ai = z;
var pme = /[.\s]+$/,
  zxn = /\.(CON|PRN|AUX|NUL|COM[1-9]|LPT[1-9])$/i;
function xU(t) {
  let e = t.indexOf("."),
    r = (e === -1 ? t : t.slice(0, e)).replace(/ +$/, "");
  return /^(con|prn|aux|nul|com[0-9\u00B9\u00B2\u00B3]|lpt[0-9\u00B9\u00B2\u00B3])$/i.test(r);
}
function ncr(t) {
  return pme.test(t) || zxn.test(t);
}
function rcr(t) {
  return /^(?=[^.]{3,8}(?:\.|$))[^.]{1,6}~[0-9]{1,6}(?:\.[^.]{1,3})?$/.test(t);
}
import { lstatSync as Ze, readlinkSync as K, realpathSync as Pt } from "fs";
import { readlink as Be } from "fs/promises";
import { homedir as Dt } from "os";
import {
  basename as U,
  dirname as c,
  isAbsolute as h,
  join as G,
  parse as Z,
  relative as V,
  resolve as f,
  sep as d,
} from "path";
import * as J from "path";
function tr(t) {
  return t.normalize("NFC");
}
function Bn(t) {
  return /^[\\/]{2}/.test(t) || Dj(t);
}
function tZe(t) {
  return /(^|[\\/])\.\.([\\/]|$)/.test(t);
}
function vu(t) {
  return /^[\\/]\?\?[\\/]/.test(t);
}
function Z5(t) {
  if (/^[\\/]{2}[?.][\\/](?!unc[\\/])/i.test(t)) return null;
  return t.match(/^[\\/]{2}(?:[?.][\\/]unc[\\/])?([^\\/]+)/i)?.[1]?.replace(/[A-Z]/g, (e) => e.toLowerCase()) ?? null;
}
function oc(t) {
  return /(^|[\\/])\.{1,2}[. ]*([\\/]|$)/.test(t);
}
function jCt(t, e = "darwin") {
  return e === "win32" ? /(^|[\\/])\.{1,2}([\\/]|$)/.test(t) : /(^|\/)\.{1,2}(\/|$)/.test(t);
}
function IU(t) {
  return jCt(t);
}
function JO(t) {
  return /^[\\/]{2}[?.][\\/]/.test(t);
}
function v5t(t) {
  return /^[\\/](GLOBAL\?\?|GLOBALROOT|DosDevices|Device)[\\/]/i.test(t);
}
function u$e(t, e) {
  if (!Bn(t)) return !1;
  if (JO(t) || Dj(t)) return !0;
  if (oc(t)) return !0;
  let r = Z5(t);
  return r === null || r !== Z5(e);
}
function T(t) {
  return (t.length === 2 && W4(t[0]) === "net") || (t.length === 3 && W4(t[0]) === "network" && W4(t[1]) === "servers");
}
function R(t) {
  if (!t.startsWith("/")) return null;
  let e = [];
  for (let r of t.split("/")) {
    if (r === "" || r === ".") continue;
    if (r === "..") {
      e.pop();
      continue;
    }
    if ((e.push(r), T(e))) return B(r);
  }
  return null;
}
function B(t) {
  return t.replace(/[A-Z]/g, (e) => e.toLowerCase());
}
function W4(t) {
  return t
    .replace(/[\u200c-\u200f\u202a-\u202e\u206a-\u206f\ufeff]/g, "")
    .toUpperCase()
    .toLowerCase();
}
function WCt(t, e = "darwin") {
  let r = t.split("/");
  if (r[0] !== "") return t;
  let n, o;
  if (e === "darwin" ? T(r.slice(1, 3)) : r[1] === "net" && r.length >= 3) (n = "/net/"), (o = 2);
  else if (e === "darwin" && T(r.slice(1, 4))) (n = "/Network/Servers/"), (o = 3);
  else return t;
  let i = r[o];
  if (i === void 0 || i === "") return t;
  let s = r.slice(o + 1);
  return n + B(i).replace(/\.$/, "") + (s.length > 0 ? "/" + s.join("/") : "");
}
function R5t(t) {
  let e = /^([\\/]{2,})([A-Za-z0-9-]+(?:\.[A-Za-z0-9-]+)*)\.(?=[\\/]|$)/.exec(t);
  if (e === null || e[2].toLowerCase() === "wsl.localhost") return t;
  return e[1][0].repeat(2) + e[2] + t.slice(e[1].length + e[2].length + 1);
}
function HR(t, e) {
  if (!_r(t)) return !1;
  let r = R(t);
  return r === null || r !== R(e);
}
function Sc(t, e) {
  let r = f(e, t);
  if (u$e(t, e) || u$e(r, e)) return !0;
  let n = R(e);
  for (let o of [t, r])
    if (_r(o)) {
      let i = R(o);
      if (i === null || i !== n) return !0;
    }
  return !1;
}
function _r(t) {
  return ns(t);
}
function nZe(t) {
  if (!t.startsWith("/")) return !1;
  let e = t.split("/").filter((n) => n !== "" && n !== ".");
  if (e.length < 1 || e.length > 3 || e.includes("..")) return !1;
  if (T(e) && (e.length === 2 || !0)) return !0;
  let r = e[0].toLowerCase();
  return e.length <= 2 && (r === "net" || r === "network" || (r === "home" && e.length === 1));
}
function Ru(t) {
  return Bn(t) || _r(t);
}
function ns(t) {
  return PU(t) !== null;
}
function PU(t) {
  if (!t.startsWith("/")) return null;
  let e = [];
  for (let r of t.split("/")) {
    if (r === "" || r === ".") continue;
    if (r === "..") {
      e.pop();
      continue;
    }
    if ((e.push(r), T(e))) return "/" + e.join("/");
  }
  return null;
}
function Xe(t, e) {
  if (!t.startsWith("/")) return null;
  let r = [];
  for (let n of t.split("/")) {
    if (n === "" || n === ".") continue;
    if (n === "..") {
      r.pop();
      continue;
    }
    if ((r.push(n), r.length === 2 && r[0].toLowerCase() === e)) return "/" + r.join("/");
  }
  return null;
}
function ocr(t) {
  let e = PU(t);
  return e === null ? null : e.slice(e.lastIndexOf("/") + 1);
}
var Ye = /^([\\/]{2}(?:wsl\.localhost|wsl\$)[\\/]+[^\\/]+)((?:[\\/].*)?)$/i;
function rZe(t) {
  let e = Ye.exec(t);
  return e === null ? void 0 : { prefix: e[1], rest: e[2] ?? "" };
}
function VC(t) {
  return ns(t) || /^\/Network\//iu.test(t) || Xe(t, "network") !== null;
}
function oZe(t) {
  return Qi(t) || ns(t) || JO(t) || Dj(t) || v5t(t);
}
function Vxn(t) {
  let e = t.split("/").filter(Boolean);
  if (e.length === 0) return t;
  let r = e.at(-1);
  return "/" + [...e.slice(0, -1).map(W4), B(r)].join("/");
}
function qCt(t) {
  if (!t.startsWith("/")) return [];
  let e = [],
    r = [];
  for (let n of t.split("/")) {
    if (n === "" || n === ".") continue;
    if (n === "..") {
      r.pop();
      continue;
    }
    if ((r.push(n), T(r))) e.push(Vxn("/" + r.join("/")));
  }
  return e;
}
function Ms(t) {
  return /^[\\/]{2}wsl(\$|\.localhost)[\\/]/i.test(t);
}
function Qi(t) {
  return Bn(t) && !Ms(t);
}
function km(t) {
  if (t === "~" || t.startsWith("~/")) return Dt() + t.slice(1);
  return t;
}
function DU(t) {
  for (let e = 0; e < t.length; e++) {
    let r = t[e];
    if (r === "*" || r === "?") return e;
    if (r === "[" && t.indexOf("]", e + 1) !== -1) return e;
  }
  return -1;
}
function gie(t) {
  if (t.startsWith("\\\\?\\UNC\\")) return "\\\\" + t.slice(8);
  if (t.startsWith("\\\\?\\") && t.length >= 7 && t[5] === ":") return t.slice(4);
  return t;
}
function SS(t) {
  if (RP(t)) {
    let e = gie(t).replace(/^([\\/])[\\/]+/, "$1");
    for (let r = 0; r < qe && Dj(e); r++) {
      e = kt(e).replace(/^([\\/])[\\/]+/, "$1");
      let n = 1;
      while (e[n] === "?" && e[n + 1] === "?" && (n + 2 === e.length || vt(e[n + 2]))) {
        n += 3;
        while (vt(e[n])) n++;
      }
      e = e.slice(0, 1) + e.slice(n);
    }
    return Dj(e) ? e.slice(0, 1) : e;
  }
  return t;
}
var qe = 4;
function vt(t) {
  return t === "\\" || t === "/";
}
function RP(t) {
  if (/^\\\\\?\\volume\{/i.test(t)) return St(t);
  if (Dj(t)) return !0;
  let e = gie(t);
  if (e !== t && St(e)) return !0;
  return Bn(e) && !Ms(e);
}
function St(t) {
  return oc(t) || t.includes("/");
}
function bS(t) {
  try {
    return gie(Pt.native(t));
  } catch {
    return null;
  }
}
var Et = 50,
  yt;
function UJ(t) {
  return Rt(t).real ?? null;
}
function GCt(t, e) {
  let r = f(e).toLowerCase(),
    n = c(f(t)).toLowerCase();
  if (n === r || Ct(n, r) || Tt(n, r, "lexical")) return !0;
  let o = UJ(e)?.toLowerCase();
  if (o == null) return !1;
  let i = UJ(c(f(t)))?.toLowerCase();
  if (i == null) return !0;
  return i === o || Ct(i, o) || Tt(i, o, "canonical");
}
var Qe = new Set([
  "node_modules",
  ".venv",
  "venv",
  "env",
  ".env",
  "virtualenv",
  ".tox",
  ".nox",
  ".direnv",
  "__pypackages__",
]);
function Ct(t, e) {
  if (!t.startsWith(j(e) + d)) return !1;
  return t.split(d).some((r) => Qe.has(r));
}
function j(t) {
  return t.endsWith(d) ? t.slice(0, -1) : t;
}
var F;
function Tt(t, e, r) {
  let n = Dt(),
    o = (process.env.LOCALAPPDATA ?? "").trim();
  if (r === "lexical")
    return M(
      Ot(n, o, (u) => f(u).toLowerCase()),
      t,
      e,
    );
  let i = n + "\x00" + o;
  if (F?.key === i) return M(F.aliasDirs, t, e);
  let s = !0,
    a = Ot(n, o, (u) => {
      if (!er(u)) return f(u).toLowerCase();
      let l = UJ(u)?.toLowerCase();
      if (l == null) return (s = !1), f(u).toLowerCase();
      return l;
    });
  if (s) F = { key: i, aliasDirs: a };
  return M(a, t, e);
}
function Ot(t, e, r) {
  let n = [];
  if (t) n.push(j(r(t)) + d + ["appdata", "local", "microsoft", "windowsapps"].join(d));
  if (e) {
    let o = j(r(e)) + d + ["microsoft", "windowsapps"].join(d);
    if (!n.includes(o)) n.push(o);
  }
  return n;
}
function er(t) {
  return t.startsWith("/") && !Bn(t);
}
function M(t, e, r) {
  let n = j(r);
  for (let o of t) {
    if (!(o === n || o.startsWith(n + d))) continue;
    if (e === o || e.startsWith(o + d)) return !0;
  }
  return !1;
}
var rr = [
  "usr/local",
  "usr/libexec/cups",
  "usr/share/snmp",
  "AppleInternal",
  "Applications",
  "Library",
  "Users",
  "Volumes",
  "cores",
  "home",
  "media",
  "mnt",
  "opt",
  "pkg",
  "private",
  "sw",
];
function BJ(t) {
  let e = t.split("/");
  if (
    e.length < 5 ||
    e[0] !== "" ||
    A(e[1] ?? "") !== "system" ||
    A(e[2] ?? "") !== "volumes" ||
    A(e[3] ?? "") !== "data"
  )
    return t;
  let r = e.slice(4);
  for (let n of rr) {
    let o = n.split("/");
    if (r.length < o.length) continue;
    if (o.every((i, s) => A(r[s] ?? "") === A(i))) return "/" + r.join("/");
  }
  return t;
}
function A(t) {
  return t.toUpperCase().toLowerCase();
}
function q4(t) {
  return U(c(t)) === "worktrees" && U(c(c(t))) === ".claude" ? c(c(c(t))) : null;
}
function kve(t, { allowLocalWsl: e = !1 } = {}) {
  let r = (i) => e && zCt(i);
  if (r(t)) return !1;
  if (SA(t)) return !0;
  let n = G4(t);
  if (!h(n)) return !0;
  if (Hve(n, { allowLocalWsl: e })) return !0;
  let o = f(n);
  for (;;) {
    let i = Rt(o);
    if (i.real !== void 0) return SA(i.real) && !r(i.real);
    if (i.code !== "ENOENT" && i.code !== "ENOTDIR") return !0;
    let s = c(o);
    if (s === o) return !1;
    o = s;
  }
}
function zCt(t) {
  return !1;
}
function d$e(t) {
  return t;
}
var nr = /^\/mnt\/[a-z](?:\/|$)/i;
function k5t(t, e, r = "") {
  if (!t.startsWith("/") || nr.test(t) || jCt(t, "linux")) return;
  let n = r === "" ? t : t.startsWith(r + "/") ? t.slice(r.length) : void 0;
  return n === void 0 ? void 0 : e.replace(/[\\/]+$/, "") + n.replace(/\//g, "\\");
}
function H5t(t, { exactDots: e = !1 } = {}) {
  return;
}
function x5t(t, { exactDots: e = !1 } = {}) {
  let r = G4(t);
  if (e ? jCt(r, "win32") : oc(r)) return;
  let n = /^([A-Za-z]):[\\/]+(.*)$/.exec(r),
    o = rZe(r)?.rest.replace(/^[\\/]+/, ""),
    i = (n?.[2] ?? o ?? "").replace(/\\/g, "/").replace(/\/+$/, "");
  if (i === "") return;
  let s = n ? `/mnt/${n[1].toLowerCase()}/${i}` : `/${i}`;
  return SA(s) ? void 0 : s;
}
function icr(t) {
  return SA(t) && !zCt(t);
}
function SA(t) {
  let e = G4(t);
  return sr(e) || VC(e) || v5t(e);
}
function I5t(t, { allowLocalWsl: e = !1 } = {}) {
  let r = G4(t);
  if (!h(r)) return !0;
  let { root: n } = Z(r);
  return w(n, _(r.slice(n.length)), 0, e);
}
function VCt(t, e) {
  return w(t, _(e), 0, !1, { trustedStart: !0, trustedRoot: t });
}
function KCt(t, e = c(t)) {
  let r = N(() => K(t));
  if (r.value === void 0) return r.code !== "ENOENT" && r.code !== "EINVAL";
  let n = r.value.replace(/^\/{2,}/, "/");
  if (Lt(n)) return !0;
  let o = !1;
  if (o && oc(n)) return !0;
  let i = o ? G(Z(e).root, n) : n,
    s = { trustedStart: !0, trustedRoot: e };
  if (h(i)) {
    let l = V(e, i);
    if (!oc(i) && l !== "" && !At(l) && !h(l)) return w(e, _(l), 1, !1, s);
    return SA(i) || Hve(i);
  }
  if (SA(i)) return !0;
  let a = V(e, c(t)),
    u = !At(a) && !h(a);
  return w(u ? e : c(t), [...(u ? _(a) : []), ..._(i)], 1, !1, s);
}
function Hve(t, e = {}) {
  return I5t(t, e) || (oc(t) && I5t(f(t), e));
}
var or = 40;
function _(t) {
  return t.split(/\/+/).filter(Boolean);
}
function L(t, e) {
  return SA(t) && !(e && zCt(t));
}
function At(t) {
  return t === ".." || /^\.\.[\\/]/.test(t);
}
function Lt(t) {
  return !1;
}
function w(t, e, r, n, { trustedStart: o = !1, trustedRoot: i = t } = {}) {
  let s = i.endsWith(d) ? i : i + d,
    a = (l) => o && (l === i || l.startsWith(s)),
    u = t;
  if (!a(u) && L(u, n)) return !0;
  for (let l = 0; l < e.length; l++) {
    let I = e[l];
    if (I === ".") continue;
    if (I === "..") {
      u = c(u);
      continue;
    }
    let v = G(u, I);
    if (!a(v) && L(v, n)) return !0;
    let P = N(() => Ze(v));
    if (P.value === void 0) {
      if (P.code !== "ENOENT" && P.code !== "ENOTDIR") return !0;
      let g = v;
      for (let D of e.slice(l + 1))
        if (((g = D === ".." ? c(g) : D === "." ? g : G(g, D)), !a(g) && L(g, n))) return !0;
      return !1;
    }
    let k;
    if (P.value.isSymbolicLink()) {
      if (r >= or) return !0;
      if (((k = N(() => K(v)).value), k === void 0)) return !0;
    } else {
      u = v;
      continue;
    }
    let p = k.replace(/^\/{2,}/, "/");
    if (!(o && h(p) && !oc(p) && a(p)) && L(p, n)) return !0;
    let H = e.slice(l + 1);
    if (Lt(p)) return !0;
    if (h(p)) {
      if (o && !oc(p) && a(p))
        return w(i, [..._(p.slice(s.length)), ...H], r + 1, n, { trustedStart: !0, trustedRoot: i });
      let g = Z(p).root;
      return w(g, [..._(p.slice(g.length)), ...H], r + 1, n);
    }
    return w(u, [..._(p), ...H], r + 1, n, { trustedStart: a(u), trustedRoot: i });
  }
  return !1;
}
function Rt(t) {
  let e = N(() => gie(Pt.native(t)));
  return { real: e.value, code: e.code };
}
function X(t) {
  return t != null && typeof t === "object" && "code" in t && typeof t.code === "string" ? t.code : void 0;
}
function N(t) {
  for (let e = 0; ; e++)
    try {
      return { value: t() };
    } catch (r) {
      let n = X(r);
      return { code: n };
    }
}
function jt(t) {
  let e = X(t);
  return e === "EINVAL"
    ? { kind: "directory" }
    : e === "ENOENT"
      ? { kind: "absent" }
      : { kind: "error", code: e, error: t };
}
function scr(t) {
  let e,
    r = N(() => {
      try {
        return K(t);
      } catch (n) {
        throw ((e = n), n);
      }
    });
  return r.value !== void 0 ? { kind: "junction", target: r.value } : jt(e);
}
async function hie(t) {
  let e,
    r = await ir(async () => {
      try {
        return await Be(t);
      } catch (n) {
        throw ((e = n), n);
      }
    });
  return r.value !== void 0 ? { kind: "junction", target: r.value } : jt(e);
}
async function ir(t) {
  for (let e = 0; ; e++)
    try {
      return { value: await t() };
    } catch (r) {
      let n = X(r);
      return { code: n };
    }
}
function sr(t) {
  return bA(t) || ns(t);
}
function It(t) {
  return (
    oc(t) ||
    /[. :](?=[\\/]|$)/.test(t) ||
    t.split(/[\\/]/).some((e) => xU(e) || xU(e.slice(0, Math.max(0, e.indexOf(":")))))
  );
}
function G4(t) {
  let e = /^(?:\\\\[?.]\\|\\\?\?\\)(?=[A-Za-z]:[\\/])/.exec(t);
  if (!e) return t;
  let r = t.slice(e[0].length),
    n = r.slice(2);
  return (e[0].includes("\\") && n.includes("/")) || It(n) ? t : r;
}
function bb(t, { foldCase: e, knownNotSuspect: r = !1 } = {}) {
  if (((e ??= !0), (t = G4(t)), bA(t) || ns(t))) {
    let a = d$e(f(d$e(t))).normalize("NFC");
    return e ? a.toLowerCase() : a;
  }
  if (!r && Hve(h(t) ? t : f(t))) {
    let a = f(t).normalize("NFC");
    return e ? a.toLowerCase() : a;
  }
  let n = f(t),
    o = [],
    i = bS(n);
  while (i === null && c(n) !== n) o.unshift(U(n)), (n = c(n)), (i = bS(n));
  i = o.length > 0 ? f(i ?? n, ...o) : (i ?? n);
  let s = d$e(i).normalize("NFC");
  return e ? s.toLowerCase() : s;
}
function n_(t, e, { alreadyComparable: r = !1, foldCase: n } = {}) {
  let o = V(r ? t : bb(t, { foldCase: n }), r ? e : bb(e, { foldCase: n }));
  return o === "" || (!h(o) && o !== ".." && !o.startsWith(`..${d}`));
}
function bA(t) {
  return Bn(t);
}
function Dj(t) {
  return Nt.test(t) || (t.includes("??") && Nt.test(kt(t)));
}
var Nt = /^[\\/]\?\?[\\/]/;
function kt(t) {
  return J.win32 ? J.win32.normalize(t) : t;
}
export {
  Q5,
  vve,
  C5t,
  YT,
  lx,
  Ij,
  Rm,
  l$e,
  dme,
  Pj,
  Rve,
  c$e,
  ai,
  pme,
  zxn,
  xU,
  ncr,
  rcr,
  tr,
  Bn,
  tZe,
  vu,
  Z5,
  oc,
  jCt,
  IU,
  JO,
  v5t,
  u$e,
  W4,
  WCt,
  R5t,
  HR,
  Sc,
  _r,
  nZe,
  Ru,
  ns,
  PU,
  ocr,
  rZe,
  VC,
  oZe,
  Vxn,
  qCt,
  Ms,
  Qi,
  km,
  DU,
  gie,
  SS,
  RP,
  bS,
  UJ,
  GCt,
  BJ,
  q4,
  kve,
  zCt,
  d$e,
  k5t,
  H5t,
  x5t,
  icr,
  SA,
  I5t,
  VCt,
  KCt,
  Hve,
  scr,
  hie,
  G4,
  bb,
  n_,
  bA,
  Dj,
};
