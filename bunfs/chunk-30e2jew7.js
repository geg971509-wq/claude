...
class f {
  ...
  memoState = c;
  getState() {
    let r = process.env[VS];
    if (!r) return c;
    let e = r === this.memoRaw ? this.memoState.error : null;
    if (r === this.memoRaw && e === null) return this.memoState;
    if (((this.memoRaw = r), (this.memoState = v(r)), this.memoState.error && this.memoState.error !== e))
      n(
        `${VS} is set but can't be used \u2014 self-spawns that require it will refuse to start rather than run unwrapped: ${this.memoState.error}`,
        { level: "error" },
      );
    else if (this.memoState.platformIgnored)
      n(
        `${VS} is set but ignored on Windows \u2014 the launcher must exec into Claude Code, which Windows can't do; sessions run unwrapped`,
        { level: "warn" },
      );
...
async function Hde() {
  let r = ru();
  if (r) return r;
  if (await ZE()) return null;
  return `${VS}: launcher \`${Xl()[0]}\` was deleted or is not executable \u2014 restore it (or fix the setting), then retry`;
}
function Gk() {
  return eSt().record;
}
function v(r) {
  ...
  let s = e[0];
  if (s === process.execPath || s === w(ZD(), "claude"))
    return i(`launcher \`${s}\` is Claude Code's own launch path \u2014 point ${VS} at your launcher, not at claude`);
  if (!d(s)) return i("the launcher must be an absolute path, not a bare name resolved via PATH");
  try {
    let o = m(s);
    if (!o.isFile() || (o.mode & 73) === 0) return i(`launcher \`${s}\` is not an executable regular file`);
  } catch {
...
export { VS, kde, eSt, Xl, ru, ZE, ryn, oyn, Hde, Gk };
