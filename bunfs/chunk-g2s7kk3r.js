// @bun @bytecode
// Claude Code is a Beta product per Anthropic's Commercial Terms of Service.
// By using Claude Code, you agree that all code acceptance or rejection decisions you make,
// and the associated conversations in context, constitute Feedback under Anthropic's Commercial Terms,
// and may be used to improve Anthropic's products, including training models.
// You are responsible for reviewing any code suggestions before use.

// (c) Anthropic PBC. All rights reserved. Use is subject to the Legal Agreements outlined here: https://code.claude.com/docs/en/legal-and-compliance.

// Version: 2.1.252
import { S5 } from "/$bunfs/root/chunk-dmc9meqw.js";
import { S, ue } from "/$bunfs/root/chunk-yz031c9r.js";
var _ = S(function (b) {
  Object.defineProperty(b, "__esModule", { value: !0 });
  b.getHomeDir = void 0;
  var W = ue("os"),
    V = ue("path"),
    m = {},
    G = () => {
      if (process && process.geteuid) return `${process.geteuid()}`;
      return "DEFAULT";
    },
    U = () => {
      let { HOME: e, USERPROFILE: t, HOMEPATH: r, HOMEDRIVE: o = `C:${V.sep}` } = process.env;
      if (e) return e;
      if (t) return t;
      if (r) return `${o}${r}`;
      let n = G();
      if (!m[n]) m[n] = (0, W.homedir)();
      return m[n];
    };
  b.getHomeDir = U;
});
var x = S(function (D) {
  Object.defineProperty(D, "__esModule", { value: !0 });
  D.getSSOTokenFilepath = void 0;
  var X = ue("crypto"),
    B = ue("path"),
    J = _(),
    z = (e) => {
      let r = (0, X.createHash)("sha1").update(e).digest("hex");
      return (0, B.join)((0, J.getHomeDir)(), ".aws", "sso", "cache", `${r}.json`);
    };
  D.getSSOTokenFilepath = z;
});
var H = S(function (y) {
  Object.defineProperty(y, "__esModule", { value: !0 });
  y.getSSOTokenFromFile = y.tokenIntercept = void 0;
  var Q = ue("fs/promises"),
    Y = x();
  y.tokenIntercept = {};
  var Z = async (e) => {
    if (y.tokenIntercept[e]) return y.tokenIntercept[e];
    let t = (0, Y.getSSOTokenFilepath)(e),
      r = await (0, Q.readFile)(t, "utf8");
    return JSON.parse(r);
  };
  y.getSSOTokenFromFile = Z;
});
var L = S(function (R) {
  Object.defineProperty(R, "__esModule", { value: !0 });
  R.readFile = R.fileIntercept = R.filePromises = void 0;
  var ee = ue("fs/promises");
  R.filePromises = {};
  R.fileIntercept = {};
  var te = (e, t) => {
    if (R.fileIntercept[e] !== void 0) return R.fileIntercept[e];
    if (!R.filePromises[e] || t?.ignoreCache) R.filePromises[e] = (0, ee.readFile)(e, "utf8");
    return R.filePromises[e];
  };
  R.readFile = te;
});
var mA = S(function (a) {
  var f = _(),
    N = x(),
    P = H(),
    F = ue("path"),
    g = S5(),
    u = L(),
    A = "AWS_PROFILE",
    q = "default",
    re = (e) => e.profile || process.env[A] || q,
    c = ".",
    oe = (e) =>
      Object.entries(e)
        .filter(([t]) => {
          let r = t.indexOf(c);
          if (r === -1) return !1;
          return Object.values(g.IniSectionType).includes(t.substring(0, r));
        })
        .reduce(
          (t, [r, o]) => {
            let n = r.indexOf(c),
              i = r.substring(0, n) === g.IniSectionType.PROFILE ? r.substring(n + 1) : r;
            return (t[i] = o), t;
          },
          { ...(e.default && { default: e.default }) },
        ),
    ne = "AWS_CONFIG_FILE",
    M = () => process.env[ne] || F.join(f.getHomeDir(), ".aws", "config"),
    ie = "AWS_SHARED_CREDENTIALS_FILE",
    se = () => process.env[ie] || F.join(f.getHomeDir(), ".aws", "credentials"),
    ce = /^([\w-]+)\s(["'])?([\w-@\+\.%:/]+)\2$/,
    ae = ["__proto__", "profile __proto__"],
    T = (e) => {
      let t = {},
        r,
        o;
      for (let n of e.split(/\r?\n/)) {
        let i = n.split(/(^|\s)[;#]/)[0].trim();
        if (i[0] === "[" && i[i.length - 1] === "]") {
          (r = void 0), (o = void 0);
          let s = i.substring(1, i.length - 1),
            l = ce.exec(s);
          if (l) {
            let [, p, , h] = l;
            if (Object.values(g.IniSectionType).includes(p)) r = [p, h].join(c);
          } else r = s;
          if (ae.includes(s)) throw Error(`Found invalid profile name "${s}"`);
        } else if (r) {
          let s = i.indexOf("=");
          if (![0, -1].includes(s)) {
            let [l, p] = [i.substring(0, s).trim(), i.substring(s + 1).trim()];
            if (p === "") o = l;
            else {
              if (o && n.trimStart() === n) o = void 0;
              t[r] = t[r] || {};
              let h = o ? [o, l].join(c) : l;
              t[r][h] = p;
            }
          }
        }
      }
      return t;
    },
    k = () => ({}),
    K = async (e = {}) => {
      let { filepath: t = se(), configFilepath: r = M() } = e,
        o = f.getHomeDir(),
        n = "~/",
        i = t;
      if (t.startsWith("~/")) i = F.join(o, t.slice(2));
      let O = r;
      if (r.startsWith("~/")) O = F.join(o, r.slice(2));
      let s = await Promise.all([
        u.readFile(O, { ignoreCache: e.ignoreCache }).then(T).then(oe).catch(k),
        u.readFile(i, { ignoreCache: e.ignoreCache }).then(T).catch(k),
      ]);
      return { configFile: s[0], credentialsFile: s[1] };
    },
    le = (e) =>
      Object.entries(e)
        .filter(([t]) => t.startsWith(g.IniSectionType.SSO_SESSION + c))
        .reduce((t, [r, o]) => ({ ...t, [r.substring(r.indexOf(c) + 1)]: o }), {}),
    pe = () => ({}),
    fe = async (e = {}) =>
      u
        .readFile(e.configFilepath ?? M())
        .then(T)
        .then(le)
        .catch(pe),
    de = (...e) => {
      let t = {};
      for (let r of e)
        for (let [o, n] of Object.entries(r))
          if (t[o] !== void 0) Object.assign(t[o], n);
          else t[o] = n;
      return t;
    },
    Fe = async (e) => {
      let t = await K(e);
      return de(t.configFile, t.credentialsFile);
    },
    ge = {
      getFileRecord() {
        return u.fileIntercept;
      },
      interceptFile(e, t) {
        u.fileIntercept[e] = Promise.resolve(t);
      },
      getTokenRecord() {
        return P.tokenIntercept;
      },
      interceptToken(e, t) {
        P.tokenIntercept[e] = t;
      },
    };
  Object.defineProperty(a, "getSSOTokenFromFile", {
    enumerable: !0,
    get: function () {
      return P.getSSOTokenFromFile;
    },
  });
  Object.defineProperty(a, "readFile", {
    enumerable: !0,
    get: function () {
      return u.readFile;
    },
  });
  a.CONFIG_PREFIX_SEPARATOR = c;
  a.DEFAULT_PROFILE = q;
  a.ENV_PROFILE = A;
  a.externalDataInterceptor = ge;
  a.getProfileName = re;
  a.loadSharedConfigFiles = K;
  a.loadSsoSessionData = fe;
  a.parseKnownFiles = Fe;
  Object.keys(f).forEach(function (e) {
    if (e !== "default" && !Object.prototype.hasOwnProperty.call(a, e))
      Object.defineProperty(a, e, {
        enumerable: !0,
        get: function () {
          return f[e];
        },
      });
  });
  Object.keys(N).forEach(function (e) {
    if (e !== "default" && !Object.prototype.hasOwnProperty.call(a, e))
      Object.defineProperty(a, e, {
        enumerable: !0,
        get: function () {
          return N[e];
        },
      });
  });
});
export { mA };
