...
// Version: 2.1.252
import { IO, Q$, hR, LCn } from "/$bunfs/root/chunk-d7nfr7mk.js";
...
function Axe(e = fetch, t) {
  if (!t) return e;
  return async (r, s) => {
    let a = { ...t, ...s, headers: s?.headers ? { ...L9e(t.headers), ...L9e(s.headers) } : t.headers };
    return e(r, a);
  };
}
var m = {
...
class AMt {
  ...
  async _startOrAuthSse(e) {
    ...
    try {
      ...
      if (!s.ok) {
        if ((await s.body?.cancel(), s.status === 401 && this._authProvider)) return await this._authThenStart();
  ...
  _scheduleReconnection(e, t = 0) {
    ...
    this._reconnectionTimeout = setTimeout(() => {
      this._startOrAuthSse(e).catch((a) => {
        this.onerror?.(Error(`Failed to reconnect SSE stream: ${a instanceof Error ? a.message : String(a)}`)),
    ...
    (async () => {
      try {
        ...
        if ((r || n) && !u && this._abortController && !this._abortController.signal.aborted)
          this._scheduleReconnection({ resumptionToken: d, onresumptiontoken: s, replayMessageId: a }, 0);
      } catch (l) {
        if (
          ...
          try {
            ...
            this.onerror?.(Error(`Failed to reconnect: ${o instanceof Error ? o.message : String(o)}`));
          }
      }
    })();
  }
  ...
  async send(e, t) {
    try {
      ...
      if (r) {
        this._startOrAuthSse({ resumptionToken: r, replayMessageId: IO(e) ? e.id : void 0 }).catch((i) =>
          this.onerror?.(i),
        );
        return;
      }
      let a = await this._commonHeaders();
      a.set("content-type", "application/json"), a.set("accept", "application/json, text/event-stream");
      let d = {
          ...this._requestInit,
          method: "POST",
          headers: a,
      ...
      if (!n.ok) {
        ...
        if (n.status === 401 && this._authProvider) {
          if (this._hasCompletedAuthFlow) throw new PW(401, "Server returned 401 after successful authentication");
          let { resourceMetadataUrl: o, scope: c } = Cxe(n);
          if (
        ...
        if (n.status === 403 && this._authProvider) {
          ...
          if (h === "insufficient_scope") {
            ...
            if (this._lastUpscopingHeader === f) throw new PW(403, "Server returned 403 after trying upscoping");
            if (c) this._scope = c;
      ...
      if (l)
        ...
        } else throw (await n.body?.cancel(), new PW(-1, `Unexpected content type: ${_}`));
  ...
  async terminateSession() {
    ...
    try {
      ...
      if ((await r.body?.cancel(), !r.ok && r.status !== 405))
        throw new PW(r.status, `Failed to terminate session: ${r.statusText}`);
