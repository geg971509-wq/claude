...
// Version: 2.1.252
import { QY, o4, Ir, ZY, fEt, mEt, h5, DN, qT, jYe, KGt, tfe, E1e, Roe, ezt, Er } from "/$bunfs/root/chunk-d7nfr7mk.js";
...
class g {
  ...
  createMessageStream(e, t) {
    ...
    if (e.messages.length > 0) {
      ...
      if (n) {
        ...
        if (!c) throw Error("tool_result blocks are not matching any tool_use from the previous message");
      }
      if (c) {
    ...
    return this.requestStream({ method: "sampling/createMessage", params: e }, tfe, t);
  }
  elicitInputStream(e, t) {
    ...
    return this.requestStream({ method: "elicitation/create", params: o }, Roe, t);
  }
  async getTask(e, t) {
...
class GH extends xYe {
  constructor(e, t) {
    ...
    if (
      ...
      this.setRequestHandler(fEt, (i) => this._oninitialize(i)),
      this.setNotificationHandler(mEt, () => this.oninitialized?.()),
      this._capabilities.logging)
    )
      this.setRequestHandler(KGt, async (i, s) => {
    ...
    if (o === "tools/call") {
      let r = async (h, c) => {
        ...
        if (!a.success) {
          ...
          throw new Er(Ir.InvalidParams, `Invalid tools/call request: ${p}`);
        }
        let { params: l } = a.data,
        ...
        if (l.task) {
          ...
          if (!p.success) {
            ...
            throw new Er(Ir.InvalidParams, `Invalid task creation result: ${d}`);
          }
          return p.data;
        }
        let f = PN(DN, u);
        if (!f.success) {
          ...
          throw new Er(Ir.InvalidParams, `Invalid tools/call result: ${p}`);
        }
        return f.data;
      };
  ...
  assertTaskCapability(e) {
    lEt(this._clientCapabilities?.tasks?.requests, e, "Client");
  }
  assertTaskHandlerCapability(e) {
    if (!this._capabilities) return;
    aEt(this._capabilities.tasks?.requests, e, "Server");
  }
  async _oninitialize(e) {
    ...
    return (
      ...
      {
        protocolVersion: o4.includes(t) ? t : QY,
  ...
  async ping() {
    return this.request({ method: "ping" }, ZY);
  }
  async createMessage(e, t) {
    ...
    if (e.tools) return this.request({ method: "sampling/createMessage", params: e }, E1e, t);
    return this.request({ method: "sampling/createMessage", params: e }, tfe, t);
  }
  async elicitInput(e, t) {
    switch (e.mode ?? "form") {
      case "url": {
        ...
        let s = e;
        return this.request({ method: "elicitation/create", params: s }, Roe, t);
      }
      case "form": {
        ...
        let s = e.mode === "form" ? e : { ...e, mode: "form" },
          o = await this.request({ method: "elicitation/create", params: s }, Roe, t);
  ...
  async listRoots(e, t) {
    return this.request({ method: "roots/list", params: e }, ezt, t);
  }
  async sendLoggingMessage(e, t) {
    if (this._capabilities.logging) {
      if (!this.isMessageIgnored(e.level, t)) return this.notification({ method: "notifications/message", params: e });
    }
  }
