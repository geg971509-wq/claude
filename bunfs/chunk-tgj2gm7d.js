...
// Version: 2.1.252
import { u9e } from "/$bunfs/root/chunk-4wsm8srq.js";
...
class sxe extends Error {
  constructor(e) {
    super(e);
    this.name = "NoExtensionConnectedError";
  }
}
class axe extends Error {
  constructor(e) {
    super(e);
    this.name = "ToolCallTimeoutError";
  }
}
class Bae extends Error {
  constructor(e) {
    super(e);
...
class v {
  ...
  async connect() {
    ...
    (this.socket = O(o)),
      (this.connectTimer = setTimeout(() => {
        if (!this.connected)
          t.info(`[${e}] Connection attempt timed out after 5000ms`), this.closeSocket(), this.scheduleReconnect();
      }, 5000)),
      ...
      this.socket.on("data", (l) => {
        ...
        while (this.responseBuffer.length >= 4) {
          ...
          try {
            ...
            } else if (L(r)) t.info(`[${e}] Received tool response: ${r}`), this.handleResponse(r);
  ...
  scheduleReconnect() {
    ...
    if (this.reconnectAttempts > n) {
      t.info(`[${e}] Giving up after ${n} attempts. Will retry on next tool call.`), (this.reconnectAttempts = 0);
      return;
    }
  ...
  async sendRequest(e, t = 30000) {
    ...
    if (!this.socket) throw new DA(`[${n}] Cannot send request: not connected`);
    let o = this.socket;
    return new Promise((l, c) => {
      let s = setTimeout(() => {
        (this.responseCallback = null), c(new DA(`[${n}] Tool request timed out after ${t}ms`));
      }, t);
  ...
  async validateSocketSecurity(e) {
    ...
    try {
      ...
      if (i !== void 0 && s.uid !== i)
        throw Error(`Socket not owned by current user (uid: ${i}, socket uid: ${s.uid}). Potential security risk.`);
...
async function _(e, t) {
  ...
  if (l)
    o.push({ type: "text", text: "Include the saved path(s) in your response so they can be attached for the user." });
  if (c)
    o.push({
      type: "text",
      text:
...
async function K(e, t) {
  ...
  let c = n
      .slice(0, 8)
      .map((r, i) => `${i + 1}. ${b(r, i)} (${r.osPlatform ?? "unknown OS"}) \u2014 deviceId: ${r.deviceId}`)
      .join(`
`),
    s =
      n.length > 8
...
\u2026and ${n.length - 8} more`
  ...
  return {
    content: [
      {
        type: "text",
        text: `Multiple Chrome browsers are connected to this account and none has been selected for this session. ${u9e(e.askUserToolName)}
...
async function I(e, t, n) {
  try {
    ...
    let l = (await t.callTool("tabs_context_mcp", {}, n ? { ...n, permissionMode: "ask" } : void 0))?.result?.content;
    if (!Array.isArray(l)) return;
    let c;
    for (let i of l) {
      if (typeof i !== "object" || i === null || i.type !== "text" || typeof i.text !== "string") continue;
      try {
    ...
    let s = c.availableTabs.filter(
        (i) => typeof i === "object" && i !== null && typeof i.tabId === "number" && typeof i.url === "string",
      ),
      r =
...
var tlt = async (e, t, n, o, l) => {
  ...
  try {
    ...
    if (
      (e.logger.silly(`[${e.serverName}] Server is connected: ${a}. Received tool call: ${n} with args: ${B(c)}.`), a)
    ) {
      let d = await K(e, t);
      if (d) return d;
      if (e.handleHostTool) {
...
function l9e(e) {
  ...
  for (let n of t) {
    if (typeof n !== "object" || n === null || n.type !== "text" || typeof n.text !== "string") continue;
    let o = n.text;
    try {
      let l = JSON.parse(o);
...
async function ne(e, t, n, o, l) {
  ...
  if (c === "back" || c === "forward")
    return {
      args: o,
      error: g('tabId is required for url:"back"/"forward". Call tabs_context_mcp first to get a tab ID.'),
    };
...
${e.tabContextJson}`,
  ...
  if (e.tabGroupId !== void 0) n = { ...n, _meta: { ...n._meta, frontLoadedTabGroupId: e.tabGroupId } };
  return n;
}
async function oe(e, t, n, o, l) {
  let c = await t.callTool(n, o, l);
  if ((e.logger.silly(`[${e.serverName}] Received result from socket bridge: ${B(c)}`), c === null || c === void 0))
  ...
  let { result: s, error: r, notices: i } = c,
    ...
    d = r || s,
    u = !!r;
  if (!d) return { content: [{ type: "text", text: "Tool execution completed" }], ...(a && { _meta: a }) };
  ...
  if (typeof p === "string") return { content: [{ type: "text", text: p }], isError: u, ...(a && { _meta: a }) };
  return (
...
async function se(e, t) {
  ...
  if (o.length > 1) c.push({ type: "text", text: `${o.length} browsers are connected. ${u9e(e.askUserToolName)}` });
  return { content: c };
}
...
function C(e) {
  ...
  return e
    .map((t) => {
      ...
      if (typeof t === "object" && t !== null && "text" in t && typeof t.text === "string") return t.text;
      return "";
    })
    .join(" ");
}
...
export { DA, sxe, axe, Bae, jLt, vAr, tlt, l9e };
