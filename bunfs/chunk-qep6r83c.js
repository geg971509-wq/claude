...
// Version: 2.1.252
import{Ri}from"/$bunfs/root/chunk-rf51999f.js";import{execFile as x,execFileSync as O}from"child_process";import{chmod as h,mkdir as l,readFile as b,writeFile as p}from"fs/promises";import{join as s,resolve as k}from"path";import{promisify as P}from"util";var f=P(x),A=/^[a-zA-Z0-9_-]+$/,cIn=[2,34];function mcr(t){let e=/git version (\d+)\.(\d+)/.exec(t);if(!e)return null;return[Number(e[1]),Number(e[2])]}function gcr(t){let e=mcr(t);if(e===null)return null;let[n,o]=e,[i,r]=cIn;return n>i||n===i&&o>=r}async function Fwr(t){let e=(await f("git",["--version"],{windowsHide:!0})).stdout,n=gcr(e);if(n===null)t.onStatus(`[runner:git] --configure-git: could not parse git version from "${e.trim()}"; proceeding without version check`);else if(!n){let[g,m]=cIn;throw Error(`--configure-git requires git >= ${g}.${m} for SSH commit signing (found: ${e.trim()}). Upgrade git in your runner image, or omit --configure-git and manage git identity yourself`)}let o=s(t.baseDir,".runner");await l(o,{recursive:!0});let i=s(o,"code-sign"),r=s(o,"commit_signing_key.pub"),a=_(t.execPath);await p(i,a,{mode:493}),await h(i,493),await p(r,"");let c=[["user.name","Claude"],["user.email","noreply@anthropic.com"],["gpg.format","ssh"],["gpg.ssh.program",i],["user.signingkey",r],["commit.gpgsign","true"],["tag.gpgsign","true"]],u=t.gitConfigPath?["--file",t.gitConfigPath]:["--global"];for(let[g,m]of c)await f("git",["config",...u,"--replace-all",g,m],{windowsHide:!0});return t.onStatus(`[runner:git] --configure-git: identity=Claude <noreply@anthropic.com>, gpg.ssh.program=${i}`),await ycr(o,u,t.onStatus),i}function _(t){return`#!/bin/sh
BIN="$CLAUDE_RUNNER_CLAUDE_BIN"
[ -n "$BIN" ] || BIN='${C(t)}'
exec "$BIN" self-hosted-runner code-sign "$@"
`}function $wr(t,e){let n=s(t,".runner");return[{path:s(n,"code-sign"),content:_(e),mode:493},{path:s(n,"commit_signing_key.pub"),content:"",mode:420}]}function B5t(t){let e=s(t,".runner");return[["user.name","Claude"],["user.email","noreply@anthropic.com"],["user.signingkey",s(e,"commit_signing_key.pub")],["gpg.format","ssh"],["gpg.ssh.program",s(e,"code-sign")],["commit.gpgsign","true"],["tag.gpgsign","true"],["core.hooksPath",s(e,"git-hooks")]]}async function Uwr(){try{let{stdout:t}=await f("git",["--version"],{windowsHide:!0});return t}catch{return""}}var hcr=`#!/bin/sh
test "$1" = get || exit 0
printf "username=unused\\npassword=%s\\n" "$CLAUDE_CODE_SESSION_ACCESS_TOKEN"
`;function _cr(t){return s(t,".runner","git-proxy-cred")}function Bwr(t){let e=s(t,".runner","git-hooks");return[...w.map((n)=>({path:s(e,n),content:y})),...["commit-msg","prepare-commit-msg"].map((n)=>({path:s(e,n),content:E}))]}async function jwr(t){let e=t.apiBaseUrl.replace(/\/+$/,""),n;try{n=new URL(e).host}catch{throw Error(`--use-anthropic-git-proxy: apiBaseUrl is not a valid URL: ${e}`)}let o=s(t.baseDir,".runner");await l(o,{recursive:!0});let i=_cr(t.baseDir);await p(i,hcr,{mode:448}),await h(i,448);let r=t.gitConfigPath?["--file",t.gitConfigPath]:["--global"],a=[[`credential.https://${n}.helper`,`!'${C(i)}'`],[`credential.https://${n}.useHttpPath`,"false"],[`credential.https://${n}.username`,"unused"],[`http.https://${n}/.proactiveAuth`,"basic"]];for(let[c,u]of a)await f("git",["config",...r,"--replace-all",c,u],{windowsHide:!0});return t.onStatus(`[runner:git] --use-anthropic-git-proxy: credential helper registered for https://${n} (${i})`),e}async function ycr(t,e,n){let o=s(t,"git-hooks"),i=e[0]==="--file"?e:[],r=await f("git",["config",...i,"--get","core.hooksPath"],{windowsHide:!0}).then((a)=>a.stdout.trim(),()=>"");if(r!==""&&r!==o){n(`[runner:git] --configure-git: core.hooksPath already set (${r}); skipping Co-authored-by hook install so existing hooks keep running`);return}await l(o,{recursive:!0});for(let a of w){let c=s(o,a);await p(c,y,{mode:493}),await h(c,493)}for(let a of["commit-msg","prepare-commit-msg"]){let c=s(o,a);await p(c,E,{mode:493}),await h(c,493)}await f("git",["config",...e,"--replace-all","core.hooksPath",o],{windowsHide:!0}),n(`[runner:git] --configure-git: core.hooksPath=${o}`)}var w=["applypatch-msg","pre-applypatch","post-applypatch","pre-commit","pre-merge-commit","post-commit","pre-rebase","post-checkout","post-merge","pre-push","reference-transaction","fsmonitor-watchman","pre-auto-gc","post-rewrite","sendemail-validate","post-index-change"],y=`#!/bin/sh
