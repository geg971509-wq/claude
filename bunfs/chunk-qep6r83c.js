...
function Scr(t) {
  ...
  if (!e) throw Error(`code-sign: only SSH-style signing (-Y sign) is supported; got: ${t.join(" ")}`);
  if (!i) throw Error("code-sign: no file specified to sign");
  return { bufferFile: k(i), namespace: n, keyFile: o };
}
async function bcr(t, e) {
...
async function Wwr(t) {
  try {
    let e = Scr(t);
    await bcr(e, { env: process.env, fetchFn: fetch }), process.exit(0);
  } catch (e) {
