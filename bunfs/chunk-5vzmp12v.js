...
async function TEr(e) {
  ...
  try {
    let {
        ...
        prepareApiRequest: o,
        getOAuthHeaders: _,
      } = await import("/$bunfs/root/chunk-f3y3fqnb.js"),
      { getOauthConfig: f } = await import("/$bunfs/root/chunk-79hv0fh4.js"),
    ...
    for (; a < d; a++) {
      ...
      try {
        ...
        let { logForDebugging: y } = await import("/$bunfs/root/chunk-170j88dd.js");
      ...
      if (
        !s ||
        (!e?.exhaustive && n.length >= LIt) ||
        (e?.stopWhenOlderThan !== void 0 && !Number.isNaN(p) && p < e.stopWhenOlderThan)
      )
        break;
    }
    let { logForDebugging: R } = await import("/$bunfs/root/chunk-170j88dd.js"),
      h = a >= d && s !== null;
    ...
    let { logForDebugging: o } = await import("/$bunfs/root/chunk-170j88dd.js");
    o(`[ccr:list] failed: ${l(r)}`);
  }
  return [];
}
export { LIt, MIt, opr, TEr };
