...
var R = S(function (P) {
  ...
  var U = HCe(),
    ...
    I = (e) => {
      ...
      if (!k.exec(e)) throw TypeError("Invalid base64 string.");
...
var q = S(function (O) {
  ...
  var D = HCe(),
    ...
    z = (e) => {
      let t;
      if (typeof e === "string") t = (0, G.fromUtf8)(e);
      else t = e;
      ...
      return (0, D.fromArrayBuffer)(t.buffer, t.byteOffset, t.byteLength).toString("base64");
    };
  O.toBase64 = z;
});
var H_ = S(function (p) {
...
var p5 = S(function (N) {
    ...
    return new Promise((t, o) => {
      if (e)
        setTimeout(() => {
          ...
          (r.name = "TimeoutError"), o(r);
        }, e);
    });
  }
  var b = { supported: void 0 };
  class g {
    config;
    configProvider;
    static create(e) {
    ...
    constructor(e) {
      if (typeof e === "function") this.configProvider = e().then((t) => t || {});
      else (this.config = e ?? {}), (this.configProvider = Promise.resolve(this.config));
      if (b.supported === void 0) b.supported = Boolean(typeof Request < "u" && "keepalive" in H("https://[::1]"));
    }
    destroy() {}
    async handle(e, { abortSignal: t, requestTimeout: o } = {}) {
      ...
      let i = e.path,
        f = F.buildQueryString(e.query || {});
      if (f) i += `?${f}`;
      if (e.fragment) i += `#${e.fragment}`;
      let u = "";
      ...
      let { port: w, method: m } = e,
        C = `${e.protocol}//${u}${e.hostname}${w ? `:${w}` : ""}${i}`,
        v = m === "GET" || m === "HEAD" ? void 0 : e.body,
        d = { body: v, headers: new Headers(e.headers), method: m, credentials: a };
      if (this.config?.cache) d.cache = this.config.cache;
      if (v) d.duplex = "half";
      if (typeof AbortController < "u") d.signal = t;
      if (b.supported) d.keepalive = s;
      if (typeof this.config.requestInit === "function") Object.assign(d, this.config.requestInit(e));
      let E = () => {},
        L = H(C, d),
        B = [
          fetch(L).then((n) => {
            let h = n.headers,
              c = {};
            for (let y of h.entries()) c[y[0]] = y[1];
            if (n.body == null)
              return n.blob().then((y) => ({
                response: new j.HttpResponse({ headers: c, reason: n.statusText, statusCode: n.status, body: y }),
              }));
            return {
              response: new j.HttpResponse({ headers: c, reason: n.statusText, statusCode: n.status, body: n.body }),
            };
          }),
          Q(r),
        ];
      ...
      return Promise.race(B).finally(E);
    }
    updateHttpClientConfig(e, t) {
      (this.config = void 0), (this.configProvider = this.configProvider.then((o) => ((o[e] = t), o)));
    }
    httpHandlerConfigs() {
      return this.config ?? {};
    }
  ...
  async function J(e) {
    ...
    while (!r) {
      ...
      if (u) t.push(u), (s += u.length);
      r = f;
    }
    let a = new Uint8Array(s),
      i = 0;
    for (let f of t) a.set(f, i), (i += f.length);
    return a;
  }
  function K(e) {
    return new Promise((t, o) => {
      ...
      (r.onloadend = () => {
        if (r.readyState !== 2) return o(Error("Reader aborted too early"));
        let s = r.result ?? "",
          a = s.indexOf(","),
          i = a > -1 ? a + 1 : s.length;
        t(s.substring(i));
      }),
        (r.onabort = () => o(Error("Read aborted"))),
        (r.onerror = () => o(r.error)),
        r.readAsDataURL(e);
    });
  }
  N.FetchHttpHandler = g;
  N.keepAliveSupport = b;
  N.streamCollector = X;
});
export { H_, p5 };
