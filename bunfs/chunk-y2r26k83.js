...
import { ca } from "/$bunfs/root/chunk-kez2zndg.js";
...
function zZ(t, o = {}) {
  ...
  let p = W(i, s),
    b = i.every((l) => D.has(l.key)) && (!!p || i.every((l) => u(l, s).length === 0)) ? s.arrowSep : "/";
  if (p) {
    let l = i.map((k) => n({ ...k, ...N }));
    return Y(p, s) + l.join(b);
  }
...
function B(t, o) {
  let s = L[t];
  if (s) return s[x[o.keyCase]];
  return o.charCase === "upper" ? t.toUpperCase() : t;
}
function v(t) {
  return t.shift && !t.ctrl && !t.alt && !t.meta && !t.super && t.key.length === 1 && t.key >= "a" && t.key <= "z";
}
function U(t, o) {
...
function H(t, o, s) {
  let n = u(t, s),
    a = u(o, s);
  return n.length === a.length && n.every((i, p) => i === a[p]);
}
function Y(t, o) {
  let s = S(t);
...
function M(ae) {
  ...
  let le = X,
    G;
  if (h[1] !== w || h[2] !== P) (G = K(w, P, le)), (h[1] = w), (h[2] = P), (h[3] = G);
  else G = h[3];
  let y = G;
  if (!y) {
    return null;
  ...
  if (ie) {
    let g;
    if (h[7] !== f || h[8] !== c)
      (g = r(ca, { children: ["(", c, " to ", f, ")"] })), (h[7] = f), (h[8] = c), (h[9] = g);
    else g = h[9];
    return g;
  }
  let g;
  if (h[10] !== f || h[11] !== c) (g = r(ca, { children: [c, " to ", f] })), (h[10] = f), (h[11] = c), (h[12] = g);
  else g = h[12];
  return g;
}
function O() {
  return Yz() === "macos" ? "macos" : "other";
}
function K(t, o, s) {
  let n = (typeof t === "string" ? [t] : t).filter((a) => a !== "");
  return zZ(n.map(qE), { ...o, platform: o?.platform ?? s });
}
export { zZ, M };
