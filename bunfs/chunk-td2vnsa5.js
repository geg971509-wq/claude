...
import { Gm, O_ } from "/$bunfs/root/chunk-q14dgq5g.js";
import { Vm, FC } from "/$bunfs/root/chunk-s4vc7est.js";
import { te } from "/$bunfs/root/chunk-wag5ye9w.js";
import { constants as F, realpathSync as A } from "fs";
import {
  access as j,
  lstat as D,
  mkdir as v,
...
import { basename as N, dirname as L, isAbsolute as _, join as x, resolve as S } from "path";
function MJe(t = {}) {
  let a = new Map(
      mkn.map((i) => {
  ...
  async function c(i, e, o, r, s = "path") {
    let f = eQe(i),
      w = t.refuse?.() ?? SVt(f, s) ?? o;
    if (w !== void 0) return { error: w };
    let { space: g, path: p } = f;
    if (await h(p, e, g)) return { error: gkn(s) };
    if (r !== void 0 && (await h(r.path, r.reach ?? e, g))) return { error: gkn(r.argument) };
    switch (d(g)) {
      case "refused":
        return { error: bj() };
      case "absent":
  ...
  return {
    ...
    async write(i, e, o) {
      let r = q(o),
        ...
        w = f !== void 0 && f !== null ? { path: f.path, argument: "opts.stagingFolder", reach: "staging" } : void 0,
        g = await c(s, "content", Olr(s, e, r), w);
      if (g.error !== void 0) return P(g.error);
      if (g.absent) return P(cCt());
    ...
    async ensureFolder(i, e) {
      let o = e === void 0 || e === null || typeof e !== "object" ? e : { mode: e.mode, recursive: e.recursive },
        r = await c(i, "folder", Llr(o));
    ...
    async rename(i, e) {
      let o = eQe(e),
        r = eQe(i),
        s = await c(r, "content", Flr(r, o), { path: o?.path ?? "", argument: "to" }, "from");
      if (s.error !== void 0) return P(s.error);
      if (s.absent) return P(cCt());
      try {
        return await K(s.path, o.path), re(void 0);
      } catch (f) {
        return P(y(f));
      }
    },
...
function q(t) {
  if (t === void 0 || t === null || typeof t !== "object") return t;
  let a = t,
    d = a.stagingFolder,
    u = {
      publish: a.publish,
      ...
      stagingFolder: d === void 0 ? void 0 : eQe(d),
    };
...
async function z(t, a) {
  ...
  let u = te([S(d), await Zzt(d)]),
    m = (h) => {
      let c = S(h);
      return u.some((i) => i === c || bVt(i, c));
    },
    b = m(t.roots.configHome);
  if (!b) {
    let h = await a;
...
async function Zzt(t) {
  let a = S(t),
    d = [],
    u = a;
  for (;;)
    try {
      return x(await R(u), ...d);
    } catch {
      let m = L(u);
      if (m === u) return a;
      d.unshift(N(u)), (u = m);
...
function ORn(t) {
  return { roots: t, resolved: G(t) };
}
async function G(t) {
  let [a, d, u] = await Promise.all([
    Zzt(t.configHome),
    Zzt(L(t.globalConfigFile)),
    Promise.all((t.heldRoots ?? []).map(Zzt)),
  ]);
  return { configHome: a, globalConfigFile: x(d, N(t.globalConfigFile)), heldRoots: u };
}
function y(t) {
  let a = E(t) ?? (t instanceof Gm ? "ELOOP" : void 0);
...
function kwr(t) {
  ...
  return {
    ...
    ...{ uid: Number(t.uid), gid: Number(t.gid) },
    ...(Vm(t.ino) && { objectId: t.dev.toString() + ":" + t.ino.toString() }),
  };
}
function X(t) {
  if (t.mtimeNs === 0n && t.mtimeMs !== 0n) return Number(t.mtimeMs);
  return FC(t.mtimeNs);
}
export { MJe, Zzt, ORn, kwr };
