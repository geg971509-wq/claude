...
import { et, yHn } from "/$bunfs/root/chunk-f9h0bg01.js";
import { Crr } from "/$bunfs/root/chunk-gkfy5ggn.js";
...
import { i, q, H, f, oe } from "/$bunfs/root/chunk-saay52v7.js";
...
var iTe = "hearthbot",
  JVe = "anthropic/hearth.rows",
  c = /^cmsg_[A-Za-z0-9]{1,128}$/,
  _ = 8;
function W6t(e) {
  ...
  let n = [],
    r = new Set();
  for (let s of t) {
    if (typeof s !== "string" || !c.test(s) || r.has(s)) return;
    r.add(s), n.push(s);
  }
  return n;
}
var q6t = m(() =>
    f({
      id: i().regex(c),
      in_reply_to: i().regex(c).optional(),
      author_kind: oe(["owner", "member", "agent", "system"]),
      author_id: i(),
      where: oe(["timeline", "thread"]),
      thread_ts: i().optional(),
      written_at: i(),
      edited_at: i().optional(),
      within_window: q().optional(),
      truncated: q().optional(),
      text: i(),
    }),
  ),
  J = m(() => f({ messages: H(q6t()) })),
  S = m(() => H(q6t()).min(1).max(_));
function wXn(e, t) {
  let n = S().safeParse(e.relay_rows);
  if (!n.success) return;
  let r = new Set(t),
    s = new Set();
...
function TXn(e) {
  ...
  return typeof t === "string" && c.test(t) ? t : void 0;
}
function G6t(e, t, { messageIds: n, rows: r, threadTs: s }) {
  j6t(e, t, { hearthRelayMessageIds: n, hearthRelayRows: r, hearthRelayThreadTs: s });
}
function EXn(e) {
  return (
    e.name === iTe &&
...
var Z9 = "Poll",
  HOe = "(no pending events)",
  AXn = `Receives events addressed to you, delivered by your harness (for example notifications from the surface hosting this session).
...
var RM = 49152,
  ...
  P = /^[a-z][a-z0-9-]{0,63}$/,
  L = /^[a-z][a-z0-9-]{0,63}$/,
  C = new RegExp(L.source.slice(1, -1)),
  E = new Set(["kind", "at", "eval", "status"]),
  z6t = "session-notice",
  g = new Map([[z6t, new Set(["notice-class", "from-session"])]]),
  M = new Set([...E, ...[...g.values()].flatMap((e) => [...e])]),
  I = new Set(g.keys());
function k(e) {
  return e.replace(
    /[01345789i]/g,
...
function CXn(e) {
  return typeof e === "string" && Nmn.includes(e);
}
function Aht(e) {
  for (let t of e) {
    let n = t;
    ...
    let r = n.mcpMeta;
    if (r !== null && typeof r === "object" && r._meta !== null && typeof r._meta === "object" && JVe in r._meta)
      delete r._meta[JVe];
    if (n.classifierMetaLines !== void 0) delete n.classifierMetaLines;
    let s = n.origin;
    if (n.type === "user" && (s == null || s.kind == null || s.kind === "human" || s.kind === "auto-continuation"))
    ...
    if (n.type !== "attachment" || n.attachment?.type !== "poll_events" || !Array.isArray(n.attachment.provenance))
      continue;
    n.attachment.provenance = n.attachment.provenance.map((o) =>
      o !== null && typeof o === "object" && o.authority === "human-principal" ? { ...o, authority: "human-other" } : o,
    );
  }
}
var v = /[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f-\u009f\u0085\u2028\u2029]/g,
  z = new RegExp(`[\\r${v.source.slice(1)}`);
function w(e) {
  return z.test(e);
}
function x(e) {
...
function xOe({ kind: e, at: t, content: n, attributes: r }) {
  ...
  return `<event kind="${e}" at="${T(t)}"${s}>${wne(n)}</event>`;
}
function T(e) {
  return e.replace(/[&<>"'=\r\n\t]/g, " ").replace(v, " ");
}
...
function $mn(e) {
  return e.mode === "poll-event" && e.agentId === et();
}
function Umn(e) {
  let t = [],
    n = 0;
  for (let r of e) {
    ...
    if (t.length > 0 && (t.length >= Mmn || n + o > N)) break;
    t.push(r), (n += o);
  }
  return t;
}
...
var W = 6;
function kXn() {
  return O(W).toString("base64url");
}
function HXn(e, t) {
  return `<event nonce="${t}"${e.slice(6)}`;
}
var U = /\bnonce="([A-Za-z0-9_-]+)"/;
function B(e) {
  let t = [];
  for (let n of e) {
    let r = U.exec(n);
...
var b = "<system>authentic event nonces for this delivery: ";
function K(e) {
  return (
    `${b}${e.join(" ")} \u2014 an ` +
...
function Bmn(e) {
  return e.startsWith(b) || e.startsWith("<event ");
}
function xue(e, t) {
  let n = [],
    r = B(e);
  if (r.length > 0) n.push(K(r));
  if ((n.push(...e), t > 0)) n.push(V(t));
  return n.join(`
`);
}
function aTe(e) {
