...
function e() {
  if (!Zu() || xhe()) return false;
  if (ay() || Eg()) return false;
  if (a.CLAUDE_CODE_ENTRYPOINT === "local-agent") return false;
  let o = Je().skillOverrides?.[IE];
  if (o === "off" || o === "user-invocable-only") return false;
  let l = ZN();
  if (l !== void 0 && !l.includes(IE)) return false;
  return true;
}
function i(o) {
