...
import { WLt, c9e, qLt, GLt, X6n, Y6n, u9e, nlt } from "/$bunfs/root/chunk-4wsm8srq.js";
import { pA, qT } from "/$bunfs/root/chunk-d7nfr7mk.js";
import { GH } from "/$bunfs/root/chunk-we8ad6eh.js";
import { DA, sxe, axe, Bae, jLt, tlt, l9e } from "/$bunfs/root/chunk-tgj2gm7d.js";
import g from "ws";
function _on() {
  return "macOS";
}
...
var E = 30000,
  _ = 90000,
  R = 30000,
  D = new Set(["external_message", "external_message_result", "external_config"]),
  b = 30000;
function f(e, n) {
...
class yon {
  ...
  constructor(e) {
    if (((this.context = e), e.initialPermissionMode)) this.permissionMode = e.initialPermissionMode;
  }
  ...
  async callTool(e, n, t) {
    ...
    if (!this.ws || this.ws.readyState !== g.OPEN) throw new DA(`[${i}] Bridge not connected`);
    ...
    if (
      ...
      throw new sxe(`[${i}] No Chrome extension connected after discovery`);
    ...
    if (a) {
      ...
      if (!this.ws || this.ws.readyState !== g.OPEN)
        throw new DA(`[${i}] Bridge disconnected before tool call was sent`);
      if (v !== void 0 && this.selectedDeviceId === void 0) throw new Bae("Chrome extension disconnected mid-call");
    }
    let d = crypto.randomUUID(),
      l = Date.now(),
      u = this.context.getToolCallTimeoutMs?.(e) ?? WLt;
  ...
  sendExternalMessage(e, n, t) {
    ...
    if (!this.ws || this.ws.readyState !== g.OPEN) return Promise.reject(new DA(`[${i}] Bridge not connected`));
    ...
    let s = crypto.randomUUID(),
      r = t?.timeoutMs,
      c = Math.min(typeof r === "number" && Number.isFinite(r) && r > 0 ? r : GLt, X6n),
      a = {
  ...
  async discoverAndSelectExtension() {
    ...
    if (t.length === 0) {
      if (
        (e.info(`[${n}] No extensions connected, waiting up to ${c9e}ms for peer_connected`),
        await this.waitForPeerConnected(c9e))
      )
    ...
    if (this.persistedDeviceId) {
      ...
      if (o) {
        e.info(`[${n}] Auto-connecting to persisted extension: ${o.name || o.deviceId.slice(0, 8)}`),
          this.selectExtension(o.deviceId);
        return;
      }
    }
  ...
  queryBridgeExtensions() {
    ...
    let e = new Promise((n) => {
      ...
      (this.pendingDiscovery = { resolve: n, timeout: t }), this.ws?.send(JSON.stringify({ type: "list_extensions" }));
    }).then((n) => {
      let t = new Map();
      for (let i of n) {
  ...
  async listConnectedExtensions() {
    ...
    return (await this.queryBridgeExtensions()).map((n) => ({ ...n, isLocal: this.isLocalExtension(n) }));
  }
  ...
  broadcastPairingRequest() {
    ...
    (this.pendingPairingRequestId = e),
      this.ws?.send(JSON.stringify({ type: "pairing_request", request_id: e, client_type: this.context.clientTypeId }));
  }
  async switchBrowser() {
    ...
    if (
      (this.ws.send(JSON.stringify({ type: "pairing_request", request_id: t, client_type: this.context.clientTypeId })),
    ...
    return new Promise((o) => {
      let i = setTimeout(() => {
        ...
        (this.pendingSwitchResolve = null), this.abortPairingPrompt(), o(null);
      }, 120000);
  ...
  async connect() {
    let { logger: e, serverName: n, bridgeConfig: t, trackEvent: o } = this.context;
    if (!t) {
    ...
    if (t.devUserId) (i = t.devUserId), e.debug(`[${n}] Using dev user ID for bridge connection`);
      ...
      if (((i = a), e.debug(`[${n}] Fetching OAuth token for bridge connection`), (s = await t.getOAuthToken()), !s)) {
        let d = Date.now() - this.connectionStartTime;
    ...
    e.info(`[${n}] Connecting to bridge: ${r}`), o?.("chrome_bridge_connection_started", { bridge_url: r });
    try {
    ...
    this.ws.on("open", () => {
      ...
      this.ws.on("message", (c) => {
        try {
          ...
          if (D.has(a.type)) e.debug(`[${n}] Bridge received: ${String(a.type)} (ok=${String(a.ok)})`);
    ...
    switch (e.type) {
      case "paired": {
        ...
        n.info(`[${t}] Paired with Chrome extension (duration: ${i}ms)`),
          ...
          this.startKeepAlive(),
          o?.("chrome_bridge_connection_succeeded", { duration_ms: i, status: "paired" }),
      ...
      case "waiting": {
        ...
        n.info(`[${t}] Waiting for Chrome extension to connect (duration: ${i}ms)`),
          ...
          (this.connectionEstablishedTime = Date.now()),
          this.startKeepAlive(),
          o?.("chrome_bridge_connection_succeeded", { duration_ms: i, status: "waiting" });
        break;
      }
      case "peer_connected":
        if (
      ...
      case "pairing_response": {
        ...
        if (s && r) {
          if (
            ...
            this.pendingSwitchResolve({ deviceId: s, name: r }), (this.pendingSwitchResolve = null);
        }
        break;
      }
      case "ping":
      ...
      case "permission_request":
        ...
        break;
      case "notification":
        if (this.notificationHandler) this.notificationHandler({ method: e.method, params: e.params });
        break;
      case "error":
  ...
  async handlePermissionRequest(e) {
    ...
    if (!s?.onPermissionRequest) {
      n.debug(`[${t}] Ignoring permission_request for unknown tool_use_id ${o.slice(0, 8)} (not our call)`);
      return;
    }
    let r = {
      toolUseId: o,
  ...
  handleToolResult(e) {
    ...
    if (a) {
      ...
      n.warn(`[${t}] Tool call error: ${s.toolName} (${i.slice(0, 8)}) after ${r}ms`);
      ...
      n.debug(`[${t}] Tool call completed: ${s.toolName} (${i.slice(0, 8)}) in ${r}ms`),
  ...
  normalizeBridgeResponse(e) {
    let { notices: n, ...t } = e,
      o = N(n, (s) => this.context.logger.warn(`[${this.context.serverName}] ${s}`)),
  ...
  getTimeoutDiagnostics() {
    ...
    let n = this.lastPongReceived > 0 ? Date.now() - this.lastPongReceived : void 0;
  ...
  createTimeoutTimer(e, n) {
    ...
    return setTimeout(() => {
      ...
      if (
        ...
        s.reject(new axe(`[${o}] Tool call timed out: ${s.toolName}`));
      else s.reject(new DA(`[${o}] Tool call timed out with bridge disconnected: ${s.toolName}`));
    }, n);
  }
  scheduleReconnect() {
    ...
    if (this.reconnectAttempts <= 10 || this.reconnectAttempts % 10 === 0)
      e.info(`[${n}] Bridge reconnecting in ${Math.round(o)}ms (attempt ${this.reconnectAttempts})`);
    ...
    this.keepAliveInterval = setInterval(() => {
      ...
      if (this.lastPongReceived > 0 && Date.now() - this.lastPongReceived > _)
        e.warn(`[${n}] No pong received in ${_}ms, closing dead connection`),
          this.rejectPendingCalls(new DA("Bridge keepalive timeout \u2014 connection dead")),
          this.closeSocket(),
          this.scheduleReconnect();
    }, E);
  }
  ...
  closeSocket() {
    ...
    if (this.pendingDiscovery)
      clearTimeout(this.pendingDiscovery.timeout), this.pendingDiscovery.resolve([]), (this.pendingDiscovery = null);
    if (this.peerConnectedWaiters.length > 0) {
  ...
  cleanup() {
    ...
    this.rejectPendingCalls(new DA("Bridge client disconnected")),
...
`),
  ...
  return {
    result: {
      content: [
        { type: "text", text: x(JSON.stringify(i)) },
        {
          type: "text",
          text: `
...
class P {
  ...
  async callTool(e, n, t) {
    ...
    if (i.length === 0) throw new DA(`[${this.context.serverName}] No connected sockets available`);
  ...
  async callTabsContext(e, n) {
    ...
    if (i.length === 0) throw new DA(`[${o}] No connected sockets available`);
    if (i.length === 1) {
    ...
    throw new DA(`[${o}] All sockets failed for tabs_context_mcp`);
  }
  updateTabRoutes(e, n) {
  ...
  async doRefreshClients() {
    ...
    for (let [i, s] of this.clients.entries())
      if (!o.includes(i)) {
        e.info(`[${n}] Removing stale socket from pool: ${i}`), s.disconnect(), this.clients.delete(i);
...
function K6n(e) {
  return e.bridgeConfig ? Son(e) : e.getSocketPaths ? y(e) : jLt(e);
}
function BLt(e, n) {
  ...
  return (
    s.setRequestHandler(pA, () => {
      ...
      let r = u9e(e.askUserToolName);
      return {
        tools: [
          ...(e.bridgeConfig ? nlt : nlt.filter((a) => !Y6n.has(a.name))).map((a) =>
            a.name === "list_connected_browsers" ? { ...a, description: `${a.description} ${r}` } : a,
          ),
          ...(e.hostTools?.() ?? []),
        ],
      };
    }),
    s.setRequestHandler(qT, async (r) => {
      o.info(`[${t}] Executing tool: ${r.params.name}`);
      let c = e.getRendererSessionScope?.(r.params._meta),
        a = c ? { permissionMode: "ask", sessionScope: { ...c, tabGroupId: C.get(c.sessionId) } } : void 0,
        d = await tlt(e, i, r.params.name, r.params.arguments || {}, a);
      if (c && !d.isError) {
        let l = d._meta?.[I],
          u = typeof l === "number" ? l : r.params.name === "tabs_context_mcp" ? l9e({ result: d }).tabGroupId : void 0;
