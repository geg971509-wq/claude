...
import { Va } from "/$bunfs/root/chunk-fec4384a.js";
import { $e } from "/$bunfs/root/chunk-2masxyqj.js";
import { rae } from "/$bunfs/root/chunk-sypj25ha.js";
import { Je } from "/$bunfs/root/chunk-988p40e0.js";
import { eB } from "/$bunfs/root/chunk-0zt38pp0.js";
...
var BKt = "com.anthropic.claude-code-url-handler",
  m = "Claude Code URL Handler",
  w = "claude-code-url-handler.desktop",
  D = "Claude Code URL Handler.app",
  c = o.join(g.homedir(), "Applications", D),
  l = o.join(c, "Contents", "MacOS", "claude");
function d() {
  return o.join(xde(), "applications", w);
}
var u = `HKEY_CURRENT_USER\\Software\\Classes\\${eB}`,
...
async function ZPn(e) {
  ...
  if (O() && e !== void 0) {
    let i = await e.stat(Te.state("deep-link-register-failed"));
    if (i.ok && Date.now() - i.value.mtimeMs < f) return;
  } else
    try {
      let i = await a.stat(r);
      if (Date.now() - i.mtimeMs < f) return;
    } catch {}
  try {
    if (
      (await S(t),
    ...
    let s = uo(i);
    if (
      (p("deep_link_register", s ?? "register_failed"),
      n(`Failed to auto-register deep link protocol handler: ${i instanceof Error ? i.message : String(i)}`, {
        level: "warn",
      }),
      s === "EACCES" || s === "ENOSPC")
    )
      if (O() && e !== void 0)
        await e.write(Te.state("deep-link-register-failed"), "", { publishDiscipline: "inPlace" });
