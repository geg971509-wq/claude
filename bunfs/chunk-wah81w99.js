...
import { Ho, bsn, hE } from "/$bunfs/root/chunk-zze8764r.js";
function o(e) {
  if (e.kind !== "resolved" || !Ho()) return;
  let t = hE(),
    i = t.lastResolvedAccountToken;
  if (((t.lastResolvedAccountToken = e.token), i === void 0 || i === e.token)) return;
  (qt().identityEpoch += 1), NWe(), d();
}
function d() {
  let e = qt();
  ...
  queueMicrotask(() => {
    ...
    try {
      ...
      n(`MCP identity-change handler failed: ${l(i)}`, { level: "warn" });
    }
  });
}
function yat(e) {
...
function lr() {
  return r(), qt().identityEpoch;
}
function MWe() {
  let e = qt();
  if (!Ho()) return e.identityEpoch;
  return (e.identityEpoch += 1), e.identityEpoch;
}
var c = "baseline:rearmed";
function O6n() {
  if (!Ho()) return;
  MWe(), (hE().lastResolvedAccountToken = c);
}
function lE(e) {
  return r(), e === qt().identityEpoch;
}
function NWe() {
  let e = qt();
