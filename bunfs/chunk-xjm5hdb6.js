  ...
  async handleGetRequest(e) {
    if (!e.headers.get("accept")?.includes("text/event-stream"))
      return (
        ...
        this.createJsonErrorResponse(406, -32000, "Not Acceptable: Client must accept text/event-stream")
      );
    ...
    if (this._streamMapping.get(this._standaloneSseStreamId) !== void 0)
      return (
        ...
        this.createJsonErrorResponse(409, -32000, "Conflict: Only one SSE stream is allowed per session")
      );
      ...
      if (this._eventStore.getStreamIdForEventId) {
        ...
        if (this._streamMapping.get(t) !== void 0)
          return (
            ...
            this.createJsonErrorResponse(409, -32000, "Conflict: Stream already has an active connection")
          );
      }
      let s = {
...
`),
  ...
  async handlePostRequest(e, t) {
    try {
      ...
      if (!s?.includes("application/json") || !s.includes("text/event-stream"))
        return (
          this.onerror?.(Error("Not Acceptable: Client must accept both application/json and text/event-stream")),
      ...
      if (!i || !i.includes("application/json"))
        return (
          ...
          this.createJsonErrorResponse(415, -32000, "Unsupported Media Type: Content-Type must be application/json")
        );
      ...
      let o;
      try {
        if (Array.isArray(n)) o = n.map((d) => hR.parse(d));
        else o = [hR.parse(n)];
      } catch {
        return (
      ...
      let c = crypto.randomUUID(),
        u = o.find((d) => jGt(d)),
        f = u ? u.params.protocolVersion : (e.headers.get("mcp-protocol-version") ?? PCn);
      if (this._enableJsonResponse)
        return new Promise((d) => {
          ...
          for (let h of o) if (IO(h)) this._requestToStreamMapping.set(h.id, c);
      ...
      let S = new TextEncoder(),
        ...
        m = { "Content-Type": "text/event-stream", "Cache-Control": "no-cache", Connection: "keep-alive" };
      ...
      for (let d of o) {
        let h, E;
        if (IO(d) && this._eventStore && f >= "2025-11-25")
          (h = () => {
        ...
        this.onmessage?.(d, { authInfo: t?.authInfo, requestInfo: r, closeSSEStream: h, closeStandaloneSSEStream: E });
      }
      return new Response(v, { status: 200, headers: m });
    } catch (s) {
      return this.onerror?.(s), this.createJsonErrorResponse(400, -32700, "Parse error", { data: String(s) });
    }
  }
  ...
  validateSession(e) {
    ...
    if (t !== this.sessionId)
      return this.onerror?.(Error("Session not found")), this.createJsonErrorResponse(404, -32001, "Session not found");
    return;
  }
  validateProtocolVersion(e) {
    let t = e.headers.get("mcp-protocol-version");
    if (t !== null && !o4.includes(t))
      return (
        this.onerror?.(Error(`Bad Request: Unsupported protocol version: ${t} (supported versions: ${o4.join(", ")})`)),
        this.createJsonErrorResponse(
          400,
          -32000,
          `Bad Request: Unsupported protocol version: ${t} (supported versions: ${o4.join(", ")})`,
        )
      );
    return;
  }
    ...
    if (Q$(e) || Qpe(e)) {
      ...
      if (n.every((a) => this._requestResponseMap.has(a))) {
        ...
        if (this._enableJsonResponse && r.resolveJson) {
          ...
          if (l.length === 1) r.resolveJson(new Response(JSON.stringify(l[0]), { status: 200, headers: a }));
          else r.resolveJson(new Response(JSON.stringify(l), { status: 200, headers: a }));
        } else r.cleanup();
        for (let a of n) this._requestResponseMap.delete(a), this._requestToStreamMapping.delete(a);
      }
    }
  }
}
