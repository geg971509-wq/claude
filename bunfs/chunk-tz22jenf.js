...
import { ibt } from "/$bunfs/root/chunk-8tgj5dp2.js";
...
import { MVe, Iz, Lm, Cue, L2, dmn } from "/$bunfs/root/chunk-y8sx7bf9.js";
...
import { OZe } from "/$bunfs/root/chunk-cx6t1c5p.js";
...
import { mkdir as z, mkdtemp as rt, open as nt, realpath as Z, rm as H, writeFile as ot } from "fs/promises";
...
function K(e, t) {
  ...
  if (Array.isArray(e)) {
    for (let a of e) K(a, t);
    return;
  }
  if (He(e)) for (let a of Object.values(e)) K(a, t);
}
function U(e, t) {
  if (typeof e === "string") {
  ...
  if (Array.isArray(e)) return e.map((a) => U(a, t));
  if (He(e)) return ts(e, (a) => U(a, t));
  return e;
}
function V(e) {
  let t = it(e);
...
async function Mt(e, t) {
  ...
  else p("ccr_mcp_call_staged", h ?? a.error_code);
...
function dt(e) {
  ...
  if (Array.isArray(e))
    return e
      .map((t) => (t !== null && typeof t === "object" && "text" in t && typeof t.text === "string" ? t.text : ""))
      .filter(Boolean)
      .join(`
`);
  return "";
...
async function ft(e, t) {
  let a = ibt().safeParse(e);
  if (!a.success) return r("tool_error", `invalid request: ${a.error.message}`);
  let i = a.data;
  if (D() === "windows") return r("tool_error", "staged mcp_call is not supported on this platform");
  if (i.output_files !== void 0 && i.expires_at === void 0)
    return r("tool_error", "invalid request: expires_at is required when output_files are declared");
  if (i.expires_at !== void 0) {
    let n = OZe(i.expires_at);
    if (Number.isNaN(n))
      return r(
        "expired",
  ...
  for (let n of i.input_files ?? [])
    try {
      h.push({ spec: n, rel: Cue(Iz, n.lane_path) });
    } catch (s) {
  ...
  for (let n of i.output_files ?? []) {
    let s;
    try {
      s = Cue(Iz, n.lane_path);
    } catch (g) {
      return r("tool_error", `invalid output lane_path ${b(n.lane_path)}: ${l(g)}`);
    }
    let f = N.get(s);
    if (f !== void 0) return r("tool_error", `duplicate output lane_path: ${n.lane_path} collides with ${f}`);
  ...
  for (let [n, s] of [
    ...
    let f = new Set();
    for (let { name: g } of s) {
      if (f.has(g)) return r("tool_error", `duplicate ${n} name: ${g}`);
      f.add(g);
    }
  }
  let W = i.input_files !== void 0 || i.output_files !== void 0;
  if (W) {
    let n = new Set([...h.map(({ spec: u }) => `{{in:${u.name}}}`), ...d.map(({ spec: u }) => `{{out:${u.name}}}`)]),
      s = new Set();
    K(i.arguments ?? {}, s);
    let f = [...s].filter((u) => !n.has(u));
    if (f.length > 0) return r("tool_error", `invalid request: args token names no declared file: ${f.join(", ")}`);
  ...
  try {
    ...
    for (let [o, { spec: _, rel: k }] of h.entries()) {
      if (t.signal?.aborted) return r("tool_error", I);
      let c = await L2(k);
      if (c.kind === "not_found") return r("input_missing", `input not found: ${_.lane_path}`);
      if (c.kind === "error")
        return r("tool_error", `input fetch failed for ${_.lane_path}: ${c.message}`, c.retryable);
      let A = x(O, "in", `${o}${V(_.lane_path)}`);
      try {
        await ot(A, c.buf, { mode: 384 });
      } catch (M) {
        return r("tool_error", `input stage failed: ${E(M) ?? "unknown"}`);
      }
      n.set(`{{in:${_.name}}}`, A), s.push({ lane_path: _.lane_path, etag: c.content_sha256 });
    }
    let f = d.map(({ spec: o, rel: _ }, k) => {
    ...
    try {
      ...
      if (F) return r("timeout", `tool did not complete within ${u}ms (${l(o)})`);
      if (t.signal?.aborted) return r("tool_error", `${I} (${l(o)})`);
      return r("tool_error", `tool call failed: ${l(o)}`);
    } finally {
      clearTimeout(tt), t.signal?.removeEventListener("abort", q);
    }
    if (R.isError) {
      if (F) return r("timeout", `tool call exceeded ${u}ms`);
      if (t.signal?.aborted) return r("tool_error", I);
      let o = dt(R.content);
    ...
    let et = await Z(O),
      j = [];
    for (let { spec: o, rel: _, path: k } of f) {
      if (t.signal?.aborted) return r("tool_error", I);
      let c;
      try {
        c = await Z(k);
      } catch (m) {
        if (X(m)) return r("tool_error", `tool did not produce output: ${o.name}`);
        return r("tool_error", `output read failed for ${o.name}: ${E(m) ?? "unknown"}`);
      }
      if (!c.startsWith(et + "/")) return r("tool_error", `output path escapes the staging dir: ${o.name}`);
      let A = Lm,
        M;
      try {
        let m = await nt(c, B.O_RDONLY | B.O_NOFOLLOW | B.O_NONBLOCK);
        try {
          let T = await m.stat();
          ...
          if (T.size > A) return r("output_too_large", `output ${o.name} is ${T.size} bytes (cap ${A})`);
          let P = Number(T.size),
            G = Buffer.allocUnsafe(P + 1),
            S = 0;
          for (;;) {
            let { bytesRead: v } = await m.read(G, S, P + 1 - S, S);
            if (v === 0) break;
            S += v;
          }
          if (S !== P) return r("tool_error", `output changed during collection: ${o.name}`);
          M = G.subarray(0, S);
        } finally {
          await m.close();
        }
      ...
      let C = await dmn(t.syncDir ?? MVe, _, M, o.if_match);
      if (!C.ok)
        return r(
          C.reason === "too_large" ? "output_too_large" : C.reason === "conflict" ? "output_conflict" : "tool_error",
