...
var e = String.raw`(?:No tab with id: \d+|Tab \d+ (?:no longer exists|is not in the same group))`,
  ...
  s = [
    { pattern: /^Permission denied for reading page content/, errorType: "get_page_text_permission_denied" },
    {
    ...
    { pattern: /^Failed to extract page text: No main text content found/, errorType: "get_page_text_no_content" },
    {
      pattern: /^Failed to extract page text: (Script execution failed|Page script returned empty result)/,
  ...
  _ = [
    { pattern: /^Permission denied by user\. Domain transition denied/, errorType: "navigation_blocked" },
    ...
    { pattern: /^This site is not allowed due to safety restrictions/, errorType: "domain_blocked" },
    {
    ...
    {
      pattern: /^Security check failed: Domain changed|^Unable to verify current URL for security check/,
