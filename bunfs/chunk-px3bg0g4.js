...
import { Jte } from "/$bunfs/root/chunk-da857b4g.js";
...
import { eD, $x } from "/$bunfs/root/chunk-debmm3sq.js";
import { EWe, Hst, xst, FDt, $Dt, UDt } from "/$bunfs/root/chunk-k7wn4k3f.js";
import { __, uC, Yue } from "/$bunfs/root/chunk-gmj1vprd.js";
import { Xse, xen } from "/$bunfs/root/chunk-swz9hbyx.js";
import { NDe, qKn } from "/$bunfs/root/chunk-qy75zxb2.js";
import { Pue, KXn, YXn } from "/$bunfs/root/chunk-5xr818a0.js";
...
import { mkdir as T, realpath as N, writeFile as M } from "fs/promises";
...
async function D(e, s, l, m, u) {
  let i = O() && u !== void 0 ? await uC(u) : __();
  if (!i) return a("skip: no oauth token"), { failure: "download" };
  let r;
  try {
    let o = `${Yue()}/api/oauth/files/${encodeURIComponent(e.file_uuid)}/content`;
    if (typeof e.file_size === "number" && e.file_size > eD) return { failure: "download" };
    let b = await st.get(o, {
  ...
  let f =
    ...
    h = (f ? C().slice(0, 8) : e.file_uuid.slice(0, 8) || C().slice(0, 8)).replace(/[^a-zA-Z0-9_-]/g, "_"),
    k = Pue(),
    _ = KXn(h, d),
    c = L(k, _),
    B = K();
  if (O() && m !== void 0 && Zt(B) && Zt(_)) {
    let o = await m.write(Te.userConfigDir("uploads", [B, _]), r, { mode: 384 });
...
async function $cr(e, s, l, m, u) {
  ...
  if (!(O() && u !== void 0 ? await uC(u) : __())) {
    if ((a("skip: no oauth token"), g("bridge_attachment_resolve", "no_token"), i.length > 0)) {
      let t = i.length - $x,
        I = t > 0 ? i.slice(0, $x) : i;
      $Dt("bridge", I.length, 0);
      let w = t > 0 ? " " + xst(t) : "";
      return {
        prefix: I.map((R) => Hst(R.file_name, "it could not be downloaded (not signed in)")).join(" ") + w + " ",
        imageBlocks: [],
        inlinedImagePaths: [],
      };
    }
  ...
  let f = [],
    ...
    h = d.length > $x;
  if (h) {
    if ((a(`dropping ${d.length - $x} attachment(s) over the ${$x} cap`), i.length > 0)) f.push(xst(d.length - $x));
    d = d.slice(0, $x);
  }
  let k = s && l,
    _ = No(4, (t) => D(t, s, k, m, u)),
  ...
  if (E === 0) p("bridge_attachment_resolve", x ? "digest_mismatch" : "all_failed");
...
function Ucr(e, s) {
  if (!s) return e;
  if (typeof e === "string") return UDt(e, s);
  let l = e.findLastIndex((m) => m.type === "text");
  if (l !== -1) {
    let m = e[l];
    if (m.type === "text") return [...e.slice(0, l), { ...m, text: UDt(m.text, s) }, ...e.slice(l + 1)];
  }
  return [...e, { type: "text", text: s.trimEnd() }];
}
function Bcr(e, s) {
  if (s.length === 0) return e;
  let l = typeof e === "string" ? (e.trim() === "" ? [] : [{ type: "text", text: e }]) : xen(e);
  return [...s, ...l];
}
async function gRt(e, s, l, m, u) {
  let i = s ?? "",
    r = Xse(e);
  if (r.length === 0) return { content: i, inlinedImagePaths: [] };
  let f = l && !(typeof i === "string" && Jte(i)),
    { prefix: d, imageBlocks: h, inlinedImagePaths: k } = await $cr(r, l, f, m, u),
  ...
  return { content: Bcr(Ucr(i, d), h), inlinedImagePaths: k };
}
export { $cr, Ucr, Bcr, gRt };
