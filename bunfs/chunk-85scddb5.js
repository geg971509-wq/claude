...
import { kRe, l7t, Akt, jUe, WUe, qUe } from "/$bunfs/root/chunk-cfn4ecna.js";
...
import { Ift } from "/$bunfs/root/chunk-3qvtfvqg.js";
...
function we(a, s) {
  ...
  if (u) {
    let l = s?.terminalSize?.columns ?? 80,
      p = Math.min(80, Math.max(40, l - 10));
    return e(Pe, { children: e(l7t, { collected: i, verbose: u, width: p }) });
  }
  return e(Pe, { children: e(E, { collected: i }) });
}
function E(xe) {
  let L = _(23),
    { collected: f } = xe,
    q;
  if (L[0] !== f.agents) (q = qUe(f.agents)), (L[0] = f.agents), (L[1] = q);
  else q = L[1];
  let { done: P, failedCount: Me, running: j, total: N, complete: g } = q,
    z = Me > 0 ? "failed" : g ? "done" : "running",
    w,
    T,
    R,
    x,
    ...
    C,
    h;
  if (L[2] !== f || L[3] !== g || L[4] !== P || L[5] !== z || L[6] !== j || L[7] !== N) {
    let A = jUe(f);
    w = Akt;
    T = P;
    R = N;
    ...
    (L[2] = f),
      ...
      (L[13] = C),
      (L[14] = h);
  } else (w = L[8]), (T = L[9]), (R = L[10]), (x = L[11]), (M = L[12]), (C = L[13]), (h = L[14]);
  let G;
  if (L[15] !== w || L[16] !== T || L[17] !== R || L[18] !== x || L[19] !== M || L[20] !== C || L[21] !== h)
    (G = e(w, { done: T, total: R, running: x, complete: M, dotState: C, phaseText: h })),
      (L[15] = w),
      (L[16] = T),
      (L[17] = R),
      (L[18] = x),
...
function K(Ce) {
  let c = _(21),
    { taskId: F } = Ce,
    J;
  if (c[0] !== F) (J = (he) => he.tasks[F]), (c[0] = F), (c[1] = J);
  else J = c[1];
  let n = ki(J);
  if (n?.type === "local_workflow" && (n.status === "completed" || n.status === "failed" || n.status === "killed")) {
    let m;
    if (c[2] !== n.endTime || c[3] !== n.startTime)
      (m = n.endTime && n.startTime ? $t(n.endTime - n.startTime) : void 0),
        (c[2] = n.endTime),
        (c[3] = n.startTime),
        (c[4] = m);
    else m = c[4];
    ...
    let S;
    if (c[7] !== n.agentCount)
      (S = n.agentCount > 0 && ` \xB7 ${n.agentCount} ${k(n.agentCount, "agent")}`), (c[7] = n.agentCount), (c[8] = S);
    else S = c[8];
    let b;
    if (c[9] !== n.totalTokens)
      (b = n.totalTokens > 0 && ` \xB7 ${Gn(n.totalTokens)} tokens`), (c[9] = n.totalTokens), (c[10] = b);
    else b = c[10];
    let O;
    ...
    let Z;
    if (c[16] !== y || c[17] !== O)
      (Z = e(Pe, { children: r(t, { children: [y, O] }) })), (c[16] = y), (c[17] = O), (c[18] = Z);
    else Z = c[18];
    return Z;
  }
...
function W() {
  let H = _(2),
    ee;
  if (H[0] === d) (ee = Ift(ie().workflowSizeGuideline)), (H[0] = ee);
  else ee = H[0];
  let { size: oe, isDefault: ye } = ee;
  if (!ye || oe === "unrestricted") {
    return null;
  }
  let te;
  if (H[1] === d)
