...
import { Ze, ft, l } from "/$bunfs/root/chunk-ypdw393e.js";
...
import { JS, bt } from "/$bunfs/root/chunk-8tgj5dp2.js";
...
import { HA, Vw, Yst, NHe, FHe, Jst, g6n, Qst } from "/$bunfs/root/chunk-7vpf91fq.js";
import { gje, aZ, _W, U0e, hje, Pen, lZ } from "/$bunfs/root/chunk-ptdw8z49.js";
import { B0e, Oen, TUn } from "/$bunfs/root/chunk-k70ej0cs.js";
import "/$bunfs/root/chunk-czh9aynm.js";
import "/$bunfs/root/chunk-c59q3nnv.js";
import "/$bunfs/root/chunk-31xy83wr.js";
import { Fze, Vpn, Sz } from "/$bunfs/root/chunk-r6t6p0zm.js";
...
import { i, v, q, H, f, ot, ps, oe, N } from "/$bunfs/root/chunk-saay52v7.js";
...
import { open as de, realpath as O, stat as pe } from "fs/promises";
import { extname as me, sep as X, resolve as E } from "path";
...
async function I(e, t, r, s) {
  ...
  if (o.status === 401 || o.status === 403) throw new T(e, o.status, o.data);
  if (o.status < 200 || o.status >= 300) throw new C(e, o.status, o.data);
  return o.data;
}
var A = "PROJECT_TYPE_DESIGN_SYSTEM";
async function x(e, t = {}, r) {
  let s = await I("ListOrgProjects", e, { ...(t.type && { type: t.type }), ...(t.cursor && { cursor: t.cursor }) }, r);
...
async function L(e, t, r) {
  let s = [],
    o = 0;
  for (let c = 0; c < 50; c++) {
    let a = await I("ListFiles", e, { projectId: t, depth: -1, ...(o > 0 && { offset: o }) }, r),
      p = a.entries ?? [];
    for (let g of p) s.push(g.path);
    if (!a.truncated || p.length === 0) return s;
    o += p.length;
  }
...
async function U(e, t, r, s) {
  ...
  return (await I("DeleteFiles", e, { projectId: t, paths: r }, s)).deleted ?? 0;
}
async function W(e, t, r) {
  let s = await I("CreateProject", e, { name: t, type: A }, r);
  if (!s.projectId) throw new C("CreateProject", 200, s);
  return { projectId: s.projectId, name: t };
}
...
async function G(e, t, r, s) {
  await I("DeleteAsset", e, { projectId: t, path: r }, s);
}
function J(e, t) {
  if (!t) return e;
...
class C extends Error {
  ...
  telemetryMessage;
  constructor(e, t, r) {
    super(`Design API ${e} failed: HTTP ${t} ${ce(r)}`);
    this.method = e;
    this.status = t;
    this.body = r;
...
var ge = m(() =>
    ot({
      path: i().min(1).max(FHe).describe("Path within the project, e.g. components/button/index.html"),
      localPath: i()
        .min(1)
        .optional()
        .describe(
      ...
      encoding: oe(["base64"]).optional().describe('Set to "base64" for binary inline data'),
      mimeType: i().optional(),
    }),
  ),
  ue = m(() =>
    ot({
      name: i().min(1).max(255).describe('Short human-readable label ("Primary buttons"), not a path'),
      path: i().min(1).max(FHe).describe("Project-relative path to the preview/spec file this card renders"),
      subtitle: i().max(255).optional().describe('Variants shown ("Primary / secondary / ghost, 3 sizes")'),
      viewport: ot({ width: v().int().positive(), height: v().int().positive().optional() })
        .optional()
  ...
  fe = m(() =>
    ot({
      ...
      projectId: i().min(1).optional().describe("Required for all methods except list_projects and create_project"),
      path: i().min(1).optional().describe("get_file: file path to read"),
      writes: H(i().min(1).max(FHe))
        .max(256)
        .optional()
        .describe(
      ...
      name: i().min(1).max(200).optional().describe("create_project: name for the new design-system project"),
      assets: H(ue())
        .max(256)
        .optional()
        .describe(
      ...
      counts: f({
        total: v().int().nonnegative(),
        bad: v().int().nonnegative(),
        thin: v().int().nonnegative(),
        variantsIdentical: v().int().nonnegative(),
        iterations: v().int().nonnegative(),
      })
        .optional()
        .describe(
...
function ye(e) {
  let t = we[e.method],
    r = t.present.filter((o) => e[o] === void 0),
    s = t.nonEmpty.filter((o) => {
      let c = e[o];
      return c === void 0 || (Array.isArray(c) && c.length === 0);
    });
  return [...r, ...s];
}
var j = { notice: i().optional() },
  _e = m(() =>
    ps("method", [
      ...
      f({ method: N("list_files"), ...j, paths: H(i()) }),
      f({ method: N("get_file"), ...j, path: i(), content: i(), contentType: i(), isBase64: q(), truncated: q() }),
      f({ method: N("finalize_plan"), ...j, planId: i(), writes: H(i()), deletes: H(i()) }),
      f({ method: N("write_files"), ...j, written: v() }),
      f({ method: N("delete_files"), ...j, deleted: v() }),
      f({ method: N("register_assets"), ...j, registered: v() }),
      f({ method: N("unregister_assets"), ...j, unregistered: v() }),
      f({ method: N("create_project"), ...j, projectId: i(), name: i() }),
      f({ method: N("report_validate"), ...j }),
    ]),
  );
function be(e) {
  return (
    e === "list_projects" || e === "get_project" || e === "list_files" || e === "get_file" || e === "report_validate"
  );
}
function K(e) {
  if (!e) return "?";
  ...
  switch (e?.method) {
    ...
    case "write_files": {
      let t = e.files?.length ?? 0,
        r = Q(e.files ?? [], (o) => o.localPath !== void 0),
        s = r > 0 && r < t ? ` (${r} from disk, ${t - r} inline)` : r === t && t > 0 ? " from disk" : "";
      return `Write ${t} ${k(t, "file")}${s}`;
    }
    case "delete_files":
      return `Delete ${e.paths?.length ?? 0} ${k(e.paths?.length ?? 0, "file")}`;
    case "register_assets":
      return `Register ${e.assets?.length ?? 0} ${k(e.assets?.length ?? 0, "asset card")}`;
    case "unregister_assets":
      return `Unregister ${e.paths?.length ?? 0} ${k(e.paths?.length ?? 0, "asset card")}`;
    case "create_project":
...
async function ze(e, t) {
  ...
  if (
    ...
    let o = await TUn(t?.signal);
    if (o.ok) return o.accessToken;
    throw new S(`DesignSync needs design-system authorization. ${o.message}`);
  }
  if (!s.ok) {
    let o = s.detail ? ` (${s.detail})` : "";
    throw new S(`${re(s.reason, { isNonInteractiveSession: t?.isNonInteractiveSession })}${o}`);
  }
  return s.accessToken;
}
function Ie(e) {
  if (!(e instanceof T) || e.status !== 403) return null;
  let t = e.body;
  if (t == null || typeof t !== "object" || t.error !== "needs_consent" || !gje(t.consent)) return null;
  return t.consent;
}
function Z(e) {
  let t = he(e);
...
var Qe = kt({
  ...
  toAutoClassifierInput(e) {
    if (e.method === "finalize_plan") {
      ...
      return `project ${e.projectId ?? "?"} from ${E(ee(), e.localDir ?? ".")}: write ${t(e.writes)}; delete ${t(e.deletes)}`;
    }
    if (e.method === "create_project") return `create project "${e.name ?? "?"}"`;
    return e.method;
  },
  renderToolUseMessage(e) {
    if (e.method === "finalize_plan") return K(e.projectId);
    return R(e);
  },
  ...
  async checkPermissions(e, t) {
    let r = await hje(t.toolState.get(HA), t.credentials),
      s = { ...e, __consentBitShown: r, __consentAskCanReachUser: Z(t) },
      o = r !== null ? aZ(r) : null,
      c =
        (await Pen(t.credentials)) && B0e() && !Oen()
...
`) || null;
    if (a && e.method !== "finalize_plan" && e.method !== "create_project" && e.method !== "report_validate")
      return {
        behavior: "ask",
        message: a,
        updatedInput: s,
          ...
          reason:
            [
              o
                ...
                : null,
              c ? "design login \u2014 approving opens a browser OAuth consent and stores a design credential" : null,
            ]
              .filter(Boolean)
    ...
    if (e.method === "finalize_plan") {
      let p = (e.writes ?? []).map(Vw),
        g = (e.deletes ?? []).map(Vw),
        h;
      try {
        h = await se(e.localDir);
      } catch (z) {
        return {
          behavior: "deny",
      ...
      let d = p.filter(NHe),
        ...
        P = u.filter((z, ae) => !D[ae]),
        ie =
          u.length - P.length > 0 && P.length > 0
            ? `\u26A0 ${P.length} of ${u.length} literal write ${k(u.length, "path")} not found under localDir \u2014 ` +
      ...
      return {
        ...
        message: [
          a,
          `To project: ${K(e.projectId)}`,
          `From folder: ${h}`,
          u.length > 0 ? `Upload ${u.length} ${k(u.length, "file")}: ${u.join(", ")}` : null,
          d.length > 0 ? `Upload files matching: ${d.join(", ")}` : null,
          ie,
          y.length > 0 ? `Delete ${y.length} ${k(y.length, "file")}: ${y.join(", ")}` : null,
...
`),
  ...
  async call(e, t) {
    ...
    try {
      if (
        ((g = await ze(t.credentials, {
          signal: r,
          isNonInteractiveSession: t.options?.isNonInteractiveSession,
          permissionMode: c.mode,
          askReachesUserAtDecision: a,
        })),
        o !== null && p && e.method === "finalize_plan")
      )
        await lZ(s, o, t.credentials).catch((d) => {
          n(
            `Proactive design consent POST for finalize_plan failed (${l(d)}); the next RPC call's 403 intercept will retry.`,
          );
        });
      let h;
      try {
        ...
        let u = Ie(d);
        if (u === null) {
          if ((d instanceof T ? d.body : null)?.error === "insufficient_scope" || (d instanceof T && d.status === 401))
            throw new S(re("needs_design_login", { isNonInteractiveSession: t.options?.isNonInteractiveSession }));
          throw d;
        } else if (u !== o)
          throw (
            (_W(s, u, !1),
        ...
        else await lZ(s, u, t.credentials), (h = await ne(s, e, g, r));
      }
      return { data: h };
    } catch (h) {
      if (r.aborted) throw new Ze();
      let d = J(l(h), g),
        u = h?.telemetryMessage,
        _ = typeof u === "string" ? u : "DesignSync tool call failed";
      if (h instanceof S) throw ft(new S(d), _);
      throw ft(Error(d), _);
    }
  },
...
async function Se(e, t) {
  ...
  let u = me(a).slice(1).toLowerCase();
  return Pe.has(u)
    ? { path: r, data: d.toString("utf8"), mimeType: e.mimeType }
    : { path: r, data: d.toString("base64"), encoding: "base64", mimeType: e.mimeType };
}
async function ne(e, t, r, s) {
  switch (t.method) {
    case "list_projects": {
      let { items: o } = await x(r, { type: A }, s);
      return {
    ...
    case "get_project": {
      let o = w(t.projectId, "projectId", t.method),
        c = await M(r, o, s);
      return {
        method: "get_project",
        ...
        isOwned: c.isOwned,
        canEdit: c.callerCanEdit ?? c.canEdit,
      };
    }
    case "list_files": {
      let o = w(t.projectId, "projectId", t.method);
      return { method: "list_files", paths: await L(r, o, s) };
    }
    case "get_file": {
      let o = w(t.projectId, "projectId", t.method),
        c = w(t.path, "path", t.method),
        a = await B(r, o, c, void 0, s);
      return {
        method: "get_file",
        path: c,
        content: a.content,
    ...
    case "finalize_plan": {
      let o = w(t.projectId, "projectId", t.method),
        c = w(t.writes, "writes", t.method).map(Vw),
        a = w(t.deletes, "deletes", t.method).map(Vw),
        p = await se(t.localDir);
      return {
        method: "finalize_plan",
        planId: g6n(e, { projectId: o, writes: c, deletes: a, localDir: p }),
        writes: c,
        deletes: a,
      };
    }
    case "write_files": {
      let o = w(t.projectId, "projectId", t.method),
        c = w(t.planId, "planId", t.method),
        a = w(t.files, "files", t.method),
        p = Qst(e, c);
      if (!p || p.projectId !== o)
        throw Error("Plan token is missing or does not match this project. Call finalize_plan first.");
      let g = a.map((y) => y.path).filter(Yst);
      if (g.length > 0)
        throw Error(
          `Cannot write reserved paths: ${g.join(", ")}. CLAUDE.md and .claude/ carry instructions to the design agent and are blocked regardless of the plan.`,
        );
      let h = a.map((y) => Vw(y.path)).filter((y) => !Jst(y, p.writes));
      if (h.length > 0)
        throw Error(
          `Cannot write paths outside the finalized plan: ${h.join(", ")}. Re-run finalize_plan with the full set.`,
        );
      let d = 32,
        u = [];
      for (let y = 0; y < a.length; y += d) {
        if (s.aborted) throw new Ze();
        let D = a.slice(y, y + d);
        u.push(...(await Promise.all(D.map((P) => Se(P, p.localDir)))));
      }
      return { method: "write_files", written: (await F(r, o, u, {}, s)).length };
    }
    case "delete_files": {
      let o = w(t.projectId, "projectId", t.method),
        c = w(t.planId, "planId", t.method),
      ...
      if (g.length > 0)
        throw Error(
          `Cannot delete reserved paths: ${g.join(", ")}. CLAUDE.md and .claude/ carry instructions to the design agent and are blocked regardless of the plan.`,
        );
      let h = a.map(Vw).filter((u) => !Jst(u, p.deletes));
      if (h.length > 0)
        throw Error(
          `Cannot delete paths outside the finalized plan: ${h.join(", ")}. Re-run finalize_plan with the full set.`,
        );
      return { method: "delete_files", deleted: await U(r, o, a.map(Vw), s) };
    }
    case "register_assets": {
      let o = w(t.projectId, "projectId", t.method),
        c = w(t.planId, "planId", t.method),
      ...
      if (!p || p.projectId !== o)
        throw Error("Plan token is missing or does not match this project. Call finalize_plan first.");
      let g = a.map((d) => Vw(d.path)).filter((d) => !Jst(d, p.writes));
      if (g.length > 0)
        throw Error(
          `Cannot register paths outside the finalized plan: ${g.join(", ")}. Re-run finalize_plan with the full set.`,
        );
      let h = 0;
      for (let d of a) {
        if (s.aborted) throw new Ze();
        await Y(r, o, { ...d, path: Vw(d.path) }, s), h++;
      }
      return { method: "register_assets", registered: h };
    }
    case "unregister_assets": {
      let o = w(t.projectId, "projectId", t.method),
        c = w(t.planId, "planId", t.method),
      ...
      if (!p || p.projectId !== o)
        throw Error("Plan token is missing or does not match this project. Call finalize_plan first.");
      let g = a.map(Vw).filter((d) => !Jst(d, p.deletes));
      if (g.length > 0)
        throw Error(
          `Cannot unregister cards for paths outside the finalized plan's deletes: ${g.join(", ")}. Re-run finalize_plan with the full set.`,
        );
      let h = 0;
      for (let d of a.map(Vw)) {
        if (s.aborted) throw new Ze();
        await G(r, o, d, s), h++;
      }
      return { method: "unregister_assets", unregistered: h };
    }
    case "create_project": {
      let o = w(t.name, "name", t.method),
        c = await W(r, o, s);
      return { method: "create_project", projectId: c.projectId, name: c.name };
    }
    case "report_validate":
      return { method: "report_validate" };
  }
}
export { S as DesignSyncPreconditionError, Qe as DesignSyncTool };
