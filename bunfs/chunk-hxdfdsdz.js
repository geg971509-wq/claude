...
// Version: 2.1.252
import { J, G, K, et } from "/$bunfs/root/chunk-f9h0bg01.js";
import { ne, jt } from "/$bunfs/root/chunk-tx16jn0x.js";
import { Qd, l, E, X } from "/$bunfs/root/chunk-ypdw393e.js";
import { vt, Ps, n } from "/$bunfs/root/chunk-fv016jr6.js";
...
import { wAe, Wwn, twt, Ga } from "/$bunfs/root/chunk-tzhtxm67.js";
import {
  Str,
  S$,
  Zde,
...
import { gE, d9n } from "/$bunfs/root/chunk-zze8764r.js";
import { ly } from "/$bunfs/root/chunk-bp3j8sq3.js";
import { li } from "/$bunfs/root/chunk-a6xhjq7b.js";
import { Fu, II, Pht, n5e, jXn, egn, tgn, ign, Z6t, sgn, agn, t3, s5e, lgn } from "/$bunfs/root/chunk-qnrh4abv.js";
import { ih, xpn } from "/$bunfs/root/chunk-6nmk3t6g.js";
import { ZVn, e5n, XGe } from "/$bunfs/root/chunk-ddbk6aje.js";
import { E0e, den, tUn, pen, K6e, C0e, men, J6e, R0e, rZ } from "/$bunfs/root/chunk-ac0gs4d8.js";
import { I$n, P$n, D$n, N$n, F$n, UZt, BZt, U$n } from "/$bunfs/root/chunk-drw3nb55.js";
import { KZt, XZt, YZt, gIt, j6e, ZZt, yIt, bIt, nen, Dot } from "/$bunfs/root/chunk-wz7pqpgq.js";
import { kxe } from "/$bunfs/root/chunk-debmm3sq.js";
import { THt, Bnt, d2e } from "/$bunfs/root/chunk-53wt2b6b.js";
import { hTe } from "/$bunfs/root/chunk-n8ecp4gs.js";
import { D } from "/$bunfs/root/chunk-7s7jqj2f.js";
import { randomBytes as _e, randomUUID as We } from "crypto";
import { unlinkSync as Ge } from "fs";
import {
  chmod as Ae,
  lstat as N,
  mkdir as te,
...
import { createServer as Xe, Socket as Be } from "net";
import { basename as le, dirname as I, isAbsolute as ie, join as F, normalize as He, resolve as ue } from "path";
var Me = 500,
  Fe = 32;
class he {
  verdicts = new Map();
...
var Pe = new J(() => new he()),
  Ne = {
    readAncestors: async (e) => {
      let { ancestors: t, readFailed: i, truncated: r } = await twt(e);
      if (r && !i && !t.includes(process.pid)) ({ ancestors: t, readFailed: i, truncated: r } = await twt(e, Fe));
...
async function Ke(e, t = Ne) {
  ...
  let m = process.pid !== 1 && w.includes(process.pid);
  return d.remember(r, m), m ? "self" : "not-self";
}
async function ye(e) {
...
function we(e) {
  let t = c();
  if (e === "key_publish_failed" && t.lastStartDegradedCause === "primary_dir_refused_fell_back") return;
...
function Se(e) {
  return hTe(e) ? e : e === void 0 ? "(none)" : "(malformed)";
}
function je(e) {
  if (!Array.isArray(e)) return [];
  let t = [];
  for (let i of e) {
    if (t.length >= egn) break;
    if (hTe(i)) t.push(i);
  }
  return t;
}
...
async function Ye(e, t, i, r, d) {
  ...
  if (typeof s !== "string" || s.length === 0) {
    n("[uds-messaging] Ignoring user message with missing or non-string content", { level: "warn" });
    return;
  }
  if (!Ie(e)) return;
  let w = typeof e.uuid === "string" ? e.uuid : We(),
    o = R0e();
  if (o !== void 0) {
    rZ("uds: dropped before attachment materialization", o),
      tUn(
        {
          kind: "peer",
          from: e.from ?? "unknown",
          ...(t !== void 0 && { verifiedPeerPid: t }),
          ...(hTe(e.msg_id) && { msg_id: e.msg_id }),
        },
        "refused",
      );
    return;
  }
  let m = e.priority === "now" || e.priority === "next" || e.priority === "later" ? e.priority : "next",
    k = s;
  if (e.file_attachments !== void 0 && kxe())
    try {
      let {
          emitPeerFileReceiveTelemetry: f,
          injectPeerFilePrefix: b,
          materializeLocalPeerFiles: C,
        } = await import("/$bunfs/root/chunk-9tr6w9kh.js"),
        S = await C(e.file_attachments);
      if (S.received > 0) (k = b(s, S.prefix)), f("uds", S.received, S.verified);
    } catch (f) {
      n(`[uds-messaging] Failed to materialize file_attachments: ${II(String(f))}`, { level: "warn" });
    }
  let u = await ce(t, r, d),
    _ = Zde(
      {
        kind: "peer",
        from: e.from ?? "unknown",
...
function Oe(e) {
  ...
  if (
    ...
    i !== void 0 &&
    wHt(t.from, i, t.verifiedPeerPid) !== void 0
  )
    xpn(t.from, t.verifiedPeerPid, t.verifiedPeerProcStart);
}
function wHt(e, t, i) {
  if (!e.startsWith("uds:") || !vre(e)) return;
  let r = mf(e).target;
  return r && Ebn(r, t, { verifiedPeerPid: i, ownerUids: c().peerDirOwnerUids }) ? r : void 0;
}
async function be(e, t, i, r, d) {
  if (!fe(e)) {
    n("[uds-messaging] Ignoring message without valid type field", { level: "warn" });
    return;
  }
  if (e.type === "user") await Ye(e, t, i, r, d);
    ...
    if (e.action === "rename" && typeof e.name === "string") c().onRename?.(e.name);
      ...
      let s = e.status === "expired" && e.status_detail === "refused" ? "refused" : e.status,
        w = sgn(e.orig_msg_id, s),
        o = w?.destination;
      if (e.status === "dropped") {
        let m = jXn(e.drop_reason),
          k = agn(je(e.dropped_msg_ids));
        if (o !== void 0) {
          let u = k.get(o) ?? { dropped: 0, wereHeld: 0 };
          if ((u.dropped++, w?.wasHeld)) u.wereHeld++;
          k.set(o, u);
        }
        if (m === "queue-full") for (let [u, { wereHeld: _ }] of k) for (let v = 0; v < _; v++) Z6t(u);
        if (k.size === 0)
          n(
            `[uds-messaging] peer_message_status dropped: neither orig_msg_id=${Se(e.orig_msg_id)} nor any named id matches an outstanding send`,
          );
        for (let [u, { dropped: _ }] of k) c().onPeerMessageStatus?.("dropped", u, { dropReason: m, droppedCount: _ });
      } else if (o === void 0)
        n(`[uds-messaging] peer_message_status dropped: no outstanding send matches orig_msg_id=${Se(e.orig_msg_id)}`);
      else {
        if (s === "held") ign(o);
        else if (s === "delivered" && w?.wasHeld) Z6t(o);
        c().onPeerMessageStatus?.(s, o);
      }
    } else if (e.action === "notify_when_idle") {
      let s = KZt().safeParse(e);
      if (!s.success) {
      ...
      let w = c().activeSocketPath,
        o = s.data.from,
        m = w !== void 0 ? wHt(o, w, t) : void 0;
      if (w === void 0)
        n("[uds-messaging] notify_when_idle dropped: own inbox not bound (shutting down)"),
          g("cross_session_notify_idle", "own_inbox_unbound");
      else if (m === void 0)
        n(
          `[uds-messaging] notify_when_idle dropped: reply address unshaped or outside our socket namespace (${Fu(o)})`,
        ),
          g("cross_session_notify_idle", "unvettable_reply_target");
      else if (tS(m) === tS(w))
        n("[uds-messaging] notify_when_idle dropped: reply target is this session (self-target)"),
          g("cross_session_notify_idle", "self_target_frame");
      else {
        let k = await ce(t, r, d),
          u = YZt(o, m, s.data.msg_id, t, i, d === "peer", Ce(s.data.from_mode), k);
        if ((n(`[uds-messaging] notify_when_idle from ${Fu(o)}: ${u}`), u === "full"))
          Dot(m, s.data.msg_id, t, d === "peer", i);
      }
    } else if (e.action === "peer_idle_notice") {
      let s = XZt().safeParse(e);
      if (!s.success) {
        n("[uds-messaging] peer_idle_notice dropped: malformed frame"),
          g("cross_session_notify_idle", "malformed_notice");
        return;
      }
      if (!j6e(s.data.orig_msg_id)) {
        n("[uds-messaging] peer_idle_notice: dropped (uncorrelated / already delivered / expired)");
        return;
      }
      let w = await ce(t, r, d);
      if (!ZZt(s.data.orig_msg_id, s.data.state, s.data.finished_at, s.data.detail, Ce(s.data.from_mode), w))
        n(
          `[uds-messaging] peer_idle_notice not admitted: subscription for orig_msg_id=${Fu(s.data.orig_msg_id)} was already consumed`,
        );
    } else if (e.action === "yield_artifact_replies") {
      let s = I$n().safeParse(e);
      if (!s.success) {
      ...
      let w = c().activeSocketPath,
        o = w !== void 0 ? wHt(s.data.from, w, t) : void 0;
      if (w === void 0 || o === void 0) {
        n(
          `[uds-messaging] yield_artifact_replies dropped: ${w === void 0 ? "own inbox not bound" : "reply address unshaped or outside our socket namespace"} (${Fu(s.data.from)})`,
        ),
          g("artifact_comments_autoreact", "yield_unvettable_target");
        return;
      }
      if (tS(o) === tS(w)) {
        n("[uds-messaging] yield_artifact_replies dropped: reply target is this session");
        return;
      }
      let m = await s5e(o);
      if (m === void 0 || m.sessionId !== K() || (t !== void 0 && m.pid !== t)) {
        n(
      ...
      BZt(
        s.data,
        o,
        { pid: t ?? m.pid, procStart: t === void 0 || t === m.pid ? m.procStart : void 0, writeToken: i },
        Date.now(),
      );
    } else if (e.action === "unyield_artifact_replies") {
      let s = D$n().safeParse(e);
      if (!s.success || !U$n(s.data, t))
        n("[uds-messaging] unyield_artifact_replies dropped: malformed, uncorrelated or already handed back");
    } else if (e.action === "artifact_replies_yielded") {
      let s = P$n().safeParse(e);
      if (!s.success) {
        n("[uds-messaging] artifact_replies_yielded dropped: malformed frame"),
          g("artifact_live_subscribe", "yield_malformed_answer");
        return;
      }
      if (!N$n(s.data.orig_msg_id)) {
        n("[uds-messaging] artifact_replies_yielded dropped: uncorrelated or already settled");
        return;
      }
      F$n(s.data, t);
    } else n(`[uds-messaging] Unhandled control action: ${Fu(String(e.action))}`);
  } else n(`[uds-messaging] Received unhandled message type: ${Fu(e.type)}`);
}
...
function Ze(e) {
  ...
  if (k) {
    let S = Pht(e),
      A = S !== void 0 ? wAe(S) : void 0;
    if (S !== void 0 && A !== void 0) m = { pid: S, token: A };
  }
  let u = "",
    _,
    v = !1,
    ...
    b = (S) => {
      ...
      if (c().authRequired && _ === void 0) {
        f(fe(S) ? `a '${Fu(S.type)}' line` : "a line");
        return;
      }
      if (!w) {
        if (
    ...
    C = (S) => {
      try {
        ...
        n(`[uds-messaging] Failed to handle line: ${A}`, { level: "warn" });
      }
    };
  e.on("data", (S) => {
    if (((u += S), u.length > n5e)) {
      n(`[uds-messaging] Line exceeded ${n5e} chars; dropping connection`, { level: "warn" }), e.destroy(), (u = "");
      return;
    }
    let A;
    while (
      ...
      if (u.trim() && !e.destroyed) {
        ...
        try {
          ...
          if (
            ...
            f("an unparseable final fragment");
        }
        if (A) C(S);
      }
      e.end();
...
async function H(e, t, i, { settleHeld: r = !0 } = {}) {
  ...
  let d = r ? K6e() : void 0;
  await Je();
  let s = r ? jt(gIt("exited"), Qe) : void 0;
  if ((await d, await s, r)) await d9n();
  try {
    await re(t);
  } catch {}
  let w = c();
  if (w.activeKeyFile !== void 0) await ktr(w.activeKeyFile, i), (w.activeKeyFile = void 0);
  j();
}
function j() {
  (c().activeSocketPath = void 0),
    ...
    mb.unset("CLAUDE_CODE_MESSAGING_TOKEN"),
    tgn(void 0),
    den(null),
    pen(null),
    yIt(null),
...
function nn(e) {
  let t = `${e.replace(/\.sock$/, "")}-${_e(4).toString("hex")}.sock`;
  if (Buffer.byteLength(t) <= Y) return t;
  let i = F(e, ".."),
    r = Y - Buffer.byteLength(F(i, ".sock"));
  return F(i, `${_e(8).toString("hex").slice(0, Math.max(1, r))}.sock`);
}
async function tn(e) {
  let t = `${le(e).replace(/\.sock$/, "")}-`,
    i;
  try {
    i = await qe(I(e));
  } catch {
  ...
  for (let r of i) {
    if (!r.startsWith(t) || !/^[0-9a-f]{8}\.sock$/.test(r.slice(t.length))) continue;
    let d = F(I(e), r);
    if ((await me(d)) === "live") continue;
    try {
      await re(d), n(`[uds-messaging] Reaped stale moved-aside socket ${d}`);
    } catch {}
  }
}
async function sn(e, t) {
  if ((await tn(t), await ee(e, t))) return t;
  if ((await me(t)) !== "live") {
    try {
      await re(t);
    } catch {}
    if (await ee(e, t)) return t;
  }
  for (let i = 0; i < 3; i++) {
    let r = nn(t);
    if (
...
function jTr(e, t) {
  return Qur(e ?? Yur(), t, { isExplicit: e !== void 0 });
}
function W() {
  mb.unset("CLAUDE_CODE_MESSAGING_SOCKET"), mb.unset("CLAUDE_CODE_MESSAGING_TOKEN");
}
var rn = [
  "directory_rule",
  "foreign_owner",
...
function x(e, t, i) {
  return Object.assign(t, { socketsDirVetKind: e, ...(i && { refusedComponent: i }) });
}
function on(e) {
  ...
  return typeof t === "object" &&
    ...
    ? {
        ...
        gid: t.gid,
        mode: t.mode,
        ownerRefused: "ownerRefused" in t && typeof t.ownerRefused === "boolean" ? t.ownerRefused : void 0,
      }
    : void 0;
}
function V(e, t, i) {
  return { path: e, uid: t.uid, gid: t.gid, mode: t.mode & 4095, ...(i !== void 0 && { ownerRefused: i }) };
}
function Te(e) {
...
var an = /^[A-Za-z0-9._\/ ~+=:@,#%-]$/,
  dn = new Set(["EACCES", "EPERM", "EROFS", "ENOSPC", "EDQUOT", "ENOTDIR"]);
function un(e) {
  switch (se(e)) {
    case "directory_rule":
    case "foreign_owner":
...
var B =
  ...
  ke = `A component of the sockets path is not a directory (a regular file is in the way). ${B}`,
  ve = `The sockets path runs through a symlink loop. ${B}`,
  cn =
...
function De(e) {
  switch (se(e)) {
    case "directory_rule":
      return `A directory on the sockets path is shared (world- or group-writable without the sticky bit, e.g. a container volume mounted at /tmp) or not owned by you or root. ${B}`;
    case "foreign_owner":
    case "leaf_shape":
      return B;
    case "not_directory":
  ...
  switch (E(e)) {
    ...
    case "EACCES":
    case "EPERM":
      return `This user lacks permission on part of the sockets path (an ancestor is not searchable, or the parent is not writable). ${B}`;
    case "ENOENT":
      return `An ancestor of the sockets path does not exist. ${B}`;
    default:
      return "";
  }
}
...
async function Re(e) {
  if (!ie(e)) throw x("internal", Error("sockets directory must be absolute here"));
  let t = process.getuid?.(),
    i = (h) => {
      if (h.isSymbolicLink())
        throw x("leaf_shape", Error("sockets directory is a symlink \u2014 refusing to use it"), V(e, h));
      if (!h.isDirectory()) throw x("leaf_shape", Error("sockets directory exists but is not a directory"), V(e, h));
      if (t !== void 0 && !r(h.uid))
        throw x(
          "foreign_owner",
  ...
  let w = ZVn(),
    o = async (h, U) => {
      try {
        let R = U.isSymbolicLink() ? F(await de(I(h)), le(h)) : await de(h),
          M = await N(R);
        return M.dev === U.dev && M.ino === U.ino ? R : void 0;
      } catch {
        return;
      }
    },
    m = (h) =>
      s !== void 0 && ((s.unmappedOwnerUid !== void 0 && h === s.unmappedOwnerUid) || (h === 0 && s.rootUidAmbiguous)),
    k = m,
      ...
      for (let T = h, z = !0; ; T = I(T), z = !1) {
        ...
        if (O !== void 0) {
          ...
          if (O.isSymbolicLink()) {
            ...
            let Z = (await Ve(T)).replace(/\/{2,}/g, "/"),
              oe = Z.length > 1 && Z.endsWith("/") ? Z.slice(0, -1) : Z,
              xe = ie(oe) ? oe : `${await de(I(T))}/${oe}`,
              { startExists: Ue } = await C(xe, q, R + 1);
            if (z) M = Ue;
          } else {
            if (!O.isDirectory())
              throw x(
                "not_directory",
                Error("a sockets-directory component exists but is not a directory \u2014 refusing to use it"),
                V(T, O),
              );
            let Q = k(O.uid) ? await o(T, O) : void 0;
            if (!v(O, q, Q))
  ...
  if (L && A.deepestExisting === S) throw Ee();
  if (L) {
    let h = [],
      U = (R) => {
        if (!R.isDirectory() || (t !== void 0 && !r(R.uid)))
          throw x(
            "raced",
            Error(
    ...
    for (let R = S; R !== A.deepestExisting && !b(R); R = I(R)) h.unshift(R);
    for (let R of h)
      try {
        await te(R, { mode: 448 });
      } catch (M) {
        if (X(M) && A.sawSymlink) throw Ee();
...
async function Jur(e) {
  ...
  if (e.split("/").includes("..")) throw new Qd(`--messaging-socket-path must not contain '..' segments, got: ${e}`);
  let i = e.split("/").at(-1);
  if (i === "" || i === ".")
    throw new Qd(`--messaging-socket-path must name a socket file inside a directory, got: ${e}`);
  let r = He(e).replace(/\/+$/, "");
  if (r === "" || !r.startsWith("/") || le(r) === "")
    throw new Qd(`--messaging-socket-path must name a socket file inside a directory, got: ${e}`);
  if (!xH(r) || !xH(e)) throw new Qd(`--messaging-socket-path must be a local socket path, got: ${e}`);
  if (Buffer.byteLength(r) > Y)
    throw new Qd(
      `--messaging-socket-path is too long for a Unix socket (${Buffer.byteLength(r)} bytes, max ${Y}): ${r}. Choose a shorter path, e.g. under $XDG_RUNTIME_DIR or /tmp/<private-dir>.`,
    );
  let d = I(r),
    s = "Use a private directory you own that only you use, e.g. mkdir -m 700 <dir> (or chmod 700 an existing one).",
    w = (f) => {
      if (f.isSymbolicLink())
        throw new Qd(`--messaging-socket-path directory must be a real directory, not a symlink: ${d}. ${s}`);
      if (!f.isDirectory()) throw new Qd(`--messaging-socket-path parent is not a directory: ${d}.`);
      let b = process.getuid?.();
      if (b !== void 0 && f.uid !== b)
        throw new Qd(`--messaging-socket-path directory ${d} is not owned by you (uid ${f.uid}). ${s}`);
      if ((f.mode & 63) !== 0)
        throw new Qd(
          `--messaging-socket-path directory ${d} is not private (mode ${(f.mode & 4095).toString(8)}); the socket directory must be mode 0700 so no other user or group can reach or replace the socket. ${s}`,
        );
    },
    o = async () => {
      try {
        ...
        throw new Qd(`--messaging-socket-path directory could not be examined: ${d} (${b ?? String(f)}). ${s}`);
      }
    },
    m = await o();
  if (m !== void 0) return w(m), r;
  ...
  for (let f = I(d); ; f = I(f)) {
    ...
    try {
      ...
      if (!X(C)) throw new Qd(`--messaging-socket-path: cannot inspect ${f} (${E(C) ?? C}). ${s}`);
      u.unshift(f);
    }
    if (b !== void 0) {
      if (k !== void 0 && b.uid !== k && b.uid !== 0)
        throw new Qd(
          `--messaging-socket-path: ${f} ${b.isSymbolicLink() ? "is a symlink" : "is a directory"} owned by another user \u2014 refusing to create your sockets directory ${b.isSymbolicLink() ? "through" : "inside"} it. ${s}`,
        );
      break;
    }
    if (I(f) === f) break;
  ...
  try {
    for (let f of u)
      try {
        ...
        let C = await N(f);
        if (!C.isDirectory() || (k !== void 0 && C.uid !== k))
          throw new Qd(
            `--messaging-socket-path: ${f} appeared while your sockets directory was being created and is not a directory you own \u2014 refusing to use it. ${s}`,
          );
      }
    try {
    ...
    if (f instanceof Qd) throw f;
    throw new Qd(`--messaging-socket-path directory ${d} does not exist and could not be created (${E(f) ?? f}). ${s}`);
  }
  let v = await o();
  if (v === void 0) throw new Qd(`--messaging-socket-path directory ${d} vanished while being set up. ${s}`);
  if (!_) return w(v), r;
  return w(v), r;
}
...
async function ln(e, t, i = {}) {
  if (((c().lastStartFailureCause = "bind_failed"), (c().lastStartDegradedCause = void 0), !xH(e))) {
    if (
      (n(
  ...
  if (i.isExplicit) {
    ...
    try {
      ...
      let v = Te(m);
      n(
        `[uds-messaging] sockets directory ${o} refused (${l(m)}${v ? `; refused component: ${v}` : ""}); trying the per-uid fallback ${_}`,
        { level: "warn" },
      );
      try {
        await Re(_);
  ...
  try {
    d = Xe({ allowHalfOpen: !0 }, (o) => {
      c().connectedClients.add(o),
        ...
        Ze(o),
        o.on("close", () => {
          c().connectedClients.delete(o), n("[uds-messaging] Client disconnected");
        });
    });
  } catch (o) {
    j(), n(`[uds-messaging] Failed to create server: ${String(o)}`, { level: "error" }), W();
    return;
  }
  d.on("error", (o) => {
    n(`[uds-messaging] Server error: ${o.message}`, { level: "error" });
  }),
    (c().authRequired = i.requireAuth ?? Tbt()),
    (c().firstLineDeadlineMs = i.firstLineDeadlineMs ?? THt);
  let w = vtr();
  c().activeTokens = w;
  try {
    if (i.isExplicit) {
      if (!(await ee(d, e))) throw Error("listen EADDRINUSE on the requested socket path");
    } else (e = await sn(d, e)), (c().activeSocketPath = e);
    {
      let u = await XGe(),
        _ = process.getuid?.();
      c().peerDirOwnerUids = [...(_ !== void 0 ? [_] : []), ...(u !== void 0 && u !== _ ? [u] : [])];
    }
    d.unref();
    let o = vt(async () => {
    ...
    (s = o), await Ae(e, 384);
    try {
      (c().activeKeyFile = await Rtr(e, w.peerToken, t, { sweepPermitted: await Rre() })), gn();
    } catch (u) {
      if (c().authRequired) {
        if (
          (n(
    ...
    (process.env.CLAUDE_CODE_MESSAGING_SOCKET = e), mb.set("CLAUDE_CODE_MESSAGING_TOKEN", w.childToken), tgn(S$(e));
    let m = S$(e);
    return (
      pen(Oe),
      (ih().senderMode = rNn),
      yIt((u, _, v, f, b) =>
        t3(
          u,
          { action: "peer_idle_notice", ..._, from: m, ...(f ? mn() : {}) },
          { ...(v !== void 0 && { expectPeerPid: v }), ...(b !== void 0 && { expectPeerProcStart: b }), storageV5: t },
        ),
      ),
      UZt(
        (u, _, v, f) =>
          t3(
            u,
            { action: "artifact_replies_yielded", ..._, from: m },
            {
              ...(v !== void 0 && { expectPeerPid: v }),
              ...(f !== void 0 && { expectPeerProcStart: f }),
              storageV5: t,
            },
          ),
        m,
      ...
      bIt((u) => lgn(u)),
      den((u, _, v) => {
        let f = u.origin?.kind === "peer" ? u.origin : void 0,
          b = f?.from;
        if (typeof b !== "string") return;
        let C = wHt(b, e, f?.verifiedPeerPid);
        if (C === void 0) {
          n(`[uds-messaging] hold-receipt skipped: reply address unshaped or outside our socket namespace (${Fu(b)})`);
          return;
        }
        return t3(
          C,
          {
            ...
            ...(_ === "refused" ? { status: "expired", status_detail: "refused" } : { status: _ }),
            reason: fn(_),
            from: m,
            ...(typeof f?.msg_id === "string" && { orig_msg_id: f.msg_id }),
            ...(_ === "dropped" && v !== void 0 && { drop_reason: v.dropReason, dropped_msg_ids: v.droppedMsgIds }),
          },
          {
            ...(f?.verifiedPeerPid !== void 0 && { expectPeerPid: f.verifiedPeerPid }),
            ...(f?.verifiedPeerProcStart !== void 0 && { expectPeerProcStart: f.verifiedPeerProcStart }),
            storageV5: t,
          },
        ).catch((S) => n(`[uds-messaging] hold-receipt send failed to ${Fu(b)}: ${II(String(S))}`));
      }),
    ...
    if (i.isExplicit) {
      ...
      if (s !== void 0)
        throw (
          ...
          new Qd(
            `--messaging-socket-path: bound ${e} but could not finish setting the socket up (${m ?? String(o)}); the inbox was closed. The filesystem there may not support socket permissions \u2014 choose another directory.`,
          ))
        );
      j(), W();
      let k =
...
function rNn() {
  ...
  let t = li().inbound.modeAtUnwire;
  return t !== void 0 ? C0e(t) : void 0;
}
function Ce(e) {
  return Str(e) ? e : void 0;
}
function ce(e, t, i) {
  return ye({
    selfSentAncestry: t,
...
function gn() {
  if (!process.listeners("exit").includes(oNn)) process.on("exit", oNn);
}
function oNn() {
  try {
...
export { NTr, FTr, yHt, SHt, $Tr, bHt, UTr, wHt, BTr, Yur, nNn, jTr, Jur, Qur, rNn, oNn };
