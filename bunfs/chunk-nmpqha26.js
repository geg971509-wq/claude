...
function YIt() {
  let t = Ty() ?? bl(Se());
  return u(t) === Nl() ? S(t) : void 0;
}
function W(t, a, s) {
  return Te.sidecar(t, a, ["workflows", `${s}.json`]);
}
async function IUn(t, a, s) {
  try {
    ...
    n(`Failed to write workflow snapshot ${t}: ${i instanceof Error ? i.message : i}`);
  }
}
function d(t, a) {
  let s = t.runId ?? a;
      ...
      i
        ...
        .map(async (e) => {
          try {
            ...
            return n(`Failed to parse workflow snapshot ${e}: ${r instanceof Error ? r.message : r}`), null;
          }
        }),
    )
  ).filter((e) => e !== null);
...
async function j(t, a) {
  let s = { namespace: "sidecar", projectKey: a, sessionId: K(), relPath: ["workflows"] },
    i = [],
    l;
  do {
    let r = await t.listEntries(s, l === void 0 ? void 0 : { cursor: l });
    if (!r.ok) return n(`Failed to list workflow snapshots: ${r.error.code}`), [];
    ...
    await Promise.all(
      i.map(async (r) => {
        ...
        try {
          ...
          if (!f.ok) return n(`Failed to parse workflow snapshot ${o}: ${f.error.code}`), null;
          let c = f.value.items[0];
          if (!c?.found) return n(`Failed to parse workflow snapshot ${o}: deleted between list and read`), null;
          let w = V(Buffer.from(c.value).toString("utf8"));
          return d(w, o.replace(/\.json$/, ""));
        } catch (f) {
          return n(`Failed to parse workflow snapshot ${o}: ${f instanceof Error ? f.message : f}`), null;
        }
      }),
    )
  ).filter((r) => r !== null);
