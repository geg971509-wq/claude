// @bun @bytecode
// Claude Code is a Beta product per Anthropic's Commercial Terms of Service.
// By using Claude Code, you agree that all code acceptance or rejection decisions you make,
// and the associated conversations in context, constitute Feedback under Anthropic's Commercial Terms,
// and may be used to improve Anthropic's products, including training models.
// You are responsible for reviewing any code suggestions before use.

// (c) Anthropic PBC. All rights reserved. Use is subject to the Legal Agreements outlined here: https://code.claude.com/docs/en/legal-and-compliance.

// Version: 2.1.252
import { S } from "/$bunfs/root/chunk-yz031c9r.js";
var he = S(function (Ce) {
  Object.defineProperty(Ce, "__esModule", { value: true });
  Ce._globalThis = void 0;
  Ce._globalThis = typeof globalThis === "object" ? globalThis : global;
});
var me = S(function (g) {
  var Mn =
      (g && g.__createBinding) ||
      (Object.create
        ? function (e, t, r, n) {
            if (n === void 0) n = r;
            Object.defineProperty(e, n, {
              enumerable: true,
              get: function () {
                return t[r];
              },
            });
          }
        : function (e, t, r, n) {
            if (n === void 0) n = r;
            e[n] = t[r];
          }),
    yn =
      (g && g.__exportStar) ||
      function (e, t) {
        for (var r in e) if (r !== "default" && !Object.prototype.hasOwnProperty.call(t, r)) Mn(t, e, r);
      };
  Object.defineProperty(g, "__esModule", { value: true });
  yn(he(), g);
});
var De = S(function (d) {
  var In =
      (d && d.__createBinding) ||
      (Object.create
        ? function (e, t, r, n) {
            if (n === void 0) n = r;
            Object.defineProperty(e, n, {
              enumerable: true,
              get: function () {
                return t[r];
              },
            });
          }
        : function (e, t, r, n) {
            if (n === void 0) n = r;
            e[n] = t[r];
          }),
    An =
      (d && d.__exportStar) ||
      function (e, t) {
        for (var r in e) if (r !== "default" && !Object.prototype.hasOwnProperty.call(t, r)) In(t, e, r);
      };
  Object.defineProperty(d, "__esModule", { value: true });
  An(me(), d);
});
var j = S(function (Le) {
  Object.defineProperty(Le, "__esModule", { value: true });
  Le.VERSION = void 0;
  Le.VERSION = "1.9.0";
});
var Be = S(function (qe) {
  Object.defineProperty(qe, "__esModule", { value: true });
  qe.isCompatible = qe._makeCompatibilityCheck = void 0;
  var Cn = j(),
    Ve = /^(\d+)\.(\d+)\.(\d+)(-(.+))?$/;
  function we(e) {
    let t = new Set([e]),
      r = new Set(),
      n = e.match(Ve);
    if (!n) return () => false;
    let o = { major: +n[1], minor: +n[2], patch: +n[3], prerelease: n[4] };
    if (o.prerelease != null)
      return function (c) {
        return c === e;
      };
    function i(u) {
      return r.add(u), false;
    }
    function s(u) {
      return t.add(u), true;
    }
    return function (c) {
      if (t.has(c)) return true;
      if (r.has(c)) return false;
      let _ = c.match(Ve);
      if (!_) return i(c);
      let l = { major: +_[1], minor: +_[2], patch: +_[3], prerelease: _[4] };
      if (l.prerelease != null) return i(c);
      if (o.major !== l.major) return i(c);
      if (o.major === 0) {
        if (o.minor === l.minor && o.patch <= l.patch) return s(c);
        return i(c);
      }
      if (o.minor <= l.minor) return s(c);
      return i(c);
    };
  }
  qe._makeCompatibilityCheck = we;
  qe.isCompatible = we(Cn.VERSION);
});
var f = S(function (Ue) {
  Object.defineProperty(Ue, "__esModule", { value: true });
  Ue.unregisterGlobal = Ue.getGlobal = Ue.registerGlobal = void 0;
  var hn = De(),
    x = j(),
    mn = Be(),
    Dn = x.VERSION.split(".")[0],
    E = Symbol.for(`opentelemetry.js.api.${Dn}`),
    T = hn._globalThis;
  function Ln(e, t, r, n = false) {
    var o;
    let i = (T[E] = (o = T[E]) !== null && o !== void 0 ? o : { version: x.VERSION });
    if (!n && i[e]) {
      let s = Error(`@opentelemetry/api: Attempted duplicate registration of API: ${e}`);
      return r.error(s.stack || s.message), false;
    }
    if (i.version !== x.VERSION) {
      let s = Error(
        `@opentelemetry/api: Registration of version v${i.version} for ${e} does not match previously registered API v${x.VERSION}`,
      );
      return r.error(s.stack || s.message), false;
    }
    return (i[e] = t), r.debug(`@opentelemetry/api: Registered a global for ${e} v${x.VERSION}.`), true;
  }
  Ue.registerGlobal = Ln;
  function jn(e) {
    var t, r;
    let n = (t = T[E]) === null || t === void 0 ? void 0 : t.version;
    if (!n || !(0, mn.isCompatible)(n)) return;
    return (r = T[E]) === null || r === void 0 ? void 0 : r[e];
  }
  Ue.getGlobal = jn;
  function Vn(e, t) {
    t.debug(`@opentelemetry/api: Unregistering a global for ${e} v${x.VERSION}.`);
    let r = T[E];
    if (r) delete r[e];
  }
  Ue.unregisterGlobal = Vn;
});
var ke = S(function ($e) {
  Object.defineProperty($e, "__esModule", { value: true });
  $e.DiagComponentLogger = void 0;
  var Gn = f();
  class Ke {
    constructor(e) {
      this._namespace = e.namespace || "DiagComponentLogger";
    }
    debug(...e) {
      return N("debug", this._namespace, e);
    }
    error(...e) {
      return N("error", this._namespace, e);
    }
    info(...e) {
      return N("info", this._namespace, e);
    }
    warn(...e) {
      return N("warn", this._namespace, e);
    }
    verbose(...e) {
      return N("verbose", this._namespace, e);
    }
  }
  $e.DiagComponentLogger = Ke;
  function N(e, t, r) {
    let n = (0, Gn.getGlobal)("diag");
    if (!n) return;
    return r.unshift(t), n[e](...r);
  }
});
var A = S(function (We) {
  Object.defineProperty(We, "__esModule", { value: true });
  We.DiagLogLevel = void 0;
  var Bn;
  (function (e) {
    (e[(e.NONE = 0)] = "NONE"),
      (e[(e.ERROR = 30)] = "ERROR"),
      (e[(e.WARN = 50)] = "WARN"),
      (e[(e.INFO = 60)] = "INFO"),
      (e[(e.DEBUG = 70)] = "DEBUG"),
      (e[(e.VERBOSE = 80)] = "VERBOSE"),
      (e[(e.ALL = 9999)] = "ALL");
  })((Bn = We.DiagLogLevel || (We.DiagLogLevel = {})));
});
var He = S(function (Ye) {
  Object.defineProperty(Ye, "__esModule", { value: true });
  Ye.createLogLevelDiagLogger = void 0;
  var p = A();
  function Un(e, t) {
    if (e < p.DiagLogLevel.NONE) e = p.DiagLogLevel.NONE;
    else if (e > p.DiagLogLevel.ALL) e = p.DiagLogLevel.ALL;
    t = t || {};
    function r(n, o) {
      let i = t[n];
      if (typeof i === "function" && e >= o) return i.bind(t);
      return function () {};
    }
    return {
      error: r("error", p.DiagLogLevel.ERROR),
      warn: r("warn", p.DiagLogLevel.WARN),
      info: r("info", p.DiagLogLevel.INFO),
      debug: r("debug", p.DiagLogLevel.DEBUG),
      verbose: r("verbose", p.DiagLogLevel.VERBOSE),
    };
  }
  Ye.createLogLevelDiagLogger = Un;
});
var O = S(function (Je) {
  Object.defineProperty(Je, "__esModule", { value: true });
  Je.DiagAPI = void 0;
  var Fn = ke(),
    Kn = He(),
    Qe = A(),
    C = f(),
    $n = "diag";
  class w {
    constructor() {
      function e(n) {
        return function (...o) {
          let i = (0, C.getGlobal)("diag");
          if (!i) return;
          return i[n](...o);
        };
      }
      let t = this,
        r = (n, o = { logLevel: Qe.DiagLogLevel.INFO }) => {
          var i, s, u;
          if (n === t) {
            let l = Error(
              "Cannot use diag as the logger for itself. Please use a DiagLogger implementation like ConsoleDiagLogger or a custom implementation",
            );
            return t.error((i = l.stack) !== null && i !== void 0 ? i : l.message), false;
          }
          if (typeof o === "number") o = { logLevel: o };
          let c = (0, C.getGlobal)("diag"),
            _ = (0, Kn.createLogLevelDiagLogger)(
              (s = o.logLevel) !== null && s !== void 0 ? s : Qe.DiagLogLevel.INFO,
              n,
            );
          if (c && !o.suppressOverrideMessage) {
            let l = (u = Error().stack) !== null && u !== void 0 ? u : "<failed to generate stacktrace>";
            c.warn(`Current logger will be overwritten from ${l}`),
              _.warn(`Current logger will overwrite one already registered from ${l}`);
          }
          return (0, C.registerGlobal)("diag", _, t, true);
        };
      (t.setLogger = r),
        (t.disable = () => {
          (0, C.unregisterGlobal)($n, t);
        }),
        (t.createComponentLogger = (n) => new Fn.DiagComponentLogger(n)),
        (t.verbose = e("verbose")),
        (t.debug = e("debug")),
        (t.info = e("info")),
        (t.warn = e("warn")),
        (t.error = e("error"));
    }
    static instance() {
      if (!this._instance) this._instance = new w();
      return this._instance;
    }
  }
  Je.DiagAPI = w;
});
var rt = S(function (et) {
  Object.defineProperty(et, "__esModule", { value: true });
  et.BaggageImpl = void 0;
  class v {
    constructor(e) {
      this._entries = e ? new Map(e) : new Map();
    }
    getEntry(e) {
      let t = this._entries.get(e);
      if (!t) return;
      return Object.assign({}, t);
    }
    getAllEntries() {
      return Array.from(this._entries.entries()).map(([e, t]) => [e, t]);
    }
    setEntry(e, t) {
      let r = new v(this._entries);
      return r._entries.set(e, t), r;
    }
    removeEntry(e) {
      let t = new v(this._entries);
      return t._entries.delete(e), t;
    }
    removeEntries(...e) {
      let t = new v(this._entries);
      for (let r of e) t._entries.delete(r);
      return t;
    }
    clear() {
      return new v();
    }
  }
  et.BaggageImpl = v;
});
var at = S(function (nt) {
  Object.defineProperty(nt, "__esModule", { value: true });
  nt.baggageEntryMetadataSymbol = void 0;
  nt.baggageEntryMetadataSymbol = Symbol("BaggageEntryMetadata");
});
var q = S(function (it) {
  Object.defineProperty(it, "__esModule", { value: true });
  it.baggageEntryMetadataFromString = it.createBaggage = void 0;
  var Xn = O(),
    kn = rt(),
    Wn = at(),
    Yn = Xn.DiagAPI.instance();
  function zn(e = {}) {
    return new kn.BaggageImpl(new Map(Object.entries(e)));
  }
  it.createBaggage = zn;
  function Hn(e) {
    if (typeof e !== "string") Yn.error(`Cannot create baggage metadata from unknown type: ${typeof e}`), (e = "");
    return {
      __TYPE__: Wn.baggageEntryMetadataSymbol,
      toString() {
        return e;
      },
    };
  }
  it.baggageEntryMetadataFromString = Hn;
});
var M = S(function (ct) {
  Object.defineProperty(ct, "__esModule", { value: true });
  ct.ROOT_CONTEXT = ct.createContextKey = void 0;
  function Jn(e) {
    return Symbol.for(e);
  }
  ct.createContextKey = Jn;
  class R {
    constructor(e) {
      let t = this;
      (t._currentContext = e ? new Map(e) : new Map()),
        (t.getValue = (r) => t._currentContext.get(r)),
        (t.setValue = (r, n) => {
          let o = new R(t._currentContext);
          return o._currentContext.set(r, n), o;
        }),
        (t.deleteValue = (r) => {
          let n = new R(t._currentContext);
          return n._currentContext.delete(r), n;
        });
    }
  }
  ct.ROOT_CONTEXT = new R();
});
var gt = S(function (_t) {
  Object.defineProperty(_t, "__esModule", { value: true });
  _t.DiagConsoleLogger = void 0;
  var G = [
    { n: "error", c: "error" },
    { n: "warn", c: "warn" },
    { n: "info", c: "info" },
    { n: "debug", c: "debug" },
    { n: "verbose", c: "trace" },
  ];
  class lt {
    constructor() {
      function e(t) {
        return function (...r) {
          if (console) {
            let n = console[t];
            if (typeof n !== "function") n = console.log;
            if (typeof n === "function") return n.apply(console, r);
          }
        };
      }
      for (let t = 0; t < G.length; t++) this[G[t].n] = e(G[t].c);
    }
  }
  _t.DiagConsoleLogger = lt;
});
var z = S(function (dt) {
  Object.defineProperty(dt, "__esModule", { value: true });
  dt.createNoopMeter =
    dt.NOOP_OBSERVABLE_UP_DOWN_COUNTER_METRIC =
    dt.NOOP_OBSERVABLE_GAUGE_METRIC =
    dt.NOOP_OBSERVABLE_COUNTER_METRIC =
    dt.NOOP_UP_DOWN_COUNTER_METRIC =
    dt.NOOP_HISTOGRAM_METRIC =
    dt.NOOP_GAUGE_METRIC =
    dt.NOOP_COUNTER_METRIC =
    dt.NOOP_METER =
    dt.NoopObservableUpDownCounterMetric =
    dt.NoopObservableGaugeMetric =
    dt.NoopObservableCounterMetric =
    dt.NoopObservableMetric =
    dt.NoopHistogramMetric =
    dt.NoopGaugeMetric =
    dt.NoopUpDownCounterMetric =
    dt.NoopCounterMetric =
    dt.NoopMetric =
    dt.NoopMeter =
      void 0;
  class B {
    constructor() {}
    createGauge(e, t) {
      return dt.NOOP_GAUGE_METRIC;
    }
    createHistogram(e, t) {
      return dt.NOOP_HISTOGRAM_METRIC;
    }
    createCounter(e, t) {
      return dt.NOOP_COUNTER_METRIC;
    }
    createUpDownCounter(e, t) {
      return dt.NOOP_UP_DOWN_COUNTER_METRIC;
    }
    createObservableGauge(e, t) {
      return dt.NOOP_OBSERVABLE_GAUGE_METRIC;
    }
    createObservableCounter(e, t) {
      return dt.NOOP_OBSERVABLE_COUNTER_METRIC;
    }
    createObservableUpDownCounter(e, t) {
      return dt.NOOP_OBSERVABLE_UP_DOWN_COUNTER_METRIC;
    }
    addBatchObservableCallback(e, t) {}
    removeBatchObservableCallback(e) {}
  }
  dt.NoopMeter = B;
  class b {}
  dt.NoopMetric = b;
  class U extends b {
    add(e, t) {}
  }
  dt.NoopCounterMetric = U;
  class F extends b {
    add(e, t) {}
  }
  dt.NoopUpDownCounterMetric = F;
  class K extends b {
    record(e, t) {}
  }
  dt.NoopGaugeMetric = K;
  class X extends b {
    record(e, t) {}
  }
  dt.NoopHistogramMetric = X;
  class y {
    addCallback(e) {}
    removeCallback(e) {}
  }
  dt.NoopObservableMetric = y;
  class k extends y {}
  dt.NoopObservableCounterMetric = k;
  class W extends y {}
  dt.NoopObservableGaugeMetric = W;
  class Y extends y {}
  dt.NoopObservableUpDownCounterMetric = Y;
  dt.NOOP_METER = new B();
  dt.NOOP_COUNTER_METRIC = new U();
  dt.NOOP_GAUGE_METRIC = new K();
  dt.NOOP_HISTOGRAM_METRIC = new X();
  dt.NOOP_UP_DOWN_COUNTER_METRIC = new F();
  dt.NOOP_OBSERVABLE_COUNTER_METRIC = new k();
  dt.NOOP_OBSERVABLE_GAUGE_METRIC = new W();
  dt.NOOP_OBSERVABLE_UP_DOWN_COUNTER_METRIC = new Y();
  function eo() {
    return dt.NOOP_METER;
  }
  dt.createNoopMeter = eo;
});
var Mt = S(function (St) {
  Object.defineProperty(St, "__esModule", { value: true });
  St.ValueType = void 0;
  var _o;
  (function (e) {
    (e[(e.INT = 0)] = "INT"), (e[(e.DOUBLE = 1)] = "DOUBLE");
  })((_o = St.ValueType || (St.ValueType = {})));
});
var Q = S(function (yt) {
  Object.defineProperty(yt, "__esModule", { value: true });
  yt.defaultTextMapSetter = yt.defaultTextMapGetter = void 0;
  yt.defaultTextMapGetter = {
    get(e, t) {
      if (e == null) return;
      return e[t];
    },
    keys(e) {
      if (e == null) return [];
      return Object.keys(e);
    },
  };
  yt.defaultTextMapSetter = {
    set(e, t, r) {
      if (e == null) return;
      e[t] = r;
    },
  };
});
var ht = S(function (Ct) {
  Object.defineProperty(Ct, "__esModule", { value: true });
  Ct.NoopContextManager = void 0;
  var go = M();
  class At {
    active() {
      return go.ROOT_CONTEXT;
    }
    with(e, t, r, ...n) {
      return t.call(r, ...n);
    }
    bind(e, t) {
      return t;
    }
    enable() {
      return this;
    }
    disable() {
      return this;
    }
  }
  Ct.NoopContextManager = At;
});
var I = S(function (Dt) {
  Object.defineProperty(Dt, "__esModule", { value: true });
  Dt.ContextAPI = void 0;
  var fo = ht(),
    J = f(),
    mt = O(),
    Z = "context",
    Oo = new fo.NoopContextManager();
  class ee {
    constructor() {}
    static getInstance() {
      if (!this._instance) this._instance = new ee();
      return this._instance;
    }
    setGlobalContextManager(e) {
      return (0, J.registerGlobal)(Z, e, mt.DiagAPI.instance());
    }
    active() {
      return this._getContextManager().active();
    }
    with(e, t, r, ...n) {
      return this._getContextManager().with(e, t, r, ...n);
    }
    bind(e, t) {
      return this._getContextManager().bind(e, t);
    }
    _getContextManager() {
      return (0, J.getGlobal)(Z) || Oo;
    }
    disable() {
      this._getContextManager().disable(), (0, J.unregisterGlobal)(Z, mt.DiagAPI.instance());
    }
  }
  Dt.ContextAPI = ee;
});
var re = S(function (jt) {
  Object.defineProperty(jt, "__esModule", { value: true });
  jt.TraceFlags = void 0;
  var xo;
  (function (e) {
    (e[(e.NONE = 0)] = "NONE"), (e[(e.SAMPLED = 1)] = "SAMPLED");
  })((xo = jt.TraceFlags || (jt.TraceFlags = {})));
});
var h = S(function (Vt) {
  Object.defineProperty(Vt, "__esModule", { value: true });
  Vt.INVALID_SPAN_CONTEXT = Vt.INVALID_TRACEID = Vt.INVALID_SPANID = void 0;
  var vo = re();
  Vt.INVALID_SPANID = "0000000000000000";
  Vt.INVALID_TRACEID = "00000000000000000000000000000000";
  Vt.INVALID_SPAN_CONTEXT = { traceId: Vt.INVALID_TRACEID, spanId: Vt.INVALID_SPANID, traceFlags: vo.TraceFlags.NONE };
});
var m = S(function (Ut) {
  Object.defineProperty(Ut, "__esModule", { value: true });
  Ut.NonRecordingSpan = void 0;
  var bo = h();
  class Bt {
    constructor(e = bo.INVALID_SPAN_CONTEXT) {
      this._spanContext = e;
    }
    spanContext() {
      return this._spanContext;
    }
    setAttribute(e, t) {
      return this;
    }
    setAttributes(e) {
      return this;
    }
    addEvent(e, t) {
      return this;
    }
    addLink(e) {
      return this;
    }
    addLinks(e) {
      return this;
    }
    setStatus(e) {
      return this;
    }
    updateName(e) {
      return this;
    }
    end(e) {}
    isRecording() {
      return false;
    }
    recordException(e, t) {}
  }
  Ut.NonRecordingSpan = Bt;
});
var ae = S(function ($t) {
  Object.defineProperty($t, "__esModule", { value: true });
  $t.getSpanContext = $t.setSpanContext = $t.deleteSpan = $t.setSpan = $t.getActiveSpan = $t.getSpan = void 0;
  var Po = M(),
    Eo = m(),
    To = I(),
    ne = (0, Po.createContextKey)("OpenTelemetry Context Key SPAN");
  function oe(e) {
    return e.getValue(ne) || void 0;
  }
  $t.getSpan = oe;
  function No() {
    return oe(To.ContextAPI.getInstance().active());
  }
  $t.getActiveSpan = No;
  function Kt(e, t) {
    return e.setValue(ne, t);
  }
  $t.setSpan = Kt;
  function So(e) {
    return e.deleteValue(ne);
  }
  $t.deleteSpan = So;
  function Mo(e, t) {
    return Kt(e, new Eo.NonRecordingSpan(t));
  }
  $t.setSpanContext = Mo;
  function yo(e) {
    var t;
    return (t = oe(e)) === null || t === void 0 ? void 0 : t.spanContext();
  }
  $t.getSpanContext = yo;
});
var D = S(function (zt) {
  Object.defineProperty(zt, "__esModule", { value: true });
  zt.wrapSpanContext = zt.isSpanContextValid = zt.isValidSpanId = zt.isValidTraceId = void 0;
  var kt = h(),
    mo = m(),
    Do = /^([0-9a-f]{32})$/i,
    Lo = /^[0-9a-f]{16}$/i;
  function Wt(e) {
    return Do.test(e) && e !== kt.INVALID_TRACEID;
  }
  zt.isValidTraceId = Wt;
  function Yt(e) {
    return Lo.test(e) && e !== kt.INVALID_SPANID;
  }
  zt.isValidSpanId = Yt;
  function jo(e) {
    return Wt(e.traceId) && Yt(e.spanId);
  }
  zt.isSpanContextValid = jo;
  function Vo(e) {
    return new mo.NonRecordingSpan(e);
  }
  zt.wrapSpanContext = Vo;
});
var ce = S(function (Zt) {
  Object.defineProperty(Zt, "__esModule", { value: true });
  Zt.NoopTracer = void 0;
  var Bo = I(),
    Qt = ae(),
    ie = m(),
    Uo = D(),
    se = Bo.ContextAPI.getInstance();
  class Jt {
    startSpan(e, t, r = se.active()) {
      if (Boolean(t === null || t === void 0 ? void 0 : t.root)) return new ie.NonRecordingSpan();
      let o = r && (0, Qt.getSpanContext)(r);
      if (Fo(o) && (0, Uo.isSpanContextValid)(o)) return new ie.NonRecordingSpan(o);
      else return new ie.NonRecordingSpan();
    }
    startActiveSpan(e, t, r, n) {
      let o, i, s;
      if (arguments.length < 2) return;
      else if (arguments.length === 2) s = t;
      else if (arguments.length === 3) (o = t), (s = r);
      else (o = t), (i = r), (s = n);
      let u = i !== null && i !== void 0 ? i : se.active(),
        c = this.startSpan(e, o, u),
        _ = (0, Qt.setSpan)(u, c);
      return se.with(_, s, void 0, c);
    }
  }
  Zt.NoopTracer = Jt;
  function Fo(e) {
    return (
      typeof e === "object" &&
      typeof e.spanId === "string" &&
      typeof e.traceId === "string" &&
      typeof e.traceFlags === "number"
    );
  }
});
var ue = S(function (rr) {
  Object.defineProperty(rr, "__esModule", { value: true });
  rr.ProxyTracer = void 0;
  var Ko = ce(),
    $o = new Ko.NoopTracer();
  class tr {
    constructor(e, t, r, n) {
      (this._provider = e), (this.name = t), (this.version = r), (this.options = n);
    }
    startSpan(e, t, r) {
      return this._getTracer().startSpan(e, t, r);
    }
    startActiveSpan(e, t, r, n) {
      let o = this._getTracer();
      return Reflect.apply(o.startActiveSpan, o, arguments);
    }
    _getTracer() {
      if (this._delegate) return this._delegate;
      let e = this._provider.getDelegateTracer(this.name, this.version, this.options);
      if (!e) return $o;
      return (this._delegate = e), this._delegate;
    }
  }
  rr.ProxyTracer = tr;
});
var sr = S(function (ar) {
  Object.defineProperty(ar, "__esModule", { value: true });
  ar.NoopTracerProvider = void 0;
  var Xo = ce();
  class or {
    getTracer(e, t, r) {
      return new Xo.NoopTracer();
    }
  }
  ar.NoopTracerProvider = or;
});
var le = S(function (ur) {
  Object.defineProperty(ur, "__esModule", { value: true });
  ur.ProxyTracerProvider = void 0;
  var ko = ue(),
    Wo = sr(),
    Yo = new Wo.NoopTracerProvider();
  class cr {
    getTracer(e, t, r) {
      var n;
      return (n = this.getDelegateTracer(e, t, r)) !== null && n !== void 0 ? n : new ko.ProxyTracer(this, e, t, r);
    }
    getDelegate() {
      var e;
      return (e = this._delegate) !== null && e !== void 0 ? e : Yo;
    }
    setDelegate(e) {
      this._delegate = e;
    }
    getDelegateTracer(e, t, r) {
      var n;
      return (n = this._delegate) === null || n === void 0 ? void 0 : n.getTracer(e, t, r);
    }
  }
  ur.ProxyTracerProvider = cr;
});
var pr = S(function (_r) {
  Object.defineProperty(_r, "__esModule", { value: true });
  _r.SamplingDecision = void 0;
  var zo;
  (function (e) {
    (e[(e.NOT_RECORD = 0)] = "NOT_RECORD"),
      (e[(e.RECORD = 1)] = "RECORD"),
      (e[(e.RECORD_AND_SAMPLED = 2)] = "RECORD_AND_SAMPLED");
  })((zo = _r.SamplingDecision || (_r.SamplingDecision = {})));
});
var dr = S(function (gr) {
  Object.defineProperty(gr, "__esModule", { value: true });
  gr.SpanKind = void 0;
  var Ho;
  (function (e) {
    (e[(e.INTERNAL = 0)] = "INTERNAL"),
      (e[(e.SERVER = 1)] = "SERVER"),
      (e[(e.CLIENT = 2)] = "CLIENT"),
      (e[(e.PRODUCER = 3)] = "PRODUCER"),
      (e[(e.CONSUMER = 4)] = "CONSUMER");
  })((Ho = gr.SpanKind || (gr.SpanKind = {})));
});
var Or = S(function (fr) {
  Object.defineProperty(fr, "__esModule", { value: true });
  fr.SpanStatusCode = void 0;
  var Qo;
  (function (e) {
    (e[(e.UNSET = 0)] = "UNSET"), (e[(e.OK = 1)] = "OK"), (e[(e.ERROR = 2)] = "ERROR");
  })((Qo = fr.SpanStatusCode || (fr.SpanStatusCode = {})));
});
var br = S(function (xr) {
  Object.defineProperty(xr, "__esModule", { value: true });
  xr.validateValue = xr.validateKey = void 0;
  var de = "[_0-9a-z-*/]",
    Jo = `[a-z]${de}{0,255}`,
    Zo = `[a-z0-9]${de}{0,240}@[a-z]${de}{0,13}`,
    ea = new RegExp(`^(?:${Jo}|${Zo})$`),
    ta = /^[ -~]{0,255}[!-~]$/,
    ra = /,|=/;
  function na(e) {
    return ea.test(e);
  }
  xr.validateKey = na;
  function oa(e) {
    return ta.test(e) && !ra.test(e);
  }
  xr.validateValue = oa;
});
var yr = S(function (Sr) {
  Object.defineProperty(Sr, "__esModule", { value: true });
  Sr.TraceStateImpl = void 0;
  var Pr = br(),
    Er = 32,
    ia = 512,
    Tr = ",",
    Nr = "=";
  class fe {
    constructor(e) {
      if (((this._internalState = new Map()), e)) this._parse(e);
    }
    set(e, t) {
      let r = this._clone();
      if (r._internalState.has(e)) r._internalState.delete(e);
      return r._internalState.set(e, t), r;
    }
    unset(e) {
      let t = this._clone();
      return t._internalState.delete(e), t;
    }
    get(e) {
      return this._internalState.get(e);
    }
    serialize() {
      return this._keys()
        .reduce((e, t) => (e.push(t + Nr + this.get(t)), e), [])
        .join(Tr);
    }
    _parse(e) {
      if (e.length > ia) return;
      if (
        ((this._internalState = e
          .split(Tr)
          .reverse()
          .reduce((t, r) => {
            let n = r.trim(),
              o = n.indexOf(Nr);
            if (o !== -1) {
              let i = n.slice(0, o),
                s = n.slice(o + 1, r.length);
              if ((0, Pr.validateKey)(i) && (0, Pr.validateValue)(s)) t.set(i, s);
            }
            return t;
          }, new Map())),
        this._internalState.size > Er)
      )
        this._internalState = new Map(Array.from(this._internalState.entries()).reverse().slice(0, Er));
    }
    _keys() {
      return Array.from(this._internalState.keys()).reverse();
    }
    _clone() {
      let e = new fe();
      return (e._internalState = new Map(this._internalState)), e;
    }
  }
  Sr.TraceStateImpl = fe;
});
var Cr = S(function (Ir) {
  Object.defineProperty(Ir, "__esModule", { value: true });
  Ir.createTraceState = void 0;
  var sa = yr();
  function ca(e) {
    return new sa.TraceStateImpl(e);
  }
  Ir.createTraceState = ca;
});
var mr = S(function (Rr) {
  Object.defineProperty(Rr, "__esModule", { value: true });
  Rr.context = void 0;
  var ua = I();
  Rr.context = ua.ContextAPI.getInstance();
});
var jr = S(function (Dr) {
  Object.defineProperty(Dr, "__esModule", { value: true });
  Dr.diag = void 0;
  var la = O();
  Dr.diag = la.DiagAPI.instance();
});
var qr = S(function (Vr) {
  Object.defineProperty(Vr, "__esModule", { value: true });
  Vr.NOOP_METER_PROVIDER = Vr.NoopMeterProvider = void 0;
  var _a = z();
  class Oe {
    getMeter(e, t, r) {
      return _a.NOOP_METER;
    }
  }
  Vr.NoopMeterProvider = Oe;
  Vr.NOOP_METER_PROVIDER = new Oe();
});
var Fr = S(function (Br) {
  Object.defineProperty(Br, "__esModule", { value: true });
  Br.MetricsAPI = void 0;
  var ga = qr(),
    xe = f(),
    Gr = O(),
    ve = "metrics";
  class be {
    constructor() {}
    static getInstance() {
      if (!this._instance) this._instance = new be();
      return this._instance;
    }
    setGlobalMeterProvider(e) {
      return (0, xe.registerGlobal)(ve, e, Gr.DiagAPI.instance());
    }
    getMeterProvider() {
      return (0, xe.getGlobal)(ve) || ga.NOOP_METER_PROVIDER;
    }
    getMeter(e, t, r) {
      return this.getMeterProvider().getMeter(e, t, r);
    }
    disable() {
      (0, xe.unregisterGlobal)(ve, Gr.DiagAPI.instance());
    }
  }
  Br.MetricsAPI = be;
});
var Xr = S(function (Kr) {
  Object.defineProperty(Kr, "__esModule", { value: true });
  Kr.metrics = void 0;
  var da = Fr();
  Kr.metrics = da.MetricsAPI.getInstance();
});
var zr = S(function (Wr) {
  Object.defineProperty(Wr, "__esModule", { value: true });
  Wr.NoopTextMapPropagator = void 0;
  class kr {
    inject(e, t) {}
    extract(e, t) {
      return e;
    }
    fields() {
      return [];
    }
  }
  Wr.NoopTextMapPropagator = kr;
});
var Zr = S(function (Qr) {
  Object.defineProperty(Qr, "__esModule", { value: true });
  Qr.deleteBaggage = Qr.setBaggage = Qr.getActiveBaggage = Qr.getBaggage = void 0;
  var fa = I(),
    Oa = M(),
    Pe = (0, Oa.createContextKey)("OpenTelemetry Baggage Key");
  function Hr(e) {
    return e.getValue(Pe) || void 0;
  }
  Qr.getBaggage = Hr;
  function xa() {
    return Hr(fa.ContextAPI.getInstance().active());
  }
  Qr.getActiveBaggage = xa;
  function va(e, t) {
    return e.setValue(Pe, t);
  }
  Qr.setBaggage = va;
  function ba(e) {
    return e.deleteValue(Pe);
  }
  Qr.deleteBaggage = ba;
});
var on = S(function (rn) {
  Object.defineProperty(rn, "__esModule", { value: true });
  rn.PropagationAPI = void 0;
  var Ee = f(),
    Na = zr(),
    en = Q(),
    L = Zr(),
    Sa = q(),
    tn = O(),
    Te = "propagation",
    Ma = new Na.NoopTextMapPropagator();
  class Ne {
    constructor() {
      (this.createBaggage = Sa.createBaggage),
        (this.getBaggage = L.getBaggage),
        (this.getActiveBaggage = L.getActiveBaggage),
        (this.setBaggage = L.setBaggage),
        (this.deleteBaggage = L.deleteBaggage);
    }
    static getInstance() {
      if (!this._instance) this._instance = new Ne();
      return this._instance;
    }
    setGlobalPropagator(e) {
      return (0, Ee.registerGlobal)(Te, e, tn.DiagAPI.instance());
    }
    inject(e, t, r = en.defaultTextMapSetter) {
      return this._getGlobalPropagator().inject(e, t, r);
    }
    extract(e, t, r = en.defaultTextMapGetter) {
      return this._getGlobalPropagator().extract(e, t, r);
    }
    fields() {
      return this._getGlobalPropagator().fields();
    }
    disable() {
      (0, Ee.unregisterGlobal)(Te, tn.DiagAPI.instance());
    }
    _getGlobalPropagator() {
      return (0, Ee.getGlobal)(Te) || Ma;
    }
  }
  rn.PropagationAPI = Ne;
});
var cn = S(function (an) {
  Object.defineProperty(an, "__esModule", { value: true });
  an.propagation = void 0;
  var ya = on();
  an.propagation = ya.PropagationAPI.getInstance();
});
var dn = S(function (pn) {
  Object.defineProperty(pn, "__esModule", { value: true });
  pn.TraceAPI = void 0;
  var Se = f(),
    un = le(),
    ln = D(),
    P = ae(),
    _n = O(),
    Me = "trace";
  class ye {
    constructor() {
      (this._proxyTracerProvider = new un.ProxyTracerProvider()),
        (this.wrapSpanContext = ln.wrapSpanContext),
        (this.isSpanContextValid = ln.isSpanContextValid),
        (this.deleteSpan = P.deleteSpan),
        (this.getSpan = P.getSpan),
        (this.getActiveSpan = P.getActiveSpan),
        (this.getSpanContext = P.getSpanContext),
        (this.setSpan = P.setSpan),
        (this.setSpanContext = P.setSpanContext);
    }
    static getInstance() {
      if (!this._instance) this._instance = new ye();
      return this._instance;
    }
    setGlobalTracerProvider(e) {
      let t = (0, Se.registerGlobal)(Me, this._proxyTracerProvider, _n.DiagAPI.instance());
      if (t) this._proxyTracerProvider.setDelegate(e);
      return t;
    }
    getTracerProvider() {
      return (0, Se.getGlobal)(Me) || this._proxyTracerProvider;
    }
    getTracer(e, t) {
      return this.getTracerProvider().getTracer(e, t);
    }
    disable() {
      (0, Se.unregisterGlobal)(Me, _n.DiagAPI.instance()), (this._proxyTracerProvider = new un.ProxyTracerProvider());
    }
  }
  pn.TraceAPI = ye;
});
var xn = S(function (fn) {
  Object.defineProperty(fn, "__esModule", { value: true });
  fn.trace = void 0;
  var Ia = dn();
  fn.trace = Ia.TraceAPI.getInstance();
});
var Li = S(function (a) {
  Object.defineProperty(a, "__esModule", { value: true });
  a.trace =
    a.propagation =
    a.metrics =
    a.diag =
    a.context =
    a.INVALID_SPAN_CONTEXT =
    a.INVALID_TRACEID =
    a.INVALID_SPANID =
    a.isValidSpanId =
    a.isValidTraceId =
    a.isSpanContextValid =
    a.createTraceState =
    a.TraceFlags =
    a.SpanStatusCode =
    a.SpanKind =
    a.SamplingDecision =
    a.ProxyTracerProvider =
    a.ProxyTracer =
    a.defaultTextMapSetter =
    a.defaultTextMapGetter =
    a.ValueType =
    a.createNoopMeter =
    a.DiagLogLevel =
    a.DiagConsoleLogger =
    a.ROOT_CONTEXT =
    a.createContextKey =
    a.baggageEntryMetadataFromString =
      void 0;
  var Aa = q();
  Object.defineProperty(a, "baggageEntryMetadataFromString", {
    enumerable: true,
    get: function () {
      return Aa.baggageEntryMetadataFromString;
    },
  });
  var vn = M();
  Object.defineProperty(a, "createContextKey", {
    enumerable: true,
    get: function () {
      return vn.createContextKey;
    },
  });
  Object.defineProperty(a, "ROOT_CONTEXT", {
    enumerable: true,
    get: function () {
      return vn.ROOT_CONTEXT;
    },
  });
  var Ca = gt();
  Object.defineProperty(a, "DiagConsoleLogger", {
    enumerable: true,
    get: function () {
      return Ca.DiagConsoleLogger;
    },
  });
  var Ra = A();
  Object.defineProperty(a, "DiagLogLevel", {
    enumerable: true,
    get: function () {
      return Ra.DiagLogLevel;
    },
  });
  var ha = z();
  Object.defineProperty(a, "createNoopMeter", {
    enumerable: true,
    get: function () {
      return ha.createNoopMeter;
    },
  });
  var ma = Mt();
  Object.defineProperty(a, "ValueType", {
    enumerable: true,
    get: function () {
      return ma.ValueType;
    },
  });
  var bn = Q();
  Object.defineProperty(a, "defaultTextMapGetter", {
    enumerable: true,
    get: function () {
      return bn.defaultTextMapGetter;
    },
  });
  Object.defineProperty(a, "defaultTextMapSetter", {
    enumerable: true,
    get: function () {
      return bn.defaultTextMapSetter;
    },
  });
  var Da = ue();
  Object.defineProperty(a, "ProxyTracer", {
    enumerable: true,
    get: function () {
      return Da.ProxyTracer;
    },
  });
  var La = le();
  Object.defineProperty(a, "ProxyTracerProvider", {
    enumerable: true,
    get: function () {
      return La.ProxyTracerProvider;
    },
  });
  var ja = pr();
  Object.defineProperty(a, "SamplingDecision", {
    enumerable: true,
    get: function () {
      return ja.SamplingDecision;
    },
  });
  var Va = dr();
  Object.defineProperty(a, "SpanKind", {
    enumerable: true,
    get: function () {
      return Va.SpanKind;
    },
  });
  var wa = Or();
  Object.defineProperty(a, "SpanStatusCode", {
    enumerable: true,
    get: function () {
      return wa.SpanStatusCode;
    },
  });
  var qa = re();
  Object.defineProperty(a, "TraceFlags", {
    enumerable: true,
    get: function () {
      return qa.TraceFlags;
    },
  });
  var Ga = Cr();
  Object.defineProperty(a, "createTraceState", {
    enumerable: true,
    get: function () {
      return Ga.createTraceState;
    },
  });
  var Ie = D();
  Object.defineProperty(a, "isSpanContextValid", {
    enumerable: true,
    get: function () {
      return Ie.isSpanContextValid;
    },
  });
  Object.defineProperty(a, "isValidTraceId", {
    enumerable: true,
    get: function () {
      return Ie.isValidTraceId;
    },
  });
  Object.defineProperty(a, "isValidSpanId", {
    enumerable: true,
    get: function () {
      return Ie.isValidSpanId;
    },
  });
  var Ae = h();
  Object.defineProperty(a, "INVALID_SPANID", {
    enumerable: true,
    get: function () {
      return Ae.INVALID_SPANID;
    },
  });
  Object.defineProperty(a, "INVALID_TRACEID", {
    enumerable: true,
    get: function () {
      return Ae.INVALID_TRACEID;
    },
  });
  Object.defineProperty(a, "INVALID_SPAN_CONTEXT", {
    enumerable: true,
    get: function () {
      return Ae.INVALID_SPAN_CONTEXT;
    },
  });
  var Pn = mr();
  Object.defineProperty(a, "context", {
    enumerable: true,
    get: function () {
      return Pn.context;
    },
  });
  var En = jr();
  Object.defineProperty(a, "diag", {
    enumerable: true,
    get: function () {
      return En.diag;
    },
  });
  var Tn = Xr();
  Object.defineProperty(a, "metrics", {
    enumerable: true,
    get: function () {
      return Tn.metrics;
    },
  });
  var Nn = cn();
  Object.defineProperty(a, "propagation", {
    enumerable: true,
    get: function () {
      return Nn.propagation;
    },
  });
  var Sn = xn();
  Object.defineProperty(a, "trace", {
    enumerable: true,
    get: function () {
      return Sn.trace;
    },
  });
  a.default = { context: Pn.context, diag: En.diag, metrics: Tn.metrics, propagation: Nn.propagation, trace: Sn.trace };
});
export { Li };
