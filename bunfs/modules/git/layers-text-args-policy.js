...
function e(s, t) {
  return { type: "text", value: C2e(s, t) };
}
var u = async (s, t) => {
  ...
  if (o === void 0 || o === "")
    return e(
      "env-missing",
      `${PUn} is not set. This command is the deterministic entry point for sessions launched with an environment-delivered workflow script; it has no interactive use.`,
    );
  let i = Uke();
  ...
  if (r !== void 0 && r !== "") {
    if (r.length > dm) return e("args-too-large", `${otn} exceeds ${dm} bytes.`);
    try {
      l = JSON.parse(r);
    } catch (n) {
      return e("args-parse", `${otn} is not valid JSON: ${n instanceof Error ? n.message : String(n)}`);
    }
  }
  return { type: "text", value: (await trt({ script: o, args: l, telemetrySource: "remote_env", context: t })).line };
};
