...
// Version: 2.1.252
import { Mye, S7, SFt, jan, ydt, R4e, Wan, zIe } from "/$bunfs/root/chunk-zze8764r.js";
...
import { fa } from "/$bunfs/root/chunk-2n1hsggr.js";
import { dK, K4, Svt, wvt, WZe, Tvt } from "/$bunfs/root/chunk-87pfeb5g.js";
...
async function et(t, n) {
  let { stdout: i, code: r } = await qe(it(), w(["ls-files", "--stage", "-z", "--", ...n]), {
    ...E(t),
    maxBuffer: 1e7,
  });
  if (r !== 0) return null;
  ...
  let a = te(Array.from(e.values(), (o) => o.oid)),
    l = await qe(it(), w(["cat-file", "--batch-check=%(objectname) %(objectsize)"]), {
      ...E(t),
      maxBuffer: 1e7,
      input:
        a.join(`
...
async function nt(t, n) {
  ...
  try {
    ...
    let s = await wvt(i, r, e);
    if (s === void 0) return { kind: "restricted" };
    if (((e = s), !_T(e, n, "read").allowed || fa(e, n, "read", "ask") !== null)) return { kind: "restricted" };
    if (K4(e, n.trustedNetworkDirectories) !== void 0) return { kind: "restricted" };
    if (!(await WZe(e, n))) return { kind: "restricted" };
    let a = await Tvt(i, D, r.size);
...
async function H(t, n) {
  ...
  return i !== void 0 && K4(i, n.trustedNetworkDirectories) === void 0 && (await WZe(i, n));
}
async function rt(t, n) {
  ...
  for (let a = 0; a + 2 < s.length; a += 3) {
    ...
    let c = e.get(l);
    if (!c)
      (c = { text: "unspecified", eol: "unspecified", filter: "unspecified", workingTreeEncoding: "unspecified" }),
        e.set(l, c);
    if (f === "text") c.text = o;
...
function ct(t, n, i) {
  let r = Mye("a", "b", t, n, "", "", { context: 3, maxEditLength: V, timeout: i });
  if (!r) return null;
  return r.hunks
    .map((e) => ({
      oldStart: e.oldStart,
...
async function VKt(t, n, i = Z) {
  ...
  let p = null;
  if (o) {
    let u = await qe(it(), w(["--no-optional-locks", "-c", "diff.relative=false", "diff", "--numstat"]), {
      ...E(s),
      maxBuffer: 1e7,
    });
  ...
  for (let u of f) {
    ...
    let m = z(s, u),
      y = dK(m, m, n.trustedNetworkDirectories);
    if (!y.ok) {
      S.push(u);
      continue;
    }
    if (!y.pathsToCheck.every((d) => _T(d, n, "read").allowed && fa(d, n, "read", "ask") === null)) {
      S.push(u);
      continue;
    }
    let h = c.get(u);
