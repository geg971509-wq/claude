...
import { nF } from "/$bunfs/root/chunk-zze8764r.js";
...
function MQ(r) {
  ...
  if (r.every((t) => t.type === n.type))
    switch (n.type) {
      case "local_bash": {
        let t = Q(r, (s) => s.type === "local_bash" && s.kind === "monitor"),
          o = e - t,
          a = [];
      ...
      case "in_process_teammate": {
        let t = new Set(r.map((o) => (o.type === "in_process_teammate" ? o.identity.teamName : ""))).size;
      ...
      case "remote_agent": {
        if (e === 1 && n.isUltraplan)
          switch (n.ultraplanPhase) {
            case "plan_ready":
              return `${hy} ultraplan ready`;
            case "needs_input":
              return `${Mf} ultraplan needs your input`;
            default:
              return `${Mf} ultraplan`;
          }
        if (r.every((t) => t.type === "remote_agent" && t.remoteTaskType === "remote-workflow"))
          return e === 1 ? `${Mf} 1 remote dynamic workflow` : `${Mf} ${e} remote dynamic workflows`;
        return e === 1 ? `${Mf} 1 cloud session` : `${Mf} ${e} cloud sessions`;
      }
      case "local_workflow":
        return e === 1 ? "1 background dynamic workflow" : `${e} background dynamic workflows`;
      case "monitor_mcp":
      case "monitor_ws": {
        if (r.every(nF)) return e === 1 ? "1 Artifact comment monitor" : `${e} Artifact comment monitors`;
