...
import { My, Ua } from "/$bunfs/root/chunk-8ktvd0rh.js";
import { y, g } from "/$bunfs/root/chunk-ca80fke8.js";
import { o, t, HS } from "/$bunfs/root/chunk-he2phymk.js";
import { HL } from "/$bunfs/root/chunk-kez2zndg.js";
import { wc } from "/$bunfs/root/chunk-f9km17e9.js";
import { Con } from "/$bunfs/root/chunk-tmxtk606.js";
...
import { We, A, qn, z, C, u, F } from "/$bunfs/root/chunk-twm95mhz.js";
import { jYn } from "/$bunfs/root/chunk-fveqn4y7.js";
import { OM } from "/$bunfs/root/chunk-pjs8szxf.js";
...
function fo(Rt) {
  let ne = _(9),
    { children: G, color: De } = Rt;
  if (Ua()) {
    let H;
    if (ne[0] !== G)
      (H = e(o, { flexDirection: "column", paddingX: Px, flexShrink: 0, children: G })), (ne[0] = G), (ne[1] = H);
    else H = ne[1];
    return H;
  }
  let H;
  if (ne[2] !== De) (H = e(ql, { color: De })), (ne[2] = De), (ne[3] = H);
  else H = ne[3];
  let oe;
  if (ne[4] !== G) (oe = e(o, { flexDirection: "column", paddingX: lv, children: G })), (ne[4] = G), (ne[5] = oe);
  else oe = ne[5];
  let Ue;
  if (ne[6] !== H || ne[7] !== oe)
    (Ue = r(o, { flexDirection: "column", paddingTop: 1, children: [H, oe] })), (ne[6] = H), (ne[7] = oe), (ne[8] = Ue);
  else Ue = ne[8];
  return Ue;
}
F();
...
var Ye = {
  ...
  ...{},
  "confirm:toggleExplanation": { description: "explanation" },
...
function Z(Bt) {
  let ie = _(18),
    ...
    Ce = Je === void 0 ? Pe : Je,
    P = wc(),
    { focusManager: Y, rootNode: It } = We(HL),
    [tick, At] = u(0),
    Qe,
    Ve;
  if (ie[0] !== P || ie[1] !== Y)
  ...
  A(Qe, Ve);
  const Me = Y?.activeElement ?? null,
    ve = Kt?.current ?? It ?? null;
  let Ze;
  if (ie[4] !== P || ie[5] !== Ce || ie[6] !== we || ie[7] !== Ne || ie[8] !== Ee || ie[9] !== Me || ie[10] !== ve)
    (Ze = Te({ ctx: P, activeElement: Me, boundaryNode: ve, order: Ee, omit: Ne, max: Ce, maxWidth: we })),
      (ie[4] = P),
      (ie[5] = Ce),
      (ie[6] = we),
      (ie[7] = Ne),
  ...
  if (ie[12] !== x.fellBack || ie[13] !== x.hadEntryWithoutDescription)
    (et = () => {
      ...
      if (V === "ok") y("keybinding_auto_hints");
...
function Te({ ctx: s, activeElement: R, boundaryNode: k, order: B, omit: S, max: N, maxWidth: a }) {
  ...
  while (p) {
    ...
    if (i)
      for (let m of i.entriesRef.current) {
  ...
  for (let n of l) {
    ...
    if (w.has(i)) continue;
    let m = jYn(n.action, n.scope ? [n.scope] : [], s.bindings);
    if (!m) continue;
    w.add(i), b.push({ action: n.action, text: `${OM(m)} ${i}`, depth: n.depth });
  }
  let I = new Map();
  (B ?? []).forEach((n, i) => I.set(n, i)),
    b.sort((n, i) => {
      let m = I.get(n.action) ?? 1 / 0,
        U = I.get(i.action) ?? 1 / 0;
      if (m !== U) return m - U;
  ...
  for (;;) {
    ...
    for (let U of b) {
      if (f.length >= N) break;
      let Re = (f.length === 0 ? 0 : J) + se(U.text);
      if (a !== void 0 && n + Re + L > a && f.length > 0) break;
      f.push(U.text), (n += Re);
    }
    let i = b.length - f.length;
    if (a === void 0 || i === 0) break;
    let m = se(`${O}+${i} more`);
    if (m <= L) break;
    L = m;
  }
...
function me(on) {
  let h = _(43),
    ...
    de;
  if (h[0] !== X || h[1] !== Be)
    (de = X ? [{ action: "confirm:no", run: Be, hint: "cancel" }] : []), (h[0] = X), (h[1] = Be), (h[2] = de);
  else de = h[2];
  let pt;
  if (h[3] !== Ke || h[4] !== de) (pt = [...de, ...Ke]), (h[3] = Ke), (h[4] = de), (h[5] = pt);
  else pt = h[5];
  let _e = pt,
    j = We(My),
    Ie = C(null),
  ...
  let Ae = le + 1 + (pe ? 0 : 2),
    at;
  if (h[9] !== j || h[10] !== Ae)
    (at = j ? { ...j, rows: Math.max(0, j.rows - Ae) } : null), (h[9] = j), (h[10] = Ae), (h[11] = at);
  else at = h[11];
  let He = at,
    ft;
  if (h[12] !== q || h[13] !== T)
  ...
  let Fe = ft,
    $e = typeof T === "function" || q.pending || T != null,
    mt;
  if (h[15] !== Fe || h[16] !== pe || h[17] !== X || h[18] !== $e)
    (mt =
      !pe &&
      e(o, {
        marginTop: 1,
  ...
  let ge;
  if (h[26] !== ae || h[27] !== ue)
    (ge = r(o, { ref: Ie, flexDirection: "column", children: [ae, ue] })), (h[26] = ae), (h[27] = ue), (h[28] = ge);
  else ge = h[28];
  let he;
  if (h[29] !== He || h[30] !== je) (he = e(My, { value: He, children: je })), (h[29] = He), (h[30] = je), (h[31] = he);
  else he = h[31];
  let ye;
  if (h[32] !== ge || h[33] !== he)
    (ye = r(o, { flexDirection: "column", gap: 1, children: [ge, he] })), (h[32] = ge), (h[33] = he), (h[34] = ye);
  else ye = h[34];
  let ut;
  if (h[35] !== _e || h[36] !== Xe || h[37] !== ye || h[38] !== Le)
    (ut = r(GZ, {
      ref: ct,
      scope: "Confirmation",
      claimFocus: true,
  ...
  let gt;
  if (h[40] !== W || h[41] !== be) (gt = e(fo, { color: W, children: be })), (h[40] = W), (h[41] = be), (h[42] = gt);
  else gt = h[42];
  return gt;
}
export { lv, Px, cv, fo, me };
