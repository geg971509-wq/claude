// @bun @bytecode
// Claude Code is a Beta product per Anthropic's Commercial Terms of Service.
// By using Claude Code, you agree that all code acceptance or rejection decisions you make,
// and the associated conversations in context, constitute Feedback under Anthropic's Commercial Terms,
// and may be used to improve Anthropic's products, including training models.
// You are responsible for reviewing any code suggestions before use.

// (c) Anthropic PBC. All rights reserved. Use is subject to the Legal Agreements outlined here: https://code.claude.com/docs/en/legal-and-compliance.

// Version: 2.1.252
import { Cu } from "/$bunfs/root/chunk-f9h0bg01.js";
import { PM } from "/$bunfs/root/chunk-83h0j7w2.js";
import { es } from "/$bunfs/root/chunk-zze8764r.js";
import { _ } from "/$bunfs/root/chunk-haan6f8z.js";
import { o, t } from "/$bunfs/root/chunk-he2phymk.js";
import { At } from "/$bunfs/root/chunk-ht28m404.js";
import { Ee } from "/$bunfs/root/chunk-rp246vnd.js";
import { Qme } from "/$bunfs/root/chunk-vrr898f6.js";
import { me } from "/$bunfs/root/chunk-489c3cyq.js";
import { wn } from "/$bunfs/root/chunk-et3g0deq.js";
import { xm } from "/$bunfs/root/chunk-ddrmb5hs.js";
import { e, r } from "/$bunfs/root/chunk-8s7kcbyw.js";
function j(to) {
  return { ...to, cacheMissAckedAtOutputTokens: Cu() };
}
var P = 6;
function lq(oo) {
  let f = _(25),
    { kind: eo, model: v, effort: d, hookReason: n, onConfirm: x, onCancel: i } = oo,
    { columns: y } = Ee(),
    C = At(),
    m = eo === "model",
    k = m ? "Switch model?" : "Change effort level?",
    A = m ? "model" : "effort level",
    I;
  if (f[0] !== d || f[1] !== m || f[2] !== v)
    (I = m ? xm(v) : d !== void 0 ? PM(d) : "auto"), (f[0] = d), (f[1] = m), (f[2] = v), (f[3] = I);
  else I = f[3];
  let a = I,
    O;
  if (f[4] !== n || f[5] !== x || f[6] !== C)
    (O = function c() {
      if (n === void 0) C(j);
      x();
    }),
      (f[4] = n),
      (f[5] = x),
      (f[6] = C),
      (f[7] = O);
  else O = f[7];
  let c = O;
  const S =
    n !== void 0
      ? "A PreModelSwitch hook asked you to confirm"
      : "Your next response will be slower and use more tokens";
  let u;
  if (f[8] !== y || f[9] !== n || f[10] !== a || f[11] !== A)
    (u =
      n !== void 0
        ? e(t, { children: Qme(es(n).text, y, P + 1) })
        : r(t, {
            children: [
              "This conversation is cached for the current ",
              A,
              ". Switching to",
              " ",
              e(t, { bold: true, children: a }),
              " means the full history gets re-read on your next message.",
            ],
          })),
      (f[8] = y),
      (f[9] = n),
      (f[10] = a),
      (f[11] = A),
      (f[12] = u);
  else u = f[12];
  const M = `Yes, switch to ${a}`;
  let p;
  if (f[13] !== c || f[14] !== i || f[15] !== M)
    (p = e(wn, { confirmLabel: M, cancelLabel: "No, go back", onConfirm: c, onCancel: i })),
      (f[13] = c),
      (f[14] = i),
      (f[15] = M),
      (f[16] = p);
  else p = f[16];
  let h;
  if (f[17] !== u || f[18] !== p)
    (h = r(o, { flexDirection: "column", gap: 1, marginBottom: 1, children: [u, p] })),
      (f[17] = u),
      (f[18] = p),
      (f[19] = h);
  else h = f[19];
  let R;
  if (f[20] !== i || f[21] !== S || f[22] !== h || f[23] !== k)
    (R = e(me, { title: k, subtitle: S, color: "warning", onCancel: i, hideInputGuide: true, children: h })),
      (f[20] = i),
      (f[21] = S),
      (f[22] = h),
      (f[23] = k),
      (f[24] = R);
  else R = f[24];
  return R;
}
export { lq };
