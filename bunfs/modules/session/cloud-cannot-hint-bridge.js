...
import { QEe, ybt, VMe } from "/$bunfs/root/chunk-8tgj5dp2.js";
import { pTe } from "/$bunfs/root/chunk-qnrh4abv.js";
...
import { Ad, Tr } from "/$bunfs/root/chunk-492vgtnr.js";
...
import { ore, A3 } from "/$bunfs/root/chunk-hz299nfa.js";
...
import { yIe } from "/$bunfs/root/chunk-zze8764r.js";
...
import { C3, A9t, uh, vV, Lde } from "/$bunfs/root/chunk-z9jgt14c.js";
...
import { Xfn, Kgt, Zfn } from "/$bunfs/root/chunk-nfmyt8cw.js";
...
import { k2t } from "/$bunfs/root/chunk-z6m3k8w5.js";
import { r8 } from "/$bunfs/root/chunk-g6s9bnn0.js";
...
async function oe(e, s) {
  let { prepareApiRequest: o, getOAuthHeaders: y, CCR_BYOC_BETA: R } = await import("/$bunfs/root/chunk-f3y3fqnb.js"),
    { getOauthConfig: P } = await import("/$bunfs/root/chunk-79hv0fh4.js"),
    T,
    w;
  try {
  ...
  let f = A3(),
    k = `${P().BASE_API_URL}${f ? "/v1/code/sessions" : "/v1/sessions"}`,
    b = { ...y(T), ...(!f && { "anthropic-beta": R, "x-organization-uuid": w }), "User-Agent": Ka() },
    p = await uh();
  ...
  for (let c = 0; c < x; c++) {
    ...
    if (E) _.set(f ? "cursor" : "after_id", E);
    let h = _.toString(),
      C = h ? `${k}?${h}` : k,
      r;
    try {
    ...
    for (let t of r.data.data)
      try {
        let m = "session_status" in t,
          I = m
            ? t.session_status
            : t.status === "archived"
              ? "archived"
              : t.worker_status === "running" || t.worker_status === "requires_action"
                ? t.worker_status
                : "idle";
        if (I === "archived") continue;
        let U = m ? t.id : Ad(t.id),
          D = Tr(U);
        if (u && D === u) {
          Xfn(t.title);
          continue;
        ...
        S.add(D),
          d.push({
            id: U,
            title: m ? t.title : t.title || null,
            status: I,
            updated_at: (m ? t.updated_at : t.last_event_at) ?? "",
            ...(N(t.environment_kind) && { environmentKind: t.environment_kind }),
  ...
  if (
    (n(
      `[bridge:peers] listed ${d.length} peer sessions${g ? ` \u2014 TRUNCATED at ${x} pages (more sessions exist)` : ""}`,
      g ? { level: "warn" } : void 0,
    ),
    e)
  )
...
function de(e) {
  if (!e) return "other";
  if (e.startsWith("auth:")) return "bridge_auth";
  if (e.startsWith("invalid session ID format")) return "invalid_target";
  let s = /^HTTP (\d)\d\d/.exec(e);
  if (s) return s[1] === "5" ? "bridge_http_5xx" : "bridge_http_4xx";
  if (/status code 5\d\d/.test(e)) return "bridge_http_5xx";
  if (/timeout/i.test(e)) return "timeout";
  return "other";
}
async function ue(e, s, o, y, R, P, T) {
  let { prepareApiRequest: w, getOAuthHeaders: f, CCR_BYOC_BETA: k } = await import("/$bunfs/root/chunk-f3y3fqnb.js"),
    { getOauthConfig: b } = await import("/$bunfs/root/chunk-79hv0fh4.js"),
    p,
    v;
  try {
  ...
  let d = Kgt() ?? Uz(),
    ...
    A = {
      ...
      session_id: u,
      uuid: O(),
      ...((y?.length ?? 0) > 0 && { file_attachments: y }),
    },
    { url: c, body: _ } = yIe(b().BASE_API_URL, u, [A], ore()),
    h = { ...f(p), "anthropic-beta": k, "x-organization-uuid": v, "User-Agent": Ka() },
    C = await uh();
  if (C) h["X-Trusted-Device-Token"] = C;
  let r;
  try {
  ...
  if ((B(u, r), r.status === 403 && S6(r.data, $m(r.data)) === "untrusted_device")) {
    ...
    if (i) {
      try {
        r = await st.post(c, _, {
          headers: { ...h, "X-Trusted-Device-Token": i },
          timeout: 1e4,
...
function N(e) {
  return e === "bridge" || e === "anthropic_cloud" || e === "byoc" || e === "snap";
}
function H(e) {
  return e === 200 || e === 201 || e === 204;
}
function B(e, s) {
  if (H(s.status)) return;
  let o = $m(s.data);
  n(`[bridge:peers] post to ${e} rejected: HTTP ${s.status}${o ? ` \u2014 ${Y$(o)}` : ""}`, { level: "warn" });
}
export {
  L as CLOUD_SESSION_CANNOT_SEND_HINT,
  de as classifyBridgeSendError,
  ae as isLikelyStaleBridgeError,
  oe as listBridgePeerSessions,
  ue as postInterClaudeMessage,
};
