...
import { DB } from "/$bunfs/root/chunk-zze8764r.js";
...
import { D0, NQ, YYt } from "/$bunfs/root/chunk-gce2rjvv.js";
...
import { v, _e, H, f } from "/$bunfs/root/chunk-saay52v7.js";
...
var C = m(() => {
    let n = H(_e())
      .optional()
      .catch(void 0);
    return f({ plugins: n, commands: n, agents: n, mcpServers: n, error_count: v().catch(0) });
  }),
  L = async (n, e) => {
    if (ba()) {
      let o = g1(
        "reload_plugins",
        C(),
        await Gr().sendControlRequest({ subtype: "reload_plugins" }, { signal: e.abortController.signal }),
      );
      if (!o || (o.plugins ?? o.commands ?? o.agents ?? o.mcpServers) === void 0)
...
${r(o.error_count, "error")} during load. Run /plugin on the remote for details.`;
    ...
    let d = null,
      g = l;
    try {
      let o = await D0(e.setAppState, e.storageV5, e.credentials),
        p = "",
        a = await sB(o.errors, e.storageV5);
      if (a.installed.length > 0) {
        ...
        if (s !== null && s.wouldInvalidateCache)
...
function h(n) {
  let e = [...n.added, ...n.removed],
    [t] = e,
    l = e.length === 1 && t !== void 0 ? Oo(t.split(":").slice(2).join(":") || t) : `${e.length} MCP servers`,
    i = [];
  if (e.length > 0) i.push(`changes MCP tools (${l})`);
  ...
  return (
    `This reload ${i.join(" and ")} \u2014 your next message will re-read ` +
    "the whole conversation instead of using the cache. Run /reload-plugins --force to apply."
  );
}
export { L as call, h as formatCacheWarningText };
