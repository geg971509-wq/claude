...
// Version: 2.1.252
import { we, l, $o } from "/$bunfs/root/chunk-ypdw393e.js";
import { V, n } from "/$bunfs/root/chunk-fv016jr6.js";
import { h } from "/$bunfs/root/chunk-wkxx62a2.js";
import { Bxe, gct, pee, pln, ISe, Hv } from "/$bunfs/root/chunk-zze8764r.js";
import { ni, i$ } from "/$bunfs/root/chunk-z15hpjf9.js";
import { Fc } from "/$bunfs/root/chunk-tveh0k24.js";
import { uc, LM, Vt } from "/$bunfs/root/chunk-x06p1jhb.js";
import { te } from "/$bunfs/root/chunk-wag5ye9w.js";
import * as p from "fs/promises";
import * as m from "path";
async function j0t(t, e) {
  if (e === "builtin") {
    let d = gct(t.name);
  ...
  let s = uc(e),
    o = Vt(t.source).name || t.name,
    r = s ? void 0 : (await Hv(e)).plugins.find((d) => d.name === o);
  ...
  let [a, i, u] = await Promise.all([
    ...
    y = t.lspServers ? Object.keys(t.lspServers) : (await L(t)).concat(P(t.manifest.lspServers)),
    k = y.length > 0 ? te(y) : P(r?.lspServers);
  return { commands: a, agents: i, skills: u, hooks: f, mcpServers: c, lspServers: k };
}
async function PTr(t, e, s) {
  let [o, r, a] = await Promise.all([
      Promise.all(t.skills.map((c) => w(c.path ? m.join(c.path, "SKILL.md") : void 0, b(c, s)))),
...
`),
  ...
  return { tokens: g, inventory: { ...t, skills: S(t.skills, o), agents: S(t.agents, r), commands: S(t.commands, a) } };
}
function DTr(t, e, s, o = 4) {
  if (s !== void 0 && e > 0) return Math.round((t / e) * s);
  return Fc(" ".repeat(t), o);
}
var E = 1048576;
function b(t, e) {
  if (!e) return t.name;
...
async function j(t, e) {
  if (!t) return 0;
  return ISe([{ role: "user", content: t }], [], e);
}
function P(t) {
  return [t]
    .flat()
...
async function O(t, e) {
  return LM(t) ? Bxe(t, e) : m.join(t.path, e);
}
async function C(t) {
  let e = [],
    s = new Set();
...
function v(t, e) {
  if ((n(`Failed to read plugin components from ${t}: ${l(e)}`, { level: "error" }), $o(e))) return;
  h(we(e));
}
export { j0t, PTr, DTr };
