...
import { tT, aIe, Sle, cUt, kp, nh } from "/$bunfs/root/chunk-zze8764r.js";
...
function A(Te) {
  ...
  if (j[6] !== L || j[7] !== b || j[8] !== M)
    (te = r(t, { children: [L, " ", b, M] })), (j[6] = L), (j[7] = b), (j[8] = M), (j[9] = te);
  else te = j[9];
  return te;
}
function he(Je) {
...
function Ee(S) {
  let { config: Xe } = S;
  return v(Xe.errors, "fatal").length > 0;
}
function Pe(S) {
  let { config: Ye } = S;
  return v(Ye.errors, "warning").length > 0;
}
function Re(S) {
  let { scope: ue, config: ve } = S;
  return e(K, { scope: ue, parsingErrors: v(ve.errors, "fatal"), warnings: v(ve.errors, "warning") }, ue);
}
function xe(oe, Ze) {
  return r(
    vl.Group,
...
function K(ze) {
  ...
  let ie;
  if (c[0] !== w) (ie = tT(w)), (c[0] = w), (c[1] = ie);
  else ie = c[1];
  let U = ie,
    ne;
  if (c[2] !== y || c[3] !== E)
    (ne = new Set([...y, ...E].map(he).filter(Boolean))), (c[2] = y), (c[3] = E), (c[4] = ne);
  else ne = c[4];
  let G = ne,
    se;
  if (c[5] !== U || c[6] !== G) (se = G.size === 1 ? [...G][0] : U), (c[5] = U), (c[6] = G), (c[7] = se);
  else se = c[7];
  let l = se,
    I;
  if (c[8] !== T || c[9] !== D)
    (I =
      (T || D) &&
      r(t, { color: T ? "error" : "warning", children: ["[", T ? "Failed to parse" : "Contains warnings", "]", " "] })),
      (c[8] = T),
      (c[9] = D),
      (c[10] = I);
  else I = c[10];
  let C;
  if (c[11] !== w) (C = aIe(w)), (c[11] = w), (c[12] = C);
  else C = c[12];
  let q;
  if (c[13] !== C) (q = e(t, { children: C })), (c[13] = C), (c[14] = q);
  else q = c[14];
  let B;
  ...
  if (c[26] !== l || c[27] !== E) {
    ...
    P = E.map(R);
    (c[26] = l), (c[27] = E), (c[28] = P);
  } else P = c[28];
  let R;
  if (c[31] !== J || c[32] !== P)
    (R = e(o, { marginLeft: 1, children: r(vl, { variant: "tree", children: [J, P] }) })),
      (c[31] = J),
      (c[32] = P),
      (c[33] = R);
  else R = c[33];
  let me;
  if (c[34] !== R || c[35] !== B || c[36] !== H)
    (me = r(o, { flexDirection: "column", marginTop: 1, children: [B, H, R] })),
      (c[34] = R),
      (c[35] = B),
      (c[36] = H),
      (c[37] = me);
...
function KBe() {
  ...
  if (X[0] === d) {
    let fe = [
      { scope: "user", config: kp("user") },
      { scope: "project", config: kp("project") },
      { scope: "local", config: kp("local") },
      { scope: "enterprise", config: kp("enterprise") },
    ];
    let Ke = cUt(fe.filter(Ne).map(Se));
    S = {
      scopes: re(fe, { enterpriseActive: nh(), mcpLocked: Nd("mcp"), isProjectServerApproved: ye }),
      conflicts: Ke,
    };
    X[0] = S;
  } else S = X[0];
  let { scopes: Y, conflicts: Z } = S,
    le = Y.some(Ee),
    Oe = Z.length > 0 || Y.some(Pe);
  if (!le && !Oe) {
    return null;
  }
  let de;
  if (X[1] === d) (de = e(A, { title: "MCP config diagnostics", status: le ? "error" : "warning" })), (X[1] = de);
  else de = X[1];
  let ge;
  if (X[2] === d)
    (ge = r(o, {
        ...
        e(o, {
          ...
          children: r(t, {
            ...
            children: [
              ...
              " ",
              e(ut, { url: "https://code.claude.com/docs/en/mcp", children: "https://code.claude.com/docs/en/mcp" }),
            ],
          }),
        }),
        Y.map(Re),
        Z.length > 0 &&
          r(o, {
            ...
            children: [
              ...
              e(vl, { variant: "tree", children: Z.map(xe) }),
            ],
          }),
      ],
    })),
...
function v(p, s) {
  return p.filter((m) => m.mcpErrorMetadata?.severity === s);
}
function re(p, s) {
  let m = (a, n) => {
