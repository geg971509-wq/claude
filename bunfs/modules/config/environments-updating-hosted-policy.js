...
import { H9, ID, PD } from "/$bunfs/root/chunk-dzv4k86d.js";
...
var v = "Select remote environment",
  H = "Configure environments at: https://claude.ai/code";
function ue(gn) {
  let s = _(35),
    { onDone: i } = gn,
    { storageV5: S, credentials: Z } = ge(),
    [G, ve] = u("loading"),
    Fe;
  if (s[0] === d) (Fe = []), (s[0] = Fe);
  else Fe = s[0];
    ...
    (Je = () => {
      ...
      let Se = async function Se() {
        try {
          ...
          pn(O.availableTargets),
            vn(O.selectedTarget),
            hn(O.selectedTargetSource),
            Rn(O.environmentsError),
            Sn(O.ignoredUntrustedPool),
            ve(null);
        } catch (ne) {
          let En = ne;
          if (Re) {
          ...
          let Xe = we(En);
          n(`Failed to fetch remote environments: ${Xe.message}`, { level: "error" }), Cn(Xe.message), ve(null);
        }
      };
      return (
        Se(),
    ...
    (ne = function j(Be) {
      ...
      let bn =
        ye("localSettings")?.remote?.defaultEnvironmentId !== void 0
          ? rn("localSettings", { remote: { defaultEnvironmentId: void 0 } }, void 0, S)
          : Promise.resolve({ error: null });
      let $n = rn("userSettings", { remote: { defaultEnvironmentId: ID(x) } }, void 0, S);
      let Nn = H9(x) ? "self-hosted environment" : "remote environment";
      Promise.all([bn, $n]).then(() => {
        let W = PD();
        let Tn =
          W.id !== void 0 && W.id !== ID(x) && W.source !== void 0
            ? ` \u2014 note: ${yw(W.source)} settings pin ${W.id}, which takes precedence here`
            : "";
        i(`Set default ${Nn} to ${ae.bold(x.name)} (${ID(x)})${Tn}`);
      });
    }),
      (s[5] = i),
      (s[6] = S),
  ...
  if (ee) {
    let l;
    if (s[12] !== ee) (l = r(t, { color: "error", children: ["Error: ", ee] })), (s[12] = ee), (s[13] = l);
    else l = s[13];
    let f;
    if (s[14] !== i || s[15] !== l)
      (f = e(me, { title: v, onCancel: i, children: l })), (s[14] = i), (s[15] = l), (s[16] = f);
    else f = s[16];
    return f;
  }
  if (!I) {
    let l;
    if (s[17] === d) (l = e(t, { children: "No remote environments available." })), (s[17] = l);
    else l = s[17];
    let f;
    if (s[18] !== T)
    ...
    let Oe;
    if (s[22] !== i || s[23] !== f || s[24] !== te)
      (Oe = r(me, { title: v, subtitle: H, onCancel: i, children: [l, f, te] })),
        (s[22] = i),
        (s[23] = f),
        (s[24] = te),
        (s[25] = Oe);
...
function de(Pn) {
  let p = _(27),
    {
      targets: oe,
      selectedTarget: L,
      selectedTargetSource: E,
      environmentsError: re,
      ignoredUntrustedPool: q,
      loadingState: Ee,
      onSelect: ie,
      onCancel: be,
    } = Pn,
    We;
  if (p[0] !== E) (We = E && E !== "userSettings" ? ` (from ${yw(E)} settings)` : ""), (p[0] = E), (p[1] = We);
  else We = p[1];
  let $e = We,
    je;
  ...
  let ce;
  if (p[15] !== Ee || p[16] !== ie || p[17] !== Te || p[18] !== L)
    (ce =
      Ee === "updating"
        ? e(rr, { message: "Updating\u2026" })
        : e(Ce, {
            options: Te,
            defaultValue: ID(L),
            onChange: ie,
