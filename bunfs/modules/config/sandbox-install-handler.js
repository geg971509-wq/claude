...
import { W2, G2, wTe, LOe, pt } from "/$bunfs/root/chunk-w3c6n7jh.js";
...
import { wu, a$ } from "/$bunfs/root/chunk-qekzccch.js";
...
async function p() {
  let n = s(),
    e = n
      ...
      : pt.areSandboxSettingsLockedByPolicy()
        ? {
            status: "unavailable",
            message: "Sandbox settings are locked by policy. Sandbox provisioning is managed by your administrator.",
          }
        : await rtt(G().host);
  return (
    await Bw(
...
async function x() {
  ...
  let o = pt.isStrictSandboxModeConfigured(),
    i = {
      statusVersion: 2,
      ...e,
      supported: pt.isSupportedPlatform() && pt.isPlatformInEnabledList(),
      enabled: pt.isSandboxingEnabled(),
      enabledSource: t({
        forced: wTe() || (wu() && a$()),
        policy: LOe("enabled"),
        settings: pt.isSandboxEnabledInSettings(),
      }),
      unavailableReason: pt.getSandboxUnavailableReason() ?? null,
      strictMode: o,
      strictModeSource: t({
        forced: pt.areUnsandboxedCommandsForbiddenByPolicy(),
        policy: LOe("allowUnsandboxedCommands"),
        settings: o,
      }),
      filesystemPolicy: G2(),
    };
...
function s() {
  if (D() !== "windows") return "Windows sandbox install is only available on native Windows.";
  if (!W2()) return "The Windows sandbox is not enabled on this build.";
