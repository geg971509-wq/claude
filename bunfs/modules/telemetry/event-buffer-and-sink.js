...
function i(t, e) {
  if (t.eventQueue.length >= xwr) t.eventQueue.shift(), t.droppedEventCount++;
  t.eventQueue.push(e);
}
class tlr {
  state = Iwr();
}
var Pwr = new J(() => new tlr());
function a() {
  return Pwr.of(G().host);
}
function DHr(t) {
  a().state = t;
}
...
function s(t, e) {
  let n = a().state;
  if (n.sink === null) {
    i(n, { eventName: t, metadata: e, async: false });
    return;
  }
  n.sink.logEvent(t, e);
}
async function Ss(t, e) {
  let n = a().state;
  if (n.sink === null) {
    i(n, { eventName: t, metadata: e, async: true });
    return;
  }
