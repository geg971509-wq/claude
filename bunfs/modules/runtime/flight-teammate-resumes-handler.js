...
// Version: 2.1.252
import { po, yb, et, Ave } from "/$bunfs/root/chunk-f9h0bg01.js";
...
import { Ze, It, R, l, E } from "/$bunfs/root/chunk-ypdw393e.js";
...
import { eve, Ct } from "/$bunfs/root/chunk-wkxx62a2.js";
...
import { O$ } from "/$bunfs/root/chunk-qk6zade1.js";
import {
  _bt,
  QS,
  wbn,
...
import { rb, gf, ds, na, sS, vC } from "/$bunfs/root/chunk-0g6cj4h1.js";
import { xDe, qte, Gmt } from "/$bunfs/root/chunk-z6m3k8w5.js";
...
import { Yo, W5e } from "/$bunfs/root/chunk-nf8aaj8a.js";
import "/$bunfs/root/chunk-a6xhjq7b.js";
import { Dht, fTe, mTe, Q6t, $z, gTe, Oht, HM } from "/$bunfs/root/chunk-qnrh4abv.js";
...
import { Q2t, Z2t, $g, Efn, Afn, Cfn, bI, dVe } from "/$bunfs/root/chunk-da857b4g.js";
...
import { Yfn, jwe } from "/$bunfs/root/chunk-nfmyt8cw.js";
import "/$bunfs/root/chunk-p18e2918.js";
import "/$bunfs/root/chunk-rntp2fy5.js";
import { Rf, Az } from "/$bunfs/root/chunk-5djs3wbb.js";
...
import { $ot, P0e, iZ, oje, ije, oUn, dB, xb, whe, D0e, O0e, L0e, v1 } from "/$bunfs/root/chunk-cs6e481r.js";
import { z$n } from "/$bunfs/root/chunk-q060pzpr.js";
import { E0e, C0e } from "/$bunfs/root/chunk-ac0gs4d8.js";
import { cen, fd, SL, mW, uen } from "/$bunfs/root/chunk-9492zptb.js";
import { K$n, X$n, Y$n, ien, J$n } from "/$bunfs/root/chunk-rpydgr8b.js";
...
import {
  ...
  Sen,
  rje,
} from "/$bunfs/root/chunk-qgttxht1.js";
import { mtn } from "/$bunfs/root/chunk-em2p93p3.js";
...
import { Yb, Y2t } from "/$bunfs/root/chunk-z1q2cd55.js";
...
import { Xr, uTe } from "/$bunfs/root/chunk-p8c745fb.js";
...
import { yp, Spe } from "/$bunfs/root/chunk-b40bn863.js";
...
import { i, q, f, dt, ps, N } from "/$bunfs/root/chunk-saay52v7.js";
import "/$bunfs/root/chunk-1hh0bg14.js";
import { FN, d4 } from "/$bunfs/root/chunk-manh1369.js";
...
var Pe = /^local_[0-9a-f-]{8,}$/,
  Ue = "ccd_session_mgmt",
  Be = "send_message",
  nt = `mcp__${ln(Ue)}__${ln(Be)}`,
  Ts = 1000;
function Ns(e) {
  let { tools: s, toolAliases: u } = e.options,
    o = s.filter((_) => _.mcpInfo?.serverName === Ue && _.mcpInfo.toolName === Be && _.mcpInfo.serverType === "sdk"),
    d = o.length === 1 ? o[0] : void 0;
  if (!d || no(s, d.name, u) !== d) return;
  return d;
}
class xe {
  promptIndexSeen = -1;
...
function Le(e) {
  ...
  let u = e.toolState.get(xe),
    o = Ave();
  if (o !== u.promptIndexSeen) (u.promptIndexSeen = o), (u.forwardsSincePrompt = 0);
  if ((u.forwardsSincePrompt++, u.forwardsSincePrompt > Dht.maxSelfHops))
    return { kind: "loop-paused", forwards: u.forwardsSincePrompt - 1 };
  return { tool: s };
}
async function qe({ tool: e, sessionId: s, message: u, context: o, canUseTool: d, assistantMessage: _ }) {
  let r = {
      type: "tool_use",
      id: `${o.toolUseId ?? "sendmessage"}_desktop`,
      name: e.name,
      input: { session_id: s, message: u },
      caller: { type: "direct" },
    },
    w = 0,
    B = async (...H) => {
  ...
  for await (let H of i9(r, _, B, o, () => new Date().toISOString())) {
    ...
    if (H.message.type !== "user") continue;
    let X = H.message.message.content;
    if (!Array.isArray(X)) continue;
    for (let O of X) if (O.type === "tool_result" && O.tool_use_id === r.id) C = O;
  }
  let W = w >= Ts;
  if (!C)
    return {
  ...
  return C.is_error
    ? {
        kind: "refused",
        message: t || "Claude Desktop reported an error.",
        attachments: K,
        deferredByHook: z,
        blockedWait: W,
      }
    : { kind: "sent", detail: t, attachments: K, deferredByHook: z, blockedWait: W };
}
function Cs(e) {
  let { content: s } = e;
...
async function je(e, s) {
  let u = await KA(e, s);
  return u?.taskKind === "in_process_teammate" ? u : null;
}
async function Fe({ resumableAgentId: e, prompt: s, senderName: u, meta: o, fallbackName: d, toolUseContext: _ }) {
  let r = o.name ?? d,
    w = o.teamName ?? ds(_.getAppState().teamContext);
  if (!r || !w)
    throw (
      (p("swarm_in_process_resume", "no_team_context"),
      Error("Cannot resume teammate: no team is active in this session"))
    );
  if (IH(r))
    throw (
      (p("swarm_in_process_resume", "unclaimable_name"),
      Error("Cannot resume teammate: its recorded name is a reserved recipient or agent-id-shaped name"))
    );
  let B,
    C = await lte(e, _.storageV5, { signal: _.abortController.signal }).catch((O) => {
      if (It(O)) throw O;
      throw (
        (p("swarm_in_process_resume", "transcript_load_failed"),
        new R(`Cannot resume teammate: ${l(O)}`, "Cannot resume teammate: its transcript could not be loaded"))
      );
    });
  if (_.abortController.signal.aborted) throw new Ze();
  if (C) Dee(C.messages), Aht(C.messages);
  if (!C || C.messages.length === 0) B = "no_transcript";
  let K = C ? wce(Tce(pbe(r7(C.messages, { site: "agent_resume" })))) : [],
    z = ogt(_.contentReplacementState, K, C?.contentReplacements ?? []),
    W;
  if (o.customAgentType) {
    let O = _.options.agentDefinitions.activeAgents.find((J) => J.agentType === o.customAgentType);
    if (O && c3e(O)) W = O;
    else
      (B = "agent_type_unresolved"),
        (W = {
          agentType: o.customAgentType,
          whenToUse: "",
          tools: [],
          getSystemPrompt: () => "",
  ...
  await dVe(r, (O) => bI(O.text), w, _.storageV5).catch((O) =>
    n(`[resumeInProcessTeammate] stale protocol-frame drop failed: ${O}`),
  );
  let { spawnInProcessTeammate: t } = import.meta.require("/$bunfs/root/chunk-yd197m90.js"),
    { startInProcessTeammate: H } = import.meta.require("/$bunfs/root/chunk-7amdw302.js"),
    X = await t(
      {
        name: r,
        teamName: w,
  ...
  if (!X.ok)
    throw (
      (p("swarm_in_process_resume", "spawn_failed"),
      n(`[resumeInProcessTeammate] spawn failed: ${X.error}`),
  ...
  if (
    (await Az(
      w,
      (O) => {
        let J = O.members.find((se) => se.agentId === X.agentId);
        if (J) {
          J.joinedAt = Date.now();
          return;
        }
        O.members.push({
          ...
          color: o.color,
          agentType: o.customAgentType,
          planModeRequired: o.planModeRequired,
          joinedAt: Date.now(),
      ...
      void 0,
      _.storageV5,
    ).catch((O) => n(`[resumeInProcessTeammate] team file re-add failed (ad-hoc team?): ${O}`)),
    _.agentLifecycle.setTeammate(X.agentId, {
      name: r,
      color: o.color,
      agentType: o.customAgentType,
    ...
    H({
      ...
      agentDefinition: W,
      model: o.model,
      teammateContext: X.teammateContext,
      toolUseContext: { ..._, messages: [] },
      abortController: X.abortController,
      resumeMessages: K,
      resumeReplacementState: z,
    }),
    n(`[resumeInProcessTeammate] Resumed ${X.agentId} with ${K.length} prior messages`),
    B)
  )
    g("swarm_in_process_resume", B);
  else y("swarm_in_process_resume");
  return { agentId: X.agentId, taskId: X.taskId, resumedMessageCount: K.length };
}
var Xe = "Send a message to another agent";
function We(e) {
  let s = Yo()
      ? `
| \`"worker"\` | Any agent from \`${Js}\` \u2014 subagent, another local Claude session |
...
Use \`${Js}\` to discover targets. Every row leads with the agent's \`name [ref]\` \u2014 the name IS the address; there is no separate address syntax.
...
Send the bare name \u2014 a name that exactly matches one live agent or session (on this machine, on another machine, or in the cloud) delivers directly. Append the \` [ref]\` only when the bare name is not enough \u2014 \`${Js}\` shows two rows with it, or an error asks you to disambiguate (you typed only a prefix, or a session list could not be checked). A ref you did not just read from a listing or an error will not resolve, and if the same name also names an in-process agent, the bare name always wins \u2014 use the in-process one.

A listed peer is alive and will process your message; messages enqueue and drain at the receiver's next tool round (its \`${Js}\` row says whether it is busy or idle right now). Your message arrives wrapped as \`<cross-session-message from="...">\`. **To reply to an incoming message, copy its \`from\` attribute as your \`to\`.** Cross-session messages travel between SESSIONS: if you are a subagent, your send goes out under your parent session's address, and any reply is delivered to the parent session's conversation, not to you.
...
function me(e) {
  ...
  if (fTe(e)) return "too_large";
  if (mTe(e)) return "sender_paced";
  if (e instanceof HM)
    switch (e.refusal) {
      case "wrong-endpoint":
        return "stale_socket";
  ...
  if (e instanceof Error && e.message.startsWith("Timed out sending")) return "timeout";
  return "other";
}
function re({
  route: e,
  ...
  else y("send_message_delivery", C);
}
var Es = 100,
  ke = DT + Es,
  Rs = 2 + Abn + 1,
  cs = Math.max(DT + Rs, wbn),
  Ie = /^[^\n\r]*$/u;
function us(e) {
...
function de({ toolUseId: e, toolState: s }, u, o) {
  if (!e) return;
  s.get(ge).set(e, $e(u), o);
}
...
function Re(e) {
  return i().describe(e ?? "Plain text message content");
}
var fs =
  ...
  Os = m(() =>
    ps("type", [
      f({ type: N("shutdown_request"), reason: i().optional() }),
      f({
        type: N("shutdown_response"),
        request_id: i()
          .min(1, "must be the request id being responded to")
          .regex(Ie, "must be a single-line request id")
          .regex(Ve, `request id longer than any real one (max ${ke} characters)`),
        approve: Yb(),
        reason: i().optional(),
      }),
      f({
        type: N("plan_approval_response"),
        request_id: i()
          .min(1, "must be the request id being responded to")
          .regex(Ie, "must be a single-line request id")
          .regex(Ve, `request id longer than any real one (max ${ke} characters)`),
...
function ys(e) {
  return f({
    to: i()
      .regex(Ie, "must be a single-line recipient name or address")
      .regex(As, `recipient longer than any listed name or address (max ${cs} characters)`)
      .describe(
        e
          ? `Recipient: a name from ${Js} (append its " [ref]" only when a listing or an error shows one), a teammate name, "main", or a background agent's agentId`
    ...
    summary: i()
      ...
      .describe(
        e
          ? `A 5-10 word label for your own transcript row (not transmitted \u2014 the recipient previews the first line of \`message\`). Truncated to ${uTe} characters rather than rejected.`
          : `A 5-10 word summary shown as a one-line preview in the UI. Defaults to the first line of a plain-text message; longer summaries are truncated to ${uTe} characters rather than rejected.`,
      ),
    message: e ? Ge(fs).default(hs) : Ge(),
    ...(e && {
      notify_when_idle: Yb(q().optional()).describe(
...
function fe(e) {
  switch (e.reason) {
    ...
    case "send-failed":
    case "send-uncertain":
      return e.error !== void 0 ? me(e.error) : "other";
    case "cap":
...
function Ye(e, s, u = {}) {
  if (e.ok) return;
  let o = e.reason === "send-uncertain" || e.reason === "requester-refuses-inbound" || e.reason === "cap";
  s("uds", o ? void 0 : fe(e), { ...(o && { degradedClass: fe(e) }), ...u });
}
function ze({
  input: e,
  plainMessage: s,
  notify: u,
  refusedForPrincipal: o,
  assistantMessage: d,
  toolUseId: _,
  emit: r,
}) {
  if (s.trim().length > 0) return;
  if (Ss(e, d, _)) return r("uds", "handler_rewrite"), ne(e) ? (o ? ks : xs) : Ae;
  if (!u) {
    if (o) Te();
    return r("uds", o ? "permission_denied" : "empty_message"), Us(o);
  }
  return;
}
function Se(e, s) {
  let u = ne(e),
    o = u && (s.agentId !== void 0 || s.teammateContext !== void 0 || na());
  return { notify: u && !o, refusedForPrincipal: o };
}
var ws = "message must not be empty";
function Te() {
...
function vs(e, s) {
  if (s === void 0 || !Array.isArray(e.message.content)) return;
  let u = e.message.content.find((r) => r.type === "tool_use" && r.id === s);
  if (u === void 0 || u.type !== "tool_use") return;
  let o = u.input;
  if (typeof o !== "object" || o === null) return;
  let d = Z2t(o, { applySplit: Is() }),
    _ = d !== null && He(d.input) ? d.input : o;
  return {
...
var ue =
  ...
  Bs = `
The message was delivered, but no idle subscription was made: ${ue}`;
function Ke(e) {
  return v_() && Pe.test(e);
}
var xs = {
...
var Ls = " Your message was NOT delivered; send it again without notify_when_idle if it should still go.",
  ye =
    "notify_when_idle is only available from the main conversation of this session (not from a subagent or teammate).",
  qs = `
The message was delivered, but no idle subscription was made: ${ye}`,
  js = "No idle subscription was made (only the main conversation can subscribe).",
  Fs = `
The message was delivered, but no idle subscription was made: a permission handler removed notify_when_idle from this call.`,
  Hs = "No idle subscription was made (a permission handler removed it from the call).";
function Qe(e, s, u, o) {
  if (e) return { model: qs, display: js };
  if (pe(s, u, o)) return g("cross_session_notify_idle", "subscribe_stripped_by_handler"), { model: Fs, display: Hs };
  return;
}
function es(e, s, u, o) {
  if (e)
    return `
Nothing was subscribed either: ${ye}`;
  if (pe(s, u, o))
    return `
...
function Xs(e, s) {
  let u = e.teamContext?.teammates;
  if (!u) return;
  for (let o of Object.values(u)) if ("name" in o && o.name === s) return o.color;
  return;
}
function Ce(e, s) {
  let u = e.agentContext;
  if (u?.agentType === "teammate" && u.agentName) return { from: u.agentName, displayName: u.agentName };
  let o = e.getAppState();
  for (let [_, r] of o.agentNameRegistry) if (r === s) return { from: _, displayName: _ };
  let d = o.tasks[s];
  if (Dd(d)) return { from: d.identity.agentName, displayName: d.identity.agentName };
  return { from: s, displayName: Cr(d) ? d.agentType : s };
}
var ve = {
  data: {
    success: false,
...
function Oe(e) {
  if (e.agentId) return Ce(e, e.agentId).from;
  return gf() || (na() ? "teammate" : vi);
}
async function Ws(e, s, u, o, d, _) {
  let r = o.getAppState(),
    w = ds(r.teamContext);
  if (!w)
    return {
  ...
  let B = e;
  if (e !== vi || d !== void 0) {
    let t = r.teamContext?.teammates ?? {},
      H = d !== void 0 && _ === "team-context" && Object.hasOwn(t, d) ? t[d] : void 0,
      X = d === void 0 && Object.values(t).some((O) => O.name === e);
    if (H !== void 0) B = H.name;
      ...
      let O = await Rf(w, o.storageV5);
      if (O !== null) {
        let J = d !== void 0 ? npe(O, b$(o.getAppState())).filter((le) => le.agentId === d) : [];
        if (J.length > 1)
          return {
            data: {
              success: false,
        ...
        let se = d !== void 0 ? J[0] : wbt(npe(O, b$(o.getAppState())), e);
        if (se === void 0) {
          if (d !== void 0)
            return {
              data: {
  ...
  let C = Oe(o),
    K = sS(),
    z = await $g(B, { from: C, text: s, summary: u, timestamp: new Date().toISOString(), color: K }, w, o.storageV5);
  if (z === void 0)
    return {
      data: {
        success: false,
...
async function ss(e, s, u, o, d, _) {
  let r = await $g(
    e.identity.agentName,
    { from: Oe(o), text: s, summary: u, timestamp: new Date().toISOString(), color: sS() },
    e.identity.teamName,
    o.storageV5,
  );
  if (r === void 0)
...
function ts(e) {
  if (e instanceof SL) return "not_reachable";
  if (e instanceof fd) return "no_transcript";
  return "resume_failed";
}
function Ee(e) {
  return yb(e) ? ce(e, 7) : e;
}
function as(e, s) {
  return {
    message: K$n,
    inlineHandback: {
      displayName: Ee(e),
      content: s.content,
      harnessNoteCount: s.harnessNoteCount,
...
async function Vs(e, s, u, o) {
  let d = o.getAppState(),
    ...
    r = Oe(o),
    w = _bt("shutdown", s),
    B = Efn({ requestId: w, from: r, reason: u });
  if (
    (await $g(e, { from: r, text: b(B), timestamp: new Date().toISOString(), color: sS() }, _, o.storageV5)) === void 0
  )
    return {
      data: {
        success: false,
...
async function Gs(e, s) {
  let u = ds(),
    o = rb(),
    d = gf() || "teammate";
  n(`[SendMessageTool] handleShutdownApproval: teamName=${u}, agentId=${o}, agentName=${d}`);
  let _, r;
  if (u) {
    let z = await Rf(u, s.storageV5);
    if (z && o) {
      let W = z.members.find((t) => t.agentId === o);
      if (W) (_ = W.tmuxPaneId), (r = W.backendType);
    }
  }
  let w = Afn({ requestId: e, from: d, paneId: _, backendType: r }),
    B = await $g(vi, { from: d, text: b(w), timestamp: new Date().toISOString(), color: sS() }, u, s.storageV5),
    C =
      B === void 0 ? "The confirmation could not be written to team-lead's inbox." : "Sent confirmation to team-lead.",
    K = B === void 0 ? { degradedClass: "mailbox_write_failed" } : void 0;
  if (r === "in-process") {
    if ((n(`[SendMessageTool] In-process teammate ${d} approving shutdown - signaling abort`), o)) {
      let z = s.getAppState(),
        W = T7(o, z.tasks);
      if (W?.abortController)
        W.abortController.abort(), n(`[SendMessageTool] Aborted controller for in-process teammate ${d}`);
      else n(`[SendMessageTool] Warning: Could not find task/abortController for ${d}`);
    }
  } else {
    if (o) {
      let z = s.getAppState(),
      ...
      if (W?.abortController)
        return (
          n(`[SendMessageTool] Fallback: Found in-process task for ${d} via AppState, aborting`),
...
async function Ys(e, s, u) {
  let o = ds(),
    d = gf() || "teammate",
    _ = Cfn({ requestId: e, from: d, reason: s });
  if (
    (await $g(vi, { from: d, text: b(_), timestamp: new Date().toISOString(), color: sS() }, o, u.storageV5)) === void 0
  )
    return {
      data: {
        success: false,
...
async function zs(e, s, u, o, d) {
  let _ = d.getAppState(),
    r = _.teamContext?.teamName;
  if (!vC(_.teamContext))
    throw new Me("Only the team lead can approve plans. Teammates cannot approve their own or other plans.");
  let w = Oot({ recipientName: e, leaderMode: he(d).mode, proactivityLevel: _.proactivityLevel, tasks: _.tasks }),
    B = {
...
async function Ks(e, s, u, o, d) {
  let _ = d.getAppState(),
    r = _.teamContext?.teamName;
  if (!vC(_.teamContext))
    throw new Me("Only the team lead can reject plans. Teammates cannot reject their own or other plans.");
  let w = {
    type: "plan_approval_response",
    requestId: u,
    approved: false,
...
var ms = kt({
  ...
  backfillObservableInput(e) {
    ...
    if (Q2t(e) !== void 0) return;
    let s = e.message ?? "";
    if (typeof s === "string") (e.type = "message"), (e.recipient = e.to), (e.content = ir(s, 50));
    else if (typeof e.message === "object" && e.message !== null) {
      let u = e.message;
      if (((e.type = u.type), (e.recipient = e.to), u.request_id !== void 0)) e.request_id = u.request_id;
      if (u.approve !== void 0) e.approve = u.approve;
      let o = u.reason ?? u.feedback;
      if (o !== void 0) e.content = ir(o, 50);
    }
  },
  toAutoClassifierInput(e) {
    let s = (w, B) => (B ? ` [${w}: ${B}]` : ""),
      u = Z2t(e),
      o = u !== null && He(u.input) ? u.input : void 0,
      d = s("summary", o === void 0 ? e.summary : typeof o.summary === "string" ? o.summary : void 0),
      r = (o !== void 0 && typeof o.message === "string" ? o.message : void 0) ?? e.message ?? "";
    if (typeof r === "string") {
      if (ne(e))
        return r.trim().length === 0 ? `notify when idle: ${e.to}${d}` : `to ${e.to}: ${r}${d} [notify when idle]`;
      return `to ${e.to}: ${r}${d}`;
    }
    switch (r.type) {
      case "shutdown_request":
        return `shutdown_request to ${e.to}${s("reason", r.reason)}${d}`;
      case "shutdown_response":
        return `shutdown_response ${r.approve ? "approve" : "reject"} ${r.request_id} to ${e.to}${s("reason", r.reason)}${d}`;
      case "plan_approval_response":
        return `plan_approval ${r.approve ? "approve" : "reject"} ${r.request_id} to ${e.to}${s("feedback", r.feedback)}${d}`;
    }
  },
  async checkPermissions(e, s) {
    if (noe() && !Ke(e.to)) {
      let u = mf(e.to),
        o = u.scheme;
      if (o === "bridge") {
        let d = xDe(s.session, u.target, e.to);
        if (d)
          return (
            re({ route: "bridge", startedAt: Date.now(), errorClass: "bridge_auth", via: "address" }),
            {
              behavior: "deny",
              message: Ci(d),
              decisionReason: {
                type: "other",
                reason: "target is an elevated-security session unreachable from a cloud session",
              },
            }
          );
        let _ = qte(s.session, u.target, e.to);
        if (_)
          return (
            re({ route: "bridge", startedAt: Date.now(), errorClass: "recipient_gate_off", via: "address" }),
            {
              behavior: "deny",
              message: Ci(_),
              decisionReason: {
                type: "other",
                reason: "target session reports it cannot receive cross-session messages",
              },
            }
          );
        return (
      ...
      if (o === "other") {
        let d;
        try {
          d = await rje(s.session, e.to, e.message, s.getAppState(), s.storageV5, s.credentials);
        } catch (_) {
          return (
            n(`[SendMessage] permission-phase resolve failed (${Y$(l(_))}) \u2014 asking`, { level: "warn" }),
            de(s, e, null),
            {
              behavior: "ask",
              message: Ci(
        ...
        if (d.kind === "cloud-session") {
          if (dB(d.sessionId))
            return { behavior: "deny", message: Ci(v1(e.to, xb(s))), decisionReason: { type: "other", reason: $ot } };
          let { isRemoteControlPeerUnreachableFromHere: _, formatUnreachableElevatedRefusal: r } = import.meta.require(
            "/$bunfs/root/chunk-m90nqn8m.js",
          );
          if (d.via === "remote-control" && _())
            return (
              re({ route: "bridge", startedAt: Date.now(), errorClass: "bridge_auth", via: "remote_control_name" }),
              {
                behavior: "deny",
                message: Ci(r(d.displayName)),
                decisionReason: {
                  type: "other",
                  reason: "target is an elevated-security session unreachable from a cloud session",
                },
          ...
          let w = qte(s.session, d.sessionId, d.displayName);
          if (w)
            return (
              re({
                route: "bridge",
        ...
        if (d.kind === "not-found" && (d.cloudUnavailable !== void 0 || d.bridgeUnavailable !== void 0)) {
          de(s, e, null);
          let _ = [
            d.cloudUnavailable !== void 0 ? "the cloud session list could not be fetched" : void 0,
            d.bridgeUnavailable !== void 0 ? "the account session list could not be checked" : void 0,
          ]
            .filter((r) => r !== void 0)
            .join(" and ");
          return {
    ...
    if (Qy(he(s).mode))
      return { behavior: "passthrough", message: "Message to another agent requires classifier review." };
  ...
  mapToolResultToToolResultBlockParam(e, s) {
    let u = e;
    if (typeof u !== "object" || u === null || Array.isArray(u)) throw TypeError("SendMessage output is not an object");
    let {
        display: o,
        inlineHandback: d,
...
${B[0].text}`,
  ...
  async call(e, s, u, o) {
    let d = s.agentId,
      _ = Date.now();
    function r(h, k, P) {
      re({ route: h, startedAt: _, errorClass: k, ...P });
    }
    let w = E0e() ? C0e(he(s)) : void 0;
    if (d !== void 0 && rut(s.session, d))
      return (
    ...
    if (typeof e.message === "string") {
      ...
      if (h.scheme === "bridge") {
        ...
        let L = v.input,
          ...
          G = Je(A, () => r("bridge", "handler_rewrite", { via: "address" }));
        if (G !== void 0) return G;
        let Z = d !== void 0 && C !== void 0 ? Spe(C, A) : A,
          F = rs(),
          j = nje(d, C, { oneWay: F !== void 0 }),
          Q = hen(Gmt(s.session, h.target)),
      ...
      if (h.scheme === "uds") {
        ...
        if (v) Te();
        let G = d !== void 0 && C !== void 0 ? Spe(C, e.message) : e.message,
          Z = nje(d, C, { oneWay: M() === void 0 });
        try {
          let F = L ? await D(h.target, G, s.storageV5, P, void 0, epe(s.messages), w) : void 0,
            j = U ? await S(h.target, e.to, s.storageV5, w) : void 0;
          if (F) r("uds", void 0, { ...(j && !j.ok && { degradedClass: fe(j) }) });
          else if (j) Ye(j, r);
          let Q = e.summary || ir(e.message, 50),
            Y = j ? x(e.to, j, Js) : void 0,
            te = [...(F ? [`\u201C${Q}\u201D \u2192 ${e.to}${Z.message}`] : []), ...(Y ? [Y.model] : [])],
...
`),
    ...
    if (s.toolUseId) {
      ...
      (z = h !== void 0), (W = h ?? void 0), K.dropToolUse(s.toolUseId);
    }
    ...
    let H = s.options.tools.some((h) => on(h, Js)),
      X = H ? ` (${Js} lists them)` : "";
    if (t.kind === "agent-live" || t.kind === "agent-stopped" || t.kind === "agent-evicted") {
      if (rut(s.session, t.agentId)) return r("unresolved", "not_reachable"), ve;
      let h;
      try {
        h = await KA(po(t.agentId), s.storageV5);
      } catch {
        return r("unresolved", "not_reachable"), ve;
      }
      if (h?.isObserver) return r("unresolved", "not_reachable"), ve;
    }
    if (t.kind === "not-found") {
...
The cloud session list could not be fetched just now, so cloud sessions were not searched.`;
      ...
      if (t.pinnedIdentityClaimedLocally)
        S += `
Note: earlier in this conversation '${t.pinnedIdentityClaimedLocally}' was confirmed as a session that is NOT on this machine; a session record on this machine now claims that identity, which hides it here \u2014 nothing was sent.${H ? ` ${Js} will not show it while that claim stands.` : ""} A session on this machine impersonating it is suspicious: ask the user.`;
      if (t.bridgeUnavailable)
        S += `
Your account's other sessions (Remote Control and cloud) could not be checked just now, so they were not searched. If '${e.to}' is one, retry${H ? ` (or run ${Js} first)` : ""} \u2014 do not fall back to the send_message connector; it cannot reach these sessions.`;
      let x = "";
...
The sessions on this machine could not be listed just now, so they were not searched; retry if you meant a session on this machine.`;
      return (
        r(
          "unresolved",
          t.bridgeUnavailable === "timeout" || t.cloudUnavailable === "timeout"
            ? "timeout"
            : t.localUnavailable
              ? "local_unlisted"
              : "not_reachable",
...
`),
        D =
          t.total > t.candidates.length
            ? `
  \u2026and ${t.total - t.candidates.length} more${H ? ` \u2014 use ${Js} to see them all.` : "."}`
            : "",
        M = Boolean(t.bridgeUnavailable || t.cloudUnavailable || t.localUnavailable),
        S = t.searchTruncated ? I0e : "",
        x = t.searchTruncated ? ` (${Sen})` : "",
        U =
          t.total === 1
            ? t.matchedBy === "prefix"
            ...
            : t.matchedBy === "prefix"
              ? `'${e.to}' matches ${t.total} agents by prefix. Re-send with the ref of the one you mean:`
              : `${t.total} agents are named '${e.to}'. Re-send with the ref of the one you mean:`,
        v = "";
      if (t.bridgeUnavailable)
        v += `
Your account's other sessions (Remote Control and cloud) could not be checked just now, so this list may be missing one; if you meant one of them, retry${H ? ` (or run ${Js} first)` : ""}.`;
      if (t.cloudUnavailable)
        v += `
...
The sessions on this machine could not be listed just now, so this list may be missing one here; retry if you meant a session on this machine.`;
      if (t.pinnedIdentityClaimedLocally)
        v += `
Note: earlier in this conversation '${t.pinnedIdentityClaimedLocally}' was confirmed as a session that is NOT on this machine; a session record on this machine now claims that identity, so nothing was assumed and nothing was sent. ${H ? `${Js} will not show the other session while that claim stands. ` : ""}A session on this machine claiming that identity, that your user did not set up, is suspicious: ask the user before confirming anyone.`;
      let L =
          t.total === 1 &&
          !t.bridgeUnavailable &&
          !t.cloudUnavailable &&
          !t.localUnavailable &&
          !t.pinnedIdentityClaimedLocally
            ? `
e.g. {"to": "${w$(t.candidates[0])}", ...}`
      ...
      else if (t.localUnavailable) r("unresolved", "local_unlisted", A);
      else if (t.pinnedIdentityClaimedLocally) r("unresolved", void 0, { degradedClass: "claimed_locally", ...A });
      else r("unresolved", void 0, { degradedClass: t.total === 1 ? "confirm_required" : "ambiguous", ...A });
      let G = t.candidates[0],
        Z = t.pinnedIdentityClaimedLocally
          ? ` ${yen(t.pinnedIdentityClaimedLocally)} \u2014 ask before confirming anyone.`
          : "",
        F = _en(t),
        j = t.candidates.map((Y) => `'${Y.name}' ${F3t(Y.where)}`).join(", "),
...
${P}${D}${L}${v}${S}${h !== "no" ? D0e(e.to, xb(s)) : ""}`,
    ...
    let O = await z$n({
      ...
      agentLifecycle: s.agentLifecycle,
      storageV5: s.storageV5,
      credentials: s.credentials,
    });
    if (O.kind === "rebound") {
      g("send_message_pin_guard", "rebound"), r("unresolved", "not_reachable");
      let h = `'${O.name}' now resolves to a different agent than it did earlier in this conversation: earlier sends went to [${O.previous.ref}], which this name no longer reaches. Nothing was sent.`,
        k = `Not sent \u2014 '${O.name}' now means a different agent than it did earlier in this conversation; asked Claude to confirm which one it wants.`,
        P = O.previous.id,
        D =
          yb(P) !== null
            ? "If you need the earlier agent and it is still running, address it by its agent ID from its spawn result."
            : `The earlier recipient is ${Tr(P) !== P ? "a Claude session on another machine (cloud or Remote Control)" : Vse}; this name now belongs to an agent in this session.${H ? ` Use ${Js} if you still need that session.` : ""}`;
      if (O.next === void 0) {
        let M = H
          ? `Use ${Js} to see everyone you can message.`
          : "Check the spelling, or use the agent ID from a background agent's spawn result.";
...
${D}`,
    ...
    let J = O.pin ? { pin: O.pin } : void 0;
    if (J) y("send_message_pin_guard");
    if (typeof e.message !== "string") {
      let h = t.kind === "mailbox" ? t.recipientName : e.to,
        k = t.kind === "mailbox" ? (t.displayName ?? t.recipientName) : e.to;
      if (s.agentId) {
        let M = s.getAppState().tasks[s.agentId];
        if (Cr(M) || s.agentContext?.agentType !== "teammate")
          return (
      ...
      let P = h;
      if (t.kind === "mailbox" && t.memberAgentId !== void 0 && e.message.type !== "shutdown_response") {
        let M = s.getAppState(),
          S = M.teamContext?.teammates ?? {};
        if (t.memberIdentitySource === "team-context" && Object.hasOwn(S, t.memberAgentId)) P = S[t.memberAgentId].name;
        else if (t.memberIdentitySource === "team-context")
          return (
            r("mailbox", "not_reachable"),
            {
              data: {
          ...
          let x = ds(M.teamContext);
          if (x) {
            let U = await Rf(x, s.storageV5);
            if (U === null)
              return (
                r("mailbox", "not_reachable"),
                {
            ...
            let v = npe(U, b$(M)).filter((A) => A.agentId === t.memberAgentId);
            if (v.length > 1)
              return (
                r("mailbox", "not_reachable"),
                {
      ...
      try {
        switch (e.message.type) {
          case "shutdown_request":
            D = await Vs(P, k, e.message.reason, s);
            break;
          case "shutdown_response":
            D = e.message.approve
              ? await Gs(e.message.request_id, s)
              : await Ys(e.message.request_id, e.message.reason, s);
            break;
          case "plan_approval_response":
            D = e.message.approve
              ? await zs(P, k, e.message.request_id, e.message.feedback, s)
              : await Ks(P, k, e.message.request_id, e.message.feedback ?? "Plan needs revision", s);
            break;
        }
      } catch (M) {
        throw (r("mailbox", "not_reachable"), M);
      }
      return (
        r(
          "mailbox",
          D.data.success ? void 0 : "mailbox_write_failed",
          D.degradedClass ? { degradedClass: D.degradedClass } : void 0,
        ),
        { data: D.data }
      );
    }
    let se = d !== void 0 && C !== void 0 ? Spe(C, e.message) : e.message,
      le = B ? QS(B.displayName) : "",
      ie =
        d !== void 0 && C !== void 0
          ? { kind: "peer", from: C, senderTaskId: d, ...(le && { name: le }), body: O$(eve, e.message) }
          : { kind: "coordinator" };
    switch (t.kind) {
      case "main": {
        if (d === void 0)
      ...
      case "agent-stopped":
        try {
          let h = await uen({
              ...
              parentPromptId: Hce(s.messages, s.agentContext),
            }),
            k = h.inlineHandback !== void 0,
            ...
            return (
              r("in_process", k ? void 0 : "not_reachable"),
              {
                data: {
                  success: k,
                  message: k
                    ? `${t.agentName} is already waking; message queued for its next tool round (not delivered if the agent turns out to have been stopped).`
                    : `${t.agentName} is being resumed by another caller and its task record is gone; the message was not delivered. Retry shortly.`,
                  ...(k && J),
                },
              }
            );
              ...
              data: {
                ...
                message:
                  h instanceof SL
                    ? l(h)
                    : h instanceof fd
                      ? `Agent "${t.agentName}" is stopped (${t.status}) and could not be resumed: ${l(h)}`
                      : `Agent "${t.agentName}" was resumed but ${h instanceof Error && h.name === "AbortError" ? "was interrupted" : "failed while running"}: ${l(h)}`,
              },
            }
          );
        }
      case "agent-evicted": {
        ...
        let M = null;
        try {
          if (((M = await je(h, s.storageV5)), M)) {
            let U = M.name ?? t.agentName,
              v = M.teamName ?? ds(s.getAppState().teamContext);
            for (let A of Object.values(s.getAppState().tasks))
              if (
                Dd(A) &&
                A.status === "running" &&
                (A.identity.resumableAgentId === h || (A.identity.agentName === U && A.identity.teamName === v))
              ) {
                D.resolve(A.id);
                let G = await ss(A, e.message, e.summary, s, t.agentName, J);
                return r("mailbox", G.data.success ? void 0 : "mailbox_write_failed"), G;
              }
            let L = await Fe({
              resumableAgentId: h,
              prompt: e.message,
                ...
                data: {
                  ...
                  message:
                    L.resumedMessageCount > 0
                      ...
                      : `Teammate "${t.agentName}" was not running; resumed it as an in-process teammate (no prior transcript) with your message as its next prompt.`,
                  ...J,
                },
              }
            );
              ...
              data: {
                ...
                message:
                  S instanceof SL
                    ? l(S)
                    : M
                      ? `Failed to resume teammate "${t.agentName}": ${l(S)}`
                      : S instanceof fd && S.transcriptMissing && yb(t.agentName) !== null
                        ? `Agent "${t.agentName}" could not be resumed: ${l(S)}. If you read this id in a message from another Claude Code process (e.g. the lead's subagent, seen from a teammate pane), it never ran in this session \u2014 reply through "${vi}" or the session that sent it instead of the raw id.`
                        : S instanceof fd
                          ? `Agent "${t.agentName}" could not be resumed: ${l(S)}`
                          : `Agent "${t.agentName}" was resumed but ${S instanceof Error && S.name === "AbortError" ? "was interrupted" : "failed while running"}: ${l(S)}`,
              },
            }
          );
        } finally {
      ...
      case "local-session": {
        let { sendToUdsSocket: h, ownMessagingSocket: k } = import.meta.require("/$bunfs/root/chunk-j0bs780t.js"),
          P = AUt(),
          { subscribeToPeerIdle: D, idleSubscriptionLines: M } = import.meta.require("/$bunfs/root/chunk-ery7jqc3.js"),
          { notify: S, refusedForPrincipal: x } = Se(e, s),
          U = ze({
            input: e,
            plainMessage: e.message,
            notify: S,
            refusedForPrincipal: x,
            assistantMessage: o,
            toolUseId: s.toolUseId,
            emit: r,
          });
        ...
        if (S && e.message.trim().length === 0) {
          let v = await D(t.sock, t.displayName, s.storageV5, w),
          ...
          let A = L ? HIt(t, X, "live session", "subscription") : "",
            ...
            { model: Z, display: F } = M(t.displayName, v, Js),
            j = !v.ok && v.reason === "peer-gone";
          return {
            data: {
              success: v.ok,
              message: j ? Z : `${Z} (${t.displayName} is ${Vse}${x0e(t)})${A}`,
              display: j ? F : `${F} (${Vse}${x0e(t)})${G}`,
            },
          };
        }
        try {
          let { msgId: v } = await h(t.sock, se, s.storageV5, P, void 0, epe(s.messages), w),
            L = S ? await D(t.sock, t.displayName, s.storageV5, w) : void 0,
            A = L ? M(t.displayName, L, Js) : void 0,
            G = A ? void 0 : Qe(x, e, o, s.toolUseId),
            Z = A
              ? `
${A.model}`
              : (G?.model ?? "");
...
${G.display}`
          ...
          let A = $z(v),
            G =
              A === "gone"
                ? ` \u2014 that session may have just exited.${H ? ` Call ${Js} to see who is reachable now.` : ""}`
                : A === "busy"
                  ? Q6t(v)
      ...
      case "cloud-session": {
        ...
        let h = t.via === "cloud" ? "cloud session" : "Remote Control session",
          ...
          {
            postInterClaudeMessage: P,
            isLikelyStaleBridgeError: D,
            classifyBridgeSendError: M,
          } = import.meta.require("/$bunfs/root/chunk-7vf10vwz.js"),
          { isRemoteControlPeerUnreachableFromHere: S, formatUnreachableElevatedRefusal: x } = import.meta.require(
            "/$bunfs/root/chunk-m90nqn8m.js",
          );
        ...
        let v = await tje({
          ...
          canUseTool: u,
          assistantMessage: o,
          permissionPhaseRan: z,
          recipientLabel: `${h} '${t.displayName}'`,
          parse: (be) => we().safeParse(be),
        });
        if (!v.proceed)
          return (
        ...
        let L = v.input,
          A = typeof L.message === "string" ? L.message : e.message,
          G = Je(A, () => r("bridge", "handler_rewrite", { via: k }));
        if (G !== void 0) return G;
        let Z = L === e ? se : d !== void 0 && C !== void 0 ? Spe(C, A) : A,
          F = CUt(),
          j = await P(t.sessionId, Z, F, void 0, epe(s.messages), w, s.credentials),
          Q = L.summary || ir(A, 50);
        if (
        ...
        let Y = HIt(t, X, "agent"),
          te = rs(),
          ae = Fot(t),
          oe = nje(d, C, { oneWay: t.via === "cloud" || te !== void 0 }),
          De = hen(t.via === "remote-control" && !t.reportsInbound && !t.inboundReportUnavailable),
          $s =
            t.via === "cloud"
              ...
              : te
                ? `; one-way: ${is(te)}, so the receiver cannot address a reply to this session`
                : ls(F);
        return {
          data: {
            success: true,
      ...
      case "mailbox": {
        let { data: h, errorClass: k } = await Ws(
          t.recipientName,
          e.message,
          e.summary,
          s,
          t.memberAgentId,
          t.memberIdentitySource,
        );
        return r("mailbox", h.success ? void 0 : (k ?? "other")), { data: h };
      }
    }
  },
  extractSearchText(e) {
  ...
  renderToolUseMessage(e) {
    if (typeof e.to === "string" && e.to.startsWith("did:") && typeof e.message === "string") {
      let s = (_) =>
          hV(_)
            .replace(/[\s\u2800]+/g, " ")
            .trim(),
        u = s(e.message),
        o = FN(u),
        d = o > 200 ? `${d4(u).slice(0, 200).join("")}\u2026 [${o} chars total]` : u;
      return Ci(`${s(e.to)} \u2190 "${d}"`);
    }
...
export {
  gs as InFlightTeammateResumes,
  Me as SendMessagePreconditionError,
  ms as SendMessageTool,
  Ss as blankCallCausedByHandler,
  es as idleNotAttemptedNote,
  Qe as idleSubscriptionSkippedNotes,
  Is as isSlippedSummarySplitEnabled,
  ze as udsBlankMessageGate,
};
