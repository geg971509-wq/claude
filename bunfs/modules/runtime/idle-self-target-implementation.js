...
import { S$, vre, Ttr, tS, j3t } from "/$bunfs/root/chunk-8tgj5dp2.js";
...
import { $z, gTe, Oht, qD, POe, s5e, MF } from "/$bunfs/root/chunk-qnrh4abv.js";
...
import { XGe } from "/$bunfs/root/chunk-ddbk6aje.js";
...
import { She, nZ } from "/$bunfs/root/chunk-ac0gs4d8.js";
import { w0e, Iot, QZt, j6e, Pot } from "/$bunfs/root/chunk-wz7pqpgq.js";
...
async function y(e, r, s, i) {
  if (nZ() === "refuse") return t("requester-refuses-inbound");
  let n = MF();
  if (n === void 0) return t("no-inbox");
  let a = tS(e);
  if (a === void 0) return t("unreachable-namespace");
  if (a === tS(n)) return t("self-target");
  let o;
  try {
    o = await s5e(e);
  } catch {
  ...
  try {
    return (
      await POe(e, { action: "notify_when_idle", from: S$(n), ...(i !== void 0 && { from_mode: i }) }, u, {
...
function _(e, r = true) {
  ...
  let a = "that session runs in the same permission class as this one (or is one this session spawned)";
  return nZ() === "accept"
...
function S(e, r) {
  ...
  switch (e.reason) {
    ...
    case "cap":
      return `notify_when_idle: this session already holds ${w0e} pending idle subscriptions \u2014 wait for some to fire or expire.`;
    case "peer-gone":
      return `notify_when_idle: no session is listening at that address any more; nothing was subscribed, and any earlier idle subscription to it is void${gTe(r)}`;
    case "send-failed": {
      let s = $z(e.error) === "busy" ? Oht(e.error) : ".";
...
function N(e, r, s) {
  try {
    return { model: r.ok ? _(e, r.peerKnownCapable) : S(r, s), display: I(e, r) };
  } catch (i) {
    h(i);
    let n = r.ok ? "Idle subscription sent." : "The idle subscription could not be set up.";
    return { model: n, display: n };
  }
}
async function x() {
  let e = (await XGe()) ?? process.getuid?.();
  return e === void 0 ? [] : [e];
}
export {
  w as idleSelfTargetMessage,
  I as idleSubscribeDisplayLine,
  S as idleSubscribeFailedLine,
  _ as idleSubscribedLine,
