...
`),
    l = Math.max(1, m - 6),
    a = r.flatMap((n) => LBe(n, l).lines.map((f) => f.text));
  return [
    i ? `Draft preview, ${r.length} ${k(r.length, "line")}:` : `${r.length} ${k(r.length, "line")}:`,
    ...a.map((n) => `> ${n}`),
    i ? "End of preview. Review the full draft with /feedback." : "End of details.",
  ].join(`
`);
}
function v(s, m, i) {
...
`)) {
    let n = LBe(a, Math.max(1, m)).lines;
    if (l + n.length <= i) {
      r.push(a), (l += n.length);
      continue;
    }
    let f = i - l;
    if (f > 0) {
      let p = n[f]?.startOffset;
      r.push(p === void 0 ? a : a.slice(0, p).trimEnd());
    }
    return (
      r.push("\u2026"),
      r.join(`
...
function XRe(z) {
  let A = _(8),
    { value: g, bold: u, dim: c, columns: b, maxRows: h, preview: w } = z,
    d = hn(),
    R;
  if (A[0] !== u || A[1] !== b || A[2] !== c || A[3] !== d || A[4] !== h || A[5] !== w || A[6] !== g) {
    let y = o_e(g);
