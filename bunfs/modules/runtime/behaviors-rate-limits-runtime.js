...
import { Tt, Wd, Fn, I } from "/$bunfs/root/chunk-8tgj5dp2.js";
...
import { ad, N1e } from "/$bunfs/root/chunk-2rx5nghb.js";
...
import { Id, ML, W9n, OG } from "/$bunfs/root/chunk-zze8764r.js";
...
import { TF, Gce } from "/$bunfs/root/chunk-86v6b72m.js";
...
import { Cse } from "/$bunfs/root/chunk-brkpapaa.js";
import { FHt, P2e } from "/$bunfs/root/chunk-nemcrcw3.js";
...
function h(t) {
  ...
  let i = n === "max" || n === "team" || n === null,
    s = [
      ...
      ...(i ? [{ title: "Current week (Sonnet only)", limit: e.seven_day_sonnet }] : []),
      ...Gce(e.limits, TF()),
    ],
    r = [];
...
function d(t) {
  let { behaviors: e } = t;
  if (!e) return null;
  let n = [f("Last 24h", e.day), f("Last 7d", e.week)].filter((i) => i !== null);
...
var g = "What's contributing to your limits usage?",
  P = async (t, e) => {
    ...
    if (Tt() && n && !iH()) {
      ...
      else s = "You are currently using your subscription to power your Claude Code usage";
      let r = Le(),
        o = Cse(),
        a = await P2e({
          includeBehaviors: r && !mo(e.session) && o.allowed,
...
${l}`;
      let c = a.behaviors && !mo(e.session) ? d(a) : null;
      if (c)
        s += `

${c}`;
      else if (r && !mo(e.session) && !o.allowed && Tt())
        s += `
...
Spend limit: ${Math.round(s.utilization * 100)}% used \xB7 resets ${ad(s.resets_at, true, true, true)}`;
  ...
  y = {
    ...
    subagent_heavy: (t) => `${t}% of your usage came from subagent-heavy sessions`,
    high_parallel: (t) => `${t}% of your usage was while 4+ sessions ran in parallel`,
    cron: (t) => `${t}% of your usage came from sessions active for 8+ hours`,
  };
function f(t, e) {
  let n = e.behaviors.filter((o) => o.pct >= FHt),
    i = [
  ...
  let s = `${e.request_count} ${k(e.request_count, "request")}`,
    r = `${e.session_count} ${k(e.session_count, "session")}`;
  return [`${t} \xB7 ${s} \xB7 ${r}`, ...n.map((o) => `  ${y[o.key](o.pct)}`), ...i.map((o) => `  ${o}`)].join(`
`);
}
function u(t, e, n) {
...
export { P as call, d as formatBehaviors, h as formatRateLimits };
