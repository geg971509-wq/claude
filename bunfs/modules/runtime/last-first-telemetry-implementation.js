...
import { zXn } from "/$bunfs/root/chunk-gdhqsw8a.js";
class o {
  active = void 0;
  ...
  markLocalTransport() {
    this.transportPersists = false;
  }
}
var dT = new Mn(() => new o());
function PE(r) {
  return zXn() && dT.of(r).transportPersists !== false;
}
function mo(r) {
  return PE(r) || (dT.of(r).remoteBridgeLive?.() ?? false);
}
function gpn(r, e) {
  if (!PE(r)) return e;
...
function h2t(r, e, t, i = "last") {
  try {
    if (!PE(r)) return i === "first" ? [e, ...t] : [...t, e];
    for (let s of t) n(`error_during_execution detail: ${s}`, { level: "error" });
    return [e];
  } catch (s) {
    return h(we(s)), [e];
  }
