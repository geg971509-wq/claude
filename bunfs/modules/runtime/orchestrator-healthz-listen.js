...
import { yoe, yh, dA } from "/$bunfs/root/chunk-rf51999f.js";
import { qi, Qh } from "/$bunfs/root/chunk-s28wf80n.js";
...
function ee(e) {
  return { Authorization: `Bearer ${e}`, "Content-Type": "application/json", "anthropic-version": "2023-06-01" };
}
function De(e) {
  if (
    e !== null &&
...
function I(e, n, o) {
  ...
  switch (e) {
    ...
    default:
      throw Object.assign(Error(`${o}: Failed with status ${e}${u ? `: ${u}` : ""}`), { httpStatus: e });
  }
}
function yme(e) {
...
var ivt = ["transport", "timeout", "5xx", "429", "4xx"];
function w$e(e) {
  if (e !== null && typeof e === "object") {
    let o = e.code;
...
function CZe(e) {
  ...
  return {
    async registerRunner(r, t) {
      ...
      let s = { runner_version: Tie };
      if (r) s.client_label = r;
      if (t) s.lock_to_account_id = t;
      let a = await st.post(`${e.baseUrl}/v1/code/runners/self-hosted/runners/register`, s, {
      ...
      return (
        ...
        rL(a.data.runner_id, "runner_id"),
        n(`[runner:api] RegisterRunner -> ${a.status} runner_id=${a.data.runner_id}`),
        a.data
      );
    },
    async pollSpawnHints(r, t) {
      let s = await st.post(`${e.baseUrl}/v1/code/runners/self-hosted/spawn-hints/poll`, r, {
        headers: le(e.poolSecret),
        timeout: 30000,
        signal: t,
    ...
    async nackSpawnHint(r, t) {
      ...
      let s = r.error.length > 512 ? r.error.slice(0, 512) : r.error,
        a = await st.post(
          ...
          { ...r, error: s },
          { headers: le(e.poolSecret), timeout: 15000, signal: t, validateStatus: (i) => i < 500 },
        );
    ...
    async pollWork(r, t, s, a, i, d) {
      ...
      let m = await st.post(
        ...
        { available_capacity: s, ...(i !== void 0 && { wake_source: i }), ...(d !== void 0 && { warmup_complete: d }) },
        { headers: Q(r), timeout: j5t, signal: a, validateStatus: (E) => E < 500 },
      );
      ...
      if (h !== void 0 && h !== null && !Array.isArray(h))
        throw Error("PollWork: response assignment_ids is not an array \u2014 rejecting the malformed poll response");
      ...
      let x = m.data.lease_expires_at;
      return {
        assignment_ids: _,
        lease_expires_at: typeof x === "string" ? x : void 0,
        session_assignments: Array.isArray(m.data.session_assignments) ? m.data.session_assignments : [],
      };
    },
    ...
    async releaseSession(r, t) {
      rL(t, "sessionId"), n(`[runner:api] POST /v1/code/runners/self-hosted/sessions/${t}/release`);
      let s = await st.post(
    ...
    async refreshToken(r) {
      ...
      if ((I(t.status, t.data, "RefreshToken"), !t.data.token)) throw Error("RefreshToken: response missing token");
      return n(`[runner:api] RefreshToken -> ${t.status}`), t.data;
    },
    async getSessionRemoteConfig(r, t, s) {
      rL(r, "sessionId"), n(`[runner:api] GET /v1/code/sessions/${r}/remote`);
      let a = await st.get(`${e.baseUrl}/v1/code/sessions/${r}/remote`, {
        headers: ee(t),
        timeout: 15000,
        signal: s,
    ...
    async registerWorker(r, t, s, a) {
      rL(t, "sessionId"), n(`[runner:api] POST ${r}/v1/code/sessions/${t}/worker/register`);
      let i = await st.post(
      ...
      if (isNaN(d)) throw Error(`RegisterWorker: invalid worker_epoch in response: ${i.data.worker_epoch}`);
      return n(`[runner:api] RegisterWorker -> ${i.status} worker_epoch=${d}`), d;
    },
    async postWorkerEvents(r, t, s, a, i, d) {
      rL(t, "sessionId"),
        n(`[runner:api] POST ${r}/v1/code/sessions/${t}/worker/events (${i.length} ${k(i.length, "event")})`);
      let p = await st.post(
    ...
    async updateSessionWorkerState(r, t, s, a, i, d) {
      rL(t, "sessionId"), n(`[runner:api] PUT ${r}/v1/code/sessions/${t}/worker`);
      let p = await st.put(
        `${r}/v1/code/sessions/${t}/worker`,
        i === void 0 ? { worker_epoch: a } : { worker_epoch: a, worker_status: i },
        { headers: ee(s), timeout: 15000, signal: d, validateStatus: (m) => m < 500 },
      );
      I(p.status, p.data, "UpdateSessionWorkerState"), n(`[runner:api] UpdateSessionWorkerState -> ${p.status}`);
    },
    ...
    async forwardDiagnostics(r, t, s, a, i) {
      if ((rL(t, "sessionId"), i.length === 0)) return;
      ...
      I(d.status, d.data, "ForwardDiagnostics"), n(`[runner:api] ForwardDiagnostics -> ${d.status}`);
    },
  };
}
var sK = "unknown",
  W5t = /^[A-Za-z0-9_.-]{1,64}$/;
function Sme(e) {
  if (!e || e === "unknown" || !W5t.test(e) || e.startsWith("-") || /^\.{1,2}$/.test(e)) return;
  return e;
}
function T$e(e) {
...
function Be(e, n = Date.now()) {
  let o = e.lastPollAt > 0 ? (n - e.lastPollAt) / 1000 : 0,
    u = `runner_id="${FU(e.runnerId)}",version="${FU(e.version)}",client_label="${FU(e.clientLabel)}"`,
    r = ke,
    t = "";
...
`);
    for (let [p, m] of i)
      t += `${r}_${a}{client_platform="${FU(p)}"} ${m}
`;
  }
  if (
...
`),
    ...
    t += `${r}_locked_account{email="${FU(e.lockedAccountEmail)}"} 1
`;
...
`);
  for (let [a, i] of e.sessionIdle) {
    let d = i === null ? 0 : ((n - i) / 1000).toFixed(3),
      p = e.sessionClientPlatform.get(a) ?? sK;
    t += `${r}_session_idle_seconds{session_id="${FU(a)}",client_platform="${FU(p)}"} ${d}
`;
  }
    ...
    for (let a of r.scopeMetrics ?? [])
      for (let i of a.metrics ?? []) {
        ...
        for (let _ of v ?? []) {
          let x = [],
            E = new Set();
          if (t !== void 0) x.push(["session_id", t]), E.add("session_id");
          if (s !== void 0) x.push(["client_platform", s]), E.add("client_platform");
          for (let D of _.attributes ?? []) {
            if (typeof D.key !== "string") continue;
            let q = be(D.value);
            if (q === void 0) continue;
            let j = Pe(D.key);
            if (j === "" || E.has(j)) continue;
            E.add(j), x.push([j, q]);
          }
          let T = x.length > 0 ? `{${x.map(([D, q]) => `${D}="${FU(q)}"`).join(",")}}` : "",
            O = typeof _.asDouble === "number" ? _.asDouble : _.asInt !== void 0 ? Number(_.asInt) : void 0;
    ...
    if (t.method === "POST" && t.url === "/v1/metrics") {
      ...
      t.on("data", (_) => {
        ...
        t.on("end", () => {
          if (h) return;
          try {
            ze(n, V(Buffer.concat(m).toString("utf8"))), s.writeHead(200).end();
          } catch (_) {
            o(`[runner:warn] /v1/metrics rejected malformed body: ${l(_)}`), s.writeHead(400).end();
          }
        });
      return;
    }
...
`);
    ...
    if (
      (s.writeHead(200, { "Content-Type": "application/json", "Content-Length": Buffer.byteLength(d) }),
      s.end(d),
      n.claimVisibility !== void 0)
    ) {
      for (let p of n.claimVisibility.values()) if (p.visibleMs === void 0) p.visibleMs = Math.max(0, a - p.pickedUpAt);
    }
  });
  u.unref();
  let r = false;
...
var ge = ["https_proxy", "HTTPS_PROXY", "http_proxy", "HTTP_PROXY"],
  vZe = "SELF_HOSTED_RUNNER_PROXY_AUTHORIZATION_COMMAND",
  RZe = "SELF_HOSTED_RUNNER_PROXY_AUTHORIZATION_FILE",
...
function $e(e) {
  let n = e.trim();
  if (!n) throw new Y("proxy authorization source returned an empty value");
  if (Buffer.byteLength(n) > me) throw new Y(`proxy authorization value exceeds ${me} bytes`);
  if (ae.test(n))
    throw new Y(
...
function at(e) {
  ...
  let r,
    t = async (a) => {
      if (n.kind === "file") {
        ...
        let p,
          ...
          v = (async () => {
            ...
            try {
              let x = Buffer.alloc(me + 1),
                { bytesRead: E } = await _.read(x, 0, x.length, 0);
              return x.subarray(0, E).toString("utf8");
            } finally {
              await _.close();
            }
          })(),
        ...
        try {
          ...
          if (!m) r = v;
          throw new Y(`proxy authorization file could not be read: ${Rl(l(_))}`);
        }
        return $e(p);
      }
      let i = {
      ...
      if (d.failed) {
        let p = d.timedOut ? `timed out after ${o}ms` : `exited ${d.exitCode ?? d.signal ?? "abnormally"}`,
          m = d.stderr?.trim();
        throw new Y(`proxy authorization command ${p}${m ? `: ${ce(Rl(m), 500)}` : ""}`);
      }
      return $e(d.stdout ?? "");
    },
    s = new Map();
...
`);
        ...
        let x = s.subarray(0, _).toString("latin1"),
          [E = "", ...T] = x.split(`\r
`);
        i(null, { startLine: E, headerLines: T, rest: s.subarray(_ + 4) });
      },
...
async function pt(e) {
  let n = new URL(e.upstreamProxyUrl),
    o = n.protocol === "https:";
  if (n.protocol !== "http:" && !o) throw Error("the upstream proxy URL must be http:// or https://");
    ...
    v = () =>
      new Promise((c, f) => {
        ...
        let L = o ? "secureConnect" : "connect",
          ...
          A = () => C(null),
          H = (P) => C(P),
          U = () => C(Error("upstream proxy closed the connection during connect"));
        (C = (P) => {
          if ((clearTimeout(w), S.removeListener(L, A), S.removeListener("error", H), S.removeListener("close", U), P))
            S.destroy(), f(P);
          else c(S);
        }),
          S.once(L, A),
...
`,
    ...
    E = (c, f, S) => {
      ...
      let L = f instanceof Y ? f.message : `upstream proxy connection failed: ${Rl(l(f))}`;
      d(`${L} (${F(S)} answered 502)`),
        N(
          c,
          B(
            502,
...
`,
    ...
    T = (c, f, S, L) => {
      f.destroy(),
        d(`upstream proxy rejected the minted Proxy-Authorization for ${F(S)} (${F(L)}); answering the client 502`),
        N(
          c,
          B(
            502,
...
`,
    ...
    O = (c, f, S, L) => {
      f.destroy(),
        d(
          `upstream response to ${F(S)} (status ${L}) reflected the minted Proxy-Authorization in its head; withheld, answering the client 502`,
        ),
        N(
          c,
          B(
    ...
    D = async (c, f, S, L) => {
      ...
      try {
        let A = await _(f, C, void 0, w),
          H = x(A.head);
        if (H === 407) {
          let M = it(oe(A.head.headerLines, "proxy-authenticate"));
          A.up.destroy(),
            i(`upstream answered 407 for CONNECT ${f}; re-minting once`),
            (A = await _(f, C, { challenge: M }, w)),
            (H = x(A.head));
        }
        let { up: U, head: P, value: z } = A;
        if (H !== 200 && he(P, z)) {
          O(c, U, `CONNECT ${f}`, H);
          return;
        }
        if (H === 407) {
          T(c, U, `CONNECT ${f}`, P.startLine);
          return;
        }
        if (H < 100) {
          U.destroy(),
            d(`upstream proxy sent an unparseable reply to CONNECT ${F(f)}; answering the client 502`),
            N(
              c,
              B(
                502,
...
`),
        ...
        U.destroy(), d(`upstream proxy refused CONNECT ${F(f)}: ${F(P.startLine)}`);
...
`,
    ...
    q = async (c, f, S) => {
      let L = ne(f.headerLines, [
        ...
        C = Number(oe(f.headerLines, "content-length") ?? "0"),
        w,
        A = () => {},
        H = () => {
...
`),
            ...
            A = ct(c, w, C, S.release(), () =>
              i("client sent bytes past its declared Content-Length on a plain HTTP request; not forwarded"),
            );
          let P = await ie(w, fe, pe),
            z = x(P),
            de = 0;
          while (z >= 100 && z < 200 && z !== 101) {
            if (S.gone()) {
              H();
              return;
            }
            if (he(P, U)) {
              A(), O(c, w, "a plain HTTP request", z);
              return;
            }
            if (++de > Te) {
              w.destroy(),
                A(),
                d(`upstream sent more than ${Te} interim responses to a plain HTTP request; answering the client 502`),
                N(
                  c,
                  B(
                    502,
...
`,
          ...
          if (z === 407) {
            A(),
              w.destroy(),
              d(
                `upstream answered 407 to a plain HTTP request (${F(P.startLine)}); it may have come from the destination site rather than the proxy, and re-mint is only attempted for CONNECT tunnels; answering the client 502`,
              ),
              N(
                c,
                B(
...
`,
          ...
          if (z < 100) {
            w.destroy(),
              A(),
              d("upstream proxy sent an unparseable reply to a plain HTTP request; answering the client 502"),
              N(
                c,
                B(
                  502,
...
`,
          ...
          if (z >= 400) {
            w.destroy(),
              A(),
              d(`upstream answered ${F(P.startLine)} to a plain HTTP request; error body withheld from the client`),
              N(
                c,
                W(
                  P.startLine +
...
`,
          ...
          let S = f.startLine.match(/^CONNECT\s+(\S+)\s+HTTP\/1\.[01]$/i),
            L = !S && /^[A-Z]+\s+http:\/\/\S+\s+HTTP\/1\.[01]$/i.test(f.startLine);
          if (!S && !L) {
            N(
              c,
              B(
...
`,
          ...
          if (!a(f.headerLines)) {
            i(`client ${S ? "CONNECT" : "request"} without valid loopback credentials \u2192 407 challenge`),
              N(c, dt());
            return;
          }
...
`,
            ...
            if (w.length > 1 || (w.length === 1 && !/^content-length:[ \t]*\d{1,15}[ \t]*$/i.test(w[0]))) {
              N(
                c,
                B(
                  400,
...
`,
          ...
          d(`internal error handling a proxied connection: ${Rl(l(f))}`), c.destroy();
        });
    });
  await new Promise((c, f) => {
    Z.once("error", f),
      Z.listen({ host: "127.0.0.1", port: 0 }, () => {
        Z.removeListener("error", f), c();
      });
  }),
    Z.on("error", (c) => d(`listener error: ${l(c)}`));
  let Ee = Z.address().port;
  return {
...
function _t(e, n = process.env) {
  if ((e.command !== void 0 && e.command.trim() === "") || (e.file !== void 0 && e.file.trim() === ""))
    throw Error("--proxy-authorization-command / --proxy-authorization-file must not be empty");
  let o = (e.command ?? n[vZe])?.trim(),
    u = (e.file ?? n[RZe])?.trim(),
    r = u ? je(u) : u,
    t = o !== void 0 && o !== "",
    s = r !== void 0 && r !== "";
...
async function hIn(e, n) {
  let o = _e(),
    ...
    s = await pt({
      upstreamProxyUrl: e.upstreamProxyUrl,
      minter: t,
      upstreamTls: { ...(u && { cert: u.cert, key: u.key, passphrase: u.passphrase }), ...(r && { ca: r }) },
      onStatus: n.onStatus,
      onDebug: n.onDebug,
    }),
  ...
  if (((o.active = a), new Set(a.rewritten.map((d) => a.original[d])).size > 1))
    n.onStatus(
      `[runner:egress-proxy] note: ${a.rewritten.join("/")} named different proxies; all of them now go through the listener to ${Rl(e.upstreamProxyUrl)} (the one this runner resolves for its own traffic)`,
    );
  if ((dA(), process.env.CLAUDE_CODE_ENABLE_PROXY_AUTH_HELPER))
    n.onStatus(
  ...
  return (
    n.onStatus(
      `[runner:egress-proxy] enabled: minting Proxy-Authorization from the configured ${e.source.kind} for every CONNECT to the upstream proxy ${Rl(e.upstreamProxyUrl)}; this runner and its sessions now use the loopback listener 127.0.0.1:${s.port} (rewrote ${a.rewritten.join(", ")})`,
    ),
    (o.handle = {
      ...s,
      close: async () => {
...
async function HZe(e, n) {
  if (!e) return null;
  let o = Oe(e, n);
  try {
    if (!(await cu(ye(o), ue, `stat ${o}`)).isFile()) return null;
    return await cu(Et(o, yt.X_OK), ue, `access ${o}`), o;
  } catch {
    return null;
  }
}
...
async function yIn(e) {
  let n = {
    ...Ne(),
    CLAUDE_RUNNER_SESSION_ID: e.sessionId.replace(/^cse_/, "session_"),
    CLAUDE_RUNNER_SESSION_UUID: JV(e.sessionId),
    ...
    CLAUDE_RUNNER_CLIENT_PLATFORM: Sme(e.clientPlatform),
    ...
    let a = Le(e.hookPath, [], {
      ...
      d = () => {
        (u = true),
          ...
          (i = setTimeout(
            (h, _) => {
              _(`[runner:hook:checkout] still running after SIGTERM, sending SIGKILL to pid=${h.pid}`),
                h.kill("SIGKILL");
            },
            5000,
            a,
...
`,
        ...
        if (_) {
          let T = o.reduce((O, D) => O + D.length, 0);
          while (T > 4096 && o.length > 1) T -= o.shift().length;
        }
        return E;
      };
    a.stdout.on("data", (h) => {
      ...
      a.on("close", (h) => {
        if (i) clearTimeout(i);
        if ((e.signal?.removeEventListener("abort", d), p)) e.onDebug(`[runner:hook:checkout] ${Rl(p)}`);
        if (m) {
          let _ = Rl(m);
        ...
        if (u) s(Error("checkout hook aborted"));
          ...
          s(h === null ? new R(_, "checkout hook killed by signal") : new Uve(_, h, `exited with status ${h}`));
        }
      });
  });
  let r;
  try {
    r = await cu(ye(e.checkoutPath), ue, `stat ${e.checkoutPath}`);
  } catch (t) {
    if (X(t))
      throw new Uve(
  ...
  if (process.env.CLAUDE_RUNNER_SKIP_GIT_VERIFY !== "1")
    try {
      await cu(ye(Oe(e.checkoutPath, ".git")), ue, `stat ${e.checkoutPath}/.git`);
    } catch (t) {
      if (X(t))
        throw new Uve(
  ...
  d.push(
    ...
    setTimeout(
      (E, T, O) => {
        if (!i) return;
        E(
          `[runner:hook:post-session] still running after SIGTERM, sending SIGKILL to the hook's process group (pgid=${T.pid})`,
        ),
          O("SIGKILL");
      },
...
`),
        O = T.pop() ?? "";
      for (let D of T) e.onDebug(`[runner:hook:post-session] ${Rl(D)}`);
      return O;
    };
  return (
    t.stdout.on("data", (E) => {
    ...
    t.on("error", (E) => {
      e.onStatus(`[runner:hook:post-session] spawn error (ignored): ${Rl(E instanceof Error ? E.message : String(E))}`),
        p();
    }),
    t.on("close", (E) => {
      if (h) e.onDebug(`[runner:hook:post-session] ${Rl(h)}`);
      if (_) e.onDebug(`[runner:hook:post-session] ${Rl(_)}`);
      let T = Date.now() - o;
      if (i) e.onStatus(`[runner:hook:post-session] killed after timeout (${T}ms, ignored)`);
      else if (E !== 0) e.onStatus(`[runner:hook:post-session] exited ${E ?? "via signal"} (${T}ms, ignored)`);
      else e.onStatus(`[runner:hook:post-session] done (exit 0, ${T}ms)`);
      p();
    }),
    r
  );
