...
import { _Te } from "/$bunfs/root/chunk-rxqmmxnb.js";
...
async function T(o, a) {
  ...
  if (Ct()) return;
  let t = o.prRepository.indexOf("/"),
    i = o.prRepository.slice(0, t),
    d = o.prRepository.slice(t + 1),
    c = await cl(a),
    _ = c === "HEAD" ? "" : c;
  try {
    let r = await bt.post(
      u,
      { owner: i, repo: d, pr_number: o.prNumber, head_ref: _ },
      { auth: "session-jwt", host: "ccr-session", headers: { "anthropic-version": "2023-06-01" }, timeout: m },
    );
    if (!r.ok) {
      g("ccr_record_created_pr", r.reason), n(`recordCreatedPrToCcr: skipped (${r.reason})`);
      return;
    }
    y("ccr_record_created_pr"), n(`recordCreatedPrToCcr: RecordCreatedPR -> ${r.status}`);
  } catch (r) {
    let { kind: s, status: e } = os(r);
    if (e === 404 || e === 501) g("ccr_record_created_pr", "not_deployed");
    else p("ccr_record_created_pr", `${s}${e ? `_${e}` : ""}`);
    n(`recordCreatedPrToCcr: RecordCreatedPR failed: ${s}${e ? ` ${e}` : ""}`);
  }
}
