...
import { Ff, Qh } from "/$bunfs/root/chunk-s28wf80n.js";
import { Wl, Va, a } from "/$bunfs/root/chunk-fec4384a.js";
import { $e } from "/$bunfs/root/chunk-2masxyqj.js";
import { An } from "/$bunfs/root/chunk-5rt2mvvk.js";
import { $pe } from "/$bunfs/root/chunk-6k63g5t6.js";
import { ib, pP } from "/$bunfs/root/chunk-0spqrdaj.js";
import { c0 } from "/$bunfs/root/chunk-18bck2pp.js";
import { l0 } from "/$bunfs/root/chunk-jck8t8fq.js";
import { iwn, ie, hNe, DY } from "/$bunfs/root/chunk-8tgj5dp2.js";
import { IC } from "/$bunfs/root/chunk-r3qa2s4k.js";
import { eYn, h5e, pt } from "/$bunfs/root/chunk-w3c6n7jh.js";
import { Mjn, ree, Z_e, Jae, _in, Njn, Ujn, Ixe, rye } from "/$bunfs/root/chunk-qwp18h36.js";
...
var q = m(() =>
  T.object({
    timestamp: T.string(),
    path: T.enum(["npm-global", "npm-local", "native"]),
    outcome: T.enum(["success", "failed"]),
    status: T.string(),
    version_from: T.string(),
    version_to: T.string().nullable(),
    error_code: T.string().nullable(),
  }),
);
function L() {
  return z(be(), ".last-update-result.json");
}
function E() {
  return Te.state("last-update-result");
}
async function Yae(e, t) {
  if (O() && t) {
    let i = await t.write(E(), b(e), { mode: 438 & ~process.umask() });
    if (!i.ok) n(`Failed to record update result: ${i.error.code}`, { level: "error" });
    return;
  }
  try {
    await an().atomicWrite(L(), b(e));
  } catch (i) {
    n(`Failed to record update result: ${i}`, { level: "error" });
  }
}
async function DMt(e) {
  let t;
  if (O() && e) {
    let i = await e.read([E()]);
    if (!i.ok) return n(`Failed to read update result: ${i.error.code}`, { level: "error" }), null;
    ...
    try {
      ...
      if (!X(i)) n(`Failed to read update result: ${i}`, { level: "error" });
      return null;
    }
  try {
    let i = q().safeParse(V(t));
    return i.success ? i.data : null;
  } catch {
    return null;
  }
}
import { readFile as Q } from "fs/promises";
function Z(e, t) {
...
var Y = new J(
  () =>
    new j({
      platform: D,
      readOsRelease: () => Q("/etc/os-release", "utf8"),
      execFileNoThrow: $e,
...
function $lt() {
  return w().detectHomebrew();
}
function Y_e() {
  return w().getHomebrewCaskName();
}
function _() {
  return w().detectWinget();
}
function F() {
  return w().detectMise();
}
...
import { readFile as ee, realpath as K } from "fs/promises";
import { homedir as x } from "os";
import { stripVTControlCharacters as te } from "util";
import { delimiter as ne, join as g, posix as R, win32 as A } from "path";
function ae() {
  let e = process.argv[1] || "",
    t = process.execPath || process.argv[0] || "";
  if (D() === "windows") (e = e.split(A.sep).join(R.sep)), (t = t.split(A.sep).join(R.sep));
  return [e, t];
}
async function nee() {
  let [e, t] = ae();
  if (Wl()) {
    let c = be().replace(/\\/g, "/").replace(/\/+$/, "") + "/local/node_modules/";
    if (t.startsWith(c)) return "npm-local";
    if (t.includes("/node_modules/@anthropic-ai/")) return "npm-global";
    if ($lt() || _() || F() || B() || (await U()) || (await H()) || (await N()) || (await W()))
      return "package-manager";
    return "native";
  }
  if (Mjn()) return "npm-local";
  if (
    [
...
async function se() {
  if (Wl()) {
    try {
      return await K(process.execPath);
    } catch {}
    try {
      let e = await Va("claude");
      if (e) return e;
    } catch {}
    try {
      return await le().stat(g(x(), ".local/bin/claude")), g(x(), ".local/bin/claude");
    } catch {}
    return "native";
  }
  try {
...
function oe() {
  try {
    if (Wl()) return process.execPath || "unknown";
...
async function re() {
  let e = le(),
    t = [],
    i = g(x(), ".claude", "local");
  if (await ree()) t.push({ type: "npm-local", path: i });
  ...
  let o = await $e("npm", ["-g", "config", "get", "prefix"]);
  if (o.code === 0 && o.stdout) {
    ...
    if (u) {
      ...
      if (!p) {
        ...
        if (l) t.push({ type: "npm-global", path: d });
      }
    } else
      for (let p of r) {
        let l = h ? g(s, "node_modules", p) : g(s, "lib", "node_modules", p);
        try {
          await e.stat(l), t.push({ type: "npm-global-orphan", path: l });
        } catch {}
      }
  }
  let c = g(x(), ".local", "bin", "claude");
  try {
    await e.stat(c), t.push({ type: "native", path: c });
  } catch {}
  if (ie().installMethod === "native") {
    let s = g(x(), ".local", "share", "claude");
    try {
      if ((await e.stat(s), !t.some((h) => h.type === "native"))) t.push({ type: "native", path: s });
    } catch {}
  }
  return t;
}
function ce(e, t) {
  let i = D() === "windows",
    r = t;
  if (i) r = t.split(A.sep).join(R.sep).toLowerCase();
  return e.some((o) => {
    let c = o;
    if (i) c = o.split(A.sep).join(R.sep).toLowerCase();
    let P = c.replace(/\/+$/, ""),
      s = o.replace(/[/\\]+$/, "");
    return P === r || s === "~/.local/bin" || s === "$HOME/.local/bin";
  });
}
async function ue(e) {
  let t = [],
    i = [ib()];
  if (D() === "wsl" && l0()) i.unshift(c0);
  for (let s of i)
    try {
      let h = await ee(g(s, "managed-settings.json"), "utf-8"),
        d = V(h),
      ...
      if (u !== void 0 && typeof u !== "boolean")
        if (!Array.isArray(u))
          t.push({
            ...
            fix: `The field is silently ignored (schema .catch rescues it). Set it to true, or an array of: ${$pe.join(", ")}.`,
          });
        else {
          let p = u.filter((l) => typeof l === "string" && !$pe.includes(l));
          if (p.length > 0)
            t.push({
              ...
              fix: `These are silently ignored (forwards-compat). Known surfaces for this version: ${$pe.join(", ")}. Either remove them, or this client is older than the managed-settings intended.`,
            });
        }
      if (d && typeof d === "object" && pP(d)) break;
    } catch {}
  let r = iwn();
  if (r)
    t.push({
  ...
  if (e === "native") {
    ...
    if (!ce(s, d))
      if (D() === "windows") {
        let l = d.split(R.sep).join(A.sep);
        t.push({
          issue: `Native installation exists but ${l} is not in your PATH`,
        ...
        let l = Z_e(),
          k = Jae()[l],
          C = k ? k.replace(x(), "~") : "your shell config file";
        t.push({
  ...
  if (!Me(process.env.DISABLE_INSTALLATION_CHECKS)) {
  ...
  let c = await _in(),
    P = await Njn();
  if (e === "npm-local") {
    if (!(await Va("claude")) && !P)
      if (c)
        t.push({
          issue: "Local installation not accessible",
          fix: `Alias exists but points to invalid target: ${c}. Update alias: alias claude="~/.claude/local/claude"`,
        });
      else
        t.push({
          issue: "Local installation not accessible",
...
function pe() {
  ...
  if (pt.canMaskCredentialWarningFire()) pt.checkDependencies();
  let e = pt.getMaskCredentialWarning();
  if (!e) return [];
...
function me() {
  if (D() !== "linux") return [];
  let e = [],
    t = pt.getLinuxGlobPatternWarnings();
  if (t.length > 0) {
    let i = M(t);
    e.push({
...
async function J_e({ probeKeychain: e = false, storageV5: t } = {}) {
  let i = await nee(),
    ...
    P = await re(),
    s = await ue(i),
    { checkInstall: h } = await import("/$bunfs/root/chunk-bm8c720h.js");
  for (let y of await h())
    if (y.type === "error") s.push({ issue: y.message, fix: "Run claude install to repair the installation." });
  if ((s.push(...me()), s.push(...fe()), s.push(...ge()), s.push(...pe()), e)) {
    let y = await de();
    if (y) s.push(y);
  }
  if (i === "native") {
    let y = P.filter((f) => f.type === "npm-global" || f.type === "npm-global-orphan" || f.type === "npm-local"),
      I = D() === "windows";
    for (let f of y)
      if (f.type === "npm-global") {
        let S = "npm -g uninstall @anthropic-ai/claude-code";
        if (
          {
            ISSUES_EXPLAINER: "report the issue at https://github.com/anthropics/claude-code/issues",
            PACKAGE_URL: "@anthropic-ai/claude-code",
  ...
  let u = ie().installMethod || "not set",
    p = null;
  if (i === "npm-global") {
    if (((p = (await Ujn()).hasPermissions), !p && !DY()))
      s.push({
  ...
  return {
    ...
    autoUpdates: (() => {
      let y = DY();
      return y ? `disabled (${hNe(y)})` : "enabled";
    })(),
    hasUpdatePermissions: p,
    lastUpdateResult: C,
    multipleInstallations: P,
    warnings: s,
    packageManager: k,
    ripgrepStatus: v,
  };
}
export { Yae, DMt, $lt, Y_e, tee, nee, J_e };
