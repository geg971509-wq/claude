...
// Version: 2.1.252
import { K, gn, Ag, G5, WO } from "/$bunfs/root/chunk-f9h0bg01.js";
...
import { cU, $Ce, O_ } from "/$bunfs/root/chunk-q14dgq5g.js";
import { $6 } from "/$bunfs/root/chunk-tzhtxm67.js";
  ...
  for (let a of o.split(",")) {
    ...
    if (c) {
      let u = parseInt(c[1], 10),
        ...
        y = e === 0 && t === 6,
        m = y ? 7 : t;
      if (u > l || g < 1 || u < e || l > m) return null;
...
function HI(o) {
  ...
  return { minute: e[0], hour: e[1], dayOfMonth: e[2], month: e[3], dayOfWeek: e[4] };
}
function IOe(o, s) {
  let e = new Set(o.minute),
    t = new Set(o.hour),
...
function w(o, s) {
  return new Date(2000, 0, 1, s, o).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" });
}
function F(o, s) {
  let e = new Date();
  return (
    e.setUTCHours(s, o, 0, 0),
    e.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", timeZoneName: "short" })
  );
}
function ry(o, s) {
  let e = s?.utc ?? false,
...
function Ane(o) {
  ...
  if (e) {
    ...
    switch (i) {
      ...
      case "m":
        if (t > 59) return { error: "minute interval must be 1\u201359 (use hours instead)" };
        a = t === 1 ? "* * * * *" : `*/${t} * * * *`;
        break;
      case "h":
        if (t > 23) return { error: "hour interval must be 1\u201323 (use days instead)" };
        a = t === 1 ? "0 * * * *" : `0 */${t} * * *`;
        break;
      case "d":
        if (t === 1) {
        ...
        if (t > 28) return { error: "day interval must be 1\u201328 (use a cron expression)" };
        a = `0 0 */${t} * *`;
        break;
      default:
        return { error: "unknown interval unit" };
    }
    return { cron: a, human: ry(a) };
  }
  if (HI(s) !== null) return { cron: s, human: ry(s) };
...
async function ZVe(o) {
  let s = le(),
    e;
  try {
    e = await s.readFile(BX(o), { encoding: "utf-8" });
  } catch (r) {
    if (Ht(r)) return [];
    return h(r), [];
  }
  ...
  for (let r of i.tasks) {
    if (
      ...
      n(`[ScheduledTasks] skipping malformed task: ${b(r)}`);
      continue;
    }
    if (!HI(r.cron)) {
      n(`[ScheduledTasks] skipping task ${r.id} with invalid cron '${r.cron}'`);
      continue;
    }
    a.push({
      id: r.id,
...
async function Hht(o, s) {
  ...
  await O_(
    ...
    b(i, null, 2) +
      `
`,
    { encoding: "utf-8", allowSymlink: !t, checkParentDir: t, stagingDir: S(e, ".claude", cU) },
  );
}
async function cTe(o, s, e, t, i) {
  ...
  let c = await ZVe();
  return (
    c.push({ ...r, createdBySessionId: K(), createdByPid: process.pid, createdByProcStart: $6() }), await Hht(c), a
  );
}
async function Fz(o, s) {
  if (o.length === 0) return;
  if (s === void 0 && WO(o) === o.length) return;
  let e = new Set(o),
    t = await ZVe(s),
    i = t.filter((a) => !e.has(a.id));
  if (i.length === t.length) return;
  await Hht(i, s);
}
async function NXn(o, s, e) {
  if (o.length === 0) return;
  let t = new Set(o),
    i = await ZVe(e),
    a = false;
...
function t5e(o, s, e, t = kM) {
  let i = e5e(o, s);
  if (i === null) return null;
  let a = e5e(o, i);
  if (a === null) return i;
  let r = a - i;
  if (P.test(o) && t.cacheLeadMs > 0 && t.cacheLeadMs < r && r >= kht && r - t.cacheLeadMs < kht)
    return s + r - t.cacheLeadMs;
  let c = Math.min(C(e) * t.recurringFrac * r, t.recurringCapMs);
  return i + c;
}
function xht(o, s, e, t = kM) {
  let i = e5e(o, s);
  if (i === null) return null;
  if (new Date(i).getMinutes() % t.oneShotMinuteMod !== 0) return i;
  let a = t.oneShotFloorMs + C(e) * (t.oneShotMaxMs - t.oneShotFloorMs);
  return Math.max(i - a, s);
}
function FXn(o, s) {
  return o.filter((e) => {
    let t = e5e(e.cron, e.createdAt);
    return t !== null && t < s;
  });
}
export { HI, IOe, ry, Ane, kht, BX, ZVe, X6t, Hht, cTe, Fz, NXn, U2, e5e, kM, t5e, xht, FXn };
