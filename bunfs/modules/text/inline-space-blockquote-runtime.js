...
import { gi, k } from "/$bunfs/root/chunk-4ddxwr9r.js";
...
import { VZ, o, t, oo, hn } from "/$bunfs/root/chunk-he2phymk.js";
...
import { L5e, M5e } from "/$bunfs/root/chunk-z9bhq3k9.js";
import { mte } from "/$bunfs/root/chunk-zze8764r.js";
import { OR } from "/$bunfs/root/chunk-tk2aqta8.js";
import { vBe, YRe, dMn, J7t, ant, nE, Q7t, Z7t, m0t, eXt } from "/$bunfs/root/chunk-ndv7bxve.js";
...
import { du, z, C, Lt, F } from "/$bunfs/root/chunk-twm95mhz.js";
...
var le = 4,
  ...
  Xe = 200;
function me(s) {
  return `\u2026 ${s.toLocaleString()} more ${k(s, "row")} not shown`;
}
var qe = "\x1B[1m",
  Ge = "\x1B[22m";
function Y(s, i, l) {
...
var we = du(function (Kt) {
  let Oe = _(14),
    ...
    ee = Jt ?? Qt,
    bt;
  if (Oe[0] !== _e || Oe[1] !== Fe || Oe[2] !== De || Oe[3] !== ee || Oe[4] !== ve || Oe[5] !== Be)
    (bt = Ye(Be, ee, ve, _e, De, Fe)),
      (Oe[0] = _e),
      (Oe[1] = Fe),
      (Oe[2] = De),
      (Oe[3] = ee),
  ...
  if (O.kind === "ansi") {
    let ue;
    if (Oe[7] !== O.text) (ue = e(oo, { children: O.text })), (Oe[7] = O.text), (Oe[8] = ue);
    else ue = Oe[8];
    return ue;
  }
  let ue;
  if (Oe[9] !== O.headers || Oe[10] !== O.rows || Oe[11] !== O.truncatedCount || Oe[12] !== ee)
    (ue = e(Re, { headers: O.headers, rows: O.rows, terminalWidth: ee, truncatedCount: O.truncatedCount })),
      (Oe[9] = O.headers),
      (Oe[10] = O.rows),
...
${me(m)}`;
  ...
  function L(c) {
    return Math.max(se(W(c)), ie);
  }
  let P = s.header.map((c, p) => {
      let g = A(c.tokens);
      for (let T of b) g = Math.max(g, A(T[p]?.tokens));
      return g;
    }),
    B = s.header.map((c, p) => {
      let g = L(c.tokens);
      for (let T of b) g = Math.max(g, L(T[p]?.tokens));
      return g;
    }),
    N = s.header.length,
    R = 1 + N * 3,
  ...
  function U() {
    return {
      ...
      rows: b.map((c) => c.map((p) => y(p.tokens))),
      truncatedCount: m,
    };
  }
  function K(c, p) {
    ...
    for (let E = 0; E < T; E++) {
      ...
      for (let q = 0; q < c.length; q++) {
        let fe = g[q],
          ...
          gt = I[q],
          kt = p ? "center" : (s.align?.[q] ?? "left");
        Q += " " + m0t(Ne, se(Ne), gt, kt) + " \u2502";
      }
      j.push(Q);
    }
    return j;
...
function Re(Zt) {
  let je = _(9),
    { headers: $e, rows: He, terminalWidth: te, truncatedCount: ne } = Zt,
    G;
  if (je[0] !== $e || je[1] !== He || je[2] !== te || je[3] !== ne) {
    G = [];
    ...
    He.forEach((tn) => {
      ...
      if (
        (tn.forEach((nn, rn) => {
          let pe = $e[rn] || "";
          let Tt = nn.trimEnd().replace(/\n+/g, " ").replace(/\s+/g, " ").trim();
          if (!pe && !Tt) {
            return;
          }
          let on = pe ? te - se(pe) - 3 : te - 1;
          let sn = te - 2 - 1;
          let Le = Y(Tt, Math.max(on, 10));
          let ln = Le[0] || "";
          let re;
    ...
    (je[0] = $e), (je[1] = He), (je[2] = te), (je[3] = ne), (je[4] = G);
  } else G = je[4];
  const oe = G.join(`
`);
  let Rt;
  if (je[7] !== oe) (Rt = e(oo, { children: oe })), (je[7] = oe), (je[8] = Rt);
  else Rt = je[8];
  return Rt;
}
var Ke = 10,
...
function Ie({ token: s, indent: i, listDepth: l, ...u }) {
  ...
  return s.items.map((f, m) =>
    e(
      ...
      {
        item: f,
        marker: Z7t(l, s.ordered ? { number: a.first + m, ...a } : null),
        indent: i,
        listDepth: l,
...
function It({
  ...
  for (let d of s.tokens) {
    ...
    let w = L.at(-1);
    if (w?.kind === "inline") w.text += x;
    else L.push({ kind: "inline", text: x });
  }
            ...
            o,
            {
              ...
              children: [
                ...
                e(o, {
                  ...
                  width: P,
                  minWidth: Pe,
                  children: x === 0 && e(t, { dimColor: b, italic: S, children: i }),
                }),
                e(o, {
                  flexDirection: "column",
                  flexShrink: 1000,
                  minWidth: A,
                  children: e(oo, { dimColor: b, italic: S, wrap: w, children: X }),
                }),
              ],
            },
            x,
...
function We(s, i) {
  try {
    ...
    if (l instanceof RangeError) n(`marked.lexer overflow on ${i.length} chars`, { level: "error" });
    else h(l);
    return ut(i);
  }
}
function vt(s, i, l) {
  let u = s === null ? "" : `${l ? "_" : ""}${i.length}:${rCt(i)}`,
    a = s?.get(u);
...
function Ni(et) {
  ...
  if (At[0] !== tt.syntaxHighlightingDisabled)
    (Wt = tt.syntaxHighlightingDisabled ? null : OR()), (At[0] = tt.syntaxHighlightingDisabled), (At[1] = Wt);
  else Wt = At[1];
  let nt = Wt,
    Nt;
  if (At[2] !== nt || At[3] !== et) (Nt = e(lt, { ...et, highlight: nt })), (At[2] = nt), (At[3] = et), (At[4] = Nt);
  else Nt = At[4];
  return Nt;
}
function lt({
...
`),
      ...
      function K(M, c) {
        U(), Z(M, w === "none" ? 0 : c === "block" || w === "block" || I ? 1 : 0, c);
      }
      let J = D.findLastIndex((M) => M.type !== "space"),
        H = !L && !f && Qe(D);
      if (
        (D.forEach((M, c) => {
          if (M.type === "table") K(e(we, { token: M, highlight: y, linkCap: A }), "block");
            ...
            K(e(pt, { token: M, theme: W, highlight: y, dimColor: l, linkCap: A, isScreenReader: L }), "block");
          else
            x += nE(M, W, {
              listDepth: 0,
...
function pt(jn) {
  ...
  if (st[0] !== he || st[1] !== ke || st[2] !== ge || st[3] !== de || st[4] !== rt.tokens) {
    ...
    Bt = ae.italic(rt.tokens.map(be).join("").replace(/^\n+/, "").trimEnd());
    (st[0] = he), (st[1] = ke), (st[2] = ge), (st[3] = de), (st[4] = rt.tokens), (st[5] = Bt);
  } else Bt = st[5];
  let it = Bt,
    be;
