...
// Version: 2.1.252
import { P_e } from "/$bunfs/root/chunk-cagtjatw.js";
import { qrn, M6n } from "/$bunfs/root/chunk-75mcd7yq.js";
...
import { cE, FWe, b8 } from "/$bunfs/root/chunk-tcj8crqm.js";
import { Lr } from "/$bunfs/root/chunk-31xy83wr.js";
import { D } from "/$bunfs/root/chunk-7s7jqj2f.js";
import { randomBytes as b } from "crypto";
...
async function oat(n) {
  let r = (await vn().readAsync())?.mcpXaaIdp?.[IWe(n)];
  if (!r) return;
  if (r.expiresAt - Date.now() <= v * 1000) return;
  return r.idToken;
}
async function k(n, t, e) {
  await vn().mutate((r) => ({ ...r, mcpXaaIdp: { ...r.mcpXaaIdp, [IWe(n)]: { idToken: t, expiresAt: e } } }));
}
...
async function aAr(n, t) {
  try {
    return await vn().mutate((e) => ({
      ...e,
      mcpXaaIdpConfig: { ...e.mcpXaaIdpConfig, [IWe(n)]: { clientSecret: t } },
    }));
  } catch (e) {
...
async function JDt(n) {
  return (await vn().readAsync())?.mcpXaaIdpConfig?.[IWe(n)]?.clientSecret;
}
async function lAr(n) {
  let t = IWe(n);
  try {
    await vn().mutate((e) => {
...
function T(n, t, e, r) {
  let i = null,
    ...
    u = null,
    m = () => {
      if ((i?.removeAllListeners(), i?.on("error", () => {}), i?.close(), (i = null), s)) clearTimeout(s), (s = null);
  ...
  return new Promise((f, g) => {
    ...
    (i = x((a, o) => {
      ...
      if (A !== t) {
        o.writeHead(400, { "Content-Type": "text/html" }),
          o.end(cE({ ok: false, heading: "Sign-in failed", message: "State mismatch. Close this tab and try again." })),
      ...
      i.on("error", (a) => {
        if (a.code === "EADDRINUSE") {
          let o = D() === "windows" ? `netstat -ano | findstr :${n}` : `lsof -ti:${n} -sTCP:LISTEN`;
          d(Error(`XAA IdP: callback port ${n} is already in use. Run \`${o}\` to find the holder.`));
      ...
      i.unref(),
      (s = setTimeout((a) => a(Error("XAA IdP: login timed out")), _, d)),
      s.unref();
  });
}
async function Urn(n) {
  return Hr("mcp_xaa_idp_login", async () => {
    ...
    let i = await iat(t),
      ...
      m = b(32).toString("base64url"),
      f = { client_id: e, ...(n.idpClientSecret && { client_secret: n.idpClientSecret }) },
      { authorizationUrl: g, codeVerifier: p } = await qrn(t, {
        metadata: i,
        clientInformation: f,
        redirectUrl: u,
        scope: "openid",
      ...
      d = await M6n(t, {
        ...
        codeVerifier: p,
        redirectUri: u,
        fetchFn: (c, h) => fetch(c, { ...h, ...Ri({ url: String(c) }), signal: AbortSignal.timeout(S) }),
      });
    if (!d.id_token) throw Error("XAA IdP: token response missing id_token (check scope=openid)");
    let a = C(d.id_token),
      o = a ? a * 1000 : Date.now() + (d.expires_in ?? 3600) * 1000;
    try {
      await k(t, d.id_token, o), Z("xaa", `Cached id_token for ${t} (expires ${new Date(o).toISOString()})`);
    } catch (c) {
...
export { IWe, oat, sAr, YDt, aAr, JDt, lAr, iat, Urn };
