...
var Ee = (e) => {
  ...
  Ie = (e) => {
    ...
    return Object.assign(o, { authSchemePreference: R.normalizeProvider(e.authSchemePreference ?? []) });
  };
var xe = (e) =>
    Object.assign(e, {
    ...
    rules: [
      {
        ...
        rules: [
          ...
          { conditions: [{ ["fn"]: "stringEquals", ["argv"]: [c, "ap-south-1"] }], endpoint: p, ["type"]: "endpoint" },
          {
          ...
          { conditions: [{ ["fn"]: "stringEquals", ["argv"]: [c, "eu-north-1"] }], endpoint: p, ["type"]: "endpoint" },
          { conditions: [{ ["fn"]: "stringEquals", ["argv"]: [c, "eu-west-1"] }], endpoint: p, ["type"]: "endpoint" },
          { conditions: [{ ["fn"]: "stringEquals", ["argv"]: [c, "eu-west-2"] }], endpoint: p, ["type"]: "endpoint" },
          { conditions: [{ ["fn"]: "stringEquals", ["argv"]: [c, "eu-west-3"] }], endpoint: p, ["type"]: "endpoint" },
          { conditions: [{ ["fn"]: "stringEquals", ["argv"]: [c, "sa-east-1"] }], endpoint: p, ["type"]: "endpoint" },
          { conditions: [{ ["fn"]: "stringEquals", ["argv"]: [c, "us-east-1"] }], endpoint: p, ["type"]: "endpoint" },
          { conditions: [{ ["fn"]: "stringEquals", ["argv"]: [c, "us-east-2"] }], endpoint: p, ["type"]: "endpoint" },
          { conditions: [{ ["fn"]: "stringEquals", ["argv"]: [c, "us-west-1"] }], endpoint: p, ["type"]: "endpoint" },
          { conditions: [{ ["fn"]: "stringEquals", ["argv"]: [c, "us-west-2"] }], endpoint: p, ["type"]: "endpoint" },
          {
            endpoint: {
        ...
        rules: [
          {
            ...
            rules: [
                    ...
                    rules: [
                      {
                        ...
                        endpoint: { url: "https://sts.{Region}.amazonaws.com", properties: l, headers: l },
                        ["type"]: "endpoint",
                      },
                      {
                        endpoint: {
              ...
              Ce,
              {
                endpoint: { url: "https://sts.{Region}.{PartitionResult#dnsSuffix}", properties: l, headers: l },
                ["type"]: "endpoint",
              },
            ],
            ["type"]: "tree",
...
var po = new I.EndpointCache({
  ...
  je = (e, t = {}) => po.get(e, () => I.resolveEndpoint(Le, { endpointParams: e, logger: t.logger }));
...
var Xe = j(Pt(), 1),
  ...
  tt = (e) => {
    ...
    return {
      ...
      runtime: "node",
      defaultsMode: t,
      authSchemePreference: e?.authSchemePreference ?? _.loadConfig(x.NODE_AUTH_SCHEME_PREFERENCE_OPTIONS, n),
      ...
      region: e?.region ?? _.loadConfig(f.NODE_REGION_CONFIG_OPTIONS, { ...f.NODE_REGION_CONFIG_FILE_OPTIONS, ...n }),
      requestHandler: W.NodeHttpHandler.create(e?.requestHandler ?? o),
      retryMode:
        e?.retryMode ??
        _.loadConfig(
          { ...k.NODE_RETRY_MODE_CONFIG_OPTIONS, default: async () => (await o()).retryMode || Ye.DEFAULT_RETRY_MODE },
          e,
        ),
      sha256: e?.sha256 ?? Be.Hash.bind(null, "sha256"),
      streamCollector: e?.streamCollector ?? W.streamCollector,
      useDualstackEndpoint: e?.useDualstackEndpoint ?? _.loadConfig(f.NODE_USE_DUALSTACK_ENDPOINT_CONFIG_OPTIONS, n),
      useFipsEndpoint: e?.useFipsEndpoint ?? _.loadConfig(f.NODE_USE_FIPS_ENDPOINT_CONFIG_OPTIONS, n),
...
var st = (e, t) => {
  let o = Object.assign(
    w.getAwsRegionExtensionConfiguration(e),
    H.getDefaultExtensionConfiguration(e),
    M.getHttpHandlerExtensionConfiguration(e),
    ot(e),
  );
  return (
    ...
    Object.assign(
      e,
      w.resolveAwsRegionExtensionConfiguration(o),
      H.resolveDefaultRuntimeConfig(o),
      M.resolveHttpHandlerRuntimeConfig(o),
      nt(o),
    )
  );
};
class u extends mt.Client {
  ...
  constructor(...[e]) {
    ...
    this.initConfig = t;
    let o = xe(t),
      s = F.resolveUserAgentConfig(o),
      n = L.resolveRetryConfig(s),
      d = at.resolveRegionConfig(n),
      r = O.resolveHostHeaderConfig(d),
      g = pt.resolveEndpointConfig(r),
      A = Ie(g),
      S = st(A, e?.extensions || []);
    (this.config = S),
      this.middlewareStack.use(ct.getSchemaSerdePlugin(this.config)),
      this.middlewareStack.use(F.getUserAgentPlugin(this.config)),
      this.middlewareStack.use(L.getRetryPlugin(this.config)),
      this.middlewareStack.use(dt.getContentLengthPlugin(this.config)),
      this.middlewareStack.use(O.getHostHeaderPlugin(this.config)),
...
var mo = [0, "com.amazonaws.sts", "accessKeySecretType", 8, 0],
  ...
  yo = [3, "com.amazonaws.sts", "AssumeRootResponse", 0, ["Credentials", "SourceIdentity"], [[() => h, 0], 0]],
  h = [
    3,
    "com.amazonaws.sts",
    "Credentials",
...
m.TypeRegistry.for("com.amazonaws.sts").registerError(Po, N);
var Co = [
  -3,
  "com.amazonaws.sts",
...
var vo = [3, "com.amazonaws.sts", "FederatedUser", 0, ["FederatedUserId", "Arn"], [0, 0]],
  ...
  ko = [3, "com.amazonaws.sts", "GetCallerIdentityResponse", 0, ["UserId", "Account", "Arn"], [0, 0, 0]],
  Wo = [3, "com.amazonaws.sts", "GetDelegatedAccessTokenRequest", 0, ["TradeInToken"], [[() => _o, 0]]],
  wo = [
    3,
    "com.amazonaws.sts",
    ...
    return async (s) => {
      if (!o) {
        let {
          ...
          se = !to(T);
        o = new t({ ...e, userAgentAppId: v, profile: S, region: ne, requestHandler: se ? T : void 0, logger: A });
      }
      let { Credentials: n, AssumedRoleUser: d } = await o.send(new C(s));
      if (!n || !n.AccessKeyId || !n.SecretAccessKey)
        throw Error(`Invalid response from STS.assumeRoleWithWebIdentity call with role ${s.RoleArn}`);
      let r = Yt(d),
        g = {
