...
var SFe = ["", "-staging-oauth", "-local-oauth", "-custom-oauth"];
function T4() {
...
var hU = 31536000,
  OHr = 365,
  tkn = Math.round(hU / 86400),
  M_ = "user:inference",
  XN = "user:profile",
  s = "org:create_api_key",
  ud = "oauth-2025-04-20",
  r = [s, XN],
  x5 = [XN, M_, "user:sessions:claude_code", "user:mcp_servers", "user:file_upload"],
  nkn = te([...r, ...x5]),
  wJ = ["user:design:read", "user:design:write"],
  Dwr = ["user:projects:read", "user:projects:write", "user:plugins"];
function Ife(t) {
...
var _ = {
  ...
  Voe = "https://claude.ai/oauth/claude-code-client-metadata",
  n = void 0;
function E() {
...
function zt() {
  ...
  if (o) {
    let a = o.replace(/\/$/, "");
    if (!bFe.includes(a)) throw Error("CLAUDE_CODE_CUSTOM_OAUTH_URL is not an approved endpoint.");
    t = {
      ...t,
      BASE_API_URL: a,
...
export { SFe, T4, hU, OHr, tkn, M_, XN, ud, x5, nkn, wJ, Dwr, Ife, Voe, bFe, zt };
