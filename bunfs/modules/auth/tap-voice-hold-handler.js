...
// Version: 2.1.252
import { p$e } from "/$bunfs/root/chunk-qq1mdtb5.js";
...
import { El, Ae, ie } from "/$bunfs/root/chunk-8tgj5dp2.js";
...
import { Sze, swe } from "/$bunfs/root/chunk-9kr4hqvs.js";
...
var I = async (a, e) => {
  if (!swe()) {
    if (!El()) return { type: "text", value: "Voice mode requires a Claude.ai account. Please run /login to sign in." };
    let t = od("allow_voice_mode", "Voice mode", "is");
...
Install SoX manually for audio recording.`
  ...
  if (!(await C(e.session.host))) {
    ...
    return (
      ...
      { type: "text", value: `Microphone access is denied. To enable it, go to ${t}, then run /voice again.` }
    );
  }
  ...
  if (f || u)
    await Ae(
      (t) => ({ ...t, voiceLangHintShownCount: m + (u ? 1 : 0), voiceLangHintLastLanguage: n.code }),
      e.storageV5,
    );
  let S = r.note ? ` ${r.note}` : "";
