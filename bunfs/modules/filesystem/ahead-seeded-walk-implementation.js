...
import { Ha, Hc, GXe } from "/$bunfs/root/chunk-6k63g5t6.js";
import { _En, LNe, KAe, soe, Y3 } from "/$bunfs/root/chunk-0spqrdaj.js";
import { l0 } from "/$bunfs/root/chunk-jck8t8fq.js";
import { uqt, KTn, XTn, gor, Bwt, YTn, JTn, QTn, Mo, $H, $T, ob } from "/$bunfs/root/chunk-988p40e0.js";
...
function v() {
  ...
  return t !== void 0 && E(t) === Y3.default ? t : void 0;
}
async function Dyn(t, e) {
  if (!O()) return;
  let s = e.epoch;
  await Promise.all([_(t, e), R(t, e), F("managed-settings file tier", () => M(t, e, s))]),
    await F("project/local layers", () => B(t, e, e.epoch));
}
async function F(t, e) {
  try {
    await e();
  } catch (s) {
    n(`settingsPrime: start-up seed of the ${t} failed: ${l(s)}; the file reads serve`, { level: "warn" });
  }
}
async function R(t, e) {
  try {
    ...
    n(`settingsPrime: ownership read-ahead for the local settings root failed: ${l(s)}; the probe runs as today`, {
      level: "warn",
    });
  }
}
async function _(t, e) {
  try {
    ...
    if (e.parsedFiles.has(s)) {
      n("settingsPrime: user settings already read before start-up; seed skipped");
      return;
    }
    let i = e.epoch,
      r = await uqt(t, s);
    if (r.kind === "absent") {
      if ((KAe(s), e.seedParsedFile(s, "userSettings", soe(), i)))
        n("settingsPrime: user settings absent; seeded as none");
      return;
    }
    if (r.kind !== "seeded") {
      n(`settingsPrime: start-up seed skipped (${Bwt(r)}); the file read serves`);
      return;
    }
    if (e.seedParsedFile(s, "userSettings", r.parsed, i))
      n(`settingsPrime: user settings seeded at start-up (${r.size} bytes)`);
  } catch (s) {
    n(`settingsPrime: start-up seed failed: ${l(s)}; the file read serves`, { level: "warn" });
  }
}
class JZn {
  storageV5;
  ...
  async run() {
    let t = ZZn(this.storageV5, this.store).then(
  ...
  async seedLogged(t, e) {
    try {
      ...
      if (!this.disposed) this.logThrowOnce(s, this.loggedHostFilesThrowMessages);
  ...
  transition(t) {
    let e = this.state;
    if (
      ((this.state = t), e.kind === t.kind && (t.kind !== "failing" || (e.kind === "failing" && e.code === t.code)))
    ) {
      this.runsInState++;
      return;
    }
    if (e.kind === "failing" && t.kind === "seeded")
      n(`settingsPrime: backend read recovered after ${this.runsInState} failing run(s)`);
    switch (((this.runsInState = 1), t.kind)) {
      ...
      case "absent":
        ...
        return;
      case "oversize":
        n("settingsPrime: user settings not seeded (oversize); raw path serves");
        return;
      case "failing":
        n(
  ...
  onThrow(t) {
    if (
      (this.consecutiveThrows++, (this.state = { kind: "broken" }), this.logThrowOnce(t), this.consecutiveThrows >= b)
    )
      n(`settingsPrime: disabled after ${b} consecutive failures; raw path serves`, { level: "warn" }), this.dispose();
  }
  logThrowOnce(t, e = this.loggedThrowMessages) {
    let s = l(t);
    if (e.has(s) || e.size >= D) return;
    e.add(s), h(t);
  }
  primes(t) {
  ...
  async whenIdle() {
    while (this.inFlight !== null || this.followUpQueued) await (this.inFlight ?? Promise.resolve());
  }
  dispose() {
    if (
      ((this.disposed = true),
...
async function Oyn(t, e) {
  if (!O() || t === void 0) return;
  if (e.primer !== void 0) {
    if (!e.primer.primes(t)) n("settingsPrime: store already primed through another backend; second prime ignored");
    return;
  }
  GXe(t).catch(h), (e.primer = new JZn(t, e)), await e.primer.whenIdle();
}
async function m8e(t) {
  let e = Ha(),
...
async function z(t, e) {
  ...
  let s =
    e.primer === void 0
      ? []
      : (await Promise.all([...qyr(t).map((i) => KTn(e, i)), N(t, e)])).flat().filter((i) => i !== void 0);
  if ((Hc(s.length > 0 ? { userLayer: "retain" } : void 0), s.length === 0)) return;
  return () => {
    for (let i of s) i();
  };
}
...
function T(t) {
  ...
  for (let s of ["projectSettings", "localSettings"]) {
    ...
    let i = Mo(s);
    if (i !== void 0) e.push(YTn(t, s, i, s === "projectSettings" ? "project settings" : "local settings"));
    if (s === "localSettings") {
      let r = $H();
      if (r !== void 0 && r !== i) e.push(YTn(t, s, r, "legacy local settings"));
    }
  }
  return e;
}
async function QZn(t, e) {
  let s = t.hostFiles;
  if (!s.serves("system")) {
  ...
  let i = LNe(l0()),
    r = e.policyWalkCount,
    a = i.map((c) => JTn(t, f(c, "managed-settings.json"), "managed settings", e)),
    [o, g] = await Promise.all([
      Promise.all(i.map((c) => gor(s, f(c, "managed-settings.d")))),
  ...
  for (let [c, S] of i.entries()) {
    ...
    if (p.names.length === 0) {
      n(`settingsPrime: ${m} has no drop-ins to read ahead; the folder read confirms`), d.unlisted.push(m);
      continue;
    }
    d.listings.push({ dir: m, names: p.names });
    for (let I of p.names) u.push(JTn(t, f(m, I), "managed settings drop-in", e));
  }
  return d.layers.push(...u), { ...d, reads: Promise.all(u.map((c) => c.read())).then((c) => [...g, ...c]) };
}
...
function P(t, e, s) {
  ...
  for (let { basePath: r, dropInDir: a } of i)
    if ((e.seedFolderListing(a, L, s), !e.walkRead(r))) e.seedParsedFile(r, "policySettings", soe(), s);
  return {
    listings: i.map(({ dropInDir: r }) => ({ dir: r, names: L })),
    layers: i.map(({ basePath: r }) => ({ path: r, parsed: soe() })),
  };
}
...
function A(t, e, s, i, r) {
  ...
  for (let [g, d] of e.layers.entries()) {
    let u = s[g];
    if (u.kind !== "seeded") {
      n(`settingsPrime: ${d.label} not seeded (${Bwt(u)}); the file read serves`),
        t.managedFileReads.delete(d.path),
        t.unseedParsedFile(d.path, d.source, r);
      continue;
    }
    o.add(d.path), t.managedFileReads.set(d.path, { contentHash: u.contentHash, parsed: u.parsed });
    let c = i.get(j(d.path)),
      S = c !== void 0 && c !== "install" ? c : t.policyInstallVerdict(d.path, u.parsed, e.walksAtReadStart);
    if (S === "raced") {
    ...
    if (t.seedParsedFile(d.path, d.source, u.parsed, r)) a.push({ layer: d, parsed: u.parsed });
  }
  for (let g of [...t.managedFileReads.keys()])
    if (!o.has(g)) t.managedFileReads.delete(g), t.unseedParsedFile(g, "policySettings", r);
  return a;
}
function U(t, e) {
  for (let s of [...t.managedFileReads.keys()])
    t.managedFileReads.delete(s), t.unseedParsedFile(s, "policySettings", e);
  for (let s of LNe(l0())) t.clearFolderListing(f(s, "managed-settings.d"), e);
}
function C(t, e, s) {
...
function x(t, e, s, i) {
  for (let [r, a] of e.entries()) {
    let o = s[r],
      g = o.kind === "seeded" ? o.parsed : o.kind === "absent" && a.whenAbsent === "seedAbsence" ? soe() : void 0;
    if (g !== void 0 && t.walkReadDiffers(a.path, g))
      n(`settingsPrime: ${a.label} not installed (the file read already saw different content this generation)`);
    else if (o.kind === "seeded") t.seedParsedFile(a.path, a.source, o.parsed, i);
    else if (g !== void 0) KAe(a.path), t.seedParsedFile(a.path, a.source, g, i);
    else n(`settingsPrime: ${a.label} not seeded (${Bwt(o)}); the file read serves`);
  }
}
async function N(t, e) {
  ...
  try {
    ...
    if (i !== void 0)
      return [
        ...i.listings.map(({ dir: g, names: d }) => e.retainFolderListing(g, d)),
        ...i.layers.map(({ path: g, parsed: d }) => e.retainLayer(g, d)),
      ];
    let r = await QZn(t, e);
    if (r === void 0) return U(e, s), [];
    let a = await r.reads;
    if (e.epoch !== s) return [];
    let o = C(e, r, s);
    return [
      ...o.kept.map(({ dir: g, names: d }) => e.retainFolderListing(g, d)),
      ...A(e, r, a, o.verdicts, s).map(({ layer: g, parsed: d }) => e.retainLayer(g.path, d)),
    ];
  } catch (i) {
    return n(`settings: managed settings not re-seeded: ${l(i)}; the file reads serve`, { level: "warn" }), [];
  }
}
async function Gyr(t, e) {
  H(e, await ZZn(t, e));
}
async function ZZn(t, e) {
  ...
  let [r, a, o] = await Promise.allSettled([
    ...
    c = W(g, d, u);
  if ("skipped" in c) {
    n(`settingsPrime: ownership of the local settings root not read ahead (${c.skipped}); the probe runs as today`);
    return;
  }
  return { root: s, uids: c.uids };
}
function H(t, e) {
  if (e !== void 0 && t.localStoreProbes.primeCanonicalRootOwnerUids(e.root, e.uids))
    n("settingsPrime: ownership of the local settings root read ahead through the backend");
}
function y(t) {
...
function W(t, e, s) {
  ...
  let i = w(t.value);
  if (i === void 0) return { skipped: t.value.kind === "absent" ? "the root is absent" : "no owner uid for the root" };
  let r = w(e.value);
  if (r === void 0)
    return {
      skipped:
        e.value.kind === "absent" ? "no .git entry (left to the probe's per-call throw)" : "no owner uid for .git",
    };
  let a = null;
...
export { Dyn, JZn, Oyn, m8e, qyr, QZn, Gyr, ZZn };
