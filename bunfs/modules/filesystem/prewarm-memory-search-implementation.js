...
import { b0, bR, WAt } from "/$bunfs/root/chunk-werv747a.js";
...
var re = (e) => e === "a" || e === "e" || e === "i" || e === "o" || e === "u",
  ...
  Ht = (e) => {
    ...
    let t = kt(e);
    return (t = Nt(t)), (t = Ct(t)), (t = Le(t, _t)), (t = Le(t, Lt)), (t = Wt(t)), (t = jt(t)), (t = Ut(t)), t;
  },
  Ue = b0(Ht, (e) => e, 50000),
  eo = bR(Ue.cache),
...
var Gt = /[\p{L}\p{M}\p{N}]+/gu,
  ...
  Qt = (e) => {
    ...
    for (let o = 1; o < e.length; o++) {
      let s = e[o - 1] ?? "",
        ...
        i = e[o + 1] ?? "",
        a = i === "s" && o + 2 === e.length;
      if (((Se(s) || xe(s)) && ae(r)) || (ae(s) && ae(r) && Se(i) && !a) || (He(s) && xe(r)) || (xe(s) && He(r)))
  ...
  $t = [
    [new RegExp(Z + String.raw`[cC]\+\+` + ie, "gu"), " cpp "],
    [new RegExp(Z + String.raw`[cC]#` + ie, "gu"), " csharp "],
    [new RegExp(Z + String.raw`[fF]#` + ie, "gu"), " fsharp "],
    [new RegExp(Z + String.raw`\.[nN][eE][tT]` + ie, "gu"), " dotnet "],
  ],
  Vt = new RegExp(Z + String.raw`([vV]?\d+(?:\.\d+)+)`, "gu"),
  Kt = /^(?![\u3040-\u30FF\u3400-\u4DBF\u4E00-\u9FFF\uAC00-\uD7AF\uF900-\uFAFF])[\p{L}\p{N}]/u,
  O = /[\u3040-\u30FF\u3400-\u4DBF\u4E00-\u9FFF\uAC00-\uD7AF\uF900-\uFAFF]/,
  Yt =
  ...
  ce = (e, t) => {
    let n = e.replace(/\[\[([^[\]]*)\]\]/g, " $1 ");
    for (let [r, i] of $t) n = n.replace(r, (a, c) => c + i);
    n = n.replace(Vt, (r, i, a, c, l) => {
      let d = l.codePointAt(c + r.length);
      if (d !== void 0 && Kt.test(String.fromCodePoint(d))) return r;
      return `${i}${a} ${a.replaceAll(".", "")}`;
    });
    let o = n.match(Gt) ?? [],
      s = o
        .flatMap((r) => {
          let i = Jt.test(r) ? r.normalize("NFKC") : r;
          if (O.test(i)) {
            let c = qt(Zt(i.match(Yt) ?? [])),
              l = t.compounds ? en(c) : [],
              d = c.flatMap((h) => {
                if (!O.test(h)) {
                  let m = Qe(h);
            ...
            return d.push(...l), [...d.filter((h) => h.compound), ...d.filter((h) => !h.compound)];
          }
          let a = Qe(i);
...
var x = ["filename", "frontmatter", "headings", "body"],
  ...
  an = (e) => {
    ...
    for (let o of e) {
      ...
      let i = n !== void 0 ? void 0 : o.match(on)?.[1];
...
`),
  ...
  we = (e, t) => {
    let n = t.replace(/^\uFEFF/, ""),
      o = n.match(tn),
      s = o !== null && nn.test(o[1] ?? ""),
      r = s ? (o[1] ?? "") : "",
      i = s ? n.slice(o[0].length) : n,
      { headings: a, body: c } = an(i.split(/\r?\n/));
    return { path: e, fields: { filename: L(rn(e)), frontmatter: L(r), headings: dn(a), body: cn(c) } };
  };
var S = x.length,
  ...
  Pe = (e, t, n) => {
    ...
    let o = e.nextDocId++;
    for (let [r, i] of ln(t)) Ee(e, r, o, i);
    let s = Object.fromEntries(x.map((r) => [r, Q(t.fields[r], (i) => i !== "")]));
    for (let r of x) if (((e.fieldLengthTotals[r] += s[r]), s[r] > 0)) e.fieldDocCounts[r]++;
    return (
      e.docs.set(o, { path: t.path, hash: n.hash, size: n.size, mtimeMs: n.mtimeMs, fieldLengths: s }),
      e.pathToId.set(t.path, o),
      o
    );
  },
  et = (e, t, n) => {
    for (let o of x) if (((e.fieldLengthTotals[o] -= n.fieldLengths[o]), n.fieldLengths[o] > 0)) e.fieldDocCounts[o]--;
  ...
  tt = (e, t, n) => {
    for (let [o, s] of e.terms) {
      let r = ue(s, t);
      if (r < 0) continue;
      if ((un(s, r), s.size === 0)) e.terms.delete(o);
    }
    et(e, t, n);
  },
  k = (e, t) => {
  ...
  nt = (e, t) => {
    ...
    for (let [s, r] of e.terms) if ((fn(r, o), r.size === 0)) e.terms.delete(s);
  ...
  ot = (e) => ({
    ...
    avgFieldLength: Object.fromEntries(
      x.map((t) => [t, e.fieldDocCounts[t] === 0 ? 0 : e.fieldLengthTotals[t] / e.fieldDocCounts[t]]),
    ),
  });
var Re = {
    k1: 1.2,
  ...
  bn = (e, t, n) => {
    ...
    let s = o.flatMap((l, d) => Array.from(l, (h) => ({ pos: h, termIdx: d }))).sort((l, d) => l.pos - d.pos),
      r = new Map(),
      i = [],
      a = 0,
      c = 1 / 0;
    ...
    return [...h]
      ...
      .map((p) => {
        let f = e.docs.get(p),
          ...
          I = s.reduce((v, R, E) => {
            if (m[E] < 0) return v;
            let D = gn(c[E], m[E], f.fieldLengths, a.avgFieldLength, o);
            return v + l.get(R) * (D / (o.k1 + D));
          }, 0),
          w = i.reduce(
            (v, R) => (st(e, p, R) ? v + o.phraseBoost * R.reduce((E, D) => E + (l.get(D) ?? d.get(D) ?? 0), 0) : v),
            0,
          ),
          { span: T, termCount: y } = o.proximityWeight > 0 ? bn(e, p, g) : dt,
...
import { lstat as Me, open as xn, readdir as Sn, realpath as ne, stat as In } from "fs/promises";
import { join as K, sep as wn } from "path";
var B = Symbol("transient-read-failure"),
  ...
  A = (e) => (Fn(e) ? "structural" : En(e) ? "permission" : "other"),
  lt = async (e, t) => {
    let n = await xn(e, ea());
    try {
      let o = await n.stat();
      ...
      if (t?.withinReal !== void 0) {
        ...
        if (s === void 0 || !s.startsWith(t.withinReal + wn))
          throw Object.assign(Error("resolves outside the store root"), { code: "ENOTREG" });
        let r = await In(s).catch((i) => {
          if (A(i) === "structural") return;
          throw i;
        });
        if (r === void 0) throw Object.assign(Error("resolves outside the store root"), { code: "ENOTREG" });
...
var Rn = (e, t, n = new Set(), o = new Set(), s) => {
  ...
  gt = async (e, t, n = N, o) => {
    ...
    if (i === B) return s;
    let a = Rn(e, r.files, r.skipped, r.skippedSubtrees, (y) => De(y, n)),
      c = [],
      l = [];
    await Promise.all(
    ...
    for (let y of a.toIndex) {
      ...
      let P = await lt(K(t, y.path), { withinReal: i, maxBytes: n.maxFileBytes }).catch((Y) =>
        ut(Y) ? fe : A(Y) === "structural" ? void 0 : B,
      );
      if (P === B) {
        w++;
        continue;
...
var yt = 10,
  ...
  ze = (e, t) => {
    let n = e[t];
    if (typeof n !== "number" || !Number.isInteger(n) || n < 0 || n > H) throw Error("malformed posting data");
    return n;
  },
  kn = (e, t, n, o) => {
    let s = x.map(() => 0),
    ...
    while (i < n.length) {
      ...
      x.forEach((u, p) => {
        ...
        for (let g = 0; g < s[p]; g++) {
          let I = ze(n, i++);
          if (((m = m < 0 ? I : m + I), (g > 0 && I === 0) || m > H)) throw Error("malformed posting positions");
          f.push(m);
        }
      }),
        Ee(e, t, d, r);
  ...
  Nn = (e) => {
    ...
    if (x.some((n) => !Number.isSafeInteger(t.fieldLengthTotals[n]))) throw Error("malformed doc metadata");
  ...
  bt = async (e, t, n = Oe) => {
    ...
    if (s > n) {
      ...
      let r = await An(t).catch(() => {
        return;
      });
      if (r !== void 0 && r.size > n) await Dn(t).catch(() => {});
      return 0;
    }
    return await Wn(t, o, 384), s;
  },
  ...
  xt = async (e) => {
    ...
    try {
      ...
      if (n.size > Oe) {
        ...
        throw Object.assign(Error("index file oversized"), { code: ke, head: s.subarray(0, r).toString("utf8") });
      }
...
var _n = ".bm25-index.json",
  ...
  Pt = async (e, t, n = 5, o = Re) => {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      for (let l of c) {
        let d = e.index.pathToId.get(l.path);
        if (!(await mt(e.index, e.dir, l.path, e.options)) && d !== void 0 && e.index.unverified.has(d)) s.add(l.path);
      }
    }
...
${vt(e.slice(n), false)}`;
  ...
  Xn = (e, t, n) => {
    if (e.terms.length <= Tt && e.boostPhrases.every((s) => s.length <= 16) && e.boostPhrases.length <= 8) return e;
...
class Dt {
  open = WAt(
    async (e, t, n, o, s) => {
      ...
      if (s && i.lastSync.scanCompleted && (!i.loadedFromDisk ? i.index.docs.size > 0 : Bn(i.lastSync)))
        if (i.persistable)
          await Et(i).catch(() => {
...
var Mt = new Mn(() => new Dt()),
  zt = async (e, t, n, o, s, r) => {
    let i = e.open(t, n, o, s, r),
      a = await i;
    if (!a.store.lastSync.scanCompleted || a.store.lastSync.transientSkips > 0 || a.store.index.docs.size === 0)
  ...
  Jo = async (e, t, n, o, s = 5, r = null, i, a = false, c) => {
    ...
    let I = typeof t === "string" ? Qn(t) : "",
      w = typeof t === "string" ? Xn(jn(Ae(I)), u.index, () => U(I)) : ct(t.exact_terms, t.conceptual_keywords),
      T = await Pt(u, w, s + 5),
      y =
        c === void 0
          ? i === void 0
      ...
      D = await Promise.all(
        y.map(async (F) => {
          ...
          let J = await Ln(_).catch((ye) => (A(ye) === "structural" ? void 0 : he));
          if (J === he) return [];
          if (J === void 0) {
            if (P !== void 0) R.push(F.path);
            return [];
    ...
    let Y = D.flat(),
      Ot = m();
    return { ranked: Y.filter((F) => F.score >= Ot).slice(0, s), stats: g(u.lastSync, Y, w.terms.length) };
  };
