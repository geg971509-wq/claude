...
import { CI, Bv, Rk } from "/$bunfs/root/chunk-y8sx7bf9.js";
import { rst, ost, a2n, ist, sst, l2n, dWe, ast, Jhe, znn, lst, cst } from "/$bunfs/root/chunk-cw939gyv.js";
...
import "/$bunfs/root/chunk-5ydykk8n.js";
...
import { mWe } from "/$bunfs/root/chunk-1y4te1a7.js";
import "/$bunfs/root/chunk-r8n98t6s.js";
import "/$bunfs/root/chunk-b4rwyymp.js";
...
async function T({
  ...
  let _ = a();
  try {
    let { store: r } = a2n({ objectFormat: "sha1" }),
      u = rst({ store: r, identity: sst });
    await using f = await CI(Bv(), { gitRoot: t, realRoot: i }).catch(
    ...
    if (e.kind === "refused")
      return e.reason === "too_large"
        ...
        : { kind: "refused", reason: e.reason, detail: O(e.detail) };
    if (e.heldBack.length > 0) {
      ...
      return {
        kind: "refused",
        reason: "busy",
        detail: `${e.heldBack.length} ${k(e.heldBack.length, "file")} changed while ${k(e.heldBack.length, "it was", "they were")} being read (${y}${e.heldBack.length > B ? ", \u2026" : ""}); let whatever is writing to this folder finish and start again`,
      };
    }
    if (e.stats.bytes > p)
      return {
        kind: "refused",
        reason: "too_large",
        detail: R({ totalBytes: e.stats.bytes, capBytes: p, largest: [] }),
      };
    let b = await u.writeCommit({ tree: e.rootTree, parents: [], message: "sync 0", whenUnix: Math.floor(a() / 1000) });
    if (!b.ok)
      return { kind: "refused", reason: "internal", detail: `the seed commit could not be written (${b.reason})` };
    let o = await u.objectsToSend({ tip: b.id, peerHolds: [] });
    if (o.missing.length > 0)
      return { kind: "refused", reason: "internal", detail: `${o.missing.length} seed objects missing from the build` };
    let S = await v(r, [...o.ids.commits, ...o.ids.trees, ...o.ids.blobs]);
    if (S === null) return { kind: "refused", reason: "internal", detail: "a seed object could not be read back" };
    let C = Buffer.concat([
        mWe({ version: 2, capabilities: [], prerequisites: [], refs: [{ name: l2n, id: b.id }], packOffset: 0 }),
        await ost(S, r.objectFormat),
      ]),
      D = new Set(o.ids.blobs);
    return {
      kind: "built",
      seed: {
        content: C,
        ...
        bytes: S.filter((y) => D.has(y.id)).reduce((y, E) => y + E.size, 0),
...
function R(t) {
  let i = t.largest.slice(0, B).map((d) => `${O(d.path)} (${Ft(d.bytes)})`);
  return `its files come to ${Ft(t.totalBytes)}, which with packaging does not fit the ${Ft(t.capBytes)} a cloud session can start with from a folder${i.length > 0 ? `; the largest: ${i.join(", ")}` : ""}. Remove or ignore what the session does not need (a .gitignore in this folder is honoured) and start again`;
}
async function v(t, i) {
  let d = await Promise.all(i.map((m) => t.getDeflated(m))),
...
function O(t) {
  return yo(t, { maxCodeUnits: 512 });
}
var L = { unreadable: "its files could not all be read", busy: "its files kept changing while they were read" },
  j = "packaging its files failed unexpectedly";
async function re({ folder: t, signal: i, onProgress: d, maxBytes: p = dWe, now: m = Date.now, storageV5: F }) {
  let g = (a, _, r) => {
    if (i.aborted) return { kind: "aborted" };
    return s(Jhe.seed, r), d?.({ kind: "bundle_failed", fallback: null }), { kind: "refused", line: M(a), reason: _ };
  };
  try {
    let a = await Rk(t);
    ...
    if (u.kind === "refused") {
      n(`[folderSeed] refused (${u.reason}): ${u.detail}`);
      let o = u.reason;
      return g(
        o !== "internal" && u.detail !== "" ? u.detail : (L[o] ?? j),
        o === "too_large" ? "folder_too_large" : "folder_seed_failed",
        { outcome: w("refused"), reason: c(o) },
      );
    }
    let { seed: f } = u;
    if (f.content.length > r)
      return g(
        ...
        "folder_too_large",
        { outcome: w("refused"), reason: w("too_large") },
      );
    ...
    return (
      s(Jhe.seed, {
        ...
        withheld: [...f.withheld.values()].reduce((o, S) => o + S, 0),
        build_ms: f.buildMs,
      }),
      { kind: "built", seed: b }
    );
  } catch (a) {
    if (It(a) || i.aborted) return { kind: "aborted" };
    return h(a), g(j, "folder_seed_failed", { outcome: w("threw") });
  }
}
export { T as buildFolderSeedBundle, re as shipFolderSeed };
