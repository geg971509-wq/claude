...
// Version: 2.1.252
import { Se, RR } from "/$bunfs/root/chunk-f9h0bg01.js";
...
function rt(n) {
  let s = ot(n) === "" ? `${n}.txt` : n,
    i = RR().workspace === "remote" ? Se() : void 0;
  return gt(s, i);
}
...
function P(Xt) {
  let x = _(32),
    { content: S, defaultFilename: nt, onDone: g } = Xt,
    [, it] = u(null),
    [E, Yt] = u(nt),
    [U, qt] = u(nt.length),
    [m, st] = u(false),
  ...
  let xt;
  if (x[12] !== G) (xt = [dt, { label: "Save to file", value: "file", description: G }]), (x[12] = G), (x[13] = xt);
  else xt = x[13];
  let I = xt,
    ht;
  if (x[14] !== m)
  ...
  if (
    ...
    (D = !m
      ? e(Ce, { options: I, onChange: j, onCancel: C })
      : r(o, {
...
function St(n) {
  ...
  if (((a = wr(a)), a.length > 50)) a = a.substring(0, 49) + "\u2026";
  return a;
}
function Et(n) {
  return n
...
async function pe(n, s, i) {
  let a = await OUe(s.session, s.messages, s.options.tools || [], { storageV5: s.storageV5 }),
    l = i.trim();
  if (l) {
    try {
      let c = await b(l, a);
      ...
      p("export_file", "write_failed"),
        n(`Failed to export conversation: ${c instanceof Error ? c.message : "Unknown error"}`);
    }
    return null;
  }
  let h = St(s.messages),
...
export { pe as call, St as extractFirstPrompt, Et as sanitizeFilename };
