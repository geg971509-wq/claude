// @bun @bytecode
// Claude Code is a Beta product per Anthropic's Commercial Terms of Service.
// By using Claude Code, you agree that all code acceptance or rejection decisions you make,
// and the associated conversations in context, constitute Feedback under Anthropic's Commercial Terms,
// and may be used to improve Anthropic's products, including training models.
// You are responsible for reviewing any code suggestions before use.

// (c) Anthropic PBC. All rights reserved. Use is subject to the Legal Agreements outlined here: https://code.claude.com/docs/en/legal-and-compliance.

// Version: 2.1.252
import { Ee } from "/$bunfs/root/chunk-rp246vnd.js";
import { ee } from "/$bunfs/root/chunk-4fwj3vnx.js";
import { _ } from "/$bunfs/root/chunk-haan6f8z.js";
import { o, t } from "/$bunfs/root/chunk-he2phymk.js";
import { Gc, Jm } from "/$bunfs/root/chunk-yzvg79ww.js";
import { o_ } from "/$bunfs/root/chunk-vrtqnmsg.js";
import { Pe } from "/$bunfs/root/chunk-q903htrr.js";
import { SQ } from "/$bunfs/root/chunk-g3nsxpwp.js";
import { U, e, r } from "/$bunfs/root/chunk-8s7kcbyw.js";
import { Q } from "/$bunfs/root/chunk-wag5ye9w.js";
import { d } from "/$bunfs/root/chunk-yz031c9r.js";
import { relative as tt } from "path";
var N = 10;
function VU(Ht) {
  let i = _(38),
    { file_path: f, operation: z, patch: A, firstLine: X, fileContent: E, content: M, style: Dt, verbose: m } = Ht,
    { columns: st } = Ee(),
    F;
  if (i[0] !== z) (F = r(t, { color: "subtle", children: ["User rejected ", z, " to "] })), (i[0] = z), (i[1] = F);
  else F = i[1];
  let G;
  if (i[2] !== f || i[3] !== m) (G = m ? f : tt(ee(), f)), (i[2] = f), (i[3] = m), (i[4] = G);
  else G = i[4];
  let J;
  if (i[5] !== G) (J = e(t, { bold: true, color: "subtle", children: G })), (i[5] = G), (i[6] = J);
  else J = i[6];
  let lt;
  if (i[7] !== F || i[8] !== J)
    (lt = r(o, { flexDirection: "row", children: [F, J] })), (i[7] = F), (i[8] = J), (i[9] = lt);
  else lt = i[9];
  let s = lt;
  if (Dt === "condensed" && !m) {
    let l;
    if (i[10] !== s) (l = e(Pe, { children: s })), (i[10] = s), (i[11] = l);
    else l = i[11];
    return l;
  }
  if (z === "write" && M !== void 0) {
    let K, l;
    if (i[12] !== M || i[13] !== m) {
      let ft = M.split(`
`);
      K = ft.length - N;
      l = m
        ? M
        : ft.slice(0, N).join(`
`);
      (i[12] = M), (i[13] = m), (i[14] = K), (i[15] = l);
    } else (K = i[14]), (l = i[15]);
    let St = l;
    const g = St || "(No content)";
    const b = st - 12;
    let V;
    if (i[16] !== f || i[17] !== g || i[18] !== b)
      (V = e(o_, { code: g, filePath: f, width: b, dim: true })), (i[16] = f), (i[17] = g), (i[18] = b), (i[19] = V);
    else V = i[19];
    let Y;
    if (i[20] !== K || i[21] !== m) (Y = !m && e(Jm, { count: K })), (i[20] = K), (i[21] = m), (i[22] = Y);
    else Y = i[22];
    let mt;
    if (i[23] !== V || i[24] !== Y || i[25] !== s)
      (mt = e(Pe, { children: r(o, { flexDirection: "column", children: [s, V, Y] }) })),
        (i[23] = V),
        (i[24] = Y),
        (i[25] = s),
        (i[26] = mt);
    else mt = i[26];
    return mt;
  }
  if (!A || A.length === 0) {
    let l;
    if (i[27] !== s) (l = e(Pe, { children: s })), (i[27] = s), (i[28] = l);
    else l = i[28];
    return l;
  }
  const l = st - 12;
  let g;
  if (i[29] !== E || i[30] !== f || i[31] !== X || i[32] !== A || i[33] !== l)
    (g = e(SQ, { hunks: A, dim: true, width: l, filePath: f, firstLine: X, fileContent: E })),
      (i[29] = E),
      (i[30] = f),
      (i[31] = X),
      (i[32] = A),
      (i[33] = l),
      (i[34] = g);
  else g = i[34];
  let b;
  if (i[35] !== g || i[36] !== s)
    (b = e(Pe, { children: r(o, { flexDirection: "column", children: [s, g] }) })),
      (i[35] = g),
      (i[36] = s),
      (i[37] = b);
  else b = i[37];
  return b;
}
function ht(Jt) {
  return Jt.startsWith("+");
}
function pt(Kt, Qt) {
  return Kt + Q(Qt.lines, ht);
}
function gt(Vt) {
  return Vt.startsWith("-");
}
function at(Yt, Zt) {
  return Yt + Q(Zt.lines, gt);
}
function GUe(zt) {
  let a = _(25),
    {
      filePath: et,
      structuredPatch: T,
      firstLine: ot,
      fileContent: nt,
      style: ct,
      verbose: rt,
      previewHint: Z,
      collapsed: Ft,
    } = zt,
    { columns: Gt } = Ee(),
    c = T.reduce(pt, 0),
    y = T.reduce(at, 0),
    v;
  if (a[0] !== c)
    (v = c > 0 ? r(U, { children: ["Added ", e(t, { bold: true, children: c }), " ", c > 1 ? "lines" : "line"] }) : null),
      (a[0] = c),
      (a[1] = v);
  else v = a[1];
  const it = c > 0 && y > 0 ? ", " : null;
  let I;
  if (a[2] !== c || a[3] !== y)
    (I =
      y > 0
        ? r(U, {
            children: [c === 0 ? "R" : "r", "emoved ", e(t, { bold: true, children: y }), " ", y > 1 ? "lines" : "line"],
          })
        : null),
      (a[2] = c),
      (a[3] = y),
      (a[4] = I);
  else I = a[4];
  let dt;
  if (a[5] !== v || a[6] !== it || a[7] !== I)
    (dt = r(t, { children: [v, it, I] })), (a[5] = v), (a[6] = it), (a[7] = I), (a[8] = dt);
  else dt = a[8];
  let k = dt;
  if (Z) {
    if (ct !== "condensed" && !rt) {
      let u;
      if (a[9] !== Z) (u = e(Pe, { children: e(t, { dimColor: true, children: Z }) })), (a[9] = Z), (a[10] = u);
      else u = a[10];
      return u;
    }
  } else if (ct === "condensed" && !rt) {
    return k;
  } else if (Ft && !rt && c + y > 0) {
    let u;
    if (a[11] === d) (u = e(Gc, {})), (a[11] = u);
    else u = a[11];
    let H;
    if (a[12] !== k) (H = e(Pe, { children: r(t, { children: [k, " ", u] }) })), (a[12] = k), (a[13] = H);
    else H = a[13];
    return H;
  }
  let u;
  if (a[14] !== k) (u = e(t, { children: k })), (a[14] = k), (a[15] = u);
  else u = a[15];
  const H = Gt - 12;
  let O;
  if (a[16] !== nt || a[17] !== et || a[18] !== ot || a[19] !== T || a[20] !== H)
    (O = e(SQ, { hunks: T, dim: false, width: H, filePath: et, firstLine: ot, fileContent: nt })),
      (a[16] = nt),
      (a[17] = et),
      (a[18] = ot),
      (a[19] = T),
      (a[20] = H),
      (a[21] = O);
  else O = a[21];
  let ut;
  if (a[22] !== u || a[23] !== O)
    (ut = e(Pe, { children: r(o, { flexDirection: "column", children: [u, O] }) })),
      (a[22] = u),
      (a[23] = O),
      (a[24] = ut);
  else ut = a[24];
  return ut;
}
export { VU, GUe };
