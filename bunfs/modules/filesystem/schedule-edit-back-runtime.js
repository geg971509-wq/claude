...
import { K$ } from "/$bunfs/root/chunk-1yr12dqr.js";
...
import { ry, Ane } from "/$bunfs/root/chunk-avrc9gay.js";
...
function Jo(Lo) {
  return Lo.trim() === "" ? null : (Ane(Lo).error ?? null);
}
function Oo(Fo) {
  if (Fo.trim() === "") {
    return;
...
function Xo(Qo) {
  return { label: K$(Qo), value: Qo };
}
async function eOn(s) {
  return Ike(void 0, s).catch((v) => (h(v), []));
}
function k8t(st) {
  let m = _(76),
    ...
    (fo = async function Y() {
      ...
      try {
        await Pge(so({ ...i, enabled: !i.enabled }), void 0, J),
          await q(),
          B(`${i.enabled ? "Disabled" : "Enabled"} scheduled task '${i.id}'.`, { display: "system" });
      } catch (re) {
        let bo = re;
  ...
  if (m[6] !== P || m[7] !== B || m[8] !== q || m[9] !== J || m[10] !== i.id)
    (re = async function G() {
      ...
      try {
        ...
        let yo = R;
        n(`Failed to remove scheduled task '${i.id}' from daemon.json: ${l(yo)}`, { level: "error" }),
  ...
  if (at) {
    ...
    let K;
    if (m[18] !== R || m[19] !== j)
      (K = e(me, { title: "Remove task?", subtitle: R, onCancel: V, color: "error", children: j })),
        (m[18] = R),
        (m[19] = j),
        (m[20] = K);
    else K = m[20];
  ...
  let V;
  if (m[21] !== R) (V = { label: R, value: "toggle" }), (m[21] = R), (m[22] = V);
  else V = m[22];
  let A, H, j;
  if (m[23] === d)
  ...
  let de;
  if (m[28] !== i.cron) (de = ry(i.cron)), (m[28] = i.cron), (m[29] = de);
  else de = m[29];
  let se;
  ...
  if (
    ...
    (he = r(o, { flexDirection: "column", marginBottom: 1, children: [se, ae, le, ue, fe, be, ye, ge] })),
      (m[52] = se),
      (m[53] = ae),
      (m[54] = le),
      (m[55] = ue),
  ...
  let xe;
  if (m[66] !== P || m[67] !== k || m[68] !== qe || m[69] !== ve)
    (xe = e(Ce, { options: qe, isDisabled: P, onChange: ve, onCancel: k })),
      (m[66] = P),
      (m[67] = k),
      (m[68] = qe),
      (m[69] = ve),
  ...
  if (m[71] !== k || m[72] !== he || m[73] !== xe || m[74] !== i.id)
    (go = r(me, { title: mt, onCancel: k, children: [he, xe] })),
      (m[71] = k),
      (m[72] = he),
      (m[73] = xe),
      (m[74] = i.id),
...
function H8t(ct) {
  let f = _(59),
    ...
    ho;
  if (f[0] !== Me) (ho = Me === void 0 ? [{ label: "default", value: "" }] : Me), (f[0] = Me), (f[1] = ho);
  else ho = f[1];
  let W = ho,
    T = c !== void 0,
    vo;
  if (f[2] !== Te || f[3] !== T || f[4] !== c)
    (vo = T ? Te.filter((ut) => ut !== c.id) : Te), (f[2] = Te), (f[3] = T), (f[4] = c), (f[5] = vo);
  else vo = f[5];
  let Ue = vo;
  const Ye = c?.prompt ?? "",
    Ge = c?.cron ?? "",
    ...
    (xo = function Z(X, To) {
      ...
      pt((eo) => {
        ...
        if (X !== "id" && !O && (X === "prompt" || X === "dir"))
          ke.id = ie(te(km(ke.dir?.trim() || x)), ke.prompt ?? "");
        return ke;
      });
    }),
      (f[13] = x),
  ...
  let Z = xo,
    Mo;
  if (f[16] !== W || f[17] !== c)
    (Mo = c?.model && !W.some((bt) => bt.value === c.model) ? [...W, { label: c.model, value: c.model }] : W),
      (f[16] = W),
      (f[17] = c),
      (f[18] = Mo);
  else Mo = f[18];
  ...
  let Pe;
  if (f[21] !== x) (Pe = { type: "text", key: "dir", label: "Directory", placeholder: x }), (f[21] = x), (f[22] = Pe);
  else Pe = f[22];
  let Re;
  if (f[23] !== Ue)
    (Re = (yt) => {
  ...
  let we;
  if (f[25] !== O) (we = () => (O ? void 0 : "Auto-generated from prompt and directory.")), (f[25] = O), (f[26] = we);
  else we = f[26];
  let Be;
  if (f[27] !== Re || f[28] !== we)
    (Be = { type: "text", key: "id", label: "Id", validate: Re, hint: we }), (f[27] = Re), (f[28] = we), (f[29] = Be);
  else Be = f[29];
  let Po;
  if (f[30] === d)
    (Po = { type: "select", key: "permissionMode", label: "Permission mode", options: Ige.map(Xo) }), (f[30] = Po);
  else Po = f[30];
  let De;
  if (f[31] !== I)
    (De = (Ro) =>
      I.find((gt) => gt.value === Ro)?.description ?? (Ro === "" ? "Uses your configured default model." : void 0)),
      (f[31] = I),
      (f[32] = De);
  else De = f[32];
  let Ve;
  if (f[33] !== I || f[34] !== De)
    (Ve = { type: "select", key: "model", label: "Model", options: I, hint: De }),
      (f[33] = I),
      (f[34] = De),
      (f[35] = Ve);
  else Ve = f[35];
  ...
  if (f[36] !== Pe || f[37] !== Be || f[38] !== Ve)
    (wo = [$o, ko, Pe, Be, Po, Ve]), (f[36] = Pe), (f[37] = Be), (f[38] = Ve), (f[39] = wo);
  else wo = f[39];
  let to = wo,
    Bo;
  if (
    ...
    (Bo = async function oe() {
      ...
      try {
        if (T && c.id !== io) await Dge(c.id, void 0, $e);
        await Pge(vt, void 0, $e), await ze(io, T);
      } catch (ee) {
        let jo = ee;
  ...
  let Eo;
  if (f[51] !== to || f[52] !== Xe || f[53] !== Z || f[54] !== ee || f[55] !== Ae || f[56] !== no || f[57] !== g)
    (Eo = e(lL, {
      title: ee,
...
function ie(s, v) {
  let p = BRt(No(s)),
    C = BRt(v.split(/\s+/).slice(0, 4).join(" "));
  return [p, C].filter(Boolean).join("-") || "task";
}
function so(s) {
  return {
    id: s.id,
