...
import { nf } from "/$bunfs/root/chunk-ky33xsw7.js";
function mjn(e) {
  return { request: of(`?${e}$p`), match: (r) => r.type === "decrpm" && r.mode === e };
}
function gjn() {
  return { request: of("?6n"), match: (e) => e.type === "cursorPosition" };
}
function hjn(e) {
  return { request: nf(e, "?"), match: (r) => r.type === "osc" && r.code === e };
}
function _jn() {
  return { request: of(">0q"), match: (e) => e.type === "xtversion" };
}
var n = of("c");
class Zon {
  stdout;
  ...
  send(e) {
    return new Promise((r) => {
      this.queue.push({ kind: "query", match: e.match, resolve: (s) => r(s) }), this.stdout.write(e.request);
    });
  }
  flush() {
