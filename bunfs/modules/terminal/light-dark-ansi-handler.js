// @bun @bytecode
// Claude Code is a Beta product per Anthropic's Commercial Terms of Service.
// By using Claude Code, you agree that all code acceptance or rejection decisions you make,
// and the associated conversations in context, constitute Feedback under Anthropic's Commercial Terms,
// and may be used to improve Anthropic's products, including training models.
// You are responsible for reviewing any code suggestions before use.

// (c) Anthropic PBC. All rights reserved. Use is subject to the Legal Agreements outlined here: https://code.claude.com/docs/en/legal-and-compliance.

// Version: 2.1.252
import { o, t } from "/$bunfs/root/chunk-he2phymk.js";
import { a } from "/$bunfs/root/chunk-fec4384a.js";
import { _ } from "/$bunfs/root/chunk-haan6f8z.js";
import { E8 } from "/$bunfs/root/chunk-f9km17e9.js";
import { Be } from "/$bunfs/root/chunk-ek4tmwbt.js";
import { Ro } from "/$bunfs/root/chunk-59pkbjrf.js";
import { rn } from "/$bunfs/root/chunk-988p40e0.js";
import { mn, uG, mlt, dG } from "/$bunfs/root/chunk-htrft0p0.js";
import { ge } from "/$bunfs/root/chunk-c3bffkxg.js";
import { IW, Mb } from "/$bunfs/root/chunk-fwwvmbqn.js";
import { Hi } from "/$bunfs/root/chunk-tmxtk606.js";
import { Ee } from "/$bunfs/root/chunk-rp246vnd.js";
import { W, At } from "/$bunfs/root/chunk-ht28m404.js";
import { Ln } from "/$bunfs/root/chunk-zze8764r.js";
import { Ce } from "/$bunfs/root/chunk-1y0jc48h.js";
import { M } from "/$bunfs/root/chunk-y2r26k83.js";
import { fe } from "/$bunfs/root/chunk-rycvm63e.js";
import { CBe, K7t } from "/$bunfs/root/chunk-m6r337am.js";
import { gx } from "/$bunfs/root/chunk-qsfnyrge.js";
import { Iy } from "/$bunfs/root/chunk-7sf2fttr.js";
import { U, e, r } from "/$bunfs/root/chunk-8s7kcbyw.js";
import { u, F } from "/$bunfs/root/chunk-twm95mhz.js";
import { d } from "/$bunfs/root/chunk-yz031c9r.js";
F();
function xt(Et) {
  return Et.settings.syntaxHighlightingDisabled;
}
function vt() {}
function St(k) {
  return {
    label: k.source === "user" ? `${k.name} (custom)` : `${k.name} (from ${k.source.plugin})`,
    value: IW(k.slug),
  };
}
var g = "__new_custom_theme__";
function EQ(Bt) {
  let i = _(89),
    {
      onThemeSelect: ue,
      showIntroText: Ye,
      helpText: je,
      showHelpTextBelow: qe,
      hideEscToCancel: Je,
      skipExitHandling: Me,
      onCancel: pe,
      onCustomTheme: l,
    } = Bt,
    P = Ye === void 0 ? false : Ye,
    m = je === void 0 ? "" : je,
    p = qe === void 0 ? false : qe,
    be = Je === void 0 ? false : Je,
    H = Me === void 0 ? false : Me,
    { storageV5: xe } = ge(),
    [ve] = mn(),
    C = uG(),
    { columns: Yt } = Ee(),
    Oe;
  if (i[0] === d) (Oe = CBe()), (i[0] = Oe);
  else Oe = i[0];
  let Se = Oe,
    Qe;
  if (i[1] !== ve) (Qe = Se === null ? K7t(ve) : null), (i[1] = ve), (i[2] = Qe);
  else Qe = i[2];
  let A = Qe,
    { setPreviewTheme: ye, savePreview: b, cancelPreview: s } = mlt(),
    R = W(xt) ?? false,
    ke = At();
  E8("ThemePicker");
  let Te = Ro("theme:toggleSyntaxHighlighting", "ThemePicker", "ctrl+t"),
    We;
  if (i[3] !== ke || i[4] !== xe || i[5] !== R)
    (We = () => {
      if (Se === null) {
        let Ze = !R;
        rn("userSettings", { syntaxHighlightingDisabled: Ze }, void 0, xe),
          ke((Ke) => ({ ...Ke, settings: { ...Ke.settings, syntaxHighlightingDisabled: Ze } }));
      }
    }),
      (i[3] = ke),
      (i[4] = xe),
      (i[5] = R),
      (i[6] = We);
  else We = i[6];
  let et;
  if (i[7] === d) (et = { context: "ThemePicker" }), (i[7] = et);
  else et = i[7];
  Be("theme:toggleSyntaxHighlighting", We, et);
  let G = Hi(H ? vt : void 0),
    { customThemes: x } = dG(),
    [De, jt] = u(C),
    tt;
  if (i[8] !== De) (tt = Mb(De)), (i[8] = De), (i[9] = tt);
  else tt = i[9];
  let I = tt,
    ot;
  if (i[10] !== x || i[11] !== I)
    (ot = I ? x.find((qt) => qt.slug === I) : void 0), (i[10] = x), (i[11] = I), (i[12] = ot);
  else ot = i[12];
  let c = ot,
    we = Ro("theme:editCustom", "ThemePicker", "ctrl+e"),
    it;
  if (i[13] !== c || i[14] !== l || i[15] !== b)
    (it = () => {
      if (c && l) b(), l(c);
    }),
      (i[13] = c),
      (i[14] = l),
      (i[15] = b),
      (i[16] = it);
  else it = i[16];
  let nt;
  if (i[17] === d) (nt = { context: "ThemePicker" }), (i[17] = nt);
  else nt = i[17];
  Be("theme:editCustom", it, nt);
  let lt, st, rt, at, mt, ct, dt;
  if (i[18] === d)
    (lt = { label: "Auto (match terminal)", value: "auto" }),
      (st = { label: "Dark mode", value: "dark" }),
      (rt = { label: "Light mode", value: "light" }),
      (at = { label: "Dark mode (colorblind-friendly)", value: "dark-daltonized" }),
      (mt = { label: "Light mode (colorblind-friendly)", value: "light-daltonized" }),
      (ct = { label: "Dark mode (ANSI colors only)", value: "dark-ansi" }),
      (dt = { label: "Light mode (ANSI colors only)", value: "light-ansi" }),
      (i[18] = lt),
      (i[19] = st),
      (i[20] = rt),
      (i[21] = at),
      (i[22] = mt),
      (i[23] = ct),
      (i[24] = dt);
  else (lt = i[18]), (st = i[19]), (rt = i[20]), (at = i[21]), (mt = i[22]), (ct = i[23]), (dt = i[24]);
  let ht;
  if (i[25] !== x || i[26] !== l) {
    let h;
    if (i[28] !== l) (h = l ? [{ label: "New custom theme\u2026", value: g }] : []), (i[28] = l), (i[29] = h);
    else h = i[29];
    ht = [lt, st, rt, at, mt, ct, dt, ...x.map(St), ...h];
    (i[25] = x), (i[26] = l), (i[27] = ht);
  } else ht = i[27];
  let V = ht,
    h;
  if (i[30] !== P)
    (h = P ? e(t, { children: "Let's get started." }) : e(t, { bold: true, color: "permission", children: "Theme" })),
      (i[30] = P),
      (i[31] = h);
  else h = i[31];
  let gt;
  if (i[32] === d)
    (gt = e(t, { bold: true, children: "Choose the text style that looks best with your terminal" })), (i[32] = gt);
  else gt = i[32];
  let N;
  if (i[33] !== m || i[34] !== p)
    (N = m && !p && e(t, { dimColor: true, children: m })), (i[33] = m), (i[34] = p), (i[35] = N);
  else N = i[35];
  let X;
  if (i[36] !== N) (X = r(o, { flexDirection: "column", children: [gt, N] })), (i[36] = N), (i[37] = X);
  else X = i[37];
  let z;
  if (i[38] !== s || i[39] !== ye)
    (z = (Le) => {
      if ((jt(Le), Le === g)) s();
      else ye(Le);
    }),
      (i[38] = s),
      (i[39] = ye),
      (i[40] = z);
  else z = i[40];
  let B;
  if (i[41] !== s || i[42] !== l || i[43] !== ue || i[44] !== b)
    (B = (ft) => {
      if (ft === g) {
        s(), l?.(void 0);
        return;
      }
      b(), ue(ft);
    }),
      (i[41] = s),
      (i[42] = l),
      (i[43] = ue),
      (i[44] = b),
      (i[45] = B);
  else B = i[45];
  let Y;
  if (i[46] !== s || i[47] !== pe || i[48] !== H)
    (Y = H
      ? () => {
          s(), pe?.();
        }
      : async () => {
          s(), await Ln(0);
        }),
      (i[46] = s),
      (i[47] = pe),
      (i[48] = H),
      (i[49] = Y);
  else Y = i[49];
  const Pe = Math.min(V.length, 12);
  let j;
  if (i[50] !== z || i[51] !== B || i[52] !== Y || i[53] !== Pe || i[54] !== V || i[55] !== C)
    (j = e(Ce, {
      options: V,
      onFocus: z,
      onChange: B,
      onCancel: Y,
      visibleOptionCount: Pe,
      defaultValue: C,
      defaultFocusValue: C,
    })),
      (i[50] = z),
      (i[51] = B),
      (i[52] = Y),
      (i[53] = Pe),
      (i[54] = V),
      (i[55] = C),
      (i[56] = j);
  else j = i[56];
  let q;
  if (i[57] !== h || i[58] !== X || i[59] !== j)
    (q = r(o, { flexDirection: "column", gap: 1, children: [h, X, j] })),
      (i[57] = h),
      (i[58] = X),
      (i[59] = j),
      (i[60] = q);
  else q = i[60];
  let ut;
  if (i[61] === d)
    (ut = {
      oldStart: 1,
      newStart: 1,
      oldLines: 3,
      newLines: 3,
      lines: [" function greet() {", '-  console.log("Hello, World!");', '+  console.log("Hello, Claude!");', " }"],
    }),
      (i[61] = ut);
  else ut = i[61];
  const He = Yt - 6;
  let E;
  if (i[62] !== He)
    (E = e(Iy, {
      paddingX: 0,
      children: e(gx, { patch: ut, dim: false, filePath: "demo.js", firstLine: null, width: He }),
    })),
      (i[62] = He),
      (i[63] = E);
  else E = i[63];
  const $e =
    Se === "env"
      ? `Syntax highlighting disabled (via CLAUDE_CODE_SYNTAX_HIGHLIGHT=${a.CLAUDE_CODE_SYNTAX_HIGHLIGHT})`
      : R
        ? `Syntax highlighting disabled (${Te} to enable)`
        : A
          ? `Syntax theme: ${A.theme}${A.source ? ` (from ${A.source})` : ""} (${Te} to disable)`
          : `Syntax highlighting enabled (${Te} to disable)`;
  let J;
  if (i[64] !== $e) (J = r(t, { dimColor: true, children: [" ", $e] })), (i[64] = $e), (i[65] = J);
  else J = i[65];
  let O;
  if (i[66] !== E || i[67] !== J)
    (O = r(o, { flexDirection: "column", width: "100%", children: [E, J] })), (i[66] = E), (i[67] = J), (i[68] = O);
  else O = i[68];
  let pt;
  if (i[69] !== q || i[70] !== O)
    (pt = r(o, { flexDirection: "column", gap: 1, children: [q, O] })), (i[69] = q), (i[70] = O), (i[71] = pt);
  else pt = i[71];
  let Q = pt;
  if (!P) {
    let Z;
    if (i[72] !== Q) (Z = e(o, { flexDirection: "column", children: Q })), (i[72] = Q), (i[73] = Z);
    else Z = i[73];
    let K;
    if (i[74] !== m || i[75] !== p)
      (K = p && m && e(o, { marginLeft: 3, children: e(t, { dimColor: true, children: m }) })),
        (i[74] = m),
        (i[75] = p),
        (i[76] = K);
    else K = i[76];
    let ee;
    if (i[77] !== we || i[78] !== G || i[79] !== c || i[80] !== be || i[81] !== l)
      (ee =
        !be &&
        e(o, {
          children: e(t, {
            dimColor: true,
            italic: true,
            children: G.pending
              ? r(U, { children: ["Press ", G.keyName, " again to exit"] })
              : r(fe, {
                  children: [
                    e(M, { chord: "enter", action: "select" }),
                    c && l && e(M, { chord: we, action: "edit" }),
                    e(M, { chord: "escape", action: "cancel" }),
                  ],
                }),
          }),
        })),
        (i[77] = we),
        (i[78] = G),
        (i[79] = c),
        (i[80] = be),
        (i[81] = l),
        (i[82] = ee);
    else ee = i[82];
    let te;
    if (i[83] !== K || i[84] !== ee)
      (te = r(o, { marginTop: 1, children: [K, ee] })), (i[83] = K), (i[84] = ee), (i[85] = te);
    else te = i[85];
    let bt;
    if (i[86] !== Z || i[87] !== te) (bt = r(U, { children: [Z, te] })), (i[86] = Z), (i[87] = te), (i[88] = bt);
    else bt = i[88];
    return bt;
  }
  return Q;
}
export { EQ };
