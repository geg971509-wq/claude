...
import { Pn } from "/$bunfs/root/chunk-ca80fke8.js";
...
async function U({ proactivity: r } = {}) {
  if ((await new Promise((e) => setImmediate(e)), !(await ZE())))
    return (
      await Pn("agent_launcher", "relaunch_launcher_not_runnable"),
      process.stderr.write(`
${ru() ?? `${VS}: launcher \`${Xl()[0]}\` was deleted or is not executable \u2014 restore it (or fix the setting), then start claude again`}
`),
      Vi(1)
    );
  let { cmd: n, prefixArgs: a } = rp(),
    c = process.argv.slice(2),
    t = { ...process.env };
  delete t[$ke], Object.assign(t, Znt());
