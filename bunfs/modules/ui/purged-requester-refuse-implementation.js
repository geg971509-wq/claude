...
import { K, et } from "/$bunfs/root/chunk-f9h0bg01.js";
import { n } from "/$bunfs/root/chunk-fv016jr6.js";
import { ce } from "/$bunfs/root/chunk-4ddxwr9r.js";
import { Fft } from "/$bunfs/root/chunk-wn1jrcrk.js";
import { y, p, g } from "/$bunfs/root/chunk-ca80fke8.js";
import { tS } from "/$bunfs/root/chunk-8tgj5dp2.js";
...
import { GNt, gE } from "/$bunfs/root/chunk-zze8764r.js";
import { Fu, II, $z, HM, Lht } from "/$bunfs/root/chunk-qnrh4abv.js";
import { nZ, fen, H0e } from "/$bunfs/root/chunk-ac0gs4d8.js";
...
var fIt = 43200000,
  ...
  F = 750,
  O = 100,
  KZt = m(() => f({ action: N("notify_when_idle"), from: i(), msg_id: i(), from_mode: _e().optional() })),
  XZt = m(() =>
    f({
      action: N("peer_idle_notice"),
      orig_msg_id: i(),
      state: i(),
      finished_at: _e().optional(),
      detail: _e().optional(),
      from: _e().optional(),
      from_mode: _e().optional(),
    }),
  );
class D {
  subscribers = [];
...
var Pu = new D(),
  ...
  q = 4,
  H = 4;
function YZt(e, t, r, o, s, l, b, _) {
  if (!hTe(r)) return g("cross_session_notify_idle", "invalid_frame"), "invalid";
  let c = tS(t);
  if (c === void 0) return g("cross_session_notify_idle", "invalid_frame"), "invalid";
  if (nZ() === "refuse") return g("cross_session_notify_idle", "refused_by_policy"), "refused";
  let a = Pu;
  if (a.exited) return g("cross_session_notify_idle", "subscription_after_exit"), "full";
  T(Date.now());
  let u = a.subscribers,
    d = (I) => o !== void 0 && I.targetKey === c && I.verifiedPeerPid === o,
    v = u.findIndex(d),
    k = v !== -1 || a.inflightBatch.some(d),
    x = () => u.length >= w0e;
  if (v !== -1) u.splice(v, 1);
  else if (o !== void 0) {
    if (Q(u, (I) => I.verifiedPeerPid === o) >= q) {
      let I = u.findIndex((S) => S.verifiedPeerPid === o);
      if (I === -1) return P();
      E(u, I);
    } else if (x()) return P();
  } else if (!l) {
    let I = (S) => S.verifiedPeerPid === void 0 && !S.authenticated;
    if (Q(u, I) >= U) {
      let S = u.findIndex(I);
      if (S === -1) return P();
      E(u, S);
    } else if (x()) return P();
  } else if (Q(u, (S) => S.verifiedPeerPid === void 0 && S.authenticated && S.targetKey === c) >= H || x()) return P();
  if (
    (u.push({
      targetKey: c,
      replyTarget: t,
    ...
    n(
      `[peer-idle] subscription ${v === -1 ? "recorded" : "refreshed"} for ${Fu(e)} (${o === void 0 ? "no peer pid" : `pid ${o}`}${l ? ", authenticated" : ""}; ${u.length} live)`,
    ),
    !k)
  )
    M(e, o !== void 0);
...
function w(e = q$n) {
  if (Pu.debounce) return;
  (Pu.debounce = setTimeout(W, e)), Pu.debounce.unref?.();
}
function W() {
  if (((Pu.debounce = null), !Pu.idle)) return;
  if ((T(Date.now()), Pu.subscribers.length === 0)) return;
  if (GNt() > 0) {
    w();
    return;
  }
...
async function gIt(e) {
  ...
  let o = t.sendNotice,
    s = t.subscribers.splice(0, t.subscribers.length);
  t.inflightBatch = t.inflightBatch.concat(s);
  try {
    await V(t, o, s, e, r);
  } catch (l) {
    h(l), p("cross_session_notify_idle", "flush_internal_error");
  } finally {
    t.inflightBatch = t.inflightBatch.filter((l) => !s.includes(l));
  }
}
async function V(e, t, r, o, s) {
  if (nZ() === "refuse") {
    g("cross_session_notify_idle", "revoked_before_fire");
    return;
  }
  let l = e.turnEnded && e.turnConversationId === K(),
    b = s === "idle" ? (l ? e.idleSince : void 0) : Date.now(),
    _ = (d) => fen({ fromMode: d.fromMode, selfSent: d.selfSent }) === "accept",
    c = s === "idle" && l && r.some((d) => d.verifiedPeerPid !== void 0 && _(d)) ? Qdr() : void 0,
    a = new Map();
  if (r.some((d) => d.verifiedPeerPid !== void 0))
    try {
      a =
        (await e.registeredInboxOfPid?.(r.flatMap((d) => (d.verifiedPeerPid === void 0 ? [] : [d.verifiedPeerPid])))) ??
        new Map();
    } catch (d) {
      n(`[peer-idle] registry unreadable at fire time; notices carry no detail (${II(String(d))})`);
    }
  if (nZ() === "refuse") {
    g("cross_session_notify_idle", "revoked_before_fire");
    return;
  }
  if (o === "idle" && (!e.idle || GNt() > 0 || H0e() > 0)) {
    if ((C(r), e.idle)) {
      let d = H0e() > 0;
      (e.parkedHoldBack = d), w(d ? G$n : q$n);
    }
    return;
  }
  let u = (d) => {
    ...
    r.map((d) => {
      ...
      return t(
        ...
        (k) => {
          ...
          if (x === "requester-gone") g("cross_session_notify_idle", "requester_gone");
          else if (x === "transient")
            g("cross_session_notify_idle", d.retried ? "notice_retry_exhausted" : "notice_transient_at_exit");
          else p("cross_session_notify_idle", "notice_send_failed");
          n(`[peer-idle] notice to ${Fu(d.replyAddress)} failed: ${II(String(k))}`);
        },
      );
    }),
  );
...
function E(e, t) {
  ...
  g("cross_session_notify_idle", "evicted_same_class"),
    Dot(r.replyTarget, r.origMsgId, r.verifiedPeerPid, r.authenticated, r.verifiedPeerProcStart);
}
function T(e) {
  let t = Pu.subscribers;
  for (let r = t.length - 1; r >= 0; r--)
    if (e - t[r].requestedAt > fIt) g("cross_session_notify_idle", "expired_unfired"), t.splice(r, 1);
}
function Qdr() {
  let e = "";
  try {
    e = Pu.getLastTurnText?.() ?? "";
  } catch (r) {
    n(`[peer-idle] last-turn text unavailable: ${II(String(r))}`);
  }
...
`)
    ...
    t === void 0 ? void 0 : JZt(t)
  );
}
function Iot(e) {
  return JZt(e) ?? "(unnamed session)";
}
function JZt(e) {
  let t = ce(e, O * 8)
    ...
    .replace(/[\s\p{Z}]+/gu, " ");
  for (;;) {
    let r = t.replace(/cross-session idle notice/giu, " ").replace(/ {2,}/g, " ");
    if (r === t) break;
    t = r;
  }
...
function QZt(e, t, r) {
  ...
  for (let a = o.length - 1; a >= 0; a--) {
    ...
    if (u.conversationId !== l)
      clearTimeout(u.expiry), o.splice(a, 1), g("cross_session_notify_idle", "purged_cleared_conversation");
  }
...
function Zdr(e) {
  let t = Pu.outstanding,
    r = t.findIndex((s) => s.msgId === e);
  if (r === -1) return;
  let [o] = t.splice(r, 1);
  if (!o) return;
  if (t.some((s) => s.targetKey === o.targetKey && s.requestedAt > o.requestedAt)) return;
  for (let s = t.length - 1; s >= 0; s--) {
    let l = t[s];
    if (l.targetKey === o.targetKey) clearTimeout(l.expiry), t.splice(s, 1);
  }
  X(o);
}
function X(e) {
  let t = e.conversationId === K();
  if ((g("cross_session_notify_idle", t ? "expired_unheard" : "purged_cleared_conversation"), t))
    R({ kind: "expired", label: e.label, gate: "local" });
}
function j6e(e) {
  return typeof e === "string" && Pu.outstanding.some((t) => t.msgId === e);
}
function Pot(e) {
  let t = Pu,
    r = t.outstanding.findIndex((s) => s.msgId === e);
  if (r === -1) return;
  let [o] = t.outstanding.splice(r, 1);
...
function ZZt(e, t, r, o, s, l) {
  ...
  if ((clearTimeout(c.expiry), t === "idle" || t === "exited"))
    for (let d = b.outstanding.length - 1; d >= 0; d--) {
      ...
      if (v.targetKey === c.targetKey) clearTimeout(v.expiry), b.outstanding.splice(d, 1);
    }
  ...
  return (
    R({
      kind: u,
      label: c.label,
      ...(u !== "unavailable" && Lht(r) && { finishedAt: r }),
...
function j(e) {
  let { gate: t, detail: r, ...o } = e,
    s = t === "local" ? nZ() : fen(t);
  if (s === "refuse") return null;
  let l = t === "local" || s === "accept";
...
function R(e) {
  let t = Pu,
    r = t.onNotice;
  if (r === null) {
    if (t.pendingNotices.length < w0e) t.pendingNotices.push({ admitted: e, conversationId: K() });
    else t.pendingNoticeOverflow++;
    n(`[peer-idle] notice for "${Fu(e.label)}" (${e.kind}) buffered until a host handler mounts`);
    return;
  }
  let o = j(e);
  if (o === null) {
  ...
  if (o.kind === "idle" || o.kind === "exited") y("cross_session_notify_idle");
  try {
    r(o);
  } catch (s) {
    h(s);
...
function hIt(e) {
  let t = Pu;
  if (((t.onNotice = e), e === null)) return;
  let r = t.pendingNotices.splice(0),
    o = t.pendingNoticeOverflow;
  t.pendingNoticeOverflow = 0;
  let s = K();
  ...
  if (o > 0)
    n(`[peer-idle] ${o} notice(s) arrived past the pre-mount buffer and were lost`),
...
function M(e, t, r = false) {
  ...
  try {
    o.onSubscribed(e, t, r);
  } catch (s) {
    h(s);
  }
}
function bIt(e) {
  Pu.registeredInboxOfPid = e;
}
function een(e, t) {
  return `A process claiming the address ${e}${t ? "" : " (no verifiable pid)"} asked to be told when this session is next idle \u2014 it will get one automated status notice; a one-line status detail is included only if that address is registered to the same process in the session registry (a same-user process can register itself).`;
}
var B =
...
function A(e) {
  if (e === void 0) return "";
  return new Date(e).toLocaleTimeString(void 0, { hour: "2-digit", minute: "2-digit", hourCycle: "h23" });
}
function ten(e) {
  switch (e.kind) {
    case "idle": {
      let t = A(e.finishedAt);
      return `${e.label} is idle${t ? ` \u2014 finished a turn at ${t}` : ""}${e.detail ? ` \xB7 \xAB${e.detail}\xBB` : ""}`;
    }
    case "exited":
    ...
    case "expired":
      return `No idle signal from ${e.label} within ${fIt / 3600000} h \u2014 idle subscription expired.`;
  }
}
function epr(e) {
  ...
  switch (e.kind) {
    ...
    case "expired":
      return `[Cross-session idle notice] No idle signal arrived from "${e.label}" within ${fIt / 3600000} hours; the subscription has expired (it may still be busy, be waiting on its user, refuse inbound requests, run a version without idle notices, or have ended abruptly). Do not keep waiting for it; if you still need to know, ask your user or list the sessions to check its status. ${r}`;
  }
}
function nen() {
  Pu.parkedHoldBack = H0e() > 0;
}
function bEr(e) {
  Pu.hostStatusUnsubscribe?.(), (Pu.hostStatusUnsubscribe = e);
}
function C(e) {
  let t = Pu.subscribers;
  Pu.inflightBatch = Pu.inflightBatch.filter(
...
function Dot(e, t, r, o, s, l = false) {
  ...
  b(e, { orig_msg_id: t, state: "unavailable" }, r, L(r, o), s).then(
    ...
    (_) => {
      let c = z(_);
      if (c === "transient" && !l) {
        let a = setTimeout(G, F, e, t, r, o, s);
        a.unref?.();
        let u = Pu.unavailableRetries.get(t);
        if (u !== void 0) clearTimeout(u);
        Pu.unavailableRetries.set(t, a), g("cross_session_notify_idle", "unavailable_send_retrying");
        return;
      }
      if (c === "requester-gone") g("cross_session_notify_idle", "requester_gone");
      else if (c === "transient") g("cross_session_notify_idle", "unavailable_retry_exhausted");
      else p("cross_session_notify_idle", "unavailable_send_failed");
      n(`[peer-idle] unavailable notice to ${Fu(e)} failed: ${II(String(_))}`);
    },
  );
}
