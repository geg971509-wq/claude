...
import { ff } from "/$bunfs/root/chunk-8tgj5dp2.js";
...
import { VDn, A8t, _Ue, Ket } from "/$bunfs/root/chunk-ym3ayhed.js";
...
function q(L) {
  let [G, U] = L;
  return { label: `Version ${G}`, description: `${U.length} ${U.length === 1 ? "item" : "items"}`, value: G };
}
var R = "__show_all__";
function J(i, n) {
  let s = `Version ${i}:`,
...
function y(i, n, s) {
  n({ type: "append", messages: [Dt(i, "notice")] }), s(void 0, { display: "skip" });
}
var W = async (i, n) => {
  try {
    let a = new Promise((h, O) => setTimeout((l) => l(Error("Timeout")), 500, O));
    await Promise.race([A8t(n.storageV5), a]);
  } catch {}
  let s = await _Ue(n.storageV5),
    g = Ket(s)
      .slice()
      .sort(([a], [h]) => (ff(a, h) ? -1 : 1));
  if (g.length === 0) return y(`See the full changelog at: ${VDn}`, n.applyMessageOp, i), null;
  return e(x, { notes: g, applyMessageOp: n.applyMessageOp, onDone: i });
};
function x(L) {
  let u = _(19),
    { notes: p, applyMessageOp: v, onDone: c } = L;
  const X = `${p.length} versions`;
  let M;
  if (u[0] !== X) (M = { label: "Show all", description: X, value: R }), (u[0] = X), (u[1] = M);
  else M = u[1];
  let N;
  ...
  let w;
  if (u[12] !== m || u[13] !== S || u[14] !== V)
    (w = e(Ce, { options: V, visibleOptionCount: 10, onChange: S, onCancel: m })),
      (u[12] = m),
      (u[13] = S),
      (u[14] = V),
      (u[15] = w);
  ...
  let E;
  if (u[16] !== m || u[17] !== w)
    (E = r(me, { title: "Release notes", onCancel: m, children: [B, w] })), (u[16] = m), (u[17] = w), (u[18] = E);
  else E = u[18];
  return E;
}
export { x as ReleaseNotesPicker, W as call, k as formatAll, J as formatVersion };
