...
// Version: 2.1.252
import { J, jC, G, Se, BQe } from "/$bunfs/root/chunk-f9h0bg01.js";
...
import { yw } from "/$bunfs/root/chunk-0spqrdaj.js";
import { Oo } from "/$bunfs/root/chunk-6k63g5t6.js";
import { CAn, aYe, Zl, Qt } from "/$bunfs/root/chunk-z2bvp3sv.js";
import { i, _e, H, f, oe } from "/$bunfs/root/chunk-saay52v7.js";
...
import {
  ...
  bb,
  n_,
} from "/$bunfs/root/chunk-56sxk8k2.js";
import { te } from "/$bunfs/root/chunk-wag5ye9w.js";
import { readdir as O, rm as N, stat as U } from "fs/promises";
import { delimiter as j, join as b } from "path";
var F = "plugins",
  w = "cowork_plugins";
function W() {
  if (BQe()) return w;
...
function XE() {
  ...
  if (!e) return [];
  return e.split(j).filter(Boolean).map(km);
}
function _(e) {
  return e.replace(/[^a-zA-Z0-9\-_]/g, "-");
}
function gyt(e) {
  return b(dc(), "data", _(e));
}
function pde(e) {
  let t = gyt(e);
  return le().mkdirSync(t), t;
}
...
import { join as T, resolve as k } from "path";
var RKe = "cache";
function Jy() {
  return k(Se(), dc());
}
...
function hyt() {
  return te([k(be(), "plugins"), k(be(), "cowork_plugins"), Jy(), ...XE().map((e) => k(Se(), e))]);
}
var B = ["available", "required", "auto_install", "not_available"],
  kKe = m(() => oe(B));
function _yt(e) {
...
function jne(e) {
  return e.type === "autoupdate-deferred-entry-helper" || e.type === "autoupdate-disabled-by-policy";
}
function jh(e) {
  return Oo(K(e));
}
function K(e) {
  switch (e.type) {
    case "generic-error":
      return aYe(e.error);
    case "path-not-found":
      return `Path not found: ${Zl(e.path)} (${Qt(e.component)}${e.errno ? `, ${Qt(e.errno)}` : ""})`;
    case "path-traversal":
      return `Path escapes plugin directory: ${Zl(e.path)} (${Qt(e.component)})`;
    case "git-auth-failed":
      return `Git authentication failed (${Qt(e.authType)}): ${Zl(e.gitUrl)}`;
    case "git-timeout":
      return `Git ${Qt(e.operation)} timeout: ${Zl(e.gitUrl)}`;
    case "network-error":
      return `Network error: ${Zl(e.url)}${e.details ? ` - ${Qt(e.details)}` : ""}`;
    case "manifest-parse-error":
      return `Manifest parse error: ${Qt(e.parseError)}`;
    case "manifest-validation-error":
      return `Manifest validation failed: ${Qt(e.validationErrors.join(", "))}`;
    case "plugin-not-found":
      return `Plugin ${Qt(e.pluginId)} not found in marketplace ${Qt(e.marketplace)}`;
    case "marketplace-not-found":
      return `Marketplace ${Qt(e.marketplace)} not found`;
    case "marketplace-load-failed":
      return `Marketplace ${Qt(e.marketplace)} failed to load: ${Qt(e.reason)}`;
    case "mcp-config-invalid":
      return `MCP server ${Qt(e.serverName)} invalid: ${Qt(e.validationError)}`;
    case "hook-load-failed":
      return `Hook load failed: ${Qt(e.reason)}`;
    case "component-load-failed":
      return `${Qt(e.component)} load failed from ${Zl(e.path)}: ${Qt(e.reason)}`;
    case "mcpb-download-failed":
      return `Failed to download MCPB from ${Zl(e.url)}: ${Qt(e.reason)}`;
    case "mcpb-extract-failed":
      return `Failed to extract MCPB ${Zl(e.mcpbPath)}: ${Qt(e.reason)}`;
    case "mcpb-invalid-manifest":
      return `MCPB manifest invalid at ${Zl(e.mcpbPath)}: ${Qt(e.validationError)}`;
    case "lsp-config-invalid":
      return `Plugin "${Qt(e.plugin)}" has invalid LSP server config for "${Qt(e.serverName)}": ${Qt(e.validationError)}`;
    case "lsp-server-start-failed":
      return `Plugin "${Qt(e.plugin)}" failed to start LSP server "${Qt(e.serverName)}": ${Qt(e.reason)}`;
    case "lsp-server-crashed":
      if (e.signal) return `Plugin "${Qt(e.plugin)}" LSP server "${Qt(e.serverName)}" crashed with signal ${e.signal}`;
      return `Plugin "${Qt(e.plugin)}" LSP server "${Qt(e.serverName)}" crashed with exit code ${e.exitCode ?? "unknown"}`;
    case "lsp-request-timeout":
      return `Plugin "${Qt(e.plugin)}" LSP server "${Qt(e.serverName)}" timed out on ${Qt(e.method)} request after ${e.timeoutMs}ms`;
    case "lsp-request-failed":
      return `Plugin "${Qt(e.plugin)}" LSP server "${Qt(e.serverName)}" ${Qt(e.method)} request failed: ${Qt(e.error)}`;
    case "marketplace-blocked-by-policy":
      if (e.blockedByBlocklist) return `Marketplace '${Qt(e.marketplace)}' is blocked by enterprise policy`;
      return `Marketplace '${Qt(e.marketplace)}' is not in the allowed marketplace list`;
    case "dependency-unsatisfied": {
      let t = Ra("plugin install", e.dependency),
        r =
      ...
      return `Dependency "${Qt(e.dependency)}" is ${r}`;
    }
    case "dependency-version-unsatisfied":
      return `Requires "${Qt(e.dependency)}" ${Qt(e.required)}, installed ${Qt(e.installed ?? "version unknown")}`;
    case "plugin-cache-miss":
      return `Plugin "${Qt(e.plugin)}" not cached at ${Zl(e.installPath)} \u2014 run /plugin to refresh`;
    case "plugin-not-installed": {
      let t = Ra("plugin install", e.source, "--scope project");
      return `Plugin "${Qt(e.plugin)}" is enabled in project settings but isn't installed${t ? ` \u2014 run \`${t}\`` : " \u2014 install it at project scope (from /plugin or claude plugin install)"}`;
    }
    case "autoupdate-deferred-entry-helper":
    case "autoupdate-disabled-by-policy":
      return e.message;
    case "autoupdate-blocked-by-pinner": {
      let t = e.heldAt ? ` at ${Zl(e.heldAt)}` : "",
        r = Qt(e.blockedBy.join(", ")),
        o =
          e.disabledPinners.length > 0
            ? ` (note: ${Qt(e.disabledPinners.join(", "))} ${e.disabledPinners.length === 1 ? "is" : "are"} currently disabled)`
            : "";
      return `Autoupdate held "${Qt(e.plugin)}"${t} \u2014 version constraint from ${r}${o}`;
    }
  }
}
function YF(e) {
...
function q(e) {
  switch (e.type) {
    case "folder-shadowed-by-manifest": {
      let t = C(e.manifestFields);
      return `Default ${Qt(e.component)}/ folder is ignored because the manifest sets ${t}`;
    }
    case "mcp-server-suppressed-duplicate": {
      let t = e.duplicateOf.startsWith("plugin:")
        ? `server provided by plugin "${Qt(e.duplicateOf.split(":")[1] ?? "?")}"`
        : `already-configured "${Qt(e.duplicateOf)}"`;
      return `MCP server "${Qt(e.serverName)}" skipped \u2014 same command/URL as ${t}`;
    }
    case "plugin-renamed":
      return e.renamedTo === null
        ? `Removed from the "${Qt(e.marketplace)}" marketplace`
        : `Renamed to "${Qt(e.renamedTo)}" in the "${Qt(e.marketplace)}" marketplace`;
    case "lsp-extension-conflict": {
      let t = e.activeServer.startsWith("plugin:") ? e.activeServer.split(":")[1] : void 0,
        r = t ? `plugin "${Qt(t)}"` : `"${Qt(e.activeServer)}"`;
      return `LSP server "${Qt(e.serverName)}" is not used for ${Qt(e.extension)} files \u2014 ${r} already registered a server for that extension`;
    }
    case "project-scope-suppressed-untrusted":
    case "project-scope-server-stripped":
      return CAn(e.warning);
    case "broken-wikilink":
      return `${Qt(e.raw)} in ${Zl(e.filePath)}:${e.line} doesn't resolve to a skill`;
    case "synced-plugin-shadowed":
      return `"${Qt(e.source)}" from claude.ai not loaded \u2014 "${Qt(e.shadowedBy)}" on this machine has the same name and takes precedence`;
    case "ineffective-disable":
      return `Disabled in ~/.claude/settings.json but still loads \u2014 ${yw(e.overriddenBy)} settings enable it, which overrides your user setting`;
  }
}
function bWt(e) {
  return Oo(X(e));
  ...
  switch (e.type) {
    ...
    case "broken-wikilink":
      return e.reason === "invalid"
        ...
        : `Create one of: ${e.tried.map((t) => Zl(t)).join(" or ")}, or fix the link spelling`;
    case "mcp-server-suppressed-duplicate": {
      if (e.duplicateOf.startsWith("plugin:")) {
        let t = e.duplicateOf.split(":")[1] ?? "the other plugin";
        return `Disable plugin "${Qt(t)}" if you want this plugin's version instead`;
      }
      return `Remove "${Qt(e.duplicateOf)}" from your MCP config if you want the plugin's version instead`;
    }
    case "lsp-extension-conflict": {
      let t = e.activeServer.startsWith("plugin:")
        ...
        : e.activeServer;
      if (t === e.plugin)
        return `Plugin "${Qt(e.plugin)}" declares two LSP servers for ${Qt(e.extension)} \u2014 remove or reorder "${Qt(e.serverName)}" in its lspServers config`;
      return `Disable plugin "${Qt(t)}" to use this plugin's LSP server for ${Qt(e.extension)} files, or disable "${Qt(e.plugin)}" to silence this warning`;
    }
    case "synced-plugin-shadowed": {
      let t = Ra("plugin enable", e.source);
      return `To use the claude.ai copy instead, ${t ? `run \`${t}\`` : "enable it from /plugin"}, then disable or remove "${Qt(e.shadowedBy)}"`;
    }
    case "plugin-renamed":
      return e.renamedTo === null
        ? `Remove "${Qt(e.source)}" from enabledPlugins if you still see this on the next start`
        : `If you still see this on the next start, update enabledPlugins to use "${Qt(e.renamedTo)}@${Qt(e.marketplace)}" (managed settings are not rewritten automatically)`;
    case "ineffective-disable":
      switch (e.overriddenBy) {
        case "projectSettings":
          return `Set "enabledPlugins": {"${Qt(e.source)}": false} in .claude/settings.local.json instead \u2014 project settings override ~/.claude/settings.json`;
        case "localSettings":
          return "Change it to false in .claude/settings.local.json \u2014 that file currently enables it";
        case "flagSettings":
          return `Remove "${Qt(e.source)}" from the --settings value \u2014 that flag overrides all settings files`;
        case "policySettings":
...
import { normalize as Y, parse as Z, resolve as M, sep as Q } from "path";
var eEe = "is network-shaped, carries a dot segment or link component, or could not be classified";
function pC(e, { trustedRoots: t = [] } = {}) {
  ...
  let d = r.networkShapedUnvouched || kve(o);
...
function ee(e, t) {
  ...
  return {
    ...
    root: o,
    tail: s,
    dotSegmentInTail: IU(s),
    networkShapedUnvouched: o === void 0 && (SA(e) || SA(r)),
  };
}
function ne(e) {
  return [dc(), ...XE(), ...e];
...
function ie(e, t) {
  ...
  for (let s of ne(t))
    for (let c of [s, Y(s)]) {
      ...
      let u;
      if (r === d) u = "";
      else if (r.startsWith(d + Q) || r.startsWith(d + "/")) u = e.slice(e.length - (r.length - d.length));
      if (u !== void 0 && (o === void 0 || u.length < o.tail.length)) o = { root: s, tail: u };
    }
...
function S(e) {
  ...
  return o === void 0 ? void 0 : { walkRoot: t, ...o };
}
function wWt(e) {
  let t = S(e);
  if (t === void 0 || t === "undeterminable") return t;
  return { walkRoot: t.walkRoot, distroDir: t.rest.replace(/[\\/]+$/, "").replace(/[\\/]+/g, "/") };
}
function TWt(e, t) {
  return (
...
import { readFileSync as de, statSync as ue } from "fs";
import { isAbsolute as L, join as pe, resolve as E } from "path";
function JF(e, t, r = Date.now()) {
  return Number.isFinite(e) && Math.abs(r - e) < t;
}
var ce = 4194304,
  ge = m(() =>
    f({
...
function r_n(e) {
  let t = Jt(),
    r = t.commandProducerDirsDenied,
    o = I();
  t.commandProducerDirsWslProviderUndeterminable = o === "undeterminable";
  let s = o === "undeterminable" ? void 0 : o;
  (t.commandProducerDirsScannedAt = Date.now()), (t.commandProducerDirsComparable = null);
  for (let c of new Set(e)) {
    let d;
    try {
      let P = pe(c, "installed_plugins.json"),
        y = ue(P);
      if (!y.isFile() || y.size > ce) continue;
      let g = de(P, "utf8");
      d = JSON.parse(g);
    } catch {
      continue;
    }
    let u = typeof d === "object" && d !== null && "plugins" in d ? d.plugins : void 0;
    if (typeof u !== "object" || u === null) continue;
    for (let P of Object.values(u)) {
      if (!Array.isArray(P)) continue;
      for (let y of P) {
        let g = ge().safeParse(y);
        if (!g.success) continue;
        for (let v of [g.data.sourceProducerPath, ...(g.data.previousProducerPaths ?? [])]) {
          let p = v === void 0 ? void 0 : G4(v),
            R = p === void 0 ? void 0 : A(p, s);
          if (R !== void 0) r.add(R);
        }
      }
    }
...
function mde(e, { emit: t = true } = {}) {
  ...
  if (o === void 0) return;
  let s = Jt().commandProducerDirsDenied;
  if (s.has(o)) return;
  if ((s.add(o), (Jt().commandProducerDirsComparable = null), t)) Wne();
}
function Wne() {
  try {
    Jt().commandProducerDirsChanged.emit();
  } catch (e) {
    h(e);
  }
}
var gQn = jC(() => Jt().commandProducerDirsChanged);
...
function I() {
  ...
  if (r?.root === t && JF(r.at, me)) return r.value;
  let o = S(t);
  if (o === "undeterminable") return "undeterminable";
  let s = o === void 0 ? void 0 : { prefix: o.prefix };
  return (e.wslProviderOfPluginsRootMemo = { root: t, value: s, at: Date.now() }), s;
}
var me = 30000;
function A(e, t) {
