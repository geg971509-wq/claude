...
import { gs, kvn, Hvn, Esr } from "/$bunfs/root/chunk-manh1369.js";
...
function rt(t, r) {
  ...
  let n = 0,
    e = "";
  for (let { segment: i } of gs().segment(t)) {
    let o = se(i);
    if (n + o > r - 1) break;
    (e += i), (n += o);
  }
...
function Wo(t) {
  let r = t >= 1000;
  return Esr("en-US", r ? w : M)
    .format(t)
    .toLowerCase();
}
function Gn(t) {
  return Wo(t).replace(".0", "");
}
function g0(t) {
  if (t < 20) return "< 20";
  return `~${Gn(Math.round(t / 10) * 10)}`;
}
function MN(t, r = {}) {
  ...
  for (let { unit: a, seconds: m, shortUnit: l } of c)
    if (Math.abs(u) >= m) {
      ...
      return kvn("long", e).format(d, a);
    }
  if (n === "narrow") return u <= 0 ? "0s ago" : "in 0s";
  return kvn(n, "auto").format(0, "second");
}
function Sy(t, r = {}) {
  let { now: n = new Date(), ...e } = r;
  if (t > n) return MN(t, { ...e, now: n });
  return MN(t, { ...e, numeric: "always", now: n });
}
function uJe(t) {
  let r = t.fileSize !== void 0 ? Ft(t.fileSize) : `${t.messageCount} messages`,
    n = [
      Sy(t.modified, { style: "short" }),
      ...(t.sessionKind === "bg" ? ["bg"] : []),
...
function ad(t, r = false, n = true, e = false) {
  ...
  if (e || u > 24) {
    ...
    return (
      i.toLocaleString("en-US", c).replace(/[ \u202f]([AP]M)/i, (m, l) => l.toLowerCase()) + (r ? ` (${Hvn()})` : "")
    );
  }
  return (
    i
...
function Hoe(t, r = "line") {
  if (t <= 0) return "";
  return `\u2026 +${t} ${k(t, r)}`;
}
export { Nf, ul, rt, ub, LN, ir, Ezt, ZEt, ECe, $t, dfe, Wo, Gn, g0, MN, Sy, uJe, ad, N1e, Hoe };
