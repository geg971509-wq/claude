...
import { Pf } from "/$bunfs/root/chunk-8tgj5dp2.js";
...
import { g0 } from "/$bunfs/root/chunk-2rx5nghb.js";
...
import { hN, yw } from "/$bunfs/root/chunk-0spqrdaj.js";
...
import { Wdt, yk } from "/$bunfs/root/chunk-zze8764r.js";
...
import { QRt, Ume } from "/$bunfs/root/chunk-h5f3xqhk.js";
...
import "/$bunfs/root/chunk-9aea0rng.js";
...
function vt(co, mo) {
  return D(co.source, co.loadedFrom).localeCompare(D(mo.source, mo.loadedFrom)) || ei(co).localeCompare(ei(mo));
}
function Ct(un) {
  return un.name;
}
...
var U = ["on", "name-only", "user-invocable-only", "off"],
  go = {
...
function ko(c, y) {
  let I = ye("policySettings")?.skillOverrides?.[y];
  if (I) return { value: I, source: "policy" };
  let M = ye("flagSettings")?.skillOverrides?.[y];
  if (M) return { value: M, source: "flag" };
...
function ho(c) {
  return ye("projectSettings")?.skillOverrides?.[c] ?? ye("userSettings")?.skillOverrides?.[c];
}
function D(c, y) {
  if (y === "syncedSkills") return hN;
  switch (c) {
    case "mcp":
    case "plugin":
      return c;
...
function Fe(Wt) {
  ...
  if (i[0] !== Be || i[1] !== J) {
    bb0: {
      ...
      if (J) {
        ...
        ue = Ne.sort((Ko, Bo) => (Eo.get(Bo) ?? 0) - (Eo.get(Ko) ?? 0) || ei(Ko).localeCompare(ei(Bo)));
        break bb0;
      }
      ue = Ne.sort(vt);
    }
  ...
  let l = ue,
    Ao;
  if (i[3] === d) (Ao = ye("localSettings")?.skillOverrides ?? {}), (i[3] = Ao);
  else Ao = i[3];
  let Xe = Ao,
    _e;
  if (i[4] !== l) {
  ...
  if (i[8] !== S || i[9] !== H || i[10] !== l)
    (_o = () => {
      ...
      for (const ie of l) {
        ...
        Ge[ie.name] = S.get(ie)?.value ?? Xe[ie.name] ?? H.get(ie.name) ?? "on";
      }
      return Ge;
    }),
      (i[8] = S),
  ...
  let Go;
  if (i[14] !== v) (Go = { isActive: v, onExit: jo, passthroughCtrlKeys: qo }), (i[14] = v), (i[15] = Go);
  else Go = i[15];
  let { query: m, setQuery: N, cursorOffset: Ve, handleKeyDown: We, handlePaste: He } = np(Go);
  Hi();
  let Ze;
  bb1: {
    if (!m) {
    ...
    if (i[16] !== m || i[17] !== l) {
      ...
      ee = l.filter(
        (pe) =>
          ...
          D(pe.source, pe.loadedFrom).toLowerCase().includes(Ye),
      );
  ...
  const ee = Yt - 13;
  let Qo;
  if (i[19] !== p.length || i[20] !== ee) (Qo = Pa(ee, 4, p.length)), (i[19] = p.length), (i[20] = ee), (i[21] = Qo);
  else Qo = i[21];
  let eo = Qo,
    Uo;
  if (i[22] !== p || i[23] !== Qe || i[24] !== S)
    (Uo = () => {
      let le = Qe;
      if (!le || !p.includes(le)) {
        return;
      ...
      Ht((Vo) => {
        ...
        let on = U[(U.indexOf(en) + 1) % U.length];
  ...
  if (i[26] !== Z || i[27] !== S || i[28] !== H || i[29] !== O || i[30] !== l || i[31] !== Je)
    (zo = async () => {
      ...
      if (Yo > 0) {
        let { error: tt } = await rn("localSettings", { skillOverrides: Zo }, void 0, Je);
        if (tt) {
      ...
      O(ke > 0 ? `Updated ${ke} skill ${k(ke, "override")}` : "No changes", { display: "system" });
    }),
      (i[26] = Z),
      (i[27] = S),
      (i[28] = H),
  ...
  let no = zo,
    nt = Ro("confirm:no", "Settings", "esc"),
    sn = Ro("settings:sortByTokens", "Settings", "t"),
    st;
  if (i[33] === d) (st = () => zt(bt)), (i[33] = st);
  else st = i[33];
  let rt;
  if (i[34] !== oo) (rt = { "select:accept": oo, "settings:sortByTokens": st }), (i[34] = oo), (i[35] = rt);
  else rt = i[35];
  const so = !v && p.length > 0;
  let it;
  ...
  let io = ct,
    mt;
  if (i[46] !== He || i[47] !== m || i[48] !== N)
    (mt = (lo) => {
      if (Y.current) {
        He(lo);
        return;
      }
      let he = lo.text.split(/\r\n|\r|\n/, 2)[0] ?? "";
      if (he.length === 0) {
        return;
      }
  ...
  if (l.length === 0) {
    let j;
    if (i[50] !== O) (j = () => O("Skills dialog dismissed", { display: "system" })), (i[50] = O), (i[51] = j);
    else j = i[51];
    let oe;
    if (i[52] === d)
      (oe = e(ze, { action: "confirm:no", context: "Confirmation", fallback: "Esc", description: "close" })),
        (i[52] = oe);
    else oe = i[52];
    let q;
    if (i[53] === d)
      (q = e(sr, {
        hint: Dr()
          ? `Custom skills are disabled in safe mode \u2014 ${_f()} to load them`
    ...
    let G;
    if (i[54] !== j)
      (G = e(me, { title: "Skills", onCancel: j, inputGuide: oe, children: q })), (i[54] = j), (i[55] = G);
    else G = i[55];
    return G;
  }
  let j;
  if (i[56] !== p.length || i[57] !== m || i[58] !== l.length)
    (j = m ? `${p.length}/${l.length} ${k(l.length, "skill")}` : `${l.length} ${k(l.length, "skill")}`),
      (i[56] = p.length),
      (i[57] = m),
      (i[58] = l.length),
      (i[59] = j);
  ...
  let cn = j,
    mn = v
      ...
      : p.length === 0
        ...
        : `enter/space to cycle, / to search, ${sn} to sort, ${nt} to close`;
  const oe = `${cn}${J ? " \xB7 sorted by tokens" : ""} \xB7 ${mn}`;
  let q;
  if (i[60] !== O) (q = () => O("Skills dialog dismissed", { display: "system" })), (i[60] = O), (i[61] = q);
  else q = i[61];
  let G;
  if (i[62] !== v || i[63] !== Ue || i[64] !== Ve || i[65] !== m)
    (G = e(Hd, {
      query: m,
      isFocused: v,
  ...
  if (
    ...
    (Se =
      p.length === 0
        ? e(o, { marginTop: 1, children: e(sr, { children: `No skills match "${m}"` }) })
        : e(
            Ume,
            {
              visibleCount: eo,
                ...
                e(
                  Ume.Item,
                  {
                    children: e(Pe, {
                      skill: te,
                      lock: S.get(te),
                      state: S.get(te)?.value ?? Z[te.name] ?? "on",
                      bytesPerToken: Ae,
                    }),
                  },
                  `${te.name}-${te.source}`,
  ...
  let Ce;
  if (i[79] !== io || i[80] !== ao || i[81] !== G || i[82] !== Se || i[83] !== ve)
    (Ce = r(o, {
...
function Pe(yn) {
  let Q = _(23),
    ...
    x = go[gn],
    pt;
  if (Q[0] !== uo || Q[1] !== w) (pt = g0(Wdt(w, uo))), (Q[0] = uo), (Q[1] = w), (Q[2] = pt);
  else pt = Q[2];
  let fo = `${pt} tok`,
    be;
  ...
  let yt;
  if (Q[8] === d) (yt = e(t, { children: "  " })), (Q[8] = yt);
  else yt = Q[8];
  const po = kn ? "suggestion" : void 0;
  let we;
  if (Q[9] !== w.name || Q[10] !== po)
    (we = e(t, { color: po, children: w.name })), (Q[9] = w.name), (Q[10] = po), (Q[11] = we);
  else we = Q[11];
  let Oe;
  if (Q[12] !== w.loadedFrom || Q[13] !== w.source)
    (Oe = D(w.source, w.loadedFrom)), (Q[12] = w.loadedFrom), (Q[13] = w.source), (Q[14] = Oe);
  else Oe = Q[14];
  ...
  if (Q[19] !== be || Q[20] !== we || Q[21] !== xe)
    (gt = r(o, { children: [be, yt, we, xe] })), (Q[19] = be), (Q[20] = we), (Q[21] = xe), (Q[22] = gt);
  else gt = Q[22];
  return gt;
}
async function Rn(c, y) {
  return e(Fe, { onExit: c, commands: y.options.commands, bytesPerToken: Pf(y.options.mainLoopModel) });
}
export { Rn as call };
