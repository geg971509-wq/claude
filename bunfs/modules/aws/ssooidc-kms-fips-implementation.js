// @bun @bytecode
// Claude Code is a Beta product per Anthropic's Commercial Terms of Service.
// By using Claude Code, you agree that all code acceptance or rejection decisions you make,
// and the associated conversations in context, constitute Feedback under Anthropic's Commercial Terms,
// and may be used to improve Anthropic's products, including training models.
// You are responsible for reviewing any code suggestions before use.

// (c) Anthropic PBC. All rights reserved. Use is subject to the Legal Agreements outlined here: https://code.claude.com/docs/en/legal-and-compliance.

// Version: 2.1.252
import { VH, _g } from "/$bunfs/root/chunk-1bw3xm1h.js";
import { oa } from "/$bunfs/root/chunk-jpaz9qpm.js";
import { by } from "/$bunfs/root/chunk-w9fwyp2c.js";
import { CYe } from "/$bunfs/root/chunk-718sa7e6.js";
import { EN, AN, CN, vO, gg, RO, vN, Rn, kO, jT, RN, kN, HN, xN, IN, _P } from "/$bunfs/root/chunk-0km1g2gp.js";
import { Pt } from "/$bunfs/root/chunk-zjp97w3p.js";
import { k_ } from "/$bunfs/root/chunk-830v7vcp.js";
import { Coe } from "/$bunfs/root/chunk-t29zxx9d.js";
import { H_ } from "/$bunfs/root/chunk-3dec2xkz.js";
import { WT } from "/$bunfs/root/chunk-37hhv0gf.js";
import { Pl } from "/$bunfs/root/chunk-127gwmpc.js";
import { yy } from "/$bunfs/root/chunk-vxdax23w.js";
import { yR } from "/$bunfs/root/chunk-a9qrd4qt.js";
import "/$bunfs/root/chunk-g2s7kk3r.js";
import { yg } from "/$bunfs/root/chunk-3m9p2z28.js";
import { S } from "/$bunfs/root/chunk-yz031c9r.js";
var A = S(function (N) {
  Object.defineProperty(N, "__esModule", { value: true });
  N.resolveHttpAuthSchemeConfig =
    N.defaultSSOOIDCHttpAuthSchemeProvider =
    N.defaultSSOOIDCHttpAuthSchemeParametersProvider =
      void 0;
  var ze = gg(),
    O = yy(),
    Ke = async (e, t, n) => ({
      operation: (0, O.getSmithyContext)(t).operation,
      region:
        (await (0, O.normalizeProvider)(e.region)()) ||
        (() => {
          throw Error("expected `region` to be configured for `aws.auth#sigv4`");
        })(),
    });
  N.defaultSSOOIDCHttpAuthSchemeParametersProvider = Ke;
  function Le(e) {
    return {
      schemeId: "aws.auth#sigv4",
      signingProperties: { name: "sso-oauth", region: e.region },
      propertiesExtractor: (t, n) => ({ signingProperties: { config: t, context: n } }),
    };
  }
  function Ve(e) {
    return { schemeId: "smithy.api#noAuth" };
  }
  var Be = (e) => {
    let t = [];
    switch (e.operation) {
      case "CreateToken": {
        t.push(Ve(e));
        break;
      }
      default:
        t.push(Le(e));
    }
    return t;
  };
  N.defaultSSOOIDCHttpAuthSchemeProvider = Be;
  var Ye = (e) => {
    let t = (0, ze.resolveAwsSdkSigV4Config)(e);
    return Object.assign(t, { authSchemePreference: (0, O.normalizeProvider)(e.authSchemePreference ?? []) });
  };
  N.resolveHttpAuthSchemeConfig = Ye;
});
var te = S(function (Z) {
  Object.defineProperty(Z, "__esModule", { value: true });
  Z.ruleSet = void 0;
  var W = "required",
    p = "fn",
    d = "argv",
    m = "ref",
    F = true,
    M = "isSet",
    I = "booleanEquals",
    f = "error",
    x = "endpoint",
    E = "tree",
    D = "PartitionResult",
    R = "getAttr",
    G = { [W]: false, type: "string" },
    j = { [W]: true, default: false, type: "boolean" },
    z = { [m]: "Endpoint" },
    J = { [p]: I, [d]: [{ [m]: "UseFIPS" }, true] },
    X = { [p]: I, [d]: [{ [m]: "UseDualStack" }, true] },
    a = {},
    K = { [p]: R, [d]: [{ [m]: D }, "supportsFIPS"] },
    Q = { [m]: D },
    L = { [p]: I, [d]: [true, { [p]: R, [d]: [Q, "supportsDualStack"] }] },
    V = [J],
    B = [X],
    Y = [{ [m]: "Region" }],
    Xe = {
      version: "1.0",
      parameters: { Region: G, UseDualStack: j, UseFIPS: j, Endpoint: G },
      rules: [
        {
          conditions: [{ [p]: M, [d]: [z] }],
          rules: [
            { conditions: V, error: "Invalid Configuration: FIPS and custom endpoint are not supported", type: f },
            { conditions: B, error: "Invalid Configuration: Dualstack and custom endpoint are not supported", type: f },
            { endpoint: { url: z, properties: a, headers: a }, type: x },
          ],
          type: E,
        },
        {
          conditions: [{ [p]: M, [d]: Y }],
          rules: [
            {
              conditions: [{ [p]: "aws.partition", [d]: Y, assign: D }],
              rules: [
                {
                  conditions: [J, X],
                  rules: [
                    {
                      conditions: [{ [p]: I, [d]: [F, K] }, L],
                      rules: [
                        {
                          endpoint: {
                            url: "https://oidc-fips.{Region}.{PartitionResult#dualStackDnsSuffix}",
                            properties: a,
                            headers: a,
                          },
                          type: x,
                        },
                      ],
                      type: E,
                    },
                    {
                      error: "FIPS and DualStack are enabled, but this partition does not support one or both",
                      type: f,
                    },
                  ],
                  type: E,
                },
                {
                  conditions: V,
                  rules: [
                    {
                      conditions: [{ [p]: I, [d]: [K, F] }],
                      rules: [
                        {
                          conditions: [{ [p]: "stringEquals", [d]: [{ [p]: R, [d]: [Q, "name"] }, "aws-us-gov"] }],
                          endpoint: { url: "https://oidc.{Region}.amazonaws.com", properties: a, headers: a },
                          type: x,
                        },
                        {
                          endpoint: {
                            url: "https://oidc-fips.{Region}.{PartitionResult#dnsSuffix}",
                            properties: a,
                            headers: a,
                          },
                          type: x,
                        },
                      ],
                      type: E,
                    },
                    { error: "FIPS is enabled but this partition does not support FIPS", type: f },
                  ],
                  type: E,
                },
                {
                  conditions: B,
                  rules: [
                    {
                      conditions: [L],
                      rules: [
                        {
                          endpoint: {
                            url: "https://oidc.{Region}.{PartitionResult#dualStackDnsSuffix}",
                            properties: a,
                            headers: a,
                          },
                          type: x,
                        },
                      ],
                      type: E,
                    },
                    { error: "DualStack is enabled but this partition does not support DualStack", type: f },
                  ],
                  type: E,
                },
                {
                  endpoint: { url: "https://oidc.{Region}.{PartitionResult#dnsSuffix}", properties: a, headers: a },
                  type: x,
                },
              ],
              type: E,
            },
          ],
          type: E,
        },
        { error: "Invalid Configuration: Missing Region", type: f },
      ],
    };
  Z.ruleSet = Xe;
});
var ne = S(function (re) {
  Object.defineProperty(re, "__esModule", { value: true });
  re.defaultEndpointResolver = void 0;
  var Qe = vO(),
    T = VH(),
    Ze = te(),
    et = new T.EndpointCache({ size: 50, params: ["Endpoint", "Region", "UseDualStack", "UseFIPS"] }),
    tt = (e, t = {}) => et.get(e, () => (0, T.resolveEndpoint)(Ze.ruleSet, { endpointParams: e, logger: t.logger }));
  re.defaultEndpointResolver = tt;
  T.customEndpointFunctions.aws = Qe.awsEndpointFunctions;
});
var pe = S(function (ce) {
  Object.defineProperty(ce, "__esModule", { value: true });
  ce.getRuntimeConfig = void 0;
  var rt = gg(),
    ot = xN(),
    nt = oa(),
    it = Pt(),
    st = yR(),
    ie = H_(),
    se = yg(),
    ct = A(),
    at = ne(),
    pt = (e) => ({
      apiVersion: "2019-06-10",
      base64Decoder: e?.base64Decoder ?? ie.fromBase64,
      base64Encoder: e?.base64Encoder ?? ie.toBase64,
      disableHostPrefix: e?.disableHostPrefix ?? false,
      endpointProvider: e?.endpointProvider ?? at.defaultEndpointResolver,
      extensions: e?.extensions ?? [],
      httpAuthSchemeProvider: e?.httpAuthSchemeProvider ?? ct.defaultSSOOIDCHttpAuthSchemeProvider,
      httpAuthSchemes: e?.httpAuthSchemes ?? [
        {
          schemeId: "aws.auth#sigv4",
          identityProvider: (t) => t.getIdentityProvider("aws.auth#sigv4"),
          signer: new rt.AwsSdkSigV4Signer(),
        },
        {
          schemeId: "smithy.api#noAuth",
          identityProvider: (t) => t.getIdentityProvider("smithy.api#noAuth") || (async () => ({})),
          signer: new nt.NoAuthSigner(),
        },
      ],
      logger: e?.logger ?? new it.NoOpLogger(),
      protocol: e?.protocol ?? new ot.AwsRestJsonProtocol({ defaultNamespace: "com.amazonaws.ssooidc" }),
      serviceId: e?.serviceId ?? "SSO OIDC",
      urlParser: e?.urlParser ?? st.parseUrl,
      utf8Decoder: e?.utf8Decoder ?? se.fromUtf8,
      utf8Encoder: e?.utf8Encoder ?? se.toUtf8,
    });
  ce.getRuntimeConfig = pt;
});
var ge = S(function (_e) {
  Object.defineProperty(_e, "__esModule", { value: true });
  _e.getRuntimeConfig = void 0;
  var dt = Coe(),
    ut = dt.__importDefault(CYe()),
    de = gg(),
    ue = RN(),
    C = _g(),
    lt = kN(),
    le = jT(),
    v = by(),
    he = WT(),
    ht = HN(),
    _t = kO(),
    Et = pe(),
    gt = Pt(),
    St = IN(),
    vt = Pt(),
    ft = (e) => {
      (0, vt.emitWarningIfUnsupportedVersion)(process.version);
      let t = (0, St.resolveDefaultsModeConfig)(e),
        n = () => t().then(gt.loadConfigsForDefaultMode),
        u = (0, Et.getRuntimeConfig)(e);
      (0, de.emitWarningIfUnsupportedVersion)(process.version);
      let i = { profile: e?.profile, logger: u.logger };
      return {
        ...u,
        ...e,
        runtime: "node",
        defaultsMode: t,
        authSchemePreference: e?.authSchemePreference ?? (0, v.loadConfig)(de.NODE_AUTH_SCHEME_PREFERENCE_OPTIONS, i),
        bodyLengthChecker: e?.bodyLengthChecker ?? ht.calculateBodyLength,
        defaultUserAgentProvider:
          e?.defaultUserAgentProvider ??
          (0, ue.createDefaultUserAgentProvider)({ serviceId: u.serviceId, clientVersion: ut.default.version }),
        maxAttempts: e?.maxAttempts ?? (0, v.loadConfig)(le.NODE_MAX_ATTEMPT_CONFIG_OPTIONS, e),
        region:
          e?.region ?? (0, v.loadConfig)(C.NODE_REGION_CONFIG_OPTIONS, { ...C.NODE_REGION_CONFIG_FILE_OPTIONS, ...i }),
        requestHandler: he.NodeHttpHandler.create(e?.requestHandler ?? n),
        retryMode:
          e?.retryMode ??
          (0, v.loadConfig)(
            {
              ...le.NODE_RETRY_MODE_CONFIG_OPTIONS,
              default: async () => (await n()).retryMode || _t.DEFAULT_RETRY_MODE,
            },
            e,
          ),
        sha256: e?.sha256 ?? lt.Hash.bind(null, "sha256"),
        streamCollector: e?.streamCollector ?? he.streamCollector,
        useDualstackEndpoint:
          e?.useDualstackEndpoint ?? (0, v.loadConfig)(C.NODE_USE_DUALSTACK_ENDPOINT_CONFIG_OPTIONS, i),
        useFipsEndpoint: e?.useFipsEndpoint ?? (0, v.loadConfig)(C.NODE_USE_FIPS_ENDPOINT_CONFIG_OPTIONS, i),
        userAgentAppId: e?.userAgentAppId ?? (0, v.loadConfig)(ue.NODE_APP_ID_CONFIG_OPTIONS, i),
      };
    };
  _e.getRuntimeConfig = ft;
});
var Wr = S(function (U) {
  var Se = EN(),
    xt = AN(),
    mt = CN(),
    ve = RO(),
    It = _g(),
    w = oa(),
    s = k_(),
    yt = vN(),
    ye = Rn(),
    fe = jT(),
    g = Pt(),
    xe = A(),
    Ct = ge(),
    me = _P(),
    Ie = Pl(),
    Ot = (e) =>
      Object.assign(e, {
        useDualstackEndpoint: e.useDualstackEndpoint ?? false,
        useFipsEndpoint: e.useFipsEndpoint ?? false,
        defaultSigningName: "sso-oauth",
      }),
    At = {
      UseFIPS: { type: "builtInParams", name: "useFipsEndpoint" },
      Endpoint: { type: "builtInParams", name: "endpoint" },
      Region: { type: "builtInParams", name: "region" },
      UseDualStack: { type: "builtInParams", name: "useDualstackEndpoint" },
    },
    Dt = (e) => {
      let { httpAuthSchemes: t, httpAuthSchemeProvider: n, credentials: u } = e;
      return {
        setHttpAuthScheme(i) {
          let y = t.findIndex((P) => P.schemeId === i.schemeId);
          if (y === -1) t.push(i);
          else t.splice(y, 1, i);
        },
        httpAuthSchemes() {
          return t;
        },
        setHttpAuthSchemeProvider(i) {
          n = i;
        },
        httpAuthSchemeProvider() {
          return n;
        },
        setCredentials(i) {
          u = i;
        },
        credentials() {
          return u;
        },
      };
    },
    Rt = (e) => ({
      httpAuthSchemes: e.httpAuthSchemes(),
      httpAuthSchemeProvider: e.httpAuthSchemeProvider(),
      credentials: e.credentials(),
    }),
    Tt = (e, t) => {
      let n = Object.assign(
        me.getAwsRegionExtensionConfiguration(e),
        g.getDefaultExtensionConfiguration(e),
        Ie.getHttpHandlerExtensionConfiguration(e),
        Dt(e),
      );
      return (
        t.forEach((u) => u.configure(n)),
        Object.assign(
          e,
          me.resolveAwsRegionExtensionConfiguration(n),
          g.resolveDefaultRuntimeConfig(n),
          Ie.resolveHttpHandlerRuntimeConfig(n),
          Rt(n),
        )
      );
    };
  class q extends g.Client {
    config;
    constructor(...[e]) {
      let t = Ct.getRuntimeConfig(e || {});
      super(t);
      this.initConfig = t;
      let n = Ot(t),
        u = ve.resolveUserAgentConfig(n),
        i = fe.resolveRetryConfig(u),
        y = It.resolveRegionConfig(i),
        P = Se.resolveHostHeaderConfig(y),
        Fe = ye.resolveEndpointConfig(P),
        Me = xe.resolveHttpAuthSchemeConfig(Fe),
        Ge = Tt(Me, e?.extensions || []);
      (this.config = Ge),
        this.middlewareStack.use(s.getSchemaSerdePlugin(this.config)),
        this.middlewareStack.use(ve.getUserAgentPlugin(this.config)),
        this.middlewareStack.use(fe.getRetryPlugin(this.config)),
        this.middlewareStack.use(yt.getContentLengthPlugin(this.config)),
        this.middlewareStack.use(Se.getHostHeaderPlugin(this.config)),
        this.middlewareStack.use(xt.getLoggerPlugin(this.config)),
        this.middlewareStack.use(mt.getRecursionDetectionPlugin(this.config)),
        this.middlewareStack.use(
          w.getHttpAuthSchemeEndpointRuleSetPlugin(this.config, {
            httpAuthSchemeParametersProvider: xe.defaultSSOOIDCHttpAuthSchemeParametersProvider,
            identityProviderConfigProvider: async (je) =>
              new w.DefaultIdentityProviderConfig({ "aws.auth#sigv4": je.credentials }),
          }),
        ),
        this.middlewareStack.use(w.getHttpSigningPlugin(this.config));
    }
    destroy() {
      super.destroy();
    }
  }
  var c = class e extends g.ServiceException {
      constructor(t) {
        super(t);
        Object.setPrototypeOf(this, e.prototype);
      }
    },
    Ce = class e extends c {
      name = "AccessDeniedException";
      $fault = "client";
      error;
      reason;
      error_description;
      constructor(t) {
        super({ name: "AccessDeniedException", $fault: "client", ...t });
        Object.setPrototypeOf(this, e.prototype),
          (this.error = t.error),
          (this.reason = t.reason),
          (this.error_description = t.error_description);
      }
    },
    Pe = class e extends c {
      name = "AuthorizationPendingException";
      $fault = "client";
      error;
      error_description;
      constructor(t) {
        super({ name: "AuthorizationPendingException", $fault: "client", ...t });
        Object.setPrototypeOf(this, e.prototype),
          (this.error = t.error),
          (this.error_description = t.error_description);
      }
    },
    Oe = class e extends c {
      name = "ExpiredTokenException";
      $fault = "client";
      error;
      error_description;
      constructor(t) {
        super({ name: "ExpiredTokenException", $fault: "client", ...t });
        Object.setPrototypeOf(this, e.prototype),
          (this.error = t.error),
          (this.error_description = t.error_description);
      }
    },
    Ae = class e extends c {
      name = "InternalServerException";
      $fault = "server";
      error;
      error_description;
      constructor(t) {
        super({ name: "InternalServerException", $fault: "server", ...t });
        Object.setPrototypeOf(this, e.prototype),
          (this.error = t.error),
          (this.error_description = t.error_description);
      }
    },
    De = class e extends c {
      name = "InvalidClientException";
      $fault = "client";
      error;
      error_description;
      constructor(t) {
        super({ name: "InvalidClientException", $fault: "client", ...t });
        Object.setPrototypeOf(this, e.prototype),
          (this.error = t.error),
          (this.error_description = t.error_description);
      }
    },
    Re = class e extends c {
      name = "InvalidGrantException";
      $fault = "client";
      error;
      error_description;
      constructor(t) {
        super({ name: "InvalidGrantException", $fault: "client", ...t });
        Object.setPrototypeOf(this, e.prototype),
          (this.error = t.error),
          (this.error_description = t.error_description);
      }
    },
    Te = class e extends c {
      name = "InvalidRequestException";
      $fault = "client";
      error;
      reason;
      error_description;
      constructor(t) {
        super({ name: "InvalidRequestException", $fault: "client", ...t });
        Object.setPrototypeOf(this, e.prototype),
          (this.error = t.error),
          (this.reason = t.reason),
          (this.error_description = t.error_description);
      }
    },
    we = class e extends c {
      name = "InvalidScopeException";
      $fault = "client";
      error;
      error_description;
      constructor(t) {
        super({ name: "InvalidScopeException", $fault: "client", ...t });
        Object.setPrototypeOf(this, e.prototype),
          (this.error = t.error),
          (this.error_description = t.error_description);
      }
    },
    qe = class e extends c {
      name = "SlowDownException";
      $fault = "client";
      error;
      error_description;
      constructor(t) {
        super({ name: "SlowDownException", $fault: "client", ...t });
        Object.setPrototypeOf(this, e.prototype),
          (this.error = t.error),
          (this.error_description = t.error_description);
      }
    },
    ke = class e extends c {
      name = "UnauthorizedClientException";
      $fault = "client";
      error;
      error_description;
      constructor(t) {
        super({ name: "UnauthorizedClientException", $fault: "client", ...t });
        Object.setPrototypeOf(this, e.prototype),
          (this.error = t.error),
          (this.error_description = t.error_description);
      }
    },
    be = class e extends c {
      name = "UnsupportedGrantTypeException";
      $fault = "client";
      error;
      error_description;
      constructor(t) {
        super({ name: "UnsupportedGrantTypeException", $fault: "client", ...t });
        Object.setPrototypeOf(this, e.prototype),
          (this.error = t.error),
          (this.error_description = t.error_description);
      }
    },
    wt = "AccessDeniedException",
    qt = "AuthorizationPendingException",
    kt = "AccessToken",
    bt = "ClientSecret",
    Ut = "CreateToken",
    Nt = "CreateTokenRequest",
    Ht = "CreateTokenResponse",
    $t = "CodeVerifier",
    Ft = "ExpiredTokenException",
    Mt = "InvalidClientException",
    Gt = "InvalidGrantException",
    jt = "InvalidRequestException",
    zt = "InternalServerException",
    Kt = "InvalidScopeException",
    Lt = "IdToken",
    Vt = "RefreshToken",
    Bt = "SlowDownException",
    Yt = "UnauthorizedClientException",
    Wt = "UnsupportedGrantTypeException",
    Jt = "accessToken",
    _ = "client",
    Xt = "clientId",
    Qt = "clientSecret",
    Zt = "codeVerifier",
    er = "code",
    tr = "deviceCode",
    o = "error",
    rr = "expiresIn",
    l = "error_description",
    or = "grantType",
    nr = "http",
    h = "httpError",
    ir = "idToken",
    Ue = "reason",
    Ne = "refreshToken",
    sr = "redirectUri",
    cr = "scope",
    ar = "server",
    He = "smithy.ts.sdk.synthetic.com.amazonaws.ssooidc",
    pr = "tokenType",
    r = "com.amazonaws.ssooidc",
    dr = [0, r, kt, 8, 0],
    ur = [0, r, bt, 8, 0],
    lr = [0, r, $t, 8, 0],
    hr = [0, r, Lt, 8, 0],
    $e = [0, r, Vt, 8, 0],
    _r = [-3, r, wt, { [o]: _, [h]: 400 }, [o, Ue, l], [0, 0, 0]];
  s.TypeRegistry.for(r).registerError(_r, Ce);
  var Er = [-3, r, qt, { [o]: _, [h]: 400 }, [o, l], [0, 0]];
  s.TypeRegistry.for(r).registerError(Er, Pe);
  var gr = [
      3,
      r,
      Nt,
      0,
      [Xt, Qt, or, tr, er, Ne, cr, sr, Zt],
      [0, [() => ur, 0], 0, 0, 0, [() => $e, 0], 64, 0, [() => lr, 0]],
    ],
    Sr = [3, r, Ht, 0, [Jt, pr, rr, Ne, ir], [[() => dr, 0], 0, 1, [() => $e, 0], [() => hr, 0]]],
    vr = [-3, r, Ft, { [o]: _, [h]: 400 }, [o, l], [0, 0]];
  s.TypeRegistry.for(r).registerError(vr, Oe);
  var fr = [-3, r, zt, { [o]: ar, [h]: 500 }, [o, l], [0, 0]];
  s.TypeRegistry.for(r).registerError(fr, Ae);
  var xr = [-3, r, Mt, { [o]: _, [h]: 401 }, [o, l], [0, 0]];
  s.TypeRegistry.for(r).registerError(xr, De);
  var mr = [-3, r, Gt, { [o]: _, [h]: 400 }, [o, l], [0, 0]];
  s.TypeRegistry.for(r).registerError(mr, Re);
  var Ir = [-3, r, jt, { [o]: _, [h]: 400 }, [o, Ue, l], [0, 0, 0]];
  s.TypeRegistry.for(r).registerError(Ir, Te);
  var yr = [-3, r, Kt, { [o]: _, [h]: 400 }, [o, l], [0, 0]];
  s.TypeRegistry.for(r).registerError(yr, we);
  var Cr = [-3, r, Bt, { [o]: _, [h]: 400 }, [o, l], [0, 0]];
  s.TypeRegistry.for(r).registerError(Cr, qe);
  var Pr = [-3, r, Yt, { [o]: _, [h]: 400 }, [o, l], [0, 0]];
  s.TypeRegistry.for(r).registerError(Pr, ke);
  var Or = [-3, r, Wt, { [o]: _, [h]: 400 }, [o, l], [0, 0]];
  s.TypeRegistry.for(r).registerError(Or, be);
  var Ar = [-3, He, "SSOOIDCServiceException", 0, [], []];
  s.TypeRegistry.for(He).registerError(Ar, c);
  var Dr = [9, r, Ut, { [nr]: ["POST", "/token", 200] }, () => gr, () => Sr];
  class k extends g.Command.classBuilder()
    .ep(At)
    .m(function (e, t, n, u) {
      return [ye.getEndpointPlugin(n, e.getEndpointParameterInstructions())];
    })
    .s("AWSSSOOIDCService", "CreateToken", {})
    .n("SSOOIDCClient", "CreateTokenCommand")
    .sc(Dr)
    .build() {}
  var Rr = { CreateTokenCommand: k };
  class b extends q {}
  g.createAggregatedClient(Rr, b);
  var Tr = { KMS_ACCESS_DENIED: "KMS_AccessDeniedException" },
    wr = {
      KMS_DISABLED_KEY: "KMS_DisabledException",
      KMS_INVALID_KEY_USAGE: "KMS_InvalidKeyUsageException",
      KMS_INVALID_STATE: "KMS_InvalidStateException",
      KMS_KEY_NOT_FOUND: "KMS_NotFoundException",
    };
  Object.defineProperty(U, "$Command", {
    enumerable: true,
    get: function () {
      return g.Command;
    },
  });
  Object.defineProperty(U, "__Client", {
    enumerable: true,
    get: function () {
      return g.Client;
    },
  });
  U.AccessDeniedException = Ce;
  U.AccessDeniedExceptionReason = Tr;
  U.AuthorizationPendingException = Pe;
  U.CreateTokenCommand = k;
  U.ExpiredTokenException = Oe;
  U.InternalServerException = Ae;
  U.InvalidClientException = De;
  U.InvalidGrantException = Re;
  U.InvalidRequestException = Te;
  U.InvalidRequestExceptionReason = wr;
  U.InvalidScopeException = we;
  U.SSOOIDC = b;
  U.SSOOIDCClient = q;
  U.SSOOIDCServiceException = c;
  U.SlowDownException = qe;
  U.UnauthorizedClientException = ke;
  U.UnsupportedGrantTypeException = be;
});
export default Wr();
