...
import { Mvn, Fvn, kzt } from "/$bunfs/root/chunk-f5rvxbzn.js";
import { AYe } from "/$bunfs/root/chunk-bn1kv3ez.js";
import { _Cn } from "/$bunfs/root/chunk-9jsjxwjw.js";
import "/$bunfs/root/chunk-g2s7kk3r.js";
import { gGt, oCn } from "/$bunfs/root/chunk-s0ffdtkp.js";
import { TN } from "/$bunfs/root/chunk-0ta8hqkv.js";
import { CCn } from "/$bunfs/root/chunk-v1m2b053.js";
import "/$bunfs/root/chunk-fknxtxqh.js";
import { SCn } from "/$bunfs/root/chunk-8c24h0ga.js";
import { _g } from "/$bunfs/root/chunk-1bw3xm1h.js";
...
function h(e) {
  return async (r) => {
    ...
    let { GetCredentialsForIdentityCommand: o, CognitoIdentityClient: t } = await import(
        "/$bunfs/root/chunk-z6yd084d.js"
      ),
      n = (w) => e.clientConfig?.[w] ?? e.parentClientConfig?.[w] ?? r?.callerClientConfig?.[w],
      {
        Credentials: { AccessKeyId: i = K(e.logger), Expiration: a, SecretKey: c = H(e.logger), SessionToken: d } = q(
          e.logger,
        ),
      } = await (
        e.client ??
    ...
    return { identityId: e.identityId, accessKeyId: i, secretAccessKey: c, sessionToken: d, expiration: a };
  };
}
function K(e) {
  throw new S.CredentialsProviderError("Response from Amazon Cognito contained no access key ID", { logger: e });
}
function q(e) {
  throw new S.CredentialsProviderError("Response from Amazon Cognito contained no credentials", { logger: e });
}
function H(e) {
  throw new S.CredentialsProviderError("Response from Amazon Cognito contained no secret key", { logger: e });
}
var P = j(qc(), 1);
class A {
  dbName;
...
function T({
  ...
  let u = c ? `aws:cognito-identity-credentials:${i}:${c}` : void 0,
    s = async (g) => {
      let { GetIdCommand: l, CognitoIdentityClient: p } = await import("/$bunfs/root/chunk-z6yd084d.js"),
      ...
      if (!C) {
        let { IdentityId: m = W(d) } = await y.send(
...
function W(e) {
  throw new P.CredentialsProviderError("Response from Amazon Cognito contained no identity ID", { logger: e });
}
var ae = (e) => h({ ...e });
var le = (e) => T({ ...e });
kzt();
var ue = (e) => (e?.logger?.debug("@smithy/credential-provider-imds", "fromContainerMetadata"), Mvn(e));
var Ce = (e) => AYe(e);
var Ie = (e = {}) => oCn({ ...e });
kzt();
var k = j(Jh(), 1),
  xe = (e) => (
    ...
    async () => Fvn(e)().then((r) => k.setCredentialFeature(r, "CREDENTIALS_IMDS", "0"))
  );
var be = (e) => gGt({ ...e });
var _ = (e = {}) => TN({ ...e });
var ke = (e) => CCn(e);
...
var M = j(oa(), 1),
  ...
  R = (e, r, o) => {
    ...
    return async (n = {}) => {
      ...
      let d = { ...e.params, RoleSessionName: e.params.RoleSessionName ?? "aws-sdk-js-" + Date.now() };
      if (d?.SerialNumber) {
      ...
      let { AssumeRoleCommand: w, STSClient: u } = await import("/$bunfs/root/chunk-fn8283n7.js");
      if (!t) {
      ...
      if (!s || !s.AccessKeyId || !s.SecretAccessKey)
        throw new b.CredentialsProviderError(`Invalid response from STS.assumeRole call with role ${d.RoleArn}`, {
          logger: c,
        });
      return {
