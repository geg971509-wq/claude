  ...
  O = (e, r, t) => {
    let o = {
      EcsContainer: async (s) => {
        let { fromHttp: n } = await import("/$bunfs/root/chunk-csen4nav.js"),
          { fromContainerMetadata: i } = await import("/$bunfs/root/chunk-wc9tsqq5.js");
        return (
      ...
      Ec2InstanceMetadata: async (s) => {
        ...
        let { fromInstanceMetadata: n } = await import("/$bunfs/root/chunk-wc9tsqq5.js");
        return async () => n(s)().then(S);
      },
      Environment: async (s) => {
        t?.debug("@aws-sdk/credential-provider-ini - credential_source is Environment");
        let { fromEnv: n } = await import("/$bunfs/root/chunk-tnh10py3.js");
        return async () => n(s)().then(S);
      },
    };
...
var K = (e, { profile: r = "default", logger: t } = {}) =>
  ...
  ie = (e, { profile: r, logger: t }) => {
    ...
    if (o) t?.debug?.(`    ${r} isCredentialSourceProfile credential_source=${e.credential_source}`);
    return o;
  },
  B = async (e, r, t, o = {}, s) => {
    ...
    if (!t.roleAssumer) {
      let { getDefaultRoleAssumer: l } = await import("/$bunfs/root/chunk-q7b9pec3.js").then((m) => j(m.default, 1));
      t.roleAssumer = l(
        {
          ...t.clientConfig,
          credentialProviderLogger: t.logger,
          parentClientConfig: { ...t?.parentClientConfig, region: u ?? t?.parentClientConfig?.region },
        },
        t.clientPlugins,
      );
    }
    ...
    if (F(n)) return g.then((l) => I.setCredentialFeature(l, "CREDENTIALS_PROFILE_SOURCE_PROFILE", "o"));
      ...
      let d = await g;
      return t.roleAssumer(d, l).then((c) => I.setCredentialFeature(c, "CREDENTIALS_PROFILE_SOURCE_PROFILE", "o"));
    }
  },
...
var f = j(qc(), 1),
  W = j(Pl(), 1),
  q = j(mA(), 1);
import { createHash as ae, createPrivateKey as ce, createPublicKey as le, sign as de } from "crypto";
...
class w {
  ...
  async refresh(e) {
    let { SigninClient: r, CreateOAuth2TokenCommand: t } = await import("/$bunfs/root/chunk-cys7bw0v.js").then((m) =>
        j(m.default, 1),
      ),
      ...
      u = this.profileData.region ?? (await this.callerClientConfig?.region?.()) ?? process.env.AWS_REGION,
      g = new r({
    ...
    let l = { tokenInput: { clientId: e.clientId, refreshToken: e.refreshToken, grantType: "refresh_token" } };
    try {
  ...
  createDPoPInterceptor(e) {
    e.add(
      (r) => async (t) => {
        if (W.HttpRequest.isInstance(t.request)) {
          let o = t.request,
...
var z = j(Jh(), 1),
  ...
  X = async (e, r) =>
    import("/$bunfs/root/chunk-xh81a9ew.js").then(({ fromProcess: t }) =>
      t({ ...e, profile: r })().then((o) => z.setCredentialFeature(o, "CREDENTIALS_PROFILE_PROCESS", "v")),
    );
var x = j(Jh(), 1),
  Q = async (e, r, t = {}) => {
    let { fromSSO: o } = await import("/$bunfs/root/chunk-nfh8e7rm.js");
    return o({
      profile: e,
      logger: t.logger,
...
var te = j(Jh(), 1),
  re = (e) =>
    ...
    ["undefined", "string"].indexOf(typeof e.role_session_name) > -1,
  oe = async (e, r) =>
    import("/$bunfs/root/chunk-d9sp039q.js").then(({ fromTokenFile: t }) =>
      t({
