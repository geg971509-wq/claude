...
import { TCn } from "/$bunfs/root/chunk-830v7vcp.js";
...
async function I(e, o) {
  ...
  if (e.statusCode >= 400 && e.statusCode < 500) {
    ...
    throw Object.assign(
      new i.CredentialsProviderError(`Server responded with status: ${e.statusCode}`, { logger: o }),
      { Code: t.Code, Message: t.Message },
    );
  }
  throw new i.CredentialsProviderError(`Server responded with status: ${e.statusCode}`, { logger: o });
}
var f = (e, o, n) => async () => {
  for (let r = 0; r < o; ++r)
    try {
...
var L = "AWS_CONTAINER_CREDENTIALS_RELATIVE_URI",
  ...
  _Cn = (e = {}) => {
    ...
    let o,
      ...
      a = e.logger?.constructor?.name === "NoOpLogger" || !e.logger?.warn ? console.warn : e.logger.warn.bind(e.logger);
    if (n && r)
      a(
...
Set AWS_CONTAINER_CREDENTIALS_FULL_URI or AWS_CONTAINER_CREDENTIALS_RELATIVE_URI.`,
    ...
    let T = new URL(o);
    h(T, e.logger);
    let u = N.NodeHttpHandler.create({ requestTimeout: e.timeout ?? 1000, connectionTimeout: e.timeout ?? 1000 });
    return f(
      async () => {
        let c = _(T);
