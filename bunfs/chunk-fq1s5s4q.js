...
import { rue } from "/$bunfs/root/chunk-wjr8v4tx.js";
...
var H = async (s, e) => {
  ...
  if (e.getAppState().endedByModel)
    throw new R(
      wk("Claude ended this conversation. Start a new session (or /clear) to continue."),
      "Claude ended this conversation",
    );
  if (((o = il(o)), o.length === 0)) throw Error("No messages to compact");
  let m = s.trim();
  try {
    return await E(o, e, m);
  } catch (r) {
    if (n.signal.aborted) throw new Ze("Compaction canceled.");
    else if (KC(r, hqe)) return { type: "text", value: hqe };
    else if (r instanceof bD) return { type: "text", value: r.message, level: "error" };
    else throw (h(r), Error(`Error during compaction: ${r instanceof Error ? r.message : String(r)}`, { cause: r }));
  }
};
async function E(s, e, n) {
  e.onCompactEvent?.({ type: "compact_progress", event: { type: "hooks_start", hookType: "pre_compact" } }),
  ...
  try {
    let [a, g] = await Promise.all([
      nz(e.session, { trigger: "manual", customInstructions: n || null }, e, e.abortController.signal),
      Prt({ toolUseContext: e, forkContextMessages: s, mainThreadAgentDefinition: void 0 }),
    ]);
    N$t(a, (d) => e.onQueryEvent?.({ type: "notification", notification: d }));
    let C = F$t(n, a.newCustomInstructions);
    e.onCompactEvent?.({ type: "stream_mode", mode: "requesting" }),
      e.onQueryEvent?.({ type: "response_length", op: "reset" }),
      e.onCompactEvent?.({ type: "compact_progress", event: { type: "compact_start" } });
    let c = await _(e.session.precompute, n, a.newCustomInstructions, s, e.abortController.signal, e.storageV5);
    i = c.reuse;
    let p = await (c.hit
      ? G1t({ ...c.finalize, startTime: o, cacheSafeParams: g, mainChainTailAtStart: m })
      : han(s, g, {
    ...
    if (!p.ok)
      switch (p.reason) {
        ...
        case "aborted":
          throw Error(ek);
        case "exhausted":
          throw new bD("Compaction failed \xB7 conversation could not be reduced below the context limit");
        case "media_unstrippable":
          throw new bD("Compaction failed \xB7 attached media exceeds size limits");
        case "error":
          throw new bD(`Error during compaction: ${p.detail || "unknown error"}`);
      }
    let f = p.result.boundaryMarker;
    if (f.subtype === "compact_boundary" && "compactMetadata" in f) t = f.compactMetadata.postTokens;
    YG(e.session, void 0, e.setAppState, void 0, void 0, void 0, e.storageV5), a4e(), ok(e.session, "compaction");
    let y =
      [a.userDisplayMessage, p.result.userDisplayMessage].filter(Boolean).join(`
`) || void 0;
    return { type: "compact", compactionResult: { ...p.result, userDisplayMessage: y }, displayText: w(e, y) };
  } catch (a) {
    throw ((r = a instanceof Error ? a.message : "reactive compaction failed"), a);
  } finally {
    e.onCompactEvent?.({ type: "stream_mode", mode: "requesting" }),
      e.onQueryEvent?.({ type: "response_length", op: "reset" }),
      e.onCompactEvent?.({ type: "compact_progress", event: { type: "compact_end" } }),
      rue({
        trigger: "manual",
        success: !r,
      ...
      e.onCompactEvent?.({
        type: "sdk_status",
        status: null,
        metadata: { compactResult: r ? "failed" : "success", ...(r && { compactError: r }) },
      });
  }
}
...
function w(s, e) {
  let n = Dq("tip"),
    o = Nh("app:toggleTranscript", "Global", "ctrl+o"),
    m = [...(s.options.verbose ? [] : [`(${o} to see full summary)`]), ...(e ? [e] : []), ...(n ? [n] : [])];
  return ae.dim(
    "Compacted " +
      m.join(`
`),
