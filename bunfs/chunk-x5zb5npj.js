...
import { hq } from "/$bunfs/root/chunk-sbv9eg4d.js";
...
import "/$bunfs/root/chunk-jmthk04s.js";
...
import { G2e, Xke, z2e } from "/$bunfs/root/chunk-0ksxhew7.js";
import "/$bunfs/root/chunk-295pyt61.js";
import "/$bunfs/root/chunk-66by9x77.js";
import "/$bunfs/root/chunk-v0p4561j.js";
import "/$bunfs/root/chunk-dytkb9e7.js";
...
import "/$bunfs/root/chunk-bcrwn930.js";
...
var J = async (t, o, i) => {
  let s = i?.trim() || "";
  if (!s) return e(hq, { onClose: t, context: o, defaultTab: "Config" });
  let n = s.toLowerCase();
  if (A0.includes(n) || _j.includes(n)) {
    t(
...
${z2e(o)}`,
  ...
  let r = G2e(s);
  if (!r) {
    t(`Expected key=value, got "${Qn(s)}". Run /config to open settings.`, { display: "system" });
    return;
  }
  return e(d, { pairs: r, context: o, onDone: t });
};
function d(N) {
  let X = _(6),
      ...
      (f.current = true),
        Xke(m, a, { setTheme: c }).then((w) =>
          l(
            w
              .map((g) =>
                mo(a.session) && g.kind === "save-failure"
                  ? "Couldn't save this setting (detail withheld on this connection)."
                  : g.message,
              )
              .join(`
`),
