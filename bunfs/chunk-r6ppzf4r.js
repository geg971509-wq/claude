...
import { wm, Ad, hc } from "/$bunfs/root/chunk-492vgtnr.js";
import { cre } from "/$bunfs/root/chunk-0vwrv5z3.js";
import { Ka, Cg } from "/$bunfs/root/chunk-fxb0gsq0.js";
var k = /cloudflare/i;
function w8e(e) {
  let r = e("request-id");
  if (typeof r === "string" && cre(r.trim())) return "origin";
  let i = e("cf-ray"),
    c = e("server");
  if ((typeof i === "string" && i !== "") || (typeof c === "string" && k.test(c))) return "nonorigin_cf";
...
function Kyr(e) {
  ...
  let r = e.error,
    ...
    u = "reason" in r ? r.reason : void 0;
  return (
    (i === "not_found_error" && c === "session_grouping") ||
    u === "public_grouping_hosted_only" ||
    u === "feature_disabled"
  );
}
function w(e) {
...
async function N9t(e, r, i, c, u, f, m, p, o, t, g) {
  let d = `${e}/v1/code/sessions`,
    s = { cwd: m ?? ee(), ...(p && { model: p }) };
  if (f) {
    let { buildGitSessionContext: h } = await import("/$bunfs/root/chunk-6ah045r1.js"),
      { sources: C, outcomes: R, report: y } = await h(f.gitRepoUrl, f.branch, f.defaultBranch);
  ...
  try {
    a = await st.post(
      d,
      { title: i, bridge: {}, ...(u?.length && { tags: u }), ...(o && { session_grouping_id: o }), config: s },
      { headers: gMe(r), timeout: c, validateStatus: (h) => h < 500 },
    );
  } catch (h) {
    return n(`[code-session] Session create request failed: ${l(h)}`), null;
  }
  ...
  if (
    ...
    !("session" in _) ||
    !_.session ||
    typeof _.session !== "object" ||
    !("id" in _.session) ||
    typeof _.session.id !== "string" ||
    !_.session.id.startsWith("cse_")
  )
    return (
...
async function F9t(e, r, i, c, u, f) {
  ...
  let o;
  try {
    o = await st.post(m, {}, { headers: p, timeout: c, validateStatus: (s) => s < 500 });
  } catch (s) {
    return n(`[code-session] /bridge request failed: ${l(s)}`), null;
  }
  if (o.status !== 200) {
    let s = $m(o.data);
    if ((n(`[code-session] /bridge failed ${o.status}${s ? `: ${s}` : ""}`), o.status === 401)) f?.();
    let a;
    if (o.status === 403) {
      let _ = S6(o.data, s);
...
function S(e, r, i, c, u, f = "") {
  let m = u.trustedDeviceToken ? { "X-Trusted-Device-Token": u.trustedDeviceToken } : void 0;
  if (u.useV2) {
    let o = hc(c);
    return { url: `${r}/v1/code/sessions/${o}${f}`, headers: { ...gMe(i), ...m }, id: o };
  }
  if (!u.orgUUID) return n(`[code-session] ${e} ${c}: v1 compat path needs an org UUID`), null;
  let p = Ad(c);
  return {
    url: `${r}/v1/sessions/${p}${f}`,
    headers: { ...gMe(i), "anthropic-beta": "ccr-byoc-2025-07-29", "x-organization-uuid": u.orgUUID, ...m },
    id: p,
  };
}
async function aRr(e, r, i, c, u) {
  if (!v(i, "Get")) return "invalid";
  let f = S("Get", e, r, i, u);
  if (!f) return "invalid";
  let { url: m, headers: p, id: o } = f;
  try {
    let t = await st.get(m, { headers: p, timeout: c, validateStatus: (s) => s < 500 });
    if (t.status !== 200) {
      let s = $m(t.data);
      return n(`[code-session] Get ${o} failed ${t.status}${s ? `: ${s}` : ""}`), t.status;
    }
    if (!u.useV2) {
      let s = t.data;
      return { ...s, status: s?.status ?? s?.session_status, id: typeof s?.id === "string" ? s.id : o };
    }
    let g = t.data,
      d = g?.response_shape ?? g?.session;
    if (d && typeof d.id === "string") return d;
    return n(`[code-session] Get ${o}: no session.id in 200 response`), t.status;
  } catch (t) {
    return (
      n(`[code-session] Get ${o} failed: ${l(t)}`),
      st.isAxiosError(t) && t.code === "ECONNABORTED" ? "timeout" : "error"
    );
  }
}
async function lRr(e, r, i, c, u, f) {
  ...
  try {
    ...
    return (
      ...
      st.isAxiosError(d) && d.code === "ECONNABORTED" ? "timeout" : "error"
    );
  }
}
async function Kyn(e, r, i, c, u) {
  return x("Archive", "/archive", e, r, i, c, u, "untrusted_device");
}
async function $9t(e, r, i, c, u) {
  return x("Unarchive", "/unarchive", e, r, i, c, u, "elevated_auth");
}
async function x(e, r, i, c, u, f, m, p) {
  if (!v(u, e)) return "invalid";
  let o = S(e, i, c, u, m, r);
  if (!o) return "invalid";
  ...
  try {
    ...
    if ((n(`[code-session] ${e} ${d} status=${s.status}`), s.status === 403)) {
      ...
      if (p === "elevated_auth" && a === "session_stale_relogin") return a;
    }
    return s.status;
  } catch (s) {
    return (
      n(`[code-session] ${e} ${d} failed: ${l(s)}`),
      st.isAxiosError(s) && s.code === "ECONNABORTED" ? "timeout" : "error"
    );
  }
}
function A(e) {
  ...
  let r = e.mcp_config;
  if (r !== null && typeof r === "object" && "content" in r && typeof r.content === "string" && r.content.length > 0)
    return { content: r.content };
  return;
}
export { w8e, lre, L9t, gMe, M9t, Kyr, N9t, v3, tO, S6, F9t, aRr, lRr, Kyn, $9t };
