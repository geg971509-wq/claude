...
import { bt, Yt, qa, Cs } from "/$bunfs/root/chunk-8tgj5dp2.js";
import { pr } from "/$bunfs/root/chunk-82w4mtvq.js";
import { h6n, _6n, NZ, oG, y6n, S6n } from "/$bunfs/root/chunk-7vpf91fq.js";
import { Rhe, Den } from "/$bunfs/root/chunk-k70ej0cs.js";
import { A6n, C6n, v6n } from "/$bunfs/root/chunk-c59q3nnv.js";
...
async function c(e) {
  ...
  let s = t?.refreshToken && t.scopes?.includes("user:design:read") ? t.accessToken : null;
  if (!s) {
    let r = await Den(e);
...
async function _(e) {
  try {
    let t = await bt.get("/v1/design/consent", { ...(await c(e)), validateStatus: (r) => r < 500 });
    ...
    return n(`Design consent pre-flight GET failed (${l(t)}); falling back to 403-seeded cache.`), {};
  }
}
function _W(e, t, s) {
...
async function hje(e, t) {
  if (!pr() || Ct()) return null;
...
async function lZ(e, t, s) {
  try {
    let r = await bt.post("/v1/design/consent", { consent: t }, { ...(await c(s)), validateStatus: (o) => o < 300 });
    if (!r.ok)
      throw new R(
...
async function k(e) {
  try {
    let t = await bt.get("/v1/design/grants", { ...(await c(e)), validateStatus: (o) => o < 500 });
    if (!t.ok || t.status !== 200) {
      let o = t.ok ? t.status : 0;
      return g("design_project_grant", o === 404 ? "probe_404_old_server" : "probe_http_error", { status: o }), null;
    }
    let s = t.data?.grants;
    ...
    return (
      n(`Design project-grant probe failed (${l(t)}); falling back to the per-batch plan flow.`),
      g("design_project_grant", "probe_network"),
      null
    );
  }
}
async function Jot(e, t, s) {
...
async function bUn(e, t, s) {
  ...
  try {
    ...
    throw (p("design_project_grant", "post_failed"), o);
  }
  if (r === 404)
    throw (
      (S6n(e, t),
      p("design_project_grant", "mint_refused_404"),
      new R(
...
function wUn(e, t, s) {
  ...
  async function o() {
    let a;
    try {
      let i = await bt.get("/v1/design/grants", { ...(await c(s)), validateStatus: (d) => d < 500 });
      if (!i.ok || i.status !== 200) return null;
      a = i.data?.grants;
    } catch (i) {
      return n(`Server-approval watcher poll failed (${l(i)}); will poll again.`), null;
    }
...
A6n(h);
C6n(async (e) => !!(await Rhe(e)));
v6n({
  wouldNeedDesignConsent: hje,
  consentPromptFor: aZ,
  postDesignConsent: lZ,
  seedDesignConsentBit: _W,
  isConsentBit: gje,
});
export { gje, aZ, _Un, _W, yUn, U0e, hje, Pen, lZ, SUn, Jot, bUn, wUn };
