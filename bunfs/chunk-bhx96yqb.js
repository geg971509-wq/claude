...
import { Rfe, KAt } from "/$bunfs/root/chunk-nqmqabr8.js";
import { be } from "/$bunfs/root/chunk-gcks6mn0.js";
import { b, n } from "/$bunfs/root/chunk-fv016jr6.js";
import { iA, Ga } from "/$bunfs/root/chunk-tzhtxm67.js";
...
async function TBe(e, r = {}) {
  try {
    process.kill(e, "SIGTERM");
  } catch (o) {
    if (E(o) === "EPERM") return "eperm";
    return "exited";
  }
...
async function d0t(e, r) {
  if (r) {
    let i = await r.write(Qie(), b(e, null, 2), { mode: 438 & ~process.umask() });
    if (!i.ok) {
...
async function oMn(e) {
  if (e) {
    let r = await e.delete(Qie());
    if (!r.ok)
      throw new R(
...
function iMn() {
  return `Stop it with \`claude daemon stop --any\` (a graceful, socket-based stop); if nothing is running at that pid, delete ${Nw()}`;
}
async function Ch(e = 1, r) {
  let t = await PP(r);
  if (!t) return null;
  try {
  ...
  if (!(await ont(t.pid))) return null;
  if (!(await EBe(t.pid, iA(t), e))) return null;
  return t;
}
async function ABe(e) {
  ...
  switch (r.origin) {
    ...
    case "transient":
    case void 0: {
      if (!zRe(r)) return { kind: "not-stopped", pid: r.pid, outcome: "unverified" };
      let t = await TBe(r.pid);
      return t === "exited" ? { kind: "stopped", pid: r.pid } : { kind: "not-stopped", pid: r.pid, outcome: t };
    }
    default:
...
async function sMn(e, r) {
  let t = await Ch(1, r).catch(() => null);
  return !!t && t.version !== e;
}
export { TBe, Nw, Qie, nMn, rMn, PP, d0t, oMn, ont, uge, p0t, EBe, zRe, iMn, Ch, ABe, CQ, int, sMn };
