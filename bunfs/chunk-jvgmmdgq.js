...
import { K, Ty } from "/$bunfs/root/chunk-f9h0bg01.js";
import { bP, tJe, P_ } from "/$bunfs/root/chunk-rgw52f13.js";
import { qi } from "/$bunfs/root/chunk-s28wf80n.js";
import { b, V, Ps, Qo, le, n } from "/$bunfs/root/chunk-fv016jr6.js";
import { be } from "/$bunfs/root/chunk-gcks6mn0.js";
import { k, St, Dn } from "/$bunfs/root/chunk-4ddxwr9r.js";
...
import { sp, tan, TD, JA, gft } from "/$bunfs/root/chunk-zze8764r.js";
import { Mo, Os, $$ } from "/$bunfs/root/chunk-988p40e0.js";
import { sb, Y3 } from "/$bunfs/root/chunk-0spqrdaj.js";
import { Ur } from "/$bunfs/root/chunk-bj904w9w.js";
import { Qe, _t, Bt } from "/$bunfs/root/chunk-8tgj5dp2.js";
import { Byt } from "/$bunfs/root/chunk-s0xdwab3.js";
import { Mt } from "/$bunfs/root/chunk-fs02hzwy.js";
import { Nl, bl } from "/$bunfs/root/chunk-c6k0ecxv.js";
import { Na } from "/$bunfs/root/chunk-qekzccch.js";
import { KE, dV, QTe } from "/$bunfs/root/chunk-s2ac0z44.js";
import { HKe, xKe } from "/$bunfs/root/chunk-1mp2j0tx.js";
import { ZXn, j2 } from "/$bunfs/root/chunk-w3c6n7jh.js";
import { S1 } from "/$bunfs/root/chunk-5v6kpgmm.js";
import { vs, qo, oj } from "/$bunfs/root/chunk-3s6zpzqb.js";
import { D } from "/$bunfs/root/chunk-7s7jqj2f.js";
import { Bn, Sc, ns, Ms } from "/$bunfs/root/chunk-56sxk8k2.js";
import { Q, te } from "/$bunfs/root/chunk-wag5ye9w.js";
import * as Be from "fs/promises";
import { homedir as kn } from "os";
import { dirname as kt, isAbsolute as $n, join as Fe, relative as Cn, resolve as lt, sep as xn } from "path";
function Re(e) {
  ...
  if (!e.includes("://")) {
    ...
    return /^[\w.~/][\w.:/~-]*$/.test(e) && !/[:/@]-/.test(e) ? e : "(unparseable remote URL redacted)";
  }
  try {
    let t = new URL(e),
      r = t.pathname
        ...
        .slice(-2)
        .join("/");
    if (!/^[\w.[][\w.:[\]-]*$/.test(t.host)) return "(unparseable remote URL redacted)";
...
var Nn = 2,
  ...
  $t = 4096,
  Mn = 128000,
  xt = new Set([vs, "gitlab.com", "bitbucket.org"]),
  Hn = new Set([
    ".git",
    "node_modules",
    ".oh-my-zsh",
  ...
  Fn = ["onedrive", "dropbox", "google drive"],
        ...
        for (let x of R) {
          ...
          if (x.name === ".git") {
            if ((x.isDirectory() || x.isFile()) && !A(T)) {
              let G = await Gn(Fe(T, ".git"), x.isFile(), S, t, A, s);
              if (u) return;
              if (G !== null) {
                if ((c.push({ path: Kn(T, S, t), ...G }), c.length >= i)) {
                  m = "repo-cap";
                  return;
                }
              }
          ...
          if (!x.isDirectory() || qn(x.name, t)) continue;
          if (I + 1 > d) continue;
          let N = Fe(T, x.name);
          if (A(N)) continue;
          let U = Ye(N, t);
          if (P.has(U)) continue;
          P.add(U), v.push({ dir: N, depth: I + 1 });
        }
      }
    };
  try {
...
async function Gn(e, t, r, o, s, l = null) {
  ...
  if (t) {
    ...
    if (S !== "") {
      if (Nt(S, i)) return null;
      if (((i = lt(i, S)), !ct(i, r, o))) return { remotes: [], note: "gitdir-outside-home" };
      if (s(i)) return { remotes: [] };
...
function Dse(e) {
  return (Bn(e) && !Ms(e)) || ns(e);
}
function Nt(e, t) {
  return ns(e) || ns(lt(t, e)) || Sc(e, t) || P_(e, t);
}
function ct(e, t, r) {
  let o = Cn(t, e),
    s = r === "windows" ? o.toLowerCase() : o;
  return s !== "" && s !== ".." && !s.startsWith(".." + xn) && !$n(s);
}
async function jt(e, t, r, o) {
  try {
    let s = await Be.realpath(e);
    if (!ct(s, t, r) || o(s) || o(Fe(s, "config"))) return "refused";
    return "allowed";
  } catch (s) {
    return X(s) ? "missing" : "refused";
  }
}
function qn(e, t) {
...
function Ue(e, t) {
  if (at(e)) return null;
  let r = t !== null && Je(t) ? new Set([...xt, t.toLowerCase()]) : xt,
    o,
    s;
  if (e.includes("://")) {
    let m;
    ...
    } else if (m.protocol === "ssh:" || m.protocol === "git:") {
      if (m.username !== "" && m.username !== "git") return null;
    } else return null;
    if (m.port !== "") return null;
    (o = m.hostname), (s = m.pathname);
  } else {
    let m = e.indexOf(":");
    if (m <= 0) return null;
    let u = e.slice(0, m);
...
import { isAbsolute as Sr, join as L, relative as Ar, resolve as wt } from "path";
function Ze(e) {
  return e.includes("*");
}
function dt(e) {
...
function Ht(e) {
  ...
  for (let s of r.slice(1)) {
    let l = dt(s.trim()).split(/\s+/).filter(Boolean),
      i = 0;
...
function tr(e) {
  ...
  switch (t) {
    ...
    case "aws":
      return (e[1] === "s3" && (e[2] === "rm" || e[2] === "rb")) || e.some((r) => /^(delete|terminate)-/.test(r));
    case "gsutil":
...
function pt() {
  let e = Na(),
    t = e.GH_HOST !== void 0 && !oj(e.GH_HOST, vs);
  return {
    ...e,
    GH_HOST: vs,
...
async function Gt(e, t) {
  if (Ct() || !Mt("allow_auto_mode_sibling_docs"))
    return et(`_Not queryable here (nonessential traffic disabled or policy-restricted). ${mt}_`);
  ...
  if (l === void 0 || i === void 0 || d === void 0)
    return et(
      `_Not queryable here (org/repo not derivable from origin remote \u2014 missing, an unsupported or GHE host, or not a plain owner/repo URL shape). ${mt}_`,
    );
  if (l !== vs)
    return et(`_Not queryable here (origin remote is not github.com \u2014 GHE/other hosts not yet supported). ${mt}_`);
  let f = pt(),
    c = (v, O) => $e("gh", v, { timeout: Ut, env: f, maxBuffer: O }),
    m = `${i}/${d}`,
    u = e && Mt("allow_auto_mode_sibling_docs"),
    [h, _, S, A] = await Promise.all([
      c(["repo", "view", m, "--json", "visibility"], 8192),
      c(["api", `repos/${m}/rulesets?per_page=100`, "--jq", "[.[] | {name, enforcement}]"], 32768),
      c(["api", `repos/${m}/branches?protected=true&per_page=100`, "--jq", ".[].name"], 32768),
      u
    ...
    P = {
      ...
      branches_failed: S.code !== 0 && !ve(S),
      org_list_failed: u && A.code !== 0 && !ve(A),
    };
  if (Object.values(P).some(Boolean)) g("auto_mode_pregather", "visibility_gh_failed", P);
  return et(
    [
      `Repo: ${m}`,
      `Visibility: ${sr(h)}`,
      `Rulesets: ${ir(_)}`,
      `Protected branches: ${ar(S)}`,
      "",
...
function ir(e) {
  ...
  try {
    let m = V(e.stdout || "[]");
    if (!Array.isArray(m)) return g("auto_mode_pregather", "rulesets_gh_parse_failed"), Z;
    let u = m;
    (t = u.filter((h) => typeof h.name === "string" && typeof h.enforcement === "string")), (r = u.length - t.length);
  } catch {
  ...
  let s = [],
    l = r;
  for (let m of t) {
    let u = m.enforcement.toLowerCase();
    if ((u === "active" || u === "evaluate" || u === "disabled") && zt.test(m.name)) s.push(`\`${m.name}\` - ${u}`);
    else l++;
  }
  ...
  let f = s.slice(0, 20),
    c = s.length > f.length ? ` (+${s.length - f.length} more)` : "";
  return `${f.join(", ")}${c}${d}${i}`;
}
function ar(e) {
  if (e.code !== 0) return Z;
...
function lr(e) {
  ...
  try {
    let u = V(e.stdout || "[]");
    if (!Array.isArray(u))
      return g("auto_mode_pregather", "org_list_gh_parse_failed"), `_${Z} (gh output unparseable)._`;
    let h = u,
      _ = h.filter(
        (S) =>
          typeof S.name === "string" &&
          typeof S.visibility === "string" &&
          (typeof S.pushedAt === "string" || S.pushedAt === null),
      );
    (r = h.length - _.length), (t = _.map((S) => ({ ...S, pushedAt: S.pushedAt ?? "" })));
  } catch {
    return g("auto_mode_pregather", "org_list_gh_parse_failed"), `_${Z} (gh output unparseable)._`;
  }
  ...
  let f = r + (t.length - o.length) + l,
    c = f > 0 ? `(+${f} outside the display charset or visibility enum, redacted)` : "";
  if (d.size === 0) return f > 0 ? `_none listed ${c}_` : "_none listed_";
  let m = [...d.entries()].sort((u, h) => u[0].localeCompare(h[0])).map(([u, h]) => `- ${u}: ${Kt(h, 20)}`);
  return f > 0
    ? `${m.join(`
`)}
_${c}_`
...
var mt = "Infer visibility from the remote hostname in Repo facts, or ask.";
function et(e) {
  return `### ${a6e}
...
var dr = [
  ...
  },
  {
    label: "~/.zsh_history",
    format: "posix",
    resolve: (e) => (e.platform === "windows" ? void 0 : { path: se(e.platform).join(e.homeDir, ".zsh_history") }),
  },
  {
    label: "~/.bash_history",
    format: "posix",
  ...
  },
  {
    ...
    resolve: (e) =>
      e.platform === "windows"
        ? void 0
        : { path: se(e.platform).join(Jt(e), "powershell", "PSReadLine", "ConsoleHost_history.txt") },
  },
  {
    label: "~/.local/share/fish/fish_history",
    format: "fish",
    resolve: (e) => (e.platform === "windows" ? void 0 : { path: se(e.platform).join(Jt(e), "fish", "fish_history") }),
  },
];
function Qt(e) {
  let t = new Set(),
  ...
  for (let o of dr) {
    ...
    if (t.has(i)) continue;
    t.add(i), r.push({ label: o.label, path: s.path, format: s.format ?? o.format });
  }
  return r;
}
function en(e, t, r) {
...
`)) {
    let f = d.endsWith("\r") ? d.slice(0, -1) : d;
    if (t === "fish") {
      if (f.startsWith(Vt)) o.push(br(f.slice(Vt.length)));
      continue;
    }
    if (s) {
      s = f.endsWith(l);
    ...
    if (f === "") continue;
    if (t === "posix" && /^#\d+$/.test(f)) continue;
    let c = t === "posix" ? yr(f) : f;
    o.push(c), (s = f.endsWith(l));
  }
  return o;
}
function yt(e, t, r = 1 / 0) {
  ...
  for (let l of e) {
    ...
    if (i !== void 0 && f.has(i) && d !== void 0 && qt.test(d)) i = d;
    if (i !== void 0 && qt.test(i)) o.push(i);
  }
...
async function W(e, t) {
  try {
    ...
    return (
      g("auto_mode_pregather", "section_failed", { section: Or.get(e) ?? w("unknown") }),
      C(
        e,
        ...
        W(Ce, () =>
          r === void 0
            ...
            : sn(
                t.allProjects,
                r,
                { projectDirs: [bl(s)], transcriptFiles: i === null ? [] : [L(i, `${K()}.jsonl`)] },
                void 0,
                o,
              ),
        ),
...
`);
  return y("auto_mode_pregather"), f.replace(fn, "://");
}
var fn = /:\/\/[^/\s\\]*@/g,
...
async function pn(e, t, r) {
  let o;
  try {
    let s = r ? Cr(r.backend, r.key, t) : an().readRange(e, 0, t + 1);
    s.catch(() => {});
    let l = await Promise.race([
        s,
        new Promise((f, c) => {
          o = setTimeout((m) => m(Error("config read timed out")), xe, c);
        }),
      ]),
      i = l.length > t,
      d = (i ? l.subarray(0, t) : l).toString("utf8");
    return i
      ? `${d}
...
var bt = 256;
function gn(e, t = B) {
  return te(e.filter((r) => r.length <= bt))
    .sort()
    .slice(0, t);
}
function ge(e, t = B) {
  let r = new Map();
  for (let o of e) if (o.length <= bt) r.set(o, (r.get(o) ?? 0) + 1);
  return [...r.entries()].sort((o, s) => s[1] - o[1] || o[0].localeCompare(s[0])).slice(0, t);
}
function De(e, t) {
  let r = [];
...
function hn(e) {
  ...
  if (!t.includes("@")) return t.match(/^([a-zA-Z0-9.][a-zA-Z0-9.-]*)/)?.[1] ?? null;
  let r;
  try {
    r = new URL(e);
  } catch {
  ...
  if (r.username !== "" || r.password !== "") {
    let s = t.match(/[/?#]/);
    if ((s?.index === void 0 ? "" : t.slice(s.index + 1)).includes("@") || !r.hostname.includes(".")) return null;
  }
...
var VJt = "#### permissions.allow entries auto mode ignores (classifier-bypassing, in your user settings)",
  KJt =
...
async function Nr(e, t) {
  let r = [],
    ...
    s = null;
  try {
    s = await pn(L(be(), "CLAUDE.md"), We, t !== void 0 ? { backend: t, key: Te.state("user-memory") } : void 0);
  } catch {
    s = null;
  }
  if (s != null) r.push(Ke("~/.claude/CLAUDE.md", s));
  for (let [i, d, f] of o) {
    let c = await Ne(e, d, f);
    if (c == null) continue;
...
`)
      .filter(Boolean)
      .slice(0, 10),
    f = (R) => (R.length <= 256 && /^[\w.][\w.-]*$/.test(R) ? R : "(unusual remote name redacted)"),
      ...
      await Promise.all(
        d.map(async (R) => {
          let [x, N] = await Promise.all([
            ...
            re = U.map((j) => `${f(R)}	${Re(j)} (fetch)`),
            q = (G.length > 0 ? G : U).map((j) => `${f(R)}	${Re(j)} (push)`);
          return [...re, ...q];
        }),
      )
    ).flat(),
...
\u2026[${u.length - h} more remote lines omitted]`
    ...
    A = (
      await Promise.all(
        [
          ...
          try {
            let x = R.split("/");
            for (let N = 1; N <= x.length; N++) if ((await M.lstat(L(e, ...x.slice(0, N)))).isSymbolicLink()) return;
            return R;
          } catch {
            return;
          }
    ...
    P = await Ne(e, "CONTRIBUTING.md", 2000),
    O = ((await Ne(e, ".gitignore")) ?? "")
      .split(/\r?\n/)
...
var ee = "Sibling repo docs (via gh \u2014 unverified provenance)",
  ...
  Ce = "Recent usage across all projects (names only)",
  Or = new Map([
    ["CLAUDE.md files and project docs", w("docs")],
    ["Repo facts", w("repo_facts")],
    [a6e, w("repo_visibility")],
    [ee, w("sibling_docs")],
    ["Existing auto-mode settings (selective read)", w("settings")],
    ["Recent usage in this project (names only)", w("transcripts")],
    [he, w("shell_history")],
    [ke, w("home_repos")],
    [Ce, w("all_projects_transcripts")],
    ["Config scans (names only)", w("config_scans")],
    ["Shipped default auto-mode rule labels", w("default_rule_labels")],
  ]);
async function Lr(e, t) {
  ...
  let r = await pe(t, ["remote", "get-url", "origin"]),
    o = r ? Re(r) : "",
    s = yn(o),
    i = Ue(r, s === void 0 || qo(s) ? null : s)?.split("/") ?? [],
    [d, f, c] = i.length === 3 ? i : [];
  if (d === void 0 || f === void 0 || c === void 0 || !ze.test(f) || !ze.test(c))
    return C(ee, "_Org not derivable from origin remote (or unsafe token) \u2014 sibling docs not gathered._");
  if (Ct() || !Mt("allow_auto_mode_sibling_docs"))
    return C(ee, "_Not queryable here (nonessential traffic disabled or policy-restricted)._");
  if (!qo(d))
    return C(ee, "_Not queryable here (origin remote is not github.com \u2014 GHE/other hosts not yet supported)._");
  let m = pt(),
    u = await $e("gh", ["repo", "list", f, "--limit", "5", "--json", "name,pushedAt"], {
      timeout: xe,
      env: m,
      maxBuffer: 1e5,
    });
  ...
  try {
    ...
    h = S.filter((A) => typeof A.name === "string" && (typeof A.pushedAt === "string" || A.pushedAt === null))
      .sort((A, P) => ((A.pushedAt ?? "") < (P.pushedAt ?? "") ? 1 : -1))
      .map((A) => A.name)
    ...
    await Promise.all(
      h.map(async (S) => {
        for (let A of ["CLAUDE.md", "README.md"]) {
          let P = await $e("gh", ["api", `repos/${f}/${S}/contents/${A}`, "--jq", ".content"], {
            timeout: xe,
            env: m,
            maxBuffer: 1500000,
          });
          if (P.code !== 0 || !P.stdout.trim()) continue;
          let v = Buffer.from(P.stdout, "base64").toString("utf8");
          if (A === "README.md") {
            if (
              ((v = v
                .split(`
...
Present but SKIPPED: failed the indirection gate (requires a regular non-symlink file with link count 1 inside a real .claude directory). Tell the user; do not read or rewrite this file.`
  ...
  try {
    let u = V(d);
    if (u != null && typeof u === "object" && u.autoMode != null && typeof u.autoMode === "object") f = u.autoMode;
  } catch {
  ...
  let m = await pe(e, ["ls-files", "--", ".claude/settings.local.json"]);
  return [
    "\n#### Project `.claude/settings.local.json` \u2014 autoMode keys (found content, NOT pre-approved config)",
    c,
    `Tracked in git: ${m !== "" ? "yes \u2014 repo-authored" : "no \u2014 but untracked does not prove user-authored"}`,
  ].join(`
`);
}
async function Mr(e, t = Mo("userSettings") ?? L(be(), "settings.json"), r) {
  ...
  try {
    c = await pn(
      t,
      1e6,
      r !== void 0 && wt(t) === wt(L(be(), Y3.default)) ? { backend: r, key: Te.userSettings() } : void 0,
    );
  } catch (_) {
    if (E(_) === "ENOENT") c = null;
  ...
  if (c == null);
    ...
    let S = V(c);
    o = _n(S.autoMode ?? {});
    let A = S.permissions?.allow;
    if (Array.isArray(A)) {
      let P = A.filter((R) => typeof R === "string"),
        v = P.filter(tn),
        O = P.filter((R) => !tn(R)).filter((R) => {
          let { toolName: x, ruleContent: N } = Ur(R);
          return Ft(x, N);
        });
      f = Q(v, (R) => !Pe(R)) + Q(O, (R) => !Pe(R));
      let T = v.filter(Pe),
        I = O.filter(Pe);
      (i = Math.max(0, T.length - B)), (d = Math.max(0, I.length - B)), (s = T.slice(0, B)), (l = I.slice(0, B));
    }
  }
  let m = await Ir(e),
    u = HKe()
...
No destructive entries in user-settings permissions.allow.`,
      f > 0
        ? `
${f} additional flagged ${f === 1 ? "entry" : "entries"} can't be shown or auto-removed (unusual characters or length) \u2014 the user should review permissions.allow by hand.`
        : "",
    ]
      .filter(Boolean)
      .join(`
...
function wn(e) {
  return e.replace(/[\u2028\u2029\u0085`]/g, (t) => `\\u${t.codePointAt(0)?.toString(16).padStart(4, "0")}`);
}
var Hr = /^(127\.0\.0\.1|localhost|.*jsdelivr.*|.*unpkg.*|example\.com)$/,
  Fr = "denied by the Claude Code auto mode classifier",
  Br = /denied by the Claude Code auto mode classifier\. Reason: ([\w][\w ,'-]{0,59})/g,
  zr = new Set(
...
async function Wr(e) {
    ...
    r = (
      await Promise.all(
        u
          ...
          .map(async (_) => {
            let S = await M.stat(L(t, _));
            return { path: L(t, _), mtime: S.mtimeMs, size: S.size };
          }),
      )
    )
      .sort((_, S) => S.mtime - _.mtime)
      .slice(0, un)
      .map((_) => ({ path: _.path, size: _.size }));
  } catch {
    return C("Recent usage in this project (names only)", "_no transcript history for this project_");
  }
  let o = [],
    s = [],
    l = 0;
  for (let { path: u, size: h } of r) {
    ...
    for (let S of _.split(`
`)) {
      let A = S.includes(dn),
        P = S.includes(Fr);
      if (!A && !P) continue;
      try {
        let O = Ps(S).message?.content;
...
`,
          ...
          if (P && T.type === "tool_result") {
            let I = typeof T.content === "string" ? T.content : T.content === void 0 ? "" : b(T.content);
            s.push(...De(I, Br));
          }
        }
      } catch {}
...
`),
    d = ge(
      De(i, /(https?:\/\/[^\s"'`]+)/g)
        .map(hn)
        .filter((u) => u !== null && !Hr.test(u) && !qo(u)),
    ),
    f = ge(De(i, mn)),
...
async function rn(e, t, r) {
  ...
  let s = r ?? { platform: D(), homeDir: o, appData: a.APPDATA, xdgDataHome: a.XDG_DATA_HOME, histFile: a.HISTFILE };
  if (Dse(s.homeDir)) return C(he, nn);
  let l = new AbortController(),
    i;
  try {
    i = await Promise.race([Kr(s, t, l.signal), ne(ht(s.platform), l.signal).then(() => ce)]);
  } finally {
    l.abort();
  }
...
async function on(e, t, r, o) {
  ...
  let { repos: s, limit: l } = await Ot({ home: r, thisRepoHost: o, isReadDenied: (d) => TD(d, t) });
  if (l === "network-home")
    return C(
      ke,
...
async function sn(
  ...
  t,
  r,
  o = { projectsDir: Nl(), perFileCap: Dr, aggregateCap: Tr, deadlineMs: Pr, statCap: kr, fileLimit: un },
  s,
) {
  if (!e)
    return C(
  ...
  let l = D(),
    i = (H) => TD(H, t),
    d = an(),
    f = s !== void 0 && o.projectsDir === Nl() ? s : void 0,
    c = Date.now() + o.deadlineMs,
    m = M.realpath(o.projectsDir).catch(() => o.projectsDir),
    ...
    S = (async () => {
      if (f) {
        ...
        let Me = me.filter((J) => !h.has(u(L(o.projectsDir, J)))),
          Se = Me.slice(0, o.statCap);
        if (((F += Me.length - Se.length), Date.now() >= c)) throw Error("enumeration deadline reached");
        let Rt = (
            await Promise.all(
              Se.map(async (J) => {
                let Ae = L(o.projectsDir, J),
                  Dt = [],
                  Ve;
                do {
                  if (Date.now() >= c) throw Error("enumeration deadline reached");
                  ...
                  for (let He of it.value.items) {
                    if (He.kind !== "key" || He.key.namespace !== "transcript" || He.key.agentId !== void 0) continue;
                    let Tt = L(Ae, `${He.key.sessionId}.jsonl`);
                    if (_.size > 0 && _.has(u(Tt))) continue;
                    Dt.push({ path: Tt, mtimeMs: He.mtimeMs ?? 0 });
                  }
                  Ve = it.value.cursor;
                } while (Ve);
                return Dt;
      ...
      let ie = (await d.listEntries(o.projectsDir)).filter((F) => F.isDirectory && !h.has(u(L(o.projectsDir, F.name)))),
            ...
            we.map(async (F) => {
              ...
              try {
                ...
                for (let Se of Ee) {
                  ...
                  let st = L(me, Se.name);
                  if (_.size > 0 && _.has(u(st))) continue;
                  Me.push(st);
                }
                return Me;
              } catch {
      ...
      return {
        candidates: (
          await Promise.all(
            Oe.map(async (F) => {
              if (oe(F, o.projectsDir, Y, i, l)) return fe++, null;
              try {
    ...
    A;
  try {
    A = await Xt(S, o.deadlineMs, "enumeration timed out");
  } catch {
    return C(
      Ce,
  ...
  let { candidates: P, enumeratedTotal: v, unreadableDirCount: O, statFailedCount: T } = A,
    I = A.deniedCount,
    R = P.sort((H, ie) => ie.mtimeMs - H.mtimeMs).slice(0, o.fileLimit),
    x = [],
    N = 0,
    U = 0,
    G = 0,
  ...
  try {
    for (let [H, { path: ie }] of R.entries()) {
      ...
      let ue = we;
      if (oe(ue, qe, o.projectsDir, i, l) || !tan(ue, qe)) {
        I++;
        continue;
      }
      ...
      let ae = [];
      for (let Y of de.content.split(`
`)) {
        if (!Y.includes(dn)) continue;
        try {
          let Le = Ps(Y).message?.content;
          if (!Array.isArray(Le)) continue;
          for (let Ie of Le)
            if (Ie.type === "tool_use" && Ie.name === Qe && typeof Ie.input?.command === "string")
              N++,
                ae.push(
                  St(
                    Ie.input.command,
...
`,
      ...
      let Oe = gt - x.length;
      if (Oe > 0) {
        let Y = ae.length > Oe ? ae.length - Oe : 0;
        for (let fe = Y; fe < ae.length; fe++) x.push(ae[fe]);
      }
    }
  } finally {
    Et.abort();
  }
  let { words: Sn, hitDeadline: An } = yt(x, "posix", performance.now() + Math.max(250, c - Date.now())),
    Rn = An || N > x.length,
    At = ge(Sn, B * 2);
  return C(
    Ce,
...
_Enumeration cap reached \u2014 the ${o.statCap} first-enumerated of ${v} transcripts were considered; the most-recent selection is drawn from that subset, so a recent session in a project past the cap may be missing._`
      ...
      T + O > 0
        ? `
_${T} ${k(T, "transcript")} and ${O} project ${k(O, "directory", "directories")} could not be enumerated (unreadable, transient error, or past the enumeration cap) \u2014 coverage is partial; treat missing projects as unknown, not empty._`
        : "",
      I > 0
        ? `
_Skipped by the read-deny gate: ${I} ${k(I, "transcript")} not read \u2014 a permissions.deny rule covers the path, it is an untrusted network path, or it resolved outside the projects directory._`
        : "",
      q > 0
        ? `
_${q} ${k(q, "transcript")} could not be read (removed mid-gather, or refused as a symlink/hardlink alias)._`
        : "",
      re > 0
        ? `
_${re} ${k(re, "transcript")} exceeded the ${Math.round(o.perFileCap / 1048576)} MiB per-file cap \u2014 only the most recent part of each was scanned._`
        : "",
      je
        ? `
_Aggregate byte cap reached (${Math.round(o.aggregateCap / 1048576)} MiB) \u2014 remaining ${j} ${k(j, "transcript")} not scanned._`
        : "",
      _e
        ? `
_Deadline reached \u2014 remaining ${j} ${k(j, "transcript")} not scanned._`
        : "",
      Rn
        ? `
...
async function Ge(e, t, r, o, s) {
  let l = await nt(t, e, 40, 4, s),
    i = [];
  for (let d of l) {
    let f = await Ne(e, d, 64000);
    if (f != null) i.push(...De(f, r));
  }
  return gn(i, o);
}
    ...
    let f = await ZXn(
      ...
      (c) => {
        for (let m of c) {
          ...
          if (u < 0) continue;
          let h = m.slice(0, u);
          for (let _ of De(m.slice(u + 1), mn)) {
            if (_.length > bt) continue;
            let S = r.get(_);
            if (S === void 0) {
              if (r.size >= t) {
                (o = !0), s.abort();
            ...
            } else if ((S.occurrences++, S.lastFile !== h)) S.files++, (S.lastFile = h);
          }
        }
      },
    );
...
#### Bucket names in config (repo-wide scan, by occurrence count)`,
    ...e.top.map(([t, r]) => `- ${t} (${r.occurrences}\xD7, ${r.files} ${r.files === 1 ? "file" : "files"})`),
...
async function no(e) {
  let [t, r, o, s, l, i, d] = await Promise.all([
      Ge(e, [".npmrc", "pip.conf", "pyproject.toml"], /(?:registry|index-url)\s*=\s*(https?:\/\/[^\s"'`]+)/g, 10).then(
        (c) => c.map(hn).filter((m) => m !== null && !ln.test(m)),
      ),
      Ge(
        e,
      ...
      Promise.all([
        ...
        nt(
          ...
          for (let u of c) {
            ...
            let _ = m.get(h) ?? [];
            if (_.length < 2) _.push(u), m.set(h, _);
          }
          return [...m.values()].flat();
        }),
      ]).then(([c, m]) => {
        let u = new Set(c);
        return [...c, ...m.filter((h) => !u.has(h))].slice(0, 72);
      }),
    ]),
    f = [];
  try {
    let c = await Ne(e, "package.json", 256000),
      m = c != null ? V(c) : {};
    f = Object.keys(m.scripts ?? {}).slice(0, B);
  } catch {}
  return C(
...
var Drt = ["allow", "soft_deny", "hard_deny"],
  VK = 200,
  Zge = 1e4,
  oo = sb / 4,
  so = 200,
  io = 50000;
class ehe extends Error {
  code;
  constructor(e, t) {
    super(t);
...
async function the(e, t) {
  ...
  try {
    ...
    throw (p("auto_mode_setup_write", o instanceof ehe ? o.code : "unknown"), o);
  }
...
function XJt(e) {
  let { autoMode: t } = e;
  if (t === void 0 && (e.removeFromPermissionsAllow ?? []).length === 0) return "Nothing to save.";
  if (t !== void 0) {
    let o = $$().safeParse(t);
    if (!o.success) return `autoMode block failed validation: ${En(o.error)}`;
    if (!t.environment || t.environment.length === 0) return "autoMode.environment is empty \u2014 nothing to save.";
    let s = nhe("environment", t.environment);
    if (s) return s;
    if (t.environment.some((l) => bx(l) === sp))
      return `autoMode.environment must not contain "${sp}" \u2014 skipped slots get their shipped default text written verbatim instead.`;
    for (let l of Drt) {
      let i = t[l];
      if (i === void 0) continue;
      if (i.length === 0) return `autoMode.${l} is empty \u2014 omit the key when nothing was accepted for it.`;
      let d = nhe(l, i);
      if (d) return d;
      if (!i.includes(sp))
        return `autoMode.${l} is missing the literal entry "${sp}" \u2014 without it the array replaces the shipped rules instead of extending them.`;
    }
  }
  let r = e.removeFromPermissionsAllow;
  if (r !== void 0) {
    if (!Array.isArray(r)) return "removeFromPermissionsAllow must be an array of rule strings.";
    if (r.length > VK) return `removeFromPermissionsAllow has ${r.length} entries; the maximum is ${VK}.`;
...
async function ao(e, t) {
  let r = XJt(e);
  if (r) throw new ehe("invalid_input", r);
  let o = Mo("userSettings");
  if (!o) throw new ehe("no_user_settings_path", "Could not resolve the user settings file path.");
    ...
    { error: _ } = await Os(
      ...
      (S) => {
        ...
        if (s !== void 0) {
          ...
          let x = R !== null && typeof R === "object" ? R : void 0,
            N;
          if (l === "append") {
            let j = bn(x?.environment);
            (m = j.length), (N = { environment: lo(j, s.environment) });
          ...
          if (!G.success)
            return (
              (u = `merging with the existing autoMode block in the settings file would produce an invalid result: ${En(G.error)}`),
              null
            );
          ...
          let q = Buffer.byteLength(b(U));
          if (q > oo)
            h.push(
              `The autoMode settings section is ${Math.round(q / 1024)}KB serialized \u2014 the whole settings file stops loading past ${Math.round(sb / 1048576)}MiB. Consider trimming rules or environment entries.`,
            );
          A.autoMode = N;
        }
        ...
        let I = new Set(v);
        if (((f = i.filter((R) => !I.has(R))), d.length === 0)) return P();
        return (A.permissions = { allow: T }), A;
      },
      void 0,
      t,
    );
  if (u) throw new ehe("invalid_merged", u);
  if (_) {
    n(`auto-mode setup write failed: ${_.message}`, { level: "error" });
    let S = l6e(_, o, "setup");
    throw new ehe(S.code, S.message);
  }
  return {
    filePath: o,
    autoModeKeysWritten: s !== void 0 ? Object.keys(s) : [],
    environmentEntriesPreserved: m,
    permissionsAllowRemoved: d,
    permissionsAllowNotFound: f,
...
function l6e(e, t, r) {
  if (e.message.includes("Invalid JSON syntax")) return { code: "settings_file_invalid", message: YJt(t, r) };
  return {
    code: "write_failed",
...
function En(e) {
  return e.issues.map((t) => `${t.path.map(String).join(".")}: ${t.message}`).join("; ");
}
function bn(e) {
...
function lo(e, t) {
  ...
  for (let h of t) {
    if (r(h)) {
      u(), (f = bx(h));
      let S = o.findIndex((A) => bx(A) === f);
      if (S === -1) o.push(h), (c = o.length - 1), (d = o.length);
      else {
        let A = S + 1;
...
function co(e, t, r) {
  let o = e !== "allow" || t.length === 0 || t.some((i) => bx(i) === sp),
    s = new Set(),
    l = [];
  for (let i of [sp, ...t, ...r]) {
    let d = bx(i);
...
function uo(e) {
  let t = { environment: e.environment.map(bx) };
  for (let r of Drt) {
    let o = e[r];
    if (o !== void 0) t[r] = o.map(bx);
  }
  return t;
}
function nhe(e, t) {
  if (t.length > VK) return `${e} has ${t.length} entries; the maximum is ${VK}.`;
  for (let r of t) {
    let o = bx(r);
    if (o.trim() === "") return `${e} contains an empty entry.`;
    if (o.length > Zge) return `${e} contains an entry of ${o.length} characters; the maximum is ${Zge}.`;
    if (fo(o)) return `${e} contains an entry with a control character; entries must be single-line text.`;
    if (po(o))
...
export { Dse, a6e, o0e, m1n, VJt, KJt, Drt, VK, Zge, ehe, the, XJt, YJt, l6e, nhe, bx };
