// @bun @bytecode
// Claude Code is a Beta product per Anthropic's Commercial Terms of Service.
// By using Claude Code, you agree that all code acceptance or rejection decisions you make,
// and the associated conversations in context, constitute Feedback under Anthropic's Commercial Terms,
// and may be used to improve Anthropic's products, including training models.
// You are responsible for reviewing any code suggestions before use.

// (c) Anthropic PBC. All rights reserved. Use is subject to the Legal Agreements outlined here: https://code.claude.com/docs/en/legal-and-compliance.

// Version: 2.1.252
import { S } from "/$bunfs/root/chunk-yz031c9r.js";
var c$ = S(function (Z, a) {
  function x(t) {
    return Array.isArray(t) ? t : [t];
  }
  var D = void 0,
    g = "",
    C = " ",
    d = "\\",
    O = /^\s+$/,
    k = /(?:[^\\]|^)\\$/,
    m = /^\\!/,
    X = /^\\#/,
    b = /\r?\n/g,
    v = /^\.{0,2}\/|^\.{1,2}$/,
    w = /\/$/,
    c = "/",
    I = "node-ignore";
  if (typeof Symbol < "u") I = Symbol.for("node-ignore");
  var P = I,
    _ = (t, e, s) => (Object.defineProperty(t, e, { value: s }), s),
    F = /([0-z])-([0-z])/g,
    N = () => false,
    H = (t) => t.replace(F, (e, s, r) => (s.charCodeAt(0) <= r.charCodeAt(0) ? e : g)),
    B = (t) => {
      let { length: e } = t;
      return t.slice(0, e - (e % 2));
    },
    W = [
      [/^\uFEFF/, () => g],
      [/((?:\\\\)*?)(\\?\s+)$/, (t, e, s) => e + (s.indexOf("\\") === 0 ? C : g)],
      [
        /(\\+?)\s/g,
        (t, e) => {
          let { length: s } = e;
          return e.slice(0, s - (s % 2)) + C;
        },
      ],
      [/[\\$.|*+(){^]/g, (t) => `\\${t}`],
      [/(?!\\)\?/g, () => "[^/]"],
      [/^\//, () => "^"],
      [/\//g, () => "\\/"],
      [/^\^*\\\*\\\*\\\//, () => "^(?:.*\\/)?"],
      [
        /^(?=[^^])/,
        function () {
          return !/\/(?!$)/.test(this) ? "(?:^|\\/)" : "^";
        },
      ],
      [/\\\/\\\*\\\*(?=\\\/|$)/g, (t, e, s) => (e + 6 < s.length ? "(?:\\/[^\\/]+)*" : "\\/.+")],
      [
        /(^|[^\\]+)(\\\*)+(?=.+)/g,
        (t, e, s) => {
          let r = s.replace(/\\\*/g, "[^\\/]*");
          return e + r;
        },
      ],
      [/\\\\\\(?=[$.|*+(){^])/g, () => d],
      [/\\\\/g, () => d],
      [
        /(\\)?\[([^\]/]*?)(\\*)($|\])/g,
        (t, e, s, r, n) =>
          e === d ? `\\[${s}${B(r)}${n}` : n === "]" ? (r.length % 2 === 0 ? `[${H(s)}${r}]` : "[]") : "[]",
      ],
      [/(?:[^*])$/, (t) => (/\/$/.test(t) ? `${t}$` : `${t}(?=$|\\/$)`)],
    ],
    K = /(^|\\\/)?\\\*$/,
    E = "regex",
    h = "checkRegex",
    L = "_",
    M = {
      [E](t, e) {
        return `${e ? `${e}[^/]+` : "[^/]*"}(?=$|\\/$)`;
      },
      [h](t, e) {
        return `${e ? `${e}[^/]*` : "[^/]*"}(?=$|\\/$)`;
      },
    },
    z = (t) => W.reduce((e, [s, r]) => e.replace(s, r.bind(t)), t),
    l = (t) => typeof t === "string",
    V = (t) => t && l(t) && !O.test(t) && !k.test(t) && t.indexOf("#") !== 0,
    j = (t) => t.split(b).filter(Boolean);
  class G {
    constructor(t, e, s, r, n, o) {
      (this.pattern = t),
        (this.mark = e),
        (this.negative = n),
        _(this, "body", s),
        _(this, "ignoreCase", r),
        _(this, "regexPrefix", o);
    }
    get regex() {
      let t = L + E;
      if (this[t]) return this[t];
      return this._make(E, t);
    }
    get checkRegex() {
      let t = L + h;
      if (this[t]) return this[t];
      return this._make(h, t);
    }
    _make(t, e) {
      let s = this.regexPrefix.replace(K, M[t]),
        r = this.ignoreCase ? new RegExp(s, "i") : new RegExp(s);
      return _(this, e, r);
    }
  }
  var Y = ({ pattern: t, mark: e }, s) => {
    let r = false,
      n = t;
    if (n.indexOf("!") === 0) (r = true), (n = n.substr(1));
    n = n.replace(m, "!").replace(X, "#");
    let o = z(n);
    return new G(t, e, n, s, r, o);
  };
  class T {
    constructor(t) {
      (this._ignoreCase = t), (this._rules = []);
    }
    _add(t) {
      if (t && t[P]) {
        (this._rules = this._rules.concat(t._rules._rules)), (this._added = true);
        return;
      }
      if (l(t)) t = { pattern: t };
      if (V(t.pattern)) {
        let e = Y(t, this._ignoreCase);
        (this._added = true), this._rules.push(e);
      }
    }
    add(t) {
      return (this._added = false), x(l(t) ? j(t) : t).forEach(this._add, this), this._added;
    }
    test(t, e, s) {
      let r = false,
        n = false,
        o;
      this._rules.forEach((f) => {
        let { negative: u } = f;
        if ((n === u && r !== n) || (u && !r && !n && !e)) return;
        if (!f[s].test(t)) return;
        (r = !u), (n = u), (o = u ? D : f);
      });
      let A = { ignored: r, unignored: n };
      if (o) A.rule = o;
      return A;
    }
  }
  var U = (t, e) => {
      throw new e(t);
    },
    i = (t, e, s) => {
      if (!l(t)) return s(`path must be a string, but got \`${e}\``, TypeError);
      if (!t) return s("path must not be empty", TypeError);
      if (i.isNotRelative(t)) return s(`path should be a \`path.relative()\`d string, but got "${e}"`, RangeError);
      return true;
    },
    y = (t) => v.test(t);
  i.isNotRelative = y;
  i.convert = (t) => t;
  class p {
    constructor({ ignorecase: t = true, ignoreCase: e = t, allowRelativePaths: s = false } = {}) {
      _(this, P, true), (this._rules = new T(e)), (this._strictPathCheck = !s), this._initCache();
    }
    _initCache() {
      (this._ignoreCache = Object.create(null)), (this._testCache = Object.create(null));
    }
    add(t) {
      if (this._rules.add(t)) this._initCache();
      return this;
    }
    addPattern(t) {
      return this.add(t);
    }
    _test(t, e, s, r) {
      let n = t && i.convert(t);
      return i(n, t, this._strictPathCheck ? U : N), this._t(n, e, s, r);
    }
    checkIgnore(t) {
      if (!w.test(t)) return this.test(t);
      let e = t.split(c).filter(Boolean);
      if ((e.pop(), e.length)) {
        let s = this._t(e.join(c) + c, this._testCache, true, e);
        if (s.ignored) return s;
      }
      return this._rules.test(t, false, h);
    }
    _t(t, e, s, r) {
      if (t in e) return e[t];
      if (!r) r = t.split(c).filter(Boolean);
      if ((r.pop(), !r.length)) return (e[t] = this._rules.test(t, s, E));
      let n = this._t(r.join(c) + c, e, s, r);
      return (e[t] = n.ignored ? n : this._rules.test(t, s, E));
    }
    ignores(t) {
      return this._test(t, this._ignoreCache, false).ignored;
    }
    createFilter() {
      return (t) => !this.ignores(t);
    }
    filter(t) {
      return x(t).filter(this.createFilter());
    }
    test(t) {
      return this._test(t, this._testCache, true);
    }
  }
  var R = (t) => new p(t),
    q = (t) => i(t && i.convert(t), t, N),
    J = () => {
      let t = (s) => (/^\\\\\?\\/.test(s) || /["<>|\u0000-\u001F]+/u.test(s) ? s : s.replace(/\\/g, "/"));
      i.convert = t;
      let e = /^[a-z]:\//i;
      i.isNotRelative = (s) => e.test(s) || y(s);
    };
  a.exports = R;
  R.default = R;
  a.exports.isPathValid = q;
  _(a.exports, Symbol.for("setupWindows"), J);
});
export { c$ };
