...
// Version: 2.1.252
import { et, e$e, wve, GQe, n$e } from "/$bunfs/root/chunk-f9h0bg01.js";
...
import { At, Xn } from "/$bunfs/root/chunk-ht28m404.js";
import { Vf } from "/$bunfs/root/chunk-9g7gyh01.js";
import { Fz } from "/$bunfs/root/chunk-avrc9gay.js";
import { Tte } from "/$bunfs/root/chunk-zk5v8tw6.js";
import { kVn, xVn, aX } from "/$bunfs/root/chunk-kdtcyk0z.js";
import "/$bunfs/root/chunk-98tgyb1d.js";
import { Fct, T7, xe, gk, dft } from "/$bunfs/root/chunk-zze8764r.js";
...
import { IKt } from "/$bunfs/root/chunk-6p5f2zpy.js";
import { Iu } from "/$bunfs/root/chunk-620fbwm6.js";
import { xje } from "/$bunfs/root/chunk-em2p93p3.js";
import { A, br, C, F } from "/$bunfs/root/chunk-twm95mhz.js";
...
function w(o) {
  let r = o.findLastIndex(M),
    s = o[r];
  if (s === void 0 || !M(s)) return { kind: "none" };
  for (let a = r - 1; a >= 0; a--) {
    let e = o[a];
    if (M(e)) break;
    if (D(e)) return { kind: "veto", fireIdx: r, reason: "blocking_system_before_anchor" };
  }
  let p,
    c = new Set();
  for (let a = r + 1; a < o.length; a++) {
    let e = o[a];
    if (D(e)) return { kind: "veto", fireIdx: r, reason: "blocking_system_in_span" };
    if (e.type === "assistant")
      ...
      if (e.toolDenialKind !== void 0)
        return {
          kind: "veto",
          fireIdx: r,
          reason: e.toolDenialKind === "interrupted" || e.toolDenialKind === "cancelled" ? "tool_abort" : "tool_denial",
        };
...
function v(o, r, s, p) {
  let a = `Claude resuming /loop wakeup (${T(new Date())})`,
    e = r ? w(o) : { kind: "none" };
  if (e.kind === "veto") g("loop_noop_fold", e.reason);
  if (e.kind !== "fold") return [...o, dft(a, { task: s, uuid: p, ...(r && { cronKind: "loop" }) })];
  let i = e.priorStreak + 1,
    d = T(new Date(e.since)),
    h = o.slice(e.fireIdx).map((S) => S.uuid);
  return (
    y("loop_noop_fold", { streak: i, span_len: h.length, tool_uses: e.toolUseCount, span_duration_s: x(o, e.fireIdx) }),
    [
      ...o,
      dft(`${a} \xB7 ${i} no-op ${k(i, "tick")} since ${d}`, {
        task: s,
        uuid: p,
        cronKind: "loop",
        noOpStreak: i,
...
var b = import.meta.require("/$bunfs/root/chunk-jemhm57r.js"),
  K = null;
          ...
          R = R.then(async () => {
            try {
              c.enqueuePendingNotification(
                m(await b.resolveLoopDefaultFireAsync(i.autonomousLoopPreamble, t, p), l, u),
              );
            } catch (f) {
              n(`[ScheduledTasks] fire enqueue (v5 arm) failed; fire skipped: ${f}`);
            }
          });
          return;
        }
        c.enqueuePendingNotification(m(b.resolveLoopDefaultFire(i.autonomousLoopPreamble, t), l, u));
      },
      L = (t) => {
        if (b.isLoopDefaultSentinel(t.prompt))
          return { ...t, prompt: b.isLoopFileSentinel(t.prompt) ? "/loop (loop.md)" : "/loop" };
        return t;
      },
      P = void 0,
      I = IKt({
        onFire: (t) => U(t, "schedule_wakeup"),
        onFireTask: (t) => {
          if (t.agentId) {
            let u = T7(t.agentId, d.getState().tasks);
            if (u && !Xs(u.status)) {
              xje(u.id, t.prompt, S, { kind: "task-notification" });
              return;
            }
            n(`[ScheduledTasks] teammate ${t.agentId} gone, removing orphaned cron ${t.id}`), Fz([t.id]);
            return;
          }
          let l = E();
          if (t.kind === "loop") {
            let u = !a();
            s.replace((f) => v(f, u, L(t), l)), wve(t.prompt), GQe();
          } else {
            let u = dft(`Running scheduled task (${T(new Date())})`, { task: L(t), uuid: l });
            s.replace((f) => [...f, u]);
          }
          U(t.prompt, Fct(t), { taskId: t.id, fireId: l });
        },
        isLoading: () => a(),
        assistantMode: r,
        getJitterConfig: Tte,
        isKilled: () => !cC(),
        getExtraTasks: K && P ? () => K.getRoutineCronTasks(i.project.projectRoot, P, p) : void 0,
      });
    return (
      I.start(),
      (e.current = I),
      () => {
        (e.current = null), I.stop();
      }
    );
  }, [r, s, d.getState, S, i]),
    A(() => {
