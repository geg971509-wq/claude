...
import { readdir as F, stat as x } from "fs/promises";
import { homedir as S, platform as A, userInfo as W } from "os";
import { join as g } from "path";
import { spawn as k } from "child_process";
import { lstat as T } from "fs/promises";
import { dirname as B, win32 as _ } from "path";
var O = "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths",
  b = 1e4,
  U = 5000;
function L() {
...
function I(e) {
  for (let i of e.split(/\r?\n/)) {
    let t = i.match(/^\s+.+?\s+REG_(?:EXPAND_)?SZ\s+(.+)$/i);
    if (t && t[1]) {
      let o = t[1].trim();
      if (o.length >= 2 && o.startsWith('"') && o.endsWith('"')) o = o.slice(1, -1);
      return o;
    }
  }
  return null;
}
function K(e, i = process.env) {
  let t = new Map();
  for (let [o, r] of Object.entries(i)) if (r !== void 0) t.set(o.toLowerCase(), r);
  return e.replace(/%([^%]+)%/g, (o, r) => t.get(r.toLowerCase()) ?? o);
}
async function P(e, i = U) {
  let t = h(),
    o = `${t}\\System32\\reg.exe`;
  for (let r of ["HKCU", "HKLM"]) {
    let u = await qe(o, ["query", `${r}\\${O}\\${e}`, "/ve"], { timeout: b, cwd: t });
    if (u.code !== 0) {
      if (u.exitCode === void 0)
        n(
          `[Claude in Chrome] ${r} App Paths query for ${e} did not run to completion: ${u.error ?? `killed at the ${b}ms bound, or reg.exe failed to spawn`}`,
        );
      continue;
    }
    let s = I(u.stdout);
    if (!s) {
      n(`[Claude in Chrome] ${r} App Paths value for ${e} had no parseable string default; skipping`);
      continue;
    }
    let m = K(s);
    if (!/^(?:[a-zA-Z]:[\\/]|\\\\)/.test(m)) {
      n(`[Claude in Chrome] Skipping ${r} App Paths candidate for ${e}: not a fully qualified path`);
      continue;
    }
    let v;
    try {
      ...
      let d = await Promise.race([
        l,
        new Promise((f) => {
          v = setTimeout((H) => H("timeout"), i, f);
        }),
      ]);
      if (d === "timeout") {
        n(`[Claude in Chrome] Skipping ${r} App Paths candidate for ${e}: existence check exceeded ${i}ms`);
        continue;
      }
      if (!d.isDirectory()) return n(`[Claude in Chrome] Resolved ${e} via ${r} App Paths`), m;
      n(`[Claude in Chrome] Skipping ${r} App Paths candidate for ${e}: resolves to a directory`);
    } catch (l) {
      let d = E(l),
        f = L();
      if (f !== void 0 && _.normalize(m).toLowerCase().startsWith(f) && d !== "ENOENT" && d !== "ENOTDIR")
        return n(`[Claude in Chrome] Resolved ${e} via ${r} App Paths (stat-odd: ${d ?? String(l)})`), m;
      n(`[Claude in Chrome] Skipping ${r} App Paths candidate for ${e}: ${d ?? String(l)}`);
    } finally {
      clearTimeout(v);
    }
  }
...
var uI = `mcp__${Ed}__`,
  ...
  rpn = ["chrome", "brave", "arc", "edge", "chromium", "vivaldi", "opera"];
function opn() {
  let e = D(),
    i = S(),
    t = [];
  for (let o of rpn) {
    ...
    switch (e) {
      case "macos":
        if (r.macos.nativeMessagingPath.length > 0) t.push({ browser: o, path: g(i, ...r.macos.nativeMessagingPath) });
        break;
      case "linux":
      case "wsl":
        if (r.linux.nativeMessagingPath.length > 0) t.push({ browser: o, path: g(i, ...r.linux.nativeMessagingPath) });
        break;
      case "windows":
        break;
    }
...
function ipn() {
  ...
  for (let i of rpn) {
    let t = d2t[i];
    if (t.windows.registryKey) e.push({ browser: i, key: t.windows.registryKey });
  }
  return e;
}
async function p2t() {
  ...
  for (let i of rpn) {
    ...
    switch (e) {
      case "macos": {
        let o = `/Applications/${t.macos.appName}.app`;
        try {
          if ((await x(o)).isDirectory()) return n(`[Claude in Chrome] Detected browser: ${t.name}`), i;
        } catch (r) {
          if (!Ht(r)) throw r;
        }
        break;
      ...
      case "wsl":
      case "linux": {
        for (let o of t.linux.binaries)
          if (await Va(o).catch(() => null)) return n(`[Claude in Chrome] Detected browser: ${t.name}`), i;
        break;
      }
      case "windows": {
        let o = S();
        if (t.windows.dataPath.length > 0) {
          let r = t.windows.useRoaming ? g(o, "AppData", "Roaming") : g(o, "AppData", "Local"),
            u = g(r, ...t.windows.dataPath);
          try {
            if ((await x(u)).isDirectory()) return n(`[Claude in Chrome] Detected browser: ${t.name}`), i;
          } catch (s) {
            if (!Ht(s)) throw s;
          }
        }
...
var V = 200;
function spn(e) {
  let i = pp().trackedTabIds;
  if (i.size >= V && !i.has(e)) i.clear();
  i.add(e);
}
function apn(e) {
  return pp().trackedTabIds.has(e);
}
function C(e) {
  return {
    exec_exit_code: e.code,
    exec_failure_kind:
      e.exitCode !== void 0 ? w("nonzero_exit") : e.code === 127 ? w("not_found") : w("spawn_failed_or_killed"),
  };
}
...
function hze() {
  return `/tmp/claude-mcp-browser-bridge-${N()}`;
}
function _ze() {
  if (A() === "win32") return `\\\\.\\pipe\\${R()}`;
  return g(hze(), `${process.pid}.sock`);
}
async function lpn() {
  if (A() === "win32") return [`\\\\.\\pipe\\${R()}`];
  let e = [],
  ...
  try {
    let t = await F(i);
    for (let o of t) if (o.endsWith(".sock")) e.push(g(i, o));
  } catch {}
  return e;
}
function R() {
  return `claude-mcp-browser-bridge-${N()}`;
}
function N() {
  try {
...
export { uI, Nte, d2t, rpn, opn, ipn, p2t, spn, apn, Fte, hze, _ze, lpn };
