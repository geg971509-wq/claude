...
import { yd, f5e, pjt, fjt, OOe, q2, WX, Oue, pt } from "/$bunfs/root/chunk-w3c6n7jh.js";
class d {
  inFlight = void 0;
  run(e) {
    return (
...
async function c() {
  try {
    let e = await pjt({ sandboxUser: fjt, srtWin: OOe() });
    if (e.cancelled) {
      if (e.user.provisioned && e.user.credPresent) {
        if (WX().source === "managed" && Oue()) {
          pt.invalidateDependencyCache();
          let s = await i(e.user);
          if ((await pt.checkDependenciesAsync(), s !== null)) return s;
          return (
        ...
        return (
          ...
          pt.invalidateDependencyCache(),
          await pt.checkDependenciesAsync(),
          {
            status: "cancelled",
            message:
    ...
    if (
      (pt.invalidateDependencyCache(),
      await pt.checkDependenciesAsync(),
      e.user.provisioned && e.user.credPresent && (e.wfp.state === "installed" || e.wfp.state === "cannot-read"))
    ) {
      if (WX().source === "managed" && Oue()) {
      ...
      if (pt.isSandboxingEnabled()) return { status: "ok", message: `${s} and active.${o()}` };
      return pt.isSandboxEnabledInSettings()
        ? { status: "ok", message: `${s}. Run /sandbox to check the current status.${o()}` }
        : { status: "ok", message: `${s}. Run /sandbox to choose a sandbox mode and turn it on.${o()}` };
    }
    ...
    if (e instanceof yd && e.code === "install_config_conflict")
      return (
    ...
    return (
      p("sandbox_windows_install", "install_threw"),
      {
        status: "error",
...
function o() {
  return Oue() && pt.needsRestartForTlsTerminate()
