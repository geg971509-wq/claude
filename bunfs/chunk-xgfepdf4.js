// @bun @bytecode
// Claude Code is a Beta product per Anthropic's Commercial Terms of Service.
// By using Claude Code, you agree that all code acceptance or rejection decisions you make,
// and the associated conversations in context, constitute Feedback under Anthropic's Commercial Terms,
// and may be used to improve Anthropic's products, including training models.
// You are responsible for reviewing any code suggestions before use.

// (c) Anthropic PBC. All rights reserved. Use is subject to the Legal Agreements outlined here: https://code.claude.com/docs/en/legal-and-compliance.

// Version: 2.1.252
import { Mn, G } from "/$bunfs/root/chunk-f9h0bg01.js";
import { cd } from "/$bunfs/root/chunk-9xb90qzc.js";
import { randomUUID as f } from "crypto";
class s {
  #e = new Map();
  #n = new Set();
  note(e, t, n) {
    if (this.#n.has(e)) return;
    let i = this.#e.get(e);
    if (i !== void 0) {
      if (i.has(t)) return;
      if (i.size >= 4) {
        this.#t(e);
        return;
      }
      i.set(t, n);
      return;
    }
    if (this.#e.size >= 256) {
      let a = 128;
      for (let r of this.#e.keys()) if ((this.#t(r), --a === 0)) break;
    }
    this.#e.set(e, new Map([[t, n]]));
  }
  #t(e) {
    this.#e.delete(e), this.#n.add(e);
  }
  take(e, t) {
    let n = this.#e.get(e);
    return this.#e.delete(e), n?.get(t);
  }
}
class u {
  #e;
  #n;
  #t;
  #r = !1;
  constructor(e, t, n) {
    (this.#e = e), (this.#n = t), (this.#t = n);
  }
  settle(e) {
    if (this.#r) return;
    if (((this.#r = !0), this.#e.get(this.#n) !== this.#t)) return;
    if ((this.#t.pending--, e.wasWatching || e.teardown !== "unsent")) this.#t.confirmed = !0;
    if (this.#t.pending === 0 && !this.#t.confirmed) this.#e.delete(this.#n);
  }
}
class l {
  #e = new Map();
  #n = 1;
  #t = new Set();
  #r = new s();
  isStopped(e) {
    return this.#e.has(e);
  }
  latchGeneration(e) {
    return this.#e.get(e)?.generation;
  }
  recordStop(e) {
    this.#t.delete(e);
    let t = this.#e.get(e);
    if (t === void 0) (t = { pending: 0, confirmed: !1, generation: this.#n++ }), this.#e.set(e, t);
    else t.generation = this.#n++;
    return t.pending++, new u(this.#e, e, t);
  }
  reaffirmStop(e) {
    this.#t.delete(e);
    let t = this.#e.get(e);
    if (t === void 0) this.confirmStop(e);
    else t.confirmed = !0;
  }
  confirmStop(e) {
    this.#t.delete(e);
    let t = this.#e.get(e);
    if (t === void 0) this.#e.set(e, { pending: 0, confirmed: !0, generation: this.#n++ });
    else (t.confirmed = !0), (t.generation = this.#n++);
  }
  noteRelatchAsk(e, t) {
    this.#r.note(e, t, this.latchGeneration(t));
  }
  takeRelatchAsk(e, t) {
    return this.#r.take(e, t);
  }
  clearByApprovedRewatch(e, t) {
    let n = this.#e.get(e);
    if (n === void 0 || n.generation !== t) return;
    this.#e.delete(e), this.#t.add(e);
  }
  noteApprovedWatch(e) {
    if (!this.#e.has(e)) this.#t.add(e);
  }
  wasClearedByRewatch(e) {
    return this.#t.has(e) && !this.#e.has(e);
  }
}
var S = {
    baseMs: 1000,
    capMs: 30000,
    minUptimeMs: 60000,
    maxConsecutiveFailures: 10,
    spreadMinMs: 1000,
    spreadMaxMs: 20000,
    idleTtlMs: 0,
    handshakeDeadlineMs: 150000,
    stallThresholdMs: 1e4,
    stallMinMs: 30000,
    stallMaxMs: 60000,
    longStallMinMs: 120000,
    longStallMaxMs: 240000,
    maxStallMs: 1800000,
    stallOutJitterMs: 900000,
  },
  h = 30000,
  b = 100,
  c = {
    baseMs: 1000,
    capMs: 30000,
    minUptimeMs: 60000,
    stallThresholdMs: 1e4,
    stallMinMs: 60000,
    stallMaxMs: 600000,
    maxConsecutiveRefusals: 10,
    handshakeDeadlineMs: 30000,
  },
  v = { ...c, presenceMs: 15000 },
  M = { ...c, keepaliveMs: 25000 };
function JM(e, t) {
  return `${e ?? "main"}
${t}`;
}
function p(e) {
  let t = [...e.codeliveredFollowups.values()];
  e.codeliveredFollowups.clear();
  for (let { revoke: n } of t) n("reset");
  e.coordinatorEditors.clear(), e.editorSettleWatch?.(), (e.editorSettleWatch = null);
}
function n8e(e, t, n) {
  let i = o().current;
  if (i === void 0) return;
  for (let [a, r] of i.codeliveredFollowups)
    if (r.slug === e && r.agentId !== n?.exceptAgentId) i.codeliveredFollowups.delete(a), r.revoke(t);
}
var w = 64,
  qh = 5,
  vde = 3 * qh;
function y() {
  return {
    live: {
      handoffGeneration: 0,
      inFlightSubscribes: new Set(),
      inFlightWiredIntent: new Set(),
      retiredInFlightArms: new Set(),
      armSettleWaiters: new Map(),
      bootingWiredArms: new Map(),
      pendingInFlightWiring: new Map(),
      supervisors: new Map(),
      commentCensus: new Map(),
      pendingRegistrations: 0,
      rewatchTiming: { ...S },
      mostRecentPublishSlug: void 0,
      activitySenders: new Map(),
      agentActivitySinks: new Map(),
      activityRefreshTimer: void 0,
      activityOnsetTimer: void 0,
      activityClearTimer: void 0,
      activityRefreshMs: h,
      activityEdgeDebounceMs: b,
      activityUnsubscribe: void 0,
      disposed: !1,
      activityOkReported: !1,
      summonDeclared: new Map(),
      summonArmOkReported: !1,
      armOutcomes: new Map(),
      mcpWriteSlugs: new Map(),
      announcedArmFailures: new Set(),
      nonEditorSlugs: new Set(),
      repliesConsent: { outstanding: new Map(), declined: new Map(), approved: new Set() },
      inFlightGenerations: new Map(),
      endAll: void 0,
    },
    liveDocWatch: {
      ownSeqs: new Map(),
      rebases: new Map(),
      pendingOwnEdits: new Map(),
      watches: new Map(),
      stopCounts: new Map(),
      retiredWatches: new Map(),
      arming: new Map(),
      notLiveDoc: new Set(),
      noticeBudgets: new Map(),
      journalUnavailable: new Set(),
      journalNegotiations: new Set(),
      stopAll: void 0,
      stop: void 0,
    },
    liveEditOps: { lastOwnAck: new Map(), unansweredKeys: new Map() },
    liveReplicas: {
      replicas: new Map(),
      unavailable: new Set(),
      opening: new Map(),
      transport: void 0,
      engine: void 0,
      renderLevel: void 0,
      headMoved: void 0,
      workingCopyPath: void 0,
      settleFileChange: void 0,
    },
    autoReact: {
      artifacts: new Map(),
      optIn: null,
      responderDispatchOptIn: null,
      userDisarmed: !1,
      enabledMemo: null,
      unattendedReplies: new Map(),
      postSeq: 0,
      probeSeq: 0,
      coalesceMsOverride: null,
      maxAutoTurnsOverride: null,
      confirmDwellMsOverride: null,
      fastAckFlagRefresh: null,
      fastAckFlagRefreshDeadlineMsOverride: null,
      fastAckSelectDeadlineMsOverride: null,
      pendingLedger: null,
      ledgerLastWritten: null,
      ledgerLastWriteAt: null,
      ledgerLastWriteSid: null,
      ledgerLastWriteAccount: void 0,
      ledgerFailureSeqAtWrite: null,
      ledgerDeferredSince: null,
      ledgerLastAppend: null,
      ledgerOwnerSid: null,
      ledgerRetiredSids: new Set(),
      ledgerExitCleanup: void 0,
      ledgerExitReStamp: void 0,
      ledgerTimer: void 0,
      ledgerStorageV5: void 0,
      ledgerMaxAgeMsOverride: null,
      ledgerDebounceMsOverride: null,
      composeDeadlineMsOverride: null,
    },
    summonSeeds: { claims: new Set(), readClaims: new Set(), graceMsOverride: null },
    wakes: {
      wakes: new Map(),
      stoppedSlugs: new Set(),
      sweptSlugs: new Set(),
      orphanedSweptSlugs: new Set(),
      yieldedSlugs: new Map(),
      takenFrom: new Map(),
      latchChangedAt: new Map(),
      liftedAtScanGeneration: new Map(),
      humanTurnAtScanGeneration: null,
      pendingResumeDisclosure: new Set(),
      scanGeneration: 0,
      slugEpochs: new Map(),
      stopGenerations: new Map(),
      resumeSights: new s(),
    },
    autoEditAttribution: { staged: new Map(), chainPublishIds: new Set() },
    summonAnswers: new Map(),
    pipelineReplyOrigins: new Map(),
    summonStatus: { sidBySlug: new Map(), seqBySid: new Map(), routeAbsentSince: null },
    noticeCoalesce: new Map(),
    shareStatus: {
      bySlug: new Map(),
      filePathToSlug: new Map(),
      pendingNoticeSlugs: new Set(),
      noticeReadSights: { comments: new s(), read_db: new s(), read: new s(), verify: new s() },
      shareProbesInFlight: new Map(),
      capsReadsInFlight: new Map(),
    },
    templateLanes: { prototypeArmed: !1, boundSlugs: new Map(), controlPlaneArmed: !1, controlPlaneSlugs: new Set() },
    postedReplyIds: new Set(),
    frameRelay: {
      declinedUntil: new Map(),
      tunnelDeclinedUntil: new Map(),
      hopFailedUntil: new Map(),
      servedUntil: new Map(),
      vouched: new Set(),
      botContextNoted: !1,
    },
    contentHostEgressDenied: new Set(),
    assetsOnRoster: !1,
    accountEpoch: 0,
    conversationEpoch: 0,
    authoringProgress: { slotsByBlockIndex: new Map(), failureReported: !1 },
    mintedStoredPageProbes: new WeakSet(),
    mintedRoundTripPublishSignals: new WeakSet(),
    durable: {
      rows: new Map(),
      registrySink: null,
      registryPublished: "null",
      unwatchedSlugs: new Set(),
      orphanTriggers: new Set(),
      pendingRestoredRows: new Map(),
      slugOps: new Map(),
      stopLatches: new l(),
      liveDocArmDeclined: new Set(),
      liveDocRegrantSights: new s(),
      firstWatchAskSights: new s(),
      pendingOps: new Set(),
      originatorRefused: !1,
      subscribeForbidden: null,
      watchUrlWithheld: null,
      watchUrlGranted: !1,
      armsInFlight: new Map(),
      armOutcomes: new Map(),
      announcedArmFailures: new Set(),
    },
    room: { conns: new Map(), nonces: new Map(), endedOwners: new Map(), timing: { ...v } },
    presence: { conns: new Map(), declined: new Set(), timing: { ...M } },
    commentMonitorIntent: {
      sid: null,
      bySlug: new Map(),
      pendingRestore: null,
      lastWritten: null,
      wroteCurrentLine: !1,
      failureSeqAtWrite: 0,
      storageV5: void 0,
      unsubscribeSwitch: void 0,
      unregisterExitDrain: void 0,
      transcriptPath: null,
      parked: new Map(),
      writeChain: Promise.resolve(),
      owedLines: new Map(),
      pendingLines: new Map(),
      exitStamped: !1,
      tornStops: new Set(),
      forgottenAt: new Map(),
      leftWith: null,
      unsubscribeMaterialized: void 0,
      earlySeed: void 0,
      onFile: !1,
      adoptPendingFor: null,
    },
    interactionSchemas: void 0,
    approvedRootBases: new Map(),
    commentTargets: new Map(),
    refusedPublishBodies: new Map(),
    approvedSourcePins: new Map(),
    gatedThumbnailHrefs: new Map(),
    approvalStashEvicted: !1,
    publishObservationNonce: f(),
    consumedPublishApprovals: new Set(),
    readDeliveries: new Map(),
    pendingHandoverReads: new Map(),
    frozenReadPageDataSchemaNames: void 0,
    frozenWatchRail: void 0,
    frozenArtifactTypes: void 0,
    createdFromType: new Map(),
    deferredSurface: new Map(),
    typeInstanceFiles: new Map(),
    typeCapabilityReads: new Map(),
    verify: { lastPublish: void 0, reads: new Map() },
    ownPublishedSlugs: new Map(),
    roomJoinArming: new Set(),
    roomStoppedByUser: new Set(),
    roomArmAsked: new Map(),
    roomArmRefusedByUser: new Set(),
    coordinatorEditors: new Map(),
    editorSettleWatch: null,
    codeliveredFollowups: new Map(),
    prReviewComposeLatch: null,
    liveEditGateLatch: null,
    livePathsGateLatch: null,
    artifactDisabledSessionEvaluated: !1,
    anchorSnippets: new Map(),
    marked: { plain: void 0, inertHtml: void 0, prReviewSubset: void 0 },
    bundleEmbedVerdicts: { hljs: void 0 },
    blockStripVerdicts: { hljs: void 0, mermaid: void 0 },
    prReviewTemplate: { chrome: null, parts: null },
    workshopBlessedHashes: void 0,
    rejectBreaker: new cd({ max: w }),
  };
}
function r8e(e) {
  if (((e.stopped = !0), e.abort.abort(), e.timer !== void 0)) clearTimeout(e.timer), (e.timer = void 0);
  delete e.lease, delete e.renewable, delete e.wake;
}
class g {
  current = void 0;
}
var R = new Mn(() => new g());
function o() {
  return R.of(G());
}
function de() {
  let e = o();
  return (e.current ??= y()), e.current;
}
function cY() {
  let e = o().current;
  if (e === void 0) return;
  for (let t of e.liveDocWatch.noticeBudgets.values()) t.disclosed = !1;
}
function gEe() {
  let e = o().current;
  if (e === void 0) return;
  e.refusedPublishBodies.clear(), tyn();
}
function tyn() {
  let e = o().current;
  if (e === void 0) return;
  for (let t of e.pendingHandoverReads.values())
    t.linesReturned.clear(), t.linesPrepared.clear(), t.completedBy.clear(), t.confirmsResendFor.clear();
  e.readDeliveries.clear();
}
function wZn(e) {
  let t = o().current;
  if (t === void 0) return;
  let n = t.refusedPublishBodies,
    i = JM(void 0, "");
  for (let [a, r] of n) {
    if (!a.startsWith(i)) continue;
    if (!e.has(r.batch)) n.delete(a);
    else if (r.observedFrom !== void 0 && r.observedFrom !== "" && !e.has(r.observedFrom)) delete r.observedFrom;
  }
}
function o8e() {
  return o().current?.room;
}
function TZn() {
  return o().current?.presence;
}
function Zyt(e) {
  let t = o().current?.presence;
  t?.disposeSlug?.(t, e);
}
function EZn() {
  return o().current !== void 0;
}
function AZn() {
  let e = o8e();
  e?.consentChanged?.(e);
}
function CZn(e) {
  let t = o8e();
  t?.accountChanged?.(t, e);
}
function tre() {
  let e = o8e();
  e?.dispose?.(e);
}
function A(e) {
  P(e);
  for (let t of e.docs.values()) C(t);
}
function P(e) {
  (e.journalPos = void 0), (e.journalPosOrd = void 0);
}
function C(e) {
  (e.journalFence = void 0), (e.journalGen += 1);
}
function T(e) {
  let t = e.journal;
  (e.journal = void 0), (e.journalCap = void 0), A(e);
  for (let n of e.docs.values()) {
    if (n.batchTimer !== void 0) clearTimeout(n.batchTimer), (n.batchTimer = void 0);
    (n.batch = void 0), (n.batchPos = void 0), (n.batchHeardAt = void 0), (n.settling = void 0);
  }
  t?.stop({ killTask: !1 });
}
function vZn(e = "signed_out", t) {
  let n = o().current;
  if (n === void 0) return;
  if (e !== "same_account") {
    (n.assetsOnRoster = !1),
      n.accountEpoch++,
      n.postedReplyIds.clear(),
      n.commentTargets.clear(),
      (n.verify.lastPublish = void 0),
      n.verify.reads.clear(),
      n.ownPublishedSlugs.clear(),
      n.roomJoinArming.clear(),
      p(n);
    for (let r of n.live.supervisors.values()) delete r.lease, delete r.renewable;
    n.live.nonEditorSlugs.clear(),
      n.live.repliesConsent.outstanding.clear(),
      n.live.repliesConsent.declined.clear(),
      n.live.repliesConsent.approved.clear();
    for (let r of n.autoReact.artifacts.values())
      for (let d of r.threads.values()) d.ownReplyIds.clear(), (d.ownReplyIdsIncomplete = !1);
    n.liveDocWatch.journalUnavailable.clear();
    for (let r of n.liveDocWatch.journalNegotiations) r.abort();
    n.liveDocWatch.journalNegotiations.clear();
    for (let r of n.liveDocWatch.arming.values()) for (let d of r.reviving) T(d);
    n.liveDocWatch.stopAll?.(e), n.live.endAll?.(e);
    for (let r of n.liveReplicas.replicas.values()) r.watcher?.close();
    n.liveReplicas.replicas.clear(),
      n.liveReplicas.unavailable.clear(),
      n.liveReplicas.opening.clear(),
      n.createdFromType.clear(),
      n.deferredSurface.clear(),
      n.typeInstanceFiles.clear(),
      n.typeCapabilityReads.clear(),
      n.anchorSnippets.clear(),
      (n.shareStatus.accountBoundaryAt = Date.now());
    for (let [r, d] of n.shareStatus.bySlug)
      if (d.probeErrorCode !== void 0) {
        let { probeErrorCode: D, ...m } = d;
        n.shareStatus.bySlug.set(r, m);
      }
  }
  let { room: i, presence: a } = n;
  i.accountChanged?.(i, t), a.accountChanged?.(a, t);
}
function hEe() {
  o().current?.liveDocWatch.stopAll?.("clear");
}
function _Ee(e) {
  let t = o().current;
  if (t === void 0) return;
  if (e?.continuesConversation !== !0)
    t.conversationEpoch++,
      t.ownPublishedSlugs.clear(),
      t.roomJoinArming.clear(),
      t.roomStoppedByUser.clear(),
      t.roomArmAsked.clear(),
      t.roomArmRefusedByUser.clear(),
      p(t),
      t.live.nonEditorSlugs.clear(),
      t.live.repliesConsent.outstanding.clear(),
      t.live.repliesConsent.declined.clear(),
      t.live.repliesConsent.approved.clear(),
      t.deferredSurface.clear();
  t.createdFromType.clear(), (t.verify.lastPublish = void 0), t.verify.reads.clear();
}
export { JM, n8e, qh, vde, r8e, de, cY, gEe, tyn, wZn, o8e, TZn, Zyt, EZn, AZn, CZn, tre, vZn, hEe, _Ee };
