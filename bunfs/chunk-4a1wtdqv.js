...
import { O8, UMt, K8, bv, wD, aT, ZA } from "/$bunfs/root/chunk-zze8764r.js";
...
var N = async (C, m) => {
  let t = ee(),
    d = UMt(m.getMcp().commands),
    s = (o) => O8(o, d),
    n = s(await ZA(t, m.storageV5)),
    i = new Set(n.map((o) => o.name));
  K8(), aT(), wD();
  let e = s(await ZA(t, m.storageV5)),
    c = new Set(e.map((o) => o.name));
  bv.emit();
  let l = Q(e, (o) => !i.has(o.name)),
    r = Q(n, (o) => !c.has(o.name)),
    a = [];
  ...
  return { type: "text", value: `Reloaded skills: ${e.length} ${k(e.length, "skill")} available (${p})${f}` };
};
export { N as call };
