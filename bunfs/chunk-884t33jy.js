...
`;
    if (g) o = p;
    l.push(a({ text: s, startOffset: u, isPrecededByNewline: h(e, u, f === 0), endsWithNewline: g }));
  }
  return Object.freeze({ text: e, columns: t, lines: Object.freeze(l) });
}
function w(e, n) {
...
function TMn(e, n) {
  ...
  let t = b(n, 0, e.text.length);
  for (let i = 0; i < e.lines.length; i++) {
    let o = e.lines[i],
      d = e.lines[i + 1];
    if (t >= o.startOffset && (!d || t < d.startOffset)) {
      let f = t - o.startOffset,
        s;
      if (o.isPrecededByNewline) s = m(o.text, f);
      else {
        let u = o.text.length - o.text.trimStart().length;
        if (f < u) s = 0;
        else s = m(o.text.slice(u), f - u);
      }
  ...
  let r = e.lines.length - 1,
    l = e.lines[r];
  return { line: r, column: se(l.text) };
}
function EMn(e, n, t) {
  if (e.lines.length === 0) return 0;
  let r = w(e, n);
  if (r.text.length === 0 && r.endsWithNewline) return r.startOffset;
  let l = r.isPrecededByNewline ? 0 : r.text.length - r.text.trimStart().length,
    i = O(r.text.slice(l), Math.max(0, t)) + l;
  return r.startOffset + i;
}
function m(e, n) {
  if (n <= 0) return 0;
  if (n >= e.length) return se(e);
  return se(e.slice(0, n));
}
function O(e, n) {
  if (n <= 0 || e.length === 0) return 0;
  let t = 0,
    r = 0;
  for (let l of e) {
    let i = se(l);
