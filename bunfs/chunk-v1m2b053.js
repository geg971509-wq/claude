...
var p = j(Jh(), 1),
  a = (r, e, o) => {
    ...
    if (e.Expiration) {
      let d = new Date();
      if (new Date(e.Expiration) < d) throw Error(`Profile ${r} credential_process returned expired credentials.`);
    }
    let s = e.AccountId;
...
var u = async (r, e, o) => {
  ...
  if (e[r]) {
    ...
    } else throw new c.CredentialsProviderError(`Profile ${r} did not contain credential_process.`, { logger: o });
  } else
    throw new c.CredentialsProviderError(`Profile ${r} could not be found in shared credentials file.`, { logger: o });
};
var CCn =
  (r = {}) =>
