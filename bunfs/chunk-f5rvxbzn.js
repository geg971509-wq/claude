...
import { j, Nr } from "/$bunfs/root/chunk-yz031c9r.js";
...
function U1e(e) {
  return new Promise((t, n) => {
    ...
    o.on("error", (r) => {
      n(Object.assign(new T.ProviderError("Unable to connect to instance metadata service"), r)), o.destroy();
    }),
      o.on("timeout", () => {
      ...
      o.on("response", (r) => {
        ...
        if (a < 200 || 300 <= a)
          n(
            Object.assign(new T.ProviderError("Error response received from instance metadata service"), {
              statusCode: a,
            }),
          ),
            o.destroy();
...
var hHr = 1000,
  _Hr = 0,
  Rzt = ({ maxRetries: e = 0, timeout: t = 1000 }) => ({ maxRetries: e, timeout: t });
var N = () => {};
var I = (e, t) => {
  let n = e();
...
var _,
  Ovn = "AWS_CONTAINER_CREDENTIALS_FULL_URI",
  Lvn = "AWS_CONTAINER_CREDENTIALS_RELATIVE_URI",
  Dsr = "AWS_CONTAINER_AUTHORIZATION_TOKEN",
  Mvn = (e = {}) => {
    let { timeout: t, maxRetries: n } = Rzt(e);
    return () =>
      I(async () => {
        let o = await se({ logger: e.logger }),
          r = JSON.parse(await oe(t, o));
        if (!A(r))
          throw new _.CredentialsProviderError("Invalid response received from instance metadata service.", {
            logger: e.logger,
          });
        return S(r);
      }, n);
  ...
  oe = async (e, t) => {
    if (process.env[Dsr]) t.headers = { ...t.headers, Authorization: process.env[Dsr] };
    return (await U1e({ ...t, timeout: e })).toString();
  },
  re = "169.254.170.2",
  ne,
  ae,
  se = async ({ logger: e }) => {
    if (process.env[Lvn]) return { hostname: re, path: process.env[Lvn] };
    if (process.env[Ovn]) {
...
var V = Nr(() => {
  O();
  k = { environmentVariableSelector: (e) => e[ie], configFileSelector: (e) => e[ce], default: l.IPv4 };
});
var D,
  U,
  Nvn = async () => U.parseUrl((await de()) || (await le())),
...
var H,
  P,
  Y = "/latest/meta-data/iam/security-credentials/",
  pe = "/latest/api/token",
  M = "AWS_EC2_METADATA_V1_DISABLED",
  G = "ec2_metadata_v1_disabled",
  ...
  me = (e = {}) => {
      ...
      p = async (s, m) => {
        if (t || m.headers?.[q] == null) {
            ...
            Z = await H.loadConfig(
              {
                environmentVariableSelector: (d) => {
                  ...
                  if (((c = !!f && f !== "false"), f === void 0))
                    throw new P.CredentialsProviderError(`${M} not set in env, checking config file next.`, {
                      logger: e.logger,
                    });
                  return c;
                },
          ...
          if (e.ec2MetadataV1Disabled || Z) {
            ...
            if (i) d.push(`config file profile (${G})`);
    ...
    return async () => {
      let s = await Nvn();
      if (t)
        return n?.debug("AWS SDK Instance Metadata", "using v1 fallback (no token fetch)"), p(a, { ...s, timeout: r });
      else {
        let m;
        try {
  ...
  fe = async (e) =>
    U1e({ ...e, path: pe, method: "PUT", headers: { "x-aws-ec2-metadata-token-ttl-seconds": "21600" } }),
  Ee = async (e) => (await U1e({ ...e, path: Y })).toString(),
  Ie = async (e, t, n) => {
    let o = JSON.parse((await U1e({ ...t, path: Y + e })).toString());
    if (!A(o))
      throw new P.CredentialsProviderError("Invalid response received from instance metadata service.", {
        logger: n.logger,
      });
    return S(o);
  };
...
export { U1e, hHr, _Hr, Rzt, Ovn, Lvn, Dsr, Mvn, fJe, Nvn, Fvn, kzt };
