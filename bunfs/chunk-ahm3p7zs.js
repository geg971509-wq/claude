...
import { xC, Mo, ye, Os, roe, U$ } from "/$bunfs/root/chunk-988p40e0.js";
...
import { sp, JA, gft, Xzn, Yzn, Pv } from "/$bunfs/root/chunk-zze8764r.js";
...
import { YJt, l6e } from "/$bunfs/root/chunk-jvgmmdgq.js";
import { Cb } from "/$bunfs/root/chunk-pwk6q778.js";
import { Tn, Es } from "/$bunfs/root/chunk-n71q9jb8.js";
...
import { stripVTControlCharacters as R } from "util";
async function C(o, i) {
  o.render(e(Cb, { children: e(t, { children: b(i, null, 2) }) })), await o.waitUntilExit();
}
async function ne(o, i) {
  y("cli_auto_mode_defaults");
  let s = JA();
  await C(o, i ? E(s, i) : s);
}
function E(o, i) {
...
async function re(o) {
  y("cli_auto_mode_config"), await C(o, Xzn(U$()));
}
async function se(o, i) {
  let s = Mo("userSettings");
  if (!s)
    return (
      await Pn("cli_auto_mode_reset", "no_user_settings_path"), Es("Could not resolve the user settings file path.")
    );
  let a = null;
  try {
    a = (await db(s, sb)).content;
  } catch (u) {
  ...
  if (d === void 0) {
    er(`Auto mode configuration is already at defaults \u2014 ${s} has no autoMode section.
`),
      M(),
      await Zs("cli_auto_mode_reset");
    return;
  }
  let f = xC(s),
    m =
      f.settings === null
        ? []
        : f.errors.filter((u) => u.severity === "warning").map((u) => A(u.path) || "unknown entry"),
    _ = F(m);
  if (m.length > 0 && o.yes)
    return (
      await Ic("cli_auto_mode_reset", "lossy_write_unconfirmed"),
      Es(
        `Not resetting: ${s} also contains ${v(m.length, "entry", "entries")} this version of Claude Code cannot parse \u2014 ${_.join(", ")} \u2014 and saving the file would delete ${m.length === 1 ? "it" : "them"} too. Fix or remove ${m.length === 1 ? "that entry" : "those entries"} first, or run the command without --yes to review and confirm.`,
      )
    );
  er(`This resets auto mode to the shipped defaults by removing the autoMode section from ${s}:
`);
  for (let u of T(d))
    er(`  - ${A(u)}
`);
  if (m.length > 0) {
    er(`Saving will ALSO delete ${v(m.length, "entry", "entries")} this version of Claude Code cannot parse \u2014 the settings writer rewrites the file from its validated view:
`);
    for (let u of _)
      er(`  - ${u}
`);
  ...
  if (!o.yes) {
    if (!(await gke("Reset auto mode configuration to defaults?")))
      return await Ic("cli_auto_mode_reset", "declined"), Es("Aborted.");
  }
  Hc();
  let { error: h } = await Os(
    "userSettings",
  ...
  if (h) {
    n(`auto-mode reset write failed: ${h.message}`, { level: "error" });
    let u = l6e(h, s, "reset");
    return await Pn("cli_auto_mode_reset", u.code), Es(u.message);
  }
  await Zs("cli_auto_mode_reset"),
    er(`Auto mode configuration reset to defaults \u2014 autoMode section removed from ${s}.
Run \`claude auto-mode config\` to see the effective rules.
`),
    M();
}
function T(o) {
  if (!He(o)) return ["autoMode (unrecognized value)"];
  return Object.entries(o).map(([i, s]) =>
    Array.isArray(s) ? `${i} (${s.length} ${k(s.length, "entry", "entries")})` : i,
  );
}
function F(o) {
  let i = new Map();
...
function M() {
  for (let o of roe) {
    ...
    if (ye(o)?.autoMode !== void 0) {
      er(`Note: auto mode rules from managed or --settings flag sources still apply \u2014 reset only changes your user settings file.
`);
      return;
    }
  }
}
var O = `You are an expert reviewer of auto mode classifier rules for Claude Code.
...
Please critique these custom rules.`,
    ...
    _ = u?.type === "text" ? u.text : "No critique was generated. Please try again.";
  } catch (h) {
    return (
      p("cli_auto_mode_critique", "cli_auto_mode_critique_query_failed"),
      o.unmount(),
      Tn("Failed to analyze rules: " + l(h))
    );
  }
  y("cli_auto_mode_critique"), o.render(e(Cb, { children: e(t, { children: _ }) })), await o.waitUntilExit();
}
function w(o) {
  return (o ?? []).some((i) => i !== sp);
}
function S(o, i, s) {
  let a = i.filter((f) => f !== sp);
  if (a.length === 0) return "";
  let g = i.length !== a.length,
    c = a
      .map((f) => "- " + f)
      .join(`
`),
...
export {
  ...
  T as describeAutoModeBlock,
  F as describeLossyPaths,
  E as filterRulesByLabelPrefix,
};
