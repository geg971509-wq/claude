...
import { Me, bo, wS } from "/$bunfs/root/chunk-qq1mdtb5.js";
...
import { Wl, Ls, a, Zn } from "/$bunfs/root/chunk-fec4384a.js";
import { SFe, T4 } from "/$bunfs/root/chunk-qm65zb83.js";
import "/$bunfs/root/chunk-vfy57cpd.js";
import { R, l, E, X } from "/$bunfs/root/chunk-ypdw393e.js";
...
import { Kt, ar, mc } from "/$bunfs/root/chunk-8tgj5dp2.js";
import "/$bunfs/root/chunk-4fwj3vnx.js";
import "/$bunfs/root/chunk-s7e0px8j.js";
import { mfe, Czt, vzt, pJe, gfe, qi } from "/$bunfs/root/chunk-s28wf80n.js";
...
import { sb } from "/$bunfs/root/chunk-0spqrdaj.js";
...
import { Ri, dA } from "/$bunfs/root/chunk-rf51999f.js";
...
import { r0, Ga } from "/$bunfs/root/chunk-tzhtxm67.js";
...
import { Q2, wd, eY, ILe } from "/$bunfs/root/chunk-2n1hsggr.js";
...
import { g_, gjt, hjt } from "/$bunfs/root/chunk-w3c6n7jh.js";
...
import { rd } from "/$bunfs/root/chunk-dc4hb8f7.js";
...
import {
  ...
  q5t,
  SIn,
} from "/$bunfs/root/chunk-ppgyw8dj.js";
import { mRt, GKt } from "/$bunfs/root/chunk-h7nv35ee.js";
import { B5t } from "/$bunfs/root/chunk-qep6r83c.js";
import { DZe } from "/$bunfs/root/chunk-sw6rvpjz.js";
import { qnt } from "/$bunfs/root/chunk-h1ypw83k.js";
import { $Ht } from "/$bunfs/root/chunk-dfmnmjy0.js";
import {
  axt,
  Ort,
  S1,
  ...
  y1n,
  Mrt,
} from "/$bunfs/root/chunk-5v6kpgmm.js";
...
import { I$, cR, _Ae } from "/$bunfs/root/chunk-szz73wxx.js";
...
import { vs, oj } from "/$bunfs/root/chunk-3s6zpzqb.js";
...
import { Bn } from "/$bunfs/root/chunk-56sxk8k2.js";
...
import { execFileSync as aa } from "child_process";
import { createWriteStream as da, fchmod as ua } from "fs";
import { lstat as ca, mkdir as Or, readFile as Ws, rm as Qn, writeFile as la } from "fs/promises";
import { homedir as vr, hostname as fa } from "os";
import { dirname as ha, join as mn, resolve as er } from "path";
...
import { randomUUID as Xn } from "crypto";
import {
  chmod as Li,
  lstat as Ht,
  mkdir as Lt,
...
import { homedir as Yn, tmpdir as xi } from "os";
import {
  basename as Mi,
  dirname as nn,
  isAbsolute as _s,
...
import { readdirSync as ii, readFileSync as Ir } from "fs";
...
async function Gr(e, t) {
  ...
  if (o === "macos")
    f = pi([process.pid, ...n.map(([u]) => u)], t).then((u) => {
      ...
      s = i.ownPgrp;
      for (let [m, T] of i.proven) r.set(m, T);
    });
  else if (o === "linux" || o === "wsl")
    (s = ci()),
      (f = Mr(n, async ([u, i]) => {
        let m = await oi(`/proc/${u}/stat`, "utf8").catch(() => {
          return;
        });
        if (m !== void 0 && mfe(m) === i && !gfe(pJe(m))) r.set(u, vzt(m));
      }));
  else
    f = Mr(n, async ([u, i]) => {
...
function di(e, t, n) {
  ...
  for (let [d, o] of n) {
    let f = e.get(d);
    if (f !== void 0 && f.token === o) s.set(d, f.pgrp);
  }
  return { proven: s, ownPgrp: r.pgrp };
}
async function Wr(e, t = Hr) {
  let { proven: n, incomplete: r } = await Gr(e, t);
  return { live: n.size, incomplete: r };
}
async function sr(e, t, n = Hr) {
  let { proven: r, ownPgrp: s, incomplete: d } = await Gr(e, n),
    o = 0;
  if (s !== void 0) {
    let h = new Set();
    for (let u of r.values()) if (u !== void 0 && u > 1 && u !== e.rootPid && u !== s && u !== process.pid) h.add(u);
    for (let u of h)
      try {
        process.kill(-u, t), o++;
      } catch {}
  ...
  return { proven: r.size, pids: f, groups: o, incomplete: d };
}
function xr(e, t) {
  let n = new Map(),
    r = [],
  ...
  while (f.length > 0) {
    ...
    else o.pinned.set(u.pid, u.token);
    f.push(...(n.get(u.pid) ?? []));
  }
  return o;
}
function ui() {
  ...
  for (let n of e) {
    ...
    try {
      let r = Ir(`/proc/${n}/stat`, "utf8"),
        s = Czt(r);
      if (s === void 0 || gfe(pJe(r))) continue;
      let d = mfe(r);
...
function ci() {
  try {
    return vzt(Ir("/proc/self/stat", "utf8"));
  } catch {
    return;
  }
}
...
async function li(e) {
  let t = await qe("ps", ["-A", "-o", "pid=", "-o", "ppid=", "-o", "pgid=", "-o", "stat=", "-o", "lstart="], {
      timeout: e,
      env: Br(),
      cwd: "/",
      stripFinalNewline: !1,
...
async function pi(e, t) {
  let n = await qe("ps", ["-o", "pid=", "-o", "pgid=", "-o", "stat=", "-o", "lstart=", "-p", e.join(",")], {
    timeout: t,
    env: Br(),
    cwd: "/",
    stripFinalNewline: !1,
...
`)) {
    let s = /^\s*(\d+)\s+(\d+)\s+(\S+)\s+(\S.*?)\s*$/.exec(r),
      d = s?.[3],
      o = s?.[4];
    if (s && d !== void 0 && o !== void 0 && !d.startsWith("Z")) t.set(Number(s[1]), { pgrp: Number(s[2]), token: o });
  }
  return t;
}
...
var hn = "/tmp",
  ...
  Mn = "ccr-byoc-prefetch-network.state",
  or = "ccr-byoc-standby-checkout.lock.d";
...
function In(e) {
  let t = new Map();
  for (let [, n, r] of e.matchAll(Ti)) t.set(n, r);
  return t;
}
function Nt(e) {
...
function ir(e) {
  ...
  let t = In(e),
    n = t.get("aborted");
  return {
    outcome: n !== void 0 ? (n === "timeout" ? "timeout" : "aborted") : t.get("yielded") === "1" ? "yielded" : "done",
    elapsedS: Nt(t.get("elapsed_s")),
  };
}
function Yr(e, t) {
  ...
  if (r !== void 0) {
    let o = r.indexOf(":");
    (s = (o === -1 ? r : r.slice(0, o)).toLowerCase()), (d = o === -1 ? void 0 : r.slice(o + 1).toLowerCase());
  }
  return {
    prefetch_phase: s,
    prefetch_phase_reason: d,
    prefetch_runs: Nt(n.get("runs")),
    prefetch_ok: Nt(n.get("ok")),
    prefetch_last_ok_age_s: yi(cr(n.get("last_ok_ts")), t),
    prefetch_last_duration_s: Nt(n.get("last_duration_s")),
    prefetch_midx: Nt(n.get("midx")),
  };
}
async function Vr(e = {}) {
  let t = await an(en(e.dir ?? hn, Mn));
...
function Oi(e, t, n, r) {
  ...
  let s = In(e),
    d = Nt(s.get("ok")),
    o = Nt(s.get("consecutive_fail")),
    f = cr(s.get("last_ok_ts")),
    h = Nt(s.get("last_duration_s")),
    u = s.get("tip");
  if (d === void 0 || d < 1 || o !== 0 || f === void 0 || h === void 0 || u === void 0 || !Xr.test(u)) return;
  let i = t - f;
  if (i < -vi) return;
  let m = Math.max(0, i) + h * 1000;
  if (m > n * 1000) return;
  let T = Math.round(m / 1000),
    k = { ref: zr, tipPrefix: u };
  switch (r) {
    case "staging":
      return { ref: zr, tipPrefix: u, ageS: T, witness: k };
    case "warm": {
      let A = s.get("warm_tip");
      if (Nt(s.get("warm_rc")) !== 0 || A === void 0 || !Xr.test(A)) return;
      return { ref: ki, tipPrefix: A, ageS: T, witness: k, trackingRef: bi };
    }
  }
}
async function qr(e) {
  let t = await an(en(e.dir ?? hn, Mn));
  return Oi(t, e.now ?? Date.now(), e.maxAgeS, e.branch);
}
var Ai = /^[0-9a-f]{40}$/,
  $i = /^[0-9a-f]{12,64}$/;
function xn(e) {
  return e !== void 0 && Ai.test(e) ? e : void 0;
}
function Ci(e) {
  if (e === void 0) return;
  let t = In(e),
  ...
  return {
    phase: r,
    head: s !== void 0 && $i.test(s) ? s : void 0,
    base: xn(t.get("co_base")),
    target: xn(t.get("co_target")),
    lastOkMs: cr(t.get("co_last_ts")),
    lastDurationS: Nt(t.get("co_last_s")),
    ok: Nt(t.get("co")),
    fail: Nt(t.get("co_fail")),
  };
}
async function Zr(e = {}) {
  let t = e.dir ?? hn,
    n = en(t, or),
    r = Ci(await an(en(t, Mn)));
  if (
    !(await mi(n).then(
      (i) => i.isDirectory(),
      () => !1,
  ...
  let [d, o, f, h] = await Promise.all(["pgid", "start", "base", "target"].map((i) => an(en(n, i)))),
    u = Nt(d);
  return {
    state: r,
    lock: {
      pgid: u !== void 0 && u > 1 ? u : void 0,
...
async function lr(e = {}) {
  ...
  return {
    ...
    prewarm_idx_outcome: f.outcome,
  };
}
function Jr(e, t) {
  let n = { ...e };
  if (e.prewarm_vda_outcome === "absent" && t.prewarm_vda_outcome !== void 0)
    (n.prewarm_vda_outcome = t.prewarm_vda_outcome), (n.prewarm_vda_elapsed_s = t.prewarm_vda_elapsed_s);
  if (e.prewarm_stat_outcome === "absent" && t.prewarm_stat_outcome !== void 0)
    (n.prewarm_stat_outcome = t.prewarm_stat_outcome), (n.prewarm_stat_elapsed_s = t.prewarm_stat_elapsed_s);
...
async function Qr(e = {}) {
  let t = e.dir ?? hn,
    n = e.now ?? Date.now(),
    [r, s] = await Promise.all([lr({ dir: t }), an(en(t, Mn))]);
  return { ...r, ...Yr(s, n) };
}
var Ni = 1e4,
  cd = 4 * Ni;
function fr(e) {
  return cR(e.replace(/^sk-ant-[a-z]+-/, ""));
}
function Un(e) {
...
function Fn({ getAccessToken: e, onRefresh: t, label: n }) {
  return _Ae({
    getAccessToken: e,
    onRefresh: t,
    label: n,
...
var Ss = 5000,
  ...
  kn = 30000;
function Ce(e, t) {
  return cu(e, Ss, `[runner:stuck] fs op '${t}' (check TMPDIR mount health)`);
}
function Fi(e) {
...
async function Es(e, t) {
  let n = process.env.SELF_HOSTED_RUNNER_HOST_CONFIG_DIR || process.env.CLAUDE_CONFIG_DIR || B(Yn(), ".claude"),
    r = new Map(),
    s = { bytes: 0 };
  try {
    await cu(
      ks(
        ...
        (f) =>
          !Hi.has(f) &&
          !/^\.claude(-[a-z-]+)?\.json(\.backup)?$/.test(f) &&
          !/^\.config\.json(\.|$)/.test(f) &&
          !/^\.claude(-[a-z-]+)?\.json\./.test(f) &&
          !f.startsWith(".session_ingress_token"),
      ),
      ws,
    ...
    if (!X(f)) {
      e(`[runner:startup] host config snapshot from ${n} failed (sessions will start with empty config): ${f}`);
      return;
    }
  }
  ...
  try {
    let h = d
        ? await (async (i) => {
            for (let m of i)
              try {
                return await Cn(B(n, m), "utf8");
              } catch (T) {
                if (!X(T)) throw T;
              }
            throw Object.assign(Error("ENOENT"), { code: "ENOENT" });
          })([".config.json", `.claude${T4()}.json`, ...SFe.map((i) => `.claude${i}.json`)])
        : O() && t !== void 0
          ? await Gi(t)
          : await Cn(Ls(), "utf8"),
      u = await V(h);
  ...
  if (o) {
    for (let [f, h] of Object.entries(o)) {
      ...
      if (u !== void 0)
        e(
          `[runner:warn] host mcpServers entry "${f}" has ${u} \u2014 ignoring (the Claude child would drop it silently)`,
        ),
          delete o[f];
    }
...
var Wi = new Set(["stdio", "sse", "http", "streamable-http", "ws", "sdk", "claudeai-proxy"]);
function Bi(e) {
  if (e === null || typeof e !== "object" || Array.isArray(e))
    return `a non-object value (${e === null ? "null" : Array.isArray(e) ? "array" : typeof e})`;
  let t = e.type;
  if (t === void 0) {
    if ("url" in e && !("command" in e)) return 'a "url" but no "type" (add "type": "http", "sse", or "ws")';
    return;
  }
  if (typeof t !== "string") return `a non-string type (${typeof t})`;
  if (!Wi.has(t)) return `unsupported type "${t}"`;
  return;
}
var Gn = 67108864;
async function ks(e, t, n, r, s) {
  ...
  for (let o of d) {
    ...
    if (o.isDirectory()) await ks(e, f, n, r);
    else if (o.isFile()) {
      let h = B(e, f),
        u = await Er(h);
      if (((r.bytes += u.size), r.bytes > Gn))
        throw new R(
          `host config exceeds ${Gn} bytes at ${f} \u2014 reduce ~/.claude size or set SELF_HOSTED_RUNNER_HOST_CONFIG_DIR to a slimmer dir`,
          "host config exceeds the snapshot size limit",
        );
      let i = await Cn(h);
      if (((r.bytes += i.length - u.size), r.bytes > Gn))
        throw new R(
          `host config exceeds ${Gn} bytes at ${f} \u2014 reduce ~/.claude size or set SELF_HOSTED_RUNNER_HOST_CONFIG_DIR to a slimmer dir`,
          "host config exceeds the snapshot size limit",
        );
      n.set(f, { buf: i, mode: u.mode & 511 });
    }
  }
}
async function bs(e) {
  let t = process.env.GIT_CONFIG_GLOBAL,
    n = process.env.HOME || Yn(),
    r = process.env.XDG_CONFIG_HOME || B(n, ".config"),
    s = t ? [t] : [B(r, "git", "config"), B(n, ".gitconfig")],
    d = [];
  for (let h of s) {
    try {
      await Ce(Er(h), `stat ${h}`);
    } catch (u) {
      if (!X(u)) e(`[runner] governed git: could not read ${h} for gitconfig seed (${u}); skipping it`);
      continue;
    }
    try {
      let u = await gr(["config", "--file", h, "--list", "-z"]);
      for (let i of u.split("\x00")) {
        if (!i) continue;
        let m = i.indexOf(`
`),
          T = m === -1 ? i : i.slice(0, m),
          k = m === -1 ? "true" : i.slice(m + 1);
        d.push([T, k]);
      }
      e(`[runner] governed git: captured gitconfig seed from ${h} (startup snapshot)`);
    } catch (u) {
      e(`[runner] governed git: could not parse ${h} for gitconfig seed (${u}); skipping it`);
    }
  }
  let o = d.filter(([h, u]) => ss(h, u));
  if (o.length < d.length) {
    let h = (m) => {
        let T = m.indexOf("."),
          k = m.lastIndexOf(".");
        return T === k ? m : `${m.slice(0, T)}.*${m.slice(k)}`;
      },
      u = d.filter(([m, T]) => !ss(m, T)),
      i = te(u.map(([m]) => h(m))).sort();
    e(
      `[runner] governed git: seed filter dropped ${u.length} non-allowlisted gitconfig entries (families: ${i.join(", ")})`,
    );
  }
  if (o.length === 0) return "";
  let f;
  try {
    f = await Pi(B(xi(), "ccr-govseed-"));
    let h = B(f, "seed.gitconfig");
    for (let [u, i] of o) await gr(["config", "--file", h, "--add", u, i]);
    return await Ce(Cn(h, "utf8"), `read ${h}`);
  } catch (h) {
...
async function gr(e) {
  return await new Promise((t, n) => {
    let r = Wt("git", e, {
      ...
      s = "",
      d = "";
    r.stdout?.on("data", (f) => (s += String(f))), r.stderr?.on("data", (f) => (d += String(f)));
    let o = setTimeout((f) => f.kill("SIGKILL"), un, r);
    r.on("close", (f) => {
      if ((clearTimeout(o), f === 0)) t(s);
      else
        n(
          new R(
            `git ${e[0]} ${e[1] ?? ""} exited ${f}: ${d.trim()}`,
...
async function Rs(e, t) {
  let n = B(e, "hooks"),
    r = await Ce(
      Ht(n).catch((s) => (X(s) ? void 0 : Promise.reject(s))),
      `lstat ${n}`,
    );
  ...
  try {
    if (
      (await cu(
        (async () => {
          let o = await Rs(e, r);
          for (let [f, { buf: h, mode: u }] of t.files) {
            let i = f.toLowerCase();
            if (!o && (i === "hooks" || i.startsWith("hooks" + Fe))) continue;
          ...
          if (t.mcpServers) {
            let f = await b({ mcpServers: t.mcpServers });
            await tn(B(e, `.claude${s}.json`), f, { mode: 384 });
          }
        })(),
        ws,
...
function Vi(e) {
  if (!Array.isArray(e)) return `launcher_hooks: not an array (got ${vn(e)})`;
  let t = new Set();
  for (let [n, r] of e.entries()) {
    if (r === null || typeof r !== "object") return `launcher_hooks[${n}]: not an object (got ${vn(r)})`;
    if (typeof r.event !== "string" || !Yi.has(r.event)) return `launcher_hooks[${n}]: unknown event ${vn(r.event)}`;
    if (typeof r.filename !== "string" || !Xi.test(r.filename))
      return `launcher_hooks[${n}]: invalid filename ${vn(r.filename)}`;
    let s = r.filename.toLowerCase();
    if (t.has(s)) return `launcher_hooks[${n}]: duplicate filename ${b(r.filename)} (case-insensitive)`;
    t.add(s);
    let d = typeof r.script === "string" ? Buffer.byteLength(r.script, "utf8") : 0;
    if (d === 0 || d > is) return `launcher_hooks[${n}] ${b(r.filename)}: script size ${d} out of range (1..${is})`;
  }
  return;
}
async function Zi(e, t, n, r, s) {
  ...
  if (d !== void 0) {
    s(
      ...
      p("self_hosted_launcher_hooks", "self_hosted_launcher_hooks_validation_failed");
    return;
  }
  let o = {};
  if (!(await Rs(e, s))) {
    p("self_hosted_launcher_hooks", "self_hosted_launcher_hooks_hooksdir_not_plain_dir");
    return;
  }
  ...
  for (let u of t) {
    ...
    n.push(i),
      await Ce(dn(i), `unlink ${i}`).catch(() => {}),
      await Ce(tn(i, u.script, { flag: "wx", mode: 448 }), `writeFile ${i}`),
      (o[u.event] ??= []).push({ matcher: "", hooks: [{ type: "command", command: i, args: [] }] });
  }
  let h = B(e, "launcher-settings.json");
  return (
    n.push(h),
    await Ce(dn(h), `unlink ${h}`).catch(() => {}),
    await Ce(tn(h, b({ hooks: o }, null, 2), { flag: "wx", mode: 384 }), `writeFile ${h}`),
    r(`[runner:session] Wrote ${t.length} launcher hook(s) + ${h}`),
    { settingsPath: h }
  );
}
...
function Ts(e, t) {
  return B(e, `.session_ingress_token.e${t}`);
}
...
async function as(e, t, n, r = On) {
  try {
    await An(dn(e), r, "[runner:session] unlink session-ingress token file", n);
  } catch (f) {
    if (!X(f)) t(`[runner:session] session-ingress token file cleanup failed: ${f}`);
  }
  ...
  let d = Number(s[1]),
    o = nn(e);
  try {
    let f = await An(qn(o), r, "[runner:session] readdir for token temp sweep", n);
    for (let h of f) {
      if (!h.startsWith(".session_ingress_token.")) continue;
      let u = /^\.session_ingress_token\.e(\d+)\.tmp\./.exec(h);
      if (!u) {
        if (h.includes(".tmp.")) t("[runner:session] token temp sweep skipped an unparseable sibling");
        continue;
      }
      if (Number(u[1]) > d) continue;
      try {
        await An(dn(B(o, h)), r, "[runner:session] unlink token temp file", n);
      } catch (i) {
        if (!X(i)) t(`[runner:session] token temp sweep failed for a sibling: ${i}`);
      }
    }
  } catch (f) {
    if (!X(f)) t(`[runner:session] token temp sweep readdir failed: ${f}`);
  }
}
var Ji = new Set([
    "sdk-url",
...
async function ys(e, t, n) {
  let {
      ...
      canonicalLocks: ie,
      tokenFileOpTimeoutMs: P,
      gitSshRewriteHosts: ae,
      gitHostRewrites: Se,
      useAnthropicGitProxy: fe,
      gitProxyGlobalConfigPath: ve,
      gitProxyGlobalConfigSnapshot: Ne,
      gitProxyCredHelper: xe,
      configureGitHookStubs: Qe,
      configureGitSigningArtifacts: kt,
      pushOutcomeOnRelease: je,
      trustWorkspace: Bt,
      confineRepoSettings: vt = "enforce",
      hostConfigSnapshot: et,
      governedGitConfigSeed: Pt,
      debugTokenDir: Dt,
      standbyIdleMs: Ot,
  ...
  let mt = !1,
    ...
    Oe,
    _t = [],
    Ye = B(d, "_sessions", `${e}.claude-config`),
    zt = B(d, "_sessions", `${e}.gitconfig`),
    at = B(Ye, "claude-code-debug.txt"),
    Y,
    Ke,
    dt,
    it = Promise.resolve(),
  ...
  try {
    ...
    Ke = B(d, "_sessions", e);
    let re = B(d, "_sessions", `${e}.uploads`),
      F = await gn(() => r.issueSessionToken(s(), e, n), {
        initialDelayMs: 500,
        maxDelayMs: 8000,
        maxAttempts: 5,
        shouldRetry: $ve,
        ...
        onRetry: (se, Ze) => {
          i(
            `[runner:session] issueSessionToken attempt ${se} transient failure (${Ze instanceof Error ? Ze.message : Ze}) \u2014 retrying`,
          );
        },
      });
    if (!F) return { result: "abandoned" };
    if (((H.current = F.session_token), Xe)) await _o(Qe, kt, i);
    if (fe) {
      if (((process.env.CLAUDE_CODE_SESSION_ACCESS_TOKEN = F.session_token), Ne !== void 0 && ve)) {
        let se = process.env.XDG_CONFIG_HOME || B(Yn(), ".config");
        await So(
          {
            globalConfigPath: ve,
            globalConfigSnapshot: Ne,
            homeGitconfigPath: B(Yn(), ".gitconfig"),
            xdgConfigPath: B(se, "git", "config"),
            credHelper: xe,
            signingArtifacts: kt,
          },
          i,
    ...
    if ((U?.(F.session_token), Dt)) os(Dt, `session_token_${e}.jwt`, F.session_token, m);
    if (n.aborted)
      return (
    ...
    if (
      ...
      (S = await gn(() => r.getSessionRemoteConfig(e, H.current, n), {
        initialDelayMs: 500,
        maxDelayMs: 8000,
        maxAttempts: 5,
        shouldRetry: $ve,
        ...
        onRetry: (se, Ze) => {
          i(
            `[runner:session] getSessionRemoteConfig attempt ${se} transient failure (${Ze instanceof Error ? Ze.message : Ze}) \u2014 retrying`,
          );
        },
      })),
      !S)
    ...
    if (
      (i(
        `[runner:session] Fetched remote config: ${S.sources.length} source(s), ${S.push_targets.length} push target(s), api_base_url=${S.api_base_url}`,
      ),
      n.aborted)
    )
      return (
        i(`[runner:session] Aborted after getSessionRemoteConfig for ${e} \u2014 skipping registration`),
        { result: "abandoned" }
      );
    let Re = S.worktree_checkpoint_hint != null,
      Le = co(
        S.governed_git,
        () => {
          let se = H.current;
    ...
    if (Le) {
      if (
        (m(
          `[runner:session] governed git ACTIVE for ${e}: pre-CLI clone via git mount, in-session git/gh via the session relay (git_config=${Le.toolConfig.gitConfig}, gh_path_shim=${Le.toolConfig.ghPathShim})` +
            (Ie
    ...
    try {
      if (
        ((Y = await gn(() => r.registerWorker(S.api_base_url, e, H.current, n), {
          initialDelayMs: 500,
          maxDelayMs: 8000,
          maxAttempts: 5,
          shouldRetry: $ve,
          ...
          onRetry: (L, I) => {
            i(
              `[runner:session] registerWorker attempt ${L} transient failure (${I instanceof Error ? I.message : I}) \u2014 retrying`,
            );
          },
        })),
        Y === void 0)
      ...
      i(`[runner:session] Registered worker, epoch=${Y}`);
      let se = Y;
      if (
        (await gn(() => r.updateSessionWorkerState(S.api_base_url, e, H.current, se, void 0, n), {
          initialDelayMs: 500,
          maxDelayMs: 8000,
          maxAttempts: 5,
          shouldRetry: $ve,
          ...
          onRetry: (L, I) => {
            i(
              `[runner:session] updateSessionWorkerState attempt ${L} transient failure (${I instanceof Error ? I.message : I}) \u2014 retrying`,
            );
          },
        }),
        n.aborted)
        ...
        we = (L) => {
          ...
          (Gt = Promise.all([Gt, I]).then(() => {})),
            I.then(() => {
              ...
              let K = H.current;
              if (!K) return;
              it = it.then(() => hr(Ze, K, m, we, P));
            });
        };
      (Xt = we),
        (it = hr(Ze, F.session_token, m, we, P)),
        await it,
        i(`[runner:session] Epoch fence passed for ${e} (epoch=${Y})`),
        (ft = ls({
          intervalMs: 30000,
          refresh: async () => {
            if (Y === void 0 || !H.current) return;
            await r.heartbeat(S.api_base_url, e, H.current, Y, n), i(`[runner:session] prep heartbeat sent for ${e}`);
            return;
          },
          onError: (L) => {
            i(`[runner:session] prep heartbeat failed: ${L}`);
          },
          signal: n,
        }));
      let yt = [],
        ke = (L, I) => {
          if (Y === void 0 || !H.current) return Promise.resolve();
          let K = Y,
            oe = H.current,
            _e = I.withDeferred ? yt.splice(0) : [],
            v = async () => {
              try {
                await r.postWorkerEvents(S.api_base_url, e, oe, K, [..._e, L], n);
              } catch (We) {
                if (Sn(We)) throw We;
                let ye = _e.length > 0 ? ` (${_e.length} deferred step event(s) dropped with it)` : "";
                i(`[runner:session] ${I.kind} event post failed${ye}: ${We}`);
              }
            };
          if (!tt) return v();
          let ue = () => (N === void 0 ? v() : Promise.resolve());
          if (w) return ue();
          if (!I.withDeferred) return _.then(ue);
          return (
            (_ = _.then(() =>
              ue().catch((We) => {
                (N = We), ft?.cancel();
        ...
        Jt = () =>
          (Rn ??= (async () => {
            ...
            return { config: await r.getSessionRemoteConfig(e, H.current, n), armedAt: L };
          })().catch((L) => {
            i(`[runner:session] early /remote re-fetch failed: ${L} \u2014 step 6 re-fetches if the session continues`);
            return;
          })),
        sn = Xo({
          gateOn: !1,
        ...
        Yt = lo(S.sources, ae, Se, i, De ? { apiBaseUrl: De, sessionId: e } : void 0, Le);
      await Mt("provision", "completed", "Runner registered", {
      ...
      if (Yt.length > 0) {
        i(
          `[runner:session] Preparing ${Yt.length} git ${Yt.length === 1 ? "repository" : "repositories"} (${Ie ? "worktree" : "canonical-direct"} mode)`,
        );
        let L = [],
          I = [],
          K = [],
          ...
          _e = [];
        for (let v of Yt) {
          if (!JJt(v.type)) continue;
          let ue = tQt(d, v),
            We = h1n(v);
          if (!ue || !We)
            throw new R(
              `Source '${v.url}' resolved to an unsafe repo path (slug='${v.repo}'). Check for path traversal in the URL.`,
              "source URL resolved to an unsafe repo path",
            );
          await rn(`Preparing ${v.repo}...`),
            await Mt("clone", "started", `Preparing ${v.repo}`, { step_detail: v.repo });
          let ye = (ln.get(wr(v.repo))?.length ?? 0) > 0,
            le = B(Ke, We),
            Ft = async () => {
              if (!Vt) return;
              if (de.includes(le)) {
                i(`[runner:hook] not removing ${le}: a prepared checkout (in use)`);
                return;
              }
              let Je = de.find((nt) => Q2(nt) === Q2(le));
              if (Je !== void 0)
                return (
                  i(`[runner:hook] not removing ${le}: same name as prepared checkout ${Je} modulo case`),
                  ee.push(le),
                  {
                    cause: "kept_prepared_twin",
                    warn: `[runner:warn] not removing ${le} for skipped context source ${v.repo}: same name as the prepared checkout ${Je} modulo case (one directory on a case-insensitive filesystem); whatever the hook wrote there may remain visible to this session. Removal is retried when the session ends.`,
                  }
                );
              let ze = await cu(
              ...
              return (
                ...
                {
                  cause: "rm_failed",
                  warn: `[runner:warn] could not remove the partial checkout the hook left for skipped context source ${v.repo} at ${le} (${l(ze)}); the session may still be able to read it. Removal is retried when the session ends.`,
                }
              );
            };
          try {
            if (Vt) {
              if (v.ref && !wS(v.ref))
                throw new R(
                  `[runner:session] refusing to pass unsafe ref to checkout hook: ${v.ref}`,
                  "refusing to pass unsafe ref to checkout hook",
                );
              let ze = v.governedMount && v.upstreamUrl ? v.upstreamUrl : v.url;
              if (!ze || !c6e(ze))
                throw new R(
                  `[runner:session] refusing to pass unsafe repo URL to checkout hook: ${ze}`,
              ...
              await yIn({
                ...
                repoRef: v.ref,
                checkoutPath: le,
                sessionAccessToken: H.current,
                apiBaseUrl: S.api_base_url,
                gitMountUrl: v.governedMount ? v.url : void 0,
                clientPlatform: M,
                onStatus: m,
                onDebug: i,
                signal: n,
                ...
                K.push(v.url ?? ""),
                ee.push(le),
                Pn("clone", "completed", `Prepared ${v.repo}`, { step_detail: v.repo });
              continue;
            }
            let Je = Ie ? B(Ke, We) : ue;
            if (
              (await Qi(ie, ue, async () => {
                if (De || (Le && !Ie)) {
                  if (ho(m, ue)) await mo(ue, i, v.url);
                } else await wo(ue, i, !Ie);
                let ze = await qo({
                    useWorktrees: Ie,
                    workerEpoch: Y,
                    source: v,
                    checkpoint: { hint: sn, checkoutPath: ue },
                    onDebug: i,
                  }),
                  nt = Qo({ claimed: bt, useWorktrees: Ie, source: v, onDebug: i });
                if (
                  (await g1n({
                    baseDir: d,
                    ...
                    onPhase: (qt, Ct, rr) => {
                      ...
                      let Js = Y,
                        Qs = H.current,
                        ei = [Go(v.repo, qt, Ct, rr)],
                        Nr = () =>
                          N !== void 0 ? Promise.resolve() : r.postWorkerEvents(S.api_base_url, e, Qs, Js, ei, n);
                      (tt ? _.then(Nr) : Nr()).catch((ti) => i(`[runner:session] prep-phase event post failed: ${ti}`));
                    },
                    onProgress: (qt, Ct) => {
                      let rr = Ct.total !== void 0 ? ` (${Ct.done}/${Ct.total})` : "";
                      ke(
                        jn("clone", "started", `${v.repo} \u2014 ${Ct.label} ${Ct.pct}%${rr}`, {
                          step_detail: v.repo,
                          clone_sub: qt,
                  ...
                  await _1n({ canonicalRepoPath: ue, worktreePath: Je, ref: v.ref, onDebug: i, signal: n });
              }),
              Ie)
            )
              xt.push({ canonicalRepoPath: ue, worktreePath: Je });
            de.push(Je),
              L.push(v.repo),
              I.push(v),
              K.push(v.url ?? ""),
            ...
            let ze = Je instanceof Error ? Je.message : String(Je);
            if (Ort(ze)) {
              if (!ye) {
                let nt = v.ref ? `'${v.ref}'` : "the configured ref";
                i(
                  `[runner:session] context source ${v.repo} ref ${nt} permanently not found on remote; ` +
                ...
                _e.push({
                  repo: v.repo,
                  sads: qt ? [Hn[qt.cause].sad] : [],
                  step:
                    `Skipped ${v.repo} \u2014 source ref ${nt} no longer exists on remote; continuing without this context repo` +
                    (qt ? Hn[qt.cause].note : ""),
                }),
                  (oe ??= new zn(v.repo, v.ref));
                continue;
              }
              throw new zn(v.repo, v.ref);
            }
            if (Vt && !ye && Je instanceof Uve && !n.aborted) {
              m(
                `[runner:warn] checkout hook failed for context source ${v.repo} (${Je.telemetryMessage}); not a work repo (no push_targets entry), skipping it: ` +
                  Rl(Je.message),
              );
              let nt = await Ft();
              if (nt) m(nt.warn);
              _e.push({
                repo: v.repo,
                sads: ["context_source_checkout_hook_failed", ...(nt ? [Hn[nt.cause].sad] : [])],
                step:
                  `Skipped ${v.repo} \u2014 its checkout hook ${Je.telemetryMessage}; continuing without this context repo` +
                  (nt ? Hn[nt.cause].note : ""),
              }),
                (oe ??= Je);
              continue;
        ...
        if (de.length === 0 && oe) throw oe;
        for (let v of _e) {
          for (let ue of v.sads) g("self_hosted_session_handle", ue);
          await Mt("clone", "completed", v.step, { step_detail: v.repo });
        }
        if ((await Mt("clone", "completed", "Finished preparing sources"), rt.start(), Jt(), De)) {
          let v = new URL(De).origin;
          for (let ue = 0; ue < de.length; ue++) {
            if (!I[ue]?.getAuthToken) continue;
            if (ee.includes(de[ue])) continue;
            Ee.push({ path: de[ue], origin: v }), await Eo(de[ue], De, i, n);
          }
        }
        if ((rt.mark("runner_prep_git_proxy_config_ms"), Le)) {
          let v = new Set();
          for (let ue = 0; ue < de.length; ue++) {
            let We = I[ue];
            if (!We?.governedMount || !We.upstreamUrl) continue;
            if (ee.includes(de[ue])) continue;
            let le = xt.find((Ft) => Ft.worktreePath === de[ue])?.canonicalRepoPath ?? de[ue];
            if (v.has(le)) continue;
            v.add(le), await ko(le, We.upstreamUrl, i, n);
          }
        }
        rt.mark("runner_prep_reset_remote_ms");
        for (let v = 0; v < de.length; v++) {
          let ue = ln.get(wr(L[v]));
          if (!ue || ue.length === 0) continue;
          let [We, ...ye] = ue,
            le = xt.find((ze) => ze.worktreePath === de[v]),
            Ft = le?.canonicalRepoPath ?? de[v];
          if (ye.length > 0) {
            let ze = I[v],
              nt = ze?.getAuthToken && !ee.includes(de[v]);
            await Lo(
              de[v],
              ye,
              i,
              ...
              K[v].startsWith("file://"),
              De && nt ? ze.url : void 0,
              ze?.governedMount && nt ? { url: ze.url, getToken: ze.getAuthToken } : void 0,
            );
          }
          let Je;
          if (je && (Y ?? 0) > 1 && !ee.includes(de[v])) {
            if (await Po(Ft, We, I[v], kn, i, n)) Je = `refs/remotes/origin/${We}`;
          }
          if ((await No(de[v], We, i, n, !!fe, Je), ee.includes(de[v]))) continue;
          He.push({
            canonicalPath: Ft,
            branch: We,
            needsDetach: !le,
            createdSha: je ? await hs(Ft, `refs/heads/${We}`) : void 0,
            source: I[v],
          });
        }
        rt.mark("runner_prep_outcome_branch_ms"), lt.push(...de);
      }
      if ((rt.start(), Jt(), ({ childCwd: pt, addDirs: It } = ps(Ke, de, S.cwd)), S.cwd)) {
        let L = Cs(Ke, S.cwd);
        if (
          !(
            L !== null &&
          ...
          i(
            `[runner:session] config.cwd=${S.cwd} rejected (not under ${Ke}, a segment is a symlink, or fs op timed out)`,
          ),
            ({ childCwd: pt, addDirs: It } = ps(Ke, de));
      }
      ...
      if (vt !== "off") {
        ...
        let L = [...It, ...fn.map((K) => K.path)],
          I = () => {
            yn(Ye, "config dir", pt, L), yn(re, "stage-file root", pt, L);
          };
        if (vt === "warn")
          try {
            I();
          ...
          try {
            ...
            throw (p("self_hosted_confine", "self_hosted_confine_overlap_enforce"), K);
          }
      }
      if (fn.length > 0) {
        let L = (oe) => oe.toLowerCase(),
          I = new Set();
        for (let oe of [pt, ...It]) {
          I.add(L(oe)), I.add(L(oe.normalize("NFC")));
          let _e = await Ce(
            En(oe).catch(() => {
              return;
            }),
        ...
        for (let oe of fn)
          if (!K(L(oe.path))) {
            let _e = new ht(
              oe,
              oe.kind === "permissions.allow (bare write-tool rule)"
                ? `grants unbounded file-tool writes (toolAlwaysAllowedRule matches a bare ${oe.raw} rule for ANY path). defaultMode:"acceptEdits" already auto-allows in-workspace edits; drop the bare rule, or use "Edit(/**)" / "Write(/**)" for an explicit workspace-scoped rule, or move it to the operator's user-level settings.json`
      ...
      if (vt !== "off" && !_n) y("self_hosted_confine");
      if (
        (yn(Ye, "config dir", wd().replace(/[/\\]$/, ""), [xl(), eY().replace(/[/\\]$/, "")]),
        rt.mark("runner_prep_repo_settings_ms"),
        Bt)
      ) {
        let L = {};
        for (let oe of new Set([pt, ...de, ...xt.map((_e) => _e.canonicalRepoPath)])) {
          let _e = Ii(oe),
            v = await Ce(
              En(oe).catch(() => {
                return;
        ...
        await Ce(tn(K, I, { mode: 384 }), `writeFile ${K}`),
          i(`[runner:session] Seeded persisted trust for ${Object.keys(L).length} path(s) in ${K}`);
      }
      if (S.mcp_config?.content) {
        let L = Buffer.from(S.mcp_config.content, "base64").toString("utf-8");
        (pe = B(Ye, "mcp-config.json")),
          await Ce(dn(pe), `unlink ${pe}`).catch(() => {}),
          await Ce(tn(pe, L, { flag: "wx", mode: 384 }), `writeFile ${pe}`),
          i(`[runner:session] Wrote MCP config to ${pe} (${L.length} bytes)`);
      }
      if (
        (rt.mark("runner_prep_config_seed_ms"),
        S.launcher_hooks && (!Array.isArray(S.launcher_hooks) || S.launcher_hooks.length > 0))
      ) {
        let L = await Zi(Ye, S.launcher_hooks, _t, i, m);
        if (L)
          if (((Oe = L.settingsPath), nr.repoDisablesAllHooks))
            m(
              ...
              g("self_hosted_launcher_hooks", "self_hosted_launcher_hooks_disabled_by_repo_disableAllHooks");
          else if (Dn) g("self_hosted_launcher_hooks", "self_hosted_launcher_hooks_confine_scan_aborted");
          else y("self_hosted_launcher_hooks");
      }
      if (n.aborted)
        return (
          i(`[runner:session] Aborted after git prep for ${e} \u2014 bailing before spawn`), { result: "abandoned" }
        );
      let pn,
        C = new Map(),
        W = !1,
...
`,
        ...
        Qt = Fn({
          getAccessToken: async () => {
            try {
              let { token: L } = await r.refreshToken(H.current);
              return L;
            } catch (L) {
              if (on(L, "refreshToken")) Qt.cancel(e);
              throw L;
            }
          },
          onRefresh: (L, I) => {
            if (((H.current = I), fe)) process.env.CLAUDE_CODE_SESSION_ACCESS_TOKEN = I;
            if (dt) {
              let K = dt;
              it = it.then(() => hr(K, I, m, Xt, P));
            }
            cs({ pendingAcks: C, sessionId: e, onStatus: m }),
              us({
                write: pn,
      ...
      Qt.schedule(e, H.current);
      let $e, ut;
      try {
        rt.lap();
        let L = await Jt();
        if (tt && N !== void 0) throw N;
        let I = L === void 0 ? void 0 : Math.round(performance.now() - L.armedAt),
          K;
        if (L !== void 0 && I !== void 0 && I <= rs) K = L.config;
        else {
          if (I !== void 0)
            i(
              `[runner:session] early /remote result is ${I}ms old (> ${rs}ms) \u2014 re-fetching for a fresh inference token`,
            );
          K = await gn(() => r.getSessionRemoteConfig(e, H.current, n), {
            initialDelayMs: 1000,
            maxDelayMs: 30000,
            signal: n,
            ...(tt && { shouldRetry: () => N === void 0 }),
            onRetry: (ye, le) => {
              i(`[runner:session] /remote re-fetch attempt ${ye} failed: ${le}. Retrying.`);
            },
          });
        }
        if (K)
          (S = K),
            i(
              `[runner:session] Re-fetched /remote for fresh inference token (expires_in=${S.inference_auth?.expires_in_seconds}s)`,
            );
        else
          i(
            "[runner:session] /remote re-fetch failed after retries \u2014 using stale step-2 token. Interval refresher will converge.",
          );
        if ((rt.mark("runner_prep_remote_refetch_ms"), Dt && S.inference_auth?.access_token))
          os(Dt, `inference_token_${e}.txt`, S.inference_auth.access_token, m);
        let oe = ls({
            intervalMs: fs(S.inference_auth?.expires_in_seconds),
            refresh: async () => {
              let ye = await r.getSessionRemoteConfig(e, H.current, n),
                le = ye.inference_auth?.access_token;
              if (!le) throw Error("getSessionRemoteConfig returned no inference_auth.access_token");
              return (
                cs({ pendingAcks: C, sessionId: e, onStatus: m }),
                us({
                  write: pn,
                  ...
                  onStatus: m,
                  expiresInSeconds: ye.inference_auth?.expires_in_seconds,
                  pendingAcks: C,
                }),
                fs(ye.inference_auth?.expires_in_seconds)
              );
            },
            onError: (ye) => {
              if (on(ye, "/remote")) {
              ...
              m(`[runner:session] inference_token refresh failed: ${ye}. Retrying shortly.`);
            },
            signal: n,
          }),
          _e = {};
        await cu(
          Promise.all([
            j?.then((ye) => (_e.atClaim = ye)).catch(() => {}),
            j === void 0
              ? void 0
              : lr()
                  .then((ye) => (_e.late = ye))
                  .catch(() => {}),
            ct?.then((ye) => (_e.checkout = ye)).catch(() => {}),
          ]),
          Ui,
          "startup probes",
        ).catch(() => {});
        let v = _e.atClaim === void 0 ? void 0 : Jr(_e.atClaim, _e.late ?? {}),
          ue = _e.checkout,
          We;
        try {
          ...
          (We = DZe({
            sessionId: e,
            apiBaseUrl: new URL(Ps(S.api_base_url, e).sdkUrl).origin,
            tokenFilePath: dt,
            ...
            ($e = await eo({
              ...
              onTokenAck: (le) => {
                ...
                if (Ft)
                  C.delete(le),
                    m(`[runner:session] ${Ft.label} update ${le} acked by child (${Date.now() - Ft.sentAtMs}ms)`);
              },
              onChildStdinReady: (le) => {
                if (((pn = le), W)) le(Ge);
              },
            }));
        } finally {
          oe.cancel(), (pn = void 0), We?.then((ye) => ye?.stop());
        }
      } finally {
        Qt.cancelAll();
      }
      x = $e.result;
      let At = typeof n.reason === "string" ? n.reason : void 0;
      if ($e.result === "failed")
      ...
      if (
        x === "failed" &&
        W &&
        ($e.stderrTail.includes("SDKStartup: exiting without result") ||
          $e.stderrTail.includes("RemoteIO: transport closed permanently") ||
          $e.stderrTail.includes("worker epoch mismatch (409)"))
      )
        m(
          `[runner:session] ${e} child exit ${$e.exitCode ?? "null"} after server end_session \u2014 session archived/deleted; clean close, not a runner failure`,
        ),
          (x = "completed"),
          (he = void 0),
          (me = void 0),
      ...
      else if (x === "interrupted") q?.(At === "idle-release" || At === "deassign" ? "completed" : "interrupted");
    } catch (se) {
      if ((ft?.cancel(), n.aborted)) {
        let ke = typeof n.reason === "string" ? n.reason : "unspecified";
        return (
          i(`[runner:session] ${e} aborted during setup (reason=${String(ke)}) \u2014 not a failure`),
          (x = "interrupted"),
          { result: "interrupted" }
        );
      }
      ...
      let we = Rl(se instanceof Error ? se.message : String(se));
      he = `setup threw: ${we}`;
      let yt = se instanceof zn || Ort(we);
      {
        let ke = yme(se);
        if (ke !== void 0 && ke >= 500) me = "SESSION_FAILURE_KIND_ANTHROPIC_CONTROL_PLANE_5XX";
        else if (yt) me = "SESSION_FAILURE_KIND_SESSION_CONFIG_ERROR";
      }
    ...
    if (n.aborted)
      return i(`[runner:session] ${e} aborted during pre-spawn \u2014 not a failure`), { result: "interrupted" };
    let F = Rl(re instanceof Error ? re.message : String(re));
    he = `pre-spawn threw: ${F}`;
    {
      let Re = yme(re);
    ...
    else i(`[runner:session] FATAL pre-spawn: ${e}: ${F} \u2014 cannot post failure to UI: no session_token obtained`);
    x = "failed";
  } finally {
    let re = t.postSessionHookTimeoutMs ?? bn;
    if (je && Ve && x !== "completed" && n.reason !== "deassign" && !Ae && He.length > 0) {
      m(
        `[runner:session] ${e} ${x}: --push-outcome-on-release pushing ${He.length} outcome branch(es) to the source remote`,
      );
      let F = Date.now() + kn;
      for (let Re of He) {
        let Le = F - Date.now();
        if (Le <= 0) {
          i(`[runner:session] push-on-release budget exhausted; skipping '${Re.branch}' (best-effort)`);
          continue;
        }
        if (Re.createdSha) {
          if ((await hs(Re.canonicalPath, `refs/heads/${Re.branch}`, Le)) === Re.createdSha) {
            i(`[runner:session] push-on-release '${Re.branch}' has no commits since creation; skipping (best-effort)`);
            continue;
          }
        }
        let De = F - Date.now();
        ...
        await Do(Re.canonicalPath, Re.branch, Re.source, De, i);
      }
    }
    if (Ve) {
      let F = await HZe(Tt, "post-session");
      if (F)
        await SIn({
          hookPath: F,
          cwd: Ke ?? d,
          ...
          i(`[runner:hook:post-session] unexpected rejection (ignored): ${Re}`);
        });
    }
    if (Y !== void 0 && H.current && S?.api_base_url)
      await Fo({
        apiClient: r,
        apiBaseUrl: S.api_base_url,
        sessionId: e,
        ...
        i(`[runner:session] debug log flush failed (best-effort): ${F}`);
      });
    if (x === "completed") await Ce(dn(at), `unlink ${at}`).catch(() => {});
    else m(`[runner:session] ${e} ${x} \u2014 child debug log preserved at ${at}`);
    if (pe) await Ce(dn(pe), `unlink ${pe}`).catch(() => {});
    for (let F of [...(Oe ? [Oe] : []), ..._t]) await Ce(dn(F), `unlink ${F}`).catch(() => {});
    if (xt.length > 0)
      await y1n({ worktrees: xt, onDebug: i }).catch((F) => {
        i(`[runner:session] worktree cleanup failed: ${F}`);
      });
    for (let F of He) await xo(F.canonicalPath, F.branch, F.needsDetach, i, !!fe);
    for (let F of Ee)
      for (let Re of [["credential.helper", "^$"], [`credential.${F.origin}.helper`]])
    ...
    if (Ue !== void 0 && !st) be?.({ kind: "exit-before-init", failed: Pe });
  }
  return { result: x, failureReason: he, setupFailureKind: ge, failureKind: me };
}
function eo(e) {
  let {
      execPath: t,
      ...
      stageFileRoot: u,
      governedGit: i,
      governedGitConfigPath: m,
      capacity: T,
      healthPort: k,
      clientPlatform: A,
      debugFile: U,
      mcpConfigPath: q,
      launcherSettingsPath: M,
      addDirs: be,
      onDebug: Z,
      onStatus: ie,
      onSessionActivity: P,
      onBgTaskLedger: ae,
      onBgFollowupPending: Se,
      signal: fe,
      onChildStdinReady: ve,
      onTokenAck: Ne,
      onChildLifecycle: xe,
      onChildInit: Qe,
      onSessionStartHookError: kt,
      runnerStartupTiming: je,
    } = e,
    { sdkUrl: Bt, resumeUrl: vt } = Ps(r.api_base_url, s),
    et = [
      ...n,
      "--print",
      "--sdk-url",
  ...
  if (q) et.push("--mcp-config", q);
  if (M) et.push("--settings", M);
  for (let _ of be ?? []) et.push("--add-dir", _);
  let Pt = GKt(r.claude_code_args);
  if (mRt(r.claude_code_args.tools)) {
    if (
      (Z("[runner:session] Stripped self-hosted-runner operator tool names from the server-supplied tools arg"),
      Pt.tools === "")
    )
      Z(
  ...
  let Dt = qnt(et, Pt, Ji, (_, N) => Z(`[runner:session] Skipping ${_} claude_code_arg: ${N}`));
  if (Dt > 0) Z(`[runner:session] Added ${Dt}/${Object.keys(r.claude_code_args).length} claude_code_args`);
  let Ot = r.inference_auth;
  if (!Ot?.access_token)
    throw new R(
      `Session ${s} /remote response is missing inference_auth.access_token`,
      "/remote response is missing inference_auth.access_token",
    );
  Z(`[runner:session] inference_auth set (expires_in=${Ot.expires_in_seconds}s)`);
  let Zt = s.replace(/^cse_/, "session_"),
    Ie = r.environment_variables.OTEL_RESOURCE_ATTRIBUTES ?? a.OTEL_RESOURCE_ATTRIBUTES,
    Xe =
      T > 1 && k !== void 0 && k > 0 && a.OTEL_METRICS_EXPORTER === "prometheus"
        ? {
            OTEL_METRICS_EXPORTER: "otlp",
            OTEL_EXPORTER_OTLP_METRICS_PROTOCOL: "http/json",
            OTEL_EXPORTER_OTLP_METRICS_ENDPOINT: `http://127.0.0.1:${k}/v1/metrics`,
            OTEL_EXPORTER_OTLP_METRICS_COMPRESSION: "none",
            OTEL_EXPORTER_OTLP_METRICS_TEMPORALITY_PREFERENCE: "cumulative",
            OTEL_RESOURCE_ATTRIBUTES: (Ie ? `${Ie},` : "") + `session.id=${Zt}` + (A ? `,client.platform=${A}` : ""),
          }
        : {},
    mt = Un(d),
    tt = {
      ...
      CLAUDE_SESSION_INGRESS_TOKEN_FILE: Ts(h, o),
      CLAUDE_CODE_OAUTH_TOKEN: Ot.access_token,
      CLAUDE_CODE_OAUTH_SCOPES: "user:inference user:ccr_inference user:file_upload",
      CLAUDE_CONFIG_DIR: h,
      CLAUDE_STAGE_FILE_ROOT: u,
      ...
      CLAUDE_RUNNER_CLIENT_PLATFORM: Sme(A),
      DISABLE_AUTOUPDATER: "1",
      CLAUDE_ENABLE_STREAM_WATCHDOG: "1",
      CLAUDE_RUNNER_CLAUDE_BIN: process.execPath,
      CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD: "1",
      CLAUDE_CODE_REMOTE_SESSION_ID: s,
      CLAUDE_CODE_REMOTE_SESSION_UUID: JV(s),
      CLAUDE_CODE_ACCOUNT_UUID: r.environment_variables?.CLAUDE_CODE_ACCOUNT_UUID ?? void 0,
      CLAUDE_CODE_ORGANIZATION_UUID: r.environment_variables?.CLAUDE_CODE_ORGANIZATION_UUID ?? void 0,
      CCR_SESSION_ACCOUNT_EMAIL: mt ?? void 0,
      ...ea(mt),
      ANTHROPIC_BASE_URL: r.api_base_url,
      SESSION_INGRESS_URL: r.api_base_url,
      ...(r.api_base_url.includes("staging") ? { USE_STAGING_OAUTH: "1" } : { USE_STAGING_OAUTH: void 0 }),
      ...
      ...(i?.toolConfig.gitConfig && { GIT_CONFIG_GLOBAL: m }),
  ...
  try {
    (bt = j.stdio[3]),
      bt?.on?.("error", (_) => {
        Z(`[runner:session] activity pipe error (${_.message}); falling back to stdout readline`), (bt = void 0);
      });
  } catch (_) {
    Z(
      `[runner:session] activity pipe unavailable (${_ instanceof Error ? _.message : String(_)}); falling back to stdout readline`,
    ),
      (bt = void 0);
  }
  ie(`[runner:session] ${s} child spawned pid=${j.pid} cwd=${f}`),
    xe?.("spawned"),
    P?.("startup"),
    ve?.((_) => {
      if (j.stdin && !j.stdin.destroyed) j.stdin.write(_);
    });
  let ct = new mr({
      child: j,
      sessionId: s,
      maxLifetimeMs: Be("SELF_HOSTED_RUNNER_MAX_LIFETIME_MS"),
      maxLifetimeGraceMs: Be("SELF_HOSTED_RUNNER_MAX_LIFETIME_GRACE_MS") || io,
      sigkillGraceMs: Math.min(
        Math.max(
          Be("SELF_HOSTED_RUNNER_SIGKILL_GRACE_MS") || 30000,
          e.postSessionHookTimeoutMs + (e.pushOutcomeOnRelease ? kn : 0) + cn,
        ),
        2147483647,
      ),
      sigkillTimeoutMs: Be("SELF_HOSTED_RUNNER_SESSION_STOP_GRACE_MS") || mr.SIGKILL_TIMEOUT_DEFAULT_MS,
      onDebug: Z,
      onStatus: ie,
    }),
    rt = () => ct.terminate();
  ...
  let Ue = new Map(),
    st = () => {
      let _ = 0;
      for (let N of Ue.values()) if (N !== "monitor_mcp" && N !== "monitor_ws") _++;
      return _;
    },
    Pe = null,
    H,
    ...
    Ye = () => ({
      liveTasks: Ue.size,
      liveTaskIds: Array.from(Ue, ([_, N]) => `${_}:${N}`),
      wakeupInMs: Pe !== null ? Math.max(0, Pe - Date.now()) : void 0,
      bgResultAwaitingFollowup: it,
    }),
    ...
    Xt = () => {
      Gt(),
        (Rt = setTimeout(() => {
          (Rt = void 0),
            ie(
              `[runner:session] ${s} no follow-up turn within ${$t(zt)} of the background task finishing \u2014 ` +
    ...
    ee = (_) => {
      if ((at.delete(_), dt.delete(_), Y && st() === 0 && !ge && !S)) xt();
    },
    Ee = (_) => {
      if (dt.has(_)) {
        ee(_);
    ...
    lt = () => {
      ...
      if (H !== void 0) clearTimeout(H), (H = void 0);
      P?.("turn-end");
    },
    Ve = (_) => {
      let N;
      ...
      let w = N;
      if (w.type === "control_response") {
        let re = w.response?.request_id;
        if (typeof re === "string" && re.startsWith("shr-token-")) Ne?.(re);
      }
      if (w.type === "assistant") Z("[runner:session] Activity: assistant message");
      else if (w.type === "result") Z(`[runner:session] Result: subtype=${w.subtype}`);
      ...
      if (w.type === "system" && w.subtype === "task_updated") {
        ...
        if (
          ...
          Ee(w.task_id), ae?.(st()), lt();
        return;
      }
      if (w.type === "system" && w.subtype === "task_notification") {
        if (typeof w.task_id === "string" && Ue.delete(w.task_id)) {
          if (w.status === "stopped") ee(w.task_id);
          else Ee(w.task_id);
          ae?.(st()), lt();
        }
        return;
      }
      if (w.type === "system" && w.subtype === "background_tasks_changed") {
        let z = w.tasks;
        if (Array.isArray(z))
          for (let re of z) {
            let F = re?.task_id;
            if (typeof F === "string") at.add(F);
          }
        return;
      }
      if (
        w.type === "system" &&
        w.subtype === "hook_response" &&
        w.hook_event === "SessionStart" &&
        w.outcome === "error"
      ) {
        kt?.();
        return;
      ...
      if (w.type === "assistant") {
        let z = w.message?.content;
        if (Array.isArray(z)) {
          for (let re of z)
            if (re && typeof re === "object" && re.type === "tool_use" && re.name === pa) {
              let F = re.input,
                Re = Number(F?.delaySeconds);
              if (Number.isFinite(Re)) {
                let Le = Math.min(3600, Math.max(60, Re));
                Pe = Date.now() + Le * 1000 + Oe;
              }
            }
        }
      }
  ...
  if (j.stdout) pr({ input: j.stdout }).on("line", Ve);
  ...
  if (j.stderr)
    pr({ input: j.stderr }).on("line", (N) => {
      ...
      let w = so(N, Tt);
      if (w.startsWith($Ht)) ie(`[runner:session] stderr: ${w}`);
...
`);
      if (ct.terminationRequested)
        ie(`[runner:session] ${s} child exited pid=${j.pid} code=${N} signal=${w} (interrupted \u2014 we asked)`),
          ct.descendantsReaped.then(() => _({ result: "interrupted", exitCode: N, exitSignal: w, stderrTail: z }));
      else if (w === "SIGTERM" || w === "SIGINT")
        ie(`[runner:session] ${s} child exited pid=${j.pid} code=${N} signal=${w} (interrupted)`),
        ...
        if (
          (ie(
            `[runner:session] ${s} child exited pid=${j.pid} code=${N} signal=${w} (failed) \u2014 debug log at ${U}`,
          ),
          !ft)
        )
          (ft = !0), xe?.("failed");
...
function Os() {
  ...
  return e === void 0 || e === "" ? kr : Be("SELF_HOSTED_RUNNER_STARTUP_TIMEOUT_MS");
}
var bn = 60000,
  Jn = 30000,
  Nn = 5000,
...
class mr {
  ...
  onDebug;
  onStatus;
  static SIGKILL_TIMEOUT_DEFAULT_MS = Nn;
  constructor(e) {
    if (
      ((this.child = e.child),
  ...
  reapDescendants(e) {
    ...
    let n = this.child.pid,
      r = (async () => {
        ...
        let d;
        if (e > 0) {
          let { live: h, incomplete: u } = await Wr(s);
          if (h > 0 || u) await ne(e), (d = await sr(s, "SIGKILL"));
        } else d = await sr(s, "SIGKILL");
        if (d?.incomplete) {
          this.onStatus(
        ...
        if (s.degraded || s.truncated || s.unpinned > 0) {
          this.onStatus(
            `[runner:session] ${this.sessionId} process-tree listing incomplete (partial=${s.degraded} truncated=${s.truncated}) \u2014 SIGKILLed ${o} of ${s.pinned.size} identified descendant(s) and ${f} group(s); ${s.unpinned} descendant(s) could not be identified and were not signalled; they, and any unlisted tool trees, may survive`,
          );
          return;
        }
        this.onDebug(
          d === void 0
            ...
            : `[runner:session] process tree pid=${n}: ${s.pinned.size} descendant(s) at SIGTERM (${s.unpinned} unidentifiable); ${o} outlived the child and were SIGKILLed, with ${f} tool-tree group(s)`,
        );
      })().catch(() => {});
    this.reap = jt(r, e + no).then(() => {});
  }
  ...
  terminate() {
    ...
    (this.terminating = !0), (this.terminatedAt = Date.now());
    {
      ...
      if (e !== void 0)
        this.treeSnapshot = Fr(
          { pid: e, alive: this.child.exitCode === null && this.child.signalCode === null, token: this.rootToken },
          to,
        );
      this.onDebug(`[runner:session] Abort signal received, sending SIGTERM to process group pgid=${this.child.pid}`),
...
async function gn(e, t) {
  ...
  for (;;) {
    ...
    try {
      ...
      await ne(d, t.signal), (n = Math.min(n * 2, t.maxDelayMs));
    }
  }
}
class As {
...
function us(e) {
  ...
  e.write(
    b({ type: "update_environment_variables", variables: { [e.envVar]: e.token }, ...(n && { request_id: n }) }) +
      `
`,
  ),
    e.onStatus(
...
function ls(e) {
  ...
  return (r = setTimeout(u, d)), { cancel: h };
}
function fs(e) {
...
function co(e, t, n) {
  ...
  let d =
    s?.protocol === "http:" && (s.hostname === "localhost" || s.hostname === "127.0.0.1" || s.hostname === "[::1]");
...
function lo(e, t = [], n = [], r, s, d) {
  let o = new Set(t.map((u) => u.toLowerCase())),
    f = new Map(n.map(([u, i]) => [u.toLowerCase(), i])),
    h = [];
  for (let u of e) {
    if (u.type !== "git_repository" || !u.url) continue;
    let i = Mo(u.url);
    if (d) {
      let U = po(u.url, d.mountBaseUrl, r);
      if (U) {
        h.push({
          type: "github",
          repo: `${U.owner}/${U.repo}`,
      ...
      r?.(`[runner:git] governed git: source ${u.url} is not mount-routable; using legacy handling for this source`);
    }
    let m = fo(u.url, s, r);
    if (m) {
      h.push({
    ...
    if (!A)
      throw new R(
        `mapSources: cannot extract repo slug from source URL '${u.url}' \u2014 not scp-form (user@host:path) and not a parseable URL`,
        "mapSources: cannot extract repo slug from source URL",
      );
    h.push({ type: yo(k) ? "github-ssh" : "github", repo: A, ref: u.revision, url: k, upstreamHost: i });
  }
  return h;
}
function fo(e, t, n) {
  ...
  let s = r[1],
    d = r[2];
  if (!Vn.test(s) || !Vn.test(d)) {
    n?.(
      `[runner:git] git proxy: rejecting unsafe owner/repo in ${e}; falling back to customer git auth for this source`,
    );
    return;
  }
  let f = `${t.apiBaseUrl.replace(/\/+$/, "")}/v1/session_ingress/session/${t.sessionId}/git_proxy/${s}/${d}.git`;
  return (
    n?.(`[runner:git] rewrote ${e} -> git_proxy/${s}/${d}.git (--use-anthropic-git-proxy)`),
    { url: f, owner: s, repo: d }
  );
}
var Vn = /^(?!\.{1,2}$)[a-zA-Z0-9._-]+$/;
function po(e, t, n) {
  let r = e.match(/^https:\/\/([^/@:]+)\/([^/]+)\/([^/]+?)(?:\.git)?\/?$/);
  if (!r) {
    if (e.startsWith("https://")) n?.(`[runner:git] governed git: could not parse host/owner/repo from ${e}`);
    return;
  }
  ...
  if (!/^(?!\.{1,2}$)[a-z0-9.-]+$/.test(s) || s.length > 255) {
    n?.(`[runner:git] governed git: rejecting unsafe host in ${e}; falling back to legacy handling for this source`);
    return;
  }
  if (!Vn.test(d) || !Vn.test(o)) {
    n?.(
  ...
  return (
    ...
    { url: f, owner: d, repo: o, upstreamUrl: `https://${s}/${d}/${o}` }
  );
}
function Tr() {
...
async function mo(e, t, n) {
  ...
  try {
    ...
    let h = E(f);
    if (h === "ENOENT" || h === "ENOTDIR") return;
    throw new R(
      `git-proxy: could not sanitize canonical ${e} before prep \u2014 ${f instanceof Error ? f.message : String(f)}`,
  ...
  if (d && /[\x00-\x1f]/.test(d)) throw Error("git-proxy: sanitize refused \u2014 control char in origin URL");
  let o =
    `[core]
...
`
  ...
  try {
    ...
    let h = B(r, "HEAD"),
      ...
      i = u && u.size <= 1024 ? await Cn(h, "utf-8").catch(() => "") : "",
      m = /^ref: (refs\/[A-Za-z0-9._/-]+)\n?$/.exec(i);
    if (
      !(
        m &&
        m[1]
          .split("/")
          .every(
            (k) => k.length > 0 && !k.startsWith(".") && !k.endsWith(".") && !k.endsWith(".lock") && !k.includes(".."),
          )
      ) &&
      !/^[0-9a-f]{40}([0-9a-f]{24})?\n?$/.test(i)
    )
...
async function wo(e, t, n) {
  ...
  await new Promise((r) => {
    let s = Wt("git", ["-C", e, "config", "--local", "--unset-all", "credential.helper", "^$"], {
      ...
      o = (f) => {
        if ((clearTimeout(d), f !== 0 && f !== 5 && f !== null))
          t(`[runner:session] stale git-proxy reset sweep in ${e} exited ${f}`);
        r();
      };
...
async function Eo(e, t, n, r) {
  let s = new URL(t).origin,
    d = [
      ...
      [[`credential.${s}.helper`, ZJt("CLAUDE_CODE_SESSION_ACCESS_TOKEN")], ["--add"]],
    ];
  for (let [[o, f], h] of d)
    await $s(
      ["-C", e, "config", "--local", ...h, o, f],
      (u) =>
        new R(
          `git proxy: 'git config ${o}' in ${e} exited ${u.code}: ${u.stderr}`,
...
async function $s(e, t, n, r = Wt, s = un) {
  await new Promise((d, o) => {
    ...
    let m = () => {
      ...
      T = setTimeout(
        (k, A) => {
          k.kill("SIGKILL"), A(t({ code: null, stderr: h.trim() }));
        },
        s,
        i,
        u,
...
async function ko(e, t, n, r) {
  await $s(
    ["-C", e, ...S1, "remote", "set-url", "origin", "--", t],
    (s) =>
      new R(
...
function Oo(e) {
  ...
  for (let n of e) {
    let r = wr(n.repo),
      s = t.get(r) ?? [];
    for (let d of n.branches ?? []) if (d && wS(d) && !s.includes(d)) s.push(d);
...
function ps(e, t, n) {
  ...
  if (t.length > 1) return { childCwd: t.every((s) => s.startsWith(e + Fe)) ? e : t[0], addDirs: [...t] };
...
async function Ao(e, t, n) {
  let r = [],
    s = (u, i, m) => (T) => {
      let k = E(T);
      if (k === "ENOENT" || k === "ENOTDIR") return;
      throw new ht(
        { ...i, path: u, kind: "scan-fs-error" },
        `could not be probed (${m} failed: ${T instanceof Error ? T.message : String(T)}) \u2014 refusing rather than fold a non-ENOENT error into "does not exist" (fail-closed)`,
      );
    },
    d = async (u, i) => {
      if (Bn(u)) return;
      let m = await Ce(En(u).catch(s(u, i, "realpath")), `realpath ${u}`);
      if (m !== void 0) return m;
      let T = u;
      for (let k = 0; k < 64; k++) {
        let A = await Ce(Ht(T).catch(s(u, i, "lstat")), `lstat ${T}`);
        if (A?.isSymbolicLink())
          throw new ht(
            { ...i, path: u },
            `traverses a symlink at ${T} whose target path cannot be realpath'd \u2014 a repo-shipped link (dangling, or pointing outside the workspace with a nonexistent leaf) can be retargeted at use time`,
          );
        if (A !== void 0) return;
        let U = nn(T);
        if (U === T) return;
    ...
    o = async (u, i) => {
      if (!_s(u)) return;
      let m = u.split(Fe).filter((k) => k.length > 0);
      if (!m.includes("..")) return;
      let T = u.startsWith(Fe) ? Fe : "";
      for (let k of m) {
        if (k === ".") continue;
        if (k === "..") {
          T = nn(T);
        ...
        if (A?.isSymbolicLink())
          throw new ht(
            { ...i, path: u },
            `contains a '..' after symlink ${T} \u2014 the kernel resolves the symlink before the '..' at bind time, diverging from the lexically-collapsed form the confine check saw`,
          );
        if (A === void 0) return;
      }
    },
  ...
  for (let u of new Set([e, ...t])) {
    ...
    if (m.isSymbolicLink())
      throw new ht(
        { path: i, sourceFile: i, raw: ".claude", kind: "permissions.additionalDirectories" },
        "is a symlink \u2014 refusing to follow (the settings read below would escape the workspace)",
      );
    for (let T of ["settings.json", "settings.local.json"]) {
      let k = B(i, T),
        A = await Ce(
          Ht(k).catch((P) => {
            if (X(P)) return;
            throw new ht(
              { path: k, sourceFile: k, raw: T, kind: "scan-fs-error" },
              `could not be lstat'd (${P instanceof Error ? P.message : String(P)}) \u2014 refusing rather than skip the scan`,
            );
          }),
          `lstat ${k}`,
        );
      ...
      if (A.isSymbolicLink())
        throw new ht(
          { path: k, sourceFile: k, raw: T, kind: "permissions.additionalDirectories" },
        ...
        throw new ht(
          {
            path: k,
            sourceFile: k,
            raw: `env: {${Object.keys(M.env).slice(0, 3).join(", ")}${Object.keys(M.env).length > 3 ? ", \u2026" : ""}}`,
      ...
      for (let [P, ae, Se] of [
        ...
        if (ae === Se)
          throw new ht(
            { path: k, sourceFile: k, raw: `${P}: ${String(Se)}`, kind: "operator-posture override" },
            `negates the operator's ${P} posture (projectSettings overrides userSettings in the merged read). Remove it from the repo settings; operator posture belongs in the user-level settings.json`,
          );
      let be = M?.permissions,
        Z = Array.isArray(be?.allow) ? be.allow : [];
      for (let P of Z) {
        if (typeof P !== "string") continue;
        let ae = Ur(P);
        if (![Kt, ar, mc].includes(ae.toolName)) continue;
        if (ae.ruleContent === void 0) {
          r.push({ path: Fe, sourceFile: k, raw: P, kind: "permissions.allow (bare write-tool rule)" });
          continue;
        }
        let { root: Se } = ILe(ae.ruleContent, u);
        {
          let Ne = { sourceFile: k, raw: P, kind: "permissions.allow (sandbox-arm projection)" },
            xe = gjt(ae.ruleContent, u),
            Qe = f(xe);
          r.push({ ...Ne, path: Qe });
          let kt = await d(Qe, Ne);
          if (kt !== void 0 && kt !== Qe) r.push({ ...Ne, path: kt });
          await o(h(xe), Ne);
        }
        if (Se === null) continue;
        let fe = { sourceFile: k, raw: P, kind: "permissions.allow" };
        r.push({ ...fe, path: Se });
        let ve = await d(Se, fe);
        if (ve !== void 0 && ve !== Se) r.push({ ...fe, path: ve });
      }
      for (let [P, ae] of [
        ["allowWrite", M?.sandbox?.filesystem?.allowWrite],
        ["allowRead", M?.sandbox?.filesystem?.allowRead],
      ]) {
        if (!Array.isArray(ae)) continue;
        for (let Se of ae) {
          if (typeof Se !== "string") continue;
          let fe = { sourceFile: k, raw: Se, kind: `sandbox.filesystem.${P}` },
            ve,
            Ne;
          try {
            (Ne = hjt(Se, u)), (ve = f(Ne));
            ...
            throw new ht(
              { ...fe, path: Se },
              `could not be resolved (${Qe instanceof Error ? Qe.message : String(Qe)}) \u2014 refusing rather than drop and risk sandbox-init failure child-side`,
            );
          }
          r.push({ ...fe, path: ve });
          let xe = await d(ve, fe);
          if (xe !== void 0 && xe !== ve) r.push({ ...fe, path: xe });
          await o(h(Ne), fe);
        }
      }
      let ie = be?.additionalDirectories;
      if (!Array.isArray(ie)) continue;
      for (let P of ie) {
        if (typeof P !== "string") continue;
        let ae = { sourceFile: k, raw: P, kind: "permissions.additionalDirectories" },
          Se;
        try {
          Se = wn(gt(P, e));
        } catch (ve) {
          throw new ht(
            { ...ae, path: String(P) },
            `could not be resolved (${ve instanceof Error ? ve.message : String(ve)}) \u2014 refusing rather than drop and risk sandbox-init failure child-side`,
          );
        }
        r.push({ ...ae, path: Se });
        let fe = await d(Se, ae);
        if (fe !== void 0 && fe !== Se) r.push({ ...ae, path: fe });
        await o(h(P), ae);
      }
    }
  }
...
function yn(e, t, n, r) {
  let s = e.toLowerCase(),
    d = (o) => (o.endsWith(Fe) ? o : o + Fe);
  for (let o of [n, ...r]) {
    let f = o.toLowerCase();
    if (s === f || s.startsWith(d(f)) || f.startsWith(d(s)))
      throw new R(
...
async function No(e, t, n, r, s, d, o = Wt) {
  ...
  let f = {
      ...process.env,
      ...KK,
      ...(s ? { GIT_CONFIG_GLOBAL: "/dev/null", CLAUDE_CODE_SESSION_ACCESS_TOKEN: void 0 } : void 0),
    },
    h = (i, m, T, k = []) =>
      new Promise((A) => {
        let U = !1,
          ...
          be = "";
        M.stderr?.on("data", (P) => (be += String(P)));
        let Z = () => {
            M.kill("SIGKILL"), q("unsettled");
          },
          ie = setTimeout(
            (P, ae, Se, fe, ve, Ne) => {
              P(`[runner:session] ${ae} in ${Se} timed out after ${ve}ms \u2014 continuing on current HEAD`),
                fe.kill("SIGKILL"),
                Ne("unsettled");
            },
            T,
        ...
        M.on("close", (P) => {
          ...
          M.on("error", (P) => {
            n(`[runner:session] ${m} spawn failed: ${P} \u2014 continuing on current HEAD`), q("unsettled");
          });
      });
  if (!d) {
    if (
      (await h(["switch", "-q", "-C", t], `switch -C '${t}'`, un, ["-c", "alias.switch="])) !== "failed" ||
      r?.aborted
    )
      return;
    ...
    if (
      (await new Promise((k) => {
        let A = !1,
          ...
          q = s(
            ...
            ],
            {
              ...
              env: {
                ...process.env,
                ...KK,
                ...(o || u ? { GIT_CONFIG_GLOBAL: "/dev/null" } : void 0),
                ...(f ? { [Lrt]: f.getToken(), CLAUDE_CODE_SESSION_ACCESS_TOKEN: void 0 } : void 0),
                ...(d && { GIT_ALLOW_PROTOCOL: `file:${KK.GIT_ALLOW_PROTOCOL}` }),
          ...
          M = "";
        q.stderr?.on("data", (ie) => (M += String(ie)));
        let be = () => {
          if (q.pid) WE(q.pid);
          U(null);
        };
        ...
        q.on("close", (ie) => {
          if (ie !== 0) n(`[runner:session] fetch outcome branch in ${e} exited ${ie}: ${M.trim()} \u2014 continuing`);
          U(ie);
        }),
          q.on("error", (ie) => {
            n(`[runner:session] fetch outcome branch spawn failed: ${ie} \u2014 continuing`), U(null);
          });
      })) === 0
    )
      h++;
  }
  if (t.length > 0) n(`[runner:session] fetched ${h}/${t.length} outcome branch(es) in ${e}`);
}
function Ns(e) {
  let t = e.url ?? "",
    n = e.getAuthToken ? e.getAuthToken() : e.token,
    r = e.getAuthToken ? t : eQt(t, e.token),
    s;
  try {
    let f = new URL(t);
    if (f.protocol === "https:" || f.protocol === "http:") s = f.origin;
  } catch {}
  let d = process.env.HTTPS_PROXY || process.env.https_proxy || process.env.ALL_PROXY || process.env.all_proxy || "",
    o = [...S1];
  if (s) {
    if (
      ...
      o.push("-c", `http.proxy=${d}`), o.push("-c", `http.${s}.proxy=${d}`), o.push("-c", `http.${t}.proxy=${d}`);
    if (n)
      if (e.getAuthToken) o.push(...lxt(t));
  ...
  return {
    ...
    env: {
      ...
      GIT_TERMINAL_PROMPT: "0",
      ...KK,
      ...(t.startsWith("file://") && { GIT_ALLOW_PROTOCOL: `file:${KK.GIT_ALLOW_PROTOCOL}` }),
      GIT_SSH_COMMAND: `${process.env.GIT_SSH_COMMAND || "ssh"} -o BatchMode=yes -o ConnectTimeout=30`,
      SELF_HOSTED_RUNNER_HOST_CONFIG_DIR: void 0,
      ...(n && { GIT_CONFIG_GLOBAL: "/dev/null", [Lrt]: e.getAuthToken ? n : void 0 }),
    },
  };
}
...
async function hs(e, t, n = un) {
  return new Promise((r) => {
    let s = !1,
      ...
      f = "";
    o.stdout?.on("data", (u) => (f += String(u)));
    let h = setTimeout(
      (u, i) => {
        u.kill("SIGKILL"), i(void 0);
      },
      n,
      o,
      d,
    );
    o.on("close", (u) => d(u === 0 ? f.trim() : void 0)), o.on("error", () => d(void 0));
  });
}
async function Do(e, t, n, r, s) {
  ...
  return new Promise((h) => {
    let u = !1,
      ...
      T = "";
    m.stderr?.on("data", (A) => (T += String(A)));
    let k = setTimeout(
      (A, U, q, M, be, Z) => {
        A(`[runner:session] push-on-release '${U}' from ${q} timed out after ${M}ms (best-effort)`),
          be.kill("SIGKILL"),
          Z();
      },
      r,
    ...
    m.on("close", (A) => {
      if (A === 0) s(`[runner:session] push-on-release pushed '${t}' to source remote from ${e}`);
      else
        s(
          `[runner:session] push-on-release '${t}' from ${e} exited ${A}: ${Mrt(T, f.authURL, f.token).trim()} (best-effort)` +
            (d
      ...
      m.on("error", (A) => {
        s(`[runner:session] push-on-release '${t}' spawn failed: ${A} (best-effort)`), i();
      });
  });
}
  ...
  let f = (u, i) =>
    new Promise((m) => {
      let T = !1,
        ...
        U = "";
      A.stderr?.on("data", (M) => (U += String(M)));
      let q = setTimeout(
        (M, be, Z, ie, P, ae) => {
          Z(`[runner:session] ${ie} in ${P} timed out after ${ae}ms (best-effort cleanup)`),
            M.kill("SIGKILL"),
            be(null);
        },
        o,
      ...
      A.on("close", (M) => {
        if (M !== 0) r(`[runner:session] ${i} in ${e} exited ${M}: ${U.trim()} (best-effort cleanup)`);
        k(M);
      }),
        A.on("error", (M) => {
          r(`[runner:session] ${i} spawn failed: ${M} (best-effort cleanup)`), k(null);
        });
    });
  if (n) await f(["checkout", "--detach"], "checkout --detach");
  if ((await f(["branch", "-D", "--end-of-options", t], `branch -D '${t}'`)) === 0)
...
async function ms(e) {
  ...
  if (m.aborted)
    return (
      u(`[runner:session] Skipping failure-result post for ${r} \u2014 session was deliberately aborted`), "skipped"
    );
  let T = () => t.postWorkerEvents(n, r, s, d, ta({ exitCode: o, exitSignal: f, stderrTail: h }), m);
  try {
    return await T(), u(`[runner:session] Posted failure result for ${r} (exitCode=${o} signal=${f})`), "posted";
  } catch (k) {
    if (_r(k))
      return (
        u(`[runner:session] failure-result rejected (session gone) \u2014 ${r} is archived or deleted server-side`),
        "session_gone"
      );
    if (Sn(k))
      return (
    ...
    let A = Rl(k instanceof Error ? k.message : String(k));
    i(`[runner:stuck] Failed to post failure-result for ${r} (attempt 1): ${A} \u2014 retrying in 2s`),
      await ne(2000, m);
  }
  try {
    return (
      await T(), u(`[runner:session] Posted failure result for ${r} on retry (exitCode=${o} signal=${f})`), "posted"
    );
  } catch (k) {
    if (_r(k))
      return (
    ...
    let A = Rl(k instanceof Error ? k.message : String(k));
    return (
      i(`[runner:stuck] Failed to post failure-result for ${r} after retry: ${A} \u2014 UI spinner may not stop`),
      "post_failed"
    );
  }
}
...
async function Fo(e) {
  let { apiClient: t, apiBaseUrl: n, sessionId: r, sessionToken: s, workerEpoch: d } = e,
    ...
    f;
  try {
    let T = await Ce(Er(e.debugFile), `stat ${e.debugFile}`),
      k = Math.max(0, T.size - 262144),
      A = await Ce(Di(e.debugFile, "r"), `open ${e.debugFile}`);
    try {
      let U = Buffer.alloc(Math.min(T.size, 262144)),
        { bytesRead: q } = await Ce(A.read(U, 0, U.length, k), `read ${e.debugFile}`);
      f = U.toString("utf-8", 0, q);
    } finally {
      await A.close().catch(() => {});
    }
  } catch {
...
`)
  ...
  let i = new Date().toISOString(),
    m = u.map((T) => ({ timestamp: i, fields: { message: T } }));
  await t.forwardDiagnostics(n, r, s, d, m),
    e.onDebug(`[runner:session] forwarded ${u.length} debug log lines to /worker/diagnostics`);
}
function Ho(e) {
  return { type: "system", uuid: Xn(), subtype: "init_milestone", message: e };
}
function jn(e, t, n, r) {
  return {
...
function Go(e, t, n, r) {
  return jn("clone", "completed", `${e} ${t} (${n}ms)`, {
    ...r,
    step_detail: e,
    clone_phase: t,
...
function jo(e, t) {
  ...
  if (!Array.isArray(n)) {
    t(`[runner:session] worktree_checkpoint_hint: unusable shape (${vn(e)}) \u2014 ignoring`);
    return;
  }
  let r = [],
    s = 0;
  for (let d of n.slice(0, Kn)) {
    let o = d && typeof d === "object" ? d.name : void 0,
      f = d && typeof d === "object" ? d.base_sha : void 0;
    if (typeof o === "string" && o !== "." && o !== ".." && Ko.test(o) && typeof f === "string" && xs.test(f))
      r.push({ name: o, baseSha: f });
    else s++;
  }
  if (s > 0 || n.length > Kn)
    t(
      `[runner:session] worktree_checkpoint_hint: ${r.length} usable entr${r.length === 1 ? "y" : "ies"}, ${s} dropped (no/malformed name or base_sha)${n.length > Kn ? `, ${n.length - Kn} beyond the cap ignored` : ""}`,
    );
  return r.length > 0 ? r : void 0;
}
function Xo({ gateOn: e, workerEpoch: t, hintedBeforeRegister: n, refetch: r, onDebug: s, capMs: d = zo }) {
  if (!e || !((t ?? 0) > 1) || !n) return Promise.resolve(void 0);
  return cu(Promise.resolve().then(r), d, "post-registration /remote re-read").then(
...
var Vo = 1800;
async function qo({ useWorktrees: e, workerEpoch: t, source: n, checkpoint: r, onDebug: s, read: d = qr }) {
  return;
}
var Zo = 30000;
...
function ta({ exitCode: e, exitSignal: t, stderrTail: n }) {
  ...
  if (n.trim().length > 0) {
    let h = n.length > ds ? `\u2026${n.slice(-ds)}` : n;
    d += `
...
var Ms = { POLL: "POLL_WAKE_SOURCE_POLL", SSE: "POLL_WAKE_SOURCE_SSE", LOCAL: "POLL_WAKE_SOURCE_LOCAL" };
class yr {
        ...
        try {
          let A = await fetch(f, {
            ...
            headers: {
              ...
              "x-self-hosted-runner-version": Tie,
            },
            ...Ri({ url: f }),
            signal: i.signal,
          });
          if (!A.ok || !A.body)
            throw (
              (d(`[runner:hints] stream HTTP ${A.status} \u2014 degrading to poll, retrying`),
              R1(A, { timeoutMs: qot }),
              Error(`HTTP ${A.status}`))
            );
          if ((d("[runner:hints] stream connected"), await oa(A.body, s, i))) u = 0;
          d("[runner:hints] stream ended \u2014 reconnecting");
        } catch (A) {
          if (h || o.aborted) break;
          if (u === 0) d(`[runner:hints] stream error: ${l(A)}`);
        }
        u++;
        let T = Math.min(ra, na * 2 ** u),
...
`)) >= 0
        ...
        let T = d.slice(0, m).replace(/\r$/, "");
        if (((d = d.slice(m + 1)), T.startsWith("event:") && T.slice(6).trimStart() === "work_available")) t();
      }
    }
    return o;
  } finally {
...
function $r() {
  ...
  return e !== void 0 && e !== ""
    ? Be("SELF_HOSTED_RUNNER_DRAIN_WAIT_MS")
    : Be("SELF_HOSTED_RUNNER_DRAIN_WAIT_BG_TASKS_MS");
}
function Sa() {
...
function va(e, t) {
  ...
  if (Number.isNaN(n) || n < 0 || n > 65535)
    throw Error(`SELF_HOSTED_RUNNER_HEALTH_PORT must be an integer in [0, 65535] (0 disables), got: ${b(e)}`);
  return n;
}
function Oa(e) {
  if (bo(e)) return !1;
...
function Aa(e) {
  ...
  throw Error(`SELF_HOSTED_RUNNER_CONFINE_REPO_SETTINGS must be one of enforce/warn/off (got: ${b(e)})`);
}
function $a(e) {
  let t = process.env.SELF_HOSTED_RUNNER_BASE_DIR || void 0,
    n = {
      ...
      gitSshRewriteHosts: [],
      gitHostRewrites: [],
      useAnthropicGitProxy: Me(process.env.CLAUDE_RUNNER_USE_GIT_PROXY),
      configureGit: Me(process.env.SELF_HOSTED_RUNNER_CONFIGURE_GIT),
      pushOutcomeOnRelease: Me(process.env.SELF_HOSTED_RUNNER_PUSH_OUTCOME_ON_RELEASE),
  ...
  if (process.env.SELF_HOSTED_RUNNER_HOOKS_DIR)
    process.env.SELF_HOSTED_RUNNER_HOOKS_DIR = er(process.env.SELF_HOSTED_RUNNER_HOOKS_DIR);
    ...
    switch (d) {
      ...
      case "--environment-secret-file":
        if (d === "--pool-secret-file")
          console.error("[runner:warn] --pool-secret-file is deprecated; use --environment-secret-file");
        if (o) (n.poolSecretFile = o), s++;
        break;
      case "--capacity":
        if (o) {
          let f = ol(o);
          if (Number.isNaN(f) || f < 1) throw Error(`--capacity must be a positive integer, got: ${o}`);
          (n.capacity = f), s++;
        }
        break;
      case "--base-dir":
        if (o) (n.baseDir = er(o)), (n.baseDirSource = "flag"), s++;
        break;
      case "--exec-path":
        if (o) (n.execPath = o), s++;
        break;
      case "--hooks-dir":
        if (o) (process.env.SELF_HOSTED_RUNNER_HOOKS_DIR = er(o)), s++;
        break;
      ...
      case "--git-host-rewrite": {
        ...
        let f = o.indexOf("=");
        if (f <= 0 || f === o.length - 1) throw Error(`--git-host-rewrite requires <from>=<to>, got: ${o}`);
        let h = o.slice(0, f).toLowerCase(),
          u = o.slice(f + 1);
        if (h.includes("://") || h.includes("/") || u.includes("://") || u.includes("/"))
          throw Error(
            `--git-host-rewrite expects bare hostnames (e.g. ext.example.com=int.example.com), not URLs. Got: ${o}`,
          );
        if (n.gitHostRewrites.some(([i]) => i === h)) throw Error(`duplicate --git-host-rewrite for '${h}'`);
        n.gitHostRewrites.push([h, u]), s++;
        break;
      }
      ...
      case "--kill-session-after-min": {
        if (o) {
          let f = Number(o);
          if (!Number.isFinite(f) || f < 0 || f > Et)
            throw Error(
              `--kill-session-after-min must be a non-negative number of minutes (0 disables, max ${Et}), got: ${o}`,
            );
          (process.env.SELF_HOSTED_RUNNER_MAX_LIFETIME_MS = String(f * 60 * 1000)),
      ...
      case "--exit-if-unused-min": {
        if (o) {
          ...
          if (!Number.isFinite(f) || f < 0 || f > Et)
            throw Error(
              `--exit-if-unused-min must be a non-negative number of minutes (0 disables, max ${Et}), got: ${o}`,
            );
          (process.env.SELF_HOSTED_RUNNER_IDLE_SHUTDOWN_MS = String(f * 60 * 1000)),
      ...
      case "--session-stop-grace-sec": {
        let f = e[++s],
          h = Number(f);
        if (!f || !Number.isFinite(h) || h <= 0)
          throw Error(`--session-stop-grace-sec must be a positive number of seconds, got: ${f}`);
        process.env.SELF_HOSTED_RUNNER_SESSION_STOP_GRACE_MS = String(h * 1000);
        break;
      }
      ...
      case "--post-session-hook-timeout-sec": {
        let f = e[++s],
          h = Number(f);
        if (!f || !Number.isFinite(h) || h <= 0)
          throw Error(`--post-session-hook-timeout-sec must be a positive number of seconds, got: ${f}`);
        process.env.SELF_HOSTED_RUNNER_POST_SESSION_HOOK_TIMEOUT_MS = String(h * 1000);
        break;
      }
      ...
      case "--drain-wait-sec": {
        ...
        let f = e[++s],
          h = Number(f);
        if (!f || !Number.isFinite(h) || h < 0 || h > 86400)
          throw Error(`${d} must be a non-negative number of seconds (max 86400), got: ${f}`);
        process.env.SELF_HOSTED_RUNNER_DRAIN_WAIT_MS = String(h * 1000);
        break;
      }
      case "--drain-grace-sec": {
        if (o) {
          let f = Number(o);
          if (!Number.isFinite(f) || f < 0 || f > Cr)
            throw Error(
              `--drain-grace-sec must be a non-negative number of seconds (0 = immediate, max ${Cr}), got: ${o}`,
            );
          (process.env.SELF_HOSTED_RUNNER_DRAIN_GRACE_MS = String(f * 1000)), s++;
        }
        break;
      }
      case "--release-idle-session-min": {
        if (o) {
          ...
          if (!Number.isFinite(f) || f < 0 || f > Et)
            throw Error(
              `--release-idle-session-min must be a non-negative number of minutes (0 disables, max ${Et}), got: ${o}`,
            );
          (process.env.SELF_HOSTED_RUNNER_SESSION_IDLE_MS = String(f * 60 * 1000)),
      ...
      case "--startup-timeout-min": {
        if (o) {
          ...
          if (!Number.isFinite(f) || f < 0 || f > Et)
            throw Error(
              `--startup-timeout-min must be a non-negative number of minutes (0 disables, max ${Et}), got: ${o}`,
            );
          (process.env.SELF_HOSTED_RUNNER_STARTUP_TIMEOUT_MS = String(f * 60 * 1000)),
      ...
      case "--defer-shutdown-max-min": {
        let f = e[++s],
          h = Number(f);
        if (f === void 0 || f.trim() === "" || !Number.isFinite(h) || h < 0 || h > Et)
          throw Error(`${d} must be a non-negative number of minutes (0 disables, max ${Et}), got: ${f}`);
        (process.env.SELF_HOSTED_RUNNER_DEFER_SHUTDOWN_MAX_MS = String(h * 60 * 1000)),
      ...
      case "--retire-at": {
        let f = e[++s],
          h = Number(f);
        if (!f || !Number.isSafeInteger(h) || h < Ys || h > Vs)
          throw Error(
            `--retire-at must be an absolute Unix timestamp in whole SECONDS (e.g. the output of 'date +%s' plus the runner's intended lifetime \u2014 not milliseconds, not a duration), got: ${f}`,
          );
        process.env.SELF_HOSTED_RUNNER_RETIRE_AT = String(h);
        break;
      }
      ...
      case "--proxy-authorization-file":
        if (o === void 0 || o.startsWith("--"))
          throw Error("--proxy-authorization-command / --proxy-authorization-file requires a value");
      ...
      default:
        ...
        if (d === "") throw Error("empty argument \u2014 a flag value may be unset (failed env/K8s substitution?)");
        if (d !== void 0) throw Error(`unexpected argument '${d}' \u2014 this command takes no positional arguments`);
        break;
    }
  }
...
async function La(e) {
  if (e.poolSecretFile) {
    let r;
    try {
      r = await cu(Ws(e.poolSecretFile, { encoding: "utf-8" }), Ta, `environment-secret read from ${e.poolSecretFile}`);
    } catch (s) {
      let d = l(s);
      if (d.includes("timed out after"))
        console.error(`[runner:fatal] ${d} \u2014 check CSI/secret mount health`), process.exit(1);
...
function Pa(e) {
  ...
  return { execPath: process.execPath, execArgs: Wl() ? [] : [process.argv[1]] };
}
async function dc(e) {
...
Runtime:
  ...
  --drain-wait-sec <n>        On SIGTERM/SIGINT, wait up to N seconds for each session's in-flight
                              ...
                              default: ${Jn / 1000}s; 0 or an unusable value falls back
...
Runner lifecycle:
  ...
  --defer-shutdown-max-min <m>
                              ...
                              gets up to 60s), and anything STILL attached ${(tr + cn) / 1000}s later (that 60s + a 15s
                              ...
                              (terminationGracePeriodSeconds etc.) to at least M minutes + ${(tr + cn) / 1000}s (that post-ceiling
                              ...
                              Max: ${Et}. [env: SELF_HOSTED_RUNNER_DEFER_SHUTDOWN_MAX_MS, in ms]
...
Per-session watchdogs:
  ...
  --startup-timeout-min <n>       Release a session slot if the child has not completed initialization
                                  ...
                                  takes over. Default: 15. 0 disables. Max: ${Et}.
...
Debug:
  ...
  try {
    if (
      ((t = $a(e)),
      (r = gIn({ command: t.proxyAuthorizationCommand, file: t.proxyAuthorizationFile })),
      process.env.SELF_HOSTED_RUNNER_HOOKS_DIR)
    )
      kZe("--hooks-dir");
    Na(t.baseDirSource), _G(), (n = await La(t)), await Pr(t.baseDir);
  } catch (S) {
    console.error(`error: ${Rl(l(S))}
Run 'claude self-hosted-runner --help' for usage.`),
  ...
  let s = t.logLevel === "debug",
    ...
    o;
  if (t.logFile)
    (o = da(t.logFile, { flags: "a", mode: 384 })),
      o.on("open", (S) => {
        ua(S, 384, () => {});
      }),
      o.on("error", (S) => {
        console.error(`${d()} [runner:warn] log-file write failed (${l(S)}); continuing stdout-only`), (o = void 0);
      });
  let f = (S) => {
      o?.write(
        S +
...
`,
    ...
    h = async () => {
      if (!o) return;
      let S = o;
      (o = void 0), await cu(new Promise((x) => S.end(x)), 500, "[runner:exit] log-file flush").catch(() => {});
    },
    u = (S) => {
      if (s) {
        let x = `${d()} [DEBUG] ${Rl(S)}`;
    ...
    i = (S) => {
      let x = `${d()} [self-hosted-runner] ${Rl(S)}`;
      console.log(x), f(x);
    };
  ...
  for (let S of [
    ...
    if (process.env[S] !== void 0)
      i(
        `[runner:warn] ${S} is set but is not a setting this runner ` +
  ...
  let m = br(
      Be("SELF_HOSTED_RUNNER_SESSION_STOP_GRACE_MS") || Nn,
      Be("SELF_HOSTED_RUNNER_POST_SESSION_HOOK_TIMEOUT_MS") || bn,
      $r(),
      t.pushOutcomeOnRelease ? kn : 0,
    ),
    T = `[runner] This runner needs up to ${m}s to stop the Claude process and run the post-session hook on shutdown, and force-exits after ${m}s (a session release already in flight when shutdown begins can add up to ${Ar / 1000}s, normally well under 1s, before the runner deregisters). Configure your process supervisor's stop timeout to at least ${m}s (e.g. terminationGracePeriodSeconds on Kubernetes, stop_grace_period on Docker Compose, TimeoutStopSec on systemd, or your platform's equivalent).`;
  i(T);
  let k = Be("SELF_HOSTED_RUNNER_DEFER_SHUTDOWN_MAX_MS"),
    A = Be("SELF_HOSTED_RUNNER_SESSION_IDLE_MS"),
    U = Math.max($r(), tr) + cn,
    q =
      k > 0
  ...
  let be = process.env.SELF_HOSTED_RUNNER_HOOKS_DIR,
    Z = t.execPath;
  if (!Z) {
    let S = await HZe(be, "command");
    if (S) (Z = S), i(`[runner:hook] using command hook ${S}`);
  }
  let { execPath: ie, execArgs: P } = Pa(Z);
  i(`Connecting to ${t.apiUrl} (capacity=${t.capacity}, baseDir=${t.baseDir}, execPath=${[ie, ...P].join(" ")})`);
  ...
  if (t.debugTokenDir)
    await Or(t.debugTokenDir, { recursive: !0, mode: 448 })
      ...
      .catch((S) => {
        i(`[runner:debug] failed to create debug token dir (best-effort): ${S}`);
      });
  let Se;
  if (r)
    try {
      (Se = await hIn(r, { onStatus: i, onDebug: u })), y("self_hosted_egress_proxy_auth");
    } catch (S) {
      p("self_hosted_egress_proxy_auth", "listener_start_failed"),
        i(`[runner:fatal] could not start the proxy-authorization listener: ${l(S)}`),
        await h(),
        process.exit(1);
    }
  let fe = CZe({ baseUrl: t.apiUrl, poolSecret: n, onDebug: u }),
    ve = t.clientLabel ?? fa();
  if (t.lockToAccountId) i(`Registering locked to account: ${t.lockToAccountId}`);
  let Ne, xe;
  try {
    let S = await gn(() => fe.registerRunner(ve, t.lockToAccountId), {
      initialDelayMs: 1000,
      maxDelayMs: 16000,
      maxAttempts: 5,
      shouldRetry: $ve,
      onRetry: (x, he) => {
        i(`RegisterRunner attempt ${x} transient failure (${he instanceof Error ? he.message : he}) \u2014 retrying`);
      },
    });
    (Ne = S.runner_id), (xe = S.runner_token), y("self_hosted_register");
  } catch (S) {
    let x = S instanceof Error ? S.message : String(S);
    if (qs(S))
      p("self_hosted_register", "self_hosted_register_auth_failed"),
        i(
          `[runner:fatal] RegisterRunner auth failed \u2014 environment secret invalid or revoked. Check --environment-secret-file or SELF_HOSTED_RUNNER_ENVIRONMENT_SECRET. (${x})`,
        );
    else p("self_hosted_register", "self_hosted_register_failed"), i(`[runner:fatal] RegisterRunner failed: ${x}`);
    await h(), process.exit(1);
  }
  i(`Registered: runner_id=${Ne}`);
  let Qe = new AbortController(),
      ...
      let x = vt.current,
        he =
          x === void 0
            ...
            : `${x.sessionIdle.size} active session(s), ${Q([...x.sessionIdle.values()], (ge) => ge !== null)} of them idle`;
      return `(${S}; uptime ${$t(process.uptime() * 1000)}; ${he})`;
    },
    Pt = (S) => {
      if (je === "running" && k > 0 && Bt) {
        if (
          ((je = "deferring"),
          i(
    ...
    Dt = (S, x = "signal") => {
      if (je === "draining") {
        ...
        if (ge > 0)
          i(
            `[runner] Forced shutdown with ${ge} post-session hook(s) still running \u2014 they continue in their own process group, but their output is no longer captured and the runner's timeout budget no longer applies.`,
          );
        else i("Forced shutdown");
        h().finally(() => process.exit(1));
        return;
      }
      (je = "draining"),
        i(
          x === "signal"
            ? "Received shutdown signal, draining active sessions... " + et(S)
            : `[runner:shutdown] ${S} \u2014 draining active sessions... ` + et("no new signal"),
        ),
        i(T);
      let he = q5t();
      if (he > 0)
        i(
          `[runner] ${he} post-session hook(s) still running \u2014 waiting for them within the budget above. Another SIGTERM force-exits the runner immediately; make sure your supervisor's stop timeout covers the full budget so hooks are not cut short.`,
        );
      Qe.abort();
    },
    Ot = () => Pt("SIGTERM"),
    Zt = () => Pt("SIGINT");
  process.on("SIGTERM", Ot), process.on("SIGINT", Zt);
  let Ie = { runnerToken: xe };
  if (t.debugTokenDir) await Gs(t.debugTokenDir, "runner_token.jwt", xe, i);
  let Xe = Fn({
    getAccessToken: async () => {
      let { token: S } = await fe.refreshToken(Ie.runnerToken);
      return S;
    },
    onRefresh: (S, x) => {
      if (t.debugTokenDir) Gs(t.debugTokenDir, "runner_token.jwt", x, i);
      (Ie.runnerToken = x), u("[runner:main] runner_token refreshed");
    },
  ...
  if (t.useAnthropicGitProxy) {
    if (t.capacity > 1)
      i(
        ...
        await h(),
        process.exit(1);
    if (Zn.CLAUDE_RUNNER_TRUST_CANONICAL_PREWARM && Be("SELF_HOSTED_RUNNER_DRAIN_GRACE_MS") > 0)
      i(
        ...
        await h(),
        process.exit(1);
    let { parseGitVersion: S, readGitVersion: x } = await import("/$bunfs/root/chunk-d5eky037.js"),
      he = await x(),
      ge = S(he);
    if (!ge || ge[0] < 2 || (ge[0] === 2 && ge[1] < 32))
      i(
        `[runner:fatal] --use-anthropic-git-proxy requires git >= 2.32 (found ${he.trim() || "<none>"}). Older git silently ignores GIT_CONFIG_GLOBAL, which this mode relies on for cross-session isolation.`,
      ),
        await h(),
        process.exit(1);
    mt = process.env.GIT_CONFIG_GLOBAL || mn(vr(), ".gitconfig");
    let me = mt,
      pe = await ca(me).catch(() => {
        return;
      });
    if (pe && (pe.isCharacterDevice() || pe.isBlockDevice()))
      i(
        `[runner:fatal] --use-anthropic-git-proxy requires a writable global git config, but GIT_CONFIG_GLOBAL resolves to ${me} which is not a regular file. The proxy flag writes its credential helper there and restores it at each session start. Unset GIT_CONFIG_GLOBAL or point it at a regular file path.`,
      ),
        await h(),
        process.exit(1);
    let Oe = process.env.XDG_CONFIG_HOME || mn(vr(), ".config");
    i(
  ...
  if (t.configureGit)
    try {
      let { configureGitForSigning: S } = await import("/$bunfs/root/chunk-d5eky037.js");
      await S({ baseDir: t.baseDir, execPath: process.execPath, onStatus: i });
    } catch (S) {
      i(`[runner:fatal] --configure-git failed: ${l(S)}. Check git is on PATH and ~/.gitconfig is writable.`),
        await h(),
        process.exit(1);
    }
  if (t.pushOutcomeOnRelease)
    try {
      let { parseGitVersion: S } = await import("/$bunfs/root/chunk-d5eky037.js"),
        x = S(
          aa("git", ["--version"], {
            encoding: "utf-8",
            cwd: void 0,
      ...
      if (x !== null && (x[0] < 2 || (x[0] === 2 && x[1] < 29)))
        i(
          `[runner:warn] --push-outcome-on-release: git ${x[0]}.${x[1]} detected. The drain-push works on any version, but resuming from the preserved branch requires git >= 2.29 (--no-write-fetch-head). Upgrade git for full behavior.`,
        );
    } catch {}
  let tt, j;
  if (t.configureGit) {
    let { coauthorHookStubs: S, codeSignArtifacts: x } = await import("/$bunfs/root/chunk-d5eky037.js");
    (tt = S(t.baseDir)), (j = x(t.baseDir, process.execPath));
  }
  let bt, ct;
  if (t.useAnthropicGitProxy)
    try {
      let {
        configureGitProxyCredential: S,
        GIT_PROXY_CRED_HELPER_CONTENT: x,
        gitProxyCredHelperPath: he,
      } = await import("/$bunfs/root/chunk-d5eky037.js");
      await S({ baseDir: t.baseDir, apiBaseUrl: t.apiUrl, onStatus: i }),
        (bt = await Ws(mt, "utf-8").catch(() => "")),
        (ct = { path: he(t.baseDir), content: x });
    } catch (S) {
      i(`[runner:fatal] --use-anthropic-git-proxy failed: ${l(S)}. Check git is on PATH and ~/.gitconfig is writable.`),
        await h(),
        process.exit(1);
    }
  let rt = {
      ...
      gitProxyGlobalConfigPath: mt,
      gitProxyGlobalConfigSnapshot: bt,
      gitProxyCredHelper: ct,
      configureGitHookStubs: tt,
      configureGitSigningArtifacts: j,
    ...
    Ue = {
      runnerId: Ne,
      activeSessions: 0,
      sessionsStarted: new Map([[sK, 0]]),
      sessionsCompleted: new Map([[sK, 0]]),
      sessionsFailed: new Map([[sK, 0]]),
      sessionsInterrupted: new Map([[sK, 0]]),
      ...
      sessionInitErrors: 0,
      sessionInitDurations: uIn(),
      sessionStartHookErrors: 0,
      pollErrors: AZe(),
      sessionIdle: new Map(),
    };
  vt.current = Ue;
  let st = t.healthPort > 0 ? fIn(t.healthPort, Ue, i) : void 0,
    Pe = await Es(i),
    H = await bs(u);
  try {
    await Da(
      {
        ...
        flushLogSink: h,
        healthState: Ue,
        hostConfigSnapshot: Pe,
        governedGitConfigSeed: H,
    ...
    if ((Xe.cancelAll(), st?.close(), Se))
      await cu(Se.close(), 2000, "[runner:exit] egress proxy close").catch(() => {});
    process.removeListener("SIGTERM", Ot), process.removeListener("SIGINT", Zt), await h();
  }
}
function qs(e) {
...
async function Da(e, t) {
  let {
    ...
    m = new Map(),
    T = Be("SELF_HOSTED_RUNNER_SESSION_STOP_GRACE_MS") || Nn,
    k = Be("SELF_HOSTED_RUNNER_POST_SESSION_HOOK_TIMEOUT_MS") || bn,
    A = $r(),
    U,
    q = new Set(),
    M = new Map(),
    ...
    Pt = Be("SELF_HOSTED_RUNNER_SESSION_IDLE_MS"),
    Dt = Os(),
    ...
    tt,
    j = e.deferShutdown,
    bt = j !== void 0 && j.maxMs > 0,
    ct = j?.ceilingGraceMsOverride ?? Math.max(A, Ie),
    rt = j?.ceilingGraceMsOverride !== void 0 ? Math.max(10, Math.floor(j.ceilingGraceMsOverride / 10)) : 1000,
    ...
    _t = () => {
      let ee = m.size;
      for (let Ee of pe.keys()) if (!m.has(Ee)) ee++;
      for (let Ee of Oe) if (!m.has(Ee) && !pe.has(Ee)) ee++;
      return ee;
    },
    Ye = new Set(),
    zt = 10,
  ...
  if (at)
    h("[runner:hints] CCR_SHR_SSE_HINTS enabled \u2014 opening stream"),
      (Gt = Ke({ baseUrl: d.apiUrl, runnerId: r, tokenState: s, onWake: () => Y.wake("SSE"), onDebug: h, signal: t }));
  let wt = () => {
      mt = void 0;
      let ee = Ot - Date.now();
      if (ee > 0) {
      ...
      if (
        ...
        u(
          `[runner:retire] retire time reached \u2014 releasing ${m.size} active session(s), refusing new work; exiting once the slots are empty`,
        ),
        Ee > 0)
      )
        u(
          `[runner:retire] ${Ee} declined session(s) held for re-spawn dropped at the retire deadline \u2014 requeued at exit instead`,
        );
      for (let lt of m.values()) lt.releaseForRetire();
      Y.wake("LOCAL");
    },
    Xt = () => {
      if (!bt || j === void 0) return;
      let ee = typeof j.signal.reason === "number" ? j.signal.reason : Date.now();
      if (st === void 0) {
        let Ee = ee + j.maxMs - Date.now();
        st = setTimeout(xt, Math.min(Math.max(0, Ee), 2147483647));
      }
      if ((y("self_hosted_defer_shutdown"), Xe)) {
    ...
    xt = () => {
      ...
      let Ee = S(),
        lt = () => {
          if (ee > 0)
            u(
              `[runner:shutdown] ${ee} declined session(s) held for re-spawn dropped at the ceiling \u2014 requeued at exit instead`,
            );
        };
      if (Ee === 0 && x() === 0) {
        lt();
      ...
      let Ve = Ue,
        Ae = () => {
          if (((Pe = void 0), t.aborted)) return;
          let Tt = S(),
            He = x();
          if (Tt === 0 && He === 0) return;
          if (Ve > 0) {
            (Ve -= rt), (Pe = setTimeout(Ae, rt));
            return;
          }
          u(
            `[runner:shutdown] ${Tt + He} session(s) still attached after the post-ceiling grace \u2014 falling back to a drain`,
          ),
            j.requestDrain("post-ceiling grace expired with sessions still attached");
        };
      Pe = setTimeout(Ae, ct);
    };
  if (Ot > 0) {
    ...
    u(
      ee > 0
        ? `[runner:retire] retire time set for ${new Date(Ot).toISOString()} (in ${$t(ee)}) \u2014 sessions will be released and the runner will exit then`
        : `[runner:retire] retire time ${new Date(Ot).toISOString()} is already in the past \u2014 refusing work and exiting`,
    ),
      wt();
  }
  if (bt && j !== void 0)
  ...
  try {
    while (!t.aborted) {
      ...
      if (et === 0 && je && _t() === 0) {
        u("[runner:exit] account workload drained \u2014 exiting (grace=0, no re-poll). Orchestrator will restart.");
        return;
      }
      let ee = Xe ? 0 : Math.max(0, d.capacity - _t()),
        Ee = Y.consume();
      if (Ee === "SSE") {
        if ((await ne(Math.floor(Math.random() * Is), t), t.aborted)) break;
      }
      let lt = ve;
      if (dt && !Rt && !je && ee > 0) {
        try {
      ...
      let Ve;
      try {
        let _ = await n.pollWork(s.runnerToken, r, ee, t, at ? Ms[Ee] : void 0, dt ? Rt : void 0);
        (Ve = _.assignment_ids), M.clear();
        for (let N of _.session_assignments ?? []) {
          if (!N || typeof N.session_id !== "string" || !N.session_id) continue;
          if (typeof N.client_platform === "string" && N.client_platform && W5t.test(N.client_platform))
            M.set(N.session_id, N.client_platform);
          else if (typeof N.client_platform === "string" && N.client_platform && !be.has(N.session_id)) {
            if (be.size > 256) be.clear();
            be.add(N.session_id),
              h(
                `[runner:session] assignment client_platform rejected by the charset gate (label falls back to "unknown"): "${T$e(N.client_platform)}" (len=${N.client_platform.length}, session=${T$e(N.session_id)})`,
              );
          }
        }
        ...
        if (qs(_)) {
          p("self_hosted_poll", "self_hosted_poll_auth_failed"),
            u(
        ...
        if (Rr(_)) {
          if ((Z++, Z >= 3)) {
            p("self_hosted_poll", "self_hosted_poll_runner_not_found"),
              u(
                `[runner:fatal] poll returned 404 ${Z}\xD7 \u2014 runner record gone server-side. Draining and exiting for clean restart.`,
              );
            break;
          }
          let re = Math.min(Ea, wa * 2 ** (Z - 1)),
            F = Math.floor(Math.random() * re);
          u(`Poll failed: ${w} \u2014 confirming 404 (${Z}/3), retrying in ${(F / 1000).toFixed(1)}s`),
        ...
        u(`Poll failed: ${w} \u2014 retrying in ${(z / 1000).toFixed(1)}s`), await ne(e.pollIntervalOverrideMs ?? z, t);
        continue;
      }
      let Ae = Ve.filter((_) => !q.has(_));
      if (Ae.length === 0 && _t() === 0) {
        Bt ??= Date.now();
        let _ = Date.now() - Bt;
        if (je) {
          if (_ >= et) {
            u(
              Xe
                ? he()
                : et > 0
                  ? `[runner:exit] account workload drained ${Math.round(_ / 1000)}s ago (grace ${Math.round(et / 1000)}s) \u2014 exiting for fresh disk. Orchestrator will restart.`
                  : "[runner:exit] account workload drained \u2014 exiting for fresh disk. Orchestrator will restart.",
            );
            return;
          }
        } else if (vt > 0 && _ >= vt) {
          u(`[runner:exit] idle ${Math.round(_ / 60000)}min with no work \u2014 exiting for autoscaler scale-down`);
          return;
        }
      } else Bt = null;
      if (Ae.length > 0) je = !0;
      ...
      for (let [_, N] of m)
        if (!He.has(_))
          u(`[runner:session] ${_} deassigned by server (deleted/archived/requeued) \u2014 aborting child`),
            N.controller.abort(ge.has(_) ? "idle-release" : "deassign");
      for (let _ of ge) if (!He.has(_)) ge.delete(_);
      for (let _ of me.keys()) if (!He.has(_)) me.delete(_);
      for (let _ of Oe) if (!He.has(_) || q.has(_)) Oe.delete(_);
      for (let _ of Ae) {
        if (m.has(_)) {
          h(`[runner:main] Ignoring duplicate assignment for ${_}`);
          continue;
        }
        if (q.has(_)) {
          Oe.delete(_), h(`[runner:main] Ignoring stuck session ${_} \u2014 already failed, not re-spawning`);
          continue;
        }
        if (ae.has(_)) {
          h(`[runner:main] Ignoring session ${_} \u2014 failure report in-flight`);
          continue;
        }
        if (ge.has(_) || pe.has(_)) {
          h(
            `[runner:main] Ignoring released session ${_} \u2014 awaiting server deassign (or its in-flight release to settle)`,
          );
          continue;
        }
        let N = Xe && tt === "shutdown" && !H;
        if (Xe && !N) {
          let C =
            tt === "shutdown"
        ...
        if (N)
          u(
            ...
            Oe.delete(_);
        Qe++, u(`Picked up session ${_} (${m.size + 1}/${d.capacity} active)`);
        let w = new AbortController(),
          z = _.replace(/^cse_/, "session_"),
          re = M.get(_) ?? sK;
        i?.sessionIdle.set(z, null), i?.sessionClientPlatform.set(z, re);
        let F = Date.now();
        i?.claimVisibility?.set(z, { pickedUpAt: F });
        let Re = m.size === 0 ? F - kt : void 0;
        if (i) {
          for (let C of [i.sessionsStarted, i.sessionsCompleted, i.sessionsFailed, i.sessionsInterrupted])
        ...
        let Le,
          ...
          Rn = () => (rn === "shutdown" ? "shutdown grace" : "retire grace"),
          Jt = 0,
          sn,
          Yt = () => {
          ...
          ln,
          Tn = [1800000, 7200000, 28800000],
          pt = () => {
            if (ln !== void 0) clearTimeout(ln), (ln = void 0);
          },
          It = (C, W) => {
            let Ge = C?.liveTasks ?? 0,
              on = (C?.liveTaskIds ?? []).slice(0, 5).join(","),
              Qt = C?.wakeupInMs !== void 0 ? `wakeup pending in ${$t(C.wakeupInMs)}` : void 0,
              $e = C?.bgResultAwaitingFollowup ? "finished background task awaiting the follow-up turn" : void 0,
              ut = [Ge > 0 ? `${Ge} background task(s) live [${on}${Ge > 5 ? ",\u2026" : ""}]` : void 0, Qt, $e].filter(
                Boolean,
              ),
              At = ut.length > 0 ? ut.join(" / ") : "planned resumption",
              L = W > 0 ? " still" : "";
            if ((u(`[runner:session] ${_} turn ended with ${At} \u2014 idle clock${L} deferred`), W < Tn.length))
              ln = setTimeout(It, Tn[W], C, W + 1);
          },
          de = (C, W) => {
            if (Le !== void 0) clearTimeout(Le), (Le = void 0);
            if (!De && i)
              if (C === "turn-end" || (C === "awaiting-action" && !we)) {
                if (i.sessionIdle.get(z) === null) i.sessionIdle.set(z, Date.now());
              } else i.sessionIdle.set(z, null);
            if (C === "init-observed") {
              if (ke && yt !== void 0) de(yt);
              return;
            }
            if (C === "activity") {
            ...
            let Ge = ke ? (Jt === 0 ? 0 : Zt) : C === "startup" ? Dt : Pt,
              ...
              $e = ke
                ? Mt()
                : C === "startup"
                  ? `no child output for ${$t(Ge)}`
                  : C === "awaiting-action"
                    ? `awaiting user action ${$t(Ge)}`
                    : `user idle ${$t(Ge)}`;
            if (!ke && Ge <= 0) return;
            if (De || w.signal.aborted) return;
            if (
              C === "awaiting-action" &&
              W !== void 0 &&
            ...
            Le = setTimeout(() => {
              ...
              if (ge.has(_)) return;
              if (se) {
                u(`[runner:session] ${_} idle timer fired mid-turn \u2014 skipping release (stdout-tee likely broken)`);
                return;
              }
              if (ke) Jt++;
              if ((u(`[runner:session] ${_} ${$e} \u2014 releasing`), ge.add(_), C !== "awaiting-action")) {
                let ut = m.get(_)?.liveBgTasks ?? 0,
                  L = (m.get(_)?.task ?? Promise.resolve())
                    .then(() => n.releaseSession(s.runnerToken, _))
                    .then(({ released: I }) => {
                      if ((pe.delete(_), ge.delete(_), me.delete(_), Xe)) Y.wake("LOCAL");
                      if (I) {
                        if (
                          (u(`[runner:session] ${_} released after post-session hook \u2014 parked server-side`),
                          se || we || ut > 0)
                        )
                      ...
                      u(
                        ke
                          ...
                          : `[runner:session] ${_} released=false after post-session hook (pending user event) \u2014 respawns on the next poll, or is requeued by this runner's exit if it is draining (grace=0) or retiring`,
                      ),
                        g("self_hosted_release_session", ke ? "released_false_retiring" : "released_false");
                    })
                    .catch((I) => {
                      if ((pe.delete(_), ge.delete(_), tt === "shutdown" && !H)) Oe.add(_);
                      if (Xe) Y.wake("LOCAL");
                      let K = I instanceof Error ? I.message : String(I);
                      u(
                        `[runner:session] ${_} ordered release failed after the post-session hook: ${K} \u2014 keeping session; respawns on the next poll, or is requeued by this runner's exit if it is draining (grace=0) or retiring`,
                      ),
                        p("self_hosted_release_session", "rpc_failed");
                    });
                pe.set(_, L), w.abort("idle-release");
                return;
              }
              n.releaseSession(s.runnerToken, _)
                .then(({ released: ut }) => {
                  if (ut) {
                    if (De || w.signal.aborted) return;
                    me.delete(_), u(`[runner:session] ${_} released \u2014 aborting child`);
                    let L = m.get(_)?.liveBgTasks ?? 0;
                    if ((w.abort("idle-release"), se || we || L > 0))
                      g("self_hosted_release_session", "released_true_mid_work");
                    else y("self_hosted_release_session");
                    return;
                  }
                  ...
                  if ((ge.delete(_), ke)) {
                    if (
                      (u(
                        `[runner:session] ${_} release declined (${C === "awaiting-action" ? "queued event behind the parked prompt" : "pending user event"}) ${Pn()} \u2014 keeping session${se || we ? "" : `, retrying ${Qt()}`}`,
                      ),
                      g(
                  ...
                  if (C === "awaiting-action") {
                    if (
                      (u(
                        `[runner:session] ${_} released=false while parked at prompt (queued event behind prompt) \u2014 keeping session${se || we ? "" : ", re-arming"}`,
                      ),
                  ...
                  if ((me.set(_, At), At >= 3)) {
                    me.delete(_),
                      u(`[runner:session] ${_} released=false ${At}x \u2014 aborting as backstop`),
                      w.abort(),
                      p("self_hosted_release_session", "released_false_backstop");
                    return;
                  }
                  if (
                    (u(
                      `[runner:session] ${_} released=false (pending user event) \u2014 keeping session${se || we ? "" : ", re-arming"}`,
                    ),
                ...
                .catch((ut) => {
                  ge.delete(_);
                  let At = ut instanceof Error ? ut.message : String(ut);
                  if (
                    (u(
                      `[runner:session] ${_} releaseSession failed: ${At} \u2014 keeping session${se || we ? "" : `, retrying ${Qt()}`}`,
                    ),
                    p("self_hosted_release_session", "rpc_failed"),
                    !se && !we)
                  )
          ...
          Vt = () => {
            ...
            if (sn !== void 0) return;
            u(
              `[runner:session] ${_} ${Mt()} with ${C} background task(s) live \u2014 allowing ${$t(Ie)} to finish before releasing`,
            ),
              (sn = setTimeout(() => {
          ...
          fn = (C) => {
            ...
            if (((ke = !0), (rn = C), yt === void 0)) {
              ...
              u(`[runner:session] ${_} ${Mt()} ${W}`);
              return;
            }
            u(`[runner:session] ${_} ${Mt()} while idle \u2014 releasing now`), de(yt);
          },
          _n = () => fn("retire"),
          Dn = () => fn("shutdown"),
          pn = (e.handleSession ?? ys)(
            _,
            {
              apiClient: n,
                ...
                ? (C) => {
                    switch (C.kind) {
                      case "start":
                        ...
                        break;
                      case "end":
                        i.initializingSessions--, dIn(i.sessionInitDurations, C.durationSec);
                        break;
                      case "exit-before-init":
                        if ((i.initializingSessions--, C.failed)) i.sessionInitErrors++;
                        break;
                      default:
                    }
                  }
              ...
              onSessionTokenIssued: (C) => {
                if (i && i.lockedAccountEmail === null) {
                  let W = Un(C);
                  if (W !== null) i.lockedAccountEmail = W;
                }
              },
              canonicalLocks: Se,
              postSessionHookTimeoutMs: k,
              pushOutcomeOnRelease: d.pushOutcomeOnRelease,
              trustWorkspace: d.trustWorkspace,
              confineRepoSettings: d.confineRepoSettings,
              onBgTaskLedger: (C) => {
                let W = m.get(_);
                if (W) W.liveBgTasks = C;
                U?.();
              },
              onBgFollowupPending: (C, W) => {
                let Ge = m.get(_);
                if (Ge) Ge.bgResultPendingFollowup = C;
                if ((U?.(), !C && !W && ke && we)) Vt();
              },
            ...
            .then(({ result: C, failureReason: W, setupFailureKind: Ge, failureKind: on }) => {
              if (W) u(`Session ${_} finished: ${C} \u2014 reason: ${W}`);
              else u(`Session ${_} finished: ${C}`);
              if (C === "failed" || (C === "interrupted" && !w.signal.aborted))
                ae.add(_),
                  n
                    .reportSessionFailure(s.runnerToken, _, W ?? "failed (no reason captured)", Ge, on)
                    .then(($e) => {
                      if (
                        (u(
                          `[runner:session] ${_} failure reported \xB7 excluded_count=${$e.excluded_count} stuck=${$e.stuck}`,
                        ),
                        $e.stuck)
                      )
                        q.add(_), u(`[runner:session] ${_} marked stuck \u2014 will not re-spawn on future polls`);
                      y("self_hosted_failure_report");
                    })
                    .catch(($e) => {
                      let ut = $e instanceof Error ? $e.message : String($e);
                      u(
                        `[runner:session] ${_} reportSessionFailure failed: ${ut} \u2014 marking stuck to bound respawn`,
                      ),
                        q.add(_),
            ...
            .finally(() => {
              ...
              let C = m.size >= d.capacity;
              if ((m.delete(_), U?.(), i)) i.activeSessions = m.size;
              if (m.size === 0) {
                if (((kt = Date.now()), Xe)) Y.wake("LOCAL");
              }
              if (at && C) Y.wake("LOCAL");
            });
        if (
          (m.set(_, {
            task: pn,
          ...
          i.activeSessions = m.size;
      }
      let ft = e.pollIntervalOverrideMs ?? xa(P);
      if ((fe++, fe % zt === 0)) {
        let _ = m.size,
          N = fr(s.runnerToken),
          w = Date.now(),
      ...
      if (at) Y.atCapacity = Xe || m.size >= d.capacity;
      await Y.wait(ft, t);
    }
  } finally {
    if (mt !== void 0) clearTimeout(mt), (mt = void 0);
    ...
    try {
      if (A > 0 && t.aborted) {
        let Ae = () => [...m.values()].reduce((N, w) => N + w.liveBgTasks, 0),
          Tt = () => [...m.values()].reduce((N, w) => N + (w.turnInFlight ? 1 : 0), 0),
          He = () => [...m.values()].reduce((N, w) => N + (w.bgResultPendingFollowup ? 1 : 0), 0),
          ft = () => (He() > 0 ? `, ${He()} finished background task(s) awaiting the follow-up turn` : ""),
          _ = () => Ae() > 0 || Tt() > 0 || He() > 0;
        if (_())
          u(
            `[runner] drain-wait: ${Tt()} turn(s) in flight, ${Ae()} background task(s) live${ft()} across ${m.size} session(s) \u2014 waiting up to ` +
              `${A / 1000}s before stopping session(s)`,
          ),
            await new Promise((N) => {
              let w = () => {
                  (U = void 0), N();
                ...
                z = setTimeout(w, A);
              U = () => {
                if (!_()) clearTimeout(z), w();
              };
            }),
            u(
              !_()
                ? "[runner] drain-wait: in-flight work finished \u2014 stopping session(s)"
                : `[runner] drain-wait: timed out with ${Tt()} turn(s) in flight, ${Ae()} task(s) still live${ft()} \u2014 stopping session(s)`,
            );
      }
      for (let { controller: Ae } of m.values()) Ae.abort();
      if (m.size > 0) {
        u(`Draining ${m.size} active session(s)...`);
        try {
          await cu(Promise.allSettled([...m.values()].map((Ae) => Ae.task)), ee, "[runner:stuck] drain"),
            u("Drain complete"),
            y("self_hosted_drain");
        } catch {
          p("self_hosted_drain", "shutdown_budget_exceeded"),
            u(
              `[runner] shutdown exceeded ${ee}ms (session-stop-grace + post-session-hook-timeout` +
                (d.pushOutcomeOnRelease ? " + push-on-release window" : "") +
                ` + ${cn / 1000}s); force-exiting. The post-session hook had its full budget; post-hook cleanup may have been cut short. ${m.size} session(s) still active.`,
            ),
      ...
      if (pe.size > 0)
        u(`[runner] shutdown: waiting for ${pe.size} in-flight session release(s) to settle before deregistering`),
          await cu(Promise.allSettled([...pe.values()]), Ar, "[runner:exit] release settle").catch(() => {
            u(
              `[runner] shutdown: ${pe.size} session release(s) did not settle within ${Ar / 1000}s \u2014 deregistering anyway (those sessions are requeued rather than parked)`,
            );
          });
    } finally {
      clearInterval(lt);
