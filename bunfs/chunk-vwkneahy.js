...
async function f(a, t) {
  let e = a.trim(),
    o = e.toLowerCase();
  if (!o || A0.includes(o) || _j.includes(o))
    return {
      type: "text",
...
${z2e(t)}`,
  ...
  let n = G2e(e);
  if (!n) return { type: "text", value: `Expected key=value, got "${Qn(e)}". Run /config to see what's available.` };
  let i = await Xke(n, t),
    r = mo(t.session);
  return {
    type: "text",
    value: i
      .map((s) =>
        r && s.kind === "save-failure" ? "Couldn't save this setting (detail withheld on this connection)." : s.message,
      )
      .join(`
`),
  };
