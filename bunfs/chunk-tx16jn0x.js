...
function ne(t, r, e) {
  return new Promise((o, i) => {
    if (r?.aborted) {
      if (e?.throwOnAbort || e?.abortError) i(e.abortError?.() ?? Error("aborted"));
      else o();
      return;
    }
    let n = setTimeout(
      (a, u, d) => {
        a?.removeEventListener("abort", u), d();
      },
      t,
      r,
      m,
...
function jt(t, r, e = b) {
  ...
  return Promise.race([t, i]).finally(o);
}
async function OU(t, r, e) {
  let o = () => {
    t.catch(() => {});
  };
  if (r.aborted) throw (o(), e());
  let i = () => {};
  try {
