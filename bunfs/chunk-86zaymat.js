...
// Version: 2.1.252
import { k, Mve } from "/$bunfs/root/chunk-4ddxwr9r.js";
import { $t } from "/$bunfs/root/chunk-2rx5nghb.js";
import { O2 } from "/$bunfs/root/chunk-tveh0k24.js";
...
import { Eo, W4t } from "/$bunfs/root/chunk-qk6zade1.js";
import { dke } from "/$bunfs/root/chunk-0naf9t00.js";
...
import { relative as J } from "path";
...
function B(e) {
  return e.suite.plugins.length > 0
    ? e.suite.plugins
        .map((s) => {
          let a = ie.test(s.name) ? s.name : b(or(F(s.name), 80));
...
${i ? j("With plugin", "m-accent", e.aggregates.score, e.aggregates.passRate, e.arms.with) + j("Baseline (no plugin)", "m-base", e.aggregates.scoreWithout, e.aggregates.passRateWithout, e.arms.without ?? []) : j("Runs", "m-accent", e.aggregates.score, e.aggregates.passRate, e.arms.with)}
    ...
    f = Q(e.arms.with, (o) => o.error !== null),
    u = Q(e.arms.with, (o) => o.skippedPaidGraders && o.error === null),
    y = e.aggregates.delta !== void 0 ? (e.arms.without ?? []) : [],
    x = f + Q(y, (o) => o.error !== null),
    m = u + Q(y, (o) => o.skippedPaidGraders && o.error === null),
    v = [
      x > 0 ? `<span class="flag">\u26A0 ${x} ${k(x, "run")} errored</span>` : "",
      m > 0 ? `<span class="flag">\u26A0 ${m} ${k(m, "run")} unjudged (cost ceiling)</span>` : "",
    ].join("");
...
function fe(e) {
  let s = ee(),
    ...
    m = Q(x, (t) => t.error !== null),
    v = Q(x, (t) => t.skippedPaidGraders && t.error === null),
    R = m + v > 0,
    o = a.filter((t) => t.aggregates.delta !== void 0).flatMap((t) => t.arms.without ?? []),
    E = m + Q(o, (t) => t.error !== null),
    M = v + Q(o, (t) => t.skippedPaidGraders && t.error === null),
    U =
      E + M > 0
        ? `<div class="banner"><span class="chip-warn">\u26A0 Averages include non-judgments</span><span>${[E > 0 ? `${E} ${k(E, "run")} errored` : "", M > 0 ? `${M} ${k(M, "run")} force-failed at the cost ceiling` : ""].filter(Boolean).join("; ")} \u2014 their scores count toward the averages shown without a complete judgment.</span></div>`
        : "",
      ...
      u !== void 0 && h.length > 0
        ? (() => {
            let { cls: t, arrow: g, sign: S } = z(u),
              I = Q(h, (_) => (_.aggregates.delta ?? 0) > 0),
              L = Q(h, (_) => (_.aggregates.delta ?? 0) < 0),
              K = h.length - I - L;
            return `<p class="verdict">Plugin effect: <span class="delta ${t}">${g} ${S}${(u * 100).toFixed(1)} pts</span> vs baseline \u2014 improved ${I} \xB7 flat ${K} \xB7 regressed ${L} of ${h.length} ${k(h.length, "case")}.</p>`;
          })()
        : "",
    H =
      u !== void 0
        ? (() => {
            let { cls: t, arrow: g, sign: S } = z(u);
            return `<div class="tile"><span class="label">Ablation \u0394</span><span class="value num ${t}">${g === "\xB7" ? "" : `${g} `}${S}${(u * 100).toFixed(1)}</span><span class="sub">score points vs baseline, ${h.length} of ${a.length} ${k(a.length, "case")}</span></div>`;
          })()
        : "",
    V =
      l.length > 0
        ? `${H}
<div class="tile"><span class="label">Baseline score</span><span class="value num">${c(dke(l.map((t) => t.aggregates.scoreWithout ?? 0)))}</span><span class="sub">without the plugin</span></div>`
        : "",
...
${i.plugins
  .map(
    (t) =>
      `<span class="mono">${r(J(i.root, t.path) || ".")}${t.problem === void 0 ? "" : ` <span class="chip ${te[t.problem] ? "chip-fail" : "chip-warn"}">${r(re[t.problem])}</span>`}</span>`,
  )
  .join(`
`)}
...
function he(e) {
  let s = e.cases.some((n) => {
      let i = dke(n.arms.with.map((l) => l.score));
      return Math.abs(i - n.aggregates.score) > 0.005;
    }),
    a = e.cases.length > 0 && Math.abs(dke(e.cases.map((n) => n.aggregates.score)) - e.aggregates.overallScore) > 0.005;
  return s || a
