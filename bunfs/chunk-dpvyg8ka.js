...
import { eb, Ae, ie } from "/$bunfs/root/chunk-8tgj5dp2.js";
...
import { PHt, DHt, k2e, OHt, H2e, LHt, MHt, x2e } from "/$bunfs/root/chunk-68ht9mgc.js";
...
import { D8, B1, Dxe, w9 } from "/$bunfs/root/chunk-zze8764r.js";
...
import { o, t, ut, Bi } from "/$bunfs/root/chunk-he2phymk.js";
...
import { yUe, Jet } from "/$bunfs/root/chunk-xn8wd38m.js";
...
function fe(ho) {
  let O = _(27),
    { subtitle: pe, body: he, scope: be, postOption: ve, onProceed: we, onCancel: E } = ho;
  ...
  let He;
  if (O[23] !== A || O[24] !== j || O[25] !== z)
    (He = e(me, { title: "Run ultrareview in the cloud?", subtitle: j, onCancel: A, children: z })),
      (O[23] = A),
      (O[24] = j),
      (O[25] = z),
      (O[26] = He);
...
function de(bo) {
  let J = _(24),
    ...
    ee = Ge ? zn(Ge) : null,
    Ve;
  if (J[0] !== ee) (Ve = ee && Jet(ee)), (J[0] = ee), (J[1] = Ve);
  else Ve = J[1];
  let oe = Ve,
    M = D
    ...
    H = i.mode === "branch" && i.diffStat ? i.diffStat : null,
    Je;
  if (J[2] !== i.instructions || J[3] !== i.mode)
    (Je =
      i.mode === "branch" && i.instructions ? `Note for findings (not a base branch): "${H2e(i.instructions)}"` : null),
      (J[2] = i.instructions),
      (J[3] = i.mode),
      (J[4] = Je);
  else Je = J[4];
  ...
  let ae;
  if (J[14] !== Oe || J[15] !== Z || J[16] !== K || J[17] !== D || J[18] !== Te || J[19] !== I)
    (ae = Oe
      ? e(ue, {})
      : D
        ? e(Ce, {
            ...
            defaultFocusValue: Te ?? (D.preferPost ? "proceed-post" : "proceed"),
            onChange: K,
            onCancel: Z,
          })
        : e(Ce, {
  ...
  let Xe;
  if (J[21] !== re || J[22] !== ae)
    (Xe = r(o, { flexDirection: "column", gap: 1, children: [re, ae] })), (J[21] = re), (J[22] = ae), (J[23] = Xe);
  else Xe = J[23];
  return Xe;
}
function ue() {
  let se = _(12),
    Ne = Ts(),
    We;
  if (se[0] !== Ne.prefersReducedMotion)
    (We = Xu(Ne.prefersReducedMotion)), (se[0] = Ne.prefersReducedMotion), (se[1] = We);
  else We = se[1];
  let X = We,
    [Ie, W] = Bi(X ? null : 50),
    De = X ? -100 : 19 - (Math.floor(W / 200) % 29),
    Ee = Math.floor(W / 120),
    le;
  if (se[2] !== Ee || se[3] !== X || se[4] !== W)
    (le = e(Jj, { frame: Ee, messageColor: "inactive", reducedMotion: X, time: W })),
      (se[2] = Ee),
      (se[3] = X),
      (se[4] = W),
      (se[5] = le);
  ...
  let qe;
  if (se[8] !== Ie || se[9] !== le || se[10] !== ce)
    (qe = r(o, { ref: Ie, flexDirection: "row", columnGap: 1, children: [le, ce] })),
      (se[8] = Ie),
      (se[9] = le),
      (se[10] = ce),
      (se[11] = qe);
...
async function ze({
  ...
  let m = await MHt(l, p, n, { applyFixesOnComplete: h, postReviewToPR: R, signal: g });
  if (g?.aborted) {
    if (m?.launched && m.taskId && l.mode === "pr")
      p.taskRegistry.update(m.taskId, (f) => ({ ...f, postReviewTo: void 0 }));
    return;
  }
  if (m) {
    let f = L && m.launched ? (L === "disabled" ? DHt : PHt) : "",
      P =
        m.blocks
          .map((y) => (y.type === "text" ? y.text : ""))
          .filter(Boolean)
          .join(`
`) + f;
    b(P, {
    ...
    b("Ultrareview failed to launch the cloud session. Check that this is a GitHub repo and try again.", {
      display: "system",
    });
}
var No = async (l, p, b, n) => {
  let h = od("allow_remote_sessions", "Cloud sessions", "are");
  if (h) return l(h, { display: "system" }), null;
  let { scopeArgs: R, applyFixes: L, postReview: g } = k2e(b),
  ...
  let f = m.scope,
    ...
    a = await LHt({ overageConfirmed: p.isUltrareviewOverageConfirmed(), credentials: p.credentials });
  switch (a.kind) {
    case "blocked": {
  ...
  \u2192 ${a.actionUrl}`
          : "",
        N =
          a.actionUrl?.includes("/admin-settings/") && eb() && !vf()
            ? `
  Run /usage-credits to request this from your admin.`
            : "";
      return l(`${a.message}${k}${N}`, { display: "system" }), null;
    }
    case "needs-confirm":
    case "proceed":
      ...
      return e(fe, {
        ...
        onProceed: async (k, N) => {
          ...
          if (
            ...
            p.markUltrareviewOverageConfirmed();
        },
        onCancel: () => {
          if (!P) y("declined");
