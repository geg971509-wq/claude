...
import { Th, G, Iw, F4 } from "/$bunfs/root/chunk-f9h0bg01.js";
...
import { ft, we } from "/$bunfs/root/chunk-ypdw393e.js";
...
import { at, Qe, Kt, _t, ar, mc, Bt } from "/$bunfs/root/chunk-8tgj5dp2.js";
...
import { pm, no, AT, d6, Yv } from "/$bunfs/root/chunk-twwd9fep.js";
import { ti } from "/$bunfs/root/chunk-a2an01ph.js";
import { gr, w_, bu } from "/$bunfs/root/chunk-dck778n3.js";
...
import { Xo, CM, F2 } from "/$bunfs/root/chunk-50g10fz0.js";
...
import { cvt } from "/$bunfs/root/chunk-87pfeb5g.js";
...
var N = new Set([Qe, _t, Kt, ar, Xo, ti, mc, Bt]),
  k = e1t;
function U(e, m) {
  if (e.properties === void 0 || m.length === 0) return e;
  let f = { ...e.properties };
  for (let a of m) delete f[a];
  let S = Array.isArray(e.required) ? e.required.filter((a) => !m.includes(a)) : e.required;
  return { ...e, properties: f, required: S };
}
async function je(e, m, f, S, a) {
  Lc(e);
  let C = q(m, f, G(), "stdio", S, a),
    L = new RYe();
  await C.connect(L), vt(() => C.close());
}
class D extends MR {
  #e;
  constructor(e) {
  ...
  get(e) {
    return e === CM ? super.get(e) : this.#e.get(e);
  }
}
function q(e, m, f, S, a, C = "raw") {
  let L = aw(a),
    c = S === "http";
  if (c) {
    let r = z2();
  ...
  if ((Dqn(fln()), K8(), Iw())) cvt(a).catch(h);
  if ((gNt(), F4())) Cct().catch(h);
  let B = GS(sw),
    H = new MR(),
    E = new GH(
      {
    ...
    E.setRequestHandler(pA, async () => {
      ...
      return {
        tools: await Promise.all(
          T.map(async (i) => {
            let _ = xSe(i.inputSchema),
              { remoteExecution: v, ...s } = i;
            return {
              ...s,
              description: await i.prompt({ getToolPermissionContext: async () => r, tools: T, agents: [] }),
              inputSchema: U(_, [...(c ? k : []), ...(d6(i).supported ? [Pi] : [])]),
              outputSchema: void 0,
            };
          }),
        ),
    ...
    E.setRequestHandler(qT, async ({ params: { name: r, arguments: R } }, { signal: T }) => {
      ...
      let F = new D(H),
        x = {
          ...
          messageQueue: Zm(),
          session: f,
          agentContext: { agentType: "main", agentId: Th() },
          options: {
            commands: [],
            tools: v,
            mainLoopModel: at(),
...
${t}`;
        ...
        return y(K4e(s.name)), l;
      } catch (o) {
        let d =
          (o instanceof Error ? Jan(o) : [String(o)])
            .filter(Boolean)
            .join(`
`)
            .trim() || "Error";
        if (X4e(o)) n(`MCP server tool call '${r}' aborted`);
        else {
          let { code: u, isSad: A } = s$t(o);
          if (A) n(`MCP server tool call '${r}' failed: ${d}`, { level: "error" }), g(K4e(s.name), u);
          else {
            let l = we(o);
...
export { N as SERVE_MODE_TOOL_ALLOWLIST, q as createMCPServer, je as startMCPServer };
