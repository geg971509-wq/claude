...
import { vt, b, V, n } from "/$bunfs/root/chunk-fv016jr6.js";
...
function s() {
  return Te.state("computer-use-lock");
}
async function u(e) {
  if (e) {
    ...
    if (!r.found) return;
    try {
      let o = V(Buffer.from(r.value).toString("utf8"));
      return w(o) ? o : void 0;
    } catch {
      return;
    }
...
function k(e) {
  let t = tf();
  t.unregisterLockCleanup?.(),
    (t.unregisterLockCleanup = vt(async () => {
      await U(e);
    }));
}
async function PYn(e) {
  let t = await u(e);
  if (!t) return { kind: "free" };
  if (c()) return { kind: "held_by_self" };
  if (t.sessionId === K()) return { kind: "held_by_self" };
  if (h(t.pid)) return { kind: "blocked", by: t.sessionId };
  if ((n(`Recovering stale computer-use lock from session ${t.sessionId} (PID ${t.pid})`), e)) await e.delete(s());
  else await a(i()).catch(() => {});
  return { kind: "free" };
}
function c() {
  return tf().unregisterLockCleanup !== void 0;
}
async function DYn(e) {
  let t = K(),
    r = { sessionId: t, pid: process.pid, acquiredAt: Date.now() };
  if (!e) await an().mkdir(be());
  if (e && c()) {
    if ((await u(e)) && c()) return y("computeruse_lock_acquire"), f;
  }
  if (await m(r, e)) return k(e), y("computeruse_lock_acquire"), l;
  let o = await u(e);
  if (!o) {
    if (e) await e.delete(s());
    else await a(i()).catch(() => {});
    if (await m(r, e)) return k(e), g("computeruse_lock_acquire", "stale_recovered"), l;
    return p("computeruse_lock_acquire", "lock_held"), { kind: "blocked", by: (await u(e))?.sessionId ?? "unknown" };
  }
  if (c()) return y("computeruse_lock_acquire"), f;
  if (o.sessionId === t) return y("computeruse_lock_acquire"), f;
  if (h(o.pid)) return p("computeruse_lock_acquire", "lock_held"), { kind: "blocked", by: o.sessionId };
  if ((n(`Recovering stale computer-use lock from session ${o.sessionId} (PID ${o.pid})`), e)) await e.delete(s());
  else await a(i()).catch(() => {});
  if (await m(r, e)) return k(e), g("computeruse_lock_acquire", "stale_recovered"), l;
  return p("computeruse_lock_acquire", "lock_held"), { kind: "blocked", by: (await u(e))?.sessionId ?? "unknown" };
}
...
function xne(e, t) {
  e((r) => {
    let o = t(r.computerUseMcpState);
    if (o === r.computerUseMcpState) return r;
    return { ...r, computerUseMcpState: o };
  });
}
export { PYn, DYn, n_t, OYn, LYn, xne };
