...
import { G0 } from "/$bunfs/root/chunk-zze8764r.js";
...
import { Re } from "/$bunfs/root/chunk-qg6z8ah8.js";
...
import { mkdir as P, realpath as v } from "fs/promises";
import { relative as _, resolve as j } from "path";
var f = {};
je(f, {
  IDENT: () => d,
  SCHEMA_DEPTH_MAX: () => M,
  ...
  key: () => T,
  literal: () => A,
  mcpToolDeclarations: () => C,
  mcpToolSchemasOf: () => N,
  tsTypeOf: () => c,
});
var p = {};
je(p, { IDENT: () => d, commentText: () => y, default: () => p, docLine: () => x, key: () => T, literal: () => A });
var y = (o) => o.replace(/\s+/g, " ").trim().replace(/\*\//g, "* /");
function x(o, t) {
  let r = typeof o.description === "string" ? y(o.description) : "";
  return r === ""
    ? ""
    : `${t}/** ${r} */
`;
...
var d = /^[A-Za-z_$][\w$]*$/;
var T = (o) => (d.test(o) ? o : b(o));
var A = (o) =>
  typeof o === "string" || typeof o === "number" || typeof o === "boolean" || o === null ? b(o) : "unknown";
var g = {};
je(g, {
  IDENT: () => d,
  SCHEMA_DEPTH_MAX: () => M,
...
function c(o, t = "", r = 0) {
  ...
  if (Array.isArray(o.anyOf) || Array.isArray(o.oneOf)) {
    let e = (o.anyOf ?? o.oneOf).map((s) => c(s, t, r + 1));
    return e.length === 0 ? "unknown" : te(e).join(" | ");
  }
  if (Array.isArray(o.allOf)) {
    let e = o.allOf.map((s) => c(s, t, r + 1));
    return e.length === 0 ? "unknown" : e.map((s) => (s.includes(" | ") ? `(${s})` : s)).join(" & ");
  }
  if (Array.isArray(o.type)) return te(o.type.map((e) => c({ ...o, type: e }, t, r + 1))).join(" | ") || "unknown";
  switch (o.type) {
    case "string":
      return "string";
    case "number":
    ...
    case "null":
      return "null";
    case "array": {
      if (Array.isArray(o.items)) return `[${o.items.map((s) => c(s, t, r + 1)).join(", ")}]`;
      let e = c(o.items, t, r + 1);
      return e.includes(" | ") || e.includes(" & ") || e.includes("/*") ? `Array<${e}>` : `${e}[]`;
    }
    case "object":
    case void 0: {
      if (!He(o.properties))
        return o.type === "object"
          ? "{}"
          : "$ref" in o
            ? `unknown /* $ref ${p.commentText(String(o.$ref))} */`
            : "unknown";
      let e = new Set(Array.isArray(o.required) ? o.required.map(String) : []),
        s = `${t}  `,
        w = Object.entries(o.properties).map(([m, u]) => {
          let l = He(u) ? p.docLine(u, s) : "",
            S = e.has(m) ? "" : "?",
            a = c(u, s, r + 1);
          return `${l}${s}${p.key(m)}${S}: ${a}`;
        });
      return w.length === 0
        ? "{}"
        : `{
...
var C = (o) =>
  [
    ...
    ...[...o]
      ...
      .map(
        (t) =>
          `${t.description === void 0 ? "" : p.docLine({ description: t.description }, "    ")}    ${p.key(t.name)}: ${c(t.inputSchema, "    ")}`,
      ),
    "  }",
    "}",
    "",
...
var D = ".claude/types",
  ...
  ho = async (o, t) => {
    let r = ee(),
      ...
      m = f.mcpToolSchemasOf(w),
      u = new Set(m.map((i) => i.name.split("__")[1] ?? i.name)),
      l = await G0.realLocation(e),
      S = await v(r).catch(() => r);
    ...
    try {
      await G0.refuseNonFile(a, h);
      let i = await G0.openForWrite(a);
      try {
        await G0.refuseOpened(i, a, { realRoot: l, refused: h }),
          await i.truncate(0),
          await i.writeFile(f.mcpToolDeclarations(m), "utf8");
      } finally {
        await i.close();
      }
    } catch (i) {
      if (E(i) === "ELOOP")
        return { type: "text", level: "error", value: `Did not write ${O}: ${a} is a symbolic link.` };
      throw i;
    }
    return {
      type: "text",
      value:
        m.length === 0
          ? `No MCP tools are connected; wrote an empty ${a} (every mcp__* tool stays loosely typed).`
          : `Wrote ${a}: ${m.length} MCP ${k(m.length, "tool")} from ${u.size} ${k(u.size, "server")}. Include it in the plugin's tsconfig beside the engine's "claude-code" module types, and e narrows per tool.`,
    };
  };
export { D as DEFAULT_PLUGIN_TYPES_DIR, O as MCP_DECLARATIONS_FILE, ho as call };
