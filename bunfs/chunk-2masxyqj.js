...
// Version: 2.1.252
import { RCe, qi, Psr } from "/$bunfs/root/chunk-s28wf80n.js";
import { ee } from "/$bunfs/root/chunk-4fwj3vnx.js";
import { ft, we, E, $o } from "/$bunfs/root/chunk-ypdw393e.js";
import { Jd, n } from "/$bunfs/root/chunk-fv016jr6.js";
import { a } from "/$bunfs/root/chunk-fec4384a.js";
import { h } from "/$bunfs/root/chunk-wkxx62a2.js";
import { Asr } from "/$bunfs/root/chunk-cf8qhmdc.js";
var R = 1000,
  B = 60;
function nJ(r, t, e = 10 * B * R) {
  let s;
  if (t === void 0) s = {};
  else if (t instanceof AbortSignal) s = { abortSignal: t, timeout: e };
  else s = t;
  let {
    abortSignal: d,
    timeout: _ = 10 * B * R,
    input: u,
    stdio: l = ["ignore", "pipe", "pipe"],
    useToolMemoryCgroup: c,
  } = s;
  d?.throwIfAborted();
  using m = Jd`exec: ${r.slice(0, 200)}`;
  try {
    let f = Psr(r, {
      env: process.env,
      maxBuffer: 1e6,
...
function $e(r, t, e = { timeout: 10 * g * p, preserveOutputOnError: true, useCwd: true }) {
  return qe(r, t, {
    ...
    timeout: e.timeout,
    preserveOutputOnError: e.preserveOutputOnError,
    cwd: e.cwd ?? (e.useCwd ? ee() : void 0),
    env: e.env,
    stdin: e.stdin,
    input: e.input,
    stdout: e.stdout,
    ...
    stripFinalNewline: e.stripFinalNewline,
    useToolMemoryCgroup: e.useToolMemoryCgroup,
    toolCgroupClass: e.toolCgroupClass,
  });
}
function F(r) {
...
function P(r) {
  ...
  return (
    ...
    e.on("error", (s) => {
      let d = E(s) ?? "unknown";
      if (D.has(d)) return;
      (t.truncatedBy ??= d), r.kill();
    }),
    t
  );
}
function G(r, t) {
  if (r.shortMessage) return r.shortMessage;
...
async function qe(
  ...
  try {
    ...
    let v = i.message;
    if ($o(i)) n(`execFileNoThrow spawn failed: ${E(i)} ${v}`, { level: "error" });
    else if (F(i))
      return (
  ...
  if (b?.truncatedBy !== void 0) {
    let i = `stdin write failed: ${b.truncatedBy}`;
