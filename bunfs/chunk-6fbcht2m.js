...
import { TX, Ewe, bgt } from "/$bunfs/root/chunk-fctnm902.js";
...
async function _je(t = {}) {
  let i = Sm(),
    l = i ? `${u}&entrypoint=${encodeURIComponent(i)}` : u;
  try {
    let e = await bt.get(l, {
      auth: "teleport-org",
...
async function HUn(t, i, l, e = {}) {
  let o = Sm(),
    r = [];
  if (o) r.push(`entrypoint=${encodeURIComponent(o)}`);
  if (l) r.push(`version=${encodeURIComponent(l)}`);
  let a = r.length > 0 ? `?${r.join("&")}` : "";
  try {
    let n = await bt.get(`/api/oauth/organizations/:orgUUID/skills/${encodeURIComponent(t)}/download${a}`, {
      auth: "teleport-org",
