...
import { yTn } from "/$bunfs/root/chunk-gtv2468w.js";
function mwt(e, r) {
  let n = qt();
  if ((n.cliOwnedConfigs.add(e), r?.getBearerToken)) n.cliOwnedBearerProviders.set(e, r.getBearerToken);
  return e;
}
function LT(e) {
  return typeof e === "object" && e !== null && qt().cliOwnedConfigs.has(e);
}
function Vre(e) {
  return typeof e === "object" && e !== null && qt().cliOwnedBearerProviders.has(e);
}
function gwt(e) {
  if (typeof e !== "object" || e === null) return null;
  let r = qt().cliOwnedBearerProviders.get(e);
  if (!r) return null;
  try {
    return r() || null;
  } catch {
...
var a = "/v2/ccr-sessions/-/meta/mcp",
  G6 = "claude-code-remote",
  Trr = "Claude Code Remote",
  c = "bf7c680d-5fdc-5ef4-b4a0-abadb619bf0a";
function hwt(e, r) {
  return {
    type: "http",
    url: e,
...
class d {
  ...
  reset() {
    (this.captured = l()), (this.carrier = p());
  }
}
var i0 = new d();
function _wt() {
  return i0.bridgeCarrier
...
var g = /^\/v2\/ccr-sessions\/([A-Za-z0-9_-]+)\/mcp$/;
function Swt(e, r, n) {
  let t = u(e, r);
  if (!t) return null;
  let s = g.exec(t.pathname),
    i = R(t, r);
  if (!s || s[1] !== n || !i) return null;
  return `${r}/v2/ccr-sessions/${n}/mcp?${i}`;
}
var E = ["mcp_url", "mcp_server_id", "toolbox_mcp_server_id"],
  C = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/;
function R(e, r) {
  let n = [...e.searchParams.keys()];
  if (
    new Set(n).size !== n.length ||
    n.some((o) => !E.includes(o)) ||
    e.searchParams.get("toolbox_mcp_server_id") !== c
  )
    return null;
  ...
  if (s !== null) {
    let o = u(s, r);
    if (!o || o.pathname !== a || o.search !== "") return null;
    t.set("mcp_url", `${r}${a}`);
  }
  let i = e.searchParams.get("mcp_server_id");
  if (i !== null) {
    if (!C.test(i)) return null;
    t.set("mcp_server_id", i);
  }
  return t.set("toolbox_mcp_server_id", c), t.toString();
}
function u(e, r) {
  let n;
  try {
...
function bwt(e) {
  return e !== void 0 && yTn(e) === null ? new URL(e).origin : null;
}
...
function Crr(e) {
  ...
  return (
    ...
    r.hash === "" &&
    r.search === "" &&
    (r.pathname === m || M.test(r.pathname))
  );
}
var _ = new Set(["bridge.claudeusercontent.com", "bridge-staging.claudeusercontent.com"]);
function X4t(e) {
  let r;
  try {
    r = new URL(e);
...
function IAe(e) {
  ...
  try {
    let r = new URL(e.url);
    return (r.search = ""), (r.username = ""), (r.password = ""), (r.hash = ""), r.toString().replace(/\/$/, "");
  } catch {
    return;
  }
}
...
function p() {
  ...
  if (process.env.CLAUDE_CODE_ENVIRONMENT_KIND === "byoc" && Me(process.env.CLAUDE_CODE_REMOTE)) {
    delete process.env.CLAUDE_CODE_BRIDGE_MCP_CARRIER;
    return;
  }
  return "descendant";
}
export { mwt, LT, Vre, gwt, G6, Trr, hwt, K4t, Err, i0, _wt, aP, ywt, Swt, Arr, bwt, Crr, X4t, sXe, aXe, IAe, MY };
