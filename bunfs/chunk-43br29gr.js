...
import { Cwe, J2t, N9 } from "/$bunfs/root/chunk-da857b4g.js";
...
import { QS, mf } from "/$bunfs/root/chunk-8tgj5dp2.js";
...
var m = "</cross-session-message>",
  h = "peer";
function z(te) {
  let c = _(19),
    { addMargin: x, param: re, hostInjected: R, verbose: ae, isTranscriptMode: ne } = te,
    { text: j } = re,
    ce = YU(ae, ne),
    i,
    D;
  if (c[0] !== R || c[1] !== j) {
    let a = j;
    let H = N9.find((ie) => a.startsWith(ie));
    if (H) a = a.slice(H.length);
    let J = a.match(/^<cross-session-message\b([^>]*)>/)?.[1] ?? "";
    let Q = J.match(/\bfrom="([^"]+)"/)?.[1];
    let U = J.match(/\bfrom-name="([^"]+)"/)?.[1];
    i = (U && QS(U)) || (Q && QS(B(Q))) || h;
    ...
    D = a.replace(/^<cross-session-message[^>]*>\n/, "").replace(n, "");
    (c[0] = R), (c[1] = j), (c[2] = i), (c[3] = D);
  } else (i = c[2]), (D = c[3]);
  let f = D;
  if (!ce) {
    let n;
    if (c[5] !== x || c[6] !== i || c[7] !== f)
      (n = e(JU, { displayName: i, addMargin: x, fallbackLabel: h, body: f })),
        (c[5] = x),
        (c[6] = i),
        (c[7] = f),
        (c[8] = n);
  ...
  if (c[15] !== n || c[16] !== k || c[17] !== y)
    (Z = r(o, { flexDirection: "column", marginTop: n, width: "100%", children: [k, y] })),
      (c[15] = n),
      (c[16] = k),
      (c[17] = y),
      (c[18] = Z);
...
export { z as UserCrossSessionMessage };
