...
// Version: 2.1.252
import { GVt } from "/$bunfs/root/chunk-f9h0bg01.js";
import { OA, kon, Hon, RW } from "/$bunfs/root/chunk-kez2zndg.js";
...
import { Sxe, C, u, F } from "/$bunfs/root/chunk-twm95mhz.js";
...
function X(O) {
  let c = _(28),
    ...
    p;
  if (c[0] !== O)
    ({ children: D, ref: z, stickyScroll: i, followGrowth: H, ...p } = O),
      (c[0] = O),
      (c[1] = D),
      (c[2] = H),
      (c[3] = z),
      (c[4] = i),
      (c[5] = p);
  else (D = c[1]), (H = c[2]), (z = c[3]), (i = c[4]), (p = c[5]);
  let n = C(null),
    [, ae] = u(0),
    I;
  ...
  if (c[9] !== i)
    (U = () => ({
      ...
      getFreshScrollHeight() {
        return n.current?.childNodes[0]?.yogaNode?.getComputedHeight() ?? n.current?.scrollHeight ?? 0;
      },
      getViewportHeight() {
      ...
      setClampBounds(re, oe) {
        ...
        (S.scrollClampMin = re), (S.scrollClampMax = oe), OA(S);
      },
      getDomElement() {
        return n.current;
      },
  ...
  Sxe(z, U, Z);
  const g = p.flexDirection ?? "row",
    G = p.flexGrow ?? 0,
    N = p.flexShrink ?? 1;
  let M;
  if (c[12] !== p || c[13] !== g || c[14] !== G || c[15] !== N)
    (M = {
      flexWrap: "nowrap",
      flexDirection: g,
      flexGrow: G,
  ...
  let B;
  if (c[17] !== i) (B = i !== void 0 && { stickyScroll: i }), (c[17] = i), (c[18] = B);
  else B = c[18];
  let E;
  if (c[19] !== H) (E = H !== void 0 && { followGrowth: H }), (c[19] = H), (c[20] = E);
  else E = c[20];
  let k;
  if (c[21] !== D)
    (k = e(Xa, { flexDirection: "column", flexGrow: 1, flexShrink: 0, width: "100%", children: D })),
      (c[21] = D),
      (c[22] = k);
  else k = c[22];
  let te;
