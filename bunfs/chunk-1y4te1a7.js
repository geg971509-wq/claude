// @bun @bytecode
// Claude Code is a Beta product per Anthropic's Commercial Terms of Service.
// By using Claude Code, you agree that all code acceptance or rejection decisions you make,
// and the associated conversations in context, constitute Feedback under Anthropic's Commercial Terms,
// and may be used to improve Anthropic's products, including training models.
// You are responsible for reviewing any code suggestions before use.

// (c) Anthropic PBC. All rights reserved. Use is subject to the Legal Agreements outlined here: https://code.claude.com/docs/en/legal-and-compliance.

// Version: 2.1.252
import { X } from "/$bunfs/root/chunk-ypdw393e.js";
import { n } from "/$bunfs/root/chunk-fv016jr6.js";
import { ce } from "/$bunfs/root/chunk-4ddxwr9r.js";
import { h } from "/$bunfs/root/chunk-wkxx62a2.js";
import { qi } from "/$bunfs/root/chunk-s28wf80n.js";
import { qe } from "/$bunfs/root/chunk-2masxyqj.js";
import { gc } from "/$bunfs/root/chunk-6k63g5t6.js";
import { Rt, cH, hmn, mne, gne, tn, N2, BVe, UD } from "/$bunfs/root/chunk-y8sx7bf9.js";
import { ea, GVe } from "/$bunfs/root/chunk-9yzzw213.js";
import { Nn } from "/$bunfs/root/chunk-c48t1p6v.js";
import { Q, te } from "/$bunfs/root/chunk-wag5ye9w.js";
import { spawn as L } from "child_process";
import { constants as z } from "fs";
import { lstat as K, open as le } from "fs/promises";
import { dirname as Oe, isAbsolute as Y, join as j, relative as Ae, resolve as ue } from "path";
var fWe = 60000,
  Ne = 16777216,
  Fe = 67108864,
  O = 2048,
  $e = /^0+$/;
async function dn(e, t, r = {}) {
  let i = await N(e);
  return i.kind === "refused" ? i.run : fe(e, t, r, i.pins);
}
async function de(e, t, { input: r, keepBytes: i, stopPastBytes: o }) {
  let s = await N(e);
  if (s.kind === "refused") return { bytes: 0, content: Buffer.alloc(0), stderr: s.run.stderr };
  let { signal: a, timeoutMs: f } = e,
    { leadingArgs: l, cwd: c } = F(e);
  return new Promise((d) => {
    let u = 0,
      y = [],
      g = !1,
      m = "",
      w = !1,
      b = (R) => {
        if (w) return;
        (w = !0),
          d({
            ...(R !== void 0 && { exitCode: R }),
            bytes: u,
            content: y === null ? null : Buffer.concat(y),
            ...(g && { stopped: !0 }),
            stderr: ce(m, O),
          });
      };
    try {
      let R = L(cH(), [...l, ...t], {
        cwd: c,
        env: M(e, s.pins),
        stdio: [r === void 0 ? "ignore" : "pipe", "pipe", "pipe"],
        signal: a,
        timeout: f,
        windowsHide: !0,
      });
      if (
        (R.stdout?.on("data", (B) => {
          if (g) return;
          if (((u += B.length), y !== null))
            if (u <= i) y.push(B);
            else y = null;
          if (!g && u > o) (g = !0), R.kill();
        }),
        R.stderr?.on("data", (B) => {
          if (m.length < O) m += B.toString("utf8");
        }),
        R.stdout?.on("error", () => {}),
        R.stderr?.on("error", () => {}),
        R.once("error", () => b(void 0)),
        R.once("close", (B) => b(B ?? void 0)),
        r !== void 0 && R.stdin)
      )
        R.stdin.on("error", () => {}), R.stdin.end(r);
    } catch {
      b(void 0);
    }
  });
}
async function N(e) {
  let t = (s) => ({ kind: "refused", run: Me(s) }),
    r = await V(e);
  if (r.kind === "refused") return t(r.why);
  let i = e.shallowFile ?? (r.lender.kind === "lender" ? r.lender.shallowFile : void 0),
    o = e.hookPins === void 0 ? await me(e) : { kind: "pins", pins: e.hookPins };
  if (o.kind === "unlisted")
    return Rt(e.signal)
      ? { kind: "refused", run: { stdout: "", stderr: "aborted" } }
      : t(`the configuration in force could not be listed (${o.detail})`);
  return { kind: "vetted", pins: { ...(i !== void 0 && { GIT_SHALLOW_FILE: i }), ...o.pins } };
}
async function V(e) {
  let t = (a) => ({ kind: "refused", why: a }),
    { gitDir: r, commonDir: i } = e,
    o = await We(r);
  if (
    (i !== void 0 && !Y(i)) ||
    o.kind !== "directory" ||
    (o.commonDir !== void 0 && (i === void 0 || Ae(ue(r, o.commonDir), i) !== ""))
  )
    return t(
      i !== void 0 && !Y(i)
        ? "the common directory it was opened with is not an absolute path"
        : o.kind !== "directory"
          ? "the git directory is not a plain directory (a gitfile, missing, or its commondir unreadable)"
          : i === void 0
            ? "a repository this code made holds a commondir file"
            : "the commondir file does not match the common directory it was opened with",
    );
  let s = i === void 0 ? await orn(r) : { kind: "none" };
  if (s.kind === "refused")
    return t(
      "objects/info/alternates is not the one-line local file this code writes (a network or device path, a quoted entry, or a file that is a link, oversized or unreadable)",
    );
  return { kind: "standing", lender: s };
}
function Me(e) {
  return { stdout: "", stderr: `dir-sync: ${e}; git was not run` };
}
async function e_e(e) {
  if ((await V(e)).kind === "refused") return e;
  let r = await me(e);
  return r.kind === "pins" ? { ...e, hookPins: r.pins } : e;
}
function F({ gitDir: e, workTree: t }) {
  return {
    leadingArgs: [
      "--no-optional-locks",
      `--git-dir=${e}`,
      ...(t === void 0 ? [] : [`--work-tree=${t}`]),
      ...mne,
      "-c",
      "gc.auto=0",
      "-c",
      "maintenance.auto=false",
      "-c",
      "advice.graftFileDeprecated=false",
    ],
    cwd: t ?? e,
  };
}
async function fe(e, t, r, i) {
  let { signal: o, timeoutMs: s } = e,
    { leadingArgs: a, cwd: f } = F(e),
    l = await qe(cH(), [...a, ...t], {
      cwd: f,
      env: M(e, i, r.env),
      extendEnv: !1,
      abortSignal: o,
      timeout: s,
      maxBuffer: Math.min(r.maxBuffer ?? Ne, Fe),
      ...(r.input === void 0 ? { stdin: "ignore" } : { input: r.input }),
    }),
    c = l.exitCode === 0 || (l.exitCode !== void 0 && (r.answerExitCodes ?? []).includes(l.exitCode));
  return {
    exitCode: l.exitCode,
    stdout: c ? l.stdout : "",
    stderr: ce(l.stderr, O),
    ...(l.maxBufferExceeded && { maxBufferExceeded: !0 }),
  };
}
async function SDt(e, t, r, i = {}) {
  let o = await N(e);
  if (o.kind === "refused") return { exitCode: void 0, stderr: o.run.stderr };
  let { signal: s, timeoutMs: a } = e,
    { leadingArgs: f, cwd: l } = F(e);
  return new Promise((c) => {
    let d = [],
      u = 0,
      y = !1,
      g;
    try {
      g = L(cH(), [...f, ...t], {
        cwd: l,
        env: M(e, o.pins, i.env),
        stdio: [i.input === void 0 ? "ignore" : "pipe", "pipe", "pipe"],
        signal: s,
        timeout: a,
        windowsHide: !0,
        ...qi("helper"),
      });
    } catch {
      c({ exitCode: void 0, stderr: "" });
      return;
    }
    let m = () => ce(Buffer.concat(d).toString("utf8"), O);
    g.stdout?.on("data", (b) => {
      if (y) return;
      try {
        r(b);
      } catch {
        (y = !0), g.kill();
      }
    }),
      g.stderr?.on("data", (b) => {
        if (u < O * 4) d.push(b), (u += b.length);
      }),
      g.stdout?.on("error", () => {}),
      g.stderr?.on("error", () => {});
    let w = !1;
    if (i.input !== void 0)
      g.stdin?.on("error", () => {
        w = !0;
      }),
        g.stdin?.end(i.input, () => {
          w ||= g.stdin?.writableFinished !== !0;
        });
    g.once("error", () => c({ exitCode: void 0, stderr: m() })),
      g.once("close", (b) => c({ exitCode: y || w || b === null ? void 0 : b, stderr: m() }));
  });
}
function rrn(e) {
  let t = [];
  return {
    push: (r) => {
      let i = 0;
      for (let o = r.indexOf(0, i); o !== -1; o = r.indexOf(0, i)) {
        let s = Buffer.concat([...t, r.subarray(i, o)]);
        e(s.toString("utf8"), s), (t = []), (i = o + 1);
      }
      if (i < r.length) t.push(r.subarray(i));
    },
    complete: () => t.length === 0,
  };
}
function De() {
  let e = [],
    t = [],
    r = rrn((i, o) => {
      e.push(i), t.push(o);
    });
  return { push: r.push, finish: () => (r.complete() ? { fields: e, fieldBytes: t } : null) };
}
async function t_e(e, t, r = {}) {
  let i = De(),
    o = await SDt(e, t, i.push, r),
    s = o.exitCode === 0 ? i.finish() : null;
  return { ...o, fields: s?.fields ?? null, fieldBytes: s?.fieldBytes ?? null };
}
async function bDt(e, t, r) {
  if (!A(t)) return !1;
  let i = await N(e);
  if (i.kind === "refused") return !1;
  if (
    await K(r).then(
      () => !0,
      (a) => !X(a),
    )
  )
    return !1;
  let s;
  try {
    s = await le(r, z.O_WRONLY | z.O_CREAT | z.O_EXCL | GVe(), 384);
  } catch {
    return !1;
  }
  try {
    return await je(e, ["cat-file", "blob", t], s.fd, i.pins);
  } catch {
    return !1;
  } finally {
    await s.close();
  }
}
async function je(e, t, r, i) {
  let { signal: o, timeoutMs: s } = e,
    { leadingArgs: a, cwd: f } = F(e);
  return new Promise((l) => {
    let c = L(cH(), [...a, ...t], {
      cwd: f,
      env: M(e, i),
      stdio: ["ignore", r, "ignore"],
      signal: o,
      timeout: s,
      windowsHide: !0,
      ...qi("helper"),
    });
    c.once("error", () => l(!1)), c.once("close", (d) => l(d === 0));
  });
}
function M({ gitDir: e, commonDir: t }, r, i = {}) {
  return hmn(
    gne({ GIT_GRAFT_FILE: "/dev/null", GIT_NO_REPLACE_OBJECTS: "1", GIT_COMMON_DIR: t ?? e, ...wDt(r, Xe(i)) }),
  );
}
function Le(e, t, r) {
  let { signal: i } = e,
    { leadingArgs: o, cwd: s } = F(e);
  return L(cH(), [...o, ...t], {
    cwd: s,
    env: M(e, r),
    stdio: ["pipe", "pipe", "ignore"],
    signal: i,
    windowsHide: !0,
  }).on("error", () => {});
}
var He = 2,
  q = 256;
async function C2n(e, { maxBytes: t }) {
  let { signal: r, timeoutMs: i } = e,
    o = 0,
    s = new Set(),
    a = null,
    f = !1,
    l = null,
    c = Promise.resolve(),
    d = async () => {
      if (f || r?.aborted === !0) return null;
      let m = await N(e);
      if (m.kind === "refused") return null;
      try {
        return new pe(Le(e, ["cat-file", "--batch"], m.pins));
      } catch {
        return null;
      }
    },
    u = (m) => {
      if ((m.end(), l === m)) l = null;
    },
    y = async (m) => {
      if (!A(m) || f || s.has(m)) return { kind: "unavailable" };
      if (l !== null && !l.alive) {
        if (l.overran && a !== null) s.add(a);
        else o += 1;
        u(l);
      }
      if (l === null) {
        if (o > He) return { kind: "unavailable" };
        let B = await d();
        if (B === null) return (f = !0), { kind: "unavailable" };
        if (f) return B.end(), { kind: "unavailable" };
        l = B;
      }
      let w = l;
      if ((await V(e)).kind === "refused") return u(w), (f = !0), { kind: "unavailable" };
      if (!w.alive) return { kind: "unavailable" };
      a = m;
      let b = await w.request(m, i);
      if (b === null) return s.add(m), u(w), { kind: "unavailable" };
      if (b.kind === "absent") return { kind: "unavailable" };
      if (b.type !== "blob" || b.size > t)
        return u(w), b.type === "blob" ? { kind: "too_large" } : { kind: "unavailable" };
      let R = await w.body(b.size, i);
      if (R === null) return s.add(m), u(w), { kind: "unavailable" };
      return { kind: "ok", bytes: R };
    },
    g = () => {
      (f = !0), l?.end(), (l = null);
    };
  return {
    read: (m) => {
      let w = c.then(() => y(m));
      return (
        (c = w.catch(() => {
          return;
        })),
        w.catch(() => ({ kind: "unavailable" }))
      );
    },
    close: g,
    [Symbol.asyncDispose]: () => (g(), Promise.resolve()),
  };
}
var Ue = /^([0-9a-f]{40}(?:[0-9a-f]{24})?) ([a-z]+) (\d{1,15})$/;
function ze(e, t) {
  if (e === t + " missing" || e === t + " ambiguous") return { kind: "absent" };
  let r = Ue.exec(e);
  return r === null || r[1] !== t ? null : { kind: "found", type: r[2] ?? "", size: Number(r[3]) };
}
class pe {
  child;
  alive = !0;
  overran = !1;
  chunks = [];
  buffered = 0;
  wake = null;
  phase = { kind: "idle" };
  constructor(e) {
    this.child = e;
    let t = () => {
      (this.alive = !1), this.wake?.();
    };
    e.stdout?.on("data", (r) => this.arrived(r)),
      e.stdout?.on("error", t),
      e.once("error", t),
      e.once("close", t),
      e.stdin?.on("error", t);
  }
  async request(e, t) {
    if (!this.alive || this.buffered > 0) return null;
    this.phase = { kind: "header", id: e };
    try {
      this.child.stdin?.write(
        e +
          `
`,
      );
    } catch {
      return null;
    }
    let r = await this.take(() => {
        let o = this.phaseNow();
        return o.kind === "sized" ? o.lineBytes : null;
      }, t),
      i = this.phaseNow();
    return r === null || i.kind !== "sized" ? null : i.header;
  }
  phaseNow() {
    return this.phase;
  }
  async body(e, t) {
    let r = await this.take((i) => (i >= e + 1 ? e + 1 : null), t);
    if (((this.phase = { kind: "idle" }), r === null || r[e] !== 10)) return null;
    return r.subarray(0, e);
  }
  end() {
    (this.alive = !1), (this.chunks = []), (this.buffered = 0);
    try {
      this.child.stdin?.end();
    } catch {}
    this.child.kill(), this.wake?.();
  }
  arrived(e) {
    if (!this.alive) return;
    if ((this.chunks.push(e), (this.buffered += e.length), this.phase.kind === "header")) {
      let r = Ye(this.chunks, 10, q);
      if (r !== -1) {
        let i = this.peek(r).toString("utf8"),
          o = ze(i, this.phase.id);
        if (o === null) {
          (this.overran = !0), this.end();
          return;
        }
        this.phase = {
          kind: "sized",
          header: o,
          lineBytes: r + 1,
          total: r + 1 + (o.kind === "found" ? o.size + 1 : 0),
        };
      } else if (this.buffered > q) {
        (this.overran = !0), this.end();
        return;
      }
    }
    let t = this.phase.kind === "sized" ? this.phase.total : this.phase.kind === "header" ? q : 0;
    if (this.buffered > t) {
      (this.overran = !0), this.end();
      return;
    }
    this.wake?.();
  }
  take(e, t) {
    return new Promise((r) => {
      let i = null,
        o = (a) => {
          if (i !== null) clearTimeout(i);
          (this.wake = null), r(a);
        },
        s = () => {
          let a = this.alive ? e(this.buffered) : null;
          if (a !== null) o(this.shift(a));
          else if (!this.alive) o(null);
        };
      (i = setTimeout((a) => a(null), t, o)), (this.wake = s), s();
    });
  }
  peek(e) {
    let t = this.chunks[0];
    return t !== void 0 && t.length >= e ? t.subarray(0, e) : Buffer.concat(this.chunks, this.buffered).subarray(0, e);
  }
  shift(e) {
    let t = this.chunks.length === 1 ? this.chunks[0] : Buffer.concat(this.chunks, this.buffered),
      r = Buffer.from(t.subarray(0, e)),
      i = t.subarray(e);
    if (((this.chunks = i.length === 0 ? [] : [i]), (this.buffered = i.length), this.phase.kind === "sized"))
      this.phase = { ...this.phase, lineBytes: 0, total: this.phase.total - e };
    return r;
  }
}
function Ye(e, t, r) {
  let i = 0;
  for (let o of e) {
    if (i >= r) return -1;
    let s = o.subarray(0, r - i).indexOf(t);
    if (s !== -1) return i + s;
    i += o.length;
  }
  return -1;
}
async function We(e) {
  try {
    if (!(await K(e)).isDirectory()) return { kind: "other" };
  } catch {
    return { kind: "other" };
  }
  let t = await n_e(j(e, "commondir"));
  switch (t.kind) {
    case "absent":
      return { kind: "directory" };
    case "text":
      return { kind: "directory", commonDir: t.text.replace(/\n$/, "") };
    case "unreadable":
      return { kind: "other" };
  }
}
var ge = /^GIT_CONFIG_(?:COUNT|KEY_\d+|VALUE_\d+)$/;
function wDt(e, t) {
  let r = [...W(e), ...W(t)],
    i = Object.fromEntries([...Object.entries(e), ...Object.entries(t)].filter(([o]) => !ge.test(o)));
  return r.length === 0
    ? i
    : {
        ...i,
        GIT_CONFIG_COUNT: String(r.length),
        ...Object.fromEntries(
          r.flatMap(([o, s], a) => [
            [`GIT_CONFIG_KEY_${a}`, o],
            [`GIT_CONFIG_VALUE_${a}`, s],
          ]),
        ),
      };
}
function W(e) {
  let t = /^\d{1,5}$/.test(e.GIT_CONFIG_COUNT ?? "") ? Number(e.GIT_CONFIG_COUNT) : 0;
  return Array.from({ length: t }, (r, i) => [e[`GIT_CONFIG_KEY_${i}`] ?? "", e[`GIT_CONFIG_VALUE_${i}`] ?? ""]).filter(
    ([r]) => r !== "",
  );
}
function Xe(e) {
  let t = W(e).filter(([r]) => !r.toLowerCase().startsWith("hook."));
  return wDt(
    gc(e, (r, i) => ge.test(i)),
    t.length === 0
      ? {}
      : {
          GIT_CONFIG_COUNT: String(t.length),
          ...Object.fromEntries(
            t.flatMap(([r, i], o) => [
              [`GIT_CONFIG_KEY_${o}`, r],
              [`GIT_CONFIG_VALUE_${o}`, i],
            ]),
          ),
        },
  );
}
var Ke = /^hook\.(.*)\.(?:command|event|enabled)$/s,
  Ve = 1024;
async function me(e) {
  let t = await fe(e, ["config", "-z", "--list", "--name-only"], {}, {});
  if (t.exitCode !== 0) return { kind: "unlisted", detail: zR("config", t) };
  let r = te(
    t.stdout.split("\x00").flatMap((o) => {
      let s = Ke.exec(o)?.[1];
      return s === void 0 ? [] : [s];
    }),
  );
  if (r.some((o) => o.includes("\uFFFD")))
    return { kind: "unlisted", detail: "a configured hook has a name that is not valid text" };
  if (r.length > Ve) return { kind: "unlisted", detail: "too many configured hooks to pin" };
  let i = r.flatMap((o) => [
    [`hook.${o}.enabled`, "false"],
    [`hook.${o}.event`, ""],
  ]);
  return {
    kind: "pins",
    pins: Object.fromEntries([
      ["GIT_CONFIG_COUNT", String(i.length)],
      ...i.flatMap(([o, s], a) => [
        [`GIT_CONFIG_KEY_${a}`, o],
        [`GIT_CONFIG_VALUE_${a}`, s],
      ]),
    ]),
  };
}
async function orn(e) {
  let t = await n_e(j(e, "objects", "info", "alternates"));
  if (t.kind === "absent") return { kind: "none" };
  if (t.kind === "unreadable") return { kind: "refused" };
  let r = t.text
    .split(`
`)
    .filter((s) => s !== "" && !s.startsWith("#"));
  if (r.some((s) => Je(s) || s.startsWith('"'))) return { kind: "refused" };
  let i = r[0];
  if (i === void 0) return { kind: "none" };
  let o = Y(i) ? i : ue(j(e, "objects"), i);
  return { kind: "lender", shallowFile: j(Oe(o), "shallow") };
}
function Je(e) {
  return /^[\\/]{2}/.test(e);
}
var ae = 4096;
async function n_e(e, { firstBytes: t } = {}) {
  try {
    if (!(await K(e)).isFile()) return { kind: "unreadable" };
  } catch (i) {
    return X(i) ? { kind: "absent" } : { kind: "unreadable" };
  }
  let r;
  try {
    r = await le(e, ea());
  } catch (i) {
    return X(i) ? { kind: "absent" } : { kind: "unreadable" };
  }
  try {
    let i = await r.stat();
    if (!i.isFile() || (t === void 0 && i.size >= ae)) return { kind: "unreadable" };
    let o = Buffer.alloc(t ?? ae),
      { bytesRead: s } = await r.read(o, 0, o.length, 0);
    return t !== void 0 || s === i.size ? { kind: "text", text: o.toString("utf8", 0, s) } : { kind: "unreadable" };
  } catch {
    return { kind: "unreadable" };
  } finally {
    await r.close().catch(() => {});
  }
}
function zR(e, t) {
  let r = t.exitCode ?? "with no status (killed, timed out, or not spawned)",
    i = t.stderr
      .split(`
`)
      .find((o) => o.trim() !== "");
  return `${e} exited ${r}${i === void 0 ? "" : `: ${i.replace(/[\p{Cc}\p{Cf}]/gu, " ").trim()}`}`;
}
async function XP(e, t) {
  if (t.length === 0) return new Map();
  if (!t.every(BVe)) return null;
  let r = await dn(e, ["for-each-ref", "--format=%(objectname) %(refname)", ...t]);
  if (r.exitCode !== 0) return null;
  let i = new Set(t),
    o = r.stdout
      .split(`
`)
      .filter((s) => s !== "")
      .map((s) => {
        let [a = "", f = ""] = s.split(" ");
        return { id: a, name: f };
      });
  if (o.some((s) => !tn.test(s.id))) return null;
  return new Map(o.filter((s) => i.has(s.name)).map((s) => [s.name, s.id]));
}
async function irn(e, t, r, i) {
  if (!UD(t) || !A(r) || (i.kind === "replace" && !A(i.current))) return !1;
  return (await dn(e, ["update-ref", "--no-deref", t, r, ...Qe(i)])).exitCode === 0;
}
async function Qq(e, t) {
  if (!t.every((i) => UD(i.name) && A(i.id))) return !1;
  if (t.length === 0) return !0;
  return (
    (
      await dn(e, ["update-ref", "--stdin"], {
        input: t
          .map(
            (i) => `option no-deref
update ${i.name} ${i.id}
`,
          )
          .join(""),
      })
    ).exitCode === 0
  );
}
async function kHe(e, t) {
  if (!t.every(UD)) return !1;
  if (t.length === 0) return !0;
  return (
    (
      await dn(e, ["update-ref", "--stdin"], {
        input: t
          .map(
            (i) => `option no-deref
delete ${i}
`,
          )
          .join(""),
      })
    ).exitCode === 0
  );
}
function Qe(e) {
  switch (e.kind) {
    case "create":
      return [""];
    case "replace":
      return [e.current];
    case "force":
      return [];
  }
}
function A(e) {
  return tn.test(e) && !$e.test(e);
}
import { randomUUID as Ze } from "crypto";
import {
  lstat as ee,
  mkdir as et,
  open as tt,
  readdir as ne,
  rename as nt,
  rm as Ee,
  unlink as re,
  writeFile as rt,
} from "fs/promises";
import { basename as it, join as C } from "path";
var wW = 104857600,
  xe = 16,
  he = 16,
  st = 256,
  ot = 65536,
  at = 64,
  ye = "PACK",
  hst = "incoming-",
  ke = "ccr-quarantine-",
  lt = 16,
  G = "the pack was refused: malformed, not self-contained, or not exactly what its range names",
  H = 67108864,
  bae = ".delivered",
  ut = 2592000000,
  dt = 134217728,
  Ce = 900000,
  ct = 32,
  ft = /^object-format=(?:sha1|sha256)$/,
  be = 16,
  pt = "ccr-sync";
function P(e, t) {
  return { ok: !1, reason: "git_error", stage: e, detail: t };
}
function E(e, t) {
  return { ok: !1, reason: "git_error", stage: e, detail: t };
}
function srn(e, { maxHeaderBytes: t = ot } = {}) {
  let r = Buffer.from(e.buffer, e.byteOffset, e.length),
    i = r.subarray(0, t).indexOf(`

`);
  if (i < 0) return null;
  let [o = "", ...s] = r.toString("utf8", 0, i).split(`
`),
    a = o === "# v2 git bundle" ? 2 : o === "# v3 git bundle" ? 3 : null;
  if (a === null) return null;
  let f = s.findIndex((u) => !u.startsWith("@")),
    l = s.slice(0, f === -1 ? s.length : f).map((u) => u.slice(1)),
    c = s.slice(l.length);
  if ((a === 2 && l.length > 0) || c.some((u) => u.startsWith("@"))) return null;
  let d = c.map((u) => {
    let y = u.startsWith("-"),
      [g = "", ...m] = (y ? u.slice(1) : u).split(" ");
    return { isPrerequisite: y, id: g, name: m.join(" ") };
  });
  if (d.some((u) => !tn.test(u.id) || (!u.isPrerequisite && u.name === ""))) return null;
  return {
    version: a,
    capabilities: l,
    prerequisites: d.filter((u) => u.isPrerequisite).map((u) => u.id),
    refs: d.filter((u) => !u.isPrerequisite).map((u) => ({ name: u.name, id: u.id })),
    packOffset: i + 2,
  };
}
function mWe(e) {
  return Buffer.from(
    [
      `# v${e.version} git bundle`,
      ...e.capabilities.map((t) => `@${t}`),
      ...e.prerequisites.map((t) => `-${t}`),
      ...e.refs.map((t) => `${t.id} ${t.name}`),
      "",
      "",
    ].join(`
`),
  );
}
function arn(e, { refNames: t, maxPrerequisites: r = st }) {
  let i = srn(e);
  if (i === null) return { ok: !1, reason: "not_a_bundle" };
  if (!i.capabilities.every((a) => ft.test(a))) return { ok: !1, reason: "unsupported_bundle" };
  let o = new Set(t),
    s = new Set(i.refs.map((a) => a.name));
  if (o.size !== t.length || o.size > xe || s.size !== i.refs.length || s.size !== o.size || !t.every((a) => s.has(a)))
    return { ok: !1, reason: "unexpected_refs" };
  if (i.prerequisites.length > r) return { ok: !1, reason: "too_many_prerequisites" };
  return { ok: !0, header: i };
}
async function HHe({ repository: e, tips: t, prerequisites: r, maxBytes: i = wW, declareForkPoints: o = !1 }) {
  return gt({ repository: e, tips: t, prerequisites: r, maxBytes: i, declareForkPoints: o }).catch(
    (s) => (h(s), P("threw", "unexpected throw")),
  );
}
async function gt({ repository: e, tips: t, prerequisites: r, maxBytes: i, declareForkPoints: o }) {
  await J(e.gitDir, (k) => k.startsWith(`${pt}-`), { directories: !0 });
  let s = { ok: !1, reason: "aborted" };
  if (t.length === 0 || t.length > xe || new Set(t).size !== t.length || !t.every(BVe))
    return P("arguments", "tips are not distinct plain ref names");
  if (r.length > he || new Set(r).size !== r.length || !r.every((k) => tn.test(k)))
    return P("arguments", "prerequisites are not distinct object ids");
  if (!(i > 0)) return P("arguments", "maxBytes is not a positive number");
  if (Rt(e.signal)) return s;
  let a = await XP(e, t);
  if (a === null || a.size !== t.length) return Rt(e.signal) ? s : P("tips", "a tip does not resolve to an object id");
  let f = t.map((k) => ({ name: k, id: a.get(k) ?? "" })),
    l = (k) =>
      k === null
        ? Rt(e.signal)
          ? s
          : P("range", "could not look the prerequisites up")
        : k > 0
          ? { ok: !1, reason: "prerequisites_missing", missingCount: k }
          : null,
    c = [...new Set(f.map((k) => k.id)), ...r.map((k) => `^${k}`), "--"],
    d = await mt(e, c);
  if (Rt(e.signal)) return s;
  if (d.kind === "failed") return l(await Z(e, r)) ?? P("range", zR("rev-list", d.run));
  let u =
      d.kind === "listed"
        ? await kt(e, r, [...d.commits, ...d.objects])
        : { missingCount: await Z(e, r), floorBytes: await bt(e, c) },
    y = l(u.missingCount);
  if (y !== null) return y;
  if (Rt(e.signal)) return s;
  if (u.floorBytes !== null && u.floorBytes > be * i) return { ok: !1, reason: "too_large", sizeBytes: u.floorBytes };
  let g = d.commits,
    m = o ? te([...r, ...d.forkPoints]) : [...r],
    w = m.length <= he ? m : [...r],
    b = g.size,
    R = f.filter((k) => g.has(k.id)),
    B = f.filter((k) => !g.has(k.id));
  if (R.length === 0) return { ok: !1, reason: "nothing_to_send", omitted: f };
  let v = await de(
    e,
    [
      "-c",
      "pack.useSparse=false",
      "-c",
      "pack.useBitmaps=false",
      "-c",
      "pack.threads=1",
      "pack-objects",
      ...(d.kind === "listed" ? [] : ["--revs"]),
      "--window=2",
      "--delta-base-offset",
      "--stdout",
      "-q",
    ],
    {
      input:
        (d.kind === "listed"
          ? [...d.commits, ...d.objects.map(yt)]
          : [...new Set(R.map((k) => k.id)), ...r.map((k) => `^${k}`)]
        ).join(`
`) +
        `
`,
      keepBytes: i,
      stopPastBytes: be * i,
    },
  );
  if (Rt(e.signal)) return s;
  if (v.stopped || (v.exitCode === 0 && v.content === null)) return { ok: !1, reason: "too_large", sizeBytes: v.bytes };
  if (v.exitCode !== 0 || v.content === null) return P("bundle_create", zR("pack-objects", v));
  let x = R.some((k) => k.id.length === 64),
    _ = Buffer.concat([
      mWe({
        version: x ? 3 : 2,
        capabilities: x ? ["object-format=sha256"] : [],
        prerequisites: w,
        refs: R,
        packOffset: 0,
      }),
      v.content,
    ]);
  if (_.length > i) return { ok: !1, reason: "too_large", sizeBytes: _.length };
  let I = await XP(e, t);
  if (I === null) return Rt(e.signal) ? s : P("header", "could not re-read the tips after packing");
  if (!t.every((k) => I.get(k) === a.get(k))) return Rt(e.signal) ? s : P("header", "a tip moved while packing");
  if (Rt(e.signal)) return s;
  return {
    ok: !0,
    content: _,
    sizeBytes: _.length,
    sha256: Nn(_),
    refs: R,
    omitted: B,
    prerequisites: w,
    commitCount: b,
  };
}
async function mt(e, t) {
  let r = await dn(e, ["rev-list", "--objects", "--boundary", ...t], { maxBuffer: H });
  if (r.exitCode === 0) {
    let o = we(r.stdout);
    return { kind: "listed", ..._e(o), objects: o.filter((s) => ht.test(s)) };
  }
  if (!r.maxBufferExceeded) return { kind: "failed", run: r };
  let i = await dn(e, ["rev-list", "--boundary", ...t], { maxBuffer: H });
  return i.exitCode === 0 ? { kind: "walked", ..._e(we(i.stdout)) } : { kind: "failed", run: i };
}
var ht = /^[0-9a-f]{40}(?:[0-9a-f]{24})? /,
  Re = 256;
function yt(e) {
  if (Buffer.byteLength(e) <= Re) return e;
  let t = e.slice(0, e.indexOf(" ")),
    r = Re - t.length - 1,
    i = [...e.slice(t.length + 1)],
    { kept: o } = i.reduceRight(
      (s, a) =>
        s.full || s.bytes + Buffer.byteLength(a) > r
          ? { bytes: s.bytes, kept: s.kept, full: !0 }
          : { bytes: s.bytes + Buffer.byteLength(a), kept: s.kept + 1, full: !1 },
      { bytes: 0, kept: 0, full: !1 },
    );
  return `${t} ${i.slice(i.length - o).join("")}`;
}
function we(e) {
  return e
    .split(`
`)
    .filter((t) => t !== "");
}
function _e(e) {
  return {
    commits: new Set(e.filter((t) => tn.test(t))),
    forkPoints: e
      .filter((t) => t.startsWith("-"))
      .map((t) => t.slice(1))
      .filter((t) => tn.test(t)),
  };
}
async function kt(e, t, r) {
  let i = await dn(e, ["cat-file", "--batch-check=%(objectsize:disk) %(objecttype) %(rest)"], {
      input: [...t, ...r]
        .map(
          (c) => `${c}
`,
        )
        .join(""),
      maxBuffer: H,
    }),
    o = i.stdout.split(`
`);
  if (i.exitCode !== 0 || o.length < t.length) return { missingCount: await Z(e, t), floorBytes: null };
  let s = Q(o.slice(0, t.length), (c) => !/^\d+ commit/.test(c)),
    a = o.slice(t.length).flatMap((c) => {
      let d = /^(\d+) (commit|tag|tree|blob) ?(.*)$/.exec(c);
      if (d === null) return [];
      let [, u = "0", y = "", g = ""] = d;
      return [{ key: y === "commit" || y === "tag" ? null : `${y} ${g}`, bytes: Number(u) }];
    }),
    f = a.reduce((c, { key: d, bytes: u }) => (d === null ? c : c.set(d, Math.max(u, c.get(d) ?? 0))), new Map()),
    l = [...a.filter(({ key: c }) => c === null).map(({ bytes: c }) => c), ...f.values()].reduce((c, d) => c + d, 0);
  return { missingCount: s, floorBytes: l };
}
async function bt(e, t) {
  let r = await dn(e, ["rev-list", "--objects", "--disk-usage", ...t]),
    i = r.stdout.trim();
  return r.exitCode === 0 && /^\d+$/.test(i) ? Number(i) : null;
}
async function wae({ repository: e, content: t, targets: r, heldBases: i, heldRefs: o, maxBytes: s = wW }) {
  return wt({ repository: e, content: t, targets: r, heldBases: i, heldRefs: o, maxBytes: s }).catch(
    (a) => (h(a), E("threw", "unexpected throw")),
  );
}
async function wt({ repository: e, content: t, targets: r, heldBases: i, heldRefs: o, maxBytes: s }) {
  let a = { ok: !1, reason: "aborted" },
    f = [...r.keys()],
    l = [...r.values()];
  if (
    r.size === 0 ||
    !(s > 0) ||
    !f.every(BVe) ||
    !l.every(UD) ||
    new Set(l).size !== l.length ||
    !i.every((_) => tn.test(_)) ||
    (o !== "all" && !St(o.glob))
  )
    return E(
      "arguments",
      "targets do not map plain ref names onto distinct refs of ours, a held basis is not an object id, the held refs are not a glob under refs/claude/, or the byte cap is not positive",
    );
  let c = { bases: i, refs: o };
  if (t.length > s) return { ok: !1, reason: "too_large", sizeBytes: t.length };
  let d = arn(t, { refNames: f });
  if (!d.ok) return d;
  let { header: u } = d,
    y = t.subarray(u.packOffset);
  if (y.length < ct || y.subarray(0, ye.length).toString("latin1") !== ye) return { ok: !1, reason: "not_a_bundle" };
  if (Rt(e.signal)) return a;
  let g = await Pe(e, u.prerequisites);
  if (g === null) return Rt(e.signal) ? a : E("prerequisites", "could not look the prerequisites up");
  let m = await Se(
    e,
    u.prerequisites.filter((_) => !g.includes(_)),
    c,
  );
  if (m === null) return Rt(e.signal) ? a : E("prerequisites", "could not walk from the prerequisites");
  let w = u.prerequisites.filter((_) => g.includes(_) || m.includes(_));
  if (w.length > 0) {
    if (m.length > 0)
      n(
        `dir-sync: a bundle names ${m.length} prerequisites this side holds only as objects, not as history of its own`,
      );
    return { ok: !1, reason: "prerequisites_missing", missingCount: w.length, missing: w.slice(0, at) };
  }
  let b = C(e.gitDir, "objects", "pack"),
    R = Math.max(Ce, lt * e.timeoutMs);
  await Promise.all([
    Ct(b, R),
    J(b, (_) => _.startsWith(ke), { directories: !0, olderThanMs: R }),
    J(C(e.gitDir, ...N2.split("/").filter(Boolean)), (_) => _.endsWith(".lock"), { recursive: !0 }),
  ]);
  let B = Ze(),
    v = C(b, `${ke}${B}`),
    x = `${hst}${B}`;
  try {
    await et(C(v, "pack"), { recursive: !0 });
  } catch {
    return E("unpack", "could not create the quarantine directory");
  }
  try {
    let _ = await _t({
      repository: e,
      header: u,
      pack: y,
      targets: r,
      quarantine: v,
      packDirectory: b,
      packName: x,
      held: c,
    });
    if (_.published) await Ge(C(b, x), [".keep"]);
    return _.outcome;
  } finally {
    await Ee(v, { recursive: !0, force: !0 }).catch(() => {
      n("dir-sync: could not remove a receive quarantine (non-fatal; swept later)");
    });
  }
}
async function _t({
  repository: e,
  header: t,
  pack: r,
  targets: i,
  quarantine: o,
  packDirectory: s,
  packName: a,
  held: f,
}) {
  let l = (p) => ({ outcome: p, published: !1 }),
    c = l({ ok: !1, reason: "aborted" }),
    d = C(o, "pack", a),
    u = await dn(
      e,
      ["index-pack", "--strict", "--stdin", "--index-version=2", "--keep=dir-sync receive", `${d}.pack`],
      { input: r },
    );
  if (Rt(e.signal)) return c;
  if (u.exitCode === void 0) return l(E("unpack", zR("index-pack", u)));
  if (u.exitCode !== 0) return n(`dir-sync: ${zR("index-pack", u)}`), l({ ok: !1, reason: "unpack_failed", detail: G });
  let y = { env: { GIT_ALTERNATE_OBJECT_DIRECTORIES: `"${o.replace(/["\\]/g, (p) => `\\${p}`)}"` } },
    g = await Bt(`${d}.idx`, r.readUInt32BE(8));
  if (g === null) return Rt(e.signal) ? c : l(E("tips", "could not read the received pack index"));
  let m = te(t.refs.map((p) => p.id));
  if (!m.every((p) => g.has(p)))
    return (
      n("dir-sync: a tip the header names is not an object the pack delivered"),
      l({ ok: !1, reason: "unpack_failed", detail: G })
    );
  let w = await dn(e, ["cat-file", "--batch-check=%(objecttype)"], {
      ...y,
      input: m
        .map(
          (p) => `${p}
`,
        )
        .join(""),
    }),
    b = w.stdout
      .split(`
`)
      .filter((p) => p !== "");
  if (w.exitCode !== 0 || b.length !== m.length) return Rt(e.signal) ? c : l(E("tips", "could not look the tips up"));
  if (!b.every((p) => p === "commit"))
    return n("dir-sync: a tip the header names is not a commit"), l({ ok: !1, reason: "unpack_failed", detail: G });
  let R = await Gt(e, g, f, y.env);
  if (R === null) return Rt(e.signal) ? c : l(E("walk", "could not read the delivered commits"));
  if (R > 0)
    return (
      n(`dir-sync: a bundle's commits reach ${R} parents outside the pack that are not history this side holds`),
      l({ ok: !1, reason: "unpack_failed", detail: G })
    );
  let B = (p) =>
      dn(e, ["rev-list", p, "--no-object-names", ...m, "--not", ...t.prerequisites, "--"], { ...y, maxBuffer: H }),
    v = (p) =>
      new Set(
        p
          .split(`
`)
          .filter((S) => S !== "" && !S.startsWith("-")),
      ),
    x = await B("--objects");
  if (Rt(e.signal)) return c;
  if (x.exitCode === void 0) return l(E("walk", zR("rev-list", x)));
  if (x.exitCode !== 0) return n(`dir-sync: ${zR("rev-list", x)}`), l({ ok: !1, reason: "unpack_failed", detail: G });
  let _ = v(x.stdout),
    I = Q([...g], (p) => !_.has(p)),
    k = Q([..._], (p) => !g.has(p));
  if (I === 0 && k > 0) {
    let p = await B("--objects-edge-aggressive");
    if (Rt(e.signal)) return c;
    if (p.exitCode === void 0) return l(E("walk", zR("rev-list", p)));
    if (p.exitCode !== 0) return n(`dir-sync: ${zR("rev-list", p)}`), l({ ok: !1, reason: "unpack_failed", detail: G });
    k = Q([...v(p.stdout)], (S) => !g.has(S));
  }
  if (I > 0 || k > 0)
    return (
      n(
        `dir-sync: a bundle's range names ${k} objects its pack lacks and its pack carries ${I} the range does not name`,
      ),
      l({ ok: !1, reason: "unpack_failed", detail: G })
    );
  if (Rt(e.signal)) return c;
  try {
    await rt(
      `${d}${bae}`,
      [...g].join(`
`) +
        `
`,
      { flag: "wx" },
    );
  } catch {
    return l(E("record", "could not record which objects the pack delivered"));
  }
  let ie = C(s, a);
  try {
    for (let p of [".pack", ".rev", ".keep", bae, ".idx"])
      await nt(`${d}${p}`, `${ie}${p}`).catch((S) => {
        if (!(p === ".rev" && X(S))) throw S;
      });
  } catch {
    return await Ge(ie, [".pack", ".rev", ".keep", bae]), l(E("record", "could not move the pack into place"));
  }
  let T = t.refs.map((p) => ({ name: i.get(p.name) ?? "", id: p.id })),
    U = { ...e, signal: void 0 },
    se = { outcome: { ok: !0, refs: T, prerequisiteCount: t.prerequisites.length }, published: !0 },
    oe = await XP(
      U,
      T.map((p) => p.name),
    );
  if (oe === null) return { outcome: E("update", "could not read the target refs"), published: !0 };
  if (await Qq(U, T)) return se;
  let D = await XP(
      U,
      T.map((p) => p.name),
    ),
    Ie = D !== null && T.every((p) => D.get(p.name) === oe.get(p.name)),
    Te = D !== null && T.every((p) => D.get(p.name) === p.id);
  if (Ie)
    return {
      outcome: { ok: !1, reason: "ref_update_failed", detail: "the ref transaction was declined" },
      published: !0,
    };
  if (Te) return se;
  return {
    outcome: E("update", "the ref transaction ended in a state that could not be read back whole"),
    published: !0,
  };
}
async function Bt(e, t) {
  let r;
  try {
    let o = await xHe(e, Math.max(1048576, 128 * t));
    if (o.kind !== "read") return null;
    r = o.content;
  } catch {
    return null;
  }
  let i = xt(r);
  return i === null || i.length !== t ? null : new Set(i.map((o) => o.id));
}
var vt = 4285812579,
  Et = 256;
function xt(e) {
  let t = 8 + Et * 4;
  if (e.length < t || e.readUInt32BE(0) !== vt || e.readUInt32BE(4) !== 2) return null;
  let r = e.readUInt32BE(t - 4),
    i = [20, 32].find((c) => {
      let d = e.length - t - r * (c + 8) - 2 * c;
      return d >= 0 && d % 8 === 0 && d / 8 <= r;
    });
  if (i === void 0) return null;
  let o = t,
    s = o + r * i + r * 4,
    a = s + r * 4,
    f = (e.length - a - 2 * i) / 8,
    l = [];
  for (let c = 0; c < r; c++) {
    let d = e.readUInt32BE(s + c * 4),
      u = d;
    if (d >= 2147483648) {
      let y = d - 2147483648;
      if (y >= f) return null;
      u = Number(e.readBigUInt64BE(a + y * 8));
    }
    l.push({ id: e.toString("hex", o + c * i, o + (c + 1) * i), offset: u });
  }
  return l;
}
async function J(e, t, r = {}) {
  let i;
  try {
    i = await ne(e, { recursive: r.recursive === !0 });
  } catch {
    return;
  }
  let o = Date.now() - (r.olderThanMs ?? Ce);
  await Promise.all(
    i
      .filter((s) => t(it(s)))
      .map(async (s) => {
        let a = C(e, s);
        try {
          let f = await ee(a);
          if (f.mtimeMs >= o) return;
          if (f.isFile() || (r.directories === !0 && f.isDirectory())) await Ee(a, { recursive: !0, force: !0 });
        } catch (f) {
          if (!X(f)) n("dir-sync: could not sweep a stale sync file (non-fatal)");
        }
      }),
  );
}
async function Ct(e, t) {
  let r;
  try {
    r = await ne(e);
  } catch {
    return;
  }
  let i = new Set(r),
    o = Date.now();
  await Promise.all(
    r
      .filter(
        (s) =>
          s.startsWith(hst) &&
          (s.endsWith(".keep") ||
            (s.endsWith(".pack") && !i.has(`${s.slice(0, -5)}.idx`)) ||
            (s.endsWith(".rev") && !i.has(`${s.slice(0, -4)}.idx`)) ||
            s.endsWith(`${bae}.part`) ||
            s.endsWith(bae)),
      )
      .map(async (s) => {
        let a = C(e, s),
          f = o - (s.endsWith(bae) ? ut : t);
        try {
          let l = await ee(a);
          if (l.isFile() && l.mtimeMs < f) await re(a);
        } catch (l) {
          if (!X(l)) n("dir-sync: could not sweep a stale received pack file (non-fatal)");
        }
      }),
  );
}
async function Z(e, t) {
  return (await Pe(e, t))?.length ?? null;
}
async function Pe(e, t) {
  if (t.length === 0) return [];
  let r = await dn(e, ["cat-file", "--batch-check=%(objecttype)"], {
      input: t
        .map(
          (o) => `${o}
`,
        )
        .join(""),
    }),
    i = r.stdout
      .split(`
`)
      .filter((o) => o !== "");
  if (r.exitCode !== 0 || i.length !== t.length) return null;
  return t.filter((o, s) => i[s] !== "commit");
}
async function v2n(e) {
  let t = C(e.gitDir, "objects", "pack"),
    r;
  try {
    r = await ne(t);
  } catch (s) {
    return X(s) ? new Set() : null;
  }
  let i = r.filter((s) => s.startsWith(hst) && s.endsWith(bae)).map((s) => C(t, s)),
    o = await Promise.all(
      i.map(async (s) => {
        try {
          let a = await xHe(s, dt);
          if (a.kind === "too_large") return n("dir-sync: a delivered-ids record is larger than this side reads"), null;
          return new Set(
            a.content
              .toString("utf8")
              .split(`
`)
              .filter((f) => tn.test(f)),
          );
        } catch (a) {
          if (X(a)) return new Set();
          return n("dir-sync: a delivered-ids record could not be read"), null;
        }
      }),
    );
  return o.includes(null) ? null : new Set(o.flatMap((s) => (s === null ? [] : [...s])));
}
var Pt = /^refs\/claude(?:\/(?!\.)[A-Za-z0-9._-]+)+\/\*$/;
function St(e) {
  return Pt.test(e) && !e.includes("..");
}
var Be = 1024,
  ve = 20000;
async function Se(e, t, r, i = {}) {
  let o = te(t);
  if (o.length === 0) return [];
  let s = await dn(
    e,
    [
      "rev-list",
      `--max-count=${Be}`,
      "--ignore-missing",
      "--stdin",
      "--not",
      ...r.bases,
      r.refs === "all" ? "--all" : `--glob=${r.refs.glob}`,
      "--",
    ],
    {
      env: i,
      input: o
        .map(
          (f) => `${f}
`,
        )
        .join(""),
    },
  );
  if (s.exitCode !== 0) return null;
  let a = new Set(
    s.stdout
      .split(`
`)
      .filter((f) => f !== ""),
  );
  return a.size >= Be ? o : o.filter((f) => a.has(f));
}
async function Gt(e, t, r, i) {
  let o = [...t];
  if (o.length === 0) return 0;
  let s = await dn(e, ["cat-file", "--batch-check=%(objecttype)"], {
      env: i,
      input: o
        .map(
          (d) => `${d}
`,
        )
        .join(""),
    }),
    a = s.stdout
      .split(`
`)
      .filter((d) => d !== "");
  if (s.exitCode !== 0 || a.length !== o.length) return null;
  let f = o.filter((d, u) => a[u] === "commit");
  if (f.length === 0) return 0;
  let l = [];
  for (let d = 0; d < f.length; d += ve) {
    let u = await dn(e, ["rev-list", "--no-walk=unsorted", "--parents", "--stdin", "--"], {
      env: i,
      input: f
        .slice(d, d + ve)
        .map(
          (y) => `${y}
`,
        )
        .join(""),
    });
    if (u.exitCode !== 0) return null;
    l = l.concat(
      u.stdout
        .split(`
`)
        .flatMap((y) => y.split(" ").slice(1)),
    );
  }
  let c = te(l.filter((d) => tn.test(d) && !t.has(d)));
  return (await Se(e, c, r, i))?.length ?? null;
}
async function xHe(e, t) {
  if (!(await ee(e)).isFile()) throw Error("not a regular file");
  let r = await tt(e, ea());
  try {
    let i = await r.stat();
    if (!i.isFile()) throw Error("not a regular file");
    return i.size > t ? { kind: "too_large", sizeBytes: i.size } : { kind: "read", content: await r.readFile() };
  } finally {
    await r.close();
  }
}
async function Ge(e, t) {
  await Promise.all(
    t.map((r) =>
      re(`${e}${r}`).catch((i) => {
        if (!X(i)) n("dir-sync: could not delete a received pack file (non-fatal)");
      }),
    ),
  );
}
async function R2n(e) {
  await Promise.all(
    [e, `${e}.lock`].map((t) =>
      re(t).catch((r) => {
        if (!X(r)) n("dir-sync: could not delete a temporary bundle file (non-fatal)");
      }),
    ),
  );
}
export {
  fWe,
  dn,
  e_e,
  SDt,
  rrn,
  t_e,
  bDt,
  C2n,
  wDt,
  orn,
  n_e,
  zR,
  XP,
  irn,
  Qq,
  kHe,
  wW,
  hst,
  bae,
  srn,
  mWe,
  arn,
  HHe,
  wae,
  v2n,
  xHe,
  R2n,
};
