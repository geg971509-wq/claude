...
import { Fp, VNe } from "/$bunfs/root/chunk-6k63g5t6.js";
import { ye, cS } from "/$bunfs/root/chunk-988p40e0.js";
import { vs, rJ, qo, pfe, ffe } from "/$bunfs/root/chunk-3s6zpzqb.js";
var G = "gitlab.com",
  B = "bitbucket.org";
function bz(t) {
  if (qo(t)) return "github";
  let e = rJ(t);
  while (e.startsWith("www.")) e = e.slice(4);
  if (e === G) return "gitlab";
  if (e === B) return "bitbucket";
  return "other";
}
function T8n(t) {
  let e = t.trim();
  if (pfe(e)) return null;
  if (e.includes("://"))
    try {
      return new URL(e).hostname || null;
    } catch {
      return null;
    }
  return /^(?:[^@:/]+@)?([^:/]+):/.exec(e)?.[1] ?? null;
}
function g(t) {
  let e = /^([^@:/[\]]+)@([^@:/[\]]+):(.*)$/s.exec(t);
...
function gM() {
  ...
  if (!t?.strictKnownMarketplaces) return null;
...
function CX(t, e) {
  if (!FS()) return null;
  if (t === void 0) return "lockdown";
  if (!Awe()) return "remote_policy_unconsented";
  let r = ye("policySettings")?.extraKnownMarketplaces ?? {};
  if (t.source === "settings" && e !== void 0)
    return (Object.hasOwn(r, e) ? r[e] : void 0)?.source.source !== "settings" ? "lockdown" : null;
  return Object.values(r).some((o) => h(t, o.source)) ? null : "lockdown";
}
function ufn(t, e) {
  return CX(t, e) !== null;
}
function eVe(t, e = []) {
  let r = e.length > 0 ? `: ${e.map((o) => Fp(o)).join(", ")}` : "";
  return `${t} ${k(t, "marketplace")} not updated (managed policy) \u2014 ask your admin${r}`;
}
var hM =
...
function uue(t) {
  return `${t.join(", ")} ${t.length === 1 ? "is" : "are"} disabled by your organization's managed settings (disableSideloadFlags). Plugins, custom agents, and MCP servers can only be loaded from sources your administrator has approved. Ask your administrator to remove disableSideloadFlags from managed settings, or use an approved marketplace / settings file instead.`;
}
function dfn() {
  let t = ye("policySettings");
  if (!t?.blockedMarketplaces) return null;
  return t.blockedMarketplaces;
}
function XDe() {
  let t = dfn();
  return gM() !== null || (t !== null && t.some((e) => e.source !== "skills-dir"));
}
function hI(t) {
  return t === void 0 ? XDe() : !Ip(t);
}
function G2t() {
...
function h(t, e) {
  ...
  switch (t.source) {
    ...
    case "github":
      return t.repo === e.repo && (t.ref || void 0) === (e.ref || void 0) && (t.path || void 0) === (e.path || void 0);
    case "git":
      return (
        M(t.url) === M(e.url) && (t.ref || void 0) === (e.ref || void 0) && (t.path || void 0) === (e.path || void 0)
      );
    case "npm":
      return t.package === e.package;
    case "file":
...
function F(t) {
  switch (t.source) {
    ...
    case "git": {
      if (t.url.includes("://")) {
        if (pfn(t.url)) return null;
        try {
          return new URL(t.url).hostname || null;
        } catch {
          return null;
        }
      }
      return t.url.match(/^[^@]+@([^:]+):/)?.[1] ?? null;
    }
    case "url":
      try {
...
function Qhr(t) {
  if (t.source === "git" && !t.url.includes("://")) {
    let e = g(t.url);
    return e ? l(e.host) : null;
  }
  return YDe(t);
}
function w(t, e, r) {
...
function ffn() {
  let t = gM();
  if (!t) return [];
  return t.filter((e) => e.source === "hostPattern").map((e) => e.hostPattern);
}
var m = "ssh.github.com";
function A(t) {
  return qo(t) || rJ(t) === m;
}
function d(t) {
  let e, r;
  if (t.includes("://")) {
    if (pfn(t)) return null;
    try {
      let s = new URL(t);
      (e = s.hostname), (r = s.pathname.replace(/^\/+/, ""));
    } catch {
      return null;
    }
  } else {
...
function y(t) {
  ...
  if (e !== null && e.includes("*"))
    return (
      n(
        `Invalid owner-wildcard url in policy settings blockedMarketplaces: ${t} (wildcards are only supported in github-form entries, as "<owner>/*"); entry does not match github.com sources`,
        { level: "error" },
      ),
      null
    );
...
function c(t, e) {
  if (t.includes("://"))
    try {
      let o = new URL(t);
      (o.hostname = S(o.hostname)), (o.username = ""), (o.password = ""), (o.search = ""), (o.hash = "");
      try {
        o.pathname = decodeURIComponent(o.pathname);
      } catch {}
      let i = H(o.pathname);
      return (o.pathname = e?.stripDotGit ? v(i) : i), o.toString();
    } catch {
      return t;
    }
  let r = t.match(/^[^@]+@([^:]+)(:.*)$/s);
  return r ? `${S(r[1] ?? "")}${r[2]}` : t;
}
function S(t) {
  let e = l(t);
  return e === m ? vs : e;
...
function M(t) {
  ...
  if (t.includes("://"))
    try {
      let o = new URL(t);
      if (((o.hostname = l(o.hostname)), E.has(o.protocol) || qo(o.hostname))) (o.username = ""), (o.password = "");
      return o.toString();
    } catch {
      return t;
    }
  let e = g(t);
  if (!e) return t;
  let r = e.host.toLowerCase().replace(/\.+$/, "");
  return qo(r) ? `${vs}:${e.path}` : `${e.user}@${r}:${e.path}`;
}
function M9(t) {
  try {
    let e = new URL(t);
    return (e.hostname = l(e.hostname)), e.toString();
  } catch {
    return t;
  }
}
...
var x = /^[A-Za-z0-9._-]+$/;
function f(t) {
  return x.test(t) && !t.startsWith("-") && t !== "." && t !== "..";
}
function C(t) {
...
function p(t, e) {
  ...
  if (o === null) {
    if (t.includes("*"))
      n(
        `Invalid owner-wildcard repo in policy settings blockedMarketplaces: ${t} (only "<owner>/*" is supported); entry only matches a literally identical repo string`,
        { level: "error" },
      );
    return r === t || e === t;
  }
...
function U(t, e) {
  ...
  if (t.source === "git" && e.source === "github") {
    let r = d(t.url);
    if (r !== null && p(e.repo, r)) return a(e.ref, t.ref) && a(e.path, t.path);
  }
  if (t.source === "github" && e.source === "git") {
    let r = y(e.url);
...
function v(t) {
  ...
  for (;;) {
    let r = e;
    while (r > 0 && t.charCodeAt(r - 1) === 47) r--;
    if (r >= 4 && t.startsWith(".git", r - 4)) r -= 4;
    if (r === e) return e === t.length ? t : t.slice(0, e);
    e = r;
  }
}
function rVe(t) {
...
function D(t, e) {
  ...
  if (t.source === "github" && e.source === "github") {
    let r = C(e.repo);
    if (r === null && e.repo.includes("*"))
      n(
        `Invalid owner-wildcard repo in policy settings strictKnownMarketplaces: ${e.repo} (only "<owner>/*" is supported); entry only matches a literally identical repo string`,
        { level: "error" },
      );
    if (r !== null)
      return (
