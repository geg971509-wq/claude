...
// Version: 2.1.252
import { _Pt, Iit } from "/$bunfs/root/chunk-zhvf30zp.js";
...
import { $Le, GM, e$ } from "/$bunfs/root/chunk-0mrbtv3q.js";
import { _ae, O1, Jg, vZ, Onn, $nn, Zit, Unn, THe, Bnn, cWe } from "/$bunfs/root/chunk-0cxwrbmy.js";
import { Htn } from "/$bunfs/root/chunk-5s233wjm.js";
  ...
  let t = new Set(),
    h = (s) => {
      if ((s.tagName ?? "").toLowerCase() === "script") {
        if ((Jg(s, "type") ?? "").trim().toLowerCase() !== "application/json" && Jg(s, "src") === void 0)
          t.add(Nn(y(s)));
      }
...
function o(e, t) {
  ...
  s += i ? " \u2026>" : ">";
  let r = e$(s),
    p = (r.length > 120 ? `${ce(r, N)}\u2026` : r).replace(/\s+/g, " ");
  return t.set(e, p), p;
}
var T = new Set(["href", "xlink:href", "src", "action", "formaction", "poster", "cite"]),
  M = /(^|[\t\n\f\r ])opener([\t\n\f\r ]|$)/i,
  A = new Set(["iframe", "embed", "object", "base", "form", "link", "noscript", "frameset", "frame", "plaintext"]),
...
function G(e, t, h, s) {
  ...
  for (let n of e.attrs ?? []) {
    ...
    if (l.startsWith("on") && l.length > 2) {
      a.push({
        ...
        hint: `Remove the ${e$(ce(n.name, 60))} attribute \u2014 inline event handlers are not allowed; interactivity comes only from the blessed scripts.`,
      });
      continue;
    }
    if (l === "ping") {
    ...
    if (l === "data-anchor" && !Onn.test(n.value)) {
      a.push({
      ...
      for (let v of n.value.split(",")) {
        ...
        if (w !== void 0 && w !== "" && !Xhe(w))
          a.push({
            rule: "unsafe-url",
            where: o(e, t.snippets),
            hint: `srcset entry "${e$(ce(w, 60))}" \u2014 only http(s), mailto, relative, or fragment URLs are allowed.`,
          });
      }
      continue;
    }
    if (r && (l === "href" || l === "xlink:href")) {
      ...
      } else if (!GM(n.value).startsWith("#"))
        a.push({
          rule: "svg-nonlocal-reference",
          where: o(e, t.snippets),
          hint: `<${e$(ce(i, 60))}> may only reference fragments within the page (href="#\u2026").`,
        });
      continue;
    }
    if (T.has(l) && !Xhe(n.value))
      a.push({
        rule: "unsafe-url",
        where: o(e, t.snippets),
        hint: "Only http(s), mailto, relative, or fragment URLs are allowed.",
  ...
  let g =
    !r &&
    i === "script" &&
    (Jg(e, "type") ?? "").trim().toLowerCase() === "application/json" &&
    Jg(e, "src") === void 0 &&
    Jg(e, "id") === "ws-decisions";
  if (Jg(e, "id") === "ws-decisions" && !g)
    t.wsIdMisuse.push(e),
      a.push({
  ...
  if (c !== void 0)
    if (((d = c), !S.test(c)))
      ...
      if (n !== void 0) n.count++;
        ...
        t.items.set(c, {
          state: Jg(e, "data-decision-state") ?? "",
          resolvedChoice: Jg(e, "data-resolved-choice"),
          resolvedCustom: Jg(e, "data-resolved-custom"),
          leanChoice: Jg(e, "data-lean-choice"),
          choices: [],
          count: 1,
        });
    }
  let u = Jg(e, "data-choice");
  if (u !== void 0)
    if (d === null) t.orphanChoices.push(e);
    else if (!S.test(u))
      a.push({ rule: "bad-choice-token", where: o(e, t.snippets), hint: "data-choice must match the slug grammar." });
  ...
  if (i === "script") {
    ...
    if (Jg(e, "src") !== void 0) {
      if (!(Jg(e, "src") === $Le && (e.attrs ?? []).length === 1 && (e.childNodes ?? []).length === 0))
        a.push({
          rule: "script-src",
          where: o(e, t.snippets),
    ...
    if ((Jg(e, "type") ?? "").trim().toLowerCase() === "application/json") {
  ...
  if (!r && i === "meta" && Jg(e, "http-equiv") !== void 0)
    return (
      a.push({
  ...
  if (!r && i === "meta" && ["referrer", "origin-trial"].includes((Jg(e, "name") ?? "").trim().toLowerCase()))
    return (
      a.push({
        rule: "meta-behavior",
        where: o(e, t.snippets),
  ...
  if (r && V.has(i)) {
    let n = (Jg(e, "attributename") ?? "").trim().toLowerCase();
    if (
      n.startsWith("on") ||
      T.has(n) ||
      n === "style" ||
...
function b(e, t, h, s, i = false, r = 0) {
  ...
  if (e.tagName !== void 0) {
    if (
      ((a = G(e, t, h, s)),
      i && (Jg(e, "id") === "ws-decisions" || Jg(e, "data-decision-id") !== void 0 || Jg(e, "data-choice") !== void 0))
    )
      t.violations.push({
      ...
      violations: [
        {
          rule: "verifier-error",
          where: "(document)",
          hint: `The verifier could not examine this page (${e$(s instanceof Error ? s.name : "error")}) \u2014 refusing rather than publishing unexamined markup; simplify the page structure.`,
        },
      ],
    };
  }
      ...
      violations: [
        {
          ...
          where: `(document, first divergence near "\u2026${e$(ce(c, 117).replace(/\s+/g, " "))}\u2026")`,
...
function k(e) {
  if (j.has((e.tagName ?? "").toLowerCase()) && e.namespaceURI === _ae) {
...
function C(e, t, h) {
  ...
  for (let c of s.islands) {
    ...
    let n = e.slice(u.startOffset, u.endOffset),
      l = THe(n, "ws-decisions");
    if (l.length !== 1 || l[0][1] !== n.length)
      r.push({
        rule: "island-noncanonical-spelling",
        where: o(c.node, s.snippets),
        hint: `The ws-decisions island's open tag must END with the exact bytes ${Unn} (double-quoted id attribute, last in the tag, as the template ships it; a page read back from the server may carry the server's own data-id after the id, nothing else) \u2014 the session's mechanical extraction scans for that sequence.`,
      });
      ...
      for (let n of c) {
        ...
        if (l.state !== n.state)
          r.push({
            rule: "island-markup-mismatch",
            where: `decision "${n.id}"`,
            hint: `Island says state "${e$(n.state)}" but the markup says "${e$(ce(l.state, 60))}" \u2014 flip them together.`,
          });
  ...
  for (let i of e.banners)
    if (!U.has(i.value))
      ...
      h.push({
        ...
        where: o(i.node, e.snippets),
        hint: `data-ws-state says "${e$(i.value)}" but the island derives "${s}" \u2014 flip them together (text stays yours; the attribute is the wire contract).`,
      });
}
export { Q as extractInlineScriptHashes, R as republishSerialization, o as snippet, ee as verifyWorkshopHtml };
