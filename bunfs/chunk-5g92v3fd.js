...
// Version: 2.1.252
import { no, Yv } from "/$bunfs/root/chunk-twwd9fep.js";
...
function m(t) {
  let r = k.exec(t);
  return r ? t.slice(0, -r[0].length) : void 0;
}
function w(t, r, a) {
  let i = no(r, t.name);
    ...
    [
      {
        ...
        name: i.name,
        input: t.input,
        ...(t.caller !== void 0 && { caller: t.caller }),
      },
    ]
  );
  try {
...
function V2t(t) {
  let r;
  for (let a of t) if (Yv(a)) (r ??= new Set()), r.add(a.underlyingV1ToolName);
  if (r === void 0) return t;
  return t.filter((a) => !r.has(a.name));
}
function K2t(t, r, a) {
  let i,
    l = [];
...
function E8n(t, r, a) {
  if (r === void 0 || r.length === 0) return t;
  let i = new Map();
  for (let e of r) if (!i.has(e.id)) i.set(e.id, e);
  let l = new Map();
  for (let e of i.values()) {
    let s = no(a, e.name);
    if (s === void 0 || !Yv(s)) continue;
    let u = [],
      f;
    for (let d of t) if (d.type === "tool_use" && m(d.id) === e.id) (f ??= d), u.push(d.input);
    try {
      l.set(e.id, {
        type: "tool_use",
        id: e.id,
        name: e.name,
        input: s.reassemble(u),
  ...
  for (let e of t) {
    if (e.type === "tool_use") {
      let s = m(e.id);
      if (s !== void 0 && l.has(s)) {
        if (!c.has(s)) c.add(s), o.push(l.get(s));
        continue;
      }
    }
    o.push(e);
...
function A8n(t, r) {
  ...
  for (let o of t) {
    ...
    for (let e of o.message.content) if (e.type === "tool_use") i.set(e.id, e);
  }
  let l = new Map();
  for (let o of t) {
    ...
    for (let e of o.message.content) {
      ...
      let s = m(e.tool_use_id);
      if (s === void 0 || !i.has(s)) continue;
      let u = l.get(s);
      if (u === void 0) (u = new Map()), l.set(s, u);
      if (!u.has(e.tool_use_id)) u.set(e.tool_use_id, e);
    }
  }
  if (l.size === 0) return t;
  let c = new Set();
  return t.flatMap((o) => {
    ...
    for (let u of o.message.content) {
      ...
      let p = i.get(f),
        T = no(r, p.name),
        _ = T !== void 0 && Yv(T) ? T : void 0;
      s.push(P(_, f, R(_, p.input, f, d)));
    }
    if (!e) return [o];
...
function R(t, r, a, i) {
  ...
  if (t !== void 0)
    try {
      let o = t.inputSchema.safeParse(r);
      if (o.success) l = t.perEntryHookInputs(o.data).entries;
    } catch {}
  let c = l?.length ?? 0;
  for (let o of i.keys()) {
    let e = k.exec(o);
    if (e) c = Math.max(c, Number(e[1]) + 1);
  }
  if (c > 1000) c = l?.length ?? i.size;
  return Array.from({ length: c }, (o, e) => {
    let s = i.get(B(a, e));
    return s !== void 0 ? { input: l?.[e], result: s } : { input: l?.[e], error: "no result" };
  });
}
function P(t, r, a) {
  let i = [],
...
${s}`;
  ...
  for (let [s, u] of a.entries()) {
    ...
    } else o(`[entry ${s + 1} error: ${u.error ?? "no result"}]`);
  }
  if (a.length === 1 && t !== void 0)
    o(
      `<system-reminder>Tip: ${t.name} accepts multiple entries in one call (\`${t.entryFieldName}: [{...}, {...}]\`). Batching related operations into a single call is faster than issuing them as separate or parallel calls. No action needed for this result.</system-reminder>`,
    );
  return (
    e(),
    {
...
function X2t(t, r) {
  return r.find((a) => a.underlyingV1ToolName === t)?.name ?? t;
}
function due(t) {
  for (let [r, a] of x) if (a.name === t) return r;
  return;
}
export { V2t, K2t, E8n, A8n, vX, Tgt, X2t, due };
