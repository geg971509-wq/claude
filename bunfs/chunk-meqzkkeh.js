...
function f_e(e, t, s, r) {
  ...
  return {
    ...
    mcp: {
      ...e.mcp,
      clients: c ? e.mcp.clients.map((i) => (i.name === t ? s.client : i)) : [...e.mcp.clients, s.client],
      tools: [...e.mcp.tools.filter((i) => !eg(i, t, n)), ...s.tools],
      commands: [...e.mcp.commands.filter((i) => !u_(i, t)), ...s.commands],
...
function f6n(e, t, s) {
  ...
  let r = Bl(t);
  return { ...e, tools: [...e.tools.filter((c) => !eg(c, t, r)), ...s] };
}
function MHe(e, t, s) {
  if (!e.mcp.clients.some((c) => c.name === t && c.type === "connected")) return e;
  let r = Bl(t);
  return { ...e, mcp: { ...e.mcp, tools: [...e.mcp.tools.filter((c) => !eg(c, t, r)), ...s] } };
}
function m6n(e, t, s) {
  ...
  if (a.length > 0) {
    let o = a.map((m) => [m.name, Bl(m.name)]);
    n = {
      ...n,
      clients: [...n.clients, ...a],
      tools: [...n.tools, ...t.tools.filter((m) => o.some(([f, p]) => eg(m, f, p)))],
    };
  }
  return n;
}
export { f_e, f6n, MHe, m6n };
