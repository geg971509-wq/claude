...
function B(t, s = {}) {
  ...
  else if (e && !Y.includes(e)) throw Error(`readdirp: Invalid type passed. Use one of ${Y.join(", ")}`);
...
class O {
  ...
  _watchWithNodeFs(t, s) {
    ...
    } else c = Ct(t, n, o, { listener: s, errHandler: this._boundHandleError, rawEmitter: this.fsw._emitRaw });
    return c;
  }
  _handleFile(t, s, e) {
  ...
  _handleRead(t, s, e, i, r, a, n) {
    ...
    let o = this.fsw._getWatchedDir(e.path),
      c = new Set(),
      h = this.fsw._readdirp(t, { fileFilter: (d) => e.filterPath(d), directoryFilter: (d) => e.filterDir(d) });
    if (!h) return;
    return (
      h
        .on(Tt, async (d) => {
          ...
          let l = d.path,
            m = _.join(t, l);
          if ((c.add(l), d.stats.isSymbolicLink() && (await this._handleSymlink(d, t, m, l)))) return;
          if (this.fsw.closed) {
            h = void 0;
            return;
          }
          if (l === i || (!i && !o.has(l)))
            this.fsw._incrReadyCount(), (m = _.join(r, _.relative(r, m))), this._addToNodeFs(m, s, e, a + 1);
        })
...
var rt = (t) => {
    let s = S(t).flat();
    if (!s.every((e) => typeof e === Gt)) throw TypeError(`Non-string provided as watch path: ${s}`);
    return s.map(ht);
  },
  nt = (t) => {
...
class ct {
  ...
  dispose() {
    this.items.clear(), (this.path = ""), (this._removeWatcher = F), (this.items = Xt), Object.freeze(this);
  }
}
var Zt = "stat",
  ts = "lstat";
...
class qgn extends jt {
  ...
  _addIgnoredPath(t) {
    if (M(t)) {
      for (let s of this._ignoredPaths) if (M(s) && s.path === t.path && s.recursive === t.recursive) return;
    }
  ...
  async _emit(t, s, e) {
    ...
    if (i.atomic) {
      ...
      if (t === u.ADD && this._pendingUnlinks.has(s)) (t = u.CHANGE), this._pendingUnlinks.delete(s);
    }
  ...
  _getWatchHelpers(t) {
    return new fJn(t, this.options.followSymlinks, this);
  }
  _getWatchedDir(t) {
    let s = f.resolve(t);
    ...
    while (i.length > 0) {
      ...
      if (!r.expanded) {
        ...
        if (
          ((r.path = l),
          (r.fullPath = m),
          (r.isDirectory = r.isDirectory != null ? r.isDirectory : this._watched.has(l) || this._watched.has(m)),
...
var GE = { watch: L_r, FSWatcher: qgn };
export { fJn, qgn, L_r, GE };
