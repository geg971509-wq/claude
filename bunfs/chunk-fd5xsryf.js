...
var X = async (t, o) => {
  if (!sn() && Nt() && (rM() || o.dispatchedAsImmediate)) {
    let i = await import("/$bunfs/root/chunk-c4v3zykj.js");
    return e(y, { onDone: t, sidebar: i });
  }
  let { DiffDialog: a } = await import("/$bunfs/root/chunk-k6awtfqt.js");
  return e(a, { messages: o.messages, onDone: t });
};
function y(G) {
  let M = _(9),
    { onDone: s, sidebar: r } = G,
    m = W(h),
    n = At(),
    { storageV5: f } = ge(),
    p = ct().host,
    ...
    (I = () => {
      ...
      if (((D.current = !0), m !== "diff")) {
        ...
        if (l < r.DIFF_SIDEBAR_MIN_COLS) {
          s(`Resize your terminal to at least ${r.DIFF_SIDEBAR_MIN_COLS} columns to show the diff panel`, {
            display: "system",
          });
          return;
        }
...
export { y as ToggleDiffSidebar, X as call };
