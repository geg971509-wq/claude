...
var g = ue("/$bunfs/root/loopAutonomousPreamble-07qcyhv4.md");
...
Use ${xk} when the loop can't move further without the user, or when something landed that they'd want to act on now: ${o}, or a major update arrived (CI went red, a review changes the plan). Progress you made yourself isn't a trigger \u2014 the transcript covers that. One ping per state, not per tick.`;
}
function b() {
  return `# Autonomous loop tick

Run the autonomous check using the loop instructions established earlier in this conversation. If you cannot find them, treat this as a no-op tick. The recurring cron will fire the next tick automatically \u2014 do not call ${pa} from this tick.${h()}`;
}
var f = `

If a ${ma} is armed (check ${hT}), keep \`delaySeconds\` at 1200\u20131800s \u2014 the ${ma} is the wake signal and this is only the fallback heartbeat. If you were woken by a \`<task-notification>\`, handle the event before deciding whether to re-arm. To stop the loop, call ${pa} with \`stop: true\` and ${ny} the monitor (use ${hT} to find its task ID if no longer in context).`;
function x() {
  return `# Autonomous loop tick (dynamic pacing)

Run the autonomous check using the loop instructions established earlier in this conversation. If you cannot find them, treat this as a no-op tick.

You scheduled this tick via the ${pa} tool (not a recurring cron). To keep the loop alive, call ${pa} again at the end of this turn with \`prompt\` set to the literal sentinel \`${Tne}\` and \`noop\` set to \`true\` if this tick changed nothing (or \`false\` if it did) \u2014 otherwise the loop ends after this tick.${f}${h()}`;
}
function L(e) {
  return e === lTe || e === Tne;
}
...
You scheduled this tick via the ${pa} tool (not a recurring cron). To keep the loop alive \u2014 and to pick up loop.md if it is recreated \u2014 call ${pa} again at the end of this turn with \`prompt\` set to the literal sentinel \`${d}\` and \`noop\` set to \`true\` if this tick changed nothing (or \`false\` if it did) \u2014 otherwise the loop ends after this tick.${f}${h()}`;
}
var l = 25000;
function _(e) {
...
function T() {
  return c(u(gn(), ".claude", "loop.md")) ?? c(u(be(), "loop.md"));
}
function c(e) {
  let t;
  try {
    ...
    if (Ht(n) || E(n) === "EISDIR") return null;
    throw n;
  }
  let o = t.trim();
...
async function D(e) {
  if (!e) return T();
  let t = c(u(gn(), ".claude", "loop.md"));
  if (t) return t;
  let o = u(be(), "loop.md"),
    n = await e.read([Te.state("loop-file")]);
  if (!n.ok) return c(o);
  let i = n.value.items[0];
  if (!i.found) return null;
  let r = Buffer.from(i.value.buffer, i.value.byteOffset, i.value.byteLength).toString("utf-8").trim();
