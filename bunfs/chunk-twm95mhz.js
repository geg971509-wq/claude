// @bun @bytecode
// Claude Code is a Beta product per Anthropic's Commercial Terms of Service.
// By using Claude Code, you agree that all code acceptance or rejection decisions you make,
// and the associated conversations in context, constitute Feedback under Anthropic's Commercial Terms,
// and may be used to improve Anthropic's products, including training models.
// You are responsible for reviewing any code suggestions before use.

// (c) Anthropic PBC. All rights reserved. Use is subject to the Legal Agreements outlined here: https://code.claude.com/docs/en/legal-and-compliance.

// Version: 2.1.252
import { Nr } from "/$bunfs/root/chunk-yz031c9r.js";
function U(t) {
  if (t === null || typeof t !== "object") return null;
  return (t = (m && t[m]) || t["@@iterator"]), typeof t === "function" ? t : null;
}
function _(t, e, n) {
  (this.props = t), (this.context = e), (this.refs = h), (this.updater = n || d);
}
function H() {}
function T(t, e, n) {
  (this.props = t), (this.context = e), (this.refs = h), (this.updater = n || d);
}
function a() {}
function O(t, e, n) {
  var r = n.ref;
  return { $$typeof: v, type: t, key: e, ref: r !== void 0 ? r : null, props: n };
}
function q(t, e) {
  return O(t.type, e, t.props);
}
function R(t) {
  return typeof t === "object" && t !== null && t.$$typeof === v;
}
function G(t) {
  var e = { "=": "=0", ":": "=2" };
  return (
    "$" +
    t.replace(/[=:]/g, function (n) {
      return e[n];
    })
  );
}
function y(t, e) {
  return typeof t === "object" && t !== null && t.key != null ? G("" + t.key) : e.toString(36);
}
function K(t) {
  switch (t.status) {
    case "fulfilled":
      return t.value;
    case "rejected":
      throw t.reason;
    default:
      switch (
        (typeof t.status === "string"
          ? t.then(a, a)
          : ((t.status = "pending"),
            t.then(
              function (e) {
                t.status === "pending" && ((t.status = "fulfilled"), (t.value = e));
              },
              function (e) {
                t.status === "pending" && ((t.status = "rejected"), (t.reason = e));
              },
            )),
        t.status)
      ) {
        case "fulfilled":
          return t.value;
        case "rejected":
          throw t.reason;
      }
  }
  throw t;
}
function c(t, e, n, r, o) {
  var i = typeof t;
  if (i === "undefined" || i === "boolean") t = null;
  var s = !1;
  if (t === null) s = !0;
  else
    switch (i) {
      case "bigint":
      case "string":
      case "number":
        s = !0;
        break;
      case "object":
        switch (t.$$typeof) {
          case v:
          case j:
            s = !0;
            break;
          case k:
            return (s = t._init), c(s(t._payload), e, n, r, o);
        }
    }
  if (s)
    return (
      (o = o(t)),
      (s = r === "" ? "." + y(t, 0) : r),
      S(o)
        ? ((n = ""),
          s != null && (n = s.replace(w, "$&/") + "/"),
          c(o, e, n, "", function (P) {
            return P;
          }))
        : o != null &&
          (R(o) &&
            (o = q(o, n + (o.key == null || (t && t.key === o.key) ? "" : ("" + o.key).replace(w, "$&/") + "/") + s)),
          e.push(o)),
      1
    );
  s = 0;
  var l = r === "" ? "." : r + ":";
  if (S(t)) for (var p = 0; p < t.length; p++) (r = t[p]), (i = l + y(r, p)), (s += c(r, e, n, i, o));
  else if (((p = U(t)), typeof p === "function"))
    for (t = p.call(t), p = 0; !(r = t.next()).done; ) (r = r.value), (i = l + y(r, p++)), (s += c(r, e, n, i, o));
  else if (i === "object") {
    if (typeof t.then === "function") return c(K(t), e, n, r, o);
    throw (
      ((e = String(t)),
      Error(
        "Objects are not valid as a React child (found: " +
          (e === "[object Object]" ? "object with keys {" + Object.keys(t).join(", ") + "}" : e) +
          "). If you meant to render a collection of children, use an array instead.",
      ))
    );
  }
  return s;
}
function E(t, e, n) {
  if (t == null) return t;
  var r = [],
    o = 0;
  return (
    c(t, r, "", "", function (i) {
      return e.call(n, i, o++);
    }),
    r
  );
}
var v,
  j,
  N,
  Y,
  I,
  M,
  L,
  k,
  m,
  d,
  b,
  h,
  x,
  S,
  f,
  D,
  w,
  g,
  W,
  Lb,
  Von,
  ml,
  Kon,
  Jn,
  R9e,
  yn = function (t) {
    return (
      (t = { $$typeof: I, _currentValue: t, _currentValue2: t, _threadCount: 0, Provider: null, Consumer: null }),
      (t.Provider = t),
      (t.Consumer = { $$typeof: Y, _context: t }),
      t
    );
  },
  Fx = function (t, e, n) {
    var r,
      o = {},
      i = null;
    if (e != null)
      for (r in (e.key !== void 0 && (i = "" + e.key), e))
        D.call(e, r) && r !== "key" && r !== "__self" && r !== "__source" && (o[r] = e[r]);
    var s = arguments.length - 2;
    if (s === 1) o.children = n;
    else if (1 < s) {
      for (var l = Array(s), p = 0; p < s; p++) l[p] = arguments[p + 2];
      o.children = l;
    }
    if (t && t.defaultProps) for (r in ((s = t.defaultProps), s)) o[r] === void 0 && (o[r] = s[r]);
    return O(t, i, o);
  },
  z_,
  du = function (t, e) {
    return { $$typeof: L, type: t, compare: e === void 0 ? null : e };
  },
  yMt = function (t) {
    var e = f.T,
      n = {};
    f.T = n;
    try {
      var r = t(),
        o = f.S;
      o !== null && o(n, r), typeof r === "object" && r !== null && typeof r.then === "function" && r.then(a, g);
    } catch (i) {
      g(i);
    } finally {
      e !== null && n.types !== null && (e.types = n.types), (f.T = e);
    }
  },
  zn = function (t) {
    return f.H.use(t);
  },
  B = function (t, e) {
    return f.H.useCallback(t, e);
  },
  We = function (t) {
    return f.H.useContext(t);
  },
  vB = function (t, e) {
    return f.H.useDeferredValue(t, e);
  },
  A = function (t, e) {
    return f.H.useEffect(t, e);
  },
  br = function (t) {
    return f.H.useEffectEvent(t);
  },
  Sxe = function (t, e, n) {
    return f.H.useImperativeHandle(t, e, n);
  },
  qae = function (t, e) {
    return f.H.useInsertionEffect(t, e);
  },
  qn = function (t, e) {
    return f.H.useLayoutEffect(t, e);
  },
  z = function (t, e) {
    return f.H.useMemo(t, e);
  },
  fv = function (t, e, n) {
    return f.H.useReducer(t, e, n);
  },
  C = function (t) {
    return f.H.useRef(t);
  },
  u = function (t) {
    return f.H.useState(t);
  },
  Lt = function (t, e, n) {
    return f.H.useSyncExternalStore(t, e, n);
  },
  SMt = function () {
    return f.H.useTransition();
  };
var F = Nr(() => {
  (v = Symbol.for("react.transitional.element")),
    (j = Symbol.for("react.portal")),
    (N = Symbol.for("react.fragment")),
    (Y = Symbol.for("react.consumer")),
    (I = Symbol.for("react.context")),
    (M = Symbol.for("react.suspense")),
    (L = Symbol.for("react.memo")),
    (k = Symbol.for("react.lazy")),
    (m = Symbol.iterator);
  (d = {
    isMounted: function () {
      return !1;
    },
    enqueueForceUpdate: function () {},
    enqueueReplaceState: function () {},
    enqueueSetState: function () {},
  }),
    (b = Object.assign),
    (h = {});
  _.prototype.isReactComponent = {};
  _.prototype.setState = function (t, e) {
    if (typeof t !== "object" && typeof t !== "function" && t != null)
      throw Error(
        "takes an object of state variables to update or a function which returns an object of state variables.",
      );
    this.updater.enqueueSetState(this, t, e, "setState");
  };
  _.prototype.forceUpdate = function (t) {
    this.updater.enqueueForceUpdate(this, t, "forceUpdate");
  };
  H.prototype = _.prototype;
  x = T.prototype = new H();
  x.constructor = T;
  b(x, _.prototype);
  x.isPureReactComponent = !0;
  S = Array.isArray;
  (f = { H: null, A: null, T: null, S: null }), (D = Object.prototype.hasOwnProperty);
  w = /\/+/g;
  (g =
    typeof reportError === "function"
      ? reportError
      : function (t) {
          if (typeof window === "object" && typeof window.ErrorEvent === "function") {
            var e = new window.ErrorEvent("error", {
              bubbles: !0,
              cancelable: !0,
              message:
                typeof t === "object" && t !== null && typeof t.message === "string" ? String(t.message) : String(t),
              error: t,
            });
            if (!window.dispatchEvent(e)) return;
          } else if (typeof process === "object" && typeof process.emit === "function") {
            process.emit("uncaughtException", t);
            return;
          }
          console.error(t);
        }),
    (W = {
      map: E,
      forEach: function (t, e, n) {
        E(
          t,
          function () {
            e.apply(this, arguments);
          },
          n,
        );
      },
      count: function (t) {
        var e = 0;
        return (
          E(t, function () {
            e++;
          }),
          e
        );
      },
      toArray: function (t) {
        return (
          E(t, function (e) {
            return e;
          }) || []
        );
      },
      only: function (t) {
        if (!R(t)) throw Error("React.Children.only expected to receive a single React element child.");
        return t;
      },
    });
  (Lb = W), (Von = _), (ml = N), (Kon = T), (Jn = M), (R9e = f), (z_ = R);
});
export { Lb, Von, ml, Kon, Jn, R9e, yn, Fx, z_, du, yMt, zn, B, We, vB, A, br, Sxe, qae, qn, z, fv, C, u, Lt, SMt, F };
