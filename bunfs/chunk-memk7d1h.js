...
import { Zpe, DN } from "/$bunfs/root/chunk-d7nfr7mk.js";
import { PZ, Dst, tG, Ost } from "/$bunfs/root/chunk-1eqdma2n.js";
import "/$bunfs/root/chunk-98tgyb1d.js";
import { Wa, wle, zx, Du } from "/$bunfs/root/chunk-zze8764r.js";
...
async function N(e, t, a, s, r) {
  try {
    await a, await tG(e, t, s, r);
  } catch (l) {
    n(`removeWatcherSidecar failed: ${String(l)}`);
  }
}
function P(e) {
  return e === "completed" || e === "failed" || e === "cancelled";
}
function B({ taskRegistry: e, registryId: t, registered: a }) {
  let s = e.get(t);
...
function Y(e, t, a) {
  ...
  if (!s)
    (s = new Map()),
      A.set(e, s),
      e.setNotificationHandler(Zpe, (r) => {
        A.get(e)?.get(r.params.taskId)?.(r.params.status, r.params.statusMessage);
      });
  return (
    s.set(t, a),
    () => {
...
var Z = m(() =>
  ...
  V = m(() => T.object({ resource: T.object({ uri: T.string().optional(), text: T.string() }) })),
  J = m(() =>
    T.object({
...
async function Q(e, t, a, s = _ue) {
  let r;
  if (typeof e === "string") r = e;
  else {
    let l = [];
    for (let i of e ?? []) {
      let { text: c, bytesPersisted: f } = await te(i, s, t);
      (s -= f), l.push(c);
    }
    r = l.join(`
`);
  ...
  try {
    let l = await AW(r, a),
      i = typeof l === "string" ? l : r,
      c = $Z();
    if (i === r && (r.length > c || Wt(r).length > c)) i = O(r, c);
    ...
    let f = `mcp-task-result-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
      d = await Qb(Buffer.from(r, "utf8"), "text/plain", f, void 0, t);
    if ("error" in d) return { text: i };
    s -= d.size;
    let o = D() === "windows" ? "python" : "python3",
      g = d.filepath.replaceAll("\\", "/").replaceAll("'", "'\\''"),
      k = ee(r)
    ...
    return { text: i, savedHint: `[The complete ${r.length}-character output was saved to ${d.filepath}; ${k}.]` };
  } catch {
    return { text: ce(r, $Z()) };
  }
}
...
async function te(e, t, a) {
  ...
  if (!s.success) {
    let i = V().safeParse(e);
    if (i.success) {
      let { uri: f, text: d } = i.data.resource;
      return { text: f ? `[Resource at ${f}] ${d}` : d, bytesPersisted: 0 };
    }
    let c = J().safeParse(e);
    if (c.success) {
      let { name: f, uri: d, description: o } = c.data;
      return { text: `[Resource link: ${f}] ${d}${o ? ` (${o})` : ""}`, bytesPersisted: 0 };
    }
  ...
  let r =
    ...
    l = (r.data.length * 3) / 4;
  if (l > _ue) return { text: `[${e.type} content too large to save]`, bytesPersisted: 0 };
  if (l > t) return { text: `[${e.type} content skipped: per-result persist budget exhausted]`, bytesPersisted: 0 };
  try {
    let i = `mcp-task-result-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
      c = await Qb(Buffer.from(r.data, "base64"), r.mimeType, i, void 0, a);
    if ("error" in c)
      return {
        text: `[${e.type} content (${r.mimeType ?? "unknown type"}) could not be saved to disk: ${c.error}]`,
        bytesPersisted: 0,
      };
    return { text: P2(c.filepath, r.mimeType, c.size, `[${e.type} result] `), bytesPersisted: c.size };
  } catch (i) {
    return (
      n(`persisting MCP task result block failed: ${String(i)}`, { level: "error" }),
...
function M(e) {
  ...
  if (t === "") return;
  return t.length > L ? `${ce(t, L)}\u2026 [truncated]` : t;
}
function j(e) {
  let a = `MCP task ${$9(e.mcpTaskId)} (${ZZ(e.serverName, e.toolName)}) ${e.status}.`,
    s = M(e.statusMessage) ?? "no detail",
    r =
      e.status === "completed"
        ? (e.resultText ?? "")
        : e.status === "failed"
...
function O(e, t) {
  let s = Math.max(0, Math.floor(e.length * (t / Wt(e).length)));
  for (;;) {
    let r = ce(e, s);
    if (Wt(r).length + 13 <= t || s === 0) return r + "\u2026 [truncated]";
    s = Math.floor(s * 0.9);
  }
}
function ae(e) {
  return ie(e).catch((t) => h(t));
}
async function ie({ client: e, taskRegistry: t, taskState: a, pollIntervalMs: s, storageV5: r, credentials: l }) {
  let { id: i, mcpTaskId: c, serverName: f, toolName: d } = a,
    o = a.mcpStatus,
    g = a.statusMessage,
    k = t.get(i) !== void 0,
    v = K(),
    ...
    b = Dst(
      ...
      r,
    ).catch((S) => n(`writeMcpTaskMetadata ${i}: ${String(S)}`));
  if (t.get(i)?.status === "running")
    t.update(i, (S) => ({ ...S, sidecarWrite: b, sidecarSessionId: v, sidecarProjectDir: I }));
  let E = (S, u) => {
      let C = M(u);
      if (S === o && C === g) return;
      if (P(o) && !P(S)) return;
      (o = S), (g = C), t.update(i, (F) => ({ ...F, mcpStatus: S, statusMessage: C }));
    },
    U = Y(e, c, E),
    W = Math.min(Math.max(s ?? z, X), q),
    _ = 0,
    x;
  try {
    while (!P(o)) {
      if (o === "input_required")
        try {
          await e.experimental.tasks.getTaskResult(c, DN);
        } catch (u) {
          n(`mcp task ${c} getTaskResult during input_required: ${u}`);
        }
      if ((await ne(W), B({ taskRegistry: t, registryId: i, registered: k }))) {
        e.experimental.tasks
          .cancelTask(c, { signal: AbortSignal.timeout(wle) })
          .catch((u) => n(`mcp task ${c} cancel after kill: ${u}`)),
          N(i, r, b, v, I);
        return;
      }
      try {
        let u = await e.experimental.tasks.getTask(c);
        (_ = 0), E(u.status, u.statusMessage);
      } catch (u) {
        if ((_++, n(`mcp task ${c} poll failed: ${u}`), _ >= G)) {
          (o = "failed"), (g = M(`Task polling failed repeatedly: ${String(u)}`)), (x = "poll_failed_repeatedly");
          break;
        }
      }
    }
    ...
    if (o === "completed")
      try {
        let u = await e.experimental.tasks.getTaskResult(c, DN);
        S = await Q(u.content ?? [], r, l);
      } catch (u) {
        (o = "failed"), (g = M(`Failed to fetch task result: ${String(u)}`)), (x = "result_fetch_failed");
      }
    if (B({ taskRegistry: t, registryId: i, registered: k })) {
      e.experimental.tasks
        .cancelTask(c, { signal: AbortSignal.timeout(wle) })
        .catch((u) => n(`mcp task ${c} cancel after kill: ${u}`)),
        N(i, r, b, v, I);
    ...
    else if (o === "cancelled") p("mcp_task_complete", "cancelled_by_server");
    else p("mcp_task_complete", x ?? "failed");
    t.update(i, (u) => ({
      ...u,
...
async function Pe(e) {
  ...
  let t;
  try {
    t = await Ost(e.storageV5);
  } catch (a) {
    p("mcp_task_restore", "list_failed"), n(`restoreMcpTasks list failed: ${String(a)}`);
    return;
  }
  for (let a of t) {
    if (a.protocol === "sep2663") {
      n(`restoreMcpTasks: sep2663 sidecar ${a.taskId} needs the v2 runtime; parked`);
      continue;
    }
    if (a.protocol !== void 0) {
      n(`restoreMcpTasks: sidecar ${a.taskId} has unknown protocol '${a.protocol}'; parked`, { level: "error" });
      continue;
    }
    oe(a, e).catch((s) => n(`restoreMcpTasks ${a.taskId}: ${String(s)}`));
  }
  y("mcp_task_restore");
}
async function oe(e, { taskRegistry: t, getMcpClients: a, storageV5: s, credentials: r }) {
  let l = t.get(e.taskId);
  if (l?.type === "mcp_task" && l.status === "running" && l.sidecarWrite !== void 0) return;
  let i = {
  ...
  while (Date.now() < c) {
    ...
    if (o?.type === "cached") {
      let g = R0();
      if (!g) {
        d = `server '${e.serverName}' has no MCP session wired`;
        break;
      }
      try {
        f = Ji((await g(o, { timeoutMs: Math.max(0, c - Date.now()), context: "MCP task restore" })).client);
        break;
      } catch (k) {
        d = `server '${e.serverName}' failed to connect: ${k instanceof Error ? k.message : String(k)}`;
        break;
      }
    }
    if (o?.type === "failed" || o?.type === "disabled" || o?.type === "needs-auth") {
      d = `server '${e.serverName}' is ${o.type}`;
      break;
    }
    await ne(500);
  }
  if (!f) {
    ...
    Wa({
      value: j({
        ...
        toolName: e.toolName,
        status: "failed",
        statusMessage: `Could not reconnect after resume: ${d}`,
      }),
      mode: "task-notification",
      agentId: et(),
      priority: "next",
  ...
  if (t.get(e.taskId)?.status !== "running") {
    w(e.taskId, s),
      f.experimental.tasks
        .cancelTask(e.mcpTaskId, { signal: AbortSignal.timeout(wle) })
        .catch((o) => n(`mcp task ${e.mcpTaskId} cancel after kill: ${o}`));
    return;
  }
  ae({ client: f, taskRegistry: t, taskState: i, pollIntervalMs: e.pollIntervalMs, storageV5: s, credentials: r });
}
export {
  q as MAX_POLL_INTERVAL_MS,
  X as MIN_POLL_INTERVAL_MS,
  M as boundMcpStatusMessage,
  j as buildMcpTaskNotification,
  Q as mcpContentToNotificationText,
  Pe as restoreMcpTasks,
