...
import { mu, DDe } from "/$bunfs/root/chunk-pdf5n9dy.js";
...
import { Z9, HOe, AXn, Nmn, $2, sTe, xue, aTe } from "/$bunfs/root/chunk-vzc4dhqq.js";
...
import { i, v, H, f, ot, oe } from "/$bunfs/root/chunk-saay52v7.js";
...
var s = m(() => ot({})),
  p = m(() =>
    f({
      content: i().describe(`Rendered event envelopes, or "${HOe}"`),
      eventCount: v().int().describe("Number of events delivered in this result"),
      remainingWakeCount: v()
        .int()
        .describe("Wake events still queued after this chunk; they follow in the next delivery"),
      provenance: H(
        f({ authority: oe(Nmn), senderId: i().optional(), senderText: i().optional() }).nullable(),
      ).optional(),
    }),
  ),
  O = kt({
    ...
    async call(t, e) {
      if (e.agentId !== void 0) throw new R("Poll is available on the main thread only");
      let n = e.messageQueue;
      if (!n.tryBeginPollCall()) return { data: { content: HOe, eventCount: 0, remainingWakeCount: 0 } };
      try {
        return await c(n, e.abortController.signal);
      } finally {
        n.endPollCall();
      }
    },
...
async function c(t, e) {
  let n = { data: { content: HOe, eventCount: 0, remainingWakeCount: 0 } };
  for (;;) {
    ...
    if (t.peek(mu) !== void 0) return n;
    await new Promise((a) => {
      function o() {
        l(), e.removeEventListener("abort", o), a();
      }
      let l = t.subscribe(o);
      if ((e.addEventListener("abort", o), e.aborted || t.peek(mu) !== void 0)) o();
    });
  }
}
export { O as PollTool };
