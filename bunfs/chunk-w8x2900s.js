...
import { Fd } from "/$bunfs/root/chunk-e53y7x75.js";
import "/$bunfs/root/chunk-5n1tbe50.js";
import "/$bunfs/root/chunk-tz5bx4mb.js";
import { OSe, lF, cc, TE, nm, AD, Cqe, tH } from "/$bunfs/root/chunk-zze8764r.js";
...
import { m1 } from "/$bunfs/root/chunk-9pc6y6sc.js";
...
async function S(a, n, r) {
  ...
  if (a.length === 1)
    switch (l) {
      ...
      case "uninstall": {
        let s = nm(),
          i = Object.entries(s.plugins).filter(([, t]) => t.some(AD));
      ...
      case "i": {
        ...
        return c(await f(g.of(G().host), r), n);
      }
      case "list":
      case "ls":
        return c(p, n);
...
async function f(a, n) {
  let r = await cc(n),
    ...
    i = a.get(s);
  if (i === void 0) {
    let e = await Promise.all(l.map(async (t) => ({ name: t, marketplace: await TE(t, n) })));
    i = [];
    for (let { name: t, marketplace: o } of e) {
      if (!o) continue;
      for (let u of o.plugins) i.push({ pluginId: lF(u.name, t), description: u.description });
    }
