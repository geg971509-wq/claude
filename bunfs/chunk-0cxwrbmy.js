...
import { Gne } from "/$bunfs/root/chunk-0mrbtv3q.js";
import { Q } from "/$bunfs/root/chunk-wag5ye9w.js";
var _ae = "http://www.w3.org/1999/xhtml";
function O1(e) {
  return e;
}
function QBn(e) {
...
function lWe(e) {
  if (e.length === 0 || e.length % 4 !== 0 || e.length > Y || !ee.test(e)) return null;
...
var ZBn = /^<!--ws-decision-[0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}-\d+-->\s*$/;
function ne(e) {
  let n = {},
    t = [];
...
`)) {
  ...
  if (t.length < L || t.length > O) return null;
  if (new Set(t.map((l) => l.token)).size !== t.length) return null;
  let f;
  if (n.lean !== void 0) {
    let l = n.lean.match(N);
    if (!l) return null;
    let p = l[1],
      d = l[2]?.trim();
    if (d !== void 0 && (d === "" || d.length > H)) return null;
    if (!t.some((m) => m.token === p)) return null;
    f = { token: p, reason: d };
  }
  if (o !== void 0 && !Onn.test(o)) return null;
...
function Lnn(e) {
  let n = _(e),
    ...
    r =
      `data-decision-id="${Eo(e.id)}" data-decision-state="${t}"` +
      (e.lean === void 0 ? "" : ` data-lean-choice="${Eo(e.lean.token)}"`) +
      (e.resolved === void 0 ? "" : ` data-resolved-choice="${Eo(e.resolved)}"`) +
      (a === null ? "" : ` data-resolved-custom="${Eo(a)}"`) +
      (e.anchor === void 0 ? "" : ` data-anchor="${Eo(e.anchor)}"`),
    o = e.lean,
    c = e.options
      .map((d) => {
        let m = `<span class="option-label">${Eo(d.label)}</span>`,
          g = Eo(d.token),
          i = o !== void 0 && d.token === o.token,
          u =
            o !== void 0 && d.token === o.token && o.reason !== void 0
              ? `<span class="why">${Eo(o.reason)}</span>`
              : "";
        if (n) {
    ...
    f =
      e.custom !== void 0
        ? `<p class="decided">Decided: ${Eo(e.custom)}</p>`
        : s === void 0
          ? ""
          : `<p class="decided">Decided: ${Eo(s.label)}</p>`,
    l = e.anchor === void 0 ? "" : `<p class="anchor">Anchor: <code>${Eo(e.anchor)}</code></p>`,
    p =
      n || e.id === E
        ? ""
        : `<div class="custom-answer"><input class="option-input" type="text" maxlength="${R}" disabled aria-disabled="true" title="Deciding from the page needs this Artifact to be able to update itself"` +
          ' placeholder="Or type your own answer\u2026" aria-label="Your own answer"></div>';
  return (
    `<div class="call-item" ${r}>` +
    '<span class="marker" aria-hidden="true">\u25CF</span>' +
    `<div class="call-body"><p><span class="q">${Eo(e.question)}</span></p>` +
    f +
    l +
    `<div class="options">${c}${p}</div></div></div>`
  );
}
var Mnn = "<!--ws-decisions-island-->",
  Nnn = "<!--ws-status-banner-->",
  Fnn = "<!--ws-status-footer-->";
function e2n(e) {
    ...
    a =
      n === "started"
        ...
        : n === "ready"
          ? "Ready to build"
          : s > 0
            ? `In progress \u2014 ${s} ${k(s, "decision")} open`
            : "In progress";
...
function n2n(e, n) {
  let t = e.resolved,
    ...
    a =
      `data-decision-id="${Eo(e.id)}" data-decision-state="${s}"` +
      (t === void 0 ? "" : ` data-resolved-choice="${Eo(t)}"`),
    r = e.options
      .map((c) => {
        let f = `<span class="option-label">${Eo(c.label)}</span>`,
          l = Eo(c.token);
        if (t !== void 0)
...
function ie(e) {
  return `<div class="ws-deliverables"><ul>${e.map((t) => `<li class="ws-deliverable" ${$nn}="${t.kind}"><a href="${Eo(t.url)}" target="_blank" rel="noopener noreferrer">${Eo(t.label)}</a></li>`).join("")}</ul></div>`;
}
function Zit(e) {
  let n = e.slice(0, S),
  ...
  return {
    n: n.length,
    pr: Q(n, (s) => t(s) === "pr"),
    artifact: Q(n, (s) => t(s) === "artifact"),
    other: Q(n, (s) => t(s) === "other"),
  };
}
function r2n(e, n, t = Lnn) {
  ...
  for (let i of a) {
    ...
    if (i.type === "code" && "lang" in i && typeof i.lang === "string" && i.lang.trim() === K && o.size < T) {
      let u = ne(i.text);
      if (u !== null && !o.has(u.id)) o.add(u.id), r.push({ offset: c, raw: i.raw, decision: u });
    } else if (i.type === "code" && "lang" in i && typeof i.lang === "string" && i.lang.trim() === W) {
      let u = re(i.text);
      if (u !== null && f + u.length <= S) (f += u.length), r.push({ offset: c, raw: i.raw, deliverables: u });
    }
    c += i.raw.length;
  }
...
function Bnn(e) {
  ...
  let s = n.items;
  if (!Array.isArray(s) || s.length > T) return null;
  let a = [],
    r = new Set();
  for (let o of s) {
    ...
    let g = f.custom;
    if (g !== null && (typeof g !== "string" || lWe(g) === null)) return null;
...
function ce(e) {
  return e.id === E && e.opts.length === 2 && y.every((n) => e.opts.includes(n)) && e.custom === null;
}
function cWe(e) {
  let n = e.find(ce);
