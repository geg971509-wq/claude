// @bun @bytecode
// Claude Code is a Beta product per Anthropic's Commercial Terms of Service.
// By using Claude Code, you agree that all code acceptance or rejection decisions you make,
// and the associated conversations in context, constitute Feedback under Anthropic's Commercial Terms,
// and may be used to improve Anthropic's products, including training models.
// You are responsible for reviewing any code suggestions before use.

// (c) Anthropic PBC. All rights reserved. Use is subject to the Legal Agreements outlined here: https://code.claude.com/docs/en/legal-and-compliance.

// Version: 2.1.252
import { J, G, K } from "/$bunfs/root/chunk-f9h0bg01.js";
import { Me } from "/$bunfs/root/chunk-qq1mdtb5.js";
import { E, X, z4 } from "/$bunfs/root/chunk-ypdw393e.js";
import { be } from "/$bunfs/root/chunk-gcks6mn0.js";
import { H4 } from "/$bunfs/root/chunk-qk7r0t2g.js";
import { Wf, St } from "/$bunfs/root/chunk-4ddxwr9r.js";
import { tr, Bn, oc, JO, u$e, HR, _r, ns, PU, VC, Ms, Qi, RP, UJ } from "/$bunfs/root/chunk-56sxk8k2.js";
import { O } from "/$bunfs/root/chunk-dqkj2bph.js";
import { appendFile as ee, mkdir as Be, rename as Ue, stat as Kt, symlink as Zt, unlink as te } from "fs/promises";
import { dirname as _, isAbsolute as Jt, join as v, resolve as Ve } from "path";
import { basename as N, dirname as H, isAbsolute as et, resolve as T, sep as de } from "path";
function Fe(e, t) {
  return { code: "InvalidArgument", argument: e, ...(t !== void 0 && { reason: t }) };
}
function Mi(e) {
  return { code: "NotFound", key: e };
}
function HFe(e) {
  return { code: "ScopeNotFound", scope: e };
}
function gb(e, t) {
  return { code: "AlreadyExists", key: e, ...(t !== void 0 && { recordId: t }) };
}
function hb(e, t) {
  return { code: "PreconditionFailed", key: e, currentVersion: t };
}
function xFe(e, t) {
  return {
    code: "LeaseHeld",
    key: e,
    ...(t?.holder !== void 0 && { holder: t.holder }),
    ...(t?.meta !== void 0 && { meta: t.meta }),
    ...(t?.expiresAtMs !== void 0 && { expiresAtMs: t.expiresAtMs }),
  };
}
function tc(e = "unknown", t) {
  return {
    code: "Unavailable",
    failureClass: e,
    ...(t?.key !== void 0 && { key: t.key }),
    ...(t?.retryAfterMs !== void 0 && { retryAfterMs: t.retryAfterMs }),
    ...(t?.telemetryCode !== void 0 && { telemetryCode: t.telemetryCode }),
  };
}
function Pr(e, t) {
  return {
    code: "Failed",
    failureClass: e,
    ...(t?.key !== void 0 && { key: t.key }),
    ...(t?.cause !== void 0 && { cause: t.cause }),
    ...(t?.telemetryCode !== void 0 && { telemetryCode: t.telemetryCode }),
  };
}
var je = new Set(["EACCES", "EPERM"]),
  Ke = new Set(["EMFILE", "ENFILE", "ENOSPC", "EDQUOT", "ENOMEM"]),
  vJ = "OtherNames",
  Clr = "StreamOverBudget",
  vlr = "StreamUnreadable",
  vm = "LeafMoved",
  ove = "HardeningUnavailable",
  RJ = "RemoteLink",
  IFe = "AsideStranded";
function aCt(e) {
  return Pr("environment", { telemetryCode: IFe, cause: e });
}
function ckn(e) {
  return (
    Xoe(e) &&
    e.code === "Failed" &&
    e.telemetryCode === IFe &&
    e.cause instanceof Error &&
    "entry" in e.cause &&
    (e.cause.entry === "occupant" || e.cause.entry === "source") &&
    "path" in e.cause &&
    typeof e.cause.path === "string"
  );
}
var Rlr = "PublishUnproven",
  Sj = "ModeUnsupported",
  lCt = "Unsupported";
function bj(e) {
  return Pr("environment", { ...(e !== void 0 && { key: e }), telemetryCode: lCt });
}
function ive(e) {
  return Xoe(e) && e.code === "Failed" && e.telemetryCode === lCt;
}
var Ze = "AttestedAbsent";
function cCt() {
  return Pr("environment", { telemetryCode: Ze });
}
var klr = "ObjectChanged",
  ae = "ByteViewUnsupported";
function ukn(e) {
  return Pr("environment", { key: e, telemetryCode: ae });
}
function PFe(e) {
  return e.code === "Failed" && "telemetryCode" in e && e.telemetryCode === ae;
}
var Je = "StoreFenced";
function dkn(e) {
  return e.code === "Failed" && "telemetryCode" in e && e.telemetryCode === Je;
}
var uCt = "NotShared",
  Hlr = "LeaseRecordUnreadable",
  pkn = "SourceAbsent",
  xlr = "SourceChanged",
  QJe = "SourceNotRegular",
  Lfe = "SourceTooLarge",
  dCt = "SourceShared",
  DFe = "SourceOutside",
  pCt = "SourceRemote",
  Ilr = "SourceInsideStore",
  sve = "SourceAborted";
function ZJe(e) {
  return e === pkn ? "unknown" : "environment";
}
var Ye = new Set(["EROFS", "ENODEV", "ENOTSUP", "ELOOP", "EISDIR", "ENXIO", "ENOTDIR", vJ, vm, ove, "ENAMETOOLONG"]);
function XT(e) {
  if (e === void 0) return "unknown";
  if (je.has(e)) return "permission";
  if (Ke.has(e)) return "resource";
  if (Ye.has(e)) return "environment";
  return "unknown";
}
var Xe = new Set([
  "InvalidArgument",
  "NotFound",
  "AlreadyExists",
  "PreconditionFailed",
  "LeaseHeld",
  "Unavailable",
  "Failed",
  "ScopeNotFound",
]);
function Xoe(e) {
  return typeof e === "object" && e !== null && "code" in e && typeof e.code === "string" && Xe.has(e.code);
}
var Yoe = "AbsentParent";
function bg(e) {
  return Pr("unknown", { telemetryCode: Yoe, key: e });
}
function kJ(e) {
  return e.code === "Failed" && "telemetryCode" in e && e.telemetryCode === Yoe;
}
function iu(e) {
  if (kJ(e)) return "ENOENT";
  return "telemetryCode" in e ? e.telemetryCode : void 0;
}
var Qe = "ListTooLarge",
  ave = "TooLarge";
function fCt() {
  return Pr("environment", { telemetryCode: Qe });
}
function Ge(e) {
  return (
    e.code +
    ("failureClass" in e ? ` ${e.failureClass}` : "") +
    ("telemetryCode" in e && e.telemetryCode ? ` ${e.telemetryCode}` : "") +
    ("cause" in e && e.cause ? `: ${qe(e.cause)}` : "")
  );
}
function qe(e) {
  return e instanceof Error ? e.message : String(e);
}
function Plr(e) {
  return e === vJ || e === vm || e === ove;
}
function re(e) {
  return { ok: !0, value: e };
}
function P(e) {
  return { ok: !1, error: e };
}
class ue extends Error {
  constructor(e) {
    super("Result was an error (see .error)");
    (this.name = "ResultError"), Object.defineProperty(this, "error", { value: e, enumerable: !1 });
  }
}
function fkn(e) {
  if (e.ok) return e.value;
  throw new ue(e.error);
}
var mkn = ["home", "workspace", "system", "userNamed"],
  su = {
    home: (e) => ({ space: "home", path: e }),
    workspace: (e) => ({ space: "workspace", path: e }),
    system: (e) => ({ space: "system", path: e }),
    userNamed: (e) => ({ space: "userNamed", path: e }),
  };
function tt(e, t, r) {
  return typeof e === "string" && e.length > 0 && !e.includes("\x00") && et(e) ? void 0 : Fe(t, r);
}
function SVt(e, t = "path") {
  if (typeof e !== "object" || e === null) return Fe(t, "a host path { space, path }");
  let { space: r, path: i } = e;
  if (typeof r !== "string" || !mkn.includes(r))
    return Fe(t, "space must be 'home', 'workspace', 'system' or 'userNamed'");
  return tt(i, t, "an absolute path with no NUL byte");
}
function eQe(e) {
  if (typeof e !== "object" || e === null) return e;
  let { space: t, path: r } = e;
  return { space: t, path: r };
}
var nt = ["completion.zsh", "completion.bash", "completion.fish"],
  rt = "a path under the config home is the store's own state: address it by its key, not as a host file";
function gkn(e = "path") {
  return Fe(e, rt);
}
function hkn(e, t, r) {
  let i = T(e);
  for (let a of t.heldRoots ?? []) {
    let u = T(a);
    if (i === u || bVt(i, u)) return !0;
  }
  let o = T(t.globalConfigFile);
  if (i === o || (H(i) === H(o) && N(i).startsWith(N(o) + "."))) return !0;
  if (r?.configHomeRuleOff === !0) return !1;
  let s = T(t.configHome);
  if (i === s) return r?.admitRoot !== !0;
  if (!bVt(i, s)) return !1;
  return !(r?.admitSanctionedFiles === !0 && H(i) === s && nt.includes(N(i)));
}
function bVt(e, t) {
  let r = t.endsWith(de) ? t : t + de;
  return e !== t && e.startsWith(r);
}
function Dlr(e, t) {
  let r =
    (i) =>
    async (...o) => {
      let s = t();
      return s !== void 0 ? P(s) : i(...o);
    };
  return {
    serves: (i) => e.serves(i),
    serving: (i) => e.serving(i),
    readText: r(e.readText.bind(e)),
    readBytes: r(e.readBytes.bind(e)),
    write: r(e.write.bind(e)),
    ensureFolder: r(e.ensureFolder.bind(e)),
    listFolder: r(e.listFolder.bind(e)),
    readLink: r(e.readLink.bind(e)),
    access: r(e.access.bind(e)),
    rename: r(e.rename.bind(e)),
    delete: r(e.delete.bind(e)),
    realPath: r(e.realPath.bind(e)),
    stat: r(e.stat.bind(e)),
  };
}
function ce(e, t) {
  return e === void 0 || (typeof e === "number" && Number.isInteger(e) && e >= 0 && e <= 4095)
    ? void 0
    : Fe(t, "mode must be an integer 0..0o7777");
}
function R(e, t, r) {
  return e === void 0 || typeof e === "boolean" ? void 0 : Fe(t, r + " must be a boolean");
}
function D(e) {
  return e === void 0 || (typeof e === "object" && e !== null) ? void 0 : Fe("opts", "options must be an object");
}
function Olr(e, t, r) {
  let i = typeof e === "object" && e !== null ? e.space : void 0;
  if (typeof t !== "string" && !(t instanceof Uint8Array)) return Fe("data", "data must be a string or a Uint8Array");
  let o = r ?? {},
    s = D(r) ?? ce(o.mode, "opts");
  if (s !== void 0) return s;
  if (o.publish === void 0 || o.publish === "inPlace")
    return (
      R(o.exclusive, "opts", "exclusive") ??
      (o.symlinks === void 0 && o.refuseLinkedParent === void 0 && o.stagingFolder === void 0
        ? void 0
        : Fe("opts", "symlinks, refuseLinkedParent and stagingFolder belong to publish 'hardenedAtomic'"))
    );
  if (o.publish !== "hardenedAtomic") return Fe("opts", "publish must be 'inPlace' or 'hardenedAtomic'");
  if (typeof t !== "string") return Fe("data", "a hardened write takes text: data must be a string");
  if (o.exclusive !== void 0) return Fe("opts", "exclusive belongs to publish 'inPlace'");
  if (o.symlinks !== void 0 && o.symlinks !== "refuse" && o.symlinks !== "through")
    return Fe("opts", "symlinks must be 'refuse' or 'through'");
  let a =
    R(o.refuseLinkedParent, "opts", "refuseLinkedParent") ??
    (o.refuseLinkedParent === !0 && o.symlinks === "through"
      ? Fe("opts", "refuseLinkedParent cannot hold under symlinks 'through'")
      : void 0);
  if (a !== void 0 || o.stagingFolder === void 0) return a;
  let u = SVt(o.stagingFolder, "opts.stagingFolder");
  if (u !== void 0) return u;
  return i === void 0 || o.stagingFolder.space === i
    ? void 0
    : Fe("opts.stagingFolder", "the staging folder must be in the same space as the path");
}
function Llr(e) {
  let t = e ?? {};
  return D(e) ?? ce(t.mode, "opts") ?? R(t.recursive, "opts", "recursive");
}
function Mlr(e) {
  let t = e ?? {};
  return (
    D(e) ??
    (t.mode === void 0 || t.mode === "exists" || t.mode === "read" || t.mode === "write" || t.mode === "execute"
      ? void 0
      : Fe("opts", "mode must be 'exists', 'read', 'write' or 'execute'"))
  );
}
function Nlr(e) {
  let t = e ?? {};
  return D(e) ?? R(t.missingOk, "opts", "missingOk");
}
function Flr(e, t) {
  let r = SVt(t, "to");
  if (r !== void 0) return r;
  let i = typeof e === "object" && e !== null ? e.space : void 0;
  return t.space === i ? void 0 : Fe("to", "a rename stays within one space");
}
function $lr(e) {
  let t = e ?? {};
  return D(e) ?? R(t.follow, "opts", "follow");
}
function Ulr(e) {
  let t = e ?? {};
  return D(e) ?? R(t.native, "opts", "native");
}
var B = globalThis.process?.getBuiltinModule?.("async_hooks"),
  fe = B !== void 0,
  U = B
    ? new B.AsyncLocalStorage()
    : {
        run: (e, t) => t(),
        getStore: () => {
          return;
        },
      };
function M({
  writeBatch: e,
  sizeOf: t,
  flushIntervalMs: r = 1000,
  maxBufferSize: i = 100,
  maxBufferBytes: o = 1 / 0,
  immediateMode: s = !1,
}) {
  let a = [],
    u = 0,
    d = null,
    c = null;
  function p() {
    if (d) clearTimeout(d), (d = null);
  }
  function g(y) {
    try {
      e(y);
    } catch {}
  }
  function h() {
    if (c) g(c), (c = null);
    if (a.length === 0) return;
    g(a), (a = []), (u = 0), p();
  }
  function k() {
    if (!d) d = setTimeout(h, r);
  }
  function m() {
    if (c) {
      c.push(...a), (a = []), (u = 0), p();
      return;
    }
    let y = a;
    (a = []),
      (u = 0),
      p(),
      (c = y),
      setImmediate(() => {
        let S = c;
        if (((c = null), S)) g(S);
      });
  }
  return {
    write(y) {
      if (s) {
        g([y]);
        return;
      }
      if ((a.push(y), (u += t(y)), k(), a.length >= i || u >= o)) m();
    },
    flush: h,
    dispose() {
      h();
    },
  };
}
function mCt({ writeFn: e, flushIntervalMs: t, maxBufferSize: r, maxBufferBytes: i, immediateMode: o }) {
  return M({
    writeBatch: (s) => e(s.join("")),
    sizeOf: (s) => s.length,
    flushIntervalMs: t,
    maxBufferSize: r,
    maxBufferBytes: i,
    immediateMode: o,
  });
}
function it(e) {
  if (typeof e === "function") return e;
  if (Symbol.asyncDispose in e) return () => e[Symbol.asyncDispose]();
  return () => e[Symbol.dispose]();
}
class W {
  #e = new Set();
  #t = !1;
  get drainStarted() {
    return this.#t;
  }
  register(e) {
    let t = it(e);
    this.#e.add(t);
    let r = () => {
      this.#e.delete(t);
    };
    return Object.assign(r, { [Symbol.dispose]: r });
  }
  async drain() {
    this.#t = !0;
    let e = Array.from(this.#e);
    this.#e.clear();
    let r = (await Promise.allSettled(e.map(async (i) => i()))).find((i) => i.status === "rejected");
    if (r !== void 0) throw r.reason;
  }
  async [Symbol.asyncDispose]() {
    await this.drain();
  }
  get sizeForTesting() {
    return this.#e.size;
  }
}
class ge {
  cleanup = new W();
  preExitFlush = new W();
}
var ot = new J(() => new ge());
function F() {
  return ot.of(G().host);
}
var Mfe = 2000;
function vt(e) {
  return F().cleanup.register(e);
}
async function lve() {
  await F().cleanup.drain();
}
function tQe() {
  return F().cleanup.drainStarted;
}
function BC(e) {
  return F().preExitFlush.register(e);
}
async function gCt() {
  await F().preExitFlush.drain();
}
class he {
  parsed = new Map();
  lookup(e) {
    return this.parsed.get(e);
  }
  remember(e, t) {
    this.parsed.set(e, t);
  }
  reset() {
    this.parsed.clear();
  }
}
var pe = new he();
function me(e) {
  let t = pe.lookup(e);
  if (t !== void 0) return t;
  let r = st(e);
  return pe.remember(e, r), r;
}
function st(e) {
  if (!e || e.trim() === "") return null;
  let t = e
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
  if (t.length === 0) return null;
  let r = t.some((s) => s.startsWith("!")),
    i = t.some((s) => !s.startsWith("!"));
  if (r && i) return null;
  let o = t.map((s) => s.replace(/^!/, "").toLowerCase());
  return { include: r ? [] : o, exclude: r ? o : [], isExclusive: r };
}
function at(e) {
  let t = [],
    r = e.match(/^MCP server ["']([^"']+)["']/);
  if (r && r[1]) t.push("mcp"), t.push(r[1].toLowerCase());
  else {
    let s = e.match(/^([^:[]+):/);
    if (s && s[1]) t.push(s[1].trim().toLowerCase());
  }
  let i = e.match(/^\[([^\]]+)]/);
  if (i && i[1]) t.push(i[1].trim().toLowerCase());
  if (e.toLowerCase().includes("1p event:")) t.push("1p");
  let o = e.match(/:\s*([^:]+?)(?:\s+(?:type|mode|status|event))?:/);
  if (o && o[1]) {
    let s = o[1].trim().toLowerCase();
    if (s.length < 30 && !s.includes(" ")) t.push(s);
  }
  return Array.from(new Set(t));
}
function ut(e, t) {
  if (!t) return !0;
  if (e.length === 0) return !1;
  if (t.isExclusive) return !e.some((r) => t.exclude.includes(r));
  else return e.some((r) => t.include.includes(r));
}
function ye(e, t) {
  if (!t) return !0;
  let r = at(e);
  return ut(r, t);
}
import * as f from "fs";
import {
  appendFile as ft,
  chmod as gt,
  copyFile as pt,
  link as ht,
  lstat as z,
  mkdir as mt,
  open as A,
  readdir as yt,
  readFile as Ee,
  readlink as bt,
  realpath as Et,
  rename as kt,
  rmdir as xt,
  rm as wt,
  stat as At,
  symlink as Ot,
  unlink as Rt,
} from "fs/promises";
import { homedir as Se } from "os";
import * as l from "path";
import { writeFileSync as dt } from "fs";
var Sn = (() => {
  let e = process.env.CLAUDE_CODE_SLOW_OPERATION_THRESHOLD_MS;
  if (e !== void 0) {
    let t = Number(e);
    if (!Number.isNaN(t) && t >= 0) return t;
  }
  return 1 / 0;
})();
var ct = { [Symbol.dispose]() {} };
function lt() {
  return ct;
}
var Jd = lt;
function b(e, t, r) {
  using i = Jd`JSON.stringify(${e})`;
  return JSON.stringify(e, t, r);
}
function P5(e) {
  return (
    JSON.stringify(e) +
    `
`
  );
}
function Uf(e) {
  return JSON.stringify(e);
}
function _kn(e) {
  using t = Jd`jsonlJoin(${e.length})`;
  let r = "";
  for (let i = 0; i < e.length; i++)
    r +=
      JSON.stringify(e[i]) +
      `
`;
  return r;
}
var V = (e, t) => {
  using r = Jd`JSON.parse(${e})`;
  return typeof t > "u" ? JSON.parse(e) : JSON.parse(e, t);
};
function Ps(e) {
  return JSON.parse(e);
}
function Wp(e, t) {
  using r = Jd`structuredClone(${e})`;
  return structuredClone(e, t);
}
function Nfe(e, t, r) {
  using i = Jd`fs.writeFileSync(${e}, ${t})`;
  dt(e, t, r);
}
var Dt = "\u2192";
function _t() {
  return f.constants.O_NOFOLLOW | f.constants.O_NONBLOCK;
}
var j = /\/+/;
function ke(e) {
  let t = "";
  for (let i of e.split(/([\\/]+)/)) {
    if (i === "." || i === "..") break;
    t += i;
  }
  let r = t.replace(/(?<=[^\\/])[\\/]+$/, "");
  return r === "" ? e : r;
}
function Lt(e) {
  if (e.startsWith("\\\\")) return ke(e);
  let t = e.replace(/^\/+/, "/"),
    r = PU(t);
  if (r !== null) return r;
  return ke(e);
}
var Ft = Lt;
function xe(e, t, r) {
  let i = oc(e);
  if (t.length === 0 || i || t.some((o) => o === "." || o === "..")) {
    if (r !== void 0 && (t.length > 0 || i)) r(l.join(e, ...t));
    return i ? Ft(e) : e;
  }
  return l.join(e, ...t);
}
var D5 = "\x00unverified-ancestry";
function we(e, t) {
  if (t?.unreadableAncestry !== "unverified") return;
  let r = e !== void 0 && typeof e === "object" && "errno" in e ? e.errno : void 0;
  return r === "ENOENT" || r === "ENOTDIR" ? void 0 : D5;
}
function Z(e) {
  return VC(e) && !ns(e);
}
function C(e, t) {
  return t === void 0 || t.every((r) => HR(e, r));
}
function I(e, t) {
  return t === void 0 || t.every((r) => u$e(e, r));
}
function Ae(e) {
  if (e?.anchors !== void 0 && e.anchors.length > 0) return e.anchors;
  return e?.anchor !== void 0 ? [e.anchor] : void 0;
}
function CP(e) {
  return Qi(e) || ns(e) || Kg(_A, e) !== void 0;
}
async function nQe(e) {
  return Qi(e) || ns(e) || (await ykn(_A, e)) !== void 0;
}
function Kg(e, t, r) {
  return Pt(Oe(t, r), (i) => {
    if (i.kind === "lstat") return e.lstatSync(i.path);
    return e.readlinkSync(i.path);
  });
}
function ykn(e, t, r) {
  return Ct(Oe(t, r), (i) => (i.kind === "lstat" ? e.lstat(i.path) : e.readlink(i.path)));
}
function Pt(e, t) {
  let r;
  for (;;) {
    let i = e.next(r);
    if (i.done) return i.value;
    try {
      r = t(i.value);
    } catch (o) {
      r = { errno: E(o) };
    }
  }
}
async function Ct(e, t) {
  let r;
  for (;;) {
    let i = e.next(r);
    if (i.done) return i.value;
    try {
      r = await t(i.value);
    } catch (o) {
      r = { errno: E(o) };
    }
  }
}
function* Oe(e, t) {
  let r = Ae(t);
  if ((Bn(e) && !Ms(e) && I(e, r)) || (_r(e) && C(e, r))) return;
  if (Z(e)) return e;
  let i = l.resolve(e),
    o = l.parse(i).root,
    s = o,
    a = i.slice(o.length).split(j).filter(Boolean),
    u = 0,
    d = 64;
  while (a.length > 0 && u < d) {
    let c = l.join(s, a[0]);
    if ((_r(c) && C(c, r)) || Z(c)) return xe(c, a.slice(1), t?.onCollapsedLanding);
    let p = yield { kind: "lstat", path: c };
    if (p === void 0 || typeof p === "string" || "errno" in p) return we(p, t);
    if (!p.isSymbolicLink()) {
      a.shift(), (s = c);
      continue;
    }
    u++;
    let h = yield { kind: "readlink", path: c };
    if (typeof h !== "string") return we(h, t);
    let k = h;
    if (!l.isAbsolute(k)) {
      a.shift(), (a = [...k.split(j).filter(Boolean), ...a]);
      continue;
    }
    let m = k;
    if ((Bn(m) && !Ms(m) && I(m, r)) || (_r(m) && C(m, r)) || Z(m)) return a.shift(), xe(m, a, t?.onCollapsedLanding);
    if (t?.surfaceNetworkRaw === !0 && oc(m)) return a.shift(), a.length === 0 ? m : m + l.sep + a.join(l.sep);
    a.shift();
    let S = l.parse(m).root || l.sep;
    (s = S), (a = [...m.slice(S.length).split(j).filter(Boolean), ...a]);
  }
  if (a.length > 0 && t?.surfaceNetworkRaw) return D5;
  return;
}
function Qo(e, t) {
  if ((Bn(t) && !Ms(t)) || _r(t)) return { resolvedPath: t, isSymlink: !1, isCanonical: !1 };
  let r = Kg(e, t);
  if (r !== void 0) return { resolvedPath: r, isSymlink: !0, isCanonical: !1 };
  try {
    let i = e.realpathSync(t);
    return { resolvedPath: i, isSymlink: i !== t, isCanonical: !0 };
  } catch (i) {
    return { resolvedPath: t, isSymlink: !1, isCanonical: !1 };
  }
}
function O5(e, t, r) {
  let { resolvedPath: i } = Qo(e, t);
  if (r.has(i)) return !0;
  return r.add(i), !1;
}
var Y = /(^|[\\/])\.\.([\\/]|$)/;
function Tt(e, t = "darwin") {
  let r = e.replace(t === "win32" ? /[\\/]\.(?=[\\/]|$)/g : /\/\.(?=\/|$)/g, "");
  if (r === "") return e[0];
  if (t === "win32" && /^[A-Za-z]:$/.test(r)) return r + e[2];
  return r;
}
function tx(e, t, r) {
  let i = Ae(r);
  if ((Bn(t) && !Ms(t)) || _r(t)) {
    if (i !== void 0 && ((_r(t) && !C(t, i)) || (Bn(t) && !Ms(t) && !I(t, i))))
      return Kg(e, t, { anchors: i, surfaceNetworkRaw: r?.surfaceNetworkRaw });
    return t;
  }
  let o = Kg(e, t, r?.surfaceNetworkRaw === !0 ? { surfaceNetworkRaw: !0, anchors: i } : void 0);
  if (o !== void 0) return o;
  let s = t,
    a = [],
    u = r?.surfaceDotDotTargets === !0,
    d = (c) => (a.length === 0 ? c : u && Y.test(c) ? [c, ...a].join(l.sep) : l.join(c, ...a));
  while (s !== l.dirname(s)) {
    let c, p;
    try {
      c = e.readlinkSync(s);
    } catch (g) {
      p = E(g);
    }
    if (c !== void 0) {
      if (u && Y.test(c)) return d(l.isAbsolute(c) ? c : l.dirname(s) + l.sep + c);
      let g = l.isAbsolute(c) ? c : l.resolve(l.dirname(s), c);
      if ((Bn(g) && !Ms(g)) || _r(g) || (r?.surfaceNetworkRaw === !0 && oc(g)))
        return r?.surfaceNetworkRaw === !0 && a.length > 0 ? g + l.sep + a.join(l.sep) : d(g);
      try {
        let h = e.realpathSync(s);
        return d(h);
      } catch {
        let h = s,
          k = 0,
          m = 64;
        while (k < m) {
          let y;
          try {
            y = e.readlinkSync(h);
          } catch {
            break;
          }
          if (u && Y.test(y)) {
            h = l.isAbsolute(y) ? y : l.dirname(h) + l.sep + y;
            break;
          }
          let S = l.isAbsolute(y) ? (u ? Tt(y) : y) : l.resolve(l.dirname(h), y);
          if ((Bn(S) && !Ms(S) && I(S, i)) || (_r(S) && C(S, i)) || (r?.surfaceNetworkRaw === !0 && oc(S))) {
            h = S;
            break;
          }
          let se;
          try {
            se = e.lstatSync(S);
          } catch {
            h = S;
            break;
          }
          if (!se.isSymbolicLink()) {
            h = S;
            break;
          }
          (h = S), k++;
        }
        if (k >= m && r?.surfaceNetworkRaw === !0) return D5;
        return r?.surfaceNetworkRaw === !0 && a.length > 0 && ((Bn(h) && !Ms(h)) || _r(h) || oc(h))
          ? h + l.sep + a.join(l.sep)
          : d(h);
      }
    }
    if (p === "ENOENT") {
      a.unshift(l.basename(s)), (s = l.dirname(s));
      continue;
    }
    try {
      let g = e.realpathSync(s);
      if (g !== s) return d(g);
    } catch (g) {
      if (u) throw g;
    }
    return;
  }
  return;
}
function ve(e) {
  return (Bn(e) && !Ms(e)) || ns(e) || JO(e);
}
function Skn(e) {
  return Re(e, !1) ?? e;
}
function Blr(e) {
  return Re(e, !0);
}
function Re(e, t) {
  if (ve(e)) return e;
  let r = e,
    i = "";
  for (;;) {
    if (!oc(r)) {
      let s = UJ(r);
      if (s !== null) {
        let a = i === "" ? s : s.endsWith(l.sep) ? s + i : s + l.sep + i;
        if (ve(a)) return t ? a : e;
        return a;
      }
      if (t)
        try {
          return f.lstatSync(r), null;
        } catch (a) {
          if (!X(a)) return null;
        }
    }
    let o = l.dirname(r);
    if (o === r) return e;
    (i = i === "" ? l.basename(r) : l.basename(r) + l.sep + i), (r = o);
  }
}
function ao(e) {
  let t = e;
  if (t === "~") t = Se().normalize("NFC");
  else if (t.startsWith("~/")) t = l.join(Se().normalize("NFC"), t.slice(2));
  let r = new Set(),
    i = le();
  if ((r.add(t), (Bn(t) && !Ms(t)) || _r(t))) return Array.from(r);
  let o = Kg(i, t, { onCollapsedLanding: (u) => r.add(u) });
  if (o !== void 0) return r.add(o), Array.from(r);
  try {
    let u = t,
      d = new Set(),
      c = 64;
    for (let p = 0; p < c; p++) {
      if (d.has(u)) break;
      d.add(u);
      let g, h;
      try {
        g = i.readlinkSync(u);
      } catch (m) {
        h = E(m);
      }
      if (g === void 0) {
        if (h === "ENOENT") {
          if (u === t) {
            let m = tx(i, t);
            if (m !== void 0) r.add(m);
          }
        }
        break;
      }
      let k = l.isAbsolute(g) ? g : l.resolve(l.dirname(u), g);
      if ((r.add(k), (Bn(k) && !Ms(k)) || _r(k))) return Array.from(r);
      u = k;
    }
  } catch {}
  let { resolvedPath: s, isSymlink: a } = Qo(i, t);
  if (a && s !== t) r.add(s);
  return Array.from(r);
}
var _A = {
  cwd() {
    return process.cwd();
  },
  existsSync(e) {
    using t = Jd`fs.existsSync(${e})`;
    return f.existsSync(e);
  },
  async stat(e) {
    return At(e);
  },
  async lstat(e) {
    return z(e);
  },
  async lstatBigint(e) {
    return z(e, { bigint: !0 });
  },
  async readdir(e) {
    return yt(e, { withFileTypes: !0 });
  },
  async unlink(e) {
    return Rt(e);
  },
  async rmdir(e) {
    return xt(e);
  },
  async rm(e, t) {
    return wt(e, t);
  },
  async mkdir(e, t) {
    try {
      await mt(e, { recursive: !0, ...t });
    } catch (r) {
      if (E(r) !== "EEXIST") throw r;
    }
  },
  async readFile(e, t) {
    return Ee(e, { encoding: t.encoding });
  },
  async rename(e, t) {
    return kt(e, t);
  },
  async realpath(e) {
    return tr(await Et(e));
  },
  async readlink(e) {
    return bt(e);
  },
  async copyFile(e, t) {
    return pt(e, t);
  },
  async appendFile(e, t, r) {
    if (r?.mode !== void 0)
      try {
        let i = await A(e, "ax", r.mode);
        try {
          await i.appendFile(t);
        } finally {
          await i.close();
        }
        return;
      } catch (i) {
        if (E(i) !== "EEXIST") throw i;
      }
    return ft(e, t);
  },
  async symlink(e, t, r) {
    return Ot(e, t, r);
  },
  async link(e, t) {
    return ht(e, t);
  },
  async chmod(e, t) {
    return gt(e, t);
  },
  statSync(e) {
    using t = Jd`fs.statSync(${e})`;
    return f.statSync(e);
  },
  lstatSync(e) {
    using t = Jd`fs.lstatSync(${e})`;
    return f.lstatSync(e);
  },
  readFileSync(e, t) {
    using r = Jd`fs.readFileSync(${e})`;
    return f.readFileSync(e, { encoding: t.encoding });
  },
  readSync(e, t) {
    using r = Jd`fs.readSync(${e}, ${t.length} bytes)`;
    let i = void 0;
    try {
      i = f.openSync(e, "r");
      let o = [],
        s = 0;
      while (s < t.length) {
        let u = Buffer.allocUnsafe(Math.min(65536, t.length - s)),
          d = f.readSync(i, u, 0, u.length, s);
        if (d === 0) break;
        o.push(u.subarray(0, d)), (s += d);
      }
      return { buffer: Buffer.concat(o, s), bytesRead: s };
    } finally {
      if (i !== void 0) f.closeSync(i);
    }
  },
  appendFileSync(e, t, r) {
    using i = Jd`fs.appendFileSync(${e}, ${t.length} chars)`;
    if (r?.mode !== void 0)
      try {
        let o = f.openSync(e, "ax", r.mode);
        try {
          f.appendFileSync(o, t);
        } finally {
          f.closeSync(o);
        }
        return;
      } catch (o) {
        if (E(o) !== "EEXIST") throw o;
      }
    f.appendFileSync(e, t);
  },
  unlinkSync(e) {
    using t = Jd`fs.unlinkSync(${e})`;
    f.unlinkSync(e);
  },
  renameSync(e, t) {
    using r = Jd`fs.renameSync(${e} ${Dt} ${t})`;
    f.renameSync(e, t);
  },
  readlinkSync(e) {
    using t = Jd`fs.readlinkSync(${e})`;
    return f.readlinkSync(e);
  },
  realpathSync(e) {
    using t = Jd`fs.realpathSync(${e})`;
    return tr(f.realpathSync(e));
  },
  mkdirSync(e, t) {
    using r = Jd`fs.mkdirSync(${e})`;
    let i = { recursive: !0 };
    if (t?.mode !== void 0) i.mode = t.mode;
    try {
      f.mkdirSync(e, i);
    } catch (o) {
      if (E(o) !== "EEXIST") throw o;
    }
  },
  readdirSync(e) {
    using t = Jd`fs.readdirSync(${e})`;
    return f.readdirSync(e, { withFileTypes: !0 });
  },
  rmSync(e, t) {
    using r = Jd`fs.rmSync(${e})`;
    f.rmSync(e, t);
  },
  createWriteStream(e) {
    return f.createWriteStream(e);
  },
  async readFileBytes(e, t) {
    if (t === void 0) return Ee(e);
    let r = await A(e, f.constants.O_RDONLY | (f.constants.O_NONBLOCK ?? 0) | (f.constants.O_NOCTTY ?? 0));
    try {
      if (!(await r.stat()).isFile()) return Buffer.alloc(0);
      return await bkn(r, t, "file");
    } finally {
      await r.close();
    }
  },
  async readFileFdGated(e, t) {
    try {
      let r = _t();
      if (r === 0) {
        if (!(await z(e)).isFile()) return null;
      }
      let i = await A(e, f.constants.O_RDONLY | r);
      try {
        let o = await i.stat();
        if (!o.isFile() || o.size > t) return null;
        let s = Number(o.size),
          a = Buffer.allocUnsafe(s),
          u = 0;
        while (u < s) {
          let { bytesRead: d } = await i.read(a, u, s - u, u);
          if (d === 0) break;
          u += d;
        }
        return { content: (u < s ? a.subarray(0, u) : a).toString("utf8"), stats: o };
      } finally {
        await i.close();
      }
    } catch {
      return null;
    }
  },
};
function le() {
  return _A;
}
function qu(e) {
  process.chdir(e);
  try {
    process.cwd();
  } catch {}
}
async function Ffe(e, t, r) {
  await using i = typeof e === "string" ? await A(e, "r") : null;
  let o = typeof e === "string" ? i : e,
    s = (await o.stat()).size;
  if (s <= t) return null;
  let a = Math.min(s - t, r),
    u = Buffer.allocUnsafe(a),
    d = 0;
  while (d < a) {
    let { bytesRead: c } = await o.read(u, d, a - d, t + d);
    if (c === 0) break;
    d += c;
  }
  return { content: u.toString("utf8", 0, d), bytesRead: d, bytesTotal: s };
}
async function bkn(e, t, r) {
  let i = r !== void 0 ? r === "file" : (await e.stat()).isFile(),
    o = [],
    s = 0;
  while (s < t) {
    let a = Buffer.allocUnsafe(Math.min(65536, t - s)),
      { bytesRead: u } = await e.read(a, 0, a.length, i ? s : null);
    if (u === 0) break;
    o.push(a.subarray(0, u)), (s += u);
  }
  return Buffer.concat(o, s);
}
async function _b(e, t) {
  await using r = typeof e === "string" ? await A(e, "r") : null;
  let i = typeof e === "string" ? r : e,
    o = (await i.stat()).size;
  if (o === 0) return { content: "", bytesRead: 0, bytesTotal: 0 };
  let s = Math.max(0, o - t),
    a = o - s,
    u = Buffer.allocUnsafe(a),
    d = 0;
  while (d < a) {
    let { bytesRead: c } = await i.read(u, d, a - d, s + d);
    if (c === 0) break;
    d += c;
  }
  return { content: u.toString("utf8", 0, d), bytesRead: d, bytesTotal: o };
}
async function* jlr(e, t = 65536) {
  let r = await A(e, "r"),
    i = Buffer.alloc(t),
    o = 0,
    s = [],
    a = 0;
  try {
    while (!0) {
      let { bytesRead: u } = await r.read(i, 0, t, o);
      if (u === 0) break;
      o += u;
      let d = i.subarray(0, u),
        c = 0;
      while (c < u) {
        let p = d.indexOf(10, c);
        if (p === -1) {
          s.push(Buffer.from(d.subarray(c))), (a += u - c);
          break;
        }
        if (a === 0) yield d.subarray(c, p);
        else yield Buffer.concat([...s, d.subarray(c, p)], a + (p - c)), (s = []), (a = 0);
        c = p + 1;
      }
    }
  } finally {
    await r.close();
  }
  if (a > 0) yield s.length === 1 ? s[0] : Buffer.concat(s, a);
}
async function* cve(e) {
  let r = await A(e, "r");
  try {
    let o = (await r.stat()).size,
      s = Buffer.alloc(0),
      a = Buffer.alloc(4096);
    while (o > 0) {
      let u = Math.min(4096, o);
      (o -= u), await r.read(a, 0, u, o);
      let d = Buffer.concat([a.subarray(0, u), s]),
        c = d.indexOf(10);
      if (c === -1) {
        s = d;
        continue;
      }
      s = Buffer.from(d.subarray(0, c));
      let p = d.toString("utf8", c + 1).split(`
`);
      for (let g = p.length - 1; g >= 0; g--) {
        let h = p[g];
        if (h) yield h;
      }
    }
    if (s.length > 0) yield s.toString("utf8");
  } finally {
    await r.close();
  }
}
var Te =
    /api[_-]?key|secret|token|password|passwd|credential|bearer|authorization|auth[_-]?header|cookie|session[_-]?(?:id|key)|connection[_-]?string|(?:private|ssh|encryption|signing|access|deploy|master|license)[_-]?key|client[_-]?secret/i,
  De = "[^\\s,;&}\\])]+",
  _e = `"[^"]*"|'[^']*'|[^\\s-]{0,4}\\[REDACTED\\]['"\`]?|(?:Bearer|Basic)\\s+(?:\\[REDACTED\\]|${De})|${De}`,
  It = ["sk", "ant", "api"].join("-"),
  x = "[\\w=-]{20,}(?:\\.[0-9a-z]{9})?",
  Ie = [
    { id: "url-userinfo", source: ":\\/\\/([^/@\\s]+)@", confidence: "low" },
    {
      id: "gcp-service-account",
      source: "\\b([a-z0-9-]+@[a-z0-9-]+\\.iam\\.gserviceaccount\\.com)\\b",
      flags: "i",
      confidence: "low",
    },
    { id: "loose-anthropic-key", source: "\\b(sk-ant-?[\\w-]{10,})", confidence: "low" },
    {
      id: "http-auth-scheme",
      source: "\\b(?:Bearer|Basic)\\s+([A-Za-z0-9+/=._~-]{20,})",
      flags: "i",
      confidence: "low",
    },
    {
      id: "loose-jwt",
      source: "\\b(eyJ[A-Za-z0-9_-]{10,}\\.[A-Za-z0-9_-]{10,}\\.[A-Za-z0-9_-]{10,})",
      confidence: "low",
    },
    {
      id: "sensitive-assign",
      source: `(?:${Te.source})[\\w.-]*["']?\\s*[=:]\\s*(${_e})`,
      flags: "i",
      confidence: "low",
    },
    {
      id: "cloud-env-var",
      source: `\\b(?:AWS|GOOGLE|GCP|GCLOUD|AZURE)_\\w+\\s*[=:]\\s*(${_e})`,
      flags: "i",
      confidence: "low",
    },
    { id: "aws-access-token", source: "\\b((?:A3T[A-Z0-9]|AKIA|ASIA|ABIA|ACCA)[A-Z2-7]{16})\\b", confidence: "high" },
    { id: "gcp-api-key", source: "\\b(AIza[\\w-]{35})(?![\\w-])", confidence: "high" },
    { id: "google-oauth-client-secret", source: "\\bGOCSPX-[\\w-]{28}(?![\\w-])", confidence: "high" },
    {
      id: "azure-ad-client-secret",
      source: `(?:^|[\\\\'"\\x60\\s>=:(,)])([a-zA-Z0-9_~.]{3}\\dQ~[a-zA-Z0-9_~.-]{31,34})(?:$|[\\\\'"\\x60\\s<),])`,
      confidence: "high",
    },
    { id: "digitalocean-pat", source: `\\b(dop_v1_[a-f0-9]{64})(?:[\\x60'"\\s;]|\\\\[nr]|$)`, confidence: "high" },
    {
      id: "digitalocean-access-token",
      source: `\\b(doo_v1_[a-f0-9]{64})(?:[\\x60'"\\s;]|\\\\[nr]|$)`,
      confidence: "high",
    },
    {
      id: "anthropic-api-key",
      source: `\\b(${It}03-[a-zA-Z0-9_\\-]{93}AA)(?:[\\x60'"\\s;]|\\\\[nr]|$)`,
      confidence: "high",
    },
    {
      id: "anthropic-admin-api-key",
      source: `\\b(sk-ant-admin01-[a-zA-Z0-9_\\-]{93}AA)(?:[\\x60'"\\s;]|\\\\[nr]|$)`,
      confidence: "high",
    },
    {
      id: "anthropic-oauth-token",
      source: `\\b(sk-ant-(?:oat|ort)\\d{2}-[\\w-]{20,})(?:[\\x60'"\\s;]|\\\\[nr]|$)`,
      confidence: "high",
    },
    { id: "openai-api-key", source: "sk-[A-Za-z0-9_-]{8,200}T3BlbkFJ[A-Za-z0-9_-]{8,200}", confidence: "high" },
    { id: "openai-legacy-api-key", source: "\\bsk-[a-zA-Z0-9]{48}(?![a-zA-Z0-9])", confidence: "high" },
    { id: "huggingface-access-token", source: `\\b(hf_[a-zA-Z]{34})(?:[\\x60'"\\s;]|\\\\[nr]|$)`, confidence: "high" },
    { id: "supabase-secret-key", source: "\\bsb_secret_[A-Za-z0-9_-]{20,}", confidence: "high" },
    { id: "supabase-access-token", source: "\\bsbp_[a-z0-9]{40,}", confidence: "high" },
    { id: "github-pat", source: "ghp_[0-9a-zA-Z]{36}", confidence: "high" },
    { id: "github-fine-grained-pat", source: "github_pat_\\w{82}", confidence: "high" },
    { id: "github-app-token", source: "(?:ghu|ghs)_[0-9a-zA-Z]{36}", confidence: "high" },
    { id: "github-oauth", source: "gho_[0-9a-zA-Z]{36}", confidence: "high" },
    { id: "github-refresh-token", source: "ghr_[0-9a-zA-Z]{36}", confidence: "high" },
    { id: "gitlab-pat", source: `glpat-${x}`, confidence: "high" },
    { id: "gitlab-deploy-token", source: `gldt-${x}`, confidence: "high" },
    { id: "gitlab-runner-authentication-token", source: `glrt-${x}`, confidence: "high" },
    { id: "gitlab-oauth-app-secret", source: `gloas-${x}`, confidence: "high" },
    { id: "gitlab-pipeline-trigger-token", source: `glptt-${x}`, confidence: "high" },
    { id: "gitlab-kubernetes-agent-token", source: `glagent-${x}`, confidence: "high" },
    { id: "gitlab-incoming-mail-token", source: `glimt-${x}`, confidence: "high" },
    { id: "gitlab-scim-oauth-token", source: `glsoat-${x}`, confidence: "high" },
    { id: "gitlab-ci-build-token", source: `glcbt-${x}`, confidence: "high" },
    { id: "gitlab-feed-token", source: `glft-${x}`, confidence: "high" },
    { id: "gitlab-feature-flag-client-token", source: `glffct-${x}`, confidence: "high" },
    { id: "slack-bot-token", source: "xoxb-[0-9]{10,13}-[0-9]{10,13}[a-zA-Z0-9-]*", confidence: "high" },
    { id: "slack-user-token", source: "xox[a-z](?:-[0-9]{10,13}){3}-[a-zA-Z0-9-]{28,34}", confidence: "high" },
    { id: "slack-rotation-token", source: "xoxe(?:\\.xox[a-z])?-[0-9]-[A-Za-z0-9-]{28,}", confidence: "high" },
    { id: "slack-app-token", source: "xapp-\\d-[A-Z0-9]+-\\d+-[a-z0-9]+", flags: "i", confidence: "high" },
    { id: "slack-workflow-token", source: "\\bxwfp-[a-zA-Z0-9-]{20,}", confidence: "high" },
    {
      id: "slack-webhook-url",
      source: "(?:https?://)?hooks\\.slack\\.com/(?:services|workflows|triggers)/[A-Za-z0-9+/_-]{40,}",
      flags: "i",
      confidence: "high",
    },
    { id: "twilio-api-key", source: "SK[0-9a-fA-F]{32}", confidence: "high" },
    {
      id: "sendgrid-api-token",
      source: `\\b(SG\\.[a-zA-Z0-9=_\\-.]{66})(?:[\\x60'"\\s;]|\\\\[nr]|$)`,
      confidence: "high",
    },
    { id: "npm-access-token", source: `\\b(npm_[a-zA-Z0-9]{36})(?:[\\x60'"\\s;]|\\\\[nr]|$)`, confidence: "high" },
    { id: "pypi-upload-token", source: "pypi-AgEIcHlwaS5vcmc[\\w-]{50,1000}", confidence: "high" },
    {
      id: "databricks-api-token",
      source: `\\b(dapi[a-f0-9]{32}(?:-\\d)?)(?:[\\x60'"\\s;]|\\\\[nr]|$)`,
      confidence: "high",
    },
    { id: "hashicorp-tf-api-token", source: "[a-zA-Z0-9]{14}\\.atlasv1\\.[a-zA-Z0-9\\-_=]{60,70}", confidence: "high" },
    { id: "pulumi-api-token", source: `\\b(pul-[a-f0-9]{40})(?:[\\x60'"\\s;]|\\\\[nr]|$)`, confidence: "high" },
    {
      id: "postman-api-token",
      source: `\\b(PMAK-[a-fA-F0-9]{24}-[a-fA-F0-9]{34})(?:[\\x60'"\\s;]|\\\\[nr]|$)`,
      confidence: "high",
    },
    {
      id: "grafana-api-key",
      source: `\\b(eyJrIjoi[A-Za-z0-9+/]{70,400}={0,3})(?:[\\x60'"\\s;]|\\\\[nr]|$)`,
      confidence: "high",
    },
    {
      id: "grafana-cloud-api-token",
      source: `\\b(glc_[A-Za-z0-9+/]{32,400}={0,3})(?:[\\x60'"\\s;]|\\\\[nr]|$)`,
      confidence: "high",
    },
    {
      id: "grafana-service-account-token",
      source: `\\b(glsa_[A-Za-z0-9]{32}_[A-Fa-f0-9]{8})(?:[\\x60'"\\s;]|\\\\[nr]|$)`,
      confidence: "high",
    },
    { id: "sentry-user-token", source: `\\b(sntryu_[a-f0-9]{64})(?:[\\x60'"\\s;]|\\\\[nr]|$)`, confidence: "high" },
    {
      id: "sentry-org-token",
      source:
        "\\bsntrys_eyJpYXQiO[a-zA-Z0-9+/]{10,200}(?:LCJyZWdpb25fdXJs|InJlZ2lvbl91cmwi|cmVnaW9uX3VybCI6)[a-zA-Z0-9+/]{10,200}={0,2}_[a-zA-Z0-9+/]{43}",
      confidence: "high",
    },
    {
      id: "stripe-access-token",
      source: `\\b((?:sk|rk)_(?:test|live|prod)_[a-zA-Z0-9]{10,99})(?:[\\x60'"\\s;]|\\\\[nr]|$)`,
      confidence: "high",
    },
    { id: "shopify-access-token", source: "shpat_[a-fA-F0-9]{32}", confidence: "high" },
    { id: "shopify-shared-secret", source: "shpss_[a-fA-F0-9]{32}", confidence: "high" },
  ],
  Nt = 64;
function Q(e, t) {
  let r = /-----BEGIN[ A-Z0-9_-]{0,100}?PRIVATE KEY(?: BLOCK)?-----/gi,
    i = /-----END[ A-Z0-9_-]{0,100}?PRIVATE KEY(?: BLOCK)?-----/gi;
  r.lastIndex = t;
  let o = r.exec(e);
  if (!o) return null;
  i.lastIndex = o.index + o[0].length + Nt;
  let s = i.exec(e);
  if (!s) return null;
  return { start: o.index, end: s.index + s[0].length };
}
function Ht(e) {
  return Q(e, 0) !== null;
}
function Bt(e) {
  let t = Q(e, 0);
  if (!t) return e;
  let r = "",
    i = 0;
  while (t) (r += e.slice(i, t.start) + "[REDACTED]"), (i = t.end), (t = Q(e, i));
  return r + e.slice(i);
}
var Le = `(?:[\\x60'"\\s;]|\\\\[nr]|$)`,
  Pe = "(?=[^a-zA-Z0-9_\\-+=]|$)",
  Ut = "(?<![a-zA-Z0-9_\\-])";
function Mt() {
  return Ie.map((e) => ({
    id: e.id,
    confidence: e.confidence,
    re: new RegExp(
      e.confidence !== "high"
        ? e.source
        : Ut + (e.source.endsWith(Le) ? e.source.slice(0, -Le.length) + Pe : e.source + Pe),
      (e.flags ?? "").replace("g", "") + "g",
    ),
  }));
}
function Ce(e) {
  return Ie.map((t) => ({
    id: t.id,
    confidence: t.confidence,
    re: new RegExp(t.source, e ? (t.flags ?? "").replace("g", "") + "g" : (t.flags ?? "")),
  }));
}
function Ne(e, t) {
  if (typeof t !== "string") return "[REDACTED]";
  let r = t.length >= 2 && (t[0] === '"' || t[0] === "'") && t.at(-1) === t[0] ? t[0] : "",
    i = e.lastIndexOf(t);
  return `${e.slice(0, i)}${r}[REDACTED]${r}${e.slice(i + t.length)}`;
}
var $t = 512,
  Wt = 512;
class He {
  testRules = null;
  redactRules = null;
  displayRules = null;
  resultCache = new Map();
  scan(e) {
    this.testRules ??= Ce(!1);
    let t = [];
    for (let r of this.testRules)
      if (r.confidence === "high" && r.re.test(e))
        t.push({
          ruleId: r.id,
          label: r.id
            .split("-")
            .map((i) => jt[i] ?? Wf(i))
            .join(" "),
        });
    if (Ht(e)) t.push({ ruleId: "private-key", label: "Private Key" });
    return t;
  }
  redact(e) {
    let t = e.length <= $t;
    if (t) {
      let i = this.resultCache.get(e);
      if (i !== void 0) return i;
    }
    this.redactRules ??= Ce(!0);
    let r = Bt(e);
    for (let i of this.redactRules) r = r.replace(i.re, Ne);
    if (t) {
      if (this.resultCache.size >= Wt) this.resultCache.delete(this.resultCache.keys().next().value);
      this.resultCache.set(e, r);
    }
    return r;
  }
  redactForDisplay(e) {
    this.displayRules ??= Mt();
    let t = e;
    for (let r of this.displayRules) {
      if (r.confidence !== "high") continue;
      t = t.replace(r.re, Vt);
    }
    return t;
  }
}
var q = new He();
function rQe(e) {
  return q.scan(e);
}
function co(e) {
  return q.redact(e);
}
var zt = /[$\x60|;&<>()\s]/,
  Gt = /[/.@:~*?\\]/;
function Vt(e, t) {
  let r = typeof t === "string" ? t : e;
  if (zt.test(r) || Gt.test(r)) return e;
  return Ne(e, t);
}
function $fe(e) {
  return q.redactForDisplay(e);
}
function SU(e, t = co) {
  if (typeof e === "string") return t(e);
  if (Array.isArray(e)) return e.map((r) => SU(r, t));
  if (e !== null && typeof e === "object") {
    let r = Object.create(null);
    for (let [i, o] of Object.entries(e))
      if (typeof o === "string") {
        let s = `${i}: `,
          a = t(s + o);
        r[i] = a.startsWith(s) ? a.slice(s.length) : t(o);
      } else r[i] = SU(o, t);
    return r;
  }
  return e;
}
function Wlr(e) {
  let t = Object.create(null);
  for (let [r, i] of Object.entries(e)) t[r] = Te.test(r) ? "[REDACTED]" : i;
  return t;
}
function Tp(e) {
  if (!e) return e;
  try {
    let t = new URL(e);
    if (!t.host) throw TypeError("opaque");
    return (t.username = ""), (t.password = ""), (t.search = ""), (t.hash = ""), t.toString().replace(/\/$/, "");
  } catch {
    let t = St(St(e, "?"), "#"),
      r = t.lastIndexOf("@");
    return r >= 0 ? t.slice(r + 1) : t;
  }
}
var jt = {
  aws: "AWS",
  gcp: "GCP",
  api: "API",
  pat: "PAT",
  ad: "AD",
  tf: "TF",
  oauth: "OAuth",
  npm: "NPM",
  pypi: "PyPI",
  jwt: "JWT",
  ci: "CI",
  scim: "SCIM",
  github: "GitHub",
  gitlab: "GitLab",
  openai: "OpenAI",
  digitalocean: "DigitalOcean",
  huggingface: "HuggingFace",
  hashicorp: "HashiCorp",
  sendgrid: "SendGrid",
};
var ie = { verbose: 0, debug: 1, info: 2, warn: 3, error: 4 },
  Lwr = 10485760;
function Yt(e) {
  return Object.hasOwn(ie, e);
}
function $e(e) {
  return RP(e) ? null : Ve(e);
}
function We() {}
var Xt = { sessionId: "", fromBackend: !1 };
function qlr(e, t, r, i) {
  let o = v(t, "debug"),
    s = v(o, `${e.sessionId}.txt`),
    a;
  if (r === null) a = s;
  else if (i !== null && r === i) a = v(i, `${e.sessionId}.txt`);
  else a = r;
  let u = !e.fromBackend && a === s ? "v5" : "raw";
  return {
    target: a,
    arm: u,
    configHome: t,
    rotate: u === "raw" && !(_(a) === o && a.endsWith(".txt")),
    pointLatest: u === "raw" && a !== s,
  };
}
function ne(e) {
  return su.userNamed(Jt(e) ? e : Ve(e));
}
function Qt(e) {
  return e.code === "Failed" && e.telemetryCode === "ENOENT";
}
function Mwr(e, t = fe) {
  return t ? e : void 0;
}
async function ze(e, t, r, i) {
  let o = { namespace: "log", sessionId: t, channel: "debug" },
    s = await U.run(!0, () => e.append(o, [{ data: r }], i ? { markLatest: !0 } : void 0));
  if (s.ok) return "landed";
  return i && s.error.code === "InvalidArgument" && s.error.argument === "opts.markLatest" ? "refused" : "dropped";
}
class Glr {
  deps;
  minLevel;
  filter;
  toStderr;
  filePath;
  runtimeDebugEnabled = !1;
  hasFormattedOutput = !1;
  debugFromLaunch;
  storageV5;
  writer = null;
  redirect = null;
  pendingWrite = Promise.resolve();
  unflushedChunks = [];
  backendLinesLogged = 0;
  exiting = !1;
  exitHandlerRegistered = !1;
  successor = null;
  writtenBytes = -1;
  rotating = !1;
  resolvedLogPath = null;
  overrideDirectory = null;
  rotationTarget = null;
  latestMarked = !1;
  latestRefused = !1;
  constructor(e) {
    this.deps = e;
    if (((this.storageV5 = Mwr(e.storageV5)), e.launchIdentity !== void 0)) {
      (this.minLevel = e.launchIdentity.minLevel),
        (this.filter = e.launchIdentity.filter),
        (this.toStderr = e.launchIdentity.toStderr),
        (this.filePath = e.launchIdentity.filePath),
        (this.debugFromLaunch = e.launchIdentity.debugFromLaunch);
      return;
    }
    let t = Array.isArray(e.argv) ? e.argv : [],
      r = t.indexOf("--"),
      i = r === -1 ? t : t.slice(0, r),
      o = e.env.CLAUDE_CODE_DEBUG_LOG_LEVEL?.toLowerCase().trim();
    this.minLevel = o && Yt(o) ? o : "debug";
    let s = i.find((u) => u.startsWith("--debug="));
    (this.filter = s ? me(s.substring(8)) : null),
      (this.toStderr = i.includes("--debug-to-stderr") || i.includes("-d2e"));
    let a = null;
    for (let u = 0; u < i.length; u++) {
      let d = i[u];
      if (d.startsWith("--debug-file=")) {
        a = $e(d.substring(13));
        break;
      }
      if (d === "--debug-file" && u + 1 < i.length) {
        a = $e(i[u + 1]);
        break;
      }
    }
    (this.filePath = a),
      (this.debugFromLaunch =
        Me(e.env.DEBUG) ||
        Me(e.env.DEBUG_SDK) ||
        i.includes("--debug") ||
        i.includes("-d") ||
        this.toStderr ||
        s !== void 0 ||
        this.filePath !== null);
  }
  isDebugMode() {
    return this.runtimeDebugEnabled || this.debugFromLaunch;
  }
  drainsSyncAtExit() {
    return !(O() && this.storageV5 !== void 0 && this.deps.syncExitDrain === !1);
  }
  launchIdentity() {
    return {
      minLevel: this.minLevel,
      filter: this.filter,
      toStderr: this.toStderr,
      filePath: this.filePath,
      debugFromLaunch: this.debugFromLaunch,
      isAnt: this.deps.isAnt,
      isTestEnvironment: this.deps.isTestEnvironment,
    };
  }
  enableDebugLogging() {
    let e = this.isDebugMode() || this.deps.isAnt;
    return (this.runtimeDebugEnabled = !0), e;
  }
  logPath() {
    return (
      this.filePath ??
      (this.overrideDirectory !== null && this.overrideDirectory === this.deps.env.CLAUDE_CODE_DEBUG_LOGS_DIR
        ? v(this.overrideDirectory, `${this.deps.sessionId()}.txt`)
        : null) ??
      this.resolvedLogPath ??
      this.deps.env.CLAUDE_CODE_DEBUG_LOGS_DIR ??
      this.defaultLogPath()
    );
  }
  defaultLogPath(e = this.deps.sessionId()) {
    return v(this.deps.configHomeDir(), "debug", `${e}.txt`);
  }
  learnedOverrideDirectory() {
    return this.overrideDirectory;
  }
  isArmed() {
    return this.storageV5 !== void 0;
  }
  log(e, { level: t } = { level: "debug" }) {
    if (ie[t] < ie[this.minLevel]) return;
    if (!this.shouldLog(e)) return;
    let r = co(e.trim());
    if (
      r.includes(`
`)
    )
      r = b(r);
    let o = `${new Date().toISOString()} [${t.toUpperCase()}] ${r}
`;
    if (this.toStderr) {
      this.deps.writeToStderr(o);
      return;
    }
    this.write(o);
  }
  write(e) {
    let t = this.getWriter(),
      r = Xt;
    if (this.storageV5 !== void 0) {
      if (((r = { sessionId: this.deps.sessionId(), fromBackend: U.getStore() === !0 }), r.fromBackend))
        this.backendLinesLogged++;
    }
    if ((t.write({ origin: r, content: e }), this.exiting)) t.flush();
  }
  async flush() {
    let e = this.backendLinesLogged;
    if ((this.writer?.flush(), await this.pendingWrite, this.storageV5 !== void 0 && this.successor))
      return this.successor.flush();
    for (let t = 0; this.backendLinesLogged !== e && t < 3; t++)
      (e = this.backendLinesLogged), this.writer?.flush(), await this.pendingWrite;
  }
  dispose(e) {
    this.redirect = e;
    try {
      this.writer?.dispose();
    } finally {
      (this.redirect = null), (this.writer = null);
    }
  }
  succeed(e) {
    (this.runtimeDebugEnabled = e.runtimeDebugEnabled),
      (this.hasFormattedOutput = e.hasFormattedOutput),
      (this.pendingWrite = e.pendingWrite),
      (this.unflushedChunks = e.unflushedChunks),
      (this.exitHandlerRegistered = e.exitHandlerRegistered),
      (e.successor = this),
      e.dispose((t) => {
        this.write(t);
      });
  }
  handleExit() {
    if (this.successor) {
      this.successor.handleExit();
      return;
    }
    (this.exiting = !0), this.writer?.flush(), this.drainSync();
  }
  async maybeRotate(e, t, r = Lwr) {
    let i = O() && this.storageV5 !== void 0 ? this.storageV5.hostFiles : void 0;
    if (this.writtenBytes < 0)
      if (i) {
        let o = await i.stat(ne(e));
        this.writtenBytes = o.ok && o.value.kind !== "absent" ? o.value.size : 0;
      } else
        this.writtenBytes = await Kt(e)
          .then((o) => o.size)
          .catch(() => 0);
    else this.writtenBytes += t;
    if (this.writtenBytes <= r || this.rotating) return;
    this.rotating = !0;
    try {
      let o = e.endsWith(".txt") ? `${e.slice(0, -4)}.1.txt` : `${e}.1`;
      if (i) {
        let s = ne(e),
          a = ne(o),
          u = await i.rename(s, a);
        if (!u.ok && !Qt(u.error)) {
          if ((await i.delete(a, { missingOk: !0 }), !(await i.rename(s, a)).ok)) await i.delete(s, { missingOk: !0 });
        }
      } else
        try {
          await Ue(e, o);
        } catch (s) {
          if (!X(s)) await te(o).catch(() => {}), await Ue(e, o).catch(() => te(e).catch(() => {}));
        }
      this.writtenBytes = 0;
    } finally {
      this.rotating = !1;
    }
  }
  shouldLog(e) {
    if (this.deps.isTestEnvironment && !this.toStderr && this.filePath === null) return !1;
    if (!this.deps.isAnt && !this.isDebugMode()) return !1;
    if (typeof process > "u" || typeof process.versions > "u" || typeof process.versions.node > "u") return !1;
    return ye(e, this.filter);
  }
  resolveDirToFile(e) {
    return (this.resolvedLogPath = v(e, `${this.deps.sessionId()}.txt`)), this.resolvedLogPath;
  }
  async appendGroup(e, t, r, i) {
    if (this.storageV5 !== void 0 && e.arm === "v5") {
      await this.appendV5AndMark(this.storageV5, t, r);
      return;
    }
    if (i) await Be(_(e.target), { recursive: !0 }).catch(() => {});
    let o = e;
    try {
      await ee(e.target, r);
    } catch (s) {
      if (!z4(s)) throw s;
      if (this.storageV5 === void 0) (o = { ...e, target: this.resolveDirToFile(e.target) }), await ee(o.target, r);
      else {
        if (((this.overrideDirectory = e.target), (o = qlr(t, e.configHome, e.target, e.target)), o.arm === "v5")) {
          await this.appendV5AndMark(this.storageV5, t, r);
          return;
        }
        await Be(_(o.target), { recursive: !0 }).catch(() => {});
        try {
          await ee(o.target, r);
        } catch {
          return;
        }
      }
    }
    if (o.rotate) {
      if (this.storageV5 !== void 0 && o.target !== this.rotationTarget)
        (this.rotationTarget = o.target), (this.writtenBytes = -1);
      await this.maybeRotate(o.target, Buffer.byteLength(r)).catch(We);
    }
    if (o.pointLatest) this.markLatestSymlink();
  }
  async appendV5AndMark(e, t, r) {
    let i = O(),
      o = i && !this.latestMarked && !this.latestRefused,
      s = await ze(e, t.sessionId, r, o);
    if (s === "refused")
      (this.latestRefused = !0),
        (s = await ze(e, t.sessionId, r, !1)),
        this.log(
          "debug log: the storage backend refused markLatest; <debug folder>/latest is not pointed in this process",
          { level: "warn" },
        );
    if (s !== "landed") return;
    if (o && !this.latestRefused) this.latestMarked = !0;
    else if (!i) this.markLatestSymlink();
  }
  markLatestSymlink() {
    if (!this.latestMarked) (this.latestMarked = !0), this.updateLatestSymlink();
  }
  shiftUnflushedChunk() {
    this.unflushedChunks.shift();
  }
  drainSync() {
    if (this.unflushedChunks.length === 0) return;
    if (!this.drainsSyncAtExit()) {
      this.unflushedChunks.length = 0;
      return;
    }
    let e = [];
    for (let t of this.unflushedChunks.splice(0)) {
      let r = e.at(-1);
      if (r !== void 0 && r.armed === t.armed && (!t.armed || r.target === t.target)) r.parts.push(t);
      else e.push({ target: t.target, armed: t.armed, parts: [t] });
    }
    for (let { target: t, armed: r, parts: i } of e) {
      let o = r ? t : this.logPath(),
        s = i.map((a) => a.content).join("");
      try {
        le().mkdirSync(_(o));
      } catch {}
      try {
        le().appendFileSync(o, s);
      } catch (a) {
        if (z4(a))
          try {
            if (!r) le().appendFileSync(this.resolveDirToFile(o), s);
            else {
              let u = (p, g) => {
                  try {
                    le().appendFileSync(v(o, `${p}.txt`), g);
                  } catch {}
                },
                d = i[0]?.sessionId,
                c = "";
              for (let p of i) {
                if (p.sessionId !== d) u(d, c), (d = p.sessionId), (c = "");
                c += p.content;
              }
              u(d, c);
            }
          } catch {}
      }
    }
  }
  groupLines(e) {
    let t = e[0];
    if (t === void 0) return [];
    if (this.storageV5 === void 0)
      return [
        {
          decision: { target: this.logPath(), arm: "raw", configHome: "", rotate: !0, pointLatest: !0 },
          origin: t.origin,
          lines: e,
        },
      ];
    let r = [];
    for (let i of e) {
      let o = r.at(-1);
      if (o !== void 0 && o.origin.sessionId === i.origin.sessionId && o.origin.fromBackend === i.origin.fromBackend)
        o.lines.push(i);
      else
        r.push({
          decision: qlr(
            i.origin,
            this.deps.configHomeDir(),
            this.filePath ?? this.deps.env.CLAUDE_CODE_DEBUG_LOGS_DIR ?? null,
            this.overrideDirectory,
          ),
          origin: i.origin,
          lines: [i],
        });
    }
    return r;
  }
  getWriter() {
    if (this.writer) return this.writer;
    let e = null,
      t = this.storageV5 !== void 0;
    if (
      ((this.writer = M({
        writeBatch: (r) => {
          if (this.redirect) {
            this.redirect(r.map((i) => i.content).join(""));
            return;
          }
          if (this.exiting) {
            for (let i of this.groupLines(r))
              this.unflushedChunks.push({
                target: i.decision.target,
                sessionId: i.origin.sessionId,
                armed: t,
                content: i.lines.map((o) => o.content).join(""),
              });
            this.drainSync();
            return;
          }
          for (let i of this.groupLines(r)) {
            let o = i.lines.map((u) => u.content).join(""),
              s = _(i.decision.target),
              a = i.decision.arm === "raw" && (i.origin.fromBackend || e !== s);
            if (i.decision.arm === "raw") e = s;
            this.unflushedChunks.push({
              target: i.decision.target,
              sessionId: i.origin.sessionId,
              armed: t,
              content: o,
            }),
              (this.pendingWrite = this.pendingWrite
                .then(this.appendGroup.bind(this, i.decision, i.origin, o, a))
                .catch(We)
                .then(this.shiftUnflushedChunk.bind(this)));
          }
        },
        sizeOf: (r) => r.content.length,
        flushIntervalMs: 1000,
        maxBufferSize: 100,
        immediateMode: this.isDebugMode(),
      })),
      this.deps.registerCleanup(async () => {
        this.writer?.dispose(), await this.pendingWrite;
      }),
      !this.exitHandlerRegistered)
    )
      (this.exitHandlerRegistered = !0), this.deps.onExit(this.handleExit.bind(this));
    return this.writer;
  }
  async updateLatestSymlink() {
    try {
      let e = this.logPath(),
        t = v(_(e), "latest");
      await te(t).catch(() => {}), await Zt(e, t);
    } catch {}
  }
}
class zlr {
  instance = void 0;
  init = {};
  setInstance(e) {
    this.instance = e;
  }
  setInit(e) {
    this.init = e;
  }
}
var Nwr = new J(() => new zlr());
function L() {
  return Nwr.of(G().host);
}
function oe(e, t) {
  return new Glr({
    argv: process.argv,
    env: process.env,
    sessionId: () => K(),
    configHomeDir: () => be(),
    onExit: (r) => {
      process.on("exit", r);
    },
    registerCleanup: (r) => {
      vt(r);
    },
    writeToStderr: (r) => {
      H4(r);
    },
    isAnt: t?.isAnt ?? !1,
    isTestEnvironment: t?.isTestEnvironment ?? !1,
    launchIdentity: t,
    storageV5: e.init.storageV5 !== void 0 && e.init.configHome === be() ? e.init.storageV5 : void 0,
    syncExitDrain: e.init.syncExitDrain,
  });
}
function ER(e) {
  let t = L();
  t.setInit({ storageV5: e.storageV5, configHome: be(), syncExitDrain: e.syncExitDrain });
  let r = t.instance;
  if (r) {
    let i = oe(t, r.launchIdentity());
    t.setInstance(i), i.succeed(r);
  }
  if (e.syncExitDrain === !1 && !w().drainsSyncAtExit())
    n("Sync exit drain off for this process: debug lines still queued at exit are dropped, so this log may end short");
}
function w() {
  let e = L(),
    t = e.instance;
  if (t) return t;
  let r = oe(e);
  return e.setInstance(r), r;
}
function oQe() {
  return w().minLevel;
}
function bU() {
  return w().isDebugMode();
}
function jO() {
  return w().toStderr;
}
function wkn() {
  return w().filePath;
}
function Tkn() {
  return w().enableDebugLogging();
}
function Ekn() {
  let e = L(),
    t = e.instance;
  if (!t) return;
  let r = oe(e);
  e.setInstance(r), r.succeed(t);
}
function OFe(e) {
  w().hasFormattedOutput = e;
}
function Akn() {
  return w().hasFormattedOutput;
}
async function L5() {
  await L().instance?.flush();
}
function n(e, t = { level: "debug" }) {
  w().log(e, t);
}
function Ufe() {
  return w().logPath();
}
function Ckn(e, t) {
  return e === w().defaultLogPath(t);
}
function MHr() {
  return L().instance?.isArmed() ?? !1;
}
function NHr() {
  return L().instance?.drainsSyncAtExit() ?? !0;
}
function wj(e, t) {
  return;
}
export {
  Fe,
  Mi,
  HFe,
  gb,
  hb,
  xFe,
  tc,
  Pr,
  vJ,
  Clr,
  vlr,
  vm,
  ove,
  RJ,
  IFe,
  aCt,
  ckn,
  Rlr,
  Sj,
  lCt,
  bj,
  ive,
  cCt,
  klr,
  ukn,
  PFe,
  dkn,
  uCt,
  Hlr,
  pkn,
  xlr,
  QJe,
  Lfe,
  dCt,
  DFe,
  pCt,
  Ilr,
  sve,
  ZJe,
  XT,
  Xoe,
  Yoe,
  bg,
  kJ,
  iu,
  ave,
  fCt,
  Ge,
  Plr,
  re,
  P,
  fkn,
  mkn,
  su,
  SVt,
  eQe,
  gkn,
  hkn,
  bVt,
  Dlr,
  Olr,
  Llr,
  Mlr,
  Nlr,
  Flr,
  $lr,
  Ulr,
  mCt,
  Mfe,
  vt,
  lve,
  tQe,
  BC,
  gCt,
  Jd,
  b,
  P5,
  Uf,
  _kn,
  V,
  Ps,
  Wp,
  Nfe,
  D5,
  CP,
  nQe,
  Kg,
  ykn,
  Qo,
  O5,
  tx,
  Skn,
  Blr,
  ao,
  _A,
  le,
  qu,
  Ffe,
  bkn,
  _b,
  jlr,
  cve,
  rQe,
  co,
  $fe,
  SU,
  Wlr,
  Tp,
  Lwr,
  qlr,
  Mwr,
  Glr,
  zlr,
  Nwr,
  ER,
  oQe,
  bU,
  jO,
  wkn,
  Tkn,
  Ekn,
  OFe,
  Akn,
  L5,
  n,
  Ufe,
  Ckn,
  MHr,
  NHr,
  wj,
};
