...
import { p6e, dQt } from "/$bunfs/root/chunk-a0rhnr39.js";
import { rhe, E1n, pQt, A1n, fQt } from "/$bunfs/root/chunk-7r0azb8v.js";
...
import { i, f, ot, oe } from "/$bunfs/root/chunk-saay52v7.js";
...
var N = m(() =>
    ot({
      mode: oe(["check", "update", "create", "delete"])
        .default("check")
        .describe(
  ...
  T = m(() =>
    f({
      status: oe(["created", "updated", "deleted", "has_existing", "unavailable"]),
  ...
  B = kt({
    ...
    async call({ mode: t = "check", short_code: a }, n) {
      ...
      if (t === "delete")
        try {
          let e = a ?? (await h(r))?.short_code;
          if (!e) return s("No guide found for this org to delete.");
          return await A1n(e, r), { data: { status: "deleted", message: `Guide ${e} deleted.` } };
        } catch (e) {
      ...
      if (t === "check")
        try {
          let e = a ? (await fQt(r)).find((o) => o.short_code === a) : await h(r);
          if (e) {
            let o = w(Se(), u),
              d = null;
            try {
      ...
      try {
        ...
        if (X(e)) return s(`${u} not found in the current directory. Write the guide first.`);
        throw e;
      }
      if (g > l) return s(`${u} is over ${l / 1024}KB. Trim it before sharing.`);
      let _ = await O(c, "utf8");
      try {
        if (t === "update") {
...
Close with: "Here's your onboarding guide: ${a}" followed by the send-to-teammates line.`
    : "";
  return { data: { status: t, share_url: a, short_code: n, message: `Share link ${t}: ${a} (short_code: ${n})${c}` } };
}
function s(t) {
