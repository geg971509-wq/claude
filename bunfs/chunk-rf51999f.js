...
import { j, ue } from "/$bunfs/root/chunk-yz031c9r.js";
var S = j(WT(), 1);
import { isIP as b } from "net";
import { domainToASCII as E } from "url";
var v = /[\u3002\uFF0E\uFF61]/g,
  D = /[\uFF1A\uFE55\uFE13]/g;
function o1e(e) {
  return e.replace(v, ".").replace(D, ":");
}
var O = /^(0|[1-9]\d{0,2})(\.(0|[1-9]\d{0,2})){3}$/,
  N = /^(0x[0-9a-f]*|\d+)$/,
  L = /[\x00-\x20\x7f#%/<>?@\\^|]|[\u180E\u206A-\u206F\u{2F868}\u{2F874}\u{2F91F}\u{2F95F}\u{2F9BF}]/u,
  B = /[\u00AD\u034F\u180B-\u180D\u180F\u200B\u2060\u2064\uFE00-\uFE0F\uFEFF\u{1BCA0}-\u{1BCA3}\u{E0100}-\u{E01EF}]/gu;
function H(e) {
  let t = e;
  if (t.includes("%"))
    try {
  ...
  return t.replace(B, "").replace(v, ".").normalize("NFKC").toLowerCase().replace(/\.$/, "");
}
function i1e(e) {
  if (L.test(e)) return "";
  let t = E(e);
...
function Rir(e) {
  return !L.test(e) && O.test(E(e));
}
function kir(e) {
  let t = H(e),
    r = t.split(".");
  return r.length <= 4 && r.every((o) => N.test(o)) ? t : null;
}
function m(e) {
  let t = o1e(e.toLowerCase()).replace(/\.+$/, ""),
    r = i1e(t);
...
function M(e) {
  ...
  if (o >= 0 && e.slice(o + 1).includes(".")) {
    if (((t = T(e.slice(o + 1))), !t)) return null;
    r = e.slice(0, o + 1);
  }
  let s = r.split("::");
  ...
  let i = [],
    l = (u, f) => {
      ...
      for (let y = 0; y < p.length; y++) {
        let g = p[y];
        if (g === "" && f && y === p.length - 1) continue;
        if (g === void 0 || g === "" || !/^[0-9a-fA-F]{1,4}$/.test(g)) return null;
        x.push(parseInt(g, 16));
      }
      return x;
    };
  if (s.length === 1) {
    ...
    if (!p || !x) return null;
    let y = t ? 2 : 0,
      g = 8 - p.length - x.length - y;
    if (g < 1) return null;
    i.push(...p);
    for (let _ = 0; _ < g; _++) i.push(0);
    if ((i.push(...x), i.length + y !== 8)) return null;
  }
  let c = new Uint8Array(16);
  for (let u = 0; u < i.length; u++) {
    let f = i[u];
    if (f === void 0) return null;
    (c[u * 2] = (f >> 8) & 255), (c[u * 2 + 1] = f & 255);
  }
  if (t) (c[12] = t[0] ?? 0), (c[13] = t[1] ?? 0), (c[14] = t[2] ?? 0), (c[15] = t[3] ?? 0);
  return c;
}
var I = j(qpe(), 1);
...
class UTt extends I.HttpsProxyAgent {
  async connect(...[e, t]) {
    let r,
      o = (l) => {
        if (l.statusCode !== 200) r = l;
      };
    ...
    s.destroy();
    let i = r.statusCode;
    throw new A(Number.isInteger(i) && i >= 100 && i <= 599 ? i : 0, r.headers);
  }
}
class w {
  keepAliveDisabled = !1;
...
var Y = ["https_proxy", "HTTPS_PROXY", "http_proxy", "HTTP_PROXY"];
function hoe(e = process.env) {
  let t = Y.find((o) => e[o]),
    r = t ? e[t] : void 0;
  return t && r ? { value: r, source: t } : void 0;
}
function _oe(e = process.env) {
  return hoe(e)?.value;
}
function yoe(e) {
  try {
...
function yh(e = process.env) {
  ...
  if (!yoe(t.value)) {
    ...
    if (!C.has(r)) C.add(r), console.error(lGt(t.source, t.value));
    return;
  }
  return t.value;
}
...
function DC(e, t = jAn()) {
  ...
  try {
    ...
    return t
      ...
      .filter(Boolean)
      .some((u) => {
        if (((u = u.toLowerCase().trim()), u.includes(":"))) return l === u;
        if (u.startsWith(".")) {
          let f = u;
          return o === u.substring(1) || o.endsWith(f);
        }
        return o === u;
      });
  } catch {
...
function F(e) {
  let t = Ew(),
    r = p0(),
    o = { ...(t && { cert: t.cert, key: t.key, passphrase: t.passphrase }), ...(r && { ca: r }) },
    s = { ...o };
...
function q(e) {
  let t = ue("undici"),
    r = Ew(),
    o = p0(),
    s = { httpProxy: e, httpsProxy: e, noProxy: jAn({ no_proxy: a.no_proxy, NO_PROXY: a.NO_PROXY }) };
  if (r || o) {
    let i = { ...(r && { cert: r.cert, key: r.key, passphrase: r.passphrase }), ...(o && { ca: o }) };
...
function ab(e) {
  ...
  if (!t) return;
  try {
    if (sJ(new URL(e).hostname)) return;
  } catch {}
  if (DC(e)) return;
  return t;
}
...
function k(e) {
  return s1e() !== void 0 && e.proxyAuthHelperConfig.fromProjectOrLocal;
}
function z() {
...
async function a1e() {
  ...
  if (!e) return null;
  let t = d();
  if (k(t) && !Le() && !t.proxyAuthHelperConfig.trustAccepted())
    return (
      n("proxyAuthHelper configured in project/local settings but workspace trust not yet accepted \u2014 skipping", {
        level: "warn",
      }),
      null
    );
  let r = t.proxyAuthPendingChallenge;
  if (!r && t.proxyAuthCache && Date.now() - t.proxyAuthCache.timestamp < z()) return t.proxyAuthCache.value;
  ...
  if (i.failed || !i.stdout?.trim()) {
    let c = i.timedOut ? "timed out" : i.failed ? `exited ${i.exitCode}` : "did not return a value",
      u = i.stderr?.trim();
    return console.error(`proxyAuthHelper failed: ${u ? `${c}: ${u}` : c}`), t.proxyAuthCache?.value ?? null;
  }
  let l = i.stdout.trim();
  return (t.proxyAuthCache = { value: l, timestamp: Date.now() }), l;
}
function sbr() {
...
function GAn() {
  if (!s1e()) return;
  let e = d();
  if (k(e) && !e.proxyAuthHelperConfig.trustAccepted()) return;
  a1e();
}
function Ri(e) {
  ...
  let s = yh();
  if (s) {
    if (e.url && DC(e.url)) return { ...o, ...l1e() };
    let i = sbr();
    return { ...o, proxy: i ? { url: s, headers: { "Proxy-Authorization": i } } : s, ...l1e() };
  }
  if (e.fallbackProxy) {
    if (e.url && (Hir(e.url, e.fallbackProxy.noProxy) || Hir(e.url, jAn()))) return { ...o, ...l1e() };
    let i = l1e();
    return {
      ...o,
...
function dA() {
  ...
  if (t) {
    ...
    (h = st.interceptors.request.use((s) => {
      if (s.url && DC(s.url))
        if (r) (s.httpsAgent = r), (s.httpAgent = void 0);
...
async function boe(e) {
  let t = await XY(e);
  if (!t) return {};
  let { defaultProvider: r } = await import("/$bunfs/root/chunk-00n540yr.js");
  return {
    requestHandler: t,
    credentials: r({
      clientConfig: { requestHandler: t },
      parentClientConfig: { requestHandler: t, ...(e.region !== void 0 && { region: e.region }) },
    }),
  };
}
async function XY(e) {
  let t = yh();
  if (!t || (e.url && DC(e.url))) return null;
  let r = F(t);
  return new S.NodeHttpHandler({
    httpAgent: r,
    httpsAgent: r,
