...
import { b, V, D5, ykn, _A } from "/$bunfs/root/chunk-fv016jr6.js";
import { ce } from "/$bunfs/root/chunk-4ddxwr9r.js";
import { rfe } from "/$bunfs/root/chunk-5rt2mvvk.js";
import { RYe } from "/$bunfs/root/chunk-5rq4tj43.js";
import { i, q, _e, H, f, dt, ps, De, oe, N, zH } from "/$bunfs/root/chunk-saay52v7.js";
import { D } from "/$bunfs/root/chunk-7s7jqj2f.js";
import { Sc, ns, VC, Ms } from "/$bunfs/root/chunk-56sxk8k2.js";
import { createHash as ge } from "crypto";
import { constants as k } from "fs";
import _ from "path";
import { open as C } from "fs/promises";
import J from "path";
var z = 120;
function L(e) {
  return e.length > z ? `${ce(e, z)}...` : e;
}
function _c(e) {
  return rfe(b(L(e)));
}
function RCn(e) {
  if (typeof e === "string") return _c(e);
  let t = b(e) ?? String(e);
  return t.length <= z ? rfe(t) : rfe(b(L(t)));
}
async function Tm(e, t, r, o = []) {
  let n = J.resolve(e, t),
    s = _c(t),
    a = [e, ...o];
  if ((VC(t) && !ns(t)) || (VC(n) && !ns(n)))
    throw new R(
  ...
  if (Sc(t, e) && o.every((d) => Sc(n, d)) && !Ms(t) && !Ms(n))
    throw new R(
...
function F(e) {
  ...
  let d = (c) => {
      let p = 1;
      for (let h = 1; h <= r; h++) p = (p * (c + h)) / h;
      return (s ? 1 : Math.max(1, c)) * p * o * (c + n);
    },
    y = 0,
    w = Q;
  while (y < w) {
    let c = Math.ceil((y + w) / 2);
    if (d(c) <= I) y = c;
    else w = c - 1;
  }
  if (y < 16)
...
var te = new Set(["string", "number", "bool", "boolean", "object", "array"]);
function T(e, t) {
  let r = e;
...
function re(e, t, r, o) {
  if (Array.isArray(t)) return t.some((s) => M(e, s)) ? null : `${r} = ${v(e)} is not one of [${t.join(", ")}]`;
  if (typeof t === "object" && t !== null) return w1e(o, t, r);
  let n = B(t);
  if (n !== null) {
    let s = typeof e === "string" ? e : typeof e === "number" || typeof e === "boolean" ? String(e) : null,
      a = F(n.source);
    if (!("maxSubject" in a)) return `${r}: ${t} is not a usable guard (${a.problem})`;
    let u = a.maxSubject;
    if (s !== null && s.length > u) return `${r} is ${s.length} characters, more than this /regex/ guard checks (${u})`;
    return s !== null && n.test(s) ? null : `${r} = ${v(e)} does not match ${t}`;
  }
  if (te.has(t)) {
    let s = t === "boolean" ? "bool" : t;
    return ne(e) === s ? null : `${r} = ${v(e)} is not a ${t}`;
  }
  return M(e, t) ? null : `${r} = ${v(e)} is not "${t}"`;
}
function M(e, t) {
...
function kCn(e, t = "") {
  ...
  if (t === "") {
    ...
    if (o > P)
      return (
        r.push(
          `expect: ${o} guards is more than the ${P} one responder may carry \u2014 every call checks each of them; keep the ones that catch a wrong call`,
        ),
        r
      );
  }
  for (let [o, n] of Object.entries(e)) {
    ...
    if (Array.isArray(n)) {
      if (n.length === 0)
        r.push(`expect.${s}: an empty list of allowed values can never be satisfied \u2014 list at least one`);
      continue;
    }
    ...
    if (/^\/.*\/[a-z]*$/s.test(n))
      try {
        let a = B(n),
          u = a ? F(a.source) : null;
        if (a && !ee.test(a.flags))
          r.push(`expect.${s}: ${n} \u2014 only the i and s flags are supported in expect: guards`);
        else if (u && u.problem !== null) r.push(`expect.${s}: ${n} \u2014 ${u.problem}`);
      } catch (a) {
        r.push(
...
async function DGt(e, t, r, o, n = Number.POSITIVE_INFINITY) {
  let s = null,
    a = [],
    u = 0;
  for (let y of e.matchAll(/\{\{\s*(input\.[A-Za-z0-9_.-]+|file:(?:[^{}]|\{input\.[A-Za-z0-9_.-]+\})+?)\s*\}\}/g)) {
    a.push(e.slice(u, y.index)), (u = y.index + y[0].length);
    let w = y[1] ?? "";
    if (w.startsWith("input.")) {
      let g = T(t, w.slice(6)),
        x = g === void 0 ? "" : typeof g === "string" ? g : b(g);
      a.push(x.length > n ? `${ce(x, n)}\u2026 [${x.length - n} more characters omitted]` : x);
      continue;
    }
    let c = w.slice(5).trim(),
      p = null;
    if (
      ((c = c.replace(/\{input\.([A-Za-z0-9_.-]+)\}/g, (g, x) => {
        let S = T(t, x),
          j = S === void 0 || S === null ? "" : String(S);
        if (!ie.test(j)) p = `{input.${x}} = ${v(S)} is not a plain file-name segment`;
        return j;
      })),
      p !== null)
    ) {
    ...
    let h = A.resolve(r, c);
    if (!se(r, h)) {
      s ??= `{{file:${c}}} names a path outside the mock's directory`;
      continue;
    }
    a.push(
      o(h).then((g) => {
        if (typeof g !== "string") return (s ??= `{{file:${c}}}: ${g.problem}`), "";
        return g;
      }),
    );
  }
...
function se(e, t) {
  ...
  return r !== "" && !r.startsWith("..") && !A.isAbsolute(r);
}
function U(e) {
  let t = 0;
  for (let r of Object.values(e))
    if (((t += typeof r === "object" && r !== null && !Array.isArray(r) ? U(r) : 1), t > P)) break;
  return t;
}
import { createConnection as le } from "net";
var vYe = "eval aborted by mock",
  JY = "mock agent responder failed";
var ue = m(() => f({ verdict: oe(["ok", "tool_error", "abort"]), text: i(), replay: oe(["hit", "miss"]).optional() })),
  pe = 4194304;
...
`),
      { verdict: "abort", text: `${JY} (harness_unreachable) \u2014 see the eval debug log` }
    );
  }
}
function fe(e, t, r) {
  return new Promise((o, n) => {
    ...
    r.addEventListener("abort", y, { once: !0 }),
      ...
      s.on("data", (w) => {
        ...
        let c = a.indexOf(10);
        if (c !== -1) d(null, a.subarray(0, c).toString("utf8"));
      }),
      s.once("error", (w) => d(w)),
...
var me = m(() => {
    let e = zH(() => dt([i(), H(i()), De(i(), e)])),
      t = De(i(), e).nullable(),
      r = ps("kind", [
        f({ kind: N("fixed"), body: i(), isError: q(), expect: t, baseDir: i() }),
        f({
          kind: N("agent"),
          prompt: i(),
          ...
          expect: t,
          baseDir: i(),
          replay: f({ mockHash: i(), replayDir: i(), pinned: De(i(), i()) }).default({
            mockHash: "",
            replayDir: "",
            pinned: {},
          }),
    ...
    return f({
      ...
      nonce: i(),
      callLogPath: i(),
      tools: H(f({ name: i(), description: i(), inputSchema: De(i(), _e()) })),
      responders: De(i(), r),
      agent: f({ socketPath: i(), token: i() }).nullable(),
    });
  }),
  Y = 1048576;
async function Q0r(e, t) {
  if (!e) throw Error("missing spec path");
  let r = await we(e);
  if (t !== void 0 && ge("sha256").update(r).digest("hex") !== t)
    throw Error(
  ...
  if (!(await K(o.callLogPath, b({ ready: o.nonce, server: o.server }))))
    throw Error("could not write the identity line to the run call log \u2014 refusing to serve unidentified");
  await n.connect(s), process.stdin.on("end", () => void n.close()), process.stdin.on("error", () => void n.close());
}
function gbr(e) {
  let t = 0,
    r = new AbortController(),
    o = { spec: e, signal: r.signal },
    n = new GH({ name: `eval-mock/${e.server}`, version: "1" }, { capabilities: { tools: {} } });
  (n.onclose = () => r.abort()),
    n.setRequestHandler(pA, async () => ({
      tools: e.tools.map((u) => ({
        name: u.name,
        description: u.description,
        inputSchema: { type: "object", ...u.inputSchema },
      })),
    }));
  let s = Promise.resolve();
  n.setRequestHandler(qT, (u) => {
    let d = s.then(() => a(u.params));
    return (s = d.catch(() => {})), d;
  });
  async function a(u) {
    let d = u.name,
      y = u.arguments ?? {},
      w = Date.now(),
      c = Object.hasOwn(e.responders, d) ? e.responders[d] : void 0,
      p = c === void 0 ? { verdict: "tool_error", text: `no mock for ${e.server}/${d}` } : await hbr(c, y, o, d),
      h = {
        nonce: e.nonce,
        seq: t++,
        server: e.server,
        tool: d,
        responder: c?.kind ?? "fixed",
        input: de(y),
        verdict: p.verdict,
        output: W(p.text),
        ms: Date.now() - w,
        ...(p.replay && { replay: p.replay }),
      };
    if (!(await K(e.callLogPath, b(h))) && p.verdict === "abort")
...
async function hbr(e, t, r, o = "") {
  if (e.expect !== null) {
    let n = w1e(t, e.expect);
    if (n !== null) return { verdict: "abort", text: `input violates expect: ${n}` };
  }
  switch (e.kind) {
    case "agent": {
      if (!r || r.spec.agent === null)
        return { verdict: "tool_error", text: "agent mock responder called without a harness relay" };
      return G(r.spec.agent, { registeredName: r.spec.registeredName, tool: o, input: t }, r.signal);
    }
    case "fixed": {
      let n = await DGt(e.body, t, e.baseDir, OGt);
      if (!n.ok) return { verdict: "tool_error", text: n.reason };
      return { verdict: e.isError ? "tool_error" : "ok", text: n.text };
    }
  }
}
async function OGt(e) {
  try {
    await Tm(_.dirname(e), _.basename(e), "mock fixture");
    let t = await C(e, k.O_RDONLY | (D() === "windows" ? 0 : k.O_NONBLOCK));
    try {
      let r = await t.stat();
      if (!r.isFile()) return { problem: "is not a regular file" };
      if (r.size > Y) return { problem: `is ${r.size} bytes, over the ${Y}-byte fixture limit` };
      return (await Z(t, r.size)).toString("utf8");
    } finally {
      await t.close();
    }
  } catch (t) {
    return { problem: X(t) ? "no such fixture" : `could not be read (${E(t) ?? l(t)})` };
  }
}
var HCn = 16384,
  O = 16384;
function W(e) {
  return e.length > O ? `${ce(e, O)}\u2026[truncated]` : e;
}
function de(e) {
  let t = b(e) ?? "";
  return t.length > O ? W(t) : e;
}
...
function oEt(e) {
  return be.test(e.trimStart()) ? `${he}${e.trimStart()}` : e;
}
var he = "| ",
  be = new RegExp(
    `^(?:${vYe.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}|MCP server "|MCP error -32000: Connection closed)`,
  );
function _br(e) {
  return me().parse(V(e));
}
var ye = 4194304;
async function we(e) {
  await Tm(_.dirname(e), _.basename(e), "mock spec");
  let t = await C(e, k.O_RDONLY | (D() === "windows" ? 0 : k.O_NONBLOCK));
  try {
    let r = await t.stat();
    if (!r.isFile()) throw Error("spec path is not a regular file \u2014 refusing to read it");
    if (r.size > ye) throw Error("spec file is over the size limit the harness writes \u2014 refusing to read it");
    return (await Z(t, r.size)).toString("utf8");
  } finally {
    await t.close();
  }
}
export { _c, RCn, Tm, w1e, kCn, DGt, vYe, JY, Q0r, gbr, hbr, OGt, HCn, oEt, _br };
