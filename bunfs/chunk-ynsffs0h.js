...
import { XG } from "/$bunfs/root/chunk-zze8764r.js";
import { H7n } from "/$bunfs/root/chunk-y8sx7bf9.js";
import { dirname as o, join as t } from "path";
var CDn = 1e4;
function vDn(e) {
  try {
    let n = wm(a.CLAUDE_CODE_REMOTE_SESSION_ID ?? "", "remote session id");
...
function RDn(e) {
  ...
  switch (n.kind) {
    case "delivered":
      if (n.threw.length > 0)
        Y("error", "dir_sync_lane_verdict_listener_threw", {
          verdict: e,
      ...
      return;
    case "out_of_order":
      Y("error", "dir_sync_lane_verdict_out_of_order", { verdict: e, basis: n.basis });
      return;
    case "repeat":
    case "queued":
      return;
...
function kDn(e) {
  XG.of(G()).markCopyCleared(e);
}
async function m8t() {
  let e = await Ec();
  if (e) XG.of(G()).openGate();
  return e;
}
