...
import { ab, dA } from "/$bunfs/root/chunk-rf51999f.js";
...
import { Tie, rL, yme, ivt, w$e, AZe, CZe, sK, Sme, T$e, FU, _In, kZe } from "/$bunfs/root/chunk-ppgyw8dj.js";
import "/$bunfs/root/chunk-z0z1xzkg.js";
import "/$bunfs/root/chunk-cf8qhmdc.js";
import { c6e } from "/$bunfs/root/chunk-5v6kpgmm.js";
...
import { open as be, readdir as ke, stat as Re, unlink as Q, writeFile as Y } from "fs/promises";
...
async function ve(e, t) {
  let r = z(Te(), `${e}-${Se(6).toString("hex")}`),
    o = await be(r, "wx", 384);
  try {
    await o.writeFile(t);
  } catch (d) {
    throw (await Q(r).catch(() => {}), d);
  } finally {
    await o.close();
  }
  return r;
...
async function Ne(e, t, r, o, d) {
  let n = z(e, t.jti);
  try {
    await Y(`${n}.jwt`, r, { mode: 384 }),
      await Y(`${n}.json`, b(t, null, 2), { mode: 384 }),
      await Y(`${n}.stderr`, o, { mode: 384 });
  } catch (s) {
  ...
  try {
    ...
    for (let c of await ke(e)) {
      ...
      if (a?.isFile() && a.mtimeMs < s) await Q(p).catch(() => {});
    }
  } catch (s) {
...
async function te(e) {
  ...
  if (e.claims.pool_id) rL(e.claims.pool_id, "spawn-hint environment_id");
  if (e.claims.account_id) rL(e.claims.account_id, "spawn-hint account_id");
  if (B(e.claims.account_email)) throw Error("spawn-hint account_email: control character");
  for (let p of e.claims.repo_sources) {
    if (p.url && (!c6e(p.url) || B(p.url))) throw Error("spawn-hint repo_sources: unsafe git URL");
    if (p.revision && !wS(p.revision)) throw Error("spawn-hint repo_sources: unsafe revision");
  }
  if (B(e.claims.correlation_id) || e.claims.correlation_id.startsWith("-") || /\s/.test(e.claims.correlation_id))
    throw Error("spawn-hint correlation_id: unsafe value");
  if (B(e.claims.server_time)) throw Error("spawn-hint server_time: unsafe value");
  if (typeof e.claims.attempt !== "number" || !Number.isInteger(e.claims.attempt) || e.claims.attempt < 0)
    throw Error("spawn-hint attempt: unsafe value");
  let t = Sme(e.claims.client_platform);
  if (e.claims.client_platform && e.claims.client_platform !== sK && t === void 0) {
    let p = T$e(e.claims.client_platform);
    e.onDebug(
  ...
  let d = {
      ...
      CLAUDE_RUNNER_SESSION_ID: Jbt("session", e.claims.session_id) ?? e.claims.session_id,
...
`),
            x = O.pop() ?? "";
          for (let A of O) {
            let L = Rl(A).replace(/[\x00-\x08\x0b-\x1f\x7f]/g, "");
...
`,
    ...
    await Q(o).catch((p) => e.onDebug(`[runner:orchestrator] failed to delete ${o}: ${l(p)}`));
  }
  if (s && !n) n = `spawn-runner hook timed out after ${e.timeoutMs}ms`;
  if (e.debugDir) await Ne(e.debugDir, e.claims, e.jwt, n, e.onDebug);
  return { ok: w === 0 && !s && !c, exitCode: w, timedOut: s, execError: c, stderrTail: n, durationMs: Date.now() - r };
}
...
function J(e) {
  if (e.includes("://") || e.includes("/") || e.includes("@"))
    throw Error("--scm-connector-host expects a bare hostname (e.g. ghe.example.com[:443]), not a URL");
  let t = e.lastIndexOf(":"),
    r = e.startsWith("["),
    o,
    d = 443;
...
function ae(e) {
  let t = e.indexOf("=");
  if (t <= 0 || t === e.length - 1) throw Error("--scm-connector-host-rewrite requires <from>=<to_host:to_port>");
...
function je(e, t) {
  let r = He.replace("{provider}", encodeURIComponent(t.provider)).replace("{id}", String(t.connectorId));
...
function le(e, t, r, o) {
  ...
  function L(m) {
    ...
    if (m === oe)
      (i = Ce),
        E(`standby (close code ${m}): another orchestrator holds connector ${e.connectorId}; retrying in ${i / 1000}s`);
    else {
      if (y > 0 && Date.now() - y > se) N = re;
      (i = N + Math.floor(Math.random() * (N / 2))), (N = Math.min(N * 2, se)), h(`reconnecting in ${i}ms`);
    }
  ...
  function U() {
    ...
    (c = m),
      ...
      (m.onclose = (i) => {
        ...
        if (i.code !== oe) E(`closed code=${i.code} reason=${i.reason || "(none)"}`);
        L(i.code);
      });
  }
  function j() {
...
var pe = "https://api.anthropic.com",
  ...
  F = [0.1, 0.25, 0.5, 1, 2.5, 5, 10, 20, 30, 60];
function rt() {
  return { buckets: F.map(() => 0), count: 0, sum: 0 };
}
function st(e, t) {
  (e.sum += t), (e.count += 1);
  for (let r = 0; r < F.length; r++) if (t <= F[r]) e.buckets[r]++;
}
var W = [0.5, 1, 2.5, 5, 10, 20, 40, 80, 160, 320, 640, 1280, 2560, 5120];
function it() {
...
function at(e, t) {
  let r = Math.max(0, t);
  (e.sum += r), (e.count += 1);
  for (let o = 0; o < W.length; o++) if (r <= W[o]) e.buckets[o]++;
}
function ct(e) {
  let t = {
      apiUrl: pe,
      hooksDir: process.env.SELF_HOSTED_RUNNER_HOOKS_DIR ? ue(process.env.SELF_HOSTED_RUNNER_HOOKS_DIR) : void 0,
      healthPort: lt(process.env.SELF_HOSTED_RUNNER_HEALTH_PORT, me, "SELF_HOSTED_RUNNER_HEALTH_PORT"),
      hookConcurrency: he,
      hookTimeoutMs: fe,
  ...
  if (r !== void 0) {
    if (o === void 0) throw Error("--scm-connector-id is required when --scm-connector-host is set");
    t.scmConnector = { host: r.host, port: r.port, provider: d, connectorId: o, caFile: s, hostRewrite: w };
...
function lt(e, t, r) {
  ...
  let o = ol(e);
  if (Number.isNaN(o) || o < 0 || o > 65535) throw Error(`${r} must be an integer in [0, 65535], got: "${e}"`);
  return o;
}
async function ut(e) {
  if (!e)
  ...
  let t = Je(e, "spawn-runner"),
    r;
  try {
    r = await cu(Qe(t), 5000, `stat ${t}`);
  } catch (o) {
    throw Error(`spawn-runner hook not found at ${t} \u2014 the orchestrator cannot start without it (${l(o)})`);
  }
  if (!r.isFile()) throw Error(`spawn-runner hook at ${t} is not a regular file`);
  try {
    await cu(Be(t, Ke.X_OK), 5000, `access ${t}`);
  } catch {
...
function pt(e, t) {
  let r = I$(e.work_order_jwt),
    o = r !== null && typeof r === "object" ? r : {},
    d = (p) => (typeof o[p] === "string" ? o[p] : ""),
    n = o.aud,
    s = Array.isArray(n) ? (n.find((p) => typeof p === "string") ?? "") : typeof n === "string" ? n : "",
...
function ht(e, t = Date.now()) {
  let r = e.last_poll_at > 0 ? (t - e.last_poll_at) / 1000 : 0,
    o = `version="${FU(e.version)}",pool_id="${FU(e.pool_id)}",orchestrator_uuid="${FU(e.orchestratorUuid)}",hostname="${FU(e.hostname)}"`,
...
`);
  ...
  for (let c = 0; c < F.length; c++)
    n += `claude_code_self_hosted_orchestrator_spawn_hook_duration_seconds_bucket{le="${F[c]}"} ${s.buckets[c]}
`;
...
`);
  ...
  for (let c = 0; c < W.length; c++)
    n += `claude_code_self_hosted_orchestrator_session_queue_wait_seconds_bucket{le="${W[c]}"} ${w.buckets[c]}
`;
...
`);
    ...
    let w = b({
      ...
      last_warm_hook_ok_at: t.last_warm_hook_ok_at > 0 ? new Date(t.last_warm_hook_ok_at).toISOString() : null,
      ...
      scm_connector: t.scm_connector
        ? {
            ...
            last_connected_at:
              t.scm_connector.last_connected_at > 0 ? new Date(t.scm_connector.last_connected_at).toISOString() : null,
    ...
    s.writeHead(200, { "Content-Type": "application/json", "Content-Length": Buffer.byteLength(w) }), s.end(w);
  });
  o.unref();
  let d = 0;
  return (
    o.on("error", (n) => {
      ...
      r(`[runner:warn] /healthz listener failed on port ${e}: ${n.message} \u2014 continuing without health endpoint`);
    }),
    o.on("listening", () => {
      let n = o.address(),
...
async function gt(e, t) {
    ...
    for (let h of R)
      d(
        ...
        await r
          ...
          .catch((f) =>
            d(`[runner:orchestrator] nack of over-returned hint failed: ${l(f)} \u2014 lease expiry will re-admit`),
          );
  ...
  async function y(_, k, R) {
    ...
    if ((st(o.spawnHookDurations, g.durationMs / 1000), g.ok)) {
      if ((o.spawnHooks.ok++, (o.last_hook_ok_at = Date.now()), R)) o.last_warm_hook_ok_at = o.last_hook_ok_at;
      n(`[runner:orchestrator] ok session=${_.session_uuid} attempt=${_.attempt} jti=${_.jti} (${g.durationMs}ms)`);
      return;
    }
    let h = Rl(
        g.stderrTail || (g.exitCode === null ? "hook killed by external signal (no stderr)" : `exit ${g.exitCode}`),
      ),
      f = g.exitCode === 1 || g.timedOut || (g.exitCode === null && !g.execError),
      S = !E ? void 0 : g.timedOut ? "SESSION_FAILURE_KIND_SPAWN_TIMEOUT" : "SESSION_FAILURE_KIND_SPAWN_HOOK_ERROR";
    ...
    try {
      await r.nackSpawnHint(
        { session_uuid: _.session_uuid, attempt: _.attempt, error: h, retryable: f, ...(S && { failure_kind: S }) },
        t,
      );
    } catch (O) {
      d(`[runner:orchestrator] nack failed session=${_.session_uuid}: ${l(O)} \u2014 lease expiry will re-admit`);
    }
  }
}
function wt(e) {
  console.error(`${new Date().toISOString()} [self-hosted-runner] [runner:fatal] ${Rl(e)}`), process.exit(1);
}
async function dn(e) {
...
Debug:
  ...
  let t;
  try {
    kZe("orchestrator mode"), _In(e), (t = ct(e));
  } catch (h) {
    console.error(`error: ${Rl(l(h))}
Run 'claude self-hosted-runner orchestrator --help' for usage.`),
      process.exit(2);
  }
  let r = t.logLevel === "debug",
    o = () => new Date().toISOString(),
    d = (h) => Rl(h).replace(/[\x00-\x08\x0b-\x1f\x7f]/g, ""),
    n = (h) => {
  ...
  let N = CZe({ baseUrl: t.apiUrl, poolSecret: c, onDebug: n }),
    y = {
      orchestratorUuid: p,
      hostname: a,
      version: Tie,
  ...
  try {
    await gt({ apiClient: N, hookPath: w, args: t, healthState: y, onStatus: s, onDebug: n }, k.signal);
  } finally {
    R?.stop(),
...
async function Et(e) {
  if (e.poolSecretFile)
    return (
      await cu(ze(e.poolSecretFile, { encoding: "utf-8" }), Ze, `environment-secret read from ${e.poolSecretFile}`)
    ).trim();
...
export {
  W as SESSION_QUEUE_WAIT_BUCKETS,
  F as SPAWN_HOOK_DURATION_BUCKETS,
