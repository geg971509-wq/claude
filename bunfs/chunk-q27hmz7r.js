...
// Version: 2.1.252
import { bn, qf, xP } from "/$bunfs/root/chunk-vfy57cpd.js";
import { iSt } from "/$bunfs/root/chunk-gxyqmzc8.js";
import { j } from "/$bunfs/root/chunk-yz031c9r.js";
var p = (e) => ((p = Array.isArray), p(e)),
  O = p;
var L = Symbol.for("brand.privateNullableHeaders");
function* G(e) {
  if (!e) return;
  if (L in e) {
    ...
    for (let r of e) {
      ...
      for (let [s, i] of G(r)) {
        ...
        if (H.has(f)) {
          ...
          else o.set(s, B(o.get(s), i)), t.delete(f);
          continue;
        }
        if (i === m || !n.has(f)) {
...
var A = j(iSt(), 1),
  ...
  z = "global",
  D = (e, o, t) => `https://claude.googleapis.com/v1alpha/projects/${e}/locations/${o}/workspaces/${t}/invoke`,
  P = "Set `project` or the `ANTHROPIC_GOOGLE_CLOUD_PROJECT` environment variable (or provide `baseURL`).",
  K = ["apiKey", "authToken", "credentials", "config", "profile"];
class v extends xP {
  get completions() {
    throw new bn("The deprecated text Completions API is not available on Claude Platform on Google Cloud");
  }
  set completions(e) {}
  constructor({
    ...
    if (!f && !n)
      ...
      else (_ = s ?? new A.GoogleAuth({ scopes: S })), c(this, u, _.getClient(), "f"), a(this, u, "f").catch(() => {});
    if (!k) this.ready = Promise.resolve();
    else {
      let h = _ ?? s ?? new A.GoogleAuth({ scopes: S, ...(i ? { authClient: i } : {}) }),
        N = t,
        U = r;
      (this.baseURL = "https://unresolved.invalid"),
        (this.ready = h.getProjectId().then(
          (y) => {
            (this.project = y), (this.baseURL = D(y, N, U));
          },
          (y) => {
            let R = new bn(`No project was given and it could not be resolved from Google credentials. ${P}`);
            throw ((R.cause = y), R);
          },
        )),
        this.ready.catch(() => {});
  ...
  async authHeaders(e) {
    ...
    let t = await a(this, C, "m", I).call(this);
...
(d = new WeakMap()),
  ...
  (I = async function () {
    ...
    if (!t) throw new bn("Failed to obtain a Google access token from Application Default Credentials.");
    return t;
  });
export { v as AnthropicGoogleCloud, qf as BaseAnthropic, v as default };
