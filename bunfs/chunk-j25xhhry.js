...
import { qpe } from "/$bunfs/root/chunk-sxxp6vcr.js";
import {
  J,
  G,
  vCt,
...
import { l, os } from "/$bunfs/root/chunk-ypdw393e.js";
import { Mfe, vt, b, Akn, n } from "/$bunfs/root/chunk-fv016jr6.js";
import "/$bunfs/root/chunk-qk7r0t2g.js";
import "/$bunfs/root/chunk-4ddxwr9r.js";
import { yj, h } from "/$bunfs/root/chunk-wkxx62a2.js";
...
import { yh, DC } from "/$bunfs/root/chunk-rf51999f.js";
...
import { L2t, bX } from "/$bunfs/root/chunk-wjr8v4tx.js";
import { MS, J5n, dpn, pz } from "/$bunfs/root/chunk-kbyvkqye.js";
import { Gpe } from "/$bunfs/root/chunk-9y48by1e.js";
import { oJt } from "/$bunfs/root/chunk-dfmnmjy0.js";
...
import { D, JCe } from "/$bunfs/root/chunk-7s7jqj2f.js";
...
class ue {
  ...
  constructor(e) {
    ...
    if (
      ...
      (this._performanceStartTime = _.otperformance.now()),
      (this._performanceOffset = t - (this._performanceStartTime + _.getTimeOrigin())),
  ...
  setAttribute(e, t) {
    ...
    if (e.length === 0) return S.diag.warn(`Invalid attribute key: ${e}`), this;
    if (!_.isAttributeValue(t)) return S.diag.warn(`Invalid attribute value set for key: ${e}`), this;
  ...
  addEvent(e, t, r) {
    ...
    if (s === 0) return S.diag.warn("No events allowed."), this._droppedEventsCount++, this;
    if (s !== void 0 && this.events.length >= s) {
      if (this._droppedEventsCount === 0) S.diag.debug("Dropping extra events.");
      this.events.shift(), this._droppedEventsCount++;
    }
    if (_.isTimeInput(t)) {
      if (!_.isTimeInput(r)) r = t;
      t = void 0;
    }
    let o = _.sanitizeAttributes(t);
    return this.events.push({ name: e, attributes: o, time: this._getTime(r), droppedAttributesCount: 0 }), this;
  }
  addLink(e) {
  ...
  setStatus(e) {
    ...
    if (((this.status = { ...e }), this.status.message != null && typeof e.message !== "string"))
      S.diag.warn(`Dropping invalid status.message of type '${typeof e.message}', expected 'string'`),
  ...
  updateName(e) {
    ...
    if (this._droppedEventsCount > 0)
      S.diag.warn(`Dropped ${this._droppedEventsCount} events because eventCountLimit reached`);
    this._spanProcessor.onEnd(this);
  }
  _getTime(e) {
    if (typeof e === "number" && e <= _.otperformance.now()) return _.hrTime(e + this._performanceOffset);
    if (typeof e === "number") return _.millisToHrTime(e);
    if (e instanceof Date) return _.millisToHrTime(e.getTime());
    if (_.isTimeInputHrTime(e)) return e;
    if (this._startTimeProvided) return _.millisToHrTime(Date.now());
    let t = _.otperformance.now() - this._performanceStartTime;
  ...
  recordException(e, t) {
    ...
    if (r[x.ATTR_EXCEPTION_TYPE] || r[x.ATTR_EXCEPTION_MESSAGE]) this.addEvent(ve, r, t);
  ...
  _truncateToSize(e) {
    ...
    if (t <= 0) return S.diag.warn(`Attribute value limit must be positive, got ${t}`), e;
...
class M {
  shouldSample() {
    return { decision: I.RECORD_AND_SAMPLED };
  }
  toString() {
...
class W {
  ...
  constructor(e) {
    if (((this._root = e.root), !this._root))
      Ce.globalErrorHandler(Error("ParentBasedSampler must have a root sampler configured")), (this._root = new M());
    (this._remoteParentSampled = e.remoteParentSampled ?? new M()),
      (this._remoteParentNotSampled = e.remoteParentNotSampled ?? new k()),
      (this._localParentSampled = e.localParentSampled ?? new M()),
      (this._localParentNotSampled = e.localParentNotSampled ?? new k());
  }
  shouldSample(e, t, r, s, o, i) {
    let c = F.trace.getSpanContext(e);
    if (!c || !F.isSpanContextValid(c)) return this._root.shouldSample(e, t, r, s, o, i);
    if (c.isRemote) {
      if (c.traceFlags & F.TraceFlags.SAMPLED) return this._remoteParentSampled.shouldSample(e, t, r, s, o, i);
      return this._remoteParentNotSampled.shouldSample(e, t, r, s, o, i);
    }
    if (c.traceFlags & F.TraceFlags.SAMPLED) return this._localParentSampled.shouldSample(e, t, r, s, o, i);
...
class te {
  ...
  shouldSample(e, t) {
    return {
      decision: be.isValidTraceId(t) && this._accumulate(t) < this._upperBound ? I.RECORD_AND_SAMPLED : I.NOT_RECORD,
    };
  }
  toString() {
...
function se() {
  return {
    ...
    generalLimits: {
      attributeValueLengthLimit: L.getNumberFromEnv("OTEL_ATTRIBUTE_VALUE_LENGTH_LIMIT") ?? 1 / 0,
      attributeCountLimit: L.getNumberFromEnv("OTEL_ATTRIBUTE_COUNT_LIMIT") ?? 128,
    },
    spanLimits: {
      attributeValueLengthLimit: L.getNumberFromEnv("OTEL_SPAN_ATTRIBUTE_VALUE_LENGTH_LIMIT") ?? 1 / 0,
      attributeCountLimit: L.getNumberFromEnv("OTEL_SPAN_ATTRIBUTE_COUNT_LIMIT") ?? 128,
      linkCountLimit: L.getNumberFromEnv("OTEL_SPAN_LINK_COUNT_LIMIT") ?? 128,
      eventCountLimit: L.getNumberFromEnv("OTEL_SPAN_EVENT_COUNT_LIMIT") ?? 128,
      attributePerEventCountLimit: L.getNumberFromEnv("OTEL_SPAN_ATTRIBUTE_PER_EVENT_COUNT_LIMIT") ?? 128,
      attributePerLinkCountLimit: L.getNumberFromEnv("OTEL_SPAN_ATTRIBUTE_PER_LINK_COUNT_LIMIT") ?? 128,
    },
  };
}
function de() {
  let e = L.getStringFromEnv("OTEL_TRACES_SAMPLER") ?? v.ParentBasedAlwaysOn;
  switch (e) {
    case v.AlwaysOn:
      return new M();
    case v.AlwaysOff:
    ...
    case v.ParentBasedAlwaysOn:
      return new W({ root: new M() });
    case v.ParentBasedAlwaysOff:
      return new W({ root: new k() });
    case v.TraceIdRatio:
      return new te(Ie());
    case v.ParentBasedTraceIdRatio:
      return new W({ root: new te(Ie()) });
    default:
      return (
        oe.diag.error(`OTEL_TRACES_SAMPLER value "${e}" invalid, defaulting to "${v.ParentBasedAlwaysOn}".`),
        new W({ root: new M() })
      );
  }
}
function Ie() {
  let e = L.getNumberFromEnv("OTEL_TRACES_SAMPLER_ARG");
  if (e == null) return oe.diag.error(`OTEL_TRACES_SAMPLER_ARG is blank, defaulting to ${re}.`), re;
  if (e < 0 || e > 1)
    return (
      oe.diag.error(`OTEL_TRACES_SAMPLER_ARG=${e} was given, but it is out of range ([0..1]), defaulting to ${re}.`), re
    );
  return e;
}
var q = j(Bc(), 1),
...
function De(e) {
  ...
  return (
    (s.generalLimits = Object.assign({}, r.generalLimits, e.generalLimits || {})),
...
function Be(e) {
  ...
  return (
    (t.attributeCountLimit =
      ...
      q.getNumberFromEnv("OTEL_SPAN_ATTRIBUTE_COUNT_LIMIT") ??
      q.getNumberFromEnv("OTEL_ATTRIBUTE_COUNT_LIMIT") ??
      ot),
    (t.attributeValueLengthLimit =
      e.spanLimits?.attributeValueLengthLimit ??
      e.generalLimits?.attributeValueLengthLimit ??
      q.getNumberFromEnv("OTEL_SPAN_ATTRIBUTE_VALUE_LENGTH_LIMIT") ??
      q.getNumberFromEnv("OTEL_ATTRIBUTE_VALUE_LENGTH_LIMIT") ??
      nt),
...
class le {
    ...
    if (
      ...
      (this._maxExportBatchSize =
        typeof t?.maxExportBatchSize === "number"
          ...
          : (P.getNumberFromEnv("OTEL_BSP_MAX_EXPORT_BATCH_SIZE") ?? 512)),
      (this._maxQueueSize =
        typeof t?.maxQueueSize === "number" ? t.maxQueueSize : (P.getNumberFromEnv("OTEL_BSP_MAX_QUEUE_SIZE") ?? 2048)),
      (this._scheduledDelayMillis =
        typeof t?.scheduledDelayMillis === "number"
          ? t.scheduledDelayMillis
          : (P.getNumberFromEnv("OTEL_BSP_SCHEDULE_DELAY") ?? 5000)),
      (this._exportTimeoutMillis =
        typeof t?.exportTimeoutMillis === "number"
          ? t.exportTimeoutMillis
          : (P.getNumberFromEnv("OTEL_BSP_EXPORT_TIMEOUT") ?? 30000)),
      (this._shutdownOnce = new P.BindOnceFuture(this._shutdown, this)),
  ...
  onEnd(e) {
    ...
    if ((e.spanContext().traceFlags & B.TraceFlags.SAMPLED) === 0) return;
    this._addToBuffer(e);
  }
  shutdown() {
  ...
  _addToBuffer(e) {
    if (this._finishedSpans.length >= this._maxQueueSize) {
      if (this._droppedSpansCount === 0) B.diag.debug("maxQueueSize reached, dropping spans");
      this._droppedSpansCount++;
      return;
    }
    if (this._droppedSpansCount > 0)
      B.diag.warn(`Dropped ${this._droppedSpansCount} spans because maxQueueSize reached`),
  ...
  _flushOneBatch() {
    ...
    return new Promise((e, t) => {
      ...
      B.context.with(P.suppressTracing(B.context.active()), () => {
        let s;
        ...
        let o = () =>
            this._exporter.export(s, (c) => {
              if ((clearTimeout(r), c.code === P.ExportResultCode.SUCCESS)) e();
              else t(c.error ?? Error("BatchSpanProcessor: span export failed"));
            }),
          i = null;
        for (let c = 0, u = s.length; c < u; c++) {
          let d = s[c];
          if (d.resource.asyncAttributesPending && d.resource.waitForAsyncAttributes)
            (i ??= []), i.push(d.resource.waitForAsyncAttributes());
        }
        if (i === null) o();
        else
          Promise.all(i).then(o, (c) => {
            P.globalErrorHandler(c), t(c);
          });
      });
    });
  }
...
class Q {
  generateTraceId = Ue(16);
  generateSpanId = Ue(8);
}
var ne = Buffer.allocUnsafe(16);
function Ue(e) {
  return function () {
    for (let r = 0; r < e / 4; r++) ne.writeUInt32BE((Math.random() * 4294967296) >>> 0, r * 4);
    for (let r = 0; r < e; r++)
      if (ne[r] > 0) break;
      else if (r === e - 1) ne[e - 1] = 1;
...
class me {
  ...
  constructor(e, t, r, s) {
    ...
    (this._sampler = o.sampler),
      ...
      (this._idGenerator = t.idGenerator || new Q()),
      (this._resource = r),
  ...
  startSpan(e, t = {}, r = g.context.active()) {
    ...
    if (Y.isTracingSuppressed(r))
      return (
        g.diag.debug("Instrumentation suppressed, returning Noop Span"), g.trace.wrapSpanContext(g.INVALID_SPAN_CONTEXT)
      );
    let o = s?.spanContext(),
      ...
      u,
      d;
    if (!o || !g.trace.isSpanContextValid(o)) u = this._idGenerator.generateTraceId();
    else (u = o.traceId), (d = o.traceState), (c = o);
    let m = t.kind ?? g.SpanKind.INTERNAL,
      f = (t.links ?? []).map((V) => ({ context: V.context, attributes: Y.sanitizeAttributes(V.attributes) })),
      O = Y.sanitizeAttributes(t.attributes),
      E = this._sampler.shouldSample(r, u, e, m, O, f);
    d = E.traceState ?? d;
    let T = E.decision === g.SamplingDecision.RECORD_AND_SAMPLED ? g.TraceFlags.SAMPLED : g.TraceFlags.NONE,
      R = { traceId: u, spanId: i, traceFlags: T, traceState: d };
    if (E.decision === g.SamplingDecision.NOT_RECORD)
      return g.diag.debug("Recording is off, propagating context in a non-recording span"), g.trace.wrapSpanContext(R);
    let z = Y.sanitizeAttributes(Object.assign(O, E.attributes));
    return new ue({
  ...
  startActiveSpan(e, t, r, s) {
    ...
    let u = i ?? g.context.active(),
      d = this.startSpan(e, o, u),
      m = g.trace.setSpan(u, d);
    return g.context.with(m, c, void 0, d);
  }
  getGeneralLimits() {
  ...
  forceFlush() {
    ...
    return new Promise((t) => {
      Promise.all(e)
        ...
        .catch((r) => {
          ke.globalErrorHandler(r || Error("MultiSpanProcessor: forceFlush failed")), t();
        });
    });
  }
  onStart(e, t) {
      ...
      t = this._activeSpanProcessor._spanProcessors.map(
        (r) =>
          new Promise((s) => {
            let o,
              i = setTimeout(() => {
                s(Error(`Span processor did not completed within timeout period of ${e} ms`)), (o = H.timeout);
              }, e);
            r.forceFlush()
              .then(() => {
                if ((clearTimeout(i), o !== H.timeout)) (o = H.resolved), s(o);
              })
              .catch((c) => {
                clearTimeout(i), (o = H.error), s(c);
              });
          }),
      );
...
class ae {
  ...
  _sendSpans(e, t) {
    ...
    if (t) return t({ code: ee.ExportResultCode.SUCCESS });
  }
}
var N = j(Fde(), 1),
  qe = j(qpe(), 1);
...
async function He(e) {
  ...
  if (r !== void 0 && r.enabled === t.enabled && Date.now() - r.timestamp < ze) return t;
...
class Ee {
  ...
  async export(e, t) {
    if (this.isShutdown) {
      n(`${rO} BigQuery metrics export arrived after shutdown, skipping`), t({ code: C.ExportResultCode.SUCCESS });
      return;
    }
  ...
  async doExport(e, t) {
    try {
      if (!(ri() || Le())) {
        n(`${rO} BigQuery metrics export: trust not established, skipping`), t({ code: C.ExportResultCode.SUCCESS });
        return;
      }
      if (!this.dispatchHostMatchesEndpoint()) {
        n(`${rO} BigQuery metrics export: WIF dispatch host differs from the metrics endpoint host, skipping`),
          t({ code: C.ExportResultCode.SUCCESS });
        return;
      }
      let s = await oO(),
        o = OEe(s, this.endpoint).reasonCode === "misrouted_credential";
      if (o && !this.isAntEndpointOverride) {
        n(`${rO} BigQuery metrics export: credential does not belong to the metrics endpoint host, skipping`),
          t({ code: C.ExportResultCode.SUCCESS });
        return;
      }
      let i =
        OEe(s, `${zt().BASE_API_URL}/api/claude_code/organizations/metrics_enabled`).reasonCode ===
        "misrouted_credential";
      if (!o && !i) {
        let f = await Ge(this.storageV5);
        if (f.hasError) {
          if (s.reasonCode === "wif_error")
            this.reportFailure("wif_error"),
              this.failures.record(`metrics opt-out check failed (WIF exchange failed: ${s.error}); skipping export`);
          else this.failures.record("metrics opt-out check failed; skipping export");
          t({ code: C.ExportResultCode.SUCCESS });
          return;
        }
        if (!f.enabled) {
          n(`${rO} BigQuery metrics export disabled by organization setting, skipping`),
            t({ code: C.ExportResultCode.SUCCESS });
          return;
        }
      }
      let c = this.transformMetricsForInternal(e),
        u = OEe(await oO(), this.endpoint),
        d = u.reasonCode === "misrouted_credential";
      if (u.error && !d) {
        if (u.reasonCode === "wif_error") this.reportFailure("wif_error");
        this.failures.record(u.error), t({ code: C.ExportResultCode.SUCCESS });
        return;
      }
      let m;
      try {
        m = await this.postWithOAuth401Recovery(c, d ? {} : u.headers);
      } catch (f) {
      ...
      if (s === "other" && o === void 0)
        n(`${rO} BigQuery metrics export threw before the request: ${Ve(r)}`, { level: "error" });
      this.failures.record(Ve(r)), t({ code: C.ExportResultCode.SUCCESS });
    }
  }
  reportFailure(e, t) {
    let r = `${e}:${t ?? ""}`;
  ...
  async postWithOAuth401Recovery(e, t) {
    let r = Yt(),
      s = r?.accessToken && r.refreshToken && t.Authorization === `Bearer ${r.accessToken}` ? r.accessToken : void 0;
    this.inFlight++;
    try {
  ...
  postOnce(e, t) {
    let r = { "Content-Type": "application/json", "User-Agent": Ka(), ...t },
      s = e3t({ url: this.endpoint, payload: e, storageV5: this.storageV5 });
    return st.post(this.endpoint, s?.body ?? e, {
    ...
    let t = e.resource.attributes,
      r = {
        ...
        "aggregation.temporality":
          this.selectAggregationTemporality() === fe.AggregationTemporality.DELTA ? "delta" : "cumulative",
      };
  ...
  extractDataPoints(e) {
    return (e.dataPoints || [])
      ...
      .map((r) => ({
        ...
        value: r.value,
        timestamp: this.hrTimeToISOString(r.endTime || r.startTime || [Date.now() / 1000, 0]),
      }));
  }
  async shutdown() {
  ...
  reportInFlightAtShutdown() {
    ...
    n(`${rO} BigQuery metrics export still in flight at the shutdown budget (${this.inFlight})`),
      this.reportFailure("pending_at_shutdown");
  }
  async forceFlush() {
    await Promise.all(this.pendingExports),
      this.failures.logSummary(),
      n(`${rO} BigQuery metrics exporter flush complete`);
  }
  convertAttributes(e) {
    let t = {};
    if (e) {
      for (let [r, s] of Object.entries(e)) if (s !== void 0 && s !== null) t[r] = String(s);
    }
    return t;
  }
  hrTimeToISOString(e) {
...
var _e = j(Bc(), 1),
  We = 64,
  Qe = { code: _e.ExportResultCode.SUCCESS };
function je(e) {
...
class Te {
  ...
  export(e, t) {
    ...
    if (this.buffer.length >= We) this.buffer.shift();
...
function Se(e, t) {
  return new Promise((r, s) => {
    setTimeout((o, i) => o(new Pe(i)), e, s, t).unref();
  });
}
function Ot() {
  if ((w.context.setGlobalContextManager(bX), !a.OTEL_EXPORTER_OTLP_METRICS_TEMPORALITY_PREFERENCE))
...
function et() {
  ...
  let t = D(),
    r = {
      [N.ATTR_SERVICE_NAME]: "claude-code",
      [N.ATTR_SERVICE_VERSION]: {
  ...
  if (t === "wsl") {
    let T = JCe();
    if (T) r["wsl.version"] = T;
  }
  let s = A.resourceFromAttributes(r),
    o = A.resourceFromAttributes(A.osDetector.detect().attributes || {}),
    i = A.hostDetector.detect(),
    c = i.attributes?.[N.SEMRESATTRS_HOST_ARCH]
      ? { [N.SEMRESATTRS_HOST_ARCH]: i.attributes[N.SEMRESATTRS_HOST_ARCH] }
      : {},
    u = A.resourceFromAttributes(c),
    d = L2t(),
    m = Object.keys(d).length > 0,
    f = A.envDetector.detect().attributes || {},
    O = A.resourceFromAttributes(m ? gc(f, (T, R) => R.startsWith("user.") || R.startsWith("identity.")) : f),
    E = s.merge(o).merge(u).merge(O).merge(A.resourceFromAttributes(d));
  return J0n(E), E;
}
function tt() {
  let e = et(),
    t = new Te(),
    r = new X(t, { scheduledDelayMillis: Ze }),
    s = new Z({ resource: e, spanProcessors: [r] });
  w.trace.setGlobalTracerProvider(s), e5t(s);
  let o = new ge(),
    i = new wMe({ resource: e, processors: [new $de(o, { scheduledDelayMillis: Je })] });
  Y9t.setGlobalLoggerProvider(i), XVt(i);
  let c = Y9t.getLogger(
  ...
  JVt(c, "beta"),
    process.on("beforeExit", async () => {
      await i.forceFlush().catch(() => {}), await s.forceFlush().catch(() => {});
    }),
    process.on("exit", () => {
      i.forceFlush().catch(() => {}), s.forceFlush().catch(() => {});
    }),
    Rt(t, o).catch((u) => n(`Beta tracing exporter wiring failed: ${u}`, { level: "error" }));
}
async function Rt(e, t) {
  let r = a.BETA_TRACING_ENDPOINT;
  if (!r) return;
  let [{ OTLPTraceExporter: s }, { OTLPLogExporter: o }] = await Promise.all([
    import("/$bunfs/root/chunk-zpj33xfk.js"),
    import("/$bunfs/root/chunk-avk1rvyv.js"),
  ]);
  e.setDelegate(new s({ url: `${r}/v1/traces` })), t.setDelegate(new o({ url: `${r}/v1/logs` }));
}
function we(e) {
  return (e || "")
    .trim()
...
async function St(e) {
  let t = a.OTEL_METRIC_EXPORT_INTERVAL ?? gt;
  n(
  ...
  for (let o of e)
    if (o === "console") {
      let i = new U.ConsoleMetricExporter(),
        c = i.export.bind(i);
      (i.export = (u, d) => {
        if (u.resource && u.resource.attributes)
          n(`
=== Resource Attributes ===`),
            n(b(u.resource.attributes)),
            n(`===========================
`);
        return c(u, d);
      }),
      ...
      switch (i) {
        case "grpc": {
          let { OTLPMetricExporter: u } = await import("/$bunfs/root/chunk-c1cvwm47.js").then((m) => j(m.default, 1));
          r.push(new u());
          break;
        }
        case "http/json": {
          let { OTLPMetricExporter: u } = await import("/$bunfs/root/chunk-j9evx42z.js").then((m) => j(m.default, 1));
          r.push(new u(c));
          break;
        }
        case "http/protobuf": {
          let { OTLPMetricExporter: u } = await import("/$bunfs/root/chunk-zhykbgbk.js");
          r.push(new u(c));
          break;
        }
        default:
      ...
      let { PrometheusExporter: i } = await import("/$bunfs/root/chunk-svd5r7vb.js").then((m) => j(m.default, 1));
      r.push(new i());
    } else throw Error(`Unknown exporter type set in OTEL_METRICS_EXPORTER env var: ${o}`);
  let s = ye("metrics");
  return r.map((o) => {
    if ("export" in o) return new U.PeriodicExportingMetricReader({ exporter: s(o), exportIntervalMillis: t });
    return o;
  });
}
async function Pt() {
  let e = we(a.OTEL_LOGS_EXPORTER),
    t = a.OTEL_EXPORTER_OTLP_LOGS_PROTOCOL || a.OTEL_EXPORTER_OTLP_PROTOCOL,
    r = a.OTEL_EXPORTER_OTLP_ENDPOINT;
  n(`[3P telemetry] getOtlpLogExporters: types=${b(e)}, protocol=${t}, endpoint=${r}`);
  let s = [];
  for (let o of e)
    if (o === "console") s.push(new J9t());
    else if (o === "otlp") {
      let i = xe("logs");
      switch (t) {
        case "grpc": {
          let { OTLPLogExporter: c } = await import("/$bunfs/root/chunk-kf2tw5x7.js").then((m) => j(m.default, 1));
          s.push(new c());
          break;
        }
        case "http/json": {
          let { OTLPLogExporter: c } = await import("/$bunfs/root/chunk-avk1rvyv.js");
          s.push(new c(i));
          break;
        }
        case "http/protobuf": {
          let { OTLPLogExporter: c } = await import("/$bunfs/root/chunk-yvgeqvny.js");
          s.push(new c(i));
          break;
        }
        default:
  ...
  return s.map(ye("logs"));
}
async function Lt() {
  let e = we(a.OTEL_TRACES_EXPORTER),
    t = [];
  for (let r of e)
    if (r === "console") t.push(new ae());
    else if (r === "otlp") {
      let s = a.OTEL_EXPORTER_OTLP_TRACES_PROTOCOL || a.OTEL_EXPORTER_OTLP_PROTOCOL,
        o = xe("traces");
      switch (s) {
        case "grpc": {
          let { OTLPTraceExporter: i } = await import("/$bunfs/root/chunk-584k0hdt.js").then((m) => j(m.default, 1));
          t.push(new i());
          break;
        }
        case "http/json": {
          let { OTLPTraceExporter: i } = await import("/$bunfs/root/chunk-zpj33xfk.js");
          t.push(new i(o));
          break;
        }
        case "http/protobuf": {
          let { OTLPTraceExporter: i } = await import("/$bunfs/root/chunk-bgfwg77a.js");
          t.push(new i(o));
          break;
        }
        default:
    ...
    } else throw Error(`Unknown exporter type set in OTEL_TRACES_EXPORTER env var: ${r}`);
  return t.map(ye("traces"));
}
function At() {
  return Me(process.env.CLAUDE_CODE_ENABLE_TELEMETRY);
}
function wt(e) {
  let t = new Ee({ storageV5: e });
  return (
    vt(() => t.armShutdownReport()), new U.PeriodicExportingMetricReader({ exporter: t, exportIntervalMillis: 300000 })
  );
}
function yt() {
  if (yj()) return !1;
...
async function rs(e) {
  if ((Mr("telemetry_init_start"), Ot(), w.propagation.setGlobalPropagator(new pe.W3CTraceContextPropagator()), Akn()))
    for (let u of ["OTEL_METRICS_EXPORTER", "OTEL_LOGS_EXPORTER", "OTEL_TRACES_EXPORTER"]) {
      let d = process.env[u];
  ...
  w.diag.setLogger(new oJt(), a.CLAUDE_CODE_IS_COWORK ? w.DiagLogLevel.WARN : w.DiagLogLevel.ERROR), J5n();
  let t = [],
    r = At(),
    s = r ? we(a.OTEL_METRICS_EXPORTER) : [];
  if (
    (n(
  ...
  if (MS()) {
    if (!Qfe()) tt();
    let u = new U.MeterProvider({ resource: o, views: [], readers: t });
    return (
      ZVt(u),
      vt(async () => {
        let m = a.CLAUDE_CODE_OTEL_SHUTDOWN_TIMEOUT_MS ?? 2000;
        try {
          pz();
          let f = vCt(),
            O = Qfe(),
            E = [u.shutdown()];
          if (f) E.push(f.forceFlush().then(() => f.shutdown()));
          if (O) E.push(O.forceFlush().then(() => O.shutdown()));
          await Promise.race([Promise.all(E), Se(m, "OpenTelemetry shutdown timeout")]);
        } catch {}
      }),
      {
        meter: u.getMeter(
  ...
  let i = new U.MeterProvider({ resource: o, views: [], readers: t });
  if ((ZVt(i), r)) {
    let u = await Pt();
    if ((n(`[3P telemetry] Created ${u.length} log exporter(s)`), u.length > 0)) {
      let d = new wMe({
        resource: o,
        processors: u.map((f) => new $de(f, { scheduledDelayMillis: a.OTEL_LOGS_EXPORT_INTERVAL ?? Je })),
      });
      Y9t.setGlobalLoggerProvider(d), XVt(d);
      let m = d.getLogger(
        "com.anthropic.claude_code.events",
        {
          ISSUES_EXPLAINER: "report the issue at https://github.com/anthropics/claude-code/issues",
      ...
      JVt(m, "org"),
        ...
        process.on("beforeExit", async () => {
          await d?.forceFlush(), await Qfe()?.forceFlush();
        }),
        process.on("exit", () => {
          d?.forceFlush(), Qfe()?.forceFlush();
        });
    }
  }
  if (r && dpn()) {
    let u = await Lt();
    if (u.length > 0) {
      let d = u.map((f) => new X(f, { scheduledDelayMillis: a.OTEL_TRACES_EXPORT_INTERVAL ?? Ze })),
        m = new Z({ resource: o, spanProcessors: d });
      w.trace.setGlobalTracerProvider(m),
        e5t(m),
        process.on("beforeExit", async () => {
          await m.forceFlush();
        });
  ...
  return (
    vt(async () => {
      let u = a.CLAUDE_CODE_OTEL_SHUTDOWN_TIMEOUT_MS ?? 2000;
      try {
        pz();
        let d = [i.shutdown()],
          m = vCt();
        if (m) d.push(m.shutdown());
        let f = Qfe();
        if (f) d.push(f.shutdown());
        await Promise.race([Promise.all(d), Se(u, "OpenTelemetry shutdown timeout")]);
      } catch (d) {
        if (d instanceof Error && d.message.includes("timeout"))
          n(
            `
OpenTelemetry telemetry flush timed out after ${u}ms
...
async function ss() {
  ...
  try {
    ...
    await Promise.race([Promise.all(r), Se(t, "OpenTelemetry flush timeout")]), n("Telemetry flushed successfully");
  } catch (r) {
    if (r instanceof Pe)
      n(`Telemetry flush timed out after ${t}ms. Some metrics may not be exported.`, { level: "warn" });
  ...
  if (GC(s)) {
    ...
    return (
      ...
      (r.headers = async () => {
        ...
        let u = mi();
        if (!u || u.url !== c) return {};
        return { Authorization: `Bearer ${u.jwt}` };
      }),
      (r.httpAgentOptions = Re(c)),
      r
    );
  }
  let o = xt(),
    i = Ct(e, !!t?.otelHeadersHelper);
  if (t?.otelHeadersHelper)
    r.headers = async () => {
      let c = await swn();
      return { ...o, ...c };
    };
  else if (i.send) return (r.url = i.url), (r.headers = i.headers), (r.httpAgentOptions = Re(i.url)), r;
  else if (Object.keys(o).length > 0) r.headers = async () => o;
  return (
    (r.httpAgentOptions = Re(
      process.env[`OTEL_EXPORTER_OTLP_${e.toUpperCase()}_ENDPOINT`] ?? a.OTEL_EXPORTER_OTLP_ENDPOINT,
    )),
    r
  );
}
function Ct(e, t) {
  let r = { send: !1 },
    s = (E) => {
      ...
      return R === "claude-gateway" || (Array.isArray(R) && R.includes("claude-gateway"));
    },
    o = (E) => {
      try {
        if (!E || !new URL(E).host) return null;
        let T = new URL(`${E.replace(/\/+$/, "")}/v1/${e}`);
        return (T.protocol === "https:" || (T.protocol === "http:" && rt(T.href))) && !T.search && !T.hash
          ? T.href
          : null;
      } catch {
        return null;
  ...
  let c = { metrics: "METRICS", logs: "LOGS", traces: "TRACES" }[e],
    u = () => !!a.OTEL_EXPORTER_OTLP_HEADERS || !!a[`OTEL_EXPORTER_OTLP_${c}_HEADERS`];
  if (t || u()) return r;
  let d = o(a.ANTHROPIC_BASE_URL);
  if (!d) return r;
  let m = a.OTEL_EXPORTER_OTLP_ENDPOINT,
    f = a[`OTEL_EXPORTER_OTLP_${c}_ENDPOINT`] ?? (m && `${m}${m.endsWith("/") ? "" : "/"}v1/${e}`),
    O = null;
  try {
    O = f ? new URL(f).href : null;
  } catch {}
  if (O !== d) return r;
  return {
    send: !0,
    ...
    headers: async () => {
      ...
      return E && s(E) && o(a.ANTHROPIC_BASE_URL) === d && !u() ? { Authorization: `Bearer ${E}` } : {};
    },
  };
}
function rt(e) {
    ...
    (t.addRequest = function (o, ...i) {
      if (!o.getHeader("content-length") && !o.getHeader("transfer-encoding")) {
        let m = function () {
          ...
          f = function (O) {
            m(), o.destroy(O instanceof Error ? O : TypeError("OTLP request body chunk conversion failed"));
          },
          c = [],
          u = o.write.bind(o),
          d = o.end.bind(o);
        (o.write = function (E, T, R) {
          ...
          (o.end = function (E, T, R) {
            ...
            let z = typeof E === "function" ? E : typeof T === "function" ? T : R,
              K = Buffer.concat(c);
            if (!o.headersSent) o.setHeader("Content-Length", String(K.byteLength));
            return m(), d(K, z);
          });
      }
      r(o, ...i);
...
export {
  Ot as bootstrapTelemetry,
  ss as flushTelemetry,
  xe as getOTLPExporterConfig,
  Pt as getOtlpLogExporters,
  rs as initializeTelemetry,
  yt as isBigQueryMetricsEnabled,
  At as isTelemetryEnabled,
  we as parseExporterTypes,
  xt as parseOtelHeadersEnvVar,
