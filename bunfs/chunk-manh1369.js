...
function ACe(e) {
  if (!e) return "";
  return gs().segment(e)[Symbol.iterator]().next().value?.segment ?? "";
}
function NN(e) {
  if (!e) return "";
  let t = "";
  for (let { segment: r } of gs().segment(e)) t = r;
  return t;
}
function FN(e) {
  if (!e) return 0;
  let t = 0;
  for (let r of gs().segment(e)) t++;
  return t;
}
function d4(e) {
  if (!e) return [];
  return Array.from(gs().segment(e), (t) => t.segment);
}
function Rvn() {
...
function wsr(e) {
  ...
  let r = t.split(/\s+/).length,
    n = 0;
  for (let i of Rvn().segment(t)) if (i.isWordLike) n++;
  return Math.max(r, n);
}
var g = new Map();
function kvn(e, t) {
