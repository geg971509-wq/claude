...
import { ha, Ae, ie } from "/$bunfs/root/chunk-8tgj5dp2.js";
...
import { Jz, xjt, mH, Ojt, qYn, Ljt, Mgn, Bg } from "/$bunfs/root/chunk-vx3kc9d2.js";
...
function Co(Pn) {
  return !Pn.name?.startsWith("mcp__ide__");
}
function fo(pe) {
  return {
    ...pe,
...
function ho(Wn) {
  let { ide: _removed, ...On } = Wn ?? {};
  return On;
}
function Fe(ln) {
  let k = _(30),
    { availableIDEs: N, unavailableIDEs: Y, selectedIDE: ze, onClose: Xe, onSelect: H } = ln,
    Rt;
  ...
  let G = xt,
    Nt;
  if (k[5] !== N) (Nt = N.reduce(so, {})), (k[5] = N), (k[6] = Nt);
  else Nt = k[6];
  let se = Nt,
    Tt;
  if (k[7] !== N || k[8] !== se) {
    ...
    if (k[10] !== se)
      (T = (ce) => {
        let gn = (se[ce.name] || 0) > 1 && ce.workspaceFolders.length > 0;
        return { label: ce.name, value: ce.port.toString(), description: gn ? fe(ce.workspaceFolders) : void 0 };
      }),
        (k[10] = se),
        (k[11] = T);
    else T = k[11];
  ...
  if (un) {
    let T;
    if (k[12] !== G || k[13] !== q) (T = e(ne, { onComplete: () => G(q) })), (k[12] = G), (k[13] = q), (k[14] = T);
    else T = k[14];
    return T;
  }
  if (mn) {
  ...
  let T;
  if (k[17] !== N || k[18] !== G || k[19] !== Qe || k[20] !== q)
    (T =
      N.length === 0
        ? e(sr, {
            children: xjt()
...
https://code.claude.com/docs/en/jetbrains`
      ...
      r(o, {
        ...
        children: [
          ...
          r(o, {
            ...
            paddingLeft: 3,
            flexDirection: "column",
            children: [Y.slice(0, 4).map(co), Y.length > 4 && e(Jm, { count: Y.length - 4, unit: "IDE" })],
          }),
        ],
      })),
      (k[22] = Y),
  ...
  let ve;
  if (k[24] !== T || k[25] !== be)
    (ve = r(o, { flexDirection: "column", children: [T, be] })), (k[24] = T), (k[25] = be), (k[26] = ve);
  else ve = k[26];
  let kt;
  if (k[27] !== Xe || k[28] !== ve)
    (kt = e(me, {
...
async function io(n, i) {
  let h = i?.ide;
  if (!h || (h.type !== "sse-ide" && h.type !== "ws-ide")) return null;
  for (let a of n) if (a.url === h.url) return a;
  return null;
}
function pt(Dn) {
  let J = _(18),
    { availableIDEs: P, onSelectIDE: He, onDone: Ue } = Dn,
    jt;
  if (J[0] !== P[0]?.port) (jt = P[0]?.port?.toString() ?? ""), (J[0] = P[0]?.port), (J[1] = jt);
  else jt = J[1];
  let [Se, Cn] = u(jt),
    Mt;
  if (J[2] !== P || J[3] !== He)
    (Mt = (yn) => {
      let bn = P.find((En) => En.port === parseInt(yn));
      He(bn);
    }),
      (J[2] = P),
      (J[3] = He),
  ...
  let Ge = Mt,
    Pt;
  if (J[5] !== P) (Pt = P.map(lo)), (J[5] = P), (J[6] = Pt);
  else Pt = J[6];
  let et = Pt,
    Wt;
  if (J[7] !== Ue)
    (Wt = function le() {
      Ue("IDE selection cancelled", { display: "system" });
    }),
      (J[7] = Ue),
      (J[8] = Wt);
  else Wt = J[8];
  ...
  let xe;
  if (J[11] !== et || J[12] !== Se || J[13] !== Re)
    (xe = e(Ce, { defaultValue: Se, defaultFocusValue: Se, options: et, onChange: Re })),
      (J[11] = et),
      (J[12] = Se),
      (J[13] = Re),
      (J[14] = xe);
  ...
  let Bt;
  if (J[15] !== le || J[16] !== xe)
    (Bt = e(me, { title: "Select an IDE to open the project", onCancel: le, color: "ide", children: xe })),
      (J[15] = le),
      (J[16] = xe),
      (J[17] = Bt);
  else Bt = J[17];
...
function mt(vn) {
  let te = _(15),
    { runningIDEs: Ne, onSelectIDE: tt, onDone: ot } = vn,
    [nt, Sn] = u(Ne[0] ?? ""),
    Yt;
  if (te[0] !== tt)
    (Yt = (Rn) => {
  ...
  let it = Yt,
    qt;
  if (te[2] !== Ne) (qt = Ne.map(uo)), (te[2] = Ne), (te[3] = qt);
  else qt = te[3];
  let rt = qt,
    Jt;
  if (te[4] !== ot)
  ...
  let Ve;
  if (te[8] !== rt || te[9] !== nt || te[10] !== Te)
    (Ve = e(Ce, { defaultFocusValue: nt, options: rt, onChange: Te })),
      (te[8] = rt),
      (te[9] = nt),
      (te[10] = Te),
      (te[11] = Ve);
  ...
  let Xt;
  if (te[12] !== de || te[13] !== Ve)
    (Xt = e(me, { title: "Select IDE to install extension", onCancel: de, color: "ide", children: Ve })),
      (te[12] = de),
      (te[13] = Ve),
      (te[14] = Xt);
  else Xt = te[14];
...
function ft(xn) {
  ...
  if (Nn[0] !== ke || Nn[1] !== _e)
    (Qt = () => {
      ...
      (Nn[2] = Qt),
      (Nn[3] = Zt);
  else (Qt = Nn[2]), (Zt = Nn[3]);
  return A(Qt, Zt), null;
}
async function ro(n, i, h, a) {
  if (!n) {
    a("No IDE selected.");
    return;
  }
  let l = qYn(n.name),
    m = l ? await Ljt(l, n.name) : null;
  if (!m) {
  ...
  if (g === 0) {
    y("ide_open_project"), a(`Opened ${h ? "worktree" : "project"} in ${ae.bold(n.name)}`);
    return;
  }
  p("ide_open_project", "ide_open_project_failed"), a(`Failed to open in ${n.name}. Try opening manually: ${i}`);
}
async function an(n, i, h) {
    ...
    let I = await Mgn(),
      R = (D) => {
        if (i.onInstallIDEExtension)
          if ((i.onInstallIDEExtension(D), Jz(D)))
            n(`Installed plugin to ${ae.bold(Bg(D))}
Please ${ae.bold("restart your IDE")} completely for it to take effect`);
          else n(`Installed extension to ${ae.bold(Bg(D))}`);
      };
    if (I.length > 1)
      return e(mt, {
        runningIDEs: I,
  ...
  let w = m.filter((I) => I.isValid),
    g = m.filter((I) => !I.isValid),
    c = await io(w, a);
  return e(ht, { availableIDEs: w, unavailableIDEs: g, currentIDE: c, onChangeDynamicMcpConfig: l, onDone: n });
}
var ut = 35000;
function ht(Tn) {
  let z = _(27),
    { availableIDEs: st, unavailableIDEs: at, currentIDE: O, onChangeDynamicMcpConfig: ue, onDone: b } = Tn,
    [v, Vn] = u(null),
    V = W(po),
    ct = At(),
    lt = C(true),
  ...
  if (z[0] !== v || z[1] !== V || z[2] !== b)
    (Ht = () => {
      ...
      if (V.type === "connected") y("ide_connect"), b(`Connected to ${v.name}.`);
      else if (V.type === "failed") p("ide_connect", "ide_connect_failed"), b(`Failed to connect to ${v.name}.`);
    }),
      (Ut = [V, v, b]),
      (z[0] = v),
      (z[1] = V),
  ...
  if (z[5] !== v || z[6] !== b)
    (Gt = () => {
      ...
      p("ide_connect", "ide_connect_timeout"), b(`Connection to ${v.name} timed out.`);
    }),
      (z[5] = v),
      (z[6] = b),
      (z[7] = Gt);
  ...
  let eo;
  if (z[8] !== v || z[9] !== b) (eo = [v, b]), (z[8] = v), (z[9] = b), (z[10] = eo);
  else eo = z[10];
  Yn(Gt, v ? ut : null, eo);
  let to;
  if (z[11] !== O || z[12] !== V || z[13] !== ue || z[14] !== b || z[15] !== ct)
    (to = (oe) => {
      if (!ue) {
        b("Error connecting to IDE.");
        return;
      }
      if (!oe) {
        if (V && V.type === "connected" && O) {
          i3(V, mo);
          let { clearServerCache: kn } = import.meta.require("/$bunfs/root/chunk-qadpp1g1.js").mcpClientModule();
          kn("ide", V.config), ct(fo);
        }
        if ((ue(ho), O)) y("ide_disconnect");
        b(O ? `Disconnected from ${O.name}.` : "No IDE selected.");
        return;
      }
      let oo = oe.url;
      let _n = {
  ...
  let X;
  if (z[19] !== b) (X = () => b("IDE selection cancelled", { display: "system" })), (z[19] = b), (z[20] = X);
  else X = z[20];
  let no;
  if (z[21] !== st || z[22] !== O || z[23] !== dt || z[24] !== X || z[25] !== at)
    (no = e(Fe, { availableIDEs: st, unavailableIDEs: at, selectedIDE: O, onClose: X, onSelect: dt })),
      (z[21] = st),
      (z[22] = O),
      (z[23] = dt),
      (z[24] = X),
...
function fe(n, i = 100) {
  ...
  let h = ee(),
    ...
    D = a
      .map((E) => {
        let x = E.normalize("NFC");
        if (x.startsWith(I + he.sep)) E = x.slice(I.length + 1);
