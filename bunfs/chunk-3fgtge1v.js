...
import { CFe } from "/$bunfs/root/chunk-wkxx62a2.js";
...
import { UMe, jV, mh } from "/$bunfs/root/chunk-8tgj5dp2.js";
...
import { od, nO } from "/$bunfs/root/chunk-fs02hzwy.js";
...
import { B1, h1t, cut, S1t, Nsn } from "/$bunfs/root/chunk-zze8764r.js";
...
import { Bw, Vi, Es } from "/$bunfs/root/chunk-n71q9jb8.js";
...
var F = 3000,
  N = 30,
  U = 5,
  I = "cloud session was archived before producing output";
...
function R(r) {
  return new v("session_start_failed", `the cloud session could not start: ${r}`);
}
...
async function le(r, t, s, i) {
  ...
  process.once("SIGINT", o), await T_();
  let a = od("allow_remote_sessions", "Cloud sessions", "are");
  if (a) {
    if (nO("allow_remote_sessions") === "cache_miss") await Ic("cli_ultrareview", "policy_cache_miss");
    else await Pn("cli_ultrareview", "policy_disallowed");
    return Es(a);
  }
  await mh().catch(() => {});
  let g = Number(t.timeout),
    p = Number.isFinite(g) && g > 0 ? g : N,
    u = gr(),
  ...
  if (e.status !== "launched") {
    let y =
      e.status === "blocked" && e.actionUrl
        ? `
  \u2192 ${e.actionUrl}`
        : "";
    return (
      await Pn("cli_ultrareview", "cli_ultrareview_launch_failed"),
      Es(`Ultrareview could not launch: ${"message" in e ? e.message : e.body}${y}`)
    );
  }
  h(e.message),
    h(`View live progress in the browser: ${e.sessionUrl}`),
    h(`Waiting for findings (${B1()})\u2026`),
    process.removeListener("SIGINT", o);
  let d = () => {
    h(`
Cancelled. The remote review is still running \u2014 view it at ${e.sessionUrl}${
      e.postReviewTo
        ? `
Nothing will be posted to the pull request.`
        : ""
    }`),
      process.exit(130);
  };
  process.once("SIGINT", d);
  let f = () => {
      process.removeListener("SIGINT", d),
        process.once("SIGINT", () => {
          h(`
Interrupted. The review already finished (findings above).${
            e.postReviewTo
              ? `
The post to the pull request may already be on its way \u2014 check the PR before posting by hand.`
              : ""
          }`),
            process.exit(130);
        });
    ...
    m;
  try {
    m = await P(e.sessionId, u.signal, p * 60 * 1000, i);
  } catch (y) {
    await Pn("cli_ultrareview", "cli_ultrareview_poll_failed", {
      reason: y instanceof v ? c(y.reason) : w("poll_unknown"),
    });
    let A = y instanceof v && y.reason === "session_start_failed";
    return Es(`Ultrareview failed: ${l(y)}
Session: ${e.sessionUrl}${
      e.postReviewTo
        ? `
Nothing was posted to the pull request, and nothing will be \u2014 the --post request ends with this run.${A ? "" : " If the review finishes in the browser, you can post the findings by hand."}`
        : ""
    }`);
  }
  let _ = O(m),
    b = _ === I ? w("session_archived") : w("orchestrator_error"),
    S = Nsn(m),
    T = S !== void 0 ? { findings_count: S } : void 0;
  if (!_) f();
  if (t.json) {
    if (
      (await Bw(
...
`,
      ...
      if ((await Pn("cli_ultrareview", "cli_ultrareview_remote_error", { reason: b }), e.postReviewTo))
        h("Nothing was posted to the pull request: the review failed.");
    } else await E(e.postReviewTo, m, s, i), await Zs("cli_ultrareview", T);
    return Vi(_ ? 1 : 0);
  }
  if (_)
    return (
      await Pn("cli_ultrareview", "cli_ultrareview_remote_error", { reason: b }),
      Es(`Review failed: ${_}
Session: ${e.sessionUrl}${
        e.postReviewTo
          ? `
...
`,
    ...
    await E(e.postReviewTo, m, s, i),
    await Zs("cli_ultrareview", T),
    Vi(0)
  );
}
async function E(r, t, s, i) {
  if (!r) return;
  h("Posting the findings to the pull request as a comment from your GitHub account\u2026");
  let o = await h1t(r, t, s, i);
  if (o.status === "posting")
    h(`Posting now \u2014 a single plain comment, not a review. Follow along: ${o.sessionUrl}`);
  else h(o.reason);
}
function O(r) {
  try {
...
async function P(r, t, s, i) {
  ...
  while (Date.now() < o) {
    ...
    try {
      let n = await jV(r, a, { credentials: i });
      if (((a = n.lastEventId), (g = 0), n.sessionStatus === "archived")) {
        if (n.newEvents.length > 0) p.push(...n.newEvents);
        let e = cut(p);
        if (e) return e;
        if (n.startupFailure && S1t()) throw R(n.startupFailure);
        return `{"error":"${I}"}`;
      }
      if (n.newEvents.length > 0) {
        p.push(...n.newEvents);
        for (let d of n.newEvents)
          if (d.type === "system" && (d.subtype === "hook_progress" || d.subtype === "hook_response")) {
            let f = D(d.stdout);
            if (f && f !== u) (u = f), h(`  ${f}`);
          }
        let e = cut(p);
      ...
      if (t.aborted || n instanceof v) throw n;
      if (!UMe(n)) throw new v("poll_api_error", l(n));
      if (++g >= U) throw new v("poll_connection_lost", "lost connection to the cloud session after repeated retries");
    }
    await ne(F, t);
  }
  throw new v("poll_timeout", `cloud session exceeded ${Math.round(s / 60000)} minutes`);
}
function D(r) {
  let t = `<${CFe}>`,
    s = `</${CFe}>`,
    i = r.lastIndexOf(s),
    o = i === -1 ? -1 : r.lastIndexOf(t, i);
  if (o === -1 || i <= o) return null;
  try {
    let a = V(r.slice(o + t.length, i)),
      g = a.stage ?? "running",
      p = a.bugs_found ?? 0,
      u = a.bugs_verified ?? 0,
      n = a.bugs_refuted ?? 0;
    return `${g} \u2014 ${p} found, ${u} verified, ${n} refuted`;
  } catch {
    return null;
  }
}
function G(r) {
  ...
  let s = t,
    i = s.length,
    o = [ae.bold(`Review complete \u2014 ${i} ${k(i, "finding")}`), ""];
  for (let a of s) {
    let g = C[a.severity ?? "normal"] ?? "\uD83D\uDD34",
      p = a.file_path ?? "?",
      u = a.start_line ?? 0,
      n = a.end_line ?? u,
      e = u === n ? `${p}:${u}` : `${p}:${u}-${n}`,
...
`),
      m = f === -1 ? d : d.slice(0, f),
      _ = f === -1 ? "" : d.slice(f + 2);
    if ((o.push(`${g} ${ae.bold(e)}`), m)) o.push(m);
    if (_) o.push(""), o.push(_);
    o.push("");
  }
  return o
    .join(`
...
export { x as resolvePostFlag, le as ultrareviewHandler };
