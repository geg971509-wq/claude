// @bun @bytecode
// Claude Code is a Beta product per Anthropic's Commercial Terms of Service.
// By using Claude Code, you agree that all code acceptance or rejection decisions you make,
// and the associated conversations in context, constitute Feedback under Anthropic's Commercial Terms,
// and may be used to improve Anthropic's products, including training models.
// You are responsible for reviewing any code suggestions before use.

// (c) Anthropic PBC. All rights reserved. Use is subject to the Legal Agreements outlined here: https://code.claude.com/docs/en/legal-and-compliance.

// Version: 2.1.252
import { qm } from "/$bunfs/root/chunk-saay52v7.js";
var jre = "(invalid tool name)";
function Np(e) {
  return qm(e);
}
function uN(e) {
  return e;
}
function zwn(e) {
  return e;
}
function orr(e) {
  return e;
}
function irr(e) {
  return e;
}
function Vwn(e) {
  if (Object.getPrototypeOf(e) !== Array.prototype)
    throw TypeError("mintDisplayedUpdates: non-plain array in mint input \u2014 plain data only");
  if (!Number.isSafeInteger(e.length) || e.length < 0)
    throw TypeError("mintDisplayedUpdates: non-integer length in mint input \u2014 plain data only");
  let t = [];
  for (let o = 0; o < e.length; o++) {
    if (!(o in e)) throw TypeError("mintDisplayedUpdates: sparse mint input \u2014 plain data only");
    t.push(f(e[o]));
  }
  return Object.freeze(t);
}
function f(e, t = new WeakMap()) {
  if (typeof e === "function") throw TypeError("deepFreezePlainData: function in mint input \u2014 plain data only");
  if (e !== null && typeof e === "object") {
    let o = t.get(e);
    if (o !== void 0) return o;
    let r = Object.getPrototypeOf(e);
    if (!(Array.isArray(e) ? r === Array.prototype : r === Object.prototype || r === null))
      throw TypeError("deepFreezePlainData: non-plain prototype in mint input \u2014 plain data only");
    let a = Object.getOwnPropertyDescriptors(e);
    for (let n of Object.keys(a)) {
      let i = a[n];
      if (i && (i.get !== void 0 || i.set !== void 0))
        throw TypeError("deepFreezePlainData: accessor property in mint input \u2014 plain data only");
    }
    if (Object.getOwnPropertySymbols(e).length > 0)
      throw TypeError("deepFreezePlainData: symbol-keyed property in mint input \u2014 plain data only");
    for (let n of Object.keys(a)) {
      let i = a[n];
      if (i && i.enumerable === false && !(Array.isArray(e) && n === "length"))
        throw TypeError("deepFreezePlainData: non-enumerable property in mint input \u2014 plain data only");
    }
    let l = Array.isArray(e) ? [] : {};
    t.set(e, l);
    for (let n of Object.keys(a)) {
      if (Array.isArray(e) && n === "length") continue;
      if (n === "__proto__" || n === "constructor")
        throw TypeError("deepFreezePlainData: prototype-key property in mint input \u2014 plain data only");
      let i = a[n];
      Object.defineProperty(l, n, { value: f(i?.value, t), enumerable: true, writable: false, configurable: false });
    }
    return Object.freeze(l), l;
  }
  return e;
}
function go(e) {
  return e;
}
function srr(e) {
  return async function (o, r, s) {
    if (Kwn(r)) return b(e, o, r, s);
    return T(e, o, r, s);
  };
}
function Kwn(e) {
  return typeof e === "object" && e !== null && Symbol.asyncIterator in e;
}
async function T(e, t, o, r) {
  let s = t.payload().safeParse(o);
  if (!s.success) return t.default;
  let a;
  try {
    let { replied: n } = e.request(
      {
        kind: t.kind,
        payload: s.data,
        userInvoked: t.userInvoked,
        yieldsToPanels: t.yieldsToPanels,
        queuesLaterAsksBehind: t.queuesLaterAsksBehind,
        yieldsToDraft: t.yieldsToDraft,
      },
      r,
    );
    a = await n;
  } catch {
    return t.default;
  }
  if ("cancelled" in a) return t.default;
  let l = t.result().safeParse(a.result);
  return l.success ? l.data : t.default;
}
async function b(e, t, o, r) {
  let s = o[Symbol.asyncIterator](),
    a = await s.next();
  if (a.done) return t.default;
  let l = t.payload().safeParse(a.value);
  if (!l.success) return s.return?.(void 0), t.default;
  let n = new AbortController(),
    i = () => n.abort();
  if (r?.signal)
    if (r.signal.aborted) n.abort();
    else r.signal.addEventListener("abort", i, { once: true });
  let { replied: P, update: m } = e.request(
      {
        kind: t.kind,
        payload: l.data,
        userInvoked: t.userInvoked,
        yieldsToPanels: t.yieldsToPanels,
        queuesLaterAsksBehind: t.queuesLaterAsksBehind,
        yieldsToDraft: t.yieldsToDraft,
      },
      {
        signal: n.signal,
        queueBehind: r?.queueBehind,
        armInputGrace: r?.armInputGrace,
        replacesAnswered: r?.replacesAnswered,
        parksBlockingDialogs: r?.parksBlockingDialogs,
      },
    ),
    u;
  (async () => {
    try {
      while (!n.signal.aborted) {
        let p = await s.next();
        if (p.done) return;
        if (n.signal.aborted) return;
        let c = t.payload().safeParse(p.value);
        if (!c.success) continue;
        m(c.data);
      }
    } catch (p) {
      (u = p), n.abort();
    }
  })().catch(() => {});
  let d;
  try {
    d = await P;
  } finally {
    r?.signal?.removeEventListener("abort", i), s.return?.(void 0);
  }
  if (u !== void 0) throw u;
  if ("cancelled" in d) return t.default;
  let y = t.result().safeParse(d.result);
  return y.success ? y.data : t.default;
}
export { jre, Np, uN, zwn, orr, irr, Vwn, go, srr, Kwn };
