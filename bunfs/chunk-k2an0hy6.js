...
function Tf(e, { validateBody: t = !0 } = {}) {
  ...
  if (o) {
    let s;
    try {
      s = r(e, o[0].length, { ecmaVersion: "latest", sourceType: "module" });
    } catch {
      s = void 0;
    }
    if (s?.type === "ObjectExpression") {
      ...
      if (!h.trimStart().startsWith("/")) {
        ...
        let m = p ? g(p) : void 0;
        if (m && !("error" in m)) return { meta: m.meta, scriptBody: h.replace(/^[;\s]*\n/, "").trimStart() };
      }
    }
  }
  let i;
  ...
  let k = e
    ...
    .replace(/^[;\s]*\n/, "")
    .trimStart();
  return { meta: d.meta, scriptBody: k };
}
function OUn(e) {
  return !("error" in Tf(e));
}
var f = 80;
function P(e, t) {
...
`)[o.line - 1]
  ...
  let a = Math.max(0, Math.min(o.column, i.length)),
    l = Math.max(0, Math.min(a - Math.floor(f / 2), i.length - f)),
    u = i.slice(l, l + f),
    c = `${gi(" ", a - l)}^`;
...
function O(e) {
  ...
  if (b.has(t)) throw Error(`reserved key name not allowed in meta: ${t}`);
  return t;
}
function g(e) {
  let t = e.name;
  if (typeof t !== "string" || t.length === 0) return { error: "meta.name must be a non-empty string" };
  let n = e.description;
  if (typeof n !== "string" || n.length === 0) return { error: "meta.description must be a non-empty string" };
  let r = typeof e.title === "string" && e.title.length > 0 ? e.title : void 0,
    o = typeof e.whenToUse === "string" ? e.whenToUse : void 0,
    i = W(e.phases);
  return { meta: { name: t, description: n, title: r, whenToUse: o, phases: i } };
}
function W(e) {
  ...
  for (let n of e)
    if (n && typeof n === "object" && "title" in n) {
      let { title: r, detail: o, model: i } = n;
      if (typeof r === "string")
        t.push({ title: r, detail: typeof o === "string" ? o : void 0, model: typeof i === "string" ? i : void 0 });
    }
