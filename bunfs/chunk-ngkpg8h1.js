...
import { GIe, _dt, K4n, Vle, YIe, Idt, $r } from "/$bunfs/root/chunk-zze8764r.js";
...
function oe({ file_path: r }, { verbose: s }) {
  if (!r) return null;
  if (r.startsWith(va())) return "";
  return e(xg, { filePath: r, children: s ? r : Fo(r) });
}
function se({ filePath: r = "", structuredPatch: s, originalFile: i }, a, { style: o, verbose: l }) {
  if (!r) return null;
  let c = r.startsWith(va());
  return e(GUe, {
    filePath: r,
    structuredPatch: s,
    firstLine: i ? wr(i) : null,
    fileContent: i || void 0,
    style: o,
    verbose: l,
    previewHint: c ? "/plan to preview" : void 0,
    collapsed: !c && (vLe(r) || RLe(r)),
  });
}
function ne(r, s) {
...
function ie(r, s) {
  ...
  return e(ep, { result: r, verbose: i });
}
function O(le) {
  let d = _(16),
    { filePath: f, oldString: R, newString: T, replaceAll: v, style: P, verbose: p } = le,
    S;
  if (d[0] !== f || d[1] !== T || d[2] !== R || d[3] !== v)
    (S = () => x(f, R, T, v)), (d[0] = f), (d[1] = T), (d[2] = R), (d[3] = v), (d[4] = S);
  else S = d[4];
  let [L] = u(S),
    g;
  if (d[5] !== f || d[6] !== p)
    (g = e(VU, { file_path: f, operation: "update", firstLine: null, verbose: p })), (d[5] = f), (d[6] = p), (d[7] = g);
  else g = d[7];
  let b;
  if (d[8] !== L || d[9] !== f || d[10] !== P || d[11] !== p)
  ...
  let j;
  if (d[13] !== g || d[14] !== b) (j = e(Jn, { fallback: g, children: b })), (d[13] = g), (d[14] = b), (d[15] = j);
  else j = d[15];
  return j;
}
function y(ce) {
  let pe = _(7),
    ...
    { patch: D, firstLine: E, fileContent: k } = zn(fe),
    U;
  if (pe[0] !== k || pe[1] !== N || pe[2] !== E || pe[3] !== D || pe[4] !== w || pe[5] !== C)
    (U = e(VU, { file_path: N, operation: "update", patch: D, firstLine: E, fileContent: k, style: w, verbose: C })),
      (pe[0] = k),
      (pe[1] = N),
      (pe[2] = E),
      (pe[3] = D),
...
async function x(r, s, i, a) {
  try {
    let o = await K4n(r, s, GIe);
    if (o === null || o.truncated || o.content === "") {
      let { patch: M } = Idt({ filePath: r, fileContents: s, oldString: s, newString: i });
      return { patch: M, firstLine: null, fileContent: void 0 };
    }
    let l = Vle(o.content, s) || s,
      c = YIe(s, l, i),
      { patch: m } = Idt({ filePath: r, fileContents: o.content, oldString: l, newString: c, replaceAll: a });
    return {
      patch: _dt(m, o.lineOffset - 1),
      firstLine: o.lineOffset === 1 ? wr(o.content) : null,
      fileContent: o.content,
    };
  } catch (o) {
    if (Ht(o)) n(`Failed to load rejection diff for ${r}: ${o.message}`, { level: "error" });
    else h(o);
