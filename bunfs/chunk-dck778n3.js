...
var A = new Set(["user-cancel", "remote-cancel", "shutdown", "interrupt", "turn-abort"]);
function GLe(e) {
  return A.has(Za(e));
}
...
function h3(e) {
  return e.aborted && Za(e.reason) === S_n;
}
var R = new DOMException(S_n, "AbortError");
function zKe() {
  return R;
}
var b_n = "subagent-park",
  E = new DOMException(b_n, "AbortError");
function gvr() {
  return E;
}
function hvr(e) {
  return e.aborted && Za(e.reason) === b_n;
}
function zLe(e) {
  switch (Za(e)) {
    case "user-cancel":
...
function p(e) {
  let r = this.deref();
  if (!r || !GLe(r.signal.reason)) return;
  e.deref()?.abort(r.signal.reason);
}
var UWt = 600000;
function k(e) {
  e.deref()?.abort(bu("recovery-timeout"));
}
function T_n(e, r = UWt) {
  let t = gr();
