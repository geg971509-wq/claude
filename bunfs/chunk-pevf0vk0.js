...
import { i, H, f, N } from "/$bunfs/root/chunk-saay52v7.js";
import { O } from "/$bunfs/root/chunk-dqkj2bph.js";
import { dirname as p, join as g } from "path";
var $fr = 50,
  w = m(() => f({ version: N(1), sessions: H(f({ id: i(), reason: i(), at: i() })) })),
...
async function u(r) {
  ...
  return e === null ? t.data.sessions : t.data.sessions.filter((s) => !y(s.at, e));
}
function D(r) {
  try {
...
async function zEr(r, o, t) {
  try {
    let e = Tr(r),
      a = [...(await u(t)).filter((c) => Tr(c.id) !== e), { id: e, reason: o, at: t.now().toISOString() }].slice(-$fr);
    await t.writeText(
...
async function krn(r, o, t) {
  try {
    let e = Tr(r),
      s = (await u(t)).find((a) => Tr(a.id) === e);
...
function Hrn(r) {
  ...
  return {
    ...
    writeText: async (e) => {
      if (o) {
        if (!(await o.write(t, e, { mode: 384 })).ok) throw Error("device unbound-creates write failed");
        return;
      }
      let s = u6n();
      await an().mkdir(p(s), 448), await an().atomicWrite(s, e, 384);
    },
    now: () => new Date(),
    retentionCutoff: async () => {
      let e = await import("/$bunfs/root/chunk-9dxxc10b.js");
      return (await e.isRetentionCleanupSafe(r)) ? e.getCutoffDate() : null;
    },
  };
}
export { $fr, u6n, zEr, krn, Hrn };
