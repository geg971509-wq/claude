...
import { dx } from "/$bunfs/root/chunk-ypdw393e.js";
...
import { qS } from "/$bunfs/root/chunk-dck778n3.js";
...
import { sqe, _i, xe, mF, oI, rz } from "/$bunfs/root/chunk-zze8764r.js";
...
import { Hk } from "/$bunfs/root/chunk-anwgkcw5.js";
...
import { Kte, iue } from "/$bunfs/root/chunk-pzvxycbq.js";
...
import { hnt } from "/$bunfs/root/chunk-drhq38fx.js";
...
async function F(n, b, e) {
  ...
  m?.({ kind: "bash_mode_progress", toolUseId: i, input: n, progress: null, verbose: e.options.verbose });
  try {
    let o = { ...e, toolUseId: `${i}${sqe}` },
      a = (f) => {
        if (f.type !== "progress") return;
        m?.({ kind: "bash_mode_progress", toolUseId: i, input: n, progress: f.data, verbose: e.options.verbose });
      },
      r = null;
    if (u) r = import.meta.require("/$bunfs/root/chunk-pwatxckn.js").PowerShellTool;
    let S = r ?? _i,
      t = (
        r
    ...
    let y = t.stderr,
      h = await iue(S, { ...t, stderr: "" }, g(), pS(e.session), e.storageV5),
      p = typeof h.content === "string" ? h.content : t.stdout,
      P = p.startsWith(Kte) ? p : Wt(p),
      c = l && !t.interrupted && !t.backgroundTaskId && !e.abortController.signal.aborted;
    return {
      messages: [
        ...(c ? [] : [rz()]),
        d,
        xe({ content: `<bash-stdout>${P}</bash-stdout><bash-stderr>${Wt(y)}</bash-stderr>` }),
      ],
      shouldQuery: c,
    };
  } catch (o) {
    if (o instanceof dx) {
      ...
      return {
        messages: [
          ...(r ? [] : [rz()]),
          d,
          xe({ content: `<bash-stdout>${Wt(o.stdout)}</bash-stdout><bash-stderr>${Wt(o.stderr)}</bash-stderr>` }),
        ],
        shouldQuery: r,
      };
    }
    ...
    return {
      messages: [
        ...(a ? [] : [rz()]),
        d,
        xe({ content: `<bash-stderr>Command failed: ${Wt(gge(o, e.session))}</bash-stderr>` }),
      ],
      shouldQuery: a,
    };
  } finally {
