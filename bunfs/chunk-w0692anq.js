...
import { kR } from "/$bunfs/root/chunk-gcks6mn0.js";
...
import { Mh, uT, sM, Kbe, Zft, ZGe, fX, tze, tC, emt } from "/$bunfs/root/chunk-ddbk6aje.js";
...
import { appendFileSync as re, createWriteStream as ie, mkdirSync as oe } from "fs";
...
async function z() {
  ...
  let r = (await te()) ?? process.execPath,
    t = [...(Wl() ? [r] : [r, process.argv[1]]), ...process.argv.slice(2)],
    o = ert(process.env);
...
async function He(r) {
  ...
  if (!t) await z();
  if (n < 3 || n === r.length - 1) return _(void 0, "bad argv: --bg-pty-host <sock> <cols> <rows> -- <file> [args...]");
  let o = r[0];
  ...
  let P = process.env.CLAUDE_BG_PTY_AUTH;
  delete process.env.CLAUDE_BG_PTY_AUTH;
  let x = process.env.CLAUDE_BG_SOCKET_TOKENS_PATH;
  if (x) {
    let e = await yAe(x);
    if (e?.ptyAuth) P = e.ptyAuth;
    else if (!P) await X(o, "tokens-file unreadable; DATA gate fail-open");
  ...
  function V(e) {
    switch (e.t) {
      case "resize": {
        let s = Number(e.cols),
          i = Number(e.rows);
        if (s > 0 && s <= fX && i > 0 && i <= fX && !y && !g) {
          if ((S.resize(s, i), D() !== "windows")) {
            try {
    ...
    let s = emt(
      (i) => {
        if (i.kind === Kbe) {
          ...
          if (!y && !g) {
            if ((S.write(i.payload), c && D() !== "windows")) {
  ...
  let E, k;
  if (D() !== "windows") {
    let e = kR(process.env.CLAUDE_PTY_HEARTBEAT_MS) || 60000,
      s = 3;
    (k = setInterval(() => {
      if (y) return;
      for (let B of p) {
        let G = b.get(B);
        if (!G?.armed) continue;
        if ((G.missed++, G.missed >= 3)) B.destroy(), p.delete(B);
        else w(B, tC({ t: "ping" }));
      }
    }, e)),
      k.unref();
    let i = kR(process.env.CLAUDE_PTY_ORPHAN_CHECK_MS) || 2000,
      u = 30;
    (E = setInterval(() => {
      if (y) return;
  ...
  if (((y = !0), c))
    try {
      ...
      QH(e, JSON.stringify({ code: T, signal: U, tail: u }), 384);
    } catch {}
  if (E) clearInterval(E);
  if (k) clearInterval(k);
  if (!g) S.close();
  if ((F?.close(), c && D() !== "windows")) {
    I = "SIGHUP";
    try {
...
async function pe(r, n, t = 5000) {
  let o = new Promise((f) => r.once("connection", () => f())),
    l = (n ? n().catch(() => {}) : Promise.resolve()).then(() => ne(t));
