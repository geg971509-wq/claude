...
import { gt, y0 } from "/$bunfs/root/chunk-q14dgq5g.js";
import { JM, de } from "/$bunfs/root/chunk-xgfepdf4.js";
import { bW, Whe, bHe } from "/$bunfs/root/chunk-qaaw3n80.js";
import { Bgt, W8n, Hz } from "/$bunfs/root/chunk-tveh0k24.js";
import { createHash as I } from "crypto";
import { readFile as D, stat as V, unlink as W } from "fs/promises";
function Uje(e, i, n) {
  return e.ver === i && (e.observers === void 0 || Object.hasOwn(e.observers, n));
}
async function iHe(
  { filepath: e, persistId: i, html: n, slug: r, ver: t, confirmsResend: d, batch: o, heldSkipsRead: l },
  f,
) {
...
`,
  ...
  let s = Uje(f.getArtifactReadObservation(r), t, a) && !Whe(f.agentId, r, t),
  ...
  let _ = I("sha256").update(n).digest("hex"),
    w = de().pendingHandoverReads,
    R = w.get(p),
    u = R !== void 0 && R.slug === r && R.ver === t && R.contentHash === _ ? R : void 0;
  ...
  for (let [F, H] of w)
    if (H.slug === r) {
      for (let q of H.unrecordableBy) T.add(q);
      w.delete(F);
    }
  let A = !n.endsWith(`
`);
...
async function sHe(e, i, n) {
  for (let [r, t] of de().pendingHandoverReads)
    if (t.slug === e && t.ver === i) {
      let d = await y0(r).catch(() => {
        return;
      });
...
async function TPt(e, i) {
  let n = gt(e),
    r = de().pendingHandoverReads,
    t = r.get(n);
  if (t === void 0) return;
  if (I("sha256").update(i).digest("hex") !== t.contentHash) {
    r.delete(n);
    return;
  }
  let d = await y0(n).catch(() => {
    return;
  });
...
async function Gtn(e, i, n) {
  let r = O(e, i, n);
  if (r === void 0) return;
  let [t, d] = r,
    o;
  try {
    o = (await y0(t)) === d.mtimeMs ? "intact" : "changed";
  } catch (l) {
    o = X(l) ? "changed" : "unknown";
  }
...
function E(e, i, n) {
  return {
    ...
    unread: gpr(i.linesReturned.get(n ?? "main") ?? [], i.requiredLines),
  };
}
function ztn(e, i, n) {
  let r = O(e, i, n);
...
function APt({ lines: e, unterminated: i, unread: n }) {
  ...
  return (
    ...
    (i
      ...
      : "") +
    (r > 0 && r < e
      ? `; you have not yet Read ${k(r, "line")} ${n.map(([t, d]) => (t === d ? `${t}` : `${t}-${d}`)).join(", ")}`
      : "")
  );
}
function sae(e, i, n, r) {
  let t = mpr(e, i, n, r);
  return t === void 0 ? "" : ` (${APt(t)})`;
}
function gpr(e, i) {
  let n = [],
    r = 0;
...
async function Vtn(e, i) {
  de().pendingHandoverReads.delete(gt(e)), W8n(i, e), await W(e).catch(() => {});
}
var P = {
  artifactRead: void 0,
  commit: () => {
...
async function hpr(e, i, n, r) {
  try {
    let t = de().pendingHandoverReads,
      d = t.get(e),
      o = i.agentId ?? "main";
    ...
    let l = [r.firstLine, r.firstLine + r.lineCount],
      f = await G(e, d.bytes, d.contentHash);
    ...
    let a = f === "holds",
      ...
      x = L(B([...(d.linesReturned.get(o) ?? []), ...C.map((s) => s.range), l]), d.requiredLines),
    ...
    let m = () => {
      try {
        ...
        let p = B([...(d.linesReturned.get(o) ?? []), l]);
        if (!a || !L(p, d.requiredLines)) {
          d.linesReturned.set(o, p);
          return;
        }
        return (
          d.linesReturned.delete(o), d.completedBy.add(o), z(d, e, i, n, o) ? { slug: d.slug, ver: d.ver } : void 0
        );
      } catch (s) {
        h(s);
        return;
    ...
    return { artifactRead: x && a && !d.unrecordableBy.has(o) ? { slug: d.slug, ver: d.ver } : void 0, commit: m };
  } catch (t) {
    return h(t), P;
  }
}
async function kEr(e, i, n, r) {
  return (await hpr(e, i, n, r)).commit();
}
function N(e, i) {
...
function j(e, i, n) {
  let r = de().refusedPublishBodies.get(JM(i, e.slug)),
...
function z(e, i, n, r, t) {
  ...
  let d = j(e, n.agentId, t),
    o = d?.observedFrom;
  try {
    n.setArtifactReadVersion(e.slug, e.ver, bW(n.agentId, r));
  } finally {
    if (d !== void 0)
...
async function G(e, i, n) {
  try {
    ...
    return X(r) ? "changed" : "unreadable";
  }
}
export { Uje, iHe, sHe, TPt, EPt, mpr, Gtn, ztn, APt, sae, gpr, Vtn, hpr, kEr };
