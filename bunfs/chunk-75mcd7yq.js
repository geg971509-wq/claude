// @bun @bytecode
// Claude Code is a Beta product per Anthropic's Commercial Terms of Service.
// By using Claude Code, you agree that all code acceptance or rejection decisions you make,
// and the associated conversations in context, constitute Feedback under Anthropic's Commercial Terms,
// and may be used to improve Anthropic's products, including training models.
// You are responsible for reviewing any code suggestions before use.

// (c) Anthropic PBC. All rights reserved. Use is subject to the Legal Agreements outlined here: https://code.claude.com/docs/en/legal-and-compliance.

// Version: 2.1.252
import {
  uOt,
  jZ,
  dOt,
  pe,
  N6n,
  pOt,
  Fr,
  $i,
  WHe,
  Ks,
  en,
  lt,
  Ia,
  Ki,
  BWe,
  Pae,
  hi,
  lc,
  nn,
  Ws,
  bat,
  qHe,
  jWe,
  GHe,
  Dae,
  WWe,
  Oae,
  zHe,
  wat,
  qWe,
  b_e,
  IA,
  Yrn,
  Tat,
  Eat,
  fOt,
  VHe,
  Aat,
  xx,
  KHe,
  PA,
  vL,
  RL,
  Cat,
  kS,
  w_e,
  vat,
  Rat,
  GWe,
  zWe,
  TB,
  Jrn,
  kat,
  mOt,
  Hat,
  gOt,
  T_e,
  Lae,
  Mae,
  hOt,
  _Ot,
  yOt,
  SOt,
  xat,
  E_e,
  XHe,
  Iat,
  bOt,
  YHe,
  Pat,
  wOt,
  TOt,
  Dat,
  EOt,
  A_e,
  C_e,
  Oat,
  Lat,
  Mat,
  v_e,
  Nae,
  R_e,
  JHe,
  AOt,
  WZ,
  COt,
  QHe,
  VWe,
  vOt,
  ROt,
  kOt,
  HOt,
  xOt,
  IOt,
  POt,
  DOt,
  Nat,
  OOt,
  LOt,
  MOt,
  NOt,
  FOt,
  $Ot,
  UOt,
  BOt,
  jOt,
  ZHe,
  WOt,
  exe,
  qOt,
  GOt,
  KWe,
  XWe,
  YWe,
  zOt,
  VOt,
  KOt,
  JWe,
  XOt,
  YOt,
  JOt,
  QOt,
  ZOt,
  k_e,
  eLt,
  H_e,
  Fae,
  QWe,
  tLt,
  nLt,
  rLt,
  Fat,
  $at,
  oLt,
  iLt,
  sLt,
  aLt,
  lLt,
  cLt,
  uLt,
  dLt,
  pLt,
  txe,
  fLt,
  mLt,
  gLt,
  hLt,
  _Lt,
  Uat,
  ZWe,
  e9e,
  Bat,
  jat,
  Wat,
  yLt,
  qat,
  Gat,
  SLt,
  bLt,
  zat,
  t9e,
  nxe,
  rxe,
  wLt,
  TLt,
  ELt,
  ALt,
  CLt,
  vLt,
  RLt,
  kLt,
  HLt,
  xLt,
  ILt,
  PLt,
  DLt,
  Qrn,
  OLt,
  x_e,
  Zrn,
  oxe,
  eon,
  ton,
  n9e,
  non,
  ron,
  oon,
  ion,
  son,
  I_e,
  aon,
  lon,
  con,
  uon,
  don,
  pon,
  Vat,
  $ae,
  LLt,
  P_e,
  D_e,
  MLt,
  O_e,
  NLt,
  FLt,
  Kat,
  fon,
  mon,
} from "/$bunfs/root/chunk-cagtjatw.js";
import { $9e } from "/$bunfs/root/chunk-wxnckmc4.js";
import { Uae } from "/$bunfs/root/chunk-fh0ptqfa.js";
import { F9e } from "/$bunfs/root/chunk-3kyt3cgx.js";
var {
    create: fo,
    defineProperty: cr,
    getOwnPropertyDescriptor: po,
    getOwnPropertyNames: So,
    getPrototypeOf: go,
  } = Object,
  _o = Object.prototype.hasOwnProperty,
  U = (e, t) => () => (t || e((t = { exports: {} }).exports, t), t.exports),
  ms = (e, t) => {
    let r = {};
    for (var s in e) cr(r, s, { get: e[s], enumerable: !0 });
    if (t) cr(r, Symbol.toStringTag, { value: "Module" });
    return r;
  },
  yo = (e, t, r, s) => {
    if ((t && typeof t === "object") || typeof t === "function") {
      for (var a = So(t), o = 0, n = a.length, c; o < n; o++)
        if (((c = a[o]), !_o.call(e, c) && c !== r))
          cr(e, c, { get: ((i) => t[i]).bind(null, c), enumerable: !(s = po(t, c)) || s.enumerable });
    }
    return e;
  },
  lr = (e, t, r) => (
    (r = e != null ? fo(go(e)) : {}),
    yo(t || !e || !e.__esModule ? cr(r, "default", { value: e, enumerable: !0 }) : r, e)
  );
var Ar = Symbol.for("mcp.sdk.errorBrands");
function Je(e, t) {
  let r = new Set(),
    s = t;
  while (typeof s === "function") {
    let a = s.mcpBrand;
    if (Object.prototype.hasOwnProperty.call(s, "mcpBrand") && typeof a === "string") r.add(a);
    s = Object.getPrototypeOf(s);
  }
  if (r.size === 0) return;
  Object.defineProperty(e, Ar, { value: r, enumerable: !1, configurable: !0 });
}
function _e(e, t) {
  try {
    if (
      typeof t === "object" &&
      t !== null &&
      Object.prototype.hasOwnProperty.call(e, "mcpBrand") &&
      typeof e.mcpBrand === "string" &&
      Object.prototype.hasOwnProperty.call(t, Ar)
    ) {
      let r = t[Ar];
      if (r && typeof r.has === "function" && r.has(e.mcpBrand)) return !0;
    }
  } catch {}
  return Function.prototype[Symbol.hasInstance].call(e, t);
}
var wB = (function (e) {
    return (
      (e.InvalidRequest = "invalid_request"),
      (e.InvalidClient = "invalid_client"),
      (e.InvalidGrant = "invalid_grant"),
      (e.UnauthorizedClient = "unauthorized_client"),
      (e.UnsupportedGrantType = "unsupported_grant_type"),
      (e.InvalidScope = "invalid_scope"),
      (e.AccessDenied = "access_denied"),
      (e.ServerError = "server_error"),
      (e.TemporarilyUnavailable = "temporarily_unavailable"),
      (e.UnsupportedResponseType = "unsupported_response_type"),
      (e.UnsupportedTokenType = "unsupported_token_type"),
      (e.InvalidToken = "invalid_token"),
      (e.MethodNotAllowed = "method_not_allowed"),
      (e.TooManyRequests = "too_many_requests"),
      (e.InvalidClientMetadata = "invalid_client_metadata"),
      (e.InvalidRedirectUri = "invalid_redirect_uri"),
      (e.InsufficientScope = "insufficient_scope"),
      (e.InvalidTarget = "invalid_target"),
      e
    );
  })({}),
  Hx = class e extends Error {
    static {
      Object.defineProperty(this, "mcpBrand", { value: "mcp.OAuthError" });
    }
    static [Symbol.hasInstance](t) {
      return _e(this, t);
    }
    static isInstance(t) {
      if (typeof this !== "function")
        throw TypeError(
          "isInstance must be called on the class (e.g. `SdkError.isInstance(value)`); for callbacks use `v => SdkError.isInstance(v)`",
        );
      return _e(this, t);
    }
    constructor(t, r, s) {
      super(r);
      (this.code = t), (this.errorUri = s), (this.name = "OAuthError"), Je(this, new.target);
    }
    toResponseObject() {
      let t = { error: this.code, error_description: this.message };
      if (this.errorUri) t.error_uri = this.errorUri;
      return t;
    }
    static fromResponse(t) {
      return new e(t.error, t.error_description ?? t.error, t.error_uri);
    }
  },
  vo = (function (e) {
    return (
      (e.NotConnected = "NOT_CONNECTED"),
      (e.AlreadyConnected = "ALREADY_CONNECTED"),
      (e.NotInitialized = "NOT_INITIALIZED"),
      (e.CapabilityNotSupported = "CAPABILITY_NOT_SUPPORTED"),
      (e.RequestTimeout = "REQUEST_TIMEOUT"),
      (e.ConnectionClosed = "CONNECTION_CLOSED"),
      (e.SendFailed = "SEND_FAILED"),
      (e.InvalidResult = "INVALID_RESULT"),
      (e.UnsupportedResultType = "UNSUPPORTED_RESULT_TYPE"),
      (e.InputRequiredRoundsExceeded = "INPUT_REQUIRED_ROUNDS_EXCEEDED"),
      (e.ListPaginationExceeded = "LIST_PAGINATION_EXCEEDED"),
      (e.MethodNotSupportedByProtocolVersion = "METHOD_NOT_SUPPORTED_BY_PROTOCOL_VERSION"),
      (e.EraNegotiationFailed = "ERA_NEGOTIATION_FAILED"),
      (e.ClientHttpNotImplemented = "CLIENT_HTTP_NOT_IMPLEMENTED"),
      (e.ClientHttpAuthentication = "CLIENT_HTTP_AUTHENTICATION"),
      (e.ClientHttpForbidden = "CLIENT_HTTP_FORBIDDEN"),
      (e.ClientHttpUnexpectedContent = "CLIENT_HTTP_UNEXPECTED_CONTENT"),
      (e.ClientHttpFailedToOpenStream = "CLIENT_HTTP_FAILED_TO_OPEN_STREAM"),
      (e.ClientHttpFailedToTerminateSession = "CLIENT_HTTP_FAILED_TO_TERMINATE_SESSION"),
      e
    );
  })({}),
  Uo = class extends Error {
    static {
      Object.defineProperty(this, "mcpBrand", { value: "mcp.SdkError" });
    }
    static [Symbol.hasInstance](e) {
      return _e(this, e);
    }
    static isInstance(e) {
      if (typeof this !== "function")
        throw TypeError(
          "isInstance must be called on the class (e.g. `SdkError.isInstance(value)`); for callbacks use `v => SdkError.isInstance(v)`",
        );
      return _e(this, e);
    }
    constructor(e, t, r) {
      super(t);
      (this.code = e), (this.data = r), (this.name = "SdkError"), Je(this, new.target);
    }
  },
  RS = class extends Uo {
    static {
      Object.defineProperty(this, "mcpBrand", { value: "mcp.SdkHttpError" });
    }
    constructor(e, t, r) {
      super(e, t, r);
      this.name = "SdkHttpError";
    }
    get status() {
      return this.data.status;
    }
    get statusText() {
      return this.data.statusText;
    }
  };
function Ms(e) {
  let t = typeof e === "string" ? new URL(e) : new URL(e.href);
  return (t.hash = ""), t;
}
function js({ requestedResource: e, configuredResource: t }) {
  let r = typeof e === "string" ? new URL(e) : new URL(e.href),
    s = typeof t === "string" ? new URL(t) : new URL(t.href);
  if (r.origin !== s.origin) return !1;
  if (r.pathname.length < s.pathname.length) return !1;
  let a = r.pathname.endsWith("/") ? r.pathname : r.pathname + "/",
    o = s.pathname.endsWith("/") ? s.pathname : s.pathname + "/";
  return a.startsWith(o);
}
var As = "2026-07-28",
  Hr = [As];
function ze(e) {
  return e >= As;
}
function xr(e) {
  return e.filter((t) => !ze(t));
}
function Vt(e) {
  return e.filter((t) => ze(t));
}
function Ls(e) {
  let t = e.structuredContent;
  if (t === void 0) return e;
  if (!(typeof t !== "object" || t === null || Array.isArray(t))) return e;
  if (e.content?.some((r) => r.type === "text") ?? !1) return e;
  return { ...e, content: [...(e.content ?? []), { type: "text", text: JSON.stringify(t) }] };
}
var Us = ["task", "inputRequests", "requestState"];
function bo(e) {
  if (e === null || typeof e !== "object" || Array.isArray(e) || e.content !== void 0 || Us.some((t) => t in e))
    return e;
  return { ...e, content: [] };
}
function Ro() {
  let e = bat(() => Ki([pe(), Fr(), $i(), WHe(), hi(pe(), e), en(e)])),
    t = hi(pe(), e),
    r = Ki([pe(), Fr().int()]),
    s = pe(),
    a = lt({ ttl: Fr().optional() }),
    o = lt({ taskId: pe() }),
    n = Ia({ progressToken: r.optional(), "io.modelcontextprotocol/related-task": o.optional() }),
    c = lt({ _meta: n.optional() }),
    i = c.extend({ task: a.optional() }),
    l = lt({ method: pe(), params: c.loose().optional() }),
    d = lt({ _meta: n.optional() }),
    S = lt({ method: pe(), params: d.loose().optional() }),
    R = Ia({ _meta: n.optional() }),
    w = Ki([pe(), Fr().int()]),
    g = R.strict(),
    _ = d.extend({ requestId: w.optional(), reason: pe().optional() }),
    h = S.extend({ method: nn("notifications/cancelled"), params: _ }),
    u = lt({
      src: pe(),
      mimeType: pe().optional(),
      sizes: en(pe()).optional(),
      theme: lc(["light", "dark"]).optional(),
    }),
    p = lt({ icons: en(u).optional() }),
    m = lt({ name: pe(), title: pe().optional() }),
    y = m.extend({ ...m.shape, ...p.shape, version: pe(), websiteUrl: pe().optional(), description: pe().optional() }),
    E = Pae(lt({ applyDefaults: $i().optional() }), t),
    v = qHe(
      (de) => {
        if (de && typeof de === "object" && !Array.isArray(de) && Object.keys(de).length === 0) return { form: {} };
        return de;
      },
      Pae(lt({ form: E.optional(), url: t.optional() }), t.optional()),
    ),
    f = Ia({
      list: t.optional(),
      cancel: t.optional(),
      requests: Ia({
        sampling: Ia({ createMessage: t.optional() }).optional(),
        elicitation: Ia({ create: t.optional() }).optional(),
      }).optional(),
    }),
    P = Ia({
      list: t.optional(),
      cancel: t.optional(),
      requests: Ia({ tools: Ia({ call: t.optional() }).optional() }).optional(),
    }),
    I = lt({
      experimental: hi(pe(), t).optional(),
      sampling: lt({ context: t.optional(), tools: t.optional() }).optional(),
      elicitation: v.optional(),
      roots: lt({ listChanged: $i().optional() }).optional(),
      tasks: f.optional(),
      extensions: hi(pe(), t).optional(),
    }),
    L = c.extend({ protocolVersion: pe(), capabilities: I, clientInfo: y }),
    k = l.extend({ method: nn("initialize"), params: L }),
    j = lt({
      experimental: hi(pe(), t).optional(),
      logging: t.optional(),
      completions: t.optional(),
      prompts: lt({ listChanged: $i().optional() }).optional(),
      resources: lt({ subscribe: $i().optional(), listChanged: $i().optional() }).optional(),
      tools: lt({ listChanged: $i().optional() }).optional(),
      tasks: P.optional(),
      extensions: hi(pe(), t).optional(),
    }),
    F = R.extend({ protocolVersion: pe(), capabilities: j, serverInfo: y, instructions: pe().optional() }),
    V = S.extend({ method: nn("notifications/initialized"), params: d.optional() }),
    Y = l.extend({ method: nn("ping"), params: c.optional() }),
    te = lt({ progress: Fr(), total: Ws(Fr()), message: Ws(pe()) }),
    Q = lt({ ...d.shape, ...te.shape, progressToken: r }),
    oe = S.extend({ method: nn("notifications/progress"), params: Q }),
    X = c.extend({ cursor: s.optional() }),
    C = l.extend({ params: X.optional() }),
    O = R.extend({ nextCursor: s.optional() }),
    A = lt({ uri: pe(), mimeType: Ws(pe()), _meta: hi(pe(), Ks()).optional() }),
    b = A.extend({ text: pe() }),
    z = pe().refine(
      (de) => {
        try {
          return atob(de), !0;
        } catch {
          return !1;
        }
      },
      { message: "Invalid Base64 string" },
    ),
    q = A.extend({ blob: z }),
    D = lc(["user", "assistant"]),
    H = lt({
      audience: en(D).optional(),
      priority: Fr().min(0).max(1).optional(),
      lastModified: jZ.datetime({ offset: !0 }).optional(),
    }),
    W = lt({
      ...m.shape,
      ...p.shape,
      uri: pe(),
      description: Ws(pe()),
      mimeType: Ws(pe()),
      size: Ws(Fr()),
      annotations: H.optional(),
      _meta: Ws(Ia({})),
    }),
    J = lt({
      ...m.shape,
      ...p.shape,
      uriTemplate: pe(),
      description: Ws(pe()),
      mimeType: Ws(pe()),
      annotations: H.optional(),
      _meta: Ws(Ia({})),
    }),
    ee = C.extend({ method: nn("resources/list") }),
    he = O.extend({ resources: en(W) }),
    T = C.extend({ method: nn("resources/templates/list") }),
    N = O.extend({ resourceTemplates: en(J) }),
    M = c.extend({ uri: pe() }),
    x = M,
    K = l.extend({ method: nn("resources/read"), params: x }),
    G = R.extend({ contents: en(Ki([b, q])) }),
    se = S.extend({ method: nn("notifications/resources/list_changed"), params: d.optional() }),
    ie = M,
    le = l.extend({ method: nn("resources/subscribe"), params: ie }),
    ne = M,
    re = l.extend({ method: nn("resources/unsubscribe"), params: ne }),
    ve = d.extend({ uri: pe() }),
    ce = S.extend({ method: nn("notifications/resources/updated"), params: ve }),
    Ee = lt({ name: pe(), description: Ws(pe()), required: Ws($i()) }),
    be = lt({ ...m.shape, ...p.shape, description: Ws(pe()), arguments: Ws(en(Ee)), _meta: Ws(Ia({})) }),
    Pe = C.extend({ method: nn("prompts/list") }),
    Ce = O.extend({ prompts: en(be) }),
    ke = c.extend({ name: pe(), arguments: hi(pe(), pe()).optional() }),
    Ne = l.extend({ method: nn("prompts/get"), params: ke }),
    Me = lt({ type: nn("text"), text: pe(), annotations: H.optional(), _meta: hi(pe(), Ks()).optional() }),
    je = lt({
      type: nn("image"),
      data: z,
      mimeType: pe(),
      annotations: H.optional(),
      _meta: hi(pe(), Ks()).optional(),
    }),
    Ae = lt({
      type: nn("audio"),
      data: z,
      mimeType: pe(),
      annotations: H.optional(),
      _meta: hi(pe(), Ks()).optional(),
    }),
    it = lt({ type: nn("tool_use"), name: pe(), id: pe(), input: hi(pe(), Ks()), _meta: hi(pe(), Ks()).optional() }),
    qe = lt({
      type: nn("resource"),
      resource: Ki([b, q]),
      annotations: H.optional(),
      _meta: hi(pe(), Ks()).optional(),
    }),
    ct = W.extend({ type: nn("resource_link") }),
    ye = Ki([Me, je, Ae, ct, qe]),
    We = lt({ role: D, content: ye }),
    Ye = R.extend({ description: pe().optional(), messages: en(We) }),
    Le = S.extend({ method: nn("notifications/prompts/list_changed"), params: d.optional() }),
    ut = lt({
      title: pe().optional(),
      readOnlyHint: $i().optional(),
      destructiveHint: $i().optional(),
      idempotentHint: $i().optional(),
      openWorldHint: $i().optional(),
    }),
    Xe = lt({ taskSupport: lc(["required", "optional", "forbidden"]).optional() }),
    Ue = lt({
      ...m.shape,
      ...p.shape,
      description: pe().optional(),
      inputSchema: lt({
        type: nn("object"),
        properties: hi(pe(), e).optional(),
        required: en(pe()).optional(),
      }).catchall(Ks()),
      outputSchema: lt({ type: nn("object"), properties: hi(pe(), e).optional(), required: en(pe()).optional() })
        .catchall(Ks())
        .optional(),
      annotations: ut.optional(),
      execution: Xe.optional(),
      _meta: hi(pe(), Ks()).optional(),
    }),
    Ze = C.extend({ method: nn("tools/list") }),
    Qe = O.extend({ tools: en(Ue) }),
    et = R.extend({ content: en(ye), structuredContent: hi(pe(), Ks()).optional(), isError: $i().optional() }),
    ue = i.extend({ name: pe(), arguments: hi(pe(), Ks()).optional() }),
    dt = l.extend({ method: nn("tools/call"), params: ue }),
    Kt = S.extend({ method: nn("notifications/tools/list_changed"), params: d.optional() }),
    tt = lc(["debug", "info", "notice", "warning", "error", "critical", "alert", "emergency"]),
    ht = c.extend({ level: tt }),
    mt = l.extend({ method: nn("logging/setLevel"), params: ht }),
    ft = d.extend({ level: tt, logger: pe().optional(), data: Ks() }),
    pt = S.extend({ method: nn("notifications/message"), params: ft }),
    St = lt({ name: pe().optional() }),
    gt = lt({
      hints: en(St).optional(),
      costPriority: Fr().min(0).max(1).optional(),
      speedPriority: Fr().min(0).max(1).optional(),
      intelligencePriority: Fr().min(0).max(1).optional(),
    }),
    _t = lt({ mode: lc(["auto", "required", "none"]).optional() }),
    Jt = lt({
      type: nn("tool_result"),
      toolUseId: pe().describe("The unique identifier for the corresponding tool call."),
      content: en(ye),
      structuredContent: lt({}).loose().optional(),
      isError: $i().optional(),
      _meta: hi(pe(), Ks()).optional(),
    }),
    vt = BWe("type", [Me, je, Ae]),
    Ie = BWe("type", [Me, je, Ae, it, Jt]),
    yt = lt({ role: D, content: Ki([Ie, en(Ie)]), _meta: hi(pe(), Ks()).optional() }),
    bt = i.extend({
      messages: en(yt),
      modelPreferences: gt.optional(),
      systemPrompt: pe().optional(),
      includeContext: lc(["none", "thisServer", "allServers"]).optional(),
      temperature: Fr().optional(),
      maxTokens: Fr().int(),
      stopSequences: en(pe()).optional(),
      metadata: t.optional(),
      tools: en(Ue).optional(),
      toolChoice: _t.optional(),
    }),
    Rt = l.extend({ method: nn("sampling/createMessage"), params: bt }),
    wt = R.extend({
      model: pe(),
      stopReason: Ws(lc(["endTurn", "stopSequence", "maxTokens"]).or(pe())),
      role: D,
      content: vt,
    }),
    $t = R.extend({
      model: pe(),
      stopReason: Ws(lc(["endTurn", "stopSequence", "maxTokens", "toolUse"]).or(pe())),
      role: D,
      content: Ki([Ie, en(Ie)]),
    }),
    Et = lt({ type: nn("boolean"), title: pe().optional(), description: pe().optional(), default: $i().optional() }),
    Pt = lt({
      type: nn("string"),
      title: pe().optional(),
      description: pe().optional(),
      minLength: Fr().optional(),
      maxLength: Fr().optional(),
      format: lc(["email", "uri", "date", "date-time"]).optional(),
      default: pe().optional(),
    }),
    Tt = lt({
      type: lc(["number", "integer"]),
      title: pe().optional(),
      description: pe().optional(),
      minimum: Fr().optional(),
      maximum: Fr().optional(),
      default: Fr().optional(),
    }),
    zt = lt({
      type: nn("string"),
      title: pe().optional(),
      description: pe().optional(),
      enum: en(pe()),
      default: pe().optional(),
    }),
    Ct = lt({
      type: nn("string"),
      title: pe().optional(),
      description: pe().optional(),
      oneOf: en(lt({ const: pe(), title: pe() })),
      default: pe().optional(),
    }),
    kt = lt({
      type: nn("string"),
      title: pe().optional(),
      description: pe().optional(),
      enum: en(pe()),
      enumNames: en(pe()).optional(),
      default: pe().optional(),
    }),
    qt = Ki([zt, Ct]),
    De = lt({
      type: nn("array"),
      title: pe().optional(),
      description: pe().optional(),
      minItems: Fr().optional(),
      maxItems: Fr().optional(),
      items: lt({ type: nn("string"), enum: en(pe()) }),
      default: en(pe()).optional(),
    }),
    Ve = lt({
      type: nn("array"),
      title: pe().optional(),
      description: pe().optional(),
      minItems: Fr().optional(),
      maxItems: Fr().optional(),
      items: lt({ anyOf: en(lt({ const: pe(), title: pe() })) }),
      default: en(pe()).optional(),
    }),
    Gt = Ki([De, Ve]),
    Bt = Ki([kt, qt, Gt]),
    Se = Ki([Bt, Et, Pt, Tt]),
    ge = i.extend({
      mode: nn("form").optional(),
      message: pe(),
      requestedSchema: lt({ type: nn("object"), properties: hi(pe(), Se), required: en(pe()).optional() }).catchall(
        Ks(),
      ),
    }),
    It = i.extend({ mode: nn("url"), message: pe(), elicitationId: pe(), url: pe().url() }),
    Re = Ki([ge, It]),
    Wt = l.extend({ method: nn("elicitation/create"), params: Re }),
    Yt = d.extend({ elicitationId: pe() }),
    Xt = S.extend({ method: nn("notifications/elicitation/complete"), params: Yt }),
    Zt = R.extend({
      action: lc(["accept", "decline", "cancel"]),
      content: qHe((de) => (de === null ? void 0 : de), hi(pe(), Ki([pe(), Fr(), $i(), en(pe())])).optional()),
    }),
    Qt = lt({ type: nn("ref/resource"), uri: pe() }),
    er = lt({ type: nn("ref/prompt"), name: pe() }),
    tr = c.extend({
      ref: Ki([er, Qt]),
      argument: lt({ name: pe(), value: pe() }),
      context: lt({ arguments: hi(pe(), pe()).optional() }).optional(),
    }),
    Ot = l.extend({ method: nn("completion/complete"), params: tr }),
    rr = R.extend({ completion: Ia({ values: en(pe()).max(100), total: Ws(Fr().int()), hasMore: Ws($i()) }) }),
    sr = lt({ uri: pe().startsWith("file://"), name: pe().optional(), _meta: hi(pe(), Ks()).optional() }),
    rt = l.extend({ method: nn("roots/list"), params: c.optional() }),
    Nt = R.extend({ roots: en(sr) }),
    ar = S.extend({ method: nn("notifications/roots/list_changed"), params: d.optional() }),
    or = Ia({ ttl: Fr().optional(), pollInterval: Fr().optional() }),
    nr = lc(["working", "input_required", "completed", "failed", "cancelled"]),
    Oe = lt({
      taskId: pe(),
      status: nr,
      ttl: Ki([Fr(), WHe()]),
      createdAt: pe(),
      lastUpdatedAt: pe(),
      pollInterval: Ws(Fr()),
      statusMessage: Ws(pe()),
    }),
    me = R.extend({ task: Oe }),
    ir = d.merge(Oe),
    Fe = S.extend({ method: nn("notifications/tasks/status"), params: ir }),
    st = l.extend({ method: nn("tasks/get"), params: c.extend({ taskId: pe() }) }),
    at = R.merge(Oe),
    ot = l.extend({ method: nn("tasks/result"), params: c.extend({ taskId: pe() }) }),
    Ir = R.loose(),
    fe = C.extend({ method: nn("tasks/list") }),
    ae = O.extend({ tasks: en(Oe) }),
    He = l.extend({ method: nn("tasks/cancel"), params: c.extend({ taskId: pe() }) });
  return {
    JSONValueSchema: e,
    JSONObjectSchema: t,
    ProgressTokenSchema: r,
    CursorSchema: s,
    TaskMetadataSchema: a,
    RelatedTaskMetadataSchema: o,
    RequestMetaSchema: n,
    BaseRequestParamsSchema: c,
    TaskAugmentedRequestParamsSchema: i,
    RequestSchema: l,
    NotificationsParamsSchema: d,
    NotificationSchema: S,
    ResultSchema: R,
    RequestIdSchema: w,
    EmptyResultSchema: g,
    CancelledNotificationParamsSchema: _,
    CancelledNotificationSchema: h,
    IconSchema: u,
    IconsSchema: p,
    BaseMetadataSchema: m,
    ImplementationSchema: y,
    ClientTasksCapabilitySchema: f,
    ServerTasksCapabilitySchema: P,
    ClientCapabilitiesSchema: I,
    InitializeRequestParamsSchema: L,
    InitializeRequestSchema: k,
    ServerCapabilitiesSchema: j,
    InitializeResultSchema: F,
    InitializedNotificationSchema: V,
    PingRequestSchema: Y,
    ProgressSchema: te,
    ProgressNotificationParamsSchema: Q,
    ProgressNotificationSchema: oe,
    PaginatedRequestParamsSchema: X,
    PaginatedRequestSchema: C,
    PaginatedResultSchema: O,
    ResourceContentsSchema: A,
    TextResourceContentsSchema: b,
    BlobResourceContentsSchema: q,
    RoleSchema: D,
    AnnotationsSchema: H,
    ResourceSchema: W,
    ResourceTemplateSchema: J,
    ListResourcesRequestSchema: ee,
    ListResourcesResultSchema: he,
    ListResourceTemplatesRequestSchema: T,
    ListResourceTemplatesResultSchema: N,
    ResourceRequestParamsSchema: M,
    ReadResourceRequestParamsSchema: x,
    ReadResourceRequestSchema: K,
    ReadResourceResultSchema: G,
    ResourceListChangedNotificationSchema: se,
    SubscribeRequestParamsSchema: ie,
    SubscribeRequestSchema: le,
    UnsubscribeRequestParamsSchema: ne,
    UnsubscribeRequestSchema: re,
    ResourceUpdatedNotificationParamsSchema: ve,
    ResourceUpdatedNotificationSchema: ce,
    PromptArgumentSchema: Ee,
    PromptSchema: be,
    ListPromptsRequestSchema: Pe,
    ListPromptsResultSchema: Ce,
    GetPromptRequestParamsSchema: ke,
    GetPromptRequestSchema: Ne,
    TextContentSchema: Me,
    ImageContentSchema: je,
    AudioContentSchema: Ae,
    ToolUseContentSchema: it,
    EmbeddedResourceSchema: qe,
    ResourceLinkSchema: ct,
    ContentBlockSchema: ye,
    PromptMessageSchema: We,
    GetPromptResultSchema: Ye,
    PromptListChangedNotificationSchema: Le,
    ToolAnnotationsSchema: ut,
    ToolExecutionSchema: Xe,
    ToolSchema: Ue,
    ListToolsRequestSchema: Ze,
    ListToolsResultSchema: Qe,
    CallToolResultSchema: et,
    CallToolRequestParamsSchema: ue,
    CallToolRequestSchema: dt,
    ToolListChangedNotificationSchema: Kt,
    LoggingLevelSchema: tt,
    SetLevelRequestParamsSchema: ht,
    SetLevelRequestSchema: mt,
    LoggingMessageNotificationParamsSchema: ft,
    LoggingMessageNotificationSchema: pt,
    ModelHintSchema: St,
    ModelPreferencesSchema: gt,
    ToolChoiceSchema: _t,
    ToolResultContentSchema: Jt,
    SamplingContentSchema: vt,
    SamplingMessageContentBlockSchema: Ie,
    SamplingMessageSchema: yt,
    CreateMessageRequestParamsSchema: bt,
    CreateMessageRequestSchema: Rt,
    CreateMessageResultSchema: wt,
    CreateMessageResultWithToolsSchema: $t,
    BooleanSchemaSchema: Et,
    StringSchemaSchema: Pt,
    NumberSchemaSchema: Tt,
    UntitledSingleSelectEnumSchemaSchema: zt,
    TitledSingleSelectEnumSchemaSchema: Ct,
    LegacyTitledEnumSchemaSchema: kt,
    SingleSelectEnumSchemaSchema: qt,
    UntitledMultiSelectEnumSchemaSchema: De,
    TitledMultiSelectEnumSchemaSchema: Ve,
    MultiSelectEnumSchemaSchema: Gt,
    EnumSchemaSchema: Bt,
    PrimitiveSchemaDefinitionSchema: Se,
    ElicitRequestFormParamsSchema: ge,
    ElicitRequestURLParamsSchema: It,
    ElicitRequestParamsSchema: Re,
    ElicitRequestSchema: Wt,
    ElicitationCompleteNotificationParamsSchema: Yt,
    ElicitationCompleteNotificationSchema: Xt,
    ElicitResultSchema: Zt,
    ResourceTemplateReferenceSchema: Qt,
    PromptReferenceSchema: er,
    CompleteRequestParamsSchema: tr,
    CompleteRequestSchema: Ot,
    CompleteResultSchema: rr,
    RootSchema: sr,
    ListRootsRequestSchema: rt,
    ListRootsResultSchema: Nt,
    RootsListChangedNotificationSchema: ar,
    TaskCreationParamsSchema: or,
    TaskStatusSchema: nr,
    TaskSchema: Oe,
    CreateTaskResultSchema: me,
    TaskStatusNotificationParamsSchema: ir,
    TaskStatusNotificationSchema: Fe,
    GetTaskRequestSchema: st,
    GetTaskResultSchema: at,
    GetTaskPayloadRequestSchema: ot,
    GetTaskPayloadResultSchema: Ir,
    ListTasksRequestSchema: fe,
    ListTasksResultSchema: ae,
    CancelTaskRequestSchema: He,
    CancelTaskResultSchema: R.merge(Oe),
    ClientRequestSchema: Ki([Y, k, Ot, mt, Ne, Pe, ee, T, K, le, re, dt, Ze, st, ot, fe, He]),
    ClientNotificationSchema: Ki([h, oe, V, ar, Fe]),
    ClientResultSchema: Ki([g, wt, $t, Zt, Nt, at, ae, me]),
    ServerRequestSchema: Ki([Y, Rt, Wt, rt, st, ot, fe, He]),
    ServerNotificationSchema: Ki([h, oe, pt, ce, se, Kt, Le, Fe, Xt]),
    ServerResultSchema: Ki([g, F, rr, Ye, Ce, he, N, G, et, Qe, at, ae, me]),
    CallToolResultWireSchema: Ks()
      .superRefine((de, mo) => {
        if (typeof de !== "object" || de === null || Array.isArray(de) || de.content !== void 0) return;
        for (let hs of Us)
          if (hs in de) {
            mo.addIssue({
              code: "custom",
              message: `content is required when the body carries '${hs}' \u2014 another result family cannot default into an empty tools/call success`,
            });
            return;
          }
      })
      .transform(bo)
      .pipe(et),
  };
}
var wo;
function Ds() {
  return (wo ??= Ro());
}
function Vs(e) {
  return e.type !== "object";
}
var $o = new Set(["const", "enum", "default", "examples"]),
  Eo = new Set(["properties", "patternProperties", "$defs", "definitions", "dependentSchemas"]);
function Po(e) {
  let t = typeof e.$schema === "string" ? e.$schema : void 0;
  if (e.$id !== void 0)
    return { ...(t !== void 0 && { $schema: t }), type: "object", properties: { result: e }, required: ["result"] };
  let r = (s, a) => {
    if (Array.isArray(s)) return s.map((n) => r(n, !1));
    if (s === null || typeof s !== "object") return s;
    if (!a && s.$id !== void 0) return s;
    let o = {};
    for (let [n, c] of Object.entries(s))
      if (a) o[n] = r(c, !1);
      else if ((n === "$ref" || n === "$dynamicRef") && typeof c === "string")
        o[n] = c === "#" ? "#/properties/result" : c.startsWith("#/") ? `#/properties/result${c.slice(1)}` : c;
      else if ($o.has(n)) o[n] = c;
      else if (Eo.has(n)) o[n] = r(c, !0);
      else o[n] = r(c, !1);
    return o;
  };
  return {
    ...(t !== void 0 && { $schema: t }),
    type: "object",
    properties: { result: r(e, !1) },
    required: ["result"],
  };
}
var Fs = {
    ping: null,
    initialize: null,
    "completion/complete": null,
    "logging/setLevel": null,
    "prompts/get": null,
    "prompts/list": null,
    "resources/list": null,
    "resources/templates/list": null,
    "resources/read": null,
    "resources/subscribe": null,
    "resources/unsubscribe": null,
    "tools/call": null,
    "tools/list": null,
    "tasks/get": null,
    "tasks/result": null,
    "tasks/list": null,
    "tasks/cancel": null,
    "sampling/createMessage": null,
    "elicitation/create": null,
    "roots/list": null,
  },
  Hs = {
    "notifications/cancelled": null,
    "notifications/progress": null,
    "notifications/initialized": null,
    "notifications/roots/list_changed": null,
    "notifications/tasks/status": null,
    "notifications/message": null,
    "notifications/resources/updated": null,
    "notifications/resources/list_changed": null,
    "notifications/tools/list_changed": null,
    "notifications/prompts/list_changed": null,
    "notifications/elicitation/complete": null,
  },
  To = {
    ping: null,
    initialize: null,
    "completion/complete": null,
    "logging/setLevel": null,
    "prompts/get": null,
    "prompts/list": null,
    "resources/list": null,
    "resources/templates/list": null,
    "resources/read": null,
    "resources/subscribe": null,
    "resources/unsubscribe": null,
    "tools/call": null,
    "tools/list": null,
    "sampling/createMessage": null,
    "elicitation/create": null,
    "roots/list": null,
  },
  ur;
function Kr() {
  if (ur) return ur;
  let e = Ds();
  return (
    (ur = {
      requestSchemas: {
        ping: e.PingRequestSchema,
        initialize: e.InitializeRequestSchema,
        "completion/complete": e.CompleteRequestSchema,
        "logging/setLevel": e.SetLevelRequestSchema,
        "prompts/get": e.GetPromptRequestSchema,
        "prompts/list": e.ListPromptsRequestSchema,
        "resources/list": e.ListResourcesRequestSchema,
        "resources/templates/list": e.ListResourceTemplatesRequestSchema,
        "resources/read": e.ReadResourceRequestSchema,
        "resources/subscribe": e.SubscribeRequestSchema,
        "resources/unsubscribe": e.UnsubscribeRequestSchema,
        "tools/call": e.CallToolRequestSchema,
        "tools/list": e.ListToolsRequestSchema,
        "tasks/get": e.GetTaskRequestSchema,
        "tasks/result": e.GetTaskPayloadRequestSchema,
        "tasks/list": e.ListTasksRequestSchema,
        "tasks/cancel": e.CancelTaskRequestSchema,
        "sampling/createMessage": e.CreateMessageRequestSchema,
        "elicitation/create": e.ElicitRequestSchema,
        "roots/list": e.ListRootsRequestSchema,
      },
      notificationSchemas: {
        "notifications/cancelled": e.CancelledNotificationSchema,
        "notifications/progress": e.ProgressNotificationSchema,
        "notifications/initialized": e.InitializedNotificationSchema,
        "notifications/roots/list_changed": e.RootsListChangedNotificationSchema,
        "notifications/tasks/status": e.TaskStatusNotificationSchema,
        "notifications/message": e.LoggingMessageNotificationSchema,
        "notifications/resources/updated": e.ResourceUpdatedNotificationSchema,
        "notifications/resources/list_changed": e.ResourceListChangedNotificationSchema,
        "notifications/tools/list_changed": e.ToolListChangedNotificationSchema,
        "notifications/prompts/list_changed": e.PromptListChangedNotificationSchema,
        "notifications/elicitation/complete": e.ElicitationCompleteNotificationSchema,
      },
      resultSchemas: {
        ping: e.EmptyResultSchema,
        initialize: e.InitializeResultSchema,
        "completion/complete": e.CompleteResultSchema,
        "logging/setLevel": e.EmptyResultSchema,
        "prompts/get": e.GetPromptResultSchema,
        "prompts/list": e.ListPromptsResultSchema,
        "resources/list": e.ListResourcesResultSchema,
        "resources/templates/list": e.ListResourceTemplatesResultSchema,
        "resources/read": e.ReadResourceResultSchema,
        "resources/subscribe": e.EmptyResultSchema,
        "resources/unsubscribe": e.EmptyResultSchema,
        "tools/call": e.CallToolResultWireSchema,
        "tools/list": e.ListToolsResultSchema,
        "sampling/createMessage": e.CreateMessageResultWithToolsSchema,
        "elicitation/create": e.ElicitResultSchema,
        "roots/list": e.ListRootsResultSchema,
      },
    }),
    ur
  );
}
function xs(e) {
  return Object.prototype.hasOwnProperty.call(Fs, e);
}
function Js(e) {
  return Object.prototype.hasOwnProperty.call(Hs, e);
}
function zo(e) {
  return Object.prototype.hasOwnProperty.call(To, e);
}
function Co(e) {
  return zo(e) ? Kr().resultSchemas[e] : void 0;
}
function ko(e) {
  return xs(e) ? Kr().requestSchemas[e] : void 0;
}
function qo(e) {
  return Js(e) ? Kr().notificationSchemas[e] : void 0;
}
var Pl = Object.keys(Fs),
  Tl = Object.keys(Hs);
function Lr(e) {
  return e !== null && typeof e === "object" && !Array.isArray(e);
}
function dr(e, t) {
  if (e === void 0) return { ok: !1, reason: "not-in-era" };
  let r = e.safeParse(t);
  return r.success ? { ok: !0, value: r.data } : { ok: !1, reason: "invalid", message: String(r.error) };
}
var Es = { ok: !1, reason: "not-in-era" };
function Ps(e) {
  return Lr(e) && Lr(e.outputSchema) && Vs(e.outputSchema);
}
function Ts(e) {
  return e;
}
var Jr = {
  era: "2025-11-25",
  hasRequestMethod: xs,
  hasNotificationMethod: Js,
  validateRequest: (e, t) => dr(ko(e), t),
  validateResult: (e, t) => dr(Co(e), t),
  validateNotification: (e, t) => dr(qo(e), t),
  hasInputRequestMethod: () => !1,
  validateInputRequest: () => Es,
  validateInputResponse: () => Es,
  samplingResultVariant: (e, t) => {
    let r = Ds();
    return dr(e ? r.CreateMessageResultWithToolsSchema : r.CreateMessageResultSchema, t);
  },
  outboundEnvelope: (e) => {
    return;
  },
  validateEnvelopeMeta: (e) => [],
  projectCallToolResult(e, t) {
    let r = Ls(e),
      s = r.structuredContent;
    if (s === void 0) return r;
    let a = typeof s !== "object" || s === null || Array.isArray(s),
      o = t !== void 0 && Vs(t);
    if (!a && !o) return r;
    return { ...r, structuredContent: { result: s } };
  },
  decodeResult(e, t) {
    if (Lr(t) && "resultType" in t) {
      let r = { ...t };
      return delete r.resultType, { kind: "complete", result: Ts(r) };
    }
    return { kind: "complete", result: Ts(t) };
  },
  encodeResult(e, t) {
    if (e !== "tools/list") return t;
    let r = t.tools;
    if (!Array.isArray(r) || !r.some((s) => Ps(s))) return t;
    return { ...t, tools: r.map((s) => (Ps(s) ? { ...s, outputSchema: Po(s.outputSchema) } : s)) };
  },
  encodeErrorCode: (e) => (e === -32002 ? -32602 : e),
  checkInboundEnvelope: (e) => {
    return;
  },
};
function Io() {
  let e = bat(() => Ki([pe(), Fr(), $i(), WHe(), hi(pe(), e), en(e)])),
    t = hi(pe(), e),
    r = Ki([pe(), Fr().int()]),
    s = pe(),
    a = Ki([pe(), Fr().int()]),
    o = lc(["user", "assistant"]),
    n = lc(["debug", "info", "notice", "warning", "error", "critical", "alert", "emergency"]),
    c = pe().refine(
      (ae) => {
        try {
          return atob(ae), !0;
        } catch {
          return !1;
        }
      },
      { message: "Invalid Base64 string" },
    ),
    i = lt({ ttl: Fr().optional() }),
    l = lt({ taskId: pe() }),
    d = Ia({ progressToken: r.optional(), "io.modelcontextprotocol/related-task": l.optional() }),
    S = lt({ _meta: d.optional() }),
    R = S.extend({ task: i.optional() }),
    w = lt({ _meta: d.optional() }),
    g = lt({ method: pe(), params: w.loose().optional() }),
    _ = lt({
      src: pe(),
      mimeType: pe().optional(),
      sizes: en(pe()).optional(),
      theme: lc(["light", "dark"]).optional(),
    }),
    h = lt({ icons: en(_).optional() }),
    u = lt({ name: pe(), title: pe().optional() }),
    p = u.extend({ ...u.shape, ...h.shape, version: pe(), websiteUrl: pe().optional(), description: pe().optional() }),
    m = Pae(lt({ applyDefaults: $i().optional() }), t),
    y = qHe(
      (ae) => {
        if (ae && typeof ae === "object" && !Array.isArray(ae) && Object.keys(ae).length === 0) return { form: {} };
        return ae;
      },
      Pae(lt({ form: m.optional(), url: t.optional() }), t.optional()),
    ),
    E = Ia({
      list: t.optional(),
      cancel: t.optional(),
      requests: Ia({
        sampling: Ia({ createMessage: t.optional() }).optional(),
        elicitation: Ia({ create: t.optional() }).optional(),
      }).optional(),
    }),
    v = Ia({
      list: t.optional(),
      cancel: t.optional(),
      requests: Ia({ tools: Ia({ call: t.optional() }).optional() }).optional(),
    }),
    f = lt({
      experimental: hi(pe(), t).optional(),
      sampling: lt({ context: t.optional(), tools: t.optional() }).optional(),
      elicitation: y.optional(),
      roots: lt({ listChanged: $i().optional() }).optional(),
      tasks: E.optional(),
      extensions: hi(pe(), t).optional(),
    }),
    P = lt({
      experimental: hi(pe(), t).optional(),
      logging: t.optional(),
      completions: t.optional(),
      prompts: lt({ listChanged: $i().optional() }).optional(),
      resources: lt({ subscribe: $i().optional(), listChanged: $i().optional() }).optional(),
      tools: lt({ listChanged: $i().optional() }).optional(),
      tasks: v.optional(),
      extensions: hi(pe(), t).optional(),
    }),
    I = lt({ progress: Fr(), total: Ws(Fr()), message: Ws(pe()) }),
    L = lt({ ...w.shape, ...I.shape, progressToken: r }),
    k = g.extend({ method: nn("notifications/progress"), params: L }),
    j = w.extend({ level: n, logger: pe().optional(), data: Ks() }),
    F = g.extend({ method: nn("notifications/message"), params: j }),
    V = lt({ uri: pe(), mimeType: Ws(pe()), _meta: hi(pe(), Ks()).optional() }),
    Y = V.extend({ text: pe() }),
    te = V.extend({ blob: c }),
    Q = lt({
      audience: en(o).optional(),
      priority: Fr().min(0).max(1).optional(),
      lastModified: jZ.datetime({ offset: !0 }).optional(),
    }),
    oe = lt({
      ...u.shape,
      ...h.shape,
      uri: pe(),
      description: Ws(pe()),
      mimeType: Ws(pe()),
      size: Ws(Fr()),
      annotations: Q.optional(),
      _meta: Ws(Ia({})),
    }),
    X = lt({
      ...u.shape,
      ...h.shape,
      uriTemplate: pe(),
      description: Ws(pe()),
      mimeType: Ws(pe()),
      annotations: Q.optional(),
      _meta: Ws(Ia({})),
    }),
    C = g.extend({ method: nn("notifications/resources/list_changed"), params: w.optional() }),
    O = w.extend({ uri: pe() }),
    A = g.extend({ method: nn("notifications/resources/updated"), params: O }),
    b = lt({ name: pe(), description: Ws(pe()), required: Ws($i()) }),
    z = lt({ ...u.shape, ...h.shape, description: Ws(pe()), arguments: Ws(en(b)), _meta: Ws(Ia({})) }),
    q = g.extend({ method: nn("notifications/prompts/list_changed"), params: w.optional() }),
    D = lt({ type: nn("text"), text: pe(), annotations: Q.optional(), _meta: hi(pe(), Ks()).optional() }),
    H = lt({ type: nn("image"), data: c, mimeType: pe(), annotations: Q.optional(), _meta: hi(pe(), Ks()).optional() }),
    W = lt({ type: nn("audio"), data: c, mimeType: pe(), annotations: Q.optional(), _meta: hi(pe(), Ks()).optional() }),
    J = lt({ type: nn("tool_use"), name: pe(), id: pe(), input: hi(pe(), Ks()), _meta: hi(pe(), Ks()).optional() }),
    ee = lt({
      type: nn("resource"),
      resource: Ki([Y, te]),
      annotations: Q.optional(),
      _meta: hi(pe(), Ks()).optional(),
    }),
    he = oe.extend({ type: nn("resource_link") }),
    T = Ki([D, H, W, he, ee]),
    N = lt({ role: o, content: T }),
    M = lt({
      title: pe().optional(),
      readOnlyHint: $i().optional(),
      destructiveHint: $i().optional(),
      idempotentHint: $i().optional(),
      openWorldHint: $i().optional(),
    }),
    x = g.extend({ method: nn("notifications/tools/list_changed"), params: w.optional() }),
    K = lt({ name: pe().optional() }),
    G = lt({
      hints: en(K).optional(),
      costPriority: Fr().min(0).max(1).optional(),
      speedPriority: Fr().min(0).max(1).optional(),
      intelligencePriority: Fr().min(0).max(1).optional(),
    }),
    se = lt({ mode: lc(["auto", "required", "none"]).optional() }),
    ie = lt({ type: nn("boolean"), title: pe().optional(), description: pe().optional(), default: $i().optional() }),
    le = lt({
      type: nn("string"),
      title: pe().optional(),
      description: pe().optional(),
      minLength: Fr().optional(),
      maxLength: Fr().optional(),
      format: lc(["email", "uri", "date", "date-time"]).optional(),
      default: pe().optional(),
    }),
    ne = lt({
      type: lc(["number", "integer"]),
      title: pe().optional(),
      description: pe().optional(),
      minimum: Fr().optional(),
      maximum: Fr().optional(),
      default: Fr().optional(),
    }),
    re = lt({
      type: nn("string"),
      title: pe().optional(),
      description: pe().optional(),
      enum: en(pe()),
      default: pe().optional(),
    }),
    ve = lt({
      type: nn("string"),
      title: pe().optional(),
      description: pe().optional(),
      oneOf: en(lt({ const: pe(), title: pe() })),
      default: pe().optional(),
    }),
    ce = lt({
      type: nn("string"),
      title: pe().optional(),
      description: pe().optional(),
      enum: en(pe()),
      enumNames: en(pe()).optional(),
      default: pe().optional(),
    }),
    Ee = Ki([re, ve]),
    be = lt({
      type: nn("array"),
      title: pe().optional(),
      description: pe().optional(),
      minItems: Fr().optional(),
      maxItems: Fr().optional(),
      items: lt({ type: nn("string"), enum: en(pe()) }),
      default: en(pe()).optional(),
    }),
    Pe = lt({
      type: nn("array"),
      title: pe().optional(),
      description: pe().optional(),
      minItems: Fr().optional(),
      maxItems: Fr().optional(),
      items: lt({ anyOf: en(lt({ const: pe(), title: pe() })) }),
      default: en(pe()).optional(),
    }),
    Ce = Ki([be, Pe]),
    ke = Ki([ce, Ee, Ce]),
    Ne = Ki([ke, ie, le, ne]),
    Me = R.extend({
      mode: nn("form").optional(),
      message: pe(),
      requestedSchema: lt({ type: nn("object"), properties: hi(pe(), Ne), required: en(pe()).optional() }).catchall(
        Ks(),
      ),
    }),
    je = lt({ type: nn("ref/resource"), uri: pe() }),
    Ae = lt({ type: nn("ref/prompt"), name: pe() }),
    it = lt({ uri: pe().startsWith("file://"), name: pe().optional(), _meta: hi(pe(), Ks()).optional() }),
    qe = f.shape,
    ct = lt({
      experimental: qe.experimental,
      sampling: qe.sampling,
      elicitation: qe.elicitation,
      roots: qe.roots,
      extensions: qe.extensions,
    }),
    ye = P.shape,
    We = lt({
      experimental: ye.experimental,
      logging: ye.logging,
      completions: ye.completions,
      prompts: ye.prompts,
      resources: ye.resources,
      tools: ye.tools,
      extensions: ye.extensions,
    }),
    Ye = Ia({ progressToken: r.optional(), [Dae]: pe(), [WWe]: p.optional(), [zHe]: ct, [qWe]: n.optional() }),
    Le = lt({
      ...u.shape,
      ...h.shape,
      description: pe().optional(),
      inputSchema: Ia({ $schema: pe().optional(), type: nn("object") }),
      outputSchema: Ia({ $schema: pe().optional() }).optional(),
      annotations: M.optional(),
      _meta: hi(pe(), Ks()).optional(),
    }),
    ut = lt({
      type: nn("tool_result"),
      toolUseId: pe(),
      content: en(T),
      structuredContent: Ks().optional(),
      isError: $i().optional(),
      _meta: hi(pe(), Ks()).optional(),
    }),
    Xe = Ki([D, H, W, J, ut]),
    Ue = lt({ role: o, content: Ki([Xe, en(Xe)]), _meta: hi(pe(), Ks()).optional() }),
    Ze = pe(),
    Qe = Ia({ [Oae]: p.optional().catch(void 0) }),
    et = Qe.optional();
  function ue(ae) {
    return Ia({ _meta: et, resultType: Ze.default("complete"), ...ae });
  }
  let dt = ue({}),
    Kt = ue({ nextCursor: s.optional() }),
    tt = ue({ content: en(T), structuredContent: Ks().optional(), isError: $i().optional() }),
    ht = ue({
      ttlMs: Fr().int().min(0),
      cacheScope: lc(["public", "private"]),
      tools: en(Le),
      nextCursor: s.optional(),
    }),
    mt = ue({
      ttlMs: Fr().int().min(0),
      cacheScope: lc(["public", "private"]),
      prompts: en(z),
      nextCursor: s.optional(),
    }),
    ft = ue({ description: pe().optional(), messages: en(N) }),
    pt = ue({
      ttlMs: Fr().int().min(0),
      cacheScope: lc(["public", "private"]),
      resources: en(oe),
      nextCursor: s.optional(),
    }),
    St = ue({
      ttlMs: Fr().int().min(0),
      cacheScope: lc(["public", "private"]),
      resourceTemplates: en(X),
      nextCursor: s.optional(),
    }),
    gt = ue({ ttlMs: Fr().int().min(0), cacheScope: lc(["public", "private"]), contents: en(Ki([Y, te])) }),
    _t = ue({
      completion: lt({ values: en(pe()).max(100), total: Fr().int().optional(), hasMore: $i().optional() }).loose(),
    }),
    Jt = ue({ ttlMs: Fr().int().min(0), cacheScope: lc(["public", "private"]) }),
    vt = ue({
      ttlMs: Fr().int().min(0).catch(0),
      cacheScope: lc(["public", "private"]).catch("private"),
      supportedVersions: en(pe()),
      capabilities: We,
      instructions: pe().optional(),
    }),
    Ie = lt({
      messages: en(Ue),
      modelPreferences: G.optional(),
      systemPrompt: pe().optional(),
      includeContext: lc(["none", "thisServer", "allServers"]).optional(),
      temperature: Fr().optional(),
      maxTokens: Fr().int(),
      stopSequences: en(pe()).optional(),
      metadata: t.optional(),
      tools: en(Le).optional(),
      toolChoice: se.optional(),
    }),
    yt = lt({ method: nn("sampling/createMessage"), params: Ie }),
    bt = lt({ method: nn("roots/list"), params: lt({ _meta: hi(pe(), Ks()).optional() }).optional() }),
    Rt = lt({ ...Ue.shape, model: pe(), stopReason: pe().optional() }),
    wt = lt({ roots: en(it) }),
    $t = lt({
      action: lc(["accept", "decline", "cancel"]),
      content: hi(pe(), Ki([pe(), Fr(), $i(), en(pe())])).optional(),
    }),
    Et = lt({ mode: nn("url"), message: pe(), url: pe().url() }),
    Pt = Ki([Me, Et]),
    Tt = lt({ method: nn("elicitation/create"), params: Pt }),
    zt = Ki([yt, bt, Tt]),
    Ct = Ki([Rt, wt, $t]),
    kt = hi(pe(), zt),
    qt = hi(pe(), Ct),
    De = ue({ inputRequests: kt.optional(), requestState: pe().optional() }),
    Ve = { inputResponses: qt.optional(), requestState: pe().optional() },
    Gt = lt({ _meta: Ye, ...Ve }),
    Bt = Ia({ progressToken: r.optional() });
  function Se(ae, He) {
    return lt({ method: nn(ae), params: lt({ _meta: Ye, ...He }) });
  }
  function ge(ae, He) {
    return lt({ method: nn(ae), params: lt({ _meta: Bt.optional(), ...He }).optional() });
  }
  let It = { name: pe(), arguments: hi(pe(), Ks()).optional(), ...Ve },
    Re = { cursor: s.optional() },
    Wt = Se("tools/call", It),
    Yt = Se("tools/list", Re),
    Xt = Se("prompts/list", Re),
    Zt = Se("prompts/get", { name: pe(), arguments: hi(pe(), pe()).optional(), ...Ve }),
    Qt = Se("resources/list", Re),
    er = Se("resources/templates/list", Re),
    tr = Se("resources/read", { uri: pe(), ...Ve }),
    Ot = {
      ref: Ki([Ae, je]),
      argument: lt({ name: pe(), value: pe() }),
      context: lt({ arguments: hi(pe(), pe()).optional() }).optional(),
    },
    rr = Se("completion/complete", Ot),
    sr = Se("server/discover", {}),
    rt = lt({
      toolsListChanged: $i().optional(),
      promptsListChanged: $i().optional(),
      resourcesListChanged: $i().optional(),
      resourceSubscriptions: en(pe()).optional(),
    }),
    Nt = { notifications: rt },
    ar = Se("subscriptions/listen", Nt),
    or = Qe.extend({ "io.modelcontextprotocol/subscriptionId": a }),
    nr = Ia({ _meta: or, resultType: Ze.default("complete") }),
    Oe = {
      "tools/call": ge("tools/call", It),
      "tools/list": ge("tools/list", Re),
      "prompts/get": ge("prompts/get", { name: pe(), arguments: hi(pe(), pe()).optional() }),
      "prompts/list": ge("prompts/list", Re),
      "resources/list": ge("resources/list", Re),
      "resources/templates/list": ge("resources/templates/list", Re),
      "resources/read": ge("resources/read", { uri: pe() }),
      "completion/complete": ge("completion/complete", Ot),
      "server/discover": ge("server/discover", {}),
      "subscriptions/listen": ge("subscriptions/listen", Nt),
    };
  function me(ae) {
    return Ia({ _meta: et, ...ae });
  }
  let ir = {
      "tools/call": me({ content: en(T), structuredContent: Ks().optional(), isError: $i().optional() }),
      "tools/list": me({
        ttlMs: Fr().int().min(0),
        cacheScope: lc(["public", "private"]),
        tools: en(Le),
        nextCursor: s.optional(),
      }),
      "prompts/get": me({ description: pe().optional(), messages: en(N) }),
      "prompts/list": me({
        ttlMs: Fr().int().min(0),
        cacheScope: lc(["public", "private"]),
        prompts: en(z),
        nextCursor: s.optional(),
      }),
      "resources/list": me({
        ttlMs: Fr().int().min(0),
        cacheScope: lc(["public", "private"]),
        resources: en(oe),
        nextCursor: s.optional(),
      }),
      "resources/templates/list": me({
        ttlMs: Fr().int().min(0),
        cacheScope: lc(["public", "private"]),
        resourceTemplates: en(X),
        nextCursor: s.optional(),
      }),
      "resources/read": me({
        ttlMs: Fr().int().min(0),
        cacheScope: lc(["public", "private"]),
        contents: en(Ki([Y, te])),
      }),
      "completion/complete": me({
        completion: lt({ values: en(pe()).max(100), total: Fr().int().optional(), hasMore: $i().optional() }).loose(),
      }),
      "server/discover": me({
        ttlMs: Fr().int().min(0).catch(0),
        cacheScope: lc(["public", "private"]).catch("private"),
        supportedVersions: en(pe()),
        capabilities: We,
        instructions: pe().optional(),
      }),
      "subscriptions/listen": me({}),
    },
    Fe = Ia({ "io.modelcontextprotocol/subscriptionId": a.optional() }),
    st = lt({
      method: nn("notifications/subscriptions/acknowledged"),
      params: lt({ _meta: Fe.optional(), notifications: rt }),
    }),
    at = lt({ _meta: Fe.optional(), requestId: a, reason: pe().optional() }),
    ot = lt({ method: nn("notifications/cancelled"), params: at }),
    Ir = {
      "notifications/cancelled": ot,
      "notifications/progress": k,
      "notifications/message": F,
      "notifications/resources/updated": A,
      "notifications/resources/list_changed": C,
      "notifications/tools/list_changed": x,
      "notifications/prompts/list_changed": q,
      "notifications/subscriptions/acknowledged": st,
    },
    fe = (ae) => lt({ jsonrpc: nn("2.0"), id: Ki([pe(), Fr().int()]), result: ae }).strict();
  return {
    JSONValueSchema: e,
    JSONObjectSchema: t,
    ProgressTokenSchema: r,
    CursorSchema: s,
    RequestIdSchema: a,
    RoleSchema: o,
    LoggingLevelSchema: n,
    TaskMetadataSchema: i,
    RelatedTaskMetadataSchema: l,
    RequestMetaSchema: d,
    BaseRequestParamsSchema: S,
    TaskAugmentedRequestParamsSchema: R,
    NotificationsParamsSchema: w,
    NotificationSchema: g,
    IconSchema: _,
    IconsSchema: h,
    BaseMetadataSchema: u,
    ImplementationSchema: p,
    ClientTasksCapabilitySchema: E,
    ServerTasksCapabilitySchema: v,
    ClientCapabilitiesSchema: f,
    ServerCapabilitiesSchema: P,
    ProgressSchema: I,
    ProgressNotificationParamsSchema: L,
    ProgressNotificationSchema: k,
    LoggingMessageNotificationParamsSchema: j,
    LoggingMessageNotificationSchema: F,
    ResourceContentsSchema: V,
    TextResourceContentsSchema: Y,
    BlobResourceContentsSchema: te,
    AnnotationsSchema: Q,
    ResourceSchema: oe,
    ResourceTemplateSchema: X,
    ResourceListChangedNotificationSchema: C,
    ResourceUpdatedNotificationParamsSchema: O,
    ResourceUpdatedNotificationSchema: A,
    PromptArgumentSchema: b,
    PromptSchema: z,
    PromptListChangedNotificationSchema: q,
    TextContentSchema: D,
    ImageContentSchema: H,
    AudioContentSchema: W,
    ToolUseContentSchema: J,
    EmbeddedResourceSchema: ee,
    ResourceLinkSchema: he,
    ContentBlockSchema: T,
    PromptMessageSchema: N,
    ToolAnnotationsSchema: M,
    ToolListChangedNotificationSchema: x,
    ModelHintSchema: K,
    ModelPreferencesSchema: G,
    ToolChoiceSchema: se,
    BooleanSchemaSchema: ie,
    StringSchemaSchema: le,
    NumberSchemaSchema: ne,
    UntitledSingleSelectEnumSchemaSchema: re,
    TitledSingleSelectEnumSchemaSchema: ve,
    LegacyTitledEnumSchemaSchema: ce,
    SingleSelectEnumSchemaSchema: Ee,
    UntitledMultiSelectEnumSchemaSchema: be,
    TitledMultiSelectEnumSchemaSchema: Pe,
    MultiSelectEnumSchemaSchema: Ce,
    EnumSchemaSchema: ke,
    PrimitiveSchemaDefinitionSchema: Ne,
    ElicitRequestFormParamsSchema: Me,
    ResourceTemplateReferenceSchema: je,
    PromptReferenceSchema: Ae,
    RootSchema: it,
    ClientCapabilities2026Schema: ct,
    ServerCapabilities2026Schema: We,
    RequestMetaEnvelopeSchema: Ye,
    ToolSchema: Le,
    ToolResultContentSchema: ut,
    SamplingMessageContentBlockSchema: Xe,
    SamplingMessageSchema: Ue,
    ResultTypeSchema: Ze,
    ResultMetaSchema: Qe,
    ResultSchema: dt,
    PaginatedResultSchema: Kt,
    CallToolResultSchema: tt,
    ListToolsResultSchema: ht,
    ListPromptsResultSchema: mt,
    GetPromptResultSchema: ft,
    ListResourcesResultSchema: pt,
    ListResourceTemplatesResultSchema: St,
    ReadResourceResultSchema: gt,
    CompleteResultSchema: _t,
    CacheableResultSchema: Jt,
    DiscoverResultSchema: vt,
    CreateMessageRequestParamsSchema: Ie,
    CreateMessageRequestSchema: yt,
    ListRootsRequestSchema: bt,
    CreateMessageResultSchema: Rt,
    ListRootsResultSchema: wt,
    ElicitResultSchema: $t,
    ElicitRequestURLParamsSchema: Et,
    ElicitRequestParamsSchema: Pt,
    ElicitRequestSchema: Tt,
    InputRequestSchema: zt,
    InputResponseSchema: Ct,
    InputRequestsSchema: kt,
    InputResponsesSchema: qt,
    InputRequiredResultSchema: De,
    InputResponseRequestParamsSchema: Gt,
    CallToolRequestSchema: Wt,
    ListToolsRequestSchema: Yt,
    ListPromptsRequestSchema: Xt,
    GetPromptRequestSchema: Zt,
    ListResourcesRequestSchema: Qt,
    ListResourceTemplatesRequestSchema: er,
    ReadResourceRequestSchema: tr,
    CompleteRequestSchema: rr,
    DiscoverRequestSchema: sr,
    SubscriptionFilterSchema: rt,
    SubscriptionsListenRequestSchema: ar,
    SubscriptionsListenResultMetaSchema: or,
    SubscriptionsListenResultSchema: nr,
    dispatchRequestSchemas: Oe,
    dispatchResultSchemas: ir,
    NotificationMetaSchema: Fe,
    SubscriptionsAcknowledgedNotificationSchema: st,
    CancelledNotificationParamsSchema: at,
    CancelledNotificationSchema: ot,
    notificationSchemas2026: Ir,
    JSONRPCResultResponseSchema: fe(dt),
    CallToolResultResponseSchema: fe(Ki([tt, De])),
    ListToolsResultResponseSchema: fe(ht),
    ListPromptsResultResponseSchema: fe(mt),
    GetPromptResultResponseSchema: fe(Ki([ft, De])),
    ListResourcesResultResponseSchema: fe(pt),
    ListResourceTemplatesResultResponseSchema: fe(St),
    ReadResourceResultResponseSchema: fe(Ki([gt, De])),
    CompleteResultResponseSchema: fe(_t),
    DiscoverResultResponseSchema: fe(vt),
  };
}
var Oo;
function xe() {
  return (Oo ??= Io());
}
var No = [
  "tools/list",
  "prompts/list",
  "resources/list",
  "resources/templates/list",
  "resources/read",
  "server/discover",
];
function Mo(e) {
  return No.includes(e);
}
var Gr = Symbol("modelcontextprotocol.resultCacheHintFallback");
function jo(e) {
  return e[Gr];
}
function Gs(e) {
  return typeof e === "number" && Number.isSafeInteger(e) && e >= 0;
}
function Bs(e) {
  return e === "public" || e === "private";
}
var Ai = (function (e) {
    return (
      (e[(e.ParseError = -32700)] = "ParseError"),
      (e[(e.InvalidRequest = -32600)] = "InvalidRequest"),
      (e[(e.MethodNotFound = -32601)] = "MethodNotFound"),
      (e[(e.InvalidParams = -32602)] = "InvalidParams"),
      (e[(e.InternalError = -32603)] = "InternalError"),
      (e[(e.ResourceNotFound = -32002)] = "ResourceNotFound"),
      (e[(e.MissingRequiredClientCapability = -32021)] = "MissingRequiredClientCapability"),
      (e[(e.UnsupportedProtocolVersion = -32022)] = "UnsupportedProtocolVersion"),
      (e[(e.UrlElicitationRequired = -32042)] = "UrlElicitationRequired"),
      e
    );
  })({}),
  wa = class e extends Error {
    static {
      Object.defineProperty(this, "mcpBrand", { value: "mcp.ProtocolError" });
    }
    static [Symbol.hasInstance](t) {
      return _e(this, t);
    }
    static isInstance(t) {
      if (typeof this !== "function")
        throw TypeError(
          "isInstance must be called on the class (e.g. `SdkError.isInstance(value)`); for callbacks use `v => SdkError.isInstance(v)`",
        );
      return _e(this, t);
    }
    constructor(t, r, s) {
      super(r);
      (this.code = t), (this.data = s), (this.name = "ProtocolError"), Je(this, new.target);
    }
    static fromError(t, r, s) {
      if (t === Ai.UrlElicitationRequired && s) {
        let a = s;
        if (a.elicitations) return new Xs(a.elicitations, r);
      }
      if (t === Ai.UnsupportedProtocolVersion && s) {
        let a = s;
        if (Array.isArray(a.supported) && typeof a.requested === "string")
          return new Sr({ supported: a.supported, requested: a.requested }, r);
      }
      if (t === Ai.InvalidParams || t === Ai.ResourceNotFound) {
        let a = s;
        if (typeof a?.uri === "string" && (t === Ai.ResourceNotFound || Object.keys(a).length === 1))
          return new Ys(a.uri, r);
      }
      if (t === Ai.MissingRequiredClientCapability && s) {
        let a = s;
        if (
          a.requiredCapabilities !== null &&
          typeof a.requiredCapabilities === "object" &&
          !Array.isArray(a.requiredCapabilities)
        )
          return new Zs({ requiredCapabilities: a.requiredCapabilities }, r);
      }
      return new e(t, r, s);
    }
  },
  Ys = class extends wa {
    static {
      Object.defineProperty(this, "mcpBrand", { value: "mcp.ResourceNotFoundError" });
    }
    constructor(e, t = `Resource not found: ${e}`) {
      super(Ai.InvalidParams, t, { uri: e });
    }
    get uri() {
      return this.data.uri;
    }
  },
  Xs = class extends wa {
    static {
      Object.defineProperty(this, "mcpBrand", { value: "mcp.UrlElicitationRequiredError" });
    }
    constructor(e, t = `URL elicitation${e.length > 1 ? "s" : ""} required`) {
      super(Ai.UrlElicitationRequired, t, { elicitations: e });
    }
    get elicitations() {
      return this.data?.elicitations ?? [];
    }
  },
  Sr = class extends wa {
    static {
      Object.defineProperty(this, "mcpBrand", { value: "mcp.UnsupportedProtocolVersionError" });
    }
    constructor(e, t = `Unsupported protocol version: ${e.requested}`) {
      super(Ai.UnsupportedProtocolVersion, t, e);
    }
    get supported() {
      return this.data.supported;
    }
    get requested() {
      return this.data.requested;
    }
  },
  Zs = class extends wa {
    static {
      Object.defineProperty(this, "mcpBrand", { value: "mcp.MissingRequiredClientCapabilityError" });
    }
    constructor(e, t = `Missing required client capabilities: ${Object.keys(e.requiredCapabilities).join(", ")}`) {
      super(Ai.MissingRequiredClientCapability, t, e);
    }
    get requiredCapabilities() {
      return this.data.requiredCapabilities;
    }
  },
  Ao = 0,
  Lo = "private",
  Do = ["tools/call", "prompts/get", "resources/read"];
function Vo(e, t) {
  let r = t.resultType;
  if (r === void 0) return { ...t, resultType: "complete" };
  if (r === "complete") return t;
  if (Do.includes(e)) return t;
  throw new wa(
    Ai.InternalError,
    `Handler for ${e} returned resultType '${String(r)}', but results of ${e} only support 'complete' on protocol revision 2026-07-28`,
  );
}
function Fo(e, t) {
  let r = jo(t);
  if (t.resultType !== "complete" || !Mo(e)) return r === void 0 ? t : Go(t);
  let s = t,
    a = Gs(s.ttlMs) ? s.ttlMs : Ko(r),
    o = Bs(s.cacheScope) ? s.cacheScope : Jo(r),
    n = { ...s, ttlMs: a, cacheScope: o };
  return delete n[Gr], n;
}
function Ho(e) {
  return e !== null && typeof e === "object" && !Array.isArray(e);
}
function xo(e, t) {
  if (t === void 0) return e;
  let r = e._meta;
  if (r === void 0) return { ...e, _meta: { [Oae]: t } };
  if (!Ho(r)) return e;
  if (r[Oae] !== void 0) return e;
  return { ...e, _meta: { ...r, [Oae]: t } };
}
function Ko(e) {
  return e !== void 0 && Gs(e.ttlMs) ? e.ttlMs : Ao;
}
function Jo(e) {
  return e !== void 0 && Bs(e.cacheScope) ? e.cacheScope : Lo;
}
function Go(e) {
  let t = { ...e };
  return delete t[Gr], t;
}
var Bo = ["elicitation/create", "sampling/createMessage", "roots/list"],
  hr;
function Qs() {
  if (hr) return hr;
  let e = xe();
  return (
    (hr = {
      request: {
        "elicitation/create": lt({ method: nn("elicitation/create"), params: e.ElicitRequestParamsSchema }),
        "sampling/createMessage": lt({
          method: nn("sampling/createMessage"),
          params: e.CreateMessageRequestParamsSchema,
        }),
        "roots/list": lt({ method: nn("roots/list"), params: Ia({}).optional() }),
      },
      response: {
        "elicitation/create": e.ElicitResultSchema,
        "sampling/createMessage": e.CreateMessageResultSchema,
        "roots/list": e.ListRootsResultSchema,
      },
    }),
    hr
  );
}
function ea(e) {
  return Bo.includes(e);
}
function Or(e) {
  return ea(e) ? Qs().request[e] : void 0;
}
function Wo(e) {
  return ea(e) ? Qs().response[e] : void 0;
}
var Br = {
    "tools/call": null,
    "tools/list": null,
    "prompts/get": null,
    "prompts/list": null,
    "resources/list": null,
    "resources/templates/list": null,
    "resources/read": null,
    "completion/complete": null,
    "server/discover": null,
    "subscriptions/listen": null,
  },
  ta = {
    "notifications/cancelled": null,
    "notifications/progress": null,
    "notifications/message": null,
    "notifications/resources/updated": null,
    "notifications/resources/list_changed": null,
    "notifications/tools/list_changed": null,
    "notifications/prompts/list_changed": null,
    "notifications/subscriptions/acknowledged": null,
  };
function ra(e) {
  return Object.prototype.hasOwnProperty.call(Br, e);
}
function sa(e) {
  return Object.prototype.hasOwnProperty.call(ta, e);
}
function Yo(e) {
  return Object.prototype.hasOwnProperty.call(Br, e);
}
function Xo(e) {
  return ra(e) ? xe().dispatchRequestSchemas[e] : void 0;
}
function Zo(e) {
  return Yo(e) ? xe().dispatchResultSchemas[e] : void 0;
}
function Qo(e) {
  return sa(e) ? xe().notificationSchemas2026[e] : void 0;
}
var zl = Object.keys(Br),
  Cl = Object.keys(ta);
function At(e) {
  return e !== null && typeof e === "object" && !Array.isArray(e);
}
function Mt(e, t) {
  if (e === void 0) return { ok: !1, reason: "not-in-era" };
  let r = e.safeParse(t);
  return r.success ? { ok: !0, value: r.data } : { ok: !1, reason: "invalid", message: String(r.error) };
}
var tn = { ok: !1, reason: "not-in-era" },
  rn = [Dae, zHe];
function sn(e, t) {
  let r = t,
    s = !1,
    a = () => {
      if (!s) (r = { ...r }), (s = !0);
      return r;
    },
    o = t.tools;
  if (e === "tools/list" && Array.isArray(o) && o.some((c) => At(c) && "execution" in c))
    a().tools = o.map((c) => {
      if (!At(c) || !("execution" in c)) return c;
      let i = { ...c };
      return delete i.execution, i;
    });
  let n = t.capabilities;
  if (At(n) && "tasks" in n) {
    let c = { ...n };
    delete c.tasks, (a().capabilities = c);
  }
  return r;
}
var an = "io.modelcontextprotocol/tasks",
  on = new Set(["working", "input_required", "completed", "failed", "cancelled"]);
function cn(e) {
  if (e === null || typeof e !== "object" || Array.isArray(e)) return !1;
  return (
    typeof e.taskId === "string" &&
    e.taskId.length > 0 &&
    on.has(e.status) &&
    typeof e.createdAt === "string" &&
    typeof e.lastUpdatedAt === "string" &&
    (e.ttlMs === null || typeof e.ttlMs === "number") &&
    (e.pollIntervalMs === void 0 || typeof e.pollIntervalMs === "number")
  );
}
var Wr = {
    era: "2026-07-28",
    hasRequestMethod: ra,
    hasNotificationMethod: sa,
    hasInputRequestMethod: (e) => Or(e) !== void 0,
    validateRequest: (e, t) => Mt(Xo(e), t),
    validateResult: (e, t) => Mt(Zo(e), t),
    validateNotification: (e, t) => Mt(Qo(e), t),
    validateInputRequest: (e, t) => Mt(Or(e), t),
    validateInputResponse: (e, t) => Mt(Wo(e), t),
    samplingResultVariant: () => tn,
    outboundEnvelope(e) {
      return {
        [Dae]: e.protocolVersion,
        [WWe]: e.clientInfo,
        [zHe]: e.clientCapabilities,
        ...(e.logLevel !== void 0 && { [qWe]: e.logLevel }),
      };
    },
    validateEnvelopeMeta(e) {
      let t = [];
      for (let s of rn) if (!(s in e)) t.push({ key: s, problem: "missing" });
      let r = xe().RequestMetaEnvelopeSchema.safeParse(e);
      if (!r.success)
        for (let s of r.error.issues) {
          let a = s.path.map(String),
            o = a.length > 0 ? a.join(".") : "_meta";
          if (a.length === 1 && t.some((n) => n.key === o && n.problem === "missing")) continue;
          t.push({ key: o, problem: s.message });
        }
      return t;
    },
    projectCallToolResult: (e) => Ls(e),
    inputRequestSchema: Or,
    decodeResult(e, t) {
      if (!At(t))
        return {
          kind: "invalid",
          error: new Uo(vo.InvalidResult, `Invalid result for ${e}: not an object`, { method: e }),
        };
      let r = t.resultType;
      if (r === void 0)
        return {
          kind: "invalid",
          error: new Uo(
            vo.InvalidResult,
            `Invalid result for ${e}: missing required resultType \u2014 servers implementing protocol revision 2026-07-28 MUST include it (the absent-means-complete bridge applies only to earlier-revision servers)`,
            { method: e, violation: "missing-resultType" },
          ),
        };
      if (typeof r !== "string")
        return {
          kind: "invalid",
          error: new Uo(vo.InvalidResult, `Invalid result for ${e}: non-string resultType`, {
            method: e,
            resultType: r,
          }),
        };
      if (r === "input_required") {
        let n = t.inputRequests,
          c = At(n) ? n : {},
          i = t.requestState;
        if (Object.keys(c).length === 0 && typeof i !== "string")
          return {
            kind: "invalid",
            error: new Uo(
              vo.InvalidResult,
              `Invalid result for ${e}: input_required carries neither inputRequests nor requestState (every input_required result must include at least one of the two)`,
              { method: e, violation: "input-required-missing-both" },
            ),
          };
        return { kind: "input_required", inputRequests: c, ...(typeof i === "string" && { requestState: i }) };
      }
      if (r === "task") return { kind: "task", result: t };
      if (r !== "complete")
        return {
          kind: "invalid",
          error: new Uo(vo.UnsupportedResultType, `Unsupported result type '${r}' for ${e}`, {
            resultType: r,
            method: e,
          }),
        };
      let s = ln(),
        a = Object.hasOwn(s, e) ? s[e] : void 0;
      if (a !== void 0) {
        let n = a.safeParse(t);
        if (!n.success)
          return {
            kind: "invalid",
            error: new Uo(vo.InvalidResult, `Invalid result for ${e}: ${n.error}`, { method: e }),
          };
      }
      let o = { ...t };
      return delete o.resultType, { kind: "complete", result: o };
    },
    encodeResult(e, t, r) {
      return xo(Fo(e, Vo(e, sn(e, t))), r);
    },
    encodeErrorCode: (e) => (e === -32002 ? -32602 : e),
    checkInboundEnvelope(e) {
      if (e.envelope === void 0)
        return "Request is missing the required _meta envelope for protocol revision 2026-07-28 (io.modelcontextprotocol/protocolVersion, io.modelcontextprotocol/clientCapabilities)";
      let t = xe().RequestMetaEnvelopeSchema.safeParse(e.envelope);
      if (!t.success)
        return `Invalid _meta envelope for protocol revision 2026-07-28: ${t.error.issues.map((r) => r.message).join("; ")}`;
    },
  },
  mr;
function ln() {
  if (mr) return mr;
  let e = xe();
  return (
    (mr = {
      "tools/call": e.CallToolResultSchema,
      "tools/list": e.ListToolsResultSchema,
      "prompts/get": e.GetPromptResultSchema,
      "prompts/list": e.ListPromptsResultSchema,
      "resources/list": e.ListResourcesResultSchema,
      "resources/templates/list": e.ListResourceTemplatesResultSchema,
      "resources/read": e.ReadResourceResultSchema,
      "completion/complete": e.CompleteResultSchema,
      "server/discover": e.DiscoverResultSchema,
    }),
    mr
  );
}
var gr = "2026-07-28";
function Te(e) {
  return e !== void 0 && ze(e) ? Wr : Jr;
}
function zs(e) {
  if (e.revision !== void 0) return Te(e.revision).era;
  return e.era === "modern" ? Wr.era : Jr.era;
}
function Nr(e) {
  return aa.some((t) => t.hasRequestMethod(e));
}
function Mr(e) {
  return aa.some((t) => t.hasNotificationMethod(e));
}
var aa = [Jr, Wr],
  un = ms({
    AnnotationsSchema: () => Nae,
    AudioContentSchema: () => YWe,
    BaseMetadataSchema: () => Lae,
    BaseRequestParamsSchema: () => xx,
    BlobResourceContentsSchema: () => Mat,
    BooleanSchemaSchema: () => Uat,
    CallToolRequestParamsSchema: () => tLt,
    CallToolRequestSchema: () => nLt,
    CallToolResultSchema: () => Fae,
    CancelTaskRequestSchema: () => son,
    CancelTaskResultSchema: () => I_e,
    CancelledNotificationParamsSchema: () => mOt,
    CancelledNotificationSchema: () => Hat,
    ClientCapabilitiesSchema: () => yOt,
    ClientNotificationSchema: () => lon,
    ClientRequestSchema: () => aon,
    ClientResultSchema: () => con,
    ClientTasksCapabilitySchema: () => hOt,
    CompatibilityCallToolResultSchema: () => QWe,
    CompleteRequestParamsSchema: () => RLt,
    CompleteRequestSchema: () => kLt,
    CompleteResultSchema: () => HLt,
    ContentBlockSchema: () => JWe,
    CreateMessageRequestParamsSchema: () => mLt,
    CreateMessageRequestSchema: () => gLt,
    CreateMessageResultSchema: () => hLt,
    CreateMessageResultWithToolsSchema: () => _Lt,
    CreateTaskResultSchema: () => Zrn,
    CursorSchema: () => Eat,
    DiscoverRequestSchema: () => bOt,
    DiscoverResultSchema: () => YHe,
    ElicitRequestFormParamsSchema: () => t9e,
    ElicitRequestParamsSchema: () => rxe,
    ElicitRequestSchema: () => wLt,
    ElicitRequestURLParamsSchema: () => nxe,
    ElicitResultSchema: () => ALt,
    ElicitationCompleteNotificationParamsSchema: () => TLt,
    ElicitationCompleteNotificationSchema: () => ELt,
    EmbeddedResourceSchema: () => VOt,
    EmptyResultSchema: () => kat,
    EnumSchemaSchema: () => bLt,
    GetPromptRequestParamsSchema: () => qOt,
    GetPromptRequestSchema: () => GOt,
    GetPromptResultSchema: () => YOt,
    GetTaskPayloadRequestSchema: () => non,
    GetTaskPayloadResultSchema: () => ron,
    GetTaskRequestSchema: () => ton,
    GetTaskResultSchema: () => n9e,
    IconSchema: () => gOt,
    IconsSchema: () => T_e,
    ImageContentSchema: () => XWe,
    ImplementationSchema: () => Mae,
    InitializeRequestParamsSchema: () => SOt,
    InitializeRequestSchema: () => xat,
    InitializeResultSchema: () => XHe,
    InitializedNotificationSchema: () => Iat,
    JSONArraySchema: () => Yrn,
    JSONObjectSchema: () => IA,
    JSONRPCErrorResponseSchema: () => zWe,
    JSONRPCMessageSchema: () => TB,
    JSONRPCNotificationSchema: () => Rat,
    JSONRPCRequestSchema: () => vat,
    JSONRPCResponseSchema: () => Jrn,
    JSONRPCResultResponseSchema: () => GWe,
    JSONValueSchema: () => b_e,
    LegacyTitledEnumSchemaSchema: () => Wat,
    ListChangedOptionsBaseSchema: () => Fat,
    ListPromptsRequestSchema: () => WOt,
    ListPromptsResultSchema: () => exe,
    ListResourceTemplatesRequestSchema: () => COt,
    ListResourceTemplatesResultSchema: () => QHe,
    ListResourcesRequestSchema: () => AOt,
    ListResourcesResultSchema: () => WZ,
    ListRootsRequestSchema: () => ILt,
    ListRootsResultSchema: () => PLt,
    ListTasksRequestSchema: () => oon,
    ListTasksResultSchema: () => ion,
    ListToolsRequestSchema: () => eLt,
    ListToolsResultSchema: () => H_e,
    LoggingLevelSchema: () => $at,
    LoggingMessageNotificationParamsSchema: () => sLt,
    LoggingMessageNotificationSchema: () => aLt,
    ModelHintSchema: () => lLt,
    ModelPreferencesSchema: () => cLt,
    MultiSelectEnumSchemaSchema: () => SLt,
    NotificationSchema: () => RL,
    NotificationsParamsSchema: () => vL,
    NumberSchemaSchema: () => e9e,
    PaginatedRequestParamsSchema: () => EOt,
    PaginatedRequestSchema: () => A_e,
    PaginatedResultSchema: () => C_e,
    PingRequestSchema: () => Pat,
    PrimitiveSchemaDefinitionSchema: () => zat,
    ProgressNotificationParamsSchema: () => TOt,
    ProgressNotificationSchema: () => Dat,
    ProgressSchema: () => wOt,
    ProgressTokenSchema: () => Tat,
    PromptArgumentSchema: () => jOt,
    PromptListChangedNotificationSchema: () => JOt,
    PromptMessageSchema: () => XOt,
    PromptReferenceSchema: () => vLt,
    PromptSchema: () => ZHe,
    ReadResourceRequestParamsSchema: () => vOt,
    ReadResourceRequestSchema: () => ROt,
    ReadResourceResultSchema: () => kOt,
    RelatedTaskMetadataSchema: () => VHe,
    RequestIdSchema: () => w_e,
    RequestMetaSchema: () => Aat,
    RequestSchema: () => PA,
    ResourceContentsSchema: () => Oat,
    ResourceLinkSchema: () => KOt,
    ResourceListChangedNotificationSchema: () => HOt,
    ResourceRequestParamsSchema: () => VWe,
    ResourceSchema: () => R_e,
    ResourceTemplateReferenceSchema: () => CLt,
    ResourceTemplateSchema: () => JHe,
    ResourceUpdatedNotificationParamsSchema: () => UOt,
    ResourceUpdatedNotificationSchema: () => BOt,
    ResultMetaObjectSchema: () => Cat,
    ResultSchema: () => kS,
    RoleSchema: () => v_e,
    RootSchema: () => xLt,
    RootsListChangedNotificationSchema: () => DLt,
    SamplingContentSchema: () => pLt,
    SamplingMessageContentBlockSchema: () => txe,
    SamplingMessageSchema: () => fLt,
    ServerCapabilitiesSchema: () => E_e,
    ServerNotificationSchema: () => don,
    ServerRequestSchema: () => uon,
    ServerResultSchema: () => pon,
    ServerTasksCapabilitySchema: () => _Ot,
    SetLevelRequestParamsSchema: () => oLt,
    SetLevelRequestSchema: () => iLt,
    SingleSelectEnumSchemaSchema: () => yLt,
    StringSchemaSchema: () => ZWe,
    SubscribeRequestParamsSchema: () => xOt,
    SubscribeRequestSchema: () => IOt,
    SubscriptionFilterSchema: () => Nat,
    SubscriptionsAcknowledgedNotificationParamsSchema: () => MOt,
    SubscriptionsAcknowledgedNotificationSchema: () => NOt,
    SubscriptionsListenRequestParamsSchema: () => OOt,
    SubscriptionsListenRequestSchema: () => LOt,
    SubscriptionsListenResultMetaSchema: () => FOt,
    SubscriptionsListenResultSchema: () => $Ot,
    TaskAugmentedRequestParamsSchema: () => KHe,
    TaskCreationParamsSchema: () => Qrn,
    TaskMetadataSchema: () => fOt,
    TaskSchema: () => x_e,
    TaskStatusNotificationParamsSchema: () => oxe,
    TaskStatusNotificationSchema: () => eon,
    TaskStatusSchema: () => OLt,
    TextContentSchema: () => KWe,
    TextResourceContentsSchema: () => Lat,
    TitledMultiSelectEnumSchemaSchema: () => Gat,
    TitledSingleSelectEnumSchemaSchema: () => jat,
    ToolAnnotationsSchema: () => QOt,
    ToolChoiceSchema: () => uLt,
    ToolExecutionSchema: () => ZOt,
    ToolListChangedNotificationSchema: () => rLt,
    ToolResultContentSchema: () => dLt,
    ToolSchema: () => k_e,
    ToolUseContentSchema: () => zOt,
    UnsubscribeRequestParamsSchema: () => POt,
    UnsubscribeRequestSchema: () => DOt,
    UntitledMultiSelectEnumSchemaSchema: () => qat,
    UntitledSingleSelectEnumSchemaSchema: () => Bat,
  });
var BZ = (e) => vat.safeParse(e).success,
  iOt = (e) => Rat.safeParse(e).success,
  Ke = (e) => GWe.safeParse(e).success,
  Ge = (e) => zWe.safeParse(e).success;
var oa = (e) => typeof e === "object" && e !== null && !Array.isArray(e) && e.resultType === "input_required";
var Yr = (e) => xat.safeParse(e).success,
  na = (e) => Iat.safeParse(e).success;
var dn = "Mcp-Param-",
  Cs = "x-mcp-header",
  hn = /^[!#$%&'*+\-.^_`|~0-9A-Za-z]+$/,
  mn = new Set(["string", "integer", "boolean", "number"]);
function Xr(e) {
  let t = [],
    r = new Map(),
    s = (o, n, c) => {
      if (o === null || typeof o !== "object") return;
      let i = o;
      if (Cs in i) {
        if (!c || n.length === 0)
          return `${fr(n)}: x-mcp-header is only permitted on properties statically reachable via a chain of 'properties' keys (not under items, additionalProperties, oneOf/anyOf/allOf/not, if/then/else, or $ref)`;
        let d = i[Cs];
        if (typeof d !== "string" || d.length === 0) return `${fr(n)}: x-mcp-header MUST be a non-empty string`;
        if (!hn.test(d))
          return `${fr(n)}: x-mcp-header '${d}' is not a valid RFC 9110 token (no spaces, control characters or HTTP delimiters)`;
        let S = typeof i.type === "string" ? i.type : void 0;
        if (S === void 0 || !mn.has(S))
          return `${fr(n)}: x-mcp-header is only permitted on primitive-typed properties (string, integer, boolean); got ${S ?? "<none>"}`;
        let R = d.toLowerCase(),
          w = r.get(R);
        if (w !== void 0) return `x-mcp-header '${d}' is not case-insensitively unique (also declared as '${w}')`;
        r.set(R, d), t.push({ path: n, headerName: d, type: S });
      }
      let l = i.properties;
      if (l !== null && typeof l === "object")
        for (let [d, S] of Object.entries(l)) {
          let R = s(S, [...n, d], c);
          if (R !== void 0) return R;
        }
      for (let d of fn) {
        let S = i[d];
        if (S === void 0) continue;
        let R = Array.isArray(S) ? S : S !== null && typeof S === "object" && pn.has(d) ? Object.values(S) : [S];
        for (let w of R) {
          let g = s(w, [...n, `<${d}>`], !1);
          if (g !== void 0) return g;
        }
      }
    },
    a = s(e, [], !0);
  return a === void 0 ? { valid: !0, declarations: t } : { valid: !1, reason: a };
}
var fn = [
    "items",
    "prefixItems",
    "contains",
    "additionalProperties",
    "unevaluatedProperties",
    "unevaluatedItems",
    "propertyNames",
    "patternProperties",
    "dependentSchemas",
    "oneOf",
    "anyOf",
    "allOf",
    "not",
    "if",
    "then",
    "else",
    "$defs",
    "definitions",
  ],
  pn = new Set(["patternProperties", "dependentSchemas", "$defs", "definitions"]);
function fr(e) {
  return e.length === 0 ? "<root>" : e.join(".");
}
var ia = "=?base64?",
  ca = "?=";
function Sn(e) {
  if (typeof e === "string") return e;
  if (typeof e === "boolean") return e ? "true" : "false";
  if (typeof e === "number") {
    if (!Number.isFinite(e)) return;
    if (Number.isInteger(e) && !Number.isSafeInteger(e)) return;
    return String(e);
  }
}
function gn(e) {
  if (e.length === 0) return !0;
  if (e.startsWith(ia) && e.endsWith(ca)) return !0;
  if (e !== e.trim()) return !0;
  for (let t = 0; t < e.length; t++) {
    let r = e.codePointAt(t);
    if (r === 9 || (r >= 32 && r <= 126)) continue;
    return !0;
  }
  return !1;
}
function _n(e) {
  let t = new TextEncoder().encode(e),
    r = "";
  for (let s of t) r += String.fromCodePoint(s);
  return btoa(r);
}
function _r(e) {
  return gn(e) ? `${ia}${_n(e)}${ca}` : e;
}
function vn(e, t) {
  let r = e;
  for (let s of t) {
    if (r === null || typeof r !== "object") return;
    r = r[s];
  }
  return r;
}
function la(e, t) {
  let r = {};
  for (let s of e) {
    let a = vn(t, s.path);
    if (a === void 0 || a === null) continue;
    let o = Sn(a);
    if (o === void 0) continue;
    r[`${dn}${s.headerName}`] = _r(o);
  }
  return r;
}
var nt = -32020,
  kl = [
    {
      rung: "http-method",
      order: 1,
      evaluatedAt: "edge",
      codes: [-32000],
      conformance: [],
      rationale:
        "The modern era is POST-only; GET/DELETE are body-less 2025-era session operations and are method-routed to legacy serving (405 when legacy serving is not configured), before any body is read.",
    },
    {
      rung: "jsonrpc-shape",
      order: 2,
      evaluatedAt: "edge",
      codes: [Ai.InvalidRequest],
      conformance: ["server-stateless"],
      rationale:
        "The body must be a JSON-RPC request or notification: posted responses and batch arrays containing a modern or invalid element are rejected before classification (element-wise batch rule); all-legacy arrays stay legacy traffic.",
    },
    {
      rung: "era-classification",
      order: 3,
      evaluatedAt: "edge",
      codes: [nt, Ai.UnsupportedProtocolVersion],
      conformance: ["server-stateless", "http-header-validation", "http-custom-header-server-validation"],
      rationale:
        "Body-primary era classification with the protocol-version header as a cross-check; a header/body disagreement is rejected with -32020 (HeaderMismatch), and an envelope-less request on a modern-only endpoint is answered with the unsupported-protocol-version error naming the supported revisions.",
    },
    {
      rung: "envelope",
      order: 4,
      evaluatedAt: "edge",
      codes: [Ai.InvalidParams],
      conformance: ["server-stateless"],
      rationale:
        "A present envelope claim with a malformed envelope \u2014 and a missing envelope on a request whose protocol-version header names a modern revision \u2014 is an invalid-params rejection naming the offending or missing key(s); never a silent fall back to legacy handling. This is the only place an invalid-params rejection maps to HTTP 400.",
    },
    {
      rung: "method-registry",
      order: 5,
      evaluatedAt: "dispatch",
      codes: [Ai.MethodNotFound],
      conformance: ["server-stateless"],
      rationale:
        "Method existence outranks parameter validity: a method absent from the negotiated revision\u2019s registry (or with no handler installed) answers method-not-found before params or capabilities are looked at.",
    },
    {
      rung: "request-params",
      order: 6,
      evaluatedAt: "dispatch",
      codes: [Ai.InvalidParams],
      conformance: [],
      rationale:
        "Per-method params validation; emitted in-band by the dispatch layer (HTTP 200), never via the ladder status table.",
    },
    {
      rung: "standard-header-validation",
      order: 7,
      evaluatedAt: "pre-dispatch",
      codes: [nt],
      conformance: ["http-header-validation"],
      rationale:
        "SEP-2243 standard `Mcp-Method` / `Mcp-Name` headers \u2014 presence, sentinel decoding, and `Mcp-Name` \u2194 body cross-check \u2014 are validated by the HTTP entry on a modern-classified request after the supported-revision gate and before dispatch. The classifier\u2019s own header-mismatch cells (protocol-version, `Mcp-Method` mismatch) stay on the edge `era-classification` rung; this rung carries the entry-layer presence/`Mcp-Name` half. Evaluated before the capability gate, the factory call, and the `Mcp-Param-*` rung so a request that fails several rungs is answered by the standard-header rung first. The documented order (after method-registry 5 and request-params 6) is NOT the observed precedence: serveModern evaluates this rung immediately after the supported-revision gate, so a request that also fails a dispatch rung is answered here before the dispatch rungs (5\u20136) are consulted.",
    },
    {
      rung: "client-capabilities",
      order: 8,
      evaluatedAt: "pre-dispatch",
      codes: [Ai.MissingRequiredClientCapability],
      conformance: ["server-stateless"],
      rationale:
        "The capability requirement is checked by the HTTP entry, pre-dispatch, against the validated envelope the classifier produced \u2014 pinning the spec-mandated HTTP 400 independently of how dispatch- and handler-produced errors are mapped. The documented order (after method resolution and params validation) is preserved observably only while the requirement table is empty: once a served method gains a requirement entry, a request that is missing the capability and would also fail a dispatch rung is answered by this gate first, so the entry must consult the method registry before the gate if the documented precedence is to stay observable.",
    },
    {
      rung: "param-header-validation",
      order: 9,
      evaluatedAt: "pre-dispatch",
      codes: [nt],
      conformance: ["http-custom-header-server-validation"],
      rationale:
        "SEP-2243 `Mcp-Param-*` headers are validated against the named tool\u2019s `x-mcp-header` declarations and the body `arguments` after the tool registry is known and before dispatch reaches the handler; a missing/disagreeing/malformed header is rejected 400 / -32020 with the same shape as the standard-header cross-checks. The documented order (after method resolution and params validation) is preserved observably only when the body `arguments` would otherwise validate: the check runs pre-dispatch, so a `tools/call` that fails BOTH this rung and a dispatch-time rung (e.g. order-6 `request-params`, -32602) is answered by this gate first with 400 / -32020, not by the earlier-ordered rung.",
    },
  ],
  ql = {
    [Ai.ParseError]: 400,
    [Ai.InvalidRequest]: 400,
    [Ai.MethodNotFound]: 404,
    [Ai.UnsupportedProtocolVersion]: 400,
    [Ai.MissingRequiredClientCapability]: 400,
    [nt]: 400,
  };
function vr(e, t) {
  return dOt(e, t);
}
function jt(e) {
  return new Set(e.flatMap((t) => Object.keys(t.shape)));
}
function Ur(e) {
  if (e == null) return !1;
  let t = typeof e;
  if (t !== "object" && t !== "function") return !1;
  if (!("~standard" in e)) return !1;
  return typeof e["~standard"]?.validate === "function";
}
var ks = !1,
  Dr = "draft-2020-12";
function yn(e, t = "input") {
  let r = e["~standard"],
    s;
  if (r.jsonSchema) s = r.jsonSchema[t]({ target: Dr });
  else if (r.vendor === "zod") {
    if (!("_zod" in e))
      throw Error(
        "Schema appears to be from zod 3, which the SDK cannot convert to JSON Schema. Upgrade to zod >=4.2.0, or wrap your JSON Schema with fromJsonSchema().",
      );
    if (!ks)
      (ks = !0),
        console.warn(
          "[mcp-sdk] Your zod version does not implement `~standard.jsonSchema` (added in zod 4.2.0). Falling back to z.toJSONSchema(). Upgrade to zod >=4.2.0 to silence this warning.",
        );
    s = uOt(e, { target: Dr, io: t });
  } else
    throw Error(
      `Schema library "${r.vendor}" does not implement StandardJSONSchemaV1 (\`~standard.jsonSchema\`). Upgrade to a version that does, or wrap your JSON Schema with fromJsonSchema().`,
    );
  if (t === "output") {
    if (s.type !== void 0) return s;
    return ua(s) ? { type: "object", ...s } : s;
  }
  if (s.type !== void 0 && s.type !== "object")
    throw Error(
      `MCP tool and prompt schemas must describe objects (got type: ${JSON.stringify(s.type)}). Wrap your schema in z.object({...}) or equivalent.`,
    );
  return { type: "object", ...s };
}
function ua(e) {
  if ("properties" in e || "patternProperties" in e || "additionalProperties" in e || "required" in e) return !0;
  for (let t of ["oneOf", "anyOf", "allOf"]) {
    let r = e[t];
    if (Array.isArray(r) && r.length > 0)
      return r.every((s) => s !== null && typeof s === "object" && (s.type === "object" || ua(s)));
  }
  return !1;
}
function bn(e) {
  if (!e.path?.length) return e.message;
  return `${e.path.map((t) => String(typeof t === "object" ? t.key : t)).join(".")}: ${e.message}`;
}
async function jr(e, t) {
  let r = await e["~standard"].validate(t);
  if (r.issues && r.issues.length > 0) return { success: !1, error: r.issues.map((s) => bn(s)).join(", ") };
  return { success: !0, data: r.value };
}
function Rn(e) {
  let t = uOt(e, { target: Dr, io: "input" });
  return typeof t.pattern === "string" ? t.pattern : void 0;
}
var wn = /\\\.\\d\{(\d+)\}/;
function $n(e) {
  let t = wn.exec(e),
    r = [void 0, -1, 0];
  if (t) r.push(Number(t[1]));
  return [!1, !0].flatMap((s) =>
    [!1, !0].flatMap((a) => r.map((o) => jZ.datetime({ local: s, offset: a, precision: o }))),
  );
}
function En(e, t) {
  let r;
  switch (e) {
    case "email":
      r = [N6n()];
      break;
    case "uri":
      r = [pOt()];
      break;
    case "date":
      r = [jZ.date()];
      break;
    case "date-time":
      r = $n(t);
      break;
  }
  return new Set(r.map((s) => Rn(s)).filter((s) => s !== void 0));
}
function Pn(e, t, r) {
  if (r !== "zod") return !0;
  return En(e, t).has(t);
}
function Ut(e) {
  return typeof e === "object" && e !== null && !Array.isArray(e);
}
function Tn(e) {
  try {
    return yn(e, "input");
  } catch (t) {
    let r = t instanceof Error ? t.message : String(t);
    throw new wa(
      Ai.InvalidParams,
      `Elicitation requestedSchema must describe an object with flat primitive properties: ${r}`,
    );
  }
}
var zn = new Set(["$comment", "deprecated", "description", "examples", "readOnly", "title", "writeOnly"]);
function Zr(e) {
  return zn.has(e) || e.startsWith("x-");
}
var Cn = new Set(["$schema", ...Object.keys(t9e.shape.requestedSchema.shape)]),
  qs = {
    string: jt([ZWe, Bat, jat, Wat]),
    number: jt([e9e]),
    integer: jt([e9e]),
    boolean: jt([Uat]),
    array: jt([qat, Gat]),
  },
  kn = new Set(ZWe.shape.format.unwrap().options);
function qn(e, t, r, s) {
  if (!Ut(e)) return e;
  let a = typeof e.type === "string" && Object.hasOwn(qs, e.type) ? qs[e.type] : void 0;
  if (a === void 0) return e;
  let o = {};
  for (let [n, c] of Object.entries(e))
    if (a.has(n) || Zr(n)) o[n] = c;
    else if (n === "pattern" && e.type === "string" && typeof e.format === "string") {
      if (!kn.has(e.format)) o[n] = c;
      else if (typeof c !== "string" || !Pn(e.format, c, r)) s.push(`${t}.${n}`);
    } else s.push(`${t}.${n}`);
  return o;
}
function In(e, t) {
  let r = {},
    s = [];
  for (let [a, o] of Object.entries(e))
    if (a === "properties" && Ut(o))
      r[a] = Object.fromEntries(Object.entries(o).map(([n, c]) => [n, qn(c, `properties.${n}`, t, s)]));
    else if (Cn.has(a)) r[a] = o;
    else if (!Zr(a)) s.push(a);
  if (s.length > 0)
    throw new wa(
      Ai.InvalidParams,
      `Elicitation requestedSchema contains unsupported JSON Schema constraint(s) after Standard Schema conversion: ${s.join(", ")}`,
    );
  return r;
}
function On(e, t) {
  if (!Ut(e.properties)) return t;
  let r = Object.entries(e.properties)
    .filter(([, s]) => !vr(zat, s).success)
    .map(([s]) => `properties.${s}`);
  return r.length > 0 ? r.join(", ") : t;
}
function Vr(e, t, r = "") {
  if (Array.isArray(e) && Array.isArray(t)) return e.flatMap((s, a) => Vr(s, t[a], `${r}[${a}]`));
  if (!Ut(e) || !Ut(t)) return [];
  return Object.entries(e).flatMap(([s, a]) => {
    let o = r ? `${r}.${s}` : s;
    if (!Object.prototype.hasOwnProperty.call(t, s)) return Zr(s) ? [] : [o];
    return Vr(a, t[s], o);
  });
}
function Nn(e) {
  if (!Ur(e.requestedSchema)) return { ...e, mode: "form", requestedSchema: e.requestedSchema };
  let t = e.requestedSchema["~standard"].vendor,
    r = In(Tn(e.requestedSchema), t),
    s = vr(t9e.shape.requestedSchema, r);
  if (!s.success)
    throw new wa(
      Ai.InvalidParams,
      `Elicitation requestedSchema only supports flat primitive properties (string, number, integer, boolean, and string enums): ${On(r, s.error.message)}`,
    );
  let a = Vr(r, s.data);
  if (a.length > 0)
    throw new wa(
      Ai.InvalidParams,
      `Elicitation requestedSchema contains unsupported JSON Schema constraint(s) after Standard Schema conversion: ${a.join(", ")}`,
    );
  let o = (s.data.required ?? []).filter((n) => !Object.prototype.hasOwnProperty.call(s.data.properties, n));
  if (o.length > 0)
    throw new wa(
      Ai.InvalidParams,
      `Elicitation requestedSchema lists required properties that are not defined in properties: ${o.join(", ")}`,
    );
  return { ...e, mode: "form", requestedSchema: s.data };
}
function Mn(e) {
  let t = e.inputRequests !== void 0 && Object.keys(e.inputRequests).length > 0,
    r = typeof e.requestState === "string";
  if (!t && !r)
    throw TypeError(
      "inputRequired() requires at least one of inputRequests (with at least one entry) or requestState (spec: every InputRequiredResult MUST include at least one of the two)",
    );
  return {
    resultType: "input_required",
    ...(e.inputRequests !== void 0 && { inputRequests: e.inputRequests }),
    ...(e.requestState !== void 0 && { requestState: e.requestState }),
  };
}
var Il = Object.assign(Mn, {
  elicit(e) {
    try {
      return { method: "elicitation/create", params: Nn(e) };
    } catch (t) {
      throw t instanceof wa ? TypeError(t.message, { cause: t }) : t;
    }
  },
  elicitUrl(e) {
    return { method: "elicitation/create", params: { ...e, mode: "url" } };
  },
  createMessage(e) {
    return { method: "sampling/createMessage", params: e };
  },
  listRoots() {
    return { method: "roots/list" };
  },
});
var jn = !0,
  An = 10,
  Ln = 250;
function da(e) {
  return { autoFulfill: e?.autoFulfill ?? jn, maxRounds: e?.maxRounds ?? An };
}
function Un(e, t, r) {
  let s = t !== void 0 && Object.keys(t).length > 0;
  if (!s && r === void 0) return e;
  return { ...e, ...(s && { inputResponses: t }), ...(r !== void 0 && { requestState: r }) };
}
function Dn(e, t) {
  return `Multi-round-trip request '${e}' still required input after ${t} rounds (inputRequired.maxRounds)`;
}
function Vn(e, t) {
  return new Promise((r, s) => {
    if (t?.aborted) {
      s(t.reason instanceof Uo ? t.reason : new Uo(vo.RequestTimeout, String(t.reason)));
      return;
    }
    let a = setTimeout(() => {
        t?.removeEventListener("abort", o), r();
      }, e),
      o = () => {
        clearTimeout(a), s(t?.reason instanceof Uo ? t.reason : new Uo(vo.RequestTimeout, String(t?.reason)));
      };
    t?.addEventListener("abort", o, { once: !0 });
  });
}
function Fn(e) {
  let t = new AbortController(),
    r = () => t.abort(e?.reason);
  if ((e?.addEventListener("abort", r, { once: !0 }), e?.aborted)) t.abort(e.reason);
  return { signal: t.signal, abort: (s) => t.abort(s), dispose: () => e?.removeEventListener("abort", r) };
}
async function Hn(e) {
  let { config: t, method: r, originalParams: s, requestOptions: a, hooks: o, signal: n } = e,
    c = e.flowStartedAt ?? Date.now(),
    i = e.firstPayload,
    l = 0;
  while (!0) {
    if (((l += 1), l > t.maxRounds))
      throw new Uo(vo.InputRequiredRoundsExceeded, Dn(r, t.maxRounds), {
        rounds: t.maxRounds,
        lastResult: {
          inputRequests: i.inputRequests,
          ...(i.requestState !== void 0 && { requestState: i.requestState }),
        },
      });
    a.onprogress?.({ progress: l, message: `Fulfilling input required by '${r}' (round ${l})` });
    let d = Object.entries(i.inputRequests ?? {}),
      S;
    if (d.length > 0) {
      let g = Fn(n);
      try {
        let _ = await Promise.all(
          d.map(async ([h, u]) => {
            try {
              return [h, await o.dispatchInputRequest(h, u, g.signal)];
            } catch (p) {
              throw (g.abort(p), p);
            }
          }),
        );
        S = Object.fromEntries(_);
      } finally {
        g.dispose();
      }
    } else await Vn(Ln, n);
    let R = { ...(a.timeout !== void 0 && { timeout: a.timeout }) };
    if (a.maxTotalTimeout !== void 0) {
      let g = Date.now() - c,
        _ = a.maxTotalTimeout - g;
      if (_ <= 0)
        throw new Uo(vo.RequestTimeout, "Maximum total timeout exceeded", {
          maxTotalTimeout: a.maxTotalTimeout,
          totalElapsed: g,
        });
      R.maxTotalTimeout = _;
    }
    let w = await o.retry(Un(s, S, i.requestState), R);
    if (oa(w)) {
      i = { inputRequests: w.inputRequests ?? {}, ...(w.requestState !== void 0 && { requestState: w.requestState }) };
      continue;
    }
    return w;
  }
}
var xn = [
    "AnnotationsSchema",
    "AudioContentSchema",
    "BaseMetadataSchema",
    "BlobResourceContentsSchema",
    "BooleanSchemaSchema",
    "CallToolRequestSchema",
    "CallToolRequestParamsSchema",
    "CallToolResultSchema",
    "CancelledNotificationSchema",
    "CancelledNotificationParamsSchema",
    "CancelTaskRequestSchema",
    "CancelTaskResultSchema",
    "ClientCapabilitiesSchema",
    "ClientNotificationSchema",
    "ClientRequestSchema",
    "ClientResultSchema",
    "CompatibilityCallToolResultSchema",
    "CompleteRequestSchema",
    "CompleteRequestParamsSchema",
    "CompleteResultSchema",
    "ContentBlockSchema",
    "CreateMessageRequestSchema",
    "CreateMessageRequestParamsSchema",
    "CreateMessageResultSchema",
    "CreateMessageResultWithToolsSchema",
    "CreateTaskResultSchema",
    "CursorSchema",
    "DiscoverRequestSchema",
    "DiscoverResultSchema",
    "ElicitationCompleteNotificationSchema",
    "ElicitationCompleteNotificationParamsSchema",
    "ElicitRequestSchema",
    "ElicitRequestFormParamsSchema",
    "ElicitRequestParamsSchema",
    "ElicitRequestURLParamsSchema",
    "ElicitResultSchema",
    "EmbeddedResourceSchema",
    "EmptyResultSchema",
    "EnumSchemaSchema",
    "GetPromptRequestSchema",
    "GetPromptRequestParamsSchema",
    "GetPromptResultSchema",
    "GetTaskPayloadRequestSchema",
    "GetTaskPayloadResultSchema",
    "GetTaskRequestSchema",
    "GetTaskResultSchema",
    "IconSchema",
    "IconsSchema",
    "ImageContentSchema",
    "ImplementationSchema",
    "InitializedNotificationSchema",
    "InitializeRequestSchema",
    "InitializeRequestParamsSchema",
    "InitializeResultSchema",
    "JSONArraySchema",
    "JSONObjectSchema",
    "JSONRPCErrorResponseSchema",
    "JSONRPCMessageSchema",
    "JSONRPCNotificationSchema",
    "JSONRPCRequestSchema",
    "JSONRPCResponseSchema",
    "JSONRPCResultResponseSchema",
    "JSONValueSchema",
    "LegacyTitledEnumSchemaSchema",
    "ListPromptsRequestSchema",
    "ListPromptsResultSchema",
    "ListResourcesRequestSchema",
    "ListResourcesResultSchema",
    "ListResourceTemplatesRequestSchema",
    "ListResourceTemplatesResultSchema",
    "ListRootsRequestSchema",
    "ListRootsResultSchema",
    "ListTasksRequestSchema",
    "ListTasksResultSchema",
    "ListToolsRequestSchema",
    "ListToolsResultSchema",
    "LoggingLevelSchema",
    "LoggingMessageNotificationSchema",
    "LoggingMessageNotificationParamsSchema",
    "ModelHintSchema",
    "ModelPreferencesSchema",
    "MultiSelectEnumSchemaSchema",
    "NotificationSchema",
    "NumberSchemaSchema",
    "PaginatedRequestSchema",
    "PaginatedRequestParamsSchema",
    "PaginatedResultSchema",
    "PingRequestSchema",
    "PrimitiveSchemaDefinitionSchema",
    "ProgressSchema",
    "ProgressNotificationSchema",
    "ProgressNotificationParamsSchema",
    "ProgressTokenSchema",
    "PromptSchema",
    "PromptArgumentSchema",
    "PromptListChangedNotificationSchema",
    "PromptMessageSchema",
    "PromptReferenceSchema",
    "ReadResourceRequestSchema",
    "ReadResourceRequestParamsSchema",
    "ReadResourceResultSchema",
    "RelatedTaskMetadataSchema",
    "RequestSchema",
    "RequestIdSchema",
    "RequestMetaSchema",
    "ResourceSchema",
    "ResourceContentsSchema",
    "ResourceLinkSchema",
    "ResourceListChangedNotificationSchema",
    "ResourceRequestParamsSchema",
    "ResourceTemplateSchema",
    "ResourceTemplateReferenceSchema",
    "ResourceUpdatedNotificationSchema",
    "ResourceUpdatedNotificationParamsSchema",
    "ResultMetaObjectSchema",
    "ResultSchema",
    "RoleSchema",
    "RootSchema",
    "RootsListChangedNotificationSchema",
    "SamplingContentSchema",
    "SamplingMessageSchema",
    "SamplingMessageContentBlockSchema",
    "ServerCapabilitiesSchema",
    "ServerNotificationSchema",
    "ServerRequestSchema",
    "ServerResultSchema",
    "SetLevelRequestSchema",
    "SetLevelRequestParamsSchema",
    "SingleSelectEnumSchemaSchema",
    "StringSchemaSchema",
    "SubscribeRequestSchema",
    "SubscribeRequestParamsSchema",
    "SubscriptionFilterSchema",
    "SubscriptionsAcknowledgedNotificationSchema",
    "SubscriptionsAcknowledgedNotificationParamsSchema",
    "SubscriptionsListenRequestSchema",
    "SubscriptionsListenRequestParamsSchema",
    "SubscriptionsListenResultSchema",
    "SubscriptionsListenResultMetaSchema",
    "TaskAugmentedRequestParamsSchema",
    "TaskCreationParamsSchema",
    "TaskMetadataSchema",
    "TaskSchema",
    "TaskStatusSchema",
    "TaskStatusNotificationSchema",
    "TaskStatusNotificationParamsSchema",
    "TextContentSchema",
    "TextResourceContentsSchema",
    "TitledMultiSelectEnumSchemaSchema",
    "TitledSingleSelectEnumSchemaSchema",
    "ToolSchema",
    "ToolAnnotationsSchema",
    "ToolChoiceSchema",
    "ToolExecutionSchema",
    "ToolListChangedNotificationSchema",
    "ToolResultContentSchema",
    "ToolUseContentSchema",
    "UnsubscribeRequestSchema",
    "UnsubscribeRequestParamsSchema",
    "UntitledMultiSelectEnumSchemaSchema",
    "UntitledSingleSelectEnumSchemaSchema",
  ],
  Kn = {
    IdJagTokenExchangeResponseSchema: MLt,
    OAuthClientInformationFullSchema: Kat,
    OAuthClientInformationSchema: FLt,
    OAuthClientMetadataSchema: NLt,
    OAuthClientRegistrationErrorSchema: fon,
    OAuthErrorResponseSchema: O_e,
    OAuthMetadataSchema: $ae,
    OAuthProtectedResourceMetadataSchema: Vat,
    OAuthTokenRevocationRequestSchema: mon,
    OAuthTokensSchema: D_e,
    OpenIdProviderDiscoveryMetadataSchema: P_e,
    OpenIdProviderMetadataSchema: LLt,
  },
  ha = {},
  ma = {};
function fa(e, t) {
  let r = e.slice(0, -6);
  (ha[r] = t), (ma[r] = (s) => t.safeParse(s).success);
}
for (let e of xn) fa(e, un[e]);
for (let [e, t] of Object.entries(Kn)) fa(e, t);
var Jn = Object.freeze(ha),
  pa = Object.freeze(ma);
function Gn(e) {
  switch (e) {
    case "initialize":
    case "notifications/initialized":
      return Te(void 0);
    case "server/discover":
      return Te(gr);
    default:
      return;
  }
}
var yr = 60000,
  Bn = [Dae, WWe, zHe, qWe],
  Wn = ["inputResponses", "requestState"];
function Is(e, t) {
  let r = e.params;
  if (!Lt(r)) return { message: e, lifted: {} };
  let s = r._meta,
    a = Lt(s) ? Bn.filter((i) => i in s) : [],
    o = t === "request" ? Wn.filter((i) => i in r) : [];
  if (a.length === 0 && o.length === 0) return { message: e, lifted: {} };
  let n = {},
    c = { ...r };
  if (a.length > 0 && Lt(s)) {
    let i = {},
      l = { ...s };
    for (let d of a) (i[d] = s[d]), delete l[d];
    if (((n.envelope = i), Object.keys(l).length > 0)) c._meta = l;
    else delete c._meta;
  }
  for (let i of o) {
    if (i === "inputResponses") n.inputResponses = c[i];
    if (i === "requestState") n.requestState = c[i];
    delete c[i];
  }
  return { message: { ...e, params: c }, lifted: n };
}
function Os(e, t) {
  let r = e.validateResult(t, void 0);
  if (!r.ok && r.reason === "not-in-era") return;
  return {
    "~standard": {
      version: 1,
      vendor: "mcp-wire-codec",
      validate(s) {
        let a = e.validateResult(t, s);
        if (a.ok) return { value: a.value };
        return { issues: [{ message: a.reason === "invalid" ? a.message : `not-in-era: ${t}` }] };
      },
    },
  };
}
function Qr(e) {
  return () => e;
}
var Yn = Qr(void 0),
  Xn,
  Sa = class {
    _transport;
    _requestMessageId = 0;
    _requestHandlers = new Map();
    _requestHandlerAbortControllers = new Map();
    _notificationHandlers = new Map();
    _responseHandlers = new Map();
    _progressHandlers = new Map();
    _timeoutInfo = new Map();
    _pendingDebouncedNotifications = new Set();
    _negotiatedProtocolVersion;
    static {
      Xn = (e, t) => {
        e._negotiatedProtocolVersion = t;
      };
    }
    _supportedProtocolVersions;
    onclose;
    onerror;
    fallbackRequestHandler;
    fallbackNotificationHandler;
    constructor(e) {
      (this._options = e),
        (this._supportedProtocolVersions = e?.supportedProtocolVersions ?? GHe),
        this.setNotificationHandler("notifications/cancelled", (t) => {
          this._oncancel(t);
        }),
        this.setNotificationHandler("notifications/progress", (t) => {
          this._onprogress(t);
        }),
        this.setRequestHandler("ping", (t) => ({}));
    }
    _shouldDropInbound(e) {}
    _outboundMetaEnvelope() {}
    _envelopeOutbound(e) {
      let t = this._outboundMetaEnvelope();
      if (t === void 0) return e;
      let r = e.params ?? {};
      return { ...e, params: { ...r, _meta: { ...t, ...r._meta } } };
    }
    _resolveNonCompleteResult(e, t) {
      return Promise.reject(
        new Uo(vo.UnsupportedResultType, `Unsupported result type '${e.kind}' for ${t.request.method}`, {
          resultType: e.kind,
          method: t.request.method,
        }),
      );
    }
    _getRequestHandler(e) {
      return this._requestHandlers.get(e);
    }
    async _oncancel(e) {
      if (!e.params.requestId) return;
      this._requestHandlerAbortControllers.get(e.params.requestId)?.abort(e.params.reason);
    }
    _setupTimeout(e, t, r, s, a = !1) {
      this._timeoutInfo.set(e, {
        timeoutId: setTimeout(s, t),
        startTime: Date.now(),
        timeout: t,
        maxTotalTimeout: r,
        resetTimeoutOnProgress: a,
        onTimeout: s,
      });
    }
    _resetTimeout(e) {
      let t = this._timeoutInfo.get(e);
      if (!t) return !1;
      let r = Date.now() - t.startTime;
      if (t.maxTotalTimeout && r >= t.maxTotalTimeout)
        throw (
          (this._timeoutInfo.delete(e),
          new Uo(vo.RequestTimeout, "Maximum total timeout exceeded", {
            maxTotalTimeout: t.maxTotalTimeout,
            totalElapsed: r,
          }))
        );
      return clearTimeout(t.timeoutId), (t.timeoutId = setTimeout(t.onTimeout, t.timeout)), !0;
    }
    _cleanupTimeout(e) {
      let t = this._timeoutInfo.get(e);
      if (t) clearTimeout(t.timeoutId), this._timeoutInfo.delete(e);
    }
    async connect(e) {
      this._transport = e;
      let t = this.transport?.onclose;
      this._transport.onclose = () => {
        try {
          t?.();
        } finally {
          this._onclose();
        }
      };
      let r = this.transport?.onerror;
      this._transport.onerror = (a) => {
        r?.(a), this._onerror(a);
      };
      let s = this._transport?.onmessage;
      (this._transport.onmessage = (a, o) => {
        if ((s?.(a, o), Ke(a) || Ge(a))) this._onresponse(a);
        else if (BZ(a)) this._onrequest(a, o);
        else if (iOt(a)) this._onnotification(a, o);
        else this._onerror(Error(`Unknown message type: ${JSON.stringify(a)}`));
      }),
        e.setSupportedProtocolVersions?.(this._supportedProtocolVersions),
        await this._transport.start();
    }
    _onclose() {
      let e = this._responseHandlers;
      (this._responseHandlers = new Map()), this._progressHandlers.clear(), this._pendingDebouncedNotifications.clear();
      for (let s of this._timeoutInfo.values()) clearTimeout(s.timeoutId);
      this._timeoutInfo.clear();
      let t = this._requestHandlerAbortControllers;
      this._requestHandlerAbortControllers = new Map();
      let r = new Uo(vo.ConnectionClosed, "Connection closed");
      this._transport = void 0;
      try {
        this.onclose?.();
      } finally {
        for (let s of e.values()) s(r);
        for (let s of t.values()) s.abort(r);
      }
    }
    _onerror(e) {
      this.onerror?.(e);
    }
    _onnotification(e, t) {
      let { message: r } = Is(e, "notification"),
        s = this._negotiatedWireCodec();
      if (t?.classification === void 0 && this._shouldDropInbound(e) === "drop") return;
      if (t?.classification !== void 0) {
        let n = zs(t.classification);
        if (n !== s.era) {
          this._onerror(
            Error(
              `Era mismatch on inbound notification '${r.method}': classified as ${n} but this instance serves ${s.era}`,
            ),
          );
          return;
        }
      }
      if (Mr(r.method) && !s.hasNotificationMethod(r.method)) return;
      let a = this._notificationHandlers.get(r.method),
        o = this.fallbackNotificationHandler;
      if (a === void 0 && o === void 0) return;
      Promise.resolve()
        .then(() => (a === void 0 ? o(r) : a(r, s)))
        .catch((n) => this._onerror(Error(`Uncaught error in notification handler: ${n}`)));
    }
    _onrequest(e, t) {
      let { message: r, lifted: s } = Is(e, "request"),
        a = this._negotiatedWireCodec();
      if (t?.classification === void 0 && this._shouldDropInbound(e) === "drop") {
        this._onerror(Error(`Dropped inbound request '${e.method}': not servable on this connection's protocol era`));
        return;
      }
      let o = this._transport,
        n = (_, h, u) => {
          let p = { jsonrpc: "2.0", id: r.id, error: { code: _, message: h, ...(u !== void 0 && { data: u }) } };
          o?.send(p).catch((m) => this._onerror(Error(`Failed to send an error response: ${m}`)));
        };
      if (t?.classification !== void 0) {
        let _ = zs(t.classification);
        if (_ !== a.era) {
          this._onerror(
            Error(
              `Era mismatch on inbound request '${r.method}': classified as ${_} but this instance serves ${a.era}`,
            ),
          );
          let h = t.classification.revision ?? _;
          n(Ai.UnsupportedProtocolVersion, `Unsupported protocol version: ${h}`, {
            supported: this._supportedProtocolVersions,
            requested: h,
          });
          return;
        }
      }
      if (Nr(r.method) && !a.hasRequestMethod(r.method)) {
        n(Ai.MethodNotFound, "Method not found");
        return;
      }
      let c = this._requestHandlers.get(r.method) ?? this.fallbackRequestHandler;
      if (c === void 0) {
        n(Ai.MethodNotFound, "Method not found");
        return;
      }
      let i = a.checkInboundEnvelope(s);
      if (i !== void 0) {
        n(Ai.InvalidParams, i);
        return;
      }
      let l = (_, h) =>
          this._notificationViaCodec(this._resolveOutboundCodec(_.method), _, { ...h, relatedRequestId: r.id }),
        d = (_, h, u) =>
          this._requestWithSchemaViaCodec(this._resolveOutboundCodec(_.method), _, h, { ...u, relatedRequestId: r.id }),
        S = new AbortController();
      this._requestHandlerAbortControllers.set(r.id, S);
      let R = s.inputResponses === void 0 ? void 0 : Zn(s.inputResponses),
        w = {
          sessionId: o?.sessionId,
          mcpReq: {
            id: r.id,
            method: r.method,
            _meta: r.params?._meta,
            ...(s.envelope !== void 0 && { envelope: s.envelope }),
            ...(R !== void 0 && { inputResponses: R.accepted }),
            ...(R !== void 0 && R.droppedKeys.length > 0 && { droppedInputResponseKeys: R.droppedKeys }),
            requestState: s.requestState === void 0 ? Yn : Qr(s.requestState),
            signal: S.signal,
            send: (_, h, u) => {
              let p = this._resolveOutboundCodec(_.method);
              if ((this._assertOutboundRequestInEra(p, _.method), Ur(h))) return d(_, h, u);
              let m = Os(p, _.method);
              if (m === void 0)
                throw TypeError(
                  `'${_.method}' is not a spec method; pass a result schema as the second argument to ctx.mcpReq.send().`,
                );
              return d(_, m, h);
            },
            notify: l,
          },
          http: t?.authInfo ? { authInfo: t.authInfo } : void 0,
        },
        g = this.buildContext(w, t);
      Promise.resolve()
        .then(() => c(r, g))
        .then(
          async (_) => {
            if (S.signal.aborted) return;
            let h;
            try {
              h = a.encodeResult(r.method, _, this._outboundServerInfo());
            } catch (p) {
              this._onerror(Error(`Failed to encode result for ${r.method}: ${p}`)),
                n(Ai.InternalError, "Internal error");
              return;
            }
            let u = { result: h, jsonrpc: "2.0", id: r.id };
            await o?.send(u);
          },
          async (_) => {
            if (S.signal.aborted) return;
            let h = Number.isSafeInteger(_.code) ? _.code : Ai.InternalError,
              u = {
                jsonrpc: "2.0",
                id: r.id,
                error: {
                  code: a.encodeErrorCode(h),
                  message: _.message ?? "Internal error",
                  ...(_.data !== void 0 && { data: _.data }),
                },
              };
            await o?.send(u);
          },
        )
        .catch((_) => this._onerror(Error(`Failed to send response: ${_}`)))
        .finally(() => {
          if (this._requestHandlerAbortControllers.get(r.id) === S) this._requestHandlerAbortControllers.delete(r.id);
        });
    }
    _onprogress(e) {
      let { progressToken: t, ...r } = e.params,
        s = Number(t),
        a = this._progressHandlers.get(s);
      if (!a) {
        this._onerror(Error(`Received a progress notification for an unknown token: ${JSON.stringify(e)}`));
        return;
      }
      let o = this._responseHandlers.get(s),
        n = this._timeoutInfo.get(s);
      if (n && o && n.resetTimeoutOnProgress)
        try {
          this._resetTimeout(s);
        } catch (c) {
          this._responseHandlers.delete(s), this._progressHandlers.delete(s), this._cleanupTimeout(s), o(c);
          return;
        }
      a(r);
    }
    _onresponse(e) {
      let t = Number(e.id),
        r = this._responseHandlers.get(t);
      if (r === void 0) {
        this._onerror(Error(`Received a response for an unknown message ID: ${JSON.stringify(e)}`));
        return;
      }
      if ((this._responseHandlers.delete(t), this._cleanupTimeout(t), this._progressHandlers.delete(t), Ke(e))) r(e);
      else r(wa.fromError(e.error.code, e.error.message, e.error.data));
    }
    get transport() {
      return this._transport;
    }
    async close() {
      await this._transport?.close();
    }
    request(e, t, r) {
      let s = this._resolveOutboundCodec(e.method);
      if ((this._assertOutboundRequestInEra(s, e.method), Ur(t))) return this._requestWithSchemaViaCodec(s, e, t, r);
      let a = Os(s, e.method);
      if (a === void 0)
        throw TypeError(
          `'${e.method}' is not a spec method; pass a result schema as the second argument to request().`,
        );
      return this._requestWithSchemaViaCodec(s, e, a, t);
    }
    _negotiatedWireCodec() {
      return Te(this._negotiatedProtocolVersion);
    }
    _wireCodec() {
      return this._negotiatedWireCodec();
    }
    _resolveOutboundCodec(e) {
      if (this._negotiatedProtocolVersion === void 0) {
        let t = Gn(e);
        if (t) return t;
      }
      return this._negotiatedWireCodec();
    }
    _assertOutboundRequestInEra(e, t) {
      if (t.startsWith("tasks/")) return;
      if (Nr(t) && !e.hasRequestMethod(t))
        throw new Uo(
          vo.MethodNotSupportedByProtocolVersion,
          `Method '${t}' is not supported by the negotiated protocol version (wire era ${e.era})`,
          { method: t, era: e.era },
        );
    }
    _requestWithSchema(e, t, r) {
      let s = this._resolveOutboundCodec(e.method);
      return this._assertOutboundRequestInEra(s, e.method), this._requestWithSchemaViaCodec(s, e, t, r);
    }
    _requestWithSchemaViaCodec(e, t, r, s) {
      let { relatedRequestId: a, resumptionToken: o, onresumptiontoken: n, headers: c } = s ?? {},
        i = Date.now(),
        l,
        d;
      return new Promise((S, R) => {
        let w = (v) => {
          R(v);
        };
        if (!this._transport) {
          w(Error("Not connected"));
          return;
        }
        if (this._options?.enforceStrictCapabilities === !0)
          try {
            this.assertCapabilityForMethod(t.method);
          } catch (v) {
            w(v);
            return;
          }
        if (s?.signal?.aborted) {
          let v = s.signal.reason;
          throw v instanceof Uo ? v : new Uo(vo.RequestTimeout, String(v));
        }
        let g = e.era === gr && this._transport.hasPerRequestStream === !0 ? new AbortController() : void 0,
          _ = this._requestMessageId++;
        d = _;
        let h = { ...t, jsonrpc: "2.0", id: _ };
        if (s?.onprogress)
          this._progressHandlers.set(_, s.onprogress),
            (h.params = { ...t.params, _meta: { ...t.params?._meta, progressToken: _ } });
        let u = this._envelopeOutbound(h),
          p = !1,
          m = (v) => {
            if (p) return;
            if ((this._progressHandlers.delete(_), g === void 0))
              this._transport
                ?.send(
                  this._envelopeOutbound({
                    jsonrpc: "2.0",
                    method: "notifications/cancelled",
                    params: { requestId: _, reason: String(v) },
                  }),
                  { relatedRequestId: a, resumptionToken: o, onresumptiontoken: n },
                )
                .catch((f) => this._onerror(Error(`Failed to send cancellation: ${f}`)));
            else g.abort();
            R(v instanceof Uo ? v : new Uo(vo.RequestTimeout, String(v)));
          };
        this._responseHandlers.set(_, (v) => {
          if (s?.signal?.aborted) return;
          if (((p = !0), v instanceof Error)) return R(v);
          let f;
          try {
            f = e.decodeResult(t.method, v.result);
          } catch (I) {
            return R(I instanceof Error ? I : Error(String(I)));
          }
          if (f.kind === "invalid") return R(f.error);
          if (f.kind === "task") {
            if (s?.allowTask === !0 && this._capabilities?.extensions?.[an] !== void 0 && cn(f.result))
              return S(f.result);
            return R(
              new Uo(vo.UnsupportedResultType, `Unsupported result type 'task' for ${t.method}`, { method: t.method }),
            );
          }
          if (f.kind === "input_required") {
            if (s?.allowInputRequired === !0) return S(ri(f));
            let I = {
              codec: e,
              request: t,
              resultSchema: r,
              options: s,
              flowStartedAt: i,
              retry: (L, k) =>
                this._requestWithSchemaViaCodec(
                  e,
                  L === void 0 ? { method: t.method } : { method: t.method, params: L },
                  r,
                  k,
                ),
            };
            return S(this._resolveNonCompleteResult(f, I));
          }
          let P = f.result;
          jr(r, P).then((I) => {
            if (I.success) S(I.data);
            else R(new Uo(vo.InvalidResult, `Invalid result for ${t.method}: ${I.error}`));
          }, R);
        }),
          (l = () => m(s?.signal?.reason)),
          s?.signal?.addEventListener("abort", l, { once: !0 });
        let y = s?.timeout ?? yr,
          E = () => m(new Uo(vo.RequestTimeout, "Request timed out", { timeout: y }));
        this._setupTimeout(_, y, s?.maxTotalTimeout, E, s?.resetTimeoutOnProgress ?? !1),
          this._transport
            .send(u, {
              relatedRequestId: a,
              resumptionToken: o,
              onresumptiontoken: n,
              headers: c,
              requestSignal: g?.signal,
            })
            .catch((v) => {
              this._progressHandlers.delete(_), R(v);
            });
      }).finally(() => {
        if (l) s?.signal?.removeEventListener("abort", l);
        if (d !== void 0) this._responseHandlers.delete(d), this._cleanupTimeout(d);
      });
    }
    async notification(e, t) {
      return this._notificationViaCodec(this._resolveOutboundCodec(e.method), e, t);
    }
    async _notificationViaCodec(e, t, r) {
      if (!this._transport) throw new Uo(vo.NotConnected, "Not connected");
      if (Mr(t.method) && !e.hasNotificationMethod(t.method))
        throw new Uo(
          vo.MethodNotSupportedByProtocolVersion,
          `Notification '${t.method}' is not supported by the negotiated protocol version (wire era ${e.era})`,
          { method: t.method, era: e.era },
        );
      this.assertNotificationCapability(t.method);
      let s = this._envelopeOutbound({ jsonrpc: "2.0", ...t });
      if ((this._options?.debouncedNotificationMethods ?? []).includes(t.method) && !t.params && !r?.relatedRequestId) {
        if (this._pendingDebouncedNotifications.has(t.method)) return;
        this._pendingDebouncedNotifications.add(t.method),
          Promise.resolve().then(() => {
            if ((this._pendingDebouncedNotifications.delete(t.method), !this._transport)) return;
            this._transport?.send(s, r).catch((a) => this._onerror(a));
          });
        return;
      }
      await this._transport.send(s, r);
    }
    setRequestHandler(e, t, r) {
      this.assertRequestHandlerCapability(e);
      let s;
      if (typeof t === "function") {
        if (!Nr(e))
          throw TypeError(
            `'${e}' is not a spec request method; pass schemas as the second argument to setRequestHandler().`,
          );
        s = (a, o) => {
          let n = this._negotiatedWireCodec(),
            c = n.validateRequest(e, a);
          if (!c.ok && c.reason === "not-in-era") c = n.validateInputRequest(e, a);
          if (!c.ok) {
            if (c.reason === "not-in-era")
              throw new wa(Ai.InternalError, `No wire schema for ${e} in the resolved era`);
            throw Error(c.message);
          }
          return Promise.resolve(t(c.value, o));
        };
      } else if (r)
        s = async (a, o) => {
          let n = await jr(t.params, { ...a.params });
          if (!n.success) throw new wa(Ai.InvalidParams, `Invalid params for ${e}: ${n.error}`);
          return r(n.data, o);
        };
      else throw TypeError("setRequestHandler: handler is required");
      this._requestHandlers.set(e, this._wrapHandler(e, s));
    }
    _wrapHandler(e, t) {
      return t;
    }
    _outboundServerInfo() {}
    removeRequestHandler(e) {
      this._requestHandlers.delete(e);
    }
    assertCanSetRequestHandler(e) {
      if (this._requestHandlers.has(e))
        throw Error(`A request handler for ${e} already exists, which would be overridden`);
    }
    setNotificationHandler(e, t, r) {
      if (typeof t === "function") {
        if (!Mr(e))
          throw TypeError(
            `'${e}' is not a spec notification method; pass schemas as the second argument to setNotificationHandler().`,
          );
        this._notificationHandlers.set(e, (s, a) => {
          let o = a.validateNotification(e, s);
          if (!o.ok) {
            if (o.reason === "not-in-era")
              throw new wa(Ai.InternalError, `No wire schema for ${e} in the resolved era`);
            throw Error(o.message);
          }
          return Promise.resolve(t(o.value));
        });
        return;
      }
      if (!r) throw TypeError("setNotificationHandler: handler is required");
      this._notificationHandlers.set(e, async (s) => {
        let a = await jr(t.params, { ...s.params });
        if (!a.success) throw new wa(Ai.InvalidParams, `Invalid params for notification ${e}: ${a.error}`);
        await r(a.data, s);
      });
    }
    removeNotificationHandler(e) {
      this._notificationHandlers.delete(e);
    }
  };
function Lt(e) {
  return e !== null && typeof e === "object" && !Array.isArray(e);
}
function ga(e, t) {
  let r = { ...e };
  for (let s in t) {
    let a = s,
      o = t[a];
    if (o === void 0) continue;
    let n = r[a];
    r[a] = Lt(n) && Lt(o) ? { ...n, ...o } : o;
  }
  return r;
}
function pr(e) {
  return typeof e === "object" && e !== null && !Array.isArray(e);
}
function Zn(e) {
  let t = {},
    r = [];
  if (!pr(e)) return { accepted: t, droppedKeys: r };
  for (let [s, a] of Object.entries(e)) {
    if (!pr(a) || "method" in a || "result" in a) {
      r.push(s);
      continue;
    }
    t[s] = a;
  }
  return { accepted: t, droppedKeys: r };
}
function Ns(e) {
  throw new Uo(
    vo.SendFailed,
    `ctx.mcpReq.${e} is not available while fulfilling an embedded input request: the request is fulfilled locally and has no related peer request`,
  );
}
function Qn(e, t, r, s, a) {
  return {
    sessionId: a,
    mcpReq: {
      id: e,
      method: t,
      _meta: r?._meta,
      requestState: Qr(void 0),
      signal: s,
      send: () => Ns("send"),
      notify: () => Ns("notify"),
    },
  };
}
async function ei(e, t, r, s, a) {
  if (!pr(s) || typeof s.method !== "string")
    throw new Uo(
      vo.InvalidResult,
      `Invalid input request '${r}': each inputRequests entry must be an embedded request object with a method`,
      { key: r },
    );
  let o = s.method;
  if (!t.hasInputRequestMethod(o))
    throw new Uo(
      vo.InvalidResult,
      `Invalid input request '${r}': '${o}' is not an embedded request the ${t.era} revision defines (expected elicitation/create, sampling/createMessage, or roots/list)`,
      { key: r, method: o },
    );
  let n = e.getRequestHandler(o);
  if (n === void 0)
    throw new Uo(
      vo.CapabilityNotSupported,
      `Cannot fulfil input request '${r}': no handler is registered for '${o}' on this client. Declare the corresponding capability and register a handler, or handle input_required results manually.`,
      { key: r, method: o },
    );
  let c = pr(s.params) ? s.params : void 0;
  return await n(
    { jsonrpc: "2.0", id: r, method: o, ...(c !== void 0 && { params: c }) },
    e.buildContext(Qn(r, o, c, a, e.sessionId)),
  );
}
function ti(e, t) {
  return {
    ...(e?.signal !== void 0 && { signal: e.signal }),
    ...(e?.onprogress !== void 0 && { onprogress: e.onprogress }),
    ...(e?.resetTimeoutOnProgress !== void 0 && { resetTimeoutOnProgress: e.resetTimeoutOnProgress }),
    ...(e?.headers !== void 0 && { headers: e.headers }),
    ...(t.timeout !== void 0 && { timeout: t.timeout }),
    ...(t.maxTotalTimeout !== void 0 && { maxTotalTimeout: t.maxTotalTimeout }),
    allowInputRequired: !0,
  };
}
function _a(e, t, r, s) {
  let { codec: a, request: o, options: n, flowStartedAt: c } = s,
    i = { inputRequests: r.inputRequests, ...(r.requestState !== void 0 && { requestState: r.requestState }) },
    l = { dispatchInputRequest: (d, S, R) => ei(e, a, d, S, R), retry: (d, S) => s.retry(d, ti(n, S)) };
  return Hn({
    config: t,
    method: o.method,
    originalParams: o.params,
    firstPayload: i,
    flowStartedAt: c,
    signal: n?.signal,
    requestOptions: {
      ...(n?.timeout !== void 0 && { timeout: n.timeout }),
      ...(n?.maxTotalTimeout !== void 0 && { maxTotalTimeout: n.maxTotalTimeout }),
      ...(n?.onprogress !== void 0 && { onprogress: n.onprogress }),
    },
    hooks: l,
  });
}
function ri(e) {
  return {
    resultType: "input_required",
    inputRequests: e.inputRequests,
    ...(e.requestState !== void 0 && { requestState: e.requestState }),
  };
} /*!
 * content-type
 * Copyright(c) 2015 Douglas Christopher Wilson
 * MIT Licensed
 */
var si = U((e) => {
    var t =
        /; *([!#$%&'*+.^_`|~0-9A-Za-z-]+) *= *("(?:[\u000b\u0020\u0021\u0023-\u005b\u005d-\u007e\u0080-\u00ff]|\\[\u000b\u0020-\u00ff])*"|[!#$%&'*+.^_`|~0-9A-Za-z-]+) */g,
      r = /\\([\u000b\u0020-\u00ff])/g,
      s = /^[!#$%&'*+.^_`|~0-9A-Za-z-]+\/[!#$%&'*+.^_`|~0-9A-Za-z-]+$/;
    e.parse = a;
    function a(c) {
      if (!c) throw TypeError("argument string is required");
      var i = typeof c === "object" ? o(c) : c;
      if (typeof i !== "string") throw TypeError("argument string is required to be a string");
      var l = i.indexOf(";"),
        d = l !== -1 ? i.slice(0, l).trim() : i.trim();
      if (!s.test(d)) throw TypeError("invalid media type");
      var S = new n(d.toLowerCase());
      if (l !== -1) {
        var R, w, g;
        t.lastIndex = l;
        while ((w = t.exec(i))) {
          if (w.index !== l) throw TypeError("invalid parameter format");
          if (((l += w[0].length), (R = w[1].toLowerCase()), (g = w[2]), g.charCodeAt(0) === 34)) {
            if (((g = g.slice(1, -1)), g.indexOf("\\") !== -1)) g = g.replace(r, "$1");
          }
          S.parameters[R] = g;
        }
        if (l !== i.length) throw TypeError("invalid parameter format");
      }
      return S;
    }
    function o(c) {
      var i;
      if (typeof c.getHeader === "function") i = c.getHeader("content-type");
      else if (typeof c.headers === "object") i = c.headers && c.headers["content-type"];
      if (typeof i !== "string") throw TypeError("content-type header is missing from object");
      return i;
    }
    function n(c) {
      (this.parameters = Object.create(null)), (this.type = c);
    }
  }),
  ai = lr(si(), 1);
function va(e) {
  if (!e) return;
  try {
    return ai.parse(e).type;
  } catch {
    let t = (e.split(";", 1)[0] ?? "").trim().toLowerCase();
    if (t === "" || e.slice(t.length).includes(",")) return;
    return t;
  }
}
var ya = 10485760,
  jrn = class {
    _buffer;
    _maxBufferSize;
    constructor(e) {
      this._maxBufferSize = e?.maxBufferSize ?? ya;
    }
    append(e) {
      if ((this._buffer?.length ?? 0) + e.length > this._maxBufferSize)
        throw (this.clear(), Error(`ReadBuffer exceeded maximum size of ${this._maxBufferSize} bytes`));
      this._buffer = this._buffer ? Buffer.concat([this._buffer, e]) : e;
    }
    readMessage() {
      while (this._buffer) {
        let e = this._buffer.indexOf(`
`);
        if (e === -1) return null;
        let t = this._buffer.toString("utf8", 0, e).replace(/\r$/, "");
        this._buffer = this._buffer.subarray(e + 1);
        try {
          return sOt(t);
        } catch (r) {
          if (r instanceof SyntaxError) continue;
          throw r;
        }
      }
      return null;
    }
    clear() {
      this._buffer = void 0;
    }
  };
function sOt(e) {
  return TB.parse(JSON.parse(e));
}
function Wrn(e) {
  return (
    JSON.stringify(e) +
    `
`
  );
}
function Dt(e) {
  if (!e) return {};
  if (e instanceof Headers) return Object.fromEntries(e.entries());
  if (Array.isArray(e)) return Object.fromEntries(e);
  return { ...e };
}
function $We(e = fetch, t) {
  if (!t) return e;
  return async (r, s) => e(r, { ...t, ...s, headers: s?.headers ? { ...Dt(t.headers), ...Dt(s.headers) } : t.headers });
}
var br = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }),
      (e.regexpCode =
        e.getEsmExportName =
        e.getProperty =
        e.safeStringify =
        e.stringify =
        e.strConcat =
        e.addCodeArg =
        e.str =
        e._ =
        e.nil =
        e._Code =
        e.Name =
        e.IDENTIFIER =
        e._CodeOrName =
          void 0);
    var t = class {};
    (e._CodeOrName = t), (e.IDENTIFIER = /^[a-z$_][a-z$_0-9]*$/i);
    var r = class extends t {
      constructor(u) {
        super();
        if (!e.IDENTIFIER.test(u)) throw Error("CodeGen: name must be a valid identifier");
        this.str = u;
      }
      toString() {
        return this.str;
      }
      emptyStr() {
        return !1;
      }
      get names() {
        return { [this.str]: 1 };
      }
    };
    e.Name = r;
    var s = class extends t {
      constructor(u) {
        super();
        this._items = typeof u === "string" ? [u] : u;
      }
      toString() {
        return this.str;
      }
      emptyStr() {
        if (this._items.length > 1) return !1;
        let u = this._items[0];
        return u === "" || u === '""';
      }
      get str() {
        var u;
        return (u = this._str) !== null && u !== void 0
          ? u
          : (this._str = this._items.reduce((p, m) => `${p}${m}`, ""));
      }
      get names() {
        var u;
        return (u = this._names) !== null && u !== void 0
          ? u
          : (this._names = this._items.reduce((p, m) => {
              if (m instanceof r) p[m.str] = (p[m.str] || 0) + 1;
              return p;
            }, {}));
      }
    };
    (e._Code = s), (e.nil = new s(""));
    function a(u, ...p) {
      let m = [u[0]],
        y = 0;
      while (y < p.length) c(m, p[y]), m.push(u[++y]);
      return new s(m);
    }
    e._ = a;
    let o = new s("+");
    function n(u, ...p) {
      let m = [w(u[0])],
        y = 0;
      while (y < p.length) m.push(o), c(m, p[y]), m.push(o, w(u[++y]));
      return i(m), new s(m);
    }
    e.str = n;
    function c(u, p) {
      if (p instanceof s) u.push(...p._items);
      else if (p instanceof r) u.push(p);
      else u.push(S(p));
    }
    e.addCodeArg = c;
    function i(u) {
      let p = 1;
      while (p < u.length - 1) {
        if (u[p] === o) {
          let m = l(u[p - 1], u[p + 1]);
          if (m !== void 0) {
            u.splice(p - 1, 3, m);
            continue;
          }
          u[p++] = "+";
        }
        p++;
      }
    }
    function l(u, p) {
      if (p === '""') return u;
      if (u === '""') return p;
      if (typeof u == "string") {
        if (p instanceof r || u[u.length - 1] !== '"') return;
        if (typeof p != "string") return `${u.slice(0, -1)}${p}"`;
        if (p[0] === '"') return u.slice(0, -1) + p.slice(1);
        return;
      }
      if (typeof p == "string" && p[0] === '"' && !(u instanceof r)) return `"${u}${p.slice(1)}`;
    }
    function d(u, p) {
      return p.emptyStr() ? u : u.emptyStr() ? p : n`${u}${p}`;
    }
    e.strConcat = d;
    function S(u) {
      return typeof u == "number" || typeof u == "boolean" || u === null ? u : w(Array.isArray(u) ? u.join(",") : u);
    }
    function R(u) {
      return new s(w(u));
    }
    e.stringify = R;
    function w(u) {
      return JSON.stringify(u)
        .replace(/\u2028/g, "\\u2028")
        .replace(/\u2029/g, "\\u2029");
    }
    e.safeStringify = w;
    function g(u) {
      return typeof u == "string" && e.IDENTIFIER.test(u) ? new s(`.${u}`) : a`[${u}]`;
    }
    e.getProperty = g;
    function _(u) {
      if (typeof u == "string" && e.IDENTIFIER.test(u)) return new s(`${u}`);
      throw Error(`CodeGen: invalid export name: ${u}, use explicit $id name mapping`);
    }
    e.getEsmExportName = _;
    function h(u) {
      return new s(u.toString());
    }
    e.regexpCode = h;
  }),
  ba = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }),
      (e.ValueScope = e.ValueScopeName = e.Scope = e.varKinds = e.UsedValueState = void 0);
    let t = br();
    var r = class extends Error {
        constructor(i) {
          super(`CodeGen: "code" for ${i} not defined`);
          this.value = i.value;
        }
      },
      s;
    (function (i) {
      (i[(i.Started = 0)] = "Started"), (i[(i.Completed = 1)] = "Completed");
    })(s || (e.UsedValueState = s = {})),
      (e.varKinds = { const: new t.Name("const"), let: new t.Name("let"), var: new t.Name("var") });
    var a = class {
      constructor({ prefixes: i, parent: l } = {}) {
        (this._names = {}), (this._prefixes = i), (this._parent = l);
      }
      toName(i) {
        return i instanceof t.Name ? i : this.name(i);
      }
      name(i) {
        return new t.Name(this._newName(i));
      }
      _newName(i) {
        let l = this._names[i] || this._nameGroup(i);
        return `${i}${l.index++}`;
      }
      _nameGroup(i) {
        var l, d;
        if (
          ((d = (l = this._parent) === null || l === void 0 ? void 0 : l._prefixes) === null || d === void 0
            ? void 0
            : d.has(i)) ||
          (this._prefixes && !this._prefixes.has(i))
        )
          throw Error(`CodeGen: prefix "${i}" is not allowed in this scope`);
        return (this._names[i] = { prefix: i, index: 0 });
      }
    };
    e.Scope = a;
    var o = class extends t.Name {
      constructor(i, l) {
        super(l);
        this.prefix = i;
      }
      setValue(i, { property: l, itemIndex: d }) {
        (this.value = i), (this.scopePath = t._`.${new t.Name(l)}[${d}]`);
      }
    };
    e.ValueScopeName = o;
    let n = t._`\n`;
    var c = class extends a {
      constructor(i) {
        super(i);
        (this._values = {}), (this._scope = i.scope), (this.opts = { ...i, _n: i.lines ? n : t.nil });
      }
      get() {
        return this._scope;
      }
      name(i) {
        return new o(i, this._newName(i));
      }
      value(i, l) {
        var d;
        if (l.ref === void 0) throw Error("CodeGen: ref must be passed in value");
        let S = this.toName(i),
          { prefix: R } = S,
          w = (d = l.key) !== null && d !== void 0 ? d : l.ref,
          g = this._values[R];
        if (g) {
          let u = g.get(w);
          if (u) return u;
        } else g = this._values[R] = new Map();
        g.set(w, S);
        let _ = this._scope[R] || (this._scope[R] = []),
          h = _.length;
        return (_[h] = l.ref), S.setValue(l, { property: R, itemIndex: h }), S;
      }
      getValue(i, l) {
        let d = this._values[i];
        if (!d) return;
        return d.get(l);
      }
      scopeRefs(i, l = this._values) {
        return this._reduceValues(l, (d) => {
          if (d.scopePath === void 0) throw Error(`CodeGen: name "${d}" has no value`);
          return t._`${i}${d.scopePath}`;
        });
      }
      scopeCode(i = this._values, l, d) {
        return this._reduceValues(
          i,
          (S) => {
            if (S.value === void 0) throw Error(`CodeGen: name "${S}" has no value`);
            return S.value.code;
          },
          l,
          d,
        );
      }
      _reduceValues(i, l, d = {}, S) {
        let R = t.nil;
        for (let w in i) {
          let g = i[w];
          if (!g) continue;
          let _ = (d[w] = d[w] || new Map());
          g.forEach((h) => {
            if (_.has(h)) return;
            _.set(h, s.Started);
            let u = l(h);
            if (u) {
              let p = this.opts.es5 ? e.varKinds.var : e.varKinds.const;
              R = t._`${R}${p} ${h} = ${u};${this.opts._n}`;
            } else if ((u = S === null || S === void 0 ? void 0 : S(h))) R = t._`${R}${u}${this.opts._n}`;
            else throw new r(h);
            _.set(h, s.Completed);
          });
        }
        return R;
      }
    };
    e.ValueScope = c;
  }),
  B = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }),
      (e.or =
        e.and =
        e.not =
        e.CodeGen =
        e.operators =
        e.varKinds =
        e.ValueScopeName =
        e.ValueScope =
        e.Scope =
        e.Name =
        e.regexpCode =
        e.stringify =
        e.getProperty =
        e.nil =
        e.strConcat =
        e.str =
        e._ =
          void 0);
    let t = br(),
      r = ba();
    var s = br();
    Object.defineProperty(e, "_", {
      enumerable: !0,
      get: function () {
        return s._;
      },
    }),
      Object.defineProperty(e, "str", {
        enumerable: !0,
        get: function () {
          return s.str;
        },
      }),
      Object.defineProperty(e, "strConcat", {
        enumerable: !0,
        get: function () {
          return s.strConcat;
        },
      }),
      Object.defineProperty(e, "nil", {
        enumerable: !0,
        get: function () {
          return s.nil;
        },
      }),
      Object.defineProperty(e, "getProperty", {
        enumerable: !0,
        get: function () {
          return s.getProperty;
        },
      }),
      Object.defineProperty(e, "stringify", {
        enumerable: !0,
        get: function () {
          return s.stringify;
        },
      }),
      Object.defineProperty(e, "regexpCode", {
        enumerable: !0,
        get: function () {
          return s.regexpCode;
        },
      }),
      Object.defineProperty(e, "Name", {
        enumerable: !0,
        get: function () {
          return s.Name;
        },
      });
    var a = ba();
    Object.defineProperty(e, "Scope", {
      enumerable: !0,
      get: function () {
        return a.Scope;
      },
    }),
      Object.defineProperty(e, "ValueScope", {
        enumerable: !0,
        get: function () {
          return a.ValueScope;
        },
      }),
      Object.defineProperty(e, "ValueScopeName", {
        enumerable: !0,
        get: function () {
          return a.ValueScopeName;
        },
      }),
      Object.defineProperty(e, "varKinds", {
        enumerable: !0,
        get: function () {
          return a.varKinds;
        },
      }),
      (e.operators = {
        GT: new t._Code(">"),
        GTE: new t._Code(">="),
        LT: new t._Code("<"),
        LTE: new t._Code("<="),
        EQ: new t._Code("==="),
        NEQ: new t._Code("!=="),
        NOT: new t._Code("!"),
        OR: new t._Code("||"),
        AND: new t._Code("&&"),
        ADD: new t._Code("+"),
      });
    var o = class {
        optimizeNodes() {
          return this;
        }
        optimizeNames(b, z) {
          return this;
        }
      },
      n = class extends o {
        constructor(b, z, q) {
          super();
          (this.varKind = b), (this.name = z), (this.rhs = q);
        }
        render({ es5: b, _n: z }) {
          let q = b ? r.varKinds.var : this.varKind,
            D = this.rhs === void 0 ? "" : ` = ${this.rhs}`;
          return `${q} ${this.name}${D};` + z;
        }
        optimizeNames(b, z) {
          if (!b[this.name.str]) return;
          if (this.rhs) this.rhs = V(this.rhs, b, z);
          return this;
        }
        get names() {
          return this.rhs instanceof t._CodeOrName ? this.rhs.names : {};
        }
      },
      c = class extends o {
        constructor(b, z, q) {
          super();
          (this.lhs = b), (this.rhs = z), (this.sideEffects = q);
        }
        render({ _n: b }) {
          return `${this.lhs} = ${this.rhs};` + b;
        }
        optimizeNames(b, z) {
          if (this.lhs instanceof t.Name && !b[this.lhs.str] && !this.sideEffects) return;
          return (this.rhs = V(this.rhs, b, z)), this;
        }
        get names() {
          return F(this.lhs instanceof t.Name ? {} : { ...this.lhs.names }, this.rhs);
        }
      },
      i = class extends c {
        constructor(b, z, q, D) {
          super(b, q, D);
          this.op = z;
        }
        render({ _n: b }) {
          return `${this.lhs} ${this.op}= ${this.rhs};` + b;
        }
      },
      l = class extends o {
        constructor(b) {
          super();
          (this.label = b), (this.names = {});
        }
        render({ _n: b }) {
          return `${this.label}:` + b;
        }
      },
      d = class extends o {
        constructor(b) {
          super();
          (this.label = b), (this.names = {});
        }
        render({ _n: b }) {
          return `break${this.label ? ` ${this.label}` : ""};` + b;
        }
      },
      S = class extends o {
        constructor(b) {
          super();
          this.error = b;
        }
        render({ _n: b }) {
          return `throw ${this.error};` + b;
        }
        get names() {
          return this.error.names;
        }
      },
      R = class extends o {
        constructor(b) {
          super();
          this.code = b;
        }
        render({ _n: b }) {
          return `${this.code};` + b;
        }
        optimizeNodes() {
          return `${this.code}` ? this : void 0;
        }
        optimizeNames(b, z) {
          return (this.code = V(this.code, b, z)), this;
        }
        get names() {
          return this.code instanceof t._CodeOrName ? this.code.names : {};
        }
      },
      w = class extends o {
        constructor(b = []) {
          super();
          this.nodes = b;
        }
        render(b) {
          return this.nodes.reduce((z, q) => z + q.render(b), "");
        }
        optimizeNodes() {
          let { nodes: b } = this,
            z = b.length;
          while (z--) {
            let q = b[z].optimizeNodes();
            if (Array.isArray(q)) b.splice(z, 1, ...q);
            else if (q) b[z] = q;
            else b.splice(z, 1);
          }
          return b.length > 0 ? this : void 0;
        }
        optimizeNames(b, z) {
          let { nodes: q } = this,
            D = q.length;
          while (D--) {
            let H = q[D];
            if (H.optimizeNames(b, z)) continue;
            Y(b, H.names), q.splice(D, 1);
          }
          return q.length > 0 ? this : void 0;
        }
        get names() {
          return this.nodes.reduce((b, z) => j(b, z.names), {});
        }
      },
      g = class extends w {
        render(b) {
          return "{" + b._n + super.render(b) + "}" + b._n;
        }
      },
      _ = class extends w {},
      h = class extends g {};
    h.kind = "else";
    var u = class b extends g {
      constructor(z, q) {
        super(q);
        this.condition = z;
      }
      render(z) {
        let q = `if(${this.condition})` + super.render(z);
        if (this.else) q += "else " + this.else.render(z);
        return q;
      }
      optimizeNodes() {
        super.optimizeNodes();
        let z = this.condition;
        if (z === !0) return this.nodes;
        let q = this.else;
        if (q) {
          let D = q.optimizeNodes();
          q = this.else = Array.isArray(D) ? new h(D) : D;
        }
        if (q) {
          if (z === !1) return q instanceof b ? q : q.nodes;
          if (this.nodes.length) return this;
          return new b(te(z), q instanceof b ? [q] : q.nodes);
        }
        if (z === !1 || !this.nodes.length) return;
        return this;
      }
      optimizeNames(z, q) {
        var D;
        if (
          ((this.else = (D = this.else) === null || D === void 0 ? void 0 : D.optimizeNames(z, q)),
          !(super.optimizeNames(z, q) || this.else))
        )
          return;
        return (this.condition = V(this.condition, z, q)), this;
      }
      get names() {
        let z = super.names;
        if ((F(z, this.condition), this.else)) j(z, this.else.names);
        return z;
      }
    };
    u.kind = "if";
    var p = class extends g {};
    p.kind = "for";
    var m = class extends p {
        constructor(b) {
          super();
          this.iteration = b;
        }
        render(b) {
          return `for(${this.iteration})` + super.render(b);
        }
        optimizeNames(b, z) {
          if (!super.optimizeNames(b, z)) return;
          return (this.iteration = V(this.iteration, b, z)), this;
        }
        get names() {
          return j(super.names, this.iteration.names);
        }
      },
      y = class extends p {
        constructor(b, z, q, D) {
          super();
          (this.varKind = b), (this.name = z), (this.from = q), (this.to = D);
        }
        render(b) {
          let z = b.es5 ? r.varKinds.var : this.varKind,
            { name: q, from: D, to: H } = this;
          return `for(${z} ${q}=${D}; ${q}<${H}; ${q}++)` + super.render(b);
        }
        get names() {
          return F(F(super.names, this.from), this.to);
        }
      },
      E = class extends p {
        constructor(b, z, q, D) {
          super();
          (this.loop = b), (this.varKind = z), (this.name = q), (this.iterable = D);
        }
        render(b) {
          return `for(${this.varKind} ${this.name} ${this.loop} ${this.iterable})` + super.render(b);
        }
        optimizeNames(b, z) {
          if (!super.optimizeNames(b, z)) return;
          return (this.iterable = V(this.iterable, b, z)), this;
        }
        get names() {
          return j(super.names, this.iterable.names);
        }
      },
      v = class extends g {
        constructor(b, z, q) {
          super();
          (this.name = b), (this.args = z), (this.async = q);
        }
        render(b) {
          return `${this.async ? "async " : ""}function ${this.name}(${this.args})` + super.render(b);
        }
      };
    v.kind = "func";
    var f = class extends w {
      render(b) {
        return "return " + super.render(b);
      }
    };
    f.kind = "return";
    var P = class extends g {
        render(b) {
          let z = "try" + super.render(b);
          if (this.catch) z += this.catch.render(b);
          if (this.finally) z += this.finally.render(b);
          return z;
        }
        optimizeNodes() {
          var b, z;
          return (
            super.optimizeNodes(),
            (b = this.catch) === null || b === void 0 || b.optimizeNodes(),
            (z = this.finally) === null || z === void 0 || z.optimizeNodes(),
            this
          );
        }
        optimizeNames(b, z) {
          var q, D;
          return (
            super.optimizeNames(b, z),
            (q = this.catch) === null || q === void 0 || q.optimizeNames(b, z),
            (D = this.finally) === null || D === void 0 || D.optimizeNames(b, z),
            this
          );
        }
        get names() {
          let b = super.names;
          if (this.catch) j(b, this.catch.names);
          if (this.finally) j(b, this.finally.names);
          return b;
        }
      },
      I = class extends g {
        constructor(b) {
          super();
          this.error = b;
        }
        render(b) {
          return `catch(${this.error})` + super.render(b);
        }
      };
    I.kind = "catch";
    var L = class extends g {
      render(b) {
        return "finally" + super.render(b);
      }
    };
    L.kind = "finally";
    var k = class {
      constructor(b, z = {}) {
        (this._values = {}),
          (this._blockStarts = []),
          (this._constants = {}),
          (this.opts = {
            ...z,
            _n: z.lines
              ? `
`
              : "",
          }),
          (this._extScope = b),
          (this._scope = new r.Scope({ parent: b })),
          (this._nodes = [new _()]);
      }
      toString() {
        return this._root.render(this.opts);
      }
      name(b) {
        return this._scope.name(b);
      }
      scopeName(b) {
        return this._extScope.name(b);
      }
      scopeValue(b, z) {
        let q = this._extScope.value(b, z);
        return (this._values[q.prefix] || (this._values[q.prefix] = new Set())).add(q), q;
      }
      getScopeValue(b, z) {
        return this._extScope.getValue(b, z);
      }
      scopeRefs(b) {
        return this._extScope.scopeRefs(b, this._values);
      }
      scopeCode() {
        return this._extScope.scopeCode(this._values);
      }
      _def(b, z, q, D) {
        let H = this._scope.toName(z);
        if (q !== void 0 && D) this._constants[H.str] = q;
        return this._leafNode(new n(b, H, q)), H;
      }
      const(b, z, q) {
        return this._def(r.varKinds.const, b, z, q);
      }
      let(b, z, q) {
        return this._def(r.varKinds.let, b, z, q);
      }
      var(b, z, q) {
        return this._def(r.varKinds.var, b, z, q);
      }
      assign(b, z, q) {
        return this._leafNode(new c(b, z, q));
      }
      add(b, z) {
        return this._leafNode(new i(b, e.operators.ADD, z));
      }
      code(b) {
        if (typeof b == "function") b();
        else if (b !== t.nil) this._leafNode(new R(b));
        return this;
      }
      object(...b) {
        let z = ["{"];
        for (let [q, D] of b) {
          if (z.length > 1) z.push(",");
          if ((z.push(q), q !== D || this.opts.es5)) z.push(":"), (0, t.addCodeArg)(z, D);
        }
        return z.push("}"), new t._Code(z);
      }
      if(b, z, q) {
        if ((this._blockNode(new u(b)), z && q)) this.code(z).else().code(q).endIf();
        else if (z) this.code(z).endIf();
        else if (q) throw Error('CodeGen: "else" body without "then" body');
        return this;
      }
      elseIf(b) {
        return this._elseNode(new u(b));
      }
      else() {
        return this._elseNode(new h());
      }
      endIf() {
        return this._endBlockNode(u, h);
      }
      _for(b, z) {
        if ((this._blockNode(b), z)) this.code(z).endFor();
        return this;
      }
      for(b, z) {
        return this._for(new m(b), z);
      }
      forRange(b, z, q, D, H = this.opts.es5 ? r.varKinds.var : r.varKinds.let) {
        let W = this._scope.toName(b);
        return this._for(new y(H, W, z, q), () => D(W));
      }
      forOf(b, z, q, D = r.varKinds.const) {
        let H = this._scope.toName(b);
        if (this.opts.es5) {
          let W = z instanceof t.Name ? z : this.var("_arr", z);
          return this.forRange("_i", 0, t._`${W}.length`, (J) => {
            this.var(H, t._`${W}[${J}]`), q(H);
          });
        }
        return this._for(new E("of", D, H, z), () => q(H));
      }
      forIn(b, z, q, D = this.opts.es5 ? r.varKinds.var : r.varKinds.const) {
        if (this.opts.ownProperties) return this.forOf(b, t._`Object.keys(${z})`, q);
        let H = this._scope.toName(b);
        return this._for(new E("in", D, H, z), () => q(H));
      }
      endFor() {
        return this._endBlockNode(p);
      }
      label(b) {
        return this._leafNode(new l(b));
      }
      break(b) {
        return this._leafNode(new d(b));
      }
      return(b) {
        let z = new f();
        if ((this._blockNode(z), this.code(b), z.nodes.length !== 1))
          throw Error('CodeGen: "return" should have one node');
        return this._endBlockNode(f);
      }
      try(b, z, q) {
        if (!z && !q) throw Error('CodeGen: "try" without "catch" and "finally"');
        let D = new P();
        if ((this._blockNode(D), this.code(b), z)) {
          let H = this.name("e");
          (this._currNode = D.catch = new I(H)), z(H);
        }
        if (q) (this._currNode = D.finally = new L()), this.code(q);
        return this._endBlockNode(I, L);
      }
      throw(b) {
        return this._leafNode(new S(b));
      }
      block(b, z) {
        if ((this._blockStarts.push(this._nodes.length), b)) this.code(b).endBlock(z);
        return this;
      }
      endBlock(b) {
        let z = this._blockStarts.pop();
        if (z === void 0) throw Error("CodeGen: not in self-balancing block");
        let q = this._nodes.length - z;
        if (q < 0 || (b !== void 0 && q !== b)) throw Error(`CodeGen: wrong number of nodes: ${q} vs ${b} expected`);
        return (this._nodes.length = z), this;
      }
      func(b, z = t.nil, q, D) {
        if ((this._blockNode(new v(b, z, q)), D)) this.code(D).endFunc();
        return this;
      }
      endFunc() {
        return this._endBlockNode(v);
      }
      optimize(b = 1) {
        while (b-- > 0) this._root.optimizeNodes(), this._root.optimizeNames(this._root.names, this._constants);
      }
      _leafNode(b) {
        return this._currNode.nodes.push(b), this;
      }
      _blockNode(b) {
        this._currNode.nodes.push(b), this._nodes.push(b);
      }
      _endBlockNode(b, z) {
        let q = this._currNode;
        if (q instanceof b || (z && q instanceof z)) return this._nodes.pop(), this;
        throw Error(`CodeGen: not in block "${z ? `${b.kind}/${z.kind}` : b.kind}"`);
      }
      _elseNode(b) {
        let z = this._currNode;
        if (!(z instanceof u)) throw Error('CodeGen: "else" without "if"');
        return (this._currNode = z.else = b), this;
      }
      get _root() {
        return this._nodes[0];
      }
      get _currNode() {
        let b = this._nodes;
        return b[b.length - 1];
      }
      set _currNode(b) {
        let z = this._nodes;
        z[z.length - 1] = b;
      }
    };
    e.CodeGen = k;
    function j(b, z) {
      for (let q in z) b[q] = (b[q] || 0) + (z[q] || 0);
      return b;
    }
    function F(b, z) {
      return z instanceof t._CodeOrName ? j(b, z.names) : b;
    }
    function V(b, z, q) {
      if (b instanceof t.Name) return D(b);
      if (!H(b)) return b;
      return new t._Code(
        b._items.reduce((W, J) => {
          if (J instanceof t.Name) J = D(J);
          if (J instanceof t._Code) W.push(...J._items);
          else W.push(J);
          return W;
        }, []),
      );
      function D(W) {
        let J = q[W.str];
        if (J === void 0 || z[W.str] !== 1) return W;
        return delete z[W.str], J;
      }
      function H(W) {
        return (
          W instanceof t._Code && W._items.some((J) => J instanceof t.Name && z[J.str] === 1 && q[J.str] !== void 0)
        );
      }
    }
    function Y(b, z) {
      for (let q in z) b[q] = (b[q] || 0) - (z[q] || 0);
    }
    function te(b) {
      return typeof b == "boolean" || typeof b == "number" || b === null ? !b : t._`!${A(b)}`;
    }
    e.not = te;
    let Q = O(e.operators.AND);
    function oe(...b) {
      return b.reduce(Q);
    }
    e.and = oe;
    let X = O(e.operators.OR);
    function C(...b) {
      return b.reduce(X);
    }
    e.or = C;
    function O(b) {
      return (z, q) => (z === t.nil ? q : q === t.nil ? z : t._`${A(z)} ${b} ${A(q)}`);
    }
    function A(b) {
      return b instanceof t.Name ? b : t._`(${b})`;
    }
  }),
  Z = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }),
      (e.checkStrictMode =
        e.getErrorPath =
        e.Type =
        e.useFunc =
        e.setEvaluated =
        e.evaluatedPropsToName =
        e.mergeEvaluated =
        e.eachItem =
        e.unescapeJsonPointer =
        e.escapeJsonPointer =
        e.escapeFragment =
        e.unescapeFragment =
        e.schemaRefOrVal =
        e.schemaHasRulesButRef =
        e.schemaHasRules =
        e.checkUnknownRules =
        e.alwaysValidSchema =
        e.toHash =
          void 0);
    let t = B(),
      r = br();
    function s(v) {
      let f = {};
      for (let P of v) f[P] = !0;
      return f;
    }
    e.toHash = s;
    function a(v, f) {
      if (typeof f == "boolean") return f;
      if (Object.keys(f).length === 0) return !0;
      return o(v, f), !n(f, v.self.RULES.all);
    }
    e.alwaysValidSchema = a;
    function o(v, f = v.schema) {
      let { opts: P, self: I } = v;
      if (!P.strictSchema) return;
      if (typeof f === "boolean") return;
      let L = I.RULES.keywords;
      for (let k in f) if (!L[k]) E(v, `unknown keyword: "${k}"`);
    }
    e.checkUnknownRules = o;
    function n(v, f) {
      if (typeof v == "boolean") return !v;
      for (let P in v) if (f[P]) return !0;
      return !1;
    }
    e.schemaHasRules = n;
    function c(v, f) {
      if (typeof v == "boolean") return !v;
      for (let P in v) if (P !== "$ref" && f.all[P]) return !0;
      return !1;
    }
    e.schemaHasRulesButRef = c;
    function i({ topSchemaRef: v, schemaPath: f }, P, I, L) {
      if (!L) {
        if (typeof P == "number" || typeof P == "boolean") return P;
        if (typeof P == "string") return t._`${P}`;
      }
      return t._`${v}${f}${(0, t.getProperty)(I)}`;
    }
    e.schemaRefOrVal = i;
    function l(v) {
      return R(decodeURIComponent(v));
    }
    e.unescapeFragment = l;
    function d(v) {
      return encodeURIComponent(S(v));
    }
    e.escapeFragment = d;
    function S(v) {
      if (typeof v == "number") return `${v}`;
      return v.replace(/~/g, "~0").replace(/\//g, "~1");
    }
    e.escapeJsonPointer = S;
    function R(v) {
      return v.replace(/~1/g, "/").replace(/~0/g, "~");
    }
    e.unescapeJsonPointer = R;
    function w(v, f) {
      if (Array.isArray(v)) for (let P of v) f(P);
      else f(v);
    }
    e.eachItem = w;
    function g({ mergeNames: v, mergeToName: f, mergeValues: P, resultToName: I }) {
      return (L, k, j, F) => {
        let V =
          j === void 0
            ? k
            : j instanceof t.Name
              ? (k instanceof t.Name ? v(L, k, j) : f(L, k, j), j)
              : k instanceof t.Name
                ? (f(L, j, k), k)
                : P(k, j);
        return F === t.Name && !(V instanceof t.Name) ? I(L, V) : V;
      };
    }
    e.mergeEvaluated = {
      props: g({
        mergeNames: (v, f, P) =>
          v.if(t._`${P} !== true && ${f} !== undefined`, () => {
            v.if(
              t._`${f} === true`,
              () => v.assign(P, !0),
              () => v.assign(P, t._`${P} || {}`).code(t._`Object.assign(${P}, ${f})`),
            );
          }),
        mergeToName: (v, f, P) =>
          v.if(t._`${P} !== true`, () => {
            if (f === !0) v.assign(P, !0);
            else v.assign(P, t._`${P} || {}`), h(v, P, f);
          }),
        mergeValues: (v, f) => (v === !0 ? !0 : { ...v, ...f }),
        resultToName: _,
      }),
      items: g({
        mergeNames: (v, f, P) =>
          v.if(t._`${P} !== true && ${f} !== undefined`, () =>
            v.assign(P, t._`${f} === true ? true : ${P} > ${f} ? ${P} : ${f}`),
          ),
        mergeToName: (v, f, P) =>
          v.if(t._`${P} !== true`, () => v.assign(P, f === !0 ? !0 : t._`${P} > ${f} ? ${P} : ${f}`)),
        mergeValues: (v, f) => (v === !0 ? !0 : Math.max(v, f)),
        resultToName: (v, f) => v.var("items", f),
      }),
    };
    function _(v, f) {
      if (f === !0) return v.var("props", !0);
      let P = v.var("props", t._`{}`);
      if (f !== void 0) h(v, P, f);
      return P;
    }
    e.evaluatedPropsToName = _;
    function h(v, f, P) {
      Object.keys(P).forEach((I) => v.assign(t._`${f}${(0, t.getProperty)(I)}`, !0));
    }
    e.setEvaluated = h;
    let u = {};
    function p(v, f) {
      return v.scopeValue("func", { ref: f, code: u[f.code] || (u[f.code] = new r._Code(f.code)) });
    }
    e.useFunc = p;
    var m;
    (function (v) {
      (v[(v.Num = 0)] = "Num"), (v[(v.Str = 1)] = "Str");
    })(m || (e.Type = m = {}));
    function y(v, f, P) {
      if (v instanceof t.Name) {
        let I = f === m.Num;
        return P
          ? I
            ? t._`"[" + ${v} + "]"`
            : t._`"['" + ${v} + "']"`
          : I
            ? t._`"/" + ${v}`
            : t._`"/" + ${v}.replace(/~/g, "~0").replace(/\\//g, "~1")`;
      }
      return P ? (0, t.getProperty)(v).toString() : "/" + S(v);
    }
    e.getErrorPath = y;
    function E(v, f, P = v.opts.strictSchema) {
      if (!P) return;
      if (((f = `strict mode: ${f}`), P === !0)) throw Error(f);
      v.self.logger.warn(f);
    }
    e.checkStrictMode = E;
  }),
  we = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = B(),
      r = {
        data: new t.Name("data"),
        valCxt: new t.Name("valCxt"),
        instancePath: new t.Name("instancePath"),
        parentData: new t.Name("parentData"),
        parentDataProperty: new t.Name("parentDataProperty"),
        rootData: new t.Name("rootData"),
        dynamicAnchors: new t.Name("dynamicAnchors"),
        vErrors: new t.Name("vErrors"),
        errors: new t.Name("errors"),
        this: new t.Name("this"),
        self: new t.Name("self"),
        scope: new t.Name("scope"),
        json: new t.Name("json"),
        jsonPos: new t.Name("jsonPos"),
        jsonLen: new t.Name("jsonLen"),
        jsonPart: new t.Name("jsonPart"),
      };
    e.default = r;
  }),
  wr = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }),
      (e.extendErrors =
        e.resetErrorsCount =
        e.reportExtraError =
        e.reportError =
        e.keyword$DataError =
        e.keywordError =
          void 0);
    let t = B(),
      r = Z(),
      s = we();
    (e.keywordError = { message: ({ keyword: h }) => t.str`must pass "${h}" keyword validation` }),
      (e.keyword$DataError = {
        message: ({ keyword: h, schemaType: u }) =>
          u ? t.str`"${h}" keyword must be ${u} ($data)` : t.str`"${h}" keyword is invalid ($data)`,
      });
    function a(h, u = e.keywordError, p, m) {
      let { it: y } = h,
        { gen: E, compositeRule: v, allErrors: f } = y,
        P = S(h, u, p);
      if (m !== null && m !== void 0 ? m : v || f) i(E, P);
      else l(y, t._`[${P}]`);
    }
    e.reportError = a;
    function o(h, u = e.keywordError, p) {
      let { it: m } = h,
        { gen: y, compositeRule: E, allErrors: v } = m;
      if ((i(y, S(h, u, p)), !(E || v))) l(m, s.default.vErrors);
    }
    e.reportExtraError = o;
    function n(h, u) {
      h.assign(s.default.errors, u),
        h.if(t._`${s.default.vErrors} !== null`, () =>
          h.if(
            u,
            () => h.assign(t._`${s.default.vErrors}.length`, u),
            () => h.assign(s.default.vErrors, null),
          ),
        );
    }
    e.resetErrorsCount = n;
    function c({ gen: h, keyword: u, schemaValue: p, data: m, errsCount: y, it: E }) {
      if (y === void 0) throw Error("ajv implementation error");
      let v = h.name("err");
      h.forRange("i", y, s.default.errors, (f) => {
        if (
          (h.const(v, t._`${s.default.vErrors}[${f}]`),
          h.if(t._`${v}.instancePath === undefined`, () =>
            h.assign(t._`${v}.instancePath`, (0, t.strConcat)(s.default.instancePath, E.errorPath)),
          ),
          h.assign(t._`${v}.schemaPath`, t.str`${E.errSchemaPath}/${u}`),
          E.opts.verbose)
        )
          h.assign(t._`${v}.schema`, p), h.assign(t._`${v}.data`, m);
      });
    }
    e.extendErrors = c;
    function i(h, u) {
      let p = h.const("err", u);
      h.if(
        t._`${s.default.vErrors} === null`,
        () => h.assign(s.default.vErrors, t._`[${p}]`),
        t._`${s.default.vErrors}.push(${p})`,
      ),
        h.code(t._`${s.default.errors}++`);
    }
    function l(h, u) {
      let { gen: p, validateName: m, schemaEnv: y } = h;
      if (y.$async) p.throw(t._`new ${h.ValidationError}(${u})`);
      else p.assign(t._`${m}.errors`, u), p.return(!1);
    }
    let d = {
      keyword: new t.Name("keyword"),
      schemaPath: new t.Name("schemaPath"),
      params: new t.Name("params"),
      propertyName: new t.Name("propertyName"),
      message: new t.Name("message"),
      schema: new t.Name("schema"),
      parentSchema: new t.Name("parentSchema"),
    };
    function S(h, u, p) {
      let { createErrors: m } = h.it;
      if (m === !1) return t._`{}`;
      return R(h, u, p);
    }
    function R(h, u, p = {}) {
      let { gen: m, it: y } = h,
        E = [w(y, p), g(h, p)];
      return _(h, u, E), m.object(...E);
    }
    function w({ errorPath: h }, { instancePath: u }) {
      let p = u ? t.str`${h}${(0, r.getErrorPath)(u, r.Type.Str)}` : h;
      return [s.default.instancePath, (0, t.strConcat)(s.default.instancePath, p)];
    }
    function g({ keyword: h, it: { errSchemaPath: u } }, { schemaPath: p, parentSchema: m }) {
      let y = m ? u : t.str`${u}/${h}`;
      if (p) y = t.str`${y}${(0, r.getErrorPath)(p, r.Type.Str)}`;
      return [d.schemaPath, y];
    }
    function _(h, { params: u, message: p }, m) {
      let { keyword: y, data: E, schemaValue: v, it: f } = h,
        { opts: P, propertyName: I, topSchemaRef: L, schemaPath: k } = f;
      if ((m.push([d.keyword, y], [d.params, typeof u == "function" ? u(h) : u || t._`{}`]), P.messages))
        m.push([d.message, typeof p == "function" ? p(h) : p]);
      if (P.verbose) m.push([d.schema, v], [d.parentSchema, t._`${L}${k}`], [s.default.data, E]);
      if (I) m.push([d.propertyName, I]);
    }
  }),
  oi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }), (e.boolOrEmptySchema = e.topBoolOrEmptySchema = void 0);
    let t = wr(),
      r = B(),
      s = we(),
      a = { message: "boolean schema is false" };
    function o(i) {
      let { gen: l, schema: d, validateName: S } = i;
      if (d === !1) c(i, !1);
      else if (typeof d == "object" && d.$async === !0) l.return(s.default.data);
      else l.assign(r._`${S}.errors`, null), l.return(!0);
    }
    e.topBoolOrEmptySchema = o;
    function n(i, l) {
      let { gen: d, schema: S } = i;
      if (S === !1) d.var(l, !1), c(i);
      else d.var(l, !0);
    }
    e.boolOrEmptySchema = n;
    function c(i, l) {
      let { gen: d, data: S } = i,
        R = {
          gen: d,
          keyword: "false schema",
          data: S,
          schema: !1,
          schemaCode: !1,
          schemaValue: !1,
          params: {},
          it: i,
        };
      (0, t.reportError)(R, a, void 0, l);
    }
  }),
  Ra = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }), (e.getRules = e.isJSONType = void 0);
    let t = new Set(["string", "number", "integer", "boolean", "null", "object", "array"]);
    function r(a) {
      return typeof a == "string" && t.has(a);
    }
    e.isJSONType = r;
    function s() {
      let a = {
        number: { type: "number", rules: [] },
        string: { type: "string", rules: [] },
        array: { type: "array", rules: [] },
        object: { type: "object", rules: [] },
      };
      return {
        types: { ...a, integer: !0, boolean: !0, null: !0 },
        rules: [{ rules: [] }, a.number, a.string, a.array, a.object],
        post: { rules: [] },
        all: {},
        keywords: {},
      };
    }
    e.getRules = s;
  }),
  $a = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }),
      (e.shouldUseRule = e.shouldUseGroup = e.schemaHasRulesForType = void 0);
    function t({ schema: a, self: o }, n) {
      let c = o.RULES.types[n];
      return c && c !== !0 && r(a, c);
    }
    e.schemaHasRulesForType = t;
    function r(a, o) {
      return o.rules.some((n) => s(a, n));
    }
    e.shouldUseGroup = r;
    function s(a, o) {
      var n;
      return (
        a[o.keyword] !== void 0 ||
        ((n = o.definition.implements) === null || n === void 0 ? void 0 : n.some((c) => a[c] !== void 0))
      );
    }
    e.shouldUseRule = s;
  }),
  Rr = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }),
      (e.reportTypeError =
        e.checkDataTypes =
        e.checkDataType =
        e.coerceAndCheckDataType =
        e.getJSONTypes =
        e.getSchemaTypes =
        e.DataType =
          void 0);
    let t = Ra(),
      r = $a(),
      s = wr(),
      a = B(),
      o = Z();
    var n;
    (function (m) {
      (m[(m.Correct = 0)] = "Correct"), (m[(m.Wrong = 1)] = "Wrong");
    })(n || (e.DataType = n = {}));
    function c(m) {
      let y = i(m.type);
      if (y.includes("null")) {
        if (m.nullable === !1) throw Error("type: null contradicts nullable: false");
      } else {
        if (!y.length && m.nullable !== void 0) throw Error('"nullable" cannot be used without "type"');
        if (m.nullable === !0) y.push("null");
      }
      return y;
    }
    e.getSchemaTypes = c;
    function i(m) {
      let y = Array.isArray(m) ? m : m ? [m] : [];
      if (y.every(t.isJSONType)) return y;
      throw Error("type must be JSONType or JSONType[]: " + y.join(","));
    }
    e.getJSONTypes = i;
    function l(m, y) {
      let { gen: E, data: v, opts: f } = m,
        P = S(y, f.coerceTypes),
        I = y.length > 0 && !(P.length === 0 && y.length === 1 && (0, r.schemaHasRulesForType)(m, y[0]));
      if (I) {
        let L = _(y, v, f.strictNumbers, n.Wrong);
        E.if(L, () => {
          if (P.length) R(m, y, P);
          else u(m);
        });
      }
      return I;
    }
    e.coerceAndCheckDataType = l;
    let d = new Set(["string", "number", "integer", "boolean", "null"]);
    function S(m, y) {
      return y ? m.filter((E) => d.has(E) || (y === "array" && E === "array")) : [];
    }
    function R(m, y, E) {
      let { gen: v, data: f, opts: P } = m,
        I = v.let("dataType", a._`typeof ${f}`),
        L = v.let("coerced", a._`undefined`);
      if (P.coerceTypes === "array")
        v.if(a._`${I} == 'object' && Array.isArray(${f}) && ${f}.length == 1`, () =>
          v
            .assign(f, a._`${f}[0]`)
            .assign(I, a._`typeof ${f}`)
            .if(_(y, f, P.strictNumbers), () => v.assign(L, f)),
        );
      v.if(a._`${L} !== undefined`);
      for (let j of E) if (d.has(j) || (j === "array" && P.coerceTypes === "array")) k(j);
      v.else(),
        u(m),
        v.endIf(),
        v.if(a._`${L} !== undefined`, () => {
          v.assign(f, L), w(m, L);
        });
      function k(j) {
        switch (j) {
          case "string":
            v.elseIf(a._`${I} == "number" || ${I} == "boolean"`)
              .assign(L, a._`"" + ${f}`)
              .elseIf(a._`${f} === null`)
              .assign(L, a._`""`);
            return;
          case "number":
            v.elseIf(a._`${I} == "boolean" || ${f} === null
              || (${I} == "string" && ${f} && ${f} == +${f})`).assign(L, a._`+${f}`);
            return;
          case "integer":
            v.elseIf(a._`${I} === "boolean" || ${f} === null
              || (${I} === "string" && ${f} && ${f} == +${f} && !(${f} % 1))`).assign(L, a._`+${f}`);
            return;
          case "boolean":
            v.elseIf(a._`${f} === "false" || ${f} === 0 || ${f} === null`)
              .assign(L, !1)
              .elseIf(a._`${f} === "true" || ${f} === 1`)
              .assign(L, !0);
            return;
          case "null":
            v.elseIf(a._`${f} === "" || ${f} === 0 || ${f} === false`), v.assign(L, null);
            return;
          case "array":
            v.elseIf(a._`${I} === "string" || ${I} === "number"
              || ${I} === "boolean" || ${f} === null`).assign(L, a._`[${f}]`);
        }
      }
    }
    function w({ gen: m, parentData: y, parentDataProperty: E }, v) {
      m.if(a._`${y} !== undefined`, () => m.assign(a._`${y}[${E}]`, v));
    }
    function g(m, y, E, v = n.Correct) {
      let f = v === n.Correct ? a.operators.EQ : a.operators.NEQ,
        P;
      switch (m) {
        case "null":
          return a._`${y} ${f} null`;
        case "array":
          P = a._`Array.isArray(${y})`;
          break;
        case "object":
          P = a._`${y} && typeof ${y} == "object" && !Array.isArray(${y})`;
          break;
        case "integer":
          P = I(a._`!(${y} % 1) && !isNaN(${y})`);
          break;
        case "number":
          P = I();
          break;
        default:
          return a._`typeof ${y} ${f} ${m}`;
      }
      return v === n.Correct ? P : (0, a.not)(P);
      function I(L = a.nil) {
        return (0, a.and)(a._`typeof ${y} == "number"`, L, E ? a._`isFinite(${y})` : a.nil);
      }
    }
    e.checkDataType = g;
    function _(m, y, E, v) {
      if (m.length === 1) return g(m[0], y, E, v);
      let f,
        P = (0, o.toHash)(m);
      if (P.array && P.object) {
        let I = a._`typeof ${y} != "object"`;
        (f = P.null ? I : a._`!${y} || ${I}`), delete P.null, delete P.array, delete P.object;
      } else f = a.nil;
      if (P.number) delete P.integer;
      for (let I in P) f = (0, a.and)(f, g(I, y, E, v));
      return f;
    }
    e.checkDataTypes = _;
    let h = {
      message: ({ schema: m }) => `must be ${m}`,
      params: ({ schema: m, schemaValue: y }) => (typeof m == "string" ? a._`{type: ${m}}` : a._`{type: ${y}}`),
    };
    function u(m) {
      let y = p(m);
      (0, s.reportError)(y, h);
    }
    e.reportTypeError = u;
    function p(m) {
      let { gen: y, data: E, schema: v } = m,
        f = (0, o.schemaRefOrVal)(m, v, "type");
      return {
        gen: y,
        keyword: "type",
        data: E,
        schema: v.type,
        schemaCode: f,
        schemaValue: f,
        parentSchema: v,
        params: {},
        it: m,
      };
    }
  }),
  ni = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }), (e.assignDefaults = void 0);
    let t = B(),
      r = Z();
    function s(o, n) {
      let { properties: c, items: i } = o.schema;
      if (n === "object" && c) for (let l in c) a(o, l, c[l].default);
      else if (n === "array" && Array.isArray(i)) i.forEach((l, d) => a(o, d, l.default));
    }
    e.assignDefaults = s;
    function a(o, n, c) {
      let { gen: i, compositeRule: l, data: d, opts: S } = o;
      if (c === void 0) return;
      let R = t._`${d}${(0, t.getProperty)(n)}`;
      if (l) {
        (0, r.checkStrictMode)(o, `default is ignored for: ${R}`);
        return;
      }
      let w = t._`${R} === undefined`;
      if (S.useDefaults === "empty") w = t._`${w} || ${R} === null || ${R} === ""`;
      i.if(w, t._`${R} = ${(0, t.stringify)(c)}`);
    }
  }),
  $e = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }),
      (e.validateUnion =
        e.validateArray =
        e.usePattern =
        e.callValidateCode =
        e.schemaProperties =
        e.allSchemaProperties =
        e.noPropertyInData =
        e.propertyInData =
        e.isOwnProperty =
        e.hasPropFunc =
        e.reportMissingProp =
        e.checkMissingProp =
        e.checkReportMissingProp =
          void 0);
    let t = B(),
      r = Z(),
      s = we(),
      a = Z();
    function o(m, y) {
      let { gen: E, data: v, it: f } = m;
      E.if(S(E, v, y, f.opts.ownProperties), () => {
        m.setParams({ missingProperty: t._`${y}` }, !0), m.error();
      });
    }
    e.checkReportMissingProp = o;
    function n({ gen: m, data: y, it: { opts: E } }, v, f) {
      return (0, t.or)(...v.map((P) => (0, t.and)(S(m, y, P, E.ownProperties), t._`${f} = ${P}`)));
    }
    e.checkMissingProp = n;
    function c(m, y) {
      m.setParams({ missingProperty: y }, !0), m.error();
    }
    e.reportMissingProp = c;
    function i(m) {
      return m.scopeValue("func", { ref: Object.prototype.hasOwnProperty, code: t._`Object.prototype.hasOwnProperty` });
    }
    e.hasPropFunc = i;
    function l(m, y, E) {
      return t._`${i(m)}.call(${y}, ${E})`;
    }
    e.isOwnProperty = l;
    function d(m, y, E, v) {
      let f = t._`${y}${(0, t.getProperty)(E)} !== undefined`;
      return v ? t._`${f} && ${l(m, y, E)}` : f;
    }
    e.propertyInData = d;
    function S(m, y, E, v) {
      let f = t._`${y}${(0, t.getProperty)(E)} === undefined`;
      return v ? (0, t.or)(f, (0, t.not)(l(m, y, E))) : f;
    }
    e.noPropertyInData = S;
    function R(m) {
      return m ? Object.keys(m).filter((y) => y !== "__proto__") : [];
    }
    e.allSchemaProperties = R;
    function w(m, y) {
      return R(y).filter((E) => !(0, r.alwaysValidSchema)(m, y[E]));
    }
    e.schemaProperties = w;
    function g(
      { schemaCode: m, data: y, it: { gen: E, topSchemaRef: v, schemaPath: f, errorPath: P }, it: I },
      L,
      k,
      j,
    ) {
      let F = j ? t._`${m}, ${y}, ${v}${f}` : y,
        V = [
          [s.default.instancePath, (0, t.strConcat)(s.default.instancePath, P)],
          [s.default.parentData, I.parentData],
          [s.default.parentDataProperty, I.parentDataProperty],
          [s.default.rootData, s.default.rootData],
        ];
      if (I.opts.dynamicRef) V.push([s.default.dynamicAnchors, s.default.dynamicAnchors]);
      let Y = t._`${F}, ${E.object(...V)}`;
      return k !== t.nil ? t._`${L}.call(${k}, ${Y})` : t._`${L}(${Y})`;
    }
    e.callValidateCode = g;
    let _ = t._`new RegExp`;
    function h({ gen: m, it: { opts: y } }, E) {
      let v = y.unicodeRegExp ? "u" : "",
        { regExp: f } = y.code,
        P = f(E, v);
      return m.scopeValue("pattern", {
        key: P.toString(),
        ref: P,
        code: t._`${f.code === "new RegExp" ? _ : ((0, a.useFunc))(m, f)}(${E}, ${v})`,
      });
    }
    e.usePattern = h;
    function u(m) {
      let { gen: y, data: E, keyword: v, it: f } = m,
        P = y.name("valid");
      if (f.allErrors) {
        let L = y.let("valid", !0);
        return I(() => y.assign(L, !1)), L;
      }
      return y.var(P, !0), I(() => y.break()), P;
      function I(L) {
        let k = y.const("len", t._`${E}.length`);
        y.forRange("i", 0, k, (j) => {
          m.subschema({ keyword: v, dataProp: j, dataPropType: r.Type.Num }, P), y.if((0, t.not)(P), L);
        });
      }
    }
    e.validateArray = u;
    function p(m) {
      let { gen: y, schema: E, keyword: v, it: f } = m;
      if (!Array.isArray(E)) throw Error("ajv implementation error");
      if (E.some((L) => (0, r.alwaysValidSchema)(f, L)) && !f.opts.unevaluated) return;
      let P = y.let("valid", !1),
        I = y.name("_valid");
      y.block(() =>
        E.forEach((L, k) => {
          let j = m.subschema({ keyword: v, schemaProp: k, compositeRule: !0 }, I);
          if ((y.assign(P, t._`${P} || ${I}`), !m.mergeValidEvaluated(j, I))) y.if((0, t.not)(P));
        }),
      ),
        m.result(
          P,
          () => m.reset(),
          () => m.error(!0),
        );
    }
    e.validateUnion = p;
  }),
  ii = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }),
      (e.validateKeywordUsage = e.validSchemaType = e.funcKeywordCode = e.macroKeywordCode = void 0);
    let t = B(),
      r = we(),
      s = $e(),
      a = wr();
    function o(w, g) {
      let { gen: _, keyword: h, schema: u, parentSchema: p, it: m } = w,
        y = g.macro.call(m.self, u, p, m),
        E = d(_, h, y);
      if (m.opts.validateSchema !== !1) m.self.validateSchema(y, !0);
      let v = _.name("valid");
      w.subschema(
        { schema: y, schemaPath: t.nil, errSchemaPath: `${m.errSchemaPath}/${h}`, topSchemaRef: E, compositeRule: !0 },
        v,
      ),
        w.pass(v, () => w.error(!0));
    }
    e.macroKeywordCode = o;
    function n(w, g) {
      var _;
      let { gen: h, keyword: u, schema: p, parentSchema: m, $data: y, it: E } = w;
      l(E, g);
      let v = d(h, u, !y && g.compile ? g.compile.call(E.self, p, m, E) : g.validate),
        f = h.let("valid");
      w.block$data(f, P), w.ok((_ = g.valid) !== null && _ !== void 0 ? _ : f);
      function P() {
        if (g.errors === !1) {
          if ((k(), g.modifying)) c(w);
          j(() => w.error());
        } else {
          let F = g.async ? I() : L();
          if (g.modifying) c(w);
          j(() => i(w, F));
        }
      }
      function I() {
        let F = h.let("ruleErrs", null);
        return (
          h.try(
            () => k(t._`await `),
            (V) =>
              h.assign(f, !1).if(
                t._`${V} instanceof ${E.ValidationError}`,
                () => h.assign(F, t._`${V}.errors`),
                () => h.throw(V),
              ),
          ),
          F
        );
      }
      function L() {
        let F = t._`${v}.errors`;
        return h.assign(F, null), k(t.nil), F;
      }
      function k(F = g.async ? t._`await ` : t.nil) {
        let V = E.opts.passContext ? r.default.this : r.default.self,
          Y = !(("compile" in g && !y) || g.schema === !1);
        h.assign(f, t._`${F}${(0, s.callValidateCode)(w, v, V, Y)}`, g.modifying);
      }
      function j(F) {
        var V;
        h.if((0, t.not)((V = g.valid) !== null && V !== void 0 ? V : f), F);
      }
    }
    e.funcKeywordCode = n;
    function c(w) {
      let { gen: g, data: _, it: h } = w;
      g.if(h.parentData, () => g.assign(_, t._`${h.parentData}[${h.parentDataProperty}]`));
    }
    function i(w, g) {
      let { gen: _ } = w;
      _.if(
        t._`Array.isArray(${g})`,
        () => {
          _.assign(
            r.default.vErrors,
            t._`${r.default.vErrors} === null ? ${g} : ${r.default.vErrors}.concat(${g})`,
          ).assign(r.default.errors, t._`${r.default.vErrors}.length`),
            (0, a.extendErrors)(w);
        },
        () => w.error(),
      );
    }
    function l({ schemaEnv: w }, g) {
      if (g.async && !w.$async) throw Error("async keyword in sync schema");
    }
    function d(w, g, _) {
      if (_ === void 0) throw Error(`keyword "${g}" failed to compile`);
      return w.scopeValue("keyword", typeof _ == "function" ? { ref: _ } : { ref: _, code: (0, t.stringify)(_) });
    }
    function S(w, g, _ = !1) {
      return (
        !g.length ||
        g.some((h) =>
          h === "array"
            ? Array.isArray(w)
            : h === "object"
              ? w && typeof w == "object" && !Array.isArray(w)
              : typeof w == h || (_ && typeof w > "u"),
        )
      );
    }
    e.validSchemaType = S;
    function R({ schema: w, opts: g, self: _, errSchemaPath: h }, u, p) {
      if (Array.isArray(u.keyword) ? !u.keyword.includes(p) : u.keyword !== p) throw Error("ajv implementation error");
      let m = u.dependencies;
      if (m === null || m === void 0 ? void 0 : m.some((y) => !Object.prototype.hasOwnProperty.call(w, y)))
        throw Error(`parent schema must have dependencies of ${p}: ${m.join(",")}`);
      if (u.validateSchema) {
        if (!u.validateSchema(w[p])) {
          let y = `keyword "${p}" value is invalid at path "${h}": ` + _.errorsText(u.validateSchema.errors);
          if (g.validateSchema === "log") _.logger.error(y);
          else throw Error(y);
        }
      }
    }
    e.validateKeywordUsage = R;
  }),
  ci = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }),
      (e.extendSubschemaMode = e.extendSubschemaData = e.getSubschema = void 0);
    let t = B(),
      r = Z();
    function s(n, { keyword: c, schemaProp: i, schema: l, schemaPath: d, errSchemaPath: S, topSchemaRef: R }) {
      if (c !== void 0 && l !== void 0) throw Error('both "keyword" and "schema" passed, only one allowed');
      if (c !== void 0) {
        let w = n.schema[c];
        return i === void 0
          ? {
              schema: w,
              schemaPath: t._`${n.schemaPath}${(0, t.getProperty)(c)}`,
              errSchemaPath: `${n.errSchemaPath}/${c}`,
            }
          : {
              schema: w[i],
              schemaPath: t._`${n.schemaPath}${(0, t.getProperty)(c)}${(0, t.getProperty)(i)}`,
              errSchemaPath: `${n.errSchemaPath}/${c}/${(0, r.escapeFragment)(i)}`,
            };
      }
      if (l !== void 0) {
        if (d === void 0 || S === void 0 || R === void 0)
          throw Error('"schemaPath", "errSchemaPath" and "topSchemaRef" are required with "schema"');
        return { schema: l, schemaPath: d, topSchemaRef: R, errSchemaPath: S };
      }
      throw Error('either "keyword" or "schema" must be passed');
    }
    e.getSubschema = s;
    function a(n, c, { dataProp: i, dataPropType: l, data: d, dataTypes: S, propertyName: R }) {
      if (d !== void 0 && i !== void 0) throw Error('both "data" and "dataProp" passed, only one allowed');
      let { gen: w } = c;
      if (i !== void 0) {
        let { errorPath: _, dataPathArr: h, opts: u } = c;
        g(w.let("data", t._`${c.data}${(0, t.getProperty)(i)}`, !0)),
          (n.errorPath = t.str`${_}${(0, r.getErrorPath)(i, l, u.jsPropertySyntax)}`),
          (n.parentDataProperty = t._`${i}`),
          (n.dataPathArr = [...h, n.parentDataProperty]);
      }
      if (d !== void 0) {
        if ((g(d instanceof t.Name ? d : w.let("data", d, !0)), R !== void 0)) n.propertyName = R;
      }
      if (S) n.dataTypes = S;
      function g(_) {
        (n.data = _),
          (n.dataLevel = c.dataLevel + 1),
          (n.dataTypes = []),
          (c.definedProperties = new Set()),
          (n.parentData = c.data),
          (n.dataNames = [...c.dataNames, _]);
      }
    }
    e.extendSubschemaData = a;
    function o(n, { jtdDiscriminator: c, jtdMetadata: i, compositeRule: l, createErrors: d, allErrors: S }) {
      if (l !== void 0) n.compositeRule = l;
      if (d !== void 0) n.createErrors = d;
      if (S !== void 0) n.allErrors = S;
      (n.jtdDiscriminator = c), (n.jtdMetadata = i);
    }
    e.extendSubschemaMode = o;
  }),
  Ea = U((e, t) => {
    t.exports = function r(s, a) {
      if (s === a) return !0;
      if (s && a && typeof s == "object" && typeof a == "object") {
        if (s.constructor !== a.constructor) return !1;
        var o, n, c;
        if (Array.isArray(s)) {
          if (((o = s.length), o != a.length)) return !1;
          for (n = o; n-- !== 0; ) if (!r(s[n], a[n])) return !1;
          return !0;
        }
        if (s.constructor === RegExp) return s.source === a.source && s.flags === a.flags;
        if (s.valueOf !== Object.prototype.valueOf) return s.valueOf() === a.valueOf();
        if (s.toString !== Object.prototype.toString) return s.toString() === a.toString();
        if (((c = Object.keys(s)), (o = c.length), o !== Object.keys(a).length)) return !1;
        for (n = o; n-- !== 0; ) if (!Object.prototype.hasOwnProperty.call(a, c[n])) return !1;
        for (n = o; n-- !== 0; ) {
          var i = c[n];
          if (!r(s[i], a[i])) return !1;
        }
        return !0;
      }
      return s !== s && a !== a;
    };
  }),
  li = U((e, t) => {
    var r = (t.exports = function (o, n, c) {
      if (typeof n == "function") (c = n), (n = {});
      c = n.cb || c;
      var i = typeof c == "function" ? c : c.pre || function () {},
        l = c.post || function () {};
      s(n, i, l, o, "", o);
    });
    (r.keywords = {
      additionalItems: !0,
      items: !0,
      contains: !0,
      additionalProperties: !0,
      propertyNames: !0,
      not: !0,
      if: !0,
      then: !0,
      else: !0,
    }),
      (r.arrayKeywords = { items: !0, allOf: !0, anyOf: !0, oneOf: !0 }),
      (r.propsKeywords = { $defs: !0, definitions: !0, properties: !0, patternProperties: !0, dependencies: !0 }),
      (r.skipKeywords = {
        default: !0,
        enum: !0,
        const: !0,
        required: !0,
        maximum: !0,
        minimum: !0,
        exclusiveMaximum: !0,
        exclusiveMinimum: !0,
        multipleOf: !0,
        maxLength: !0,
        minLength: !0,
        pattern: !0,
        format: !0,
        maxItems: !0,
        minItems: !0,
        uniqueItems: !0,
        maxProperties: !0,
        minProperties: !0,
      });
    function s(o, n, c, i, l, d, S, R, w, g) {
      if (i && typeof i == "object" && !Array.isArray(i)) {
        n(i, l, d, S, R, w, g);
        for (var _ in i) {
          var h = i[_];
          if (Array.isArray(h)) {
            if (_ in r.arrayKeywords)
              for (var u = 0; u < h.length; u++) s(o, n, c, h[u], l + "/" + _ + "/" + u, d, l, _, i, u);
          } else if (_ in r.propsKeywords) {
            if (h && typeof h == "object") for (var p in h) s(o, n, c, h[p], l + "/" + _ + "/" + a(p), d, l, _, i, p);
          } else if (_ in r.keywords || (o.allKeys && !(_ in r.skipKeywords))) s(o, n, c, h, l + "/" + _, d, l, _, i);
        }
        c(i, l, d, S, R, w, g);
      }
    }
    function a(o) {
      return o.replace(/~/g, "~0").replace(/\//g, "~1");
    }
  }),
  $r = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }),
      (e.getSchemaRefs = e.resolveUrl = e.normalizeId = e._getFullPath = e.getFullPath = e.inlineRef = void 0);
    let t = Z(),
      r = Ea(),
      s = li(),
      a = new Set([
        "type",
        "format",
        "pattern",
        "maxLength",
        "minLength",
        "maxProperties",
        "minProperties",
        "maxItems",
        "minItems",
        "maximum",
        "minimum",
        "uniqueItems",
        "multipleOf",
        "required",
        "enum",
        "const",
      ]);
    function o(h, u = !0) {
      if (typeof h == "boolean") return !0;
      if (u === !0) return !c(h);
      if (!u) return !1;
      return i(h) <= u;
    }
    e.inlineRef = o;
    let n = new Set(["$ref", "$recursiveRef", "$recursiveAnchor", "$dynamicRef", "$dynamicAnchor"]);
    function c(h) {
      for (let u in h) {
        if (n.has(u)) return !0;
        let p = h[u];
        if (Array.isArray(p) && p.some(c)) return !0;
        if (typeof p == "object" && c(p)) return !0;
      }
      return !1;
    }
    function i(h) {
      let u = 0;
      for (let p in h) {
        if (p === "$ref") return 1 / 0;
        if ((u++, a.has(p))) continue;
        if (typeof h[p] == "object") (0, t.eachItem)(h[p], (m) => (u += i(m)));
        if (u === 1 / 0) return 1 / 0;
      }
      return u;
    }
    function l(h, u = "", p) {
      if (p !== !1) u = R(u);
      return d(h, h.parse(u));
    }
    e.getFullPath = l;
    function d(h, u) {
      return h.serialize(u).split("#")[0] + "#";
    }
    e._getFullPath = d;
    let S = /#\/?$/;
    function R(h) {
      return h ? h.replace(S, "") : "";
    }
    e.normalizeId = R;
    function w(h, u, p) {
      return (p = R(p)), h.resolve(u, p);
    }
    e.resolveUrl = w;
    let g = /^[a-z_][-a-z0-9._]*$/i;
    function _(h, u) {
      if (typeof h == "boolean") return {};
      let { schemaId: p, uriResolver: m } = this.opts,
        y = R(h[p] || u),
        E = { "": y },
        v = l(m, y, !1),
        f = {},
        P = new Set();
      return (
        s(h, { allKeys: !0 }, (k, j, F, V) => {
          if (V === void 0) return;
          let Y = v + j,
            te = E[V];
          if (typeof k[p] == "string") te = Q.call(this, k[p]);
          oe.call(this, k.$anchor), oe.call(this, k.$dynamicAnchor), (E[j] = te);
          function Q(X) {
            let C = this.opts.uriResolver.resolve;
            if (((X = R(te ? C(te, X) : X)), P.has(X))) throw L(X);
            P.add(X);
            let O = this.refs[X];
            if (typeof O == "string") O = this.refs[O];
            if (typeof O == "object") I(k, O.schema, X);
            else if (X !== R(Y))
              if (X[0] === "#") I(k, f[X], X), (f[X] = k);
              else this.refs[X] = Y;
            return X;
          }
          function oe(X) {
            if (typeof X == "string") {
              if (!g.test(X)) throw Error(`invalid anchor "${X}"`);
              Q.call(this, `#${X}`);
            }
          }
        }),
        f
      );
      function I(k, j, F) {
        if (j !== void 0 && !r(k, j)) throw L(F);
      }
      function L(k) {
        return Error(`reference "${k}" resolves to more than one schema`);
      }
    }
    e.getSchemaRefs = _;
  }),
  Ft = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }), (e.getData = e.KeywordCxt = e.validateFunctionCode = void 0);
    let t = oi(),
      r = Rr(),
      s = $a(),
      a = Rr(),
      o = ni(),
      n = ii(),
      c = ci(),
      i = B(),
      l = we(),
      d = $r(),
      S = Z(),
      R = wr();
    function w(T) {
      if (v(T)) {
        if ((P(T), E(T))) {
          u(T);
          return;
        }
      }
      g(T, () => (0, t.topBoolOrEmptySchema)(T));
    }
    e.validateFunctionCode = w;
    function g({ gen: T, validateName: N, schema: M, schemaEnv: x, opts: K }, G) {
      if (K.code.es5)
        T.func(N, i._`${l.default.data}, ${l.default.valCxt}`, x.$async, () => {
          T.code(i._`"use strict"; ${m(M, K)}`), h(T, K), T.code(G);
        });
      else T.func(N, i._`${l.default.data}, ${_(K)}`, x.$async, () => T.code(m(M, K)).code(G));
    }
    function _(T) {
      return i._`{${l.default.instancePath}="", ${l.default.parentData}, ${l.default.parentDataProperty}, ${l.default.rootData}=${l.default.data}${T.dynamicRef ? i._`, ${l.default.dynamicAnchors}={}` : i.nil}}={}`;
    }
    function h(T, N) {
      T.if(
        l.default.valCxt,
        () => {
          if (
            (T.var(l.default.instancePath, i._`${l.default.valCxt}.${l.default.instancePath}`),
            T.var(l.default.parentData, i._`${l.default.valCxt}.${l.default.parentData}`),
            T.var(l.default.parentDataProperty, i._`${l.default.valCxt}.${l.default.parentDataProperty}`),
            T.var(l.default.rootData, i._`${l.default.valCxt}.${l.default.rootData}`),
            N.dynamicRef)
          )
            T.var(l.default.dynamicAnchors, i._`${l.default.valCxt}.${l.default.dynamicAnchors}`);
        },
        () => {
          if (
            (T.var(l.default.instancePath, i._`""`),
            T.var(l.default.parentData, i._`undefined`),
            T.var(l.default.parentDataProperty, i._`undefined`),
            T.var(l.default.rootData, l.default.data),
            N.dynamicRef)
          )
            T.var(l.default.dynamicAnchors, i._`{}`);
        },
      );
    }
    function u(T) {
      let { schema: N, opts: M, gen: x } = T;
      g(T, () => {
        if (M.$comment && N.$comment) V(T);
        if ((k(T), x.let(l.default.vErrors, null), x.let(l.default.errors, 0), M.unevaluated)) p(T);
        I(T), Y(T);
      });
    }
    function p(T) {
      let { gen: N, validateName: M } = T;
      (T.evaluated = N.const("evaluated", i._`${M}.evaluated`)),
        N.if(i._`${T.evaluated}.dynamicProps`, () => N.assign(i._`${T.evaluated}.props`, i._`undefined`)),
        N.if(i._`${T.evaluated}.dynamicItems`, () => N.assign(i._`${T.evaluated}.items`, i._`undefined`));
    }
    function m(T, N) {
      let M = typeof T == "object" && T[N.schemaId];
      return M && (N.code.source || N.code.process) ? i._`/*# sourceURL=${M} */` : i.nil;
    }
    function y(T, N) {
      if (v(T)) {
        if ((P(T), E(T))) {
          f(T, N);
          return;
        }
      }
      (0, t.boolOrEmptySchema)(T, N);
    }
    function E({ schema: T, self: N }) {
      if (typeof T == "boolean") return !T;
      for (let M in T) if (N.RULES.all[M]) return !0;
      return !1;
    }
    function v(T) {
      return typeof T.schema != "boolean";
    }
    function f(T, N) {
      let { schema: M, gen: x, opts: K } = T;
      if (K.$comment && M.$comment) V(T);
      j(T), F(T);
      let G = x.const("_errs", l.default.errors);
      I(T, G), x.var(N, i._`${G} === ${l.default.errors}`);
    }
    function P(T) {
      (0, S.checkUnknownRules)(T), L(T);
    }
    function I(T, N) {
      if (T.opts.jtd) return Q(T, [], !1, N);
      let M = (0, r.getSchemaTypes)(T.schema);
      Q(T, M, !(0, r.coerceAndCheckDataType)(T, M), N);
    }
    function L(T) {
      let { schema: N, errSchemaPath: M, opts: x, self: K } = T;
      if (N.$ref && x.ignoreKeywordsWithRef && (0, S.schemaHasRulesButRef)(N, K.RULES))
        K.logger.warn(`$ref: keywords ignored in schema at path "${M}"`);
    }
    function k(T) {
      let { schema: N, opts: M } = T;
      if (N.default !== void 0 && M.useDefaults && M.strictSchema)
        (0, S.checkStrictMode)(T, "default is ignored in the schema root");
    }
    function j(T) {
      let N = T.schema[T.opts.schemaId];
      if (N) T.baseId = (0, d.resolveUrl)(T.opts.uriResolver, T.baseId, N);
    }
    function F(T) {
      if (T.schema.$async && !T.schemaEnv.$async) throw Error("async schema in sync schema");
    }
    function V({ gen: T, schemaEnv: N, schema: M, errSchemaPath: x, opts: K }) {
      let G = M.$comment;
      if (K.$comment === !0) T.code(i._`${l.default.self}.logger.log(${G})`);
      else if (typeof K.$comment == "function") {
        let se = i.str`${x}/$comment`,
          ie = T.scopeValue("root", { ref: N.root });
        T.code(i._`${l.default.self}.opts.$comment(${G}, ${se}, ${ie}.schema)`);
      }
    }
    function Y(T) {
      let { gen: N, schemaEnv: M, validateName: x, ValidationError: K, opts: G } = T;
      if (M.$async)
        N.if(
          i._`${l.default.errors} === 0`,
          () => N.return(l.default.data),
          () => N.throw(i._`new ${K}(${l.default.vErrors})`),
        );
      else {
        if ((N.assign(i._`${x}.errors`, l.default.vErrors), G.unevaluated)) te(T);
        N.return(i._`${l.default.errors} === 0`);
      }
    }
    function te({ gen: T, evaluated: N, props: M, items: x }) {
      if (M instanceof i.Name) T.assign(i._`${N}.props`, M);
      if (x instanceof i.Name) T.assign(i._`${N}.items`, x);
    }
    function Q(T, N, M, x) {
      let { gen: K, schema: G, data: se, allErrors: ie, opts: le, self: ne } = T,
        { RULES: re } = ne;
      if (G.$ref && (le.ignoreKeywordsWithRef || !(0, S.schemaHasRulesButRef)(G, re))) {
        K.block(() => W(T, "$ref", re.all.$ref.definition));
        return;
      }
      if (!le.jtd) X(T, N);
      K.block(() => {
        for (let ce of re.rules) ve(ce);
        ve(re.post);
      });
      function ve(ce) {
        if (!(0, s.shouldUseGroup)(G, ce)) return;
        if (ce.type) {
          if (
            (K.if((0, a.checkDataType)(ce.type, se, le.strictNumbers)),
            oe(T, ce),
            N.length === 1 && N[0] === ce.type && M)
          )
            K.else(), (0, a.reportTypeError)(T);
          K.endIf();
        } else oe(T, ce);
        if (!ie) K.if(i._`${l.default.errors} === ${x || 0}`);
      }
    }
    function oe(T, N) {
      let {
        gen: M,
        schema: x,
        opts: { useDefaults: K },
      } = T;
      if (K) (0, o.assignDefaults)(T, N.type);
      M.block(() => {
        for (let G of N.rules) if ((0, s.shouldUseRule)(x, G)) W(T, G.keyword, G.definition, N.type);
      });
    }
    function X(T, N) {
      if (T.schemaEnv.meta || !T.opts.strictTypes) return;
      if ((C(T, N), !T.opts.allowUnionTypes)) O(T, N);
      A(T, T.dataTypes);
    }
    function C(T, N) {
      if (!N.length) return;
      if (!T.dataTypes.length) {
        T.dataTypes = N;
        return;
      }
      N.forEach((M) => {
        if (!z(T.dataTypes, M)) D(T, `type "${M}" not allowed by context "${T.dataTypes.join(",")}"`);
      }),
        q(T, N);
    }
    function O(T, N) {
      if (N.length > 1 && !(N.length === 2 && N.includes("null")))
        D(T, "use allowUnionTypes to allow union type keyword");
    }
    function A(T, N) {
      let M = T.self.RULES.all;
      for (let x in M) {
        let K = M[x];
        if (typeof K == "object" && (0, s.shouldUseRule)(T.schema, K)) {
          let { type: G } = K.definition;
          if (G.length && !G.some((se) => b(N, se))) D(T, `missing type "${G.join(",")}" for keyword "${x}"`);
        }
      }
    }
    function b(T, N) {
      return T.includes(N) || (N === "number" && T.includes("integer"));
    }
    function z(T, N) {
      return T.includes(N) || (N === "integer" && T.includes("number"));
    }
    function q(T, N) {
      let M = [];
      for (let x of T.dataTypes)
        if (z(N, x)) M.push(x);
        else if (N.includes("integer") && x === "number") M.push("integer");
      T.dataTypes = M;
    }
    function D(T, N) {
      let M = T.schemaEnv.baseId + T.errSchemaPath;
      (N += ` at "${M}" (strictTypes)`), (0, S.checkStrictMode)(T, N, T.opts.strictTypes);
    }
    var H = class {
      constructor(T, N, M) {
        if (
          ((0, n.validateKeywordUsage)(T, N, M),
          (this.gen = T.gen),
          (this.allErrors = T.allErrors),
          (this.keyword = M),
          (this.data = T.data),
          (this.schema = T.schema[M]),
          (this.$data = N.$data && T.opts.$data && this.schema && this.schema.$data),
          (this.schemaValue = (0, S.schemaRefOrVal)(T, this.schema, M, this.$data)),
          (this.schemaType = N.schemaType),
          (this.parentSchema = T.schema),
          (this.params = {}),
          (this.it = T),
          (this.def = N),
          this.$data)
        )
          this.schemaCode = T.gen.const("vSchema", he(this.$data, T));
        else if (
          ((this.schemaCode = this.schemaValue), !(0, n.validSchemaType)(this.schema, N.schemaType, N.allowUndefined))
        )
          throw Error(`${M} value must be ${JSON.stringify(N.schemaType)}`);
        if ("code" in N ? N.trackErrors : N.errors !== !1) this.errsCount = T.gen.const("_errs", l.default.errors);
      }
      result(T, N, M) {
        this.failResult((0, i.not)(T), N, M);
      }
      failResult(T, N, M) {
        if ((this.gen.if(T), M)) M();
        else this.error();
        if (N) {
          if ((this.gen.else(), N(), this.allErrors)) this.gen.endIf();
        } else if (this.allErrors) this.gen.endIf();
        else this.gen.else();
      }
      pass(T, N) {
        this.failResult((0, i.not)(T), void 0, N);
      }
      fail(T) {
        if (T === void 0) {
          if ((this.error(), !this.allErrors)) this.gen.if(!1);
          return;
        }
        if ((this.gen.if(T), this.error(), this.allErrors)) this.gen.endIf();
        else this.gen.else();
      }
      fail$data(T) {
        if (!this.$data) return this.fail(T);
        let { schemaCode: N } = this;
        this.fail(i._`${N} !== undefined && (${(0, i.or)(this.invalid$data(), T)})`);
      }
      error(T, N, M) {
        if (N) {
          this.setParams(N), this._error(T, M), this.setParams({});
          return;
        }
        this._error(T, M);
      }
      _error(T, N) {
        (T ? R.reportExtraError : R.reportError)(this, this.def.error, N);
      }
      $dataError() {
        (0, R.reportError)(this, this.def.$dataError || R.keyword$DataError);
      }
      reset() {
        if (this.errsCount === void 0) throw Error('add "trackErrors" to keyword definition');
        (0, R.resetErrorsCount)(this.gen, this.errsCount);
      }
      ok(T) {
        if (!this.allErrors) this.gen.if(T);
      }
      setParams(T, N) {
        if (N) Object.assign(this.params, T);
        else this.params = T;
      }
      block$data(T, N, M = i.nil) {
        this.gen.block(() => {
          this.check$data(T, M), N();
        });
      }
      check$data(T = i.nil, N = i.nil) {
        if (!this.$data) return;
        let { gen: M, schemaCode: x, schemaType: K, def: G } = this;
        if ((M.if((0, i.or)(i._`${x} === undefined`, N)), T !== i.nil)) M.assign(T, !0);
        if (K.length || G.validateSchema) {
          if ((M.elseIf(this.invalid$data()), this.$dataError(), T !== i.nil)) M.assign(T, !1);
        }
        M.else();
      }
      invalid$data() {
        let { gen: T, schemaCode: N, schemaType: M, def: x, it: K } = this;
        return (0, i.or)(G(), se());
        function G() {
          if (M.length) {
            if (!(N instanceof i.Name)) throw Error("ajv implementation error");
            let ie = Array.isArray(M) ? M : [M];
            return i._`${(0, a.checkDataTypes)(ie, N, K.opts.strictNumbers, a.DataType.Wrong)}`;
          }
          return i.nil;
        }
        function se() {
          if (x.validateSchema) {
            let ie = T.scopeValue("validate$data", { ref: x.validateSchema });
            return i._`!${ie}(${N})`;
          }
          return i.nil;
        }
      }
      subschema(T, N) {
        let M = (0, c.getSubschema)(this.it, T);
        (0, c.extendSubschemaData)(M, this.it, T), (0, c.extendSubschemaMode)(M, T);
        let x = { ...this.it, ...M, items: void 0, props: void 0 };
        return y(x, N), x;
      }
      mergeEvaluated(T, N) {
        let { it: M, gen: x } = this;
        if (!M.opts.unevaluated) return;
        if (M.props !== !0 && T.props !== void 0) M.props = S.mergeEvaluated.props(x, T.props, M.props, N);
        if (M.items !== !0 && T.items !== void 0) M.items = S.mergeEvaluated.items(x, T.items, M.items, N);
      }
      mergeValidEvaluated(T, N) {
        let { it: M, gen: x } = this;
        if (M.opts.unevaluated && (M.props !== !0 || M.items !== !0))
          return x.if(N, () => this.mergeEvaluated(T, i.Name)), !0;
      }
    };
    e.KeywordCxt = H;
    function W(T, N, M, x) {
      let K = new H(T, M, N);
      if ("code" in M) M.code(K, x);
      else if (K.$data && M.validate) (0, n.funcKeywordCode)(K, M);
      else if ("macro" in M) (0, n.macroKeywordCode)(K, M);
      else if (M.compile || M.validate) (0, n.funcKeywordCode)(K, M);
    }
    let J = /^\/(?:[^~]|~0|~1)*$/,
      ee = /^([0-9]+)(#|\/(?:[^~]|~0|~1)*)?$/;
    function he(T, { dataLevel: N, dataNames: M, dataPathArr: x }) {
      let K, G;
      if (T === "") return l.default.rootData;
      if (T[0] === "/") {
        if (!J.test(T)) throw Error(`Invalid JSON-pointer: ${T}`);
        (K = T), (G = l.default.rootData);
      } else {
        let ne = ee.exec(T);
        if (!ne) throw Error(`Invalid JSON-pointer: ${T}`);
        let re = +ne[1];
        if (((K = ne[2]), K === "#")) {
          if (re >= N) throw Error(le("property/index", re));
          return x[N - re];
        }
        if (re > N) throw Error(le("data", re));
        if (((G = M[N - re]), !K)) return G;
      }
      let se = G,
        ie = K.split("/");
      for (let ne of ie)
        if (ne) (G = i._`${G}${(0, i.getProperty)((0, S.unescapeJsonPointer)(ne))}`), (se = i._`${se} && ${G}`);
      return se;
      function le(ne, re) {
        return `Cannot access ${ne} ${re} levels up, current level is ${N}`;
      }
    }
    e.getData = he;
  }),
  Er = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    var t = class extends Error {
      constructor(r) {
        super("validation failed");
        (this.errors = r), (this.ajv = this.validation = !0);
      }
    };
    e.default = t;
  }),
  Ht = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = $r();
    var r = class extends Error {
      constructor(s, a, o, n) {
        super(n || `can't resolve reference ${o} from id ${a}`);
        (this.missingRef = (0, t.resolveUrl)(s, a, o)),
          (this.missingSchema = (0, t.normalizeId)((0, t.getFullPath)(s, this.missingRef)));
      }
    };
    e.default = r;
  }),
  Pr = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }),
      (e.resolveSchema = e.getCompilingSchema = e.resolveRef = e.compileSchema = e.SchemaEnv = void 0);
    let t = B(),
      r = Er(),
      s = we(),
      a = $r(),
      o = Z(),
      n = Ft();
    var c = class {
      constructor(u) {
        var p;
        (this.refs = {}), (this.dynamicAnchors = {});
        let m;
        if (typeof u.schema == "object") m = u.schema;
        (this.schema = u.schema),
          (this.schemaId = u.schemaId),
          (this.root = u.root || this),
          (this.baseId =
            (p = u.baseId) !== null && p !== void 0
              ? p
              : (0, a.normalizeId)(m === null || m === void 0 ? void 0 : m[u.schemaId || "$id"])),
          (this.schemaPath = u.schemaPath),
          (this.localRefs = u.localRefs),
          (this.meta = u.meta),
          (this.$async = m === null || m === void 0 ? void 0 : m.$async),
          (this.refs = {});
      }
    };
    e.SchemaEnv = c;
    function i(u) {
      let p = S.call(this, u);
      if (p) return p;
      let m = (0, a.getFullPath)(this.opts.uriResolver, u.root.baseId),
        { es5: y, lines: E } = this.opts.code,
        { ownProperties: v } = this.opts,
        f = new t.CodeGen(this.scope, { es5: y, lines: E, ownProperties: v }),
        P;
      if (u.$async)
        P = f.scopeValue("Error", { ref: r.default, code: t._`require("ajv/dist/runtime/validation_error").default` });
      let I = f.scopeName("validate");
      u.validateName = I;
      let L = {
          gen: f,
          allErrors: this.opts.allErrors,
          data: s.default.data,
          parentData: s.default.parentData,
          parentDataProperty: s.default.parentDataProperty,
          dataNames: [s.default.data],
          dataPathArr: [t.nil],
          dataLevel: 0,
          dataTypes: [],
          definedProperties: new Set(),
          topSchemaRef: f.scopeValue(
            "schema",
            this.opts.code.source === !0 ? { ref: u.schema, code: (0, t.stringify)(u.schema) } : { ref: u.schema },
          ),
          validateName: I,
          ValidationError: P,
          schema: u.schema,
          schemaEnv: u,
          rootId: m,
          baseId: u.baseId || m,
          schemaPath: t.nil,
          errSchemaPath: u.schemaPath || (this.opts.jtd ? "" : "#"),
          errorPath: t._`""`,
          opts: this.opts,
          self: this,
        },
        k;
      try {
        this._compilations.add(u), (0, n.validateFunctionCode)(L), f.optimize(this.opts.code.optimize);
        let j = f.toString();
        if (((k = `${f.scopeRefs(s.default.scope)}return ${j}`), this.opts.code.process))
          k = this.opts.code.process(k, u);
        let F = Function(`${s.default.self}`, `${s.default.scope}`, k)(this, this.scope.get());
        if ((this.scope.value(I, { ref: F }), (F.errors = null), (F.schema = u.schema), (F.schemaEnv = u), u.$async))
          F.$async = !0;
        if (this.opts.code.source === !0) F.source = { validateName: I, validateCode: j, scopeValues: f._values };
        if (this.opts.unevaluated) {
          let { props: V, items: Y } = L;
          if (
            ((F.evaluated = {
              props: V instanceof t.Name ? void 0 : V,
              items: Y instanceof t.Name ? void 0 : Y,
              dynamicProps: V instanceof t.Name,
              dynamicItems: Y instanceof t.Name,
            }),
            F.source)
          )
            F.source.evaluated = (0, t.stringify)(F.evaluated);
        }
        return (u.validate = F), u;
      } catch (j) {
        if ((delete u.validate, delete u.validateName, k))
          this.logger.error("Error compiling schema, function code:", k);
        throw j;
      } finally {
        this._compilations.delete(u);
      }
    }
    e.compileSchema = i;
    function l(u, p, m) {
      var y;
      m = (0, a.resolveUrl)(this.opts.uriResolver, p, m);
      let E = u.refs[m];
      if (E) return E;
      let v = w.call(this, u, m);
      if (v === void 0) {
        let f = (y = u.localRefs) === null || y === void 0 ? void 0 : y[m],
          { schemaId: P } = this.opts;
        if (f) v = new c({ schema: f, schemaId: P, root: u, baseId: p });
      }
      if (v === void 0) return;
      return (u.refs[m] = d.call(this, v));
    }
    e.resolveRef = l;
    function d(u) {
      if ((0, a.inlineRef)(u.schema, this.opts.inlineRefs)) return u.schema;
      return u.validate ? u : i.call(this, u);
    }
    function S(u) {
      for (let p of this._compilations) if (R(p, u)) return p;
    }
    e.getCompilingSchema = S;
    function R(u, p) {
      return u.schema === p.schema && u.root === p.root && u.baseId === p.baseId;
    }
    function w(u, p) {
      let m;
      while (typeof (m = this.refs[p]) == "string") p = m;
      return m || this.schemas[p] || g.call(this, u, p);
    }
    function g(u, p) {
      let m = this.opts.uriResolver.parse(p),
        y = (0, a._getFullPath)(this.opts.uriResolver, m),
        E = (0, a.getFullPath)(this.opts.uriResolver, u.baseId, void 0);
      if (Object.keys(u.schema).length > 0 && y === E) return h.call(this, m, u);
      let v = (0, a.normalizeId)(y),
        f = this.refs[v] || this.schemas[v];
      if (typeof f == "string") {
        let P = g.call(this, u, f);
        if (typeof (P === null || P === void 0 ? void 0 : P.schema) !== "object") return;
        return h.call(this, m, P);
      }
      if (typeof (f === null || f === void 0 ? void 0 : f.schema) !== "object") return;
      if (!f.validate) i.call(this, f);
      if (v === (0, a.normalizeId)(p)) {
        let { schema: P } = f,
          { schemaId: I } = this.opts,
          L = P[I];
        if (L) E = (0, a.resolveUrl)(this.opts.uriResolver, E, L);
        return new c({ schema: P, schemaId: I, root: u, baseId: E });
      }
      return h.call(this, m, f);
    }
    e.resolveSchema = g;
    let _ = new Set(["properties", "patternProperties", "enum", "dependencies", "definitions"]);
    function h(u, { baseId: p, schema: m, root: y }) {
      var E;
      if (((E = u.fragment) === null || E === void 0 ? void 0 : E[0]) !== "/") return;
      for (let P of u.fragment.slice(1).split("/")) {
        if (typeof m === "boolean") return;
        let I = m[(0, o.unescapeFragment)(P)];
        if (I === void 0) return;
        m = I;
        let L = typeof m === "object" && m[this.opts.schemaId];
        if (!_.has(P) && L) p = (0, a.resolveUrl)(this.opts.uriResolver, p, L);
      }
      let v;
      if (typeof m != "boolean" && m.$ref && !(0, o.schemaHasRulesButRef)(m, this.RULES)) {
        let P = (0, a.resolveUrl)(this.opts.uriResolver, p, m.$ref);
        v = g.call(this, y, P);
      }
      let { schemaId: f } = this.opts;
      if (((v = v || new c({ schema: m, schemaId: f, root: y, baseId: p })), v.schema !== v.root.schema)) return v;
    }
  }),
  ui = U((e, t) => {
    t.exports = {
      $id: "https://raw.githubusercontent.com/ajv-validator/ajv/master/lib/refs/data.json#",
      description: "Meta-schema for $data reference (JSON AnySchema extension proposal)",
      type: "object",
      required: ["$data"],
      properties: {
        $data: { type: "string", anyOf: [{ format: "relative-json-pointer" }, { format: "json-pointer" }] },
      },
      additionalProperties: !1,
    };
  }),
  Pa = U((e, t) => {
    let r = RegExp.prototype.test.bind(/^[\da-f]{8}-[\da-f]{4}-[\da-f]{4}-[\da-f]{4}-[\da-f]{12}$/iu),
      s = RegExp.prototype.test.bind(
        /^(?:(?:25[0-5]|2[0-4]\d|1\d{2}|[1-9]\d|\d)\.){3}(?:25[0-5]|2[0-4]\d|1\d{2}|[1-9]\d|\d)$/u,
      );
    function a(g) {
      let _ = "",
        h = 0,
        u = 0;
      for (u = 0; u < g.length; u++) {
        if (((h = g[u].charCodeAt(0)), h === 48)) continue;
        if (!((h >= 48 && h <= 57) || (h >= 65 && h <= 70) || (h >= 97 && h <= 102))) return "";
        _ += g[u];
        break;
      }
      for (u += 1; u < g.length; u++) {
        if (((h = g[u].charCodeAt(0)), !((h >= 48 && h <= 57) || (h >= 65 && h <= 70) || (h >= 97 && h <= 102))))
          return "";
        _ += g[u];
      }
      return _;
    }
    let o = RegExp.prototype.test.bind(/[^!"$&'()*+,\-.;=_`a-z{}~]/u);
    function n(g) {
      return (g.length = 0), !0;
    }
    function c(g, _, h) {
      if (g.length) {
        let u = a(g);
        if (u !== "") _.push(u);
        else return (h.error = !0), !1;
        g.length = 0;
      }
      return !0;
    }
    function i(g) {
      let _ = 0,
        h = { error: !1, address: "", zone: "" },
        u = [],
        p = [],
        m = !1,
        y = !1,
        E = c;
      for (let v = 0; v < g.length; v++) {
        let f = g[v];
        if (f === "[" || f === "]") continue;
        if (f === ":") {
          if (m === !0) y = !0;
          if (!E(p, u, h)) break;
          if (++_ > 7) {
            h.error = !0;
            break;
          }
          if (v > 0 && g[v - 1] === ":") m = !0;
          u.push(":");
          continue;
        } else if (f === "%") {
          if (!E(p, u, h)) break;
          E = n;
        } else {
          p.push(f);
          continue;
        }
      }
      if (p.length)
        if (E === n) h.zone = p.join("");
        else if (y) u.push(p.join(""));
        else u.push(a(p));
      return (h.address = u.join("")), h;
    }
    function l(g) {
      if (d(g, ":") < 2) return { host: g, isIPV6: !1 };
      let _ = i(g);
      if (!_.error) {
        let { address: h, address: u } = _;
        if (_.zone) (h += "%" + _.zone), (u += "%25" + _.zone);
        return { host: h, isIPV6: !0, escapedHost: u };
      } else return { host: g, isIPV6: !1 };
    }
    function d(g, _) {
      let h = 0;
      for (let u = 0; u < g.length; u++) if (g[u] === _) h++;
      return h;
    }
    function S(g) {
      let _ = g,
        h = [],
        u = -1,
        p = 0;
      while ((p = _.length)) {
        if (p === 1)
          if (_ === ".") break;
          else if (_ === "/") {
            h.push("/");
            break;
          } else {
            h.push(_);
            break;
          }
        else if (p === 2) {
          if (_[0] === ".") {
            if (_[1] === ".") break;
            else if (_[1] === "/") {
              _ = _.slice(2);
              continue;
            }
          } else if (_[0] === "/") {
            if (_[1] === "." || _[1] === "/") {
              h.push("/");
              break;
            }
          }
        } else if (p === 3) {
          if (_ === "/..") {
            if (h.length !== 0) h.pop();
            h.push("/");
            break;
          }
        }
        if (_[0] === ".") {
          if (_[1] === ".") {
            if (_[2] === "/") {
              _ = _.slice(3);
              continue;
            }
          } else if (_[1] === "/") {
            _ = _.slice(2);
            continue;
          }
        } else if (_[0] === "/") {
          if (_[1] === ".") {
            if (_[2] === "/") {
              _ = _.slice(2);
              continue;
            } else if (_[2] === ".") {
              if (_[3] === "/") {
                if (((_ = _.slice(3)), h.length !== 0)) h.pop();
                continue;
              }
            }
          }
        }
        if ((u = _.indexOf("/", 1)) === -1) {
          h.push(_);
          break;
        } else h.push(_.slice(0, u)), (_ = _.slice(u));
      }
      return h.join("");
    }
    function R(g, _) {
      let h = _ !== !0 ? escape : unescape;
      if (g.scheme !== void 0) g.scheme = h(g.scheme);
      if (g.userinfo !== void 0) g.userinfo = h(g.userinfo);
      if (g.host !== void 0) g.host = h(g.host);
      if (g.path !== void 0) g.path = h(g.path);
      if (g.query !== void 0) g.query = h(g.query);
      if (g.fragment !== void 0) g.fragment = h(g.fragment);
      return g;
    }
    function w(g) {
      let _ = [];
      if (g.userinfo !== void 0) _.push(g.userinfo), _.push("@");
      if (g.host !== void 0) {
        let h = unescape(g.host);
        if (!s(h)) {
          let u = l(h);
          if (u.isIPV6 === !0) h = `[${u.escapedHost}]`;
          else h = g.host;
        }
        _.push(h);
      }
      if (typeof g.port === "number" || typeof g.port === "string") _.push(":"), _.push(String(g.port));
      return _.length ? _.join("") : void 0;
    }
    t.exports = {
      nonSimpleDomain: o,
      recomposeAuthority: w,
      normalizeComponentEncoding: R,
      removeDotSegments: S,
      isIPv4: s,
      isUUID: r,
      normalizeIPv6: l,
      stringArrayToHexStripped: a,
    };
  }),
  di = U((e, t) => {
    let { isUUID: r } = Pa(),
      s = /([\da-z][\d\-a-z]{0,31}):((?:[\w!$'()*+,\-.:;=@]|%[\da-f]{2})+)/iu,
      a = ["http", "https", "ws", "wss", "urn", "urn:uuid"];
    function o(f) {
      return a.indexOf(f) !== -1;
    }
    function n(f) {
      if (f.secure === !0) return !0;
      else if (f.secure === !1) return !1;
      else if (f.scheme)
        return (
          f.scheme.length === 3 &&
          (f.scheme[0] === "w" || f.scheme[0] === "W") &&
          (f.scheme[1] === "s" || f.scheme[1] === "S") &&
          (f.scheme[2] === "s" || f.scheme[2] === "S")
        );
      else return !1;
    }
    function c(f) {
      if (!f.host) f.error = f.error || "HTTP URIs must have a host.";
      return f;
    }
    function i(f) {
      let P = String(f.scheme).toLowerCase() === "https";
      if (f.port === (P ? 443 : 80) || f.port === "") f.port = void 0;
      if (!f.path) f.path = "/";
      return f;
    }
    function l(f) {
      return (
        (f.secure = n(f)),
        (f.resourceName = (f.path || "/") + (f.query ? "?" + f.query : "")),
        (f.path = void 0),
        (f.query = void 0),
        f
      );
    }
    function d(f) {
      if (f.port === (n(f) ? 443 : 80) || f.port === "") f.port = void 0;
      if (typeof f.secure === "boolean") (f.scheme = f.secure ? "wss" : "ws"), (f.secure = void 0);
      if (f.resourceName) {
        let [P, I] = f.resourceName.split("?");
        (f.path = P && P !== "/" ? P : void 0), (f.query = I), (f.resourceName = void 0);
      }
      return (f.fragment = void 0), f;
    }
    function S(f, P) {
      if (!f.path) return (f.error = "URN can not be parsed"), f;
      let I = f.path.match(s);
      if (I) {
        let L = P.scheme || f.scheme || "urn";
        (f.nid = I[1].toLowerCase()), (f.nss = I[2]);
        let k = v(`${L}:${P.nid || f.nid}`);
        if (((f.path = void 0), k)) f = k.parse(f, P);
      } else f.error = f.error || "URN can not be parsed.";
      return f;
    }
    function R(f, P) {
      if (f.nid === void 0) throw Error("URN without nid cannot be serialized");
      let I = P.scheme || f.scheme || "urn",
        L = f.nid.toLowerCase(),
        k = v(`${I}:${P.nid || L}`);
      if (k) f = k.serialize(f, P);
      let j = f,
        F = f.nss;
      return (j.path = `${L || P.nid}:${F}`), (P.skipEscape = !0), j;
    }
    function w(f, P) {
      let I = f;
      if (((I.uuid = I.nss), (I.nss = void 0), !P.tolerant && (!I.uuid || !r(I.uuid))))
        I.error = I.error || "UUID is not valid.";
      return I;
    }
    function g(f) {
      let P = f;
      return (P.nss = (f.uuid || "").toLowerCase()), P;
    }
    let _ = { scheme: "http", domainHost: !0, parse: c, serialize: i },
      h = { scheme: "https", domainHost: _.domainHost, parse: c, serialize: i },
      u = { scheme: "ws", domainHost: !0, parse: l, serialize: d },
      p = { scheme: "wss", domainHost: u.domainHost, parse: u.parse, serialize: u.serialize },
      E = {
        http: _,
        https: h,
        ws: u,
        wss: p,
        urn: { scheme: "urn", parse: S, serialize: R, skipNormalize: !0 },
        "urn:uuid": { scheme: "urn:uuid", parse: w, serialize: g, skipNormalize: !0 },
      };
    Object.setPrototypeOf(E, null);
    function v(f) {
      return (f && (E[f] || E[f.toLowerCase()])) || void 0;
    }
    t.exports = { wsIsSecure: n, SCHEMES: E, isValidSchemeName: o, getSchemeHandler: v };
  }),
  mi = U((e, t) => {
    let {
        normalizeIPv6: r,
        removeDotSegments: s,
        recomposeAuthority: a,
        normalizeComponentEncoding: o,
        isIPv4: n,
        nonSimpleDomain: c,
      } = Pa(),
      { SCHEMES: i, getSchemeHandler: l } = di();
    function d(p, m) {
      if (typeof p === "string") p = g(h(p, m), m);
      else if (typeof p === "object") p = h(g(p, m), m);
      return p;
    }
    function S(p, m, y) {
      let E = y ? Object.assign({ scheme: "null" }, y) : { scheme: "null" },
        v = R(h(p, E), h(m, E), E, !0);
      return (E.skipEscape = !0), g(v, E);
    }
    function R(p, m, y, E) {
      let v = {};
      if (!E) (p = h(g(p, y), y)), (m = h(g(m, y), y));
      if (((y = y || {}), !y.tolerant && m.scheme))
        (v.scheme = m.scheme),
          (v.userinfo = m.userinfo),
          (v.host = m.host),
          (v.port = m.port),
          (v.path = s(m.path || "")),
          (v.query = m.query);
      else {
        if (m.userinfo !== void 0 || m.host !== void 0 || m.port !== void 0)
          (v.userinfo = m.userinfo),
            (v.host = m.host),
            (v.port = m.port),
            (v.path = s(m.path || "")),
            (v.query = m.query);
        else {
          if (!m.path)
            if (((v.path = p.path), m.query !== void 0)) v.query = m.query;
            else v.query = p.query;
          else {
            if (m.path[0] === "/") v.path = s(m.path);
            else {
              if ((p.userinfo !== void 0 || p.host !== void 0 || p.port !== void 0) && !p.path) v.path = "/" + m.path;
              else if (!p.path) v.path = m.path;
              else v.path = p.path.slice(0, p.path.lastIndexOf("/") + 1) + m.path;
              v.path = s(v.path);
            }
            v.query = m.query;
          }
          (v.userinfo = p.userinfo), (v.host = p.host), (v.port = p.port);
        }
        v.scheme = p.scheme;
      }
      return (v.fragment = m.fragment), v;
    }
    function w(p, m, y) {
      if (typeof p === "string") (p = unescape(p)), (p = g(o(h(p, y), !0), { ...y, skipEscape: !0 }));
      else if (typeof p === "object") p = g(o(p, !0), { ...y, skipEscape: !0 });
      if (typeof m === "string") (m = unescape(m)), (m = g(o(h(m, y), !0), { ...y, skipEscape: !0 }));
      else if (typeof m === "object") m = g(o(m, !0), { ...y, skipEscape: !0 });
      return p.toLowerCase() === m.toLowerCase();
    }
    function g(p, m) {
      let y = {
          host: p.host,
          scheme: p.scheme,
          userinfo: p.userinfo,
          port: p.port,
          path: p.path,
          query: p.query,
          nid: p.nid,
          nss: p.nss,
          uuid: p.uuid,
          fragment: p.fragment,
          reference: p.reference,
          resourceName: p.resourceName,
          secure: p.secure,
          error: "",
        },
        E = Object.assign({}, m),
        v = [],
        f = l(E.scheme || y.scheme);
      if (f && f.serialize) f.serialize(y, E);
      if (y.path !== void 0)
        if (!E.skipEscape) {
          if (((y.path = escape(y.path)), y.scheme !== void 0)) y.path = y.path.split("%3A").join(":");
        } else y.path = unescape(y.path);
      if (E.reference !== "suffix" && y.scheme) v.push(y.scheme, ":");
      let P = a(y);
      if (P !== void 0) {
        if (E.reference !== "suffix") v.push("//");
        if ((v.push(P), y.path && y.path[0] !== "/")) v.push("/");
      }
      if (y.path !== void 0) {
        let I = y.path;
        if (!E.absolutePath && (!f || !f.absolutePath)) I = s(I);
        if (P === void 0 && I[0] === "/" && I[1] === "/") I = "/%2F" + I.slice(2);
        v.push(I);
      }
      if (y.query !== void 0) v.push("?", y.query);
      if (y.fragment !== void 0) v.push("#", y.fragment);
      return v.join("");
    }
    let _ =
      /^(?:([^#/:?]+):)?(?:\/\/((?:([^#/?@]*)@)?(\[[^#/?\]]+\]|[^#/:?]*)(?::(\d*))?))?([^#?]*)(?:\?([^#]*))?(?:#((?:.|[\n\r])*))?/u;
    function h(p, m) {
      let y = Object.assign({}, m),
        E = { scheme: void 0, userinfo: void 0, host: "", port: void 0, path: "", query: void 0, fragment: void 0 },
        v = !1;
      if (y.reference === "suffix")
        if (y.scheme) p = y.scheme + ":" + p;
        else p = "//" + p;
      let f = p.match(_);
      if (f) {
        if (
          ((E.scheme = f[1]),
          (E.userinfo = f[3]),
          (E.host = f[4]),
          (E.port = parseInt(f[5], 10)),
          (E.path = f[6] || ""),
          (E.query = f[7]),
          (E.fragment = f[8]),
          isNaN(E.port))
        )
          E.port = f[5];
        if (E.host)
          if (n(E.host) === !1) {
            let I = r(E.host);
            (E.host = I.host.toLowerCase()), (v = I.isIPV6);
          } else v = !0;
        if (
          E.scheme === void 0 &&
          E.userinfo === void 0 &&
          E.host === void 0 &&
          E.port === void 0 &&
          E.query === void 0 &&
          !E.path
        )
          E.reference = "same-document";
        else if (E.scheme === void 0) E.reference = "relative";
        else if (E.fragment === void 0) E.reference = "absolute";
        else E.reference = "uri";
        if (y.reference && y.reference !== "suffix" && y.reference !== E.reference)
          E.error = E.error || "URI is not a " + y.reference + " reference.";
        let P = l(y.scheme || E.scheme);
        if (!y.unicodeSupport && (!P || !P.unicodeSupport)) {
          if (E.host && (y.domainHost || (P && P.domainHost)) && v === !1 && c(E.host))
            try {
              E.host = URL.domainToASCII(E.host.toLowerCase());
            } catch (I) {
              E.error = E.error || "Host's domain name can not be converted to ASCII: " + I;
            }
        }
        if (!P || (P && !P.skipNormalize)) {
          if (p.indexOf("%") !== -1) {
            if (E.scheme !== void 0) E.scheme = unescape(E.scheme);
            if (E.host !== void 0) E.host = unescape(E.host);
          }
          if (E.path) E.path = escape(unescape(E.path));
          if (E.fragment) E.fragment = encodeURI(decodeURIComponent(E.fragment));
        }
        if (P && P.parse) P.parse(E, y);
      } else E.error = E.error || "URI can not be parsed.";
      return E;
    }
    let u = { SCHEMES: i, normalize: d, resolve: S, resolveComponent: R, equal: w, serialize: g, parse: h };
    (t.exports = u), (t.exports.default = u), (t.exports.fastUri = u);
  }),
  fi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = mi();
    (t.code = 'require("ajv/dist/runtime/uri").default'), (e.default = t);
  }),
  Ta = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }),
      (e.CodeGen = e.Name = e.nil = e.stringify = e.str = e._ = e.KeywordCxt = void 0);
    var t = Ft();
    Object.defineProperty(e, "KeywordCxt", {
      enumerable: !0,
      get: function () {
        return t.KeywordCxt;
      },
    });
    var r = B();
    Object.defineProperty(e, "_", {
      enumerable: !0,
      get: function () {
        return r._;
      },
    }),
      Object.defineProperty(e, "str", {
        enumerable: !0,
        get: function () {
          return r.str;
        },
      }),
      Object.defineProperty(e, "stringify", {
        enumerable: !0,
        get: function () {
          return r.stringify;
        },
      }),
      Object.defineProperty(e, "nil", {
        enumerable: !0,
        get: function () {
          return r.nil;
        },
      }),
      Object.defineProperty(e, "Name", {
        enumerable: !0,
        get: function () {
          return r.Name;
        },
      }),
      Object.defineProperty(e, "CodeGen", {
        enumerable: !0,
        get: function () {
          return r.CodeGen;
        },
      });
    let s = Er(),
      a = Ht(),
      o = Ra(),
      n = Pr(),
      c = B(),
      i = $r(),
      l = Rr(),
      d = Z(),
      S = ui(),
      R = fi(),
      w = (C, O) => new RegExp(C, O);
    w.code = "new RegExp";
    let g = ["removeAdditional", "useDefaults", "coerceTypes"],
      _ = new Set([
        "validate",
        "serialize",
        "parse",
        "wrapper",
        "root",
        "schema",
        "keyword",
        "pattern",
        "formats",
        "validate$data",
        "func",
        "obj",
        "Error",
      ]),
      h = {
        errorDataPath: "",
        format: "`validateFormats: false` can be used instead.",
        nullable: '"nullable" keyword is supported by default.',
        jsonPointers: "Deprecated jsPropertySyntax can be used instead.",
        extendRefs: "Deprecated ignoreKeywordsWithRef can be used instead.",
        missingRefs: "Pass empty schema with $id that should be ignored to ajv.addSchema.",
        processCode: "Use option `code: {process: (code, schemaEnv: object) => string}`",
        sourceCode: "Use option `code: {source: true}`",
        strictDefaults: "It is default now, see option `strict`.",
        strictKeywords: "It is default now, see option `strict`.",
        uniqueItems: '"uniqueItems" keyword is always validated.',
        unknownFormats: "Disable strict mode or pass `true` to `ajv.addFormat` (or `formats` option).",
        cache: "Map is used as cache, schema object as key.",
        serialize: "Map is used as cache, schema object as key.",
        ajvErrors: "It is default now.",
      },
      u = {
        ignoreKeywordsWithRef: "",
        jsPropertySyntax: "",
        unicode: '"minLength"/"maxLength" account for unicode characters by default.',
      },
      p = 200;
    function m(C) {
      var O, A, b, z, q, D, H, W, J, ee, he, T, N, M, x, K, G, se, ie, le, ne, re, ve, ce, Ee;
      let be = C.strict,
        Pe = (O = C.code) === null || O === void 0 ? void 0 : O.optimize,
        Ce = Pe === !0 || Pe === void 0 ? 1 : Pe || 0,
        ke = (b = (A = C.code) === null || A === void 0 ? void 0 : A.regExp) !== null && b !== void 0 ? b : w,
        Ne = (z = C.uriResolver) !== null && z !== void 0 ? z : R.default;
      return {
        strictSchema: (D = (q = C.strictSchema) !== null && q !== void 0 ? q : be) !== null && D !== void 0 ? D : !0,
        strictNumbers: (W = (H = C.strictNumbers) !== null && H !== void 0 ? H : be) !== null && W !== void 0 ? W : !0,
        strictTypes:
          (ee = (J = C.strictTypes) !== null && J !== void 0 ? J : be) !== null && ee !== void 0 ? ee : "log",
        strictTuples:
          (T = (he = C.strictTuples) !== null && he !== void 0 ? he : be) !== null && T !== void 0 ? T : "log",
        strictRequired:
          (M = (N = C.strictRequired) !== null && N !== void 0 ? N : be) !== null && M !== void 0 ? M : !1,
        code: C.code ? { ...C.code, optimize: Ce, regExp: ke } : { optimize: Ce, regExp: ke },
        loopRequired: (x = C.loopRequired) !== null && x !== void 0 ? x : p,
        loopEnum: (K = C.loopEnum) !== null && K !== void 0 ? K : p,
        meta: (G = C.meta) !== null && G !== void 0 ? G : !0,
        messages: (se = C.messages) !== null && se !== void 0 ? se : !0,
        inlineRefs: (ie = C.inlineRefs) !== null && ie !== void 0 ? ie : !0,
        schemaId: (le = C.schemaId) !== null && le !== void 0 ? le : "$id",
        addUsedSchema: (ne = C.addUsedSchema) !== null && ne !== void 0 ? ne : !0,
        validateSchema: (re = C.validateSchema) !== null && re !== void 0 ? re : !0,
        validateFormats: (ve = C.validateFormats) !== null && ve !== void 0 ? ve : !0,
        unicodeRegExp: (ce = C.unicodeRegExp) !== null && ce !== void 0 ? ce : !0,
        int32range: (Ee = C.int32range) !== null && Ee !== void 0 ? Ee : !0,
        uriResolver: Ne,
      };
    }
    var y = class {
      constructor(C = {}) {
        (this.schemas = {}),
          (this.refs = {}),
          (this.formats = {}),
          (this._compilations = new Set()),
          (this._loading = {}),
          (this._cache = new Map()),
          (C = this.opts = { ...C, ...m(C) });
        let { es5: O, lines: A } = this.opts.code;
        (this.scope = new c.ValueScope({ scope: {}, prefixes: _, es5: O, lines: A })), (this.logger = j(C.logger));
        let b = C.validateFormats;
        if (
          ((C.validateFormats = !1),
          (this.RULES = (0, o.getRules)()),
          E.call(this, h, C, "NOT SUPPORTED"),
          E.call(this, u, C, "DEPRECATED", "warn"),
          (this._metaOpts = L.call(this)),
          C.formats)
        )
          P.call(this);
        if ((this._addVocabularies(), this._addDefaultMetaSchema(), C.keywords)) I.call(this, C.keywords);
        if (typeof C.meta == "object") this.addMetaSchema(C.meta);
        f.call(this), (C.validateFormats = b);
      }
      _addVocabularies() {
        this.addKeyword("$async");
      }
      _addDefaultMetaSchema() {
        let { $data: C, meta: O, schemaId: A } = this.opts,
          b = S;
        if (A === "id") (b = { ...S }), (b.id = b.$id), delete b.$id;
        if (O && C) this.addMetaSchema(b, b[A], !1);
      }
      defaultMeta() {
        let { meta: C, schemaId: O } = this.opts;
        return (this.opts.defaultMeta = typeof C == "object" ? C[O] || C : void 0);
      }
      validate(C, O) {
        let A;
        if (typeof C == "string") {
          if (((A = this.getSchema(C)), !A)) throw Error(`no schema with key or ref "${C}"`);
        } else A = this.compile(C);
        let b = A(O);
        if (!("$async" in A)) this.errors = A.errors;
        return b;
      }
      compile(C, O) {
        let A = this._addSchema(C, O);
        return A.validate || this._compileSchemaEnv(A);
      }
      compileAsync(C, O) {
        if (typeof this.opts.loadSchema != "function") throw Error("options.loadSchema should be a function");
        let { loadSchema: A } = this.opts;
        return b.call(this, C, O);
        async function b(J, ee) {
          await z.call(this, J.$schema);
          let he = this._addSchema(J, ee);
          return he.validate || q.call(this, he);
        }
        async function z(J) {
          if (J && !this.getSchema(J)) await b.call(this, { $ref: J }, !0);
        }
        async function q(J) {
          try {
            return this._compileSchemaEnv(J);
          } catch (ee) {
            if (!(ee instanceof a.default)) throw ee;
            return D.call(this, ee), await H.call(this, ee.missingSchema), q.call(this, J);
          }
        }
        function D({ missingSchema: J, missingRef: ee }) {
          if (this.refs[J]) throw Error(`AnySchema ${J} is loaded but ${ee} cannot be resolved`);
        }
        async function H(J) {
          let ee = await W.call(this, J);
          if (!this.refs[J]) await z.call(this, ee.$schema);
          if (!this.refs[J]) this.addSchema(ee, J, O);
        }
        async function W(J) {
          let ee = this._loading[J];
          if (ee) return ee;
          try {
            return await (this._loading[J] = A(J));
          } finally {
            delete this._loading[J];
          }
        }
      }
      addSchema(C, O, A, b = this.opts.validateSchema) {
        if (Array.isArray(C)) {
          for (let q of C) this.addSchema(q, void 0, A, b);
          return this;
        }
        let z;
        if (typeof C === "object") {
          let { schemaId: q } = this.opts;
          if (((z = C[q]), z !== void 0 && typeof z != "string")) throw Error(`schema ${q} must be string`);
        }
        return (
          (O = (0, i.normalizeId)(O || z)),
          this._checkUnique(O),
          (this.schemas[O] = this._addSchema(C, A, O, b, !0)),
          this
        );
      }
      addMetaSchema(C, O, A = this.opts.validateSchema) {
        return this.addSchema(C, O, !0, A), this;
      }
      validateSchema(C, O) {
        if (typeof C == "boolean") return !0;
        let A;
        if (((A = C.$schema), A !== void 0 && typeof A != "string")) throw Error("$schema must be a string");
        if (((A = A || this.opts.defaultMeta || this.defaultMeta()), !A))
          return this.logger.warn("meta-schema not available"), (this.errors = null), !0;
        let b = this.validate(A, C);
        if (!b && O) {
          let z = "schema is invalid: " + this.errorsText();
          if (this.opts.validateSchema === "log") this.logger.error(z);
          else throw Error(z);
        }
        return b;
      }
      getSchema(C) {
        let O;
        while (typeof (O = v.call(this, C)) == "string") C = O;
        if (O === void 0) {
          let { schemaId: A } = this.opts,
            b = new n.SchemaEnv({ schema: {}, schemaId: A });
          if (((O = n.resolveSchema.call(this, b, C)), !O)) return;
          this.refs[C] = O;
        }
        return O.validate || this._compileSchemaEnv(O);
      }
      removeSchema(C) {
        if (C instanceof RegExp)
          return this._removeAllSchemas(this.schemas, C), this._removeAllSchemas(this.refs, C), this;
        switch (typeof C) {
          case "undefined":
            return this._removeAllSchemas(this.schemas), this._removeAllSchemas(this.refs), this._cache.clear(), this;
          case "string": {
            let O = v.call(this, C);
            if (typeof O == "object") this._cache.delete(O.schema);
            return delete this.schemas[C], delete this.refs[C], this;
          }
          case "object": {
            let O = C;
            this._cache.delete(O);
            let A = C[this.opts.schemaId];
            if (A) (A = (0, i.normalizeId)(A)), delete this.schemas[A], delete this.refs[A];
            return this;
          }
          default:
            throw Error("ajv.removeSchema: invalid parameter");
        }
      }
      addVocabulary(C) {
        for (let O of C) this.addKeyword(O);
        return this;
      }
      addKeyword(C, O) {
        let A;
        if (typeof C == "string") {
          if (((A = C), typeof O == "object"))
            this.logger.warn("these parameters are deprecated, see docs for addKeyword"), (O.keyword = A);
        } else if (typeof C == "object" && O === void 0) {
          if (((O = C), (A = O.keyword), Array.isArray(A) && !A.length))
            throw Error("addKeywords: keyword must be string or non-empty array");
        } else throw Error("invalid addKeywords parameters");
        if ((V.call(this, A, O), !O)) return (0, d.eachItem)(A, (z) => Y.call(this, z)), this;
        Q.call(this, O);
        let b = { ...O, type: (0, l.getJSONTypes)(O.type), schemaType: (0, l.getJSONTypes)(O.schemaType) };
        return (
          (0, d.eachItem)(
            A,
            b.type.length === 0 ? (z) => Y.call(this, z, b) : (z) => b.type.forEach((q) => Y.call(this, z, b, q)),
          ),
          this
        );
      }
      getKeyword(C) {
        let O = this.RULES.all[C];
        return typeof O == "object" ? O.definition : !!O;
      }
      removeKeyword(C) {
        let { RULES: O } = this;
        delete O.keywords[C], delete O.all[C];
        for (let A of O.rules) {
          let b = A.rules.findIndex((z) => z.keyword === C);
          if (b >= 0) A.rules.splice(b, 1);
        }
        return this;
      }
      addFormat(C, O) {
        if (typeof O == "string") O = new RegExp(O);
        return (this.formats[C] = O), this;
      }
      errorsText(C = this.errors, { separator: O = ", ", dataVar: A = "data" } = {}) {
        if (!C || C.length === 0) return "No errors";
        return C.map((b) => `${A}${b.instancePath} ${b.message}`).reduce((b, z) => b + O + z);
      }
      $dataMetaSchema(C, O) {
        let A = this.RULES.all;
        C = JSON.parse(JSON.stringify(C));
        for (let b of O) {
          let z = b.split("/").slice(1),
            q = C;
          for (let D of z) q = q[D];
          for (let D in A) {
            let H = A[D];
            if (typeof H != "object") continue;
            let { $data: W } = H.definition,
              J = q[D];
            if (W && J) q[D] = X(J);
          }
        }
        return C;
      }
      _removeAllSchemas(C, O) {
        for (let A in C) {
          let b = C[A];
          if (!O || O.test(A)) {
            if (typeof b == "string") delete C[A];
            else if (b && !b.meta) this._cache.delete(b.schema), delete C[A];
          }
        }
      }
      _addSchema(C, O, A, b = this.opts.validateSchema, z = this.opts.addUsedSchema) {
        let q,
          { schemaId: D } = this.opts;
        if (typeof C == "object") q = C[D];
        else if (this.opts.jtd) throw Error("schema must be object");
        else if (typeof C != "boolean") throw Error("schema must be object or boolean");
        let H = this._cache.get(C);
        if (H !== void 0) return H;
        A = (0, i.normalizeId)(q || A);
        let W = i.getSchemaRefs.call(this, C, A);
        if (
          ((H = new n.SchemaEnv({ schema: C, schemaId: D, meta: O, baseId: A, localRefs: W })),
          this._cache.set(H.schema, H),
          z && !A.startsWith("#"))
        ) {
          if (A) this._checkUnique(A);
          this.refs[A] = H;
        }
        if (b) this.validateSchema(C, !0);
        return H;
      }
      _checkUnique(C) {
        if (this.schemas[C] || this.refs[C]) throw Error(`schema with key or id "${C}" already exists`);
      }
      _compileSchemaEnv(C) {
        if (C.meta) this._compileMetaSchema(C);
        else n.compileSchema.call(this, C);
        if (!C.validate) throw Error("ajv implementation error");
        return C.validate;
      }
      _compileMetaSchema(C) {
        let O = this.opts;
        this.opts = this._metaOpts;
        try {
          n.compileSchema.call(this, C);
        } finally {
          this.opts = O;
        }
      }
    };
    (y.ValidationError = s.default), (y.MissingRefError = a.default), (e.default = y);
    function E(C, O, A, b = "error") {
      for (let z in C) {
        let q = z;
        if (q in O) this.logger[b](`${A}: option ${z}. ${C[q]}`);
      }
    }
    function v(C) {
      return (C = (0, i.normalizeId)(C)), this.schemas[C] || this.refs[C];
    }
    function f() {
      let C = this.opts.schemas;
      if (!C) return;
      if (Array.isArray(C)) this.addSchema(C);
      else for (let O in C) this.addSchema(C[O], O);
    }
    function P() {
      for (let C in this.opts.formats) {
        let O = this.opts.formats[C];
        if (O) this.addFormat(C, O);
      }
    }
    function I(C) {
      if (Array.isArray(C)) {
        this.addVocabulary(C);
        return;
      }
      this.logger.warn("keywords option as map is deprecated, pass array");
      for (let O in C) {
        let A = C[O];
        if (!A.keyword) A.keyword = O;
        this.addKeyword(A);
      }
    }
    function L() {
      let C = { ...this.opts };
      for (let O of g) delete C[O];
      return C;
    }
    let k = { log() {}, warn() {}, error() {} };
    function j(C) {
      if (C === !1) return k;
      if (C === void 0) return console;
      if (C.log && C.warn && C.error) return C;
      throw Error("logger must implement log, warn and error methods");
    }
    let F = /^[a-z_$][a-z0-9_$:-]*$/i;
    function V(C, O) {
      let { RULES: A } = this;
      if (
        ((0, d.eachItem)(C, (b) => {
          if (A.keywords[b]) throw Error(`Keyword ${b} is already defined`);
          if (!F.test(b)) throw Error(`Keyword ${b} has invalid name`);
        }),
        !O)
      )
        return;
      if (O.$data && !("code" in O || "validate" in O))
        throw Error('$data keyword must have "code" or "validate" function');
    }
    function Y(C, O, A) {
      var b;
      let z = O === null || O === void 0 ? void 0 : O.post;
      if (A && z) throw Error('keyword with "post" flag cannot have "type"');
      let { RULES: q } = this,
        D = z ? q.post : q.rules.find(({ type: W }) => W === A);
      if (!D) (D = { type: A, rules: [] }), q.rules.push(D);
      if (((q.keywords[C] = !0), !O)) return;
      let H = {
        keyword: C,
        definition: { ...O, type: (0, l.getJSONTypes)(O.type), schemaType: (0, l.getJSONTypes)(O.schemaType) },
      };
      if (O.before) te.call(this, D, H, O.before);
      else D.rules.push(H);
      (q.all[C] = H), (b = O.implements) === null || b === void 0 || b.forEach((W) => this.addKeyword(W));
    }
    function te(C, O, A) {
      let b = C.rules.findIndex((z) => z.keyword === A);
      if (b >= 0) C.rules.splice(b, 0, O);
      else C.rules.push(O), this.logger.warn(`rule ${A} is not defined`);
    }
    function Q(C) {
      let { metaSchema: O } = C;
      if (O === void 0) return;
      if (C.$data && this.opts.$data) O = X(O);
      C.validateSchema = this.compile(O, !0);
    }
    let oe = { $ref: "https://raw.githubusercontent.com/ajv-validator/ajv/master/lib/refs/data.json#" };
    function X(C) {
      return { anyOf: [C, oe] };
    }
  }),
  pi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = {
      keyword: "id",
      code() {
        throw Error('NOT SUPPORTED: keyword "id", use "$id" for schema ID');
      },
    };
    e.default = t;
  }),
  es = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }), (e.callRef = e.getValidate = void 0);
    let t = Ht(),
      r = $e(),
      s = B(),
      a = we(),
      o = Pr(),
      n = Z(),
      c = {
        keyword: "$ref",
        schemaType: "string",
        code(d) {
          let { gen: S, schema: R, it: w } = d,
            { baseId: g, schemaEnv: _, validateName: h, opts: u, self: p } = w,
            { root: m } = _;
          if ((R === "#" || R === "#/") && g === m.baseId) return E();
          let y = o.resolveRef.call(p, m, g, R);
          if (y === void 0) throw new t.default(w.opts.uriResolver, g, R);
          if (y instanceof o.SchemaEnv) return v(y);
          return f(y);
          function E() {
            if (_ === m) return l(d, h, _, _.$async);
            let P = S.scopeValue("root", { ref: m });
            return l(d, s._`${P}.validate`, m, m.$async);
          }
          function v(P) {
            l(d, i(d, P), P, P.$async);
          }
          function f(P) {
            let I = S.scopeValue("schema", u.code.source === !0 ? { ref: P, code: (0, s.stringify)(P) } : { ref: P }),
              L = S.name("valid"),
              k = d.subschema({ schema: P, dataTypes: [], schemaPath: s.nil, topSchemaRef: I, errSchemaPath: R }, L);
            d.mergeEvaluated(k), d.ok(L);
          }
        },
      };
    function i(d, S) {
      let { gen: R } = d;
      return S.validate
        ? R.scopeValue("validate", { ref: S.validate })
        : s._`${R.scopeValue("wrapper", { ref: S })}.validate`;
    }
    e.getValidate = i;
    function l(d, S, R, w) {
      let { gen: g, it: _ } = d,
        { allErrors: h, schemaEnv: u, opts: p } = _,
        m = p.passContext ? a.default.this : s.nil;
      if (w) y();
      else E();
      function y() {
        if (!u.$async) throw Error("async schema referenced by sync schema");
        let P = g.let("valid");
        g.try(
          () => {
            if ((g.code(s._`await ${(0, r.callValidateCode)(d, S, m)}`), f(S), !h)) g.assign(P, !0);
          },
          (I) => {
            if ((g.if(s._`!(${I} instanceof ${_.ValidationError})`, () => g.throw(I)), v(I), !h)) g.assign(P, !1);
          },
        ),
          d.ok(P);
      }
      function E() {
        d.result(
          (0, r.callValidateCode)(d, S, m),
          () => f(S),
          () => v(S),
        );
      }
      function v(P) {
        let I = s._`${P}.errors`;
        g.assign(a.default.vErrors, s._`${a.default.vErrors} === null ? ${I} : ${a.default.vErrors}.concat(${I})`),
          g.assign(a.default.errors, s._`${a.default.vErrors}.length`);
      }
      function f(P) {
        var I;
        if (!_.opts.unevaluated) return;
        let L = (I = R === null || R === void 0 ? void 0 : R.validate) === null || I === void 0 ? void 0 : I.evaluated;
        if (_.props !== !0)
          if (L && !L.dynamicProps) {
            if (L.props !== void 0) _.props = n.mergeEvaluated.props(g, L.props, _.props);
          } else {
            let k = g.var("props", s._`${P}.evaluated.props`);
            _.props = n.mergeEvaluated.props(g, k, _.props, s.Name);
          }
        if (_.items !== !0)
          if (L && !L.dynamicItems) {
            if (L.items !== void 0) _.items = n.mergeEvaluated.items(g, L.items, _.items);
          } else {
            let k = g.var("items", s._`${P}.evaluated.items`);
            _.items = n.mergeEvaluated.items(g, k, _.items, s.Name);
          }
      }
    }
    (e.callRef = l), (e.default = c);
  }),
  za = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = pi(),
      r = es(),
      s = ["$schema", "$id", "$defs", "$vocabulary", { keyword: "$comment" }, "definitions", t.default, r.default];
    e.default = s;
  }),
  Si = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = B(),
      r = t.operators,
      s = {
        maximum: { okStr: "<=", ok: r.LTE, fail: r.GT },
        minimum: { okStr: ">=", ok: r.GTE, fail: r.LT },
        exclusiveMaximum: { okStr: "<", ok: r.LT, fail: r.GTE },
        exclusiveMinimum: { okStr: ">", ok: r.GT, fail: r.LTE },
      },
      a = {
        keyword: Object.keys(s),
        type: "number",
        schemaType: "number",
        $data: !0,
        error: {
          message: ({ keyword: o, schemaCode: n }) => t.str`must be ${s[o].okStr} ${n}`,
          params: ({ keyword: o, schemaCode: n }) => t._`{comparison: ${s[o].okStr}, limit: ${n}}`,
        },
        code(o) {
          let { keyword: n, data: c, schemaCode: i } = o;
          o.fail$data(t._`${c} ${s[n].fail} ${i} || isNaN(${c})`);
        },
      };
    e.default = a;
  }),
  gi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = B(),
      r = {
        keyword: "multipleOf",
        type: "number",
        schemaType: "number",
        $data: !0,
        error: {
          message: ({ schemaCode: s }) => t.str`must be multiple of ${s}`,
          params: ({ schemaCode: s }) => t._`{multipleOf: ${s}}`,
        },
        code(s) {
          let { gen: a, data: o, schemaCode: n, it: c } = s,
            i = c.opts.multipleOfPrecision,
            l = a.let("res"),
            d = i ? t._`Math.abs(Math.round(${l}) - ${l}) > 1e-${i}` : t._`${l} !== parseInt(${l})`;
          s.fail$data(t._`(${n} === 0 || (${l} = ${o}/${n}, ${d}))`);
        },
      };
    e.default = r;
  }),
  _i = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    function t(r) {
      let s = r.length,
        a = 0,
        o = 0,
        n;
      while (o < s)
        if ((a++, (n = r.charCodeAt(o++)), n >= 55296 && n <= 56319 && o < s)) {
          if (((n = r.charCodeAt(o)), (n & 64512) === 56320)) o++;
        }
      return a;
    }
    (e.default = t), (t.code = 'require("ajv/dist/runtime/ucs2length").default');
  }),
  vi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = B(),
      r = Z(),
      s = _i(),
      a = {
        keyword: ["maxLength", "minLength"],
        type: "string",
        schemaType: "number",
        $data: !0,
        error: {
          message({ keyword: o, schemaCode: n }) {
            let c = o === "maxLength" ? "more" : "fewer";
            return t.str`must NOT have ${c} than ${n} characters`;
          },
          params: ({ schemaCode: o }) => t._`{limit: ${o}}`,
        },
        code(o) {
          let { keyword: n, data: c, schemaCode: i, it: l } = o,
            d = n === "maxLength" ? t.operators.GT : t.operators.LT,
            S = l.opts.unicode === !1 ? t._`${c}.length` : t._`${(0, r.useFunc)(o.gen, s.default)}(${c})`;
          o.fail$data(t._`${S} ${d} ${i}`);
        },
      };
    e.default = a;
  }),
  yi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = $e(),
      r = Z(),
      s = B(),
      a = {
        keyword: "pattern",
        type: "string",
        schemaType: "string",
        $data: !0,
        error: {
          message: ({ schemaCode: o }) => s.str`must match pattern "${o}"`,
          params: ({ schemaCode: o }) => s._`{pattern: ${o}}`,
        },
        code(o) {
          let { gen: n, data: c, $data: i, schema: l, schemaCode: d, it: S } = o,
            R = S.opts.unicodeRegExp ? "u" : "";
          if (i) {
            let { regExp: w } = S.opts.code,
              g = w.code === "new RegExp" ? s._`new RegExp` : (0, r.useFunc)(n, w),
              _ = n.let("valid");
            n.try(
              () => n.assign(_, s._`${g}(${d}, ${R}).test(${c})`),
              () => n.assign(_, !1),
            ),
              o.fail$data(s._`!${_}`);
          } else {
            let w = (0, t.usePattern)(o, l);
            o.fail$data(s._`!${w}.test(${c})`);
          }
        },
      };
    e.default = a;
  }),
  bi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = B(),
      r = {
        keyword: ["maxProperties", "minProperties"],
        type: "object",
        schemaType: "number",
        $data: !0,
        error: {
          message({ keyword: s, schemaCode: a }) {
            let o = s === "maxProperties" ? "more" : "fewer";
            return t.str`must NOT have ${o} than ${a} properties`;
          },
          params: ({ schemaCode: s }) => t._`{limit: ${s}}`,
        },
        code(s) {
          let { keyword: a, data: o, schemaCode: n } = s,
            c = a === "maxProperties" ? t.operators.GT : t.operators.LT;
          s.fail$data(t._`Object.keys(${o}).length ${c} ${n}`);
        },
      };
    e.default = r;
  }),
  Ri = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = $e(),
      r = B(),
      s = Z(),
      a = {
        keyword: "required",
        type: "object",
        schemaType: "array",
        $data: !0,
        error: {
          message: ({ params: { missingProperty: o } }) => r.str`must have required property '${o}'`,
          params: ({ params: { missingProperty: o } }) => r._`{missingProperty: ${o}}`,
        },
        code(o) {
          let { gen: n, schema: c, schemaCode: i, data: l, $data: d, it: S } = o,
            { opts: R } = S;
          if (!d && c.length === 0) return;
          let w = c.length >= R.loopRequired;
          if (S.allErrors) g();
          else _();
          if (R.strictRequired) {
            let p = o.parentSchema.properties,
              { definedProperties: m } = o.it;
            for (let y of c)
              if ((p === null || p === void 0 ? void 0 : p[y]) === void 0 && !m.has(y)) {
                let E = `required property "${y}" is not defined at "${S.schemaEnv.baseId + S.errSchemaPath}" (strictRequired)`;
                (0, s.checkStrictMode)(S, E, S.opts.strictRequired);
              }
          }
          function g() {
            if (w || d) o.block$data(r.nil, h);
            else for (let p of c) (0, t.checkReportMissingProp)(o, p);
          }
          function _() {
            let p = n.let("missing");
            if (w || d) {
              let m = n.let("valid", !0);
              o.block$data(m, () => u(p, m)), o.ok(m);
            } else n.if((0, t.checkMissingProp)(o, c, p)), (0, t.reportMissingProp)(o, p), n.else();
          }
          function h() {
            n.forOf("prop", i, (p) => {
              o.setParams({ missingProperty: p }),
                n.if((0, t.noPropertyInData)(n, l, p, R.ownProperties), () => o.error());
            });
          }
          function u(p, m) {
            o.setParams({ missingProperty: p }),
              n.forOf(
                p,
                i,
                () => {
                  n.assign(m, (0, t.propertyInData)(n, l, p, R.ownProperties)),
                    n.if((0, r.not)(m), () => {
                      o.error(), n.break();
                    });
                },
                r.nil,
              );
          }
        },
      };
    e.default = a;
  }),
  wi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = B(),
      r = {
        keyword: ["maxItems", "minItems"],
        type: "array",
        schemaType: "number",
        $data: !0,
        error: {
          message({ keyword: s, schemaCode: a }) {
            let o = s === "maxItems" ? "more" : "fewer";
            return t.str`must NOT have ${o} than ${a} items`;
          },
          params: ({ schemaCode: s }) => t._`{limit: ${s}}`,
        },
        code(s) {
          let { keyword: a, data: o, schemaCode: n } = s,
            c = a === "maxItems" ? t.operators.GT : t.operators.LT;
          s.fail$data(t._`${o}.length ${c} ${n}`);
        },
      };
    e.default = r;
  }),
  ts = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = Ea();
    (t.code = 'require("ajv/dist/runtime/equal").default'), (e.default = t);
  }),
  Ei = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = Rr(),
      r = B(),
      s = Z(),
      a = ts(),
      o = {
        keyword: "uniqueItems",
        type: "array",
        schemaType: "boolean",
        $data: !0,
        error: {
          message: ({ params: { i: n, j: c } }) =>
            r.str`must NOT have duplicate items (items ## ${c} and ${n} are identical)`,
          params: ({ params: { i: n, j: c } }) => r._`{i: ${n}, j: ${c}}`,
        },
        code(n) {
          let { gen: c, data: i, $data: l, schema: d, parentSchema: S, schemaCode: R, it: w } = n;
          if (!l && !d) return;
          let g = c.let("valid"),
            _ = S.items ? (0, t.getSchemaTypes)(S.items) : [];
          n.block$data(g, h, r._`${R} === false`), n.ok(g);
          function h() {
            let y = c.let("i", r._`${i}.length`),
              E = c.let("j");
            n.setParams({ i: y, j: E }), c.assign(g, !0), c.if(r._`${y} > 1`, () => (u() ? p : m)(y, E));
          }
          function u() {
            return _.length > 0 && !_.some((y) => y === "object" || y === "array");
          }
          function p(y, E) {
            let v = c.name("item"),
              f = (0, t.checkDataTypes)(_, v, w.opts.strictNumbers, t.DataType.Wrong),
              P = c.const("indices", r._`{}`);
            c.for(r._`;${y}--;`, () => {
              if ((c.let(v, r._`${i}[${y}]`), c.if(f, r._`continue`), _.length > 1))
                c.if(r._`typeof ${v} == "string"`, r._`${v} += "_"`);
              c.if(r._`typeof ${P}[${v}] == "number"`, () => {
                c.assign(E, r._`${P}[${v}]`), n.error(), c.assign(g, !1).break();
              }).code(r._`${P}[${v}] = ${y}`);
            });
          }
          function m(y, E) {
            let v = (0, s.useFunc)(c, a.default),
              f = c.name("outer");
            c.label(f).for(r._`;${y}--;`, () =>
              c.for(r._`${E} = ${y}; ${E}--;`, () =>
                c.if(r._`${v}(${i}[${y}], ${i}[${E}])`, () => {
                  n.error(), c.assign(g, !1).break(f);
                }),
              ),
            );
          }
        },
      };
    e.default = o;
  }),
  Pi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = B(),
      r = Z(),
      s = ts(),
      a = {
        keyword: "const",
        $data: !0,
        error: { message: "must be equal to constant", params: ({ schemaCode: o }) => t._`{allowedValue: ${o}}` },
        code(o) {
          let { gen: n, data: c, $data: i, schemaCode: l, schema: d } = o;
          if (i || (d && typeof d == "object")) o.fail$data(t._`!${(0, r.useFunc)(n, s.default)}(${c}, ${l})`);
          else o.fail(t._`${d} !== ${c}`);
        },
      };
    e.default = a;
  }),
  Ti = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = B(),
      r = Z(),
      s = ts(),
      a = {
        keyword: "enum",
        schemaType: "array",
        $data: !0,
        error: {
          message: "must be equal to one of the allowed values",
          params: ({ schemaCode: o }) => t._`{allowedValues: ${o}}`,
        },
        code(o) {
          let { gen: n, data: c, $data: i, schema: l, schemaCode: d, it: S } = o;
          if (!i && l.length === 0) throw Error("enum must have non-empty array");
          let R = l.length >= S.opts.loopEnum,
            w,
            g = () => (w !== null && w !== void 0 ? w : (w = (0, r.useFunc)(n, s.default))),
            _;
          if (R || i) (_ = n.let("valid")), o.block$data(_, h);
          else {
            if (!Array.isArray(l)) throw Error("ajv implementation error");
            let p = n.const("vSchema", d);
            _ = (0, t.or)(...l.map((m, y) => u(p, y)));
          }
          o.pass(_);
          function h() {
            n.assign(_, !1), n.forOf("v", d, (p) => n.if(t._`${g()}(${c}, ${p})`, () => n.assign(_, !0).break()));
          }
          function u(p, m) {
            let y = l[m];
            return typeof y === "object" && y !== null ? t._`${g()}(${c}, ${p}[${m}])` : t._`${c} === ${y}`;
          }
        },
      };
    e.default = a;
  }),
  Ca = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = Si(),
      r = gi(),
      s = vi(),
      a = yi(),
      o = bi(),
      n = Ri(),
      c = wi(),
      i = Ei(),
      l = Pi(),
      d = Ti(),
      S = [
        t.default,
        r.default,
        s.default,
        a.default,
        o.default,
        n.default,
        c.default,
        i.default,
        { keyword: "type", schemaType: ["string", "array"] },
        { keyword: "nullable", schemaType: "boolean" },
        l.default,
        d.default,
      ];
    e.default = S;
  }),
  ka = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }), (e.validateAdditionalItems = void 0);
    let t = B(),
      r = Z(),
      s = {
        keyword: "additionalItems",
        type: "array",
        schemaType: ["boolean", "object"],
        before: "uniqueItems",
        error: {
          message: ({ params: { len: o } }) => t.str`must NOT have more than ${o} items`,
          params: ({ params: { len: o } }) => t._`{limit: ${o}}`,
        },
        code(o) {
          let { parentSchema: n, it: c } = o,
            { items: i } = n;
          if (!Array.isArray(i)) {
            (0, r.checkStrictMode)(c, '"additionalItems" is ignored when "items" is not an array of schemas');
            return;
          }
          a(o, i);
        },
      };
    function a(o, n) {
      let { gen: c, schema: i, data: l, keyword: d, it: S } = o;
      S.items = !0;
      let R = c.const("len", t._`${l}.length`);
      if (i === !1) o.setParams({ len: n.length }), o.pass(t._`${R} <= ${n.length}`);
      else if (typeof i == "object" && !(0, r.alwaysValidSchema)(S, i)) {
        let g = c.var("valid", t._`${R} <= ${n.length}`);
        c.if((0, t.not)(g), () => w(g)), o.ok(g);
      }
      function w(g) {
        c.forRange("i", n.length, R, (_) => {
          if ((o.subschema({ keyword: d, dataProp: _, dataPropType: r.Type.Num }, g), !S.allErrors))
            c.if((0, t.not)(g), () => c.break());
        });
      }
    }
    (e.validateAdditionalItems = a), (e.default = s);
  }),
  qa = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }), (e.validateTuple = void 0);
    let t = B(),
      r = Z(),
      s = $e(),
      a = {
        keyword: "items",
        type: "array",
        schemaType: ["object", "array", "boolean"],
        before: "uniqueItems",
        code(n) {
          let { schema: c, it: i } = n;
          if (Array.isArray(c)) return o(n, "additionalItems", c);
          if (((i.items = !0), (0, r.alwaysValidSchema)(i, c))) return;
          n.ok((0, s.validateArray)(n));
        },
      };
    function o(n, c, i = n.schema) {
      let { gen: l, parentSchema: d, data: S, keyword: R, it: w } = n;
      if ((h(d), w.opts.unevaluated && i.length && w.items !== !0))
        w.items = r.mergeEvaluated.items(l, i.length, w.items);
      let g = l.name("valid"),
        _ = l.const("len", t._`${S}.length`);
      i.forEach((u, p) => {
        if ((0, r.alwaysValidSchema)(w, u)) return;
        l.if(t._`${_} > ${p}`, () => n.subschema({ keyword: R, schemaProp: p, dataProp: p }, g)), n.ok(g);
      });
      function h(u) {
        let { opts: p, errSchemaPath: m } = w,
          y = i.length,
          E = y === u.minItems && (y === u.maxItems || u[c] === !1);
        if (p.strictTuples && !E) {
          let v = `"${R}" is ${y}-tuple, but minItems or maxItems/${c} are not specified or different at path "${m}"`;
          (0, r.checkStrictMode)(w, v, p.strictTuples);
        }
      }
    }
    (e.validateTuple = o), (e.default = a);
  }),
  zi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = qa(),
      r = {
        keyword: "prefixItems",
        type: "array",
        schemaType: ["array"],
        before: "uniqueItems",
        code: (s) => (0, t.validateTuple)(s, "items"),
      };
    e.default = r;
  }),
  Ci = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = B(),
      r = Z(),
      s = $e(),
      a = ka(),
      o = {
        keyword: "items",
        type: "array",
        schemaType: ["object", "boolean"],
        before: "uniqueItems",
        error: {
          message: ({ params: { len: n } }) => t.str`must NOT have more than ${n} items`,
          params: ({ params: { len: n } }) => t._`{limit: ${n}}`,
        },
        code(n) {
          let { schema: c, parentSchema: i, it: l } = n,
            { prefixItems: d } = i;
          if (((l.items = !0), (0, r.alwaysValidSchema)(l, c))) return;
          if (d) (0, a.validateAdditionalItems)(n, d);
          else n.ok((0, s.validateArray)(n));
        },
      };
    e.default = o;
  }),
  ki = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = B(),
      r = Z(),
      s = {
        keyword: "contains",
        type: "array",
        schemaType: ["object", "boolean"],
        before: "uniqueItems",
        trackErrors: !0,
        error: {
          message: ({ params: { min: a, max: o } }) =>
            o === void 0
              ? t.str`must contain at least ${a} valid item(s)`
              : t.str`must contain at least ${a} and no more than ${o} valid item(s)`,
          params: ({ params: { min: a, max: o } }) =>
            o === void 0 ? t._`{minContains: ${a}}` : t._`{minContains: ${a}, maxContains: ${o}}`,
        },
        code(a) {
          let { gen: o, schema: n, parentSchema: c, data: i, it: l } = a,
            d,
            S,
            { minContains: R, maxContains: w } = c;
          if (l.opts.next) (d = R === void 0 ? 1 : R), (S = w);
          else d = 1;
          let g = o.const("len", t._`${i}.length`);
          if ((a.setParams({ min: d, max: S }), S === void 0 && d === 0)) {
            (0, r.checkStrictMode)(l, '"minContains" == 0 without "maxContains": "contains" keyword ignored');
            return;
          }
          if (S !== void 0 && d > S) {
            (0, r.checkStrictMode)(l, '"minContains" > "maxContains" is always invalid'), a.fail();
            return;
          }
          if ((0, r.alwaysValidSchema)(l, n)) {
            let m = t._`${g} >= ${d}`;
            if (S !== void 0) m = t._`${m} && ${g} <= ${S}`;
            a.pass(m);
            return;
          }
          l.items = !0;
          let _ = o.name("valid");
          if (S === void 0 && d === 1) u(_, () => o.if(_, () => o.break()));
          else if (d === 0) {
            if ((o.let(_, !0), S !== void 0)) o.if(t._`${i}.length > 0`, h);
          } else o.let(_, !1), h();
          a.result(_, () => a.reset());
          function h() {
            let m = o.name("_valid"),
              y = o.let("count", 0);
            u(m, () => o.if(m, () => p(y)));
          }
          function u(m, y) {
            o.forRange("i", 0, g, (E) => {
              a.subschema({ keyword: "contains", dataProp: E, dataPropType: r.Type.Num, compositeRule: !0 }, m), y();
            });
          }
          function p(m) {
            if ((o.code(t._`${m}++`), S === void 0)) o.if(t._`${m} >= ${d}`, () => o.assign(_, !0).break());
            else if ((o.if(t._`${m} > ${S}`, () => o.assign(_, !1).break()), d === 1)) o.assign(_, !0);
            else o.if(t._`${m} >= ${d}`, () => o.assign(_, !0));
          }
        },
      };
    e.default = s;
  }),
  rs = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }),
      (e.validateSchemaDeps = e.validatePropertyDeps = e.error = void 0);
    let t = B(),
      r = Z(),
      s = $e();
    e.error = {
      message: ({ params: { property: i, depsCount: l, deps: d } }) => {
        let S = l === 1 ? "property" : "properties";
        return t.str`must have ${S} ${d} when property ${i} is present`;
      },
      params: ({ params: { property: i, depsCount: l, deps: d, missingProperty: S } }) => t._`{property: ${i},
    missingProperty: ${S},
    depsCount: ${l},
    deps: ${d}}`,
    };
    let a = {
      keyword: "dependencies",
      type: "object",
      schemaType: "object",
      error: e.error,
      code(i) {
        let [l, d] = o(i);
        n(i, l), c(i, d);
      },
    };
    function o({ schema: i }) {
      let l = {},
        d = {};
      for (let S in i) {
        if (S === "__proto__") continue;
        let R = Array.isArray(i[S]) ? l : d;
        R[S] = i[S];
      }
      return [l, d];
    }
    function n(i, l = i.schema) {
      let { gen: d, data: S, it: R } = i;
      if (Object.keys(l).length === 0) return;
      let w = d.let("missing");
      for (let g in l) {
        let _ = l[g];
        if (_.length === 0) continue;
        let h = (0, s.propertyInData)(d, S, g, R.opts.ownProperties);
        if ((i.setParams({ property: g, depsCount: _.length, deps: _.join(", ") }), R.allErrors))
          d.if(h, () => {
            for (let u of _) (0, s.checkReportMissingProp)(i, u);
          });
        else d.if(t._`${h} && (${(0, s.checkMissingProp)(i, _, w)})`), (0, s.reportMissingProp)(i, w), d.else();
      }
    }
    e.validatePropertyDeps = n;
    function c(i, l = i.schema) {
      let { gen: d, data: S, keyword: R, it: w } = i,
        g = d.name("valid");
      for (let _ in l) {
        if ((0, r.alwaysValidSchema)(w, l[_])) continue;
        d.if(
          (0, s.propertyInData)(d, S, _, w.opts.ownProperties),
          () => {
            let h = i.subschema({ keyword: R, schemaProp: _ }, g);
            i.mergeValidEvaluated(h, g);
          },
          () => d.var(g, !0),
        ),
          i.ok(g);
      }
    }
    (e.validateSchemaDeps = c), (e.default = a);
  }),
  qi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = B(),
      r = Z(),
      s = {
        keyword: "propertyNames",
        type: "object",
        schemaType: ["object", "boolean"],
        error: {
          message: "property name must be valid",
          params: ({ params: a }) => t._`{propertyName: ${a.propertyName}}`,
        },
        code(a) {
          let { gen: o, schema: n, data: c, it: i } = a;
          if ((0, r.alwaysValidSchema)(i, n)) return;
          let l = o.name("valid");
          o.forIn("key", c, (d) => {
            a.setParams({ propertyName: d }),
              a.subschema(
                { keyword: "propertyNames", data: d, dataTypes: ["string"], propertyName: d, compositeRule: !0 },
                l,
              ),
              o.if((0, t.not)(l), () => {
                if ((a.error(!0), !i.allErrors)) o.break();
              });
          }),
            a.ok(l);
        },
      };
    e.default = s;
  }),
  Oa = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = $e(),
      r = B(),
      s = we(),
      a = Z(),
      o = {
        keyword: "additionalProperties",
        type: ["object"],
        schemaType: ["boolean", "object"],
        allowUndefined: !0,
        trackErrors: !0,
        error: {
          message: "must NOT have additional properties",
          params: ({ params: n }) => r._`{additionalProperty: ${n.additionalProperty}}`,
        },
        code(n) {
          let { gen: c, schema: i, parentSchema: l, data: d, errsCount: S, it: R } = n;
          if (!S) throw Error("ajv implementation error");
          let { allErrors: w, opts: g } = R;
          if (((R.props = !0), g.removeAdditional !== "all" && (0, a.alwaysValidSchema)(R, i))) return;
          let _ = (0, t.allSchemaProperties)(l.properties),
            h = (0, t.allSchemaProperties)(l.patternProperties);
          u(), n.ok(r._`${S} === ${s.default.errors}`);
          function u() {
            c.forIn("key", d, (v) => {
              if (!_.length && !h.length) y(v);
              else c.if(p(v), () => y(v));
            });
          }
          function p(v) {
            let f;
            if (_.length > 8) {
              let P = (0, a.schemaRefOrVal)(R, l.properties, "properties");
              f = (0, t.isOwnProperty)(c, P, v);
            } else if (_.length) f = (0, r.or)(..._.map((P) => r._`${v} === ${P}`));
            else f = r.nil;
            if (h.length) f = (0, r.or)(f, ...h.map((P) => r._`${(0, t.usePattern)(n, P)}.test(${v})`));
            return (0, r.not)(f);
          }
          function m(v) {
            c.code(r._`delete ${d}[${v}]`);
          }
          function y(v) {
            if (g.removeAdditional === "all" || (g.removeAdditional && i === !1)) {
              m(v);
              return;
            }
            if (i === !1) {
              if ((n.setParams({ additionalProperty: v }), n.error(), !w)) c.break();
              return;
            }
            if (typeof i == "object" && !(0, a.alwaysValidSchema)(R, i)) {
              let f = c.name("valid");
              if (g.removeAdditional === "failing")
                E(v, f, !1),
                  c.if((0, r.not)(f), () => {
                    n.reset(), m(v);
                  });
              else if ((E(v, f), !w)) c.if((0, r.not)(f), () => c.break());
            }
          }
          function E(v, f, P) {
            let I = { keyword: "additionalProperties", dataProp: v, dataPropType: a.Type.Str };
            if (P === !1) Object.assign(I, { compositeRule: !0, createErrors: !1, allErrors: !1 });
            n.subschema(I, f);
          }
        },
      };
    e.default = o;
  }),
  Ii = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = Ft(),
      r = $e(),
      s = Z(),
      a = Oa(),
      o = {
        keyword: "properties",
        type: "object",
        schemaType: "object",
        code(n) {
          let { gen: c, schema: i, parentSchema: l, data: d, it: S } = n;
          if (S.opts.removeAdditional === "all" && l.additionalProperties === void 0)
            a.default.code(new t.KeywordCxt(S, a.default, "additionalProperties"));
          let R = (0, r.allSchemaProperties)(i);
          for (let u of R) S.definedProperties.add(u);
          if (S.opts.unevaluated && R.length && S.props !== !0)
            S.props = s.mergeEvaluated.props(c, (0, s.toHash)(R), S.props);
          let w = R.filter((u) => !(0, s.alwaysValidSchema)(S, i[u]));
          if (w.length === 0) return;
          let g = c.name("valid");
          for (let u of w) {
            if (_(u)) h(u);
            else {
              if ((c.if((0, r.propertyInData)(c, d, u, S.opts.ownProperties)), h(u), !S.allErrors)) c.else().var(g, !0);
              c.endIf();
            }
            n.it.definedProperties.add(u), n.ok(g);
          }
          function _(u) {
            return S.opts.useDefaults && !S.compositeRule && i[u].default !== void 0;
          }
          function h(u) {
            n.subschema({ keyword: "properties", schemaProp: u, dataProp: u }, g);
          }
        },
      };
    e.default = o;
  }),
  Oi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = $e(),
      r = B(),
      s = Z(),
      a = Z(),
      o = {
        keyword: "patternProperties",
        type: "object",
        schemaType: "object",
        code(n) {
          let { gen: c, schema: i, data: l, parentSchema: d, it: S } = n,
            { opts: R } = S,
            w = (0, t.allSchemaProperties)(i),
            g = w.filter((E) => (0, s.alwaysValidSchema)(S, i[E]));
          if (w.length === 0 || (g.length === w.length && (!S.opts.unevaluated || S.props === !0))) return;
          let _ = R.strictSchema && !R.allowMatchingProperties && d.properties,
            h = c.name("valid");
          if (S.props !== !0 && !(S.props instanceof r.Name)) S.props = (0, a.evaluatedPropsToName)(c, S.props);
          let { props: u } = S;
          p();
          function p() {
            for (let E of w) {
              if (_) m(E);
              if (S.allErrors) y(E);
              else c.var(h, !0), y(E), c.if(h);
            }
          }
          function m(E) {
            for (let v in _)
              if (new RegExp(E).test(v))
                (0, s.checkStrictMode)(S, `property ${v} matches pattern ${E} (use allowMatchingProperties)`);
          }
          function y(E) {
            c.forIn("key", l, (v) => {
              c.if(r._`${(0, t.usePattern)(n, E)}.test(${v})`, () => {
                let f = g.includes(E);
                if (!f)
                  n.subschema(
                    { keyword: "patternProperties", schemaProp: E, dataProp: v, dataPropType: a.Type.Str },
                    h,
                  );
                if (S.opts.unevaluated && u !== !0) c.assign(r._`${u}[${v}]`, !0);
                else if (!f && !S.allErrors) c.if((0, r.not)(h), () => c.break());
              });
            });
          }
        },
      };
    e.default = o;
  }),
  Ni = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = Z(),
      r = {
        keyword: "not",
        schemaType: ["object", "boolean"],
        trackErrors: !0,
        code(s) {
          let { gen: a, schema: o, it: n } = s;
          if ((0, t.alwaysValidSchema)(n, o)) {
            s.fail();
            return;
          }
          let c = a.name("valid");
          s.subschema({ keyword: "not", compositeRule: !0, createErrors: !1, allErrors: !1 }, c),
            s.failResult(
              c,
              () => s.reset(),
              () => s.error(),
            );
        },
        error: { message: "must NOT be valid" },
      };
    e.default = r;
  }),
  Mi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = {
      keyword: "anyOf",
      schemaType: "array",
      trackErrors: !0,
      code: $e().validateUnion,
      error: { message: "must match a schema in anyOf" },
    };
    e.default = t;
  }),
  ji = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = B(),
      r = Z(),
      s = {
        keyword: "oneOf",
        schemaType: "array",
        trackErrors: !0,
        error: {
          message: "must match exactly one schema in oneOf",
          params: ({ params: a }) => t._`{passingSchemas: ${a.passing}}`,
        },
        code(a) {
          let { gen: o, schema: n, parentSchema: c, it: i } = a;
          if (!Array.isArray(n)) throw Error("ajv implementation error");
          if (i.opts.discriminator && c.discriminator) return;
          let l = n,
            d = o.let("valid", !1),
            S = o.let("passing", null),
            R = o.name("_valid");
          a.setParams({ passing: S }),
            o.block(w),
            a.result(
              d,
              () => a.reset(),
              () => a.error(!0),
            );
          function w() {
            l.forEach((g, _) => {
              let h;
              if ((0, r.alwaysValidSchema)(i, g)) o.var(R, !0);
              else h = a.subschema({ keyword: "oneOf", schemaProp: _, compositeRule: !0 }, R);
              if (_ > 0) o.if(t._`${R} && ${d}`).assign(d, !1).assign(S, t._`[${S}, ${_}]`).else();
              o.if(R, () => {
                if ((o.assign(d, !0), o.assign(S, _), h)) a.mergeEvaluated(h, t.Name);
              });
            });
          }
        },
      };
    e.default = s;
  }),
  Li = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = Z(),
      r = {
        keyword: "allOf",
        schemaType: "array",
        code(s) {
          let { gen: a, schema: o, it: n } = s;
          if (!Array.isArray(o)) throw Error("ajv implementation error");
          let c = a.name("valid");
          o.forEach((i, l) => {
            if ((0, t.alwaysValidSchema)(n, i)) return;
            let d = s.subschema({ keyword: "allOf", schemaProp: l }, c);
            s.ok(c), s.mergeEvaluated(d);
          });
        },
      };
    e.default = r;
  }),
  Ui = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = B(),
      r = Z(),
      s = {
        keyword: "if",
        schemaType: ["object", "boolean"],
        trackErrors: !0,
        error: {
          message: ({ params: o }) => t.str`must match "${o.ifClause}" schema`,
          params: ({ params: o }) => t._`{failingKeyword: ${o.ifClause}}`,
        },
        code(o) {
          let { gen: n, parentSchema: c, it: i } = o;
          if (c.then === void 0 && c.else === void 0)
            (0, r.checkStrictMode)(i, '"if" without "then" and "else" is ignored');
          let l = a(i, "then"),
            d = a(i, "else");
          if (!l && !d) return;
          let S = n.let("valid", !0),
            R = n.name("_valid");
          if ((w(), o.reset(), l && d)) {
            let _ = n.let("ifClause");
            o.setParams({ ifClause: _ }), n.if(R, g("then", _), g("else", _));
          } else if (l) n.if(R, g("then"));
          else n.if((0, t.not)(R), g("else"));
          o.pass(S, () => o.error(!0));
          function w() {
            let _ = o.subschema({ keyword: "if", compositeRule: !0, createErrors: !1, allErrors: !1 }, R);
            o.mergeEvaluated(_);
          }
          function g(_, h) {
            return () => {
              let u = o.subschema({ keyword: _ }, R);
              if ((n.assign(S, R), o.mergeValidEvaluated(u, S), h)) n.assign(h, t._`${_}`);
              else o.setParams({ ifClause: _ });
            };
          }
        },
      };
    function a(o, n) {
      let c = o.schema[n];
      return c !== void 0 && !(0, r.alwaysValidSchema)(o, c);
    }
    e.default = s;
  }),
  Di = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = Z(),
      r = {
        keyword: ["then", "else"],
        schemaType: ["object", "boolean"],
        code({ keyword: s, parentSchema: a, it: o }) {
          if (a.if === void 0) (0, t.checkStrictMode)(o, `"${s}" without "if" is ignored`);
        },
      };
    e.default = r;
  }),
  Na = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = ka(),
      r = zi(),
      s = qa(),
      a = Ci(),
      o = ki(),
      n = rs(),
      c = qi(),
      i = Oa(),
      l = Ii(),
      d = Oi(),
      S = Ni(),
      R = Mi(),
      w = ji(),
      g = Li(),
      _ = Ui(),
      h = Di();
    function u(p = !1) {
      let m = [
        S.default,
        R.default,
        w.default,
        g.default,
        _.default,
        h.default,
        c.default,
        i.default,
        n.default,
        l.default,
        d.default,
      ];
      if (p) m.push(r.default, a.default);
      else m.push(t.default, s.default);
      return m.push(o.default), m;
    }
    e.default = u;
  }),
  Vi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = B(),
      r = {
        keyword: "format",
        type: ["number", "string"],
        schemaType: "string",
        $data: !0,
        error: {
          message: ({ schemaCode: s }) => t.str`must match format "${s}"`,
          params: ({ schemaCode: s }) => t._`{format: ${s}}`,
        },
        code(s, a) {
          let { gen: o, data: n, $data: c, schema: i, schemaCode: l, it: d } = s,
            { opts: S, errSchemaPath: R, schemaEnv: w, self: g } = d;
          if (!S.validateFormats) return;
          if (c) _();
          else h();
          function _() {
            let u = o.scopeValue("formats", { ref: g.formats, code: S.code.formats }),
              p = o.const("fDef", t._`${u}[${l}]`),
              m = o.let("fType"),
              y = o.let("format");
            o.if(
              t._`typeof ${p} == "object" && !(${p} instanceof RegExp)`,
              () => o.assign(m, t._`${p}.type || "string"`).assign(y, t._`${p}.validate`),
              () => o.assign(m, t._`"string"`).assign(y, p),
            ),
              s.fail$data((0, t.or)(E(), v()));
            function E() {
              if (S.strictSchema === !1) return t.nil;
              return t._`${l} && !${y}`;
            }
            function v() {
              let f = w.$async ? t._`(${p}.async ? await ${y}(${n}) : ${y}(${n}))` : t._`${y}(${n})`,
                P = t._`(typeof ${y} == "function" ? ${f} : ${y}.test(${n}))`;
              return t._`${y} && ${y} !== true && ${m} === ${a} && !${P}`;
            }
          }
          function h() {
            let u = g.formats[i];
            if (!u) {
              E();
              return;
            }
            if (u === !0) return;
            let [p, m, y] = v(u);
            if (p === a) s.pass(f());
            function E() {
              if (S.strictSchema === !1) {
                g.logger.warn(P());
                return;
              }
              throw Error(P());
              function P() {
                return `unknown format "${i}" ignored in schema at path "${R}"`;
              }
            }
            function v(P) {
              let I =
                  P instanceof RegExp
                    ? (0, t.regexpCode)(P)
                    : S.code.formats
                      ? t._`${S.code.formats}${(0, t.getProperty)(i)}`
                      : void 0,
                L = o.scopeValue("formats", { key: i, ref: P, code: I });
              if (typeof P == "object" && !(P instanceof RegExp))
                return [P.type || "string", P.validate, t._`${L}.validate`];
              return ["string", P, L];
            }
            function f() {
              if (typeof u == "object" && !(u instanceof RegExp) && u.async) {
                if (!w.$async) throw Error("async format in sync schema");
                return t._`await ${y}(${n})`;
              }
              return typeof m == "function" ? t._`${y}(${n})` : t._`${y}.test(${n})`;
            }
          }
        },
      };
    e.default = r;
  }),
  Ma = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = [Vi().default];
    e.default = t;
  }),
  ja = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }),
      (e.contentVocabulary = e.metadataVocabulary = void 0),
      (e.metadataVocabulary = ["title", "description", "default", "deprecated", "readOnly", "writeOnly", "examples"]),
      (e.contentVocabulary = ["contentMediaType", "contentEncoding", "contentSchema"]);
  }),
  Fi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = za(),
      r = Ca(),
      s = Na(),
      a = Ma(),
      o = ja(),
      n = [t.default, r.default, (0, s.default)(), a.default, o.metadataVocabulary, o.contentVocabulary];
    e.default = n;
  }),
  Hi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }), (e.DiscrError = void 0);
    var t;
    (function (r) {
      (r.Tag = "tag"), (r.Mapping = "mapping");
    })(t || (e.DiscrError = t = {}));
  }),
  Aa = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = B(),
      r = Hi(),
      s = Pr(),
      a = Ht(),
      o = Z(),
      n = {
        keyword: "discriminator",
        type: "object",
        schemaType: "object",
        error: {
          message: ({ params: { discrError: c, tagName: i } }) =>
            c === r.DiscrError.Tag ? `tag "${i}" must be string` : `value of tag "${i}" must be in oneOf`,
          params: ({ params: { discrError: c, tag: i, tagName: l } }) => t._`{error: ${c}, tag: ${l}, tagValue: ${i}}`,
        },
        code(c) {
          let { gen: i, data: l, schema: d, parentSchema: S, it: R } = c,
            { oneOf: w } = S;
          if (!R.opts.discriminator) throw Error("discriminator: requires discriminator option");
          let g = d.propertyName;
          if (typeof g != "string") throw Error("discriminator: requires propertyName");
          if (d.mapping) throw Error("discriminator: mapping is not supported");
          if (!w) throw Error("discriminator: requires oneOf keyword");
          let _ = i.let("valid", !1),
            h = i.const("tag", t._`${l}${(0, t.getProperty)(g)}`);
          i.if(
            t._`typeof ${h} == "string"`,
            () => u(),
            () => c.error(!1, { discrError: r.DiscrError.Tag, tag: h, tagName: g }),
          ),
            c.ok(_);
          function u() {
            let y = m();
            i.if(!1);
            for (let E in y) i.elseIf(t._`${h} === ${E}`), i.assign(_, p(y[E]));
            i.else(), c.error(!1, { discrError: r.DiscrError.Mapping, tag: h, tagName: g }), i.endIf();
          }
          function p(y) {
            let E = i.name("valid"),
              v = c.subschema({ keyword: "oneOf", schemaProp: y }, E);
            return c.mergeEvaluated(v, t.Name), E;
          }
          function m() {
            var y;
            let E = {},
              v = P(S),
              f = !0;
            for (let k = 0; k < w.length; k++) {
              let j = w[k];
              if ((j === null || j === void 0 ? void 0 : j.$ref) && !(0, o.schemaHasRulesButRef)(j, R.self.RULES)) {
                let V = j.$ref;
                if (((j = s.resolveRef.call(R.self, R.schemaEnv.root, R.baseId, V)), j instanceof s.SchemaEnv))
                  j = j.schema;
                if (j === void 0) throw new a.default(R.opts.uriResolver, R.baseId, V);
              }
              let F = (y = j === null || j === void 0 ? void 0 : j.properties) === null || y === void 0 ? void 0 : y[g];
              if (typeof F != "object")
                throw Error(`discriminator: oneOf subschemas (or referenced schemas) must have "properties/${g}"`);
              (f = f && (v || P(j))), I(F, k);
            }
            if (!f) throw Error(`discriminator: "${g}" must be required`);
            return E;
            function P({ required: k }) {
              return Array.isArray(k) && k.includes(g);
            }
            function I(k, j) {
              if (k.const) L(k.const, j);
              else if (k.enum) for (let F of k.enum) L(F, j);
              else throw Error(`discriminator: "properties/${g}" must have "const" or "enum"`);
            }
            function L(k, j) {
              if (typeof k != "string" || k in E) throw Error(`discriminator: "${g}" values must be unique strings`);
              E[k] = j;
            }
          }
        },
      };
    e.default = n;
  }),
  xi = U((e, t) => {
    t.exports = {
      $schema: "http://json-schema.org/draft-07/schema#",
      $id: "http://json-schema.org/draft-07/schema#",
      title: "Core schema meta-schema",
      definitions: {
        schemaArray: { type: "array", minItems: 1, items: { $ref: "#" } },
        nonNegativeInteger: { type: "integer", minimum: 0 },
        nonNegativeIntegerDefault0: { allOf: [{ $ref: "#/definitions/nonNegativeInteger" }, { default: 0 }] },
        simpleTypes: { enum: ["array", "boolean", "integer", "null", "number", "object", "string"] },
        stringArray: { type: "array", items: { type: "string" }, uniqueItems: !0, default: [] },
      },
      type: ["object", "boolean"],
      properties: {
        $id: { type: "string", format: "uri-reference" },
        $schema: { type: "string", format: "uri" },
        $ref: { type: "string", format: "uri-reference" },
        $comment: { type: "string" },
        title: { type: "string" },
        description: { type: "string" },
        default: !0,
        readOnly: { type: "boolean", default: !1 },
        examples: { type: "array", items: !0 },
        multipleOf: { type: "number", exclusiveMinimum: 0 },
        maximum: { type: "number" },
        exclusiveMaximum: { type: "number" },
        minimum: { type: "number" },
        exclusiveMinimum: { type: "number" },
        maxLength: { $ref: "#/definitions/nonNegativeInteger" },
        minLength: { $ref: "#/definitions/nonNegativeIntegerDefault0" },
        pattern: { type: "string", format: "regex" },
        additionalItems: { $ref: "#" },
        items: { anyOf: [{ $ref: "#" }, { $ref: "#/definitions/schemaArray" }], default: !0 },
        maxItems: { $ref: "#/definitions/nonNegativeInteger" },
        minItems: { $ref: "#/definitions/nonNegativeIntegerDefault0" },
        uniqueItems: { type: "boolean", default: !1 },
        contains: { $ref: "#" },
        maxProperties: { $ref: "#/definitions/nonNegativeInteger" },
        minProperties: { $ref: "#/definitions/nonNegativeIntegerDefault0" },
        required: { $ref: "#/definitions/stringArray" },
        additionalProperties: { $ref: "#" },
        definitions: { type: "object", additionalProperties: { $ref: "#" }, default: {} },
        properties: { type: "object", additionalProperties: { $ref: "#" }, default: {} },
        patternProperties: {
          type: "object",
          additionalProperties: { $ref: "#" },
          propertyNames: { format: "regex" },
          default: {},
        },
        dependencies: {
          type: "object",
          additionalProperties: { anyOf: [{ $ref: "#" }, { $ref: "#/definitions/stringArray" }] },
        },
        propertyNames: { $ref: "#" },
        const: !0,
        enum: { type: "array", items: !0, minItems: 1, uniqueItems: !0 },
        type: {
          anyOf: [
            { $ref: "#/definitions/simpleTypes" },
            { type: "array", items: { $ref: "#/definitions/simpleTypes" }, minItems: 1, uniqueItems: !0 },
          ],
        },
        format: { type: "string" },
        contentMediaType: { type: "string" },
        contentEncoding: { type: "string" },
        if: { $ref: "#" },
        then: { $ref: "#" },
        else: { $ref: "#" },
        allOf: { $ref: "#/definitions/schemaArray" },
        anyOf: { $ref: "#/definitions/schemaArray" },
        oneOf: { $ref: "#/definitions/schemaArray" },
        not: { $ref: "#" },
      },
      default: !0,
    };
  }),
  La = U((e, t) => {
    Object.defineProperty(e, "__esModule", { value: !0 }),
      (e.MissingRefError =
        e.ValidationError =
        e.CodeGen =
        e.Name =
        e.nil =
        e.stringify =
        e.str =
        e._ =
        e.KeywordCxt =
        e.Ajv =
          void 0);
    let r = Ta(),
      s = Fi(),
      a = Aa(),
      o = xi(),
      n = ["/properties"],
      c = "http://json-schema.org/draft-07/schema";
    var i = class extends r.default {
      _addVocabularies() {
        if ((super._addVocabularies(), s.default.forEach((w) => this.addVocabulary(w)), this.opts.discriminator))
          this.addKeyword(a.default);
      }
      _addDefaultMetaSchema() {
        if ((super._addDefaultMetaSchema(), !this.opts.meta)) return;
        let w = this.opts.$data ? this.$dataMetaSchema(o, n) : o;
        this.addMetaSchema(w, c, !1), (this.refs["http://json-schema.org/schema"] = c);
      }
      defaultMeta() {
        return (this.opts.defaultMeta = super.defaultMeta() || (this.getSchema(c) ? c : void 0));
      }
    };
    (e.Ajv = i),
      (t.exports = e = i),
      (t.exports.Ajv = i),
      Object.defineProperty(e, "__esModule", { value: !0 }),
      (e.default = i);
    var l = Ft();
    Object.defineProperty(e, "KeywordCxt", {
      enumerable: !0,
      get: function () {
        return l.KeywordCxt;
      },
    });
    var d = B();
    Object.defineProperty(e, "_", {
      enumerable: !0,
      get: function () {
        return d._;
      },
    }),
      Object.defineProperty(e, "str", {
        enumerable: !0,
        get: function () {
          return d.str;
        },
      }),
      Object.defineProperty(e, "stringify", {
        enumerable: !0,
        get: function () {
          return d.stringify;
        },
      }),
      Object.defineProperty(e, "nil", {
        enumerable: !0,
        get: function () {
          return d.nil;
        },
      }),
      Object.defineProperty(e, "Name", {
        enumerable: !0,
        get: function () {
          return d.Name;
        },
      }),
      Object.defineProperty(e, "CodeGen", {
        enumerable: !0,
        get: function () {
          return d.CodeGen;
        },
      });
    var S = Er();
    Object.defineProperty(e, "ValidationError", {
      enumerable: !0,
      get: function () {
        return S.default;
      },
    });
    var R = Ht();
    Object.defineProperty(e, "MissingRefError", {
      enumerable: !0,
      get: function () {
        return R.default;
      },
    });
  }),
  Ua = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }), (e.dynamicAnchor = void 0);
    let t = B(),
      r = we(),
      s = Pr(),
      a = es(),
      o = { keyword: "$dynamicAnchor", schemaType: "string", code: (i) => n(i, i.schema) };
    function n(i, l) {
      let { gen: d, it: S } = i;
      S.schemaEnv.root.dynamicAnchors[l] = !0;
      let R = t._`${r.default.dynamicAnchors}${(0, t.getProperty)(l)}`,
        w = S.errSchemaPath === "#" ? S.validateName : c(i);
      d.if(t._`!${R}`, () => d.assign(R, w));
    }
    e.dynamicAnchor = n;
    function c(i) {
      let { schemaEnv: l, schema: d, self: S } = i.it,
        { root: R, baseId: w, localRefs: g, meta: _ } = l.root,
        { schemaId: h } = S.opts,
        u = new s.SchemaEnv({ schema: d, schemaId: h, root: R, baseId: w, localRefs: g, meta: _ });
      return s.compileSchema.call(S, u), (0, a.getValidate)(i, u);
    }
    e.default = o;
  }),
  Da = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }), (e.dynamicRef = void 0);
    let t = B(),
      r = we(),
      s = es(),
      a = { keyword: "$dynamicRef", schemaType: "string", code: (n) => o(n, n.schema) };
    function o(n, c) {
      let { gen: i, keyword: l, it: d } = n;
      if (c[0] !== "#") throw Error(`"${l}" only supports hash fragment reference`);
      let S = c.slice(1);
      if (d.allErrors) R();
      else {
        let g = i.let("valid", !1);
        R(g), n.ok(g);
      }
      function R(g) {
        if (d.schemaEnv.root.dynamicAnchors[S]) {
          let _ = i.let("_v", t._`${r.default.dynamicAnchors}${(0, t.getProperty)(S)}`);
          i.if(_, w(_, g), w(d.validateName, g));
        } else w(d.validateName, g)();
      }
      function w(g, _) {
        return _
          ? () =>
              i.block(() => {
                (0, s.callRef)(n, g), i.let(_, !0);
              })
          : () => (0, s.callRef)(n, g);
      }
    }
    (e.dynamicRef = o), (e.default = a);
  }),
  Ji = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = Ua(),
      r = Z(),
      s = {
        keyword: "$recursiveAnchor",
        schemaType: "boolean",
        code(a) {
          if (a.schema) (0, t.dynamicAnchor)(a, "");
          else (0, r.checkStrictMode)(a.it, "$recursiveAnchor: false is ignored");
        },
      };
    e.default = s;
  }),
  Gi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = Da(),
      r = { keyword: "$recursiveRef", schemaType: "string", code: (s) => (0, t.dynamicRef)(s, s.schema) };
    e.default = r;
  }),
  Bi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = Ua(),
      r = Da(),
      s = Ji(),
      a = Gi(),
      o = [t.default, r.default, s.default, a.default];
    e.default = o;
  }),
  Wi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = rs(),
      r = {
        keyword: "dependentRequired",
        type: "object",
        schemaType: "object",
        error: t.error,
        code: (s) => (0, t.validatePropertyDeps)(s),
      };
    e.default = r;
  }),
  Yi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = rs(),
      r = {
        keyword: "dependentSchemas",
        type: "object",
        schemaType: "object",
        code: (s) => (0, t.validateSchemaDeps)(s),
      };
    e.default = r;
  }),
  Xi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = Z(),
      r = {
        keyword: ["maxContains", "minContains"],
        type: "array",
        schemaType: "number",
        code({ keyword: s, parentSchema: a, it: o }) {
          if (a.contains === void 0) (0, t.checkStrictMode)(o, `"${s}" without "contains" is ignored`);
        },
      };
    e.default = r;
  }),
  Zi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = Wi(),
      r = Yi(),
      s = Xi(),
      a = [t.default, r.default, s.default];
    e.default = a;
  }),
  Qi = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = B(),
      r = Z(),
      s = we(),
      a = {
        keyword: "unevaluatedProperties",
        type: "object",
        schemaType: ["boolean", "object"],
        trackErrors: !0,
        error: {
          message: "must NOT have unevaluated properties",
          params: ({ params: o }) => t._`{unevaluatedProperty: ${o.unevaluatedProperty}}`,
        },
        code(o) {
          let { gen: n, schema: c, data: i, errsCount: l, it: d } = o;
          if (!l) throw Error("ajv implementation error");
          let { allErrors: S, props: R } = d;
          if (R instanceof t.Name) n.if(t._`${R} !== true`, () => n.forIn("key", i, (h) => n.if(g(R, h), () => w(h))));
          else if (R !== !0) n.forIn("key", i, (h) => (R === void 0 ? w(h) : n.if(_(R, h), () => w(h))));
          (d.props = !0), o.ok(t._`${l} === ${s.default.errors}`);
          function w(h) {
            if (c === !1) {
              if ((o.setParams({ unevaluatedProperty: h }), o.error(), !S)) n.break();
              return;
            }
            if (!(0, r.alwaysValidSchema)(d, c)) {
              let u = n.name("valid");
              if ((o.subschema({ keyword: "unevaluatedProperties", dataProp: h, dataPropType: r.Type.Str }, u), !S))
                n.if((0, t.not)(u), () => n.break());
            }
          }
          function g(h, u) {
            return t._`!${h} || !${h}[${u}]`;
          }
          function _(h, u) {
            let p = [];
            for (let m in h) if (h[m] === !0) p.push(t._`${u} !== ${m}`);
            return (0, t.and)(...p);
          }
        },
      };
    e.default = a;
  }),
  ec = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = B(),
      r = Z(),
      s = {
        keyword: "unevaluatedItems",
        type: "array",
        schemaType: ["boolean", "object"],
        error: {
          message: ({ params: { len: a } }) => t.str`must NOT have more than ${a} items`,
          params: ({ params: { len: a } }) => t._`{limit: ${a}}`,
        },
        code(a) {
          let { gen: o, schema: n, data: c, it: i } = a,
            l = i.items || 0;
          if (l === !0) return;
          let d = o.const("len", t._`${c}.length`);
          if (n === !1) a.setParams({ len: l }), a.fail(t._`${d} > ${l}`);
          else if (typeof n == "object" && !(0, r.alwaysValidSchema)(i, n)) {
            let R = o.var("valid", t._`${d} <= ${l}`);
            o.if((0, t.not)(R), () => S(R, l)), a.ok(R);
          }
          i.items = !0;
          function S(R, w) {
            o.forRange("i", w, d, (g) => {
              if (
                (a.subschema({ keyword: "unevaluatedItems", dataProp: g, dataPropType: r.Type.Num }, R), !i.allErrors)
              )
                o.if((0, t.not)(R), () => o.break());
            });
          }
        },
      };
    e.default = s;
  }),
  tc = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = Qi(),
      r = ec(),
      s = [t.default, r.default];
    e.default = s;
  }),
  rc = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = za(),
      r = Ca(),
      s = Na(),
      a = Bi(),
      o = Zi(),
      n = tc(),
      c = Ma(),
      i = ja(),
      l = [
        a.default,
        t.default,
        r.default,
        (0, s.default)(!0),
        c.default,
        i.metadataVocabulary,
        i.contentVocabulary,
        o.default,
        n.default,
      ];
    e.default = l;
  }),
  sc = U((e, t) => {
    t.exports = {
      $schema: "https://json-schema.org/draft/2020-12/schema",
      $id: "https://json-schema.org/draft/2020-12/schema",
      $vocabulary: {
        "https://json-schema.org/draft/2020-12/vocab/core": !0,
        "https://json-schema.org/draft/2020-12/vocab/applicator": !0,
        "https://json-schema.org/draft/2020-12/vocab/unevaluated": !0,
        "https://json-schema.org/draft/2020-12/vocab/validation": !0,
        "https://json-schema.org/draft/2020-12/vocab/meta-data": !0,
        "https://json-schema.org/draft/2020-12/vocab/format-annotation": !0,
        "https://json-schema.org/draft/2020-12/vocab/content": !0,
      },
      $dynamicAnchor: "meta",
      title: "Core and Validation specifications meta-schema",
      allOf: [
        { $ref: "meta/core" },
        { $ref: "meta/applicator" },
        { $ref: "meta/unevaluated" },
        { $ref: "meta/validation" },
        { $ref: "meta/meta-data" },
        { $ref: "meta/format-annotation" },
        { $ref: "meta/content" },
      ],
      type: ["object", "boolean"],
      $comment:
        "This meta-schema also defines keywords that have appeared in previous drafts in order to prevent incompatible extensions as they remain in common use.",
      properties: {
        definitions: {
          $comment: '"definitions" has been replaced by "$defs".',
          type: "object",
          additionalProperties: { $dynamicRef: "#meta" },
          deprecated: !0,
          default: {},
        },
        dependencies: {
          $comment:
            '"dependencies" has been split and replaced by "dependentSchemas" and "dependentRequired" in order to serve their differing semantics.',
          type: "object",
          additionalProperties: { anyOf: [{ $dynamicRef: "#meta" }, { $ref: "meta/validation#/$defs/stringArray" }] },
          deprecated: !0,
          default: {},
        },
        $recursiveAnchor: {
          $comment: '"$recursiveAnchor" has been replaced by "$dynamicAnchor".',
          $ref: "meta/core#/$defs/anchorString",
          deprecated: !0,
        },
        $recursiveRef: {
          $comment: '"$recursiveRef" has been replaced by "$dynamicRef".',
          $ref: "meta/core#/$defs/uriReferenceString",
          deprecated: !0,
        },
      },
    };
  }),
  ac = U((e, t) => {
    t.exports = {
      $schema: "https://json-schema.org/draft/2020-12/schema",
      $id: "https://json-schema.org/draft/2020-12/meta/applicator",
      $vocabulary: { "https://json-schema.org/draft/2020-12/vocab/applicator": !0 },
      $dynamicAnchor: "meta",
      title: "Applicator vocabulary meta-schema",
      type: ["object", "boolean"],
      properties: {
        prefixItems: { $ref: "#/$defs/schemaArray" },
        items: { $dynamicRef: "#meta" },
        contains: { $dynamicRef: "#meta" },
        additionalProperties: { $dynamicRef: "#meta" },
        properties: { type: "object", additionalProperties: { $dynamicRef: "#meta" }, default: {} },
        patternProperties: {
          type: "object",
          additionalProperties: { $dynamicRef: "#meta" },
          propertyNames: { format: "regex" },
          default: {},
        },
        dependentSchemas: { type: "object", additionalProperties: { $dynamicRef: "#meta" }, default: {} },
        propertyNames: { $dynamicRef: "#meta" },
        if: { $dynamicRef: "#meta" },
        then: { $dynamicRef: "#meta" },
        else: { $dynamicRef: "#meta" },
        allOf: { $ref: "#/$defs/schemaArray" },
        anyOf: { $ref: "#/$defs/schemaArray" },
        oneOf: { $ref: "#/$defs/schemaArray" },
        not: { $dynamicRef: "#meta" },
      },
      $defs: { schemaArray: { type: "array", minItems: 1, items: { $dynamicRef: "#meta" } } },
    };
  }),
  oc = U((e, t) => {
    t.exports = {
      $schema: "https://json-schema.org/draft/2020-12/schema",
      $id: "https://json-schema.org/draft/2020-12/meta/unevaluated",
      $vocabulary: { "https://json-schema.org/draft/2020-12/vocab/unevaluated": !0 },
      $dynamicAnchor: "meta",
      title: "Unevaluated applicator vocabulary meta-schema",
      type: ["object", "boolean"],
      properties: { unevaluatedItems: { $dynamicRef: "#meta" }, unevaluatedProperties: { $dynamicRef: "#meta" } },
    };
  }),
  nc = U((e, t) => {
    t.exports = {
      $schema: "https://json-schema.org/draft/2020-12/schema",
      $id: "https://json-schema.org/draft/2020-12/meta/content",
      $vocabulary: { "https://json-schema.org/draft/2020-12/vocab/content": !0 },
      $dynamicAnchor: "meta",
      title: "Content vocabulary meta-schema",
      type: ["object", "boolean"],
      properties: {
        contentEncoding: { type: "string" },
        contentMediaType: { type: "string" },
        contentSchema: { $dynamicRef: "#meta" },
      },
    };
  }),
  ic = U((e, t) => {
    t.exports = {
      $schema: "https://json-schema.org/draft/2020-12/schema",
      $id: "https://json-schema.org/draft/2020-12/meta/core",
      $vocabulary: { "https://json-schema.org/draft/2020-12/vocab/core": !0 },
      $dynamicAnchor: "meta",
      title: "Core vocabulary meta-schema",
      type: ["object", "boolean"],
      properties: {
        $id: { $ref: "#/$defs/uriReferenceString", $comment: "Non-empty fragments not allowed.", pattern: "^[^#]*#?$" },
        $schema: { $ref: "#/$defs/uriString" },
        $ref: { $ref: "#/$defs/uriReferenceString" },
        $anchor: { $ref: "#/$defs/anchorString" },
        $dynamicRef: { $ref: "#/$defs/uriReferenceString" },
        $dynamicAnchor: { $ref: "#/$defs/anchorString" },
        $vocabulary: {
          type: "object",
          propertyNames: { $ref: "#/$defs/uriString" },
          additionalProperties: { type: "boolean" },
        },
        $comment: { type: "string" },
        $defs: { type: "object", additionalProperties: { $dynamicRef: "#meta" } },
      },
      $defs: {
        anchorString: { type: "string", pattern: "^[A-Za-z_][-A-Za-z0-9._]*$" },
        uriString: { type: "string", format: "uri" },
        uriReferenceString: { type: "string", format: "uri-reference" },
      },
    };
  }),
  cc = U((e, t) => {
    t.exports = {
      $schema: "https://json-schema.org/draft/2020-12/schema",
      $id: "https://json-schema.org/draft/2020-12/meta/format-annotation",
      $vocabulary: { "https://json-schema.org/draft/2020-12/vocab/format-annotation": !0 },
      $dynamicAnchor: "meta",
      title: "Format vocabulary meta-schema for annotation results",
      type: ["object", "boolean"],
      properties: { format: { type: "string" } },
    };
  }),
  uc = U((e, t) => {
    t.exports = {
      $schema: "https://json-schema.org/draft/2020-12/schema",
      $id: "https://json-schema.org/draft/2020-12/meta/meta-data",
      $vocabulary: { "https://json-schema.org/draft/2020-12/vocab/meta-data": !0 },
      $dynamicAnchor: "meta",
      title: "Meta-data vocabulary meta-schema",
      type: ["object", "boolean"],
      properties: {
        title: { type: "string" },
        description: { type: "string" },
        default: !0,
        deprecated: { type: "boolean", default: !1 },
        readOnly: { type: "boolean", default: !1 },
        writeOnly: { type: "boolean", default: !1 },
        examples: { type: "array", items: !0 },
      },
    };
  }),
  dc = U((e, t) => {
    t.exports = {
      $schema: "https://json-schema.org/draft/2020-12/schema",
      $id: "https://json-schema.org/draft/2020-12/meta/validation",
      $vocabulary: { "https://json-schema.org/draft/2020-12/vocab/validation": !0 },
      $dynamicAnchor: "meta",
      title: "Validation vocabulary meta-schema",
      type: ["object", "boolean"],
      properties: {
        type: {
          anyOf: [
            { $ref: "#/$defs/simpleTypes" },
            { type: "array", items: { $ref: "#/$defs/simpleTypes" }, minItems: 1, uniqueItems: !0 },
          ],
        },
        const: !0,
        enum: { type: "array", items: !0 },
        multipleOf: { type: "number", exclusiveMinimum: 0 },
        maximum: { type: "number" },
        exclusiveMaximum: { type: "number" },
        minimum: { type: "number" },
        exclusiveMinimum: { type: "number" },
        maxLength: { $ref: "#/$defs/nonNegativeInteger" },
        minLength: { $ref: "#/$defs/nonNegativeIntegerDefault0" },
        pattern: { type: "string", format: "regex" },
        maxItems: { $ref: "#/$defs/nonNegativeInteger" },
        minItems: { $ref: "#/$defs/nonNegativeIntegerDefault0" },
        uniqueItems: { type: "boolean", default: !1 },
        maxContains: { $ref: "#/$defs/nonNegativeInteger" },
        minContains: { $ref: "#/$defs/nonNegativeInteger", default: 1 },
        maxProperties: { $ref: "#/$defs/nonNegativeInteger" },
        minProperties: { $ref: "#/$defs/nonNegativeIntegerDefault0" },
        required: { $ref: "#/$defs/stringArray" },
        dependentRequired: { type: "object", additionalProperties: { $ref: "#/$defs/stringArray" } },
      },
      $defs: {
        nonNegativeInteger: { type: "integer", minimum: 0 },
        nonNegativeIntegerDefault0: { $ref: "#/$defs/nonNegativeInteger", default: 0 },
        simpleTypes: { enum: ["array", "boolean", "integer", "null", "number", "object", "string"] },
        stringArray: { type: "array", items: { type: "string" }, uniqueItems: !0, default: [] },
      },
    };
  }),
  hc = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let t = sc(),
      r = ac(),
      s = oc(),
      a = nc(),
      o = ic(),
      n = cc(),
      c = uc(),
      i = dc(),
      l = ["/properties"];
    function d(S) {
      return [t, r, s, a, o, R(this, n), c, R(this, i)].forEach((w) => this.addMetaSchema(w, void 0, !1)), this;
      function R(w, g) {
        return S ? w.$dataMetaSchema(g, l) : g;
      }
    }
    e.default = d;
  }),
  mc = U((e, t) => {
    Object.defineProperty(e, "__esModule", { value: !0 }),
      (e.MissingRefError =
        e.ValidationError =
        e.CodeGen =
        e.Name =
        e.nil =
        e.stringify =
        e.str =
        e._ =
        e.KeywordCxt =
        e.Ajv2020 =
          void 0);
    let r = Ta(),
      s = rc(),
      a = Aa(),
      o = hc(),
      n = "https://json-schema.org/draft/2020-12/schema";
    var c = class extends r.default {
      constructor(R = {}) {
        super({ ...R, dynamicRef: !0, next: !0, unevaluated: !0 });
      }
      _addVocabularies() {
        if ((super._addVocabularies(), s.default.forEach((R) => this.addVocabulary(R)), this.opts.discriminator))
          this.addKeyword(a.default);
      }
      _addDefaultMetaSchema() {
        super._addDefaultMetaSchema();
        let { $data: R, meta: w } = this.opts;
        if (!w) return;
        o.default.call(this, R), (this.refs["http://json-schema.org/schema"] = n);
      }
      defaultMeta() {
        return (this.opts.defaultMeta = super.defaultMeta() || (this.getSchema(n) ? n : void 0));
      }
    };
    (e.Ajv2020 = c),
      (t.exports = e = c),
      (t.exports.Ajv2020 = c),
      Object.defineProperty(e, "__esModule", { value: !0 }),
      (e.default = c);
    var i = Ft();
    Object.defineProperty(e, "KeywordCxt", {
      enumerable: !0,
      get: function () {
        return i.KeywordCxt;
      },
    });
    var l = B();
    Object.defineProperty(e, "_", {
      enumerable: !0,
      get: function () {
        return l._;
      },
    }),
      Object.defineProperty(e, "str", {
        enumerable: !0,
        get: function () {
          return l.str;
        },
      }),
      Object.defineProperty(e, "stringify", {
        enumerable: !0,
        get: function () {
          return l.stringify;
        },
      }),
      Object.defineProperty(e, "nil", {
        enumerable: !0,
        get: function () {
          return l.nil;
        },
      }),
      Object.defineProperty(e, "Name", {
        enumerable: !0,
        get: function () {
          return l.Name;
        },
      }),
      Object.defineProperty(e, "CodeGen", {
        enumerable: !0,
        get: function () {
          return l.CodeGen;
        },
      });
    var d = Er();
    Object.defineProperty(e, "ValidationError", {
      enumerable: !0,
      get: function () {
        return d.default;
      },
    });
    var S = Ht();
    Object.defineProperty(e, "MissingRefError", {
      enumerable: !0,
      get: function () {
        return S.default;
      },
    });
  }),
  fc = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }), (e.formatNames = e.fastFormats = e.fullFormats = void 0);
    function t(k, j) {
      return { validate: k, compare: j };
    }
    (e.fullFormats = {
      date: t(o, n),
      time: t(i(!0), l),
      "date-time": t(R(!0), w),
      "iso-time": t(i(), d),
      "iso-date-time": t(R(), g),
      duration: /^P(?!$)((\d+Y)?(\d+M)?(\d+D)?(T(?=\d)(\d+H)?(\d+M)?(\d+S)?)?|(\d+W)?)$/,
      uri: u,
      "uri-reference":
        /^(?:[a-z][a-z0-9+\-.]*:)?(?:\/?\/(?:(?:[a-z0-9\-._~!$&'()*+,;=:]|%[0-9a-f]{2})*@)?(?:\[(?:(?:(?:(?:[0-9a-f]{1,4}:){6}|::(?:[0-9a-f]{1,4}:){5}|(?:[0-9a-f]{1,4})?::(?:[0-9a-f]{1,4}:){4}|(?:(?:[0-9a-f]{1,4}:){0,1}[0-9a-f]{1,4})?::(?:[0-9a-f]{1,4}:){3}|(?:(?:[0-9a-f]{1,4}:){0,2}[0-9a-f]{1,4})?::(?:[0-9a-f]{1,4}:){2}|(?:(?:[0-9a-f]{1,4}:){0,3}[0-9a-f]{1,4})?::[0-9a-f]{1,4}:|(?:(?:[0-9a-f]{1,4}:){0,4}[0-9a-f]{1,4})?::)(?:[0-9a-f]{1,4}:[0-9a-f]{1,4}|(?:(?:25[0-5]|2[0-4]\d|[01]?\d\d?)\.){3}(?:25[0-5]|2[0-4]\d|[01]?\d\d?))|(?:(?:[0-9a-f]{1,4}:){0,5}[0-9a-f]{1,4})?::[0-9a-f]{1,4}|(?:(?:[0-9a-f]{1,4}:){0,6}[0-9a-f]{1,4})?::)|[Vv][0-9a-f]+\.[a-z0-9\-._~!$&'()*+,;=:]+)\]|(?:(?:25[0-5]|2[0-4]\d|[01]?\d\d?)\.){3}(?:25[0-5]|2[0-4]\d|[01]?\d\d?)|(?:[a-z0-9\-._~!$&'"()*+,;=]|%[0-9a-f]{2})*)(?::\d*)?(?:\/(?:[a-z0-9\-._~!$&'"()*+,;=:@]|%[0-9a-f]{2})*)*|\/(?:(?:[a-z0-9\-._~!$&'"()*+,;=:@]|%[0-9a-f]{2})+(?:\/(?:[a-z0-9\-._~!$&'"()*+,;=:@]|%[0-9a-f]{2})*)*)?|(?:[a-z0-9\-._~!$&'"()*+,;=:@]|%[0-9a-f]{2})+(?:\/(?:[a-z0-9\-._~!$&'"()*+,;=:@]|%[0-9a-f]{2})*)*)?(?:\?(?:[a-z0-9\-._~!$&'"()*+,;=:@/?]|%[0-9a-f]{2})*)?(?:#(?:[a-z0-9\-._~!$&'"()*+,;=:@/?]|%[0-9a-f]{2})*)?$/i,
      "uri-template":
        /^(?:(?:[^\x00-\x20"'<>%\\^`{|}]|%[0-9a-f]{2})|\{[+#./;?&=,!@|]?(?:[a-z0-9_]|%[0-9a-f]{2})+(?::[1-9][0-9]{0,3}|\*)?(?:,(?:[a-z0-9_]|%[0-9a-f]{2})+(?::[1-9][0-9]{0,3}|\*)?)*\})*$/i,
      url: /^(?:https?|ftp):\/\/(?:\S+(?::\S*)?@)?(?:(?!(?:10|127)(?:\.\d{1,3}){3})(?!(?:169\.254|192\.168)(?:\.\d{1,3}){2})(?!172\.(?:1[6-9]|2\d|3[0-1])(?:\.\d{1,3}){2})(?:[1-9]\d?|1\d\d|2[01]\d|22[0-3])(?:\.(?:1?\d{1,2}|2[0-4]\d|25[0-5])){2}(?:\.(?:[1-9]\d?|1\d\d|2[0-4]\d|25[0-4]))|(?:(?:[a-z0-9\u{00a1}-\u{ffff}]+-)*[a-z0-9\u{00a1}-\u{ffff}]+)(?:\.(?:[a-z0-9\u{00a1}-\u{ffff}]+-)*[a-z0-9\u{00a1}-\u{ffff}]+)*(?:\.(?:[a-z\u{00a1}-\u{ffff}]{2,})))(?::\d{2,5})?(?:\/[^\s]*)?$/iu,
      email:
        /^[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/i,
      hostname: /^(?=.{1,253}\.?$)[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?(?:\.[a-z0-9](?:[-0-9a-z]{0,61}[0-9a-z])?)*\.?$/i,
      ipv4: /^(?:(?:25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)\.){3}(?:25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)$/,
      ipv6: /^((([0-9a-f]{1,4}:){7}([0-9a-f]{1,4}|:))|(([0-9a-f]{1,4}:){6}(:[0-9a-f]{1,4}|((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3})|:))|(([0-9a-f]{1,4}:){5}(((:[0-9a-f]{1,4}){1,2})|:((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3})|:))|(([0-9a-f]{1,4}:){4}(((:[0-9a-f]{1,4}){1,3})|((:[0-9a-f]{1,4})?:((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3}))|:))|(([0-9a-f]{1,4}:){3}(((:[0-9a-f]{1,4}){1,4})|((:[0-9a-f]{1,4}){0,2}:((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3}))|:))|(([0-9a-f]{1,4}:){2}(((:[0-9a-f]{1,4}){1,5})|((:[0-9a-f]{1,4}){0,3}:((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3}))|:))|(([0-9a-f]{1,4}:){1}(((:[0-9a-f]{1,4}){1,6})|((:[0-9a-f]{1,4}){0,4}:((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3}))|:))|(:(((:[0-9a-f]{1,4}){1,7})|((:[0-9a-f]{1,4}){0,5}:((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3}))|:)))$/i,
      regex: L,
      uuid: /^(?:urn:uuid:)?[0-9a-f]{8}-(?:[0-9a-f]{4}-){3}[0-9a-f]{12}$/i,
      "json-pointer": /^(?:\/(?:[^~/]|~0|~1)*)*$/,
      "json-pointer-uri-fragment": /^#(?:\/(?:[a-z0-9_\-.!$&'()*+,;:=@]|%[0-9a-f]{2}|~0|~1)*)*$/i,
      "relative-json-pointer": /^(?:0|[1-9][0-9]*)(?:#|(?:\/(?:[^~/]|~0|~1)*)*)$/,
      byte: m,
      int32: { type: "number", validate: v },
      int64: { type: "number", validate: f },
      float: { type: "number", validate: P },
      double: { type: "number", validate: P },
      password: !0,
      binary: !0,
    }),
      (e.fastFormats = {
        ...e.fullFormats,
        date: t(/^\d\d\d\d-[0-1]\d-[0-3]\d$/, n),
        time: t(/^(?:[0-2]\d:[0-5]\d:[0-5]\d|23:59:60)(?:\.\d+)?(?:z|[+-]\d\d(?::?\d\d)?)$/i, l),
        "date-time": t(
          /^\d\d\d\d-[0-1]\d-[0-3]\dt(?:[0-2]\d:[0-5]\d:[0-5]\d|23:59:60)(?:\.\d+)?(?:z|[+-]\d\d(?::?\d\d)?)$/i,
          w,
        ),
        "iso-time": t(/^(?:[0-2]\d:[0-5]\d:[0-5]\d|23:59:60)(?:\.\d+)?(?:z|[+-]\d\d(?::?\d\d)?)?$/i, d),
        "iso-date-time": t(
          /^\d\d\d\d-[0-1]\d-[0-3]\d[t\s](?:[0-2]\d:[0-5]\d:[0-5]\d|23:59:60)(?:\.\d+)?(?:z|[+-]\d\d(?::?\d\d)?)?$/i,
          g,
        ),
        uri: /^(?:[a-z][a-z0-9+\-.]*:)(?:\/?\/)?[^\s]*$/i,
        "uri-reference": /^(?:(?:[a-z][a-z0-9+\-.]*:)?\/?\/)?(?:[^\\\s#][^\s#]*)?(?:#[^\\\s]*)?$/i,
        email:
          /^[a-z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?)*$/i,
      }),
      (e.formatNames = Object.keys(e.fullFormats));
    function r(k) {
      return k % 4 === 0 && (k % 100 !== 0 || k % 400 === 0);
    }
    let s = /^(\d\d\d\d)-(\d\d)-(\d\d)$/,
      a = [0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
    function o(k) {
      let j = s.exec(k);
      if (!j) return !1;
      let F = +j[1],
        V = +j[2],
        Y = +j[3];
      return V >= 1 && V <= 12 && Y >= 1 && Y <= (V === 2 && r(F) ? 29 : a[V]);
    }
    function n(k, j) {
      if (!(k && j)) return;
      if (k > j) return 1;
      if (k < j) return -1;
      return 0;
    }
    let c = /^(\d\d):(\d\d):(\d\d(?:\.\d+)?)(z|([+-])(\d\d)(?::?(\d\d))?)?$/i;
    function i(k) {
      return function (F) {
        let V = c.exec(F);
        if (!V) return !1;
        let Y = +V[1],
          te = +V[2],
          Q = +V[3],
          oe = V[4],
          X = V[5] === "-" ? -1 : 1,
          C = +(V[6] || 0),
          O = +(V[7] || 0);
        if (C > 23 || O > 59 || (k && !oe)) return !1;
        if (Y <= 23 && te <= 59 && Q < 60) return !0;
        let A = te - O * X,
          b = Y - C * X - (A < 0 ? 1 : 0);
        return (b === 23 || b === -1) && (A === 59 || A === -1) && Q < 61;
      };
    }
    function l(k, j) {
      if (!(k && j)) return;
      let F = new Date("2020-01-01T" + k).valueOf(),
        V = new Date("2020-01-01T" + j).valueOf();
      if (!(F && V)) return;
      return F - V;
    }
    function d(k, j) {
      if (!(k && j)) return;
      let F = c.exec(k),
        V = c.exec(j);
      if (!(F && V)) return;
      if (((k = F[1] + F[2] + F[3]), (j = V[1] + V[2] + V[3]), k > j)) return 1;
      if (k < j) return -1;
      return 0;
    }
    let S = /t|\s/i;
    function R(k) {
      let j = i(k);
      return function (V) {
        let Y = V.split(S);
        return Y.length === 2 && o(Y[0]) && j(Y[1]);
      };
    }
    function w(k, j) {
      if (!(k && j)) return;
      let F = new Date(k).valueOf(),
        V = new Date(j).valueOf();
      if (!(F && V)) return;
      return F - V;
    }
    function g(k, j) {
      if (!(k && j)) return;
      let [F, V] = k.split(S),
        [Y, te] = j.split(S),
        Q = n(F, Y);
      if (Q === void 0) return;
      return Q || l(V, te);
    }
    let _ = /\/|:/,
      h =
        /^(?:[a-z][a-z0-9+\-.]*:)(?:\/?\/(?:(?:[a-z0-9\-._~!$&'()*+,;=:]|%[0-9a-f]{2})*@)?(?:\[(?:(?:(?:(?:[0-9a-f]{1,4}:){6}|::(?:[0-9a-f]{1,4}:){5}|(?:[0-9a-f]{1,4})?::(?:[0-9a-f]{1,4}:){4}|(?:(?:[0-9a-f]{1,4}:){0,1}[0-9a-f]{1,4})?::(?:[0-9a-f]{1,4}:){3}|(?:(?:[0-9a-f]{1,4}:){0,2}[0-9a-f]{1,4})?::(?:[0-9a-f]{1,4}:){2}|(?:(?:[0-9a-f]{1,4}:){0,3}[0-9a-f]{1,4})?::[0-9a-f]{1,4}:|(?:(?:[0-9a-f]{1,4}:){0,4}[0-9a-f]{1,4})?::)(?:[0-9a-f]{1,4}:[0-9a-f]{1,4}|(?:(?:25[0-5]|2[0-4]\d|[01]?\d\d?)\.){3}(?:25[0-5]|2[0-4]\d|[01]?\d\d?))|(?:(?:[0-9a-f]{1,4}:){0,5}[0-9a-f]{1,4})?::[0-9a-f]{1,4}|(?:(?:[0-9a-f]{1,4}:){0,6}[0-9a-f]{1,4})?::)|[Vv][0-9a-f]+\.[a-z0-9\-._~!$&'()*+,;=:]+)\]|(?:(?:25[0-5]|2[0-4]\d|[01]?\d\d?)\.){3}(?:25[0-5]|2[0-4]\d|[01]?\d\d?)|(?:[a-z0-9\-._~!$&'()*+,;=]|%[0-9a-f]{2})*)(?::\d*)?(?:\/(?:[a-z0-9\-._~!$&'()*+,;=:@]|%[0-9a-f]{2})*)*|\/(?:(?:[a-z0-9\-._~!$&'()*+,;=:@]|%[0-9a-f]{2})+(?:\/(?:[a-z0-9\-._~!$&'()*+,;=:@]|%[0-9a-f]{2})*)*)?|(?:[a-z0-9\-._~!$&'()*+,;=:@]|%[0-9a-f]{2})+(?:\/(?:[a-z0-9\-._~!$&'()*+,;=:@]|%[0-9a-f]{2})*)*)(?:\?(?:[a-z0-9\-._~!$&'()*+,;=:@/?]|%[0-9a-f]{2})*)?(?:#(?:[a-z0-9\-._~!$&'()*+,;=:@/?]|%[0-9a-f]{2})*)?$/i;
    function u(k) {
      return _.test(k) && h.test(k);
    }
    let p = /^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/gm;
    function m(k) {
      return (p.lastIndex = 0), p.test(k);
    }
    let y = -2147483648,
      E = 2147483647;
    function v(k) {
      return Number.isInteger(k) && k <= E && k >= y;
    }
    function f(k) {
      return Number.isInteger(k);
    }
    function P() {
      return !0;
    }
    let I = /[^\\]\\Z/;
    function L(k) {
      if (I.test(k)) return !1;
      try {
        return new RegExp(k), !0;
      } catch (j) {
        return !1;
      }
    }
  }),
  pc = U((e) => {
    Object.defineProperty(e, "__esModule", { value: !0 }), (e.formatLimitDefinition = void 0);
    let t = La(),
      r = B(),
      s = r.operators,
      a = {
        formatMaximum: { okStr: "<=", ok: s.LTE, fail: s.GT },
        formatMinimum: { okStr: ">=", ok: s.GTE, fail: s.LT },
        formatExclusiveMaximum: { okStr: "<", ok: s.LT, fail: s.GTE },
        formatExclusiveMinimum: { okStr: ">", ok: s.GT, fail: s.LTE },
      },
      o = {
        message: ({ keyword: c, schemaCode: i }) => r.str`should be ${a[c].okStr} ${i}`,
        params: ({ keyword: c, schemaCode: i }) => r._`{comparison: ${a[c].okStr}, limit: ${i}}`,
      };
    e.formatLimitDefinition = {
      keyword: Object.keys(a),
      type: "string",
      schemaType: "string",
      $data: !0,
      error: o,
      code(c) {
        let { gen: i, data: l, schemaCode: d, keyword: S, it: R } = c,
          { opts: w, self: g } = R;
        if (!w.validateFormats) return;
        let _ = new t.KeywordCxt(R, g.RULES.all.format.definition, "format");
        if (_.$data) h();
        else u();
        function h() {
          let m = i.scopeValue("formats", { ref: g.formats, code: w.code.formats }),
            y = i.const("fmt", r._`${m}[${_.schemaCode}]`);
          c.fail$data(
            (0, r.or)(
              r._`typeof ${y} != "object"`,
              r._`${y} instanceof RegExp`,
              r._`typeof ${y}.compare != "function"`,
              p(y),
            ),
          );
        }
        function u() {
          let m = _.schema,
            y = g.formats[m];
          if (!y || y === !0) return;
          if (typeof y != "object" || y instanceof RegExp || typeof y.compare != "function")
            throw Error(`"${S}": format "${m}" does not define "compare" function`);
          let E = i.scopeValue("formats", {
            key: m,
            ref: y,
            code: w.code.formats ? r._`${w.code.formats}${(0, r.getProperty)(m)}` : void 0,
          });
          c.fail$data(p(E));
        }
        function p(m) {
          return r._`${m}.compare(${l}, ${d}) ${a[S].fail} 0`;
        }
      },
      dependencies: ["format"],
    };
    let n = (c) => (c.addKeyword(e.formatLimitDefinition), c);
    e.default = n;
  }),
  Sc = U((e, t) => {
    Object.defineProperty(e, "__esModule", { value: !0 });
    let r = fc(),
      s = pc(),
      a = B(),
      o = new a.Name("fullFormats"),
      n = new a.Name("fastFormats"),
      c = (l, d = { keywords: !0 }) => {
        if (Array.isArray(d)) return i(l, d, r.fullFormats, o), l;
        let [S, R] = d.mode === "fast" ? [r.fastFormats, n] : [r.fullFormats, o];
        if ((i(l, d.formats || r.formatNames, S, R), d.keywords)) (0, s.default)(l);
        return l;
      };
    c.get = (l, d = "full") => {
      let S = (d === "fast" ? r.fastFormats : r.fullFormats)[l];
      if (!S) throw Error(`Unknown format "${l}"`);
      return S;
    };
    function i(l, d, S, R) {
      var w, g;
      ((w = (g = l.opts.code).formats) !== null && w !== void 0) ||
        (g.formats = a._`require("ajv-formats/dist/formats").${R}`);
      for (let _ of d) l.addFormat(_, S[_]);
    }
    (t.exports = e = c), Object.defineProperty(e, "__esModule", { value: !0 }), (e.default = c);
  }),
  gc = La(),
  _c = mc(),
  vc = lr(Sc(), 1),
  yc = new Set(["https://json-schema.org/draft/2020-12/schema", "http://json-schema.org/draft/2020-12/schema"]),
  bc = vc.default;
function Rc() {
  let e = new _c.Ajv2020({ strict: !1, validateFormats: !0, validateSchema: !1, allErrors: !0 });
  return bc(e), e;
}
var UWe = class {
    _ajv;
    _userAjv;
    constructor(e) {
      (this._userAjv = e !== void 0), (this._ajv = e);
    }
    get ajv() {
      return (this._ajv ??= Rc());
    }
    getValidator(e) {
      if (!this._userAjv && "$schema" in e && typeof e.$schema === "string" && !yc.has(e.$schema.replace(/#$/, ""))) {
        let s = e.$schema.slice(0, 200);
        throw Error(
          `JSON Schema declares an unsupported dialect ("$schema": "${s}"). The default validator supports JSON Schema 2020-12 only; pass a pre-configured Ajv instance to AjvJsonSchemaValidator(ajv) to validate other dialects.`,
        );
      }
      let t = this.ajv,
        r = "$id" in e && typeof e.$id === "string" ? (t.getSchema(e.$id) ?? t.compile(e)) : t.compile(e);
      return (s) =>
        r(s)
          ? { valid: !0, data: s, errorMessage: void 0 }
          : { valid: !1, data: void 0, errorMessage: t.errorsText(r.errors) };
    }
  },
  Hl = gc.Ajv;
var Va = !1;
var xt = class extends Error {
    static {
      Object.defineProperty(this, "mcpBrand", { value: "mcp.OAuthClientFlowError" });
    }
    static [Symbol.hasInstance](e) {
      return _e(this, e);
    }
    static isInstance(e) {
      if (typeof this !== "function")
        throw TypeError(
          "isInstance must be called on the class (e.g. `SdkError.isInstance(value)`); for callbacks use `v => SdkError.isInstance(v)`",
        );
      return _e(this, e);
    }
    constructor(e) {
      super(e);
      (this.name = new.target.name), Je(this, new.target);
    }
  },
  S_e = class extends xt {
    static {
      Object.defineProperty(this, "mcpBrand", { value: "mcp.IssuerMismatchError" });
    }
    kind;
    expected;
    received;
    constructor(e, t, r) {
      super(
        `Issuer mismatch in ${e === "metadata" ? "authorization server metadata (RFC 8414 \xA73.3)" : "authorization response (RFC 9207)"}: expected ${JSON.stringify(t)}, received ${JSON.stringify(r)}`,
      );
      (this.kind = e), (this.expected = t), (this.received = r);
    }
  },
  aOt = class extends xt {
    static {
      Object.defineProperty(this, "mcpBrand", { value: "mcp.RegistrationRejectedError" });
    }
    status;
    body;
    submittedMetadata;
    constructor(e) {
      super(`Dynamic Client Registration rejected (HTTP ${e.status}): ${e.body}`);
      (this.status = e.status), (this.body = e.body), (this.submittedMetadata = e.submittedMetadata);
    }
  },
  ro = class extends xt {
    static {
      Object.defineProperty(this, "mcpBrand", { value: "mcp.InsecureTokenEndpointError" });
    }
    tokenEndpoint;
    constructor(e) {
      super(
        `Refusing to send credentials to non-https token endpoint '${e}'. OAuth token requests MUST use TLS (localhost / 127.0.0.1 / ::1 are exempt).`,
      );
      this.tokenEndpoint = e;
    }
  },
  ss = class extends xt {
    static {
      Object.defineProperty(this, "mcpBrand", { value: "mcp.AuthorizationServerMismatchError" });
    }
    constructor(e, t) {
      super(
        `Authorization server changed between redirect and callback (redirected to ${JSON.stringify(e)}, callback resolved ${JSON.stringify(t)}); refusing to send authorization_code/code_verifier to a different token endpoint`,
      );
      (this.recordedIssuer = e), (this.currentIssuer = t);
    }
  },
  Fa = class extends xt {
    static {
      Object.defineProperty(this, "mcpBrand", { value: "mcp.InsufficientScopeError" });
    }
    requiredScope;
    resourceMetadataUrl;
    errorDescription;
    constructor(e) {
      super(`Insufficient scope${e.requiredScope ? `: required "${e.requiredScope}"` : ""}`);
      (this.requiredScope = e.requiredScope),
        (this.resourceMetadataUrl = e.resourceMetadataUrl),
        (this.errorDescription = e.errorDescription);
    }
  };
function Ha(e, t, r) {
  if (e === void 0) return;
  if (e.issuer === void 0) {
    if (r?.canPersistStamp !== !1)
      console.warn(
        "[mcp-sdk] SEP-2352: stored OAuth credential has no 'issuer' stamp (pre-upgrade storage or provider not round-tripping the value). SEP-2352 isolation is inactive for this read; ensure your provider round-trips the issuer field.",
      );
    return e;
  }
  return so(e.issuer, t) ? e : void 0;
}
function so(e, t) {
  return e === t || (e.endsWith("/") && e.slice(0, -1) === t) || (t.endsWith("/") && t.slice(0, -1) === e);
}
function ao(e) {
  if (e == null) return !1;
  let t = e;
  return typeof t.tokens === "function" && typeof t.clientInformation === "function";
}
async function wc(e, t, r) {
  let { resourceMetadataUrl: s, scope: a } = Be(t.response);
  if (
    (await BHe(e, { serverUrl: t.serverUrl, resourceMetadataUrl: s, scope: a, fetchFn: t.fetchFn, ...r })) !==
    "AUTHORIZED"
  )
    throw new xA();
}
function oo(e, t) {
  return { token: async () => (await e.tokens())?.access_token, onUnauthorized: async (r) => wc(e, r, t) };
}
var xA = class extends Error {
  static {
    Object.defineProperty(this, "mcpBrand", { value: "mcp.UnauthorizedError" });
  }
  static [Symbol.hasInstance](e) {
    return _e(this, e);
  }
  static isInstance(e) {
    if (typeof this !== "function")
      throw TypeError(
        "isInstance must be called on the class (e.g. `SdkError.isInstance(value)`); for callbacks use `v => SdkError.isInstance(v)`",
      );
    return _e(this, e);
  }
  constructor(e) {
    super(e ?? "Unauthorized");
    (this.name = "UnauthorizedError"), Je(this, new.target);
  }
};
function kr(e) {
  return e?.authorization_response_iss_parameter_supported === !0;
}
function qr({ iss: e, expectedIssuer: t, issParameterSupported: r }) {
  if (t === void 0) return;
  if (e === void 0) {
    if (r) throw new S_e("authorization_response", t, void 0);
    return;
  }
  if (e !== t) throw new S_e("authorization_response", t, e);
}
function as(...e) {
  let t = new Set();
  for (let r of e) {
    if (!r) continue;
    for (let s of r.split(/\s+/)) if (s) t.add(s);
  }
  return t.size > 0 ? [...t].join(" ") : void 0;
}
function $c(e, t) {
  if (!e) return !1;
  let r = new Set((t ?? "").split(/\s+/).filter(Boolean));
  for (let s of e.split(/\s+/)) if (s && !r.has(s)) return !0;
  return !1;
}
async function no(e, t, r, s, a) {
  if (typeof e === "string") return { authorizationCode: e, iss: t };
  let o = e.get("iss") ?? void 0,
    n = e.get("code");
  if (n) return { authorizationCode: n, iss: o };
  let c = (await r.discoveryState?.())?.authorizationServerMetadata;
  if (!c)
    try {
      c = (await cOt(s, a)).authorizationServerMetadata;
    } catch {
      c = void 0;
    }
  if (!c) throw new xA("Authorization callback failed and the issuer could not be verified");
  qr({ iss: o, expectedIssuer: c.issuer, issParameterSupported: kr(c) });
  let i = e.get("error");
  if (i) throw new Hx(i, e.get("error_description") ?? i, e.get("error_uri") ?? void 0);
  throw new xA("Authorization callback contained neither `code` nor `error`");
}
function Ec(e) {
  return ["client_secret_basic", "client_secret_post", "none"].includes(e);
}
var os = "code",
  ns = "S256";
function Pc(e, t) {
  let r = e.client_secret !== void 0;
  if (
    "token_endpoint_auth_method" in e &&
    e.token_endpoint_auth_method &&
    Ec(e.token_endpoint_auth_method) &&
    (t.length === 0 || t.includes(e.token_endpoint_auth_method))
  )
    return e.token_endpoint_auth_method;
  if (t.length === 0) return r ? "client_secret_basic" : "none";
  if (r && t.includes("client_secret_basic")) return "client_secret_basic";
  if (r && t.includes("client_secret_post")) return "client_secret_post";
  if (t.includes("none")) return "none";
  return r ? "client_secret_post" : "none";
}
function Tc(e, t, r, s) {
  let { client_id: a, client_secret: o } = t;
  switch (e) {
    case "client_secret_basic":
      zc(a, o, r);
      return;
    case "client_secret_post":
      Cc(a, o, s);
      return;
    case "none":
      kc(a, s);
      return;
    default:
      throw Error(`Unsupported client authentication method: ${e}`);
  }
}
function zc(e, t, r) {
  if (!t) throw Error("client_secret_basic authentication requires a client_secret");
  let s = btoa(`${e}:${t}`);
  r.set("Authorization", `Basic ${s}`);
}
function Cc(e, t, r) {
  if ((r.set("client_id", e), t)) r.set("client_secret", t);
}
function kc(e, t) {
  t.set("client_id", e);
}
function io(e) {
  return e === "localhost" || e === "127.0.0.1" || e === "[::1]" || e === "::1";
}
function qc(e) {
  let t = new URL(String(e));
  if (t.protocol !== "https:" && !io(t.hostname)) throw new ro(t.href);
  return t;
}
function Ic(e) {
  for (let t of e ?? []) {
    let r;
    try {
      r = new URL(t);
    } catch {
      continue;
    }
    if (r.protocol !== "http:" && r.protocol !== "https:") return "native";
    if (io(r.hostname)) return "native";
  }
  return "web";
}
function Oc(e) {
  let t = e.clientMetadata;
  return {
    ...t,
    grant_types: t.grant_types ?? (e.redirectUrl === void 0 ? void 0 : ["authorization_code", "refresh_token"]),
    application_type: t.application_type ?? Ic(t.redirect_uris),
  };
}
async function xa(e) {
  let t = e instanceof Response ? e.status : void 0,
    r = e instanceof Response ? await e.text() : e;
  try {
    let s = O_e.parse(JSON.parse(r));
    return Hx.fromResponse(s);
  } catch (s) {
    let a = `${t ? `HTTP ${t}: ` : ""}Invalid OAuth error response: ${s}. Raw body: ${r}`;
    return new Hx(wB.ServerError, a);
  }
}
async function BHe(e, t) {
  try {
    return await is(e, t);
  } catch (r) {
    if (r instanceof Hx) {
      if (r.code === wB.InvalidClient || r.code === wB.UnauthorizedClient)
        return await e.invalidateCredentials?.("client"), await e.invalidateCredentials?.("tokens"), await is(e, t);
      else if (r.code === wB.InvalidGrant) return await e.invalidateCredentials?.("tokens"), await is(e, t);
    }
    throw r;
  }
}
function Nc(e) {
  let { requestedScope: t, resourceMetadata: r, authServerMetadata: s, clientMetadata: a } = e,
    o = t || r?.scopes_supported?.join(" ") || a.scope;
  if (
    o &&
    s?.scopes_supported?.includes("offline_access") &&
    !o.split(" ").includes("offline_access") &&
    a.grant_types?.includes("refresh_token")
  )
    o = `${o} offline_access`;
  return o;
}
async function is(
  e,
  {
    serverUrl: t,
    authorizationCode: r,
    iss: s,
    scope: a,
    resourceMetadataUrl: o,
    fetchFn: n,
    skipIssuerMetadataValidation: c,
    forceReauthorization: i,
  },
) {
  let l = Oc(e),
    d = await e.discoveryState?.(),
    S,
    R,
    w,
    g,
    _ = o;
  if (!_ && d?.resourceMetadataUrl) _ = new URL(d.resourceMetadataUrl);
  if (d?.authorizationServerUrl) {
    if (
      ((R = d.authorizationServerUrl),
      (S = d.resourceMetadata),
      (w = d.authorizationServerMetadata ?? (await jHe(R, { fetchFn: n, skipIssuerValidation: c }))),
      !S)
    )
      try {
        S = await lOt(t, { resourceMetadataUrl: _ }, n);
      } catch (k) {
        if (k instanceof TypeError) throw k;
      }
    if (w !== d.authorizationServerMetadata || S !== d.resourceMetadata)
      await e.saveDiscoveryState?.({
        authorizationServerUrl: String(R),
        resourceMetadataUrl: _?.toString(),
        resourceMetadata: S,
        authorizationServerMetadata: w,
      });
  } else {
    let k = await cOt(t, { resourceMetadataUrl: _, fetchFn: n, skipIssuerMetadataValidation: c });
    (R = k.authorizationServerUrl),
      (w = k.authorizationServerMetadata),
      (S = k.resourceMetadata),
      (g = {
        authorizationServerUrl: String(R),
        resourceMetadataUrl: _?.toString(),
        resourceMetadata: S,
        authorizationServerMetadata: w,
      });
  }
  let h = w?.issuer ?? String(R),
    u = { issuer: h };
  if ((await e.saveAuthorizationServerUrl?.(h), r !== void 0)) {
    let k = d?.authorizationServerMetadata?.issuer ?? d?.authorizationServerUrl;
    if (k === void 0) {
      if (e.saveDiscoveryState !== void 0)
        throw new ss(
          "discoveryState was not available on the callback leg; ensure your provider persists discoveryState alongside codeVerifier",
          h,
        );
      console.warn(
        "[mcp-sdk] OAuthClientProvider does not implement saveDiscoveryState()/discoveryState(); the SEP-2352 callback-leg authorization-server binding cannot be checked. Implement discoveryState (persist alongside codeVerifier) \u2014 see docs/migration/upgrade-to-v2.md \xA7SEP-2352.",
      );
    } else if (!so(k, h)) throw new ss(k, h);
  }
  if (g) await e.saveDiscoveryState?.(g);
  let p = await jc(t, e, S);
  if (p) await e.saveResourceUrl?.(String(p));
  let m = Nc({ requestedScope: a, resourceMetadata: S, authServerMetadata: w, clientMetadata: e.clientMetadata }),
    y = await Promise.resolve(e.clientInformation(u)),
    E = Ha(y, h, { canPersistStamp: e.saveClientInformation !== void 0 });
  if (E === void 0 && y?.issuer && e.saveClientInformation === void 0) throw new ss(y.issuer, h);
  if (E && E.issuer === void 0) (E = { ...E, issuer: h }), await e.saveClientInformation?.(E, u);
  if (!E) {
    if (r !== void 0)
      throw Error("Existing OAuth client information is required when exchanging an authorization code");
    let k = w?.client_id_metadata_document_supported === !0,
      j = e.clientMetadataUrl;
    if (j && !Mc(j))
      throw new Hx(
        wB.InvalidClientMetadata,
        `clientMetadataUrl must be a valid HTTPS URL with a non-root pathname, got: ${j}`,
      );
    if (k && j) (E = { client_id: j, issuer: h }), await e.saveClientInformation?.(E, u);
    else {
      if (!e.saveClientInformation) throw Error("OAuth client information must be saveable for dynamic registration");
      (E = { ...(await Fc(R, { metadata: w, clientMetadata: l, scope: m, fetchFn: n })), issuer: h }),
        await e.saveClientInformation(E, u);
    }
  }
  let v = !e.redirectUrl;
  if (r !== void 0 || v) {
    if (r !== void 0) qr({ iss: s, expectedIssuer: w?.issuer, issParameterSupported: kr(w) });
    let k = await Vc(e, R, { metadata: w, resource: p, authorizationCode: r, iss: s, scope: m, fetchFn: n });
    return await e.saveTokens({ ...k, issuer: h }, u), "AUTHORIZED";
  }
  let f = Ha(await e.tokens(u), h);
  if (f && f.issuer === void 0) (f = { ...f, issuer: h }), await e.saveTokens(f, u);
  if (f?.refresh_token && !i)
    try {
      let k = await Grn(R, {
        metadata: w,
        clientInformation: E,
        refreshToken: f.refresh_token,
        resource: p,
        addClientAuthentication: e.addClientAuthentication,
        fetchFn: n,
      });
      return await e.saveTokens({ ...k, issuer: h }, u), "AUTHORIZED";
    } catch (k) {
      if (k instanceof ro) throw k;
      if (!(k instanceof Hx) || k.code === wB.ServerError);
      else throw k;
    }
  let P = e.state ? await e.state() : void 0,
    { authorizationUrl: I, codeVerifier: L } = await qrn(R, {
      metadata: w,
      clientInformation: E,
      state: P,
      redirectUrl: e.redirectUrl,
      scope: m,
      resource: p,
    });
  return await e.saveCodeVerifier(L), await e.redirectToAuthorization(I), "REDIRECT";
}
function Mc(e) {
  if (!e) return !1;
  try {
    let t = new URL(e);
    return t.protocol === "https:" && t.pathname !== "/";
  } catch {
    return !1;
  }
}
async function jc(e, t, r) {
  let s = Ms(e);
  if (t.validateResourceURL) return await t.validateResourceURL(s, r?.resource);
  if (!r) return;
  if (!js({ requestedResource: s, configuredResource: r.resource }))
    throw Error(`Protected resource ${r.resource} does not match expected ${s} (or origin)`);
  return new URL(r.resource);
}
function Be(e) {
  let t = e.headers.get("WWW-Authenticate");
  if (!t) return {};
  let [r, s] = t.split(" ");
  if (r?.toLowerCase() !== "bearer" || !s) return {};
  let a = Tr(e, "resource_metadata") || void 0,
    o;
  if (a)
    try {
      o = new URL(a);
    } catch {}
  let n = Tr(e, "scope") || void 0,
    c = Tr(e, "error") || void 0,
    i = Tr(e, "error_description") || void 0;
  return { resourceMetadataUrl: o, scope: n, error: c, errorDescription: i };
}
function Tr(e, t) {
  let r = e.headers.get("WWW-Authenticate");
  if (!r) return null;
  let s = new RegExp(String.raw`${t}=(?:"([^"]+)"|([^\s,]+))`),
    a = r.match(s);
  if (a) {
    let o = a[1] || a[2];
    if (o) return o;
  }
  return null;
}
async function lOt(e, t, r = fetch) {
  let s = await Uc(e, "oauth-protected-resource", r, {
    protocolVersion: t?.protocolVersion,
    metadataUrl: t?.resourceMetadataUrl,
  });
  if (!s || s.status === 404)
    throw (
      (await s?.text?.().catch(() => {}),
      Error("Resource server does not implement OAuth 2.0 Protected Resource Metadata."))
    );
  if (!s.ok)
    throw (
      (await s.text?.().catch(() => {}),
      Error(`HTTP ${s.status} trying to load well-known OAuth protected resource metadata.`))
    );
  return Vat.parse(await s.json());
}
async function co(e, t, r = fetch) {
  try {
    return await r(e, { headers: t });
  } catch (s) {
    if (!(s instanceof TypeError) || !Va) throw s;
    if (t)
      try {
        return await r(e, {});
      } catch (a) {
        if (!(a instanceof TypeError)) throw a;
        return;
      }
    return;
  }
}
function Ac(e, t = "", r = {}) {
  if (t.endsWith("/")) t = t.slice(0, -1);
  return r.prependPathname ? `${t}/.well-known/${e}` : `/.well-known/${e}${t}`;
}
async function Ka(e, t, r = fetch) {
  return await co(e, { "MCP-Protocol-Version": t }, r);
}
function Lc(e, t) {
  if (!e) return !0;
  if (t === "/") return !1;
  return (e.status >= 400 && e.status < 500) || e.status === 502;
}
async function Uc(e, t, r, s) {
  let a = new URL(e),
    o = s?.protocolVersion ?? jWe,
    n;
  if (s?.metadataUrl) n = new URL(s.metadataUrl);
  else {
    let i = Ac(t, a.pathname);
    (n = new URL(i, s?.metadataServerUrl ?? a)), (n.search = a.search);
  }
  let c = await Ka(n, o, r);
  if (!s?.metadataUrl && Lc(c, a.pathname)) c = await Ka(new URL(`/.well-known/${t}`, a), o, r);
  return c;
}
function Dc(e) {
  let t = typeof e === "string" ? new URL(e) : e,
    r = t.pathname !== "/",
    s = [];
  if (!r)
    return (
      s.push(
        { url: new URL("/.well-known/oauth-authorization-server", t.origin), type: "oauth" },
        { url: new URL("/.well-known/openid-configuration", t.origin), type: "oidc" },
      ),
      s
    );
  let a = t.pathname;
  if (a.endsWith("/")) a = a.slice(0, -1);
  return (
    s.push(
      { url: new URL(`/.well-known/oauth-authorization-server${a}`, t.origin), type: "oauth" },
      { url: new URL(`/.well-known/openid-configuration${a}`, t.origin), type: "oidc" },
      { url: new URL(`${a}/.well-known/openid-configuration`, t.origin), type: "oidc" },
    ),
    s
  );
}
async function jHe(e, { fetchFn: t = fetch, protocolVersion: r = jWe, skipIssuerValidation: s = !1 } = {}) {
  let a = { "MCP-Protocol-Version": r, Accept: "application/json" },
    o = Dc(e);
  for (let { url: n, type: c } of o) {
    let i = await co(n, a, t);
    if (!i) continue;
    if (!i.ok) {
      if ((await i.text?.().catch(() => {}), (i.status >= 400 && i.status < 500) || i.status === 502)) continue;
      throw Error(`HTTP ${i.status} trying to load ${c === "oauth" ? "OAuth" : "OpenID provider"} metadata from ${n}`);
    }
    let l = c === "oauth" ? $ae.parse(await i.json()) : P_e.parse(await i.json());
    if (!s) {
      let d = typeof e === "string" ? e : e.href;
      if (!(l.issuer === d || (d.endsWith("/") && l.issuer === d.slice(0, -1)))) throw new S_e("metadata", d, l.issuer);
    }
    return l;
  }
}
async function cOt(e, t) {
  let r, s;
  try {
    if (
      ((r = await lOt(e, { resourceMetadataUrl: t?.resourceMetadataUrl }, t?.fetchFn)),
      r.authorization_servers && r.authorization_servers.length > 0)
    )
      s = r.authorization_servers[0];
  } catch (o) {
    if (o instanceof TypeError) throw o;
  }
  if (!s) s = String(new URL("/", e));
  let a = await jHe(s, { fetchFn: t?.fetchFn, skipIssuerValidation: t?.skipIssuerMetadataValidation });
  return { authorizationServerUrl: s, authorizationServerMetadata: a, resourceMetadata: r };
}
async function qrn(e, { metadata: t, clientInformation: r, redirectUrl: s, scope: a, state: o, resource: n }) {
  let c;
  if (t) {
    if (((c = new URL(t.authorization_endpoint)), !t.response_types_supported.includes(os)))
      throw Error(`Incompatible auth server: does not support response type ${os}`);
    if (t.code_challenge_methods_supported && !t.code_challenge_methods_supported.includes(ns))
      throw Error(`Incompatible auth server: does not support code challenge method ${ns}`);
  } else c = new URL("/authorize", e);
  let i = await $9e(),
    l = i.code_verifier,
    d = i.code_challenge;
  if (
    (c.searchParams.set("response_type", os),
    c.searchParams.set("client_id", r.client_id),
    c.searchParams.set("code_challenge", d),
    c.searchParams.set("code_challenge_method", ns),
    c.searchParams.set("redirect_uri", String(s)),
    o)
  )
    c.searchParams.set("state", o);
  if (a) c.searchParams.set("scope", a);
  if (a?.split(" ").includes("offline_access")) c.searchParams.append("prompt", "consent");
  if (n) c.searchParams.set("resource", n.href);
  return { authorizationUrl: c, codeVerifier: l };
}
function lo(e, t, r) {
  return new URLSearchParams({ grant_type: "authorization_code", code: e, code_verifier: t, redirect_uri: String(r) });
}
async function ds(
  e,
  { metadata: t, tokenRequestParams: r, clientInformation: s, addClientAuthentication: a, resource: o, fetchFn: n },
) {
  let c = qc(t?.token_endpoint ?? new URL("/token", e)),
    i = new Headers({ "Content-Type": "application/x-www-form-urlencoded", Accept: "application/json" });
  if (o) r.set("resource", o.href);
  if (a) await a(i, r, c, t);
  else if (s) Tc(Pc(s, t?.token_endpoint_auth_methods_supported ?? []), s, i, r);
  let l = await (n ?? fetch)(c, { method: "POST", headers: i, body: r });
  if (!l.ok) throw await xa(l);
  let d = await l.json();
  try {
    return D_e.parse(d);
  } catch (S) {
    if (typeof d === "object" && d !== null && "error" in d) throw await xa(JSON.stringify(d));
    throw S;
  }
}
async function M6n(
  e,
  {
    metadata: t,
    clientInformation: r,
    authorizationCode: s,
    iss: a,
    codeVerifier: o,
    redirectUri: n,
    resource: c,
    addClientAuthentication: i,
    fetchFn: l,
  },
) {
  return (
    qr({ iss: a, expectedIssuer: t?.issuer, issParameterSupported: kr(t) }),
    ds(e, {
      metadata: t,
      tokenRequestParams: lo(s, o, n),
      clientInformation: r,
      addClientAuthentication: i,
      resource: c,
      fetchFn: l,
    })
  );
}
async function Grn(
  e,
  { metadata: t, clientInformation: r, refreshToken: s, resource: a, addClientAuthentication: o, fetchFn: n },
) {
  return {
    refresh_token: s,
    ...(await ds(e, {
      metadata: t,
      tokenRequestParams: new URLSearchParams({ grant_type: "refresh_token", refresh_token: s }),
      clientInformation: r,
      addClientAuthentication: o,
      resource: a,
      fetchFn: n,
    })),
  };
}
async function Vc(e, t, { metadata: r, resource: s, authorizationCode: a, iss: o, scope: n, fetchFn: c } = {}) {
  if (a !== void 0) qr({ iss: o, expectedIssuer: r?.issuer, issParameterSupported: kr(r) });
  let i = n ?? e.clientMetadata.scope,
    l;
  if (e.prepareTokenRequest) l = await e.prepareTokenRequest(i);
  if (!l) {
    if (!a) throw Error("Either provider.prepareTokenRequest() or authorizationCode is required");
    if (!e.redirectUrl) throw Error("redirectUrl is required for authorization_code flow");
    l = lo(a, await e.codeVerifier(), e.redirectUrl);
  }
  let d = await e.clientInformation({ issuer: r?.issuer ?? String(t) });
  return ds(t, {
    metadata: r,
    tokenRequestParams: l,
    clientInformation: d ?? void 0,
    addClientAuthentication: e.addClientAuthentication,
    resource: s,
    fetchFn: c,
  });
}
async function Fc(e, { metadata: t, clientMetadata: r, scope: s, fetchFn: a }) {
  let o;
  if (t) {
    if (!t.registration_endpoint) throw Error("Incompatible auth server: does not support dynamic client registration");
    o = new URL(t.registration_endpoint);
  } else o = new URL("/register", e);
  let n = { ...r, ...(s === void 0 ? {} : { scope: s }) },
    c = await (a ?? fetch)(o, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(n),
    });
  if (!c.ok) throw new aOt({ status: c.status, body: await c.text(), submittedMetadata: n });
  return Kat.parse(await c.json());
}
var zr = new Set(["tools/list", "prompts/list", "resources/list", "resources/templates/list", "server/discover"]),
  Hc = class {
    _entries = new Map();
    _maxEntries;
    _stamp = 0;
    _cappedSize = 0;
    constructor(e) {
      this._maxEntries = e?.maxEntries ?? 512;
    }
    get size() {
      return this._entries.size;
    }
    get(e) {
      return this._entries.get(cs(e));
    }
    set(e, t) {
      let r = cs(e),
        s = zr.has(e.method),
        a = !this._entries.has(r);
      if (!s && a && this._maxEntries > 0 && this._cappedSize >= this._maxEntries) {
        for (let n of this._entries.keys())
          if (!zr.has(n.slice(0, n.indexOf("\x00")))) {
            this._entries.delete(n), this._cappedSize--;
            break;
          }
      }
      let o = ++this._stamp;
      if ((this._entries.set(r, { ...t, stamp: o }), a && !s)) this._cappedSize++;
      return o;
    }
    delete(e) {
      if (this._entries.delete(cs(e)) && !zr.has(e.method)) this._cappedSize--;
    }
    evict(e) {
      let t = `${e}\x00`,
        r = zr.has(e);
      for (let s of this._entries.keys())
        if (s.startsWith(t)) {
          if ((this._entries.delete(s), !r)) this._cappedSize--;
        }
    }
    clear() {
      this._entries.clear(), (this._cappedSize = 0);
    }
  };
function cs(e) {
  return `${e.method}\x00${JSON.stringify([e.partition ?? "", e.params ?? ""])}`;
}
function ls(e, t) {
  return t === void 0 ? e : `${e}\x00${t}`;
}
var xc = 86400000,
  Kc = class {
    _evictionGeneration = new Map();
    _toolIndex;
    _toolOutputValidatorIndex;
    _serverIdentity = "";
    constructor(e, t, r = () => {}, s = "", a = Date.now) {
      (this._store = e),
        (this._isUserSupplied = t),
        (this._reportError = r),
        (this._cachePartition = s),
        (this._now = a);
    }
    now() {
      return this._now();
    }
    setServerIdentity(e) {
      this._serverIdentity = e;
    }
    _partitionFor(e) {
      return JSON.stringify([this._serverIdentity, e === "public" ? "" : this._cachePartition]);
    }
    async _probe(e, t) {
      let r = { method: e, params: t ?? "" },
        s = this._partitionFor("private"),
        a = await this._store.get({ ...r, partition: s });
      if (a !== void 0) return a;
      let o = this._partitionFor("public");
      if (o === s) return;
      let n = await this._store.get({ ...r, partition: o });
      return n?.scope === "public" ? n : void 0;
    }
    async evict(e) {
      this._evictionGeneration.set(e, (this._evictionGeneration.get(e) ?? 0) + 1), await this._deleteBoth(e, "");
    }
    async _deleteBoth(e, t) {
      let r = this._partitionFor("private"),
        s = this._partitionFor("public");
      try {
        await this._store.delete({ method: e, params: t, partition: r });
      } catch (a) {
        this._reportError(a);
      }
      if (s !== r)
        try {
          await this._store.delete({ method: e, params: t, partition: s });
        } catch (a) {
          this._reportError(a);
        }
    }
    async evictKey(e, t) {
      let r = ls(e, t),
        s = this._evictionGeneration.get(r);
      if (s !== void 0) this._evictionGeneration.set(r, s + 1);
      await this._deleteBoth(e, t);
    }
    captureGeneration(e, t) {
      let r = ls(e, t),
        s = this._evictionGeneration.get(r) ?? 0;
      return this._evictionGeneration.set(r, s), s;
    }
    async write(e, t, r, s) {
      if ((this._evictionGeneration.get(ls(e, s?.params)) ?? 0) !== r) return;
      let a = s?.params ?? "",
        o = this._partitionFor("private"),
        n = this._partitionFor("public"),
        c = (s?.scope ?? "private") === "public" ? n : o;
      try {
        await this._store.set(
          { method: e, params: a, partition: c },
          { value: Jc(t), expiresAt: s?.expiresAt, scope: s?.scope },
        );
      } catch (i) {
        this._reportError(i);
      }
      if (n !== o)
        try {
          await this._store.delete({ method: e, params: a, partition: c === o ? n : o });
        } catch (i) {
          this._reportError(i);
        }
    }
    async read(e, t) {
      let r = await this._probe(e, t);
      if (r?.expiresAt === void 0 || !(r.expiresAt > this.now())) return;
      try {
        let s = JSON.parse(r.value);
        if (typeof s !== "object" || s === null || Array.isArray(s))
          throw TypeError("cached document is not an object");
        return { value: s };
      } catch (s) {
        this._reportError(s), await this._deleteBoth(e, t ?? "");
        return;
      }
    }
    resetForReconnect() {
      if (!this._isUserSupplied) this._store.clear();
      this._evictionGeneration.clear(),
        (this._toolIndex = void 0),
        (this._toolOutputValidatorIndex = void 0),
        (this._serverIdentity = "");
    }
    async toolDefinition(e) {
      let t = await this._probe("tools/list");
      if (t === void 0) {
        this._toolIndex = void 0;
        return;
      }
      if (this._toolIndex?.stamp !== t.stamp) {
        let r = this._decodeListTools(t),
          s = new Map();
        if (r !== void 0) for (let a of r.tools) s.set(a.name, a);
        this._toolIndex = { stamp: t.stamp, byName: s };
      }
      return this._toolIndex.byName.get(e);
    }
    async outputValidator(e, t) {
      let r = await this._probe("tools/list");
      if (r === void 0) {
        this._toolOutputValidatorIndex = void 0;
        return;
      }
      if (this._toolOutputValidatorIndex?.stamp !== r.stamp) {
        let s = this._decodeListTools(r) ?? { tools: [] },
          a = new Map();
        for (let o of s.tools) {
          let n = t(o);
          if (n !== void 0) a.set(o.name, n);
        }
        this._toolOutputValidatorIndex = { stamp: r.stamp, byName: a };
      }
      return this._toolOutputValidatorIndex.byName.get(e);
    }
    _decodeListTools(e) {
      try {
        let t = JSON.parse(e.value);
        if (!Array.isArray(t?.tools) || !t.tools.every((r) => r !== null && typeof r === "object"))
          throw TypeError("cached tools/list document has a malformed tools array");
        return t;
      } catch (t) {
        this._reportError(t);
        return;
      }
    }
  };
function Jc(e) {
  let t;
  try {
    t = JSON.stringify(e);
  } catch (r) {
    throw TypeError(`cache value is not JSON-serializable: ${r instanceof Error ? r.message : String(r)}`);
  }
  if (typeof t !== "string") throw TypeError("cache value is not JSON-serializable: it has no JSON representation");
  return t;
}
var Gc = -32022,
  Bc = new Set([-32001, -32020, -32021]);
function Wc(e, t) {
  switch (e.kind) {
    case "result":
      return Yc(e.result, t);
    case "rpc-error":
      return uo(e, t);
    case "http-error":
      return Xc(e, t);
    case "network-error":
      return Ja(e.error, t);
    case "auth-required":
      return { kind: "error", error: e.error };
    case "closed":
      if (t.transportKind === "stdio") return { kind: "legacy" };
      return Ja(Error("Connection closed during the version negotiation probe"), t);
    case "timeout":
      if (t.transportKind === "stdio") return { kind: "legacy" };
      return {
        kind: "error",
        error: new Uo(vo.RequestTimeout, `Version negotiation probe timed out after ${e.timeoutMs}ms`, {
          timeout: e.timeoutMs,
        }),
      };
    case "id-mismatch":
    case "malformed":
    case "stream-end":
      return { kind: "legacy" };
  }
}
function Yc(e, t) {
  let r = Te(gr).validateResult("server/discover", e);
  if (!r.ok) return { kind: "legacy" };
  let s = r.value.supportedVersions,
    a = t.clientModernVersions.find((o) => s.includes(o));
  if (a !== void 0) return { kind: "modern", version: a, discover: r.value };
  if (t.fallbackAvailable) return { kind: "legacy" };
  return { kind: "error", error: new Sr({ supported: [...s], requested: t.requestedVersion }) };
}
function uo(e, t) {
  let { code: r, message: s, data: a } = e;
  if (r === Gc) {
    let o = el(a);
    if (o === void 0) return { kind: "legacy" };
    let n = new Sr({ supported: o, requested: tl(a) ?? t.requestedVersion }, s),
      c = Vt(o),
      i = t.clientModernVersions.find((l) => c.includes(l));
    if (i !== void 0) return { kind: "corrective", version: i, error: n };
    if (c.length > 0) return { kind: "error", error: n };
    return t.fallbackAvailable ? { kind: "legacy" } : { kind: "error", error: n };
  }
  if (Bc.has(r)) return { kind: "legacy" };
  return { kind: "legacy" };
}
function Xc(e, t) {
  let r = rl(e.body);
  if (r !== void 0) return uo(r, t);
  return { kind: "legacy" };
}
function Ja(e, t) {
  if (t.environment === "browser" && Zc(e)) return { kind: "legacy" };
  return {
    kind: "error",
    error: new Uo(vo.EraNegotiationFailed, `Version negotiation probe failed: ${Qc(e)}`, { cause: e }),
  };
}
function Zc(e) {
  return e instanceof TypeError || (e instanceof Error && e.name === "TypeError");
}
function Qc(e) {
  return e instanceof Error ? e.message : String(e);
}
function el(e) {
  if (typeof e !== "object" || e === null) return;
  let t = e.supported;
  if (!Array.isArray(t) || t.length === 0 || !t.every((r) => typeof r === "string")) return;
  return t;
}
function tl(e) {
  if (typeof e !== "object" || e === null) return;
  let t = e.requested;
  return typeof t === "string" ? t : void 0;
}
function rl(e) {
  if (e === void 0 || e === "") return;
  let t;
  try {
    t = JSON.parse(e);
  } catch {
    return;
  }
  if (typeof t !== "object" || t === null) return;
  let r = t.error;
  if (typeof r !== "object" || r === null) return;
  let { code: s, message: a, data: o } = r;
  if (typeof s !== "number") return;
  return { code: s, message: typeof a === "string" ? a : "", data: o };
}
var sl = "legacy";
function al(e, t) {
  let r = e?.mode ?? sl;
  if (r === "legacy") return { kind: "legacy" };
  let s = e?.probe ?? {};
  if (typeof r === "object") {
    if (!ze(r.pin))
      throw TypeError(
        `versionNegotiation: { pin: '${r.pin}' } is not a modern protocol revision \u2014 pinning is for 2026-07-28 and later; omit versionNegotiation (or use mode: 'legacy') for 2025-era servers.`,
      );
    return { kind: "pin", version: r.pin, probe: s };
  }
  let a = t ? Vt(t) : [];
  return {
    kind: "auto",
    modernVersions: a.length > 0 ? a : [...Hr],
    fallbackAvailable: t ? xr(t).length > 0 : !0,
    probe: s,
  };
}
function Ga() {
  let e = globalThis;
  return e.window !== void 0 && e.document !== void 0 ? "browser" : "node";
}
function Ba(e) {
  return "stderr" in e && "pid" in e ? "stdio" : "http";
}
var ol = class e {
    _pending;
    _probeCounter = 0;
    _transportKind;
    _issuedIds = new Set();
    _errorSinceSend;
    _savedOnMessage;
    _savedOnError;
    _savedOnClose;
    _closeDelivered = !1;
    constructor(t) {
      (this._transport = t),
        (this._savedOnMessage = t.onmessage),
        (this._savedOnError = t.onerror),
        (this._savedOnClose = t.onclose);
    }
    static async open(t, r) {
      let s = new e(t);
      (s._transportKind = r),
        (t.onmessage = (a) => {
          let o = s._pending;
          if (o !== void 0 && (Ke(a) || Ge(a)) && a.id === o.id) {
            if (((s._pending = void 0), Ke(a))) o.resolve({ kind: "response", result: a.result });
            else o.resolve({ kind: "response", error: a.error });
            return;
          }
          if (o !== void 0 && s._transportKind === "http" && (Ke(a) || Ge(a)) && !s._issuedIds.has(a.id)) {
            s._pending = void 0;
            let n = Wa(JSON.stringify(a.id)),
              c = Error(
                `server/discover probe reply did not echo the request id (expected ${JSON.stringify(o.id)}, got ${n}) \u2014 treating the server as pre-2026-07-28`,
              );
            (c.name = "McpProbeIdMismatchError"), s._savedOnError?.(c), o.resolve({ kind: "id-mismatch" });
            return;
          }
        }),
        (t.onerror = (a) => {
          if (s._pending !== void 0 && s._errorSinceSend === void 0) s._errorSinceSend = a;
          let o = s._pending;
          if (o !== void 0 && s._transportKind === "http" && Ya(a))
            setTimeout(() => {
              if (s._pending !== o) return;
              s._pending = void 0;
              let n = Error(
                `server/discover probe reply failed JSON-RPC message decoding (${Wa(a instanceof Error ? a.message : String(a))}) \u2014 treating the server as pre-2026-07-28`,
              );
              (n.name = "McpProbeMalformedReplyError"), s._savedOnError?.(n), o.resolve({ kind: "malformed" });
            }, 0);
          s._savedOnError?.(a);
        }),
        (t.onclose = () => {
          let a = s._pending;
          if (a !== void 0) (s._pending = void 0), a.resolve({ kind: "closed" });
          (s._closeDelivered = !0), s._savedOnClose?.();
        });
      try {
        await t.start();
      } catch (a) {
        throw (s.detach(), a);
      }
      return s;
    }
    async exchange(t, r) {
      let s = `server-discover-probe-${++this._probeCounter}`;
      return (
        this._issuedIds.add(s),
        (this._errorSinceSend = void 0),
        new Promise((a) => {
          let o = !1,
            n = (i) => {
              if (o) return;
              if (((o = !0), clearTimeout(c), this._pending?.id === s)) this._pending = void 0;
              a(i);
            },
            c = setTimeout(() => n({ kind: "timeout" }), r);
          (this._pending = { id: s, resolve: n }),
            this._transport
              .send(t(s), {
                onRequestStreamEnd: () => {
                  setTimeout(() => {
                    let i = this._errorSinceSend;
                    if (i !== void 0 && !Ya(i)) {
                      n({ kind: "send-error", error: i instanceof Error ? i : Error(String(i)) });
                      return;
                    }
                    n({ kind: "stream-end" });
                  }, 0);
                },
              })
              .catch((i) => n({ kind: "send-error", error: i }));
        })
      );
    }
    detach() {
      if (
        ((this._pending = void 0),
        (this._transport.onmessage = this._savedOnMessage),
        (this._transport.onerror = this._savedOnError),
        this._closeDelivered && this._savedOnClose !== void 0)
      ) {
        let t = this._savedOnClose,
          r = this._transport,
          s = !1,
          a = () => {
            if (!s) {
              s = !0;
              return;
            }
            t();
          };
        (r.onclose = a),
          us.set(r, () => {
            if (r.onclose === a) r.onclose = t;
          });
      } else this._transport.onclose = this._savedOnClose;
    }
    release() {
      this.detach();
      let t = this._transport,
        r = t.start,
        s = !0;
      t.start = async function () {
        if (s) {
          (s = !1), (t.start = r);
          return;
        }
        return r.call(t);
      };
    }
  },
  us = new WeakMap();
function Wa(e) {
  let t = String(e);
  if (t.length <= 200) return t;
  let r = (t.charCodeAt(199) & 64512) === 55296 ? 199 : 200;
  return `${t.slice(0, r)}\u2026 [truncated]`;
}
function Ya(e) {
  if (e instanceof SyntaxError) return !0;
  return typeof e === "object" && e !== null && (e.name === "ZodError" || Array.isArray(e.issues));
}
function Xa(e) {
  let t = us.get(e);
  us.delete(e), t?.();
}
function nl(e, t, r, s) {
  return {
    jsonrpc: "2.0",
    id: e,
    method: "server/discover",
    params: { _meta: Te(t).outboundEnvelope({ protocolVersion: t, clientInfo: r, clientCapabilities: s }) },
  };
}
function il(e, t) {
  switch (e.kind) {
    case "response":
      return e.error === void 0 ? { kind: "result", result: e.result } : { kind: "rpc-error", ...e.error };
    case "send-error": {
      let r = e.error;
      if (r instanceof RS) {
        let s = r.data?.text;
        return { kind: "http-error", status: r.data.status, body: typeof s === "string" ? s : void 0 };
      }
      if (r instanceof xA || (r instanceof Error && r.name === "UnauthorizedError"))
        return { kind: "auth-required", error: r };
      return { kind: "network-error", error: r };
    }
    case "closed":
      return { kind: "closed" };
    case "id-mismatch":
      return { kind: "id-mismatch" };
    case "malformed":
      return { kind: "malformed" };
    case "stream-end":
      return { kind: "stream-end" };
    case "timeout":
      return { kind: "timeout", timeoutMs: t };
  }
}
async function ho(e, t) {
  let r = e.probe.timeoutMs ?? t.defaultTimeoutMs,
    s = Math.max(0, e.probe.maxRetries ?? 0),
    a = e.kind === "pin" ? [e.version] : e.modernVersions,
    o = e.kind === "auto" && e.fallbackAvailable,
    n = await ol.open(t.transport, t.transportKind),
    c = async () => {
      let l = a[0],
        d = !1,
        S = s;
      for (;;) {
        let R = await n.exchange((_) => nl(_, l, t.clientInfo, t.capabilities), r);
        if (R.kind === "timeout" && S > 0) {
          S--;
          continue;
        }
        let w = il(R, r);
        if (w.kind === "timeout") t.transport._anthropicProbeTimedOut = !0;
        let g = Wc(w, {
          clientModernVersions: a,
          requestedVersion: l,
          fallbackAvailable: o,
          environment: t.environment,
          transportKind: t.transportKind,
        });
        switch (g.kind) {
          case "modern":
            return { era: "modern", version: g.version, discover: g.discover };
          case "corrective":
            if (d) throw g.error;
            (d = !0), (l = g.version);
            continue;
          case "legacy": {
            if (w.kind === "id-mismatch" || w.kind === "malformed" || w.kind === "stream-end")
              t.transport._anthropicProbeLegacyEvidence = w.kind;
            let _ = w.kind === "closed" ? "the connection closed during the server/discover probe" : void 0;
            if (e.kind === "pin")
              throw new Uo(
                vo.EraNegotiationFailed,
                _ === void 0
                  ? `Version negotiation failed: the server did not offer pinned protocol version ${e.version} via server/discover (no fallback in pin mode)`
                  : `Version negotiation failed: ${_} before the server offered pinned protocol version ${e.version} (no fallback in pin mode)`,
              );
            if (!e.fallbackAvailable)
              throw new Uo(
                vo.EraNegotiationFailed,
                _ === void 0
                  ? "Version negotiation failed: the server gave no modern evidence and this client supports no pre-2026-07-28 protocol version to fall back to"
                  : `Version negotiation failed: ${_} and this client supports no pre-2026-07-28 protocol version to fall back to`,
              );
            if (_ !== void 0 && t.disposableProbe !== !0)
              throw new Uo(
                vo.EraNegotiationFailed,
                `Version negotiation failed: ${_} (this transport probed in place \u2014 the disposable sibling probe requires the SDK's base StdioClientTransport)`,
              );
            return { era: "legacy" };
          }
          case "error":
            throw g.error;
        }
      }
    },
    i;
  try {
    i = await c();
  } catch (l) {
    throw (n.detach(), l);
  }
  return n.release(), i;
}
function cl(e) {
  let t = Object.getPrototypeOf(e);
  if (t === null || !Object.prototype.hasOwnProperty.call(t, "_dispose")) return;
  let r = e._serverParams;
  return typeof r === "object" && r !== null && typeof r.command === "string" ? r : void 0;
}
async function ll(e, t, r, s) {
  let o = new t.constructor({ ...r, stderr: "ignore" }),
    n = t.close,
    c = !1,
    i,
    l = new Promise((S, R) => {
      i = () => R(Za());
    });
  t.close = async function () {
    return (c = !0), i?.(), n.call(t);
  };
  let d;
  try {
    let S = ho(e, { ...s, transport: o, transportKind: "stdio", disposableProbe: !0 });
    S.catch(() => {}), (d = await Promise.race([S, l]));
  } finally {
    await ul(o), (t.close = n);
  }
  if (c) throw Za();
  return d;
}
function Za() {
  return new Uo(
    vo.EraNegotiationFailed,
    "Version negotiation failed: the transport was closed during the server/discover probe",
  );
}
async function ul(e) {
  try {
    let t = e._dispose;
    await (typeof t === "function" ? t.call(e) : e.close());
  } catch {}
}
function Qa(e) {
  let t = e._meta?.[Oae];
  return pa.Implementation(t) ? t : void 0;
}
function Cr(e, t) {
  if (!e || t === null || typeof t !== "object") return;
  if (e.type === "object" && e.properties && typeof e.properties === "object") {
    let r = t,
      s = e.properties;
    for (let a of Object.keys(s)) {
      let o = s[a];
      if (r[a] === void 0 && Object.prototype.hasOwnProperty.call(o, "default")) r[a] = o.default;
      if (r[a] !== void 0) Cr(o, r[a]);
    }
  }
  if (Array.isArray(e.anyOf)) {
    for (let r of e.anyOf) if (typeof r !== "boolean") Cr(r, t);
  }
  if (Array.isArray(e.oneOf)) {
    for (let r of e.oneOf) if (typeof r !== "boolean") Cr(r, t);
  }
}
function dl(e) {
  if (!e) return { supportsFormMode: !1, supportsUrlMode: !1 };
  let t = e.form !== void 0,
    r = e.url !== void 0;
  return { supportsFormMode: t || (!t && !r), supportsUrlMode: r };
}
function hl(e) {
  if (typeof e === "object" && e !== null) {
    if (e.kind === "legacy" && !("supportedVersions" in e) && !("discover" in e)) return e;
    if (e.kind === "modern" && YHe.safeParse(e.discover).success) return e;
  }
  throw new Uo(
    vo.EraNegotiationFailed,
    "connect({ prior }): unrecognized prior \u2014 expected { kind: 'modern', discover } or { kind: 'legacy' }",
  );
}
var eo = {
    "notifications/tools/list_changed": ["tools/list"],
    "notifications/prompts/list_changed": ["prompts/list"],
    "notifications/resources/list_changed": ["resources/list", "resources/templates/list"],
  },
  ml = 64,
  zrn = class extends Sa {
    _serverCapabilities;
    _serverVersion;
    _capabilities;
    _instructions;
    _jsonSchemaValidator;
    _cache;
    _defaultCacheTtlMs;
    _listMaxPages;
    _listChangedDebounceTimers = new Map();
    _listChangedConfig;
    _enforceStrictCapabilities;
    _versionNegotiation;
    _supportedProtocolVersionsOption;
    _inputRequiredDriverConfig;
    _listenState = new Map();
    _nextListenId = 0;
    _autoOpenedSubscription;
    _discoverResult;
    _resetConnectionState() {
      if (
        ((this._negotiatedProtocolVersion = void 0),
        (this._serverCapabilities = void 0),
        (this._serverVersion = void 0),
        (this._instructions = void 0),
        (this._discoverResult = void 0),
        (this._autoOpenedSubscription = void 0),
        this._listenState.size > 0)
      ) {
        let e = new Uo(
          vo.ConnectionClosed,
          "subscriptions/listen: client reconnected or closed; subscription state from the previous connection was reset",
        );
        for (let t of this._listenState.values()) t.settle({ cause: "remote", error: e });
      }
      this._listenState.clear();
      for (let e of this._listChangedDebounceTimers.values()) clearTimeout(e);
      this._listChangedDebounceTimers.clear(), this._cache.resetForReconnect();
    }
    async close() {
      try {
        await super.close();
      } finally {
        this._resetConnectionState();
      }
    }
    constructor(e, t) {
      super(t);
      if (
        ((this._clientInfo = e),
        (this._capabilities = t?.capabilities ? { ...t.capabilities } : {}),
        (this._jsonSchemaValidator = t?.jsonSchemaValidator ?? new UWe()),
        (this._enforceStrictCapabilities = t?.enforceStrictCapabilities ?? !1),
        (this._versionNegotiation = t?.versionNegotiation),
        (this._supportedProtocolVersionsOption = t?.supportedProtocolVersions),
        (this._inputRequiredDriverConfig = da(t?.inputRequired)),
        (this._cache = new Kc(
          t?.responseCacheStore ?? new Hc(),
          t?.responseCacheStore !== void 0,
          (r) => this._reportStoreError(r),
          t?.cachePartition ?? "",
        )),
        (this._defaultCacheTtlMs = t?.defaultCacheTtlMs ?? 0),
        (this._listMaxPages = t?.listMaxPages ?? ml),
        t?.listChanged)
      )
        this._listChangedConfig = t.listChanged;
    }
    buildContext(e, t) {
      return e;
    }
    _shouldDropInbound(e) {
      if (this._negotiatedProtocolVersion !== void 0 && ze(this._negotiatedProtocolVersion) && BZ(e)) return "drop";
    }
    _outboundMetaEnvelope() {
      let e = this._negotiatedProtocolVersion;
      if (e === void 0) return;
      return this._wireCodec().outboundEnvelope({
        protocolVersion: e,
        clientInfo: this._clientInfo,
        clientCapabilities: this._capabilities,
      });
    }
    _resolveNonCompleteResult(e, t) {
      if (!this._inputRequiredDriverConfig.autoFulfill)
        return Promise.reject(
          new Uo(
            vo.UnsupportedResultType,
            `Unsupported result type 'input_required' for ${t.request.method}: multi-round-trip auto-fulfilment is not enabled on this instance \u2014 pass allowInputRequired: true to handle it manually, or enable inputRequired.autoFulfill`,
            { resultType: "input_required", method: t.request.method },
          ),
        );
      return _a(
        {
          getRequestHandler: (r) => this._getRequestHandler(r),
          buildContext: (r) => this.buildContext(r, void 0),
          sessionId: this.transport?.sessionId,
        },
        this._inputRequiredDriverConfig,
        e,
        t,
      );
    }
    _setupListChangedHandlers(e) {
      if (e.tools && this._serverCapabilities?.tools?.listChanged)
        this._setupListChangedHandler(
          "tools",
          "notifications/tools/list_changed",
          e.tools,
          async () => (await this.listTools(void 0, { cacheMode: "refresh" })).tools,
        );
      if (e.prompts && this._serverCapabilities?.prompts?.listChanged)
        this._setupListChangedHandler(
          "prompts",
          "notifications/prompts/list_changed",
          e.prompts,
          async () => (await this.listPrompts(void 0, { cacheMode: "refresh" })).prompts,
        );
      if (e.resources && this._serverCapabilities?.resources?.listChanged)
        this._setupListChangedHandler(
          "resources",
          "notifications/resources/list_changed",
          e.resources,
          async () => (await this.listResources(void 0, { cacheMode: "refresh" })).resources,
        );
    }
    registerCapabilities(e) {
      if (this.transport) throw Error("Cannot register capabilities after connecting to transport");
      this._capabilities = ga(this._capabilities, e);
    }
    setVersionNegotiation(e) {
      if (this.transport) throw Error("Cannot configure version negotiation after connecting to transport");
      this._versionNegotiation = e;
    }
    _wrapHandler(e, t) {
      if (e === "elicitation/create")
        return async (r, s) => {
          let a = Te(this._negotiatedProtocolVersion),
            o = a.validateRequest("elicitation/create", r);
          if (!o.ok && o.reason === "not-in-era") o = a.validateInputRequest("elicitation/create", r);
          if (!o.ok)
            throw new wa(
              o.reason === "not-in-era" ? Ai.InternalError : Ai.InvalidParams,
              o.reason === "not-in-era"
                ? "No wire schema for elicitation/create in the resolved era"
                : `Invalid elicitation request: ${o.message}`,
            );
          let { params: n } = o.value;
          n.mode = n.mode ?? "form";
          let { supportsFormMode: c, supportsUrlMode: i } = dl(this._capabilities.elicitation);
          if (n.mode === "form" && !c)
            throw new wa(Ai.InvalidParams, "Client does not support form-mode elicitation requests");
          if (n.mode === "url" && !i)
            throw new wa(Ai.InvalidParams, "Client does not support URL-mode elicitation requests");
          let l = await t(r, s),
            d = a.validateResult("elicitation/create", l);
          if (!d.ok && d.reason === "not-in-era") d = a.validateInputResponse("elicitation/create", l);
          if (!d.ok)
            throw new wa(
              d.reason === "not-in-era" ? Ai.InternalError : Ai.InvalidParams,
              d.reason === "not-in-era"
                ? "No wire schema for elicitation/create in the resolved era"
                : `Invalid elicitation result: ${d.message}`,
            );
          let S = d.value,
            R = n.mode === "form" ? n.requestedSchema : void 0;
          if (
            n.mode === "form" &&
            S.action === "accept" &&
            S.content &&
            R &&
            this._capabilities.elicitation?.form?.applyDefaults
          )
            try {
              Cr(R, S.content);
            } catch {}
          return S;
        };
      if (e === "sampling/createMessage")
        return async (r, s) => {
          let a = Te(this._negotiatedProtocolVersion),
            o = a.validateRequest("sampling/createMessage", r);
          if (!o.ok && o.reason === "not-in-era") o = a.validateInputRequest("sampling/createMessage", r);
          if (!o.ok)
            throw new wa(
              o.reason === "not-in-era" ? Ai.InternalError : Ai.InvalidParams,
              o.reason === "not-in-era"
                ? "No wire schema for sampling/createMessage in the resolved era"
                : `Invalid sampling request: ${o.message}`,
            );
          let { params: n } = o.value,
            c = await t(r, s),
            i = Boolean(n.tools || n.toolChoice),
            l = a.samplingResultVariant(i, c);
          if (!l.ok && l.reason === "not-in-era") l = a.validateInputResponse("sampling/createMessage", c);
          if (!l.ok)
            throw new wa(
              l.reason === "not-in-era" ? Ai.InternalError : Ai.InvalidParams,
              l.reason === "not-in-era"
                ? "No result schema for sampling/createMessage in the resolved era"
                : `Invalid sampling result: ${l.message}`,
            );
          return l.value;
        };
      return t;
    }
    assertCapability(e, t) {
      if (!this._serverCapabilities?.[e])
        throw new Uo(vo.CapabilityNotSupported, `Server does not support ${e} (required for ${t})`);
    }
    async connect(e, t) {
      if (t?.prior != null) return this._connectFromPrior(e, hl(t.prior), t);
      let r = al(this._versionNegotiation, this._supportedProtocolVersionsOption);
      if (r.kind !== "legacy") return this._connectNegotiated(e, r, t);
      return this._connectPlainLegacy(e, t);
    }
    async _connectPlainLegacy(e, t) {
      if ((await super.connect(e), e.sessionId !== void 0)) {
        let r = this._negotiatedProtocolVersion;
        if (r !== void 0) e.setProtocolVersion?.(r);
        return;
      }
      this._resetConnectionState(), await this._legacyHandshake(e, t);
    }
    async _legacyHandshake(e, t) {
      let r = xr(this._supportedProtocolVersions);
      try {
        let s = r[0];
        if (s === void 0)
          throw new Uo(
            vo.EraNegotiationFailed,
            "Cannot run the initialize handshake: supportedProtocolVersions contains no pre-2026-07-28 protocol version",
          );
        let a = await this.request(
          {
            method: "initialize",
            params: { protocolVersion: s, capabilities: this._capabilities, clientInfo: this._clientInfo },
          },
          t,
        );
        if (a === void 0) throw Error(`Server sent invalid initialize result: ${a}`);
        if (!r.includes(a.protocolVersion))
          throw Error(`Server's protocol version is not supported: ${a.protocolVersion}`);
        if (
          ((this._serverCapabilities = a.capabilities),
          (this._serverVersion = a.serverInfo),
          this._cache.setServerIdentity(this._deriveServerIdentity(e)),
          e.setProtocolVersion)
        )
          e.setProtocolVersion(a.protocolVersion);
        if (
          ((this._instructions = a.instructions),
          await this.notification({ method: "notifications/initialized" }),
          (this._negotiatedProtocolVersion = a.protocolVersion),
          this._listChangedConfig)
        )
          this._setupListChangedHandlers(this._listChangedConfig);
      } catch (s) {
        throw (this.close(), s);
      }
    }
    async _connectNegotiated(e, t, r) {
      if (e.sessionId !== void 0) {
        await super.connect(e);
        let a = this._negotiatedProtocolVersion;
        if (a !== void 0 && e.setProtocolVersion) e.setProtocolVersion(a);
        return;
      }
      this._resetConnectionState();
      let s;
      try {
        let a = Ba(e),
          o = {
            clientInfo: this._clientInfo,
            capabilities: this._capabilities,
            environment: Ga(),
            defaultTimeoutMs: r?.timeout ?? yr,
          },
          n = a === "stdio" ? cl(e) : void 0;
        s = n === void 0 ? await ho(t, { ...o, transport: e, transportKind: a }) : await ll(t, e, n, o);
      } catch (a) {
        throw (await e.close().catch(() => {}), Xa(e), a);
      }
      if ((Xa(e), await super.connect(e), s.era === "legacy")) {
        await this._legacyHandshake(e, r);
        return;
      }
      if (
        ((this._serverCapabilities = s.discover.capabilities),
        (this._serverVersion = Qa(s.discover)),
        this._cache.setServerIdentity(this._deriveServerIdentity(e)),
        (this._instructions = s.discover.instructions),
        (this._discoverResult = s.discover),
        (this._negotiatedProtocolVersion = s.version),
        e.setProtocolVersion)
      )
        e.setProtocolVersion(s.version);
      if (this._listChangedConfig) {
        let a = this._listChangedConfig,
          o = this._serverCapabilities,
          n = {
            ...(a.tools && o?.tools?.listChanged && { tools: a.tools }),
            ...(a.prompts && o?.prompts?.listChanged && { prompts: a.prompts }),
            ...(a.resources && o?.resources?.listChanged && { resources: a.resources }),
          },
          c = !0;
        try {
          this._setupListChangedHandlers(n);
        } catch (l) {
          (c = !1), this.onerror?.(l instanceof Error ? l : Error(String(l)));
        }
        let i = c
          ? {
              ...(n.tools && { toolsListChanged: !0 }),
              ...(n.prompts && { promptsListChanged: !0 }),
              ...(n.resources && { resourcesListChanged: !0 }),
            }
          : {};
        if (Object.keys(i).length > 0) {
          let l = new AbortController(),
            d = () => l.abort(r?.signal?.reason);
          if (r?.signal?.aborted) d();
          r?.signal?.addEventListener("abort", d);
          try {
            this._autoOpenedSubscription = await this.listen(i, { timeout: r?.timeout, signal: l.signal });
          } catch (S) {
            if (r?.signal?.aborted) throw (await this.close().catch(() => {}), S);
            this.onerror?.(S instanceof Error ? S : Error(String(S)));
          } finally {
            r?.signal?.removeEventListener("abort", d);
          }
        }
      }
    }
    async _connectFromPrior(e, t, r) {
      if (t.kind === "legacy") return this._connectPlainLegacy(e, r);
      let s = t.discover;
      this._resetConnectionState();
      let a = this._supportedProtocolVersionsOption,
        o = (a && Vt(a).length > 0 ? Vt(a) : Hr).find((n) => s.supportedVersions.includes(n));
      if (o === void 0)
        throw new Uo(
          vo.EraNegotiationFailed,
          "connect({ prior }) with a modern verdict requires a 2026-07-28+ mutual protocol version; the supplied DiscoverResult and this client's supportedProtocolVersions have no modern overlap. For a server known to be legacy, pass prior: { kind: 'legacy' } to skip the probe and initialize directly, or use versionNegotiation: { mode: 'auto' } to re-probe with legacy fallback.",
        );
      if (
        (await super.connect(e),
        (this._discoverResult = s),
        (this._serverCapabilities = s.capabilities),
        (this._serverVersion = Qa(s)),
        this._cache.setServerIdentity(this._deriveServerIdentity(e)),
        (this._instructions = s.instructions),
        (this._negotiatedProtocolVersion = o),
        e.setProtocolVersion?.(o),
        this._listChangedConfig)
      )
        try {
          this._setupListChangedHandlers(this._listChangedConfig);
        } catch (n) {
          this.onerror?.(n instanceof Error ? n : Error(String(n)));
        }
    }
    getServerCapabilities() {
      return this._serverCapabilities;
    }
    getServerVersion() {
      return this._serverVersion;
    }
    _deriveServerIdentity(e) {
      let t = this._serverVersion;
      if (t !== void 0) return `${t.name}@${t.version}`;
      return e.sessionId ?? `anonymous:${Date.now()}-${Math.random().toString(36).slice(2)}`;
    }
    getNegotiatedProtocolVersion() {
      return this._negotiatedProtocolVersion;
    }
    getProtocolEra() {
      let e = this._negotiatedProtocolVersion;
      if (e === void 0) return;
      return ze(e) ? "modern" : "legacy";
    }
    getInstructions() {
      return this._instructions;
    }
    getDiscoverResult() {
      return this._discoverResult;
    }
    assertCapabilityForMethod(e) {
      switch (e) {
        case "logging/setLevel":
          if (!this._serverCapabilities?.logging)
            throw new Uo(vo.CapabilityNotSupported, `Server does not support logging (required for ${e})`);
          break;
        case "prompts/get":
        case "prompts/list":
          if (!this._serverCapabilities?.prompts)
            throw new Uo(vo.CapabilityNotSupported, `Server does not support prompts (required for ${e})`);
          break;
        case "resources/list":
        case "resources/templates/list":
        case "resources/read":
        case "resources/subscribe":
        case "resources/unsubscribe":
          if (!this._serverCapabilities?.resources)
            throw new Uo(vo.CapabilityNotSupported, `Server does not support resources (required for ${e})`);
          if (e === "resources/subscribe" && !this._serverCapabilities.resources.subscribe)
            throw new Uo(
              vo.CapabilityNotSupported,
              `Server does not support resource subscriptions (required for ${e})`,
            );
          break;
        case "tools/call":
        case "tools/list":
          if (!this._serverCapabilities?.tools)
            throw new Uo(vo.CapabilityNotSupported, `Server does not support tools (required for ${e})`);
          break;
        case "completion/complete":
          if (!this._serverCapabilities?.completions)
            throw new Uo(vo.CapabilityNotSupported, `Server does not support completions (required for ${e})`);
          break;
        case "initialize":
          break;
        case "server/discover":
          break;
        case "ping":
          break;
      }
    }
    assertNotificationCapability(e) {
      switch (e) {
        case "notifications/roots/list_changed":
          if (!this._capabilities.roots?.listChanged)
            throw new Uo(
              vo.CapabilityNotSupported,
              `Client does not support roots list changed notifications (required for ${e})`,
            );
          break;
        case "notifications/initialized":
          break;
        case "notifications/cancelled":
          break;
        case "notifications/progress":
          break;
      }
    }
    assertRequestHandlerCapability(e) {
      switch (e) {
        case "sampling/createMessage":
          if (!this._capabilities.sampling)
            throw new Uo(vo.CapabilityNotSupported, `Client does not support sampling capability (required for ${e})`);
          break;
        case "elicitation/create":
          if (!this._capabilities.elicitation)
            throw new Uo(
              vo.CapabilityNotSupported,
              `Client does not support elicitation capability (required for ${e})`,
            );
          break;
        case "roots/list":
          if (!this._capabilities.roots)
            throw new Uo(vo.CapabilityNotSupported, `Client does not support roots capability (required for ${e})`);
          break;
        case "ping":
          break;
      }
    }
    async ping(e) {
      return this.request({ method: "ping" }, e);
    }
    async discover(e) {
      let t = await this._requestWithSchema({ method: "server/discover" }, YHe, e);
      return (this._discoverResult = t), t;
    }
    async complete(e, t) {
      return this.request({ method: "completion/complete", params: e }, t);
    }
    async setLoggingLevel(e, t) {
      return this.request({ method: "logging/setLevel", params: { level: e } }, t);
    }
    async getPrompt(e, t) {
      return this.request({ method: "prompts/get", params: e }, t);
    }
    async listPrompts(e, t) {
      if (!this._serverCapabilities?.prompts && !this._enforceStrictCapabilities)
        return (
          console.debug(
            "Client.listPrompts() called but server does not advertise prompts capability - returning empty list",
          ),
          { prompts: [] }
        );
      if (e?.cursor !== void 0) return this.request({ method: "prompts/list", params: e }, t);
      let r = await this._serveFromCache("prompts/list", void 0, t);
      if (r !== void 0) return r;
      return this._listAllPages("prompts/list", e, t, (s, a) => s.prompts.push(...a.prompts));
    }
    async listResources(e, t) {
      if (!this._serverCapabilities?.resources && !this._enforceStrictCapabilities)
        return (
          console.debug(
            "Client.listResources() called but server does not advertise resources capability - returning empty list",
          ),
          { resources: [] }
        );
      if (e?.cursor !== void 0) return this.request({ method: "resources/list", params: e }, t);
      let r = await this._serveFromCache("resources/list", void 0, t);
      if (r !== void 0) return r;
      return this._listAllPages("resources/list", e, t, (s, a) => s.resources.push(...a.resources));
    }
    async listResourceTemplates(e, t) {
      if (!this._serverCapabilities?.resources && !this._enforceStrictCapabilities)
        return (
          console.debug(
            "Client.listResourceTemplates() called but server does not advertise resources capability - returning empty list",
          ),
          { resourceTemplates: [] }
        );
      if (e?.cursor !== void 0) return this.request({ method: "resources/templates/list", params: e }, t);
      let r = await this._serveFromCache("resources/templates/list", void 0, t);
      if (r !== void 0) return r;
      return this._listAllPages("resources/templates/list", e, t, (s, a) =>
        s.resourceTemplates.push(...a.resourceTemplates),
      );
    }
    async _listAllPages(e, t, r, s, a) {
      let o = r?.cacheMode === "bypass",
        n = this._cache.captureGeneration(e),
        c = await this.request({ method: e, ...(t && { params: { ...t } }) }, r),
        i = c.nextCursor,
        l = new Set(),
        d = 1;
      while (i !== void 0 && !l.has(i)) {
        if (this._listMaxPages !== 0 && d >= this._listMaxPages)
          throw new Uo(
            vo.ListPaginationExceeded,
            `${e}: exceeded listMaxPages (${this._listMaxPages}); server pagination did not terminate`,
            { method: e, listMaxPages: this._listMaxPages },
          );
        l.add(i);
        let S = await this.request({ method: e, params: { ...t, cursor: i } }, r);
        s(c, S), (i = S.nextCursor), d++;
      }
      if ((delete c.nextCursor, a?.(c), o)) return c;
      return await this._cache.write(e, c, n, this._freshness(c)), c;
    }
    _freshness(e, t) {
      let r = e,
        s = typeof r.ttlMs === "number" ? r.ttlMs : this._defaultCacheTtlMs,
        a = r.cacheScope === "public" ? "public" : "private";
      return { expiresAt: this._cache.now() + Math.min(Math.max(0, s), xc), scope: a, params: t };
    }
    async _serveFromCache(e, t, r) {
      if (r?.cacheMode === "bypass" || r?.cacheMode === "refresh") return;
      let s = await this._cache.read(e, t).catch((a) => void this._reportStoreError(a));
      if (s !== void 0) {
        if (r?.signal?.aborted) {
          let a = r.signal.reason;
          throw a instanceof Uo ? a : new Uo(vo.RequestTimeout, String(a));
        }
        return s.value;
      }
    }
    _reportStoreError(e) {
      this.onerror?.(e instanceof Error ? e : Error(String(e)));
    }
    _compileOutputValidator(e) {
      if (!e.outputSchema) return;
      try {
        return { ok: !0, validator: this._jsonSchemaValidator.getValidator(e.outputSchema) };
      } catch (t) {
        return { ok: !1, compileError: t };
      }
    }
    async _resolveXMcpHeaderScan(e, t) {
      let r = t ?? (await this._cache.toolDefinition(e));
      return r === void 0 ? void 0 : Xr(r.inputSchema);
    }
    async readResource(e, t) {
      let r = await this._serveFromCache("resources/read", e.uri, t);
      if (r !== void 0) return r;
      let s = this._cache.captureGeneration("resources/read", e.uri),
        a = await this.request({ method: "resources/read", params: e }, t);
      if (t?.cacheMode !== "bypass") {
        let o = this._freshness(a, e.uri);
        if (o.expiresAt > this._cache.now()) await this._cache.write("resources/read", a, s, o);
        else if (t?.cacheMode === "refresh") await this._cache.evictKey("resources/read", e.uri);
      }
      return a;
    }
    async subscribeResource(e, t) {
      return this.request({ method: "resources/subscribe", params: e }, t);
    }
    async unsubscribeResource(e, t) {
      return this.request({ method: "resources/unsubscribe", params: e }, t);
    }
    async listen(e, t) {
      if (this.transport === void 0) throw new Uo(vo.NotConnected, "Not connected");
      let r = this._negotiatedProtocolVersion;
      if (r === void 0 || !ze(r))
        throw new Uo(
          vo.MethodNotSupportedByProtocolVersion,
          `subscriptions/listen requires a 2026-07-28-era connection (negotiated: ${r ?? "none"}). On a 2025-era connection, change notifications are delivered unsolicited: use ClientOptions.listChanged and resources/subscribe instead.`,
          { method: "subscriptions/listen", protocolVersion: r },
        );
      if (t?.signal?.aborted) {
        let p = t.signal.reason;
        throw p instanceof Uo ? p : new Uo(vo.RequestTimeout, String(p));
      }
      let s = new AbortController(),
        a = `listen:${this._nextListenId++}`,
        o = "opening",
        n,
        c,
        i,
        l,
        d = new Promise((p, m) => {
          (i = p), (l = m);
        });
      d.catch(() => {});
      let S,
        R = new Promise((p) => {
          S = p;
        }),
        w = (p) => {
          if (o === "closed") return;
          let m = o === "opening";
          if (n !== void 0) clearTimeout(n), (n = void 0);
          if ("ack" in p) {
            (o = "open"), i(p.ack);
            return;
          }
          if (((o = "closed"), c !== void 0)) t?.signal?.removeEventListener("abort", c);
          if ((this._listenState.delete(a), s.abort(), S(p.cause), m))
            l(p.error ?? new Uo(vo.ConnectionClosed, "subscriptions/listen closed before the server acknowledged"));
        },
        g = async () => {
          s.abort(),
            await this.notification({ method: "notifications/cancelled", params: { requestId: a } }).catch(() => {});
        },
        _ = async () => {
          if (o === "closed") return;
          w({ cause: "local" }), await g();
        };
      this._listenState.set(a, { settle: w });
      let h = t?.timeout ?? yr;
      if (
        ((n = setTimeout(() => {
          w({
            cause: "remote",
            error: new Uo(vo.RequestTimeout, "subscriptions/listen ack timed out", { timeout: h }),
          }),
            g().catch(() => {});
        }, h)),
        t?.signal)
      ) {
        let p = t.signal;
        (c = () => {
          if (o === "closed") return;
          let m = p.reason;
          w({ cause: "local", error: m instanceof Error ? m : Error(String(m ?? "Aborted")) }), g().catch(() => {});
        }),
          p.addEventListener("abort", c, { once: !0 });
      }
      let u = {
        jsonrpc: "2.0",
        id: a,
        method: "subscriptions/listen",
        params: { _meta: { ...this._outboundMetaEnvelope() }, notifications: e },
      };
      try {
        await this.transport.send(u, {
          requestSignal: s.signal,
          onRequestStreamEnd: () => w({ cause: "remote", error: Error("subscriptions/listen: stream ended") }),
        });
      } catch (p) {
        w({ cause: "remote", error: p instanceof Error ? p : Error(String(p)) });
      }
      return { honoredFilter: await d, close: _, closed: R };
    }
    get autoOpenedSubscription() {
      return this._autoOpenedSubscription;
    }
    _onnotification(e, t) {
      let r = Object.hasOwn(eo, e.method) ? eo[e.method] : void 0;
      if (e.method === "notifications/resources/updated") {
        let s = e.params?.uri;
        if (typeof s === "string") this._cache.evictKey("resources/read", s);
      } else if (r !== void 0) for (let s of r) this._cache.evict(s);
      if (e.method === "notifications/subscriptions/acknowledged") {
        let s = e.params?._meta?.[wat],
          a = typeof s === "string" ? this._listenState.get(s) : void 0;
        if (a !== void 0) {
          let o = this._wireCodec().validateNotification("notifications/subscriptions/acknowledged", e);
          a.settle({ ack: o.ok ? o.value.params.notifications : {} });
          return;
        }
      }
      if (e.method === "notifications/cancelled") {
        let s = e.params?.requestId,
          a = typeof s === "string" ? this._listenState.get(s) : void 0;
        if (a !== void 0) {
          a.settle({ cause: "remote", error: Error("subscriptions/listen: server cancelled the subscription") });
          return;
        }
      }
      super._onnotification(e, t);
    }
    _onresponse(e) {
      let t = e.id,
        r = typeof t === "string" ? this._listenState.get(t) : void 0;
      if (r !== void 0) {
        if (Ge(e)) r.settle({ cause: "remote", error: wa.fromError(e.error.code, e.error.message, e.error.data) });
        else
          r.settle({
            cause: "graceful",
            error: new Uo(
              vo.ConnectionClosed,
              "subscriptions/listen: server closed the subscription gracefully before acknowledging",
            ),
          });
        return;
      }
      super._onresponse(e);
    }
    _onclose() {
      if (this._listenState.size > 0) {
        let e = new Uo(vo.ConnectionClosed, "Connection closed");
        for (let t of this._listenState.values()) t.settle({ cause: "remote", error: e });
        this._listenState.clear();
      }
      super._onclose();
    }
    async callTool(e, t) {
      let r = this.getProtocolEra() === "modern" && Ga() !== "browser",
        s = async () => {
          if (!r) return t;
          let i;
          try {
            i = await this._resolveXMcpHeaderScan(e.name, t?.toolDefinition);
          } catch (d) {
            this._reportStoreError(d);
          }
          if (!i?.valid || i.declarations.length === 0) return t;
          let l = la(i.declarations, e.arguments);
          return Object.keys(l).length === 0 ? t : { ...t, headers: { ...t?.headers, ...l } };
        },
        a =
          t?.toolDefinition === void 0
            ? await this._cache
                .outputValidator(e.name, (i) => this._compileOutputValidator(i))
                .catch((i) => void this._reportStoreError(i))
            : this._compileOutputValidator(t.toolDefinition),
        o = () => {
          if (a === void 0 || a.ok) return;
          let i = a.compileError,
            l = (i instanceof Error ? i.message : String(i)).slice(0, 200);
          throw new wa(Ai.InvalidParams, `Tool '${e.name}' has an invalid outputSchema: ${l}`);
        };
      o();
      let n;
      try {
        n = await this.request({ method: "tools/call", params: e }, await s());
      } catch (i) {
        let l = i instanceof wa && i.code === nt;
        if (!r || !l || t?.toolDefinition !== void 0) throw i;
        let d = { signal: t?.signal, timeout: t?.timeout, cacheMode: "refresh" };
        await this._cache.evict("tools/list"),
          await this.listTools(void 0, d).catch((S) => this._reportStoreError(S)),
          (a = await this._cache
            .outputValidator(e.name, (S) => this._compileOutputValidator(S))
            .catch((S) => void this._reportStoreError(S))),
          o(),
          (n = await this.request({ method: "tools/call", params: e }, await s()));
      }
      if (t?.allowTask === !0 && n.resultType === "task") return n;
      let c = a !== void 0 && a.ok ? a.validator : void 0;
      if (c) {
        if (n.structuredContent === void 0 && !n.isError)
          throw new wa(Ai.InvalidRequest, `Tool ${e.name} has an output schema but did not return structured content`);
        if (n.structuredContent !== void 0 && !n.isError)
          try {
            let i = c(n.structuredContent);
            if (!i.valid)
              throw new wa(
                Ai.InvalidParams,
                `Structured content does not match the tool's output schema: ${i.errorMessage}`,
              );
          } catch (i) {
            if (i instanceof wa) throw i;
            throw new wa(
              Ai.InvalidParams,
              `Failed to validate structured content: ${i instanceof Error ? i.message : String(i)}`,
            );
          }
      }
      return n;
    }
    async listTools(e, t) {
      if (!this._serverCapabilities?.tools && !this._enforceStrictCapabilities)
        return (
          console.debug(
            "Client.listTools() called but server does not advertise tools capability - returning empty list",
          ),
          { tools: [] }
        );
      if (e?.cursor !== void 0) {
        let s = await this.request({ method: "tools/list", params: e }, t);
        return this._excludeInvalidXMcpHeaderTools(s), s;
      }
      let r = await this._serveFromCache("tools/list", void 0, t);
      if (r !== void 0) return r;
      return this._listAllPages(
        "tools/list",
        e,
        t,
        (s, a) => s.tools.push(...a.tools),
        (s) => this._excludeInvalidXMcpHeaderTools(s),
      );
    }
    _excludeInvalidXMcpHeaderTools(e) {
      if (this.getProtocolEra() !== "modern" || !this.transport || Ba(this.transport) === "stdio") return;
      let t = e.tools.filter((r) => {
        let s = Xr(r.inputSchema);
        if (!s.valid)
          return (
            console.warn(
              `[mcp-sdk] excluding tool '${r.name}' from tools/list: invalid x-mcp-header declaration \u2014 ${s.reason}`,
            ),
            !1
          );
        return !0;
      });
      if (t.length !== e.tools.length) e.tools = t;
    }
    _setupListChangedHandler(e, t, r, s) {
      let a = vr(Fat, r);
      if (!a.success) throw Error(`Invalid ${e} listChanged options: ${a.error.message}`);
      if (typeof r.onChanged !== "function")
        throw TypeError(`Invalid ${e} listChanged options: onChanged must be a function`);
      let { autoRefresh: o, debounceMs: n } = a.data,
        { onChanged: c } = r,
        i = async () => {
          if (!o) {
            c(null, null);
            return;
          }
          try {
            c(null, await s());
          } catch (d) {
            c(d instanceof Error ? d : Error(String(d)), null);
          }
        },
        l = () => {
          if (n) {
            let d = this._listChangedDebounceTimers.get(e);
            if (d) clearTimeout(d);
            let S = setTimeout(i, n);
            this._listChangedDebounceTimers.set(e, S);
          } else i();
        };
      this.setNotificationHandler(t, l);
    }
    async sendRootsListChanged() {
      return this.notification({ method: "notifications/roots/list_changed" });
    }
  };
var fl = class extends Error {
    static {
      Object.defineProperty(this, "mcpBrand", { value: "mcp.SseError" });
    }
    static [Symbol.hasInstance](e) {
      return _e(this, e);
    }
    static isInstance(e) {
      if (typeof this !== "function")
        throw TypeError(
          "isInstance must be called on the class (e.g. `SdkError.isInstance(value)`); for callbacks use `v => SdkError.isInstance(v)`",
        );
      return _e(this, e);
    }
    constructor(e, t, r) {
      super(`SSE error: ${t}`);
      (this.code = e), (this.event = r), Je(this, new.target);
    }
  },
  Vrn = class {
    _eventSource;
    _endpoint;
    _abortController;
    _url;
    _resourceMetadataUrl;
    _scope;
    _eventSourceInit;
    _requestInit;
    _authProvider;
    _oauthProvider;
    _skipIssuerMetadataValidation;
    _fetch;
    _fetchWithInit;
    _protocolVersion;
    onclose;
    onerror;
    onmessage;
    constructor(e, t) {
      if (
        ((this._url = e),
        (this._resourceMetadataUrl = void 0),
        (this._scope = void 0),
        (this._eventSourceInit = t?.eventSourceInit),
        (this._requestInit = t?.requestInit),
        (this._skipIssuerMetadataValidation = t?.skipIssuerMetadataValidation),
        ao(t?.authProvider))
      )
        (this._oauthProvider = t.authProvider),
          (this._authProvider = oo(t.authProvider, { skipIssuerMetadataValidation: t.skipIssuerMetadataValidation }));
      else this._authProvider = t?.authProvider;
      (this._fetch = t?.fetch), (this._fetchWithInit = $We(t?.fetch, t?.requestInit));
    }
    _last401Response;
    async _commonHeaders() {
      let e = {},
        t = await this._authProvider?.token();
      if (t) e.Authorization = `Bearer ${t}`;
      if (this._protocolVersion) e["mcp-protocol-version"] = this._protocolVersion;
      let r = Dt(this._requestInit?.headers);
      return new Headers({ ...e, ...r });
    }
    _startOrAuth() {
      let e = this?._eventSourceInit?.fetch ?? this._fetch ?? fetch;
      return new Promise((t, r) => {
        (this._eventSource = new Uae(this._url.href, {
          ...this._eventSourceInit,
          fetch: async (s, a) => {
            let o = await this._commonHeaders();
            o.set("Accept", "text/event-stream");
            let n = await e(s, { ...a, headers: o });
            if (n.status === 401) {
              if (((this._last401Response = n), n.headers.has("www-authenticate"))) {
                let { resourceMetadataUrl: c, scope: i } = Be(n);
                (this._resourceMetadataUrl = c), (this._scope = i);
              }
            }
            return n;
          },
        })),
          (this._abortController = new AbortController()),
          (this._eventSource.onerror = (s) => {
            if (s.code === 401 && this._authProvider) {
              if (this._authProvider.onUnauthorized && this._last401Response) {
                let n = this._last401Response;
                (this._last401Response = void 0),
                  this._eventSource?.close(),
                  this._authProvider
                    .onUnauthorized({ response: n, serverUrl: this._url, fetchFn: this._fetchWithInit })
                    .then(
                      () => this._startOrAuth().then(t, r),
                      (c) => {
                        this.onerror?.(c), r(c);
                      },
                    );
                return;
              }
              let o = new xA();
              r(o), this.onerror?.(o);
              return;
            }
            let a = new fl(s.code, s.message, s);
            r(a), this.onerror?.(a);
          }),
          (this._eventSource.onopen = () => {}),
          this._eventSource.addEventListener("endpoint", (s) => {
            let a = s;
            try {
              if (((this._endpoint = new URL(a.data, this._url)), this._endpoint.origin !== this._url.origin))
                throw Error(`Endpoint origin does not match connection origin: ${this._endpoint.origin}`);
            } catch (o) {
              r(o), this.onerror?.(o), this.close();
              return;
            }
            t();
          }),
          (this._eventSource.onmessage = (s) => {
            let a = s,
              o;
            try {
              o = TB.parse(JSON.parse(a.data));
            } catch (n) {
              this.onerror?.(n);
              return;
            }
            this.onmessage?.(o);
          });
      });
    }
    async start() {
      if (this._eventSource)
        throw Error(
          "SSEClientTransport already started! If using Client class, note that connect() calls start() automatically.",
        );
      return await this._startOrAuth();
    }
    async finishAuth(e, t) {
      if (!this._oauthProvider) throw new xA("finishAuth requires an OAuthClientProvider");
      let { authorizationCode: r, iss: s } = await no(e, t, this._oauthProvider, this._url, {
        fetchFn: this._fetchWithInit,
        resourceMetadataUrl: this._resourceMetadataUrl,
      });
      if (
        (await BHe(this._oauthProvider, {
          serverUrl: this._url,
          authorizationCode: r,
          iss: s,
          resourceMetadataUrl: this._resourceMetadataUrl,
          scope: this._scope,
          fetchFn: this._fetchWithInit,
          skipIssuerMetadataValidation: this._skipIssuerMetadataValidation,
        })) !== "AUTHORIZED"
      )
        throw new xA("Failed to authorize");
    }
    async close() {
      this._abortController?.abort(), this._eventSource?.close(), this.onclose?.();
    }
    async send(e) {
      return this._send(e, !1);
    }
    async _send(e, t) {
      if (!this._endpoint) throw new Uo(vo.NotConnected, "Not connected");
      try {
        let r = await this._commonHeaders();
        r.set("content-type", "application/json");
        let s = {
            ...this._requestInit,
            method: "POST",
            headers: r,
            body: JSON.stringify(e),
            signal: this._abortController?.signal,
          },
          a = await (this._fetch ?? fetch)(this._endpoint, s);
        if (!a.ok) {
          if (a.status === 401 && this._authProvider) {
            if (a.headers.has("www-authenticate")) {
              let { resourceMetadataUrl: n, scope: c } = Be(a);
              (this._resourceMetadataUrl = n), (this._scope = c);
            }
            if (this._authProvider.onUnauthorized && !t)
              return (
                await this._authProvider.onUnauthorized({
                  response: a,
                  serverUrl: this._url,
                  fetchFn: this._fetchWithInit,
                }),
                await a.text?.().catch(() => {}),
                this._send(e, !0)
              );
            if ((await a.text?.().catch(() => {}), t))
              throw new RS(vo.ClientHttpAuthentication, "Server returned 401 after re-authentication", {
                status: 401,
                statusText: a.statusText,
              });
            throw new xA();
          }
          let o = await a.text?.().catch(() => null);
          throw Error(`Error POSTing to endpoint (HTTP ${a.status}): ${o}`);
        }
        await a.text?.().catch(() => {});
      } catch (r) {
        throw (this.onerror?.(r), r);
      }
    }
    setProtocolVersion(e) {
      this._protocolVersion = e;
    }
  },
  pl = 1,
  Sl = { initialReconnectionDelay: 1000, maxReconnectionDelay: 30000, reconnectionDelayGrowFactor: 1.5, maxRetries: 2 },
  gl = new Set(["authorization", "content-type", "mcp-protocol-version", "mcp-method", "mcp-name", "mcp-session-id"]);
function to(e, t) {
  if (typeof AbortSignal.any === "function") return AbortSignal.any([e, t]);
  let r = new AbortController();
  if (e.aborted) return r.abort(e.reason), r.signal;
  if (t.aborted) return r.abort(t.reason), r.signal;
  let s = () => {
    e.removeEventListener("abort", a), t.removeEventListener("abort", o);
  };
  function a() {
    s(), r.abort(e.reason);
  }
  function o() {
    s(), r.abort(t.reason);
  }
  return e.addEventListener("abort", a, { once: !0 }), t.addEventListener("abort", o, { once: !0 }), r.signal;
}
var Krn = class {
  _abortController;
  _url;
  _resourceMetadataUrl;
  _scope;
  _requestInit;
  _authProvider;
  _oauthProvider;
  _skipIssuerMetadataValidation;
  _fetch;
  _fetchWithInit;
  _sessionId;
  _reconnectionOptions;
  _protocolVersion;
  _onInsufficientScope;
  _maxStepUpRetries;
  _serverRetryMs;
  _reconnectionScheduler;
  _cancelReconnection;
  onclose;
  onerror;
  onmessage;
  hasPerRequestStream = !0;
  constructor(e, t) {
    if (
      ((this._url = e),
      (this._resourceMetadataUrl = void 0),
      (this._scope = void 0),
      (this._requestInit = t?.requestInit),
      (this._skipIssuerMetadataValidation = t?.skipIssuerMetadataValidation),
      ao(t?.authProvider))
    )
      (this._oauthProvider = t.authProvider),
        (this._authProvider = oo(t.authProvider, { skipIssuerMetadataValidation: t.skipIssuerMetadataValidation }));
    else this._authProvider = t?.authProvider;
    (this._fetch = t?.fetch),
      (this._fetchWithInit = $We(t?.fetch, t?.requestInit)),
      (this._sessionId = t?.sessionId),
      (this._protocolVersion = t?.protocolVersion),
      (this._reconnectionOptions = t?.reconnectionOptions ?? Sl),
      (this._reconnectionScheduler = t?.reconnectionScheduler),
      (this._onInsufficientScope = t?.onInsufficientScope ?? "reauthorize"),
      (this._maxStepUpRetries = Math.max(0, t?.maxStepUpRetries ?? pl));
  }
  async _stepUpAuthorize(e, t) {
    if (this._onInsufficientScope === "throw")
      throw new Fa({
        requiredScope: e.scope,
        resourceMetadataUrl: e.resourceMetadataUrl,
        errorDescription: e.errorDescription,
      });
    if (!this._oauthProvider)
      throw new Fa({
        requiredScope: e.scope,
        resourceMetadataUrl: e.resourceMetadataUrl,
        errorDescription: e.errorDescription,
      });
    if (t >= this._maxStepUpRetries)
      throw new RS(
        vo.ClientHttpForbidden,
        `Server returned 403 insufficient_scope after step-up re-authorization (retry limit ${this._maxStepUpRetries} reached)`,
        { status: 403, statusText: e.statusText ?? "Forbidden", text: e.text },
      );
    if (e.resourceMetadataUrl) this._resourceMetadataUrl = e.resourceMetadataUrl;
    let r = await this._oauthProvider.tokens(),
      s = as(this._scope, r?.scope, e.scope);
    this._scope = s;
    let a = $c(s, r?.scope);
    return BHe(this._oauthProvider, {
      serverUrl: this._url,
      resourceMetadataUrl: this._resourceMetadataUrl,
      scope: s,
      forceReauthorization: a,
      fetchFn: this._fetchWithInit,
      skipIssuerMetadataValidation: this._skipIssuerMetadataValidation,
    });
  }
  async _commonHeaders() {
    let e = {},
      t = await this._authProvider?.token();
    if (t) e.Authorization = `Bearer ${t}`;
    if (this._sessionId) e["mcp-session-id"] = this._sessionId;
    if (this._protocolVersion) e["mcp-protocol-version"] = this._protocolVersion;
    let r = Dt(this._requestInit?.headers);
    return new Headers({ ...e, ...r });
  }
  _applyBodyDerivedHeaders(e, t) {
    if (Array.isArray(t) || !BZ(t)) return;
    let r = t.params?._meta?.[Dae];
    if (typeof r !== "string") return;
    e.set("mcp-protocol-version", r), e.set("mcp-method", t.method);
    let s = t.params,
      a =
        t.method === "resources/read"
          ? typeof s?.uri === "string"
            ? s.uri
            : void 0
          : typeof s?.name === "string"
            ? s.name
            : void 0;
    if (a !== void 0) e.set("mcp-name", _r(a));
    let o = t.method.startsWith("tasks/") && typeof s?.taskId === "string" ? s.taskId : void 0;
    if (o !== void 0) e.set("mcp-name", _r(o));
  }
  _isModernEnvelopedRequest(e) {
    if (Array.isArray(e) || !BZ(e)) return !1;
    let t = e.params?._meta?.[Dae];
    return typeof t === "string" && ze(t);
  }
  async _startOrAuthSse(e, t = !1, r = 0) {
    let { resumptionToken: s, requestSignal: a } = e,
      o = () => this._abortController?.signal.aborted === !0 || a?.aborted === !0;
    try {
      let n = await this._commonHeaders(),
        c = [
          ...(n
            .get("accept")
            ?.split(",")
            .map((S) => S.trim().toLowerCase()) ?? []),
          "text/event-stream",
        ];
      if ((n.set("accept", [...new Set(c)].join(", ")), s)) n.set("last-event-id", s);
      let i = this._abortController?.signal,
        l = a !== void 0 && i !== void 0 ? to(i, a) : (a ?? i),
        d = await (this._fetch ?? fetch)(this._url, { ...this._requestInit, method: "GET", headers: n, signal: l });
      if (!d.ok) {
        if (d.status === 401 && this._authProvider) {
          if (d.headers.has("www-authenticate")) {
            let { resourceMetadataUrl: S, scope: R } = Be(d);
            (this._resourceMetadataUrl = S), (this._scope = as(this._scope, R));
          }
          if (this._authProvider.onUnauthorized && !t)
            return (
              await this._authProvider.onUnauthorized({
                response: d,
                serverUrl: this._url,
                fetchFn: this._fetchWithInit,
              }),
              await d.text?.().catch(() => {}),
              this._startOrAuthSse(e, !0, r)
            );
          if ((await d.text?.().catch(() => {}), t))
            throw new RS(vo.ClientHttpAuthentication, "Server returned 401 after re-authentication", {
              status: 401,
              statusText: d.statusText,
            });
          throw new xA();
        }
        if (d.status === 403) {
          let { resourceMetadataUrl: S, scope: R, error: w, errorDescription: g } = Be(d);
          if (w === "insufficient_scope") {
            let _ = await d.text?.().catch(() => null);
            if (
              (await this._stepUpAuthorize(
                { scope: R, resourceMetadataUrl: S, errorDescription: g, statusText: d.statusText, text: _ },
                r,
              )) !== "AUTHORIZED"
            )
              throw new xA();
            return this._startOrAuthSse(e, t, r + 1);
          }
        }
        if ((await d.text?.().catch(() => {}), d.status === 405)) {
          e.onRequestStreamEnd?.();
          return;
        }
        throw new RS(vo.ClientHttpFailedToOpenStream, `Failed to open SSE stream: ${d.statusText}`, {
          status: d.status,
          statusText: d.statusText,
        });
      }
      this._handleSseStream(d.body, e, !0);
    } catch (n) {
      if (!o()) this.onerror?.(n);
      throw n;
    }
  }
  _getNextReconnectionDelay(e) {
    if (this._serverRetryMs !== void 0) return this._serverRetryMs;
    let t = this._reconnectionOptions.initialReconnectionDelay,
      r = this._reconnectionOptions.reconnectionDelayGrowFactor,
      s = this._reconnectionOptions.maxReconnectionDelay;
    return Math.min(t * Math.pow(r, e), s);
  }
  _scheduleReconnection(e, t = 0) {
    let r = this._reconnectionOptions.maxRetries;
    if (t >= r) {
      this.onerror?.(Error(`Maximum reconnection attempts (${r}) exceeded.`)), e.onRequestStreamEnd?.();
      return;
    }
    let s = this._getNextReconnectionDelay(t),
      a = () => {
        if (((this._cancelReconnection = void 0), this._abortController?.signal.aborted || e.requestSignal?.aborted))
          return;
        this._startOrAuthSse(e).catch((o) => {
          if (this._abortController?.signal.aborted || e.requestSignal?.aborted) return;
          this.onerror?.(Error(`Failed to reconnect SSE stream: ${o instanceof Error ? o.message : String(o)}`));
          try {
            this._scheduleReconnection(e, t + 1);
          } catch (n) {
            this.onerror?.(n instanceof Error ? n : Error(String(n)));
          }
        });
      };
    if (this._reconnectionScheduler) {
      let o = this._reconnectionScheduler(a, s, t);
      this._cancelReconnection = typeof o === "function" ? o : void 0;
    } else {
      let o = setTimeout(a, s);
      this._cancelReconnection = () => clearTimeout(o);
    }
  }
  _handleSseStream(e, t, r) {
    if (!e) {
      t.onRequestStreamEnd?.();
      return;
    }
    let { onresumptiontoken: s, replayMessageId: a, requestSignal: o, onRequestStreamEnd: n } = t,
      c = () => this._abortController?.signal.aborted === !0 || o?.aborted === !0,
      i,
      l = !1,
      d = !1;
    (async () => {
      try {
        let R = e
          .pipeThrough(new TextDecoderStream())
          .pipeThrough(
            new F9e({
              onRetry: (w) => {
                this._serverRetryMs = w;
              },
            }),
          )
          .getReader();
        while (!0) {
          let { value: w, done: g } = await R.read();
          if (g) break;
          if (w.id) (i = w.id), (l = !0), s?.(w.id);
          if (!w.data) continue;
          if (!w.event || w.event === "message")
            try {
              let _ = TB.parse(JSON.parse(w.data));
              if (Ke(_) || Ge(_)) {
                if (((d = !0), a !== void 0)) _.id = a;
              }
              this.onmessage?.(_);
            } catch (_) {
              this.onerror?.(_);
            }
        }
        if ((r || l) && !d && this._abortController && !c())
          this._scheduleReconnection(
            { resumptionToken: i, onresumptiontoken: s, replayMessageId: a, requestSignal: o, onRequestStreamEnd: n },
            0,
          );
        else if (!c()) n?.();
      } catch (R) {
        if (c()) return;
        if ((this.onerror?.(Error(`SSE stream disconnected: ${R}`)), (r || l) && !d && this._abortController && !c()))
          try {
            this._scheduleReconnection(
              { resumptionToken: i, onresumptiontoken: s, replayMessageId: a, requestSignal: o, onRequestStreamEnd: n },
              0,
            );
          } catch (w) {
            this.onerror?.(Error(`Failed to reconnect: ${w instanceof Error ? w.message : String(w)}`)), n?.();
          }
        else n?.();
      }
    })();
  }
  async start() {
    if (this._abortController)
      throw Error(
        "StreamableHTTPClientTransport already started! If using Client class, note that connect() calls start() automatically.",
      );
    this._abortController = new AbortController();
  }
  async finishAuth(e, t) {
    if (!this._oauthProvider) throw new xA("finishAuth requires an OAuthClientProvider");
    let { authorizationCode: r, iss: s } = await no(e, t, this._oauthProvider, this._url, {
      fetchFn: this._fetchWithInit,
      resourceMetadataUrl: this._resourceMetadataUrl,
    });
    if (
      (await BHe(this._oauthProvider, {
        serverUrl: this._url,
        authorizationCode: r,
        iss: s,
        resourceMetadataUrl: this._resourceMetadataUrl,
        scope: this._scope,
        fetchFn: this._fetchWithInit,
        skipIssuerMetadataValidation: this._skipIssuerMetadataValidation,
      })) !== "AUTHORIZED"
    )
      throw new xA("Failed to authorize");
  }
  async close() {
    try {
      this._cancelReconnection?.();
    } finally {
      (this._cancelReconnection = void 0), this._abortController?.abort(), this.onclose?.();
    }
  }
  async send(e, t) {
    return this._send(e, t, !1);
  }
  async _send(e, t, r, s = 0) {
    try {
      let { resumptionToken: a, onresumptiontoken: o } = t || {};
      if (a) {
        this._startOrAuthSse({
          resumptionToken: a,
          replayMessageId: BZ(e) ? e.id : void 0,
          requestSignal: t?.requestSignal,
        }).catch((h) => this.onerror?.(h));
        return;
      }
      let n = await this._commonHeaders();
      this._applyBodyDerivedHeaders(n, e);
      let c = Array.isArray(e) ? e.some((h) => Yr(h)) : Yr(e);
      if (c) n.delete("mcp-session-id");
      if (t?.headers !== void 0)
        for (let [h, u] of Object.entries(t.headers)) {
          if (gl.has(h.toLowerCase())) continue;
          n.set(h, u);
        }
      n.set("content-type", "application/json");
      let i = [
        ...(n
          .get("accept")
          ?.split(",")
          .map((h) => h.trim().toLowerCase()) ?? []),
        "application/json",
        "text/event-stream",
      ];
      n.set("accept", [...new Set(i)].join(", "));
      let l = this._abortController?.signal,
        d = t?.requestSignal !== void 0 && l !== void 0 ? to(l, t.requestSignal) : (t?.requestSignal ?? l),
        S = { ...this._requestInit, method: "POST", headers: n, body: JSON.stringify(e), signal: d },
        R = await (this._fetch ?? fetch)(this._url, S);
      if (c && R.ok) this._sessionId = R.headers.get("mcp-session-id") || void 0;
      if (!R.ok) {
        if (R.status === 401 && this._authProvider) {
          if (R.headers.has("www-authenticate")) {
            let { resourceMetadataUrl: u, scope: p } = Be(R);
            (this._resourceMetadataUrl = u), (this._scope = as(this._scope, p));
          }
          if (this._authProvider.onUnauthorized && !r)
            return (
              await this._authProvider.onUnauthorized({
                response: R,
                serverUrl: this._url,
                fetchFn: this._fetchWithInit,
              }),
              await R.text?.().catch(() => {}),
              this._send(e, t, !0, s)
            );
          if ((await R.text?.().catch(() => {}), r))
            throw new RS(vo.ClientHttpAuthentication, "Server returned 401 after re-authentication", {
              status: 401,
              statusText: R.statusText,
            });
          throw new xA();
        }
        let h = await R.text?.().catch(() => null);
        if (R.status === 403) {
          let { resourceMetadataUrl: u, scope: p, error: m, errorDescription: y } = Be(R);
          if (m === "insufficient_scope") {
            if (
              (await this._stepUpAuthorize(
                { scope: p, resourceMetadataUrl: u, errorDescription: y, statusText: R.statusText, text: h },
                s,
              )) !== "AUTHORIZED"
            )
              throw new xA();
            return this._send(e, t, r, s + 1);
          }
        }
        if (R.status === 400 && typeof h === "string" && this._isModernEnvelopedRequest(e))
          try {
            let u = TB.parse(JSON.parse(h)),
              p = (Array.isArray(e) ? e : [e]).filter((m) => BZ(m));
            if (Ge(u) && p.some((m) => m.id === u.id)) {
              this.onmessage?.(u);
              return;
            }
          } catch {}
        throw new RS(vo.ClientHttpNotImplemented, `Error POSTing to endpoint: ${h}`, {
          status: R.status,
          statusText: R.statusText,
          text: h,
        });
      }
      if (R.status === 202) {
        if ((await R.text?.().catch(() => {}), na(e)))
          this._startOrAuthSse({ resumptionToken: void 0 }).catch((h) => this.onerror?.(h));
        return;
      }
      let w = (Array.isArray(e) ? e : [e]).some((h) => "method" in h && "id" in h && h.id !== void 0),
        g = R.headers.get("content-type"),
        _ = va(g);
      if (w)
        if (_ === "text/event-stream")
          this._handleSseStream(
            R.body,
            { onresumptiontoken: o, requestSignal: t?.requestSignal, onRequestStreamEnd: t?.onRequestStreamEnd },
            !1,
          );
        else if (_ === "application/json") {
          let h = await R.json(),
            u = Array.isArray(h) ? h.map((p) => TB.parse(p)) : [TB.parse(h)];
          for (let p of u) this.onmessage?.(p);
        } else
          throw (
            (await R.text?.().catch(() => {}),
            new Uo(vo.ClientHttpUnexpectedContent, `Unexpected content type: ${g}`, { contentType: g }))
          );
      else await R.text?.().catch(() => {});
    } catch (a) {
      if (t?.requestSignal?.aborted !== !0) this.onerror?.(a);
      throw a;
    }
  }
  get sessionId() {
    return this._sessionId;
  }
  async terminateSession() {
    if (!this._sessionId) return;
    try {
      let e = await this._commonHeaders(),
        t = { ...this._requestInit, method: "DELETE", headers: e, signal: this._abortController?.signal },
        r = await (this._fetch ?? fetch)(this._url, t);
      if ((await r.text?.().catch(() => {}), !r.ok && r.status !== 405))
        throw new RS(vo.ClientHttpFailedToTerminateSession, `Failed to terminate session: ${r.statusText}`, {
          status: r.status,
          statusText: r.statusText,
        });
      this._sessionId = void 0;
    } catch (e) {
      throw (this.onerror?.(e), e);
    }
  }
  setProtocolVersion(e) {
    this._protocolVersion = e;
  }
  get protocolVersion() {
    return this._protocolVersion;
  }
  async resumeStream(e, t) {
    await this._startOrAuthSse({ resumptionToken: e, onresumptiontoken: t?.onresumptiontoken });
  }
};
export {
  wB,
  Hx,
  vo,
  Uo,
  RS,
  Ai,
  wa,
  BZ,
  iOt,
  jrn,
  sOt,
  Wrn,
  $We,
  UWe,
  S_e,
  aOt,
  xA,
  BHe,
  lOt,
  jHe,
  cOt,
  qrn,
  M6n,
  Grn,
  zrn,
  Vrn,
  Krn,
};
