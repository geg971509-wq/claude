...
import { B, F } from "/$bunfs/root/chunk-twm95mhz.js";
...
function R(s) {
  return s.kind === "plugin" ? `plugin:${s.name}@${s.marketplace}` : `server:${s.name}`;
}
function k(j) {
  let a = _(13),
    { channels: f, onAccept: h } = j,
    v;
  if (a[0] !== h)
    (v = function l(G) {
      bb2: switch (G) {
  ...
  if (a[2] === d)
    (x = e(t, {
      ...
      (y = e(t, { children: "Please use --channels to run a list of approved channels." })),
      (a[2] = x),
      (a[3] = y);
  else (x = a[2]), (y = a[3]);
  let i;
  if (a[4] !== f) (i = f.map(R).join(", ")), (a[4] = f), (a[5] = i);
  else i = a[5];
  let c;
  if (a[6] !== i)
    (c = r(o, {
  ...
  let b;
  if (a[10] !== c || a[11] !== p)
    (b = r(me, { title: "WARNING: Loading development channels", color: "error", onCancel: T, children: [c, p] })),
      (a[10] = c),
      (a[11] = p),
      (a[12] = b);
  else b = a[12];
