// @bun @bytecode
// Claude Code is a Beta product per Anthropic's Commercial Terms of Service.
// By using Claude Code, you agree that all code acceptance or rejection decisions you make,
// and the associated conversations in context, constitute Feedback under Anthropic's Commercial Terms,
// and may be used to improve Anthropic's products, including training models.
// You are responsible for reviewing any code suggestions before use.

// (c) Anthropic PBC. All rights reserved. Use is subject to the Legal Agreements outlined here: https://code.claude.com/docs/en/legal-and-compliance.

// Version: 2.1.252
import {
  Qoe,
  lQe,
  NFe,
  wg,
  FFe,
  nx,
  Bfe,
  Zoe,
  jfe,
  uQe,
  eie,
  dQe,
  $Fe,
  N5,
  x4,
  _Ct,
  Wfe,
  pQe,
  F5,
  DVt,
  nie,
  P4,
  Tj,
  qfe,
  OVt,
  fQe,
  ome,
} from "/$bunfs/root/chunk-f9h0bg01.js";
import { Wf, k, St } from "/$bunfs/root/chunk-4ddxwr9r.js";
import { be } from "/$bunfs/root/chunk-gcks6mn0.js";
import { m } from "/$bunfs/root/chunk-bzx56g36.js";
import { a } from "/$bunfs/root/chunk-fec4384a.js";
import { E, X } from "/$bunfs/root/chunk-ypdw393e.js";
import { b, V, Wp, Qo, le, n } from "/$bunfs/root/chunk-fv016jr6.js";
import {
  Ipe,
  Aqt,
  vXe,
  Cqt,
  aoe,
  RXe,
  Ppe,
  OEn,
  Ha,
  J6,
  MEn,
  zor,
  Vor,
  Kor,
  xXe,
  IXe,
  ZAe,
  Npe,
  eCe,
  FEn,
  $En,
  UEn,
  BEn,
  Pqt,
  Dqt,
  Q6,
  tCe,
  bw,
  YEn,
  BXe,
  J3,
} from "/$bunfs/root/chunk-6k63g5t6.js";
import { Ut } from "/$bunfs/root/chunk-ntyhd04p.js";
import { _y, Eir, kAn, pCe, tGt } from "/$bunfs/root/chunk-z2bvp3sv.js";
import { Y } from "/$bunfs/root/chunk-s7e0px8j.js";
import { c0 } from "/$bunfs/root/chunk-18bck2pp.js";
import { eU } from "/$bunfs/root/chunk-saay52v7.js";
import { SR } from "/$bunfs/root/chunk-052zvbpg.js";
import { D } from "/$bunfs/root/chunk-7s7jqj2f.js";
import { Q5, YT, lx, Ij, Rm, l$e, tr, bS } from "/$bunfs/root/chunk-56sxk8k2.js";
import { He } from "/$bunfs/root/chunk-79g5tayq.js";
import { Q, te } from "/$bunfs/root/chunk-wag5ye9w.js";
import { je } from "/$bunfs/root/chunk-yz031c9r.js";
function Ht(e, t, r) {
  if ((r !== void 0 && !Q5(e[t], r)) || (r === void 0 && !(t in e))) F5(e, t, r);
}
var N = Ht;
var K = {};
je(K, { default: () => L });
var ye = typeof K == "object" && K && !K.nodeType && K,
  ge = ye && typeof H == "object" && H && !H.nodeType && H,
  Kt = ge && ge.exports === ye,
  pe = Kt ? YT.Buffer : void 0,
  me = pe ? pe.allocUnsafe : void 0;
function Wt(e, t) {
  if (t) return e.slice();
  var r = e.length,
    o = me ? me(r) : new e.constructor(r);
  return e.copy(o), o;
}
var L = Wt;
function Gt(e) {
  var t = new e.constructor(e.byteLength);
  return new lQe(t).set(new lQe(e)), t;
}
var x = Gt;
function zt(e, t) {
  var r = t ? x(e.buffer) : e.buffer;
  return new e.constructor(r, e.byteOffset, e.length);
}
var W = zt;
function Yt(e, t) {
  var r = -1,
    o = e.length;
  t || (t = Array(o));
  while (++r < o) t[r] = e[r];
  return t;
}
var G = Yt;
var Se = Object.create,
  Zt = (function () {
    function e() {}
    return function (t) {
      if (!Rm(t)) return {};
      if (Se) return Se(t);
      e.prototype = t;
      var r = new e();
      return (e.prototype = void 0), r;
    };
  })(),
  he = Zt;
function qt(e) {
  return typeof e.constructor == "function" && !$Fe(e) ? he(vXe(e)) : {};
}
var z = qt;
function Xt(e) {
  return nx(e) && N5(e);
}
var ve = Xt;
var Qt = "[object Object]",
  en = Function.prototype,
  tn = Object.prototype,
  we = en.toString,
  nn = tn.hasOwnProperty,
  rn = we.call(Object);
function on(e) {
  if (!nx(e) || Ij(e) != Qt) return false;
  var t = vXe(e);
  if (t === null) return true;
  var r = nn.call(t, "constructor") && t.constructor;
  return typeof r == "function" && r instanceof r && we.call(r) == rn;
}
var yXe = on;
function sn(e, t) {
  if (t === "constructor" && typeof e[t] === "function") return;
  if (t == "__proto__") return;
  return e[t];
}
var U = sn;
function an(e, t, r, o) {
  var i = !r;
  r || (r = {});
  var s = -1,
    l = t.length;
  while (++s < l) {
    var c = t[s],
      d = o ? o(r[c], e[c], c, r, e) : void 0;
    if (d === void 0) d = e[c];
    if (i) F5(r, c, d);
    else Ipe(r, c, d);
  }
  return r;
}
var M = an;
function ln(e) {
  return M(e, aoe(e));
}
var Ee = ln;
function cn(e, t, r, o, i, s, l) {
  var c = U(e, r),
    d = U(t, r),
    u = l.get(d);
  if (u) {
    N(e, r, u);
    return;
  }
  var f = s ? s(c, d, r + "", e, t, l) : void 0,
    g = f === void 0;
  if (g) {
    var y = wg(d),
      S = !y && Zoe(d),
      p = !y && !S && dQe(d);
    if (((f = d), y || S || p))
      if (wg(c)) f = c;
      else if (ve(c)) f = G(c);
      else if (S) (g = false), (f = L(d, true));
      else if (p) (g = false), (f = W(d, true));
      else f = [];
    else if (yXe(d) || Bfe(d)) {
      if (((f = c), Bfe(c))) f = Ee(c);
      else if (!Rm(c) || l$e(c)) f = z(d);
    } else g = false;
  }
  if (g) l.set(d, f), i(f, d, o, s, l), l.delete(d);
  N(e, r, f);
}
var ke = cn;
function Oe(e, t, r, o, i) {
  if (e === t) return;
  DVt(
    t,
    function (s, l) {
      if ((i || (i = new Qoe()), Rm(s))) ke(e, t, l, r, Oe, o, i);
      else {
        var c = o ? o(U(e, l), s, l + "", e, t, i) : void 0;
        if (c === void 0) c = s;
        N(e, l, c);
      }
    },
    aoe,
  );
}
var Ae = Oe;
function dn(e, t, r) {
  switch (r.length) {
    case 0:
      return e.call(t);
    case 1:
      return e.call(t, r[0]);
    case 2:
      return e.call(t, r[0], r[1]);
    case 3:
      return e.call(t, r[0], r[1], r[2]);
  }
  return e.apply(t, r);
}
var Pe = dn;
var _e = Math.max;
function un(e, t, r) {
  return (
    (t = _e(t === void 0 ? e.length - 1 : t, 0)),
    function () {
      var o = arguments,
        i = -1,
        s = _e(o.length - t, 0),
        l = Array(s);
      while (++i < s) l[i] = o[t + i];
      i = -1;
      var c = Array(t + 1);
      while (++i < t) c[i] = o[i];
      return (c[t] = r(l)), Pe(e, this, c);
    }
  );
}
var Z = un;
function fn(e) {
  return function () {
    return e;
  };
}
var Me = fn;
var gn = !pQe
    ? fQe
    : function (e, t) {
        return pQe(e, "toString", { configurable: true, enumerable: false, value: Me(t), writable: true });
      },
  Re = gn;
var pn = 800,
  mn = 16,
  yn = Date.now;
function Sn(e) {
  var t = 0,
    r = 0;
  return function () {
    var o = yn(),
      i = mn - (o - r);
    if (((r = o), i > 0)) {
      if (++t >= pn) return arguments[0];
    } else t = 0;
    return e.apply(void 0, arguments);
  };
}
var Ce = Sn;
var hn = Ce(Re),
  q = hn;
function vn(e, t) {
  return q(Z(e, t, fQe), e + "");
}
var Te = vn;
function bn(e, t, r) {
  if (!Rm(r)) return false;
  var o = typeof t;
  if (o == "number" ? N5(r) && jfe(t, r.length) : o == "string" && t in r) return Q5(r[t], e);
  return false;
}
var mqt = bn;
function wn(e) {
  return Te(function (t, r) {
    var o = -1,
      i = r.length,
      s = i > 1 ? r[i - 1] : void 0,
      l = i > 2 ? r[2] : void 0;
    if (((s = e.length > 3 && typeof s == "function" ? (i--, s) : void 0), l && mqt(r[0], r[1], l)))
      (s = i < 3 ? void 0 : s), (i = 1);
    t = Object(t);
    while (++o < i) {
      var c = r[o];
      if (c) e(t, c, o, s);
    }
    return t;
  });
}
var Ie = wn;
var En = Ie(function (e, t, r, o) {
    Ae(e, t, r, o);
  }),
  dP = En;
var hN = "claude.ai sync",
  kn = {
    advisor: "config",
    agents: "config",
    "auto-mode-setup": "config",
    autocompact: "config",
    brief: "config",
    channel: "config",
    chrome: "config",
    "cloud-plugins": "config",
    color: "config",
    config: "config",
    effort: "config",
    env: "config",
    experiments: "config",
    "extra-usage": "config",
    fast: "config",
    focus: "config",
    goal: "config",
    hooks: "config",
    ide: "config",
    "install-github-app": "config",
    "install-slack-app": "config",
    issue: "config",
    keybindings: "config",
    mcp: "config",
    memory: "config",
    model: "config",
    "output-style": "config",
    passes: "config",
    "pause-memory": "config",
    permissions: "config",
    plan: "config",
    plugin: "config",
    powerup: "config",
    "privacy-settings": "config",
    "pro-trial-expired": "config",
    "rate-limit-options": "config",
    "remote-control": "config",
    "remote-env": "config",
    sandbox: "config",
    "scroll-speed": "config",
    "setup-bedrock": "config",
    "setup-vertex": "config",
    "terminal-setup": "config",
    theme: "config",
    tui: "config",
    upgrade: "config",
    "usage-credits": "config",
    vim: "config",
    voice: "config",
    "web-setup": "config",
    wellbeing: "config",
    "add-dir": "action",
    "ant-trace": "action",
    artifacts: "action",
    background: "action",
    branch: "action",
    btw: "action",
    bug: "action",
    cd: "action",
    clear: "action",
    compact: "action",
    copy: "action",
    "debug-tool-call": "action",
    desktop: "action",
    exit: "action",
    export: "action",
    feedback: "action",
    heapdump: "action",
    design: "action",
    "design-consent": "action",
    "design-revoke": "action",
    "design-login": "action",
    login: "action",
    logout: "action",
    "low-priority": "action",
    "limit-reset": "action",
    import: "action",
    "mock-limits": "action",
    "oauth-refresh": "action",
    onboarding: "action",
    "perf-issue": "action",
    "plugin-types": "action",
    radio: "action",
    "reload-plugins": "action",
    "reload-skills": "action",
    rename: "action",
    "reset-limits": "action",
    resume: "action",
    "simulate-usage": "action",
    rewind: "action",
    stickers: "action",
    stop: "action",
    teleport: "action",
    thrash: "action",
    update: "action",
    context: "info",
    diff: "info",
    help: "info",
    "input-debug": "info",
    mobile: "info",
    recap: "info",
    "release-notes": "info",
    "render-debug": "info",
    session: "info",
    "skill-doctor": "info",
    skills: "info",
    status: "info",
    usage: "info",
    version: "info",
    "__remote-workflow": "agent",
    "workflow-launch-exec": "agent",
    "autofix-pr": "agent",
    autopilot: "agent",
    bugfix: "agent",
    daemon: "agent",
    dashboard: "agent",
    docs: "agent",
    fork: "agent",
    subtask: "agent",
    investigate: "agent",
    schedule: "agent",
    "list-agents": "agent",
    loops: "agent",
    tasks: "agent",
    ultraplan: "agent",
    ultrareview: "agent",
    workflows: "agent",
  };
function bor(e) {
  if (e.type === "prompt") return "skill";
  return kn[e.name] ?? "action";
}
function wor(e) {
  if (e.type !== "prompt") return "builtin";
  if (e.loadedFrom === "syncedSkills") return "synced";
  switch (e.source) {
    case "builtin":
      return "builtin";
    case "bundled":
      return "bundled";
    case "mcp":
    case "memoryStore":
      return "remote";
    case "plugin":
      return "plugin";
    case "userSettings":
      return "user";
    case "projectSettings":
    case "localSettings":
      return "project";
    case "policySettings":
      return "managed";
    case "flagSettings":
      return "flag";
  }
}
var Is = ["userSettings", "projectSettings", "localSettings", "flagSettings", "policySettings"];
function yw(e) {
  switch (e) {
    case "userSettings":
      return "user";
    case "projectSettings":
      return "project";
    case "localSettings":
      return "project, gitignored";
    case "flagSettings":
      return "cli flag";
    case "policySettings":
      return "managed";
  }
}
function Gwt(e) {
  switch (e) {
    case "userSettings":
      return "User";
    case "projectSettings":
      return "Project";
    case "localSettings":
      return "Local";
    case "flagSettings":
      return "Flag";
    case "policySettings":
      return "Managed";
    case "plugin":
      return "Plugin";
    case "built-in":
      return "Built-in";
    case "mcp":
      return "MCP";
    case "memoryStore":
      return "Memory store";
    case "syncedSkills":
      return hN;
  }
}
function SXe(e) {
  switch (e) {
    case "userSettings":
      return "user settings";
    case "projectSettings":
      return "shared project settings";
    case "localSettings":
      return "project local settings";
    case "flagSettings":
      return "command line arguments";
    case "policySettings":
      return "enterprise managed settings";
    case "cliArg":
      return "CLI argument";
    case "command":
      return "command configuration";
    case "session":
      return "current session";
    case "toolsNarrowing":
      return "CLI tool narrowing";
    case "mcpServerPolicy":
      return "MCP server policy";
  }
}
function Tor(e) {
  return Wf(SXe(e));
}
function Eor(e) {
  if (e === "") return [];
  let t = e.split(",").map((o) => o.trim()),
    r = [];
  for (let o of t)
    switch (o) {
      case "user":
        r.push("userSettings");
        break;
      case "project":
        r.push("projectSettings");
        break;
      case "local":
        r.push("localSettings");
        break;
      default:
        throw Error(`Invalid setting source: ${o}. Valid options are: user, project, local`);
    }
  return r;
}
function xi() {
  let e = ome(),
    t = Ha();
  if (t.enabledSources?.allowed === e) return t.enabledSources.result;
  let r = new Set(e);
  r.add("flagSettings"), r.add("policySettings");
  let o = Is.filter((i) => r.has(i));
  return (t.enabledSources = { allowed: e, result: o }), o;
}
function _o(e) {
  return xi().includes(e);
}
var UH = ["userSettings", "projectSettings", "localSettings"],
  BH = new Set(["projectSettings", "localSettings"]),
  GAe = ["localSettings", "projectSettings", "userSettings"];
import { join as On } from "path";
class xe {
  managedFilePath = void 0;
  dropInDir = void 0;
  getManagedFilePath() {
    return (this.managedFilePath ??= An()), this.managedFilePath;
  }
  getDropInDir() {
    return (this.dropInDir ??= On(ib(), "managed-settings.d")), this.dropInDir;
  }
  clearDropInDir() {
    this.dropInDir = void 0;
  }
  reset() {
    (this.managedFilePath = void 0), (this.dropInDir = void 0);
  }
}
var Ne = new xe();
function ib() {
  return Ne.getManagedFilePath();
}
function An() {
  let e = _En();
  if (e !== void 0) return e;
  switch (D()) {
    case "macos":
      return "/Library/Application Support/ClaudeCode";
    case "windows":
      return "C:\\Program Files\\ClaudeCode";
    default:
      return "/etc/claude-code";
  }
}
function _En() {
  return;
}
function zwt() {
  return Ne.getDropInDir();
}
function Pn(e, t) {
  var r = -1,
    o = e == null ? 0 : e.length;
  while (++r < o) if (t(e[r], r, e) === false) break;
  return e;
}
var Le = Pn;
function _n(e, t) {
  return e && M(t, x4(t), e);
}
var De = _n;
function Mn(e, t) {
  return e && M(t, aoe(t), e);
}
var Ue = Mn;
function Rn(e, t) {
  return M(e, FFe(e), t);
}
var Fe = Rn;
function Cn(e, t) {
  return M(e, Cqt(e), t);
}
var Ve = Cn;
var Tn = Object.prototype,
  In = Tn.hasOwnProperty;
function xn(e) {
  var t = e.length,
    r = new e.constructor(t);
  if (t && typeof e[0] == "string" && In.call(e, "index")) (r.index = e.index), (r.input = e.input);
  return r;
}
var $e = xn;
function jn(e, t) {
  var r = t ? x(e.buffer) : e.buffer;
  return new e.constructor(r, e.byteOffset, e.byteLength);
}
var Je = jn;
var Nn = /\w*$/;
function Ln(e) {
  var t = new e.constructor(e.source, Nn.exec(e));
  return (t.lastIndex = e.lastIndex), t;
}
var Be = Ln;
var Ke = lx ? lx.prototype : void 0,
  We = Ke ? Ke.valueOf : void 0;
function Dn(e) {
  return We ? Object(We.call(e)) : {};
}
var Ge = Dn;
var Un = "[object Boolean]",
  Fn = "[object Date]",
  Vn = "[object Map]",
  $n = "[object Number]",
  Jn = "[object RegExp]",
  Bn = "[object Set]",
  Hn = "[object String]",
  Kn = "[object Symbol]",
  Wn = "[object ArrayBuffer]",
  Gn = "[object DataView]",
  zn = "[object Float32Array]",
  Yn = "[object Float64Array]",
  Zn = "[object Int8Array]",
  qn = "[object Int16Array]",
  Xn = "[object Int32Array]",
  Qn = "[object Uint8Array]",
  er = "[object Uint8ClampedArray]",
  nr = "[object Uint16Array]",
  rr = "[object Uint32Array]";
function or(e, t, r) {
  var o = e.constructor;
  switch (t) {
    case Wn:
      return x(e);
    case Un:
    case Fn:
      return new o(+e);
    case Gn:
      return Je(e, r);
    case zn:
    case Yn:
    case Zn:
    case qn:
    case Xn:
    case Qn:
    case er:
    case nr:
    case rr:
      return W(e, r);
    case Vn:
      return new o();
    case $n:
    case Hn:
      return new o(e);
    case Jn:
      return Be(e);
    case Bn:
      return new o();
    case Kn:
      return Ge(e);
  }
}
var ze = or;
var ir = "[object Map]";
function sr(e) {
  return nx(e) && Wfe(e) == ir;
}
var Ye = sr;
var Ze = eie && eie.isMap,
  ar = Ze ? uQe(Ze) : Ye,
  qe = ar;
var lr = "[object Set]";
function cr(e) {
  return nx(e) && Wfe(e) == lr;
}
var Xe = cr;
var Qe = eie && eie.isSet,
  dr = Qe ? uQe(Qe) : Xe,
  et = dr;
var ur = 1,
  fr = 2,
  gr = 4,
  tt = "[object Arguments]",
  pr = "[object Array]",
  mr = "[object Boolean]",
  yr = "[object Date]",
  Sr = "[object Error]",
  nt = "[object Function]",
  hr = "[object GeneratorFunction]",
  vr = "[object Map]",
  br = "[object Number]",
  rt = "[object Object]",
  wr = "[object RegExp]",
  Er = "[object Set]",
  kr = "[object String]",
  Or = "[object Symbol]",
  Ar = "[object WeakMap]",
  Pr = "[object ArrayBuffer]",
  _r = "[object DataView]",
  Mr = "[object Float32Array]",
  Rr = "[object Float64Array]",
  Cr = "[object Int8Array]",
  Tr = "[object Int16Array]",
  Ir = "[object Int32Array]",
  xr = "[object Uint8Array]",
  jr = "[object Uint8ClampedArray]",
  Nr = "[object Uint16Array]",
  Lr = "[object Uint32Array]",
  h = {};
h[tt] =
  h[pr] =
  h[Pr] =
  h[_r] =
  h[mr] =
  h[yr] =
  h[Mr] =
  h[Rr] =
  h[Cr] =
  h[Tr] =
  h[Ir] =
  h[vr] =
  h[br] =
  h[rt] =
  h[wr] =
  h[Er] =
  h[kr] =
  h[Or] =
  h[xr] =
  h[jr] =
  h[Nr] =
  h[Lr] =
    true;
h[Sr] = h[nt] = h[Ar] = false;
function ee(e, t, r, o, i, s) {
  var l,
    c = t & ur,
    d = t & fr,
    u = t & gr;
  if (r) l = i ? r(e, o, i, s) : r(e);
  if (l !== void 0) return l;
  if (!Rm(e)) return e;
  var f = wg(e);
  if (f) {
    if (((l = $e(e)), !c)) return G(e, l);
  } else {
    var g = Wfe(e),
      y = g == nt || g == hr;
    if (Zoe(e)) return L(e, c);
    if (g == rt || g == tt || (y && !i)) {
      if (((l = d || y ? {} : z(e)), !c)) return d ? Ve(e, Ue(l, e)) : Fe(e, De(l, e));
    } else {
      if (!h[g]) return i ? e : {};
      l = ze(e, g, c);
    }
  }
  s || (s = new Qoe());
  var S = s.get(e);
  if (S) return S;
  if ((s.set(e, l), et(e)))
    e.forEach(function (O) {
      l.add(ee(O, t, r, O, e, s));
    });
  else if (qe(e))
    e.forEach(function (O, w) {
      l.set(w, ee(O, t, r, w, e, s));
    });
  var p = u ? (d ? RXe : _Ct) : d ? aoe : x4,
    v = f ? void 0 : p(e);
  return (
    Le(v || e, function (O, w) {
      if (v) (w = O), (O = e[w]);
      Ipe(l, w, ee(O, t, r, w, e, s));
    }),
    l
  );
}
var ot = ee;
function Dr(e) {
  var t = e == null ? 0 : e.length;
  return t ? e[t - 1] : void 0;
}
var bO = Dr;
function Ur(e, t, r) {
  var o = -1,
    i = e.length;
  if (t < 0) t = -t > i ? 0 : i + t;
  if (((r = r > i ? i : r), r < 0)) r += i;
  (i = t > r ? 0 : (r - t) >>> 0), (t >>>= 0);
  var s = Array(i);
  while (++o < i) s[o] = e[o + t];
  return s;
}
var DNe = Ur;
function Fr(e, t) {
  return t.length < 2 ? e : qfe(e, DNe(t, 0, -1));
}
var it = Fr;
var Vr = Object.prototype,
  $r = Vr.hasOwnProperty;
function Jr(e, t) {
  t = P4(t, e);
  var r = -1,
    o = t.length;
  if (!o) return true;
  while (++r < o) {
    var i = Tj(t[r]);
    if (i === "__proto__" && !$r.call(e, "__proto__")) return false;
    if ((i === "constructor" || i === "prototype") && r < o - 1) return false;
  }
  var s = it(e, t);
  return s == null || delete s[Tj(bO(t))];
}
var st = Jr;
function Br(e) {
  return yXe(e) ? void 0 : e;
}
var at = Br;
var lt = lx ? lx.isConcatSpreadable : void 0;
function Hr(e) {
  return wg(e) || Bfe(e) || !!(lt && e && e[lt]);
}
var ct = Hr;
function dt(e, t, r, o, i) {
  var s = -1,
    l = e.length;
  r || (r = ct), i || (i = []);
  while (++s < l) {
    var c = e[s];
    if (t > 0 && r(c))
      if (t > 1) dt(c, t - 1, r, o, i);
      else NFe(i, c);
    else if (!o) i[i.length] = c;
  }
  return i;
}
var ut = dt;
function Kr(e) {
  var t = e == null ? 0 : e.length;
  return t ? ut(e, 1) : [];
}
var ft = Kr;
function Wr(e) {
  return q(Z(e, void 0, ft), e + "");
}
var ne = Wr;
var Gr = 1,
  zr = 2,
  Yr = 4,
  Zr = ne(function (e, t) {
    var r = {};
    if (e == null) return r;
    var o = false;
    if (
      ((t = nie(t, function (s) {
        return (s = P4(s, e)), o || (o = s.length > 1), s;
      })),
      M(e, RXe(e), r),
      o)
    )
      r = ot(r, Gr | zr | Yr, at);
    var i = t.length;
    while (i--) st(r, t[i]);
    return r;
  }),
  Ul = Zr;
function qr(e, t) {
  return Aqt(e, t, function (r, o) {
    return OVt(e, o);
  });
}
var gt = qr;
var Xr = ne(function (e, t) {
    return e == null ? {} : gt(e, t);
  }),
  bXe = Xr;
import { homedir as vo } from "os";
import { dirname as bo, join as R, resolve as _ } from "path";
function yEn(e) {
  return e !== void 0 && (e.commit !== void 0 || e.pr !== void 0);
}
function pt(e, t) {
  let r = e?.commitTrailers;
  if (typeof r === "boolean") return r ? "explicit-enabled" : "disabled";
  if (e !== void 0 && yEn(e)) return e.commit === "" ? "disabled" : "implicit-enabled";
  if (t !== void 0) return t ? "implicit-enabled" : "disabled";
  return;
}
var mt = 512;
function gqt(e) {
  return e.map(Qr);
}
function Qr(e) {
  let {
      file: t,
      severity: r,
      docLink: o,
      statusOnly: i,
      startupFatal: s,
      mcpErrorMetadata: l,
      path: c,
      message: d,
      expected: u,
      suggestion: f,
      invalidValue: g,
      ...y
    } = e,
    S = y;
  return {
    file: t,
    severity: r,
    docLink: o,
    statusOnly: i,
    startupFatal: s,
    mcpErrorMetadata: l,
    path: F(c),
    message: F(d),
    expected: u === void 0 ? void 0 : F(u),
    suggestion: f === void 0 ? void 0 : F(f),
    invalidValue: typeof g === "string" ? F(g) : void 0,
  };
}
function F(e) {
  let t = BXe(e.replace(/\s+/gu, " "));
  return t.length > mt ? `${t.slice(0, mt - 1)}\u2026` : t;
}
function ae(e) {
  let t = e ? tCe(e) : bw(),
    r = eU(t, { unrepresentable: "any" });
  return Vwt(r, false), b(r, null, 2);
}
var eo = /^@internal(?:\b|$)/;
function yt(e) {
  return typeof e === "string" && eo.test(e);
}
function Vwt(e, t) {
  if (Array.isArray(e)) {
    for (let i of e) Vwt(i, t);
    return;
  }
  if (e === null || typeof e !== "object") return;
  let r = e;
  if (t && yt(r.description)) {
    let i = r.description.replace(/^@internal\s*/, "").trim();
    if (i) r.description = i;
    else delete r.description;
  }
  let o = r.properties;
  if (!t && o !== null && typeof o === "object" && !Array.isArray(o)) {
    let i = o;
    for (let s of Object.keys(i)) {
      let l = i[s],
        c = l !== null && typeof l === "object" && !Array.isArray(l) ? l.description : void 0;
      if (yt(c)) {
        if ((delete i[s], Array.isArray(r.required))) r.required = r.required.filter((d) => d !== s);
      }
    }
  }
  for (let i of Object.values(r)) Vwt(i, t);
}
var C = "https://code.claude.com/docs/en",
  to = [
    {
      matches: (e) => e.path === "permissions.defaultMode" && e.code === "invalid_value",
      tip: {
        suggestion:
          'Valid modes: "acceptEdits" (ask before file changes), "plan" (analysis only), "bypassPermissions" (auto-accept all), or "default" (standard behavior)',
        docLink: `${C}/iam#permission-modes`,
      },
    },
    {
      matches: (e) => e.path === "apiKeyHelper" && e.code === "invalid_type",
      tip: {
        suggestion:
          'Provide a shell command that outputs your API key to stdout. The script should output only the API key. Example: "/bin/generate_temp_api_key.sh"',
      },
    },
    {
      matches: (e) => e.path === "cleanupPeriodDays" && e.code === "too_small",
      tip: {
        suggestion:
          'cleanupPeriodDays must be at least 1. To keep transcripts for a long time, set a large number (e.g. 3650 for ~10 years). To disable transcript writes entirely, remove this setting and use the --no-session-persistence CLI flag or the SDK persistSession:false option instead. (0 is rejected because it previously silently disabled all transcript writes, which users setting it to mean "never clean up" did not expect.)',
      },
    },
    {
      matches: (e) => e.path.startsWith("env.") && e.code === "invalid_type",
      tip: {
        suggestion:
          'Environment variables must be strings. Wrap numbers and booleans in quotes. Example: "DEBUG": "true", "PORT": "3000"',
        docLink: `${C}/settings#environment-variables`,
      },
    },
    {
      matches: (e) =>
        (e.path === "permissions.allow" || e.path === "permissions.deny") &&
        e.code === "invalid_type" &&
        e.expected === "array",
      tip: {
        suggestion:
          'Permission rules must be in an array. Format: ["Tool(specifier)"]. Examples: ["Bash(npm run build)", "Edit(docs/**)", "Read(~/.zshrc)"]. Use * for wildcards.',
      },
    },
    {
      matches: (e) => e.path.startsWith("hooks.") && e.code === "invalid_key",
      tip: {
        suggestion:
          "Not a recognized hook event. Common events: PreToolUse, PostToolUse, UserPromptSubmit, SessionStart, SessionEnd, Stop. Check spelling and capitalization.",
        docLink: `${C}/hooks`,
      },
    },
    {
      matches: (e) => /\.hooks\.\d+\.command$/.test(e.path) && e.code === "invalid_type" && e.received === "undefined",
      tip: {
        suggestion:
          'Command hooks require `command`. For exec form (no shell), set `command` to the executable and `args` to its arguments: {"type": "command", "command": "echo", "args": ["hi"]}. For shell form, set `command` to the full shell string: {"type": "command", "command": "echo hi"}.',
        docLink: `${C}/hooks#exec-form-and-shell-form`,
      },
    },
    {
      matches: (e) => e.path.includes("hooks") && e.code === "invalid_type",
      tip: {
        suggestion:
          'Hooks use a matcher + hooks array. The matcher is a string: a tool name ("Bash"), pipe-separated list ("Edit|Write"), or empty to match all. Example: {"PostToolUse": [{"matcher": "Edit|Write", "hooks": [{"type": "command", "command": "echo Done"}]}]}',
      },
    },
    {
      matches: (e) => e.code === "invalid_type" && e.expected === "boolean",
      tip: { suggestion: 'Use true or false without quotes. Example: "includeCoAuthoredBy": true' },
    },
    {
      matches: (e) => e.code === "unrecognized_keys",
      tip: { suggestion: "Check for typos or refer to the documentation for valid fields", docLink: `${C}/settings` },
    },
    { matches: (e) => e.code === "invalid_value" && e.enumValues !== void 0, tip: { suggestion: void 0 } },
    {
      matches: (e) => e.code === "invalid_type" && e.expected === "object" && e.received === null && e.path === "",
      tip: {
        suggestion:
          "Check for missing commas, unmatched brackets, or trailing commas. Use a JSON validator to identify the exact syntax error.",
      },
    },
    {
      matches: (e) => e.path === "permissions.additionalDirectories" && e.code === "invalid_type",
      tip: {
        suggestion:
          'Must be an array of directory paths. Example: ["~/projects", "/tmp/workspace"]. You can also use --add-dir flag or /add-dir command',
        docLink: `${C}/iam#working-directories`,
      },
    },
  ],
  no = {
    permissions: `${C}/iam#configuring-permissions`,
    env: `${C}/settings#environment-variables`,
    hooks: `${C}/hooks`,
  };
function ht(e) {
  let t = to.find((o) => o.matches(e));
  if (!t) return null;
  let r = { ...t.tip };
  if (e.code === "invalid_value" && e.enumValues && !r.suggestion)
    r.suggestion = `Valid values: ${e.enumValues.map((o) => `"${o}"`).join(", ")}`;
  if (!r.docLink && e.path) r.docLink = no[St(e.path, ".")];
  return r;
}
var ro = m(() => tCe(ZAe(), { strictPolicyHelperKeys: true }).strict());
function vt(e) {
  return e.code === "invalid_type";
}
function bt(e) {
  return e.code === "invalid_value";
}
function oo(e) {
  return e.code === "unrecognized_keys";
}
function wt(e) {
  return e.code === "too_small";
}
function P(e) {
  if (e === null) return "null";
  if (e === void 0) return "undefined";
  if (Array.isArray(e)) return "array";
  return typeof e;
}
function Et(e) {
  let t = e.match(/received (\w+)/);
  return t ? t[1] : void 0;
}
function j(e, t) {
  return e.issues.map((r) => {
    let o = r.path.map(String).join("."),
      i = r.message,
      s,
      l,
      c,
      d,
      u;
    if (bt(r)) (l = r.values.map((g) => String(g))), (c = l.join(" | ")), (d = void 0), (u = void 0);
    else if (vt(r)) {
      c = r.expected;
      let g = Et(r.message);
      (d = g ?? P(r.input)), (u = g ?? P(r.input));
    } else if (wt(r)) c = String(r.minimum);
    else if (r.code === "custom" && "params" in r) (d = r.params.received), (u = d);
    let f = ht({ path: o, code: r.code, expected: c, received: d, enumValues: l, message: r.message, value: d });
    if (bt(r)) (s = l?.map((g) => `"${g}"`).join(", ")), (i = `Invalid value. Expected one of: ${s}`);
    else if (vt(r)) {
      let g = Et(r.message) ?? P(r.input);
      if (r.expected === "object" && g === "null" && o === "") i = "Invalid or malformed JSON";
      else i = `Expected ${r.expected}, but received ${g}`;
    } else if (oo(r)) {
      let g = r.keys.join(", ");
      i = `Unrecognized ${k(r.keys.length, "field")}: ${g}`;
    } else if (wt(r)) (i = `Number must be greater than or equal to ${r.minimum}`), (s = String(r.minimum));
    return {
      file: t,
      path: o,
      message: i,
      expected: s,
      invalidValue: u,
      suggestion: f?.suggestion,
      docLink: f?.docLink,
    };
  });
}
function SEn(e) {
  try {
    let t = V(e),
      r = J6(He(t) ? { ...t } : t, "settings").map(MEn),
      o = ro().safeParse(t),
      i = o.success ? [] : j(o.error, "settings"),
      s = wEn(t);
    if (s !== void 0) i.push({ path: _qt, message: `"crossSessionInbound" ${s}.` });
    let l = bEn(t);
    if (i.length === 0 && r.length === 0 && l.length === 0) return { isValid: true };
    return {
      isValid: false,
      error:
        `Settings validation failed:
` +
        [
          ...r.map((d) => `- ${d}`),
          ...i.map((d) => {
            let u = `- ${d.path}: ${d.message}`;
            if (d.suggestion) u += `. ${d.suggestion}`;
            return u;
          }),
          ...l.map((d) => `- ${d}`),
        ].join(`
`),
      fullSchema: ae(),
    };
  } catch (t) {
    return {
      isValid: false,
      error: `Invalid JSON: ${t instanceof Error ? t.message : "Unknown parsing error"}`,
      fullSchema: ae(),
    };
  }
}
function hqt(e, t) {
  let r = Wp(e),
    o = [];
  if (r && typeof r === "object") {
    let l = r;
    for (let c of ["policyHelper", "policyHelpers"])
      if (l[c] !== void 0) {
        if (l[c] !== null) o.push(c);
        delete l[c];
      }
  }
  let i = zAe(r, t, { mcpServerEntrySalvageOnly: true, policySource: true }),
    s = bw().safeParse(r);
  if (!s.success) {
    let l = t.startsWith("policyHelpers.") ? "payload" : "managedSettings",
      c = s.error.issues
        .slice(0, 3)
        .map((d) => (d.path.length ? `${d.path.join(".")}: ${d.message}` : d.message))
        .join("; ");
    return { error: `${l} rejected: ${c}`, warnings: i, strippedKeys: o };
  }
  return { settings: s.data, warnings: i, strippedKeys: o };
}
function bEn(e) {
  if (!e || typeof e !== "object" || Array.isArray(e)) return [];
  let t = e.policyHelpers;
  if (!t || typeof t !== "object" || Array.isArray(t)) return [];
  let r = [],
    o = t;
  if (o.default !== void 0 && o.default !== null) r.push(["policyHelpers.default", o.default]);
  for (let s of Q6) {
    let l = o[s];
    if (l && typeof l === "object" && !Array.isArray(l)) {
      let c = l.defaultSettings;
      if (c !== void 0 && c !== null) r.push([`policyHelpers.${s}.defaultSettings`, c]);
    }
  }
  let i = [];
  for (let [s, l] of r) {
    for (let d of J6(He(l) ? { ...l } : l, s)) i.push(`${s}: ${MEn(d)}`);
    let c = hqt(l, s);
    if ("error" in c)
      i.push(
        `${s}: not a valid static settings payload \u2014 Claude Code refuses to start on it when delivered from an OS-admin policy source (${c.error})`,
      );
  }
  return i;
}
function io(e, t) {
  if (!e || typeof e !== "object") return [];
  let r = e;
  if (!r.permissions || typeof r.permissions !== "object") return [];
  let o = r.permissions,
    i = [];
  for (let s of ["allow", "deny", "ask"]) {
    let l = o[s];
    if (!Array.isArray(l)) continue;
    o[s] = l.filter((c) => {
      if (typeof c !== "string")
        return (
          i.push({
            file: t,
            path: `permissions.${s}`,
            message: `Non-string value in ${s} array was removed`,
            severity: "warning",
            invalidValue: c,
          }),
          false
        );
      let d = Npe(c, s);
      if (!d.valid) {
        let u = `Invalid permission rule "${c}" was skipped: ${d.error}`;
        if (d.suggestion) u += `. ${d.suggestion}`;
        return i.push({ file: t, path: `permissions.${s}`, message: u, severity: "warning", invalidValue: c }), false;
      }
      return true;
    });
  }
  return i;
}
var so = new Set(_y);
function ao(e, t) {
  if (!e || typeof e !== "object") return [];
  let r = e;
  if (!("hooks" in r)) return [];
  if (r.hooks === null || typeof r.hooks !== "object" || Array.isArray(r.hooks)) {
    let s = P(r.hooks);
    return (
      delete r.hooks,
      [
        {
          file: t,
          path: "hooks",
          message: `"hooks" must be an object mapping event names to matcher arrays; received ${s}. This field was ignored.`,
          severity: "warning",
          invalidValue: s,
          docLink: "https://code.claude.com/docs/en/hooks",
        },
      ]
    );
  }
  let o = r.hooks,
    i = [];
  for (let s of Object.keys(o)) {
    if (!so.has(s)) {
      delete o[s],
        i.push({
          file: t,
          path: `hooks.${s}`,
          message: `Unknown hook event "${s}" was ignored. Valid events: ${_y.join(", ")}`,
          severity: "warning",
          invalidValue: s,
          docLink: "https://code.claude.com/docs/en/hooks",
        });
      continue;
    }
    if (!Array.isArray(o[s])) {
      let l = P(o[s]);
      delete o[s],
        i.push({
          file: t,
          path: `hooks.${s}`,
          message: `Hook event "${s}" must be an array of matchers; received ${l}. This entry was ignored.`,
          severity: "warning",
          invalidValue: l,
          docLink: "https://code.claude.com/docs/en/hooks",
        });
    }
  }
  if (i.length > 0 && Object.keys(o).length === 0) delete r.hooks;
  return i;
}
var lo = [
  { key: "allowedMcpServers", schema: Pqt },
  { key: "deniedMcpServers", schema: Dqt },
];
function co(e, t, r) {
  if (!e || typeof e !== "object") return [];
  let o = e,
    i = [];
  for (let { key: s, schema: l } of lo) {
    if (!(s in o)) continue;
    if (!Array.isArray(o[s])) {
      if (r?.keepWholeFieldInvalid) continue;
      let u = o[s];
      delete o[s],
        i.push({
          file: t,
          path: s,
          message: `"${s}" must be an array; received ${P(u)}. This field was ignored.`,
          severity: "warning",
          invalidValue: u,
        });
      continue;
    }
    let c = o[s],
      d = [];
    for (let u = 0; u < c.length; u++) {
      let f = l().safeParse(c[u]);
      if (f.success) d.push(c[u]);
      else
        i.push({
          file: t,
          path: `${s}[${u}]`,
          message: `Invalid entry was ignored: ${f.error.issues[0]?.message ?? "failed validation"}`,
          severity: "warning",
          invalidValue: c[u],
        });
    }
    if (d.length < c.length) o[s] = d;
  }
  return i;
}
function uo(e) {
  if (!e || typeof e !== "object") return;
  let t = e.source;
  if (!t || typeof t !== "object") return;
  let r = t.source;
  return typeof r === "string" ? r : void 0;
}
function fo(e) {
  if (!e || typeof e !== "object") return false;
  let t = e.source;
  if (!t || typeof t !== "object") return false;
  let r = t.plugins;
  return Array.isArray(r) && r.some(kAn);
}
function go(e, t) {
  if (!e || typeof e !== "object") return [];
  let r = e,
    o = "extraKnownMarketplaces";
  if (!(o in r)) return [];
  let i = r[o];
  if (!i || typeof i !== "object" || Array.isArray(i)) {
    let c = P(i);
    return (
      delete r[o],
      [
        {
          file: t,
          path: o,
          message: `"${o}" must be an object mapping marketplace names to declarations; received ${c}. This field was ignored.`,
          severity: "warning",
          invalidValue: c,
        },
      ]
    );
  }
  let s = i,
    l = [];
  for (let c of Object.keys(s)) {
    let d = FEn().safeParse(s[c]),
      u;
    if (!d.success) {
      let f = uo(s[c]);
      if ((f !== void 0 && !Eir.has(f)) || fo(s[c])) continue;
      u = tGt(d.error.issues);
    } else if (d.data.source.source === "settings" && d.data.source.name !== c)
      u = `key "${pCe(c)}" must match the settings source name "${pCe(d.data.source.name)}"`;
    if (u !== void 0)
      l.push({
        file: t,
        path: `${o}.${pCe(c)}`,
        message: `Invalid marketplace entry was ignored: ${u}`,
        severity: "warning",
      }),
        delete s[c];
  }
  return l;
}
function po(e, t) {
  if (!e || typeof e !== "object") return [];
  let r = e,
    o = "modelPicker";
  if (!(o in r)) return [];
  let i = r[o];
  if (!He(i) || !Array.isArray(i.options)) {
    let d = He(i) ? `options: ${P(i.options)}` : P(i);
    return (
      delete r[o],
      [
        {
          file: t,
          path: o,
          message: `"${o}" must be an object with an "options" array of { model, label?, description? } rows; received ${d}. This field was ignored.`,
          severity: "warning",
          invalidValue: d,
        },
      ]
    );
  }
  let s = [];
  if ("replaceBuiltInOptions" in i && typeof i.replaceBuiltInOptions !== "boolean") {
    let d = P(i.replaceBuiltInOptions);
    delete i.replaceBuiltInOptions,
      s.push({
        file: t,
        path: `${o}.replaceBuiltInOptions`,
        message: `"replaceBuiltInOptions" must be true or false; received ${d}. This entry was ignored (the rows are added to the built-in lineup).`,
        severity: "warning",
        invalidValue: d,
      });
  }
  let l = i.options,
    c = [];
  for (let d = 0; d < l.length; d++) {
    let u = BEn().safeParse(l[d]);
    if (u.success) {
      c.push(l[d]);
      continue;
    }
    s.push({
      file: t,
      path: `${o}.options.${d}`,
      message: `${yo(u.error.issues[0])}. This row was ignored; the other rows still apply.`,
      severity: "warning",
      invalidValue: P(l[d]),
    });
  }
  if (c.length !== l.length) i.options = c;
  return s;
}
function mo(e, t) {
  if (!e || typeof e !== "object" || Array.isArray(e)) return [];
  let r = e;
  if (!("modelPricing" in r) || r.modelPricing === void 0) return [];
  let o = r.modelPricing;
  if (!o || typeof o !== "object" || Array.isArray(o))
    return (
      delete r.modelPricing,
      [
        {
          file: t,
          path: "modelPricing",
          message: `"modelPricing" must be an object; received ${P(o)}. It was ignored.`,
          severity: "warning",
        },
      ]
    );
  let i = o,
    s = [];
  if (i.multiplier !== void 0 && !UEn().safeParse(i.multiplier).success)
    s.push({
      file: t,
      path: "modelPricing.multiplier",
      message: '"multiplier" must be a number in (0, 1]. It was ignored.',
      severity: "warning",
      invalidValue: i.multiplier,
    }),
      delete i.multiplier;
  if (i.overrides !== void 0) {
    let l = i.overrides;
    if (!l || typeof l !== "object" || Array.isArray(l))
      s.push({
        file: t,
        path: "modelPricing.overrides",
        message: `"overrides" must be an object mapping model ID to rates; received ${P(l)}. It was ignored.`,
        severity: "warning",
      }),
        delete i.overrides;
    else {
      let c = l;
      for (let d of Object.keys(c)) {
        let u = $En().safeParse(c[d]);
        if (u.success && d !== "constructor" && d !== "__proto__") continue;
        let f = u.success ? void 0 : u.error.issues[0];
        s.push({
          file: t,
          path: `modelPricing.overrides.${pCe(d)}`,
          message: `Invalid pricing row was ignored (${f ? `${f.path.join(".") || "row"}: ${f.message}` : "not a model ID"}).`,
          severity: "warning",
          invalidValue: c[d],
        }),
          delete c[d];
      }
    }
  }
  return s;
}
function yo(e) {
  if (!e) return "Invalid modelPicker row";
  return `modelPicker row ${e.path.length > 0 ? `"${e.path.join(".")}" ` : ""}${e.message}`.trim();
}
function So(e, t, r) {
  if (!e || typeof e !== "object" || Array.isArray(e)) return [];
  let o = e,
    i = o.crossSessionInbound;
  if (i === void 0 || kt(i)) return [];
  if (r?.policySource) o.crossSessionInbound = "refuse";
  else delete o.crossSessionInbound;
  return [ho(i, t, r?.policySource === true)];
}
function kt(e) {
  return typeof e === "string" && eCe.includes(e);
}
var _qt = "crossSessionInbound";
function wEn(e) {
  if (!He(e)) return;
  let t = e.crossSessionInbound;
  if (t === void 0 || kt(t)) return;
  return Ot(t);
}
function Ot(e) {
  let t = eCe.map((o) => `"${o}"`).join(", "),
    r = typeof e === "string" ? `"${pCe(e).replace(/^<key>$/, "<value>")}"` : P(e);
  return `must be one of ${t}; received ${r}`;
}
function ho(e, t, r = false) {
  let o = eCe.map((s) => `"${s}"`).join(", "),
    i = r
      ? 'In managed settings an unrecognized value is treated as "refuse" (the most restrictive): cross-session messages to this session are turned away until an administrator fixes it.'
      : "This value was ignored; while it is present, cross-session messages are held for your approval instead of being delivered. Set it to one of the values above.";
  return {
    file: t,
    path: _qt,
    message: `"crossSessionInbound" ${Ot(e)}. ${i}`,
    severity: "warning",
    expected: o,
    ...(r && { statusOnly: true }),
  };
}
function zAe(e, t, r) {
  return [
    ...J6(e, t),
    ...io(e, t),
    ...ao(e, t),
    ...go(e, t),
    ...po(e, t),
    ...mo(e, t),
    ...So(e, t, { policySource: r?.policySource }),
    ...(r?.skipMcpServerEntryFilter ? [] : co(e, t, { keepWholeFieldInvalid: r?.mcpServerEntrySalvageOnly })),
  ];
}
function Kwt(e) {
  let t = new Set(e.allowedSources);
  return t.add("flagSettings"), t.add("policySettings"), Is.filter((r) => t.has(r));
}
function wo() {
  return R(ib(), "managed-settings.json");
}
function ONe(e) {
  if (D() === "wsl" && e.wslInherits?.()) {
    let t = ce(c0, e.store);
    if (t.settings) return t;
    let r = ce(ib(), e.store);
    return { settings: r.settings, errors: [...t.errors, ...r.errors] };
  }
  return ce(ib(), e.store);
}
function LNe(e) {
  return D() === "wsl" && e ? [c0, ib()] : [ib()];
}
function Xwt(e) {
  return e.endsWith(".json") && !e.startsWith(".");
}
function ce(e, t) {
  let r = [],
    o = {},
    i = false,
    { settings: s, errors: l } = r5(R(e, "managed-settings.json"), t, void 0, true);
  if ((r.push(...l), s && Object.keys(s).length > 0)) (o = dP(o, s, i5)), (i = true);
  let c = R(e, "managed-settings.d");
  try {
    let d = t.folderListingForPolicyWalk(c),
      u;
    if (d !== void 0) u = d;
    else
      (u = le()
        .readdirSync(c)
        .filter((f) => (f.isFile() || f.isSymbolicLink()) && Xwt(f.name))
        .map((f) => f.name)
        .sort()),
        t.noteWalkListing(c, u);
    for (let f of u) {
      let { settings: g, errors: y } = r5(R(c, f), t, void 0, true);
      if ((r.push(...y), g && Object.keys(g).length > 0)) (o = dP(o, g, i5)), (i = true);
    }
  } catch (d) {
    let u = E(d);
    if (u !== "ENOENT" && u !== "ENOTDIR")
      n(`managed-settings.d read failed: ${d}`, { level: "error" }),
        r.push({
          file: c,
          path: "",
          message: `Managed settings drop-in directory could not be read: ${u ?? "unknown error"}`,
          severity: "fatal",
        });
  }
  return { settings: i && pP(o) ? o : null, errors: r };
}
function TEn(e, t) {
  if (X(e)) KAe(t);
  else n(`settings file read failed at ${t}: ${e}`, { level: "error" });
}
function r5(e, t, r, o) {
  let i = r !== void 0 ? `${e}\x00pinned` : e,
    s = t.parsedFiles.get(i);
  if (s) return { settings: s.settings ? Wp(s.settings) : null, errors: s.errors };
  let l = TXe(e, r, o);
  return t.parsedFiles.set(i, l), { settings: l.settings ? Wp(l.settings) : null, errors: l.errors };
}
function ue(e) {
  if (!e.mdm) return { settings: null, errors: [] };
  let t = e.mdm();
  return { settings: Object.keys(t.settings).length > 0 ? t.settings : null, errors: t.errors };
}
function VAe(e, t) {
  let r = At.get(e),
    o = r?.get(t);
  if (o) return { settings: o.settings && Wp(o.settings), errors: o.errors.map((s) => ({ ...s })) };
  let i = Eo(e, t);
  if (r) r.set(t, i);
  else At.set(e, new Map([[t, i]]));
  return { settings: i.settings && Wp(i.settings), errors: i.errors.map((s) => ({ ...s })) };
}
var At = new WeakMap();
function Eo(e, t) {
  let r = Wp(e),
    o = zAe(r, t, { skipMcpServerEntryFilter: true, policySource: true }),
    i = [],
    s = YEn(Mt(t, i), t).safeParse(r);
  if (!s.success) return { settings: null, errors: [...o, ...j(s.error, t)] };
  return { settings: Object.keys(s.data).length > 0 ? s.data : null, errors: [...o, ...i] };
}
function Mt(e, t) {
  return (r) => {
    if (
      (t.push({
        file: e,
        path: r.path,
        message: r.message,
        severity: "warning",
        ...(r.statusOnly && { statusOnly: r.statusOnly }),
        ...(r.startupFatal && { startupFatal: r.startupFatal }),
      }),
      r.statusOnly || r.startupFatal)
    )
      n(`${e}: ${r.path}: ${r.message}`, { level: r.startupFatal ? "error" : "warn" });
  };
}
function o5(e) {
  let t = e?.remote ? e.remote() : J3();
  if (!t || Object.keys(t).length === 0) return { settings: null, errors: [] };
  let { settings: r, errors: o } = VAe(t, "remote managed settings");
  return { settings: r, errors: gqt(o) };
}
var EEn = "parent managed settings",
  ioe = ["cleanupPeriodDays", "desktopSessionCleanupPeriodDays"];
function wXe(e) {
  let t = e.parentManaged;
  if (!t || Object.keys(t).length === 0) return { settings: null, errors: [] };
  return VAe(t, EEn);
}
function Ywt(e) {
  let t = e.flagInline;
  if (!t) return { settings: null, errors: [] };
  let r = Wp(t),
    o = zAe(r, "SDK inline settings"),
    i = bw().safeParse(r);
  if (!i.success) return { settings: null, errors: [...o, ...j(i.error, "SDK inline settings")] };
  return { settings: i.data, errors: o };
}
var sb = 2097152;
function TXe(e, t, r) {
  try {
    let o;
    if (t !== void 0) o = t;
    else {
      let { resolvedPath: i } = Qo(le(), e);
      o = SR(i, sb);
    }
    return MNe(o, e, r);
  } catch (o) {
    return ko(o, e);
  }
}
function MNe(e, t, r) {
  if (e.trim() === "") return { settings: {}, errors: [] };
  let o = Wp(Ut(e, false));
  if (r) {
    let l = zAe(o, t, { skipMcpServerEntryFilter: true, policySource: true }),
      c = [],
      d = YEn(Mt(t, c), t).safeParse(o);
    if (!d.success) return { settings: null, errors: [...l, ...j(d.error, t)] };
    return { settings: d.data, errors: [...l, ...c] };
  }
  let i = zAe(o, t),
    s = bw().safeParse(o);
  if (!s.success) {
    let l = j(s.error, t);
    return { settings: null, errors: [...i, ...l] };
  }
  return { settings: s.data, errors: i };
}
function KAe(e) {
  n(`Broken symlink or missing file encountered for settings.json at path: ${e}`);
}
function soe() {
  return { settings: null, errors: [] };
}
function ko(e, t) {
  if ((TEn(e, t), X(e))) return soe();
  return {
    settings: null,
    errors: [
      {
        file: t,
        path: "",
        message: `Settings file could not be read: ${e instanceof Error ? e.message : String(e)}`,
        severity: "fatal",
      },
    ],
  };
}
function Jwt(e, t) {
  switch (e) {
    case "userSettings":
      return _(be());
    case "policySettings":
    case "projectSettings":
      return _(t.cwd);
    case "localSettings":
      return Y6(t.cwd, t.canonicalGitRoot);
    case "flagSettings":
      return t.flagPath ? bo(_(t.flagPath)) : _(t.cwd);
  }
}
function Y6(e, t) {
  let r = AEn(e, t);
  if (r.decided !== void 0) return r.decided;
  if (!Po(r.root)) return r.cwdResolved;
  return r.root;
}
function AEn(e, t) {
  let r = t?.(e);
  if (!r) return { decided: _(e) };
  let o = _(r),
    i = _(e);
  if (o === i) return { decided: o };
  let s;
  try {
    s = Mo();
  } catch {
    return { decided: i };
  }
  if (o === s) return { decided: i };
  return { decided: void 0, root: o, cwdResolved: i };
}
function Oo(e) {
  return Ha().localStoreProbes.canonicalRootOwnerUids(e, Ao);
}
function Ao(e) {
  let t = le(),
    r = null;
  try {
    r = t.lstatSync(R(e, ".claude")).uid;
  } catch (o) {
    if (!X(o)) throw o;
  }
  return { rootUid: t.statSync(e).uid, gitEntryUid: t.lstatSync(R(e, ".git")).uid, claudeEntryUid: r };
}
function Po(e) {
  if (typeof process.getuid !== "function" && typeof process.geteuid !== "function")
    return (
      n(
        `localSettings: not canonicalizing the consent store to ${e} \u2014 this platform has no uid semantics to verify directory ownership with, so the store stays at the session cwd (canonicalization is POSIX-only)`,
        { level: "warn" },
      ),
      false
    );
  let t = typeof process.geteuid === "function" ? process.geteuid() : process.getuid?.();
  try {
    let { rootUid: r, gitEntryUid: o, claudeEntryUid: i } = Oo(e);
    if (r === t && o === t && (i === null || i === t)) return true;
    return (
      n(
        `localSettings: not canonicalizing the consent store to ${e} \u2014 it (uid ${r}), its .git entry (uid ${o}), or its .claude entry (uid ${i ?? "absent"}) is not owned by the current user (uid ${t}); the store stays at the session cwd (the pre-canonicalization behavior). If you own this repo, chown it (including .git and .claude) or run from a directory you own.`,
        { level: "warn" },
      ),
      false
    );
  } catch (r) {
    return (
      n(
        `localSettings: not canonicalizing the consent store to ${e} \u2014 its ownership could not be verified (${r instanceof Error ? r.message : String(r)}); the store stays at the session cwd`,
        { level: "warn" },
      ),
      false
    );
  }
}
function Mo() {
  return Ha().localStoreProbes.normalizedRealHomeDir(Ro);
}
function Ro() {
  let e = bS(vo());
  if (e === null) throw Error("home directory realpath unavailable");
  return tr(e);
}
function Aor(e, t) {
  return e === "localSettings" ? _(t.cwd) : Jwt(e, t);
}
var Y3 = { default: "settings.json", cowork: "cowork_settings.json" };
function Co(e) {
  if (e.coworkPlugins || a.CLAUDE_CODE_USE_COWORK_PLUGINS) return Y3.cowork;
  return Y3.default;
}
function BY(e, t) {
  switch (e) {
    case "userSettings":
      return R(Jwt(e, t), Co(t));
    case "projectSettings":
    case "localSettings":
      return R(Jwt(e, t), jH(e));
    case "policySettings":
      return wo();
    case "flagSettings":
      return t.flagPath;
  }
}
function jH(e) {
  switch (e) {
    case "projectSettings":
      return R(".claude", "settings.json");
    case "localSettings":
      return R(".claude", "settings.local.json");
  }
}
function XAe(e) {
  if (Y6(e.cwd, e.canonicalGitRoot) === _(e.cwd)) return;
  return R(_(e.cwd), jH("localSettings"));
}
function Qwt(e, t) {
  let r = t.store.perSource.get(e);
  if (r !== void 0) return r;
  let o = bqt(e, t);
  return t.store.perSource.set(e, o), o;
}
function Rt(e) {
  return !e || e.parentSettingsBehavior === "merge";
}
var Ct = [
  { path: ["allowManagedPermissionRulesOnly"], restrictive: true },
  { path: ["allowManagedHooksOnly"], restrictive: true },
  { path: ["allowManagedMcpServersOnly"], restrictive: true },
  { path: ["enforceAvailableModels"], restrictive: true },
  { path: ["disableAllHooks"], restrictive: true },
  { path: ["disableClaudeAiConnectors"], restrictive: true },
  { path: ["disableCommandPluginSources"], restrictive: true },
  { path: ["disableSideloadFlags"], restrictive: true },
  { path: ["disableSkillShellExecution"], restrictive: true },
  { path: ["disableRemoteControl"], restrictive: true },
  { path: ["disableAgentView"], restrictive: true },
  { path: ["disableWorkflows"], restrictive: true },
  { path: ["disableArtifact"], restrictive: true },
  { path: ["disableBundledSkills"], restrictive: true },
  { path: ["fastModePerSessionOptIn"], restrictive: true },
  { path: ["isolatePeerMachines"], restrictive: true },
  { path: ["strictPluginOnlyCustomization"], restrictive: true },
  { path: ["disableAutoMode"], restrictive: "disable" },
  { path: ["disableDeepLinkRegistration"], restrictive: "disable" },
  { path: ["permissions", "disableBypassPermissionsMode"], restrictive: "disable" },
  { path: ["permissions", "disableAutoMode"], restrictive: "disable" },
  { path: ["autoMode", "classifyAllShell"], restrictive: true },
  ...[],
  { path: ["worktree", "bgIsolation"], restrictive: "worktree" },
  { path: ["enableArtifact"], restrictive: false },
  { path: ["enableWorkflows"], restrictive: false },
  { path: ["syncClaudeAiSkills"], restrictive: false },
  { path: ["syncClaudeAiPlugins"], restrictive: false },
  { path: ["useAutoModeDuringPlan"], restrictive: false },
  { path: ["skipDangerousModePermissionPrompt"], restrictive: false },
  { path: ["skipAutoPermissionPrompt"], restrictive: false },
  { path: ["enableAllProjectMcpServers"], restrictive: false },
  { path: ["channelsEnabled"], restrictive: false },
  { path: ["skipWebFetchPreflight"], restrictive: false },
  { path: ["skipWorkflowUsageWarning"], restrictive: false },
  { path: ["autoUploadSessions"], restrictive: false },
  { path: ["remoteControlAtStartup"], restrictive: false },
  { path: ["autoContinueAtUsageLimit"], restrictive: false },
  ...[],
  { path: ["attribution", "sessionUrl"], restrictive: false },
  { path: ["crossSessionInbound"], restrictive: ["refuse", "hold"] },
  { path: ["modelProposedGoals"], restrictive: ["disabled", "alwaysAsk"] },
  { path: ["feedbackDrafts"], restrictive: "off" },
  { path: ["askUserQuestionTimeout"], restrictive: "never" },
  { path: ["dialogExpiry"], restrictive: "never" },
  { path: ["sandbox", "enabled"], restrictive: true },
  { path: ["sandbox", "failIfUnavailable"], restrictive: true },
  { path: ["sandbox", "autoAllowBashIfSandboxed"], restrictive: false },
  { path: ["sandbox", "allowUnsandboxedCommands"], restrictive: false },
  { path: ["sandbox", "enableWeakerNestedSandbox"], restrictive: false },
  { path: ["sandbox", "enableWeakerNetworkIsolation"], restrictive: false },
  { path: ["sandbox", "allowAppleEvents"], restrictive: false },
  { path: ["sandbox", "network", "allowManagedDomainsOnly"], restrictive: true },
  { path: ["sandbox", "network", "strictAllowlist"], restrictive: true },
  { path: ["sandbox", "network", "allowAllUnixSockets"], restrictive: false },
  { path: ["sandbox", "network", "allowLocalBinding"], restrictive: false },
  { path: ["sandbox", "filesystem", "allowManagedReadPathsOnly"], restrictive: true },
  { path: ["sandbox", "filesystem", "disabled"], restrictive: false },
  { path: ["sandbox", "credentials", "allowPlaintextInject"], restrictive: false },
  { path: ["sandbox", "credentials", "sigv4", "streaming"], restrictive: "deny" },
  { path: ["sandbox", "credentials", "sigv4", "presigned"], restrictive: "deny" },
  { path: ["sandbox", "credentials", "sigv4", "sigv4a"], restrictive: "deny" },
];
function Tt(e) {
  return Array.isArray(e) ? e : [e];
}
var To = [
  ["permissions", "defaultMode"],
  ["modelPicker", "replaceBuiltInOptions"],
];
function I(e, t) {
  let r = e;
  for (let o of t) {
    if (r === null || typeof r !== "object") return;
    r = r[o];
  }
  return r;
}
function J(e, t, r) {
  let o = [e],
    i = e;
  for (let l of t.slice(0, -1)) {
    let c = { ...i[l] };
    (i[l] = c), (i = c), o.push(i);
  }
  let s = t.at(-1);
  if (r !== void 0) {
    i[s] = r;
    return;
  }
  delete i[s];
  for (let l = o.length - 1; l > 0; l--) {
    if (Object.keys(o[l]).length > 0) break;
    delete o[l - 1][t[l - 1]];
  }
}
function Io(e) {
  let t = {};
  for (let { path: r, restrictive: o } of Ct) {
    let i = I(e, r);
    if (r[0] === "sandbox" && Tt(o).includes(i)) J(t, r, i);
  }
  return t.sandbox ?? {};
}
function yqt(e, t) {
  return I(e, t);
}
function xo(e, t) {
  let r = {};
  if (e.allowManagedHooksOnly === true) r.allowManagedHooksOnly = true;
  if (e.disableCommandPluginSources === true) r.disableCommandPluginSources = true;
  if (e.allowManagedMcpServersOnly === true) r.allowManagedMcpServersOnly = true;
  if (e.disableClaudeAiConnectors === true) r.disableClaudeAiConnectors = true;
  if (e.syncClaudeAiSkills === false) r.syncClaudeAiSkills = false;
  if (e.syncClaudeAiPlugins === false) r.syncClaudeAiPlugins = false;
  if (e.allowManagedPermissionRulesOnly === true) r.allowManagedPermissionRulesOnly = true;
  if (pt(e.attribution, e.includeCoAuthoredBy) === "disabled") r.attribution = { ...r.attribution, commitTrailers: false };
  if (e.attribution?.sessionUrl === false) r.attribution = { ...r.attribution, sessionUrl: false };
  let o = e.strictPluginOnlyCustomization;
  if (o === true || (Array.isArray(o) && o.length > 0)) r.strictPluginOnlyCustomization = o;
  if (e.deniedMcpServers) r.deniedMcpServers = e.deniedMcpServers;
  if (t.forceLoginOrgUUID === void 0 && e.forceLoginOrgUUID) r.forceLoginOrgUUID = e.forceLoginOrgUUID;
  if (t.allowedMcpServers === void 0 && e.allowedMcpServers) r.allowedMcpServers = e.allowedMcpServers;
  if (t.availableModels === void 0 && e.availableModels) r.availableModels = e.availableModels;
  if (e.enforceAvailableModels === true) r.enforceAvailableModels = true;
  if (e.permissions) {
    let i = bXe(e.permissions, ["deny", "ask"]);
    if (e.permissions.disableBypassPermissionsMode === "disable") i.disableBypassPermissionsMode = "disable";
    if (t.allowManagedPermissionRulesOnly !== true) {
      let { allow: s, additionalDirectories: l } = e.permissions;
      if (s && t.sandbox?.network?.allowManagedDomainsOnly !== true) i.allow = s;
      if (l) i.additionalDirectories = l;
    }
    if (Object.keys(i).length > 0) r.permissions = i;
  }
  if (e.sandbox) {
    let { network: i, filesystem: s, credentials: l } = e.sandbox,
      c = {},
      d = i ? bXe(i, ["deniedDomains"]) : {},
      u = s ? bXe(s, ["denyRead", "denyWrite"]) : {};
    if (i) {
      if (t.sandbox?.network?.allowManagedDomainsOnly !== true && i.allowedDomains) d.allowedDomains = i.allowedDomains;
    }
    if (Object.keys(d).length > 0) c.network = d;
    if (s) {
      if (t.sandbox?.filesystem?.allowManagedReadPathsOnly !== true && s.allowRead) u.allowRead = s.allowRead;
    }
    if (Object.keys(u).length > 0) c.filesystem = u;
    if (l) {
      let f = (l.files ?? []).map((S) =>
          S.mode === "deny" ? { path: S.path, mode: "deny" } : { path: S.path, mode: "mask", injectHosts: [] },
        ),
        g = (l.envVars ?? []).filter((S) => S.mode === "deny").map((S) => ({ name: S.name, mode: "deny" })),
        y = { ...(f.length > 0 && { files: f }), ...(g.length > 0 && { envVars: g }) };
      if (l.sigv4) {
        let S = {};
        for (let p of ["streaming", "presigned", "sigv4a"]) if (l.sigv4[p] === "deny") S[p] = "deny";
        y.sigv4 = S;
      }
      {
        let S = Ppe,
          p = te(
            (l.awsPairs ?? [])
              .flatMap((v) => [
                v.accessKeyIdVar,
                v.secretAccessKeyVar,
                ...(v.sessionTokenVar !== void 0 ? [v.sessionTokenVar] : []),
              ])
              .filter((v) => S.includes(v)),
          );
        if (p.length > 0) y.awsPairs = p.map((v, O) => ({ accessKeyIdVar: v, secretAccessKeyVar: `${OEn}${O + 1}_` }));
      }
      if (Object.keys(y).length > 0) c.credentials = y;
    }
    if ((dP(c, Io(e)), Object.keys(c).length > 0)) r.sandbox = c;
  }
  return r;
}
var jo = new Set([
  ...[...xXe].filter((e) => e !== "ANTHROPIC_SMALL_FAST_MODEL_AWS_REGION"),
  ...IXe,
  "CLAUDE_CODE_AUTO_MODE_MODEL",
  "CLAUDE_CODE_BG_CLASSIFIER_MODEL",
  "CLAUDE_CONTEXT_COLLAPSE_MODEL",
]);
function No(e, t) {
  if (!t || !e) return null;
  let r = {};
  if (e.model !== void 0) r.model = e.model;
  if (e.availableModels !== void 0) r.availableModels = e.availableModels;
  if (e.enforceAvailableModels !== void 0) r.enforceAvailableModels = e.enforceAvailableModels;
  if (e.fallbackModel !== void 0) r.fallbackModel = e.fallbackModel;
  if (e.modelPicker !== void 0) r.modelPicker = e.modelPicker;
  return Object.keys(r).length > 0 ? r : null;
}
function re(e) {
  if ((delete e.model, delete e.fallbackModel, delete e.modelPicker, delete e.modelOverrides, e.env)) {
    let t = {};
    for (let [r, o] of Object.entries(e.env)) if (!jo.has(r.toUpperCase())) t[r] = o;
    e.env = t;
  }
}
function Cor(e) {
  let t = e.store.policy.pairedModelOverrides;
  if (t !== void 0) return t.value;
  try {
    Qwt("policySettings", e);
  } catch {}
  return e.store.policy.pairedModelOverrides?.value;
}
function vor(e) {
  if (!e.hostManagedProvider || Sqt(e).some((t) => t.modelPricing !== void 0)) return;
  return wXe(e).settings?.modelPricing;
}
function Ror(e) {
  let t = e.store.policy.hostToolSearchEnv;
  if (t !== void 0) return t.value;
  let r;
  if (e.hostManagedProvider) {
    let o = wXe(e).settings?.env ?? {};
    for (let [i, s] of Object.entries(o)) {
      if (i === "ENABLE_TOOL_SEARCH") {
        r = s;
        break;
      }
      if (r === void 0 && i.toUpperCase() === "ENABLE_TOOL_SEARCH") r = s;
    }
  }
  return (e.store.policy.hostToolSearchEnv = { value: r }), r;
}
function Sqt(e) {
  let t = e.store.policy.allTiers;
  if (t !== void 0) return t;
  let r = Bo(e);
  return (e.store.policy.allTiers = r), r;
}
function WH(e) {
  return e === "helper" || e === "plist" || e === "hklm" || e === "file";
}
function fe() {
  return D() === "macos" ? "plist" : "hklm";
}
function EXe(e) {
  let t = jY(e);
  if (t.composes === "tier") return "helper";
  let r = T(e);
  if (t.composes === "remoteSlot" || r.present.remote) return "remote";
  if (r.present.mdm) return fe();
  if (r.present.file) return "file";
  if (r.parentSlice || r.hostModelOverlay) return "parent";
  let o = e.hkcu?.();
  return o && Object.keys(o.settings).length > 0 ? "hkcu" : null;
}
function kor(e) {
  let t = jY(e),
    r = t.composes === "none" ? null : t.mergedOver,
    o = T(e),
    i = [];
  if (t.composes === "tier") i.push("helper");
  if (t.composes === "remoteSlot" || o.present.remote) i.push("remote");
  if (o.present.mdm) i.push(fe());
  if (o.present.file) i.push("file");
  let s = CEn(e) ?? [],
    [l, ...c] = i,
    d = c.filter((f) => f !== r && !s.includes(f)),
    u = e.hkcu?.();
  if (u && Object.keys(u.settings).length > 0 && (l || o.parentSlice)) d.push("hkcu");
  return d;
}
function T(e) {
  let t = [],
    { settings: r, errors: o } = o5(e);
  t.push(...o);
  let i = jY(e),
    s = i.composes === "remoteSlot" ? i.helper : de(r),
    { settings: l, errors: c } = ue(e);
  t.push(...c);
  let d = de(l),
    { settings: u, errors: f } = e.file?.() ?? ONe(e);
  t.push(...f);
  let g = de(u),
    { settings: y, errors: S } = wXe(e);
  t.push(...S);
  let p = [s, d, g].filter((A) => A !== null),
    O = (
      i.composes === "remoteSlot"
        ? i.helper
        : [r, l, u].find((A) => A !== null && (A.managedSourcesBehavior !== void 0 || pP(A)))
    )?.managedSourcesBehavior,
    { admin: w, merged: B } = Lo(p, O),
    oe = { remote: s !== null, mdm: d !== null, file: g !== null },
    ie = (A) => A !== null && (A === p[0] || Object.keys(Ul(A, Nt)).length > 0),
    Vt = { remote: ie(s), mdm: ie(d), file: ie(g) },
    $t = {
      allowManagedPermissionRulesOnly: p.some((A) => A.allowManagedPermissionRulesOnly === true) || void 0,
      forceLoginOrgUUID: w?.forceLoginOrgUUID,
      allowedMcpServers: w?.allowedMcpServers,
      availableModels: w?.availableModels,
      sandbox: {
        network: {
          allowManagedDomainsOnly: p.some((A) => A.sandbox?.network?.allowManagedDomainsOnly === true) || void 0,
        },
        filesystem: {
          allowManagedReadPathsOnly: p.some((A) => A.sandbox?.filesystem?.allowManagedReadPathsOnly === true) || void 0,
        },
      },
    },
    se = y && Rt(w) ? xo(y, $t) : null,
    Jt = se && Object.keys(se).length > 0 ? se : null,
    Bt = No(y, e.hostManagedProvider);
  return { tiers: p, admin: w, parentSlice: Jt, hostModelOverlay: Bt, errors: t, present: oe, merged: B, composed: Vt };
}
function Lo(e, t) {
  let r = e[0];
  if (!r) return { admin: null, merged: false };
  let { managedSourcesBehavior: o, ...i } = r;
  if (t !== "merge" || e.length < 2) return { admin: o === void 0 ? r : i, merged: false };
  let s = e.slice(1).map((c) => {
      let d = Ul(c, Nt);
      for (let f of To) if (I(d, f) !== void 0) J(d, f, void 0);
      let u = I(d.sandbox, ["enabledPlatforms"]);
      if (Array.isArray(u) && u.includes(D())) J(d, ["sandbox", "enabledPlatforms"], void 0);
      else if (u !== void 0) delete d.sandbox;
      return d;
    }),
    l = {};
  for (let c of [...s].reverse()) dP(l, c, Pt);
  return dP(l, i, Pt), Fo(l, [r, ...s]), Do(l, [r, ...s]), { admin: l, merged: true };
}
var It = ["managedSourcesBehavior", "wslInheritsWindowsSettings"];
function pP(e) {
  return Object.keys(e).some((t) => !It.includes(t));
}
function de(e) {
  return e && pP(e) ? e : null;
}
function Do(e, t) {
  let r = t.findIndex((i) => i.availableModels !== void 0),
    o = t.findIndex((i) => i.modelOverrides !== void 0);
  if (o !== -1 && (r === -1 || o <= r)) e.modelOverrides = { ...t[o].modelOverrides };
  else delete e.modelOverrides;
}
var Uo = ["allowedMcpServers", "availableModels", "strictKnownMarketplaces", "allowedChannelPlugins"];
function Pt(e, t, r) {
  if (r !== void 0 && Uo.includes(r)) {
    if (t === void 0) return e;
    return Array.isArray(t) ? [...t] : t;
  }
  if (Array.isArray(e) && Array.isArray(t) && r !== "fallbackModel") return te([...t, ...e]);
  return i5(e, t, r);
}
function Fo(e, t) {
  let r = t[0],
    o = e;
  for (let { path: i, restrictive: s } of Ct) {
    let l = Tt(s),
      c = Math.min(...t.map((d) => l.indexOf(I(d, i))).filter((d) => d !== -1));
    if (Number.isFinite(c)) J(o, i, l[c]);
    else if (I(r, i) === void 0 && I(e, i) !== void 0) J(o, i, void 0);
  }
  if (e.strictPluginOnlyCustomization !== true) {
    let i = te(
      t.flatMap((s) => (Array.isArray(s.strictPluginOnlyCustomization) ? s.strictPluginOnlyCustomization : [])),
    );
    if (i.length > 0) e.strictPluginOnlyCustomization = i;
    else if (r?.strictPluginOnlyCustomization === void 0) delete e.strictPluginOnlyCustomization;
  }
}
function CEn(e) {
  let t = e.store.policy.mergedSources;
  if (t !== void 0) return t.value;
  let r = Vo(e);
  return (e.store.policy.mergedSources = { value: r }), r;
}
function Vo(e) {
  if (jY(e).composes === "tier") return null;
  let { merged: t, composed: r } = T(e);
  if (!t) return null;
  let o = [];
  if (r.remote) o.push("remote");
  if (r.mdm) o.push(fe());
  if (r.file) o.push("file");
  return o;
}
function jY(e) {
  let t = e.helper?.() ?? null;
  if (!t) return { composes: "none" };
  let r = e.helperArmedFromRemote?.() === false ? "tier" : "remoteSlot";
  if (e.helperMergesOutput?.() !== true) return { composes: r, helper: t, mergedOver: null };
  if (!e.store.policy.mergedHelper) {
    let o, i;
    if (r === "remoteSlot") (o = o5(e).settings), (i = "remote");
    else if ((o = ue(e).settings)) i = D() === "macos" ? "plist" : "hklm";
    else (o = (e.file?.() ?? ONe(e)).settings), (i = "file");
    e.store.policy.mergedHelper = { helper: Jo(o, t), mergedOver: Object.keys(xt(o)).length > 0 ? i : null };
  }
  return { composes: r, ...e.store.policy.mergedHelper };
}
function xt(e) {
  let { policyHelper: t, policyHelpers: r, ...o } = e ?? {};
  return o;
}
function $o(e, t) {
  let r = new Map();
  for (let [i, s] of Object.entries(t)) {
    let l = i.toUpperCase();
    if (!r.has(l)) r.set(l, s);
  }
  let o = {};
  for (let [i, s] of Object.entries(e)) o[i] = r.get(i.toUpperCase()) ?? s;
  return Object.assign(o, t);
}
function Jo(e, t) {
  let r = xt(e),
    o = dP({}, r, t, (i, s, l) => (Array.isArray(s) ? [...s] : i5(i, s, l)));
  if (r.env && t.env) o.env = $o(r.env, t.env);
  return o;
}
function Bo(e) {
  let t = jY(e);
  if (t.composes === "tier") return [t.helper];
  let { tiers: r, parentSlice: o } = T(e);
  return o ? [...r, o] : r;
}
function Hor(e) {
  if (jY(e).composes === "tier") return false;
  return Rt(T(e).admin);
}
function xor(e) {
  let t = e.store.policy.durableTiers;
  if (t !== void 0) return t;
  let r = Ho(e);
  return (e.store.policy.durableTiers = r), r;
}
function Ho(e) {
  let t = jY(e);
  if (t.composes === "tier") return [t.helper];
  let { tiers: r, admin: o } = T(e);
  if (!o) {
    let i = e.hkcu?.();
    if (i && Object.keys(i.settings).length > 0) return [i.settings];
  }
  return r;
}
var jt = ["apiKeyHelper", "awsAuthRefresh", "awsCredentialExport", "gcpAuthRefresh"];
var Nt = [
  ...jt,
  "otelHeadersHelper",
  "proxyAuthHelper",
  "forceLoginOrgUUID",
  "forceLoginMethod",
  "forceLoginGatewayUrl",
  "parentSettingsBehavior",
  "env",
  "modelPicker",
  ...It,
];
function Ko(e) {
  return Vor.some((t) => e.startsWith(t)) || zor.includes(e);
}
function Lt(e, t, r) {
  let o = new Map(),
    i = new Map(),
    s = false;
  for (let [l, c] of e.entries()) {
    let d = new Set(),
      u = t?.[l] === true;
    for (let [f, g] of Object.entries(c ?? {})) {
      let y = f.toUpperCase(),
        S = Ko(y);
      if (S && g.trim() === "") continue;
      if (S && s) continue;
      if (S && l > 0 && t?.[l] === true) continue;
      let p = o.get(y);
      if (p !== void 0) {
        if (d.has(y)) i.set(f, g);
        else if (!i.has(f)) i.set(f, p);
        continue;
      }
      if (l > 0 && r?.[l]?.has(y)) continue;
      if ((i.set(f, g), d.add(y), o.set(y, g), S)) u = true;
    }
    if (u) s = true;
  }
  return Object.fromEntries(i);
}
var Wo = "CLAUDE_CODE_DISABLE_ADMIN_ENV_UNION";
function Ior(e, t) {
  let r = e.store.policy.adminTierEnvView;
  if (r !== void 0) return r[t];
  let o = Sqt(e),
    i = a.CLAUDE_CODE_DISABLE_ADMIN_ENV_UNION === true ? (o[0]?.env ?? {}) : Lt(o.map((l) => l.env)),
    s = {};
  for (let [l, c] of Object.entries(i)) {
    let d = l.toUpperCase();
    if (l === d || !(d in s)) s[d] = c;
  }
  return (e.store.policy.adminTierEnvView = s), s[t];
}
function _t(e) {
  if (!e) return;
  let t = {};
  for (let [r, o] of Object.entries(e)) if (r.toUpperCase() !== Wo) t[r] = o;
  return t;
}
function Go(e, t) {
  let r = Object.entries(e ?? {}),
    o = t ?? {};
  return r.length === Object.keys(o).length && r.every(([i, s]) => o[i] === s);
}
function zo(e) {
  let t = ue(e);
  return t.settings ? t.errors : [...t.errors, ...(e.file?.() ?? ONe(e)).errors];
}
function Dt(e) {
  let t = jY(e),
    r = e.helperWarnings?.() ?? [];
  if (t.composes === "tier") {
    let { helper: p } = t;
    e.store.lastPolicyEnvComposition = null;
    let v = e.hostManagedProvider ? { ...p } : p;
    if (e.hostManagedProvider) re(v);
    e.store.policy.pairedModelOverrides = {
      value: e.hostManagedProvider && p.availableModels !== void 0 ? p.modelOverrides : void 0,
    };
    let O = (e.store.policy.helperBaseStatusNotices ??= [
      ...(e.helperMergesOutput?.() === true ? zo(e).filter((w) => !w.statusOnly) : []),
      ...T(e).errors.filter((w) => w.statusOnly),
    ]);
    return { settings: v, errors: [...O, ...r] };
  }
  let { tiers: o, admin: i, parentSlice: s, hostModelOverlay: l, errors: c, present: d } = T(e);
  if ((c.push(...r), !i && !s)) {
    (e.store.lastPolicyEnvComposition = null), (e.store.policy.pairedModelOverrides = { value: void 0 });
    let p = e.hkcu?.();
    if (p && Object.keys(p.settings).length > 0) {
      let v = e.hostManagedProvider ? { ...p.settings } : p.settings;
      if (e.hostManagedProvider) {
        if ((re(v), l)) Object.assign(v, l);
      }
      return { settings: v, errors: [...c, ...p.errors] };
    }
    if (l) return { settings: { ...l }, errors: [...c, ...(p?.errors ?? [])] };
    return { settings: null, errors: [...c, ...(p?.errors ?? [])] };
  }
  let u = dP({}, s ?? {}, i ?? {}, i5);
  if (o.some((p) => p.forceRemoteSettingsRefresh === true)) u.forceRemoteSettingsRefresh = true;
  let f = a.CLAUDE_CODE_DISABLE_ADMIN_ENV_UNION === true,
    g = u.env;
  if (!f) {
    let p = Lt(
      o.map((v) => v.env),
      o.map((v) => (v.otelHeadersHelper ?? "").trim() !== ""),
      o.map((v) => {
        let O = new Set();
        for (let w of jt) {
          let B = v[w];
          if (typeof B === "string" && B.trim() !== "") for (let oe of Kor[w]) O.add(oe);
        }
        return O.size > 0 ? O : void 0;
      }),
    );
    if (Object.keys(p).length > 0) u.env = p;
    else delete u.env;
  }
  if (u.env) {
    let p = _t(u.env);
    if (p && Object.keys(p).length > 0) u.env = p;
    else delete u.env;
  }
  if (e.hostManagedProvider) {
    if ((re(u), l)) Object.assign(u, l);
  }
  let y = { env: _t(g) };
  if (e.hostManagedProvider) re(y);
  let S = !f && !Go(u.env, y.env);
  return (
    (e.store.lastPolicyEnvComposition = {
      unionOptedOut: f,
      unionChangedEnv: S,
      remoteTierPresent: d.remote,
      mdmTierPresent: d.mdm,
      fileTierPresent: d.file,
      adminTierCount: o.length,
      tiersWithEnv: Q(o, (p) => Object.keys(p.env ?? {}).length > 0),
    }),
    (e.store.policy.pairedModelOverrides = {
      value:
        e.hostManagedProvider &&
        i?.availableModels !== void 0 &&
        i.modelOverrides !== void 0 &&
        l?.availableModels === void 0
          ? i.modelOverrides
          : void 0,
    }),
    { settings: u, errors: c }
  );
}
function bqt(e, t, { includeLegacyLocalSettings: r = true } = {}) {
  if (e === "policySettings") return Dt(t).settings;
  let o = BY(e, t),
    { settings: i } = o ? r5(o, t.store, e === "flagSettings" ? t.flagExpectedContent : void 0) : { settings: null };
  if (e === "flagSettings") {
    let { settings: s } = Ywt(t);
    if (s) return dP(i || {}, s, i5);
  }
  if (e === "localSettings" && r) {
    let s = XAe(t);
    if (s) {
      let { settings: l } = r5(s, t.store);
      if (l) return t.onLegacyLocalSettingsRead?.("per_source"), dP(l, i || {}, i5);
    }
  }
  return i;
}
function AXe(e, t) {
  return { ...e, ...t };
}
function i5(e, t, r) {
  if (r === "modelPicker" && t !== void 0) return Ft(t);
  if (Array.isArray(e) && Array.isArray(t)) {
    if (r === "fallbackModel") return t;
    return te([...e, ...t]);
  }
  if (r === "extraKnownMarketplaces" && He(e) && He(t)) return AXe(e, t);
  return;
}
function Ft(e) {
  if (!He(e)) return e;
  let t = e.options;
  return { ...e, ...(Array.isArray(t) && { options: t.map((r) => (He(r) ? { ...r } : r)) }) };
}
function vEn(e) {
  if (e.store.isLoadingFromDisk) return { settings: {}, errors: [] };
  let t = Date.now();
  Y("info", "settings_load_started"), (e.store.isLoadingFromDisk = true);
  try {
    let r = e.store.pluginBase,
      o = {};
    if (r) o = dP(o, r, i5);
    let i = [],
      s = new Set(),
      l = new Set(),
      c = (u) => {
        for (let f of u) {
          let g = `${f.file}:${f.path}:${f.message}`;
          if (!s.has(g)) s.add(g), i.push(f);
        }
      },
      d = null;
    for (let u of Kwt(e)) {
      if (u === "policySettings") {
        let { settings: g, errors: y } = Dt(e);
        if (((d = g), g)) o = dP(o, g, i5);
        c(y);
        continue;
      }
      if (u === "localSettings") {
        let g = XAe(e);
        if (g && !l.has(_(g))) {
          l.add(_(g));
          let { settings: y, errors: S } = r5(g, e.store);
          if ((c(S), y)) e.onLegacyLocalSettingsRead?.("cascade"), (o = dP(o, y, i5));
        }
      }
      let f = BY(u, e);
      if (f) {
        let g = _(f),
          y = u === "flagSettings" && e.flagExpectedContent !== void 0;
        if (!l.has(g) || y) {
          l.add(g);
          let { settings: S, errors: p } = r5(f, e.store, u === "flagSettings" ? e.flagExpectedContent : void 0);
          if ((c(p), S)) o = dP(o, S, i5);
        }
      }
      if (u === "flagSettings") {
        let { settings: g, errors: y } = Ywt(e);
        if ((c(y), g)) o = dP(o, g, i5);
      }
    }
    if (d) {
      if (d.availableModels !== void 0) o.availableModels = [...d.availableModels];
      if (d.enforceAvailableModels !== void 0) o.enforceAvailableModels = d.enforceAvailableModels;
      if (d.modelPicker !== void 0) o.modelPicker = Ft(d.modelPicker);
    }
    return (
      Y("info", "settings_load_completed", {
        duration_ms: Date.now() - t,
        source_count: l.size,
        error_count: i.length,
      }),
      { settings: o, errors: i }
    );
  } finally {
    e.store.isLoadingFromDisk = false;
  }
}
function Yo(e) {
  let t = e.store.mergedSettings;
  if (t !== null) return t;
  let r = vEn(e);
  return (e.store.mergedSettings = r), r;
}
function Zo(e) {
  let { settings: t } = Yo(e);
  return t || {};
}
function Por(e) {
  e.store.invalidateAll();
  let t = [];
  for (let r of Kwt(e)) {
    let o = Qwt(r, e);
    if (o && Object.keys(o).length > 0) t.push({ source: r, settings: o });
  }
  return { effective: Zo(e), sources: t };
}
function Dor(e, t) {
  let r = Kwt(t);
  for (let o = r.length - 1; o >= 0; o--) {
    let i = r[o];
    if (Qwt(i, t)?.[e] !== void 0) return i;
  }
  return null;
}
export {
  yXe,
  mqt,
  dP,
  hN,
  bor,
  wor,
  Is,
  yw,
  Gwt,
  SXe,
  Tor,
  Eor,
  xi,
  _o,
  UH,
  BH,
  GAe,
  ib,
  _En,
  zwt,
  bO,
  DNe,
  Ul,
  bXe,
  yEn,
  gqt,
  Vwt,
  SEn,
  hqt,
  bEn,
  _qt,
  wEn,
  zAe,
  Kwt,
  ONe,
  LNe,
  Xwt,
  TEn,
  r5,
  VAe,
  o5,
  EEn,
  ioe,
  wXe,
  Ywt,
  sb,
  TXe,
  MNe,
  KAe,
  soe,
  Jwt,
  Y6,
  AEn,
  Aor,
  Y3,
  BY,
  jH,
  XAe,
  Qwt,
  yqt,
  Cor,
  vor,
  Ror,
  Sqt,
  WH,
  EXe,
  kor,
  pP,
  CEn,
  jY,
  Hor,
  xor,
  Ior,
  bqt,
  AXe,
  i5,
  vEn,
  Por,
  Dor,
};
