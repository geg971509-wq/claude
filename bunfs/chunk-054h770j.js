...
import { LW, pce, Jc, p2, V7, eM, X7, xPe, Qqe, ebe, ute } from "/$bunfs/root/chunk-zze8764r.js";
...
import "/$bunfs/root/chunk-m2tvdtdb.js";
...
import { ttt, ntt } from "/$bunfs/root/chunk-4jqxf63q.js";
...
function G(s) {
  switch (s.resultType) {
    case "sessionNotFound":
      return `Session ${ae.bold(s.arg)} was not found.`;
    case "multipleMatches":
      return `Found ${s.count} sessions matching ${ae.bold(s.arg)}. Please use /resume to pick a specific session.`;
  }
}
function j(je) {
  let q = _(7),
  ...
  let te;
  if (q[4] !== U || q[5] !== D)
    (te = r(o, { flexDirection: "column", children: [U, D] })), (q[4] = U), (q[5] = D), (q[6] = te);
  else te = q[6];
  return te;
}
function se({ onDone: s, onResume: d }) {
  ...
  async function Z(m) {
    ...
    let R, p;
    try {
      if (((R = p2(m) ? await V7(m, { storageV5: P }) : m), S.current)) return;
      if (((p = await ntt(R, n, g)), S.current)) return;
      if (p) {
        let y = await h_(p);
        if (S.current) return;
      ...
      if (S.current) return;
      h(ft(we(y), "resume command: handleSelect failed")), s("Failed to resume conversation");
      return;
    }
    if (p) {
      let y = [
...
function Q(s, d) {
  return s.filter((v) => !v.isSidechain && Jc(v) !== d);
}
var Fe = async (s, d, v) => {
  let T = async (a, n, c) => {
      let f = await uq(a);
      ...
      try {
        await d.resume?.(a, n, c), s(void 0, { display: "skip" });
      } catch (I) {
        h(ft(we(I), "resume: context.resume failed")), s(`Failed to resume: ${l(I)}`);
      }
    },
    g = v?.trim();
  if (!g) return e(se, { onDone: s, onResume: T }, Date.now());
  let O = await LW(Se()),
    b = await Qqe(O, void 0, void 0, d.storageV5);
  if (b.length === 0)
    return e(j, {
  ...
  let w = Kr(g);
  if (w) {
    let a = b.filter((c) => Jc(c) === w).sort((c, f) => f.modified.getTime() - c.modified.getTime());
    if (a.length > 0) {
      let c = a[0],
        f = p2(c) ? await V7(c, { storageV5: d.storageV5 }) : c;
      return T(w, f, "slash_command_session_id"), null;
    }
    let n = await X7(w, void 0, d.storageV5);
  ...
  if (pce()) {
    ...
    if (a.length === 1) {
      let n = a[0],
        c = Kr(Jc(n));
      if (c) {
        let f = p2(n) ? await V7(n, { storageV5: d.storageV5 }) : n;
        return T(c, f, "slash_command_title"), null;
      }
    }
    if (a.length > 1) {
      let n = G({ resultType: "multipleMatches", arg: g, count: a.length });
      return e(j, { message: n, args: g, onDone: () => s(n) });
    }
  }
  let M = G({ resultType: "sessionNotFound", arg: g });
  return e(j, { message: M, args: g, onDone: () => s(M) });
};
export { Fe as call, Q as filterResumableSessions };
