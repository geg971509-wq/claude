...
var j = m(() => f({ projectId: i(), writes: H(i()), deletes: H(i()), localDir: i().optional() })),
...
function F(e) {
  return e.length > 0 && b(e).length <= vWe && !u(e);
}
var LZ = new Set(["update_sharing", "add_member", "update_member_role"]);
function MZ(e, t) {
  let n = a(t);
...
function w6n(e, t, n, o = Date.now()) {
  ...
  return (
    e.approvedPlans.set(t, {
      ...
      writes: new Set(n.writes.map(Vw)),
...
function C(e) {
  if (u(e)) return `rejected character in path: ${e}`;
  if (Yst(e)) return `reserved path: ${e}`;
  if (
    Vw(e)
      .toLowerCase()
      .split("/")
...
function RWe(e, t) {
  ...
  for (let l of r) {
    let c = C(l);
    if (c !== null) return { outcome: "ask", reason: c, targets: r };
  }
  return { outcome: "pass", targets: r, set: s };
}
function T6n(e, t, n, o = Date.now()) {
  ...
  if (o >= l.expiresAt) return e.approvedPlans.delete(r), { outcome: "ask", reason: "approved plan expired" };
  if (l.projectId !== s) return { outcome: "ask", reason: "plan_token is for a different project" };
  let c = RWe(t, n);
  if (c.outcome !== "pass") return { outcome: "ask", reason: c.reason };
  let w = c.set === "writes" ? l.writes : l.deletes;
  for (let d of c.targets) if (!w.has(Vw(d))) return { outcome: "ask", reason: `path not in approved plan: ${d}` };
