...
var Wvn = S(function (Ge, B) {
  ...
  B.exports = function (e, t) {
    t = t || {};
    var r = typeof e;
    if (r === "string" && e.length > 0) return se(e);
    else if (r === "number" && isFinite(e)) return t.long ? oe(e) : ne(e);
  ...
  function ne(e) {
    ...
    if (t >= x) return Math.round(e / x) + "m";
    if (t >= y) return Math.round(e / y) + "s";
    return e + "ms";
  }
  function oe(e) {
    var t = Math.abs(e);
    if (t >= h) return v(e, t, h, "day");
    if (t >= b) return v(e, t, b, "hour");
    if (t >= x) return v(e, t, x, "minute");
    if (t >= y) return v(e, t, y, "second");
    return e + " ms";
  }
  function v(e, t, r, s) {
    var u = t >= r * 1.5;
    return Math.round(e / r) + " " + s + (u ? "s" : "");
  }
});
var _ = S(function (Pe, j) {
  function ie(e) {
    (r.debug = r),
      (r.default = r),
      (r.coerce = H),
    ...
    function t(n) {
      let o = 0;
      for (let i = 0; i < n.length; i++) (o = (o << 5) - o + n.charCodeAt(i)), (o |= 0);
    ...
    function r(n) {
      ...
      function l(...f) {
        ...
        let d = l,
          O = Number(new Date()),
          Q = O - (o || O);
        if (((d.diff = Q), (d.prev = o), (d.curr = O), (o = O), (f[0] = r.coerce(f[0])), typeof f[0] !== "string"))
          f.unshift("%O");
        let w = 0;
        (f[0] = f[0].replace(/%([a-zA-Z%])/g, (M, X) => {
          if (M === "%%") return "%";
          w++;
          let k = r.formatters[X];
          if (typeof k === "function") {
            let ee = f[w];
            (M = k.call(d, ee)), f.splice(w, 1), w--;
          }
          return M;
        })),
          r.formatArgs.call(d, f),
          (d.log || r.log).apply(d, f);
      }
      if (
        ((l.namespace = n),
        (l.useColors = r.useColors()),
        (l.color = r.selectColor(n)),
        (l.extend = s),
        (l.destroy = r.destroy),
    ...
    function s(n, o) {
      let i = r(this.namespace + (typeof o > "u" ? ":" : o) + n);
      return (i.log = this.log), i;
    }
    function u(n) {
      r.save(n), (r.namespaces = n), (r.names = []), (r.skips = []);
      let o = (typeof n === "string" ? n : "").trim().replace(" ", ",").split(",").filter(Boolean);
      for (let i of o)
        if (i[0] === "-") r.skips.push(i.slice(1));
        else r.names.push(i);
    }
    function C(n, o) {
      let i = 0,
    ...
    function F() {
      let n = [...r.names, ...r.skips.map((o) => "-" + o)].join(",");
      return r.enable(""), n;
    }
    function Z(n) {
...
var q = S(function (D, E) {
  ...
  function le(e) {
    try {
      if (e) D.storage.setItem("debug", e);
      else D.storage.removeItem("debug");
    } catch (t) {}
  }
  function fe() {
    let e;
    try {
      e = D.storage.getItem("debug");
    } catch (t) {}
...
var Hzt = S(function (Ye, G) {
  G.exports = (e, t = process.argv) => {
...
var Y = S(function ($e, U) {
  var ze = ue("os"),
    P = ue("tty"),
    p = Hzt(),
    { env: c } = process,
    I;
  if (p("no-color") || p("no-colors") || p("color=false") || p("color=never")) I = 0;
  else if (p("color") || p("colors") || p("color=true") || p("color=always")) I = 1;
  function be() {
    if ("FORCE_COLOR" in c) {
      if (c.FORCE_COLOR === "true") return 1;
      if (c.FORCE_COLOR === "false") return 0;
  ...
  function we(e, { streamIsTTY: t, sniffFlags: r = !0 } = {}) {
    ...
    if (u === 0) return 0;
    if (r) {
      if (p("color=16m") || p("color=full") || p("color=truecolor")) return 3;
      if (p("color=256")) return 2;
    }
    if (e && !t && u === void 0) return 0;
    let C = u || 0;
    if (c.TERM === "dumb") return C;
    if ("CI" in c) {
      if (
        ["TRAVIS", "CIRCLECI", "APPVEYOR", "GITLAB_CI", "GITHUB_ACTIONS", "BUILDKITE", "DRONE"].some((F) => F in c) ||
        c.CI_NAME === "codeship"
      )
        return 1;
      return C;
    ...
    if ("TERM_PROGRAM" in c) {
      let F = Number.parseInt((c.TERM_PROGRAM_VERSION || "").split(".")[0], 10);
  ...
  function L(e, t = {}) {
    let r = we(e, { streamIsTTY: e && e.isTTY, ...t });
    return Oe(r);
  }
  U.exports = { supportsColor: L, stdout: L({ isTTY: P.isatty(1) }), stderr: L({ isTTY: P.isatty(2) }) };
});
var W = S(function (V, A) {
  var ve = ue("tty"),
    T = ue("util");
  V.init = _e;
  V.log = Te;
  ...
  function Re() {
    return "colors" in V.inspectOpts ? Boolean(V.inspectOpts.colors) : ve.isatty(process.stderr.fd);
  }
  function Ee(e) {
...
` + C,
        ...
        e.push(u + "m+" + A.exports.humanize(this.diff) + "\x1B[0m");
    } else e[0] = Ie() + t + " " + e[0];
  }
  function Ie() {
    if (V.inspectOpts.hideDate) return "";
    return new Date().toISOString() + " ";
  }
  function Te(...e) {
    return process.stderr.write(
      T.formatWithOptions(V.inspectOpts, ...e) +
        `
`,
    );
  }
