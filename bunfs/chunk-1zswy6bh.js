...
// Version: 2.1.252
import { bn, kd, qf, xP } from "/$bunfs/root/chunk-vfy57cpd.js";
...
function Q(e, t) {
  ...
  let n = { error: w("error", e, t), warn: w("warn", e, t), info: w("info", e, t), debug: w("debug", e, t) };
...
var X = (e, t) =>
  ...
  D = async (e, t) => {
    ...
    if (t.awsAccessKey && t.awsSecretAccessKey)
      ...
      let f = await (t.providerChainResolver ? t.providerChainResolver() : X(t.awsProfile, t.logger));
      try {
        r = await f();
      } catch (l) {
        throw new kd({ message: "Failed to resolve AWS credentials from the credential provider chain.", cause: I(l) });
      }
    }
    let n = new F.SignatureV4({ service: t.serviceName, region: t.regionName, credentials: r, sha256: Rxe }),
      o = new URL(t.url),
      i = !e.headers
        ? {}
...
class U extends xP {
  constructor({
    ...
    let K = n != null && o != null,
      ...
      h;
    if (B) h = r;
    else if (!K && !W) h = c("ANTHROPIC_AWS_API_KEY") ?? void 0;
  ...
  validateHeaders() {}
  backendMiddleware() {
    return [async (e, t) => t(await Z(this, _, "m", H).call(this, e))];
  }
}
(_ = new WeakSet()),
  (H = async function (t) {
    if (this.skipAuth || !this._useSigV4) return t;
    if (!this.awsRegion) throw P();
    let r = await D(t, {
      url: t.url,
...
export { U as AnthropicAws, qf as BaseAnthropic, U as default };
