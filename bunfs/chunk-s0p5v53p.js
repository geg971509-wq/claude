// @bun @bytecode
// Claude Code is a Beta product per Anthropic's Commercial Terms of Service.
// By using Claude Code, you agree that all code acceptance or rejection decisions you make,
// and the associated conversations in context, constitute Feedback under Anthropic's Commercial Terms,
// and may be used to improve Anthropic's products, including training models.
// You are responsible for reviewing any code suggestions before use.

// (c) Anthropic PBC. All rights reserved. Use is subject to the Legal Agreements outlined here: https://code.claude.com/docs/en/legal-and-compliance.

// Version: 2.1.252
import { _ } from "/$bunfs/root/chunk-haan6f8z.js";
import { My, Ua, Ui, T8, dxe } from "/$bunfs/root/chunk-8ktvd0rh.js";
import { Ee } from "/$bunfs/root/chunk-rp246vnd.js";
import { Ab } from "/$bunfs/root/chunk-78v32s6b.js";
import { Xf } from "/$bunfs/root/chunk-ngn87vcm.js";
import { o, t, pxe, HS } from "/$bunfs/root/chunk-he2phymk.js";
import { ht } from "/$bunfs/root/chunk-ek4tmwbt.js";
import { Dc } from "/$bunfs/root/chunk-1ckq10h3.js";
import { e, r } from "/$bunfs/root/chunk-8s7kcbyw.js";
import { yn, B, We, qn, z, C, u, F } from "/$bunfs/root/chunk-twm95mhz.js";
import { se } from "/$bunfs/root/chunk-q1fyd2qk.js";
import { d } from "/$bunfs/root/chunk-yz031c9r.js";
F();
var x = yn({
  selectedTab: void 0,
  width: void 0,
  headerFocused: false,
  focusHeader: () => {},
  blurHeader: () => {},
  registerOptIn: () => () => {},
});
function Pg({
  title: i,
  color: l,
  defaultTab: f,
  children: c,
  hidden: a,
  useFullWidth: p,
  selectedTab: v,
  onTabChange: R,
  banner: D,
  disableNavigation: b,
  initialHeaderFocused: O = true,
  contentHeight: q,
  navFromContent: ge = false,
}) {
  let { columns: J } = Ee(),
    h = c.map((n) => [n.props.id ?? n.props.title, n.props.title]),
    Q = f ? h.findIndex((n) => f === n[0]) : 0,
    A = v !== void 0,
    [ye, Te] = u(Q !== -1 ? Q : 0),
    X = A ? h.findIndex((n) => n[0] === v) : -1,
    I = A ? (X !== -1 ? X : 0) : ye,
    E = We(My),
    k = T8(),
    g = dxe(),
    P = C(null),
    [Z, Se] = u(0);
  qn(() => {
    let n = P.current ? HS(P.current).height : 0;
    if (n !== Z) Se(n);
  });
  let K = (a ? 0 : 2) + Z;
  qn(() => {
    if (!g) return;
    return g(K), () => g(null);
  }, [g, K]);
  let { rows: Re } = Ui({ rows: 0, columns: 0 }),
    M = g !== null && k !== null,
    Ce = M ? Re - K : void 0,
    De = z(() => (E && M ? { ...E, claimScrollBox: null } : null), [E, M]),
    s = false,
    W = C(null),
    { focus: ee, focusDirection: Ie, blur: te } = pxe(),
    [y, w] = u(O),
    ne = B(() => {
      if (s && W.current) ee(W.current);
      w(true);
    }, [s, ee]),
    Be = B(() => {
      if (s) te();
      w(false);
    }, [s, te]),
    [Fe, oe] = u(0),
    Me = B(() => (oe((n) => n + 1), () => oe((n) => n - 1)), []),
    T = Fe > 0,
    re = y || !T,
    ie = (n) => {
      let m = h[n]?.[0];
      if (A && R && m) R(m);
      else Te(n);
      ne();
    },
    H = (n) => {
      ie((I + h.length + n) % h.length);
    };
  ht({ "tabs:next": () => H(1), "tabs:previous": () => H(-1) }, { context: "Tabs", isActive: !a && !b && re });
  let ae = (n) => {
    if (a || b) return;
    if (s) {
      if (!y) {
        if (n.key === "left" || n.key === "right" || n.key === "tab" || (T && (n.key === "up" || n.key === "down")))
          n.preventDefault();
        return;
      }
      if (n.key === "left" || n.key === "right" || n.key === "tab") n.preventDefault();
      else if (n.key === "down" && T) n.preventDefault(), Ie("down"), w(false);
      return;
    }
    if (!T) return;
    if (n.key === "up" || n.key === "down") {
      if ((n.preventDefault(), y && n.key === "down")) w(false);
    }
  };
  ht(
    { "tabs:next": () => H(1), "tabs:previous": () => H(-1) },
    { context: "Tabs", isActive: !s && ge && !y && T && !a && !b },
  );
  let He = i ? se(i) + 1 : 0,
    Oe = h.reduce((n, [, m]) => n + (m ? se(m) : 0) + 2 + 1, 0),
    Ae = He + Oe,
    le = p ? Math.max(0, J - Ae) : 0,
    N = p ? J : void 0;
  return e(x.Provider, {
    value: { selectedTab: h[I][0], width: N, headerFocused: y, focusHeader: ne, blurHeader: Be, registerOptIn: Me },
    children: r(o, {
      flexDirection: "column",
      tabIndex: s ? void 0 : 0,
      autoFocus: s ? void 0 : O,
      onKeyDown: s ? void 0 : ae,
      flexShrink: k ? 0 : void 0,
      children: [
        !a &&
          r(o, {
            ref: s ? W : void 0,
            tabIndex: s ? 0 : void 0,
            autoFocus: s ? O : void 0,
            onFocus: s ? () => w(true) : void 0,
            onBlur: s ? () => w(false) : void 0,
            onKeyDown: s ? ae : void 0,
            flexDirection: "row",
            gap: 1,
            flexShrink: k ? 0 : void 0,
            alignSelf: s && !p ? "flex-start" : void 0,
            children: [
              i !== void 0 && e(t, { bold: true, color: l, children: i }),
              h.map(([n, m], de) =>
                e(
                  pnt,
                  {
                    title: m,
                    isCurrent: I === de,
                    headerFocused: re && !b,
                    color: l,
                    onClick: b ? void 0 : () => ie(de),
                  },
                  n,
                ),
              ),
              le > 0 && e(t, { children: " ".repeat(le) }),
            ],
          }),
        D != null && e(o, { ref: P, flexDirection: "column", flexShrink: 0, children: D }),
        M
          ? e(o, {
              width: N,
              marginTop: a ? 0 : 1,
              flexShrink: 0,
              children: e(
                Ab,
                {
                  ref: k.attach,
                  flexDirection: "column",
                  flexShrink: 0,
                  maxHeight: Ce,
                  stickyScroll: false,
                  children: e(My, { value: De, children: c }),
                },
                I,
              ),
            })
          : e(o, {
              width: N,
              marginTop: a ? 0 : 1,
              height: q,
              overflowY: q !== void 0 ? "hidden" : void 0,
              flexShrink: k ? 0 : void 0,
              children: c,
            }),
      ],
    }),
  });
}
function pnt(at) {
  let G = _(15),
    { title: V, isCurrent: S, headerFocused: Pe, color: j, onClick: L } = at,
    [ce, Ke] = u(false),
    ue = L !== void 0;
  const fe = S && Pe;
  let Ne;
  if (G[0] !== fe) (Ne = { line: 0, column: 1, active: fe }), (G[0] = fe), (G[1] = Ne);
  else Ne = G[1];
  let he = Xf(Ne),
    pe = j && S && Pe,
    Ve,
    je;
  if (G[2] === d) (Ve = () => Ke(true)), (je = () => Ke(false)), (G[2] = Ve), (G[3] = je);
  else (Ve = G[2]), (je = G[3]);
  let U;
  if (G[4] !== ue || G[5] !== j || G[6] !== pe || G[7] !== ce || G[8] !== S || G[9] !== V)
    (U = pe
      ? e(Dc, { color: j, bold: true, padded: true, children: V })
      : r(t, { inverse: S, bold: S, underline: ce && ue, children: [" ", V, " "] })),
      (G[4] = ue),
      (G[5] = j),
      (G[6] = pe),
      (G[7] = ce),
      (G[8] = S),
      (G[9] = V),
      (G[10] = U);
  else U = G[10];
  let ze;
  if (G[11] !== he || G[12] !== L || G[13] !== U)
    (ze = e(o, { ref: he, onClick: L, onMouseEnter: Ve, onMouseLeave: je, children: U })),
      (G[11] = he),
      (G[12] = L),
      (G[13] = U),
      (G[14] = ze);
  else ze = G[14];
  return ze;
}
function Zi(lt) {
  let pt = _(4),
    { title: dt, id: ct, children: be } = lt,
    { selectedTab: ut, width: me } = We(x),
    ft = Ua();
  if (ut !== (ct ?? dt)) {
    return null;
  }
  const xe = ft ? 0 : void 0;
  let Le;
  if (pt[0] !== be || pt[1] !== xe || pt[2] !== me)
    (Le = e(o, { width: me, flexShrink: xe, children: be })), (pt[0] = be), (pt[1] = xe), (pt[2] = me), (pt[3] = Le);
  else Le = pt[3];
  return Le;
}
function tke() {
  let { width: bt } = We(x);
  return bt;
}
function bf() {
  let Ge = _(6),
    { headerFocused: we, focusHeader: ve, blurHeader: ke, registerOptIn: Y } = We(x),
    Ue;
  if (Ge[0] !== Y) (Ue = [Y]), (Ge[0] = Y), (Ge[1] = Ue);
  else Ue = Ge[1];
  qn(Y, Ue);
  let Ye;
  if (Ge[2] !== ke || Ge[3] !== ve || Ge[4] !== we)
    (Ye = { headerFocused: we, focusHeader: ve, blurHeader: ke }),
      (Ge[2] = ke),
      (Ge[3] = ve),
      (Ge[4] = we),
      (Ge[5] = Ye);
  else Ye = Ge[5];
  return Ye;
}
export { Pg, pnt, Zi, tke, bf };
