...
import { et, Xm, B4 } from "/$bunfs/root/chunk-f9h0bg01.js";
...
import { aD, ok, bv, tSe, nSe, wD, pUt, nbe, yce, YA, aT } from "/$bunfs/root/chunk-zze8764r.js";
...
import { adn } from "/$bunfs/root/chunk-wn1jrcrk.js";
...
function M(go) {
  ...
  let N;
  if (U[4] !== u || U[5] !== y)
    (N = r(o, { flexDirection: "column", children: [u, y] })), (U[4] = u), (U[5] = y), (U[6] = N);
  else N = U[6];
  return N;
}
async function co(m, i, c) {
  let C = (c ?? "").trim(),
    ...
    D = async (s, h = false) => {
      ...
      let P = Xm();
      if (!P.includes(s)) B4([...P, s]), YA(i.session), ok(i.session, "directory_added"), aT(), aD(), wD(), bv.emit();
      pt.refreshConfig(), i.storageV5 ? pUt(s, i.storageV5) : pUt(s), adn("--add-dir", s, i.storageV5);
      let g;
      if (h)
        try {
          await p3(A, i.storageV5), (g = `Added ${ae.bold(s)} as a working directory and saved to local settings`);
        } catch (a) {
          g = `Added ${ae.bold(s)} as a working directory. Failed to save to local settings: ${a instanceof Error ? a.message : "Unknown error"}`;
        }
      else g = `Added ${ae.bold(s)} as a working directory for this session`;
      let b = `${g} ${ae.dim("\xB7 /permissions to manage")}`;
      m(b);
      let F = H();
      nbe(i.session, s, "slash_command", { storageV5: i.storageV5, credentials: i.credentials })
        .then(async ({ results: a, systemMessages: V }) => {
          for (let d of a)
            if (!d.succeeded && d.output) n(`DirectoryAdded hook failed: ${d.output}`, { level: "error" });
          let f = Q(a, (d) => !d.succeeded),
            v = [
              ...(await Promise.all(
                V.map((d, R) =>
                  yce(d, `add-dir-${F}-${R}`, "systemMessage", { storageV5: i.storageV5 }).then(
                    (I) => `DirectoryAdded hook: ${I}`,
                  ),
                ),
              )),
              ...(f > 0
                ? [`${f} DirectoryAdded ${k(f, "hook")} failed; output is in the debug log, not shown here`]
                : []),
            ];
          if (v.length > 0)
...
`),
  ...
  if (!C)
    return e(mge, {
      ...
      onAddDirectory: D,
      onCancel: () => {
        m("Did not add a working directory.");
      },
    });
  let l = await tSe(C, p);
  if (l.resultType !== "success") {
    let s = nSe(l);
    return e(M, { message: s, args: c ?? "", onDone: () => m(s) });
  }
  return e(mge, {
    ...
    onAddDirectory: D,
    onCancel: () => {
      m(`Did not add ${ae.bold(l.absolutePath)} as a working directory.`);
    },
  });
}
export { co as call };
