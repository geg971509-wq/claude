...
import { BDe, hwe, ngt, _we, ywe } from "/$bunfs/root/chunk-btcxpqw5.js";
...
var u = async (l, o) => {
  ...
  if (e === "") {
    ...
    let r = t.iterations === 0 ? "not yet evaluated" : `${t.iterations} ${k(t.iterations, "turn")}`,
      n = t.lastReason
        ? `
Last check: ${wr(t.lastReason.trim())}`
        : "";
  ...
  if (e.length > BDe)
    return (
      g("goal_set", "too_long"),
      { type: "text", value: `Goal condition is limited to ${BDe} characters (got ${e.length})` }
    );
  let a = _we(e, o);
  if (a !== null) return { type: "text", value: a };
  return { type: "query", value: `Goal set: ${e}`, prompt: ngt(e) };
};
export { u as call };
