// @bun @bytecode
// Claude Code is a Beta product per Anthropic's Commercial Terms of Service.
// By using Claude Code, you agree that all code acceptance or rejection decisions you make,
// and the associated conversations in context, constitute Feedback under Anthropic's Commercial Terms,
// and may be used to improve Anthropic's products, including training models.
// You are responsible for reviewing any code suggestions before use.

// (c) Anthropic PBC. All rights reserved. Use is subject to the Legal Agreements outlined here: https://code.claude.com/docs/en/legal-and-compliance.

// Version: 2.1.252
import { wS, Ue } from "/$bunfs/root/chunk-qq1mdtb5.js";
import { R, l, E, uo, X, Zd } from "/$bunfs/root/chunk-ypdw393e.js";
import { J, Mn, G, K, Se, SQe, RR, $n } from "/$bunfs/root/chunk-f9h0bg01.js";
import { ee } from "/$bunfs/root/chunk-4fwj3vnx.js";
import { su, vt, le, n } from "/$bunfs/root/chunk-fv016jr6.js";
import { k, St } from "/$bunfs/root/chunk-4ddxwr9r.js";
import { $C } from "/$bunfs/root/chunk-fec4384a.js";
import { h } from "/$bunfs/root/chunk-wkxx62a2.js";
import { cn } from "/$bunfs/root/chunk-cf8qhmdc.js";
import { y, p, g } from "/$bunfs/root/chunk-ca80fke8.js";
import { $e, qe } from "/$bunfs/root/chunk-2masxyqj.js";
import { Cm, Wu } from "/$bunfs/root/chunk-f5hrzy3k.js";
import { an } from "/$bunfs/root/chunk-c47snwm2.js";
import { Zt } from "/$bunfs/root/chunk-jpf4kat5.js";
import { Y } from "/$bunfs/root/chunk-s7e0px8j.js";
import { vw } from "/$bunfs/root/chunk-1hh0bg14.js";
import { vs, qo } from "/$bunfs/root/chunk-3s6zpzqb.js";
import { Ft } from "/$bunfs/root/chunk-ef7xmm9b.js";
import { Ao } from "/$bunfs/root/chunk-czmxr2ps.js";
import { cd } from "/$bunfs/root/chunk-9xb90qzc.js";
import { tr, Bn, HR, Sc, _r } from "/$bunfs/root/chunk-56sxk8k2.js";
import { O } from "/$bunfs/root/chunk-dqkj2bph.js";
import { lstatSync as or, readlinkSync as ir, unwatchFile as N } from "fs";
import { readdir as sr, readFile as Q, stat as Ke } from "fs/promises";
import { constants as ht } from "fs";
import * as T from "fs/promises";
import { randomUUID as rt } from "crypto";
import { constants as ge } from "fs";
import { mkdir as nt, open as ot, readdir as st, readFile as at } from "fs/promises";
import { basename as lt, dirname as ct, join as he } from "path";
var rsr = 10485760;
var x1e = 512000;
var Qe = { imageMaxRawBytes: 307200, wholePdfMaxRawBytes: 0, pdfMaxPagesPerRead: 3 },
  Je = { imageMaxRawBytes: 512000, wholePdfMaxRawBytes: 2500000, pdfMaxPagesPerRead: 6 };
function dzt(e) {
  return e === "session" ? Je : Qe;
}
var pe = 28;
function pzt(e, t) {
  if (e <= 0 || t <= 0) return 0;
  return Math.ceil(e / pe) * Math.ceil(t / pe);
}
var fA = { maxWidth: 2000, maxHeight: 2000, maxBase64Size: 5242880, targetRawSize: 3932160 },
  eJe = 33554432,
  GEt = 20971520,
  osr = 100,
  isr = 3145728,
  zEt = 104857600,
  koe = 20,
  fzt = 10,
  ssr = 100,
  asr = 600,
  lsr = 20,
  csr = 78643200,
  usr = eJe - 8388608,
  dsr = 10485760;
import { basename as et, join as me } from "path";
var afe = "tool-results";
function pS(e) {
  return me(Wu(e.root.project.originalCwd), e.root.id, afe);
}
function I_() {
  return pS({ root: { id: K(), project: { originalCwd: Se() } } });
}
function SP(e, t) {
  if (et(e) !== afe || !Zt(t)) return;
  return vw(me(e, t));
}
function tt(e) {
  let t = SP(e, "probe");
  if (t === void 0 || t.namespace !== "sidecar") return;
  return { namespace: "sidecar", projectKey: t.projectKey, sessionId: t.sessionId, relPath: t.relPath.slice(0, -1) };
}
async function nU(e, t) {
  if (O() && t !== void 0) {
    let r = tt(e);
    if (r !== void 0)
      try {
        if ((await t.ensureScope(r)).ok) return;
      } catch {}
  }
  try {
    await an().mkdir(e);
  } catch {}
}
function ut(e) {
  let { firstPage: t, lastPage: r } = e ?? {};
  if (t === void 0) return "page range";
  if (r === void 0 || r === 1 / 0) return `pages ${t}-`;
  if (t === r) return `page ${t}`;
  return `pages ${t}-${r}`;
}
async function psr(e) {
  try {
    let o = (await le().stat(e)).size;
    if (o === 0) return { success: false, error: { reason: "empty", message: `PDF file is empty: ${e}` } };
    if (o > GEt)
      return {
        success: false,
        error: { reason: "too_large", message: `PDF file exceeds maximum allowed size of ${Ft(GEt)}.` },
      };
    let i = await at(e);
    if (!i.subarray(0, 5).toString("ascii").startsWith("%PDF-"))
      return {
        success: false,
        error: { reason: "corrupted", message: `File is not a valid PDF (missing %PDF- header): ${e}` },
      };
    let a = i.toString("base64");
    return { success: true, data: { type: "pdf", file: { filePath: e, base64: a, originalSize: o } } };
  } catch (t) {
    if (Zd(t)) throw t;
    return { success: false, error: { reason: "unknown", message: l(t), category: ne(t), site: "read" } };
  }
}
async function mzt(e) {
  let { code: t, stdout: r } = await $e("pdfinfo", [e], { timeout: 1e4, useCwd: false });
  if (t !== 0) return null;
  let o = /^Pages:\s+(\d+)/m.exec(r);
  if (!o) return null;
  let i = parseInt(o[1], 10);
  return isNaN(i) ? null : i;
}
function oe(e) {
  let t = e === "win32" ? 0 : (ge.O_NONBLOCK ?? 0);
  return ge.O_RDONLY | t;
}
class we {
  available = void 0;
  markAvailable() {
    this.available = true;
  }
  reset() {
    this.available = void 0;
  }
}
var ft = new J(() => new we());
function dt() {
  return ft.of(G().host);
}
async function pt() {
  let e = dt();
  if (e.available !== void 0) return e.available;
  let { code: t, stderr: r } = await $e("pdftoppm", ["-v"], { timeout: 5000, useCwd: false }),
    o = t === 0 || (t !== 127 && r.length > 0);
  if (o) e.markAvailable();
  return o;
}
async function avn(e, t, r) {
  try {
    let o = await ot(e, oe("darwin")),
      i = await o.stat().finally(() => o.close());
    if (!i.isFile())
      return { success: false, error: { reason: "corrupted", message: `Path is not a regular file: ${e}` } };
    let s = i.size;
    if (s === 0) return { success: false, error: { reason: "empty", message: `PDF file is empty: ${e}` } };
    if (s > zEt)
      return {
        success: false,
        error: {
          reason: "too_large",
          message: `PDF file exceeds maximum allowed size for text extraction (${Ft(zEt)}).`,
        },
      };
    if (!(await pt()))
      return {
        success: false,
        error: {
          reason: "unavailable",
          message:
            "pdftoppm is not installed. Install poppler-utils (e.g. `brew install poppler` or `apt-get install poppler-utils`) to enable PDF page rendering.",
        },
      };
    let u = rt(),
      b = he(I_(), `pdf-${u}`),
      m = r === void 0 ? void 0 : be(b);
    await nt(b, { recursive: true });
    let w = he(b, "page"),
      d = ["-jpeg", "-r", "100"];
    if (t?.firstPage) d.push("-f", String(t.firstPage));
    if (t?.lastPage && t.lastPage !== 1 / 0) d.push("-l", String(t.lastPage));
    d.push(e, w);
    let { code: c, stderr: f, exitCode: x } = await $e("pdftoppm", d, { timeout: 120000, useCwd: false });
    if (c !== 0) {
      if (/password/i.test(f))
        return {
          success: false,
          error: {
            reason: "password_protected",
            message: "PDF is password-protected. Please provide an unprotected version.",
          },
        };
      let D = /Wrong page range given.*last page \((\d+)\)/i.exec(f);
      if (D) {
        let A = Number(D[1]);
        if (A === 0)
          return {
            success: false,
            error: { reason: "corrupted", message: "PDF reports 0 pages (empty page tree). The PDF may be invalid." },
          };
        let Ye = Math.min(A, koe);
        return {
          success: false,
          error: {
            reason: "page_out_of_range",
            message: `Requested ${ut(t)} is outside the document (PDF has ${A} ${k(A, "page")}). Use a range within 1-${A}, maximum ${koe} pages per request (e.g. pages: "1-${Ye}").`,
          },
        };
      }
      let re = /Syntax Error(?: \(\d+\))?: Couldn't (?:find trailer dictionary|read xref table)/i;
      if (/damaged|corrupt|invalid/i.test(f) || re.test(f))
        return { success: false, error: { reason: "corrupted", message: "PDF file is corrupted or invalid." } };
      let de = f.split(`
`),
        z = de[0] ?? "";
      if (
        ((z.startsWith("I/O Error: ") && z.includes(`'${e}'`)) || z.startsWith("Permission Error: ")) &&
        !de.some((A) => /^(Command Line Error|Internal Error)(?: \(\d+\))?: /.test(A))
      )
        return { success: false, error: { reason: "pdftoppm_input_error", message: `Could not render PDF: ${z}` } };
      return {
        success: false,
        error: { reason: "unknown", message: `pdftoppm failed: ${f}`, category: gt(f), site: "extract", exitCode: x },
      };
    }
    let _;
    if (r !== void 0) {
      let D = await lvn(r, b, m);
      if (!D.ok)
        return {
          success: false,
          error: {
            reason: "unknown",
            message: `Failed to list extracted PDF pages: ${D.code}`,
            category: `storage_list_${D.code}`,
            site: "extract",
          },
        };
      if (D.names.length === 0 && !D.directoryExists)
        return {
          success: false,
          error: {
            reason: "unknown",
            message: `Extraction directory missing after pdftoppm ran: ${b}`,
            category: ne({ code: "ENOENT" }),
            site: "extract",
          },
        };
      _ = D.names;
    } else _ = (await st(b)).filter((re) => re.endsWith(".jpg")).sort();
    if (_.length === 0)
      return {
        success: false,
        error: { reason: "corrupted", message: "pdftoppm produced no output pages. The PDF may be invalid." },
      };
    let I = _.length;
    return {
      success: true,
      data: {
        type: "parts",
        file: { filePath: e, originalSize: s, outputDir: b, count: I },
        ...(m !== void 0 && { v5SidecarScope: { ...m, pageNames: _ } }),
      },
    };
  } catch (o) {
    let i = o && typeof o === "object" && "path" in o ? o.path : void 0;
    if (Zd(o) && (i === e || i === void 0)) throw o;
    return { success: false, error: { reason: "unknown", message: l(o), category: ne(o), site: "extract" } };
  }
}
var mt = [
  [/^Internal Error(?: \(\d+\))?: /, "internal_error"],
  [/^Command Line Error(?: \(\d+\))?: /, "command_line_error"],
  [/^Config Error(?: \(\d+\))?: /, "config_error"],
  [/^Unimplemented Feature(?: \(\d+\))?: /, "unimplemented_feature"],
  [/^I\/O Error(?: \(\d+\))?: /, "io_output"],
  [/^Syntax (?:Error|Warning)(?: \(\d+\))?: /, "syntax_diagnostics"],
];
function gt(e) {
  if (e.trim() === "") return "empty_stderr";
  let t = e.split(`
`);
  return mt.find(([o]) => t.some((i) => o.test(i)))?.[1] ?? "other";
}
function ne(e) {
  return `fs_wrapped_${uo(e) ?? "none"}`;
}
var ye = "ListCapExceeded";
function be(e) {
  let t = I_();
  if (ct(e) !== t)
    throw new R(
      `PDF extraction directory is outside the session tool-results store: ${e}`,
      "pdf extraction dir outside tool-results",
    );
  return { projectKey: Cm(Se()), sessionId: K(), relPath: [afe, lt(e)] };
}
async function lvn(e, t, r) {
  let o = r ?? be(t),
    { projectKey: i, sessionId: s, relPath: a } = o,
    u = [],
    b = await Ao(
      (w) =>
        e.listEntries(
          { namespace: "sidecar", projectKey: i, sessionId: s, relPath: a },
          { cursor: w, skipKeyStats: true, skipScopeStats: true },
        ),
      (w) => {
        for (let d of w) {
          if (d.kind !== "key" || d.key.namespace !== "sidecar") continue;
          let c = d.key.relPath.at(-1);
          if (c === void 0 || d.key.relPath.length !== a.length + 1) continue;
          if (!c.endsWith(".jpg")) continue;
          u.push(c);
        }
      },
    );
  switch (b.status) {
    case "done":
      break;
    case "error":
      return { ok: false, code: b.error.code };
    case "capped":
      return { ok: false, code: ye };
  }
  u.sort();
  let m = true;
  if (u.length === 0) {
    m = false;
    let w = a.at(-1),
      d = await Ao(
        (c) =>
          e.listEntries(
            { namespace: "sidecar", projectKey: i, sessionId: s, relPath: a.slice(0, -1) },
            { suffix: w, cursor: c, skipScopeStats: true },
          ),
        (c) => {
          m ||= c.some(
            (f) =>
              f.kind === "scope" &&
              f.scope.namespace === "sidecar" &&
              (f.scope.relPath ?? []).length === a.length &&
              f.scope.relPath?.at(-1) === w,
          );
        },
        { until: () => m },
      );
    switch (d.status) {
      case "done":
        break;
      case "error":
        return { ok: false, code: d.error.code };
      case "capped":
        return { ok: false, code: ye };
    }
  }
  return { ok: true, names: u, directoryExists: m, scope: o };
}
async function bP(e, t, r) {
  let o = r?.fromTail === true ? "unreadable" : null,
    i;
  try {
    let s = r?.noFollow ? (ht.O_NOFOLLOW ?? 0) : 0;
    i = await T.open(e, oe("darwin") | s);
    let a = await i.stat();
    if (!a.isFile()) return o;
    if (r?.requireNlink1 && a.nlink !== 1) return o;
    if (r?.verifyHandlePath) {
      let d = null;
      if (d === null) {
        d = await T.realpath(e);
        let c = await T.stat(d);
        if (c.dev !== a.dev || c.ino !== a.ino) return o;
      }
      if (!r.verifyHandlePath(d)) return o;
    }
    if (r?.fromTail === true) {
      let d = a.size > t ? a.size - t : 0,
        c = Buffer.alloc(a.size - d),
        f = await _e(i, c, d),
        x = ie(c.subarray(0, f), r.sniffEncoding);
      if (d === 0) return { content: x, truncated: false };
      let _ = x.indexOf(`
`);
      return { content: _ === -1 ? "" : x.slice(_ + 1), truncated: true };
    }
    let u = Math.min(a.size, t + 1),
      b = Buffer.alloc(u),
      m = await _e(i, b, 0);
    if (r?.sniffEncoding === true) {
      let d = a.size > t ? Math.min(m, t) : m,
        c = b.subarray(0, d),
        f = ie(c, true),
        x = a.size > t,
        _ = x
          ? `${f}
\u2026[truncated at ${t} bytes of ${a.size} bytes]`
          : f;
      return r.withBytes === true ? { content: _, bytes: c, truncated: x } : _;
    }
    let w = ie(b.subarray(0, m), false);
    return w.length > t
      ? `${w.slice(0, t)}
\u2026[truncated at ${t} chars of ${a.size} bytes]`
      : w;
  } catch (s) {
    return tJe(s) ? null : o;
  } finally {
    await i?.close().catch(() => {
      return;
    });
  }
}
function tJe(e) {
  let t = E(e);
  return t === "ENOENT" || t === "ENOTDIR";
}
async function _e(e, t, r) {
  let o = 0;
  while (o < t.length) {
    let { bytesRead: i } = await e.read(t, o, t.length - o, r + o);
    if (i === 0) break;
    o += i;
  }
  return o;
}
function ie(e, t) {
  if (t !== true) return e.toString("utf8");
  if (e.length >= 2 && e[0] === 255 && e[1] === 254) return e.subarray(2).toString("utf16le");
  if (e.length >= 3 && e[0] === 239 && e[1] === 187 && e[2] === 191) return e.subarray(3).toString("utf8");
  let r = e.subarray(0, Math.min(e.length, 512)),
    o = 0,
    i = 0;
  for (let s = 0; s < r.length; s++)
    if (r[s] === 0)
      if (s % 2 === 0) o++;
      else i++;
  if (r.length >= 8 && (o + i) * 4 > r.length) return e.subarray(o > i ? 1 : 0).toString("utf16le");
  return e.toString("utf8");
}
import { dirname as fe, isAbsolute as ar, join as v, relative as lr, resolve as W, sep as V } from "path";
var Re = {
  ccr: {
    controlChannel: true,
    modelCatalog: true,
    setPermissionMode: true,
    fanout: true,
    presence: true,
    catchupReplay: true,
    bashExec: true,
    fileRead: true,
  },
  ssh: {
    controlChannel: true,
    modelCatalog: true,
    setPermissionMode: true,
    fanout: false,
    presence: false,
    catchupReplay: false,
    bashExec: false,
    fileRead: true,
  },
  direct: {
    controlChannel: false,
    modelCatalog: false,
    setPermissionMode: false,
    fanout: false,
    presence: false,
    catchupReplay: false,
    bashExec: false,
    fileRead: false,
  },
};
var azt = { isRemoteMode: false };
function Gr() {
  return RR().remote;
}
var qEt = "Stopped waiting for the remote \u2014 the command may still complete there";
function sn() {
  return $n() || Gr() !== null;
}
function ba() {
  let e = Gr();
  return e?.caps?.controlChannel === true && !e.viewerOnly;
}
function zT(e) {
  return Gr()?.caps?.[e] === true;
}
function lzt(e, t) {
  return t && !(e.isRemoteMode && e.viewerOnly);
}
function yt(e) {
  return Promise.reject(Error(`sendControlRequest not yet wired for ${e} transport`));
}
function czt(e, t, r, o) {
  if (!t.isRemoteMode) return azt;
  return {
    kind: e,
    isRemoteMode: true,
    viewerOnly: r,
    caps: Re[e],
    sessionId: o,
    sendMessage: t.sendMessage,
    cancelRequest: t.cancelRequest,
    disconnect: t.disconnect,
    sendControlRequest: t.sendControlRequest ?? (() => yt(e)),
  };
}
import { createHash as kt } from "crypto";
import {
  accessSync as It,
  constants as At,
  lstatSync as Ie,
  readFileSync as F,
  realpathSync as q,
  statSync as Fe,
} from "fs";
import { lstat as Tt, open as Ot, readFile as Mt, realpath as Pe, stat as Gt } from "fs/promises";
import { homedir as Ht } from "os";
import { basename as j, dirname as H, isAbsolute as ce, join as P, resolve as M, sep as C } from "path";
var wt = new Set([
  ".png",
  ".jpg",
  ".jpeg",
  ".gif",
  ".bmp",
  ".ico",
  ".webp",
  ".tiff",
  ".tif",
  ".mp4",
  ".mov",
  ".avi",
  ".mkv",
  ".webm",
  ".wmv",
  ".flv",
  ".m4v",
  ".mpeg",
  ".mpg",
  ".mp3",
  ".wav",
  ".ogg",
  ".flac",
  ".aac",
  ".m4a",
  ".wma",
  ".aiff",
  ".opus",
  ".zip",
  ".tar",
  ".gz",
  ".bz2",
  ".7z",
  ".rar",
  ".xz",
  ".z",
  ".tgz",
  ".iso",
  ".exe",
  ".dll",
  ".so",
  ".dylib",
  ".bin",
  ".o",
  ".a",
  ".obj",
  ".lib",
  ".app",
  ".msi",
  ".deb",
  ".rpm",
  ".pdf",
  ".doc",
  ".docx",
  ".xls",
  ".xlsx",
  ".ppt",
  ".pptx",
  ".odt",
  ".ods",
  ".odp",
  ".ttf",
  ".otf",
  ".woff",
  ".woff2",
  ".eot",
  ".pyc",
  ".pyo",
  ".class",
  ".jar",
  ".war",
  ".ear",
  ".node",
  ".wasm",
  ".rlib",
  ".sqlite",
  ".sqlite3",
  ".db",
  ".mdb",
  ".idx",
  ".psd",
  ".ai",
  ".eps",
  ".sketch",
  ".fig",
  ".xd",
  ".blend",
  ".3ds",
  ".max",
  ".swf",
  ".fla",
  ".lockb",
  ".dat",
  ".data",
]);
function uzt(e) {
  let t = e.slice(e.lastIndexOf(".")).toLowerCase();
  return wt.has(t);
}
var bt = 8192;
function xe(e) {
  let t = Math.min(e.length, bt),
    r = 0;
  for (let o = 0; o < t; o++) {
    let i = e[o];
    if (i === 0) return true;
    if (i < 32 && i !== 9 && i !== 10 && i !== 13) r++;
  }
  return r / t > 0.1;
}
import { open as _t } from "fs/promises";
import { join as Rt } from "path";
async function ZYe(e, t, r, o) {
  try {
    let i = await _t(Rt(e, "config"), "r");
    try {
      let a = Buffer.allocUnsafe(1e6),
        u = 0;
      while (u < 1e6) {
        let { bytesRead: w } = await i.read(a, u, 1e6 - u, u);
        if (w === 0) break;
        u += w;
      }
      let b = false;
      if (u === 1e6) {
        let w = Buffer.allocUnsafe(1),
          { bytesRead: d } = await i.read(w, 0, 1, u);
        b = d > 0;
      }
      let m = a.subarray(0, u).toString("utf-8");
      if (b) {
        let w = m.lastIndexOf(`
`);
        m = w === -1 ? "" : m.slice(0, w);
      }
      return se(m, t, r, o);
    } finally {
      await i.close();
    }
  } catch {
    return null;
  }
}
function se(e, t, r, o) {
  let i = e.split(`
`),
    s = t.toLowerCase(),
    a = o.toLowerCase(),
    u = false;
  for (let b of i) {
    let m = b.trim();
    if (m.length === 0 || m[0] === "#" || m[0] === ";") continue;
    if (m[0] === "[") {
      u = Ct(m, s, r);
      continue;
    }
    if (!u) continue;
    let w = xt(m);
    if (w && w.key.toLowerCase() === a) return w.value;
  }
  return null;
}
function xt(e) {
  let t = 0;
  while (t < e.length && Dt(e[t])) t++;
  if (t === 0) return null;
  let r = e.slice(0, t);
  while (t < e.length && (e[t] === " " || e[t] === "\t")) t++;
  if (t >= e.length || e[t] !== "=") return null;
  t++;
  while (t < e.length && (e[t] === " " || e[t] === "\t")) t++;
  let o = Et(e, t);
  return { key: r, value: o };
}
function Et(e, t) {
  let r = "",
    o = false,
    i = t;
  while (i < e.length) {
    let s = e[i];
    if (!o && (s === "#" || s === ";")) break;
    if (s === '"') {
      (o = !o), i++;
      continue;
    }
    if (s === "\\" && i + 1 < e.length) {
      let a = e[i + 1];
      if (o) {
        switch (a) {
          case "n":
            r += `
`;
            break;
          case "t":
            r += "\t";
            break;
          case "b":
            r += "\b";
            break;
          case '"':
            r += '"';
            break;
          case "\\":
            r += "\\";
            break;
          default:
            r += a;
            break;
        }
        i += 2;
        continue;
      }
      if (a === "\\") {
        (r += "\\"), (i += 2);
        continue;
      }
    }
    (r += s), i++;
  }
  if (!o) r = Pt(r);
  return r;
}
function Pt(e) {
  let t = e.length;
  while (t > 0 && (e[t - 1] === " " || e[t - 1] === "\t")) t--;
  return e.slice(0, t);
}
function Ct(e, t, r) {
  let o = 1;
  while (o < e.length && e[o] !== "]" && e[o] !== " " && e[o] !== "\t" && e[o] !== '"') o++;
  if (e.slice(1, o).toLowerCase() !== t) return false;
  if (r === null) return o < e.length && e[o] === "]";
  while (o < e.length && (e[o] === " " || e[o] === "\t")) o++;
  if (o >= e.length || e[o] !== '"') return false;
  o++;
  let s = "";
  while (o < e.length && e[o] !== '"') {
    if (e[o] === "\\" && o + 1 < e.length) {
      let a = e[o + 1];
      if (a === "\\" || a === '"') {
        (s += a), (o += 2);
        continue;
      }
      (s += a), (o += 2);
      continue;
    }
    (s += e[o]), o++;
  }
  if (o >= e.length || e[o] !== '"') return false;
  if ((o++, o >= e.length || e[o] !== "]")) return false;
  return s === r;
}
function Dt(e) {
  return (e >= "a" && e <= "z") || (e >= "A" && e <= "Z") || (e >= "0" && e <= "9") || e === "-";
}
var f0 = Symbol("git-repo-negative-result");
class Ee {
  rootByPath = new cd({ max: 50 });
  canonicalRootByRoot = new cd({ max: 50 });
  remoteSlugByRoot = new cd({ max: 50 });
  monorepoRuledOutByRoot = new cd({ max: 50 });
  gitDirByCwd = new Map();
  linkedFromRootByPin = new cd({ max: 64 });
  markersByRoot = new cd({ max: 50 });
  repositoryByCwd = new Map();
  repoClassByCanonicalRoot = new Map();
  linguistGeneratedByPath = new Map();
  wcRootByPath = new cd({ max: 50 });
  gitExecutable = null;
  reportedHookInstallSkips = new Set();
  clear() {
    this.rootByPath.clear(),
      this.canonicalRootByRoot.clear(),
      this.remoteSlugByRoot.clear(),
      this.monorepoRuledOutByRoot.clear(),
      this.gitDirByCwd.clear(),
      this.linkedFromRootByPin.clear(),
      this.markersByRoot.clear(),
      this.repositoryByCwd.clear(),
      this.repoClassByCanonicalRoot.clear(),
      this.linguistGeneratedByPath.clear(),
      this.wcRootByPath.clear(),
      (this.gitExecutable = null),
      this.reportedHookInstallSkips.clear();
  }
}
function rU(e, t, r) {
  let o = e.get(t);
  if (o !== void 0) return o;
  let i = r(t);
  return e.set(t, i), i;
}
var Bt = new J(() => new Ee());
function vd() {
  return Bt.of(G().host);
}
function ve(e, t) {
  try {
    let r = Ie(e);
    if (r.isSymbolicLink()) {
      let o = uvn(e);
      if (o === null) return false;
      if (Sc(o, t)) return false;
      if (P_(o, t)) return false;
      let i = Fe(e);
      return i.isDirectory() || i.isFile();
    }
    return r.isDirectory() || r.isFile();
  } catch {
    return false;
  }
}
function Ae(e) {
  let t = Date.now();
  Y("info", "find_git_root_started");
  let r = M(e),
    o = r.substring(0, r.indexOf(C) + 1) || C,
    i = 0;
  while (r !== o) {
    let s = P(r, ".git");
    if ((i++, ve(s, r)))
      return Y("info", "find_git_root_completed", { duration_ms: Date.now() - t, stat_count: i, found: true }), tr(r);
    let a = H(r);
    if (a === r) break;
    r = a;
  }
  if ((i++, ve(P(o, ".git"), o)))
    return Y("info", "find_git_root_completed", { duration_ms: Date.now() - t, stat_count: i, found: true }), tr(o);
  return Y("info", "find_git_root_completed", { duration_ms: Date.now() - t, stat_count: i, found: false }), f0;
}
function Hn(e) {
  let t = rU(vd().rootByPath, e, Ae);
  return t === f0 ? null : t;
}
function YEt(e) {
  let t = Ae(e);
  return t === f0 ? null : t;
}
async function hzt(e, t) {
  let r = M(t),
    o = r.substring(0, r.indexOf(C) + 1) || C;
  for (;;) {
    let i = await e.stat(su.workspace(P(r, ".git")), { follow: false });
    if (!i.ok || i.value.kind === "link") return;
    if (i.value.kind === "directory" || i.value.kind === "file") return { gitRoot: tr(r), entry: i.value.kind };
    if (r === o) return { gitRoot: null };
    let s = H(r);
    r = s === r ? o : s;
  }
}
function a4(e) {
  let t = vd().rootByPath;
  if (t.peek(e) === f0) t.delete(e);
  return Hn(e);
}
async function rJe(e) {
  let t = a4(e);
  if (t === null) return null;
  try {
    return await Tt(P(t, ".git")), t;
  } catch (r) {
    let o = r?.code;
    if (o !== "ENOENT" && o !== "ENOTDIR") return t;
    return vd().rootByPath.delete(e), a4(e);
  }
}
function ue(e) {
  try {
    return Te(e, F(P(e, ".git"), "utf-8"));
  } catch {
    return e;
  }
}
function Te(e, t) {
  try {
    let r = t.trim();
    if (!r.startsWith("gitdir:")) return e;
    let o = r.slice(7).trim();
    if (Sc(o, e)) return e;
    if (P_(o, e)) return e;
    let i = M(e, o);
    if (KH(P(i, "commondir"), i)) return e;
    let s = F(P(i, "commondir"), "utf-8").trim();
    if (Sc(s, i)) return e;
    if (P_(s, i)) return e;
    let a = M(i, s);
    if (M(H(i)) !== P(a, "worktrees")) return e;
    if (KH(P(i, "gitdir"), i)) return e;
    let u = F(P(i, "gitdir"), "utf-8").trim();
    if (Sc(u, e)) return e;
    if (P_(u, i, e)) return e;
    if (q(M(i, u)) !== P(q(e), ".git")) return e;
    if (j(a) !== ".git") return tr(a);
    return tr(H(a));
  } catch {
    return e;
  }
}
async function Lt(e, t) {
  let r = await e.readText(su.workspace(P(t, ".git")));
  if (!r.ok || !r.value.found) return;
  return Te(t, r.value.value);
}
function Vr(e) {
  let t = Hn(e);
  if (!t) return null;
  return rU(vd().canonicalRootByRoot, t, ue);
}
function DO(e) {
  let t = YEt(e);
  if (!t) return null;
  return ue(t);
}
async function yvn(e, t) {
  if (e.hostFiles.serving("workspace") !== "host") return;
  let r = vd(),
    o = r.rootByPath.peek(t);
  if (o !== void 0) {
    let a = o === f0 ? null : o;
    return { gitRoot: a, canonicalRoot: a === null ? null : r.canonicalRootByRoot.peek(a) };
  }
  let i = await hzt(e.hostFiles, t);
  if (i === void 0) return;
  let s =
    i.gitRoot === null
      ? { gitRoot: null, canonicalRoot: null }
      : { gitRoot: i.gitRoot, canonicalRoot: i.entry === "directory" ? i.gitRoot : await Lt(e.hostFiles, i.gitRoot) };
  return _zt(t, s.gitRoot, s.canonicalRoot), s;
}
function _zt(e, t, r) {
  let o = vd(),
    i = t ?? f0,
    s = o.rootByPath.peek(e);
  if (s === void 0) o.rootByPath.set(e, i);
  else if (s !== i) {
    n("git root prime: findGitRoot already holds a different answer for the start path; keeping the first", {
      level: "warn",
    });
    return;
  }
  if (t === null || r === void 0 || r === null) return;
  let a = o.canonicalRootByRoot.peek(t);
  if (a === void 0) o.canonicalRootByRoot.set(t, r);
  else if (a !== r)
    n("git root prime: findCanonicalGitRoot already holds a different answer for the root; keeping the first", {
      level: "warn",
    });
}
function m0(e) {
  let t = Hn(e);
  return t !== null && Vr(e) !== t;
}
function Svn(e) {
  let t = YEt(e);
  return t !== null && ue(t) !== t;
}
async function bvn(e) {
  if (sn()) return null;
  let t = await Aw(e);
  if (!t || j(t) === ".git" || j(H(t)) !== "worktrees") return null;
  return j(t);
}
var oJe = ["--no-ext-diff", "--no-textconv"];
function it() {
  let e = vd();
  return (e.gitExecutable ??= $C("git") || "git"), e.gitExecutable;
}
var Ssr = new Mn(() => new Map());
async function Nt() {
  let e = Date.now();
  Y("info", "is_git_check_started");
  let t = Hn(ee()) !== null;
  return Y("info", "is_git_check_completed", { duration_ms: Date.now() - e, is_git: t }), t;
}
function Oe() {
  return G();
}
function Vg() {
  let e = Ssr.of(Oe()),
    t = K(),
    r = e.get(t);
  if (r !== void 0) return r;
  let o = Nt();
  return e.set(t, o), o;
}
function P1e(e) {
  Ssr.of(e).clear();
}
function lfe() {
  P1e(Oe());
}
function l4(e) {
  return Aw(e);
}
async function mHr() {
  let e = ee(),
    t = Hn(e);
  if (!t) return false;
  try {
    let [r, o] = await Promise.all([Pe(e), Pe(t)]);
    return r === o;
  } catch {
    return e === t;
  }
}
var wvn = async (e) => Hn(e) !== null,
  D1e = async () => XEt(),
  cl = async (e) => {
    if (e === void 0) return msr();
    let { stdout: t, code: r } = await qe(it(), [...cn, "rev-parse", "--abbrev-ref", "HEAD"], {
      cwd: e,
      preserveOutputOnError: false,
    });
    return r === 0 ? t.trim() || "HEAD" : "HEAD";
  };
async function iJe(e, t) {
  return (
    (
      await qe(it(), [...cn, "show-ref", "--verify", "--quiet", `refs/remotes/origin/${e}`], {
        cwd: t ?? ee(),
        preserveOutputOnError: false,
      })
    ).code === 0
  );
}
var Cw = async (e) => {
    if (e === void 0) return gsr();
    let t = await yzt(e);
    if (t) return t;
    for (let r of I1e)
      if (
        (
          await qe(it(), [...cn, "show-ref", "--verify", "--quiet", `refs/remotes/origin/${r}`], {
            cwd: e,
            preserveOutputOnError: false,
          })
        ).code === 0
      )
        return r;
    return "main";
  },
  yzt = async (e) => {
    let { stdout: t, code: r } = await qe(it(), [...cn, "symbolic-ref", "--short", "refs/remotes/origin/HEAD"], {
      cwd: e ?? ee(),
      preserveOutputOnError: false,
    });
    if (r !== 0) return null;
    let o = t.trim().replace(/^origin\//, "");
    if (!o) return null;
    return (
      await qe(it(), [...cn, "show-ref", "--verify", "--quiet", `refs/remotes/origin/${o}`], {
        cwd: e ?? ee(),
        preserveOutputOnError: false,
      })
    ).code === 0
      ? o
      : null;
  },
  Wt = /^[A-Za-z0-9][A-Za-z0-9._-]*$/;
async function JEt(e) {
  let { stdout: t, code: r } = await $e(it(), [...cn, "config", "--get", e], { preserveOutputOnError: false, useCwd: true });
  if (r !== 0) return null;
  let o = t.trim();
  return o.length > 0 ? o : null;
}
async function $t() {
  let { stdout: e, code: t } = await $e(it(), [...cn, "remote"], { preserveOutputOnError: false, useCwd: true });
  if (t !== 0) return null;
  let r = e
      .split(`
`)
      .map((i) => i.trim())
      .filter(Boolean),
    o = await JEt("remote.pushDefault");
  if (o && r.includes(o)) return o;
  if (r.length === 1) return r[0] ?? null;
  return null;
}
async function sJe() {
  let e = new Set(["origin"]),
    t = await $t();
  if (t && Wt.test(t)) e.add(t);
  return [...e].flatMap((r) => [`git push ${r} *`, `git push -u ${r} *`]);
}
var c4 = async () => dvn(),
  Ewr = async () => {
    let e = Hn(ee());
    if (e === null) return null;
    try {
      let t = Ht();
      if (t && e === tr(M(t))) return null;
    } catch {}
    return dvn();
  },
  Tvn = async () => {
    let [e, t, r] = await Promise.all([cl(), Ewr(), hsr()]);
    return { branch: e, gitRepoUrl: t, defaultBranch: r };
  };
function tJ(e) {
  return e == null ? e : e.replace(/:\/\/[^/]*@/, "://***@");
}
function u4(e) {
  let t = e.trim();
  if (!t) return null;
  let r = t.match(/^git@([^:/@]+):(.+?)(?:\.git)?$/);
  if (r && r[1] && r[2]) return `${r[1]}/${r[2]}`.toLowerCase();
  let o = t.match(/^(?:https?|ssh):\/\/(?:[^@/?#]*@)?([^/?#@]+)\/(.+?)(?:\.git)?$/);
  if (o && o[1] && o[2]) {
    let i = o[1],
      s = o[2];
    if (Tzt(i) && s.startsWith("git/")) {
      let a = s.slice(4),
        u = a.split("/");
      if (u.length >= 3 && u[0].includes(".")) return a.toLowerCase();
      return `github.com/${a}`.toLowerCase();
    }
    return `${i}/${s}`.toLowerCase();
  }
  return null;
}
function Awr(e) {
  let t = e ? St(e, ":").toLowerCase() : null;
  if (!t) return "none";
  if (qo(t) || t.endsWith(`.${vs}`)) return "github";
  if (t.endsWith(".ghe.com") || t.includes("github")) return "ghe";
  if (t === "gitlab.com" || t.includes("gitlab")) return "gitlab";
  if (t === "bitbucket.org" || t.includes("bitbucket")) return "bitbucket";
  if (t === "dev.azure.com" || t === "ssh.dev.azure.com" || t.endsWith(".visualstudio.com") || t.endsWith(".azure.com"))
    return "azure";
  return "other";
}
async function Evn() {
  if (sn()) return null;
  let [e, t] = await Promise.all([Vg(), c4()]).catch(() => [false, null]),
    r = t ? u4(t) : null;
  return { is_git: e, has_remote: r !== null, remote_host_class: Awr(r ? St(r, "/") : null) };
}
function Ut(e) {
  for (let t of [P(e, ".git", "config"), P(e, "config")])
    try {
      if (oU(t, e)) continue;
      return F(t, "utf-8");
    } catch {}
  return null;
}
function zt(e) {
  let t = Ut(e);
  if (!t) return f0;
  let r = (o) => {
    let i = se(t, "remote", "origin", o);
    return i ? u4(i) : null;
  };
  return r("pushurl") ?? r("url") ?? f0;
}
function aJe(e) {
  let t = rU(vd().remoteSlugByRoot, e, zt);
  return t === f0 ? null : t;
}
var Kt = 128000;
function Avn(e) {
  try {
    let t = true;
    try {
      Ie(P(e, ".git"));
    } catch (i) {
      if (!X(i)) return null;
      t = false;
    }
    let r = t ? P(e, ".git", "config") : P(e, "config");
    if (oU(r, e)) return null;
    let o = Fe(r);
    if (!o.isFile() || o.size > Kt) return null;
    return F(r, "utf-8");
  } catch {
    return null;
  }
}
async function Cvn() {
  let e = await c4();
  if (!e) return null;
  let t = u4(e);
  if (!t) return null;
  return kt("sha256").update(t).digest("hex").substring(0, 16);
}
var Szt = async () => {
    let { code: e } = await $e(it(), [...cn, "rev-parse", "@{u}"], { preserveOutputOnError: false });
    return e === 0;
  },
  lJe = async (e) => {
    let { stdout: t, code: r } = await qe(it(), [...cn, "rev-list", "--count", "@{u}..HEAD"], {
      cwd: e,
      preserveOutputOnError: false,
    });
    return r === 0 && parseInt(t.trim(), 10) > 0;
  },
  cfe = async (e) => {
    let t = [...cn, "--no-optional-locks", "status", "--porcelain"];
    if (e?.ignoreUntracked) t.push("-uno");
    let { stdout: r } = await $e(it(), t, { preserveOutputOnError: false });
    return r.trim().length === 0;
  },
  bzt = async () => {
    let { stdout: e } = await $e(it(), [...cn, "--no-optional-locks", "status", "--porcelain"], {
        preserveOutputOnError: false,
      }),
      t = [],
      r = [];
    return (
      e
        .trim()
        .split(`
`)
        .filter((o) => o.length > 0)
        .forEach((o) => {
          let i = o.substring(0, 2),
            s = o.substring(2).trim();
          if (i === "??") r.push(s);
          else if (s) t.push(s);
        }),
      { tracked: t, untracked: r }
    );
  },
  O1e = async () => ysr(),
  vvn = async (e) => {
    try {
      let t = e || `Claude Code auto-stash - ${new Date().toISOString()}`,
        { untracked: r } = await bzt();
      if (r.length > 0) {
        let { code: i } = await $e(it(), [...cn, "add", "--", ...r], { preserveOutputOnError: false });
        if (i !== 0) return false;
      }
      let { code: o } = await $e(it(), [...cn, "stash", "push", "--message", t], { preserveOutputOnError: false });
      return o === 0;
    } catch (t) {
      return false;
    }
  };
async function wzt() {
  try {
    let [e, t, r, o, i, s] = await Promise.all([D1e(), cl(), c4(), Szt(), cfe(), O1e()]);
    return (
      y("git_status_fetch"),
      { commitHash: e, branchName: t, remoteUrl: r, isHeadOnRemote: o, isClean: i, worktreeCount: s }
    );
  } catch (e) {
    return p("git_status_fetch", "git_status_fetch_failed"), null;
  }
}
async function L1e() {
  let { parseGitRemote: e } = await import("/$bunfs/root/chunk-q9mtfzh4.js"),
    t = await c4();
  if (!t) return n("Local GitHub repo: unknown"), null;
  let r = e(t);
  if (r && qo(r.host)) {
    let o = `${r.owner}/${r.name}`;
    return n(`Local GitHub repo: ${o}`), o;
  }
  return n("Local GitHub repo: unknown"), null;
}
var Ce = 1048576,
  De = 52428800,
  Be = 200,
  jt = 65536;
async function Cwr() {
  let { stdout: e, code: t } = await $e(it(), [...cn, "rev-parse", "--abbrev-ref", "--symbolic-full-name", "@{u}"], {
    preserveOutputOnError: false,
  });
  if (t === 0 && e.trim()) return e.trim();
  let { stdout: r, code: o } = await $e(it(), [...cn, "symbolic-ref", "--short", "refs/remotes/origin/HEAD"], {
      preserveOutputOnError: false,
    }),
    i = ["origin/main", "origin/staging", "origin/master"],
    s = r.trim();
  if (o === 0 && s) i.unshift(s);
  for (let a of i) {
    let { code: u } = await $e(it(), [...cn, "rev-parse", "--verify", a], { preserveOutputOnError: false });
    if (u === 0) return a;
  }
  return null;
}
function Xt() {
  return _sr();
}
async function Me(e) {
  let { stdout: t, code: r } = await $e(it(), [...cn, "ls-files", "--others", "--exclude-standard"], {
      preserveOutputOnError: false,
      maxBuffer: e,
    }),
    o = t.trim();
  if (r !== 0 || !o) return [];
  let i = o
      .split(`
`)
      .filter(Boolean),
    s = [],
    a = 0;
  for (let u of i) {
    if (s.length >= Be) {
      n(`Untracked file capture: reached max file count (${Be})`);
      break;
    }
    if (uzt(u)) continue;
    try {
      let m = (await Gt(u)).size;
      if (m > Ce) {
        n(`Untracked file capture: skipping ${u} (exceeds ${Ce} bytes)`);
        continue;
      }
      if (a + m > De) {
        n(`Untracked file capture: reached total size limit (${De} bytes)`);
        break;
      }
      if (m === 0) {
        s.push({ path: u, content: "" });
        continue;
      }
      let w = Math.min(jt, m),
        d = await Ot(u, "r");
      try {
        let c = Buffer.alloc(w),
          { bytesRead: f } = await d.read(c, 0, w, 0),
          x = c.subarray(0, f);
        if (xe(x)) continue;
        let _;
        if (m <= w) _ = x.toString("utf-8");
        else _ = await Mt(u, "utf-8");
        s.push({ path: u, content: _ }), (a += m);
      } finally {
        await d.close();
      }
    } catch (b) {
      n(`Failed to read untracked file ${u}: ${b}`);
    }
  }
  return s;
}
function Ge(e) {
  if (e.code !== 0)
    return (
      n(
        `preserveGitStateForIssue: bounded git diff failed or overflowed (code=${e.code}, ${e.stdout.length} bytes buffered) \u2014 dropping patch`,
        { level: "warn" },
      ),
      { patch: "", dropped: true }
    );
  return { patch: e.stdout || "", dropped: false };
}
async function ae(e, t) {
  n(e);
  let [r, o] = await Promise.all([$e(it(), [...cn, "diff", ...oJe, "HEAD"], { useCwd: true, maxBuffer: t }), Me(t)]),
    { patch: i, dropped: s } = Ge(r);
  return {
    remote_base_sha: null,
    remote_base: null,
    patch: i,
    untracked_files: o,
    format_patch: null,
    head_sha: null,
    branch_name: null,
    patch_dropped: s,
  };
}
async function gHr(e) {
  let t = e?.maxBuffer;
  try {
    if (!(await Vg())) return null;
    if (await Xt()) return await ae("Shallow clone detected, using HEAD-only mode for issue", t);
    let o = await Cwr();
    if (!o) return await ae("No remote found, using HEAD-only mode for issue", t);
    let { stdout: i, code: s } = await $e(it(), [...cn, "merge-base", "HEAD", o], { preserveOutputOnError: false });
    if (s !== 0 || !i.trim()) return await ae("Merge-base failed, using HEAD-only mode for issue", t);
    let a = i.trim(),
      [u, b, { stdout: m, code: w }, { stdout: d }, { stdout: c }] = await Promise.all([
        $e(it(), [...cn, "diff", ...oJe, a], { useCwd: true, maxBuffer: t }),
        Me(t),
        $e(it(), [...cn, "format-patch", "--no-ext-diff", "--no-textconv", `${a}..HEAD`, "--stdout"], {
          useCwd: true,
          maxBuffer: t,
        }),
        $e(it(), [...cn, "rev-parse", "HEAD"]),
        $e(it(), [...cn, "rev-parse", "--abbrev-ref", "HEAD"]),
      ]),
      f = null;
    if (w === 0 && m && m.trim()) f = m;
    let { patch: x, dropped: _ } = Ge(u),
      S = c?.trim();
    return {
      remote_base_sha: a,
      remote_base: o,
      patch: x,
      untracked_files: b,
      format_patch: f,
      head_sha: d?.trim() || null,
      branch_name: S && S !== "HEAD" ? S : null,
      patch_dropped: _,
    };
  } catch (r) {
    return n(`Failed to preserve git state for issue: ${r}`, { level: "error" }), null;
  }
}
function Tzt(e) {
  let t = e.indexOf(":"),
    r = t === -1 ? e : e.slice(0, t),
    o = t === -1 ? "" : e.slice(t + 1);
  if (o !== "" && !/^\d+$/.test(o)) return false;
  return r === "localhost" || /^127\.\d{1,3}\.\d{1,3}\.\d{1,3}$/.test(r);
}
var qt = 4096;
function ke(e, t, r) {
  let o = /\/+/,
    i = t.split(o),
    s = i[0] === "" ? C : i[0] + C,
    a = 0;
  for (let u = 1; u < i.length; u++) {
    let b = i[u];
    if (b === "" || b === ".") continue;
    if (b === "..") {
      s = H(s);
      continue;
    }
    let m = s.endsWith(C) ? s + b : s + C + b;
    if (r !== void 0 ? HR(m, r) : _r(m)) return null;
    let w;
    try {
      w = e.lstatSync(m);
    } catch {
      return null;
    }
    if (!w.isSymbolicLink()) {
      s = m;
      continue;
    }
    if (++a > 64) return null;
    let d;
    try {
      d = e.readlinkSync(m);
    } catch {
      return null;
    }
    if (Bn(d)) return null;
    if (r !== void 0 ? HR(d, r) : _r(d)) return null;
    let c = d.split(o);
    if (ce(d)) i.splice(0, u + 1, ...c), (s = c[0] === "" ? C : c[0] + C), (u = 0);
    else i.splice(u, 1, ...c), u--;
  }
  return s;
}
function ufe() {
  let e = le(),
    t = ee(),
    r = ke(e, t, t) ?? t;
  try {
    r = q.native(r);
  } catch {}
  r = r.normalize("NFC").toLowerCase();
  let o = r.endsWith(C) ? r : r + C,
    i = (c) => {
      let f = c.toLowerCase();
      return f === r || f.startsWith(o);
    };
  function s(c) {
    if (HR(c, t)) return null;
    if (Bn(c)) {
      if (!/^[\\/]{2}wsl(\$|\.localhost)[\\/]/i.test(c)) return null;
      if (!Bn(t)) return { canonical: c.normalize("NFC"), crossOs: true };
    }
    let f = ke(e, c, t);
    if (f === null) return null;
    try {
      f = q.native(f);
    } catch {
      return null;
    }
    return { canonical: f.normalize("NFC"), crossOs: false };
  }
  function a(c, f) {
    if (!f && i(c)) return true;
    for (let x of c.split(f ? /[\\/]+/ : C)) if (x.toLowerCase() === ".git") return false;
    return true;
  }
  function u(c) {
    try {
      let f = e.lstatSync(P(c, "HEAD"));
      if (!f.isFile() || f.size > 4096) return false;
      let x = F(P(c, "HEAD"), "utf8").slice(0, 255);
      return /^ref:[ \t]*refs\//.test(x) || /^[0-9a-f]{40}([0-9a-f]{24})?[ \t\n\r]*$/.test(x);
    } catch {
      return false;
    }
  }
  function b(c) {
    try {
      let f = P(c, "HEAD"),
        x = e.lstatSync(f);
      if (!x.isFile() || x.size > 4096) return false;
      let _ = F(f, "utf8").slice(0, 255);
      if (!/^ref:[ \t]*refs\//.test(_) && !/^[0-9a-f]{40}([0-9a-f]{24})?[ \t\n\r]*$/.test(_)) return false;
      for (let S of ["objects", "refs"]) {
        let I = P(c, S);
        if (!e.statSync(I).isDirectory()) return false;
        It(I, At.X_OK);
      }
      try {
        return e.statSync(P(c, "commondir")), false;
      } catch {}
      return true;
    } catch {
      return false;
    }
  }
  function m(c) {
    try {
      let f = e.lstatSync(P(c, "HEAD"));
      if (f.isFile() || f.isSymbolicLink()) return true;
    } catch {}
    for (let f of ["objects", "refs"])
      try {
        return e.statSync(P(c, f)), true;
      } catch {}
    return false;
  }
  function w(c) {
    let f = (x) => {
      let _ = s(x);
      if (_ === null) return g("git_bare_repo_gate", "gitdir_target_uncanonical"), "plantable";
      if (a(_.canonical, _.crossOs)) return g("git_bare_repo_gate", "gitdir_target_plantable"), "plantable";
      return u(_.canonical) ? "trusted" : "none";
    };
    try {
      let x = e.lstatSync(P(c, ".git"));
      if (x.isSymbolicLink()) {
        let _;
        try {
          _ = e.readlinkSync(P(c, ".git"));
        } catch {
          return "plantable";
        }
        return f(ce(_) ? _ : c + C + _);
      }
      if (x.isFile()) {
        if (x.size > qt) return "oversized";
        try {
          let _ = F(P(c, ".git"), "utf8");
          if (_.includes("\x00")) return "plantable";
          if (!_.startsWith("gitdir: ")) return "none";
          let S = _.slice(8).replace(/[\r\n]+$/, "");
          return f(ce(S) ? S : c + C + S);
        } catch {
          return "none";
        }
      }
      if (x.isDirectory()) return b(P(c, ".git")) ? "trusted" : "none";
    } catch {}
    return "none";
  }
  switch (w(t)) {
    case "plantable":
      return "gitdir-redirect-plantable";
    case "oversized":
      return "gitdir-file-oversized";
    case "trusted":
      return y("git_bare_repo_gate"), false;
    case "none":
      break;
  }
  let d = t;
  for (;;) {
    if (m(d)) return g("git_bare_repo_gate", "bare_indicators"), "bare-indicators";
    let c = H(d);
    if (c === d) break;
    switch (w(c)) {
      case "trusted":
        return y("git_bare_repo_gate"), false;
      case "plantable":
        return "gitdir-redirect-plantable";
      case "oversized":
        return "gitdir-file-oversized";
      case "none":
        break;
    }
    d = c;
  }
  return y("git_bare_repo_gate"), false;
}
import { watchFile as Vt } from "fs";
function VEt(e, t, r) {
  let o = (i, s) => {
    try {
      r(i, s);
    } catch (a) {
      h(a);
    }
  };
  try {
    Vt(e, t, o);
  } catch (i) {
    h(i);
  }
  return o;
}
import { open as Yt } from "fs/promises";
import { join as Qt } from "path";
var He = 65536,
  Jt = 4096,
  L = 10,
  er = 32,
  Le = 94,
  rr = 35,
  nr = /^[0-9a-f]+$/;
async function Ne(e, t) {
  let r = Buffer.from(t, "utf-8"),
    o = r.length,
    i = Buffer.allocUnsafe(He + 1),
    s,
    a = 0,
    u = 0,
    b = 0,
    m = 0,
    w = 0,
    d = 0,
    c = 0,
    f = 0,
    x = 0,
    _ = 0,
    S = 0,
    I = 0,
    D;
  try {
    (s = await Yt(Qt(e, "packed-refs"), "r")), (a = (await s.stat()).size), (b = a);
    while (u < b) {
      if (
        ((m = u + Math.floor((b - u) / 2)),
        (d = Math.max(u, m - Jt)),
        (c = (await s.read(i, 0, Math.min(He, a - d), d)).bytesRead),
        (w = m - d),
        w >= c)
      )
        return null;
      if (((i[c] = L), (f = w > 0 ? i.lastIndexOf(L, w - 1) + 1 : 0), f > 1 && i[f] === Le))
        f = i.lastIndexOf(L, f - 2) + 1;
      if (i[f] === rr) {
        (S = i.indexOf(L, f)), (u = d + (S < 0 || S >= c ? c : S + 1));
        continue;
      }
      if (x === 0) x = f + 40 < c && i[f + 40] === er ? 40 : 64;
      if (((_ = f + x + 1), _ >= c)) return null;
      if (((I = i.compare(r, 0, o, _, Math.min(_ + o, c))), I === 0)) {
        if (_ + o < c && i[_ + o] !== L) {
          b = d + f;
          continue;
        }
        return (D = i.toString("ascii", f, f + x)), nr.test(D) ? D : null;
      }
      if (I < 0) {
        if (((S = i.indexOf(L, w)), S < 0 || S >= c)) S = c - 1;
        if (S + 1 < c && i[S + 1] === Le) {
          if (((S = i.indexOf(L, S + 1)), S < 0 || S >= c)) S = c - 1;
        }
        u = d + S + 1;
      } else b = d + f;
    }
    return null;
  } catch {
    return null;
  } finally {
    await s?.close();
  }
}
function cvn() {
  vd().gitDirByCwd.clear();
}
async function Aw(e) {
  let t = W(e ?? ee()),
    r = vd().gitDirByCwd,
    o = r.get(t);
  if (o !== void 0) return o;
  let i = Hn(t);
  if (!i) return r.set(t, null), null;
  let s = v(i, ".git");
  try {
    if ((await Ke(s)).isFile()) {
      let u = await bP(s, Ve, { sniffEncoding: true, withBytes: true });
      if (u === null || u.truncated) return r.set(t, null), null;
      let b = u.content.trim();
      if (b.startsWith("gitdir:")) {
        let m = b.slice(7).trim();
        if (Sc(m, i)) return r.set(t, null), null;
        let w = W(i, m);
        if (te(m, i)) return r.set(t, null), null;
        return r.set(t, w), w;
      }
    }
    return r.set(t, s), s;
  } catch {
    return r.set(t, null), null;
  }
}
function je(e) {
  try {
    let t = or(e);
    if (t.isSymbolicLink()) return "symlink";
    return t.isFile() ? "file" : t.isDirectory() ? "dir" : "other";
  } catch (t) {
    let r = E(t);
    return r === "ENOENT" || r === "ENOTDIR" ? "absent" : "other";
  }
}
function uvn(e) {
  try {
    let t = ir(e, { encoding: "buffer" }),
      r = t.toString("utf-8");
    if (r.includes("\x00") || !Buffer.from(r, "utf-8").equals(t)) return null;
    return r;
  } catch {
    return null;
  }
}
function KH(e, t) {
  if (!U(e, t)) return true;
  let r = je(e);
  return r === "symlink" || r === "other";
}
function oU(e, t) {
  let r = lr(t, e);
  if (r === "" || r.startsWith("..") || ar(r)) return true;
  return !U(e, t);
}
function cr(e, t) {
  return Sc(e, t);
}
var ur = /\/+/,
  fr = /\/$/,
  Xe = /^\//,
  dr = /^\//;
function U(e, t, r = 40, o = t) {
  if (r <= 0) return false;
  let i = dr.exec(e),
    s = i ? i[0] : "",
    a = e.slice(s.length).split(ur);
  for (let u = 0; u < a.length; u++) {
    let b = a[u];
    if (b === "" || b === ".") continue;
    if (b === "..") {
      s = fe(s);
      continue;
    }
    if (((s = v(s, b)), HR(s, o))) return false;
    let m = je(s);
    if (m === "other") return false;
    if (m === "symlink") {
      let w = uvn(s);
      if (w === null) return false;
      if (cr(w, fe(s))) return false;
      let d = Xe.test(w) ? w : fe(s) + V + w,
        c = a.slice(u + 1).join(V),
        f = c ? (fr.test(d) ? d + c : d + V + c) : d;
      return U(f, t, r - 1, o);
    }
  }
  return true;
}
function te(e, t, r = t) {
  if (!U(Xe.test(e) ? e : t + V + e, t, void 0, r)) return true;
  return !U(W(t, e), t, void 0, r);
}
function P_(e, t, r = t) {
  return te(e, t, r);
}
function KEt(e) {
  return /^[0-9a-f]{40}$/.test(e) || /^[0-9a-f]{64}$/.test(e);
}
async function nJe(e) {
  try {
    if (oU(v(e, "HEAD"), e)) return null;
    let t = (await Q(v(e, "HEAD"), "utf-8")).trim();
    if (t.startsWith("ref:")) {
      let r = t.slice(4).trim();
      if (r.startsWith("refs/heads/")) {
        let i = r.slice(11);
        if (!wS(i)) return null;
        return { type: "branch", name: i };
      }
      if (!wS(r)) return null;
      let o = await rj(e, r);
      return o ? { type: "detached", sha: o } : { type: "detached", sha: "" };
    }
    if (!KEt(t)) return null;
    return { type: "detached", sha: t };
  } catch {
    return null;
  }
}
async function rj(e, t, r = 5) {
  if (r <= 0) return null;
  let o = await We(e, t, r);
  if (o) return o;
  let i = await PO(e);
  if (i && i !== e) return We(i, t, r);
  return null;
}
async function We(e, t, r) {
  try {
    if (oU(v(e, t), e)) return null;
    let o = (await Q(v(e, t), "utf-8")).trim();
    if (o.startsWith("ref:")) {
      let i = o.slice(4).trim();
      if (!wS(i)) return null;
      return rj(e, i, r - 1);
    }
    if (!KEt(o)) return null;
    return o;
  } catch {}
  if (oU(v(e, "packed-refs"), e)) return null;
  return Ne(e, t);
}
var Ve = 65536;
async function PO(e) {
  try {
    if (KH(v(e, "commondir"), e)) return null;
    let t = await bP(v(e, "commondir"), Ve, { sniffEncoding: true, withBytes: true });
    if (t === null || t.truncated) return null;
    let r = t.content.trim();
    if (Sc(r, e)) return null;
    if (te(r, e)) return null;
    return W(e, r);
  } catch {
    return null;
  }
}
async function fsr(e, t, r) {
  try {
    if (oU(v(e, t), e)) return null;
    let o = (await Q(v(e, t), "utf-8")).trim();
    if (o.startsWith("ref:")) {
      let i = o.slice(4).trim();
      if (i.startsWith(r)) {
        let s = i.slice(r.length);
        if (!wS(s)) return null;
        return s;
      }
    }
  } catch {}
  return null;
}
var ze = 1000;
class Ze {
  gitDir = null;
  commonDir = null;
  initialized = false;
  initPromise = null;
  watchedFiles = [];
  branchRefPath = null;
  generation = 0;
  cache = new Map();
  stateChanged = Ue();
  repoBranches = new Map();
  repoWatchers = new Map();
  repoBranchListeners = [];
  async ensureStarted() {
    if (this.initialized) return;
    if (this.initPromise) return this.initPromise;
    return (this.initPromise = this.start()), this.initPromise;
  }
  cleanupHandle = null;
  async start() {
    let e = this.generation;
    if (sn()) {
      (this.gitDir = null), (this.initialized = true);
      return;
    }
    let t = await Aw();
    if (e !== this.generation) return;
    if (((this.gitDir = t), (this.initialized = true), !this.cleanupHandle))
      this.cleanupHandle = vt(async () => {
        this.stopWatching();
      });
    if (!this.gitDir) return;
    let r = await PO(this.gitDir);
    if (e !== this.generation) return;
    (this.commonDir = r),
      this.watchPath(v(this.gitDir, "HEAD"), this.gitDir, () => {
        this.onHeadChanged();
      }),
      this.watchPath(v(this.commonDir ?? this.gitDir, "config"), this.commonDir ?? this.gitDir, () => {
        this.invalidate();
      }),
      this.watchPath(
        v(this.commonDir ?? this.gitDir, "refs", "remotes", "origin", "HEAD"),
        this.commonDir ?? this.gitDir,
        () => {
          this.invalidate();
        },
      ),
      await this.watchCurrentBranchRef();
  }
  watchPath(e, t, r) {
    if (oU(e, t)) return;
    let o = VEt(e, { interval: ze }, r);
    this.watchedFiles.push({ path: e, listener: o });
  }
  async watchCurrentBranchRef() {
    if (!this.gitDir) return;
    let e = this.generation,
      t = await nJe(this.gitDir);
    if (e !== this.generation) return;
    let r = this.commonDir ?? this.gitDir,
      o = t?.type === "branch" ? v(r, "refs", "heads", t.name) : null;
    if (o === this.branchRefPath) return;
    if (this.branchRefPath) {
      for (let { path: i, listener: s } of this.watchedFiles) if (i === this.branchRefPath) N(i, s);
      this.watchedFiles = this.watchedFiles.filter((i) => i.path !== this.branchRefPath);
    }
    if (((this.branchRefPath = o), !o)) return;
    this.watchPath(o, r, () => {
      this.invalidate();
    });
  }
  async onHeadChanged() {
    this.invalidate(), await SQe(), await this.watchCurrentBranchRef();
  }
  invalidate() {
    for (let e of this.cache.values()) e.dirty = true;
    this.stateChanged.emit();
  }
  onStateChange(e) {
    return this.stateChanged.subscribe(e);
  }
  stopWatching() {
    for (let { path: e, listener: t } of this.watchedFiles) N(e, t);
    for (let { headPath: e, listener: t } of this.repoWatchers.values()) N(e, t);
    (this.watchedFiles = []), (this.branchRefPath = null);
  }
  async get(e, t) {
    for (;;) {
      let r = this.generation;
      await this.ensureStarted();
      let o = this.cache.get(e);
      if (o && !o.dirty) return o.value;
      if (o) o.dirty = false;
      let i = await t();
      if (r !== this.generation) continue;
      let s = this.cache.get(e);
      if (s && !s.dirty) s.value = i;
      if (!s) this.cache.set(e, { value: i, dirty: false });
      return i;
    }
  }
  async getDefaultBranchIfKnownAnchored() {
    let e = await this.get("defaultBranchIfKnown", this.computeDefaultBranchIfKnownAnchored);
    if (e === null) return e;
    let t = this.commonDir ?? this.gitDir;
    if (t && (await rj(t, `refs/remotes/origin/${e}`))) return e;
    let r = this.cache.get("defaultBranchIfKnown");
    if (r) r.dirty = true;
    return this.get("defaultBranchIfKnown", this.computeDefaultBranchIfKnownAnchored);
  }
  computeDefaultBranchIfKnownAnchored = async () => {
    let e = this.gitDir;
    if (!e) return null;
    let t = this.commonDir ?? e,
      r = await fsr(t, "refs/remotes/origin/HEAD", "refs/remotes/origin/");
    if (r && (await rj(t, `refs/remotes/origin/${r}`))) return r;
    return null;
  };
  async addRepo(e) {
    if (this.repoWatchers.has(e)) return;
    let t = await Aw(e);
    if (!t) return;
    if (this.repoWatchers.has(e)) return;
    let r = v(t, "HEAD");
    if (oU(r, t)) return;
    let o = VEt(r, { interval: ze }, () => {
      this.repoBranches.delete(e);
      for (let i of this.repoBranchListeners) i();
    });
    this.repoWatchers.set(e, { gitDir: t, headPath: r, listener: o });
  }
  removeRepo(e) {
    let t = this.repoWatchers.get(e);
    if (!t) return;
    N(t.headPath, t.listener), this.repoWatchers.delete(e), this.repoBranches.delete(e);
  }
  onRepoBranchChange(e) {
    return (
      this.repoBranchListeners.push(e),
      () => {
        let t = this.repoBranchListeners.indexOf(e);
        if (t !== -1) this.repoBranchListeners.splice(t, 1);
      }
    );
  }
  async getBranchForRepo(e) {
    if (this.repoBranches.has(e)) return this.repoBranches.get(e);
    let t = this.repoWatchers.get(e)?.gitDir;
    if (!t) return;
    let r = await nJe(t),
      o = r?.type === "branch" ? r.name : null;
    return this.repoBranches.set(e, o), o;
  }
  reset() {
    this.generation++,
      this.stopWatching(),
      this.cleanupHandle?.(),
      (this.cleanupHandle = null),
      this.cache.clear(),
      this.stateChanged.clear(),
      this.repoBranches.clear(),
      this.repoWatchers.clear(),
      (this.repoBranchListeners = []),
      (this.initialized = false),
      (this.initPromise = null),
      (this.gitDir = null),
      (this.commonDir = null);
  }
  reanchor() {
    this.generation++;
    for (let { path: e, listener: t } of this.watchedFiles) N(e, t);
    (this.watchedFiles = []),
      (this.branchRefPath = null),
      this.cache.clear(),
      (this.initialized = false),
      (this.initPromise = null),
      (this.gitDir = null),
      (this.commonDir = null);
  }
}
var Twr = new J(() => new Ze());
function B() {
  return Twr.of(G().host);
}
async function pr() {
  let e = await Aw();
  if (!e) return "HEAD";
  let t = await nJe(e);
  if (!t) return "HEAD";
  return t.type === "branch" ? t.name : "HEAD";
}
async function mr() {
  let e = await Aw();
  if (!e) return "";
  let t = await nJe(e);
  if (!t) return "";
  if (t.type === "branch") return (await rj(e, `refs/heads/${t.name}`)) ?? "";
  return t.sha;
}
async function Z(e) {
  if (oU(v(e, "config"), e)) return null;
  return (await ZYe(e, "remote", "origin", "pushurl")) || (await ZYe(e, "remote", "origin", "url"));
}
async function gr() {
  let e = await Aw();
  if (!e) return null;
  let t = await Z(e);
  if (t) return t;
  let r = await PO(e);
  if (r && r !== e) return Z(r);
  return null;
}
var I1e = ["main", "master"];
async function hr() {
  let e = await Aw();
  if (!e) return "main";
  let t = (await PO(e)) ?? e,
    r = await fsr(t, "refs/remotes/origin/HEAD", "refs/remotes/origin/");
  if (r && (await rj(t, `refs/remotes/origin/${r}`))) return r;
  for (let o of I1e) if (await rj(t, `refs/remotes/origin/${o}`)) return o;
  return "main";
}
function msr() {
  return B().get("branch", pr);
}
function XEt() {
  return B().get("head", mr);
}
function dvn() {
  return B().get("remoteUrl", gr);
}
function gsr() {
  return B().get("defaultBranch", hr);
}
function hsr() {
  return B().getDefaultBranchIfKnownAnchored();
}
function pvn(e) {
  return B().addRepo(e);
}
function fHr(e) {
  B().removeRepo(e);
}
function fvn(e) {
  return B().onRepoBranchChange(e);
}
function mvn(e) {
  let t = B();
  return t.ensureStarted(), t.onStateChange(e);
}
function gvn(e) {
  return B().getBranchForRepo(e);
}
function hvn() {
  B().reset();
}
function iU() {
  B().reanchor();
}
async function _vn(e) {
  let t = await Aw(e);
  if (!t) return null;
  let r = await nJe(t);
  if (!r) return null;
  if (r.type === "branch") return rj(t, `refs/heads/${r.name}`);
  return r.sha;
}
async function gzt(e) {
  let t;
  try {
    if (KH(v(e, ".git"), e)) return null;
    let o = (await Q(v(e, ".git"), "utf-8")).trim();
    if (!o.startsWith("gitdir:")) return null;
    let i = o.slice(7).trim();
    if (Sc(i, e)) return null;
    if (te(i, e)) return null;
    t = W(e, i);
  } catch {
    return null;
  }
  let r = await nJe(t);
  if (!r) return null;
  if (r.type === "branch") return rj(t, `refs/heads/${r.name}`);
  return r.sha;
}
async function TCe(e) {
  let t = await Aw(e);
  if (!t) return null;
  let r = await Z(t);
  if (r) return r;
  let o = await PO(t);
  if (o && o !== t) return Z(o);
  return null;
}
async function _sr() {
  let e = await Aw();
  if (!e) return false;
  let t = (await PO(e)) ?? e;
  try {
    if (oU(v(t, "shallow"), t)) return false;
    return await Ke(v(t, "shallow")), true;
  } catch {
    return false;
  }
}
async function ysr() {
  try {
    let e = await Aw();
    if (!e) return 0;
    let t = (await PO(e)) ?? e;
    if (oU(v(t, "worktrees"), t)) return 1;
    return (await sr(v(t, "worktrees"))).length + 1;
  } catch {
    return 1;
  }
}
export {
  azt,
  Gr,
  qEt,
  sn,
  ba,
  zT,
  lzt,
  czt,
  uzt,
  ZYe,
  rsr,
  x1e,
  dzt,
  pzt,
  fA,
  eJe,
  GEt,
  osr,
  isr,
  zEt,
  koe,
  fzt,
  ssr,
  asr,
  lsr,
  csr,
  usr,
  dsr,
  afe,
  pS,
  I_,
  SP,
  nU,
  psr,
  mzt,
  avn,
  lvn,
  bP,
  tJe,
  VEt,
  f0,
  rU,
  vd,
  cvn,
  Aw,
  uvn,
  KH,
  oU,
  P_,
  KEt,
  nJe,
  rj,
  PO,
  fsr,
  Twr,
  I1e,
  msr,
  XEt,
  dvn,
  gsr,
  hsr,
  pvn,
  fHr,
  fvn,
  mvn,
  gvn,
  hvn,
  iU,
  _vn,
  gzt,
  TCe,
  _sr,
  ysr,
  Hn,
  YEt,
  hzt,
  a4,
  rJe,
  Vr,
  DO,
  yvn,
  _zt,
  m0,
  Svn,
  bvn,
  oJe,
  it,
  Ssr,
  Vg,
  P1e,
  lfe,
  l4,
  mHr,
  wvn,
  D1e,
  cl,
  iJe,
  Cw,
  yzt,
  JEt,
  sJe,
  c4,
  Ewr,
  Tvn,
  tJ,
  u4,
  Awr,
  Evn,
  aJe,
  Avn,
  Cvn,
  Szt,
  lJe,
  cfe,
  bzt,
  O1e,
  vvn,
  wzt,
  L1e,
  Cwr,
  gHr,
  Tzt,
  ufe,
};
