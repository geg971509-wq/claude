...
var d = j(qc(), 1),
  ...
  O = async ({
    ...
    if (f)
      try {
        let c = await nEt({ profile: t, filepath: k, configFilepath: o, ignoreCache: u })();
    ...
    let { accessToken: C } = g,
      { SSOClient: A, GetRoleCredentialsCommand: p } = await import("/$bunfs/root/chunk-96yrq7s1.js"),
      w =
        i ||
        new A(
          Object.assign({}, a ?? {}, {
...
var SCn =
  ...
  async ({ callerClientConfig: f } = {}) => {
    ...
    if (!s && !n && !r && !i && !a) {
      ...
      if (!o) throw new S.CredentialsProviderError(`Profile ${t} was not found.`, { logger: e.logger });
      if (!$ir(o))
        throw new S.CredentialsProviderError(`Profile ${t} is not configured with SSO credentials.`, {
          logger: e.logger,
        });
      if (o?.sso_session) {
