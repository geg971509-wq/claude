...
import { ce, vg } from "/$bunfs/root/chunk-4ddxwr9r.js";
import { sU } from "/$bunfs/root/chunk-cf8qhmdc.js";
...
import { basename as Ve, dirname as q, isAbsolute as Ke, join as G, resolve as jt, sep as Ye } from "path";
var fe = 50;
function Re(e) {
  if (e) return [];
  let { depth: t } = Ae();
  ...
  for (let u of s) {
    ...
    let [, h, T, m, l, d, _] = P,
      o = {
        ...
        total: l !== void 0 ? Number(l) : void 0,
        bytes: d !== void 0 && _ !== void 0 ? Math.round(Number(d) * Ze[_]) : void 0,
        raw: `${c ? "remote: " : ""}${h}: ${T}%${m !== void 0 ? ` (${m}/${l})` : ""}`,
      };
    if (((n = o), !c)) r = o;
  }
...
var st = 1e4,
  ...
  ct = 15;
function ve() {
  return Boolean(Zn.CLAUDE_RUNNER_FAIL_FAST_FETCH);
}
function ut() {
  return Boolean(Zn.CLAUDE_RUNNER_STALL_GIVEUP);
}
function dt(e) {
  return e
    .split(`
...
function ft() {
  return Boolean(Zn.CCR_DELTA_RESET);
}
...
async function gt(e) {
  ...
  try {
    ...
    y("byoc_git_prepare_repo", { repo_existed: n, duration_ms: Date.now() - t });
  } catch (n) {
    throw (p("byoc_git_prepare_repo", "error", { duration_ms: Date.now() - t }), n);
  }
}
async function pt(e) {
  let {
    ...
    T = Ct(n),
    m = n.getAuthToken ? T : eQt(T, n.token),
    l = tQt(t, n);
  if (!l) throw Error(`Could not determine repository directory for: ${n.repo}`);
  if (n.ref && !wS(n.ref)) throw Error(`Invalid ref: ${n.ref}`);
  if (r && !wS(r)) throw Error(`Invalid target branch: ${r}`);
  let d = Date.now();
  c(`[byoc:git] Preparing ${n.repo} at ${l} (ref=${n.ref ?? "HEAD"}, auth=${n.getAuthToken ? "git-proxy" : m !== T})`);
  let { invalid: _ } = Ae();
  if (_ !== void 0)
    c(
  ...
  let f = await ge(o, "ensureRepository", () => mt(o, t));
  if (s) await de(o, n.ref, u, f, n.prefetchedTip, n.standbyCheckout);
  else if (f && !n.ref) c("[byoc:git] Using prefetched repo as-is (no ref requested)");
  else if (f && n.ref) {
    if (r && (await Le(o, r)))
      return (
...
async function ht(e) {
  ...
  throw Error(`Repository access validation failed after ${ue} attempts: ${L(n)}`);
}
async function mt(e, t) {
  try {
    return (
      await g(e, e.repoPath, ["--git-dir", G(e.repoPath, ".git"), "remote", "set-url", "origin", e.authURL]),
      await g(e, e.repoPath, [
        "--git-dir",
        G(e.repoPath, ".git"),
        "config",
        "--unset-all",
...
async function de(e, t, n, r, i, s) {
  ...
  if (
    (await g(e, e.repoPath, ["update-ref", "refs/remotes/origin/HEAD", "FETCH_HEAD^{commit}"]).catch((f) =>
      e.onDebug(`[byoc:git] Could not set origin/HEAD: ${L(f)}`),
    ),
    n)
  ) {
  ...
  let h = () => g(e, e.repoPath, ["update-ref", "--no-deref", "HEAD", "HEAD"]).catch(() => {}),
    T = () =>
      g(e, e.repoPath, [
        "-c",
    ...
    d =
      l === void 0 ||
      l.outcome === "absent" ||
      (l.outcome === "ready" && l.state?.phase === "idle" && l.state.head === void 0)
        ? void 0
        : l,
    _ = d && { standby_outcome: d.outcome, standby_wait_ms: String(d.waitedMs) },
    o =
      d === void 0
        ? { action: "usual" }
        : wt({
  ...
  switch (o.action) {
    case "skip": {
      ...
      let f = d?.state?.lastOkMs === void 0 ? void 0 : Math.max(0, Math.round((Date.now() - d.state.lastOkMs) / 1000));
      M(e, "reset", Date.now() - m, {
        ..._,
        skipped: "same_commit",
        ...(f !== void 0 && { standby_checkout_age_s: String(f) }),
      });
      return;
    }
    case "forced": {
      e.onDebug(`[byoc:git] Standby checkout left the tree suspect (${o.reason}); forcing reset --hard`),
        await h(),
        await T();
      let f = await _t(e, { base: o.base ?? d?.lockBase, target: o.target ?? d?.lockTarget });
      M(e, "reset", Date.now() - m, {
        ..._,
        standby_forced: o.reason,
        standby_strays_removed: f === void 0 ? "unknown" : String(f),
      });
      return;
    }
    case "usual":
    ...
    async () => {
      if (w)
        try {
          ...
          e.onDebug(`[byoc:git] Delta reset failed, falling back to reset --hard: ${L(f)}`), await h();
        }
      await T();
    },
    _,
...
async function J(e, t) {
  return g(e, e.repoPath, ["rev-parse", "--verify", "--quiet", `${t}^{commit}`]).then(
    (n) => n.trim() || void 0,
    () => {
      return;
    },
...
async function _t(e, { base: t, target: n }) {
  ...
  try {
    ...
    let s = async (l, d) => {
        let _ = {},
          o = await g(
            e,
            e.repoPath,
            ["diff", "--no-renames", "--name-only", "--diff-filter=A", "-z", l, d, "--"],
            void 0,
            void 0,
            _,
          );
        if (_.stdoutTruncated || (o.length > 0 && !o.endsWith("\x00"))) {
          e.onDebug(
            `[byoc:git] Standby stray clean-up skipped: the ${l.slice(0, 12)}\u2192${d.slice(0, 12)} listing was truncated`,
          );
          return;
        }
        return o.split("\x00").filter((f) => f.length > 0);
      },
      u = await s(r, i);
    if (u === void 0) return;
    ...
    for (let l of P) {
      if (!Et(l)) {
        e.onDebug(`[byoc:git] Standby stray clean-up: refusing path '${l}'`);
        continue;
      }
      let d = G(h, l);
      try {
        if ((await _e(q(d))) !== q(d)) {
          e.onDebug(`[byoc:git] Standby stray clean-up: refusing '${l}' (its directory resolves to another path)`);
          continue;
        }
        let o = await He(d);
        ...
        if (E(_) !== "ENOENT") e.onDebug(`[byoc:git] Standby stray clean-up: '${l}': ${L(_)}`);
      }
    }
    for (let l of [...m].sort((d, _) => _.length - d.length))
      for (let d = l; d !== h && d.startsWith(h + Ye); d = q(d))
        try {
          await We(d);
        } catch {
          break;
...
async function Oe(e, t, n, r = !1, i = Tt) {
  let s = Date.now(),
    ...
    P,
    h = [],
    T = (o) => (o.length <= Te ? o : ce(o, Pe) + "\u2026" + vg(o, Te - Pe - 1)),
    m = () => (h.length > 0 ? { attempt_errors: b(h) } : {}),
    l = 0,
    d = 0,
    _ = (o) => ({
      failfast_aborts: String(l),
  ...
  for (let o = 0; o < V; o++) {
    ...
    try {
      ...
      P = A;
      let D = L(A);
      e.onDebug(`[byoc:git] Fetch attempt ${o + 1} failed: ${D}`), h.push(T(Rl(D)));
      let F = r && o < Ce && dt(D);
      ...
      if (
        ...
        throw (
          (e.onDebug(`[byoc:git] Fetch cumulative wall-clock ${Date.now() - s}ms \u2265 hard cap \u2014 not retrying`),
      ...
      if (Ort(D))
        throw (
          ...
          M(e, "fetch", Date.now() - s, { attempts: String(o + 1), permanent: "true", ..._(), ...m() }),
      ...
      if (c && o === K - 1 && w === K)
        throw (
          ...
          M(e, "fetch", Date.now() - s, { attempts: String(o + 1), exhausted: "true", giveup: "mute", ..._(), ...m() }),
          Error(
  ...
  throw (
    (M(e, "fetch", Date.now() - s, { attempts: String(V), exhausted: "true", ..._(), ...m() }),
    Error(`git fetch failed after ${V} attempts: ${L(P)}`))
  );
}
async function Pt(e, t, n) {
  if ((await g(e, e.repoPath, ["branch", "--show-current"]).catch(() => "")).trim() === t) {
    e.onDebug(`[byoc:git] Already on target branch ${t}`);
    return;
  }
  if (await Le(e, t)) e.onDebug("[byoc:git] Target branch exists on remote, fetching"), await Ge(e, t, n);
  else
    e.onDebug(`[byoc:git] Creating local branch ${t} from HEAD`),
      await g(e, e.repoPath, ["checkout", "-B", t]),
      await g(e, e.repoPath, ["update-ref", `refs/remotes/origin/${t}`, "HEAD"]).catch(() => {});
}
async function Ge(e, t, n) {
  await Oe(e, ["fetch", ...Re(n), "--progress", "origin", t], void 0, n && ve()),
    await g(e, e.repoPath, ["checkout", "--progress", "--force", "-B", t, "FETCH_HEAD"]);
}
...
var Lrt = "SHR_GIT_PROXY_TOKEN",
  QJt = ["-c", "lfs.locksverify=false"];
function ZJt(e) {
  return `!f() { if test "$1" = get; then printf 'username=unused\\npassword=%s\\n' "$${e}"; fi; }; f`;
}
function lxt(e) {
  let t = new URL(e).origin;
  return ["-c", "credential.helper=", "-c", `credential.${t}.helper=`, "-c", `credential.${t}.helper=${ZJt(Lrt)}`];
}
function cxt(e) {
  let n = ["", `${new URL(e).origin}.`, `${e}.`],
    r = process.env.HTTPS_PROXY || process.env.https_proxy || process.env.ALL_PROXY || process.env.all_proxy || "",
...
async function g(e, t, n, r, i = le, s) {
  let u = e.getAuthToken !== void 0,
    c = At(n),
    w = c !== void 0 && Rt.has(c),
    P = u && w ? e.getAuthToken() : void 0,
    h = [
      ...S1,
      "-c",
      ...
      ...(u && w ? cxt(e.gitURL) : []),
      ...(P ? lxt(e.gitURL) : []),
      ...(e.governedMount && w ? QJt : []),
      ...n,
    ],
    T = P ?? e.token,
    m = h.map((F) => Ot(F, e.authURL, e.gitURL, T));
  ...
  let l = {
      ...
      GCM_INTERACTIVE: sU.GCM_INTERACTIVE,
      LC_ALL: "C",
      GIT_PROGRESS_DELAY: "0",
      ...KK,
      ...(e.authURL.startsWith("file://") && { GIT_ALLOW_PROTOCOL: `file:${KK.GIT_ALLOW_PROTOCOL}` }),
      GIT_SSH_COMMAND: `${process.env.GIT_SSH_COMMAND || "ssh"} -o BatchMode=yes -o ConnectTimeout=30`,
    },
    { signal: d, dispose: _ } = Dt(e.signal, i),
    o = c !== void 0 && Qe.has(c) ? axt : ze,
    f,
    k,
    Y = Date.now(),
    R = !1,
  ...
  try {
      ...
      C.on("error", (S) => {
        ...
        C.on("close", (S, O) => {
          ...
          if (e.signal?.aborted && f === void 0) {
            ...
            return (v.name = "AbortError"), H(Object.assign(v, { stdout: B, stderr: U, code: S, signal: O }));
          }
    ...
    if (f !== void 0) {
      ...
      e.onDebug(`[runner:stuck] git ${m.join(" ")} (dir=${t || "."}) \u2014 ${X} \u2014 killing subprocess`);
    }
    let H =
        I.stderr !== void 0 && I.stderr.length > 0
          ? it(I.stderr, R, e.authURL, T)
          : Ne(Mrt(I.stdout || I.message || "", e.authURL, T)).slice(0, 2000),
      C = f !== void 0 ? ` (watchdog: ${f})` : "";
    throw Error(`git ${c ?? "?"} failed${C}: ${H}`);
  } finally {
    if (s !== void 0 && f !== void 0) s.killReason = f;
    _();
  }
}
function c6e(e) {
...
function Ct(e) {
  if (e.url) {
    if (!c6e(e.url)) throw Error(`Unsupported git URL protocol (only https/http/ssh/user@host:path allowed): ${e.url}`);
    return e.url;
  }
  switch (e.type) {
    case "github":
...
async function _1n(e) {
  ...
  let u = { repoPath: t, gitURL: "", authURL: "", onDebug: i, signal: s },
    c = (await g(u, t, ["rev-parse", "--verify", "FETCH_HEAD^{commit}"])).trim(),
    w;
  try {
    w = (await g(u, n, ["--git-dir", G(n, ".git"), "rev-parse", "HEAD"])).trim();
  } catch (h) {
    if (s?.aborted) throw h;
  }
  if (w === c) {
    let h = (await g(u, n, ["rev-parse", "--absolute-git-dir"])).trim();
    await Ee(
      G(h, "FETCH_HEAD"),
      `${c}
`,
  ...
  i(
    ...
    await g(u, t, ["worktree", "prune"]).catch((h) => {
      i(`[byoc:git] worktree prune failed (continuing): ${h}`);
    }),
    await g(u, t, ["worktree", "add", "--detach", n, c]);
  let P = (await g(u, n, ["rev-parse", "--absolute-git-dir"])).trim();
  await Ee(
    G(P, "FETCH_HEAD"),
    `${c}
`,
...
async function y1n(e) {
  ...
  for (let { canonicalRepoPath: s, worktreePath: u } of t) {
    ...
    try {
      ...
      await g(c, s, ["worktree", "remove", "--force", u]), n(`[byoc:git] Removed worktree ${u}`);
    } catch (w) {
      n(`[byoc:git] worktree remove ${u} failed: ${w}`);
    }
  }
  for (let s of i)
    await g({ repoPath: s, gitURL: "", authURL: "", onDebug: n, signal: r }, s, ["worktree", "prune"]).catch((c) => {
...
function vt(e) {
  ...
  for (let n of e) {
    let r = n.branches ?? [],
      [i, ...s] = r;
    if (s.length > 0) throw Error(`Outcome for ${n.repo} has ${r.length} branches, expected 0 or 1`);
    if (i) t.set(n.repo, i);
  }
  return t;
}
...
function Mrt(e, t, n) {
  ...
  return e
    ...
    .replace(/gl(?:pat|dt|rt|ft|soat|oas|agent|ptt|cbt|imt|ffct)-[A-Za-z0-9_=-]+(?:\.[0-9a-z]{9})?/g, "<token>");
}
function pe(e) {
  let t;
  try {
...
export { axt, Ort, S1, KK, g1n, JJt, Lrt, QJt, ZJt, lxt, cxt, c6e, eQt, tQt, h1n, _1n, y1n, Mrt };
