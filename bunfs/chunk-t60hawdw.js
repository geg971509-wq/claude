...
import { Ln, hl } from "/$bunfs/root/chunk-zze8764r.js";
...
import { kvt, pKt, KIn } from "/$bunfs/root/chunk-87pfeb5g.js";
import "/$bunfs/root/chunk-sypj25ha.js";
import "/$bunfs/root/chunk-9r67t977.js";
import { p1, f1 } from "/$bunfs/root/chunk-wfer9wy8.js";
...
function C({ onDone: m, handoff: n, getTranscript: x }) {
  let { storageV5: H } = ge(),
    [d, f] = u(n.current?.state ?? "checking"),
    [w, g] = u(n.current?.error ?? null),
    [M, y] = u(n.current?.downloadMessage ?? ""),
    E = xt();
  A(() => {
    let s = n.current;
    if (s)
      (s.setters = { setState: f, setError: g, setDownloadMessage: y }), f(s.state), g(s.error), y(s.downloadMessage);
    return () => {
  ...
  function S(s) {
    if (s.key === "escape" || ((s.ctrl || s.meta) && (s.key === "c" || s.key === "d"))) {
      s.preventDefault(), p(`Cancelled. Learn more about Claude Desktop at ${D}`, { display: "system" });
      return;
    }
    if (s.ctrl || s.meta) return;
    if (d === "error") {
      s.preventDefault(), p(w ?? "Unknown error", { display: "system" });
      return;
    }
...
Learn more at ${D}`,
        ...
        s.preventDefault(), p(`The desktop app is required for /desktop. Learn more at ${D}`, { display: "system" });
    }
  }
  if (
    (A(() => {
      ...
      async function R() {
        ...
        if (a.status === "not-installed") {
          s({ state: "prompt-download", downloadMessage: "Claude Desktop is not installed." });
          return;
        }
        if (a.status === "version-too-old") {
          s({
            state: "prompt-download",
            downloadMessage: `Claude Desktop needs to be updated (found v${a.version}, need v${kvt}+).`,
          });
          return;
        }
        (c = await p1(x(), "desktop_handoff", {}, H)), s({ state: "flushing" }), await hl(), s({ state: "opening" });
        let i = await KIn();
        if (!i.success) {
          s({ state: "error", error: f1(i.error, c) });
          return;
        }
        s({ state: "success" }),
          E.setTimeout(async () => {
      ...
      R().catch((a) => {
        s({ state: "error", error: f1(l(a), c, { as: "clause" }) });
      });
    }, []),
    d === "error")
  )
...
async function Q(m, n) {
  return e(C, { onDone: m, handoff: v(), getTranscript: () => EF(n) });
}
export { Q as call };
